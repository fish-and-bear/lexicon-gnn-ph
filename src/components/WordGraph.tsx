...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const { filteredNodes, filteredLinks } = useMemo<{ filteredNodes: CustomNode[], filteredLinks: CustomLink[] }>(() => {
    ...
    // --- Find Main Word's String ID --- 
    const mainWordNodeData = baseNodes.find(n => n.id === mainWord); // Find by label first
    if (!mainWordNodeData || mainWordNodeData.originalId === undefined) {
        console.error(`[FILTER] ERROR: Could not find main word node or its originalId for label '${mainWord}' in baseNodes.`);
        // Return just the main node if possible, otherwise empty
        const fallbackNode = baseNodes.find(n => n.id === mainWord);
        return {
            filteredNodes: fallbackNode ? [fallbackNode] : [],
            filteredLinks: []
        };
    }
    const mainWordStringId = String(mainWordNodeData.originalId);
    console.log(`[FILTER] Main word string ID: ${mainWordStringId}`);

    // --- Refactored BFS using STRING IDs --- 
    // Step 1: Collect nodes within depth/breadth using BFS with STRING IDs
    const nodeMapByStringId = new Map(baseNodes.map(n => [String(n.originalId), n])); // Key is String ID
    const connectedNodeStringIds = new Set<string>([mainWordStringId]); // Start with main word STRING ID
    const queue: [string, number][] = [[mainWordStringId, 0]]; // [nodeStringId, depth]
    const visitedStringIds = new Set<string>();

    console.log("[FILTER-BFS] Starting BFS with mainWord String ID:", mainWordStringId);
    console.log("[FILTER-BFS] Node map (by string ID) size:", nodeMapByStringId.size);

    while (queue.length > 0) {
        const [currentStringId, currentDepth] = queue.shift()!;

        if (currentDepth >= depth) continue;
        if (visitedStringIds.has(currentStringId)) continue;
        visitedStringIds.add(currentStringId);

        // Find related links using baseLinks (which use string IDs)
        const relatedLinks = baseLinks.filter(link => 
            link.source === currentStringId || link.target === currentStringId
        );

        // Get neighbor STRING IDs
        const neighborStringIds = relatedLinks.map(link => 
            link.source === currentStringId ? link.target as string : link.source as string
        ).filter(id => !visitedStringIds.has(id)); // Filter already visited STRING IDs

        // Sort neighbors (optional but helps consistency)
        const sortedNeighborStringIds = [...neighborStringIds].sort((aId, bId) => {
            const aNode = nodeMapByStringId.get(aId);
            const bNode = nodeMapByStringId.get(bId);
            if (!aNode) return 1;
            if (!bNode) return -1;
            // Basic sort: main first, then alphabetical by group, then label
            if (aNode.group === 'main') return -1;
            if (bNode.group === 'main') return 1;
            if (aNode.group !== bNode.group) return aNode.group.localeCompare(bNode.group);
            return aNode.id.localeCompare(bNode.id); 
        });

        // Apply breadth limit
        const neighborsToAdd = sortedNeighborStringIds.slice(0, breadth);

        neighborsToAdd.forEach(neighborStringId => {
            if (nodeMapByStringId.has(neighborStringId)) { // Check if node exists in our map
                connectedNodeStringIds.add(neighborStringId);
                queue.push([neighborStringId, currentDepth + 1]);
            }
        });
    }
    console.log(`[FILTER-BFS] BFS finished. Found ${connectedNodeStringIds.size} connected node IDs within limits.`);

    // --- Filter nodes and links based on collected STRING IDs --- 
    // Step 2: Filter baseNodes using the collected STRING IDs
    const depthLimitedNodes = baseNodes.filter(node => 
        connectedNodeStringIds.has(String(node.originalId))
    );

    // Step 3: Filter baseLinks using the collected STRING IDs
    const depthLimitedLinks = baseLinks.filter(link => 
        connectedNodeStringIds.has(link.source as string) && 
        connectedNodeStringIds.has(link.target as string)
    );
    
    console.log(`[FILTER] After depth/breadth (using String IDs): ${depthLimitedNodes.length}/${baseNodes.length} nodes and ${depthLimitedLinks.length}/${baseLinks.length} links`);

    // Step 4: Apply relationship filtering (if any)
    ...
    // Filter nodes further based on relationship group
    const relationshipFilteredNodes = depthLimitedNodes.filter(node => {
        if (String(node.originalId) === mainWordStringId) return true; // Always keep main word
        
        // Get the group assigned during baseNodes calculation
        const nodeGroup = node.group.toLowerCase(); 
        // Keep node if its group is NOT in the filtered list
        return !filteredRelationships.includes(nodeGroup);
    });

    // Filter links: only keep links where BOTH endpoints remain after relationship filtering
    const relationshipFilteredNodeStringIds = new Set(relationshipFilteredNodes.map(n => String(n.originalId)));
    const relationshipFilteredLinks = depthLimitedLinks.filter(link => 
        relationshipFilteredNodeStringIds.has(link.source as string) &&
        relationshipFilteredNodeStringIds.has(link.target as string)
    );
  ...
  }, [baseNodes, baseLinks, mainWord, depth, breadth, filteredRelationships, wordNetwork]); // Added wordNetwork dependency
