...
def evaluate_link_prediction(
    ...
    with torch.no_grad():
        ...
        if edge_key not in data.edge_index_dict:
            print(f"[evaluation] Edge type {edge_key} not found in data")
            return 0.0
        
        pos_edge_index = data.edge_index_dict[edge_key]
        
        if pos_edge_index.size(1) == 0:
            print(f"[evaluation] No edges of type {edge_key} to evaluate")
            return 0.5
        
        # Generate negative edges
        num_nodes = out_dict["Word"].size(0)
        num_pos_edges = pos_edge_index.size(1)
        
        # Simple negative sampling
        neg_edges = []
        edge_set = set(zip(pos_edge_index[0].tolist(), pos_edge_index[1].tolist()))
        
        attempts = 0
        while len(neg_edges) < num_pos_edges and attempts < num_pos_edges * 10:
            src = torch.randint(0, num_nodes, (num_pos_edges,))
            dst = torch.randint(0, num_nodes, (num_pos_edges,))
