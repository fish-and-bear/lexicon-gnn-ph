...
def create_model(model_type: str, metadata: Tuple[List[str], List[Tuple[str, str, str]]], 
    ...
    if model_type == "rgcn":
        return HeteroRGCN(metadata, in_channels_dict, hidden_dim, out_dim, num_layers)
    elif model_type == "sage":
        return HeteroGraphSAGE(metadata, in_channels_dict, hidden_dim, out_dim, num_layers)
    elif model_type == "gatv2":
        heads = config.get("heads", 4)
        return HeteroGATv2(metadata, in_channels_dict, hidden_dim, out_dim, heads, num_layers)
    else:
