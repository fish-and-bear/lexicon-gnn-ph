...
psql -U postgres -t -f backend/setup_db.sql
if ($LASTEXITCODE -ne 0) {
    Write-Host "Database setup completed with warnings (this is normal if the database already exists)"
}
...
# Run migrations
Write-Host "Running database migrations to add any missing columns..."
python migrate.py
