...
# Additional patterns for complex trailing number formats
TRAILING_NUMBER_WITH_DASH_PATTERN = re.compile(r'^(.+?)(\d+)-$')  # e.g., "úngol1-"
TRAILING_NUMBER_WITH_COMMA_PATTERN = re.compile(r'^(.+?),(\d+)$')  # e.g., "úngol,2"
COMPLEX_TRAILING_NUMBER_PATTERN = re.compile(r'^(.+?)(\d+)[,-](\d+)$')  # e.g., "úngol1-2" or "úngol1,2"
...
# This pattern specifically targets Filipino/Tagalog words ending with numbers
# It ignores years, chemical numbers, atomic numbers, etc.
WORD_WITH_TRAILING_NUMBER_PATTERN = re.compile(r'(\b[a-zA-ZáàâäãåąćçéèêëęíìîïłńñóòôöõøùúûüÿýźżÁÀÂÄÃÅĄĆÇÉÈÊËĘÍÌÎÏŁŃÑÓÒÔÖÕØÙÚÛÜŸÝŹŻŠČĐĎŤŇŽÞÆŒƏƆƵǄǦǮȤΔΦΩк\-]+?)(\d+)([.,;)\s]|$)')

# Additional patterns for complex formats within text
WORD_WITH_TRAILING_NUMBER_DASH_PATTERN = re.compile(r'(\b[a-zA-ZáàâäãåąćçéèêëęíìîïłńñóòôöõøùúûüÿýźżÁÀÂÄÃÅĄĆÇÉÈÊËĘÍÌÎÏŁŃÑÓÒÔÖÕØÙÚÛÜŸÝŹŻŠČĐĎŤŇŽÞÆŒƏƆƵǄǦǮȤΔΦΩк\-]+?)(\d+)-([.,;)\s]|$)')  # e.g., "úngol1-"
WORD_WITH_TRAILING_NUMBER_COMMA_PATTERN = re.compile(r'(\b[a-zA-ZáàâäãåąćçéèêëęíìîïłńñóòôöõøùúûüÿýźżÁÀÂÄÃÅĄĆÇÉÈÊËĘÍÌÎÏŁŃÑÓÒÔÖÕØÙÚÛÜŸÝŹŻŠČĐĎŤŇŽÞÆŒƏƆƵǄǦǮȤΔΦΩк\-]+?),(\d+)([.,;)\s]|$)')  # e.g., "úngol,2"
WORD_WITH_COMPLEX_TRAILING_NUMBER_PATTERN = re.compile(r'(\b[a-zA-ZáàâäãåąćçéèêëęíìîïłńñóòôöõøùúûüÿýźżÁÀÂÄÃÅĄĆÇÉÈÊËĘÍÌÎÏŁŃÑÓÒÔÖÕØÙÚÛÜŸÝŹŻŠČĐĎŤŇŽÞÆŒƏƆƵǄǦǮȤΔΦΩк\-]+?)(\d+)[,-](\d+)([.,;)\s]|$)')  # e.g., "úngol1-2" or "úngol1,2"

# Pattern that should be ignored even if they match
IGNORED_PATTERNS = [
    re.compile(r'\(\d{4}-\d{4}\)'),   # Years range like (1799-1872)
    re.compile(r'atomic number \d+'),  # Atomic number references
    re.compile(r'\b\d+\s*[A-Z]{1,2}\b'),  # Chemical symbols with numbers
    re.compile(r'\b[A-Z]{1,2}\s*\d+\b'),  # Chemical symbols with numbers
...
def should_ignore_match(text, match):
    """Check if a match should be ignored based on context"""
    match_start, match_end = match.span()
    
    # Check all ignored patterns
    for pattern in IGNORED_PATTERNS:
        for ignore_match in pattern.finditer(text):
            ignore_start, ignore_end = ignore_match.span()
            
            # If the match overlaps with an ignored pattern, skip it
            if (match_start <= ignore_end and match_end >= ignore_start):
                return True
...
def clean_word_in_text(text):
    ...
    def replace_word(match):
        ...
        # Skip this replacement if it's in an ignored pattern
        if should_ignore_match(text, match):
    ...
    # Process words with trailing numbers and a dash (e.g., "úngol1-")
    def replace_word_with_dash(match):
        nonlocal changes
        ...
        word = match.group(1)  # The word without the number and dash
        ending = match.group(3)  # The character after (space, comma, etc.)
        changes += 1
        return word + ending
    
    # Process words with a comma followed by a number (e.g., "úngol,2")
    def replace_word_with_comma(match):
        nonlocal changes
        ...
        word = match.group(1)  # The word without the comma and number
        ending = match.group(3)  # The character after (space, comma, etc.)
        changes += 1
        return word + ending
    
    # Process words with complex trailing number patterns (e.g., "úngol1-2" or "úngol1,2")
    def replace_complex_word(match):
        nonlocal changes
        ...
        word = match.group(1)  # The word without the numbers
        ending = match.group(4)  # The character after (space, comma, etc.)
        changes += 1
        return word + ending
    
    # Process each pattern
    processed_text = WORD_WITH_TRAILING_NUMBER_PATTERN.sub(replace_word, result)
    processed_text = WORD_WITH_TRAILING_NUMBER_DASH_PATTERN.sub(replace_word_with_dash, processed_text)
    processed_text = WORD_WITH_TRAILING_NUMBER_COMMA_PATTERN.sub(replace_word_with_comma, processed_text)
    processed_text = WORD_WITH_COMPLEX_TRAILING_NUMBER_PATTERN.sub(replace_complex_word, processed_text)
    
    # Handle whole text patterns only if it's not a definition field
    # (definitions are handled by the word pattern above)
    if not any(pattern.search(text) for pattern in IGNORED_PATTERNS):
        # Handle special case for "úngol1-" pattern
        dash_match = TRAILING_NUMBER_WITH_DASH_PATTERN.match(result)
        if dash_match and not any(p in result for p in ('(', ')')):
            result = dash_match.group(1)  # Get the base word without the number and dash
            changes += 1
        
        # Handle special case for "úngol,2" pattern
        comma_match = TRAILING_NUMBER_WITH_COMMA_PATTERN.match(result)
        if comma_match and not any(p in result for p in ('(', ')')):
            result = comma_match.group(1)  # Get the base word without the comma and number
            changes += 1
        
        # Handle special case for "úngol1-2" or "úngol1,2" pattern
        complex_match = COMPLEX_TRAILING_NUMBER_PATTERN.match(result)
        if complex_match and not any(p in result for p in ('(', ')')):
            result = complex_match.group(1)  # Get the base word without the numbers
            changes += 1
        
        # Handle special case for hyphenated words with trailing numbers
        hyphen_match = HYPHENATED_NUMBER_PATTERN.match(result)
        if hyphen_match and not any(p in result for p in ('(', ')')):
        ...
        if match and not any(p in result for p in ('(', ')')):
...
def remove_trailing_numbers(value, path=""):
    ...
    if isinstance(value, str):
        ...
        # Skip if the value contains patterns we should ignore
        if any(pattern.search(value) for pattern in IGNORED_PATTERNS):
            return value, 0
        
        # Handle special case for "úngol1-" pattern
        dash_match = TRAILING_NUMBER_WITH_DASH_PATTERN.match(value)
        if dash_match:
            new_value = dash_match.group(1)  # Get the base word without the number and dash
        ...
        # Handle special case for "úngol,2" pattern
        comma_match = TRAILING_NUMBER_WITH_COMMA_PATTERN.match(value)
        if comma_match:
            new_value = comma_match.group(1)  # Get the base word without the comma and number
        ...
        # Handle special case for "úngol1-2" or "úngol1,2" pattern
        complex_match = COMPLEX_TRAILING_NUMBER_PATTERN.match(value)
        if complex_match:
            new_value = complex_match.group(1)  # Get the base word without the numbers
