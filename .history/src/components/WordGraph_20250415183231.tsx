...
const getRelationshipTypeLabel = (type: string): { group: string, label: string } => {
  ...
  return (
    <div className={`word-graph ${theme}`} style={{ width: '100%', height: '100%' }}>
      {isValidNetwork ? (
        <svg
          ref={svgRef}
          className="graph-svg"
        ...
        <div className="invalid-network-message">
          <p>Invalid network data structure.</p>
          {wordNetwork && <p>Please check console for details.</p>}
        </div>
      )}
      ...
      {hoveredNode && (
        <div 
          ...
          style={{
            ...
            top: `${hoveredNode.y ? (hoveredNode.y - 40) : 0}px`
          }}
        >
          <div className="tooltip-content">
            <h4>{hoveredNode.word}</h4>
            {hoveredNode.definitions && hoveredNode.definitions.length > 0 && (
      ...
      {/* Network Controls in a separate container */}
      <NetworkControls 
        onRelationshipFilterChange={handleToggleRelationshipFilter}
        onShowDisconnectedNodesChange={setShowDisconnectedNodes}
        showDisconnectedNodes={showDisconnectedNodes}
        selectedRelationships={filteredRelationships}
        allRelationships={['main', 'root', 'root_of', 'synonym', 'antonym', 'related', 'variant', 'derived', 'etymology', 'cognate', 'usage', 'variant', 'taxonomic', 'part_whole', 'component', 'component_of']}
        relationshipLabels={['Core', 'Origin', 'Meaning', 'Form', 'Structure', 'Info']}
