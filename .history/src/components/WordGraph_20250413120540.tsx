...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const handleToggleRelationshipFilter = useCallback((relationship: string) => {
    console.log("Toggling filter for:", relationship);
    console.log("Current filters:", filteredRelationships);
    
    const relationshipLower = relationship.toLowerCase();
    const isCurrentlyFiltered = filteredRelationships.includes(relationshipLower);
    
    // Update filter state
    setFilteredRelationships(prevFilters => {
      if (isCurrentlyFiltered) {
        console.log("Removing filter:", relationshipLower);
        return prevFilters.filter(f => f !== relationshipLower);
      } else {
        console.log("Adding filter:", relationshipLower);
        return [...prevFilters, relationshipLower];
      }
    });
    
    // IMPORTANT: Directly update the DOM elements for immediate visual feedback
    if (!svgRef.current) return;
    
    const svg = d3.select(svgRef.current);
    
    // Update the nodes with this relationship type
    svg.selectAll(".node")
      .filter(function(d: any) {
        return d.group.toLowerCase() === relationshipLower;
      })
      .classed("filtered", !isCurrentlyFiltered) // Toggle the filtered class
      .transition()
      .duration(300)
      .style("opacity", isCurrentlyFiltered ? 1 : 0) // If currently filtered, we're removing the filter
      .style("pointer-events", isCurrentlyFiltered ? "all" : "none");
    
    // Update the links with this relationship type
    svg.selectAll(".link")
      .filter(function(d: any) {
        return d.type?.toLowerCase() === relationshipLower;
      })
      .classed("filtered", !isCurrentlyFiltered) // Toggle the filtered class
      .transition()
      .duration(300)
      .style("opacity", isCurrentlyFiltered ? 0.6 : 0) // If currently filtered, we're removing the filter
      .style("pointer-events", isCurrentlyFiltered ? "all" : "none");
    
    // Force a redraw of the entire graph when filters change completely
    if ((filteredRelationships.length === 1 && isCurrentlyFiltered) || 
        (filteredRelationships.length === 0 && !isCurrentlyFiltered)) {
      console.log("First filter added or last filter removed - forcing full redraw");
