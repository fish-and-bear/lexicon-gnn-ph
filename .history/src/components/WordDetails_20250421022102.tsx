              ...
              <Stack direction="row" spacing={2} flexWrap="wrap" alignItems="center">
                {wordInfo.pronunciations.map((pron, index) => (
                  <Box key={index} sx={{ display: 'inline-flex', alignItems: 'center', 
                    ...
                    <Typography variant="caption" sx={{ color: alpha(headerTextColor, 0.7), fontWeight: 500, mr: 0.5 }}>
                      {pron.type || 'Pronunciation'}:
                    </Typography>
                    <Typography variant="body2" sx={{ fontStyle: pron.type === 'IPA' ? 'italic' : 'normal' }}>
                      {pron.type === 'IPA' ? `/${pron.value}/` : pron.value}
                    </Typography>
                  </Box>
                ))}
              </Stack>
