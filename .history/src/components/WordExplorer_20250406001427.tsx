...
const WordExplorer: React.FC = () => {
  ...
  const handleSearch = useCallback(async (wordToSearchParam?: string) => {
    ...
    try {
      // Special handling for known problematic terms
      const problematicTerms = ['lima', 'dala', 'gawa'];
      const isProblematicTerm = problematicTerms.includes(normalizedInput.toLowerCase());
      
      if (isProblematicTerm) {
        console.log(`"${normalizedInput}" is a known problematic term. Using direct search.`);
        // Try direct search first, bypassing circuit breaker
        try {
          const directResults = await directSearch(normalizedInput);
          console.log('Direct search results:', directResults);
          
          if (directResults.words.length > 0) {
            const firstResult = directResults.words[0];
            ...
            // Continue with fetching details
            fetchWordDetails(firstResult.id.toString())
              ...
              .catch(err => {
                console.error("Error fetching details after direct search:", err);
          ...
          console.warn('Direct search failed, falling back to normal flow:', directError);
          // Continue with normal flow
        }
      }
        
      if (selectedWordInfo && selectedWordInfo.lemma) {
        console.log('Direct fetch successful, word data:', selectedWordInfo);
        setSelectedWordInfo(selectedWordInfo);
        setMainWord(selectedWordInfo.lemma);
        setInputValue(selectedWordInfo.lemma);

        setWordHistory(prev => [...prev.slice(0, currentHistoryIndex + 1), selectedWordInfo!.lemma]);
        setCurrentHistoryIndex(prev => prev + 1);
        setShowSuggestions(false);

        await Promise.all([
          fetchWordNetworkData(selectedWordInfo.lemma, depth),
          fetchEtymologyTree(selectedWordInfo.id)
        ]);
      } else {
