...
import { WordNetwork, WordInfo, SearchWordResult, SearchOptions, EtymologyTree, Statistics, Definition, WordNetworkOptions, Etymology } from "../types";
...
const WordExplorer: React.FC = () => {
  ...
  const performSearch = useCallback(async (query: string, currentOffset: number, replace = false) => {
      ...
      try {
          const options: SearchOptions = { q: query, offset: currentOffset, limit };
          // Assuming searchWords returns { words: SearchWordResult[], pagination: { has_more: boolean } }
          const data = await searchWords(options);
          const words = data.words || [];
          setSearchResults(prev => replace ? words : [...prev, ...words]);
