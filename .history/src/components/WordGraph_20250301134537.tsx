...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  // Ensure wordNetwork has the expected structure
  ...
  const getNodeRelation = useCallback((word: string, node: NetworkNode): string => {
    ...
    if (node.type === 'related') return "associated";
    
    // Check path to determine relationship
    if (node.path && node.path.length > 0) {
      const lastPathItem = node.path[node.path.length - 1];
      switch (lastPathItem.type) {
        case 'synonym': return "associated";
        case 'antonym': return "associated";
        case 'derived': return "derivative";
