...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  useEffect(() => {
    ...
    // Create a more compact multi-column legend
    const legendContainer = svg.append("g")
      .attr("class", "legend")
      .attr("transform", `translate(${width - 220}, 15)`);
    
    // Define compact legend properties
    const legendPadding = 12;
    const legendItemHeight = 18;
    const legendItemWidth = 95;
    const legendCols = 2; // Two columns for better space efficiency
    const dotRadius = 4.5; // Slightly larger dots for better visibility
    const colSpacing = 12; // Spacing between columns
    ...
    // Find total rows needed for legend layout
    let totalRows = 0;
    categories.forEach(cat => {
      // Each category needs 1 row for header + rows for items
      totalRows += 1 + Math.ceil(cat.types.length / legendCols);
    });
    
    // Calculate legend dimensions
    const legendWidth = (legendItemWidth * legendCols) + (colSpacing * (legendCols - 1)) + (legendPadding * 2);
    const legendHeight = (totalRows * legendItemHeight) + (legendPadding * 2);
    
    // Add legend background rectangle with better styling
    legendContainer.append("rect")
      .attr("width", legendWidth)
      .attr("height", legendHeight)
      .attr("rx", 8)
      .attr("ry", 8)
      .attr("fill", theme === "dark" ? "rgba(25, 25, 30, 0.85)" : "rgba(255, 255, 255, 0.92)")
    ...
    let yPos = legendPadding + 14;
    
    // Render each category
    categories.forEach(category => {
      // Add category header with better styling
      ...
      // Add category items in columns
      let itemsAdded = 0;
      category.types.forEach(type => {
        // Calculate position in grid
        const col = itemsAdded % legendCols;
        const row = Math.floor(itemsAdded / legendCols);
        
        // Calculate x,y coordinates
        const xPos = legendPadding + (col * (legendItemWidth + colSpacing));
        const rowYPos = yPos + ((row + 1) * legendItemHeight);
        
        // Create legend entry group with hover interaction
        const entry = legendContainer.append("g")
          .attr("transform", `translate(${xPos}, ${rowYPos})`);
          
        // Create a hover target rectangle (invisible)
        entry.append("rect")
        ...
        // Add color dot with subtle animation
        entry.append("circle")
          .attr("cx", 4)
          .attr("cy", 0)
          .attr("r", dotRadius)
        ...
        entry.append("text")
          ...
          .text(getRelationshipTypeLabel(type).label);
        
        itemsAdded++;
      });
      
      // Update y position for next category 
      // Move to next row if we have an incomplete row
      if (category.types.length % legendCols !== 0) {
        yPos += Math.ceil(category.types.length / legendCols) * legendItemHeight;
      } else {
        yPos += (category.types.length / legendCols) * legendItemHeight;
      }
    });
