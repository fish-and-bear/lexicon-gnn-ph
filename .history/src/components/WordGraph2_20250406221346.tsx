...
const WordGraph2: React.FC<WordGraphProps> = ({
  ...
  useEffect(() => {
    ...
    // Continue with graph rendering if we have data...
    ...
    if (links.length > 0) {
      g.selectAll('.link')
        ...
        .attr('stroke-opacity', 0.8) // Increase opacity for better visibility
        .attr('stroke-width', 2.5) // Increase width for better visibility
        .attr('marker-end', (d: any) => {
          // Add arrow markers for directed relationships
          if (d.type === 'hypernym' || d.type === 'hyponym') {
            return 'url(#arrow)';
          }
          return null;
        });

      // Add arrowhead marker definition for directed relationships
      svg.append("defs").selectAll("marker")
        .data(["arrow"])
        .enter().append("marker")
        .attr("id", d => d)
        .attr("viewBox", "0 -5 10 10")
        .attr("refX", 20)  // Position the arrowhead away from the end
        .attr("refY", 0)
        .attr("markerWidth", 6)
        .attr("markerHeight", 6)
        .attr("orient", "auto")
