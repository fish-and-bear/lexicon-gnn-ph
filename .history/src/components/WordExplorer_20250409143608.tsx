      ...
      if (!isConnected) {
        ...
        try {
          ...
          if (directFetchSuccess) {
            ...
            setError("Cannot connect to the API server. Please check that the backend server is running on port 10000.");
          }
        } catch (e) {
          console.error("Direct fetch attempt failed:", e);
          setApiConnected(false);
          setError("Cannot connect to the API server. Please check that the backend server is running on port 10000 and network settings allow the connection.");
        }
      } else {
        // If connection is successful, try to use the API
        if (inputValue) {
