  ...
  const handleNodeClick = useCallback(async (word: string) => {
    ...
    if (word !== mainWord) {
      ...
      try {
        ...
        if (!isNaN(wordId)) {
          // If it's a numeric ID, try to fetch directly using the ID endpoint
          console.log(`Treating ${word} as a word ID, fetching directly`);
          try {
            // Use a dedicated endpoint for fetching by ID, not the word text endpoint
            const response = await axios.get(`/words/id/${wordId}`, {
              headers: {
            ...
            if (response.status === 200 && response.data) {
              ...
              console.error("Invalid response from ID endpoint:", response);
              wordData = null;
            }
          } catch (idError) {
