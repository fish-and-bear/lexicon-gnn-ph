...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const filteredNodes = useMemo<CustomNode[]>(() => {
    ...
    while (queue.length > 0) {
      ...
      // Get all links connected to the current node
      const relatedLinks = baseLinks.filter(link =>
        link.source === currentWordId || link.target === currentWordId
      );

      // Sort links by relationship priority for breadth filtering
      ...
      // Apply breadth parameter - more breadth = more connections shown
      // Only limit if we have more links than our breadth value
      const maxConnectionsToShow = Math.min(prioritizedLinks.length, breadth);
      const limitedLinks = prioritizedLinks.slice(0, maxConnectionsToShow);

      // Process each connection and add to BFS queue
      for (const link of limitedLinks) {
        const connectedId = link.source === currentWordId ? link.target : link.source;
        
        // Skip already visited nodes
        if (visited.has(connectedId)) continue;
        
        // Add to connected set and queue for next depth level
  ...
  const centerOnMainWord = useCallback((svg: d3.Selection<SVGSVGElement, unknown, null, undefined>, nodesToSearch: CustomNode[]) => {
    ...
    if (mainNodeData && mainNodeData.x !== undefined && mainNodeData.y !== undefined) {
      ...
      const targetScale = Math.max(0.8, Math.min(2, currentTransform.k));
      
      // Center the main node in the view
      ...
      // Use transition for smooth centering
  ...
  useEffect(() => {
    ...
    // Setup base SVG with zoom behavior
    ...
    // Create the simulation with proper dimensions
    ...
    const drag = d3.drag<SVGCircleElement, CustomNode>()
      .on("start", (event, d) => {
  ...
  }, [filteredNodes, filteredLinks, getNodeColor, getNodeRadius, mainWord, theme, onNodeClick, centerOnMainWord]);
