...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const setupNodeInteractions = useCallback((
      ...
      nodeSelection.on("mouseenter", function(event, d) {
          ...
          connectedLinks.each((l: CustomLink) => {
              const sourceId = typeof l.source === 'object' ? (l.source as CustomNode).id : l.source as string;
              const targetId = typeof l.target === 'object' ? (l.target as CustomNode).id : l.target as string;
      ...
      nodeSelection.on("mouseleave", function(event, d) {
          ...
          // Reset connected nodes (except main node)
          d3.selectAll<SVGGElement, CustomNode>(".node")
            .style("opacity", n => n.id === mainWord ? 1.0 : 0.8);
      });
      
      // ONLY double-click to navigate - remove single click behavior entirely
      nodeSelection.on("dblclick", function(event, d) {
          ...
          // Add extra delay to avoid interference with other operations
          setTimeout(() => {
              ...
              // Explicitly navigate to this word
              if (onNodeClick) {
                  console.log(`Calling onNodeClick with: ${d.word}`);
                  onNodeClick(d.word);
              }
          }, 50);
      });
  ...
  }, [mainWord, onNodeClick, getNodeColor]);

  // Make drag behavior more robust to prevent interference with double-clicks
  const createDragBehavior = useCallback((simulation: d3.Simulation<CustomNode, CustomLink>) => {
    let dragStartTime = 0;
  ...
  // Simplified CSS without selection styling (based on hover only)
  useEffect(() => {
  ...
  // Update node data attributes for CSS targeting
  useEffect(() => {
    if (svgRef.current) {
  ...
  // Helper function to highlight a node and its connections
  const highlightConnections = useCallback((nodeId: string) => {
      ...
      d3.selectAll<SVGLineElement, CustomLink>(".link").filter((l: CustomLink) => {
          const sourceId = typeof l.source === 'object' ? (l.source as CustomNode).id : l.source as string;
          const targetId = typeof l.target === 'object' ? (l.target as CustomNode).id : l.target as string;
          if (sourceId === nodeId) { connectedIds.add(targetId); return true; }
          if (targetId === nodeId) { connectedIds.add(sourceId); return true; }
          return false;
      }).each(function() { connectedLinkElements.push(this); });
      
      // Show connected nodes
      d3.selectAll<SVGGElement, CustomNode>(".node")
          .filter(n => connectedIds.has(n.id) && n.id !== nodeId)
      ...
      d3.selectAll(connectedLinkElements)
          ...
          .attr("stroke", function(this: SVGLineElement, datum: any) {
              ...
              if (!nodeData) return theme === "dark" ? "#aaa" : "#666";
              const neighbourNode = sourceId === nodeId ? nodeMap.get(targetId) : nodeMap.get(sourceId);
              return neighbourNode ? getNodeColor(neighbourNode.group) : (theme === "dark" ? "#aaa" : "#666");
          })
          .attr("stroke-opacity", 0.9)
          .attr("stroke-width", 2.0);
  }, [theme, getNodeColor, nodeMap]);
