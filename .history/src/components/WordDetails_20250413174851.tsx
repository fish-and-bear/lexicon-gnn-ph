...
import { WordInfo, RelatedWord, Definition, NetworkLink, NetworkNode } from '../types';
...
const relationColors: Record<string, string> = {
  synonym: '#4CAF50',  // Green
  antonym: '#F44336',  // Red
  hypernym: '#2196F3', // Blue
  hyponym: '#03A9F4',  // Light Blue
  meronym: '#9C27B0',  // Purple
  holonym: '#673AB7',  // Deep Purple
  related: '#FF9800',  // Orange
  derived: '#795548',  // Brown
  form_of: '#607D8B',  // Blue Grey
  variant: '#FFEB3B',  // Yellow
  other: '#9E9E9E'     // Grey
};
...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({
  ...
  const renderRelationsTab = () => {
    ...
    const hasRelationSummary = Boolean(wordInfo.relation_summary?.total_relations);
    const hasSemanticNetwork = Boolean(wordInfo.semantic_network?.links?.length);
    
    // Check if this is a case where we know we SHOULD have relations but they're not loaded
    const relationsLoadingOrMissing = 
      (incoming_relations.length === 0 && outgoing_relations.length === 0) && 
      (hasRelationSummary || hasSemanticNetwork);
    
    // Normal case - no relations actually exist
    if (!relationsLoadingOrMissing && incoming_relations.length === 0 && outgoing_relations.length === 0) {
      return <Alert severity="info">No relations available for this word.</Alert>;
    }

    // When relations are missing but should exist based on graph data
    if (relationsLoadingOrMissing) {
      return (
        <Box sx={{ pt: theme.spacing(1) }}>
          <Alert 
            severity="info" 
          ...
          >
            Word relationships exist in the semantic network but haven't been loaded yet.
          </Alert>
          
          {/* Show any placeholder relationships from graph data if available */}
          {wordInfo.semantic_network?.links && wordInfo.semantic_network.links.length > 0 && (
            <Box sx={{ mt: theme.spacing(2) }}>
              ...
              <Stack direction="row" spacing={1} useFlexGap flexWrap="wrap">
                {wordInfo.semantic_network.links.map((link: NetworkLink, index: number) => {
                  // Make a safe reference to nodes
                  const nodes = wordInfo.semantic_network?.nodes || [];
                  
                  // Find the target node
                  const targetNode = nodes.find(
                    (n: NetworkNode) => n.id === (typeof link.target === 'object' ? link.target.id : link.target)
                  );
                  if (!targetNode) return null;
                  
                  const relType = link.type || 'related';
                  const relColor = relationColors[relType] || relationColors.other;
                  const isDark = !isColorLight(relColor);
                  
                  return (
                    <Chip
                      key={`graph-relation-${index}`}
                      label={`${targetNode.label || targetNode.id} (${relType})`}
                      className="relation-chip"
                      onClick={() => onWordLinkClick(targetNode.label || String(targetNode.id))}
                      sx={{
                        backgroundColor: alpha(relColor, 0.9),
    ...
    // Enhanced relation visualization
    
    // Group relations by type for better organization
    const groupRelationsByType = (relations: WordInfo['outgoing_relations'] | WordInfo['incoming_relations'], direction: 'in' | 'out') => {
      if (!relations || relations.length === 0) return {};
      
      const groupedRelations: Record<string, any[]> = {};
      
      relations.forEach((rel) => {
        const anyRel = rel as AnyRecord;
        ...
        groupedRelations[relType].push({
          ...anyRel,
          wordObj: direction === 'in' ? anyRel.source_word : anyRel.target_word
        });
      });
    ...
    // Create categorized relation groups
    const categorizeRelations = () => {
      const incomingByType = groupRelationsByType(incoming_relations, 'in');
      const outgoingByType = groupRelationsByType(outgoing_relations, 'out');
      
      // Define categories and their priority order
      const categories = [
        {
          name: 'Semantic',
          types: ['synonym', 'antonym', 'similar', 'related'],
          color: graphColors.synonym,
          icon: '🔄'  // Circular arrows for semantic relations
        },
        {
          name: 'Taxonomic',
          types: ['hypernym', 'hyponym', 'meronym', 'holonym'],
          color: graphColors.associated,
          icon: '🌲'  // Tree for taxonomic hierarchy
        },
        {
          name: 'Derivational',
          types: ['derived', 'root', 'affix'],
          color: graphColors.derived,
          icon: '🔀'  // Shuffle arrows for derivations
        },
        {
          name: 'Variant',
          types: ['variant', 'spelling', 'abbreviation'],
          color: graphColors.variant,
          icon: '🔤'  // ABC for variants
        },
        {
          name: 'Other',
          types: ['other'],
          color: graphColors.default,
          icon: '🔗'  // Chain for other connections
        }
      ];

      return categories.map(category => {
        const incomingInCategory: Record<string, any[]> = {};
        const outgoingInCategory: Record<string, any[]> = {};
        
        // Filter relations by category
        category.types.forEach(type => {
          if (incomingByType[type]) incomingInCategory[type] = incomingByType[type];
          if (outgoingByType[type]) outgoingInCategory[type] = outgoingByType[type];
        });
        
        const hasAnyRelations = 
          Object.keys(incomingInCategory).length > 0 || 
          Object.keys(outgoingInCategory).length > 0;
        
        return {
          ...category,
          hasRelations: hasAnyRelations,
          incoming: incomingInCategory,
          outgoing: outgoingInCategory
        };
      }).filter(cat => cat.hasRelations); // Only keep categories with relations
    };

    const relationCategories = categorizeRelations();
    
    // Interactive relation node component
    const RelationNode = ({ 
      relation, 
      relType, 
      direction,
      ...
      const relColor = relationColors[relType] || relationColors.other;
      const baseColor = categoryColor || relColor;
      const isDark = !isColorLight(baseColor);
      
      // Check for additional data
      const hasSources = relation.sources && 
        (typeof relation.sources === 'string' || 
        (Array.isArray(relation.sources) && relation.sources.length > 0));
      
      const hasMetadata = relation.metadata && Object.keys(relation.metadata).length > 0;
        ...
        >
          ...
          <Box
            sx={{
              position: 'absolute',
              top: 0,
              [direction === 'in' ? 'left' : 'right']: 0,
              width: 0,
              height: 0,
              borderStyle: 'solid',
              borderWidth: direction === 'in' ? '0 12px 12px 0' : '0 0 12px 12px',
              borderColor: direction === 'in' 
                ? `transparent ${alpha(baseColor, 0.7)} transparent transparent` 
                : `transparent transparent ${alpha(baseColor, 0.7)} transparent`,
              zIndex: 2
            }}
          />
          ...
          <Box
            ...
            sx={{
              ...
              bgcolor: isExpanded ? alpha(baseColor, 0.05) : 'background.paper',
            }}
          >
            <Box sx={{ display: 'flex', alignItems: 'center', overflow: 'hidden' }}>
              {/* Word with Baybayin if available */}
              <Box sx={{ minWidth: 0, flexGrow: 1 }}>
                ...
                {wordObj.has_baybayin && wordObj.baybayin_form && (
                  <Typography 
                    variant="caption" 
                    sx={{ 
                      fontFamily: 'Noto Sans Baybayin, sans-serif',
            ...
            {/* Expand button if there's additional data */}
            {(hasSources || hasMetadata) && (
              <IconButton 
                size="small" 
                onClick={(e) => {
                  e.stopPropagation();
          ...
          {/* Expandable details */}
          {isExpanded && (hasSources || hasMetadata) && (
            <Box sx={{ 
              ...
              {hasSources && (
                <Box sx={{ mb: 1 }}>
                  ...
                  <Stack direction="row" spacing={0.5} useFlexGap flexWrap="wrap" sx={{ mt: 0.5 }}>
                    {(Array.isArray(relation.sources) ? relation.sources : [relation.sources]).map((source: string, i: number) => (
                      <Chip
                        key={i}
                        label={source}
                        size="small"
    ...
    return (
      <Box sx={{ pt: theme.spacing(1) }}>
        {relationCategories.length === 0 ? (
          <Alert severity="info">No relations available for this word.</Alert>
        ) : (
          <Box>
            {/* Stats summary */}
            <Box sx={{ mb: theme.spacing(3) }}>
              <Typography variant="body2" sx={{ color: 'text.secondary', display: 'flex', alignItems: 'center', gap: 1 }}>
                <Box component="span" sx={{ fontWeight: 500 }}>
                  Total Relations: {incoming_relations.length + outgoing_relations.length}
                </Box>
                •
                <Box component="span" sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                  ...
                  />
                  Incoming: {incoming_relations.length}
                </Box>
                •
                <Box component="span" sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                  <Box
                  ...
                  />
                  Outgoing: {outgoing_relations.length}
                </Box>
              </Typography>
            </Box>
            
            {/* Relation categories */}
            {relationCategories.map((category, categoryIndex) => (
              <Box key={category.name} sx={{ mb: theme.spacing(3) }}>
                <Typography 
                  variant="h6" 
                  sx={{ 
                ...
                >
                  <Box component="span" sx={{ mr: 1 }}>{category.icon}</Box>
                  {category.name} Relations
                </Typography>
                
                <Box sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, gap: 2 }}>
                  {/* Incoming relations */}
                  {Object.keys(category.incoming).length > 0 && (
                    <Box sx={{ 
                      flex: 1,
                      minWidth: 0,
                      p: 1.5, 
                      ...
                      bgcolor: alpha('success.main', 0.03)
                    }}>
                      <Typography 
                        variant="subtitle2" 
                        sx={{ 
                      ...
                      >
                        ...
                        Incoming ({Object.values(category.incoming).flat().length})
                      </Typography>
                      
                      {Object.entries(category.incoming).map(([relType, rels]) => (
                        <Box key={relType} sx={{ mb: 2 }}>
                          {rels.map((rel, index) => (
                            <RelationNode 
                              key={`${rel.id || index}`}
                              relation={rel} 
                  ...
                  {/* Outgoing relations */}
                  {Object.keys(category.outgoing).length > 0 && (
                    <Box sx={{ 
                      flex: 1,
                      minWidth: 0,
                      p: 1.5, 
                      ...
                      >
                        Outgoing ({Object.values(category.outgoing).flat().length})
                        <Box 
                          component="span" 
                          sx={{ 
                            ml: 1,
                      ...
                      {Object.entries(category.outgoing).map(([relType, rels]) => (
                        <Box key={relType} sx={{ mb: 2 }}>
                          {rels.map((rel, index) => (
                            <RelationNode 
                              key={`${rel.id || index}`}
                  ...
                  <Tab 
                    label={
                      <Box sx={{ position: 'relative' }}>
                        Relations
                        {Boolean(wordInfo.incoming_relations?.length || wordInfo.outgoing_relations?.length) && (
                          <Box
                            component="span"
                            sx={{
                              width: 8,
                              height: 8,
                              bgcolor: graphColors.main,
                              borderRadius: '50%',
                              position: 'absolute',
                              top: -2,
                              right: -12,
