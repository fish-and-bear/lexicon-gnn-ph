       ...
       nodeSelection
        ...
        .on("mouseout", (event, d_unknown) => {
            ...
            // Revert hover effect for the circle
             ...
             // Restore opacity for all nodes and links (respecting selection)
             d3.selectAll<SVGGElement, CustomNode>(".node")
                .style("opacity", n => (selectedNodeId === null || n.id === selectedNodeId || connectedIdsRef.current.has(n.id)) ? 1 : 0.1);
             d3.selectAll<SVGLineElement, CustomLink>(".link") // Select lines
                 .filter(l => !d3.select(l as any).classed("highlighted")) // Don't revert highlighted links here
