...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const updateGraph = useCallback(() => {
    try {
      // Skip if there's no container or no data
      if (!containerRef.current || !svgRef.current || !wordNetwork?.nodes?.length) {
        return;
      }
      ...
      createLinks(g);
      createNodes(g, simulation);
      
      // Setup node interactions after creating nodes
      const nodeSelection = g.selectAll(".node");
      if (nodeSelection && !nodeSelection.empty()) {
        setupNodeInteractions(nodeSelection as any, svg);
      }

      // Center the graph immediately with slight delay to allow for initial layout
      setTimeout(() => {
        // Initial centering
        ...
        // After brief stabilization, release the main node if it was fixed
        if (simulation) {
          setTimeout(() => {
      ...
      // Track if this is an update for a new main word
      if (prevMainWordRef.current !== mainWord) {
        // If main word changed, center on it after brief delay to allow layout to adjust
        setTimeout(() => {
      ...
      return undefined; // Ensure we always return a value with the correct type
    }
  }, [
    wordNetwork, 
    filteredNodes, 
  ...
  useEffect(() => {
    // Call updateGraph and store its cleanup function if it returns one
  ...
  const renderTooltip = useCallback(() => {
    if (!hoveredNode || isDraggingRef.current) return null;
    
    try {
      // Get the node's current position
      const nodeElement = svgRef.current?.querySelector(`.node[data-id="${hoveredNode.id}"]`);
      ...
      // Apply any SVG transformations
      const transformedPos = getTransformedPosition(x, y);
      ...
      let left = transformedPos.x + 15;
      let top = transformedPos.y - 15;
      
      // Adjust if tooltip would go off the right edge
      if (left + 250 > width) {
        left = transformedPos.x - 250 - 15;
      }
      ...
      // Combine positioning with base style
      const combinedStyle = {
        ...tooltipStyle,
        left: `${left}px`,
        top: `${top}px`,
      ...
      // Get the definition safely from info object if available
      const getDefinition = () => {
        if (!hoveredNode.info || !hoveredNode.info.definitions || !hoveredNode.info.definitions.length) {
          return null;
        }
        
        const firstDef = hoveredNode.info.definitions[0];
        if (typeof firstDef === 'string') {
          return firstDef;
        } else if (firstDef && typeof firstDef === 'object') {
          return (firstDef as any).text || (firstDef as any).definition_text || 'No definition available';
        }
        
        return null;
      };
      
      // Render the tooltip content with proper type checking
      return (
        <div className="node-tooltip" style={combinedStyle}>
          <div style={{ fontWeight: 'bold', marginBottom: '5px' }}>
            {hoveredNode.word}
          </div>
          
          {hoveredNode.info && hoveredNode.info.type && (
            <div style={{ fontSize: '0.8rem', opacity: 0.8, marginBottom: '5px' }}>
              {hoveredNode.info.type}
            </div>
          )}
  ...
  }, [hoveredNode, theme, getTransformedPosition, isDraggingRef]);
  ...
  return (
    <div 
      ...
      style={{ 
        ...
        height: '100%', 
        overflow: 'hidden',
        backgroundColor: theme === 'dark' ? '#1a1a1a' : '#f7f7f7' 
      }}
      ref={containerRef}
    >
      {isLoading && (
