...
const relationColors: Record<string, string> = {
  ...
  root: '#5D4037',        // Dark Brown
  affix: '#A1887F',       // Light Brown
  
  // Variant relations
  variant: '#FFEB3B',     // Yellow
  spelling: '#FBC02D',    // Amber
  abbreviation: '#FFC107', // Amber
  
  // Form relations
  form_of: '#607D8B',     // Blue Grey
  
  // Specific Filipino relations
  kaugnay: '#FF5722',     // Deep Orange
  salita: '#E91E63',      // Pink
  kahulugan: '#9C27B0',   // Purple
  kasalungat: '#F44336',  // Red
  
  // Additional relations
  cognate: '#009688',     // Teal
  etymology: '#00BCD4',   // Cyan
  component: '#3F51B5',   // Indigo
  component_of: '#3949AB', // Darker Indigo
  main: '#1A237E',        // Dark Blue
  derivative: '#827717',  // Olive
  associated: '#78909C',  // Blue Grey
  
  // Fall back for any other relation types
  other: '#9E9E9E'        // Grey
};
...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({
  ...
  const renderRelationsTab = () => {
    ...
    // Use semantic network links if regular relations are empty
    ...
    // Group relations by type (sorting them alphabetically)
    const relationsByType: Record<string, any[]> = {};
    
    allRelations.forEach(rel => {
      const relType = rel.relation_type || 'other';
      if (!relationsByType[relType]) {
        relationsByType[relType] = [];
      }
      relationsByType[relType].push(rel);
    });
    
    // Sort relation types by priority and then alphabetically
    const priorityOrder = [
      'synonym', 'antonym', 'hypernym', 'hyponym', 'similar', 'related', 
      'derived', 'root', 'kaugnay', 'kasalungat', 'kahulugan',
      'variant', 'cognate', 'etymology'
    ];
    
    const sortedRelationTypes = Object.keys(relationsByType).sort((a, b) => {
    ...
    // Log the relation types found to help debugging
    console.debug("Relation types found:", sortedRelationTypes);
    
    // Calculate the number of unique related words
    const uniqueRelatedWords = new Set(
      allRelations.map(rel => rel.wordObj?.id).filter(Boolean)
    ).size;
    ...
    return (
      <Box sx={{ pt: theme.spacing(1) }}>
        ...
        <Box sx={{ mb: theme.spacing(3) }}>
          {sortedRelationTypes.map((relType) => {
            ...
            return (
              <Box 
                ...
                sx={{ 
                  mb: theme.spacing(2),
                  pb: theme.spacing(2),
                  borderBottom: relType !== sortedRelationTypes[sortedRelationTypes.length-1] ? 
              ...
              >
                ...
                >
                  {relations.map((relation, index) => {
                    ...
                    return (
                      <Chip
                        key={`${relation.id || index}-${wordObj.id || index}`}
                        label={wordObj.lemma || wordObj.word}
