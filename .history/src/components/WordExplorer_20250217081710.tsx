...
const WordExplorer: React.FC = () => {
  ...
  const handleSearch = useCallback(async (searchWord?: string) => {
    ...
    try {
      ...
      if (!networkData || !networkData.nodes || Object.keys(networkData.nodes).length === 0) {
          ...
          <div className="details-content">
            {isLoading ? (
              ...
              <div className="word-details">
                ...
                {renderArraySection(
                  "Synonyms",
                  selectedWordInfo.relations.from
                ...
                {renderArraySection(
                  "Derivatives",
                  selectedWordInfo.relations.to
                    .filter(rel => rel.type === 'derived_from')
                    .map(rel => rel.from_word)
                )}
                {selectedWordInfo.relations.from.find(rel => rel.type === 'derived_from')?.to_word && (
                  <p>
