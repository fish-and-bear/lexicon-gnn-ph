...
const WordExplorer: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Defer the initial API check slightly
    const connectionCheckTimer = setTimeout(() => {
      testApiConnection().then(connected => {
        ...
        if (connected) {
          // Define and call fetchInitialData *after* connection is confirmed
          // This ensures fetchStatistics etc. defined via useCallback are available
          const fetchInitialData = async () => {
            try {
              if (!isMounted) return; // Check mount status again
              
              // Get parts of speech and statistics
              const posData = await getPartsOfSpeech();
              if (isMounted) setPartsOfSpeech(posData);
              
              // fetchStatistics is stable due to useCallback
              fetchStatistics(); 

              // --- MODIFIED: Less Aggressive Random Cache Prefetch ---
              if (isMounted && randomWordCache.length === 0) { 
                console.log("Initial random word cache empty, prefetching smaller batch later...");
                // Fetch a smaller cache after other setup, with a further delay
                const cacheTimer = setTimeout(() => {
                  if (isMounted) { // Check mount status again
                    fetchRandomWordsForCache(RANDOM_CACHE_SIZE);
                  }
                }, 2000); // Wait 2 seconds
                // Return a cleanup function for this specific timer
            ...
            // Indicate that initial data fetching is complete (or attempted)
            // You might want a separate state for this if needed
          };

          // Call fetchInitialData now
          fetchInitialData(); // This now correctly calls fetchStatistics etc.
          
        } else {
          console.error("API Connection Failed on Initial Load");
          setError("Failed to connect to the backend API on startup. Ensure server is running.");
        }
      }).catch(error => {
        if (isMounted) {
          console.error("API connection error during initial check:", error);
          setApiConnected(false);
          setError("Error connecting to the backend API. Check server status.");
        }
      });
    ...
    return () => {
      ...
      // Note: The cacheTimer cleanup is handled by returning it from fetchInitialData,
      // but React doesn't automatically call nested cleanup functions.
      // For simplicity here, we rely on the isMounted check within the cacheTimer's setTimeout.
      // A more robust solution might involve lifting the cacheTimer ref out.
    };
  // Correct dependencies: Include functions from useCallback and state values read directly.
