...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const getNodeRelation = useCallback((word: string, info: NetworkNode): string => {
  ...
  const nodes: CustomNode[] = useMemo(() => {
    ...
    const addNode = (word: string, info: NetworkNode) => {
      if (!nodeMap.has(word)) {
        nodeMap.set(word, {
          id: word,
  ...
  const getNodeGroup = (node: NetworkNode, mainWord: string): string => {
    ...
    if (wordNetwork?.clusters) {
      for (const clusterName in wordNetwork.clusters) {
        const nodeIds = wordNetwork.clusters[clusterName];
        if (Array.isArray(nodeIds) && nodeIds.includes(node.id)) {
          return clusterName;
        }
      }
    }
