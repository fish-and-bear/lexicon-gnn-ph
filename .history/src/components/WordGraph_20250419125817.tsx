...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const setupNodeInteractions = useCallback((
      ...
      // Add unified click handler with improved detection
      nodeSelection.on("mousedown", (event) => {
      ...
      nodeSelection.on("click", (event, d) => {
          ...
          // Log the click detection info
          console.log(`Click on node: ${d.id}, time diff: ${timeDiff}ms, same node: ${isSameNode}`);
          ...
          // Schedule a single click action with a delay
          clickTimeoutRef.current = setTimeout(() => {
              ...
              d3.selectAll<SVGGElement, CustomNode>(".node")
                ...
                .transition("reset_node_opacity").duration(150)
                .style("opacity", 1)
                .select("circle")
                  .attr("stroke-width", 1.5)
              ...
              d3.selectAll<SVGLineElement, CustomLink>(connectedLinkElements)
                ...
                .attr("stroke-width", 2.5);
          }, 270); // Slightly longer delay for better distinction
      });
  ...
  // Enhanced cleanup for the click timeout
  useEffect(() => {
    return () => {
      if (clickTimeoutRef.current) {
        clearTimeout(clickTimeoutRef.current);
