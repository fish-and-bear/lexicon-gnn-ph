...
const WordExplorer: React.FC = () => {
    ...
    try {
      ...
      if (!fallbackToSearch && wordData) {
        ...
        try {
          ...
          setWordData(prev => prev ? { ...prev, semantic_network: { nodes: networkData.nodes, links: networkData.edges } } : prev);
  ...
  }, [wordHistory, depth, breadth, setDepth, setBreadth, fetchWordDetails, searchWords, fetchEtymologyTree]);
      ...
      if (wordData) {
        ...
        if (!wordData.outgoing_relations?.length && !wordData.incoming_relations?.length) {
          try {
            ...
            fetchWordNetwork(wordData.lemma, { 
              depth,
              breadth,
              include_affixes: true,
              include_etymology: true,
  ...
  }, [wordHistory, depth, breadth, fetchWordDetails, fetchEtymologyTree]);
  ...
  }, [depth, breadth, wordHistory, currentHistoryIndex, fetchEtymologyTree]);
  ...
  const handleSuggestionClick = useCallback(async (suggestion: SearchWordResult) => {
    ...
    try {
      ...
      try {
        ...
        await fetchWordNetwork(suggestion.lemma, { 
          depth,
          breadth,
          include_affixes: true,
          include_etymology: true,
  ...
  }, [loadWordData, depth, breadth, fetchEtymologyTree]);
  ...
  const handleBack = useCallback(() => {
    if (currentHistoryIndex > 0) {
      ...
      Promise.all([
        ...
        fetchWordNetwork(wordText, { depth, breadth, include_affixes: true, include_etymology: true, cluster_threshold: 0.3 })
      ])
  ...
  }, [currentHistoryIndex, wordHistory, depth, breadth, fetchWordDetails, fetchEtymologyTree]);
          ...
          depth && breadth 
            ? fetchWordNetwork(randomWord.lemma, { 
              ...
              .then(networkData => {
                ...
                setWordData(networkData); // Ensure network is visualized
              })
              .catch(err => {
  ...
  }, [mainWord, depth, breadth, setDepth, setBreadth]);
  ...
  // Instead, use the new useWordNetwork hook to fetch the network automatically.
  // Assume that 'selectedWord' is the currently selected word (already defined in the component state).
  const { data: wordNetwork, isLoading: networkLoading, error: networkError } = useWordNetwork(selectedWord, { depth, breadth, include_affixes: true, include_etymology: true, cluster_threshold: 0.3 });
