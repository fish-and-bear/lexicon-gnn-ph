...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  // Clear tooltip timeout on unmount or when hoveredNode changes
  useEffect(() => {
    return () => {
      if (tooltipTimeoutId) {
  ...
  const handleMouseOver = useCallback((event: MouseEvent, d: CustomNode) => {
    // Clear any existing timeout before setting a new one
    if (tooltipTimeoutId) {
      clearTimeout(tooltipTimeoutId);
    }
    
    // Set a new timeout to show tooltip after a delay
    const newTimeoutId = setTimeout(() => {
      setHoveredNode(d);
      // Call onNodeSelect when the tooltip becomes visible
      onNodeSelect(d.id);
    }, 300); // 300ms delay
    
    setTooltipTimeoutId(newTimeoutId);
  }, [onNodeSelect, tooltipTimeoutId]); // Added dependencies

  const handleMouseOut = useCallback(() => {
    // Clear timeout if mouse moves out before tooltip appears
    if (tooltipTimeoutId) {
      clearTimeout(tooltipTimeoutId);
