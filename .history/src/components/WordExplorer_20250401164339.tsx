...
const WordExplorer: React.FC = () => {
  ...
  // DEFINE handleSearch BEFORE handleWordLinkClick
    ...
    try {
      ...
      try {
        ...
        if (wordData && wordData.lemma) {
          ...
          // Fetch word network and etymology tree in parallel - USE wordData.id
          const [networkData] = await Promise.all([
            fetchWordNetworkData(normalizedInput, depth, breadth),
            fetchEtymologyTree(wordData.id) // Use word ID here
          ]);
      ...
      const searchOptions: SearchOptions = { // Explicitly type searchOptions
          page: 1, 
          per_page: 20, 
          exclude_baybayin: true,
          language: 'tl', // Or use selectedLanguage
          mode: 'all', // Ensure mode matches the allowed types
          sort: 'relevance',
          order: 'desc'
        };
      ...
      if (searchResults && searchResults.words && searchResults.words.length > 0) {
        ...
        // Fetch details for the first result - USE LEMMA
        const wordData = await fetchWordDetails(firstResult.lemma); // <<< CORRECTED
        ...
        if (wordData && wordData.lemma) {
          ...
          setWordNetwork(networkData);
          // Use lemma for history
          setWordHistory(prevHistory => [...prevHistory.slice(0, currentHistoryIndex + 1), wordData.lemma]); // <<< CORRECTED
          setCurrentHistoryIndex(prevIndex => prevIndex + 1);
          setInputValue(wordData.lemma); // Update input field with found lemma
          setShowSuggestions(false);
        } else {
          // Use lemma from search result if details fetch failed somehow
          setError(`Could not fetch full details for "${firstResult.lemma}". Displaying basic info.`);
          setMainWord(firstResult.lemma); // <<< CORRECTED
          // Create a minimal WordInfo from the search result
          setSelectedWordInfo({
              id: firstResult.id,
              ...
              definitions: (firstResult.definitions || []).map(def => ({ // Map basic definitions
                  id: def.id,
                  text: def.definition_text,
                  definition_text: def.definition_text,
                  original_pos: def.part_of_speech,
                  part_of_speech: null, // No detailed POS in search result
                  examples: [],
                  usage_notes: [],
                  tags: [],
                  sources: [],
              ...
              // Initialize other required fields as empty/default
              pronunciation: null,
              etymologies: [],
              relations: {},
              tags: [],
          ...
          setWordNetwork(null); // No network if details failed
          setEtymologyTree(null); // No etymology tree
        }
      } else {
  ...
  // Add dependencies as needed, e.g., depth, breadth
  }, [inputValue, depth, breadth, fetchWordNetworkData, fetchEtymologyTree, currentHistoryIndex]); 

  // Function to fetch etymology tree - USE WORD ID
  const fetchEtymologyTree = useCallback(async (wordId: number) => {
    ...
    try {
      ...
      // Use correct arguments: wordId and depth
  ...
  // Depend on wordId if passed, or perhaps selectedWordInfo.id
  }, []); // Dependencies might need adjustment based on usage

  // Correct handler name for clicking nodes/links - DEFINED AFTER handleSearch
  const handleWordLinkClick = useCallback(async (word: string) => {
    console.log("Word link clicked:", word);
    if (word !== mainWord) {
      await handleSearch(word); // Now handleSearch is defined
      // Scroll to top of details maybe?
      detailsContainerRef.current?.scrollTo(0, 0);
    }
  }, [mainWord, handleSearch]);

  const handleNodeClick = handleWordLinkClick; // Use the same handler for graph nodes
  ...
  const fetchWordNetworkData = useCallback(async (word: string, depth: number = 2, breadth: number = 10) => {
    try {
      ...
      // Ensure all required clusters exist
      const requiredClusters = [
        'etymology', 'affixes', 'synonyms', 'antonyms', 'variants',
        'root_words', 'derived_words', 'related', 'kaugnay', 
        'component_of', 'cognate'
      ];
