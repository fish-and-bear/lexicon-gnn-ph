...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({ wordInfo, onWordClick }) => {
  ...
  const renderRelatedWords = () => {
    ...
    const derivedFrom = wordInfo.relations.derived_from?.map(item => item.word) || [];
    const variants = wordInfo.relations.variants?.map(item => item.word) || [];
    const related = wordInfo.relations.related?.map(item => item.word) || [];
    const componentOf = wordInfo.relations.component_of?.map(item => item.word) || [];
    const cognates = wordInfo.relations.cognate?.map(item => item.word) || [];
    
    // Derived words should include both derived and derived_from
    const allDerivatives = [...new Set([...derivatives, ...derivedFrom])];
    
    // Also include any words from the semantic categories
    const mainWords = wordInfo.relations.main?.map(item => item.word) || [];
    const derivativeWords = wordInfo.relations.derivative?.map(item => item.word) || [];
    const etymologyWords = wordInfo.relations.etymology?.map(item => item.word) || [];
    const associatedWords = wordInfo.relations.associated?.map(item => item.word) || [];
    const otherWords = wordInfo.relations.other?.map(item => item.word) || [];
    
    // Get the root word
    const rootWord = wordInfo.relations.root?.word;
    const rootOfWords = wordInfo.relations.root_of?.map(item => item.word) || [];
