...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({ wordInfo, onWordClick }) => {
  ...
  const renderRelatedWords = () => {
    ...
    // Check if relations exist in any format
    if (!wordInfo.relations) {
      console.log("No relations found in the standard format");
      
      // Try to find relations in alternative formats
      const possibleRelations = (wordInfo as any).related_words || 
                               (wordInfo as any).related || 
                               (wordInfo as any).data?.relations;
      
      if (possibleRelations) {
        console.log("Found relations in alternative format:", possibleRelations);
        (wordInfo as any).relations = possibleRelations;
      } else {
        return (
          <div className="empty-state">
    ...
    // Initialize relations object if it doesn't exist
    const relations = wordInfo.relations || {};
    
    // Extract related words from the relations structure
    const synonyms = relations.synonyms?.map(item => item.word) || [];
    const antonyms = relations.antonyms?.map(item => item.word) || [];
    const derivatives = relations.derived?.map(item => item.word) || [];
    const variants = relations.variants?.map(item => item.word) || [];
    const related = relations.related?.map(item => item.word) || [];
    const rootWord = relations.root?.word;
      ...
      <div className="word-details">
        <div className="word-header">
          ...
          <div className="word-meta">
            ...
            {wordInfo.tags && wordInfo.tags.length > 0 && wordInfo.tags.map((tag, index) => (
              <span key={index} className="tag">{tag}</span>
            ))}
            {wordInfo.is_root_word && (
              <span className="tag">Root Word</span>
            )}
          </div>
        </div>
