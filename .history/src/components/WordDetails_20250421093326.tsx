...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({
    ...
    return (
      ...
      <Box sx={{ 
        ...
        <Stack spacing={1.5} sx={{ mb: theme.spacing(0) }}> { /* Keep mb at 0 */ }
          ...
          {wordInfo.pronunciations && wordInfo.pronunciations.filter(p => 
            p.type?.toLowerCase() !== 'ipa' && p.type?.toLowerCase() !== 'audio'
          ).length > 0 && (
            <Box sx={{ mt: 1 }}>
              <Stack direction="row" spacing={2} flexWrap="wrap">
                {wordInfo.pronunciations.filter(p => 
                  p.type?.toLowerCase() !== 'ipa' && p.type?.toLowerCase() !== 'audio'
                ).map((pron, index) => (
                  <Box key={index}>
        ...
        {wordInfo.has_baybayin && wordInfo.baybayin_form && wordInfo.baybayin_form.trim() !== '' && (
          <Box sx={{ my: 0, mt: theme.spacing(1.5) }}> { /* Keep my:0, increase mt slightly */ }
        ...
        {wordInfo.badlit_form && wordInfo.badlit_form.trim() !== '' && (
          <Box sx={{ my: 0, mt: theme.spacing(1.5) }}> { /* Keep my:0, increase mt slightly */ }
            <Typography variant="caption" sx={{ color: alpha(headerTextColor, 0.75), display: 'block', mb: 0.5 }}>
              Badlit Form
            </Typography>
  ...
  const renderRelationsTab = () => {
    ...
    const nonEmptyCategories = relationCategories.filter(
      ...
      return (
                        <Box key={relationType} sx={{ mb: theme.spacing(1.5) }}>
                          ...
                          >
                            {typeRelations.map((relation, index) => {
                              ...
                              const primaryType = relation.primaryType || 'other'; // Get the primary type
                              const displayLabel = getRelationshipTypeLabel(primaryType).label;
                              const allTypeLabels = relation.relationTypes?.map((t: string) => getRelationshipTypeLabel(t).label).join(', ') || displayLabel;
              ...
              return (
                <Chip
                                  key={`${wordObj.id || index}-${wordObj.lemma}`} // More robust key
                                  label={
                                  ...
                                  onClick={() => wordObj.lemma && onWordLinkClick(wordObj.lemma)} // Ensure lemma exists
                                  title={hasMultipleTypes ? `Also: ${allTypeLabels}` : `Type: ${displayLabel}`}
                                  variant="outlined"
                                  sx={{
                                    fontSize: '0.75rem',
                                    height: 'auto',
                                    ...
                                    my: 0.5,
                                    maxWidth: '100%',
                                    border: '1px solid grey', // Simple static border
                                    backgroundColor: 'lightgrey', // Simple static background
                                    color: 'black', // Simple static text color
                                    '& .MuiChip-label': { 
                                      px: 1, 
                                      py: 0.25, 
                                      fontWeight: 500,
  ...
  return (
    <Paper 
      ...
      sx={{ 
        display: 'flex', 
        flexDirection: 'column', // Ensure this is column
        height: '100%', 
        width: '100%', 
