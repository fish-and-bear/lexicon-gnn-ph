  ...
  const setupNodeInteractions = useCallback((
      ...
      nodeSelection
        ...
        .on("mouseout", (event, d_unknown) => {
            ...
            // Revert hover effect for the circle, respecting selected/pinned state
             nodeElement.select<SVGCircleElement>("circle")
                .data([d])
                .transition("hover_border").duration(150)
                ...
                .attr("stroke", (n: CustomNode) => {
                    ...
                    // Provide fallback for formatHex in selected case
                    ...
                    // Provide fallback for formatHex in default case
        ...
        .on("dblclick", (event, d) => {
             ...
             // Update circle style for pinned state
             d3.select(event.currentTarget as Element).select('circle')
                 .transition().duration(150)
                 .attr("stroke-width", d.pinned ? 3 : (d.id === selectedNodeId ? 2.5 : 1.5))
                 .attr("stroke-dasharray", d.pinned ? "5,3" : "none")
                 .attr("stroke", d.pinned ? getNodeColor(d.group) : (d.id === selectedNodeId ? d3.color(getNodeColor(d.group))?.brighter(0.8).formatHex() : d3.color(getNodeColor(d.group))?.darker(0.8).formatHex() ?? "#888"));
        });
