...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const setupSimulation = useCallback((nodes: CustomNode[], links: CustomLink[]) => {
    ...
    const sim = d3
      ...
      .on('tick', () => {
        ...
        nodeElements.attr('transform', d => {
          ...
          const margin = 20;
          const x = Math.max(margin, Math.min(svgWidth - margin, d.x || 0));
          const y = Math.max(margin, Math.min(svgHeight - margin, d.y || 0));
    ...
    return sim;
  }, [svgWidth, svgHeight, simulationObject]);
  ...
  const renderTooltip = useCallback(() => {
    ...
    let allDefinitions = nodeData?.definitions || definitions;
    
    // Safely extract definition text
    ...
    if (allDefinitions && allDefinitions.length > 0) {
      ...
      if (typeof firstDef === 'string') {
        ...
        const defObj = firstDef as any;
        definitionText = defObj.text || defObj.definition_text || 'No definition text';
      }
    }
