...
const WordExplorer: React.FC = () => {
  ...
  const handleNodeClick = useCallback(async (identifier: string) => {
    ...
    try {
      ...
      if (wordData) {
        ...
        try {
          console.log(`[handleNodeClick] Fetching network for main word: ${wordData.lemma}`); // Revert: Use lemma
          fetchWordNetworkData(wordData.lemma, depth, breadth) // <-- Revert: Use wordData.lemma
  ...
  const handleBack = useCallback(() => {
    if (currentHistoryIndex > 0) {
      ...
      Promise.all([
        ...
        fetchWordNetworkData(wordId, depth, breadth)
      ])
  ...
  const handleNetworkChange = useCallback((newDepth: number, newBreadth: number) => {
    ...
    // Use the ID from the current wordData if available
    if (wordData?.id) {
      const networkIdentifier = `id:${wordData.id}`;
      setIsLoading(true);
      fetchWordNetworkData(networkIdentifier, newDepth, newBreadth)
        .then(networkData => {
          setWordNetwork(networkData);
        })
        .catch(error => {
      ...
      // Fallback to selectedNode (lemma) if wordData or ID is not available
      console.warn("[handleNetworkChange] wordData.id not available, falling back to selectedNode lemma for network fetch.")
      setIsLoading(true);
  ...
  }, [wordData, selectedNode, fetchWordNetworkData]); // Add wordData dependency
