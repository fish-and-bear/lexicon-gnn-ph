  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      testApiConnection().then(connected => {
        ...
        if (connected) {
          // Define fetchInitialData inside the .then() to ensure access to scope
    ...
    // Cleanup timeout and set isMounted to false on unmount
    return () => {
      isMounted = false;
      clearTimeout(timer);
    };
  // Restore correct dependencies - include state values used conditionally (randomWordCache.length)
  // and callbacks used directly (fetchRandomWordsForCache, fetchStatistics, getPartsOfSpeech)
  }, [fetchRandomWordsForCache, fetchStatistics, getPartsOfSpeech, randomWordCache.length]);
