...
const WordExplorer: React.FC = () => {
  ...
  const [suggestions, setSuggestions] = useState<SearchWordResult[]>([]);
  const [isLoadingSuggestions, setIsLoadingSuggestions] = useState<boolean>(false);
  const [isSuggestionsOpen, setIsSuggestionsOpen] = useState<boolean>(false);
  const [networkWord, setNetworkWord] = useState<WordInfo | null>(null);
  ...
  const isOverallLoading = isLoadingNetwork || isLoadingDetails || isLoadingEtymology;
  ...
  return (
    <div className={`word-explorer ${theme} ${isLoading ? 'loading' : ''}`}>
      ...
      <main ref={mainRef} className={`main ${theme}`}>
        <div className="graph-container" style={{ width: `${leftPanelWidth}px`, flexBasis: `${leftPanelWidth}px`, flexShrink: 0 }}>
          <div className="graph-content">
            {isLoadingNetwork && (
               <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%' }}>
                 <CircularProgress />
                 <Typography variant="caption" sx={{ mt: 1 }}>Loading network...</Typography>
               </Box>
            )}
            {networkError && !isLoadingNetwork && (
               <ErrorDisplay message={networkError} />
            )}
        ...
        <div className="details-container" style={{ width: `calc(100% - ${leftPanelWidth}px - 8px)` }}>
          {isLoadingDetails && (
              <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', p: 3 }}>
                 <CircularProgress />
                 <Typography variant="caption" sx={{ mt: 1 }}>Loading details...</Typography>
               </Box>
          )}
          ...
          {!isLoadingDetails && !detailsError && !selectedWordInfo && mainWord && (
             <Box sx={{ p: 3, textAlign: 'center' }}>
               <Typography color="text.secondary">Select a node or search to view details.</Typography>
             </Box>
          )}
           {!mainWord && !isLoadingDetails && (
             <Box sx={{ p: 3, textAlign: 'center' }}>
                 <Typography color="text.secondary">Details will appear here.</Typography>
             </Box>
          )}
        </div>
      </main>
