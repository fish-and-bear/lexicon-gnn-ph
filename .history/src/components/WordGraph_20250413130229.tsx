...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  useEffect(() => {
    ...
    // Ensure legend is wide enough for toggle text plus toggle control (44px) plus padding
    const toggleRequiredWidth = toggleTextWidth + 60 + (legendPadding * 2); // Increased from 44 to 60
    const legendWidth = Math.max(
      maxCategoryWidth, 
      maxTextWidth + textPadding + 20,
      toggleRequiredWidth
    ) + (legendPadding * 2);
    ...
    // Add background container that's clickable - REMOVE GREY BACKGROUND
    toggleContainer.append("rect")
      .attr("width", legendWidth - (legendPadding * 2))
      .attr("height", legendItemHeight + 6)
      .attr("x", -5)
      .attr("y", -12)
      .attr("rx", 4)
      .attr("fill", "transparent") // Changed from grey background to transparent
      .attr("cursor", "pointer");
    
    // Create a more elegant toggle switch
    // 1. Create pill background
    const toggleTrack = toggleContainer.append("rect")
      ...
      .attr("fill", showDisconnectedNodes ? 
        (theme === "dark" ? "#4c6ef5" : "#3b82f6") : 
        (theme === "dark" ? "#3a3f4b" : "#cbd5e1"))
      .attr("opacity", showDisconnectedNodes ? 1 : 0.8)
    ...
    toggleContainer.append("rect")
      ...
      .attr("y", -7)
      .attr("fill", showDisconnectedNodes ?
        (theme === "dark" ? "url(#toggleGradientOn)" : "url(#toggleGradientOn)") :
        (theme === "dark" ? "url(#toggleGradientOff)" : "url(#toggleGradientOff)"))
      .attr("opacity", 0.4);
    
    // Create gradients for the toggle
    const defs = svg.append("defs");
    
    // Gradient for enabled state
    const gradientOn = defs.append("linearGradient")
      .attr("id", "toggleGradientOn")
      .attr("x1", "0%")
      .attr("y1", "0%")
      .attr("x2", "0%")
      .attr("y2", "100%");
    
    gradientOn.append("stop")
      .attr("offset", "0%")
    ...
    // Gradient for disabled state
    const gradientOff = defs.append("linearGradient")
      .attr("id", "toggleGradientOff")
      .attr("x1", "0%")
      .attr("y1", "0%")
      .attr("x2", "0%")
      .attr("y2", "100%");
    
    gradientOff.append("stop")
      .attr("offset", "0%")
      .attr("stop-color", "#ffffff")
    ...
    // 3. Add toggle handle with better styling
    const toggleHandle = toggleContainer.append("circle")
      .attr("r", 7)
      .attr("cx", showDisconnectedNodes ? 26 : 10)
      .attr("cy", 2)
      .attr("fill", theme === "dark" ? "#f8f9fa" : "#ffffff")
      .attr("stroke", theme === "dark" ? "rgba(0, 0, 0, 0.1)" : "rgba(0, 0, 0, 0.1)")
      .attr("stroke-width", 1)
      .attr("filter", "drop-shadow(0px 2px 3px rgba(0, 0, 0, 0.15))");
    
    // 4. Add subtle inner shadow to handle for depth
    toggleContainer.append("circle")
      .attr("r", 3.5)
      .attr("cx", showDisconnectedNodes ? 26 : 10)
      .attr("cy", 2)
      .attr("fill", showDisconnectedNodes ? 
        (theme === "dark" ? "rgba(255,255,255,0.3)" : "rgba(59,130,246,0.2)") : 
        (theme === "dark" ? "rgba(255,255,255,0.1)" : "rgba(0,0,0,0.05)"))
      .attr("class", "toggle-handle-inner");
    
    // 5. Add better label with improved positioning that fits in the legend width
    const labelText = "Show disconnected nodes";
    
    // Create the text element with FULL text first, not truncated
    const labelElement = toggleContainer.append("text")
      .attr("x", 44)
      .attr("y", 0)
      .attr("dy", ".25em")
      .attr("font-size", "11px")
    ...
    // We'll just force the full text to be shown
    // Tooltip will be added either way for clarity
    toggleContainer.append("title").text(labelText);
    
    // Check if label overflows and truncate if needed
    const labelWidth = labelElement.node()?.getBBox().width || 0;
    const availableLabelWidth = legendWidth - 44 - (legendPadding * 2);
    
    if (labelWidth > availableLabelWidth) {
      // Calculate how many characters we can fit
      const approxCharsPerWidth = labelText.length / labelWidth;
      const visibleChars = Math.max(15, Math.floor(approxCharsPerWidth * availableLabelWidth) - 3);
      labelElement.text(labelText.substring(0, visibleChars) + "...");
      
      // Add title for tooltip
      toggleContainer.append("title").text(labelText);
    }
    
    // Add click handler for the toggle with improved transitions
    toggleContainer.on("click", function() {
      const newValue = !showDisconnectedNodes;
      setShowDisconnectedNodes(newValue);
      
      // Update toggle track color with nice transition
      toggleTrack
        .transition().duration(250).ease(d3.easeCubic)
        .attr("fill", newValue ? 
          (theme === "dark" ? "#4c6ef5" : "#3b82f6") : 
          (theme === "dark" ? "#3a3f4b" : "#cbd5e1"))
        .attr("opacity", newValue ? 1 : 0.8);
      
      // Move handle with bouncy transition
      toggleHandle
        .transition()
        .duration(300)
        .ease(d3.easeCubicOut)
        .attr("cx", newValue ? 26 : 10);
      
      // Move inner shadow with the handle
      d3.select(".toggle-handle-inner")
        .transition()
        .duration(300)
        .ease(d3.easeCubicOut)
        .attr("cx", newValue ? 26 : 10)
