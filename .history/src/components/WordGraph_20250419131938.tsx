...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const createDragBehavior = useCallback((simulation: d3.Simulation<CustomNode, CustomLink>) => {
    return d3.drag<SVGGElement, CustomNode>()
      .on("start", (event, d) => {
          ...
          // Clear any pending click action when drag starts
          ...
          d3.selectAll(".link").filter((l: any) => {
      ...
      .on("end", (event, d) => {
          ...
          // Smooth transition for links too  
          d3.selectAll(".link.connected-link")
          ...
          // Add small delay before clearing isDragging to avoid immediate click
          setTimeout(() => {
  ...
  const setupNodeInteractions = useCallback((
      ...
      nodeSelection.on(".click", null)
                  .on(".dblclick", null)
                  .on("mousedown.click", null)
                  .on("mouseup.click", null)
                  .on("mouseenter", null)
                  .on("mouseleave", null);

      // Add a soft pulse animation to indicate interactivity on hover
      nodeSelection.on("mouseenter", function(event, d) {
        if (isDraggingRef.current) return;
        
        // Quick highlight with subtle animation
        d3.select(this)
          .transition("pulse")
          .duration(200)
          ...
          .transition()
          .duration(200)
          .attr("transform", `translate(${d.x ?? 0}, ${d.y ?? 0}) scale(1.0)`);
          
        // Highlight connected links on hover with lighter effect
        d3.selectAll<SVGLineElement, CustomLink>(".link").filter((l: CustomLink) => {
          const sourceId = typeof l.source === 'object' ? (l.source as CustomNode).id : l.source as string;
          const targetId = typeof l.target === 'object' ? (l.target as CustomNode).id : l.target as string;
          return sourceId === d.id || targetId === d.id;
        })
      ...
      nodeSelection.on("mouseleave", function(event, d) {
        ...
        // Reset link highlighting if not the selected node
        if (selectedNodeId !== d.id) {
          d3.selectAll<SVGLineElement, CustomLink>(".link").filter((l: CustomLink) => {
            const sourceId = typeof l.source === 'object' ? (l.source as CustomNode).id : l.source as string;
            const targetId = typeof l.target === 'object' ? (l.target as CustomNode).id : l.target as string;
            // Using a traditional filter check rather than checking d3.select(l)
            return (sourceId === d.id || targetId === d.id);
          })
          .each(function() {
            // Only reset links that aren't already highlighted from other interactions
      ...
      // Simplified click system - single click for focus, with immediate feedback
      nodeSelection.on("click", (event, d) => {
          ...
          if (isDraggingRef.current) {
              console.log("Ignoring click because dragging is active");
              return;
          }
          
          const clickTime = new Date().getTime();
          const timeDiff = clickTime - lastClickTimeRef.current;
          const isSameNode = lastClickedNodeRef.current === d.id;
          
          // Update refs for next click
          lastClickTimeRef.current = clickTime;
          lastClickedNodeRef.current = d.id;
          ...
          if (isSameNode && timeDiff < 300) {
              // Visual feedback for navigation action
              d3.select(event.currentTarget as Element)
                .transition()
                .duration(200)
                .style("opacity", 0.8)
              ...
              if (onNodeClick) {
                  console.log(`Double-clicked word node: ${d.word} - Navigating`);
                  onNodeClick(d.word);
              }
              return;
          }
          
          // Single click - schedule focus action with short delay
          clickTimeoutRef.current = setTimeout(() => {
              clickTimeoutRef.current = null;
              
              // Only proceed if not currently dragging
              if (isDraggingRef.current) {
                  console.log("Canceling click action because dragging started");
                  d3.select(event.currentTarget as Element).classed("active", false);
                  return;
              }
              
              console.log(`Processing single click on: ${d.word}`);
              
              // First apply simple highlight to differentiate click from double-click
              d3.select(event.currentTarget as Element)
              ...
              // Reset all nodes to normal state first with a smoother fade
              d3.selectAll<SVGGElement, CustomNode>(".node")
                .filter(n => n.id !== d.id)
                .classed("selected connected", false)
                .transition("reset_node_opacity")
                .duration(200)
                .style("opacity", 0.1)
                .attr("transform", function(d) { 
                    return `translate(${d.x ?? 0}, ${d.y ?? 0}) scale(1.0)`;
                });

              // Reset all links first
              d3.selectAll<SVGLineElement, CustomLink>(".link")
                .classed("highlighted", false)
                .transition("reset_link_opacity")
                .duration(150)
                .attr("stroke", theme === "dark" ? "#666" : "#ccc")
              ...
              // Animate connected nodes with a staggered reveal
              d3.selectAll<SVGGElement, CustomNode>(".node")
                  .filter(n => connectedIds.has(n.id) && n.id !== d.id)
                  .classed("connected", true)
                  .transition("highlight_connected")
                  .delay((_, i) => i * 30) // Staggered animation for connected nodes
                  .duration(250)
                  .style("opacity", 1)
                  .attr("transform", function(d) { 
                      return `translate(${d.x ?? 0}, ${d.y ?? 0}) scale(1.05)`;
                  })
                  .select("circle")
                      .attr("stroke", n => d3.color(getNodeColor(n.group))?.darker(0.8).formatHex() ?? "#888")
                      .attr("stroke-width", 1.8);

              // Animate connected links with flow effect
              d3.selectAll<SVGLineElement, CustomLink>(connectedLinkElements)
                .classed("highlighted", true)
                .raise()
                .transition("highlight_links")
                .delay((_, i) => i * 15) // Slight stagger for links too
                .duration(250)
              ...
              // Add a subtle hint tooltip for first-time users (shown briefly)
              if (!localStorage.getItem('graph-interaction-hint-shown')) {
                const hintText = document.createElement('div');
                hintText.className = 'graph-hint';
                hintText.textContent = 'Double-click to navigate to this word';
                hintText.style.position = 'absolute';
                hintText.style.left = `${event.clientX + 15}px`;
                hintText.style.top = `${event.clientY - 30}px`;
                hintText.style.background = theme === 'dark' ? 'rgba(30,30,30,0.85)' : 'rgba(255,255,255,0.85)';
                hintText.style.color = theme === 'dark' ? '#eee' : '#333';
                hintText.style.padding = '6px 10px';
                hintText.style.borderRadius = '4px';
                hintText.style.fontSize = '12px';
                hintText.style.pointerEvents = 'none';
                hintText.style.opacity = '0';
                hintText.style.transition = 'opacity 0.3s ease';
                hintText.style.zIndex = '1000';
                hintText.style.boxShadow = theme === 'dark' ? '0 2px 8px rgba(0,0,0,0.4)' : '0 2px 8px rgba(0,0,0,0.15)';
                
                document.body.appendChild(hintText);
                
                // Fade in the hint
                setTimeout(() => { hintText.style.opacity = '1'; }, 10);
                
                // Store that we've shown the hint
                localStorage.setItem('graph-interaction-hint-shown', 'true');
          ...
          }, 220); // Slightly shorter delay for more responsiveness
      });
      
      // Keep the mouseover/mouseout behaviors for tooltips
      nodeSelection
        .on("mouseover", (event, d) => {
            if (isDraggingRef.current) return;
            if (tooltipTimeoutId) clearTimeout(tooltipTimeoutId);

            // Slightly faster tooltip for better responsiveness
            const timeoutId = setTimeout(() => setHoveredNode({ ...d }), 120);
  ...
  }, [selectedNodeId, onNodeClick, getNodeColor, theme, nodeMap, onNodeSelect]);

  // Enhanced tooltip with clearer interaction hints
  ...
  // Add CSS class definitions for hover effects
  useEffect(() => {
    // Add dynamic CSS styles for node hover and interaction effects
  ...
  const getRelationshipTypeLabel = useCallback((type: string): { group: string, label: string } => {
