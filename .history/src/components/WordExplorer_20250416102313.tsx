  ...
  const fetchRandomWordsForCache = useCallback(async (count: number = RANDOM_CACHE_SIZE): Promise<any[]> => {
    ...
    try {
      ...
      if (validResults.length === 0) {
        ...
        // Update the cache while keeping more words in reserve
        setRandomWordCache(prev => [...prev, ...newCache].slice(0, RANDOM_CACHE_SIZE * 4)); // Keep 4x the normal size
        console.log(`Added ${validResults.length} words to random word cache (total: ${Math.min(randomWordCache.length + validResults.length, RANDOM_CACHE_SIZE * 4)})`);
        return [...randomWordCache, ...newCache];
      }
    } catch (error) {
