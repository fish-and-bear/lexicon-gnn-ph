  ...
  const handleSearch = useCallback(async (wordToSearchParam?: string) => {
    ...
    try {
      ...
      if (searchResult && Array.isArray(searchResult.results) && searchResult.results.length > 0) {
        ...
        console.log('[handleSearch] Found result, processing details:', firstResult);
        ...
        if (searchOptions.include_full && 'definitions' in firstResult) {
          ...
          try {
            const fullWordData = await fetchWordDetails(firstResult.id.toString());
      ...
      } else if (searchResult && Array.isArray(searchResult.words) && searchResult.words.length > 0) {
        // Handle old API response format
        const firstResult = searchResult.words[0];
        console.log('[handleSearch] Found result in words array:', firstResult);
