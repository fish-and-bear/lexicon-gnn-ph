...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  // Add a ref to track if we're doing a planned transition
  ...
  const setupSimulation = useCallback((nodes: CustomNode[], links: CustomLink[]) => {
    ...
    const sim = d3
      ...
      .alphaDecay(0.028) // Slightly faster cooling
      .velocityDecay(0.4) // Higher damping for smoother motion
      .force('link', d3.forceLink<CustomNode, CustomLink>(links)
        .id(d => d.id)
        .distance(100)
        .strength(0.7)) // Stronger links for more stability
      .force('charge', d3.forceManyBody()
        .strength(-250)  // Stronger repulsion
        .theta(0.9)      // Performance parameter
        .distanceMax(containerWidth / 1.5)) // Limit long-range effects
      .force('center', d3.forceCenter(0, 0))
      .force('collision', d3.forceCollide().radius(40).strength(0.7)) // Prevent overlap
      .force('x', d3.forceX(0).strength(0.07)) // Gentle force toward center x
      .force('y', d3.forceY(0).strength(0.07)) // Gentle force toward center y
      .on('tick', () => {
        if (!svgRef.current) return;

        // Only update visuals if not dragging or transitioning
        if (!isDraggingRef.current && !isTransitioningRef.current) {
          // Update node positions using the transform attribute
  ...
  const createDragBehavior = useCallback(() => {
    return d3.drag<SVGGElement, CustomNode>()
      .on('start', (event, d) => {
        ...
        // Warm up the simulation while dragging
        if (!event.active && simulationObject) {
          simulationObject.alphaTarget(0.1).restart();
        }
      ...
      .on('drag', (event, d) => {
        ...
        // Update the connected links for the dragged node
        d3.select(svgRef.current)
          .selectAll<SVGLineElement, CustomLink>('.link')
          .filter((link: any) => {
    ...
    node
      .on("click", (event: MouseEvent, d: CustomNode) => {
        ...
        if (d.id !== selectedNodeId) {
          ...
          // Call onNodeClick to change the selected word
  ...
  }, [onNodeClick, selectedNodeId, showConnectedNodes, highlightConnectedLinks]);
  ...
  const updateGraph = useCallback(() => {
    ...
    try {
      ...
      // Center on main word after the graph is created - with smoother transition
      const centerOnMainWord = () => {
        // Only center if not currently dragging
        if (isDraggingRef.current) return;
        
        const mainNode = filteredNodes.find(n => n.id === mainWord);
        if (mainNode && mainNode.x && mainNode.y && zoom) {
          // Calculate distance from center
          const dist = Math.sqrt(mainNode.x * mainNode.x + mainNode.y * mainNode.y);
          if (dist > 30) { // Only recenter if it's drifted significantly
      ...
      // Center after a short delay to let the simulation run
      setTimeout(centerOnMainWord, 500);
      
      // Add grid background for better spatial awareness (optional)
  ...
  }, [filteredNodes, links, theme, getNodeColor, createLinks, createNodes, setupNodeInteractions, setupSimulation, setupSvgDimensions, setupZoom]);
