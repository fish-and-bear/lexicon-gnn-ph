...
// Commenting out missing components to resolve import errors
...
const WordExplorer: React.FC = () => {
  ...
  const fetchWordDetailsData = useCallback(async (wordOrId: string | number) => {
      ...
      try {
          ...
          const details = await fetchWordDetails(String(wordOrId));
  ...
  const handleShowDiagnostic = () => {
    setDiagnostic(prev => ({ ...prev, shown: true }));
  };
  ...
  useEffect(() => {
    ...
    const onMouseMove = (e: MouseEvent) => {
      ...
      const constrainedPosition = Math.max(20, Math.min(80, position));
  ...
  const memoizedWordNetwork = useMemo(() => {
    if (!wordNetwork) {
      return { nodes: [], edges: [] };
    }
    const validNodes: NodeData[] = wordNetwork.nodes
      .filter(node => typeof node.lemma === 'string' && node.lemma.trim() !== '')
      .map(node => ({
        ...node,
        id: typeof node.id === 'number' ? node.id : parseInt(String(node.id), 10) || Date.now() + Math.random(),
        lemma: node.lemma!,
        language_code: node.language_code || 'tl'
      }));

    const nodeMapById = new Map(validNodes.map(node => [node.id, node]));

    const validEdges = (wordNetwork.edges || [])
      .map((edge, index) => {
        const originalSourceId = typeof edge.source === 'object' ? edge.source.id : edge.source;
        const originalTargetId = typeof edge.target === 'object' ? edge.target.id : edge.target;
        
        const sourceNode = validNodes.find(n => String(n.id) === String(originalSourceId) || n.id === originalSourceId);
        const targetNode = validNodes.find(n => String(n.id) === String(originalTargetId) || n.id === originalTargetId);

        if (!sourceNode || !targetNode) {
          console.warn(`Skipping edge: Cannot find source (${originalSourceId}) or target (${originalTargetId}) node.`);
          return null;
        }
  ...
  return (
    <div className={`word-explorer ${theme}`}>
      {/* <Header 
         ...
         canGoForward={currentHistoryIndex < wordHistory.length - 1}
      ...
      <main ref={mainRef} style={{ display: 'flex', flexDirection: 'row', flexGrow: 1, overflow: 'hidden' }}>
        <div className="graph-container" style={{ flexBasis: leftPanelWidth, display: 'flex', flexDirection: 'column' }}>
          <div className="graph-content" style={{ flexGrow: 1, position: 'relative' }}>
            {(isLoadingNetwork || (isLoading && !wordNetwork)) && (
              <div className="loading-message">Loading network... <CircularProgress size={24} /></div>
            )}
            {networkError && !isLoadingNetwork && (
              <div className="error-details">
                  <p>Error loading network: {networkError}</p>
                  <p className="error-suggestion">Try searching for a different word or check the API connection.</p>
              </div>
            )}
            {!isLoadingNetwork && !networkError && wordNetwork && wordNetwork.nodes.length === 0 && (
                <div className="empty-graph">
                    <p>No network data found for "{mainWord || 'selected word'}".</p>
                    <p className="empty-suggestion">This might be a less common word.</p>
                </div>
            )}
            {!isLoadingNetwork && !networkError && wordNetwork && wordNetwork.nodes.length > 0 && (
              <WordGraph
                wordNetwork={memoizedWordNetwork}
                mainWord={mainWord}
                onNodeClick={handleNodeClick}
                initialDepth={depth} 
                initialBreadth={breadth}
                onNetworkChange={() => { console.log('Network change triggered - handled by depth/breadth state'); }}
              />
            )}
          </div>
          <NetworkControls
        ...
        <div ref={resizerRef} className={`resizer ${isResizing ? 'active' : ''}`}></div>

        <div className="details-container" style={{ flexBasis: `calc(100% - ${leftPanelWidth} - 8px)`, overflowY: 'auto' }}>
          {(isLoadingDetails || (isLoading && !selectedWordInfo)) && (
            <div className="loading-details">Loading details...<CircularProgress size={24} /></div>
          )}
          {detailsError && !isLoadingDetails && (
            <div className="error-details">
              <p>Error loading details: {detailsError}</p>
              <p className="error-suggestion">Please try again or search for another word.</p>
            </div>
          )}
          {!isLoadingDetails && !detailsError && !selectedWordInfo && !isLoading && (
             <div className="no-word-selected">
                <p>Select a word or search to view details.</p>
                <p className="empty-tip">Click on a node in the graph or use the search bar above.</p>
            </div>
          )}
          {!isLoadingDetails && !detailsError && selectedWordInfo && (
            <WordDetails
      ...
      <button 
        ...
        style={{position: 'fixed', bottom: '60px', right: '20px', zIndex: 1001, padding: '8px 12px', fontSize: '12px'}}
      ...
      {diagnostic.shown && (
        <div className="diagnostic-container">
          <div className="diagnostic-header">
            ...
            <button onClick={handleCloseDiagnostic} className="close-button">&times;</button>
          </div>
