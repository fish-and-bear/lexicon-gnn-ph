...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  useEffect(() => {
    ...
    const legendContainer = svg.append("g")
      .attr("class", "legend")
      .attr("transform", `translate(${width - 190}, 20)`); // Reduced margin to match slider spacing

    // Define refined legend properties
    const legendPadding = 10; // Further reduced padding
    const legendItemHeight = 22; // Slightly smaller item height
    const dotRadius = 5; // Slightly smaller dots
    const textPadding = 14; // Further reduced padding between dot and text
    const categorySpacing = 10; // Keep spacing between categories
    const maxLabelWidth = 100; // Smaller max width for labels before truncation
    ...
    const toggleRequiredWidth = toggleTextWidth + 50 + (legendPadding * 2); // Reduced extra space
    const legendWidth = Math.max(
      maxCategoryWidth + 10, // Just a bit wider than category headers 
      maxTextWidth + textPadding + 10, // Less extra space
      toggleRequiredWidth
    ) + (legendPadding * 2);
