...
const WordExplorer: React.FC = () => {
  ...
  const [detailsPanelWidth, setDetailsPanelWidth] = useState<number | null>(null); // State for panel width
  ...
  // Effect to observe panel width
  useEffect(() => {
    const targetElement = detailsContainerRef.current;
    ...
    const resizeObserver = new ResizeObserver(entries => {
      for (const entry of entries) {
        if (entry.target === targetElement) {
          ...
          console.log("[ResizeObserver] Detected width:", newWidth); // <-- ADD LOG HERE
    ...
    // Set initial width
    setDetailsPanelWidth(targetElement.offsetWidth);
  ...
  return (
    <div className={`word-explorer ${theme} ${isLoading ? 'loading' : ''}`}>
      ...
      <main>
        {isWideLayout ? (
          <PanelGroup direction="horizontal" autoSaveId="wordExplorerLayout">
            <Panel defaultSize={60} minSize={30} order={1}>
              {/* Graph Container Content */}
              <div className="graph-container" style={{ height: '100%', borderRight: 'none' }}>
            ...
            <Panel defaultSize={40} minSize={25} order={2}>
              ...
              <div ref={detailsContainerRef} className="details-container" style={{ height: '100%', width: '100%', borderLeft: 'none' }}>
                ...
                {!isLoading && selectedWordInfo && (
                  <WordDetails 
                    ...
                    containerWidth={detailsPanelWidth} // Pass measured width
                  />
                )}
                {!isLoading && !selectedWordInfo && (
