...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const ticked = useCallback(() => {
      ...
      const linkSelection = svg.selectAll<SVGPathElement, CustomLink>(".link");
      ...
      linkSelection.attr("d", d => {
          ...
          if (dist === 0) return null; // Avoid division by zero for self-loops or coincident nodes

          // Calculate the point on the target node's circumference
          const targetRadius = getNodeRadius(targetNode);
          const arrowheadOffset = 8; // Estimated space for arrowhead + padding
          const endOffset = targetRadius + arrowheadOffset;

          // Shorten the effective line segment by the offset
          const effectiveDist = Math.max(0, dist - endOffset);
          const ratio = effectiveDist / dist;
          const effectiveTargetX = sourceX + dx * ratio;
          const effectiveTargetY = sourceY + dy * ratio;

          // Adjust curvature based on distance
          const dr = dist * 1.5;
          // Use the adjusted target point for the curve end
          return `M${sourceX},${sourceY} A${dr},${dr} 0 0,1 ${effectiveTargetX},${effectiveTargetY}`;
      });
  }, [getNodeRadius]);
  ...
  const setupSimulation = useCallback((nodes: CustomNode[], links: CustomLink[]) => {
      simulationRef.current = d3.forceSimulation<CustomNode>(nodes)
        ...
        .force("link", d3.forceLink<CustomNode, CustomLink>()
          ...
          .distance(100)
          .strength(0.4))
        .force("charge", d3.forceManyBody<CustomNode>().strength(-220).distanceMax(280))
        .force("collide", d3.forceCollide<CustomNode>().radius(d => getNodeRadius(d) + 5).strength(0.9))
  ...
  const createLinks = useCallback((g: d3.Selection<SVGGElement, unknown, null, undefined>, linksData: CustomLink[]) => {
      ...
      return g.append("g")
        ...
        .attr("fill", "none")
        .attr("stroke", d => {
            const targetId = typeof d.target === 'object' && d.target !== null ? (d.target as CustomNode).id : d.target as string;
            const targetNode = nodeMap.get(targetId);
            return targetNode ? getNodeColor(targetNode.group) : '#999';
         })
        .attr("stroke-opacity", d => (d.relationship === 'associated') ? 0.3 : 0.6)
        .attr("stroke-width", d => {
            const rel = d.relationship.toLowerCase();
            if (rel === 'synonym' || rel === 'antonym' || rel === 'root') return 2.5;
            if (rel === 'associated') return 1;
            return 1.5;
        })
        .attr("marker-end", d => { // Add arrowheads for directed relationships
            const rel = d.relationship.toLowerCase();
            // Add more types here if needed
            if (rel === 'derived' || rel === 'derived_from' || rel === 'root_of' || rel === 'etymology' || rel === 'taxonomic' || rel === 'meronym' || rel === 'holonym' || rel === 'component_of') {
               return "url(#arrowhead)";
            }
            return null;
        });
  }, [getNodeColor, filteredNodes]);
      ...
      nodeSelection: d3.Selection<d3.BaseType, CustomNode, SVGGElement, unknown>
       ...
       nodeSelection
        .on("click", (event, d) => {
          ...
          const connectedLinkElements: SVGPathElement[] = [];
           d3.selectAll<SVGPathElement, CustomLink>(".link").filter(l => {
          ...
          d3.selectAll(".node").classed("selected connected", false)
            .transition().duration(200).style("opacity", 0.15);
          d3.selectAll(".link").classed("highlighted", false)
            .transition().duration(200).attr("stroke-opacity", 0.05);

          const targetNode = d3.select(event.currentTarget as Element);
          targetNode.classed("selected", true)
            .transition().duration(200)
            .style("opacity", 1)
            .select("circle").attr("stroke-width", 2.5);
          ...
          d3.selectAll<SVGGElement, CustomNode>(".node")
           ...
           d3.selectAll<SVGPathElement, CustomLink>(connectedLinkElements)
        ...
        .on("mouseover", (event, d) => {
            ...
            nodeElement.select("circle")
               ...
               .attr("stroke-width", 2.5)
               .attr("stroke", theme === "dark" ? "rgba(255,255,255,0.8)" : "rgba(0,0,0,0.8)"); // Brighter outline on hover
            nodeElement.raise();
             d3.selectAll<SVGGElement, CustomNode>(".node:not(:hover)")
                 .style("opacity", (d: CustomNode) => d.id === selectedNodeId || selectedNodeId === null ? 0.7 : 0.3 );
             d3.selectAll<SVGPathElement, CustomLink>(".link:not(.highlighted)").attr("stroke-opacity", 0.2);
        ...
        .on("mouseout", (event, d_unknown) => {
            ...
            // Revert hover effect for the circle
             nodeElement.select<SVGCircleElement>("circle")
                ...
                .attr("stroke-width", (n: CustomNode) => n.id === selectedNodeId ? 2.5 : 1) // Now n should be CustomNode
                .attr("stroke", (n: CustomNode) => d3.color(getNodeColor(n.group))?.darker(0.6).formatHex() ?? (theme === "dark" ? "rgba(255,255,255,0.1)" : "rgba(0,0,0,0.1)")); // Back to darker shade of fill

             // Restore opacity for all nodes and links
             d3.selectAll<SVGGElement, CustomNode>(".node").style("opacity", 1);
             d3.selectAll<SVGPathElement, CustomLink>(".link")
                 .attr("stroke-opacity", (l: CustomLink) => (l.relationship === 'associated') ? 0.3 : 0.6);

             if (selectedNodeId) {
                 const selectedNode = d3.select(`.node [id='${selectedNodeId}']`);
                 if (!selectedNode.empty()) {
                    // Maybe re-highlight connected links/nodes if needed, or rely on click logic
                 }
             }
        })
        .on("dblclick", (event, d) => {
