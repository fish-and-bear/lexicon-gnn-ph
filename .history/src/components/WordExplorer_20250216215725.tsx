...
const WordExplorer: React.FC = () => {
  ...
  const handleSearch = useCallback(async (searchWord?: string) => {
    ...
    try {
      ...
      if (!detailsData.definitions || detailsData.definitions.length === 0) {
      ...
      setInputValue(detailsData.lemma); // Update input value with the searched word
    } catch (error) {
  ...
  const renderDefinitions = useCallback((wordInfo: WordInfo) => {
    ...
    return wordInfo.definitions.map((definition, index) => (
          ...
          <div className="details-content">
            {isLoading ? (
              ...
              <div className="word-details">
                ...
                {selectedWordInfo.etymology?.text &&
                  selectedWordInfo.etymology.text.length > 0 && (
                    <p>
                      <strong>Etymology:</strong>{" "}
                      {selectedWordInfo.etymology.text}
                    </p>
                  )}
                {selectedWordInfo.languages &&
                  selectedWordInfo.languages.length > 0 && (
                    <p>
                      <strong>Language Codes:</strong>{" "}
                      {selectedWordInfo.languages.join(", ")}
                    </p>
                  )}
                {renderDefinitions(selectedWordInfo)}
                {renderArraySection(
                  "Synonyms",
                  selectedWordInfo.relationships?.synonyms
                )}
                {renderArraySection(
                  "Antonyms",
                  selectedWordInfo.relationships?.antonyms
                )}
                {renderArraySection(
                  "Associated Words",
                  selectedWordInfo.relationships?.associatedWords
                )}
                {renderArraySection(
                  "Derivatives",
                  selectedWordInfo.relationships?.derivatives
                )}
                {selectedWordInfo.relationships?.rootWord && (
                  <p>
