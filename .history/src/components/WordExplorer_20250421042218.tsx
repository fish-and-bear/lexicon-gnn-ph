...
const WordExplorer: React.FC = () => {
  ...
  const handleSearch = useCallback(async (searchTerm: string | WordSuggestion | null) => {
    ...
    // Revert: Use only the lemma string for search
    const lemmaToSearch = typeof searchTerm === 'string' ? searchTerm.trim() : searchTerm.lemma;
    if (!lemmaToSearch) return;

    setInputValue(lemmaToSearch);
    setSuggestions([]);
    setAutocompleteOpen(false);
    // Revert: Pass only the lemma string to handleNodeClick
    await handleNodeClickRef.current(lemmaToSearch);
  }, [handleNodeClickRef]);
