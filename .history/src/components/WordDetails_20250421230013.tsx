...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({
  ...
  // Add new refs and state for mobile optimizations
  ...
  useEffect(() => {
    const checkViewport = () => {
      ...
      setIsTabletView(window.innerWidth >= 600 && window.innerWidth < 960);
    };
  ...
  useEffect(() => {
    if (!isMobileView || !detailsContainerRef.current) return;
  ...
  const renderHeader = () => {
    ...
    const ipaPronunciation = wordInfo.pronunciations?.find(p => p.type?.toLowerCase() === 'ipa');
    ...
    return (
      <Box
        sx={{
          p: isMobileView ? theme.spacing(2) : theme.spacing(3),
      ...
      >
        ...
        >
              ...
              {hasAudio && (
                <IconButton
                  ...
                  sx={{ 
                    ...
                    p: isMobileView ? 0.75 : 1,
                    '&:hover': {
                      bgcolor: alpha(headerTextColor, 0.2),
                    },
                    fontSize: isMobileView ? '0.9rem' : '1rem',
                    minWidth: isMobileView ? '36px' : '40px',
                    minHeight: isMobileView ? '36px' : '40px'
                  }}
                  size={isMobileView ? "small" : "medium"}
                  aria-label={isAudioPlaying ? "Stop pronunciation" : "Play pronunciation"}
                >
          ...
          {/* Second column with additional metadata */}
          <Stack 
            ...
            sx={{ 
              ...
              gap: theme.spacing(isMobileView ? 1 : 0.5),
              mt: isMobileView ? 1 : 0
            }}
          >
        ...
        {tags.length > 0 && (
          <Stack 
            ...
            flexWrap="wrap" 
            sx={{ 
              mt: theme.spacing(isMobileView ? 1.5 : 2),
              mx: theme.spacing(-0.5) // Negative margin to offset chip padding
            }}
          >
            {tags.map((tag) => (
              <Chip
  ...
  const renderDefinitionsTab = () => {
    ...
    return (
      <Box sx={{ 
        pt: theme.spacing(isMobileView ? 0.5 : 1), 
        width: '100%', 
        maxWidth: '100%', 
        overflow: 'hidden', 
        px: isMobileView ? theme.spacing(1) : theme.spacing(2) 
      }}>
        {Object.entries(definitionsByPosThenSource).map(([posName, defsBySource]) => (
          <Box key={posName} sx={{ 
            ...
            {/* Part of Speech Header - adjusted for mobile */}
            <Typography 
            ...
            {/* Iterate through definitions with mobile optimizations */}
            {Object.entries(defsBySource).map(([sourceName, defs]) => (
              <Box key={sourceName} sx={{ mb: theme.spacing(2), width: '100%', maxWidth: '100%' }}> 
                <List disablePadding>
                  {defs.map((def: Definition, index: number) => {
                    ...
                    <ListItem 
                      ...
                      sx={{ 
                        ...
                        gap: isMobileView ? 0.25 : 0.5, 
                        py: theme.spacing(isMobileView ? 1 : 1.5), 
                        pl: 0, 
                        position: 'relative',
                        pb: isLastDefinitionForSource && sourceName !== 'Unknown Source' 
                          ? theme.spacing(isMobileView ? 2 : 3) 
                          : theme.spacing(isMobileView ? 1 : 1.5),
                      }}
                    >
                      {/* Definition text with mobile optimization */}
                      <ListItemText
                        primaryTypographyProps={{ 
                          variant: isMobileView ? 'body2' : 'body1', 
                          fontWeight: 500,
                          fontSize: isMobileView ? '0.875rem' : 'inherit',
                          lineHeight: isMobileView ? 1.5 : 1.6
                        }}
                        primary={def.text}
                      />
                      
                      {/* Examples with responsive styling */}
                      {def.examples && def.examples.length > 0 && (
                        <Box sx={{ 
                          pl: theme.spacing(isMobileView ? 1.5 : 2), 
                          mb: theme.spacing(isMobileView ? 0.5 : 1) 
                        }}>
                          {def.examples.map((example, exIndex) => (
                            <Typography 
                              ...
                              sx={{ 
                                ...
                                position: 'relative',
                                pl: theme.spacing(isMobileView ? 2 : 3),
                                fontSize: isMobileView ? '0.75rem' : 'inherit',
                        ...
                        >
                          {def.tags.map((tag, tagIndex) => (
                            <Chip
                              ...
                              size="small"
                              sx={{
                                fontSize: isMobileView ? '0.65rem' : '0.7rem',
                                height: 'auto',
                                padding: theme.spacing(0.25, 0),
                                bgcolor: alpha(theme.palette.primary.main, 0.1),
                                color: theme.palette.primary.main,
                                '& .MuiChip-label': { 
                                  px: isMobileView ? 0.5 : 0.75, 
                                  py: 0.2,
                                  whiteSpace: 'normal'
                                },
                                margin: theme.spacing(0.25),
                                minHeight: isMobileView ? '22px' : '24px'
                              }}
                            />
                          ))}
                        </Stack>
  ...
  return (
    <Paper 
      ...
      sx={{ 
        height: '100%', 
        overflow: 'auto',
        // Optional touch scrolling improvement for iOS
        WebkitOverflowScrolling: 'touch'
      }}
    >
      {/* Header should shrink-wrap its content */}
      {renderHeader()}
      
      {/* Content area (Tabs + Panels) should take remaining space and scroll */}
      <Box sx={{ 
          flexGrow: 1, // Allow this Box to grow
          display: 'flex', 
          flexDirection: isWideScreen ? 'row' : 'column', // Adapt flex direction
          overflow: 'hidden', // Prevent this Box from scrolling, children will scroll
          width: '100%'
      }}>
        {/* Tabs container */}
        <Box sx={{ 
            minWidth: isWideScreen ? 160 : 'auto',
            maxWidth: isWideScreen ? 160 : 'auto',
        ...
        {/* Tab Panel Content Area - This needs to scroll */}
        <Box sx={{ 
            flexGrow: 1, 
            overflowY: 'auto', // <<< Ensure this allows vertical scrolling
            position: 'relative', 
            bgcolor: isDarkMode ? 'var(--card-bg-color-elevated)' : 'background.default',
            width: isWideScreen ? 'calc(100% - 160px)' : '100%',
            maxWidth: '100%'
        }}>
           <Box sx={{ px: theme.spacing(3), py: theme.spacing(2), width: '100%', maxWidth: '100%' }}>
              {/* ... Tab panel rendering logic ... */}
