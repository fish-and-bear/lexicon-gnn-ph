...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  // Ref to store previous node positions { id: { x, y, vx, vy } }
  const previousNodePositions = useRef<Map<string, { x: number; y: number; vx?: number; vy?: number }>>(new Map());
  ...
  useEffect(() => {
    // --- Guard Clause: Exit if essential data or SVG ref is missing ---
    if (!svgRef.current || !wordNetwork || !mainWord || baseNodes.length === 0) {
      // Clear SVG content if ref exists
      if (svgRef.current) {
          d3.select(svgRef.current).selectAll("*").remove();
      }
      // Stop simulation and clear stored positions
    ...
    // --- Valid SVG Element Guaranteed Beyond This Point ---
    const svgElement: SVGSVGElement = svgRef.current; // Assert non-null based on guard clause
    const svg = d3.select(svgElement); // Create D3 selection from the guaranteed element
    ...
    try {
      ...
      console.log(`[GRAPH] Filtered Nodes: ${filteredNodes.length}, Filtered Links: ${filteredLinks.length}`);

      // --- Capture Previous Positions & Stop Simulation ---
      if (simulationRef.current) {
        ...
        previousNodePositions.current = posMap;
        simulationRef.current.stop();
      }

      // --- Prepare SVG and Setup Basics ---
      svg.selectAll("*").remove(); // Use the non-null svg selection
      const { width, height } = setupSvgDimensions(svg); // Pass non-null selection
      const g = svg.append("g")
        .attr("class", "graph-content")
        .attr("data-filter-key", filteredRelationships.join(','));
      const zoom = setupZoom(svg, g); // Pass non-null selection
      zoomRef.current = zoom;

      // --- Initialize Node Positions ---
      const nodesForSimulation = filteredNodes.map(newNode => {
        const oldPos = previousNodePositions.current.get(newNode.id);
        if (oldPos) {
          return { ...newNode, x: oldPos.x, y: oldPos.y, vx: oldPos.vx || 0, vy: oldPos.vy || 0 };
        } else {
          return { ...newNode, vx: 0, vy: 0 };
        }
      });

      // --- Setup and Start Simulation ---
      const currentSim = setupSimulation(nodesForSimulation, filteredLinks, width, height);
      simulationRef.current = currentSim;

      // --- Create SVG Elements ---
      createLinks(g, filteredLinks);
      const nodeElements = createNodes(g, nodesForSimulation, currentSim);
      setupNodeInteractions(nodeElements);

      // --- Initial Simulation Adjustments ---
      if (currentSim) {
        const mainNodeData = nodesForSimulation.find(n => n.id === mainWord);
        if (mainNodeData) {
          mainNodeData.fx = 0;
          mainNodeData.fy = 0;
          setTimeout(() => {
            if (simulationRef.current) {
              ...
              const currentMainNode = simNodes.find(n => n.id === mainWord);
              if (currentMainNode) { currentMainNode.fx = null; currentMainNode.fy = null; }
            }
          }, 1500);
        }
      ...
      // --- Center View and Finalize ---
      const centerTimeout = setTimeout(() => {
        if (svgRef.current) centerOnMainWord(svg, nodesForSimulation); // Pass non-null selection
      }, 800);

      // Draw the legend (assuming drawLegend is defined)
      ...
      return () => {
        if (simulationRef.current) {
          const finalNodes = simulationRef.current.nodes();
          const finalPosMap = new Map<string, { x: number; y: number; vx?: number; vy?: number }>();
          finalNodes.forEach(n => { if (n.id && n.x !== undefined && n.y !== undefined) { finalPosMap.set(n.id, { x: n.x, y: n.y, vx: n.vx, vy: n.vy }); } });
          previousNodePositions.current = finalPosMap;
          simulationRef.current.stop();
        }
        clearTimeout(centerTimeout);
        if (svgRef.current) {
          // Use the svg selection from the outer scope
...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  useEffect(() => {
    ...
    if (!svgRef.current || !wordNetwork || !mainWord || baseNodes.length === 0) {
      ...
      if (simulationRef.current) {
        simulationRef.current.stop();
        previousNodePositions.current.clear(); // Clear positions if we exit early
      }
      setError(null);
      setIsLoading(false);
    ...
    // *** At this point, svgRef.current is guaranteed non-null ***
    ...
    try {
      ...
      // *** Pass the guaranteed non-null svg selection to helper functions ***
      const { width, height } = setupSvgDimensions(svg);
      const g = svg.append("g")
        .attr("class", "graph-content")
        .attr("data-filter-key", filteredRelationships.join(','));
      const zoom = setupZoom(svg, g); // Pass the correct type
      zoomRef.current = zoom;
      ...
      // *** Pass the guaranteed non-null svg selection to helper functions ***
      const centerTimeout = setTimeout(() => {
        if (svgRef.current) centerOnMainWord(svg, nodesForSimulation);
      }, 800);
      ...
      return () => {
        ...
        clearTimeout(centerTimeout);
        // *** Use the guaranteed non-null svg selection in cleanup ***
        if (svgRef.current) {
          svg.selectAll(".node").on("click", null).on("dblclick", null).on("mouseover", null).on("mouseout", null);
        }
      };
...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  useEffect(() => {
    try {
      ...
      // --- Guard Clause: Exit if essential data is missing ---
      if (!svgRef.current || !wordNetwork || !mainWord || baseNodes.length === 0) {
        svg.selectAll("*").remove(); // Clear SVG content
        if (simulationRef.current) {
          simulationRef.current.stop(); // Stop any running simulation
        }
        // Reset state related to rendering
        setError(null);
        setIsLoading(false);
        previousNodePositions.current.clear(); // Clear stored positions
        return; // Exit the effect
      }
      ...
      // --- Capture Previous Positions & Stop Simulation ---
      // Capture positions *before* stopping the simulation
      if (simulationRef.current) {
        const currentNodes = simulationRef.current.nodes();
        const posMap = new Map<string, { x: number; y: number; vx?: number; vy?: number }>();
        currentNodes.forEach(n => {
        ...
        previousNodePositions.current = posMap; // Update the ref
        simulationRef.current.stop(); // Stop the old simulation
      }
      ...
      const g = svg.append("g")
        .attr("class", "graph-content")
        .attr("data-filter-key", filteredRelationships.join(',')); // For debugging filters
      const zoom = setupZoom(svg, g);
      zoomRef.current = zoom; // Store zoom behavior reference

      // --- Initialize Node Positions ---
      // Create a new array for simulation, initializing with previous positions
      const nodesForSimulation = filteredNodes.map(newNode => {
        const oldPos = previousNodePositions.current.get(newNode.id);
        if (oldPos) {
          // If old position exists, use it
          return {
            ...newNode,
            x: oldPos.x,
            y: oldPos.y,
            vx: oldPos.vx || 0, // Carry over velocity
            vy: oldPos.vy || 0
          };
        } else {
          // If it's a new node, let the simulation place it (no initial x/y)
          // Set initial velocity to 0 for new nodes
          return { ...newNode, vx: 0, vy: 0 };
        }
      });
      ...
      // --- Setup and Start Simulation ---
      const currentSim = setupSimulation(nodesForSimulation, filteredLinks, width, height);
      simulationRef.current = currentSim; // Store simulation reference

      // --- Create SVG Elements ---
      createLinks(g, filteredLinks);
      const nodeElements = createNodes(g, nodesForSimulation, currentSim);
      setupNodeInteractions(nodeElements); // Attach interactions

      // --- Initial Simulation Adjustments ---
      if (currentSim) {
        // Gently pin the main node towards the center initially, then release
        const mainNodeData = nodesForSimulation.find(n => n.id === mainWord);
        if (mainNodeData) {
          mainNodeData.fx = 0;
          mainNodeData.fy = 0;
