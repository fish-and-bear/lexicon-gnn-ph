  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!svgRef.current || !wordNetwork || !mainWord) {
        // If no network or main word, clear SVG and stop simulation
      ...
      // Initial Setup (if graph content group doesn't exist)
      if (isInitialRender || !g.node() || !simulation) {
        ...
        console.log("[GRAPH] Updating existing graph.");
      ...
      nodeUpdate.transition("update").duration(300)
        .style("opacity", 1); // Fade in/update opacity

      // Update existing node circles (color/radius might change)
      nodeUpdate.select<SVGCircleElement>("circle")
        .transition("update_circle").duration(300)
        .attr("r", (d: CustomNode) => getNodeRadius(d))
        .attr("fill", (d: CustomNode) => getNodeColor(d.group))
        .attr("stroke", (d: CustomNode) => d3.color(getNodeColor(d.group))?.darker(0.8).formatHex() ?? "#888");
      ...
      labelUpdate
        ...
        .transition("update").duration(300)
          .style("opacity", 1); // Fade in/update opacity
        

      // Re-setup interactions for the updated node selection
      ...
      if (simulation) {
        ...
        if (mainNodeData) {
          mainNodeData.fx = 0; // Pin new main word to center initially
          mainNodeData.fy = 0;
        }
        simulation.alpha(0.5).restart(); // Restart with moderate energy
      }

      // Center view on the main word
      centerOnMainWord(svg, filteredNodes);

      // Draw legend (assuming this doesn't need complex updates)
      // Remove previous legend if any
      svg.select("g.legend").remove();
      const legendContainer = svg.append("g")
          .attr("class", "legend")
          .attr("transform", `translate(${width - 240}, 20)`);
      // ... (rest of the legend drawing logic) ...
      // Note: Legend drawing code might need adjustments if complex

      setIsLoading(false);

      // Cleanup function remains mostly the same
      return () => {
        // Stop simulation on cleanup
        // simulationRef.current?.stop(); // Don't stop simulation if component might re-render with same instance
        
        // No need to detach all handlers here if graph elements might persist
        // D3's exit selection should handle removing handlers on removed elements
      };
    } catch (err) {
    ...
    // Dependencies that trigger graph update/rebuild
    wordNetwork, // Major data source change
    mainWord,    // Change in focus node
    theme,       // Affects colors
    filteredNodes, // Change in nodes to display (due to filters/data)
    filteredLinks, // Change in links to display (derived from nodes)
    ...
    centerOnMainWord,
    setupSvgDimensions
    // Note: Filter state variables (depth, breadth, filteredRelationships, etc.)
    // trigger updates indirectly via their effect on filteredNodes/filteredLinks.
    // Explicit update triggers (forceUpdate, filterUpdateKey) also cause rebuilds indirectly.
  ]);
