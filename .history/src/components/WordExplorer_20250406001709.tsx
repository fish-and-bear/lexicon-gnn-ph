  ...
  const handleSearch = useCallback(async (wordToSearchParam?: string) => {
    ...
    try {
      ...
      if (wordData && wordData.lemma) {
        ...
        // Create a local constant to safely capture the non-null values
        const { lemma, id } = wordData;
        
        setSelectedWordInfo(wordData);
        setMainWord(lemma);
        setInputValue(lemma);

        setWordHistory(prev => [...prev.slice(0, currentHistoryIndex + 1), lemma]);
        setCurrentHistoryIndex(prev => prev + 1);
        setShowSuggestions(false);

        await Promise.all([
          fetchWordNetworkData(lemma, depth),
          fetchEtymologyTree(id)
        ]);
      } else {
