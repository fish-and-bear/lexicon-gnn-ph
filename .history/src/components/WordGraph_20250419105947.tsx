...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  useEffect(() => {
    // --- START OF TRY BLOCK ---
    try {
      const isInitialRender = !svgRef.current?.querySelector('g.graph-content');
      console.log(`[GRAPH EFFECT] Running. Initial Render: ${isInitialRender}. Main Word: ${mainWord}`);

      if (!svgRef.current || !wordNetwork || !mainWord) {
        console.log("[GRAPH EFFECT] Aborting: Missing SVG ref, wordNetwork, or mainWord.");
      ...
      // Initial Setup (if graph content group doesn't exist or simulation is null)
      if (isInitialRender || !g.node() || !simulation) {
        console.log("[GRAPH] Initial render or simulation missing, performing full setup.");
        svg.selectAll("*").remove(); // Clear for full setup
        ...
        // Setup simulation
        simulation = setupSimulation(filteredNodes, filteredLinks, width, height);
        simulationRef.current = simulation;

        // Create persistent groups for links, nodes, labels
        g.append("g").attr("class", "links");
        g.append("g").attr("class", "nodes");
        g.append("g").attr("class", "labels");

        // Initial centering
        centerOnMainWord(svg, filteredNodes);

      } else {
        console.log("[GRAPH] Updating existing graph data and simulation.");
        // Update simulation data if it exists
        simulation.nodes(filteredNodes);
        simulation.force<d3.ForceLink<CustomNode, CustomLink>>("link")?.links(filteredLinks);
      }

      // --- DATA JOIN & UPDATE --- 
      console.log(`[GRAPH JOIN] Updating with ${filteredNodes.length} nodes and ${filteredLinks.length} links.`);

      // LINKS
      const linkSelection = g.select<SVGGElement>("g.links")
        .selectAll<SVGLineElement, CustomLink>("line.link")
        .data(filteredLinks, (d: CustomLink) => `${(typeof d.source === 'object' ? d.source.id : d.source)}_${(typeof d.target === 'object' ? d.target.id : d.target)}`); // Key function

      linkSelection.exit()
        .transition("exit").duration(300)
      ...
      linkEnter.append("title").text((d: CustomLink) => d.relationship);

      // Merge enter and update selections for links
      const linkUpdate = linkEnter.merge(linkSelection);

      linkUpdate.transition("update").duration(300)
        .attr("stroke-opacity", 0.6); // Fade in/update opacity
        
      // NODES (Circles)
      const nodeSelection = g.select<SVGGElement>("g.nodes")
        .selectAll<SVGGElement, CustomNode>("g.node")
        .data(filteredNodes, (d: CustomNode) => d.id); // Key function

      nodeSelection.exit<CustomNode>() // Ensure correct type for exit data
        .transition("exit").duration(300)
        .style("opacity", 0)
        .remove();
      ...
      const nodeEnter = nodeSelection.enter()
        .append("g")
        .attr("class", (d: CustomNode) => `node node-group-${d.group} ${d.id === mainWord ? "main-node" : ""}`)
        .style("opacity", 0) // Start transparent
        // Set initial position for entering nodes near the main word if possible
        .attr("transform", (d: CustomNode) => {
           const mainNodePos = filteredNodes.find(n => n.id === mainWord);
           const x = d.x ?? mainNodePos?.x ?? width / 2;
           const y = d.y ?? mainNodePos?.y ?? height / 2;
           return `translate(${x}, ${y})`;
         });


      nodeEnter.append("circle")
        .attr("r", 0) // Start radius at 0 for enter animation
        .attr("fill", (d: CustomNode) => getNodeColor(d.group))
        .attr("stroke", (d: CustomNode) => d3.color(getNodeColor(d.group))?.darker(0.8).formatHex() ?? "#888")
        .attr("stroke-width", 1.5);
      
      // Apply drag ONLY to the enter selection if simulation exists
      if (simulation) {
      ...
      // Transition opacity for entering/updating nodes
      nodeUpdate.transition("update_opacity").duration(300).delay(100) // Slight delay after potential exit
        .style("opacity", 1);

      // Transition circle radius and color for entering/updating nodes
      nodeUpdate.select<SVGCircleElement>("circle")
        .transition("update_circle").duration(400).ease(d3.easeQuadOut) // Smoother ease
        .attr("r", (d: CustomNode) => getNodeRadius(d))
        .attr("fill", (d: CustomNode) => getNodeColor(d.group))
        .attr("stroke", (d: CustomNode) => d3.color(getNodeColor(d.group))?.darker(0.8).formatHex() ?? "#888");

      // LABELS (External Text)
      const labelSelection = g.select<SVGGElement>("g.labels")
        .selectAll<SVGTextElement, CustomNode>("text.node-label")
        .data(filteredNodes, (d: CustomNode) => d.id); // Key function

      labelSelection.exit<CustomNode>() // Ensure correct type for exit data
        .transition("exit").duration(300)
        .style("opacity", 0)
        .remove();
      ...
      const labelEnter = labelSelection.enter()
        ...
        .style("user-select", "none")
        .style("opacity", 0) // Start transparent
        // Set initial position for entering labels
         .attr("x", (d: CustomNode) => d.x ?? width / 2)
         .attr("y", (d: CustomNode) => (d.y ?? height / 2) + getNodeRadius(d) + 12);

      // Halo for contrast
      labelEnter.clone(true)
        .lower()
        .attr("fill", "none")
        .attr("stroke", theme === "dark" ? "rgba(0,0,0,0.8)" : "rgba(255,255,255,0.8)")
        .attr("stroke-width", 3)
      ...
      // Update text content and style for all labels (enter + update)
      labelUpdate
        .text((d: CustomNode) => d.word)
        .attr("font-weight", (d: CustomNode) => d.id === mainWord ? "600" : "400")
        .attr("fill", theme === "dark" ? "#eee" : "#222");

      // Transition opacity for entering/updating labels
      labelUpdate.transition("update_opacity").duration(300).delay(150) // Delay slightly more
          .style("opacity", 1);
        

      // Re-setup interactions for the updated node selection (all nodes currently displayed)
      setupNodeInteractions(nodeUpdate); 

      // Reheat the simulation
      if (simulation) {
        // Gently pin the new main word towards the center, but allow simulation to move it
        const mainNodeData = filteredNodes.find(n => n.id === mainWord);
        if (mainNodeData) {
           mainNodeData.fx = 0;
           mainNodeData.fy = 0;
           // Release the fix after a short while to let simulation take over
           setTimeout(() => {
              if (mainNodeData) {
                  mainNodeData.fx = null;
                  mainNodeData.fy = null;
              }
           }, 1500); // Release after 1.5 seconds
        }
        simulation.alpha(0.5).restart(); // Restart with moderate energy
        console.log("[GRAPH] Simulation restarted.");
      }

      // Update legend (simple redraw for now)
      svg.select("g.legend").remove();
       // (Paste the legend drawing code from the previous state here)
      // --- Start Pasted Legend Code ---
      ...
      let totalRows = 0;
      categories.forEach(cat => { totalRows += 1 + cat.types.length; });
      ...
      legendContainer.append("rect")
        .attr("width", legendWidth).attr("height", legendHeight).attr("rx", 10).attr("ry", 10)
        .attr("fill", theme === "dark" ? "rgba(28, 30, 38, 0.85)" : "rgba(255, 255, 255, 0.92)")
        .attr("stroke", theme === "dark" ? "rgba(255, 255, 255, 0.12)" : "rgba(0, 0, 0, 0.06)").attr("stroke-width", 1);
      legendContainer.append("text")
        .attr("x", legendWidth / 2).attr("y", legendPadding + 7).attr("text-anchor", "middle")
        .attr("font-weight", "600").attr("font-size", "12px").attr("fill", theme === "dark" ? "#eee" : "#333")
        .text("Relationship Types");
      legendContainer.append("text")
        .attr("x", legendWidth / 2).attr("y", legendPadding + 22).attr("text-anchor", "middle")
        .attr("font-weight", "400").attr("font-size", "9px").attr("fill", theme === "dark" ? "#aaa" : "#666")
        .text("Click to filter by type");
      legendContainer.append("line")
        .attr("x1", legendPadding).attr("y1", legendPadding + 30).attr("x2", legendWidth - legendPadding).attr("y2", legendPadding + 30)
        .attr("stroke", theme === "dark" ? "rgba(255, 255, 255, 0.1)" : "rgba(0, 0, 0, 0.06)").attr("stroke-width", 1);
      let yPos = legendPadding + 38;
      categories.forEach((category, categoryIndex) => {
        yPos += legendItemHeight;
        const categoryTextElement = legendContainer.append("text")
          .attr("x", legendPadding).attr("y", yPos).attr("font-weight", "600").attr("font-size", "11px")
          .attr("fill", theme === "dark" ? "#ccc" : "#555").text(category.name);
        const categoryTextBBox = categoryTextElement.node()?.getBBox();
        if (categoryTextBBox) {
          legendContainer.append("rect")
            .attr("x", legendPadding - 4).attr("y", yPos - categoryTextBBox.height + 2).attr("width", categoryTextBBox.width + 8)
            .attr("height", categoryTextBBox.height + 4).attr("rx", 3).attr("fill", theme === "dark" ? "rgba(255, 255, 255, 0.07)" : "rgba(0, 0, 0, 0.04)").lower();
        }
        category.types.forEach(type => {
          yPos += legendItemHeight;
          const isFiltered = filteredRelationships.includes(type.toLowerCase());
          const entry = legendContainer.append("g")
            .attr("transform", `translate(${legendPadding}, ${yPos})`).attr("class", "legend-item").attr("data-type", type);
          entry.append("rect")
            .attr("width", legendWidth - (legendPadding * 2)).attr("height", legendItemHeight).attr("x", -5).attr("y", -12).attr("rx", 4)
            .attr("fill", isFiltered ? (theme === "dark" ? "rgba(255, 255, 255, 0.05)" : "rgba(0, 0, 0, 0.03)") : "transparent").attr("cursor", "pointer")
            .on("mouseover", function(this: SVGRectElement) { /* ... hover logic ... */ }).on("mouseout", function(this: SVGRectElement) { /* ... mouseout logic ... */ })
            .on("click", function(this: SVGRectElement) {
              const relType = d3.select(this.parentNode as SVGGElement).attr("data-type");
              handleToggleRelationshipFilter(relType);
            });
          entry.append("circle")
            .attr("cx", 5).attr("cy", 0).attr("r", dotRadius).attr("fill", getNodeColor(type))
            .attr("stroke", theme === "dark" ? "rgba(255, 255, 255, 0.15)" : "rgba(0, 0, 0, 0.08)").attr("stroke-width", 0.5).style("opacity", isFiltered ? 0.5 : 1);
          const labelText = getRelationshipTypeLabel(type).label;
          const availableWidth = legendWidth - textPadding - (legendPadding * 2) - 10;
          const textElement = entry.append("text")
            .attr("x", textPadding).attr("y", 0).attr("dy", ".25em").attr("font-size", "11px")
            .attr("font-weight", "500").attr("fill", theme === "dark" ? "#ddd" : "#333")
            .style("text-decoration", isFiltered ? "line-through" : "none").style("opacity", isFiltered ? 0.7 : 1);
        ...
        if (categoryIndex < categories.length - 1) { yPos += categorySpacing; }
      });
      // Disconnected nodes toggle logic
      const checkboxY = yPos + legendItemHeight + 10;
      legendContainer.append("line")
          .attr("x1", legendPadding).attr("y1", checkboxY - 15)
          .attr("x2", legendWidth - legendPadding).attr("y2", checkboxY - 15)
          .attr("stroke", theme === "dark" ? "rgba(255, 255, 255, 0.1)" : "rgba(0, 0, 0, 0.06)").attr("stroke-width", 1);
      const toggleContainer = legendContainer.append("g")
          .attr("transform", `translate(${legendPadding}, ${checkboxY})`).attr("class", "disconnected-nodes-option").style("cursor", "pointer");
      const toggleTrack = toggleContainer.append("rect")
          .attr("width", 30).attr("height", 14).attr("rx", 7).attr("x", 0).attr("y", -5)
          .attr("fill", showDisconnectedNodes ? (theme === "dark" ? "#4873c4" : "#3873e8") : (theme === "dark" ? "#505868" : "#d9dee6"))
          .attr("stroke", "none").attr("filter", theme === "dark" ? "drop-shadow(0px 1px 1px rgba(0, 0, 0, 0.2))" : "drop-shadow(0px 1px 1px rgba(0, 0, 0, 0.1))");
      const toggleHandle = toggleContainer.append("circle")
          .attr("r", 5).attr("cx", showDisconnectedNodes ? 21 : 9).attr("cy", 2)
          .attr("fill", "#ffffff").attr("stroke", theme === "dark" ? "rgba(0, 0, 0, 0.05)" : "rgba(0, 0, 0, 0.1)").attr("stroke-width", 0.5)
          .attr("filter", "drop-shadow(0px 1px 1px rgba(0, 0, 0, 0.15))");
      toggleContainer.append("text")
          .attr("x", 38).attr("y", 2).attr("dominant-baseline", "middle").attr("font-size", "11px")
          .attr("font-weight", "500").attr("fill", theme === "dark" ? "#ddd" : "#444")
      ...
      toggleContainer.on("click", function() {
          ...
          toggleTrack.transition().duration(250).attr("fill", newValue ? (theme === "dark" ? "#4873c4" : "#3873e8") : (theme === "dark" ? "#505868" : "#d9dee6"));
          toggleHandle.transition().duration(250).attr("cx", newValue ? 21 : 9);
       });
      // --- End Pasted Legend Code ---


      // --- Finish async operations ---
      const loadingTimeout = setTimeout(() => {
         setIsLoading(false);
         console.log("[GRAPH EFFECT] Finished processing.");
      ...
      return () => {
        ...
        // Don't stop simulation here, let it settle or be stopped by unmount
      };
    } catch (err) {
    ...
    // Primary data dependencies that MUST trigger a full update/rebuild
    wordNetwork, // If the core network data changes
    mainWord,    // If the central word changes
    theme,       // If the theme changes (affects colors)
    
    // Dependencies derived from filters or primary data
    filteredNodes, // The actual set of nodes determined by filters/data
    filteredLinks, // The actual set of links derived from filteredNodes
    
    // Filter state itself (indirectly affects filteredNodes/Links)
    // We rely on filteredNodes/Links changing to trigger the effect
    // filteredRelationships, 
    // showDisconnectedNodes, 
    
    // Stable callback references (assuming defined with useCallback)
    ...
    createLinks,
    createNodes,
    setupNodeInteractions, // Setup needs to run if nodes/links change
    centerOnMainWord,
    setupSvgDimensions,
    handleToggleRelationshipFilter // Needed for legend click handler
  ]);
