...
const WordExplorer: React.FC = () => {
  ...
  const handleSearch = useCallback(async (wordToSearchParam?: string) => {
    ...
    let searchOptions: SearchOptions = { q: normalizedInput, limit: 1, include_full: false }; // Limit 1 initially
    ...
    try {
      ...
      if (searchResult && Array.isArray(searchResult.results) && searchResult.results.length > 0) {
        const firstResult = searchResult.results[0] as SearchWordResult;
        console.log('[handleSearch] Found result, fetching details for ID:', firstResult.id);
        
        // Keep track of the word we are fetching details for
        setMainWord(firstResult.lemma);
        setInputValue(firstResult.lemma); // Update input field to match found word
        setWordHistory(prev => {
          // Avoid adding duplicates to history if the last entry is the same
          if (prev[prev.length - 1] === firstResult.lemma) return prev;
          return [...prev.slice(0, currentHistoryIndex + 1), firstResult.lemma];
        });
        setCurrentHistoryIndex(prev => prev + 1);
        
        // Set a minimal loading state *before* fetching full details
        // This prevents the flicker but shows something is happening
        setSelectedWordInfo({ id: firstResult.id, lemma: firstResult.lemma }); 
        ...
        setSelectedWordInfo(null); // Ensure cleared if no results
        setWordNetwork(null);
        setEtymologyTree(null);
      }
  ...
  const debouncedSearch = useCallback(
    debounce(async (query: string) => {
      ...
      try {
        ...
        if (searchResult && Array.isArray(searchResult.results) && searchResult.results.length > 0) {
          console.log("Setting search results:", searchResult.results.length, "items");
          setSearchResults(searchResult.results as SearchWordResult[]);
