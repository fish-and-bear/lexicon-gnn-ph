...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  }, [selectedNodeId, onNodeClick, getNodeRadius, getNodeColor, theme, nodeMap, onNodeSelect, tooltipTimeoutId]); // Added onNodeSelect, tooltipTimeoutId
  ...
  useEffect(() => {
    ...
    const svgElement = svgRef.current; // Capture current value
    ...
    const centerTimeout = setTimeout(() => {
         if (svgElement) centerOnMainWord(d3.select(svgElement), filteredNodes); // Use captured value
     }, 800);
      ...
      if (svgElement) { // Use captured value
        const svg = d3.select(svgElement);
    ...
    centerOnMainWord,
     setupSvgDimensions,
     filteredNodes,
     baseLinks, // Added baseLinks (implicitly used via filteredLinks)
     filteredLinks, // Added filteredLinks
     filteredRelationships,
     filterUpdateKey,
     forceUpdate,
     handleToggleRelationshipFilter,
     showDisconnectedNodes,
     baseNodes.length // Added baseNodes.length
  ]);
