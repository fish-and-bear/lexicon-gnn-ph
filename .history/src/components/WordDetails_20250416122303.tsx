...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({
      ...
      <Box sx={{ 
        ...
        <Stack direction="row" spacing={1} alignItems="flex-start" flexWrap="nowrap" sx={{ mb: theme.spacing(1.5), width: '100%' }}>
          ...
          <Typography 
            ...
            sx={{ 
              flexGrow: 1, // Keep allowing it to take space
              minWidth: 0, // Keep allowing shrinking
              // Force block-like behavior for wrapping:
              display: 'block', 
              width: '100%', // Explicitly use parent's width
              // Re-add overflowWrap for robustness:
              overflowWrap: 'break-word',
              wordBreak: 'break-word', // Redundant but safe
  ...
  return (
    <Paper elevation={2} square sx={{ display: 'flex', flexDirection: 'column', height: '100%', width: '100%', bgcolor: 'background.paper', overflow: 'hidden' }}>
