...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({
  ...
  const renderSourcesInfoTab = () => {
     ...
     const renderJsonData = (title: string, data: Record<string, any>) => {
       ...
       // Filter out meta-metadata keys
       const filteredData = { ...data };
       if (title === 'Word Metadata' && filteredData.meta) {
         delete filteredData.meta;
       }
  ...
  // Add a new renderDataTab function before renderSourcesInfoTab
  const renderDataTab = () => {
    // Fields to exclude (meta-metadata or fields shown in other tabs)
    const excludedFields = [
      'id', 'lemma', 'normalized_lemma', 'language_code', 'has_baybayin', 
      'baybayin_form', 'romanized_form', 'root_word_id', 'preferred_spelling', 
      'tags', 'data_hash', 'search_text', 'created_at', 'updated_at', 
      'definitions', 'etymologies', 'pronunciations', 'credits',
