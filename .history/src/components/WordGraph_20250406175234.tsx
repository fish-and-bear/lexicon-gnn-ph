...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const filteredNodes = useMemo<CustomNode[]>(() => {
    ...
    console.log(`Filtering network with depth=${depth}, breadth=${breadth}, nodes=${baseNodes.length}, edges=${baseLinks.length}`);
    ...
    console.log(`Filtered nodes: ${nodesToInclude.size} of ${baseNodes.length}`);
  ...
  useEffect(() => {
    if (wordNetwork) {
      // Check if we have valid nodes and links arrays
      const validNodes = Array.isArray(wordNetwork.nodes) ? wordNetwork.nodes : [];
      const validLinks = Array.isArray(wordNetwork.edges) ? wordNetwork.edges : [];
      
      console.log(`WordGraph received data: ${validNodes.length} nodes, ${validLinks.length} links`);
      
      // Set local state with the verified data
      setBaseNodes(validNodes);
      setBaseLinks(validLinks);
    } else {
      // Reset if no network data
      setBaseNodes([]);
      setBaseLinks([]);
    }
  }, [wordNetwork]);
