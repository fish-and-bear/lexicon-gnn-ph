...
const WordExplorer: React.FC = () => {
  ...
  const fetchRandomWordsForCache = useCallback(async (count: number = RANDOM_CACHE_SIZE): Promise<any[]> => {
    ...
    try {
      ...
      // Optimize by using fewer parallel requests (reduce from 2x to 1.5x count)
      // This reduces server load while still ensuring we get enough words
      ...
      if (validResults.length === 0) {
        ...
        // Use exponential backoff for retries (first retry at 3s, then 6s, etc.)
        const retryDelay = Math.min((retryCountRef.current + 1) * 3000, 15000);
        retryCountRef.current++;
        
        setTimeout(() => {
          fetchRandomWordsForCache(count);
          // Reset retry count after successful retry
        ...
        // Combine, shuffle, and update the cache - limit size for better memory usage
  ...
  const handleRandomWord = useCallback(async () => {
    ...
    try {
      ...
      if (currentCache.length > 0) {
        ...
        // Reduced threshold from RANDOM_CACHE_SIZE * 2 to RANDOM_CACHE_SIZE * 1.5 for faster response
      ...
      if (randomWord) {
        // Create a normalized WordInfo object with semantic_network data
        ...
        // Fetch network data in parallel - don't block the UI
        Promise.all([
          ...
          (depth && breadth) ? 
            fetchWordNetworkData(randomWord.lemma, depth, breadth)
              .then(networkData => {
                ...
                // If the word doesn't have relations but we have network data, 
                // update semantic_network in the wordInfo to display relationships
                if ((!wordInfo.outgoing_relations?.length && !wordInfo.incoming_relations?.length) || 
                    wordInfo.server_error) {
              ...
              .catch(err => {
                ...
                // Don't fail the whole operation if network fails
              }) : Promise.resolve(),
          ...
          fetchEtymologyTree(randomWord.id)
            ...
            .catch(err => {
              console.error("Error fetching etymology tree:", err);
              // Don't fail the whole operation if etymology fails
            })
        ]);
        
        // Get the semantic network specifically if not included in the random word data
        if (!wordInfo.semantic_network && !wordInfo.outgoing_relations?.length && !wordInfo.incoming_relations?.length) {
          fetchWordNetworkData(randomWord.lemma, 1, 10)
            .then(networkData => {
              // Update the word info with semantic network data for displaying relationships
              setSelectedWordInfo(prev => ({
                ...prev,
                semantic_network: {
                  nodes: networkData.nodes || [],
            ...
            .catch(err => {
              console.error("Error fetching semantic network:", err);
            });
        }
      } else {
      ...
      // Set a smaller delay before allowing another click
      randomWordTimeoutRef.current = setTimeout(() => {
        setIsRandomLoading(false);
      }, 50); // Keep at 50ms to maintain responsiveness
    }
  }, [
    depth, 
    breadth, 
