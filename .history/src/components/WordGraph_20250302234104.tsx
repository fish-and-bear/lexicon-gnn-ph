...
const WordGraph: React.FC<WordGraphProps> = ({
    ...
    const sim = d3
      ...
      .on('tick', () => {
        ...
        nodeElements.attr('transform', d => {
          // Constrain nodes to stay within the SVG bounds with a margin
          const margin = 20;
          const x = Math.max(margin, Math.min(containerWidth - margin, d.x || 0));
          const y = Math.max(margin, Math.min(containerHeight - margin, d.y || 0));
  ...
  // Create an improved drag behavior for nodes
  const createDragBehavior = useCallback(() => {
    return d3.drag<SVGGElement, CustomNode>()
      .on('start', (event, d) => {
        // Prevent default browser behavior which might interfere with drag
        if (event.sourceEvent) {
          event.sourceEvent.preventDefault();
          event.sourceEvent.stopPropagation();
        }
        
        // When drag starts, warm up the simulation and fix the node position
      ...
      .on('drag', (event, d) => {
        // During drag, update the fixed position to match cursor
        d.fx = event.x;
        d.fy = event.y;
        
        // Update the node's visual position - use the selection directly
        // rather than trying to re-select via currentTarget which can cause issues
        d3.select(event.subject.id ? `g.node[id="${event.subject.id}"]` : event.sourceEvent.target.closest('.node'))
      ...
      .on('end', (event, d) => {
        ...
        // Only release node if shift key is not pressed (for pinning)
        if (!event.sourceEvent || !event.sourceEvent.shiftKey) {
          d.fx = null;
          d.fy = null;
        } else {
          // Shift key is pressed - keep node pinned and mark it visually
          d3.select(event.sourceEvent.target.closest('.node')).select("circle")
            .attr("stroke", "#fca311")
            .attr("stroke-width", 3);
        }
      });
  ...
  const setupNodeInteractions = useCallback((node: d3.Selection<SVGGElement, CustomNode, SVGGElement, unknown>, svg: d3.Selection<SVGSVGElement, unknown, null, undefined>) => {
    node
      .on("click", (event: MouseEvent, d: CustomNode) => {
        ...
        // Call onNodeClick to change the selected word - this will update the main word in the parent component
        onNodeClick(d.id);
        
        // Print debug info
        console.log(`Selected node changed to: ${d.id}, passing to parent component`);
      })
  ...
  const renderTooltip = useCallback(() => {
    ...
    // Try different sources for definitions
    const infoDefinitions = hoveredNode.info?.definitions || [];
    const nodeDefinitions = nodeData?.definitions || [];
    
    // Combine all possible definition sources
    let definitionText = "No definition available";
    
    // Try to extract definition from various sources and formats
    if (infoDefinitions.length > 0 || nodeDefinitions.length > 0) {
      const allDefinitions = [...infoDefinitions, ...nodeDefinitions];
      
      // Check each definition until we find valid text
      for (const def of allDefinitions) {
        if (typeof def === 'string' && def.trim()) {
          definitionText = def;
          break;
        } else if (def && typeof def === 'object') {
          // Check all possible property names that might contain the definition
          const possibleTextProps = ['text', 'definition_text', 'definition', 'meaning', 'description'];
          for (const prop of possibleTextProps) {
            if ((def as any)[prop] && typeof (def as any)[prop] === 'string') {
              definitionText = (def as any)[prop];
              break;
            }
          }
          if (definitionText !== "No definition available") break;
        }
      }
    }
    
    // If we still don't have a definition, check if there's any custom properties
    if (definitionText === "No definition available" && nodeData) {
      // Safely check for any properties that might contain definition information
      const nodeDataAny = nodeData as any;
      
      if (nodeDataAny.gloss && typeof nodeDataAny.gloss === 'string') {
        definitionText = nodeDataAny.gloss;
      } else if (nodeData.type && typeof nodeData.type === 'string' && nodeData.type !== 'word') {
        definitionText = `Type: ${nodeData.type}`;
      }
    }
    ...
    return (
      ...
      >
        ...
        <p style={{ 
          ...
          lineHeight: "1.4",
          fontStyle: definitionText === "No definition available" ? "italic" : "normal",
        }}>
          {definitionText}
        </p>
        <div style={{ 
