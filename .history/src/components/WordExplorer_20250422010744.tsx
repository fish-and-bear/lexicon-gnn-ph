...
const WordExplorer: React.FC = () => {
    ...
    return (
      <AppBar position="static" color="default" elevation={1}>
        <Toolbar variant="dense">
          ...
          <Typography variant="h6" sx={{ flexGrow: 1, display: { xs: 'none', sm: 'block' } }}> {/* Hide title on very small screens */}
             Filipino Root Explorer
          </Typography>

          {/* Search Autocomplete */}
          <Box sx={{ flexGrow: 1, mr: 1, maxWidth: '400px' }}> {/* Limit search width */}
            <Autocomplete<
              WordSuggestion | string,
              false, // Multiple
              false, // DisableClearable
          ...
          <Box sx={{ display: 'flex', gap: 0.5 }}>
            <Button
              ...
              disabled={isLoading || !inputValue.trim()}
              title="Search"
              sx={{ minWidth: 'auto', px: 1 }} // Allow shrinking
            >
  ...
  return (
    <div className={`word-explorer ${themeName} ${isLoading ? 'loading' : ''}`}>
      ...
      <main style={{ height: isMobile ? 'auto' : 'calc(100vh - 160px)' }}> {/* Adjust height based on mobile view */}
        {isMobile ? (
          // Mobile: Stacked layout (Graph above Details)
          <div className="explorer-content-mobile">
            <Box className="graph-area-mobile" sx={{ minHeight: '300px', borderBottom: '1px solid var(--card-border-color)', pb: 1, mb: 1 }}>
            ...
            {/* Details Area */}
             <Box className="details-area-mobile" sx={{ flexGrow: 1, overflowY: 'auto' }} ref={detailsContainerRef}>
               {wordData && (
                 <WordDetails
                  wordData={wordData}
          ...
          <PanelGroup direction="horizontal" className="explorer-panel-group">
            <Panel defaultSize={65} minSize={30} className="explorer-panel-main">
              <div className="explorer-content" style={{ height: '100%', overflow: 'hidden' }}> {/* Ensure graph area fills panel */}
