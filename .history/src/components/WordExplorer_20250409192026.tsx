...
const WordExplorer: React.FC = () => {
  ...
  const handleSearch = useCallback(async (searchWord?: string) => {
    ...
    try {
      ...
      if (!isConnected) {
        throw new Error("Cannot connect to the backend server. Please ensure the backend server is running on port 10000.");
      }
      ...
      try {
        ...
        if (wordData && wordData.lemma) {
          ...
          try {
            console.log('Fetching word network and etymology tree...');
            ...
            if (!networkData) {
              throw new Error('No network data received from server');
            }
            ...
            if (networkError instanceof Error) {
              if (networkError.message.includes('backend server')) {
                throw networkError; // Re-throw backend connection errors
              }
              setError(`Error fetching word network: ${networkError.message}`);
            } else {
              setError('Failed to fetch word network. Please try again.');
            }
            // Continue showing word details even if network fetch fails
            return;
          }
        } else {
        ...
        if (detailsError instanceof Error && detailsError.message.includes('backend server')) {
          throw detailsError; // Re-throw backend connection errors
        }
      }
      ...
      if (searchResults && searchResults.words && searchResults.words.length > 0) {
        ...
        if (wordData && wordData.lemma) {
          ...
          try {
            ...
            console.error('Error fetching network data for search result:', networkError);
            if (networkError instanceof Error) {
              if (networkError.message.includes('backend server')) {
                throw networkError; // Re-throw backend connection errors
              }
      ...
      if (err instanceof Error) {
        if (err.message.includes('backend server')) {
          ...
          // Show backend server instructions in the error UI
  ...
  const handleDebouncedSearch = useCallback(async (query?: string) => {
    ...
    setIsLoadingSuggestions(true);
      ...
      // Map the results to match SearchWordResult interface
      const mappedResults: SearchWordResult[] = results.words.map(word => ({
        id: word.id,
        lemma: word.lemma,
        normalized_lemma: word.normalized_lemma || word.lemma,
        language_code: word.language_code || 'tl',
        has_baybayin: word.has_baybayin || false,
        baybayin_form: word.baybayin_form || null,
        romanized_form: word.romanized_form || null,
  ...
  const handleSuggestionClick = (suggestion: SearchWordResult) => {
    setInputValue(suggestion.lemma);
    handleSearch(suggestion.lemma);
    setShowSuggestions(false);
  };
  ...
  const handleTestApiConnection = async () => {
    ...
    try {
      ...
      if (!isConnected) {
        ...
        setError("Cannot connect to the API server. Please check that the backend server is running on port 10000 and CORS settings allow the connection.");
      } else {
      ...
      setError("Error testing API connection. Please check the console for details.");
    }
  };
                ...
                searchResults.map((suggestion) => (
                  ...
                  >
                    ...
                    {suggestion.has_baybayin && suggestion.baybayin_form && (
                      <span className="baybayin">{suggestion.baybayin_form}</span>
                    )}
                  </div>
                ))
              ) : (
