...
const WordExplorer: React.FC = () => {
  ...
  useEffect(() => {
    const panelRef = detailsContainerRef.current;
    if (!panelRef) return;

    // Get the panel ID - Assuming the Panel component has id="details-panel"
    const panelId = panelRef.getId(); 
    const panelElement = document.getElementById(panelId); 

    if (!panelElement) {
      console.warn("Could not find details panel element for ResizeObserver using ID:", panelId);
      return; 
    }
    ...
    const resizeObserver = new ResizeObserver(entries => {
      for (const entry of entries) {
        if (entry.target === panelElement) {
          // Get size percentage from the panel handle
          const sizePercent = detailsContainerRef.current?.getSize(); 
          if (sizePercent !== undefined) {
             localStorage.setItem('wordDetailsWidthPercent', sizePercent.toString()); 
          }
        }
      }
    });
