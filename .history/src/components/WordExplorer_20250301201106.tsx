...
import { fetchWordNetwork, fetchWordDetails, searchWords, resetCircuitBreaker } from "../api/wordApi";
...
const WordExplorer: React.FC = () => {
        ...
        try {
          ...
          if (results && results.words) {
            const searchResults = results.words
              .filter(word => word.word && typeof word.word === 'string')
              .map(word => ({
                id: word.id,
                word: word.word.trim()
              }))
  ...
  const handleSearch = useCallback(async (searchWord?: string) => {
    ...
    try {
      ...
      if (!networkData || !networkData.nodes || networkData.nodes.length === 0) {
        throw new Error("No word relationships found.");
      }
      ...
      navigate(`/words/${encodeURIComponent(detailsData.lemma)}`, { replace: true });
    } catch (error) {
  ...
  }, [inputValue, depth, breadth, currentHistoryIndex, fetchWordNetworkData, navigate]);
  ...
  }, [depth, breadth, currentHistoryIndex, fetchWordNetworkData, navigate]);
              ...
              <div className="word-details">
                ...
                {selectedWordInfo.etymologies && selectedWordInfo.etymologies.length > 0 && (
                  <p>
                    ...
                    {selectedWordInfo.etymologies[0].text}
                  </p>
                )}
