...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({
  ...
  const renderRelationsTab = () => {
      ...
      return (
        <Box sx={{ mb: theme.spacing(3) }} key={title}>
          ...
          {Object.entries(groupedRelations).map(([relType, rels]) => (
            <Box key={relType} sx={{ mb: theme.spacing(2) }}>
              ...
              <Stack direction="row" spacing={1} useFlexGap flexWrap="wrap">
                {rels.map((rel, index) => {
                  ...
                  const hasSources = rel.sources && 
                                    (typeof rel.sources === 'string' || 
                                     (Array.isArray(rel.sources) && rel.sources.length > 0));
                  
                  return (
                    <Chip
                      key={`${rel.id || index}-${wordObj?.id || index}`}
                      label={wordObj?.lemma || wordObj?.word || 'Unknown'}
                      className="relation-chip"
                      ...
                      // Show sources as tooltip if available
                      title={hasSources ? (
                        typeof rel.sources === 'string' 
                          ? rel.sources 
                          : (Array.isArray(rel.sources) ? rel.sources.join(', ') : '')
                      ) : undefined}
                    />
                  );
                })}
    ...
    return (
      <Box sx={{ pt: theme.spacing(1) }}> {/* Fix padding to match other tabs */}
