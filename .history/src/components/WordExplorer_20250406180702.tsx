...
const WordExplorer: React.FC = () => {
  ...
  const handleDepthChange = useCallback((newDepth: number) => {
    ...
    if (wordNetwork && mainWord) {
      if (newDepth > 3) {
        console.warn(`Deep search (depth=${newDepth}) may be slower and generate a complex network`);
      }
      // If we already have a network, refetch with new depth
      fetchWordNetworkData(normalizeInput(mainWord), newDepth, breadth);
    }
  }, [wordNetwork, mainWord, breadth, fetchWordNetworkData]);

  const handleBreadthChange = useCallback((newBreadth: number) => {
    setBreadth(newBreadth);
    if (wordNetwork && mainWord) {
      if (newBreadth > 40) {
        console.warn(`Wide search (breadth=${newBreadth}) may generate a very dense network`);
      }
      // If we already have a network, refetch with new breadth
      fetchWordNetworkData(normalizeInput(mainWord), depth, newBreadth);
    }
  }, [wordNetwork, mainWord, depth, fetchWordNetworkData]);
  ...
  const handleNetworkReset = useCallback(() => {
    if (mainWord) {
      ...
      setBreadth(20);  // Reset to higher default breadth
      fetchWordNetworkData(normalizeInput(mainWord), 2, 20);
    }
              ...
              <WordGraph
                wordNetwork={{
                  ...
                  stats: wordNetwork.stats || {
                    node_count: (wordNetwork.nodes || []).length,
                    edge_count: (wordNetwork.edges || []).length,
                    depth: depth,
                    breadth: breadth
                  }
                }}
                ...
                onNetworkChange={handleDepthChange}
                initialDepth={depth}
              ...
              <div className="network-controls">
                ...
                <div className="slider-container">
                  ...
                  <input
                    ...
                    value={breadth}
                    onChange={(e) => {
                      const newBreadth = parseInt(e.target.value);
                      if (newBreadth !== breadth) {
                        handleBreadthChange(newBreadth);
                      }
                    }}
                  />
                  {breadth > 40 && (
