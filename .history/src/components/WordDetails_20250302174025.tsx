...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({ wordInfo, onWordClick }) => {
  ...
  // Save container width to localStorage when it changes
  useEffect(() => {
    if (containerWidth) {
      localStorage.setItem('wordDetailsWidth', containerWidth.toString());
    }
  }, [containerWidth]);

  // Initialize container width from localStorage
  useEffect(() => {
  ...
  return (
    <div className="word-details-container" ref={containerRef}>
      <div className="word-details">
        ...
        <div className="word-content-tabs">
          ...
          {wordInfo.baybayin_form && (
            <button 
              className={`tab-button ${activeTab === 'baybayin' ? 'active' : ''}`}
