...
// Removed WordDetails.css import as styles are primarily handled by MUI now
...
// Define relation type colors dynamically based on theme
const getRelationColors = (theme: Theme): { [key: string]: string } => {
  const isDarkMode = theme.palette.mode === 'dark';
  return {
    // Core relations
    main: theme.palette.primary.main,
    root: theme.palette.error.main,

    // Meaning group - Blues/Indigos
    synonym: theme.palette.info.main,
    antonym: isDarkMode ? theme.palette.secondary.light : theme.palette.secondary.dark, // Contrast needed
    related: theme.palette.info.light,
    similar: theme.palette.primary.light,

    // Origin group - Reds and oranges
    etymology: isDarkMode ? theme.palette.error.light : theme.palette.error.dark,
    cognate: theme.palette.warning.main,

    // Form group - Purples/Magentas
    variant: theme.palette.secondary.main,
    spelling_variant: isDarkMode ? theme.palette.secondary.light : alpha(theme.palette.secondary.main, 0.7),
    regional_variant: isDarkMode ? theme.palette.secondary.light : alpha(theme.palette.secondary.main, 0.5),

    // Hierarchy group - Greens
    hypernym: theme.palette.success.main,
    hyponym: isDarkMode ? theme.palette.success.dark : theme.palette.success.light,
    taxonomic: theme.palette.success.light,
    meronym: alpha(theme.palette.success.main, 0.7),
    holonym: alpha(theme.palette.success.main, 0.5),
    part_whole: theme.palette.success.light,
    component: alpha(theme.palette.success.main, 0.7),

    // Derivational group - Greens/Teals
    derived: theme.palette.success.dark,
    affix: alpha(theme.palette.success.dark, 0.8),
    derivative: alpha(theme.palette.success.dark, 0.6),

    // Info group - Yellows/Oranges
    usage: theme.palette.warning.light,

    // Specific Filipino relations - Oranges and pinks (use secondary/error)
    kaugnay: theme.palette.warning.dark,
    salita: theme.palette.secondary.main,
    kahulugan: theme.palette.secondary.dark,
    kasalungat: theme.palette.error.main,

    // Fallback
    associated: theme.palette.grey[500],
    other: theme.palette.grey[600]
  };
};


// Define key graph colors locally for styling, adapted for theme
const getGraphColors = (theme: Theme) => ({
  main: theme.palette.primary.main,
  root: theme.palette.error.main,
  synonym: theme.palette.info.main,
  antonym: theme.palette.secondary.dark,
  derived: theme.palette.success.dark,
  variant: theme.palette.secondary.main,
  related: theme.palette.info.light,
  associated: theme.palette.grey[500],
...
// --- Styled Components ---
// Simplified Accordion Styling (remains mostly the same, ensure divider is used)
...
const StyledAccordionDetails = styled(AccordionDetails)(({ theme }: { theme: Theme }) => ({ // Add Theme type
  ...
  backgroundColor: theme.palette.mode === 'dark' ? alpha(theme.palette.background.paper, 0.5) : 'transparent', // Use paper bg with alpha in dark mode
}));
...
const WordDetailsComponent = React.forwardRef<HTMLDivElement, WordDetailsProps>((
  ...
  const relationColors = getRelationColors(theme);
  ...
  const renderHeader = () => {
    ...
    const mainColor = theme.palette.primary.main; // Use theme's primary color directly
    
    // Determine text color based on background - simplified to use theme defaults
    const headerTextColor = theme.palette.text.primary; // Default text color should contrast well

    return (
      // More elegant header with subtle styling
      <Box sx={{ 
        bgcolor: isDarkMode ? alpha(theme.palette.background.paper, 0.3) : headerBgColor, // Adjust dark bg slightly
        ...
        <Stack direction="row" spacing={1} alignItems="flex-start" flexWrap="nowrap" sx={{ mb: theme.spacing(0) }}>
          ...
          <Typography 
            ...
            sx={{ 
              ...
              color: theme.palette.text.primary, // Ensure primary text color
              ...
              '&::after': {
                ...
                height: '2px',
                bgcolor: mainColor, // Use theme primary
                display: 'block',
                borderRadius: '2px',
              }
            }}
          ...
          {hasAudio && (
            <IconButton
              ...
              sx={{ 
                ...
                color: mainColor, // Use theme primary
                mt: 0.5,
                bgcolor: 'transparent',
                '&:hover': {
                  bgcolor: alpha(mainColor, 0.1) // Use theme primary for hover
                }
              }}
            >
            ...
            <Box sx={{ mt: 1 }}>
              <Stack direction="row" spacing={2} flexWrap="wrap">
                {wordData.pronunciations.filter(p => 
                  ...
                  <Box key={index}>
                    <Typography variant="caption" sx={{ color: theme.palette.text.secondary, fontWeight: 500 }}> {/* Use secondary text */}
                      {/* Capitalize type name for display */}
                      {pron.type ? pron.type.charAt(0).toUpperCase() + pron.type.slice(1) : 'Pronunciation'}
                    </Typography>
                    <Typography variant="body2" sx={{ fontWeight: 500, color: theme.palette.text.primary }}> {/* Use primary text */}
                      {pron.value}
                    </Typography>
                  </Box>
                ))}
        ...
        {wordData.has_baybayin && wordData.baybayin_form && wordData.baybayin_form.trim() !== '' && (
          <Box sx={{ my: 0, mt: theme.spacing(isMobile ? 1 : 1.5) }}> 
            <Typography variant="caption" sx={{ color: theme.palette.text.secondary, display: 'block', mb: 0.25, fontSize: isMobile ? '0.7rem' : '0.75rem' }}> {/* Use secondary text */}
              Baybayin Script
            </Typography>
            <Box // Changed from div to Box for sx prop
              className="baybayin-text" // Keep class for potential global styles if needed
              sx={{ // Moved inline styles to sx prop
                ...
                background: isDarkMode ? alpha(theme.palette.common.white, 0.1) : alpha(theme.palette.common.black, 0.04), // Theme aware subtle background
                borderRadius: '4px',
                display: 'inline-block',
                lineHeight: 1.2,
                minHeight: isMobile ? '36px' : '48px',
                marginTop: '4px',
                color: 'text.primary', // Inherit text color
        ...
        {wordData.badlit_form && wordData.badlit_form.trim() !== '' && (
          <Box sx={{ my: 0, mt: theme.spacing(isMobile ? 1 : 1.5) }}> 
            ...
            <Box // Changed from div to Box for sx prop
              className="badlit-text" // Keep class
              sx={{ // Moved inline styles to sx prop
            ...
            <Box> 
              ...
              <Typography 
                ...
                sx={{ 
                  ...
                  borderRadius: '3px',
                  bgcolor: isDarkMode ? alpha(mainColor, 0.25) : alpha(mainColor, 0.1), // Slightly adjust alpha
                  color: isDarkMode ? theme.palette.primary.contrastText : mainColor, // Use contrast text or main color
                  border: `1px solid ${alpha(mainColor, 0.3)}`, // Adjust border alpha
          ...
          {wordData.romanized_form && (
            <Box> // REMOVED COMMENT
              ...
              <Typography 
                ...
                sx={{ 
                  fontWeight: 500,
                  color: theme.palette.text.primary, // Use primary text
            ...
            {tags.map((tag) => (
              <Chip
                ...
                sx={{
                  ...
                  bgcolor: alpha(theme.palette.text.primary, 0.05), // Subtle background
                  ...
                  {filteredDefs.map((def: Definition, index: number) => {
                    ...
                    >
                      ...
                      {def.usage_notes && def.usage_notes.length > 0 && (
                        <Box sx={{ pl: 1, mb: 1, mt: 1, py: 1, px: 1.5, bgcolor: alpha(theme.palette.info.light, 0.1), borderRadius: 1 }}> {/* Added background */}
                          <Typography
                            variant="caption"
                            component="div"
                            sx={{ fontWeight: 500, mb: 0.5, fontSize: isMobile ? '0.75rem' : '0.8rem', color: theme.palette.info.dark }} // Use info color
                          >
                            Usage Notes:
                          </Typography>
                          ...
                          {def.tags.map((tag, tagIndex) => (
                            <Chip
                              ...
                              sx={{
                                ...
                                bgcolor: alpha(theme.palette.secondary.main, 0.1), // Use secondary theme color
                      ...
                      {isLastDefinitionForSource && sourceName !== 'Unknown Source' && (
                        <Chip
                          ...
                          sx={{
                            ...
                            height: 'auto',
                            padding: theme.spacing(0.25, 0),
                            borderColor: alpha(theme.palette.grey[500], 0.4), // Use grey
              ...
              >
                <Typography 
                  ...
                  sx={{ 
                    ...
                    borderBottom: `1px solid ${alpha(category.color, 0.3)}`, // Use category color from theme
                    color: theme.palette.text.primary, // Use primary text
                    fontWeight: 600
                  }}
                >
                  <Box 
                    ...
                    sx={{
                      ...
                      height: 12,
                      borderRadius: '50%',
                      bgcolor: category.color, // Use category color from theme
                      boxShadow: `0 0 0 2px ${alpha(category.color, 0.3)}` // Use category color from theme
                    }}
                  />
                  {category.name}
                  <Typography 
                ...
                <Box sx={{ pl: theme.spacing(2) }}>
                  {(() => {
                    ...
                    return sortedTypes.map(relationType => {
                      ...
                      const relColor = relationColors[relationType] || category.color; // Use theme-derived colors
                        ...
                        <Box key={relationType} sx={{ mb: theme.spacing(1.5) }}>
                          <Typography 
                            ...
                            sx={{ 
                              fontWeight: 500,
                              fontSize: '0.85rem',
                              color: relColor, // Use theme-derived color
                          ...
                          >
                            <Box 
                              ...
                              sx={{
                                ...
                                height: 8,
                                borderRadius: '50%',
                                bgcolor: relColor, // Use theme-derived color
                                boxShadow: `0 0 0 2px ${alpha(relColor, 0.3)}` // Use theme-derived color
                              }}
                            />
  ...
  const renderFormsAndTemplatesTab = () => {
    ...
    return (
      ...
      <Box sx={{ pt: isMobile ? theme.spacing(0.5) : theme.spacing(1) }}>
        {/* Forms Section */}
        {forms.length > 0 && (
          <StyledAccordion defaultExpanded sx={{ mb: 2, bgcolor: alpha(theme.palette.background.paper, 0.5) }}> {/* Subtle bg */}
            <StyledAccordionSummary expandIcon={<ExpandMoreIcon />}>
                ...
                {forms.map((form: WordForm, index: number) => (
                  <ListItem key={form.id || index} disableGutters sx={{ py: isMobile ? 0.1 : 0.25 }}>
                    ...
                    <Stack direction="row" spacing={0.5} useFlexGap flexWrap="wrap">
                       {form.is_canonical && <Chip label="Canonical" size="small" color="primary" variant={isDarkMode ? "filled" : "outlined"} sx={{ height: 'auto', fontSize: isMobile ? '0.6rem' : '0.65rem', py: 0.1, px: 0.25 }} />} {/* Filled in dark mode */}
                       {form.is_primary && <Chip label="Primary" size="small" color="secondary" variant={isDarkMode ? "filled" : "outlined"} sx={{ height: 'auto', fontSize: isMobile ? '0.6rem' : '0.65rem', py: 0.1, px: 0.25 }} />} {/* Filled in dark mode */}
                       {form.tags && Object.entries(form.tags).map(([key, value]) => (
                          <Chip key={key} label={`${key}: ${value}`} size="small" sx={{ height: 'auto', fontSize: isMobile ? '0.6rem' : '0.65rem', py: 0.1, px: 0.25, bgcolor: alpha(theme.palette.text.secondary, 0.1), color: 'text.secondary' }} /> {/* Theme-aware default chip */}
                       ))}
                    </Stack>
                  </ListItem>
                ))}
        ...
        {/* Templates Section */}
        {templates.length > 0 && (
          <StyledAccordion defaultExpanded sx={{ bgcolor: alpha(theme.palette.background.paper, 0.5) }}> {/* Subtle bg */}
            <StyledAccordionSummary expandIcon={<ExpandMoreIcon />}>
            ...
            <List dense={isMobile} disablePadding>
              {rhymes.map((rhyme, index) => (
                <ListItem key={`rhyme-${index}`} disableGutters sx={{ py: isMobile ? 0.1 : 0.25 }}>
                  <ListItemText 
                    primary={rhyme.value} 
                    primaryTypographyProps={{ sx: { fontFamily: 'monospace', fontSize: isMobile ? '0.8rem' : '0.875rem', color: 'text.secondary'} }} /* Use secondary color */
                  />
                </ListItem>
              ))}
            </List>
                            ...
                            {etym.components.map((comp, i) => (
                              <Chip 
                                ...
                                sx={{
                                  ...
                                  bgcolor: alpha(theme.palette.success.main, 0.15), // Use theme success color
                                  color: theme.palette.success.dark,
                                  fontWeight: 500,
                                  '&:hover': {
                                    bgcolor: alpha(theme.palette.success.main, 0.25), // Darken hover
                                  }
                                }}
                              />
                            ))}
                            ...
                            {etym.languages.map((lang, i) => (
                              <Chip 
                                ...
                                sx={{ 
                                  fontSize: '0.75rem',
                                  height: 24,
                                  borderColor: alpha(theme.palette.secondary.main, 0.6), // Use theme secondary color
                                  color: theme.palette.secondary.main
                                }}
                              />
                            ))}
                            ...
                            {etym.sources.map((source, i) => (
                              <Chip 
                                ...
                                sx={{ 
                                  ...
                                  height: 'auto',
                                  padding: theme.spacing(0.25, 0),
                                  bgcolor: alpha(theme.palette.grey[500], 0.1), // Use grey
                                  color: theme.palette.grey[700],
                                  '& .MuiChip-label': { 
                                    px: 1, 
                                    py: 0.25 
    ...
    const renderNode = (nodeId: number, nodes: EtymologyTreeMap, edges: EtymologyEdge[], level = 0): React.ReactNode => {
      ...
      return (
        <Box key={node.id} sx={{ ml: level * 2.5, mb: 1.5 }}>
          <Paper elevation={1} sx={{ p: 1.5, display: 'flex', alignItems: 'center', gap: 1.5, borderRadius: '8px', bgcolor: alpha(theme.palette.background.paper, 0.7) }}> {/* Use paper bg */}
            <Chip 
              label={node.language?.toUpperCase() || 'UNK'} 
              size="small" 
              sx={{ fontWeight: 600, bgcolor: getNodeColor(node.language || 'associated', theme.palette.mode), color: getTextColorForBackground(getNodeColor(node.language || 'associated', theme.palette.mode)) }} // Pass theme mode to color utils - Re-corrected
            />
            <Link
              component="button"
              ...
              sx={{ 
                ...
                '&:hover': { color: 'primary.main' } // Use theme primary
              }}
            >
              {node.label}
            </Link>
          ...
          {childrenEdges.length > 0 && (
            <Box sx={{ 
              ...
              borderLeft: `1px solid ${theme.palette.divider}` // Use theme divider
            }}>
  ...
  if (!wordData?.id) {
    ...
    return (
        <Paper elevation={1} sx={{ 
            ...
            <Typography color={'text.secondary'}>Select a word to see details.</Typography> {/* Use theme secondary text color */}
        </Paper>
    );
  }
  ...
  return (
    <Box 
      ...
      sx={{ // *** ADD COMPREHENSIVE DARK MODE STYLING ***
        display: 'flex',
        flexDirection: 'column',
        height: '100%', 
        overflow: 'hidden', 
        ...
        p: 0, 
        m: 0, 
        color: 'text.primary' // Use theme default text color
      }}
    >
      {/* Header Section */}
      <Box sx={{ 
          // *** ENSURE HEADER AREA RESPECTS THEME ***
          p: isMobile ? 1.5 : 0, // Keep padding adjustments
          borderBottom: 1, 
          borderColor: 'divider', // Use theme divider color
          bgcolor: theme.palette.mode === 'dark' ? alpha(theme.palette.background.paper, 0.6) : 'background.paper', // Subtle paper background for header container
          flexShrink: 0 
        }}>
        {renderHeader()}
      </Box>
      ...
      <Box sx={{ // Corrected Structure Start
          display: 'flex', 
          flexGrow: 1, 
          overflow: 'hidden' 
        }}>
        ...
        {!isMobile && (
          <Tabs
            ...
            sx={{ // *** ADD DARK MODE FOR VERTICAL TABS ***
              borderRight: 1,
              borderColor: 'divider',
              minWidth: 160, 
              bgcolor: 'background.paper', // Use theme paper background
              flexShrink: 0,
               '& .MuiTab-root': {
                    ...
                    '&.Mui-selected': {
                      ...
                      color: 'primary.main', // Keep primary color for selected
                      bgcolor: alpha(theme.palette.primary.main, 0.1) // Subtle background for selected
                    },
                    '&:hover': {
                      backgroundColor: alpha(theme.palette.text.primary, 0.05), // Subtle hover for both modes
                      color: 'text.primary'
                    },
                  },
                  '& .MuiTabs-indicator': {
                    ...
                    bgcolor: 'primary.main' // Keep primary color for indicator
                  },
            }}
          >
        ...
        <Box
          ...
          sx={{ // *** ENSURE CONTENT PANEL RESPECTS THEME ***
            flexGrow: 1, 
            overflowY: 'auto', 
            p: isMobile ? 1.5 : 2, 
            width: '100%', 
            minWidth: 0, 
            bgcolor: 'background.default', // Use theme default background for content
            color: 'text.primary' // Ensure text color inherits correctly
          }}
        >
      ...
      </Box> { /* End Main Content Area Box - Corrected Structure End */}
      ...
      {isMobile && (
        <Paper 
          ...
          sx={{ // *** ADD DARK MODE FOR HORIZONTAL TABS ***
            borderTop: 1, 
            borderColor: 'divider', 
            flexShrink: 0, 
            bgcolor: 'background.paper', // Use theme paper background
        ...
        >
          <Tabs
            ...
            sx={{ 
              ...
              // Style scroll buttons for dark mode if needed
