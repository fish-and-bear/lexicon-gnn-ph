...
const WordExplorer: React.FC = () => {
  ...
  const fetchRandomWordsForCache = useCallback(async (count: number = RANDOM_CACHE_SIZE): Promise<any[]> => {
    if (isRefreshingCache || !isMountedRef.current) return randomWordCache;
    ...
    try {
      ...
      if (!isMountedRef.current) return randomWordCache; // Check after async
      if (validResults.length > 0) {
        const currentCache = randomWordCacheRef.current; // Use ref for latest cache
        const existingLemmas = new Set(currentCache.map(w => w?.lemma?.toLowerCase()));
        const uniqueNewWords = validResults.filter(w => !existingLemmas.has(w.lemma.toLowerCase()));
        const finalCache = shuffleArray([...currentCache, ...uniqueNewWords]).slice(0, RANDOM_CACHE_SIZE * 3);
      ...
      if (isMountedRef.current) setIsRefreshingCache(false);
    }
  }, [isRefreshingCache]); // Removed randomWordCache dependency, use ref instead
  ...
  useEffect(() => {
    ...
    let cachePrefetchTimerId: NodeJS.Timeout | null = null;
    ...
    const initialLoadSequence = async () => {
      try {
        ...
        if (connected) {
          ...
          // Call the useCallback hooks directly
          ...
          // Use ref to get current cache length
          if (isMountedRef.current && randomWordCacheRef.current.length === 0) {
            console.log("Initial random cache empty, scheduling prefetch...");
            cachePrefetchTimerId = setTimeout(() => {
          ...
          if (isMountedRef.current) setError("Failed to connect to backend API. Ensure server is running.");
        }
      } catch (error) {
        // This catch might not be strictly necessary with Promise.allSettled
        // if the individual functions handle their errors, but good for safety.
        if (isMounted) {
          console.error("Error during initial load sequence wrapper:", error);
    ...
    const initialLoadTimerId = setTimeout(initialLoadSequence, 500);

    // Cleanup function
    return () => {
      isMounted = false;
      clearTimeout(initialLoadTimerId);
      if (cachePrefetchTimerId) {
        clearTimeout(cachePrefetchTimerId);
      }
    };

  // Dependencies: Include stable callbacks and state length used in conditional logic.
  }, [fetchPartsOfSpeech, fetchStatistics, fetchRandomWordsForCache, randomWordCache.length]);
