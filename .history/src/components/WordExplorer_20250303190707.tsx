...
const WordExplorer: React.FC = () => {
  ...
  const fetchWordNetworkData = useCallback(async (word: string, depth: number = 2, breadth: number = 10) => {
    try {
      ...
      // Process nodes and edges more efficiently
      const networkData: WordNetworkGraph = {
        word,
        nodes: [],
        edges: [],
        ...
        metadata: {
          ...
          cluster_count: Object.keys(data.clusters || {}).length
        }
      };

      // Pre-process nodes in a single pass and limit by breadth
      const nodeMap = new Map();
      if (Array.isArray(data.nodes)) {
        // Sort nodes by importance (main word first, then root words, then others)
        ...
        // Take only the first breadth * depth nodes
        const limitedNodes = sortedNodes.slice(0, breadth * depth);
        
        networkData.nodes = limitedNodes.map(node => {
          const processedNode = {
            id: typeof node.id === 'number' ? node.id : parseInt(String(node.id)) || Date.now(),
      ...
      // Process edges using the nodeMap and only include edges between included nodes
      ...
      // Add main word node if not present
      if (!nodeMap.has(word) && !networkData.nodes.find(n => n.word === word)) {
        const mainNode = {
          id: Date.now(),
          word: word,
          normalized_lemma: word.toLowerCase(),
          language: 'tl',
          has_baybayin: false,
          baybayin_form: null,
          type: 'main',
        ...
        networkData.nodes.push(mainNode);
      }
