        ...
        try {
          ...
          fetchWordNetworkData(wordData.lemma, depth, breadth)
            .then(networkData => {
              ...
              // *** ADD LOGGING HERE ***
              console.log('[DEBUG] Checking networkData contents:');
              console.log('[DEBUG] Network Nodes:', networkData?.nodes);
              console.log('[DEBUG] Network Edges/Links:', networkData?.edges);
              console.log(`[DEBUG] Has Nodes: ${!!networkData?.nodes?.length}, Has Edges: ${!!networkData?.edges?.length}`);
              // *** END LOGGING ***
              
              // Set wordNetwork state
              setWordNetwork(networkData);
              
              // CRITICAL FIX: Update selectedWordInfo with semantic_network if direct relations are missing
              // This matches what handleRandomWord does
              if (!wordData.outgoing_relations?.length && !wordData.incoming_relations?.length) {
                console.log('[DEBUG] No direct relations found, adding semantic network to selectedWordInfo');
                setSelectedWordInfo(prevInfo => {
                  if (!prevInfo) return prevInfo;
                  return {
                    ...prevInfo,
                ...
                console.log('[DEBUG] Direct relations exist, not adding semantic network');
              }
            })
            .catch(networkErr => {
