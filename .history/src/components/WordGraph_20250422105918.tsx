...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  useEffect(() => {
    ...
    if (!isMobile) {
      ...
      const initialLegendWidth = 240;
      
      // Create legend container with initial width estimate
      ...
      const toggleHeight = 30; // Approx height for checkbox row
      const legendContentHeight = (totalRows * legendItemHeight) + 
                                ((legendCategories.length - 1) * categorySpacing);
      const legendHeight = legendHeaderHeight + legendContentHeight + toggleHeight + (legendPadding * 2);
      ...
      yPos += toggleHeight + categorySpacing; // Add space before the checkbox
      
      const checkboxGroup = legendContainer.append("g")
        .attr("transform", `translate(${legendPadding}, ${yPos})`)
        .attr("class", "legend-toggle-item")
        .style("cursor", "pointer")
        .on("click", () => {
          setShowDisconnectedNodes(prev => !prev);
          // Force re-render / update visual state if needed
          // Note: Graph rebuild is handled by the effect watching showDisconnectedNodes
        });
        
      checkboxGroup.append("rect") // Background hover/click area
        .attr("x", -5)
        .attr("y", -15)
        .attr("width", legendWidth - (legendPadding * 2))
        .attr("height", 24)
        ...
        .on("mouseover", function() { d3.select(this).attr("fill", theme === "dark" ? "rgba(255, 255, 255, 0.05)" : "rgba(0, 0, 0, 0.03)"); })
        .on("mouseout", function() { d3.select(this).attr("fill", "transparent"); });

      // Checkbox visual representation (could be improved with SVG paths)
      checkboxGroup.append("rect")
        ...
        .attr("height", 12)
        .attr("rx", 2)
        .attr("fill", showDisconnectedNodes ? (muiTheme.palette.mode === "dark" ? muiTheme.palette.primary.dark : muiTheme.palette.primary.main) : (muiTheme.palette.mode === "dark" ? "rgba(255,255,255,0.2)" : "rgba(0,0,0,0.1)")) // Use muiTheme
        .attr("stroke", theme === "dark" ? "rgba(255,255,255,0.4)" : "rgba(0,0,0,0.3)") // theme string is OK here
        .attr("stroke-width", 1);
        
      // Add checkmark if checked
      if (showDisconnectedNodes) {
        checkboxGroup.append("path")
          .attr("d", "M3,6 L5,9 L9,3") // Simple checkmark path
          .attr("fill", "none")
          .attr("stroke", muiTheme.palette.mode === "dark" ? muiTheme.palette.getContrastText(muiTheme.palette.primary.dark) : muiTheme.palette.primary.contrastText) // Use muiTheme
          .attr("stroke-width", 1.5)
          .attr("transform", "translate(0, -1)"); // Adjust position slightly
      }
