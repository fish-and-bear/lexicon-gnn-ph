...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  useEffect(() => {
    ...
    const dragstarted = (event: d3.D3DragEvent<SVGCircleElement, CustomNode, any>, d: CustomNode) => {
      ...
      d.fx = d.x;
      d.fy = d.y;
      // Cast event.sourceEvent.target to SVGCircleElement
      d3.select(event.sourceEvent.target as SVGCircleElement).raise().attr("stroke", "black"); 
    };
    ...
    const dragended = (event: d3.D3DragEvent<SVGCircleElement, CustomNode, any>, d: CustomNode) => {
      ...
      // Cast event.sourceEvent.target to SVGCircleElement
      d3.select(event.sourceEvent.target as SVGCircleElement).attr("stroke", d.pinned ? "#ff0000" : (theme === 'dark' ? "#fff" : "#333"));
