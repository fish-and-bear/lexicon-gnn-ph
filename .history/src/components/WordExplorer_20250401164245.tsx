  ...
  const handleSearch = useCallback(async (searchWord?: string) => {
    ...
    try {
      ...
      if (searchResults && searchResults.words && searchResults.words.length > 0) {
        ...
        const wordData = await fetchWordDetails(firstResult.lemma); // Use .lemma
        ...
        if (wordData && wordData.lemma) {
          ...
          console.log('Fetching word network and etymology tree for search result:', wordData.lemma);
           // Fetch word network and etymology tree in parallel - USE wordData.id
          const [networkData] = await Promise.all([
            fetchWordNetworkData(wordData.lemma, depth, breadth), // Use lemma for network
            fetchEtymologyTree(wordData.id) // Use word ID for etymology tree
          ]);
        ...
        <div ref={detailsContainerRef} className="details-container">
          {selectedWordInfo && !isLoading && (
            <WordDetails
              ...
              toggleMetadata={(value) => setShowMetadata(value)}
