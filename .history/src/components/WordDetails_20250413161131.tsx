    ...
    const renderRelationGroup = (title: string, relations: WordInfo['outgoing_relations'] | WordInfo['incoming_relations'], direction: 'in' | 'out') => {
      ...
      if (!hasValidStructure) {
        ...
        const sanitizedRelations = relations.map((rel) => {
          ...
          // Ensure source_word and target_word exist
          if (direction === 'in' && !fixed.source_word) {
            fixed.source_word = { id: 'unknown', word: 'Unknown', definitions: [] };
          } else if (direction === 'out' && !fixed.target_word) {
            fixed.target_word = { id: 'unknown', word: 'Unknown', definitions: [] };
          }
        ...
        return (
          <div className="relation-group" key={title}>
            <Typography variant="subtitle1" className="relation-title">{title}</Typography>
            <div className="relations-list">
              {sanitizedRelations.map((anyRel, index: number) => {
                const wordObj = direction === 'in' ? anyRel.source_word : anyRel.target_word;
                const relType = anyRel.relation_type;
                const relColor = relationColors[relType] || relationColors.other;
                const isDark = !isColorLight(relColor);
                
                return (
                  <Chip
                    key={`${anyRel.id || index}-${wordObj?.id || index}`}
                    label={(
                      <>
                        <span style={{ fontWeight: 'bold', marginRight: '4px' }}>
                          {formatRelationType(relType)}:
                        </span>
                        {wordObj?.word || 'Unknown'}
                      </>
                    )}
                    ...
                    onClick={() => onWordLinkClick(wordObj.word)}
                  />
                );
              })}
            </div>
      ...
      // Default rendering when relations data is valid
      return (
        <div className="relation-group" key={title}>
          <Typography variant="subtitle1" className="relation-title">{title}</Typography>
          <div className="relations-list">
            {relations.map((rel, index: number) => {
              // Properly type the relation
              const anyRel = rel as AnyRecord;
              const wordObj = direction === 'in' ? anyRel.source_word : anyRel.target_word;
              const relType = anyRel.relation_type;
              const relColor = relationColors[relType] || relationColors.other;
