...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const filteredNodes = useMemo<CustomNode[]>(() => {
    ...
    // Collect all nodes within depth/breadth, regardless of filter
    const nodesConnectedByDepth = baseNodes.filter(node => connectedNodeIds.has(node.id));
    
    // ONLY apply filtering after collecting all connected nodes
    // This allows previously filtered nodes to reappear when unfiltered
    const result = nodesConnectedByDepth.filter(node => {
      // Always include the main word
      if (node.id === mainWord) return true;
      
      // If no filters active, include all connected nodes
      if (filteredRelationships.length === 0) return true;
      
      // Only filter out nodes whose group is in the filtered list
  ...
  const toggleRelationshipFilter = useCallback((relationshipType: string) => {
    ...
    console.log(`Toggling filter for: ${type}`);
    
    // Calculate next filter state BEFORE updating DOM
    const isCurrentlyFiltered = filteredRelationships.includes(type);
    const nextFilters = isCurrentlyFiltered 
      ? filteredRelationships.filter(r => r !== type) 
      : [...filteredRelationships, type];
    
    // First apply visual changes IMMEDIATELY
    if (svgRef.current) {
      const svg = d3.select(svgRef.current);
      
      // Manipulate node visibility directly
      svg.selectAll<SVGGElement, CustomNode>(".node")
        .filter(d => d.group.toLowerCase() === type)
        .each(function(d) {
          if (d.id === mainWord) return; // Never filter main word
          
          const node = d3.select(this);
          
          if (isCurrentlyFiltered) {
            // UNFILTERING: make nodes appear
            console.log(`Showing node ${d.id} of type ${type}`);
            ...
            svg.selectAll<SVGLineElement, CustomLink>(".link")
              .each(function(link) {
                ...
                // If this link connects to our node and other end is not filtered
                if ((source.id === d.id || target.id === d.id)) {
                  const otherNode = source.id === d.id ? target : source;
                  const otherNodeType = otherNode.group.toLowerCase();
                  
                  // Only show if other end isn't filtered or is main word
                  if (otherNode.id === mainWord || !nextFilters.includes(otherNodeType)) {
            ...
            // FILTERING: make nodes disappear 
            console.log(`Hiding node ${d.id} of type ${type}`);
            node.transition()
                .duration(300)
                .style("opacity", 0)
                .on("end", function() {
            ...
            svg.selectAll<SVGLineElement, CustomLink>(".link")
              .each(function(link) {
                ...
                // If either end is our node (but not the main word)
                if ((source.id === d.id && source.id !== mainWord) || 
                    (target.id === d.id && target.id !== mainWord)) {
                  linkElem.transition()
                        .duration(300)
                        .style("opacity", 0)
                        .on("end", function() {
    ...
    // Only update state AFTER visual changes are applied
    setFilteredRelationships(nextFilters);
  }, [filteredRelationships, mainWord]);
    ...
    <div className="graph-container">
      <div className="graph-svg-container">
        ...
        <svg 
          ...
          key={`graph-${forceUpdate}-filters-${Date.now()}`}
        >
        </svg>
      </div>
