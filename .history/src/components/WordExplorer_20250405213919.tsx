  ...
  const handleSearch = useCallback(async (wordToSearchParam?: string) => {
    ...
    try {
      ...
      try {
        ...
        console.log('Direct fetch successful:', wordData);
      ...
      const searchOptions: SearchOptions = { 
        ...
        const searchResult: SearchResult = await searchWords(searchOptions.q, searchOptions);
            ...
            fetchWordDetails(firstResult.id.toString())
              .then(fullWordData => {
                if (fullWordData && selectedWordInfo && fullWordData.id === selectedWordInfo.id) {
                  ...
                  ]).then(([networkData, etymologyData]) => {
                      if (networkData) {
  ...
  const debouncedSearch = useCallback(
    debounce(async (query: string) => {
      ...
      try {
        ...
        const searchResult: SearchResult = await searchWords(query, searchOptions);
        if (searchResult && searchResult.words && Array.isArray(searchResult.words)) {
