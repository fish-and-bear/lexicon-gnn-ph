...
import { useQuery, useQueryClient, QueryClient, QueryClientProvider } from 'react-query'; // Added QueryClient related imports
...
// Create a react-query client instance (outside component or in App)
// For simplicity here, we place it outside, but ideally it lives higher up
// const queryClientInstance = new QueryClient(); 
// ^^^^ If placing here, wrap export default in provider. Better in index.tsx

const WordExplorer: React.FC = () => {
  const [searchTerm, setSearchTerm] = useState<string>("");
  // State to hold the identifier (lemma or id:X) of the currently selected/searched word
  const [selectedWordIdentifier, setSelectedWordIdentifier] = useState<string | null>(null);
  ...
  // Note: Etymology tree fetching logic will be integrated with useQuery below
  ...
  const fetchWordNetworkData = useCallback(async (word: string, depth: number = 2, breadth: number = 10) => {
    try {
      ...
      // Set a longer timeout for network requests to accommodate slower connections
      const timeoutDuration = 15000; // 15 seconds
      const controller = new AbortController();
      const timeoutId = setTimeout(() => controller.abort(), timeoutDuration);
      
      // Create a fetchPromise with the abort controller signal
      ...
      try {
        ...
        // Handle timeout or abort specifically
        if (error.name === 'AbortError' || (typeof error.message === 'string' && error.message.includes('timeout'))) {
          ...
          // Create a minimal fallback network with just the main word
          // This allows relations UI to still work with direct relations if available
          const fallbackNetwork = {
            nodes: [{ 
              id: word.startsWith('id:') ? word : `id:${word}`, 
              label: word, 
              word: word,
              type: 'main' 
            }],
            links: [] // Use links here
          };
          
          // Don't throw here - return the fallback data
          console.log('Using fallback network data due to timeout');
      ...
      if (data && data.nodes && data.links) { // Use links here
        console.log('Word network data received:', data);
        console.log(`Network has ${data.nodes.length} nodes and ${data.links.length} links`); // Use links here
        ...
        if (wordData && wordData.id) {
          ...
          if (mainNode) {
            ...
            // Process each link to build incoming and outgoing relations
            data.links.forEach((edge: any) => { // Use links here, added type any for now
            ...
            console.log("Syncing relations (logic removed):", {
              incoming: incomingRelations.length,
              outgoing: outgoingRelations.length
            });
            // Removed setWordData update
          }
        }
      ...
      if (error instanceof Error) {
        // Don't show timeout errors to the user
      ...
      // Return a minimal network structure instead of throwing
      // This allows the UI to still function with some data
      return {
        nodes: [{ 
          id: `id:${word}`, 
          label: word, 
  ...
  // Refactor this to primarily update the selectedWordIdentifier state
  const handleNodeClick = useCallback(async (wordIdentifier: string) => {
    if (!wordIdentifier) {
      console.error("Empty identifier received in handleNodeClick");
      return;
    }
    console.log(`Node clicked, setting identifier: ${wordIdentifier}`);
    // Update input field only if it's not already the same (avoid unnecessary updates)
    if (inputValue !== wordIdentifier) {
        setInputValue(wordIdentifier.startsWith('id:') ? wordIdentifier : wordIdentifier); 
    }
    updateHistory(wordIdentifier); // Update history and trigger query refetch
  }, [updateHistory, inputValue]); // Added inputValue dependency

  // New function for handling node selection (single click) without navigation
  // Refactor: This can update the identifier to focus details without changing history
  const handleNodeSelect = useCallback(async (wordIdentifier: string) => {
    if (!wordIdentifier) {
      console.error("Empty word received in handleNodeSelect");
      return;
    }
 
    console.log(`Node selected: ${wordIdentifier}. Updating selected identifier.`);
    // Only update the identifier, react-query will fetch details
    setSelectedWordIdentifier(wordIdentifier);
  ...
  // This might not be needed anymore with react-query managing fetches
  useEffect(() => {
    handleNodeClickRef.current = handleNodeClick;
  }, [handleNodeClick]);

  // NOW DEFINE HANDLERS THAT USE FETCHERS
  // Refactor: This now only triggers the query via updateHistory
  const handleSearch = useCallback((query: string) => {
      ...
      setInputValue(query); // Update input field
      setSearchTerm(query); // Update search term state
      setShowSuggestions(false);
      updateHistory(query); // Trigger fetch by updating identifier/history with the lemma
  }, [updateHistory]);
  ...
  const handleResetCircuitBreaker = () => {
    ...
    // Clear query errors by potentially refetching or resetting query state
    // Invalidate queries related to the potentially failed state
    if (selectedWordIdentifier) {
        queryClient.invalidateQueries(['wordDetails', selectedWordIdentifier]);
        if (wordInfo?.lemma) {
            queryClient.invalidateQueries(['wordNetwork', wordInfo.lemma]);
        }
        if (wordInfo?.id) {
            queryClient.invalidateQueries(['etymologyTree', wordInfo.id]);
        }
    }
    // Add invalidation for other relevant queries if needed
  ...
  }, [isRefreshingCache, randomWordCache]); // Removed getRandomWord from deps
  ...
  // Refactor: This now primarily updates the identifier
  const handleRandomWord = useCallback(async (forceRefresh = false) => {
    ...
    try {
      ...
      // Use a simple flag reset, timeout logic removed as loading is handled by useQuery
...
const WordExplorer: React.FC = () => {
  ...
  // New function for handling node selection (single click) without navigation
  // This function might be redundant now, as details are fetched based on selectedWordIdentifier
      ...
      if (wordData) {
        ...
        if (!wordData.outgoing_relations?.length && !wordData.incoming_relations?.length) {
          try {
            ...
            // Use existing depth and breadth
  ...
  const handleSearch = useCallback(async (query: string) => {
    ...
    try {
      ...
      if (result.error) {
        ...
        setSearchError(result.error); // Use specific search error state
          ...
          if (hasMissingOrMinimalRelations && !wordData.semantic_network) {
            ...
            if (networkData && networkData.nodes && networkData.links) {
              ...
              console.log('[DEBUG] Enhanced word data with semantic network:', {
                ...
                edgeCount: networkData.links.length
              });
  ...
  useEffect(() => {
    ...
    testApiConnection().then(async connected => { // Make async
  ...
  }, [fetchRandomWordsForCache, handleRandomWord, selectedWordIdentifier]); // Added handleRandomWord and identifier
  ...
  const toggleMetadata = useCallback(() => {
    // This state was removed, handle visibility directly where needed or re-add state
    console.log("Toggle metadata clicked - state removed");
  }, []);
  ...
  const {
    data: wordInfo, // Renamed from selectedWordInfo
    isLoading: isLoadingDetails,
    error: detailsError,
    // refetch: refetchDetails // Can be used for manual refetching if needed
  } = useQuery<WordInfo, Error>(
    ...
    () => fetchWordDetails(selectedWordIdentifier!), // Fetch function
    {
      ...
      keepPreviousData: true, // Keep displaying old data while loading new
      onError: (err) => {
        console.error('Error fetching word details via useQuery:', err);
        // Error is captured in 'detailsError' state
      },
      onSuccess: (data) => {
          console.log('Successfully fetched word details via useQuery:', data?.lemma);
          // Pre-fetch network data if relations are minimal (or on hover/focus?)
  ...
  const mainWordLemma = wordInfo?.lemma; // Get lemma from successfully fetched details
  const {
      data: wordNetwork, 
      isLoading: isLoadingNetwork, 
      error: networkError, 
      // refetch: refetchNetwork
  } = useQuery<ApiWordNetwork, Error>(
    ['wordNetwork', mainWordLemma, depth, breadth], // Query key includes lemma, depth, breadth
  ...
  const {
      data: etymologyTree, 
      isLoading: isLoadingEtymology, 
      error: etymologyError, 
      // refetch: refetchEtymology
   } = useQuery<EtymologyTree, Error>(
  ...
  // Combined loading state derived from individual query states
  const isLoading = isLoadingDetails || (!!mainWordLemma && isLoadingNetwork) || (!!wordIdForEtymology && isLoadingEtymology);
  // Combined error state (can be made more specific if needed)
