...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const nodeClick = useCallback((event: PointerEvent, d: CustomNode) => {
      ...
      if (isDraggingRef.current) {
          isDraggingRef.current = false; // Reset drag flag on click end/cancel
          return; // Don't process click if it was part of a drag
      }
      ...
      if (now - lastClickTimeRef.current < DOUBLE_CLICK_DELAY) {
          ...
          d3.select(event.currentTarget as SVGGElement).select("circle")
            ...
            .attr("stroke", d.pinned ? "#ff00ff" : (d3.color(getNodeColor(d.group))?.darker(0.8).formatHex() ?? "#888")) // Use pin color or default outline
            .attr("stroke-width", d.pinned ? 2.5 : 1.5);
          setSelectedNodeId(d.id); // Select on double click too
      } else {
          // Single click: Call the prop handler and select node
          setSelectedNodeId(d.id);
          onNodeClick(d.id); // Call the passed handler with the word/id
      }
      lastClickTimeRef.current = now;
  }, [onNodeClick, getNodeColor]);
