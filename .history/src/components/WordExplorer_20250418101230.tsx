...
const WordExplorer: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Define fetchInitialData here so it has access to the component's scope
    const fetchInitialData = async () => {
      try {
        // Ensure we only update state if the component is still mounted
        if (!isMounted) return;
        
        // Get parts of speech and statistics (relatively quick calls)
        const posData = await getPartsOfSpeech();
        if (isMounted) setPartsOfSpeech(posData);
        
        // Fetch statistics (already uses useCallback)
        fetchStatistics(); // fetchStatistics is stable due to useCallback

        // --- MODIFIED: Less Aggressive Random Cache Prefetch ---
        // Check cache length directly from state
        if (isMounted && randomWordCache.length === 0) { 
          console.log("Initial random word cache is empty, prefetching a smaller batch later...");
          // Fetch a smaller initial cache AFTER other setup, with a further delay
          const cacheTimer = setTimeout(() => {
            if (isMounted) { // Check mount status again inside setTimeout
              fetchRandomWordsForCache(RANDOM_CACHE_SIZE); // Fetch only 1x cache size initially
            }
          }, 2000); // Wait 2 seconds after initial data fetch
          // We should clean up this timer as well if the component unmounts
        ...
        if (isMounted) {
          ...
          // Optionally set an error state here if needed for initial data fetch failure
          setError("Failed to fetch initial dictionary data."); 
        }
      }
    };

    // Defer the initial API check slightly to allow faster initial paint
    const connectionCheckTimer = setTimeout(() => {
      testApiConnection().then(connected => {
        ...
        if (connected) {
          // Now call fetchInitialData after connection is confirmed
          fetchInitialData();
        } else {
          console.error("API Connection Failed on Initial Load");
          setError("Failed to connect to the backend API on startup. Please ensure the server is running.");
        }
      }).catch(error => {
        if (isMounted) {
          console.error("API connection error during initial check:", error);
          setApiConnected(false);
          setError("Error connecting to the backend API. Please check the server status.");
        }
      });
    }, 500); // Wait 500ms before starting API checks
    ...
    return () => {
      ...
      if (cacheCleanup) {
        cacheCleanup(); // Clear the cache timer if it was set
      }
    };
  // Correct dependencies: Include functions called and state values read directly.
  // State setters (like setPartsOfSpeech, setError, setApiConnected) don't need to be dependencies.
  }, [getPartsOfSpeech, fetchStatistics, fetchRandomWordsForCache, randomWordCache.length]);
