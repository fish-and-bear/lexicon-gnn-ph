...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({
 ...
 const handleTabChange = (event: React.SyntheticEvent, newValue: string) => {
  ...
  // UPDATED renderRelationsTab -> renderRelationsAffixesTab
  const renderRelationsAffixesTab = () => {
    ...
    function createRelationsFromNetwork() {
        console.log("Creating relations from semantic network as fallback");
         ...
         semanticNetworkLinks.forEach(link => {
           const targetNode = semanticNetworkNodes.find(n => n.id === (typeof link.target === 'object' ? link.target.id : link.target));
           ...
           relations.push({
             ...
             degree: degree,
             wordObj: { id: targetNode.id, lemma: targetNode.label || targetNode.word || String(targetNode.id), has_baybayin: targetNode.has_baybayin || false, baybayin_form: targetNode.baybayin_form || null }
           });
         });
    ...
    // Check if there are any standard relations or fallback relations OR affixations
    const hasStandardRelations = (incoming_relations.length > 0 || outgoing_relations.length > 0) && !hasError;
    const hasAffixations = (wordInfo.root_affixations && wordInfo.root_affixations.length > 0) || (wordInfo.affixed_affixations && wordInfo.affixed_affixations.length > 0);
    const hasAnyData = hasStandardRelations || semanticRelations.length > 0 || hasAffixations;
              ...
              <Box>
                ...
                <List dense disablePadding sx={{ pl: 1 }}>
                  {rootAffix.map((affix: Affixation, index: number) => (
                    <ListItem key={`root-${affix.id || index}`} disableGutters sx={{ py: 0.25 }}>
                      <ListItemText
    ...
    // Helper function to render Relations Categories
    const renderRelationCategories = () => {
        ...
        const relationsWithDegree = allRelations.map(rel => ({
            ...rel,
            wordObj: rel.source_word || rel.target_word || rel.wordObj,
            degree: rel.degree || 1
        }));
        const filteredRelations = relationsWithDegree.filter(rel => rel.wordObj?.lemma !== wordInfo.lemma);
        ...
        if (nonEmptyCategories.length === 0) return null; // Return null if no relations to show

        return (
            <>
                {/* Elegant relationship summary */}
                 <Box sx={{ mb: theme.spacing(3), pb: theme.spacing(1), display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: `1px solid ${alpha(theme.palette.divider, 0.6)}` }}>
                   <Typography variant="subtitle1" component="h3" sx={{ fontWeight: 600, color: graphColors.main }}>
                     Related Words ({uniqueRelatedWords})
                   </Typography>
                   {/* Add Filters/Controls Here Later if needed */}
                 </Box>

                {/* Render Relation Categories */}
                {nonEmptyCategories.map((category) => (
                  <StyledAccordion key={category.name} defaultExpanded>
                    <StyledAccordionSummary expandIcon={<ExpandMoreIcon />}>
                      <Typography variant="subtitle2" sx={{ fontWeight: 500 }}>{category.name}</Typography>
                    </StyledAccordionSummary>
                    <StyledAccordionDetails>
                      <List dense disablePadding>
                        {categorizedWords[category.name]
                          .sort((a, b) => a.degree - b.degree || a.wordObj?.lemma.localeCompare(b.wordObj?.lemma)) // Sort by degree, then lemma
                          .map((relation, index) => (
                            <ListItem key={relation.id || `${category.name}-${index}`} disableGutters sx={{ py: 0.25 }}>
                              <ListItemText
                                primary={relation.wordObj?.lemma || 'Unknown'}
                                secondary={`(${relation.primaryType}${relation.degree > 1 ? ` - deg ${relation.degree}` : ''})`}
                                primaryTypographyProps={{
                                  variant: 'body2',
                                  sx: { cursor: 'pointer', textDecoration: 'underline', color: theme.palette.primary.main, mr: 1 },
                                  onClick: () => relation.wordObj && onWordLinkClick(relation.wordObj.lemma)
                                }}
                                secondaryTypographyProps={{ variant: 'caption', color: 'text.secondary' }}
                                sx={{ m: 0 }}
                              />
                              {/* Optionally show Baybayin chip */}
                               {relation.wordObj?.has_baybayin && relation.wordObj?.baybayin_form && (
                                 <Chip label={relation.wordObj.baybayin_form} size="small" variant="outlined" sx={{ height: 'auto', fontSize: '0.7rem', fontFamily: '"Noto Sans Baybayin", sans-serif' }}/>
                               )}
                            </ListItem>
                        ))}
                      </List>
    ...
    return (
      <Box sx={{ pt: theme.spacing(1) }}>
        {/* Show error banner if using fallback */}
        {hasError && (
            ...
            const tabContent = (
               <Box sx={{ flexGrow: 1, overflowY: 'auto', position: 'relative', bgcolor: 'background.default' }}>
                  <Box sx={{ p: theme.spacing(3) }}>
                      ...
                      {activeTab === 'relations' && renderRelationsAffixesTab()}
    ...
    >
      ...
      <Box sx={{ flexGrow: 1, display: 'flex', flexDirection: isWideScreen ? 'row' : 'column', overflow: 'hidden' }}>
        ...
        <Box sx={{ flexGrow: 1, overflowY: 'auto', p: theme.spacing(2) }}>
          ...
          {activeTab === 'definitions' && renderDefinitionsTab()}
          {activeTab === 'relations-affixes' && renderRelationsAffixesTab()} {/* Needs update */}
