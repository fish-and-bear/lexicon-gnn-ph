...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const getNodeRelation = useCallback((word: string, node: NetworkNode): string => {
    ...
    const calculatedGroup = "associated"; // Default
    // --- DEBUG LOG --- 
    console.log(`[getNodeRelation] Word: ${word}, Node Type: ${node.type}, Node Path: ${JSON.stringify(node.path)}, Calculated Group: ${calculatedGroup}`);
    // --- DEBUG LOG --- 
  ...
  const nodes: CustomNode[] = useMemo(() => {
    console.log("[WordGraph] Raw wordNetwork.nodes:", wordNetwork?.nodes); // Log raw nodes
    if (!wordNetwork || !wordNetwork.nodes || !Array.isArray(wordNetwork.nodes)) {
      return [];
    }
    ...
    return wordNetwork.nodes.map(node => ({
      ...
      word: node.label,
      label: node.label,
      group: getNodeRelation(node.label, node),
      info: node,
      connections: 0,
      pinned: false
    }));
