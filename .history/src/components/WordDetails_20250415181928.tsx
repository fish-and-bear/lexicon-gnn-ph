...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({
  ...
  const renderDefinitionsTab = () => {
    if (!wordInfo.definitions || wordInfo.definitions.length === 0) {
      // If no definitions, still render source/metadata info if available
      ...
      if (sourceMetadataContent) {
        return (
          <Box sx={{ pt: theme.spacing(1) }}>
            ...
            {/* Render metadata even if no definitions */}
      ...
      return <Alert severity="info">No definitions or metadata available for this word.</Alert>;
    }
    ...
    return (
      <Box sx={{ pt: theme.spacing(1) }}>
        ...
        {/* Append Source/Metadata Info at the end of the Definitions tab */}
        <Box sx={{ mt: 3, pt: 2, borderTop: `1px solid ${theme.palette.divider}` }}>
          {renderSourcesInfoTab()} 
        </Box>
      </Box>
    );
  };
