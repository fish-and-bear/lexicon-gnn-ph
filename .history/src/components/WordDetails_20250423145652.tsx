...
import './WordDetails.css'; // Ensure CSS is imported
...
import { styled, alpha } from '@mui/material/styles'; // Keep styled and alpha if needed elsewhere, remove Theme
import useMediaQuery from '@mui/material/useMediaQuery'; // Keep if needed for responsiveness
...
const StyledAccordion = styled(Accordion)(({ theme }) => ({ // Remove Theme type if not used
  border: 'none', // Remove explicit border
  boxShadow: 'none', // Remove default shadow
  backgroundColor: 'transparent',
  '&:not(:last-child)': {
    borderBottom: `1px solid rgba(0, 0, 0, 0.08)`, // Revert to original or use CSS var
  },
  '&::before': {
...
const StyledAccordionSummary = styled(AccordionSummary)(({ theme }) => ({ // Remove Theme type if not used
  padding: theme.spacing(0, 1), // Adjust padding - Keep theme.spacing for now, or replace with fixed value
  minHeight: 48,
  backgroundColor: 'rgba(0, 0, 0, .02)', // Revert to original or use CSS var
  '&:hover': {
    backgroundColor: 'rgba(0, 0, 0, .03)', // Revert to original or use CSS var
  },
  '& .MuiAccordionSummary-content': {
    margin: theme.spacing(1.5, 0), // Adjust margin - Keep theme.spacing or replace
...
const StyledAccordionDetails = styled(AccordionDetails)(({ theme }) => ({ // Remove Theme type if not used
  padding: theme.spacing(2, 2, 2, 2), // Consistent padding - Keep theme.spacing or replace
  borderTop: 'none', // Remove internal border
  backgroundColor: 'transparent', // Revert to original or use CSS var
}));
...
const WordDetailsComponent = React.forwardRef<HTMLDivElement, WordDetailsProps>((
  ...
  const theme = useTheme(); // REMOVE useTheme()
  const isDarkMode = theme.palette.mode === 'dark'; // REMOVE isDarkMode based on theme
  const isDarkMode = false; // TODO: Replace with actual dark mode state if needed outside MUI theme
  ...
  const renderHeader = () => {
    ...
    // Use hardcoded graph color or CSS var
    const mainColor = graphColors.main; 
    
    // Determine text color based on background - revert to original logic or use CSS vars
    const headerTextColor = isDarkMode 
      ? '#ffffff' // Example hardcoded dark mode text
      : '#000000'; // Example hardcoded light mode text

    return (
      // More elegant header with subtle styling
      <Box sx={{ 
        bgcolor: isDarkMode ? 'rgba(30, 40, 60, 0.4)' : headerBgColor, // Revert to original or CSS Var
        ...
        borderBottom: `1px solid ${isDarkMode ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)'}`, // Revert or use CSS Var
        ...
        <Stack direction="row" spacing={1} alignItems="flex-start" flexWrap="nowrap" sx={{ mb: theme.spacing(0) }}>
          ...
          <Typography 
            ...
            sx={{ 
              ...
              color: headerTextColor, // Use calculated header text color
          ...
          {ipaPronunciation && (
            <Typography 
              ...
              sx={{ 
                ...
                color: alpha(headerTextColor, 0.85),
          ...
          {wordData.language_code && (
            <Box> 
              <Typography 
                variant="caption" 
                sx={{ 
                  color: alpha(headerTextColor, 0.7) : alpha(mainColor, 0.8),
                  fontWeight: 500,
                  display: 'block', 
          ...
          {wordData.romanized_form && (
            <Box> // REMOVED COMMENT
              <Typography 
                variant="caption" 
                sx={{ 
                  color: alpha(headerTextColor, 0.7) : alpha(theme.palette.text.primary, 0.7),
                  fontWeight: 500,
                  display: 'block', 
                  mb: 0.25, // Reduced margin
                  fontSize: isMobile ? '0.7rem' : '0.75rem'
            ...
            {tags.map((tag) => (
              <Chip
                ...
                sx={{
                  ...
                  borderColor: alpha(headerTextColor, 0.3),
                  bgcolor: alpha(headerTextColor, 0.07),
