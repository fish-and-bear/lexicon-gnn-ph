...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const setupSimulation = useCallback((nodes: CustomNode[], links: CustomLink[]) => {
      simulationRef.current = d3.forceSimulation<CustomNode>(nodes)
        ...
        .force("link", d3.forceLink<CustomNode, CustomLink>()
          ...
          .distance(90)
          .strength(0.5))
        .force("charge", d3.forceManyBody<CustomNode>().strength(-150).distanceMax(250))
        .force("collide", d3.forceCollide<CustomNode>().radius(d => getNodeRadius(d) + 4).strength(0.8))
  ...
  const createLinks = useCallback((g: d3.Selection<SVGGElement, unknown, null, undefined>, linksData: CustomLink[]) => {
      ...
      return g.append("g")
        ...
        .attr("class", "link")
        .attr("stroke", d => {
            // Handle target being string ID or node object
            const targetId = typeof d.target === 'object' && d.target !== null ? (d.target as CustomNode).id : d.target as string;
            const targetNode = nodeMap.get(targetId);
            // Use target node's group color directly, with fallback
            return targetNode ? getNodeColor(targetNode.group) : '#999';
         })
        .attr("stroke-opacity", d => (d.relationship === 'associated') ? 0.4 : 0.7)
        .attr("stroke-width", d => (d.relationship === 'associated') ? 1 : 1.5)
        .attr("stroke-dasharray", d => (d.relationship === 'etymology') ? "2,2" : "none")
        .attr("marker-end", d => {
             const rel = d.relationship.toLowerCase();
             return (rel === 'derived' || rel === 'derived_from' || rel === 'root_of' || rel === 'etymology' || rel === 'taxonomic' || rel === 'part_whole') ? "url(#arrowhead)" : null;
         });
  ...
  const createNodes = useCallback((
      ...
      nodeGroups.append("circle")
        ...
        .attr("stroke", theme === "dark" ? "rgba(255,255,255,0.5)" : "rgba(0,0,0,0.3)")
      ...
      nodeGroups.append("text")
        ...
        .text(d => d.word)
          ...
          .lower()
          .attr("fill", "none")
          .attr("stroke", theme === "dark" ? "#111" : "#fff")
          .attr("stroke-width", 3)
          .attr("stroke-linejoin", "round");

       nodeGroups.select("text:not([stroke])")
          .attr("fill", theme === "dark" ? "#eee" : "#111")
       ...
       nodeSelection
        .on("click", (event, d) => {
          ...
          d3.selectAll(".node").classed("selected connected", false)
            .transition().duration(150).style("opacity", 0.7)
            .select("circle").attr("r", (n: unknown) => getNodeRadius(n as CustomNode));
          d3.selectAll(".link").classed("highlighted", false)
            .transition().duration(150).attr("stroke-opacity", 0.3).attr("stroke-width", 1.5);

          const targetNode = d3.select(event.currentTarget as Element);
          targetNode.classed("selected", true).style("opacity", 1)
            .raise()
            .transition().duration(150)
            .select("circle").attr("r", getNodeRadius(d) * 1.2);

          const connectedLinkSelection = d3.selectAll<SVGLineElement, CustomLink>(".link")
            .filter(l => l.source === d.id || l.target === d.id);

          console.log(`[Node Click] Found ${connectedLinkSelection.size()} links connected to ${d.id}`);

          connectedLinkSelection
            .classed("highlighted", true)
            .raise();

          const connectedIds = new Set<string>();
          connectedLinkSelection.each(l => {
          ...
          console.log(`[Node Click] Simulation alpha after click handler: ${simulationRef.current?.alpha()}`);
        })
        .on("mouseover", (event, d) => {
            if (isDraggingRef.current) return;
            const nodeElement = d3.select(event.currentTarget as Element);
            nodeElement.select("circle").transition().duration(100).attr("r", getNodeRadius(d) * 1.1);
            nodeElement.raise();
            setHoveredNode({ ...d });
        })
        .on("mouseout", (event, d) => {
        ...
        .on("dblclick", (event, d) => {
             ...
             d3.select(event.currentTarget as Element).select('circle')
                 ...
                 .attr("stroke-dasharray", d.pinned ? "3,2" : "none")
                 .attr("stroke", d.pinned ? getNodeColor(d.group) : (theme === "dark" ? "#DDD" : "#333"));
        });
  ...
  const centerOnMainWord = useCallback((svg: d3.Selection<SVGSVGElement, unknown, null, undefined>, nodesToSearch: CustomNode[]) => {
      ...
      if (mainNodeData && mainNodeData.x !== undefined && mainNodeData.y !== undefined) {
        ...
        const targetScale = currentTransform.k;
  ...
  const addGridBackground = useCallback((g: d3.Selection<SVGGElement, unknown, null, undefined>, width: number, height: number) => {
    const gridSize = 40;
    const lineColor = theme === 'dark' ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.04)';
    const grid = g.insert("g", ":first-child").attr("class", "grid-background");
    const xOffset = width / 2 % gridSize;
    const yOffset = height / 2 % gridSize;
    for (let x = -width; x <= width; x += gridSize) {
      grid.append("line").attr("x1", x - xOffset).attr("y1", -height).attr("x2", x - xOffset).attr("y2", height).attr("stroke", lineColor).attr("stroke-width", 0.5);
    }
    for (let y = -height; y <= height; y += gridSize) {
      grid.append("line").attr("x1", -width).attr("y1", y - yOffset).attr("x2", width).attr("y2", y - yOffset).attr("stroke", lineColor).attr("stroke-width", 0.5);
    }
  }, [theme]);
  ...
  useEffect(() => {
    ...
    // Filter links based on the CURRENT filteredNodes
    console.log("[Graph Effect] Base links count:", baseLinks.length);
    const filteredNodeIds = new Set(filteredNodes.map(n => n.id));
    console.log("[Graph Effect] Filtered node IDs:", Array.from(filteredNodeIds)); // Log the IDs being filtered by
    ...
    const currentFilteredLinks = baseLinks.filter(link => {
      // Handle links where source/target might be string ID or node object
  ...
  const renderTooltip = useCallback(() => {
    if (!hoveredNode?.x || !hoveredNode?.y || !svgRef.current) return null;
