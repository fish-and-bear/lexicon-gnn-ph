...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const [showDisconnectedNodes, setShowDisconnectedNodes] = useState<boolean>(false); // Add state for disconnected nodes toggle
  ...
  // Modify the filteredNodes useMemo to respect the showDisconnectedNodes setting
  const { filteredNodes, filteredLinks } = useMemo(() => {
    ...
    const depthLimitedNodes = showDisconnectedNodes 
      ? baseNodes // If showing disconnected nodes, use all base nodes
      : baseNodes.filter(node => connectedNodeIds.has(node.id)); // Otherwise, filter by connectivity
    
    // Find links where both source and target are in our node set
    const depthLimitedLinks = baseLinks.filter(link => {
      ...
      // If showing disconnected nodes, we keep all links where both endpoints exist
      if (showDisconnectedNodes) {
        const sourceExists = baseNodes.some(n => n.id === sourceId);
        const targetExists = baseNodes.some(n => n.id === targetId);
        return sourceExists && targetExists;
      }
      
      // Otherwise, only keep links where both endpoints are in our connected set
  ...
  useEffect(() => {
    ...
    // Add "Show disconnected nodes" checkbox at the bottom of the legend
    yPos += legendItemHeight + 10; // Add extra spacing before the option
    
    // Add another subtle divider line before the checkbox
    legendContainer.append("line")
      .attr("x1", legendPadding)
      .attr("y1", yPos - 5)
      .attr("x2", legendWidth - legendPadding)
    ...
    // Create a foreign object to contain the HTML checkbox
    const checkboxContainer = legendContainer.append("foreignObject")
      .attr("x", legendPadding)
      .attr("y", yPos)
      .attr("width", legendWidth - (legendPadding * 2))
      .attr("height", 30);
    ...
    const checkboxDiv = checkboxContainer.append("xhtml:div")
      ...
      .on("click", function() {
        ...
        const newState = !showDisconnectedNodes;
        setShowDisconnectedNodes(newState);
        // Force graph rebuild when setting changes
        setForceUpdate(prev => prev + 1);
        
        // Update the checkbox visually
        d3.select(this).select("input").property("checked", newState);
      });
      
    // Add the checkbox input
    checkboxDiv.append("xhtml:input")
      .attr("type", "checkbox")
      .attr("id", "show-disconnected")
      .style("margin-right", "8px")
      .style("cursor", "pointer")
      .property("checked", showDisconnectedNodes);
      
    // Add the label
    checkboxDiv.append("xhtml:label")
      .attr("for", "show-disconnected")
      .style("cursor", "pointer")
      .text("Show disconnected nodes");
    
    // Adjust the legend height to accommodate the checkbox
    legendContainer.select("rect")
      .attr("height", legendHeight + 40); // Add extra height for checkbox
    ...
    <div className="graph-container">
      <div className="graph-svg-container">
        ...
        <svg 
          ...
          key={`graph-${mainWord}-${depth}-${breadth}-${filteredRelationships.join('.')}-${forceUpdate}-${filterUpdateKey}-${showDisconnectedNodes}`}
        >
        </svg>
      </div>
