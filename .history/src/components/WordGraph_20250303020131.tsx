...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const createDragBehavior = useCallback(() => {
    return d3.drag<SVGGElement, CustomNode>()
      ...
      .on('drag', (event, d) => {
        ...
        // Update the connected links for the dragged node for more immediate feedback
        d3.select(svgRef.current)
          .selectAll<SVGLineElement, CustomLink>('.link')
          .filter((link: any) => {
          ...
          .attr('x1', (link: any) => {
            const source = typeof link.source === 'object' ? link.source.id : link.source;
            return source === d.id ? event.x : (typeof link.source === 'object' ? link.source.x : 0);
          })
          .attr('y1', (link: any) => {
            const source = typeof link.source === 'object' ? link.source.id : link.source;
            return source === d.id ? event.y : (typeof link.source === 'object' ? link.source.y : 0);
          })
          .attr('x2', (link: any) => {
            const target = typeof link.target === 'object' ? link.target.id : link.target;
            return target === d.id ? event.x : (typeof link.target === 'object' ? link.target.x : 0);
          })
          .attr('y2', (link: any) => {
            const target = typeof link.target === 'object' ? link.target.id : link.target;
            return target === d.id ? event.y : (typeof link.target === 'object' ? link.target.y : 0);
          });
      })
      .on('end', (event, d) => {
  ...
  }, [simulationObject, selectedNodeId]);
