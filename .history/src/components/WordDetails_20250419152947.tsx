...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({
  ...
  const renderRelationsTab = () => {
    ...
    useEffect(() => {
      // Check if we need to use semantic network as fallback
      if (
        (!wordInfo.relations || 
         (wordInfo.relations.incoming.length === 0 && 
          wordInfo.relations.outgoing.length === 0)) && 
        wordInfo?.semantic_network
      ) {
        console.log('Using semantic network as fallback for relations');
    ...
    if (etymologyError) {
      return <ErrorMessage message={etymologyError} />;
    }

    if (!wordInfo) {
      return <NoDataMessage>No word data available</NoDataMessage>;
    }

    // Create relation objects from semantic network data or use standard relations
    let relations: RelationsByType = { incoming: {}, outgoing: {} };

    if (usingFallback && wordInfo.semantic_network) {
      console.log('Creating relations from semantic network', wordInfo.semantic_network);
      // Convert semantic network to relations format
      Object.entries(wordInfo.semantic_network).forEach(([relationType, words]) => {
        if (!words || !Array.isArray(words) || words.length === 0) return;

        words.forEach(word => {
          // Skip self-references
          if (word.id === wordInfo.id) return;

          // Categorize by incoming vs outgoing and relation type
          const direction = relationType.startsWith('to_') ? 'outgoing' : 'incoming';
          const type = relationType.replace(/^(from_|to_)/, '');

          if (!relations[direction][type]) {
            relations[direction][type] = [];
          }

          // Check if this word is already in relations to avoid duplicates
          const isDuplicate = relations[direction][type].some(rel => 
            rel.word.lemma === word.lemma
          );

          if (!isDuplicate) {
            relations[direction][type].push({
              id: `rel_${direction}_${type}_${word.id}`,
              relation_type: type,
              word
            });
          }
    ...
    } else if (wordInfo.relations) {
      // Use standard relations from wordInfo
      relations = wordInfo.relations;
    }

    // Function to render relations of a specific direction (incoming/outgoing)
    const renderRelations = (direction: 'incoming' | 'outgoing') => {
      const relationTypes = Object.keys(relations[direction]);
      
      if (relationTypes.length === 0) {
        return (
          <NoRelationsMessage>
            No {direction} relations found
          </NoRelationsMessage>
        );
      }
      ...
      return (
        <div className="relations-container">
          {relationTypes.map(type => {
            const relationsOfType = relations[direction][type];
            if (!relationsOfType || relationsOfType.length === 0) return null;
            
            return (
              <RelationGroup key={`${direction}-${type}`}>
                <RelationTypeHeading>
                ...
                <RelationList>
                  {relationsOfType.map((relation) => (
                    <RelationItem 
                      ...
                      onClick={() => handleRelationClick(relation.word.id)}
                    >
                      <div className="relation-content">
                        <span className="relation-lemma">{relation.word.lemma}</span>
                        {relation.word.language && (
                          <span className="relation-language">({relation.word.language})</span>
                        )}
                      </div>
                    </RelationItem>
                  ))}
    ...
    return (
      <TabContent>
        <RelationsTabContainer>
          <RelationsSection>
            <RelationHeader>Incoming Relations</RelationHeader>
            {renderRelations('incoming')}
          </RelationsSection>
          <RelationsSection>
            <RelationHeader>Outgoing Relations</RelationHeader>
            {renderRelations('outgoing')}
          </RelationsSection>
        </RelationsTabContainer>
      </TabContent>
    );
