...
import { WordGraphComponent, WordDetails } from "./index";
import { useTheme } from "../contexts/ThemeContext";
import "./WordExplorer.css";
import { WordNetwork, WordNetworkGraph, WordNetworkLists, WordInfo, SearchResult, SearchOptions, EtymologyTree, Statistics, Definition, NetworkNode } from "../types";
...
function isWordNetworkGraph(network: WordNetwork): network is WordNetworkGraph {
  return network && 'nodes' in network && 'edges' in network && 
         Array.isArray(network.nodes) && Array.isArray(network.edges);
}

function isWordNetworkLists(network: WordNetwork): network is WordNetworkLists {
  return network && 'main_words' in network && 'root_words' in network;
}
...
const WordExplorer: React.FC = () => {
  ...
  // Cache for word network data
  const networkCache = useRef<Map<string, { data: WordNetworkGraph; timestamp: number }>>(new Map());
  const CACHE_TTL = 5 * 60 * 1000; // 5 minutes

  const buildNetworkFromWordDetails = useCallback((wordDetails: WordInfo, word: string, depth: number): WordNetworkGraph => {
    const network: WordNetworkGraph = {
      word,
      nodes: [],
      edges: [],
      clusters: {
    ...
    const mainNode: NetworkNode = {
      ...
      normalized_lemma: wordDetails.normalized_lemma,
      language: wordDetails.language_code,
      has_baybayin: wordDetails.has_baybayin,
      baybayin_form: wordDetails.baybayin_form,
      type: 'main',
      path: [{ type: 'main', word: wordDetails.lemma }],
      definitions: wordDetails.definitions.map(d => d.text)
    };
    network.nodes.push(mainNode);

    // Process relations from WordInfo
    if (wordDetails.relations) {
      let nodeId = 2;

      // Process synonyms
      if (wordDetails.relations.synonyms?.length) {
        wordDetails.relations.synonyms.forEach(rel => {
          network.nodes.push({
            id: nodeId,
            word: rel.word,
            normalized_lemma: rel.word.toLowerCase(),
            language: 'tl',
            has_baybayin: false,
            baybayin_form: null,
            type: 'synonym',
            path: [{ type: 'synonym', word: rel.word }],
            definitions: []
          });
          network.edges.push({
            source: 1,
          ...
          network.clusters.synonyms.push(network.nodes[nodeId - 1]);
          nodeId++;
        });
      }

      // Process antonyms
      if (wordDetails.relations.antonyms?.length) {
        wordDetails.relations.antonyms.forEach(rel => {
          network.nodes.push({
            id: nodeId,
            word: rel.word,
            normalized_lemma: rel.word.toLowerCase(),
            ...
            baybayin_form: null,
            type: 'antonym',
            path: [{ type: 'antonym', word: rel.word }],
            definitions: []
          });
          network.edges.push({
            source: 1,
          ...
          network.clusters.antonyms.push(network.nodes[nodeId - 1]);
          nodeId++;
        });
      }

      // Process root words
      if (wordDetails.relations.root) {
        const rootWord = wordDetails.relations.root;
        network.nodes.push({
          id: nodeId,
          word: rootWord.word,
          normalized_lemma: rootWord.word.toLowerCase(),
          language: 'tl',
          has_baybayin: false,
          baybayin_form: null,
          type: 'root',
          path: [{ type: 'root', word: rootWord.word }],
          definitions: []
        });
        network.edges.push({
          source: nodeId,
        ...
        network.clusters.root_words.push(network.nodes[nodeId - 1]);
        nodeId++;
      }

      // Process derived words
      if (wordDetails.relations.derived?.length) {
        wordDetails.relations.derived.forEach(rel => {
          network.nodes.push({
            id: nodeId,
            word: rel.word,
            normalized_lemma: rel.word.toLowerCase(),
            ...
            baybayin_form: null,
            type: 'derived',
            path: [{ type: 'derived', word: rel.word }],
            definitions: []
          });
          network.edges.push({
            source: 1,
          ...
          network.clusters.derived_words.push(network.nodes[nodeId - 1]);
          nodeId++;
        });
      }

      // Process variants
      if (wordDetails.relations.variants?.length) {
        wordDetails.relations.variants.forEach(rel => {
          network.nodes.push({
            id: nodeId,
            word: rel.word,
            normalized_lemma: rel.word.toLowerCase(),
            ...
            baybayin_form: null,
            type: 'variant',
            path: [{ type: 'variant', word: rel.word }],
            definitions: []
          });
          network.edges.push({
            source: 1,
          ...
          network.clusters.variants.push(network.nodes[nodeId - 1]);
          nodeId++;
        });
      }
    }
    ...
    // Update metadata
    network.metadata.total_nodes = network.nodes.length;
    network.metadata.total_edges = network.edges.length;
    network.metadata.cluster_count = Object.values(network.clusters).filter(arr => arr.length > 0).length;
  ...
  const fetchWordNetworkData = useCallback(async (word: string, depth: number = 2, breadth: number = 10) => {
    try {
      ...
      // Check cache first
      const cacheKey = `${word}-${depth}-${breadth}`;
      const cached = networkCache.current.get(cacheKey);
      if (cached && Date.now() - cached.timestamp < CACHE_TTL) {
        if (cached.data.nodes.length > 0) {
          console.log('Using cached network data for:', word);
          setWordNetwork(cached.data);
          setIsLoading(false);
          return cached.data;
        } else {
          // Clear invalid cache entry
          networkCache.current.delete(cacheKey);
        }
      }

      // Batch multiple requests together
      const [networkData, wordDetails] = await Promise.all([
        fetchWordNetwork(word, {
          depth,
      ...
      // If no direct network data, try to build a network from word details
      if ((!networkData?.nodes?.length || networkData.nodes.length === 1) && wordDetails) {
        console.log('Building network from word details');
        const builtNetwork = buildNetworkFromWordDetails(wordDetails, word, depth);

        // If we built a valid network, use it
        if (builtNetwork.nodes.length > 1) {
          setWordNetwork(builtNetwork);
          networkCache.current.set(cacheKey, {
            data: builtNetwork,
            timestamp: Date.now()
          });
          setIsLoading(false);
      ...
      // If we have network data, process it
      if (networkData?.nodes?.length) {
        // Process data more efficiently using Set for faster lookups
        const processedData: WordNetworkGraph = {
          word,
          nodes: [],
          edges: [],
          ...
          metadata: {
            ...
            cluster_count: Object.keys(networkData.clusters || {}).length
          }
        };

        // Pre-process nodes using Set for faster lookups
        const nodeIds = new Set<number>();
        const nodeMap = new Map<number, NetworkNode>();
        
        if (Array.isArray(networkData.nodes)) {
          // First, add the main word and important nodes
          const mainNode = networkData.nodes.find(n => n.word === word);
          if (mainNode) {
            nodeIds.add(mainNode.id);
            nodeMap.set(mainNode.id, mainNode);
          }

          // Then add root words and closely related nodes
          const importantNodes = networkData.nodes.filter(n => 
            n.type === 'root' || 
            n.type === 'synonym' || 
            n.type === 'derived'
          ).slice(0, breadth);
          ...
          const remainingSlots = (breadth * depth) - nodeIds.size;
          if (remainingSlots > 0) {
            networkData.nodes
          ...
          processedData.nodes = Array.from(nodeMap.values()).map(node => ({
            id: node.id,
            word: String(node.word),
            normalized_lemma: String(node.normalized_lemma || node.word).toLowerCase(),
            language: String(node.language || 'tl'),
            has_baybayin: Boolean(node.has_baybayin),
            baybayin_form: node.baybayin_form || null,
            type: String(node.type || 'other'),
            path: Array.isArray(node.path) ? node.path : [{ type: 'direct', word: String(node.word) }],
            definitions: Array.isArray(node.definitions) ? node.definitions : []
          }));
        }

        // Process edges efficiently
        if (Array.isArray(networkData.edges)) {
          processedData.edges = networkData.edges
            .filter(edge => {
              const sourceId = typeof edge.source === 'number' ? edge.source : parseInt(String(edge.source));
              const targetId = typeof edge.target === 'number' ? edge.target : parseInt(String(edge.target));
        ...
        const now = Date.now();
        Array.from(networkCache.current.entries()).forEach(([key, value]) => {
      ...
      throw new Error('No word network data available. Try adjusting the depth and breadth settings or searching for a different word.');

    } catch (error) {
      console.error("Error in fetchWordNetworkData:", error);
      setError(error instanceof Error ? error.message : "Failed to fetch word network");
      return null;
    } finally {
      setIsLoading(false);
    }
  }, [buildNetworkFromWordDetails]);
  ...
  const handleSearch = useCallback(async (searchWord?: string) => {
    ...
    try {
      ...
      if (searchResults && searchResults.words && searchResults.words.length > 0) {
        ...
        const wordData = await fetchWordDetails(firstResult.word);
        ...
        if (wordData && wordData.lemma) {
          ...
          console.log('Fetching word network for search result:', wordData.lemma);
          let networkData = await fetchWordNetworkData(wordData.lemma, depth, breadth);
  ...
  const handleNodeClick = useCallback(async (word: string) => {
    ...
    try {
      ...
      const [wordData, networkData] = await Promise.all([
        fetchWordDetails(word),
  ...
  const handleRandomWord = useCallback(async () => {
    ...
    try {
      ...
      if (randomWordData && randomWordData.lemma) {
        ...
        fetchEtymologyTree(wordData.lemma);
      } else {
      ...
      <main>
        <div className="graph-container">
          <div className="graph-content">
            {isLoading ? (
            ...
            ) : wordNetwork && mainWord && isWordNetworkGraph(wordNetwork) && wordNetwork.nodes.length > 0 ? (
              <WordGraphComponent
              ...
              <div className="no-graph-message">
                ...
                <p>Try adjusting the depth and breadth settings or searching for a different word.</p>
                {wordNetwork && (
