...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const getNodeColor = useCallback((group: string): string => {
    // Colors with more distinct hues for better differentiation
    switch (group.toLowerCase()) {
      // Core
      case "main": return "#1e40af"; // Deeper blue for main word
      
      // Origin group - More distinct reds/oranges
      case "root": return "#dc2626"; // Pure red
      case "etymology": return "#9d174d"; // Magenta-red
      case "cognate": return "#ea580c"; // Bright orange
      
      // Meaning group - More distinct blues
      case "synonym": return "#0284c7"; // Sky blue
      case "related": return "#06b6d4"; // Cyan
      case "antonym": return "#1e3a8a"; // Navy blue
      
      // Form group - Purples
      case "variant": return "#7e22ce"; // Bright purple
      
      // Hierarchy group - More distinct greens
      case "taxonomic": return "#059669"; // Emerald green
      case "part_whole": return "#16a34a"; // Pure green
      case "component_of": return "#16a34a"; // Same as part_whole
      
      // Info group - Yellows
      case "usage": return "#ca8a04"; // Amber
  ...
  useEffect(() => {
    ...
    const legendItemHeight = 22; // Reduced height
    const dotRadius = 4.5; // Slightly increased dot size for better visibility
    const textPadding = 14; // Distance from dot to text
    const categorySpacing = 8; // Space between categories
    const maxLabelWidth = 140; // Maximum width for labels

    // Organize relation types by category in a single, compact column
    const categories = [
    ...
    // Calculate legend width based on content plus padding
    const legendWidth = Math.max(
      maxTextWidth + textPadding + 15, // Text width plus dot size and padding
      maxCategoryWidth + 10, // Category name width plus padding
      toggleTextWidth + 50 // Toggle text plus switch width and padding
    ) + (legendPadding * 2);

    // Find total rows
    let totalRows = categories.reduce((sum, cat) => sum + 1 + cat.types.length, 0);
    ...
    categories.forEach((category, categoryIndex) => {
      ...
      if (categoryTextBBox) {
        ...
        legendContainer.append("rect")
          ...
          .attr("width", categoryTextBBox.width + 6)
          .attr("height", categoryTextBBox.height + 3)
          .attr("rx", 3)
      ...
      category.types.forEach(type => {
        ...
        const availableWidth = legendWidth - textPadding - legendPadding - 15;
