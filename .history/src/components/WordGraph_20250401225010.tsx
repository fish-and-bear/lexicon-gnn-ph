      ...
      nodeSelection
        ...
        .on("dblclick", (event, d) => {
             ...
             d3.select(event.currentTarget as Element).select('circle')
                 ...
                 .attr("stroke", (n: CustomNode) => { // Use n for clarity within callback
                     const baseColor = getNodeColor(n.group);
                     let finalColor: string;
                     if (n.pinned) {
                         finalColor = baseColor;
                     } else {
                         // If unpinned, determine if it was selected to set correct state
                         if (n.id === selectedNodeId) {
                             // Use the brighter color state (same as selected state)
                             finalColor = d3.color(baseColor)?.brighter(0.8).formatHex() ?? baseColor ?? (theme === "dark" ? "#eee" : "#333");
                         } else {
                             // Use the default darker outline state
