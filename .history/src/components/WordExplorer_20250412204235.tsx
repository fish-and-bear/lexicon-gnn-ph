...
const WordExplorer: React.FC = () => {
  ...
  const handleSearch = useCallback(async (query: string) => {
    ...
    try {
      ...
      console.log(`[DEBUG] Search API result: total=${result.total}, words.length=${result.words?.length || 0}`);
      
      // Specially handle 'kain' search results
      if (query.toLowerCase() === 'kain') {
        console.log(`[KAIN DEBUG] Full search results for kain:`, result);
        if (!result.words || result.words.length === 0) {
          console.log(`[KAIN DEBUG] No results found for kain in the response`);
        }
      }
      
      if (result.words && result.words.length > 0) {
        console.log(`[DEBUG] First search result:`, result.words[0]);
      } else {
        console.log(`[DEBUG] No words found in search result`);
      }
      ...
      console.log(`[DEBUG] Found search results, selecting first result:`, result.words[0]);
      ...
      try {
        ...
        console.log(`[DEBUG] Network data received:`, networkData);
  ...
  const renderSearchBar = () => {
    return (
      <div className="search-container">
        ...
        <div className="search-input-container">
          <input
            ...
            onChange={(e) => {
              ...
              // Special case for "kain" to force immediate search
              if (newValue.toLowerCase() === "kain") {
                console.log("[KAIN DEBUG] Detected 'kain' in input, forcing immediate search");
                handleSearch("kain");
                return;
              }
        ...
        <button
          onClick={() => {
            // Force search for "kain" to test
            console.log("[DEBUG] Forcing search for 'kain'");
            setInputValue("kain");
            handleSearch("kain");
          }}
