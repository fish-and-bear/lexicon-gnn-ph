...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({
  ...
  const renderRelationsTab = () => {
    ...
    // Check for loading state - if we have an ID but no relations data yet
    const isLoading = wordInfo?.id && 
      semanticNetworkLinks.length === 0 && 
      incoming_relations.length === 0 && 
      outgoing_relations.length === 0 && 
      !wordInfo.server_error;
    
    // Show loading indicator if relations data might still be loading
    if (isLoading) {
      return (
    ...
    // Improved error detection: Check for database transaction errors
    ...
    return (
      <Box sx={{ pt: theme.spacing(1) }}>
        ...
        {/* Render semantic relations if in fallback mode - FIX DUPLICATE HEADERS */}
        {useSemanticNetwork ? (
          <Box>
            <Typography variant="subtitle1" sx={{ 
              fontWeight: 600,
            ...
            {/* Remove the duplicate heading in renderRelationGroup by passing empty title */}
            {renderRelationGroup("", semanticRelations, 'out')}
          </Box>
        ) : (
          <>
