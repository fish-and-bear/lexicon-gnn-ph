...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const { filteredNodes, filteredLinks } = useMemo(() => {
    ...
    console.log("[FILTER] Initial queue:", queue);
    ...
    while (queue.length > 0) {
      ...
      if (currentDepth >= depth || visited.has(currentWordId)) {
        console.log(`[FILTER] Skipping - depth limit or already visited`);
        continue;
      }
      ...
      const relatedLinks = baseLinks.filter(link => {
        // Ensure we handle both string and object source/target
      ...
      console.log(`[FILTER] Found ${relatedWordIds.length} connected nodes: ${relatedWordIds.join(', ')}`);
