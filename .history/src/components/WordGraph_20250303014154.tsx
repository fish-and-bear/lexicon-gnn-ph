...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  // Update setupZoom for smoother interactions
  const setupZoom = useCallback((svg: d3.Selection<SVGSVGElement, unknown, null, undefined>, g: d3.Selection<SVGGElement, unknown, null, undefined>) => {
    ...
    // Apply zoom to SVG with new easing functions
    svg.call(zoom);
  ...
  const setupSimulation = useCallback((nodes: CustomNode[], links: CustomLink[]) => {
    ...
    const sim = d3
      ...
      .alphaDecay(0.022) // Faster cooling for more stability
      .velocityDecay(0.45) // Higher damping for smoother motion
      .force('link', d3.forceLink<CustomNode, CustomLink>(links)
        ...
        .strength(0.75)) // Stronger links for more stability
      ...
      }).strength(0.85)) // Stronger collision prevention
      .force('x', d3.forceX(0).strength(0.08)) // Gentle force toward center x
      .force('y', d3.forceY(0).strength(0.08)) // Gentle force toward center y
      .on('tick', () => {
        ...
        if (!isDraggingRef.current && !isTransitioningRef.current) {
          ...
          d3.select(svgRef.current)
            ...
            .attr('transform', d => {
              ...
              if (d.vx !== undefined) d.vx *= 0.98;
              if (d.vy !== undefined) d.vy *= 0.98;
  ...
  const createDragBehavior = useCallback(() => {
    return d3.drag<SVGGElement, CustomNode>()
      .on('start', (event, d) => {
        ...
        // Warm up the simulation while dragging, but not too much
        if (!event.active && simulationObject) {
          simulationObject.alphaTarget(0.08).restart();
        }
      ...
      .on('drag', (event, d) => {
        ...
        // Update the connected links for the dragged node with smoother transitions
        d3.select(svgRef.current)
          .selectAll<SVGLineElement, CustomLink>('.link')
          .filter((link: any) => {
      ...
      .on('end', (event, d) => {
        // Apply a brief delay before resetting the dragging flag to prevent accidental clicks
        setTimeout(() => {
        ...
        if (!event.active && simulationObject) {
          simulationObject.alphaTarget(0.01);
          setTimeout(() => {
  ...
  const highlightConnectedLinks = useCallback((nodeId: string, isHighlighted: boolean) => {
    d3.selectAll(".link")
      ...
      .transition()
      .duration(isHighlighted ? 180 : 350)
  ...
  const setupNodeInteractions = useCallback((node: d3.Selection<SVGGElement, CustomNode, SVGGElement, unknown>, svg: d3.Selection<SVGSVGElement, unknown, null, undefined>) => {
    node
      .on("click", (event: MouseEvent, d: CustomNode) => {
        ...
        const now = Date.now();
        if (isDraggingRef.current || now - lastClickTimeRef.current < 350) return;
        ...
        if (d.id !== selectedNodeId) {
          ...
          // This gives time for the visual transition to begin
          setTimeout(() => {
            console.log(`Node clicked: ${d.id}, updating main word`);
            onNodeClick(d.id);
            
            // Reset transitioning flag after a longer delay to allow for state updates
            setTimeout(() => {
  ...
  const handleZoom = useCallback((scale: number) => {
    if (zoomRef.current && svgRef.current) {
      const svg = d3.select(svgRef.current);
      svg.transition()
        .duration(400) // Longer, smoother transition
  ...
  const handleResetZoom = useCallback(() => {
    if (zoomRef.current && svgRef.current) {
      ...
      setTimeout(() => centerOnMainWord(svg), 700);
    }
  }, [centerOnMainWord]);
