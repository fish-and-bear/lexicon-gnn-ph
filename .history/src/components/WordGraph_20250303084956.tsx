...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const getNodeColor = (node: CustomNode): string => {
    // Main word is highlighted
    if (node.id === mainWord) {
      return theme === "dark" ? "#6d77af" : "#4f5db9";
    }
    
    // Check node group or relationship
    if (node.group) {
      switch (node.group) {
        case "synonym": return theme === "dark" ? "#4caf50" : "#81c784";
        case "antonym": return theme === "dark" ? "#f44336" : "#e57373";
        case "derived": return theme === "dark" ? "#ff9800" : "#ffb74d";
        case "root": return theme === "dark" ? "#9c27b0" : "#ba68c8";
        default: return theme === "dark" ? "#7986cb" : "#9fa8da";
      }
    }
    
    // Default color
    return theme === "dark" ? "#78909c" : "#90a4ae";
  };
