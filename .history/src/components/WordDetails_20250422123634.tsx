              ...
              <Box key={sourceName} sx={{ mb: theme.spacing(2), width: '100%', maxWidth: '100%' }}> {/* Add margin between source groups */}
                <List disablePadding>
                  {defs.map((def: Definition, index: number) => {
                    ...
                    console.log(`[DefinitionsTab]     Def ID ${def.id}: Last? ${isLastDefinitionForSource}, Sources:`, def.sources); // Log each definition
                    return (
                    <ListItem 
                      key={def.id || index} 
  ...
  if (!wordData?.id) {
    ...
    return (
        <Paper elevation={1} sx={{ 
            ...
            return (
    <Box sx={{ display: 'flex', flexDirection: 'column', flexGrow: 1 /* Ensure this Box fills parent height */ }}>
      {renderHeader()} {/* Re-add header rendering */}
      <Box sx={{ display: 'flex', /* Remove height: '100%' */ flexDirection: isMobile ? 'column' : 'row', flexGrow: 1, minHeight: 0 /* Allow inner content to determine height/scroll */ }}>
        {/* Tabs */}
        <Tabs
          orientation={isMobile ? 'horizontal' : 'vertical'} // Use isMobile prop
          variant="scrollable" // Make tabs scrollable
          allowScrollButtonsMobile // Show scroll buttons on mobile if needed
          value={activeTab}
          ...
          sx={{
            ...
            borderColor: 'divider',
            flexShrink: 0,
            // Add styles for horizontal tabs container if needed
            ...(isMobile && {
              bgcolor: 'background.paper',
              position: 'sticky', // Make tabs sticky on mobile?
              top: 0, // Stick to top
        ...
        >
          ...
          <Tab 
            ...
            iconPosition="start" 
            wrapped={!isMobile} // Allow wrapping on desktop (vertical)
          />
          <Tab 
            value="relations" 
            label="Relations" 
          ...
          <Tab 
            value="forms" 
            label="Forms" 
            icon={<span role="img" aria-label="Shapes">🧩</span>} 
            iconPosition="start" 
            wrapped={!isMobile} 
          />
          {/* Optional: Add other tabs back if needed */} 
          {/* <Tab value="sources" label="Sources & Info" /> */}
        </Tabs>
        ...
        <Box
          sx={{
            flexGrow: 1,
            overflowY: 'auto', // Allow content to scroll independently
            p: isMobile ? 1.5 : 2.5, // Adjust padding based on mobile
            // pb: isMobile ? 4 : 2.5, // Let paddingBottom handle it explicitly
            paddingBottom: isMobile ? theme.spacing(6) : theme.spacing(2.5), // << INCREASED bottom padding explicitly for mobile 
            bgcolor: 'background.default', // Use theme background
            minHeight: 0, // Crucial for flex children that need to scroll
          }}
        >
