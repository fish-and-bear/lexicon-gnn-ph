...
const WordExplorer: React.FC = () => {
  ...
  const fetchWordNetworkData = useCallback(async (word: string, depth: number = 2, breadth: number = 10) => {
    try {
      ...
      if (error instanceof Error) {
        ...
        // If it's a connection error, show instructions
        if (error.message.includes('backend server')) {
          setWordNetwork(null); // Clear any existing network data
        }
      } else {
        setError('An unexpected error occurred while fetching the word network');
      }
      
      // Clear word network data on error
      setWordNetwork(null);
      throw error;
    } finally {
      setIsLoading(false);
  ...
  const handleRandomWord = useCallback(async () => {
    ...
    try {
      ...
      if (randomWordData && randomWordData.lemma) {
        ...
        // Try to fetch word network
        try {
          await fetchWordNetworkData(randomWordData.lemma);
        } catch (networkError) {
          console.error("Error fetching random word network:", networkError);
          // Don't rethrow - we want to keep the word data even if network fails
        }
  ...
  }, [fetchWordNetworkData, fetchEtymologyTree]);
  ...
  useEffect(() => {
              ...
              setTimeout(async () => {
                ...
                try {
                  ...
                  if (finalRetrySuccess) {
                    ...
                    // Fetch data only after successful connection
                    fetchInitialData();
                  } else {
    ...
    const fetchInitialData = () => {
      console.log("Fetching initial data after API connection confirmed.");
      fetchPartsOfSpeech();
      fetchStatistics();
      fetchBaybayinWords();
      // fetchAffixes(); // Uncomment if endpoint exists and needed
      // fetchRelations(); // Uncomment if endpoint exists and needed
      // fetchAllWords(); // Uncomment if endpoint exists and needed
    };
    
    checkApiConnection(); // Start the API check process

  }, [fetchStatistics, fetchBaybayinWords, getPartsOfSpeech]); // Add dependencies
