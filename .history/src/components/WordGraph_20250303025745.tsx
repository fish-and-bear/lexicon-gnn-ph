...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const createDragBehavior = useCallback((simulation: d3.Simulation<CustomNode, CustomLink>) => {
    ...
    return d3.drag<SVGGElement, CustomNode>()
      .on("start", (event, d) => {
        ...
        const nodeElement = d3.select(event.sourceEvent.target.closest(".node") as Element);
        ...
        d3.selectAll(".link")
          .each(function(linkData: any) {
            const link = d3.select(this as Element);
  ...
  const renderTooltip = useCallback(() => {
    ...
    if (definition === "No definition available" && hoveredNode.info?.definitions) {
      const infoDefObj = hoveredNode.info.definitions.find((d: any) => typeof d === 'object');
      if (infoDefObj) {
