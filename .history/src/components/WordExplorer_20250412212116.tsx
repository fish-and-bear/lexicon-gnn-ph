    ...
    try {
      ...
      if (result && result.words && result.words.length > 0) {
        ...
        try {
          ...
          const wordId = firstResult.id.toString().startsWith('id:') 
            ? firstResult.id.toString() 
            : `id:${firstResult.id}`;
          ...
          try {
            ...
            const etymologyId = typeof wordData.id === 'string' && wordData.id.startsWith('id:') 
              ? parseInt(wordData.id.substring(3), 10) 
              : wordData.id;
            await fetchEtymologyTree(etymologyId);
              ...
              {searchResults.map((result) => (
                <li 
                  ...
                  onClick={() => {
                    ...
                    const wordId = typeof result.id === 'number' ? `id:${result.id}` : result.id.toString();
