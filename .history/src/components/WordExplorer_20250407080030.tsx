...
const WordExplorer: React.FC = () => {
  ...
  const [leftPanelWidth, setLeftPanelWidth] = useState<number>(() => {
    // Initialize width from local storage or default to 50%
    const savedWidth = localStorage.getItem('leftPanelWidthPx');
    if (savedWidth) {
      return parseInt(savedWidth, 10);
    } 
    // Calculate initial 50% based on current viewport width if possible
  ...
  useEffect(() => {
    ...
    const onMouseDown = (e: MouseEvent) => {
      ...
      setInitialWidth(leftPanelWidth);
        ...
        <div className="graph-container" style={{ width: `${leftPanelWidth}px`, flexBasis: `${leftPanelWidth}px`, flexShrink: 0 }}>
          <div className="graph-content">
            ...
            {networkError && !isLoadingNetwork && (
               <Alert severity="error" sx={{ m: 2, width: 'calc(100% - 32px)' }}>{networkError}</Alert>
            )}
