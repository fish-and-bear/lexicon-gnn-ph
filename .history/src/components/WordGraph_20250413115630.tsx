...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const toggleRelationshipFilter = useCallback((type: string) => {
    ...
    const nextRelationshipFilters = [...filteredRelationships];
    const index = nextRelationshipFilters.indexOf(type);
    
    if (index === -1) {
      nextRelationshipFilters.push(type);
    } else {
      nextRelationshipFilters.splice(index, 1);
    }
    
    console.log(`Toggle filter for ${type}, new filters:`, nextRelationshipFilters);
    
    // Apply DOM changes immediately for visual feedback
    ...
    svg.selectAll(".node")
      .each(function(d: any) {
        ...
        const isVisible = nextRelationshipFilters.includes(nodeRelationship);
    ...
    svg.selectAll(".link")
      .each(function(d: any) {
        ...
        console.log("Link relationship:", linkRelationship, "Is in filters:", nextRelationshipFilters.includes(linkRelationship));
        const isVisible = nextRelationshipFilters.includes(linkRelationship);
    ...
    // Update state
    setFilteredRelationships(nextRelationshipFilters);
  }, [filteredRelationships, mainWord]);
