...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  // Create a simpler but more robust drag behavior for nodes
  const createDragBehavior = useCallback(() => {
    return d3.drag<SVGGElement, CustomNode>()
      ...
      .on('drag', (event, d) => {
        ...
        // This is more reliable than manually updating the transform
      })
      .on('end', (event, d) => {
        if (!event.active && simulationObject) {
        ...
        if (event.sourceEvent && event.sourceEvent.shiftKey) {
          ...
          // Mark it visually using the circle's parent node to ensure we have the right element
