...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const filteredNodes = useMemo<CustomNode[]>(() => {
    ...
    while (queue.length > 0) {
      ...
      // Sort links by relationship priority to limit by breadth
      ...
      // Limit the number of connections by breadth parameter
      const maxConnections = Math.min(breadth, prioritizedLinks.length);
      const limitedLinks = prioritizedLinks.slice(0, maxConnections);

      // Get connected word IDs and add to queue for BFS
