...
const WordExplorer: React.FC = () => {
  ...
  const fetchSuggestionsDebounced = React.useMemo(
    () =>
      debounce(
        ...
        400, // Debounce time in ms (adjust as needed)
      ),
    [],
  );

  // Handle search submission (could be from button or Autocomplete selection)
  const handleSearch = useCallback(async (searchTerm: string | null) => {
    if (!searchTerm) return;
    setInputValue(searchTerm); // Update the input value state
    setSuggestions([]); // Clear suggestions
    setAutocompleteOpen(false); // Close the dropdown
    // Reuse handleNodeClick or create a specific search handler
    // Assuming handleNodeClick can handle direct word searches
    await handleNodeClickRef.current(searchTerm);
  }, [handleNodeClickRef]); // Add dependencies as needed
  ...
  const renderSearchBar = () => {
    ...
    return (
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, width: '100%', padding: muiTheme.spacing(1, 2) }}>
         <Autocomplete
          ...
          getOptionLabel={(option) => option} // Suggestions are strings
          options={suggestions}
          loading={isSuggestionsLoading}
          filterOptions={(x) => x} // Disable built-in filtering, rely on server
          freeSolo // Allows arbitrary input not just suggestions
          autoComplete
          includeInputInList
          ...
          onChange={(event, newValue, reason) => {
            if (reason === 'selectOption' && newValue) {
              handleSearch(newValue);
                ...
                // Handle Enter key press with custom input
                handleSearch(newValue);
            }
          }}
          renderInput={(params) => (
        ...
        <Button 
          ...
          sx={{ height: '40px' }} // Match TextField height
        >
          {isRandomLoading ? <CircularProgress size={24} /> : 'Random'}
        </Button>
      </Box>
    );
  };
