  ...
  const setupSimulation = useCallback((nodes: CustomNode[], links: CustomLink[]) => {
    ...
    const sim = d3
      ...
      .force('charge', d3.forceManyBody()
        .strength(d => {
          // Adaptive repulsion force
          return d.group === 'main' ? -350 : -250;
        })
        .theta(0.85)      // Performance parameter
        .distanceMax(containerWidth / 1.2)) // Limit long-range effects
      .force('center', d3.forceCenter(0, 0))
      .force('collision', d3.forceCollide().radius(d => {
        // Larger collision radius for main word
        return d.group === 'main' ? 45 : 35;
    ...
    node
      .on("click", (event: MouseEvent, d: CustomNode) => {
        ...
        if (d.id !== selectedNodeId) {
          ...
          const currentNodeElement = d3.select(event.currentTarget);
          currentNodeElement
            .select('circle')
            .transition()
            .duration(300)
