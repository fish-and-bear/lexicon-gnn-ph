      ...
      nodeSelection
          ...
          targetNodeElement.classed("selected", true)
           ...
           d3.selectAll<SVGLineElement, CustomLink>(connectedLinkElements)
            ...
            .attr("stroke", (l: CustomLink) => { // Color link based on target
                ...
                // Use direct color, not brightened
                return targetNode ? getNodeColor(targetNode.group) : (theme === "dark" ? "#aaa" : "#666");
            })
            .attr("stroke-opacity", 0.9) // Clearly visible
        ...
        .on("mouseout", (event, d_unknown) => {
          ...
          targetNodeElement.classed("selected", true)
            ...
            .style("opacity", 1)
            .attr("transform", `translate(${d.x ?? 0},${d.y ?? 0}) scale(1)`);
            // REMOVE explicit selected style setting here (rely on class or mouseout)
        ...
        .on("dblclick", (event, d) => {
             ...
             d3.select(event.currentTarget as Element).select('circle')
                 ...
                 .attr("stroke-dasharray", d.pinned ? "5,3" : "none") // Keep pinned dash
                 // Reset stroke color correctly on unpin
