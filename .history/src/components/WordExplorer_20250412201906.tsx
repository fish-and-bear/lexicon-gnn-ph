    ...
    if (word !== mainWord) {
      ...
      try {
        ...
        if (word.startsWith('id:')) {
          ...
          try {
            // Use the ID-specific endpoint by passing just the numeric ID
            console.log(`Calling fetchWordDetails with ID: ${nodeId}`);
            wordData = await fetchWordDetails(nodeId);
          ...
          // For plain text words (non-ID format), we'll directly request word details
          console.log(`Fetching details for word text: "${word}"`);
          
          try {
            // Since we're working with word text, pass it directly to fetchWordDetails
            wordData = await fetchWordDetails(word);
            console.log("Word data received for text lookup:", wordData);
          } catch (wordLookupError) {
            console.error("Error in direct word lookup, falling back to search:", wordLookupError);
            wordData = null;
          }
        }
        
        // If we don't have word data yet (direct lookup failed), search for it
        if (!wordData) {
          ...
          // Fetch word details by ID - this is more reliable than by text
        ...
        let errorMessage = "Failed to load word data";
        if (err instanceof Error) {
          ...
          if (msg.includes("Database error")) {
            ...
            errorMessage = `Word "${word}" was found in the graph but its details could not be retrieved.`;
