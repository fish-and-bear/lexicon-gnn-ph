...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({ wordInfo, onWordClick }) => {
  ...
  const renderBaybayin = () => {
    ...
    return (
      <div className={`baybayin-section ${theme}`} style={theme === 'dark' ? { 
        background: 'linear-gradient(135deg, rgba(25, 31, 52, 0.7) 0%, rgba(18, 24, 43, 0.6) 100%)',
        borderLeft: '3px solid #ffd166',
        boxShadow: '0 4px 16px rgba(0, 0, 0, 0.25)'
      } : {}}>
        <div className={`section-title ${theme}`}>
          <h3 className={theme} style={theme === 'dark' ? { color: '#ffd166' } : {}}>Baybayin Script</h3>
        </div>
        
        <div className={`baybayin-display ${theme}`} style={theme === 'dark' ? { backgroundColor: '#171d31', borderColor: 'rgba(75, 86, 115, 0.4)' } : {}}>
          <div className={`baybayin-text ${theme}`} style={theme === 'dark' ? { color: '#ffd166' } : {}}>{wordInfo.baybayin_form}</div>
          <div className={`romanized-text ${theme}`} style={theme === 'dark' ? { color: '#a0c3e2' } : {}}>{wordInfo.romanized_form || wordInfo.lemma}</div>
        </div>
        
        <div className={`baybayin-info ${theme}`} style={theme === 'dark' ? { backgroundColor: 'rgba(23, 29, 49, 0.6)', border: '1px solid rgba(75, 86, 115, 0.4)' } : {}}>
          <h4 className={theme} style={theme === 'dark' ? { color: '#ffd166' } : {}}>About Baybayin</h4>
          <p className={theme} style={theme === 'dark' ? { color: '#a0a8c0' } : {}}>
