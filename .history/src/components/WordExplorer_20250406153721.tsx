...
const WordExplorer: React.FC = () => {
  ...
  const handleSearch = useCallback(async (wordToSearchParam?: string) => {
    ...
    let searchFn: (query: string | SearchOptions, options?: SearchOptions, bypass?: boolean) => Promise<SearchResult> = searchWords;
    let searchOptions: SearchOptions = { q: normalizedInput, limit: 1, include_full: true }; // Use include_full: true to get more data

    try {
      // Check for problematic terms and use directSearch if needed
      const problematicTerms = ['lima', 'dala', 'gawa']; 
      if (problematicTerms.includes(normalizedInput.toLowerCase())) {
        console.log(`[handleSearch] Using directSearch for problematic term: "${normalizedInput}"`);
        searchFn = directSearch; 
        searchOptions = { q: normalizedInput, limit: 1 }; // directSearch might not need all options
      }

      console.log('[handleSearch] Performing search with options:', searchOptions);
      const searchResult = await searchFn(searchOptions);
      console.log('[handleSearch] Search API response:', searchResult);

      // Check if we have results either in results or words arrays
      const hasResults = 
        (Array.isArray(searchResult.results) && searchResult.results.length > 0) ||
        (Array.isArray(searchResult.words) && searchResult.words.length > 0);
        
      if (hasResults) {
        console.log('[handleSearch] Found result, processing details');
        
        // Get the first result from either results or words array
        const firstResult = 
          (Array.isArray(searchResult.results) && searchResult.results.length > 0) ? 
            searchResult.results[0] : 
          (Array.isArray(searchResult.words) && searchResult.words.length > 0) ?
            searchResult.words[0] : null;
            
        if (!firstResult) {
          throw new Error("No results found despite arrays being present");
        }
        
        // Keep track of the word we are processing
        const wordLemma = firstResult.lemma || normalizedInput;
        setMainWord(wordLemma);
        setInputValue(wordLemma); // Update input field to match found word
        setWordHistory(prev => {
          // Avoid adding duplicates to history if the last entry is the same
          if (prev.length > 0 && prev[prev.length - 1] === wordLemma) return prev;
          return [...prev.slice(0, currentHistoryIndex + 1), wordLemma];
        });
        setCurrentHistoryIndex(prev => prev + 1);
        
        // Process the search result
        if (searchOptions.include_full && 'definitions' in firstResult) {
          // We already have full word data from search response
          console.log('[handleSearch] Using full details from search response');
          setSelectedWordInfo(firstResult as WordInfo);
          
          // Fetch network and etymology in parallel
          Promise.allSettled([
            fetchWordNetworkData(wordLemma, depth),
            fetchEtymologyTree(firstResult.id)
          ]).then(results => {
            results.forEach((result, index) => {
              if (result.status === 'rejected') {
                console.error(`[handleSearch] Error fetching ${index === 0 ? 'network' : 'etymology'}:`, result.reason);
              }
            });
          });
        } else {
          // Set a minimal loading state *before* fetching full details
          // This prevents the flicker but shows something is happening
          setSelectedWordInfo({ id: firstResult.id, lemma: wordLemma }); 

          try {
            const fullWordData = await fetchWordDetails(String(firstResult.id));
            console.log('[handleSearch] Full details received:', fullWordData);
            setSelectedWordInfo(fullWordData); // Update with full data

            // Fetch network and etymology in parallel after getting details
            Promise.allSettled([
            ...
            console.error("[handleSearch] Error fetching full details:", detailsError);
            // Keep the minimal info, but show an error
            setError(`Found "${firstResult.lemma}", but failed to fetch full details: ${detailsError instanceof Error ? detailsError.message : String(detailsError)}`);
          }
        }
      } else if (searchResult.count && searchResult.count > 0) {
        // Special case: API reports matches but returned no actual results
        console.log(`[handleSearch] API reports ${searchResult.count} matches but returned no results. Trying fallback search with different options.`);
        
        try {
          // Try another search with different options
          const fallbackOptions: SearchOptions = { 
            q: normalizedInput, 
            limit: 5, 
            include_full: false  // Don't request full data to avoid validation errors
          };
          
          const fallbackResult = await searchWords(fallbackOptions);
          
          if (
            (Array.isArray(fallbackResult.results) && fallbackResult.results.length > 0) ||
            (Array.isArray(fallbackResult.words) && fallbackResult.words.length > 0)
          ) {
            const firstFallbackResult = 
              (Array.isArray(fallbackResult.results) && fallbackResult.results.length > 0) ? 
                fallbackResult.results[0] : 
              (Array.isArray(fallbackResult.words) && fallbackResult.words.length > 0) ?
                fallbackResult.words[0] : null;
                
            if (firstFallbackResult) {
              // Keep track of the word we are processing
              const fallbackWordLemma = firstFallbackResult.lemma || normalizedInput;
              setMainWord(fallbackWordLemma);
              setInputValue(fallbackWordLemma);
              setWordHistory(prev => {
                if (prev.length > 0 && prev[prev.length - 1] === fallbackWordLemma) return prev;
                return [...prev.slice(0, currentHistoryIndex + 1), fallbackWordLemma];
              });
              setCurrentHistoryIndex(prev => prev + 1);
              
              // Set minimal info for display
              setSelectedWordInfo({ 
                id: firstFallbackResult.id, 
                lemma: fallbackWordLemma,
                language_code: firstFallbackResult.language_code || 'tl'
              });
              
              // Try to fetch full details separately
              try {
                const fullWordData = await fetchWordDetails(String(firstFallbackResult.id));
                ...
                console.error("[handleSearch] Error in fallback full details fetch:", detailsError);
                // We already have minimal info, so we can continue
                fetchWordNetworkData(fallbackWordLemma, depth).catch(networkError => {
                  console.error("[handleSearch] Error in fallback network fetch:", networkError);
                });
              }
            }
          } else {
            console.log('[handleSearch] No results found from fallback search API.');
            setError(`No results found for "${wordToSearch}"`);
          }
        } catch (fallbackError) {
          console.error("[handleSearch] Error in fallback search:", fallbackError);
          setError(`No results found for "${wordToSearch}". API reported matches but couldn't retrieve them.`);
        }
      } else {
        console.log('[handleSearch] No results found from search API.');
        setError(`No results found for "${wordToSearch}"`);
      ...
      console.error("[handleSearch] Error during search API call:", searchError);
      let errorMessage = "An error occurred during the search.";
      if (searchError?.message) {
        // Use the refined error message directly from the API layer if possible
        errorMessage = searchError.message;
      } 
      // Keep specific checks as fallbacks
      else if (String(searchError).includes("No response received")) {
        errorMessage = `Search for "${wordToSearch}" is taking too long. Backend might be busy.`;
      } else if (String(searchError).includes("Network Error")) {
        errorMessage = `Network error searching for "${wordToSearch}". Check connection.`;
      } 
  ...
  // Refined dependencies: include functions called inside, but be mindful of state vars that might cause loops
  // selectedWordInfo removed as we clear it first, preventing loops
