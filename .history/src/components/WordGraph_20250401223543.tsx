...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const ticked = useCallback(() => {
    // ... Keep ticked logic for straight lines ...
  ...
  const setupSimulation = useCallback((nodes: CustomNode[], links: CustomLink[]) => {
      ...
      simulationRef.current = d3.forceSimulation<CustomNode>(nodes)
        ...
        .force("link", d3.forceLink<CustomNode, CustomLink>()
          .id(d => d.id)
          .links(links)
          .distance(100) // Consistent distance
          .strength(0.45))
        .force("charge", d3.forceManyBody<CustomNode>().strength(-250).distanceMax(300)) // Moderate charge
        .force("collide", d3.forceCollide<CustomNode>().radius(d => getNodeRadius(d) + 5).strength(0.9)) // Moderate collision
      ...
      return g.append("g")
        ...
        .join(
            enter => enter.append("line")
                .attr("class", "link")
                .attr("stroke", theme === "dark" ? "#555" : "#bbb") // Default grey
                .attr("stroke-opacity", 0) // Start transparent
                // Restore subtle default thickness variation
                ...
                // Use easing and adjust default opacity
                .call(enter => enter.transition().duration(300).ease(d3.easeCubicInOut).attr("stroke-opacity", 0.4)),
            update => update
                // Use easing for updates
                .call(update => update.transition().duration(300).ease(d3.easeCubicInOut)),
            exit => exit
                // Use easing for exit
                .call(exit => exit.transition().duration(300).ease(d3.easeCubicInOut).attr("stroke-opacity", 0))
                .remove()
        );
  }, [theme]);
        ...
        .join(
            enter => {
                ...
                nodeGroup.append("circle")
                    ...
                    .attr("stroke-width", 1.5); // Slightly thicker default border
                ...
                textGroup.clone(true)
                    ...
                    .attr("stroke-width", d => d.id === mainWord ? 4 : 3.5) // Keep main word halo thicker
                ...
                // Use easing for fade-in
                nodeGroup.call(enter => enter.transition().duration(300).ease(d3.easeCubicInOut).style("opacity", 1));
                return nodeGroup;
            },
            update => update,
            exit => exit
                // Use easing for fade-out
  ...
  const setupNodeInteractions = useCallback((
      ...
      nodeSelection
        .on("click", (event, d) => {
            ...
            // Dim others moderately
            d3.selectAll(".node").classed("selected connected", false)
                .transition().duration(300).ease(d3.easeCubicInOut) // Use easing
                .attr("transform", n => `translate(${(n as CustomNode).x ?? 0},${(n as CustomNode).y ?? 0}) scale(1)`)
                .style("opacity", 0.15);
            ...
            // Highlight selected node and connected ones
            ...
            targetNodeElement.classed("selected", true)
                ...
                // Apply selected border style directly here for clarity
            targetNodeElement.select("circle")
                .transition().duration(300).ease(d3.easeCubicInOut)
                .attr("stroke-width", 3) // Selected thickness
                .attr("stroke", d3.color(getNodeColor(d.group))?.brighter(1.0).formatHex() ?? (theme === "dark" ? "#fff" : "#000")) // Selected bright border
                .attr("stroke-dasharray", "none"); // Ensure selected is not dashed if previously pinned
        ...
        .on("mouseover", (event, d) => {
            ...
            nodeElement.select("circle")
               .transition().duration(100).ease(d3.easeCubicInOut) // Use easing
               .attr("stroke-width", 2.5)
               .attr("stroke", d3.color(getNodeColor(d.group))?.brighter(0.8).formatHex() ?? (theme === "dark" ? "#eee" : "#333"));
            nodeElement.raise();

            d3.selectAll<SVGGElement, CustomNode>(".node:not(:hover)").transition().duration(100).ease(d3.easeCubicInOut).style("opacity", 0.6);
            d3.selectAll<SVGLineElement, CustomLink>(".link").transition().duration(100).ease(d3.easeCubicInOut).attr("stroke-opacity", 0.2);
        ...
        .on("mouseout", (event, d_unknown) => {
            ...
            nodeElement.select<SVGCircleElement>("circle")
                .data([d])
                .transition().duration(150).ease(d3.easeCubicInOut) // Use easing
                // Reset stroke width based on selected/pinned state
                .attr("stroke-width", (n: CustomNode) => n.pinned ? 3 : (n.id === selectedNodeId ? 3 : 1.5))
                // Reset stroke color based on selected/pinned state
                .attr("stroke", (n: CustomNode) => {
                    if (n.pinned) return getNodeColor(n.group); // Pinned color
                    if (n.id === selectedNodeId) return d3.color(getNodeColor(n.group))?.brighter(1.0).formatHex() ?? (theme === "dark" ? "#fff" : "#000"); // Selected bright border
                ...
                // Reset dasharray based on pinned state
                .attr("stroke-dasharray", (n: CustomNode) => n.pinned ? "6,3" : "none");

             d3.selectAll<SVGGElement, CustomNode>(".node").transition().duration(150).ease(d3.easeCubicInOut).style("opacity", 1);
             d3.selectAll<SVGLineElement, CustomLink>(".link").transition().duration(150).ease(d3.easeCubicInOut).attr("stroke-opacity", 0.4); // Restore default opacity
        })
        .on("dblclick", (event, d) => {
            // Keep simplified dblclick
             ...
             d3.select(event.currentTarget as Element).select('circle')
                 ...
                 .attr("stroke", d.pinned ? getNodeColor(d.group) : (d.id === selectedNodeId ? (d3.color(getNodeColor(d.group))?.brighter(1.0).formatHex() ?? "#fff") : (d3.color(getNodeColor(d.group))?.darker(0.6).formatHex() ?? (theme === "dark" ? "rgba(255,255,255,0.1)" : "rgba(0,0,0,0.1)"))));
         });
    ...
    const offsetY = (screenY > window.innerHeight / 2) ? -20 - 80 : 20;
      ...
      return (
        ...
        >
          ...
          <p>{hoveredNode.definitions?.join(", ")}</p>
        </div>
      );
  }, [hoveredNode, theme]);

  return (
    <div className="word-graph">
