...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({
  ...
  const renderDefinitionsTab = () => {
    // Capture result of renderSourcesInfoTab first
    const sourceMetadataContent = renderSourcesInfoTab(); 

    const noDefinitions = !wordInfo.definitions || wordInfo.definitions.length === 0;

    if (noDefinitions && !sourceMetadataContent) {
       // No definitions AND no metadata
       return <Alert severity="info">No definitions available for this word.</Alert>;
    }
    
    if (noDefinitions && sourceMetadataContent) {
       // No definitions BUT has metadata
       return (
...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({
  ...
  const renderDefinitionsTab = () => {
    ...
    if (!wordInfo.definitions || wordInfo.definitions.length === 0) {
      if (sourceMetadataContent) {
        return (
          <Box sx={{ pt: theme.spacing(1) }}>
            <Alert severity="info">No definitions available for this word.</Alert>
            {/* Render metadata even if no definitions, only if content exists */}
            <Box sx={{ mt: 3, pt: 2, borderTop: `1px solid ${theme.palette.divider}` }}>
              {sourceMetadataContent}
            </Box>
          </Box>
      ...
      // Only show alert if definitions AND metadata are missing
      return <Alert severity="info">No definitions available for this word.</Alert>;
    }
