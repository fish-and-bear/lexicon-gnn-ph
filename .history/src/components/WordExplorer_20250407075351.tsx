...
const WordExplorer: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const onMouseDown = (e: MouseEvent) => {
      ...
      setInitialWidth(leftPanelWidth); // Store the width when starting resize
      document.body.style.cursor = 'col-resize';
      document.documentElement.style.userSelect = 'none';
      resizer.classList.add('active'); // Add active class to resizer
    };
    ...
    const onMouseMove = (e: MouseEvent) => {
      ...
      const minWidth = 200; // Minimum width in pixels for both panels
      const maxWidth = containerRect.width - minWidth; // Max width for left panel

      // Calculate the difference in X position
      const dx = e.clientX - resizeStartX;
      
      // Calculate the new width based on the initial width and the drag distance
    ...
    const onMouseUp = () => {
      ...
      if (resizer) {
        resizer.classList.remove('active'); // Remove active class
      }
      // Save the final width to local storage
    ...
    document.addEventListener('mouseleave', onMouseUp); // Stop resizing if mouse leaves window
  ...
  }, [isResizing, leftPanelWidth, resizeStartX, initialWidth]); // Dependencies for the effect
