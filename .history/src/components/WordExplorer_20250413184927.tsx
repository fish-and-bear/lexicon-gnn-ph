...
import { WordNetwork, WordInfo, SearchResult, SearchOptions, EtymologyTree, Statistics, Definition, SearchWordResult, Relation, WordNetworkOptions } from "../types";
...
const WordExplorer: React.FC = () => {
  ...
  const loadWordData = useCallback(async (wordText: string, networkFilterOptions: WordNetworkOptions = {}) => {
    try {
      setError(null);
      setIsLoading(true);
      const wordData = await fetchWordDetails(wordText);
      setWordData(wordData);
      fetchWordNetwork(wordText, networkFilterOptions)
        .then(network => {
          setWordNetwork(network);
          setIsLoading(false);
          console.log(`Word network loaded: ${network.nodes.length} nodes, ${network.edges.length} edges`);
        })
        .catch(error => {
          console.error('Error loading word network:', error);
          setError(error.message);
          setIsLoading(false);
        });
    } catch (error) {
      ...
      let errorMessage = 'An error occurred while loading the word.';
      
      // Use instanceof to check if it's an Error object
      if (error instanceof Error) {
        // Handle specific known error messages with more user-friendly text
        if (error.message.includes('not found in the database')) {
          errorMessage = `Word "${wordText}" not found in the dictionary.`;
        } else if (error.message.includes('SQL error') || error.message.includes('Database error')) {
          errorMessage = `Database error. Please try again later or try a different word.`;
        } else if (error.message.includes('column r.relation_data does not exist') || 
                  error.message.includes('relations couldn\'t be loaded due to a missing column')) {
          // Special handling for the relation_data column error
          errorMessage = `Word found, but some details couldn't be loaded due to a database configuration issue. The basic information will still be displayed.`;
          
          // Continue loading the word network even if the details had an error
          fetchWordNetwork(wordText, networkFilterOptions)
            .then(network => {
              // If we can get network data, create a minimal word object from it
              setWordNetwork(network);
              
              // Try to construct a minimal word object from the network
              if (network.nodes.length > 0) {
                const mainNode = network.nodes.find(n => n.main) || network.nodes[0];
                const minimalWord: WordInfo = {
                  id: parseInt(mainNode.id),
                  lemma: mainNode.label,
                  normalized_lemma: mainNode.normalized_lemma,
                  language_code: mainNode.language,
                  has_baybayin: mainNode.has_baybayin || false,
                  baybayin_form: mainNode.baybayin_form,
                  // Use the network data for the semantic network
                  semantic_network: {
                ...
                setWordData(minimalWord);
              }
            })
            .catch(networkError => {
              console.error('Error loading word network:', networkError);
              setError(networkError.message);
            });
        } else {
          // For any other error, just use the error message directly
          errorMessage = error.message;
        }
      }
    ...
    try {
      ...
      if (error.message.includes('not found')) {
        ...
        setError(`Database error when retrieving the word '${word}'. Try searching for this word instead.`);
                ...
                error.message.includes('relations couldn\'t be loaded due to a missing column')) {
        // Special case for the missing relation_data column
        setError(`Word '${word}' was found, but relations couldn't be loaded due to a database configuration issue. Basic information will still be displayed.`);
        
        // Try to fetch the word network anyway to show some data
        try {
          console.log(`Fetching network for word with missing relation_data: ${word}`);
          const networkData = await fetchWordNetworkData(word, depth, breadth);
          setWordNetwork(networkData);
          
          // If we have network data, we can show a basic version of the word
          if (networkData && networkData.nodes && networkData.nodes.length > 0) {
            // Find the main node (type='main' or first node)
            const mainNode = networkData.nodes.find(n => n.type === 'main') || networkData.nodes[0];
            
            // Create a minimal word object with the network data
            const minimalWord: WordInfo = {
              id: Number(mainNode.id),
              lemma: mainNode.label || mainNode.word,
              normalized_lemma: mainNode.label?.toLowerCase() || null,
              language_code: mainNode.language || null,
              has_baybayin: !!mainNode.has_baybayin,
              baybayin_form: mainNode.baybayin_form || null,
          ...
          console.error("Error fetching network fallback:", networkError);
        }
  ...
  const handleSuggestionClick = useCallback(async (suggestion: SearchWordResult) => {
    ...
    try {
      ...
      await loadWordData(suggestion.lemma);
  ...
  useEffect(() => {
    // Only proceed if we have wordData
    if (wordData && wordData.id) {
      // Use direct word relations instead of extracting from network
      const hasRelations = (
      ...
      // If there are no relations in the current wordData, try both approaches
      if (!hasRelations && mainWord) {
        console.log("Word has no relations. Trying direct relations API endpoint.");
        
        // First, try the dedicated relations endpoint
        fetchWordRelations(mainWord)
          .then(relations => {
            const hasApiRelations = (
              relations.incoming_relations.length > 0 || 
              relations.outgoing_relations.length > 0
            );
            
            if (hasApiRelations) {
              console.log("Got relations from direct relations API:", {
                incoming: relations.incoming_relations.length,
                outgoing: relations.outgoing_relations.length
              });
              
              // Update both wordData and selectedWordInfo to ensure the UI shows the relations
              const updatedWordData = {
                ...wordData,
                incoming_relations: relations.incoming_relations,
                outgoing_relations: relations.outgoing_relations
              };
              
              setWordData(updatedWordData);
              
              // Also update selectedWordInfo if it's the same word
              if (selectedWordInfo && selectedWordInfo.id === wordData.id) {
                setSelectedWordInfo(updatedWordData);
              }
            } else if (wordNetwork) {
              // If direct relations API didn't return anything but we have a network,
              // try to fetch complete word details as a fallback
              console.log("No relations from direct API. Trying complete word details as fallback.");
              fetchWordDetails(wordData.id.toString())
                .then(fullWordData => {
                  const hasDetailRelations = (
                    (fullWordData.incoming_relations && fullWordData.incoming_relations.length > 0) || 
                    (fullWordData.outgoing_relations && fullWordData.outgoing_relations.length > 0)
                  );
                  
                  if (hasDetailRelations) {
                    console.log("Got relations from complete word details:", {
                      incoming: fullWordData.incoming_relations?.length || 0,
                      outgoing: fullWordData.outgoing_relations?.length || 0
                    });
                    
                    setWordData(fullWordData);
                    ...
                    console.log("No relations found in any API. Word might not have relations.");
                  }
                })
                .catch(error => {
                  console.error("Error fetching complete word details:", error);
                });
            }
          })
          .catch(error => {
            ...
            // If direct relations API fails, try the fallback approach
            if (wordNetwork) {
              console.log("Error with relations API. Trying word details as fallback.");
              fetchWordDetails(wordData.id.toString())
                .then(fullWordData => {
                  // Only update if we got relations
                  if (
                    (fullWordData.incoming_relations && fullWordData.incoming_relations.length > 0) || 
                    (fullWordData.outgoing_relations && fullWordData.outgoing_relations.length > 0)
                  ) {
                    console.log("Got relations from word details fallback:", {
                      incoming: fullWordData.incoming_relations?.length || 0,
                      outgoing: fullWordData.outgoing_relations?.length || 0
                    });
                ...
                .catch(detailsError => {
                  console.error("Error with fallback word details:", detailsError);
                });
            }
          });
      }
    }
  }, [wordData?.id, wordNetwork, mainWord, selectedWordInfo]);
