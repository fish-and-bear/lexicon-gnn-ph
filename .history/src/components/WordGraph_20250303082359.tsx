...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  // Improved simulation setup for better layout
  const setupSimulation = useCallback((nodes: CustomNode[], links: CustomLink[]) => {
    try {
      ...
      const simulation = d3.forceSimulation<CustomNode>()
        ...
        .alpha(0.3)           // Start with moderate energy
        ...
        .force("center", d3.forceCenter(0, 0).strength(0.05)) // Gentle centering force
        ...
        .force("x", d3.forceX(0).strength(0.03))  // Very gentle force toward center x
        .force("y", d3.forceY(0).strength(0.03)); // Very gentle force toward center y

      // Give main word a stronger initial position
      const mainNode = nodes.find(n => n.id === mainWord);
      if (mainNode) {
        mainNode.fx = 0;
        mainNode.fy = 0;
        
        // Release the main node after initial layout
        setTimeout(() => {
          if (mainNode) {
            mainNode.fx = null;
            mainNode.fy = null;
    ...
    return d3.drag<SVGGElement, CustomNode>()
      .on("start", (event, d) => {
        try {
          ...
          const nodeElement = d3.select(event.sourceEvent.currentTarget).closest('.node');
          if (nodeElement) {
