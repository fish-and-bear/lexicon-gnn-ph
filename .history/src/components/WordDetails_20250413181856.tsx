...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({
  ...
  const renderRelationsTab = () => {
    ...
    // Add degree information for regular relations (incoming/outgoing are always direct/degree=1)
    ...
    // Deduplicate relations by word lemma
    ...
    filteredRelations.forEach(relation => {
      ...
      if (!wordMap.has(wordLemma)) {
        ...
        newRelation.relationTypes = [relation.relation_type];
        wordMap.set(wordLemma, newRelation);
      } else {
        // Word already exists, add this relation type if it's new
    ...
    const deduplicatedRelations = Array.from(wordMap.values());
    
    // Define relation categories matching the graph legend
    ...
    // Group relations into categories
    const categorizedRelations: Record<string, any[]> = {};
    
    // Initialize categories
    relationCategories.forEach(category => {
      categorizedRelations[category.name] = [];
    });
    
    // Categorize all relations
    deduplicatedRelations.forEach(relation => {
      const relType = relation.relation_type || 'other';
      let placed = false;
      
      // Try to place in a category
      for (const category of relationCategories) {
        if (category.types.includes(relType)) {
          categorizedRelations[category.name].push({
            ...relation,
            originalType: relType // Store original type for reference
          });
          placed = true;
          break;
        }
      ...
      // If not placed in any category, put in Other
      if (!placed) {
    ...
    const uniqueRelatedWords = deduplicatedRelations.length;
    
    // Filter out empty categories
    const nonEmptyCategories = relationCategories.filter(
      category => categorizedRelations[category.name].length > 0
    );
      ...
      <Box sx={{ pt: theme.spacing(1) }}>
        ...
        <Box sx={{ mb: theme.spacing(3) }}>
          {nonEmptyCategories.map((category, categoryIndex) => {
            const relations = categorizedRelations[category.name];
            ...
            return (
              ...
              >
                ...
                <Box sx={{ pl: theme.spacing(2) }}>
                  {(() => {
                    // Group relations by their original type
                    const typeGroups: Record<string, any[]> = {};
                    
                    relations.forEach(relation => {
                      const type = relation.originalType || 'other';
                      if (!typeGroups[type]) {
                        typeGroups[type] = [];
                      }
                    ...
                    // Sort relation types alphabetically
                                ...
                                <Chip
                                      ...
                                      {relation.degree > 1 && (
                                        <Box 
                                          ...
                                          sx={{ 
                                            ...
                                            display: 'inline-block',
                                            mr: 0.8,
                                            boxShadow: theme.palette.mode === 'dark' ? '0 0 0 1px rgba(255,255,255,0.2)' : '0 0 0 1px rgba(0,0,0,0.1)'
                                          }}
                                  ...
                                  sx={{
                                    ...
                                    '& .MuiChip-label': { 
                                      px: 1.5, 
                                      fontWeight: 500,
                                      overflow: 'visible'  // Allow count to be visible
                                    },
                                    '&:hover': {
