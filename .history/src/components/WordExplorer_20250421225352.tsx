...
const WordExplorer: React.FC = () => {
  ...
  // Add new state for mobile panel selection
  const [activeMobilePanel, setActiveMobilePanel] = useState<'graph' | 'details'>('details');
  
  // Improve media query checks for more precise device targeting
  ...
  return (
    ...
    >
      ...
      {!isWideLayout && !error && wordData && (
        <div className="mobile-panel-tabs">
          <button 
            className={`panel-tab ${activeMobilePanel === 'details' ? 'active' : ''}`}
            onClick={() => setActiveMobilePanel('details')}
          >
            Word Details
          </button>
          <button 
            className={`panel-tab ${activeMobilePanel === 'graph' ? 'active' : ''}`}
            onClick={() => setActiveMobilePanel('graph')}
          >
            Word Network
          </button>
        </div>
        ...
        {isWideLayout ? (
          ...
          <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div className="details-container" 
                 style={{ 
                   display: (activeMobilePanel === 'details' || !wordData) ? 'block' : 'none',
                   height: '100%', 
                   width: '100%', 
                   overflow: 'auto'
                 }}
            ...
            {/* Graph Panel - Only show when selected and word data exists */}
            <div className="graph-container" 
                 style={{ 
                   display: (activeMobilePanel === 'graph' && wordData) ? 'block' : 'none',
                   height: '100%', 
                   width: '100%'
                 }}>
              <div className="graph-content" style={{ width: '100%', height: '100%' }}>
