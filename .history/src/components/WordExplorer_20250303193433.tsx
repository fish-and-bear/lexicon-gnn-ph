...
const WordExplorer: React.FC<WordExplorerProps> = ({ initialWord }) => {
  ...
  const fetchWordNetworkData = useCallback(async (searchWord: string, searchDepth: number = 2, searchBreadth: number = 10) => {
    try {
      setIsLoading(true);
      setError(null);
  ...
  const handleSearch = useCallback(async (searchWord: string) => {
    ...
    if (normalizedWord) {
      ...
      await fetchWordNetworkData(normalizedWord, depth, breadth);
    }
  ...
  const handleNetworkChange = useCallback(async (newDepth: number, newBreadth: number) => {
    setDepth(newDepth);
    setBreadth(newBreadth);
    if (mainWord) {
      await fetchWordNetworkData(mainWord, newDepth, newBreadth);
    }
