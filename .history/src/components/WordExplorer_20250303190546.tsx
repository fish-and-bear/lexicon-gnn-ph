  ...
  const fetchWordNetworkData = useCallback(async (word: string, depth: number = 2, breadth: number = 10) => {
    try {
      ...
      const data = await fetchWordNetwork(word, { 
        ...
        max_nodes: breadth * depth, // Limit total nodes for better performance
        limit_nodes: true, // Enable node limiting
        limit_edges: true, // Enable edge limiting
        min_weight: 0.2 // Only include stronger relationships
      });
      ...
      // Pre-process nodes in a single pass
      ...
      // Process edges using the nodeMap
      if (Array.isArray(data.edges)) {
        networkData.edges = data.edges
          .filter(edge => edge.weight === undefined || edge.weight >= 0.2) // Filter weak relationships
          .map(edge => ({
            source: edge.source,
            target: edge.target,
