...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({
  ...
  const renderRelationsTab = () => {
    ...
    function createRelationsFromNetwork() {
      ...
      // Try to find the main node ID
      const mainNodeId = semanticNetworkNodes.find(n => 
        n.type === 'main' || 
        (n as any).main === true || 
        n.label === mainWord ||
        n.word === mainWord
      ...
      console.log("Main node identification:", { 
        mainWord, 
        mainNodeId, 
        nodeCount: semanticNetworkNodes.length,
        mainNodesFound: semanticNetworkNodes.filter(n => 
          n.type === 'main' || 
          (n as any).main === true || 
          n.label === mainWord ||
          n.word === mainWord
      ...
      // If no main node could be found, use a simpler approach to create relations
      if (!mainNodeId) {
        console.log("No main node found in semantic network, using simplified approach");
        
        // If there's no clear main node, simply use all links to build relations
        semanticNetworkLinks.forEach(link => {
          const sourceNode = semanticNetworkNodes.find(
          ...
          relations.push({
            ...
            degree: link.distance || link.depth || 1,
            direction: 'outgoing',
            wordObj: {
              id: targetNode.id,
        ...
        console.log(`Created ${relations.length} relations using simplified approach`);
        return relations;
      }
      ...
      semanticNetworkLinks.forEach(link => {
        ...
        // Create a relation based on direction
        if (sourceNode.id === mainNodeId) {
          // Outgoing relation (from main word to target)
          relations.push({
        ...
        } else if (targetNode.id === mainNodeId) {
          // Incoming relation (from source to main word)
          relations.push({
            id: `semantic-${sourceNode.id}`,
            relation_type: link.type || 'related',
    ...
    const hasAnyRelations = hasStandardRelations || semanticRelations.length > 0 || hasSemanticNetwork || 
