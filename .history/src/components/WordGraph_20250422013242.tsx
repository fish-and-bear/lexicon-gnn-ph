...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const renderLegendContent = useCallback(() => {
    // Define categories directly within this scope
    const categories = [
      { name: "Core", types: ["main"] },
      { name: "Origin", types: ["root", "derived"] }, 
      { name: "Meaning", types: ["synonym", "antonym", "related"] },
      { name: "Form", types: ["variant"] },
      { name: "Structure", types: ["taxonomic", "part_whole"] }, // Use internal types consistently
      { name: "Info", types: ["usage"] }
    ];

    return (
      <List dense sx={{ bgcolor: 'background.paper', borderRadius: 1, mt: 1, maxHeight: 'calc(100% - 150px)', overflow: 'auto', width: '100%' }}>
        <ListSubheader sx={{ bgcolor: 'transparent', lineHeight: '24px' }}>Relationship Types (Click to filter)</ListSubheader>
    ...
    // Remove categories from dependency array as it's defined inside
