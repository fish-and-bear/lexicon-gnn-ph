...
import { WordNetworkOptions } from "../api/wordApi";
          ...
          fetchWordNetwork(wordText, networkFilterOptions)
            .then(network => {
              ...
              if (network.nodes.length > 0) {
                const mainNode = network.nodes.find(n => n.type === 'main') || network.nodes[0];
                const minimalWord: WordInfo = {
                  id: Number(mainNode.id),
                  lemma: mainNode.label,
                  normalized_lemma: mainNode.label.toLowerCase(),
  ...
  const handleNodeClick = useCallback(async (word: string) => {
    ...
    try {
      ...
      if (error.message.includes('not found')) {
        ...
        // Try to fetch the word network to at least show the graph
        try {
          ...
          // Just set the network data - don't try to create a word object from it
          setWordNetwork(networkData);
        } catch (networkError) {
    ...
    <div className="word-explorer-container">
      <div className="search-section">
        <form onSubmit={(e) => { e.preventDefault(); handleSearch(inputValue); }}>
          ...
          <button type="button" onClick={handleRandomWord}>Random Word</button>
        </form>
      ...
      {/* You can conditionally render additional content like statistics, etc. */}
    </div>
  );
}; // This is the end of the component function
