...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const getUniqueRelationshipGroups = useCallback((): RelationshipGroups => {
    // Original mapping of relationship types to category, label, color
    const typeMappings: Record<string, RelationshipTypeInfo> = {
       // Core
      main: { category: "Core", label: "Main Word", color: getNodeColor("main") },
      
      // Origin
      root: { category: "Origin", label: "Root/Derived", color: getNodeColor("root") },
      root_of: { category: "Origin", label: "Root/Derived", color: getNodeColor("root") },
      ...
      // Derived (consolidated with Root in display)
      derived: { category: "Origin", label: "Root/Derived", color: getNodeColor("derived") },
      derived_from: { category: "Origin", label: "Root/Derived", color: getNodeColor("derived") },
      derivative: { category: "Origin", label: "Root/Derived", color: getNodeColor("derived") },
      ...
      // Form (all variants consolidated)
      ...
      // Structure (part-whole relationships)
      ...
      part_whole: { category: "Structure", label: "Components/Parts", color: getNodeColor("part_whole") },
      ...
      // Info
      affix: { category: "Info", label: "Usage Note", color: getNodeColor("derived") },
      usage: { category: "Info", label: "Usage Note", color: getNodeColor("usage") },
      associated: { category: "Info", label: "Usage Note", color: getNodeColor("related") },
      other: { category: "Info", label: "Usage Note", color: "#adb5bd" }
    };

    // Intermediate structure to group types by category and then by unique label
    ...
    Object.entries(typeMappings).forEach(([type, info]) => {
      ...
      if (!groupedByCategoryAndLabel[info.category][info.label]) {
        groupedByCategoryAndLabel[info.category][info.label] = {
          label: info.label,
          color: info.color, // Use color from the first type encountered for this label
          types: []
        };
      }
    ...
    const categoryOrder = ["Core", "Origin", "Meaning", "Form", "Structure", "Info"];

    // Build the final categories array in the desired order
    const finalCategories: Array<{ name: string; labels: RelationshipLabelInfo[] }> = categoryOrder
      .filter(categoryName => groupedByCategoryAndLabel[categoryName]) // Only include categories present in data
      .map(categoryName => ({
        name: categoryName,
    ...
    return {
      uniqueTypes: typeMappings, // Return original full mapping
  ...
  const { filteredNodes, filteredLinks } = useMemo(() => {
    ...
    const finalNodes = relationshipFilteredNodes;
  ...
  const getNodeRadius = useCallback((node: CustomNode) => {
    const baseRadius = isMobile ? 10 : 13; // Smaller base radius for mobile
    const mainMultiplier = isMobile ? 1.6 : 2.0; // Less pronounced main node on mobile
    const rootMultiplier = isMobile ? 1.3 : 1.6;

    if (node.id === mainWord) return baseRadius * mainMultiplier;
    if (node.group === 'root') return baseRadius * rootMultiplier;
    return baseRadius;
  }, [mainWord, isMobile]); // Add isMobile dependency
  ...
  const createNodes = useCallback((
    ...
    const nodeGroups = g.append("g")
      ...
      .attr("fill", d => getNodeColor(d.group))
                  ...
                  .attr("stroke-width", 1.5)
                  .attr("shape-rendering", "geometricPrecision"); // Add shape-rendering for smoother circles
        ...
        .join(
            enter => {
                const textElement = enter.append("text")
                    ...
                    // Adjust vertical offset based on mobile status
                    .attr("y", d => (d.y ?? 0) + getNodeRadius(d) + (isMobile ? 8 : 12))
                    .style("opacity", 0) // Start transparent
                ...
                textElement.clone(true)
                    ...
                    .attr("stroke-width", isMobile ? 2 : 3) // Thinner halo on mobile
  ...
  }, [createDragBehavior, getNodeRadius, getNodeColor, theme, mainWord, isMobile]);
  ...
  useEffect(() => {
    ...
    if (!isMobile) {
      // --- Apple-Inspired Legend Redesign ---
      const legendPadding = 16; // Increased padding
      const legendItemHeight = 24; // Increased height for more spacing
      const dotRadius = 4; // Slightly smaller dots
      const textPadding = 12; // Space between dot and text
      const categorySpacing = 12; // Increased space between categories
      const maxLabelWidth = 120; // Max width for item labels
      ...
      const initialLegendWidth = 200; // Start with an estimate
      ...
      legendCategories.forEach((category) => {
        tempText.style("font-weight", 600).text(category.name); // Bolder for category
        ...
        category.labels.forEach(labelInfo => {
          tempText.style("font-weight", 400).text(labelInfo.label); // Regular for item
      ...
      // Adjust container position based on calculated width
      legendContainer.attr("transform", `translate(${Math.max(width - legendWidth - 20, 10)}, 20)`);

      // Calculate height dynamically
      let calculatedHeight = legendPadding; // Start with top padding
      calculatedHeight += 24; // Space for title
      calculatedHeight += 18; // Space for subtitle
      calculatedHeight += categorySpacing; // Space after subtitle
      ...
      legendCategories.forEach(category => {
        ...
        calculatedHeight += categorySpacing; // Space after category
      });
      calculatedHeight += legendPadding; // Bottom padding
      const legendHeight = calculatedHeight - categorySpacing; // Remove last category spacing

      // --- Render Legend Elements --- 
      // Background Rectangle (Softer)
      legendContainer.append("rect")
        ...
        .attr("ry", 10)
        .attr("fill", bgColor)
        // .attr("filter", "url(#legend-blur)") // Optional: requires defining SVG filter
        // Remove stroke or make very subtle
      ...
      legendContainer.append("text")
        ...
        .attr("y", legendPadding + 10) // Adjust position
        .attr("text-anchor", "middle")
        .style("font-size", muiTheme.typography.pxToRem(13))
        .style("font-weight", 600)
        .attr("fill", textColorPrimary)
        .text("Relationship Types");

      // Subtitle (Smaller, less prominent)
      legendContainer.append("text")
        .attr("x", legendWidth / 2)
        .attr("y", legendPadding + 26) // Position below title
        .attr("text-anchor", "middle")
      ...
      let yPos = legendPadding + 40 + categorySpacing; // Start below subtitle

      legendCategories.forEach((category) => {
        // Category Header
        legendContainer.append("text")
          .attr("x", legendPadding)
          .attr("y", yPos + legendItemHeight / 2) // Center vertically in allocated space
        ...
        yPos += legendItemHeight; // Move down past header
        ...
        category.labels.forEach(labelInfo => {
          ...
          const entry = legendContainer.append("g")
            ...
            .style("opacity", itemOpacity) // Apply initial opacity
            .on("mouseover", function(this: SVGGElement) {
              if (itemOpacity === 1) { // Only brighten if not filtered
            ...
            entry.append("circle")
              .attr("cx", dotRadius)
              .attr("cy", 0) // Vertically centered by the group transform
              .attr("r", dotRadius)
              .attr("fill", labelInfo.color)
              .attr("stroke", alpha(labelInfo.color, 0.5))
              .attr("stroke-width", 0.5);

            // Label Text
            entry.append("text")
              .attr("x", dotRadius * 2 + textPadding)
              .attr("y", 0) // Vertically centered by the group transform
              .attr("dy", ".35em") // Vertical alignment helper
              .style("font-size", muiTheme.typography.pxToRem(11))
              .style("font-weight", 400)
              .attr("fill", textColorPrimary)
              .text(labelInfo.label);

          yPos += legendItemHeight; // Move down for next item
        });
        yPos += categorySpacing; // Add space after category
      });
  ...
  return (
    <div className="graph-container" style={{ position: 'relative', width: '100%', height: '100%', overflow: 'hidden' }}>
      <div className="graph-svg-container" style={{ width: '100%', height: '100%', pointerEvents: 'none' }}> {/* Make container ignore clicks */}
        {isLoading && (
        ...
        <svg 
          ...
          style={{ width: '100%', height: '100%', pointerEvents: 'auto' }} // Make SVG capture events
        >
        </svg>
      </div>
      
      {/* CONTROLS AREA */} 
      <div className="controls-container" style={{ pointerEvents: 'none' /* Ignore clicks on container */ }}>
        {/* Zoom Controls - Always visible */} 
        <div className="zoom-controls" style={{ pointerEvents: 'auto' /* Capture clicks */ }}>
        ...
        {!isMobile ? (
          ...
          <IconButton 
            ...
            sx={{ 
              ...
              minWidth: 40, // Enforce minimum width
              minHeight: 40, // Enforce minimum height
