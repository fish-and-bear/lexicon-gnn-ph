  ...
  const handleRandomWord = useCallback(async () => {
    ...
    try {
      ...
      if (randomWordCache.length > 0) {
        ...
        const currentWord = selectedWordInfo?.lemma?.toLowerCase();
        
        // Create a filtered cache that excludes the current word
        let filteredCache = [...randomWordCache];
        if (currentWord && filteredCache.length > 1) {
          // Only filter if we have more than one word to choose from
          filteredCache = filteredCache.filter(word => 
            word.lemma.toLowerCase() !== currentWord
          );
        }
        ...
        } else if (randomWordCache.length > 0) {
          // Fallback: If filtering removed all options, but original cache still has words
          console.warn("Could not find a different random word, picking from original cache.");
          const originalCacheIndex = Math.floor(Math.random() * randomWordCache.length);
          randomWord = randomWordCache[originalCacheIndex];
        } else {
          // If both caches are somehow empty here, randomWord will remain null
          // This case should ideally be handled by the 'else' block below (direct fetch)
          console.error("Error: Filtered and original caches are empty during selection.");
        }
        
        // Now remove this word from the actual cache
        const indexInOriginalCache = randomWordCache.findIndex(
        ...
        if (indexInOriginalCache !== -1) {
          ...
          console.log(`Selected new random word: ${randomWord.lemma}${
            currentWord ? ` (different from current: ${currentWord})` : ''
          }`);
        }
        
        // Refill the cache in the background when it gets low
        ...
        if (words.length > 0) {
          randomWord = words[0];
          // Update the cache with the remaining words
          setRandomWordCache(words.slice(1)); 
        } else {
          // This case should ideally not happen if the previous check works
