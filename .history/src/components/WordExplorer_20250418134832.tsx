...
const WordExplorer: React.FC = () => {
  ...
  useEffect(() => {
    // debouncedSearchRef.current = debounce(async (query: string) => { // Step 4: Comment out usage
    const searchFunction = async (query: string) => { // Use non-debounced version
    ...
    // }, 300); // Step 4: Comment out usage
    debouncedSearchRef.current = searchFunction; // Assign non-debounced function
    
    // Cleanup function
    return () => {
      // No debounce cancellation needed
    };
  }, []); // Dependencies might need adjustment if searchFunction uses state/props
