...
const DefinitionCard = styled(Box)(({ theme }) => ({
  ...
  border: theme.palette.mode === 'dark'
    ? 'none' // Remove border completely in dark mode
    : `1px solid ${theme.palette.divider}`,
  boxShadow: theme.palette.mode === 'dark'
    ? '0 2px 4px rgba(0, 0, 0, 0.3)' // Slightly darker shadow for depth instead of border
    : 'none',
}));
...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({
  ...
  // Add these new render functions before the final return statement

  // Render the pronunciation tab
  const renderPronunciationTab = () => {
    if (!wordInfo.pronunciations || wordInfo.pronunciations.length === 0) {
      return <Alert severity="info">No pronunciation data available for this word.</Alert>;
    }

    return (
      <Box>
        <Typography variant="h6" gutterBottom>Pronunciation</Typography>
        <List>
          {wordInfo.pronunciations.map((pronunciation, index) => (
            <ListItem key={pronunciation.id || index} sx={{ py: 1 }}>
              <Box width="100%">
                <Stack direction="row" alignItems="center" spacing={1}>
                  <Typography variant="subtitle1" fontWeight="medium">
                    {pronunciation.type === 'IPA' ? 'IPA:' : 
                     pronunciation.type === 'audio' ? 'Audio:' : 
                     pronunciation.type + ':'}
                  </Typography>
                  ...
                  {pronunciation.type === 'audio' && (
                    <Button 
                      onClick={playAudio}
                      startIcon={isAudioPlaying ? <StopCircleIcon /> : <VolumeUpIcon />}
                      variant="outlined"
                      size="small"
                    >
                  ...
                  {pronunciation.type !== 'IPA' && pronunciation.type !== 'audio' && (
                ...
                {pronunciation.tags && (
                  <Box mt={1}>
                    <Typography variant="caption" color="text.secondary">
                      Tags: {typeof pronunciation.tags === 'string' ? 
                            pronunciation.tags : 
                            JSON.stringify(pronunciation.tags)}
                    </Typography>
                  </Box>
                )}
                {pronunciation.sources && (
                  <Box mt={0.5}>
                    <Typography variant="caption" color="text.secondary">
                      Source: {typeof pronunciation.sources === 'string' 
                              ? pronunciation.sources 
                              : (typeof pronunciation.sources === 'object' && pronunciation.sources !== null)
                                ? JSON.stringify(pronunciation.sources)
                                : ''}
                    </Typography>
                  </Box>
                )}
  ...
  // Render the idioms tab
  const renderIdiomsTab = () => {
    if (!wordInfo.idioms || (Array.isArray(wordInfo.idioms) && wordInfo.idioms.length === 0) || 
        (typeof wordInfo.idioms === 'object' && Object.keys(wordInfo.idioms).length === 0)) {
      return <Alert severity="info">No idioms available for this word.</Alert>;
    }

    // Handle both array and object formats
    const idiomsList = Array.isArray(wordInfo.idioms) 
      ? wordInfo.idioms 
      : Object.entries(wordInfo.idioms).map(([key, value]) => {
          if (typeof value === 'string') {
            return { phrase: key, meaning: value };
          }
    ...
    return (
      <Box>
        <Typography variant="h6" gutterBottom>Idioms</Typography>
        {idiomsList.map((idiom, index) => (
          <DefinitionCard key={index} sx={{ mb: 2 }}>
            <Typography variant="subtitle1" fontWeight="medium" gutterBottom>
              {idiom.phrase || idiom.text || `Idiom ${index + 1}`}
            </Typography>
            {idiom.meaning && (
  ...
  // Dedicated Affixations Tab
  const renderAffixationsTab = () => {
    const rootAffix = wordInfo.root_affixations || [];
    const affixedAffix = wordInfo.affixed_affixations || [];

    if (rootAffix.length === 0 && affixedAffix.length === 0) {
      return <Alert severity="info">No affixation data available for this word.</Alert>;
    }

    return (
      <Box>
        <Typography variant="h6" gutterBottom>Affixations</Typography>
        {rootAffix.length > 0 && (
          <DefinitionCard sx={{ mb: 2 }}>
            <Typography variant="subtitle1" fontWeight="medium" gutterBottom>
              Derived via Affixation
            </Typography>
            <List dense>
              {rootAffix.map((affix: Affixation, index: number) => (
                <ListItem key={affix.id || index} disableGutters sx={{ py: 1 }}>
                  <ListItemText
                    primary={
                      <Typography component="span" variant="body1" sx={{ 
                        ...
                        mr: 1 
                      }} onClick={() => affix.affixed_word && onWordLinkClick(affix.affixed_word.lemma)}>
                      ...
                      <Box>
                        ...
                        {affix.sources && (
                          <Typography variant="caption" display="block" sx={{ mt: 0.5 }}>
                            Source: {typeof affix.sources === 'string' 
                                    ? affix.sources 
                                    : (typeof affix.sources === 'object' && affix.sources !== null)
                                      ? JSON.stringify(affix.sources)
                                      : ''}
                          </Typography>
                        )}
                      </Box>
                  ...
                  <ListItemText
                    primary={
                      <Typography component="span" variant="body1" sx={{ 
                        ...
                        textDecoration: 'underline',
                        mr: 1 
                      }} onClick={() => affix.root_word && onWordLinkClick(affix.root_word.lemma)}>
  ...
  // Render the credits tab
  const renderCreditsTab = () => {
    if (!wordInfo.credits || wordInfo.credits.length === 0) {
      return <Alert severity="info">No credits information available for this word.</Alert>;
    }
          ...
          {wordInfo.credits.map((credit, index) => (
            <ListItem key={credit.id || index} sx={{ py: 1 }}>
              <ListItemText
                ...
                secondary={
                  <Typography variant="caption" color="text.secondary">
                    {credit.created_at && `Added: ${new Date(credit.created_at).toLocaleDateString()}`}
                    {credit.updated_at && credit.created_at !== credit.updated_at && 
                     ` • Updated: ${new Date(credit.updated_at).toLocaleDateString()}`}
                  </Typography>
                }
              />
            </ListItem>
            ...
            const tabContent = (
               <Box sx={{ 
                  ...
                  <Box sx={{ px: theme.spacing(3), py: theme.spacing(2) }}>
                      ...
                      {activeTab === 'idioms' && renderIdiomsTab()}
               ...
               <Box sx={{ height: '100%' }}>
                 ...
                 >
                   ...
                   {wordInfo.pronunciations && wordInfo.pronunciations.length > 0 && (
                     <Tab label="Pronunciation" value="pronunciation" />
                   )}
                   {/* Tab Affixations */}
