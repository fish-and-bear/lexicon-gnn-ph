...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const { filteredNodes, filteredLinks } = useMemo(() => {
    ...
    // If showDisconnectedNodes is true, include all nodes regardless of connectivity
    const depthLimitedNodes = showDisconnectedNodes 
      ? baseNodes 
      : baseNodes.filter(node => connectedNodeIds.has(node.id));
    ...
    const relationshipFilteredNodes = depthLimitedNodes.filter(node => {
      ...
      // If this node is disconnected but we're showing disconnected nodes, keep it
      if (showDisconnectedNodes && !connectedNodeIds.has(node.id)) {
        console.log(`[FILTER] Keeping disconnected node ${node.id} (showDisconnectedNodes=true)`);
        return true;
      }
  ...
  useEffect(() => {
    ...
    const toggleTrack = toggleContainer.append("rect")
      ...
      .attr("fill", showDisconnectedNodes ? 
        ...
        (theme === "dark" ? "#4e5462" : "#d1d5db"))
      .attr("opacity", showDisconnectedNodes ? 1 : 0.7)
      .attr("stroke", theme === "dark" ? "rgba(255, 255, 255, 0.1)" : "rgba(0, 0, 0, 0.05)")
      .attr("stroke-width", 1);
    
    // Add toggle handle (circle)
    const toggleHandle = toggleContainer.append("circle")
      .attr("r", 6)
      .attr("cx", showDisconnectedNodes ? 24 : 8)
      .attr("cy", 2)
    ...
    toggleContainer.on("click", function() {
      ...
      // Update toggle visual state with nice transition
      toggleTrack
        .transition().duration(250)
        .attr("fill", newValue ? 
          (theme === "dark" ? "#4c6ef5" : "#3b82f6") : 
          (theme === "dark" ? "#4e5462" : "#d1d5db"))
        .attr("opacity", newValue ? 1 : 0.7);
      
      toggleHandle
        .transition().duration(250)
        .attr("cx", newValue ? 24 : 8);
    });
