...
const WordExplorer: React.FC = () => {
  ...
  const [detailsWidth, setDetailsWidth] = useState<number>(400);

  // Initialize details width from localStorage
  useEffect(() => {
    const savedWidth = localStorage.getItem('wordDetailsWidth');
    if (savedWidth) {
      setDetailsWidth(parseInt(savedWidth, 10));
    }
  }, []);

  // Save details width to localStorage when it changes
  useEffect(() => {
    if (detailsWidth) {
      localStorage.setItem('wordDetailsWidth', detailsWidth.toString());
    }
  }, [detailsWidth]);
    ...
    const resizeObserver = new ResizeObserver(entries => {
      for (const entry of entries) {
        if (entry.target === detailsContainerRef.current) {
          setDetailsWidth(entry.contentRect.width);
        }
      }
    });
  ...
  return (
    <div className={`word-explorer ${theme}`}>
      ...
      <main>
        ...
        <div className="word-details-container" ref={detailsContainerRef} style={{ width: `${detailsWidth}px` }}>
