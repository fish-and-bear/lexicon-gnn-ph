...
const WordExplorer: React.FC = () => {
  ...
  const [breadth, setBreadth] = useState<number>(8);
  ...
  const fetchWordNetworkData = useCallback(async (word: string, currentDepth: number = 2, currentBreadth: number = 8) => {
    try {
      setIsLoading(true);
      const data = await fetchWordNetwork(word, { depth: currentDepth, breadth: currentBreadth });
      setWordNetwork(data);
      return data;
    } catch (fetchError) {
  ...
  }, [mainWord, fetchWordNetworkData, breadth]);
  ...
  const handleNetworkReset = useCallback(() => {
    if (mainWord) {
      ...
      setBreadth(8);  // Reset to default breadth
      fetchWordNetworkData(normalizeInput(mainWord), 2, 8);
    }
