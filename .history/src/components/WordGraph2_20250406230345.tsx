...
const WordGraph2: React.FC<WordGraphProps> = ({
  ...
  useEffect(() => {
    ...
    console.log(`Rendering graph with input:`, wordNetwork);
    
    // Get container dimensions
    const width = svg.node()?.parentElement?.clientWidth || 600;
    const height = svg.node()?.parentElement?.clientHeight || 400;
    console.log(`Rendering area dimensions: ${width}x${height}`);

    // Create the SVG group for the graph
    const g = svg.append('g')
      .attr('class', 'word-graph');

    // Check if we have valid data to display - both nodes and edges array should exist
    ...
    console.log(`Found ${wordNetwork.nodes.length} nodes and ${edgesArray.length} edges`);
    ...
    const nodes = wordNetwork.nodes.map((node, index) => {
      ...
      const radius = nodeLemma === mainWord ? 0 : 150; // Larger radius for better spacing
      ...
      return {
        ...
        index: index,
        x: x, // Position with initial coordinates
        y: y, // Position with initial coordinates
        vx: 0,
        vy: 0,
        fx: nodeLemma === mainWord ? width/2 : undefined, // Fix main word in center
        fy: nodeLemma === mainWord ? height/2 : undefined  // Fix main word in center
      };
    });
    ...
    const links = edgesArray
      .filter(edge => {
        ...
        if (!sourceExists || !targetExists) {
          console.warn(`Skipping edge - missing nodes: ${source} → ${target}`);
        }
