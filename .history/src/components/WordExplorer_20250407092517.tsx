...
const WordExplorer: React.FC = () => {
  ...
  const memoizedWordNetwork = useMemo(() => {
    ...
    const validEdges = (wordNetwork.edges || []).map((edge, index) => {
        const sourceId = typeof edge.source === 'object' ? String(edge.source.id) : String(edge.source);
        const targetId = typeof edge.target === 'object' ? String(edge.target.id) : String(edge.target);
        return {
           ...edge,
