...
const relationColors: Record<string, string> = {
  // Core
  main: "#1a5eaa",      // Deep blue - standout color for main word
  
  // Origin group - Reds and oranges
  root: "#e63946",      // Bright red
  etymology: "#d00000", // Dark red
  cognate: "#fa6d35",   // Light orange
  
  // Meaning group - Blues
  synonym: "#3580b5",   // Medium blue
  related: "#0db3d9",   // Light blue
  antonym: "#004daa",   // Dark blue
  similar: "#8fcbd2",   // Pale blue (similar to related)
...
// Define separate, more vibrant dark mode colors
const darkModeRelationColors: Record<string, string> = {
  // Core - Brighter versions for dark mode
  main: "#5d9cec",       // Brighter blue
  
  // Origin group - Brighter reds and oranges
  root: "#ff6b81",       // Brighter red
  etymology: "#ff5555",  // Vivid red
  cognate: "#ff8c63",    // Saturated orange
  
  // Meaning group - Brighter blues
  synonym: "#74c5ff",    // Brighter blue
  related: "#67daff",    // Vivid light blue
  antonym: "#6b8aff",    // Vibrant dark blue
  similar: "#b6e1e4",    // Light blue
  
  // Form group - Brighter purples
  variant: "#bf86ff",    // Brighter purple
  spelling: "#d17dff",   // Bright purple
  abbreviation: "#9c7bff", // Saturated purple
  form_of: "#b68cff",    // Brighter blue-purple
  
  // Hierarchy group - Brighter greens
  taxonomic: "#3dccbd",  // Bright teal
  hypernym: "#4cdbcc",   // Different shade of teal to avoid duplicate
  hyponym: "#6adba0",    // Bright green
  meronym: "#50c78a",    // Bright forest green
  holonym: "#5ed69a",    // Different shade of green to avoid duplicate
  part_whole: "#50c78a", // Bright forest green
  component_of: "#42b97d", // Different shade of green to avoid duplicate
  component: "#5ad9a4",  // Different shade of green to avoid duplicate
  
  // Derivational group - Brighter yellow-greens
  derived: "#45e6d4",    // Bright teal
  affix: "#75b175",      // Bright olive
  derivative: "#8cb33f", // Bright olive
  
  // Info group - Brighter yellows/oranges
  usage: "#ffcc54",      // Bright gold
  
  // Specific Filipino relations - Brighter oranges and pinks
  kaugnay: "#ff9f2e",    // Bright orange
  salita: "#ff5c8d",     // Bright pink
  kahulugan: "#ff3366",  // Vivid pink
  kasalungat: "#ff7088", // Bright red
  
  // Fallback
  associated: "#c0c8d0", // Lighter gray
  other: "#9eacba"       // Medium gray
};
...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({
  ...
  const renderRelationsTab = () => {
    ...
    const hasRelations = 
      (incoming_relations.length > 0 && !hasError) || 
      (outgoing_relations.length > 0 && !hasError) || 
    ...
    // Helper function to get the appropriate color based on theme and relation type
    const getRelationColor = (type: string) => {
      // Use dark mode colors when in dark mode
      if (isDarkMode) {
        // Use darkModeRelationColors with fallback to relationColors
        return darkModeRelationColors[type] || darkModeRelationColors.other;
      }
      return relationColors[type] || relationColors.other;
    };
    ...
    const nonEmptyCategories = relationCategories.filter(
              ...
              >
                <Typography 
                  ...
                  sx={{ 
                    ...
                    borderBottom: `1px solid ${alpha(category.color, isDarkMode ? 0.5 : 0.2)}`,
                    color: isDarkMode ? alpha(category.color, 0.9) : theme.palette.text.primary,
                    fontWeight: 600
                  }}
                >
                  <Box 
                    ...
                    sx={{
                      ...
                      boxShadow: isDarkMode ? 
                        `0 0 0 2px ${alpha(category.color, 0.4)}, 0 0 6px ${alpha(category.color, 0.5)}` : 
                  ...
                  <Typography 
                    component="span" 
                    sx={{ 
                      color: isDarkMode ? alpha(category.color, 0.7) : alpha(theme.palette.text.secondary, 0.6),
                      fontSize: '0.8rem',
                      fontWeight: 'normal'
                    }}
                  >
                ...
                <Box sx={{ pl: theme.spacing(2) }}>
                  {(() => {
                    ...
                    return sortedTypes.map(relationType => {
                      ...
                      const relColor = getRelationColor(relationType);
      ...
      return (
                ...
                fontWeight: 500,
                              fontSize: '0.85rem',
                              color: isDarkMode ? alpha(relColor, 1) : alpha(relColor, 0.9),
                              mb: theme.spacing(0.75),
                              display: 'flex',
                              alignItems: 'center',
                              gap: 0.75,
                              py: 0.5,
                              textShadow: isDarkMode ? `0px 0px 1px ${alpha(relColor, 0.3)}` : 'none'
                            }}
                          >
                            <Box 
                              component="span"
                              sx={{
                                ...
                                bgcolor: relColor,
                                boxShadow: isDarkMode ? 
                                  `0 0 0 2px ${alpha(relColor, 0.4)}, 0 0 3px ${alpha(relColor, 0.6)}` : 
                            ...
                            <Typography 
                              component="span" 
                              sx={{ 
                                color: alpha(relColor, isDarkMode ? 0.8 : 0.6),
                                fontSize: '0.75rem',
                                fontWeight: 'normal'
                              }}
                            >
                  ...
                  return (
                    <Chip
                      ...
                      sx={{
                                  ...
                                  sx={{
                                    borderColor: alpha(relColor, isDarkMode ? 1 : 0.7),
                                    borderWidth: '1px',
                                    color: isDarkMode ? '#ffffff' : relColor,
                                    fontSize: '0.75rem',
                                    height: 'auto',
                                    padding: theme.spacing(0.25, 0),
                                    bgcolor: isDarkMode ? alpha(relColor, 0.7) : alpha(relColor, 0.15),
                                    my: 0.5,
                                    '& .MuiChip-label': { 
                                      px: 1, 
                                      py: 0.25, 
                                    ...
                                    '&:hover': {
                                      backgroundColor: isDarkMode ? alpha(relColor, 0.8) : alpha(relColor, 0.25),
                                      borderColor: relColor,
                                      boxShadow: isDarkMode ? `0 0 5px ${alpha(relColor, 0.7)}` : `0 0 2px ${alpha(relColor, 0.4)}`
                                    }
                                  }}
                                />
                  );
