...
import { WordNetworkResponse, NetworkNode } from "../types";
...
interface WordGraphProps {
  ...
  onNetworkChange: (depth: number) => Promise<void>;
  initialDepth: number;
...
function isWordNetwork(network: any): network is WordNetworkResponse {
...
const WordGraphComponent: React.FC<WordGraphProps> = ({
  ...
  const [breadth, setBreadth] = useState<number>(initialBreadth || 1);
  ...
  const nodes: CustomNode[] = useMemo(() => {
    ...
    return wordNetwork.nodes.map((node: NetworkNode) => ({
      id: String(node.id),
      word: node.lemma || mainWord || '', // Ensure word is never undefined
      group: (node.lemma === mainWord) ? 'main' : 'other',
  ...
  const links: CustomLink[] = useMemo(() => {
    ...
    return wordNetwork.edges.map((edge) => {
      // Handle source and target which could be objects or numbers
