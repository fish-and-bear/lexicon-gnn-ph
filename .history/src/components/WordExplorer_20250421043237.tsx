  ...
  const handleNodeClick = useCallback(async (identifier: string) => {
    ...
    try {
      ...
      if (wordData) {
        ...
        const confirmedIdentifier = identifier; // Use the original identifier for fetching network/etymology
        
        try {
          console.log(`Fetching network for identifier: ${confirmedIdentifier}`); // Use identifier
          fetchWordNetworkData(confirmedIdentifier, depth, breadth) // <-- Use identifier
