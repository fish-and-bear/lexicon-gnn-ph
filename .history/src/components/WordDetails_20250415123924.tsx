...
import { WordInfo, RelatedWord, Definition, NetworkLink, NetworkNode, WordForm, WordTemplate, Idiom, Affixation, DefinitionCategory, DefinitionLink, DefinitionRelation } from '../types';
...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({
  ...
  const renderDefinitionsTab = () => {
    ...
    // Render Idioms section first if available
          ...
          <StyledAccordionDetails>
            <List disablePadding>
              {idioms.map((idiom, index) => {
                 const phrase = Array.isArray(idiom) ? idiom[0] : (idiom.phrase || idiom.text);
                 const meaning = Array.isArray(idiom) ? idiom[1] : (idiom.meaning || idiom.text);
              ...
              {def.examples && def.examples.length > 0 && (
                  ...
                  {def.categories && def.categories.length > 0 && (
                    <Stack direction="row" spacing={0.5} useFlexGap flexWrap="wrap" sx={{ mt: 1 }}>
                      <Typography variant="caption" sx={{ fontWeight: 500, alignSelf: 'center', mr: 0.5 }}>Categories:</Typography>
                      {def.categories.map((cat, catIndex) => (
                        <Chip
                          ...
                          variant="outlined"
                          sx={{
                            fontSize: '0.7rem', height: 'auto', padding: theme.spacing(0.25, 0.5),
                  ...
                  {/* ADDED: Definition Relations */}
                  {def.definition_relations && def.definition_relations.length > 0 && (
                     <Box sx={{ pl: 1, mt: 1 }}>
                        <Typography variant="caption" sx={{ fontWeight: 500 }}>Related Concepts:</Typography>
                  ...
                  {def.links && def.links.length > 0 && (
                     <Box sx={{ pl: 1, mt: 1 }}>
                        <Typography variant="caption" sx={{ fontWeight: 500 }}>Links:</Typography>
                        <List dense disablePadding sx={{ pl: 1 }}>
