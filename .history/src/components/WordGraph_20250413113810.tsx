    ...
    setFilteredRelationships(prev => {
      ...
      if (prev.includes(type)) {
        ...
        if (svgRef.current) {
          ...
          // When unfiltering - first find all nodes of this type
          svg.selectAll<SVGGElement, CustomNode>(".node")
            .filter(d => d.group.toLowerCase() === type && d.id !== mainWord)
            .interrupt() // Cancel any ongoing transitions
            .style("display", "block") // First make it display
            .style("opacity", 0) // Start fully transparent
            .transition() // Then fade in
            .duration(300)
            .style("opacity", 1);
          ...
          svg.selectAll<SVGLineElement, CustomLink>(".link")
            .each(function(d) {
              ...
              if (sourceNode.group.toLowerCase() === type || targetNode.group.toLowerCase() === type) {
                ...
                if (!newFilters.includes(otherEnd) || sourceNode.id === mainWord || targetNode.id === mainWord) {
                  link.interrupt() // Cancel any ongoing transitions
                    .style("display", "block") // First make it display
                    .style("opacity", 0) // Start fully transparent
                    .transition() // Then fade in
                    .duration(300)
      ...
      if (svgRef.current) {
        ...
        // When filtering - find all nodes of this type
        svg.selectAll<SVGGElement, CustomNode>(".node")
          .filter(d => d.group.toLowerCase() === type && d.id !== mainWord)
          .interrupt() // Cancel any ongoing transitions
          .transition()
        ...
        svg.selectAll<SVGLineElement, CustomLink>(".link")
          .each(function(d) {
            ...
            // But only if neither end is the main word
            if ((sourceNode.group.toLowerCase() === type && sourceNode.id !== mainWord) || 
                (targetNode.group.toLowerCase() === type && targetNode.id !== mainWord)) {
              link.interrupt()
                .transition()
                .duration(300)
                .style("opacity", 0)
