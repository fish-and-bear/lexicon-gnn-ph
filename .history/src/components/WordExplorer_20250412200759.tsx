...
const WordExplorer: React.FC = () => {
  ...
  const handleNodeClick = useCallback(async (word: string) => {
    console.log("handleNodeClick called with word:", word);
    if (word !== mainWord) {
      setIsLoading(true);
      setError(null);
      ...
      try {
        ...
        // Check if the word parameter uses our special ID format (id:12345)
        if (word.startsWith('id:')) {
          // Extract the actual ID from the format
          const nodeId = word.substring(3); // Skip the 'id:' prefix
          console.log(`Processing node click with ID format: ${nodeId}`);
          
          try {
            // Use the ID-specific endpoint
            console.log(`Calling fetchWordDetails with id:${nodeId}`);
            wordData = await fetchWordDetails(`id:${nodeId}`);
            console.log("Word data received from ID:", wordData);
          } catch (idError) {
            console.error("Error fetching by ID, will try search by text:", idError);
            wordData = null; // Will trigger the search fallback
          }
        } else {
          // Check if the word parameter is numeric (legacy format)
          const wordId = parseInt(word, 10);
          
          if (!isNaN(wordId)) {
            // If it's a numeric ID, try to fetch directly
            console.log(`Treating ${word} as a legacy word ID, fetching directly`);
            try {
              // Use the ID format for fetchWordDetails
              console.log(`Calling fetchWordDetails with id:${wordId}`);
              wordData = await fetchWordDetails(`id:${wordId}`);
              console.log("Word data received from legacy ID:", wordData);
            } catch (idError) {
              console.error("Error fetching by legacy ID, will try search:", idError);
              wordData = null; // Will trigger the search fallback
            }
          }
        }
        ...
        if (!wordData) {
          // If it was an ID format that failed, extract the original word for search
          const searchTerm = word.startsWith('id:') ? word.substring(3) : word;
          console.log(`No word data yet, searching for term: ${searchTerm}`);
          ...
          console.log(`Calling searchWords with term: ${searchTerm}`);
          const searchResult = await searchWords(searchTerm, searchOptions);
          console.log("Search results:", searchResult);
          
          if (!searchResult || !searchResult.words || searchResult.words.length === 0) {
            throw new Error(`Word "${searchTerm}" not found`);
          }
          
          const wordResult = searchResult.words[0];
          console.log(`Using first search result: ${wordResult.lemma} (ID: ${wordResult.id})`);
          
          // Fetch word details by ID
          console.log(`Calling fetchWordDetails with ID: ${wordResult.id}`);
          wordData = await fetchWordDetails(wordResult.id.toString());
          console.log("Word data received from search:", wordData);
        }
        
        // We now have word data one way or another
        const wordLemma = wordData.lemma;
        console.log(`Got word lemma: ${wordLemma}`);
        
        // Then load the network
        console.log(`Calling fetchWordNetworkData for: ${wordLemma}`);
        const networkData = await fetchWordNetworkData(wordLemma, depth, breadth);
        ...
        try {
          ...
          console.log("Etymology tree data received");
        } catch (etymErr) {
        ...
        console.log("Updating UI state with word data");
        ...
        console.log("handleNodeClick completed successfully");
      } catch (err) {
      ...
      console.log("Word is already the main word, no action needed");
    }
  ...
  const handleSearch = useCallback(async (query: string) => {
    ...
    try {
      ...
      if (result && result.words && result.words.length > 0) {
        ...
        // Use handleNodeClick to load the first result's details
        try {
          await handleNodeClick(result.words[0].id.toString());
          console.log("Successfully loaded details for:", result.words[0].lemma);
        } catch (nodeClickError) {
          console.error("Error in handleNodeClick during search:", nodeClickError);
          setError(nodeClickError instanceof Error ? nodeClickError.message : 'Error loading word details');
        }
      } else {
