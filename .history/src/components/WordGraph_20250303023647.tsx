...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const createDragBehavior = useCallback((simulation: d3.Simulation<CustomNode, CustomLink>) => {
    ...
    return d3.drag<SVGGElement, CustomNode>()
      .on("start", (event, d) => {
        ...
        d3.selectAll(".link")
          .each(function(this: SVGLineElement, linkData: any) {
   ...
   */
  const createNodes = useCallback((g: d3.Selection<SVGGElement, unknown, null, undefined>, simulation: d3.Simulation<CustomNode, CustomLink>) => {
    ...
    const nodeGroups = g.append("g")
      ...
      .data(filteredNodes)
      .join("g")
      .attr("class", d => `node ${d.id === mainWordId ? "main-node" : ""} ${d.group}`)
      .call(drag as any);
    ...
    nodeGroups.append("text")
      ...
      .attr("font-weight", d => d.id === mainWordId ? "bold" : "normal")
    ...
    return nodeGroups;
  }, [filteredNodes, createDragBehavior, getNodeRadius, getNodeColor, theme, mainWordId]);
