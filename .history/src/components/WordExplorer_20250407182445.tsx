...
interface DiagnosticData {
  network?: WordNetworkResponse | null;
  details?: WordInfo | null;
...
const WordExplorer: React.FC = () => {
  ...
  const fetchWordDetailsData = useCallback(async (wordOrId: string | number) => {
      ...
      try {
          ...
          const maxRetries = 3;
          let success = false;
          let details: WordInfo | null = null;
          ...
          setDiagnostic(prev => ({ ...prev, data: { ...prev.data, details } }));
          ...
          setDiagnostic(prev => ({ ...prev, data: { ...prev.data, error } }));
          return null;
      } finally {
          setIsLoadingDetails(false);
    }
  }, [fetchEtymologyTreeData]);
  ...
  useEffect(() => {
    // On component mount, check API connectivity
    const checkApiStatus = async () => {
      try {
        ...
        setError("Error checking API connection. The backend server may be unavailable.");
      }
    };
    
    checkApiStatus();
    
    // Load static data if needed (parts of speech, relation types, etc.)
    ...
    // Set up a window event listener for network status changes
    const handleOnline = () => {
      console.log("Browser reports online status, testing API connection");
      testApiConnection().then(isConnected => {
        setApiConnected(isConnected);
        if (isConnected && error) {
          setError(null); // Clear error if we're back online
        }
      });
    };
    ...
    const handleOffline = () => {
      ...
      setApiConnected(false);
      setError("Browser reports that you are offline. Please check your network connection.");
    };
  ...
  const handleSearch = useCallback((e?: FormEvent) => {
    // If an event was passed, prevent default form submission
  ...
  const handleInputChange = async (value: string) => {
    ...
    if (value.trim().length > 2) {
      ...
      try {
        ...
        const result = await searchWords({ q: value, limit: 10 });
        if (result && Array.isArray(result.results)) {
          const validResults: SearchWordResult[] = result.results
            .filter((item): item is SearchWordResult => 
              typeof item === 'object' && 
              item !== null && 
        ...
        setSearchError(error instanceof Error ? error.message : 'Error searching words');
        setSearchResults([]);
        setShowSuggestions(false);
        
        // Test API connection on search error to see if it's a connectivity issue
        const isConnected = await testApiConnection();
        setApiConnected(isConnected);
        
        if (!isConnected) {
          setError("Search failed: Cannot connect to the backend API. Please check that the server is running.");
        }
      } finally {
        setIsLoadingSuggestions(false);
      }
  ...
  const handleResetCircuitBreaker = useCallback(() => {
    ...
    resetCircuitBreaker();
    forceCircuitBreakerClosed(); // Force the circuit breaker to close
    setError(null);
    ...
    testApiConnection()
      .then(isConnected => {
        setApiConnected(isConnected);
        if (isConnected && mainWord) {
          console.log("Connection successful after reset, reloading data...");
          loadWordData(mainWord);
        } else if (!isConnected) {
          setError("API connection still unavailable after reset. Please check that the backend server is running.");
        }
      })
      .catch(error => {
        console.error("Error testing API after reset:", error);
        setApiConnected(false);
        setError("Error testing API connection after reset. The backend server may be unavailable.");
      });
  }, [mainWord, loadWordData]);
    ...
    try {
      ...
      if (!isConnected) {
        ...
        try {
          ...
          setError("Cannot connect to the API server at http://localhost:10000. Please check that the backend server is running.");
  ...
  useEffect(() => {
    if (!mainWord && inputValue) {
      console.log('Loading initial data for:', inputValue);
      loadWordData(inputValue);
    }
  }, [mainWord, inputValue, loadWordData]);
  ...
  const handleMouseMove = useCallback((e: MouseEvent) => {
    ...
    // Calculate percentage (clamped between 30% and 70%)
