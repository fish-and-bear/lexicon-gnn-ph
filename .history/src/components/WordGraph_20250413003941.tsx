...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  // State for tracking filtered relationships
  ...
  const applyFilters = useCallback(() => {
    ...
    svg.selectAll<SVGGElement, CustomNode>(".node")
      .style("opacity", d => isNodeVisible(d) ? null : 0.15)
        ...
        entry.append("rect")
          ...
          .on("click", function(this: SVGRectElement) {
            ...
            setFilteredRelationships(prev => {
              ...
              const newFilters = wasFiltered 
                ? prev.filter(r => r !== type)
                : [...prev, type];
              
              console.log(`Toggled filter for ${type}. Filtered types:`, newFilters);
              
              // Get graph elements using svgRef directly
              if (svgRef.current) {
                const graphSvg = d3.select(svgRef.current);
                
                console.log(`Applying filters to ${graphSvg.selectAll<SVGGElement, CustomNode>(".node").size()} nodes and ${graphSvg.selectAll<SVGLineElement, CustomLink>(".link").size()} links`);
                
                // Apply filtering immediately for responsive UI
                const allNodes = graphSvg.selectAll<SVGGElement, CustomNode>(".node");
                const allLinks = graphSvg.selectAll<SVGLineElement, CustomLink>(".link");
                
                console.log(`Applying filters to ${allNodes.size()} nodes and ${allLinks.size()} links`);
                
                // Apply filtering immediately for responsive UI
                allNodes.each(function(this: SVGGElement, d: CustomNode) {
                  const node = d3.select(this);
                  // Always show main word
                  if (d.id === mainWord) {
                  ...
                  // Filter nodes based on relationship type
                  if (newFilters.length > 0 && newFilters.includes(d.group.toLowerCase())) {
                    console.log(`Filtering out node: ${d.id} (${d.group})`);
                    node.style("opacity", 0.15);
                    node.style("pointer-events", "none");
                  } else {
                    console.log(`Showing node: ${d.id} (${d.group})`);
                    node.style("opacity", 1);
                    node.style("pointer-events", "all");
                  }
                });
                ...
                allLinks.each(function(this: SVGLineElement, d: CustomLink) {
                  ...
                  const sourceFiltered = newFilters.length > 0 && 
                    sourceNode.id !== mainWord && 
                    newFilters.includes(sourceNode.group.toLowerCase());
                    
                  const targetFiltered = newFilters.length > 0 && 
                    targetNode.id !== mainWord && 
                    newFilters.includes(targetNode.group.toLowerCase());
