...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({ wordInfo, onWordClick }) => {
  ...
  const renderRelatedWords = () => {
    ...
    // Extract related words from the relations structure with all types
    const synonyms = wordInfo.relations.synonyms?.map((item: RelatedWord) => item.word) || [];
    const antonyms = wordInfo.relations.antonyms?.map((item: RelatedWord) => item.word) || [];
    const derivatives = wordInfo.relations.derived?.map((item: RelatedWord) => item.word) || [];
    const derivedFrom = wordInfo.relations.derived_from?.map((item: RelatedWord) => item.word) || [];
    const variants = wordInfo.relations.variants?.map((item: RelatedWord) => item.word) || [];
    const related = wordInfo.relations.related?.map((item: RelatedWord) => item.word) || [];
    const componentOf = wordInfo.relations.component_of?.map((item: RelatedWord) => item.word) || [];
    const cognates = wordInfo.relations.cognate?.map((item: RelatedWord) => item.word) || [];
    const rootOfWords = wordInfo.relations.root_of?.map((item: RelatedWord) => item.word) || [];
    const kaugnayWords = wordInfo.relations.kaugnay?.map((item: RelatedWord) => item.word) || [];
    
    // Derived words should include both derived and derived_from
    const allDerivatives = Array.from(new Set([...derivatives, ...derivedFrom]));
    
    // Also include any words from the semantic categories
    const mainWords = wordInfo.relations.main?.map((item: RelatedWord) => item.word) || [];
    const derivativeWords = wordInfo.relations.derivative?.map((item: RelatedWord) => item.word) || [];
    const etymologyWords = wordInfo.relations.etymology?.map((item: RelatedWord) => item.word) || [];
    const associatedWords = wordInfo.relations.associated?.map((item: RelatedWord) => item.word) || [];
    const otherWords = wordInfo.relations.other?.map((item: RelatedWord) => item.word) || [];
    
    // Get the root word
    const rootWord = wordInfo.relations.root?.word;

    const hasRelations = synonyms.length > 0 || antonyms.length > 0 || 
                       allDerivatives.length > 0 || variants.length > 0 || 
                       related.length > 0 || componentOf.length > 0 ||
                       cognates.length > 0 || mainWords.length > 0 ||
                       derivativeWords.length > 0 || etymologyWords.length > 0 ||
                       associatedWords.length > 0 || otherWords.length > 0 ||
                       rootWord || rootOfWords.length > 0 || kaugnayWords.length > 0;
    ...
    <div className="word-details-container">
      <div className="word-details">
        ...
        <div className="word-content-tabs">
          ...
          >
            <span className="tab-icon">🔗</span> Related {wordInfo.relations && 'Words'}
          </div>
