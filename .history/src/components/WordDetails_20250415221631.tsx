...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({
  ...
  useEffect(() => {
    // Clear converted baybayin when word changes
    if (wordInfo.lemma) {
    ...
    if (!wordInfo.has_baybayin && wordInfo.lemma && !isConvertingBaybayin) {
      const fetchBaybayinConversion = async () => {
        try {
          ...
          console.log(`Converting "${wordInfo.lemma}" to baybayin using language: ${langCode}`);
          const result = await convertToBaybayin(wordInfo.lemma, langCode);
          
          if (result && result.baybayin) {
            console.log(`Successfully converted to baybayin: ${result.baybayin}`);
            setConvertedBaybayin(result.baybayin);
          } else {
            console.warn('Conversion returned without baybayin text', result);
            setConvertedBaybayin(null);
          }
        } catch (error) {
      ...
      // Add a small delay to prevent immediate conversion on word change
  ...
  }, [wordInfo.lemma, wordInfo.has_baybayin, wordInfo.language_code]); // Remove convertedBaybayin and isConvertingBaybayin from dependencies
