...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({
  ...
  useEffect(() => {
    // Add debug log to check wordInfo data
    console.log('WordDetails: Baybayin display check:', {
    ...
    // If the word already has Baybayin from database, don't try to convert
    if (wordInfo.has_baybayin && wordInfo.baybayin_form) {
      console.log('WordDetails: Word already has Baybayin from database:', wordInfo.baybayin_form);
      // Reset conversion state to ensure we use the database version
      if (convertedBaybayin !== null || isConvertingBaybayin) {
    ...
    // Always reset state on word change when we don't have database Baybayin
    if (wordInfo.lemma) {
      if (convertedBaybayin !== null || isConvertingBaybayin) {
        console.log('WordDetails: Resetting Baybayin conversion state for new word');
        setConvertedBaybayin(null);
        setIsConvertingBaybayin(false);
      }
    }
    
    // Only try to convert if there's no existing baybayin and we have a valid lemma
        ...
        try {
          ...
          if (result && result.baybayin) {
            ...
            try {
              ...
              const codePoints = [...result.baybayin].map(char => char.codePointAt(0)?.toString(16).toUpperCase()).join(' ');
  ...
  }, [wordInfo.lemma, wordInfo.has_baybayin, wordInfo.baybayin_form, wordInfo.language_code]); // Add baybayin_form to dependencies
          ...
          <Box sx={{ my: theme.spacing(2) }}>
            ...
            {!wordInfo.has_baybayin && convertedBaybayin && (
              <Typography variant="caption" sx={{ display: 'block', mt: 0.5, fontStyle: 'italic', color: alpha(headerTextColor, 0.6) }}>
                Auto-generated • May not be accurate
              </Typography>
            )}
          </Box>
        ) : isConvertingBaybayin ? (
