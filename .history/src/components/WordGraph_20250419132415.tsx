...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const createDragBehavior = useCallback((simulation: d3.Simulation<CustomNode, CustomLink>) => {
    return d3.drag<SVGGElement, CustomNode>()
      .on("start", (event, d) => {
          ...
          // This prevents click handlers from firing
          event.sourceEvent.stopPropagation();
          
          // Simple visual feedback for dragging
      ...
      .on("end", (event, d) => {
          ...
          // Add longer delay before clearing isDragging to avoid accidental clicks
          setTimeout(() => {
  ...
  const setupNodeInteractions = useCallback((
      ...
      // First completely clear ALL event handlers
      ...
      // SIMPLIFY: Add a very basic hover effect without animations that cause jitter
      nodeSelection.on("mouseenter", function(event, d) {
          if (isDraggingRef.current) return;
          
          // Set fixed hover state without animations that cause jitter
          d3.select(this).select("circle")
            .attr("stroke-width", 2.5)
            .attr("stroke-opacity", 1);
            
          // Highlight connected links with solid styling (no animations)
      ...
      nodeSelection.on("mouseleave", function(event, d) {
          ...
          // If not the selected node, reset styling without animations
          ...
          // Reset link styling without animations if not the selected node
          if (selectedNodeId !== d.id) {
              d3.selectAll<SVGLineElement, CustomLink>(".link").filter((l: CustomLink) => {
                  const sourceId = typeof l.source === 'object' ? (l.source as CustomNode).id : l.source as string;
                  const targetId = typeof l.target === 'object' ? (l.target as CustomNode).id : l.target as string;
                  // Only reset if not highlighted
                  return (sourceId === d.id || targetId === d.id) && !d3.select(this).classed("highlighted");
              })
      ...
      // COMPLETELY REPLACE click handling with more direct approach 
      // Use traditional mousedown/mouseup for more reliable detection
      let isDoubleClickPossible = false;
      ...
      nodeSelection.on("mousedown", function(event, d) {
          ...
          // Track click state
          if (timeSinceLastClick < 300 && lastClickedNodeRef.current === d.id) {
              // This is potentially a double-click
      ...
      nodeSelection.on("mouseup", function(event, d) {
          // Prevent default browser behavior
          event.preventDefault();
          event.stopPropagation();
          ...
          const now = Date.now();
          const timeSinceMouseDown = now - lastClickTimeRef.current;
          
          // If mouse was held down too long, it's not a click
          if (timeSinceMouseDown > 300) {
              console.log("Mouse held down too long - not a click");
              return;
          }
          
          // Check if this is a double-click
          if (isDoubleClickPossible) {
              console.log(`Double-click detected on node: ${d.word}`);
          ...
          // This is a single click - handle it after a short delay
          // to allow for possible double-click detection
          if (singleClickTimer) {
              window.clearTimeout(singleClickTimer);
          }
          
          singleClickTimer = window.setTimeout(() => {
              console.log(`Single-click action on node: ${d.word}`);
              
              // Apply selection styling
              d3.selectAll(".node").classed("selected", false);
              d3.select(this).classed("selected", true);
              
              // Reset all nodes to faded state
              ...
              d3.selectAll(connectedLinkElements)
                  ...
                  .attr("stroke", function(this: SVGLineElement, datum: any) {
                      const l = datum as CustomLink;
                      const sourceId = typeof l.source === 'object' ? (l.source as CustomNode).id : l.source as string;
                      const targetId = typeof l.target === 'object' ? (l.target as CustomNode).id : l.target as string;
          ...
          }, 200); // Short delay to allow for double-click detection
      });
  ...
  // Add CSS class definitions for hover effects - simplify for stability
  useEffect(() => {
