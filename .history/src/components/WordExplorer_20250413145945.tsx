...
const WordExplorer: React.FC = () => {
  ...
  useEffect(() => {
    // Only proceed if we have both wordData and wordNetwork
    if (wordData && wordData.id && wordNetwork && wordNetwork.nodes && wordNetwork.nodes.length > 0) {
      // Find the main node
      const mainNode = wordNetwork.nodes.find(node => 
        node.type === 'main' || 
        (mainWord && (node.word === mainWord || node.label === mainWord))
      );
      ...
      if (mainNode) {
        ...
        // Only update if we have relations to sync
        if (incomingRelations.length > 0 || outgoingRelations.length > 0) {
          console.log("Syncing relations from useEffect:", {
            incoming: incomingRelations.length,
            outgoing: outgoingRelations.length
          });
