...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({
  ...
  const renderRelationsTab = () => {
    ...
    // If no relations and semantic network is disabled or empty, show info message
    if (!hasRelations) {
      return (
        <Box>
          <Alert severity="info" sx={{ mb: 2 }}>No relations available for this word.</Alert>
          
          {/* Add message about possible database configuration issues */}
          {(wordInfo.id && !wordInfo.incoming_relations && !wordInfo.outgoing_relations) && (
            <Alert severity="warning" sx={{ mt: 2 }}>
              Relations may be missing due to a database configuration issue. 
              The backend database is missing an expected column (relation_data).
            </Alert>
          )}
        </Box>
      );
    ...
    // Debug relations data structure
    console.log("Incoming relations sample:", incoming_relations.length > 0 ? incoming_relations[0] : "none");
    console.log("Outgoing relations sample:", outgoing_relations.length > 0 ? outgoing_relations[0] : "none");
    ...
    // Fix: Ensure all relations have the necessary properties and normalize structure
    const normalizedRelations = allRelations.map(rel => {
      // Create a standard object with required properties
      return {
        ...rel,
        id: rel.id || `rel-${Math.random().toString(36).substring(2, 9)}`,
        relation_type: rel.relation_type || 'related',
        degree: rel.degree || 1,
        // Handle both relation.metadata and relation.target_word structures
        wordObj: rel.wordObj || rel.target_word || rel.source_word || {
          id: -1,
          lemma: 'Unknown Word',
          has_baybayin: false,
          baybayin_form: null
    ...
    // Filter out invalid relations or self-references
    const filteredRelations = normalizedRelations.filter(rel => {
      return rel.wordObj?.lemma && rel.wordObj.lemma !== wordInfo.lemma;
    });
                                ...
                                <Chip
                                  ...
                                  label={
                                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
                                      ...
                                      <span>{wordObj.lemma || wordObj.word || 'Unknown Word'}</span>
                                      {hasMultipleTypes && (
                                        <Typography 
                                          component="span" 
                                          sx={{ 
