...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const [linkDistance, setLinkDistance] = useState<number>(110);
  ...
  // Define mapRelationshipToGroup first to avoid circular dependency
  ...
  // First declare baseLinks, baseNodes, and filteredNodes
  ...
  // Now that filteredNodes is defined, we can define groups
  const groups = useMemo(() => {
    const uniqueGroups = new Set<string>();
    filteredNodes.forEach(node => {
      if (node.group) uniqueGroups.add(node.group);
    });
  ...
  ): d3.Selection<Element, CustomLink, SVGGElement, unknown> => {
      ...
      .join(
        enter => {
          ...
          path.call(enter => enter.transition()
            ...
            .duration(600)
            .attrTween("stroke-dashoffset", function(this: SVGPathElement) {
              const length = this.getTotalLength();
              return function(t: number): string {
                return String(d3.interpolateNumber(length, 0)(t));
              };
            })
            .attr("opacity", 0.6)
          );
  ...
  const setupNodeInteractions = useCallback((
    nodeGroups: d3.Selection<any, CustomNode, any, unknown>,
    linkGroups: d3.Selection<any, CustomLink, any, unknown>
  ) => {
  ...
  useEffect(() => {
    ...
    // Use more generic types with 'any' to avoid TypeScript errors
    setupNodeInteractions(nodeElements as any, linkGroups as any);
  ...
  return (
    <div className="graph-container" ref={containerRef}>
      ...
      <svg ref={svgRef} className="word-graph">
        {/* Background pattern */}
        <defs>
          ...
          <radialGradient id="background-gradient" cx="50%" cy="50%" r="70%" fx="50%" fy="50%">
            <stop offset="0%" stopColor={theme === "dark" ? "#202124" : "#f8f9fa"} />
            <stop offset="100%" stopColor={theme === "dark" ? "#121212" : "#e9ecef"} />
          </radialGradient>
        </defs>
        
        {/* Background fill with gradient and pattern */}
      ...
      <div className="graph-title">
        {mainWord && (
          <h3>Word Web of <span className="highlight">{mainWord}</span></h3>
        )}
      </div>
      
      {/* Enhanced legend with animation on hover */}
      {groups.length > 0 && (
        <div className="legend-container">
          <div className="legend-header">Relationship Types</div>
                ...
                <span
                  ...
                  style={{
                    background: `radial-gradient(circle at 30% 30%, 
                      ${d3.color(getNodeColor(group))?.brighter(0.7).formatHex() ?? "#fff"} 0%, 
                      ${getNodeColor(group)} 100%)`,
                    boxShadow: highlightedGroup === group ? 
                      `0 0 8px ${getNodeColor(group)}` : 'none',
                    transform: highlightedGroup === group ? 'scale(1.1)' : 'scale(1)'
                  }}
                />
      ...
      <div className="controls-container">
        ...
        <div className="slider-group">
          <label htmlFor="linkDistance" className="slider-label">
            <span>Link Distance</span>
            <div className="tooltip">Adjust spacing between nodes</div>
          </label>
          <input
            id="linkDistance"
            type="range"
        ...
        <div className="slider-group">
          <label htmlFor="chargeStrength" className="slider-label">
            <span>Node Repulsion</span>
            <div className="tooltip">Adjust how strongly nodes repel each other</div>
          </label>
          <input
            id="chargeStrength"
            type="range"
      ...
      {hoveredNode && (
        <div
          ...
          style={{
            ...
            transform: hoveredNode ? "scale(1)" : "scale(0.8)"
          }}
