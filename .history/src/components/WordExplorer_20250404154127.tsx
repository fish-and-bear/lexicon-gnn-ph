...
const WordExplorer: React.FC = () => {
  ...
  const { searchTerm: wordSearchTerm, setSearchTerm, searchResults: wordSearchResults, loadMore, isLoading: isSearchLoading, error: searchError, hasMore, clearSearch } = useWordSearch();
  ...
  useEffect(() => {
    if (selectedWordInfo?.etymology_trees && selectedWordInfo.etymology_trees.length > 0) {
        // For now, just load the first tree if available
        const treeId = selectedWordInfo.etymology_trees[0].id; 
      ...
      <Box sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <Box className="graph-pane" sx={{ flexBasis: '50%', position: 'relative' }}>
           {selectedWordInfo ? (
             ...
             !isSearchLoading && <Typography color="text.secondary" sx={{ p: 2 }}>Word network will appear here.</Typography>
           )}
