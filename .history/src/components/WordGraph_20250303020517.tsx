      ...
      .on('drag', (event, d) => {
        ...
        if (currentElement && currentElement instanceof Element) {
          ...
          if (svgRef.current) {
            ...
            // Type safety: Ensure we're working with proper link elements
            svg.selectAll<SVGLineElement, CustomLink>('.link')
              .filter((link) => {
                // Make sure we're comparing data consistently
                const source = typeof link.source === 'object' ? link.source.id : link.source;
                const target = typeof link.target === 'object' ? link.target.id : link.target;
                return source === d.id || target === d.id;
              })
              .each(function(link) {
                // Use function to access correct 'this' context
                ...
                // Update attributes only if we have valid data
                if (source && target) {
                  const sourceX = source.id === d.id ? event.x : (source.x || 0);
                  const sourceY = source.id === d.id ? event.y : (source.y || 0);
                  const targetX = target.id === d.id ? event.x : (target.x || 0);
                  const targetY = target.id === d.id ? event.y : (target.y || 0);
