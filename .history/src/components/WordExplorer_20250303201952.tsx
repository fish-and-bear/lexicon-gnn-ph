...
import { WordNetwork, WordNetworkGraph, WordNetworkLists, WordInfo, SearchResult, SearchOptions, EtymologyTree, Statistics, Definition, NetworkNode, NetworkEdge } from "../types";
...
const WordExplorer: React.FC<WordExplorerProps> = ({ initialWord }) => {
  const [word, setWord] = useState<string>(initialWord || '');
  const [wordNetwork, setWordNetwork] = useState<WordNetworkGraph | null>(null);
  ...
  const normalizeInput = useCallback((input: string): string => {
    return unidecode(input.trim().toLowerCase());
  }, []);
  ...
  const buildNetworkFromWordDetails = useCallback((wordDetails: WordInfo, word: string, depth: number): WordNetworkGraph => {
    ...
    const addRelatedWords = (words: Array<{ word: string; sources: string[] }> | undefined, type: string) => {
      ...
      words.forEach(rel => {
        const node: NetworkNode = {
          ...
          baybayin_form: null,
          type,
          path: [{ type, word: rel.word }],
          definitions: []
        };
        
        network.nodes.push(node);
        network.edges.push({
          source: type === 'root' ? nodeId : 1,
          target: type === 'root' ? 1 : nodeId,
          type,
          weight: type === 'root' ? 1.5 : 1
        });
        
        // Add to appropriate cluster
        switch (type) {
          case 'synonym':
            network.clusters.synonyms.push(node);
            break;
          case 'antonym':
            network.clusters.antonyms.push(node);
            break;
          case 'root':
            network.clusters.root_words.push(node);
            break;
          case 'derived':
            network.clusters.derived_words.push(node);
            break;
          case 'variant':
            network.clusters.variants.push(node);
            break;
        }
    ...
    if (wordDetails.relations) {
      addRelatedWords(wordDetails.relations.synonyms, 'synonym');
      addRelatedWords(wordDetails.relations.antonyms, 'antonym');
      addRelatedWords(wordDetails.relations.variants, 'variant');
      addRelatedWords(wordDetails.relations.derived, 'derived');
      if (wordDetails.relations.root) {
        addRelatedWords([wordDetails.relations.root], 'root');
      }
    }
    ...
    network.metadata.cluster_count = Object.values(network.clusters)
      .filter(arr => arr.length > 0).length;

    return network;
  }, []);

  const fetchWordNetworkData = useCallback(async (searchWord: string, searchDepth: number = 2, searchBreadth: number = 10): Promise<WordNetworkGraph | null> => {
    try {
      setIsLoading(true);
      setError(null);

      // Check cache first
      const cacheKey = `${searchWord}-${searchDepth}-${searchBreadth}`;
      const cached = networkCache.current.get(cacheKey);
      if (cached && Date.now() - cached.timestamp < CACHE_TTL) {
        console.log('Using cached network data for:', searchWord);
        return cached.data;
      }

      // Fetch both network and word details
      const [networkData, wordDetails] = await Promise.all([
        fetchWordNetwork(searchWord, {
          depth: searchDepth,
        ...
        fetchWordDetails(searchWord).catch(error => {
          console.error('Error fetching details:', error);
          return null;
        })
      ]);

      let finalNetwork: WordNetworkGraph | null = null;

      // Try to use network data first
      if (networkData && Array.isArray(networkData.nodes) && networkData.nodes.length > 1) {
        finalNetwork = networkData;
      }
      // If no network data or only one node, try building from word details
      else if (wordDetails) {
        console.log('Building network from word details');
        const builtNetwork = buildNetworkFromWordDetails(wordDetails, searchWord, searchDepth);
        if (builtNetwork.nodes.length > 1) {
          finalNetwork = builtNetwork;
        }
      }
      ...
      throw new Error('No word network data available');
    } catch (error) {
  ...
  const handleSearch = useCallback(async (searchWord: string): Promise<void> => {
    const normalizedWord = normalizeInput(searchWord);
    if (normalizedWord) {
      setMainWord(normalizedWord);
      const network = await fetchWordNetworkData(normalizedWord, depth, breadth);
      setWordNetwork(network);
    }
  }, [depth, breadth, fetchWordNetworkData, normalizeInput]);
          ...
          if (results && results.words && results.words.length > 0) {
            ...
            try {
              ...
              if (wordDetails && wordDetails.lemma) {
                ...
                await handleSearch(wordDetails.lemma);
                return;
              }
  ...
  const handleNetworkChange = useCallback(async (newDepth: number, newBreadth: number): Promise<void> => {
    setDepth(newDepth);
    setBreadth(newBreadth);
    if (mainWord) {
      const network = await fetchWordNetworkData(mainWord, newDepth, newBreadth);
      setWordNetwork(network);
    }
  ...
  return (
    <div className={`word-explorer theme-${theme}`}>
