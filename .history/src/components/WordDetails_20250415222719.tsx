  ...
  useEffect(() => {
    ...
    console.log('WordDetails: Raw Baybayin data check:', {
      ...
      unicode_points: wordInfo.has_baybayin && wordInfo.baybayin_form ? 
        [...wordInfo.baybayin_form].map(char => `U+${char.codePointAt(0)?.toString(16).toUpperCase().padStart(4, '0')}`).join(' ') : 
        'N/A',
      ...
      const fetchBaybayinConversion = async () => {
        try {
          ...
          if (result && result.baybayin) {
            ...
            // Encode Unicode characters explicitly for better compatibility
            try {
              // Display Unicode code points for debugging
              const codePoints = [...result.baybayin].map(char => 
                `U+${char.codePointAt(0)?.toString(16).toUpperCase().padStart(4, '0')}`
              ).join(' ');
              console.log(`WordDetails: Baybayin Unicode code points: ${codePoints}`);
            } catch (e) {
              console.error('Error logging code points:', e);
            }
        ...
        {(wordInfo.has_baybayin && wordInfo.baybayin_form) || convertedBaybayin ? (
          <Box sx={{ my: theme.spacing(2) }}>
            ...
            <div 
              ...
              style={{
                fontFamily: "'Noto Sans Baybayin', sans-serif !important",
                fontSize: '2rem',
                padding: '8px 12px',
