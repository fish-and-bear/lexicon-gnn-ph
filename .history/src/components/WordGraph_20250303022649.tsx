  ...
  const createDragBehavior = useCallback(() => {
    return d3.drag<SVGGElement, CustomNode>()
      ...
      .on('drag', (event, d) => {
        ...
        // Only proceed if we have a valid SVG reference
        if (!svgRef.current) return;
        
        // Update node position
        const nodeElement = d3.select(event.sourceEvent.currentTarget);
        nodeElement.attr('transform', `translate(${event.x}, ${event.y})`);
        
        // Get a reference to all links in the SVG
        const svg = d3.select(svgRef.current);
        
        // Update connected links using D3's selection methods
        svg.selectAll<SVGLineElement, CustomLink>('.link')
          .each(function(link) {
            // Get a D3 selection for this link element
            const linkSelection = d3.select(this);
            
            // Extract source and target info safely
            ...
            // Only update links connected to the dragged node
            if (sourceId === d.id || targetId === d.id) {
              ...
              if (sourceId === d.id) {
                // Use drag event coordinates for dragged node
                sourceX = event.x;
                sourceY = event.y;
              } else if (source && typeof source.x === 'number' && typeof source.y === 'number') {
                // Use coordinates from data for non-dragged end
                sourceX = source.x;
                sourceY = source.y;
              } else {
                // Find node in filtered nodes as fallback
              ...
              // Update link positions using D3's attr method
              linkSelection
                .attr('x1', sourceX)
                .attr('y1', sourceY)
                .attr('x2', targetX)
