          ...
          <div key={posName} className="pos-group">
            ...
            <div className="definition-cards-container">
              {definitions.map((definition: Definition, index: number) => {
                ...
                // Process definition text to handle superscripts for numbers at the end
                // Instead of using useMemo inside the callback, we'll use a regular function
                const processDefinitionText = (text: string) => {
                  if (!text) return text;
                  
                  // Check if the text ends with a number
                  // This regex looks for any text followed by one or more digits at the very end
                  const match = text.match(/^(.*[^\d])(\d+)$/);
                  if (match) {
                    const [_, textPart, numberPart] = match;
                    return (
                      <>
                        {textPart}<sup>{numberPart}</sup>
                      </>
                    );
                  }
                  return text;
                ...
                const processedText = processDefinitionText(definitionText);
                
                return (
      <div key={index} className="definition-card">
                    <div className="definition-number">{index + 1}</div>
                    <div className="definition-content">
                      <p className="definition-text">{processedText}</p>
          ...
          <div className="details-content">
            {isLoading ? (
              ...
              <div className="word-details">
                            ...
                            {etymology.components && etymology.components.length > 0 && (
                              <div className="etymology-components-list">
                                ...
                                <div className="component-tags">
                                  {etymology.components.map((component, i) => (
                    <span
                                      key={i} 
                        ...
                        {/* Integrate etymology tree if available */}
                        {etymologyTree && (
                          <div className="etymology-tree-container">
                            {isLoadingEtymology ? (
                              <div className="loading-spinner">Loading etymology details...</div>
                            ) : etymologyError ? (
                              <div className="error-message">
                                <p>{etymologyError}</p>
              </div>
                                    ...
                                    <div className="component-words-list">
                                      {etymologyTree.component_words.map((compWord, index) => (
                                        <div key={index} className="component-word-card">
                                          <h5 
                                            className="component-word-title"
                                            onClick={() => handleNodeClick(compWord.word)}
                                          >
                                            {compWord.word}
                                          </h5>
                                          {compWord.etymologies && compWord.etymologies.length > 0 && (
                                            <p className="component-word-etymology">
                                              {compWord.etymologies[0].text || compWord.etymologies[0].etymology_text}
                                            </p>
                                          )}
                                          {compWord.components && compWord.components.length > 0 && (
                                            <div className="sub-components">
                                              <span className="sub-components-label">Components: </span>
                                              <div className="sub-components-list">
                ...
                {showMetadata && (
                  <div className="word-content technical-details">
