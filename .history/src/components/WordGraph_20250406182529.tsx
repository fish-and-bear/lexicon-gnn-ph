...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  // Update findHiddenConnections to use filteredNodes
  const findHiddenConnections = useCallback((sourceId: string, depth = 1): string[] => {
    ...
    ).map(link => link.source === sourceId ? link.target : link.source);
    ...
    directConnections.forEach(targetId => {
      ...
      ).map(link => link.source === targetId ? link.target : link.source);
