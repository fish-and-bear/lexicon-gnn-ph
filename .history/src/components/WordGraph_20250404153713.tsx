...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const gRef = useRef<SVGGElement>(null); // Ref for the main group element
  ...
  const filteredNodes = useMemo<CustomNode[]>(() => {
    ...
    while (queue.length > 0) {
      ...
      const limitedNeighbors = sortedWords.slice(0, breadth);
  ...
  const filteredLinks = useMemo<CustomLink[]>(() => {
    ...
    return baseLinks
      ...
      .map(link => ({
        ...link,
        source: filteredNodes.find(n => n.id === link.source)!, // Add non-null assertion
  ...
  const createDragBehavior = useCallback((simulation: d3.Simulation<CustomNode, CustomLink>) => {
    const dragstarted = (event: d3.D3DragEvent<SVGGElement, CustomNode, any>, d: CustomNode) => {
        const nodeElement = event.sourceEvent.currentTarget as SVGGElement;
    ...
    const dragged = (event: d3.D3DragEvent<SVGGElement, CustomNode, any>, d: CustomNode) => {
        d.fx = event.x;
        d.fy = event.y;
    };

    const dragended = (event: d3.D3DragEvent<SVGGElement, CustomNode, any>, d: CustomNode) => {
        const nodeElement = event.sourceEvent.currentTarget as SVGGElement;
        ...
        nodeElement.dataset.dragEndTime = Date.now().toString();
        d3.select(nodeElement).classed("dragging", false);

        setTimeout(() => {
            isDraggingRef.current = false;
            if (nodeElement?.dataset?.dragEndTime) {
                delete nodeElement.dataset.dragEndTime;
            }
        }, 50);
    };
  ...
  const createLinks = useCallback((g: d3.Selection<SVGGElement, unknown, null, undefined>, linksData: CustomLink[]) => {
      return g.append("g")
          ...
          .data(linksData, d => `${(typeof d.source === 'string' ? d.source : d.source.id)}-${(typeof d.target === 'string' ? d.target : d.target.id)}`)
          .join(
  ...
  const nodeClick = useCallback((event: PointerEvent, d: CustomNode) => {
      const now = Date.now();
      const DOUBLE_CLICK_DELAY = 300;

      const targetElement = event.currentTarget as SVGGElement;
      const dragEndTime = targetElement?.dataset?.dragEndTime;

      if (isDraggingRef.current || (dragEndTime && now - parseInt(dragEndTime, 10) < 50)) {
          isDraggingRef.current = false;
          if (dragEndTime) delete targetElement.dataset.dragEndTime;
          return;
      }
      if (dragEndTime) delete targetElement.dataset.dragEndTime;

      if (now - lastClickTimeRef.current < DOUBLE_CLICK_DELAY && d.id === selectedNodeId) {
          d.pinned = !d.pinned;
          d.fx = d.pinned ? d.x : null;
          d.fy = d.pinned ? d.y : null;
          d3.select(targetElement).select("circle")
            .transition().duration(150)
            .attr("stroke", d.pinned ? "#ff00ff" : (d3.color(getNodeColor(d.group))?.darker(0.8).formatHex() ?? "#888"))
            .attr("stroke-width", d.pinned ? 2.5 : (d.id === selectedNodeId ? 2.5 : 1.5));
      } else {
          setSelectedNodeId(d.id);
          onNodeClick(d.id);
      }
      lastClickTimeRef.current = now;
  }, [onNodeClick, getNodeColor, selectedNodeId]);

  const createNodes = useCallback((
      g: d3.Selection<SVGGElement, unknown, null, undefined>,
      nodesData: CustomNode[],
      simulation: d3.Simulation<CustomNode, CustomLink> | null
      ): Selection<SVGGElement, CustomNode, SVGGElement, unknown> => {
          ...
          update => {
              update.select("circle")
                  ...
                  .attr("stroke", d => d.id === selectedNodeId 
                      ? (theme === 'dark' ? '#fff' : '#000') 
                      : (d.pinned ? "#ff00ff" : (d3.color(getNodeColor(d.group))?.darker(0.8).formatHex() ?? "#888")))
                  .attr("stroke-width", d => d.id === selectedNodeId ? 2.5 : (d.pinned ? 2.5 : 1.5));
              return update;
          },
          exit => exit
    ...
    const labelGroup = g.append("g")
      ...
      if (drag) nodeGroups.call(drag);

      nodeGroups.on("click", (event: PointerEvent, d: CustomNode) => nodeClick(event, d));

      nodeGroups
          .on("mouseenter", (event: MouseEvent, d: CustomNode) => {
              if (isDraggingRef.current) return;
              const nodeElement = d3.select(event.currentTarget as SVGGElement);
              if (tooltipTimeoutId) clearTimeout(tooltipTimeoutId);

              nodeElement.select('circle').attr('filter', 'url(#glow)');
              nodeElement.raise();

              const timeoutId = setTimeout(() => setHoveredNode({ ...d }); }, 200);
              setTooltipTimeoutId(timeoutId);
          })
          .on("mouseleave", (event: MouseEvent, d: CustomNode) => {
              if (isDraggingRef.current) return;
              if (tooltipTimeoutId) clearTimeout(tooltipTimeoutId);
              setHoveredNode(null);

              const nodeElement = d3.select(event.currentTarget as SVGGElement);
              const d = d as CustomNode;

              nodeElement.select<SVGCircleElement>("circle").data([d]);

              nodeElement.transition("hover_border_width").duration(150)
                  .attr("stroke-width", (n: CustomNode) => n.id === selectedNodeId ? 2.5 : (n.pinned ? 3 : 1.5));

              nodeElement.transition("hover_border_color").duration(150)
    ...
    return nodeGroups;
  }, [createDragBehavior, getNodeRadius, getNodeColor, theme, mainWord, nodeClick, selectedNodeId]);

  const createLabels = useCallback((
        g: d3.Selection<SVGGElement, unknown, null, undefined>,
        nodesData: CustomNode[]
    ): Selection<SVGTextElement, CustomNode, SVGGElement, unknown> => {
        return g.append("g")
            .attr("class", "labels")
            .selectAll<SVGTextElement, CustomNode>("text.node-label")
            .data(nodesData, d => d.id)
...
import * as d3 from "d3";
import "./WordGraph.css";
import { WordNetwork, NodeData, EdgeData } from "../types";
import { useTheme } from "../contexts/ThemeContext";
import Slider from "@mui/material/Slider";
import Typography from "@mui/material/Typography";
import axios from 'axios';
...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const createDragBehavior = useCallback((simulation: d3.Simulation<CustomNode, CustomLink>) => {
    const dragstarted = (event: d3.D3DragEvent<SVGGElement, CustomNode, any>, d: CustomNode) => {
        ...
        isDraggingRef.current = true;
        d3.select(event.sourceEvent.target.closest(".node")).raise().classed("dragging", true);
    };
    ...
    const dragended = (event: d3.D3DragEvent<SVGGElement, CustomNode, any>, d: CustomNode) => {
        if (!event.active) simulation.alphaTarget(0);
        if (!d.pinned) { // Only release if not pinned
           d.fx = null;
           d.fy = null;
        }
        // Set a timestamp to help differentiate drag end from click
        (event.sourceEvent.target as SVGGElement).dataset.dragEndTime = Date.now().toString();
        d3.select(event.sourceEvent.target.closest(".node")).classed("dragging", false);
        // Reset drag ref slightly later to allow click suppression
  ...
  const nodeClick = useCallback((event: PointerEvent, d: CustomNode) => {
      ...
      if (isDraggingRef.current) {
          isDraggingRef.current = false; // Reset drag flag if click happens after drag attempt
          return;
      }

      // Prevent click action immediately after drag ends
      const dragEndTime = (event.target as SVGGElement)?.dataset?.dragEndTime;
      if (dragEndTime && now - parseInt(dragEndTime, 10) < 50) {
          return;
      }

      if (now - lastClickTimeRef.current < DOUBLE_CLICK_DELAY && d.id === selectedNodeId) {
          // Double click on the *selected* node: Toggle pin
          d.pinned = !d.pinned;
          d.fx = d.pinned ? d.x : null;
          d.fy = d.pinned ? d.y : null;
          d3.select(event.currentTarget as SVGGElement).select("circle")
            .transition().duration(150)
            .attr("stroke", d.pinned ? "#ff00ff" : (d3.color(getNodeColor(d.group))?.darker(0.8).formatHex() ?? "#888"))
            .attr("stroke-width", d.pinned ? 2.5 : (d.id === selectedNodeId ? 2.5 : 1.5)); // Keep selected highlight if pinned
      } else {
          // Single click: Select node and call the prop handler
          setSelectedNodeId(d.id);
          onNodeClick(d.id);
      }
      lastClickTimeRef.current = now;
  ...
  const createNodes = useCallback((
    ...
    const nodeGroups = g.append("g")
      .attr("class", "nodes")
      .selectAll<SVGGElement, CustomNode>("g.node") // Specify types for selectAll
        .data(nodesData, d => d.id) // Use d => d.id for key function
      .join(
          enter => {
              const nodeGroup = enter.append("g")
                  .attr("class", d => `node node-group-${d.group} ${d.id === mainWord ? "main-node" : ""}`)
      ...
      .attr("fill", d => getNodeColor(d.group))
          ...
          update => {
              // Update circle attributes if needed (e.g., color based on selection)
              update.select("circle")
                  .transition().duration(150)
                  .attr("stroke", d => d.id === selectedNodeId 
                      ? (theme === 'dark' ? '#fff' : '#000') 
    ...
    const labelGroup = g.append("g")
        .attr("class", "labels")
        .selectAll<SVGTextElement, CustomNode>("text.node-label") // More specific selector
        .data(nodesData, (d: any) => (d as CustomNode).id)
        .join(
            enter => {
                const textElement = enter.append("text")
      ...
      nodeGroups
          .on("mouseenter", (event: MouseEvent, d: CustomNode) => {
              if (isDraggingRef.current) return;
              d3.select(event.currentTarget).select('circle').attr('stroke-opacity', 0.7);
          })
          .on("mouseleave", (event: MouseEvent, d: CustomNode) => {
              if (isDraggingRef.current) return;
  ...
  return (
    <Box className={`word-graph-container ${theme}-theme`} sx={{ position: 'relative', width: '100%', height: '100%', minHeight: '400px', border: '1px solid', borderColor: 'divider', borderRadius: 1, overflow: 'hidden' }}>
       {/* Controls Overlay */} 
       <Box
       ...
       >
           <Box sx={{ width: 120 }}>
               <Typography gutterBottom variant="caption" sx={{ color: 'text.secondary' }}>Depth</Typography>
               <Slider
                   value={depth}
                   ...
                   marks
                   min={1}
                   max={5} // Sensible max depth
                   size="small"
               />
           </Box>
           <Box sx={{ width: 120 }}>
               <Typography gutterBottom variant="caption" sx={{ color: 'text.secondary' }}>Breadth</Typography>
               <Slider
                   value={breadth}
       ...
       <svg ref={svgRef} style={{ display: 'block', width: '100%', height: '100%' }}>
           <g ref={gRef}></g> {/* Group for zoom/pan */} 
       </svg>
    </Box>
  );
};
