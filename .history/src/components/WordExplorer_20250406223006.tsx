...
import WordGraph2 from './WordGraph2';
  ...
  const fetchWordNetworkData = useCallback(async (word: string, currentDepth: number = 2, currentBreadth: number = 8) => {
    try {
      ...
      if (data && Array.isArray(data.nodes) && data.nodes.length > 0) {
        ...
        console.log(`Network data received: ${data.nodes.length} nodes, ${data.edges?.length || 0} edges`);
        setWordNetwork(data);
        return data;
      } else {
        console.warn("Received empty or invalid network data:", data);
        // Create minimal network with just the main word
        const minimalNetwork = {
          nodes: [{ id: 0, lemma: word, language_code: 'tl' }],
          edges: [],
            ...
            {!isLoading && wordNetwork && (
              <div style={{ height: '100%', width: '100%' }}>
                <WordGraph2 
                  wordNetwork={{
                    ...
                    edges: Array.isArray(wordNetwork.edges) ? wordNetwork.edges.map(edge => ({
                      ...
                      source: typeof edge.source === 'number' ? edge.source : parseInt(String(edge.source), 10) || 0,
                      target: typeof edge.target === 'number' ? edge.target : parseInt(String(edge.target), 10) || 0,
                ...
                <div className="debug-info" style={{ 
                  ...
                  <p>API data: {Array.isArray(wordNetwork.nodes) ? wordNetwork.nodes.length : 0} nodes, {Array.isArray(wordNetwork.edges) ? wordNetwork.edges.length : 0} edges</p>
                </div>
              </div>
            )}
