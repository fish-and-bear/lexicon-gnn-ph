...
import { useWordData, useWordNetwork } from '../api/hooks';
import PartOfSpeechTabs from './PartOfSpeechTabs';
import WordDefinitions from './WordDefinitions';
import SimpleWordGraph from './SimpleWordGraph';
import RelationBreakdown from './RelationBreakdown';
import { DepthIcon, BreadthIcon } from './Icons';
...
const WordExplorer: React.FC = () => {
  ...
  const handleDepthChange = useCallback((newDepth: number) => {
    setDepth(newDepth);
    if (mainWord) {
      fetchWordNetworkData(normalizeInput(mainWord), newDepth, breadth);
    }
  }, [mainWord, breadth, fetchWordNetworkData]);

  const handleBreadthChange = useCallback((newBreadth: number) => {
    setBreadth(newBreadth);
    if (mainWord) {
      fetchWordNetworkData(normalizeInput(mainWord), depth, newBreadth);
    }
  }, [mainWord, depth, fetchWordNetworkData]);
  ...
  const handleNetworkReset = useCallback(() => {
    if (mainWord) {
      ...
      setBreadth(15);  // Reset to higher default breadth
      fetchWordNetworkData(normalizeInput(mainWord), 2, 15);
    }
  }, [mainWord, fetchWordNetworkData]);

  const { data: wordData, isLoading: isWordDataLoading } = useWordData(mainWord);
  const { data: wordNetworkData, isLoading: isNetworkLoading } = useWordNetwork(mainWord, { 
    depth, 
    breadth,
    relationTypes: []
  });
            ...
            {!isLoading && wordNetwork && (
              <div style={{ height: '100%', width: '100%' }}>
                ...
                <div className="debug-info" style={{ 
                  ...
                  <p>API data: {wordNetwork.nodes.length} nodes, {wordNetwork.edges.length} edges</p>
                </div>
              </div>
            )}
          ...
          {wordNetwork && (
            <div className="explorer-controls">
              <div className="network-controls">
                ...
                <div className="network-controls-info">
                  Adjust these controls to explore the word network at different levels of detail.
                </div>
                
                <div className="slider-container">
                  <Tooltip title={getDepthTooltip(depth)} arrow placement="top">
                    <Box className="slider-with-icon">
                      <DepthIcon />
                ...
                <div className="slider-container">
                  <Tooltip title={getBreadthTooltip(breadth)} arrow placement="top">
                    <Box className="slider-with-icon">
                      <BreadthIcon />
                      <Box sx={{ width: '100%', ml: 2 }}>
                        <Typography variant="body2">
                          Results per Level: {breadth}
                        </Typography>
                        <Slider
                          value={breadth}
              ...
              {isLoading && <div className="loading-indicator">Loading word network...</div>}
            </div>
          )}
        </div>
