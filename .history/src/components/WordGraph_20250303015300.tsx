...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const setupZoom = useCallback((svg: d3.Selection<SVGSVGElement, unknown, null, undefined>, g: d3.Selection<SVGGElement, unknown, null, undefined>) => {
    ...
    svg.transition()
      .duration(200) // Faster initial centering
  ...
  const setupSimulation = useCallback((nodes: CustomNode[], links: CustomLink[]) => {
    ...
    const sim = d3
      ...
      .alphaDecay(0.018) // Slightly slower cooling for smoother transitions
      .velocityDecay(0.5) // Higher damping for extremely smooth motion
      .force('link', d3.forceLink<CustomNode, CustomLink>(links)
        ...
        .distance(d => {
          ...
          if (sourceGroup === 'main') return 120; // Main nodes get more space
          if (sourceGroup === 'root' || sourceGroup === 'root_of') return 110;
          return 90; // Default distance
        })
        .strength(0.8)) // Even stronger links for more stability
      .force('charge', d3.forceManyBody()
        .strength(d => {
          // Adaptive repulsion force - use type assertion to access group property
          return (d as CustomNode).group === 'main' ? -350 : -250;
        })
        .theta(0.85)      // Performance parameter
        .distanceMax(containerWidth / 1.2)) // Limit long-range effects
      .force('center', d3.forceCenter(0, 0))
      .force('collision', d3.forceCollide().radius(d => {
        // Larger collision radius for main word - use type assertion to access group property
        return (d as CustomNode).group === 'main' ? 45 : 35;
      }).strength(0.9)) // Even stronger collision prevention
      .force('x', d3.forceX(0).strength(0.09)) // Slightly stronger force toward center x
      .force('y', d3.forceY(0).strength(0.09)) // Slightly stronger force toward center y
      .on('tick', () => {
    ...
    if (mainWordNode) {
      ...
      setTimeout(() => {
        if (mainWordNode) {
          ...
          setTimeout(() => sim.alphaTarget(0.001), 1000);
    ...
    return d3.drag<SVGGElement, CustomNode>()
      .on('start', (event, d) => {
        ...
        if (!event.active && simulationObject) {
          simulationObject.alphaTarget(0.05).restart();
        }
        ...
        d3.select(event.sourceEvent.currentTarget as Element)
          .select('circle')
          .transition()
          .duration(120) // Faster transition for more immediate feedback
          .ease(d3.easeCubicOut)
  ...
  const createNodes = useCallback((g: d3.Selection<SVGGElement, unknown, null, undefined>, simulation: d3.Simulation<CustomNode, undefined>) => {
    ...
    node.append("text")
      ...
      // Remove drop shadow for cleaner look
  ...
  const highlightConnectedLinks = useCallback((nodeId: string, isHighlighted: boolean) => {
    d3.selectAll(".link")
      ...
      .transition()
      .duration(isHighlighted ? 150 : 300) // Faster highlight, slower unhighlight
      .ease(isHighlighted ? d3.easeCubicOut : d3.easeCubicInOut)
    ...
    node
      ...
      .on("mouseover", (event: MouseEvent, d: CustomNode) => {
        ...
        d3.select(event.currentTarget as Element).select("circle")
          .transition()
          .duration(150) // Faster for more immediate feedback
          .ease(d3.easeCubicOut)
          .attr("r", d.group === "main" ? 28 : 18)
          .attr("stroke-opacity", 1);
    ...
    node.on("dblclick", (event: MouseEvent, d: CustomNode) => {
      ...
      if (d.fx !== null || d.fy !== null) {
        ...
        d3.select(event.currentTarget as Element).select("circle")
          .transition()
          .duration(250)
          .ease(d3.easeCubicOut) // Switch to cubic for more smoothness
          .attr("stroke", "#ffffff")
