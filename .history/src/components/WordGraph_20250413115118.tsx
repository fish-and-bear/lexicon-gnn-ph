...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const toggleRelationshipFilter = useCallback((relationshipType: string) => {
    console.log(`Toggling filter for: ${relationshipType}`);
    
    // Force complete remount
    setForceUpdate(prev => {
      console.log(`Incrementing forceUpdate from ${prev} to ${prev + 1}`);
      return prev + 1;
    });
    
    // Update filtered relationships state
    setFilteredRelationships(prev => {
      const type = relationshipType.toLowerCase();
      
      if (prev.includes(type)) {
        // Removing filter - should show these nodes again
        console.log(`UNFILTERING: ${type} - nodes of this type should reappear`);
        const newFilters = prev.filter(r => r !== type);
        console.log(`New filters will be: [${newFilters.join(', ')}]`);
        return newFilters;
      } else {
        // Adding filter - should hide these nodes
        console.log(`FILTERING: ${type} - nodes of this type should be hidden`);
