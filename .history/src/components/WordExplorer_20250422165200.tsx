      ...
      <main style={{ flexGrow: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}> {/* Ensure main takes remaining height and contains flex children */}
        {isMobile ? (
          ...
          <PanelGroup direction="horizontal" className="explorer-panel-group" style={{ flexGrow: 1, minHeight: 0 }}> {/* Ensure PanelGroup fills space */}
            ...
            <Panel
              id="details-panel"
              // Add ts-expect-error to suppress the specific linter error for this line
              // @ts-expect-error Type mismatch between Panel's expected ref and ResizeObserver usage elsewhere
              ...
              maxSize={75}
              order={2}
              // Add explicit type for the 'collapsed' parameter
              onCollapse={(collapsed: boolean) => {
                console.log('Details panel collapsed:', collapsed);
                // You might want to update state here if needed
              }}
              className={`panel-container ${selectedNode ? 'panel-visible' : 'panel-hidden'}`}
            >
              <div
                id="details-container-observer-target"
                style={{
                  height: '100%',
                  overflowY: 'auto',
                  overflowX: 'hidden',
              ...
              >
                {wordData && (
                  <WordDetails
                    wordData={wordData}
                    containerId="details-container-observer-target" // Pass the ID for ResizeObserver
                    handleNodeClick={handleNodeClick} // Pass node click handler
                  />
                )}
                {!wordData && (
                  <Box display="flex" justifyContent="center" alignItems="center" height="100%">
                    <Typography variant="caption" color="textSecondary">
                      Select a word node or use the search bar.
                    </Typography>
                  </Box>
                )}
              </div>
