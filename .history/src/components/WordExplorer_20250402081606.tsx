...
const WordExplorer: React.FC = () => {
  ...
  // Define handler for Allotment size change
  const handlePaneResize = (sizes: number[]) => {
    if (sizes.length === 2) {
      // Update state with the size of the second (details) pane
      const newDetailSize = sizes[1];
      const clampedSize = Math.max(300, Math.min(800, newDetailSize));
      setDetailPaneSize(clampedSize); // Update state during drag
      // Save to localStorage on drag end (or throttled during drag if preferred)
      localStorage.setItem('wordDetailsWidth', clampedSize.toString());
      console.log('Pane resized, saved size:', clampedSize);
    }
  };
    ...
    <div className={`container ${theme}`}>
      ...
      <main className="main-content">
        <Allotment 
          ...
          onChange={handlePaneResize} // Update state and save size on change
        >
          <Allotment.Pane minSize={400}> {/* Graph Pane - give it a min size */}
            <div className="graph-container" style={{ height: '100%', width: '100%', overflow: 'auto' }}> {/* Use auto overflow */}
          ...
          <Allotment.Pane 
            minSize={300} 
            maxSize={800} 
            preferredSize={detailPaneSize} // Set preferred initial/saved size
          > {/* Details Pane */}
            <div className="details-container" style={{ height: '100%', width: '100%', overflow: 'auto' }}> {/* Use auto overflow */}
