...
const WordExplorer: React.FC = () => {
  ...
  const [resizeHint, setResizeHint] = useState<{ visible: boolean; x: number; y: number; width: number }>({
    visible: false,
    x: 0,
    y: 0,
    width: 0,
  ...
  const handleNodeClick = useCallback(async (nodeData: NodeData) => {
    ...
    setDiagnostic({ show: false, data: null });
    ...
    try {
      ...
      if (details?.id) { 
        await fetchEtymologyTree(details.id);
      }
      
    } catch (error: any) {
      console.error(`Error fetching details for clicked node ${word}:`, error);
      setDetailsError(error.message || 'Failed to load word details');
    } finally {
      setIsLoadingDetails(false);
    }
  }, [mainWord, fetchWordDetails, fetchEtymologyTree]);
  ...
  const handleWordLinkClick = useCallback(async (word: string) => { 
    ...
    try {
      ...
      console.error(`Error fetching details for linked word ${word}:`, error);
      setDetailsError(error.message || 'Failed to load word details');
    } finally {
      setIsLoadingDetails(false);
    }
  ...
  useEffect(() => {
    ...
    const onMouseDown = (e: MouseEvent) => {
      ...
      const currentPixelWidth = parseFloat(leftPanelWidth); 
      setInitialWidth(isNaN(currentPixelWidth) ? (container?.getBoundingClientRect().width || window.innerWidth) / 2 : currentPixelWidth);
      document.body.style.cursor = 'col-resize';
      document.documentElement.style.userSelect = 'none';
      document.documentElement.style.pointerEvents = 'none'; // Disable pointer events on other elements
    ...
    const onMouseMove = (e: MouseEvent) => {
      ...
      animationFrameId = requestAnimationFrame(() => {
        ...
        const minWidth = 200; // Min width in pixels
        const maxWidth = containerRect.width - minWidth; // Max width for left panel
    ...
    const onMouseUp = () => {
      ...
      document.documentElement.style.pointerEvents = ''; // Re-enable pointer events
      if (resizer) {
        resizer.classList.remove('active');
      }
      localStorage.setItem('leftPanelWidthPx', String(leftPanelWidth));
      setResizeHint({ ...resizeHint, visible: false }); // Hide hint
      if (animationFrameId) cancelAnimationFrame(animationFrameId); // Clear any pending frame
    };

    resizer.addEventListener('mousedown', onMouseDown);
    document.addEventListener('mousemove', onMouseMove);
    document.addEventListener('mouseup', onMouseUp);
    document.addEventListener('mouseleave', onMouseUp); // Stop if mouse leaves window
    ...
    return () => {
      ...
      if (animationFrameId) cancelAnimationFrame(animationFrameId); // Cleanup animation frame
    };
  }, [isResizing, leftPanelWidth, resizeStartX, initialWidth, resizeHint]);
  ...
  // Function to render the content of the details panel
  const renderDetailsContent = () => {
    if (isLoadingDetails) {
      return <div className="loading-details"><div className="loading-spinner"></div><p>Loading details...</p></div>;
    }
    if (detailsError) {
      return <div className="error-details"><h3>Error Loading Details</h3><p>{detailsError}</p><p className="error-suggestion">Please try searching again or selecting a different node.</p></div>;
    }
    if (selectedWordInfo) {
      return (
        <WordDetails 
    ...
    return <div className="no-word-selected"><p>Select a node in the graph or search for a word to see details.</p><p className="empty-tip">Tip: Click on a word bubble!</p></div>;
  };
  ...
  return (
    <div className={`word-explorer ${theme} ${isLoading ? 'loading' : ''}`}>
      ...
      <main ref={mainRef} style={{ display: 'flex', height: 'calc(100vh - 120px)' }}> 
        <div className="graph-container" style={{ flexBasis: leftPanelWidth, flexShrink: 0 }}>
          <div className="graph-content">
            {isLoadingNetwork && (
               <div className="loading-placeholder">Loading Network...</div>
            )}
            {networkError && !isLoadingNetwork && (
               <div className="error-placeholder">Error: {networkError}</div>
            )}
            {!isLoadingNetwork && !networkError && wordNetwork && (
              <div style={{ height: '100%', width: '100%', position: 'relative' }}>
                <WordGraph 
                  ...
                  onNetworkChange={(newDepth: number) => setDepth(newDepth)}
                  initialDepth={depth}
            ...
            {!isLoadingNetwork && !wordNetwork && !error && !selectedWordInfo && (
        ...
        >
          {isResizing && <div className="resize-hint">{`${parseFloat(leftPanelWidth).toFixed(0)}%`}</div>}
        </div>
        
        <div className="details-container" style={{ flexGrow: 1 }}>
