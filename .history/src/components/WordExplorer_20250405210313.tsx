...
const safeParse = (input: string | Record<string, any> | null | undefined, returnType: 'array' | 'object' = 'array'): any[] | Record<string, any> => {
  // ... existing safeParse implementation ...
};
...
const processRawWordData = (rawData: RawWordData): WordInfo => {
  ...
  if (!rawData) {
    ...
    // Return a default/empty WordInfo structure or throw an error
    // Ensure the default object matches the WordInfo structure minimally
    return { id: 0, lemma: 'Error: No Data' };
  }

  // Process definitions
  const cleanDefinitions = (rawData.definitions || []).map((def: RawDefinition): Definition => {
     const relatedDefs = def.related_definitions
        ? def.related_definitions
            // Filter out null/undefined before mapping
            .filter(r => r != null)
            // Map safely, providing a minimal default if needed
            // Recursive call needs RawWordData input
            .map(r => processRawWordData({ ...r, id: r.id ?? 0, lemma: r.lemma ?? '' } as RawWordData).definitions?.[0])
            // Filter out results that might be undefined after processing
    ...
    return {
      ...
      // Assign processed related definitions
      related_definitions: relatedDefs,
      // Explicitly exclude fields not in the clean Definition type by not including them
      // standardized_pos_id: undefined, // No need for this
      // original_pos: undefined, // No need for this
    };
  });
  ...
  const cleanEtymologies = (rawData.etymologies || []).map((ety: RawEtymology): Etymology => ({
    ...
    // normalized_components: undefined,
    // etymology_structure: undefined,
  ...
  const cleanTemplates = (rawData.templates || []).map((template: WordTemplate): WordTemplate => ({
    ...
    const processRelation = (rel: Relation): Relation => ({
        ...
        sources: safeParse(rel.sources, 'array') as string[] | null,
    });
    ...
    const processAffixation = (affix: Affixation): Affixation => ({
        ...affix,
        sources: safeParse(affix.sources, 'array') as string[] | null,
    });
