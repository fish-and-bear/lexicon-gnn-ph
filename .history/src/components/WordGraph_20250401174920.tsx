...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const links: CustomLink[] = useMemo(() => {
    ...
    return wordNetwork.edges
      ...
      .filter((link): link is { source: number; target: number; relationship: string; } => link !== null)
  ...
  const filteredNodes = useMemo(() => {
    ...
    while (queue.length > 0) {
      ...
      const relatedWords = links
        .filter(link => 
          (typeof link.source === 'object' ? link.source : link.source.id) === currentWord ||
          (typeof link.target === 'object' ? link.target : link.target.id) === currentWord
        )
        .map(link => {
