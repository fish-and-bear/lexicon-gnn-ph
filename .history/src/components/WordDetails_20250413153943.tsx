...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({
  ...
  const renderRelationsTab = () => {
    ...
    // Check if we have relations in either array
    const hasIncoming = incoming_relations.length > 0;
    const hasOutgoing = outgoing_relations.length > 0;

    // Enhanced debug logging
    console.log("WordDetails renderRelationsTab - detailed data:", {
      id: wordInfo.id,
      lemma: wordInfo.lemma,
      incomingLength: incoming_relations.length,
      outgoingLength: outgoing_relations.length,
      incomingSample: hasIncoming ? JSON.stringify(incoming_relations[0]) : null,
      outgoingSample: hasOutgoing ? JSON.stringify(outgoing_relations[0]) : null,
      hasSourceWord: hasIncoming ? Boolean(incoming_relations[0]?.source_word) : false,
      hasTargetWord: hasOutgoing ? Boolean(outgoing_relations[0]?.target_word) : false,
      hasRelationType: hasIncoming ? Boolean(incoming_relations[0]?.relation_type) : 
                      (hasOutgoing ? Boolean(outgoing_relations[0]?.relation_type) : false)
    });
    ...
    const renderRelationGroup = (title: string, relations: WordInfo['outgoing_relations'] | WordInfo['incoming_relations'], direction: 'in' | 'out') => {
      ...
      // ADDED: Try to fix incomplete relations data
      if (!hasValidStructure) {
        console.warn(`Attempting to fix invalid ${direction === 'in' ? 'incoming' : 'outgoing'} relations structure`);
        
        // Try to repair the relations data - create a sanitized version
        const sanitizedRelations = relations.map(rel => {
          // Cast rel to a type that allows string indexing
          const anyRel = rel as Record<string, any>;
          const fixed = {...rel};
          
          // Ensure relation_type exists
          if (!fixed.relation_type) {
            fixed.relation_type = 'related'; // Default to 'related'
          }
          
          // Ensure source_word or target_word exists depending on direction
          if (direction === 'in' && !fixed.source_word) {
            // Create minimal source_word from raw data if possible
            if (typeof anyRel === 'object' && anyRel !== null) {
              const candidateProps = ['from', 'from_word', 'source', 'lemma', 'word'];
              for (const prop of candidateProps) {
                if (anyRel[prop] && typeof anyRel[prop] === 'string') {
                  fixed.source_word = { 
                    id: 0,
                    lemma: anyRel[prop]
                  };
                  break;
                } else if (anyRel[prop] && typeof anyRel[prop] === 'object') {
                  fixed.source_word = { 
                    id: anyRel[prop].id || 0,
                    lemma: anyRel[prop].lemma || anyRel[prop].word || 'Unknown'
                  };
                  break;
                }
              }
            ...
            // If we still don't have a source_word, create a placeholder
            if (!fixed.source_word) {
              fixed.source_word = { 
                id: 0,
                lemma: 'Unknown Source'
              };
            }
          } else if (direction === 'out' && !fixed.target_word) {
            // Create minimal target_word from raw data if possible
            if (typeof anyRel === 'object' && anyRel !== null) {
              const candidateProps = ['to', 'to_word', 'target', 'lemma', 'word'];
              for (const prop of candidateProps) {
                if (anyRel[prop] && typeof anyRel[prop] === 'string') {
                  fixed.target_word = { 
                    id: 0,
            ...
            // If we still don't have a target_word, create a placeholder
            if (!fixed.target_word) {
              fixed.target_word = { 
                id: 0,
                lemma: 'Unknown Target'
              };
            }
          }
          ...
          // Only keep relations that have both a relation_type and the appropriate word
          rel.relation_type && (direction === 'in' ? rel.source_word : rel.target_word)
        );
        
        if (sanitizedRelations.length === 0) {
          console.error(`Failed to repair ${direction === 'in' ? 'incoming' : 'outgoing'} relations`);
          return (
        ...
        console.log(`Successfully repaired ${sanitizedRelations.length} relations`);
        
        // Continue with the repaired relations
        return renderRelationGroupContent(title, sanitizedRelations, direction);
      }

      return renderRelationGroupContent(title, relations, direction);
    };

    const renderRelationGroupContent = (title: string, relations: any[], direction: 'in' | 'out') => {
                ...
                return (
                  <Box key={type}>
                    ...
                    <Stack direction="row" spacing={theme.spacing(1)} useFlexGap flexWrap="wrap">
                      {grouped[type].map((relatedItem: any, index: number) => {
