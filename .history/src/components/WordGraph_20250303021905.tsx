...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  // Improve the simulation to stabilize motion and maintain center position
  const setupSimulation = useCallback((nodes: CustomNode[], links: CustomLink[]) => {
    ...
    // Create a new simulation with optimized forces for stability
    const sim = d3
      .forceSimulation<CustomNode>(nodes)
      .alphaDecay(0.015) // Perfect cooling for smooth transitions
      .velocityDecay(0.55) // Higher damping for ultra-smooth motion
      .force('link', d3.forceLink<CustomNode, CustomLink>(links)
        .id(d => d.id)
        .distance(d => {
          // Adaptive link distance based on node relationship
          const source = typeof d.source === 'object' ? d.source : nodes.find(n => n.id === d.source);
          const sourceGroup = source ? source.group : 'other';
          if (sourceGroup === 'main') return 130; // Main nodes get more space
          if (sourceGroup === 'root' || sourceGroup === 'root_of') return 115;
          return 95; // Default distance
        })
        .strength(0.85)) // Stronger links for more stability
      .force('charge', d3.forceManyBody()
        .strength(d => {
          // Adaptive repulsion force - use type assertion to access group property
          return (d as CustomNode).group === 'main' ? -400 : -280;
        })
        .theta(0.85)      // Performance parameter
        .distanceMax(containerWidth / 1.1)) // Limit long-range effects
      .force('center', d3.forceCenter(0, 0))
      .force('collision', d3.forceCollide().radius(d => {
        // Larger collision radius for main word - use type assertion to access group property
        return (d as CustomNode).group === 'main' ? 48 : 38;
      }).strength(0.95)) // Even stronger collision prevention
      .force('x', d3.forceX(0).strength(0.1)) // Slightly stronger force toward center x
      .force('y', d3.forceY(0).strength(0.1)) // Slightly stronger force toward center y
      .on('tick', () => {
        if (!svgRef.current) return;

        // Only update visuals if not dragging or transitioning
        if (!isDraggingRef.current && !isTransitioningRef.current) {
          // Update node positions with smoother damping
          d3.select(svgRef.current)
            .selectAll<SVGGElement, CustomNode>('.node')
            .attr('transform', d => {
              // Constrain nodes to stay within reasonable bounds
              const margin = 65;
              const boundX = Math.max(-containerWidth + margin, Math.min(containerWidth - margin, d.x || 0));
              const boundY = Math.max(-containerHeight + margin, Math.min(containerHeight - margin, d.y || 0));
              
              // Apply even more damping for ultra-smooth motion
              if (d.vx !== undefined) d.vx *= 0.95;
              if (d.vy !== undefined) d.vy *= 0.95;
              
              // Update the node's position for future reference
              d.x = boundX;
              d.y = boundY;
          ...
          // Update link positions with the same smoothness
          d3.select(svgRef.current)
            .selectAll<SVGLineElement, CustomLink>('.link')
            .attr('x1', d => {
    ...
    // Position the main word at the center initially
    const mainWordNode = nodes.find(n => n.id === mainWord);
    if (mainWordNode) {
      mainWordNode.fx = 0;
      mainWordNode.fy = 0;
      
      // Release the fixed position after a delay to allow natural layout
      setTimeout(() => {
        if (mainWordNode) {
          // Smoothly transition from fixed to unfixed
          sim.alphaTarget(0.01).restart();
          mainWordNode.fx = null;
          mainWordNode.fy = null;
          
          // More gradual cooldown
          setTimeout(() => sim.alphaTarget(0.001), 800);
          setTimeout(() => sim.alphaTarget(0), 1600);
        }
      }, 2000);
    }
    ...
    return sim;
  }, [simulationObject, mainWord]);

  // Create an improved drag behavior for smoother interactions
  const createDragBehavior = useCallback(() => {
    return d3.drag<SVGGElement, CustomNode>()
      .on('start', (event, d) => {
        ...
        // Prevent the default behavior which can interfere with drag
        event.sourceEvent.preventDefault();
        event.sourceEvent.stopPropagation();
        
        // Warm up the simulation while dragging, but more gently
        if (!event.active && simulationObject) {
          simulationObject.alphaTarget(0.03).restart();
        }
        ...
        // Give visual feedback with better transition
        const currentElement = event.sourceEvent.currentTarget;
        if (currentElement && currentElement instanceof Element) {
          d3.select(currentElement)
            .select('circle')
            .transition()
            .duration(100) // Even faster transition for immediate feedback
            .ease(d3.easeExpOut) // More natural feeling elastic effect
            .attr('r', (d.group === "main" ? 29 : 19))
            .attr('stroke-width', 3);
        }
      })
      .on('drag', (event, d) => {
        ...
        if (currentElement && currentElement instanceof Element) {
          ...
          // Only update connected links if we have a valid SVG reference
          if (svgRef.current) {
            const svg = d3.select(svgRef.current);
            
            // Get all link elements that connect to this node
            svg.selectAll<SVGLineElement, CustomLink>('.link')
              .each(function(link) {
                // Safe type checking - make sure link data is properly structured
                const source = typeof link.source === 'object' ? link.source : null;
                const target = typeof link.target === 'object' ? link.target : null;
                const sourceId = source ? source.id : (typeof link.source === 'string' ? link.source : null);
                const targetId = target ? target.id : (typeof link.target === 'string' ? link.target : null);
                
                // Only proceed if this link connects to our dragged node
                if (sourceId === d.id || targetId === d.id) {
                  // Get the link element and ensure it's a valid DOM element
                  ...
                  if (linkElement && linkElement instanceof Element) {
                    ...
                    if (sourceId === d.id) {
                      // This node is the source - use drag event coordinates
                      sourceX = event.x;
                      sourceY = event.y;
                    } else if (source && typeof source.x === 'number' && typeof source.y === 'number') {
                      // Use the existing coordinates from the source node
                      sourceX = source.x;
                      sourceY = source.y;
                    } else {
                      // Try to find the node in filtered nodes
                    ...
                    if (targetId === d.id) {
                      // This node is the target - use drag event coordinates
                      targetX = event.x;
                      targetY = event.y;
                    } else if (target && typeof target.x === 'number' && typeof target.y === 'number') {
                      // Use the existing coordinates from the target node
                      targetX = target.x;
                      targetY = target.y;
                    } else {
                      // Try to find the node in filtered nodes
                    ...
                    // Update link position directly using setAttribute for better compatibility
      ...
      .on('end', (event, d) => {
        // Apply a short delay before resetting the dragging flag to prevent accidental clicks
        // but keep it minimal for better responsiveness
        setTimeout(() => {
          isDraggingRef.current = false;
        }, 50);
        
        // Cool down the simulation more gradually for smoother transitions
        if (!event.active && simulationObject) {
          // Three-stage cooldown for more natural motion
          simulationObject.alphaTarget(0.03);
          setTimeout(() => {
            if (simulationObject) simulationObject.alphaTarget(0.01);
            setTimeout(() => {
              if (simulationObject) simulationObject.alphaTarget(0);
            }, 300);
          }, 300);
        }
        ...
        const currentElement = event.sourceEvent.currentTarget;
        if (currentElement && currentElement instanceof Element) {
          // Check if we should keep the node fixed (pinned)
          if (event.sourceEvent.shiftKey) {
            // Keep node pinned at its current position
            // Find the node and mark it visually
            d3.select(currentElement)
              .select('circle')
              .transition()
              .duration(200)
              ...
              .attr("stroke", "#fca311")
              .attr("stroke-width", 3)
              .attr("stroke-dasharray", "3,2"); // Add dashed border for pinned nodes
          } else {
            // Unpin the node
            d.fx = null;
            d.fy = null;
            
            // Reset visual style with smoother transition
            d3.select(currentElement)
              .select('circle')
              .transition()
              .duration(200)
  ...
  }, [simulationObject, selectedNodeId, filteredNodes]);
  ...
  const createLinks = useCallback((g: d3.Selection<SVGGElement, unknown, null, undefined>) => {
    ...
    return g
      ...
      // Add hover interaction with smoother transitions
  ...
  const createNodes = useCallback((g: d3.Selection<SVGGElement, unknown, null, undefined>, simulation: d3.Simulation<CustomNode, undefined>) => {
    ...
    const node = g
      ...
      // Add a data-id attribute to make node selection easier
      .attr("id", (d: CustomNode) => `node-${d.id.replace(/\s+/g, '-')}`)
      .call(dragBehavior);
    ...
    return node;
  }, [filteredNodes, selectedNodeId, theme, getNodeColor, createDragBehavior]);

  // Helper function to highlight connected links with smoother transitions
  const highlightConnectedLinks = useCallback((nodeId: string, isHighlighted: boolean) => {
    d3.selectAll(".link")
      ...
      .transition()
      .duration(isHighlighted ? 120 : 250) // Faster highlight, slower unhighlight
      .ease(isHighlighted ? d3.easeExpOut : d3.easeExpInOut)
      .attr("stroke-width", isHighlighted ? 2.5 : 1.5)
      .attr("stroke-opacity", isHighlighted ? 1 : 0.65);
  }, []);
  
  // Helper function to emphasize connected nodes with smoother highlighting
  const showConnectedNodes = useCallback((nodeId: string) => {
    ...
    // Apply subtle highlighting to connected nodes with smooth transitions
    d3.selectAll(".node")
  ...
  // Update setupNodeInteractions for smoother transitions
  const setupNodeInteractions = useCallback((node: d3.Selection<SVGGElement, CustomNode, SVGGElement, unknown>, svg: d3.Selection<SVGSVGElement, unknown, null, undefined>) => {
    node
      .on("click", (event: MouseEvent, d: CustomNode) => {
        // Ignore click if currently dragging or if it's too soon after the last click
        const now = Date.now();
        if (isDraggingRef.current || now - lastClickTimeRef.current < 300) return;
        ...
        // First, reset styling on all nodes with smooth transitions
        d3.selectAll(".node").classed("selected", false);
        d3.selectAll(".node circle")
          .transition()
          .duration(200) // Slightly faster
          .ease(d3.easeExpInOut) // More natural elastic transition
          .attr("stroke-width", 2)
          .attr("r", n => (n as any).group === "main" ? 25 : 15);
        
        // Apply selected styling to clicked node with smooth animation
        d3.select(event.currentTarget as Element)
          .classed("selected", true)
          .select("circle")
          .transition()
          .duration(250)
          .ease(d3.easeBackOut.overshoot(1.5)) // Add slight bounce effect
          .attr("stroke-width", 3)
        ...
        // Only call onNodeClick if this is a new selection
        if (d.id !== selectedNodeId) {
          // Store the current main word to handle transition
          prevMainWordRef.current = mainWord;
          
          // Set selected node ID
          setSelectedNodeId(d.id);
          
          // Set transitioning flag to prevent simulation updates during transition
          isTransitioningRef.current = true;
          
          // First, mark the transition visually to prevent jarring changes
          const currentNodeElement = d3.select(event.currentTarget as Element);
          currentNodeElement
            .select('circle')
            .transition()
            .duration(200) // Slightly faster
            .ease(d3.easeExpOut) // More natural elastic transition
            .attr('r', 28)
            .attr('fill', getNodeColor('main'))
            .attr('stroke-width', 3);
            
          // Call onNodeClick to change the selected word after a slight delay
          // Small delay for the visual transition to begin
          setTimeout(() => {
            console.log(`Node clicked: ${d.id}, updating main word`);
            onNodeClick(d.id);
            
            // Reset transitioning flag after a delay to allow for state updates
            // But keep it shorter for better responsiveness
            setTimeout(() => {
      ...
      .on("mouseover", (event: MouseEvent, d: CustomNode) => {
        ...
        // Enlarge the hovered node with smoother animation
        d3.select(event.currentTarget as Element).select("circle")
          .transition()
          .duration(120) // Faster for more immediate feedback
          .ease(d3.easeExpOut) // More natural elastic effect
          .attr("r", d.group === "main" ? 28 : 18)
          .attr("stroke-opacity", 1);
      ...
      .on("mouseout", (event: MouseEvent, d: CustomNode) => {
        ...
        // Return to normal size if not selected with smooth animation
        if (d.id !== selectedNodeId) {
          d3.select(event.currentTarget as Element).select("circle")
            .transition()
            .duration(180)
        ...
        // Reset link highlighting with smooth fade
        highlightConnectedLinks(d.id, false);
      });
    
    // Double click to pin/unpin nodes with improved visual feedback
    node.on("dblclick", (event: MouseEvent, d: CustomNode) => {
      event.stopPropagation(); // Prevent event bubbling
      event.preventDefault(); // Prevent default behavior
      
      if (d.fx !== null || d.fy !== null) {
        // Unpin node with animation
        d.fx = null;
        d.fy = null;
        d3.select(event.currentTarget as Element).select("circle")
          .transition()
          .duration(200)
          .ease(d3.easeExpInOut) // Switch to elastic for more smoothness
          .attr("stroke", "#ffffff")
          .attr("stroke-dasharray", null);
      } else {
        // Pin node with animation
        d.fx = d.x;
        d.fy = d.y;
        d3.select(event.currentTarget as Element).select("circle")
          .transition()
          ...
          .attr("stroke-dasharray", "3,2"); // Add dashed effect for pinned nodes
      }
    });
  }, [onNodeClick, selectedNodeId, showConnectedNodes, highlightConnectedLinks, mainWord, getNodeColor]);
  ...
  const updateGraph = useCallback(() => {
    ...
    try {
      ...
      // Start with a translation to the center of the SVG
      ...
      // Initialize the simulation with the nodes centered around (0,0)
      ...
      // Center on main word after the graph is created
      setTimeout(() => centerOnMainWord(svg), 500);
      
      // Add event listener for window resize to maintain centering
      const handleResize = () => {
        // Wait a moment for the resize to complete
        setTimeout(() => centerOnMainWord(svg), 100);
      };
      window.addEventListener('resize', handleResize);
      
      // Add grid background for better spatial awareness 
      ...
      for (let x = -gridExtent; x <= gridExtent; x += gridSize) {
        gridGroup.append("line")
          ...
          .attr("x2", x)
          .attr("y2", gridExtent)
          .attr("stroke", theme === "dark" ? "rgba(255,255,255,0.02)" : "rgba(0,0,0,0.02)")
          .attr("stroke-width", 1);
      }
      ...
      // The legend code remains unchanged
  ...
  }, [filteredNodes, links, theme, getNodeColor, createLinks, createNodes, setupNodeInteractions, setupSimulation, setupSvgDimensions, setupZoom, centerOnMainWord]);
  ...
  const renderTooltip = useCallback(() => {
    ...
    if (nodeData.definitions && nodeData.definitions.length > 0) {
      // Try to get a definition object first
      const defObj = nodeData.definitions.find(d => typeof d === 'object');
      if (defObj) {
    ...
    if (definition === "No definition available" && hoveredNode.info.definitions) {
      const infoDefObj = hoveredNode.info.definitions.find(d => typeof d === 'object');
      if (infoDefObj) {
        ...
        const infoDefString = hoveredNode.info.definitions.find(d => typeof d === 'string');
        if (infoDefString) definition = infoDefString;
      }
    }
    
    // Get relationship label
    let relationshipLabel = hoveredNode.group.charAt(0).toUpperCase() + hoveredNode.group.slice(1).replace(/_/g, ' ');
    ...
    if (prevMainWordRef.current && prevMainWordRef.current !== mainWord) {
      ...
      if (svgRef.current) {
        setTimeout(() => {
          ...
          if (svgRef.current) {
            // Fix: Use non-null assertion to help TypeScript understand this is safe
