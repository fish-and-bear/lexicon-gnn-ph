    ...
    try {
      ...
      const wordNetworkData: WordNetwork = {
        ...
        metadata: {
          ...
          // Use available depth/breadth params instead of options
          include_affixes: apiData.metadata?.include_affixes ?? true, // Default to true? Or based on breadth/depth?
          include_etymology: apiData.metadata?.include_etymology ?? true, // Default to true?
          cluster_threshold: apiData.metadata?.cluster_threshold ?? 0.3 // Default threshold?
        }
      };
            ...
            setSelectedWordInfo(prevInfo => {
              ...
              return {
                ...
                semantic_network: {
                  ...
                  // Map edges back to links for WordInfo.semantic_network
                  links: mappedNetworkData.edges.map(edge => ({ 
                    source: edge.source,
                    target: edge.target,
                    type: edge.type
                  })) as NetworkLink[] 
        ...
        if (!wordData.outgoing_relations?.length && !wordData.incoming_relations?.length) {
          try {
            ...
            fetchWordNetworkData(wordData.lemma, depth, breadth)
              .then(mappedNetworkData => { // Use mapped data
                if (mappedNetworkData && (mappedNetworkData.nodes?.length || mappedNetworkData.edges?.length)) { // Check edges
              .then(networkData => {
