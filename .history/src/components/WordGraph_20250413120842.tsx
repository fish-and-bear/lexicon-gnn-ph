      ...
      category.types.forEach(type => {
        ...
        entry.append("rect")
          ...
          .on("click", function(this: SVGRectElement) {
            ...
            // Check current filter state before we toggle
            const wasFiltered = filteredRelationships.includes(relType.toLowerCase());
            
            // Toggle filtering for this relationship type - this updates the state
            // and triggers our filtering useEffect
            handleToggleRelationshipFilter(relType);
            
            // Just update the visual appearance of the legend item
            // The node visibility will be handled by the useEffect
            d3.select(this)
              .transition()
              .duration(300)
            ...
            // Update circle opacity for visual consistency
            parentGroup.select("circle")
              .transition()
              .duration(300)
