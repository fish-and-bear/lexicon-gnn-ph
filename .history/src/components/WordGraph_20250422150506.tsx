...
import FilterListIcon from '@mui/icons-material/FilterList'; // Import icon for legend trigger
...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const [legendDrawerOpen, setLegendDrawerOpen] = useState(false); // State for desktop legend drawer
  ...
  useEffect(() => {
    ...
    // --- Removed Desktop SVG Legend Rendering --- 
    // } // Closing brace for the removed `if (!isMobile)` block
    ...
    const centerTimeout = setTimeout(() => {
      ...
      if (svgRef.current) {
        ...
        // svg.selectAll(".legend-item rect").on("click", null).on("mouseover", null).on("mouseout", null); // Legend items removed
      }
      };
    ...
    centerOnMainWord,
     setupSvgDimensions,
     filteredNodes,
   baseLinks, // Include baseLinks if used in interactions/tooltips
   filteredRelationships,
   filterUpdateKey,
   forceUpdate,
   handleToggleRelationshipFilter,
   // getUniqueRelationshipGroups, // Removed from dependencies as it's only used for removed legend
   muiTheme, // Keep MUI theme if used elsewhere or in planned drawer
   isMobile // Keep isMobile as it might affect other parts or drawer logic
