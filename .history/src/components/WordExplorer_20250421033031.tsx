...
const WordExplorer: React.FC = () => {
  ...
  const fetchSuggestionsDebounced = React.useMemo(
    () =>
      debounce(
        async (request: { input: string }, callback: (results: string[]) => void) => {
          if (!request.input) {
            callback([]);
            return;
          }
  ...
  const handleSearch = useCallback(async (query: string, options?: SearchOptions) => {
    ...
    try {
      ...
      if (result && result.words && result.words.length > 0) {
        ...
        // Decide whether to show suggestions based on search results
        // Original logic might have varied, setting to false after search seems common
        setShowSuggestions(false); 
        ...
        try {
          ...
          // History update logic might need adjustment based on how you stored it
          const historyEntry = typeof wordData.id === 'number' || typeof wordData.id === 'string' ? { id: wordData.id, text: wordData.lemma } : wordData.lemma;
          const newHistory = [...wordHistory.slice(0, currentHistoryIndex + 1), historyEntry];
          setWordHistory(newHistory as any); // Use type assertion carefully
          setCurrentHistoryIndex(newHistory.length - 1);
          ...
          try {
            ...
            const etymologyIdString = String(wordData.id);
            // Ensure wordData.id is treated as number if possible
            const etymologyId = typeof wordData.id === 'number' ? wordData.id : 
                                (etymologyIdString.startsWith('id:') ? parseInt(etymologyIdString.substring(3), 10) : NaN);
            if (!isNaN(etymologyId)) {
              fetchEtymologyTree(etymologyId)
                .then(tree => {
               ...
               console.warn(`Could not determine valid numeric ID for etymology fetch from wordData.id: ${wordData.id}`);
            }
          } catch (etymErr) {
          ...
          setSearchResults(result.words); // Keep search results displayed on error
        }
      } else {
      ...
      setIsLoading(false);
      setIsLoadingSuggestions(false); // Ensure this is handled
    }
  }, [fetchWordNetworkData, fetchEtymologyTree, wordHistory, currentHistoryIndex, depth, breadth, setWordHistory, setCurrentHistoryIndex, setWordData, setWordNetwork, setSelectedNode, setError, setIsLoading, setSearchResults, setShowSuggestions, setIsLoadingSuggestions, setEtymologyTree]);
                   ...
                   endAdornment: (
                     <React.Fragment>
                       {isSuggestionsLoading && (
                         <div className="search-loader" style={{ position: 'absolute', right: '40px', top: '50%', transform: 'translateY(-50%)' }}>
