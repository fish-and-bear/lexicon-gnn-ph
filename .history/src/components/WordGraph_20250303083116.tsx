    ...
    try {
      ...
      return (
        <div className="node-tooltip" style={combinedStyle}>
          ...
          {hoveredNode.info && hoveredNode.info.definitions && hoveredNode.info.definitions.length > 0 && (
            <div style={{ marginBottom: '5px' }}>
              {typeof hoveredNode.info.definitions[0] === 'string' 
                ? hoveredNode.info.definitions[0] 
                : (hoveredNode.info.definitions[0] as any)?.text || (hoveredNode.info.definitions[0] as any)?.definition_text || 'No definition available'}
            </div>
          )}
          
          {hoveredNode.info && hoveredNode.info.examples && hoveredNode.info.examples.length > 0 && (
            <div style={{ fontStyle: 'italic', fontSize: '0.85rem', opacity: 0.9 }}>
              "{typeof hoveredNode.info.examples[0] === 'string' ? hoveredNode.info.examples[0] : ''}"
            </div>
          )}
