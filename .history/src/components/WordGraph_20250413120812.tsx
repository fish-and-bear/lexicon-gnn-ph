...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const filteredNodes = useMemo<CustomNode[]>(() => {
    ...
    // IMPORTANT: First collect ALL nodes based on depth/breadth, ignoring filters
    ...
    // Collect ALL possible connected nodes within depth/breadth limits
    // WITHOUT considering filters during traversal
    while (queue.length > 0) {
      ...
      // Get all connected nodes, ignoring filters
      ...
      // Add nodes to the breadth-first search, limited by breadth parameter
      sortedWords.slice(0, breadth).forEach(wordId => {
        if (nodeMap.has(wordId)) {
          // Always collect the nodes, even if they'll be filtered later
    ...
    // Always include all nodes connected by depth/breadth first
    const nodesConnectedByDepth = baseNodes.filter(node => connectedNodeIds.has(node.id));
    
    console.log(`Depth/breadth filtering: ${nodesConnectedByDepth.length} nodes from ${baseNodes.length}`);
    
    // If no relationship filters are active, return all connected nodes
    if (filteredRelationships.length === 0) {
      console.log("No relationship filters active, showing all connected nodes");
      return nodesConnectedByDepth;
    }
    
    // Otherwise, apply relationship filtering
    const result = nodesConnectedByDepth.filter(node => {
      // Always include the main word
      if (node.id === mainWord) return true;
      
      // For other nodes, exclude those whose group is in the filtered list
      return !filteredRelationships.includes(node.group.toLowerCase());
    });
    
    console.log(`After relationship filtering: ${result.length} nodes visible (from ${nodesConnectedByDepth.length})`);
  ...
  // Complete redesign of the filtering logic
  useEffect(() => {
    // Don't run this effect during initial render or when there's no graph
    if (!svgRef.current || !mainWord) return;
    
    console.log("[FILTER DEBUG] Updating node visibility for filters:", filteredRelationships);
    
    // Get all existing nodes in the DOM
    const svg = d3.select(svgRef.current);
    
    // First, log what nodes exist in the DOM
    let nodeCount = 0;
    let nodeGroupCounts: Record<string, number> = {};
    
    svg.selectAll(".node").each(function(d: any) {
      nodeCount++;
      const group = d.group.toLowerCase();
      nodeGroupCounts[group] = (nodeGroupCounts[group] || 0) + 1;
    });
    
    console.log(`[FILTER DEBUG] Total DOM nodes: ${nodeCount}`, nodeGroupCounts);
    
    // For each relationship type that is now filtered
    filteredRelationships.forEach(relationship => {
      // Set opacity to 0 for filtered nodes
      svg.selectAll(".node")
        .filter(function(d: any) {
          return d.group.toLowerCase() === relationship.toLowerCase() && d.id !== mainWord;
        })
        .classed("filtered", true)
        .style("opacity", 0)
        .style("pointer-events", "none");
      
      // Set opacity to 0 for filtered links
      svg.selectAll(".link")
        .filter(function(d: any) {
          return d.type?.toLowerCase() === relationship.toLowerCase();
        })
        .classed("filtered", true)
        .style("opacity", 0)
        .style("pointer-events", "none");
    ...
    // For each relationship type that is NOT filtered
    Object.keys(nodeGroupCounts).forEach(group => {
      if (!filteredRelationships.includes(group.toLowerCase())) {
        // Set opacity to 1 for visible nodes
        svg.selectAll(".node")
          .filter(function(d: any) {
            return d.group.toLowerCase() === group.toLowerCase() || d.id === mainWord;
          })
          .classed("filtered", false)
          .style("opacity", 1)
          .style("pointer-events", "auto");
        
        // Set opacity to 0.6 for visible links
        svg.selectAll(".link")
          .filter(function(d: any) {
            return d.type?.toLowerCase() === group.toLowerCase();
          })
          .classed("filtered", false)
          .style("opacity", 0.6)
          .style("pointer-events", "auto");
    ...
    svg.selectAll(".node")
      .filter(function(d: any) {
        return d.id === mainWord;
      })
      .classed("filtered", false)
      .style("opacity", 1)
      .style("pointer-events", "auto");
    
    // Count filtered vs visible nodes after updates
    let visibleNodes = 0;
    let filteredNodes = 0;
    ...
    console.log(`[FILTER DEBUG] After filter update - Visible: ${visibleNodes}, Filtered: ${filteredNodes}`);
    
  }, [filteredRelationships, mainWord]);

  // Completely redesigned toggle filter handler
  const handleToggleRelationshipFilter = useCallback((relationship: string) => {
    console.log("[FILTER DEBUG] Toggling filter for:", relationship);
    ...
    setFilteredRelationships(prevFilters => {
      ...
      if (newFilters.includes(relationshipLower)) {
        console.log("[FILTER DEBUG] Removing filter:", relationshipLower);
        const index = newFilters.indexOf(relationshipLower);
        newFilters.splice(index, 1);
      } else {
        console.log("[FILTER DEBUG] Adding filter:", relationshipLower);
        newFilters.push(relationshipLower);
      }
      
      console.log("[FILTER DEBUG] New filters:", newFilters);
      return newFilters;
    });
    
    // Don't force a graph rebuild - let the useEffect handle visibility
  }, []);
  ...
  useEffect(() => {
    ...
    // Add a unique key based on filtered relationships to force recreation
      ...
      category.types.forEach(type => {
        ...
        entry.append("rect")
          ...
          .on("click", function(this: SVGRectElement) {
            ...
            // Update visual state of the legend item immediately
            ...
            // Directly update nodes of this type in the DOM
            const svg = d3.select(svgRef.current);
            svg.selectAll(".node")
              .filter(function(d: any) {
                return d.group.toLowerCase() === relType.toLowerCase() && d.id !== mainWord;
              })
              .classed("filtered", !wasFiltered)
              .transition()
              .duration(300)
              .style("opacity", wasFiltered ? 1 : 0)
              .style("pointer-events", wasFiltered ? "all" : "none");
              
            // Update links with this relationship type
            svg.selectAll(".link")
              .filter(function(d: any) {
                return d.type?.toLowerCase() === relType.toLowerCase();
              })
              .classed("filtered", !wasFiltered)
              .transition()
              .duration(300)
              .style("opacity", wasFiltered ? 0.6 : 0)
              .style("pointer-events", wasFiltered ? "all" : "none");
          });
    ...
    <div className="graph-container">
      <div className="graph-svg-container">
        ...
        <svg 
          ...
          key={`graph-${forceUpdate}-${filteredRelationships.join('-')}`}
        >
        </svg>
      </div>
