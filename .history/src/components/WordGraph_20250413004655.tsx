...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const filteredNodes = useMemo<CustomNode[]>(() => {
    ...
    // First collect nodes based on depth and breadth
    ...
    console.log(`Connected nodes before filtering: ${nodesConnectedByDepth.length}, with groups:`, 
      nodesConnectedByDepth.reduce((acc, node) => {
    ...
    const result = nodesConnectedByDepth.filter(node => {
      ...
      if (filteredRelationships.length === 0) {
        console.log(`Node ${node.id} (${node.group}) included - no filters active`);
        return true;
      }
      ...
      if (isFiltered) {
        console.log(`Node ${node.id} (${node.group}) filtered out - matches filter`);
      } else {
        console.log(`Node ${node.id} (${node.group}) included - doesn't match filters`);
      }
    ...
    console.log(`After filtering: ${result.length} nodes visible (was ${nodesConnectedByDepth.length})`);
    console.log(`Filtered nodes by group:`, 
      result.reduce((acc, node) => {
        acc[node.group] = (acc[node.group] || 0) + 1;
        return acc;
      }, {} as Record<string, number>));
      ...
      category.types.forEach(type => {
        ...
        entry.append("rect")
          ...
          .on("click", function(this: SVGRectElement) {
            ...
            // Always force simulation to reset
            if (simulationRef.current) {
              console.log("Stopping simulation to force complete rebuild");
            ...
            setFilteredRelationships(prev => {
              ...
              console.log(`Legend click: ${wasFiltered ? 'UNFILTERING' : 'FILTERING'} ${type}.`);
              console.log(`New filtered types: ${JSON.stringify(newFilters)}`);
              
              // Set a timeout to completely rebuild the graph
              setTimeout(() => {
                if (svgRef.current) {
                  console.log(`Forcing graph rebuild after ${wasFiltered ? 'unfiltering' : 'filtering'} ${type}...`);
                  // This will help ensure React updates the filteredNodes
                  setIsLoading(true);
                  
                  // Then quickly turn off loading to signal completion
