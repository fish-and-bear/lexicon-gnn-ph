...
const WordGraph2: React.FC<WordGraphProps> = ({
  ...
  useEffect(() => {
    ...
    const simulation = d3.forceSimulation(nodes as any)
      ...
      .force('collision', d3.forceCollide().radius((d: any) => d.isMain ? 40 : 30));
    
    // Only add link force if we have links
    if (links.length > 0) {
      try {
        // Add more debugging
        console.log("Setting up force link with links:", links);
        ...
        console.log("Valid links:", validLinks.length, "of", links.length);
        ...
        if (validLinks.length > 0) {
          ...
          console.warn("No valid links with numeric source/target found");
        }
      } catch (e) {
        console.error("Error setting up force link:", e);
      }
    }
    ...
    simulation.on('tick', () => {
      try {
        // Update link positions - make sure to handle edge cases where source/target may be objects
        link
