...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  useEffect(() => {
    if (wordNetwork) {
      ...
      const links = Array.isArray(wordNetwork.links) ? wordNetwork.links : [];
      // Alternative property name for links
      const edges = Array.isArray(wordNetwork.edges) ? wordNetwork.edges : [];
      
      console.log(`WordGraph received data: ${nodes.length} nodes, ${links.length || edges.length} links/edges`);
      
      setBaseNodes(nodes);
      setBaseLinks(links.length > 0 ? links : edges);
    } else {
      setBaseNodes([]);
      setBaseLinks([]);
    }
