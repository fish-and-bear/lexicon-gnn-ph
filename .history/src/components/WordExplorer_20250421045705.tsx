...
const WordExplorer: React.FC = () => {
  ...
  const handleBack = useCallback(() => {
    if (currentHistoryIndex > 0) {
      ...
      // Determine the identifier to use: prioritize ID
      const identifier = typeof previousWord === 'object' && previousWord.id 
        ? `id:${previousWord.id}`
        : wordText; // Fallback to text/lemma
      ...
      Promise.all([
        fetchWordDetails(identifier),
        fetchWordNetworkData(identifier, depth, breadth)
      ])
  ...
  const handleForward = useCallback(() => {
    if (currentHistoryIndex < wordHistory.length - 1) {
      ...
      // Determine the identifier to use: prioritize ID
      const identifier = typeof nextWord === 'object' && nextWord.id
        ? `id:${nextWord.id}`
        : wordText; // Fallback to text/lemma
