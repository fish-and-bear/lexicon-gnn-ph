...
const WordExplorer: React.FC = () => {
  ...
  const handleSearch = useCallback(async (query: string, event?: FormEvent) => {
    if (event) {
    ...
    try {
      ...
      setSearchError(err instanceof Error ? err.message : 'Failed to fetch suggestions');
      setSearchResults([]);
      setShowSuggestions(false);
      
      // If it's a connection error, show appropriate message
  ...
  const loadWordData = async (id: number) => {
    try {
      setIsLoading(true);
      setError(null);
      
      // Fetch word details
      const data = await fetchWordDetails(id.toString());
      if (!data) {
      ...
      setMainWord(data.lemma);
      setInputValue(data.lemma);
      
      // Add to history
      setWordHistory(prevHistory => [...prevHistory.slice(0, currentHistoryIndex + 1), data.lemma]);
      setCurrentHistoryIndex(prevIndex => prevIndex + 1);
      
      // Fetch network data
      try {
        const networkData = await fetchWordNetworkData(data.lemma, depth, breadth);
        setWordNetwork(networkData);
      } catch (networkError) {
        console.error('Error fetching word network:', networkError);
        // Don't throw here, we still have the word data
      }
      
      // Fetch etymology tree
      try {
        await fetchEtymologyTree(id);
