...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  // Helper function to map API relationship types to color groups
  const mapRelationshipToGroup = (relationship?: string): string => {
    if (!relationship) return 'associated'; // Default if no direct link type
    const relLower = relationship.toLowerCase();
    switch (relLower) {
      case 'main': return 'main'; // Explicitly handle main
      case 'synonym': return 'synonym';
      case 'antonym': return 'antonym';
      case 'related': return 'related';
      case 'kaugnay': return 'related'; // Group with related
      case 'variant': 
      case 'spelling_variant':
      case 'regional_variant': return 'variant';
      case 'derived': 
      case 'derived_from': 
      case 'root_of': return 'derived'; // Group derivation types
      case 'root': return 'root'; // If API explicitly sends root
      case 'hypernym': 
      case 'hyponym': return 'taxonomic'; // New group for hierarchy
      case 'meronym':
      case 'holonym': return 'part_whole'; // New group for part/whole
      case 'etymology': return 'etymology';
      case 'component_of': return 'component_of';
      case 'cognate': return 'cognate';
      case 'see_also':
      case 'compare_with': return 'usage'; // New group for usage links
      default: return 'associated'; // Catch-all for others
    }
  };
  ...
  const getNodeColor = useCallback((group: string): string => {
    switch (group) {
      ...
      case "antonym": return "#f77f00"; // Changed antonym color
      case "variant": return "#f4a261"; 
      case "related": return "#fcbf49"; // Changed related color
      case "taxonomic": return "#8338ec"; // New: Purple
      case "part_whole": return "#3a86ff"; // New: Blue
      case "usage": return "#0ead69"; // New: Green
      case "etymology": return "#3d5a80"; 
      case "component_of": return "#ffb01f"; 
      case "cognate": return "#9381ff"; 
      case "associated": return "#adb5bd"; // Changed associated color (more neutral gray)
      default: return "#6c757d"; // Darker gray default
    }
  }, []);

  // Calculate LINKS first, as nodes depend on them for grouping
  const links: CustomLink[] = useMemo(() => {
    console.log("[WordGraph] Raw wordNetwork.edges:", wordNetwork?.edges);
    ...
    return wordNetwork.edges
      ...
      .map(link => link as unknown as CustomLink);
  }, [wordNetwork]);

  // Calculate NODES using the memoized links for grouping
  const nodes: CustomNode[] = useMemo(() => {
    console.log("[WordGraph] Raw wordNetwork.nodes:", wordNetwork?.nodes);
    if (!wordNetwork || !wordNetwork.nodes || !Array.isArray(wordNetwork.nodes)) {
      return [];
    }

    const memoizedLinks = links; // Use the links calculated above
    ...
    return wordNetwork.nodes.map(node => {
      ...
      console.log(`[Node Creation] Word: ${node.label}, Calculated Group: ${calculatedGroup}`);
  ...
  }, [wordNetwork, links, mainWord]); // Now links is a valid dependency
  ...
  const filteredNodes = useMemo(() => {
    ...
    while (queue.length > 0) {
      ...
      // Simplify logic: link.source and link.target are always strings here
      const relatedWords = links
        .filter(link => 
          link.source === currentWord || link.target === currentWord
        )
        .map(link => {
          // Assert that source/target are strings here, based on how `links` is created
          const otherWord = (link.source === currentWord ? link.target : link.source) as string;
          return otherWord;
        })
        .filter(word => !visited.has(word)); // Now word is correctly inferred as string
  ...
  const setupSimulation = useCallback((nodes: CustomNode[], links: CustomLink[]) => {
    try {
      // --- DEBUG LOGS START ---
      // Log the final nodes and links being used by D3
      console.log("[WordGraph] Nodes passed to simulation (IDs):", JSON.stringify(nodes.map(n => n.id)));
      // Refined link logging
      console.log("[WordGraph] Links passed to simulation (count):", links.length);
      if(links.length > 0 && links.length < 10) { // Log sample if not too many
        console.log("[WordGraph] Links sample passed to simulation:", JSON.stringify(links.map(l => ({s: l.source, t: l.target}))));
      }
      // --- DEBUG LOGS END ---
   ...
   */
  const createLinks = useCallback((g: d3.Selection<SVGGElement, unknown, null, undefined>, links: CustomLink[]) => {
    return g.append("g")
      .attr("class", "links")
      .selectAll("line")
      .data(links)
  ...
  const updateGraph = useCallback(() => {
    ...
    try {
      ...
      // Filter links to only include those connecting nodes within filteredNodes
      ...
      // --- NEW DEBUG LOGS ---
      console.log("[WordGraph] Filtered nodes (IDs):", JSON.stringify(filteredNodes.map(n => n.id)));
      console.log("[WordGraph] Filtered links (count):", filteredLinks.length);
      if(filteredLinks.length > 0 && filteredLinks.length < 10) { // Log sample if not too many
         console.log("[WordGraph] Filtered links sample:", JSON.stringify(filteredLinks.map(l => ({s: l.source, t: l.target}))));
      }
      // --- END NEW DEBUG LOGS ---

      // Set up the simulation with our filtered nodes and links
      const sim = setupSimulation(filteredNodes, filteredLinks);

      // Create links first (so they appear behind nodes)
      const link = createLinks(g, filteredLinks);
