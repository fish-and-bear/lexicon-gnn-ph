...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const memoizedNodes = useMemo<CustomNode[]>(() => {
    ...
    const mappedNodes = rawNodes.map((node: NetworkNode): CustomNode => {
      ...
      const connections = memoizedLinks.filter(l => l.source === node.lemma || l.target === node.lemma).length;
  ...
  const filteredNodes = useMemo<CustomNode[]>(() => {
    ...
    console.log(`Filtered nodes: ${nodesToInclude.size} of ${memoizedNodes.length}`);
    
    // Return only the nodes that were included in our traversal
    return memoizedNodes.filter(node => nodesToInclude.has(node.id));
