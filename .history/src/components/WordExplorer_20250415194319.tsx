...
const WordExplorer: React.FC = () => {
  ...
  const fetchEtymologyTree = useCallback(async (wordId: number): Promise<EtymologyTree | null> => {
    ...
    try {
      ...
      // Check if we have the actual tree data structure we need
      const treeData = data?.etymology_tree || data;
      
      // Create a properly structured EtymologyTree object
      const etymologyTreeData: EtymologyTree = {
        word: typeof treeData?.word === 'string' ? treeData.word : '',
        etymology_tree: treeData,
        complete: Boolean(treeData?.nodes?.length > 0),
        nodes: treeData?.nodes || [],
        edges: treeData?.edges || []
      };
      
      // Check if we actually got meaningful data
      if (etymologyTreeData.nodes && etymologyTreeData.nodes.length > 0) {
        console.log(`Received valid etymology tree with ${etymologyTreeData.nodes.length} nodes`);
        setEtymologyTree(etymologyTreeData);
        return etymologyTreeData;
      } else {
