...
const WordExplorer: React.FC = () => {
  ...
  const handleNodeClick = useCallback(async (identifier: string) => {
    ...
    if (identifier.includes(':')) {
      ...
      lemma = parts[0];
      targetLanguageCode = parts[1].toLowerCase(); // Normalize language code
      console.log(`Node clicked (specific language): Lemma='${lemma}', Lang='${targetLanguageCode}'`);
    } else {
      lemma = identifier;
      console.log(`Node clicked (any language): Lemma='${lemma}'`);
    }
    ...
    try {
      let wordDataResult: WordInfo | null = null;
      let fallbackToSearch = false;

      try {
        // Fetch using only the lemma
        wordDataResult = await fetchWordDetails(lemma);
      } catch (error: any) {
        console.warn(`Initial fetch failed for lemma '${lemma}', error:`, error.message);
        fallbackToSearch = true;
        
        // --- Existing fallback logic (simplified for clarity) ---
        ...
        {
          console.log(`Falling back to search for lemma: ${lemma}`);
          try {
            const searchResults = await searchWords(lemma, {
              page: 1,
              per_page: 10,
              mode: 'exact', // Try exact match first
              sort: 'relevance',
              language: targetLanguageCode || '' // Use target language if available
            });

            if (searchResults.words && searchResults.words.length > 0) {
              console.log(`Fallback search successful, found ${searchResults.words.length} results`);
              const firstResult = searchResults.words[0];
              // Fetch details using the ID from search result
              wordDataResult = await fetchWordDetails(`id:${firstResult.id}`);
              fallbackToSearch = false; // Success, no longer falling back
            } else {
              throw new Error(`Word '${lemma}' not found via fallback search.`);
            }
          } catch(searchError: any) {
             console.error(`Fallback search also failed for lemma '${lemma}':`, searchError);
             // Re-throw the original error if fallback fails
             throw error; 
          }
        } else {
          // If not a 'not found' type error, re-throw it
          throw error;
        }
        // --- End of existing fallback logic ---
      }

      // Proceed only if we successfully fetched data (directly or via fallback)
      if (wordDataResult) {
        console.log(`Word data retrieved for lemma '${lemma}':`, wordDataResult);

        // *** Language Verification Step ***
        if (targetLanguageCode && wordDataResult.language_code.toLowerCase() !== targetLanguageCode) {
          console.warn(`Language mismatch: Requested ${targetLanguageCode}, but API returned ${wordDataResult.language_code}`);
          setError(`Showing details for '${wordDataResult.lemma}' (${wordDataResult.language_code.toUpperCase()}). Could not directly load the requested ${targetLanguageCode.toUpperCase()} version.`);
          // Set the data for the returned language anyway, but show the error
          setWordData(wordDataResult); 
          setSelectedNode(wordDataResult.lemma);
          // Optionally fetch network/etymology for the *returned* word
          fetchWordNetworkData(wordDataResult.lemma, depth, breadth).catch(err => console.error("Error fetching network data after lang mismatch:", err));
          if (wordDataResult.id) fetchEtymologyTree(wordDataResult.id).catch(err => console.error("Error fetching etymology tree after lang mismatch:", err));
          // Update history with the *returned* word
          const wordId = String(wordDataResult.id);
          if (!wordHistory.some(w => typeof w === 'object' && 'id' in w && String(w.id) === wordId)) {
              const newHistory = [...wordHistory.slice(0, currentHistoryIndex + 1), { id: wordDataResult.id, text: wordDataResult.lemma }];
              setWordHistory(newHistory as any);
              setCurrentHistoryIndex(newHistory.length - 1);
          }
          setIsLoading(false); // Stop loading, showing the error is the final state here
          return; // Important: Stop further processing
        }
        // *** End Language Verification ***

        // Language matches or no specific language was requested, proceed as normal
        setWordData(wordDataResult);
        setSelectedNode(wordDataResult.lemma);

        const wordId = String(wordDataResult.id);
        // Update history only if it's a new word or language variant wasn't specifically requested and matched
        if (!wordHistory.some(w => typeof w === 'object' && 'id' in w && String(w.id) === wordId)) {
          const newHistory = [...wordHistory.slice(0, currentHistoryIndex + 1), { id: wordDataResult.id, text: wordDataResult.lemma }];
          setWordHistory(newHistory as any);
          setCurrentHistoryIndex(newHistory.length - 1);
        ...
        // Reset depth/breadth if needed (consider if this logic needs adjustment)
        // const currentNetworkWordId = depth && breadth ? wordDataResult.id : null;
        // if (wordDataResult.id !== currentNetworkWordId) {
        //   setDepth(2);
        //   setBreadth(10);
        // }

        // Fetch related data (Network and Etymology)
        // Use the confirmed lemma/id for these fetches
        const confirmedLemma = wordDataResult.lemma;
        const confirmedId = wordDataResult.id;
        
        try {
          console.log(`Fetching network for confirmed word: ${confirmedLemma}`);
          fetchWordNetworkData(confirmedLemma, depth, breadth)
        ...
        try {
          if (confirmedId) { // Ensure ID exists before fetching
            fetchEtymologyTree(confirmedId)
             ...
             console.warn("No confirmed ID available to fetch etymology tree.");
             setEtymologyTree(null); // Clear any previous tree
          }
        } catch (etymologyError) {
          console.error("Error initiating etymology tree fetch:", etymologyError);
        }
      }
      // If wordDataResult is null after try/catch (e.g., fallback search failed), error is handled below
      ...
      if (error.message.includes('not found')) {
        setError(`Word '${lemma}' was not found in the dictionary or could not be retrieved.`);
        ...
        setError(`Database error when retrieving the word '${lemma}'.`);
      } else {
      ...
      setWordData(null);
      setWordNetwork(null);
      // Optionally clear selected node? 
      ...
      depth, 
      breadth, 
      // Removed setDepth, setBreadth as dependencies unless needed for reset logic
      fetchWordDetails, 
      searchWords, 
  ...
  const handleSearch = useCallback(async (searchTerm: string | WordSuggestion | null) => {
    ...
    if (typeof searchTerm === 'string') {
      ...
      // If it's an object, create an identifier. Adjust format if API expects differently.
      // Example format: "lemma:lang_code" - check if fetchWordDetails handles this.
      identifierToSearch = `${searchTerm.lemma}:${searchTerm.language_code}`;
      displayLemma = searchTerm.lemma; 
      console.log(`Searching with identifier: ${identifierToSearch}`);
    }
    ...
    // Call handleNodeClick with the potentially combined identifier
