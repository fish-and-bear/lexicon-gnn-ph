...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  // Create a simplified and more stable drag behavior
    ...
    return d3.drag<SVGGElement, CustomNode>()
      .on('start', (event, d) => {
        ...
        if (!event.active && simulationObject) {
          simulationObject.alphaTarget(0.2).restart();
        }
      ...
      .on('drag', (event, d) => {
        ...
        // Force the simulation to reheat slightly on each drag
        if (simulationObject) {
          simulationObject.alpha(0.1);
          simulationObject.restart();
        }
      })
      .on('end', (event, d) => {
        // Reset dragging flag
        ...
        // Check for shift key to pin/unpin nodes
        if (event.sourceEvent && event.sourceEvent.shiftKey) {
          // Keep position fixed (pinned)
          d3.select(event.sourceEvent.currentTarget)
            .select('circle')
            .transition()
            .duration(200)
  ...
  // Simplified simulation setup for better stability
  const setupSimulation = useCallback((nodes: CustomNode[], links: CustomLink[]) => {
    ...
    // Create a new simulation with simplified forces
    const sim = d3
      .forceSimulation<CustomNode>(nodes)
      .alphaDecay(0.028)       // Standard decay
      .velocityDecay(0.4)      // Moderate damping
      .force('link', d3.forceLink<CustomNode, CustomLink>(links)
        .id(d => d.id)
        .distance(80)          // Fixed distance for stability
        .strength(0.6))        // Moderate strength
      .force('charge', d3.forceManyBody()
        .strength(-120)        // Fixed charge strength
        .distanceMax(200))     // Limit long-range effects
      ...
      .on('tick', () => {
        ...
        // Simple update of node and link positions
        if (!isDraggingRef.current) {
          ...
          d3.select(svgRef.current)
            ...
            .attr('transform', d => {
              // Bound the positions
              const x = Math.max(-containerWidth/2, Math.min(containerWidth/2, d.x || 0));
              const y = Math.max(-containerHeight/2, Math.min(containerHeight/2, d.y || 0));
    ...
    // Position the main word at the center
  ...
  // Simplified function to create links
  const createLinks = useCallback((g: d3.Selection<SVGGElement, unknown, null, undefined>) => {
    // Only create links between nodes that exist in the filtered set
  ...
  // Simplified function to create nodes
  const createNodes = useCallback((g: d3.Selection<SVGGElement, unknown, null, undefined>, simulation: d3.Simulation<CustomNode, undefined>) => {
    ...
    const node = g
      ...
      .data(filteredNodes)
      .join("g")
      .attr("class", d => `node ${d.id === selectedNodeId ? "selected" : ""} ${d.group}`)
      .attr("id", d => `node-${d.id.replace(/\s+/g, '-')}`)
      .call(dragBehavior);

    // Create circles with simpler styling
    node.append("circle")
      .attr("r", d => (d.group === "main" ? 25 : 15))
    ...
    node.append("text")
      ...
      .style("font-size", d => d.group === "main" ? "12px" : "10px")
  ...
  const setupNodeInteractions = useCallback((node: d3.Selection<SVGGElement, CustomNode, SVGGElement, unknown>, svg: d3.Selection<SVGSVGElement, unknown, null, undefined>) => {
    node
      .on("click", (event, d) => {
        ...
        // Call onNodeClick if this is a new selection
        if (d.id !== mainWord) {
          onNodeClick(d.id);
        }
      })
