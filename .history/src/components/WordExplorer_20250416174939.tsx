...
const WordExplorer: React.FC = () => {
  ...
  // Add apiConnectionChecked ref here, at the top level of component
  const apiConnectionChecked = React.useRef(false);
    ...
    try {
      ...
      if (data && data.nodes && data.edges) {
        ...
        if (wordData && wordData.id) {
          // Manually add the semantic network to the wordData
          console.log('Setting semantic network data on wordInfo');
          // Set the semantic network data on the wordData object
          ...
          // Update the state with the enhanced wordData
          setWordData(updatedWordData);
          
          // If selectedWordInfo exists and is the same word, update it too
          if (selectedWordInfo && selectedWordInfo.id === wordData.id) {
            console.log('Synchronizing network data with selectedWordInfo');
            const updatedSelectedWordInfo = {
              ...selectedWordInfo,
              semantic_network: {
                nodes: data.nodes,
                links: data.edges
            ...
            setSelectedWordInfo(updatedSelectedWordInfo);
          }
          
          // Find matching node in network for current word
          const mainNode = data.nodes.find(node => {
            return (
              // Match by various criteria to ensure we find the correct node
              node.id === wordData.id ||
              (typeof node.word === 'string' && node.word === wordData.lemma) ||
              (typeof node.label === 'string' && node.label === wordData.lemma)
            );
          });
          
          if (mainNode) {
            console.log('Found main node in network:', mainNode);
            setMainWord(mainNode.label || mainNode.word || wordData.lemma);
          } else {
            console.log('No main node found in network, using wordData.lemma');
            setMainWord(wordData.lemma);
          }
        }
      } else {
        console.error('Invalid network data received:', data);
        setError('Invalid network data received from the server');
      }
    } catch (error) {
      console.error('Error fetching word network:', error);
      setError(error instanceof Error ? error.message : 'Error fetching word network');
    } finally {
      setIsLoading(false);
    }
  }, [wordData, selectedWordInfo]);
  ...
  // Function to load complete word data (details, network, etymology)
  const loadWordData = useCallback(async (wordIdentifier: string | number) => {
    ...
    setWordNetwork(null);
    setEtymologyTree(null);
    console.log(`[DEBUG] loadWordData called with identifier: ${wordIdentifier}`);

    let wordToLoad = wordIdentifier;
    let wordLemmaForNetwork = "";
    let wordIdForEtymology = 0;

    try {
      // Ensure wordToLoad is a string for fetchWordDetails
      if (typeof wordToLoad === 'number') {
        wordToLoad = `id:${wordToLoad}`;
      }

      console.log(`[DEBUG] Fetching details using identifier: ${wordToLoad}`);
      const details = await fetchWordDetails(wordToLoad);
      console.log('[DEBUG] Fetched word details:', details);
      setSelectedWordInfo(details);
      setMainWord(details.lemma);
      setInputValue(details.lemma);
      
      // Extract lemma and ID for subsequent fetches
      wordLemmaForNetwork = details.lemma;
      wordIdForEtymology = details.id;

      // Update history only if the loaded word is different from the last entry
      const lastHistoryEntry = wordHistory[currentHistoryIndex];
      const lastLemma = typeof lastHistoryEntry === 'string' ? lastHistoryEntry : lastHistoryEntry?.text;
      
      if (lastLemma !== details.lemma) {
        const newHistory = [...wordHistory.slice(0, currentHistoryIndex + 1), { id: details.id, text: details.lemma }];
        setWordHistory(newHistory);
        setCurrentHistoryIndex(newHistory.length - 1);
        console.log("[DEBUG] Updated history with:", { id: details.id, text: details.lemma });
      }

      // Scroll details container to top
      detailsContainerRef.current?.scrollTo(0, 0);

      // Fetch network and etymology in parallel AFTER details are loaded
      try {
        console.log(`[DEBUG] Fetching network for lemma: ${wordLemmaForNetwork}`);
        // Don't await here - let it run in background
        fetchWordNetworkData(wordLemmaForNetwork, depth, breadth);
      } catch (networkError) {
        console.error('[DEBUG] Error fetching network (will continue):', networkError);
        // Don't block UI if network fails
      }

      try {
        console.log(`[DEBUG] Fetching etymology tree for ID: ${wordIdForEtymology}`);
        // Don't await here - let it run in background
        fetchEtymologyTree(wordIdForEtymology);
      } catch (etymologyError) {
        console.error('[DEBUG] Error fetching etymology tree (will continue):', etymologyError);
        // Don't block UI if etymology fails
      }

    } catch (error: any) {
      console.error(`[DEBUG] Error in loadWordData for ${wordIdentifier}:`, error);
      setError(error.message || "Failed to load word data");
    } finally {
      setIsLoading(false);
    }
  }, [depth, breadth, fetchWordDetails, fetchWordNetworkData, fetchEtymologyTree, wordHistory, currentHistoryIndex]); // Added dependencies

  // Callback when a word link is clicked in WordDetails
  const handleWordLinkClick = useCallback((word: string) => {
    console.log("[DEBUG] handleWordLinkClick:", word);
    loadWordData(word);
  }, [loadWordData]);

  // Callback when an etymology node is clicked
  const handleEtymologyNodeClick = useCallback((nodeId: number) => {
    console.log("[DEBUG] handleEtymologyNodeClick:", nodeId);
    loadWordData(nodeId); // Load by ID
  }, [loadWordData]);

  // Callback when a search result is clicked
  const handleSearchResultClick = useCallback((result: SearchWordResult) => {
    console.log("[DEBUG] handleSearchResultClick:", result.lemma, result.id);
    setShowSuggestions(false); // Hide suggestions pane
    loadWordData(result.id); // Load full data using the ID
  }, [loadWordData]);

  // Function to fetch suggestions using searchWords API
  const fetchSuggestions = useCallback(async (query: string) => {
    ...
    const searchOptions: SearchOptions = {
      per_page: 10,
      mode: 'prefix', // Use prefix mode for suggestions
    ...
    try {
      ...
      if (result.error) {
        console.error(`[DEBUG] Suggestions search returned an error: ${result.error}`);
        setSearchError(result.error);
        setSearchResults([]);
        setShowSuggestions(false);
      } else if (result && result.words && result.words.length > 0) {
        console.log(`[DEBUG] Found ${result.words.length} suggestions.`);
      ...
      let errorMessage = "Error fetching suggestions";
      if (error instanceof Error) {
        errorMessage = error.message;
      }
  ...
  }, []); // Dependencies might be needed if options change dynamically

  // Debounced version of fetchSuggestions
  const debouncedFetchSuggestions = useCallback(
    debounce(fetchSuggestions, 300), // 300ms debounce
    [fetchSuggestions]
  );
  ...
  const handleInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setSearchError(null); // Clear previous search errors on new input

    if (newQuery.trim().length >= 2) {
      debouncedFetchSuggestions(newQuery.trim());
    } else {
      setSearchResults([]);
      setShowSuggestions(false);
  ...
  // Function to handle form submission (Enter key)
  const handleSearchSubmit = useCallback((event?: React.FormEvent<HTMLFormElement>) => {
    event?.preventDefault(); // Prevent default form submission
    const query = inputValue.trim();
    console.log('[DEBUG] Enter key pressed for search:', `"${query}"`);
    ...
    debouncedFetchSuggestions.cancel(); // Cancel pending suggestions fetch
    loadWordData(query); // Use loadWordData for main search submit

  }, [inputValue, loadWordData, debouncedFetchSuggestions]);

  // Create a ref to hold the actual handleNodeClick function
  const handleNodeClickRef = useRef<(node: any) => void>(() => {});
  ...
  const fetchRandomWordsForCache = useCallback(async (count: number = RANDOM_CACHE_SIZE): Promise<any[]> => {
    ...
    try {
      ...
      const promises = Array.from({ length: count * 2 }, () => // Double the count for better availability
      ...
      if (validResults.length === 0) {
        ...
        // Schedule another attempt after a longer delay (5 seconds)
        setTimeout(() => fetchRandomWordsForCache(count), 5000);
        return randomWordCache;
      } else {
        newCache.push(...validResults);
        // Combine, shuffle, and update the cache
        const combinedCache = [...randomWordCache, ...newCache];
        const shuffledCache = shuffleArray(combinedCache);
        const finalCache = shuffledCache.slice(0, RANDOM_CACHE_SIZE * 4); // Keep 4x the normal size
        
        // Update both state and ref
        setRandomWordCache(finalCache);
        randomWordCacheRef.current = finalCache; 
        
        console.log(`Shuffled and added ${validResults.length} words to random word cache (total: ${finalCache.length})`);
        return finalCache;
      }
    } catch (error) {
  ...
  const handleRandomWord = useCallback(async () => {
    ...
    try {
      ...
      if (currentCache.length > 0) {
        ...
        // Refill the cache in the background if the REF cache gets low
        if (randomWordCacheRef.current.length < RANDOM_CACHE_SIZE * 2) {
          fetchRandomWordsForCache(RANDOM_CACHE_SIZE * 2).then(cachedWords => {
        ...
        // Cache REF is empty - fetch directly
        console.log("Random word cache (ref) empty, fetching directly");
        const words = await fetchRandomWordsForCache(RANDOM_CACHE_SIZE * 2);
        
        if (words.length === 0) {
          throw new Error("Failed to fetch random words");
        }
        
        // Ensure we have at least one word before assigning
        if (words.length > 0) {
          randomWord = words[0];
          // Update the REF and state with the remaining words
          const remainingWords = words.slice(1);
          randomWordCacheRef.current = remainingWords;
          setRandomWordCache(remainingWords); 
        } else {
          throw new Error("Fetched words but array is empty, cannot select random word.");
        }
      }
      ...
      }, 50); // Reduced from 500ms to 50ms for much faster response
    }
  }, [
    depth, 
    breadth, 
  ...
  const handleTestApiConnection = useCallback(async () => {
        setError(null);
    setApiConnected(false); // Change null to false for checking state
  ...
  useEffect(() => {
    ...
    if (apiConnectionChecked.current) {
      console.log("API connection already checked, skipping");
      return;
    }
    
    apiConnectionChecked.current = true;
      ...
      if (connected) {
        ...
        const fetchInitialData = async () => {
          try {
            ...
            if (randomWordCacheRef.current.length === 0) {
              ...
              console.log("Random word cache already has", randomWordCacheRef.current.length, "words, skipping prefetch");
            }
          } catch (error) {
  ...
  // Update selectedWordInfo with relations from wordData when it changes
  useEffect(() => {
    if (wordData && selectedWordInfo && wordData.id === selectedWordInfo.id) {
      // Only update if we have relations in wordData that aren't in selectedWordInfo
      const hasNewRelations = 
        ((wordData.incoming_relations?.length ?? 0) > 0 && (!selectedWordInfo.incoming_relations || selectedWordInfo.incoming_relations.length === 0)) ||
        ((wordData.outgoing_relations?.length ?? 0) > 0 && (!selectedWordInfo.outgoing_relations || selectedWordInfo.outgoing_relations.length === 0));
      
      if (hasNewRelations) {
        console.log("Syncing relations from wordData to selectedWordInfo:", {
          incoming: wordData.incoming_relations?.length ?? 0,
          outgoing: wordData.outgoing_relations?.length ?? 0
        });
        
        setSelectedWordInfo({
          ...selectedWordInfo,
          incoming_relations: wordData.incoming_relations || selectedWordInfo.incoming_relations || [],
          outgoing_relations: wordData.outgoing_relations || selectedWordInfo.outgoing_relations || []
        });
      }
    }
  }, [wordData, selectedWordInfo]);
