...
// Add the import for the new utility function
import { extractRelationsFromNetwork } from '../utils/wordUtils';
...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({
  ...
  const renderRelationsTab = () => {
    ...
    // Enhanced fallback logic: Use semantic network in more cases
    // 1. When relations are empty
    // 2. When there's a server_error
    // 3. When we have semantic_network data but no or few relations
    const hasError = typeof wordInfo.server_error === 'string';
    const hasMinimalRelations = incoming_relations.length + outgoing_relations.length < 3;
    const hasSemanticNetwork = semanticNetworkNodes.length > 0 && semanticNetworkLinks.length > 0;
    
    // More aggressive fallback strategy
    const useSemanticNetwork = 
      hasSemanticNetwork && (hasError || hasMinimalRelations || incoming_relations.length === 0);
    
    // Log the fallback status for debugging
    if (useSemanticNetwork) {
      console.log("Using semantic network fallback for relations:", {
        reason: hasError ? "server_error" : (hasMinimalRelations ? "minimal_relations" : "empty_relations"),
        error: hasError ? wordInfo.server_error : null,
        relationCount: {
    ...
    // Create relations from semantic network if needed - use the utility function
    const semanticRelations = useSemanticNetwork ? extractRelationsFromNetwork(wordInfo) : [];
            ...
            {hasSemanticNetwork && (
              <Box mt={2}>
                <Button 
                  ...
                  size="small"
                  onClick={() => {
                    // Force using semantic network on button click
                    const forcedRelations = extractRelationsFromNetwork(wordInfo);
                    if (forcedRelations.length > 0) {
                      // This is a hack - we'd ideally update the state, but for now
                      // let's just force a re-render by setting a timeout
                      setTimeout(() => {
                        console.log("Forcing relations display from semantic network");
    ...
    const relationsWithDegree = allRelations.map((rel: any) => ({
      ...rel, 
    ...
    const filteredRelations = relationsWithDegree.filter((rel: any) => {
    ...
    filteredRelations.forEach((relation: any) => {
