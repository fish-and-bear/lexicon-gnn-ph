...
const WordExplorer: React.FC = () => {
  ...
  useEffect(() => {
    ...
    let cacheTimer: NodeJS.Timeout | null = null; // Timer for cache fetch
    ...
    const connectionCheckTimer = setTimeout(() => {
      testApiConnection().then(connected => {
        ...
        if (connected) {
          // fetchInitialData can now safely call fetchStatistics etc.
          const fetchInitialData = async () => {
            try {
              ...
              // Call fetchStatistics (now guaranteed to be defined)
              fetchStatistics(); 
    ...
    return () => {
      ...
      if (cacheTimer) {
        clearTimeout(cacheTimer); // Clear the cache timer if it was set
      }
    };
  // Correct dependencies: Include stable functions and state values read directly.
