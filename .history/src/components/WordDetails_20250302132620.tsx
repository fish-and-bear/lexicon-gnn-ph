...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({ wordInfo, onWordClick }) => {
  ...
  const renderEtymology = (word: WordInfo) => {
    ...
    // Function to parse components in [Language] [component] format
    const parseComponent = (component: string) => {
      // Check if component follows [Language] [component] pattern
      const languageComponentMatch = component.match(/^([A-Z][a-z]+(?:\s[A-Z][a-z]+)*)\s+(.+)$/);
      
      if (languageComponentMatch) {
        return {
          language: languageComponentMatch[1],
          word: languageComponentMatch[2],
          original: component
        };
      }
      
      // Handle components with "inherited from" and remove that text
      const inheritedMatch = component.match(/inherited\s+from\s+(.+)/i);
      if (inheritedMatch) {
        return {
          language: '',
          word: inheritedMatch[1],
          original: component
        };
      }
      
      // Handle "English word" format
      if (component.toLowerCase().startsWith('english ')) {
        return {
          language: 'English',
          word: component.substring(8),
          original: component
        };
      }
    ...
    const getLanguageClass = (language: string): string => {
      const languageMap: Record<string, string> = {
        ...
        'English': 'language-germanic',
        'Old English': 'language-germanic',
        'Middle English': 'language-germanic',
        'Old Norse': 'language-germanic',
        'Old High German': 'language-germanic',
        'French': 'language-romance',
        'Spanish': 'language-romance',
        'Italian': 'language-romance',
        'Portuguese': 'language-romance',
        'Romanian': 'language-romance',
        'Russian': 'language-slavic',
        'Polish': 'language-slavic',
        'Czech': 'language-slavic',
        'Bulgarian': 'language-slavic',
        'Sanskrit': 'language-sanskrit',
        'Hindi': 'language-sanskrit',
        'Bengali': 'language-sanskrit',
        'Urdu': 'language-sanskrit'
      };
    ...
    const renderOrganizedComponents = () => {
      ...
      // Get languages with at least 2 components for better organization
      const significantLanguages = Object.keys(componentsByLanguage)
        .filter(lang => componentsByLanguage[lang].length >= 2 && lang !== 'Other')
        .sort((a, b) => componentsByLanguage[b].length - componentsByLanguage[a].length);
        ...
        <div className="component-organization">
                      ...
                      <div className="evolution-branch" key={idx}>
                        <div className="evolution-node">
                          <div 
                            ...
                            onClick={() => onWordClick(component)}
                          >
                            <span className="evolution-node-title">
                              {parsed.language ? (
                                <>
                                  <strong>{parsed.language}</strong> {parsed.word}
                                </>
          ...
          {significantLanguages.length > 0 && (
            <div className="component-cards">
              {significantLanguages.map(language => (
                <div key={language} className={`component-card ${getLanguageClass(language)}`}>
                  <div className="component-card-header">
                    ...
                    <h5 className="component-card-title">{language} Components</h5>
                    <span className="component-card-count">{componentsByLanguage[language].length}</span>
                  </div>
                  <div className="component-card-content">
                    {componentsByLanguage[language].map((component, idx) => {
                      const parsed = parseComponent(component);
                      return (
                        <span 
                          key={idx} 
