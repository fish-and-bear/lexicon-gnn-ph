...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  useEffect(() => {
    ...
    const legendContainer = svg.append("g")
      .attr("class", "legend")
      .attr("transform", `translate(${width - 280}, 20)`); // Increased right margin from 220 to 280

    // Define refined legend properties with smaller values for width efficiency
    const legendPadding = 12; // Reduced from 16
    const legendItemHeight = 22; // Reduced from 24
    const dotRadius = 4; // Reduced from 5.5
    const textPadding = 14; // Reduced from 20
    const categorySpacing = 8; // Reduced from 10
    const maxLabelWidth = 100; // Reduced from 120

    // Organize relation types by category in two columns for better space efficiency
    const leftColumn = [
    ...
    // Pre-measure text to determine legend width with more efficient sizing
    const tempText = svg.append("text")
      .attr("font-size", "11px")
      .attr("font-weight", "500")
      .style("opacity", 0);

    // Determine maximum text width for a single column
    let maxTextWidth = 0;
    ...
    // Measure all category headers and labels
    [...leftColumn, ...rightColumn].forEach(category => {
    ...
    // Calculate column width with more efficient spacing
    const columnWidth = Math.max(
      maxCategoryWidth, 
      maxTextWidth + textPadding + 12 // Reduced from 20
    ) + (legendPadding);
    
    // Calculate legend width based on two columns plus center gap
    const legendWidth = (columnWidth * 2) + legendPadding + 8; // Added small gap between columns
    
    // Ensure toggle fits or adjust if needed
    const toggleMinWidth = toggleTextWidth + 44 + legendPadding;
    const finalLegendWidth = Math.max(legendWidth, toggleMinWidth);

    // Find max rows in either column
    let leftRows = leftColumn.reduce((total, cat) => total + 1 + cat.types.length, 0);
    let rightRows = rightColumn.reduce((total, cat) => total + 1 + cat.types.length, 0);
    let maxRows = Math.max(leftRows, rightRows);

    // Calculate more compact legend height
    const legendHeight = (maxRows * legendItemHeight) + 
                        (Math.min(leftColumn.length, rightColumn.length) * categorySpacing) + 
                        (legendPadding * 2) + 
                        40 + // Title area
                        40; // Toggle switch area

    // Add legend background rectangle with optimized dimensions
    legendContainer.append("rect")
      .attr("width", finalLegendWidth)
      .attr("height", legendHeight)
      .attr("rx", 8) // Reduced from 10
      .attr("ry", 8) // Reduced from 10
    ...
    legendContainer.append("text")
      .attr("x", finalLegendWidth / 2)
      .attr("y", legendPadding + 6) // Reduced from 7
      .attr("text-anchor", "middle")
    ...
    // Add subtitle with more compact styling
    legendContainer.append("text")
      .attr("x", finalLegendWidth / 2)
      .attr("y", legendPadding + 20) // Reduced from 22
      .attr("text-anchor", "middle")
    ...
    legendContainer.append("line")
      ...
      .attr("y1", legendPadding + 28) // Reduced from 30
      .attr("x2", finalLegendWidth - legendPadding)
      .attr("y2", legendPadding + 28) // Reduced from 30
      .attr("stroke", theme === "dark" ? "rgba(255, 255, 255, 0.1)" : "rgba(0, 0, 0, 0.06)")
      .attr("stroke-width", 1);

    // Track current y position for legend items
    let leftYPos = legendPadding + 34; // Reduced from 38
    let rightYPos = legendPadding + 34; // Same starting point

    // Render left column
    leftColumn.forEach((category, categoryIndex) => {
      // Add category header
      leftYPos += legendItemHeight;
      
      // Add category name with more compact styling
      ...
      if (categoryTextBBox) {
        ...
        legendContainer.append("rect")
          .attr("x", legendPadding - 3) // Reduced from 4
          .attr("y", leftYPos - categoryTextBBox.height + 2)
          .attr("width", categoryTextBBox.width + 6) // Reduced from 8
          .attr("height", categoryTextBBox.height + 3) // Reduced from 4
          .attr("rx", 3)
      ...
      category.types.forEach(type => {
        ...
        const entry = legendContainer.append("g")
          .attr("transform", `translate(${legendPadding}, ${leftYPos})`)
        ...
        // Add color dot with more compact styling
        entry.append("circle")
          .attr("cx", 4) // Reduced from 5
          .attr("cy", 0)
          .attr("r", dotRadius)
        ...
        // Get label text with more efficient truncation
        const labelText = getRelationshipTypeLabel(type).label;
        const availableWidth = columnWidth - textPadding - 8; // Reduced space allocation
        ...
        if (textWidth > availableWidth) {
          ...
          const visibleChars = Math.floor(approxCharsPerWidth * availableWidth) - 2; // Reduced buffer
      ...
      if (categoryIndex < leftColumn.length - 1) {
        leftYPos += categorySpacing;
      }
    });

    // Render right column
    rightColumn.forEach((category, categoryIndex) => {
      rightYPos += legendItemHeight;
      
      // Add category name
      const categoryTextElement = legendContainer.append("text")
        .attr("x", legendPadding + columnWidth + 8) // Added column gap
        .attr("y", rightYPos)
        .attr("font-weight", "600")
        .attr("font-size", "10px") // Reduced from 11px
        .attr("fill", theme === "dark" ? "#ccc" : "#555")
      ...
      if (categoryTextBBox) {
        // Add subtle background for category headers
        legendContainer.append("rect")
          .attr("x", legendPadding + columnWidth + 5) // Adjusted for column gap
          .attr("y", rightYPos - categoryTextBBox.height + 2)
          .attr("width", categoryTextBBox.width + 6) // Reduced from 8
          .attr("height", categoryTextBBox.height + 3) // Reduced from 4
          .attr("rx", 3)
          .attr("fill", theme === "dark" ? "rgba(255, 255, 255, 0.07)" : "rgba(0, 0, 0, 0.04)")
      ...
      category.types.forEach(type => {
        ...
        // Create legend entry group with hover interaction
        const entry = legendContainer.append("g")
          .attr("transform", `translate(${legendPadding + columnWidth + 8}, ${rightYPos})`) // Added column gap
          .attr("class", "legend-item")
          .attr("data-type", type);
      ...
      // Add spacing after each category
      if (categoryIndex < rightColumn.length - 1) {
        rightYPos += categorySpacing;
      }
    });

    // Determine final y position (maximum of left and right columns)
    const finalYPos = Math.max(leftYPos, rightYPos) + legendItemHeight + 10;

    // Add a subtle divider line before the checkbox
    legendContainer.append("line")
      .attr("x1", legendPadding)
      .attr("y1", finalYPos - 12) // Adjusted position
      .attr("x2", finalLegendWidth - legendPadding)
      .attr("y2", finalYPos - 12) // Adjusted position
      .attr("stroke", theme === "dark" ? "rgba(255, 255, 255, 0.1)" : "rgba(0, 0, 0, 0.06)")
      .attr("stroke-width", 1);
    
    // Add a container for the toggle switch
    const toggleContainer = legendContainer.append("g")
      .attr("transform", `translate(${legendPadding}, ${finalYPos})`)
    ...
    const toggleTrack = toggleContainer.append("rect")
      .attr("width", 30) // Reduced from 34
      .attr("height", 14) // Reduced from 16
      .attr("rx", 7) // Reduced from 8
      .attr("x", 0)
      .attr("y", -5) // Reduced from -6
    ...
    // Add toggle handle with more compact styling
    const toggleHandle = toggleContainer.append("circle")
      .attr("r", 5) // Reduced from 6
    ...
    // Add more compact label
    toggleContainer.append("text")
      .attr("x", 38) // Reduced from 42
      .attr("y", 2)
    ...
    toggleContainer.on("click", function() {
      ...
      toggleHandle
        .transition()
        .duration(250)
        .attr("cx", newValue ? 21 : 9); // Adjusted positions
    });
