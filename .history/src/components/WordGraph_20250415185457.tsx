...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const handleResetZoom = useCallback(() => {
    if (zoomRef.current && svgRef.current) {
      ...
      svgNode.transition().duration(750).call(
        zoomRef.current.transform,
        d3.zoomIdentity.translate(0, 0).scale(1)
      );
      
      // Center on the main word after resetting zoom
      setTimeout(() => {
        if (svgRef.current) centerOnMainWord(svgNode, filteredNodes);
      }, 800);
    }
  ...
  const centerOnMainWord = useCallback((svg: d3.Selection<SVGSVGElement, unknown, null, undefined>, nodesToSearch: CustomNode[]) => {
    ...
    if (mainNodeData && mainNodeData.x !== undefined && mainNodeData.y !== undefined) {
      ...
      svg.transition().duration(750).ease(d3.easeCubicInOut)
        ...
        // Use a simple reset instead of calling handleResetZoom to avoid circular reference
        if (zoomRef.current) {
  ...
  // Now define handleResetZoom after centerOnMainWord
  const handleResetZoom = useCallback(() => {
    if (zoomRef.current && svgRef.current) {
      const svgNode = d3.select(svgRef.current);
      const containerRect = svgNode.node()?.parentElement?.getBoundingClientRect();
      const width = containerRect ? containerRect.width : 800;
      const height = containerRect ? containerRect.height : 600;
      
      // First reset zoom to center of view
      svgNode.transition().duration(750).call(
        zoomRef.current.transform,
        d3.zoomIdentity.translate(width/2, height/2).scale(1)
      );
      
      // Then try to center on main word
      setTimeout(() => {
        if (svgRef.current) centerOnMainWord(svgNode, filteredNodes);
      }, 800);
    }
