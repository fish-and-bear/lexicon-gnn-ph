          ...
          <Box key={posName} sx={{ mb: theme.spacing(3) }}>
            ...
            {Object.entries(defsBySource).map(([sourceName, defs]) => (
              <Box key={sourceName} sx={{ mb: theme.spacing(2) }}> {/* Add margin between source groups */}
                ...
                {/* Display Source only once after all definitions for this source, left-aligned */}
                {sourceName !== 'Unknown Source' && (
                   <Box sx={{ 
                       pl: 1.5, // Indent the source 
                       mt: 0.5, 
                       mb: 1.5 // Add margin below the source before the next group
                     }}>
                    <Chip
