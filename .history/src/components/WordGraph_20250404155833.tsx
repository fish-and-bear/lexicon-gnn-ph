...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  useEffect(() => {
    ...
    const link = g.append("g")
      ...
      .attr("stroke-width", d => Math.sqrt(d.value || 1)); // Example: Use value if links have it
    ...
    const dragstarted = (event: d3.D3DragEvent<SVGCircleElement, CustomNode, any>, d: CustomNode) => {
      ...
      d.fx = d.x;
      d.fy = d.y;
      // Make dragged node more prominent
    ...
    const dragended = (event: d3.D3DragEvent<SVGCircleElement, CustomNode, any>, d: CustomNode) => {
      ...
      d3.select(event.sourceEvent.target).attr("stroke", d.pinned ? "#ff0000" : (theme === 'dark' ? "#fff" : "#333"));
