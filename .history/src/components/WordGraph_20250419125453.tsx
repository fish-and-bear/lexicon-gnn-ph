...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const setupNodeInteractions = useCallback((
      ...
      // Remove current click listeners
      nodeSelection.on("dblclick", null).on("click", null);

      // Add unified click handler with proper delay management
      nodeSelection.on("click", (event, d) => {
          event.stopPropagation();
          if (isDraggingRef.current) return;
          
          // If we already have a click timer, this is a double click
          if (clickTimeoutRef.current) {
              ...
              if (onNodeClick) {
                // Make this the main word (navigation action)
                onNodeClick(d.word);
              }
              return;
          }
          
          // This is the first click - schedule a single click action
          clickTimeoutRef.current = setTimeout(() => {
              // This will only run if a second click didn't happen
              clickTimeoutRef.current = null;
  ...
  }, [selectedNodeId, onNodeClick, getNodeRadius, getNodeColor, theme, nodeMap, onNodeSelect]);
  
  // Add cleanup for the click timeout
  useEffect(() => {
    return () => {
      if (clickTimeoutRef.current) {
        clearTimeout(clickTimeoutRef.current);
