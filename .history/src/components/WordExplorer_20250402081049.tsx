...
const WordExplorer: React.FC = () => {
  ...
  const handleDetailPaneResize = () => {
    // Read the size from the state, which was updated by onChange
    // Ensure size stays within bounds (may be slightly redundant if onChange already handles it, but safe)
    const clampedSize = Math.max(300, Math.min(800, detailPaneSize)); 
    // setDetailPaneSize(clampedSize); // No need to set state again, onChange did it.
    localStorage.setItem('wordDetailsWidth', clampedSize.toString());
    console.log('Drag finished, saved size:', clampedSize);
  };
  ...
  return (
    <div className={`container ${theme}`}>
      ...
      <main className="main-content">
        {/* Temporarily ignore TS error to test runtime functionality */}
        {/* @ts-ignore */}
        <SplitPane
        ...
        >
          <div className="graph-container" style={{ height: '100%', width: '100%', overflow: 'hidden' }}>
          ...
          <div className="details-container" style={{ height: '100%', width: '100%', overflow: 'hidden' }}>
