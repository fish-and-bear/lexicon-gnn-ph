...
const WordExplorer: React.FC = () => {
  ...
  useEffect(() => {
    // This keeps track of the latest search term to avoid race conditions
    const currentSearchTerm = inputValue;
    
    if (searchResults.length > 0 && !showSuggestions) {
      // Select the first result automatically
      handleNodeClick(searchResults[0].id.toString());
    }
  }, [searchResults, showSuggestions, handleNodeClick]);
