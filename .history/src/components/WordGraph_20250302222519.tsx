...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const setupSimulation = useCallback((width: number, height: number) => {
    ...
    const sim = d3
      ...
      // Charge force pushes nodes apart - use type assertion to fix the error
      .force("charge", d3.forceManyBody()
        .strength((d) => (d as CustomNode).group === 'main' ? -400 : -250)
        .distanceMax(350)
      )
  ...
  const updateGraph = useCallback(() => {
    ...
    try {
      ...
      simulation.current = sim; // Store simulation in ref
