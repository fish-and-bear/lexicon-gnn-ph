...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const filteredNodes = useMemo<CustomNode[]>(() => {
    ...
    while (queue.length > 0) {
      ...
      const connectedLinks = baseLinks.filter(link => 
        link.source === currentId || link.target === currentId
      );
      
      // If we have more connections than our breadth allows, we need to prioritize
      if (connectedLinks.length > breadth) {
        // Sort links by type priority
      ...
      // Limited number of connections to process based on breadth parameter
      const linksToProcess = connectedLinks.slice(0, breadth);
    ...
    // Return only the nodes that were included in our traversal
    return baseNodes.filter(node => nodesToInclude.has(node.id));
