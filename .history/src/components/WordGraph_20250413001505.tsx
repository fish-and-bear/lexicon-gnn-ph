...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  useEffect(() => {
    ...
    // Create a more compact single-column legend with auto-sizing
    ...
    const dotRadius = 5; // Slightly larger dots for better visibility
    const textPadding = 18; // Padding between dot and text
    ...
    const legendWidth = Math.max(maxCategoryWidth, maxTextWidth + textPadding + 10) + (legendPadding * 2);
    ...
    // Add small spacing between categories (5px per category except the last one)
    const categorySpacing = 5;
    const legendHeight = (totalRows * legendItemHeight) + ((categories.length - 1) * categorySpacing) + (legendPadding * 2);

    // Add legend background rectangle with refined styling
    ...
    // Add title with refined styling
    ...
    categories.forEach((category, categoryIndex) => {
      ...
      category.types.forEach(type => {
        ...
        // Add color dot with subtle styling
        entry.append("circle")
          .attr("cx", 5)
          .attr("cy", 0)
          .attr("r", dotRadius)
      ...
      // Add small spacing after each category (except the last one)
