...
const WordExplorer: React.FC = () => {
  ...
  const handleSearch = useCallback(async (wordToSearchParam?: string) => {
    ...
    try {
      // Perform the search with full word data for the single result
      const searchOptions: SearchOptions = { q: normalizedInput, limit: 1, include_full: true };
      ...
      // If we have at least one word/result, process it
      if ((searchResponse.words && searchResponse.words.length > 0) || 
          (searchResponse.results && searchResponse.results.length > 0)) {
        
        // Use words array if available, otherwise try to get from results
        let matchedWord: any = searchResponse.words?.[0];
        
        // If no words but have results with include_full=true, the results should contain full word data
        if (!matchedWord && searchResponse.results?.[0]) {
          matchedWord = searchResponse.results[0];
        }
        
        // If we found a word, process it
        if (matchedWord) {
          console.log('[handleSearch] Found word match:', matchedWord);
          
          // If the result has enough data, use it directly
          if (matchedWord.definitions?.length > 0 || matchedWord.etymologies?.length > 0) {
            setSelectedWordInfo(matchedWord as WordInfo);
            setMainWord(matchedWord.lemma);
            
            // Fetch additional data in the background if needed
            if (!matchedWord.outgoing_relations || !matchedWord.incoming_relations) {
              try {
                // Fetch complete word data in the background
                const fullWordData = await fetchWordDetails(matchedWord.id.toString());
                setSelectedWordInfo(fullWordData);
              } catch (detailsError) {
                console.error('[handleSearch] Error fetching full word details:', detailsError);
                // Keep using the partial data we already have
              }
            }
            
            // Fetch word network asynchronously
            fetchWordNetworkData(matchedWord.lemma, depth);
          } 
          // If the result doesn't have enough data, fetch full details
          else {
            console.log('[handleSearch] Found basic match, fetching full details');
            try {
              const wordDetails = await fetchWordDetails(matchedWord.id.toString());
              setSelectedWordInfo(wordDetails);
              setMainWord(wordDetails.lemma);
              
              // Fetch word network asynchronously
              fetchWordNetworkData(wordDetails.lemma, depth);
            } catch (detailsError) {
              console.error('[handleSearch] Error fetching word details:', detailsError);
              setError(`Could not retrieve details for "${matchedWord.lemma}"`);
            }
          }
        } 
        // If no complete word in the response but we have a count > 0, try a different approach
        else if (searchResponse.count && searchResponse.count > 0) {
          console.log('[handleSearch] Search found matches but returned no full data. Trying fallback...');
          
          // Try fetching by lemma directly
          try {
            ...
            console.error('[handleSearch] Fallback lookup failed:', detailsError);
            setError(`Found matches for "${normalizedInput}" but could not retrieve details.`);
          }
        }
        // No results
        else {
  ...
  }, [inputValue, fetchWordNetworkData, depth]);
