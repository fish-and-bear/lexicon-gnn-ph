...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const getNodeRadius = useCallback((node: CustomNode) => {
    ...
    let sizeBonus = 0;
    if (node.id === mainWord) {
        sizeBonus = 10; // Largest bonus for main word
    } else if (node.group === 'root') {
        sizeBonus = 6;
    } else if (node.connections && node.connections > 0) {
        // Logarithmic scale for connections - prominent hubs without excessive size
        sizeBonus = Math.log2(node.connections + 1) * 2.5; // Adjust multiplier as needed
    }
    // Ensure a minimum radius and cap maximum bonus
    return Math.max(baseRadius, baseRadius + Math.min(sizeBonus, 15)); // Cap bonus effect
  }, [mainWord]);
  ...
  const setupSimulation = useCallback((nodes: CustomNode[], links: CustomLink[]) => {
      simulationRef.current = d3.forceSimulation<CustomNode>(nodes)
        ...
        .force("link", d3.forceLink<CustomNode, CustomLink>()
          .id(d => d.id)
          .links(links)
          // Dynamic distance based on relationship
          .distance((link: CustomLink) => {
              const rel = link.relationship.toLowerCase();
              if (rel === 'synonym' || rel === 'antonym') return 60; // Closer for strong relations
              if (rel === 'root' || rel === 'derived' || rel === 'etymology') return 90;
              if (rel === 'associated') return 140; // Further for weak relations
              return 110; // Default
          })
          .strength(0.5)) // Slightly increased strength to enforce distances
        .force("charge", d3.forceManyBody<CustomNode>().strength(-280).distanceMax(300)) // Slightly adjusted charge
  ...
  const createLinks = useCallback((g: d3.Selection<SVGGElement, unknown, null, undefined>, linksData: CustomLink[]) => {
    return g.append("g")
      ...
      .attr("class", "link")
              ...
              .attr("x2", d => (typeof d.target === 'object' ? d.target.x ?? 0 : 0))
              .attr("y2", d => (typeof d.target === 'object' ? d.target.y ?? 0 : 0))
              .attr("stroke-dasharray", (d: CustomLink) => {
                  const rel = d.relationship.toLowerCase();
                  if (rel === 'synonym' || rel === 'antonym') return "none"; // Solid
                  if (rel === 'derived' || rel === 'derived_from' || rel === 'root_of' || rel === 'etymology') return "4,2"; // Dashed
                  // Dotted for associated/other types
                  return "1,3";
              })
      ...
      nodeSelection
        .on("click", (event, d) => {
          ...
          d3.selectAll<SVGLineElement, CustomLink>(".link")
              ...
              .attr("stroke", theme === "dark" ? "#444" : "#ccc") // Dimmer default stroke when dimmed
              .attr("stroke-opacity", 0.04) // Dim unrelated links even more
              .attr("stroke-width", 1.5);
          ...
          targetNodeElement.classed("selected", true)
            .transition().duration(200)
            .style("opacity", 1)
            // Apply scaling transform to the selected node group
           ...
           d3.selectAll<SVGLineElement, CustomLink>(connectedLinkElements)
            ...
            .attr("stroke", (l: CustomLink) => { // Color link based on target
                ...
                // Brighter color for highlighted links
                return targetNode ? d3.color(getNodeColor(targetNode.group))?.brighter(0.5).formatHex() ?? (theme === "dark" ? "#ccc" : "#555") : (theme === "dark" ? "#aaa" : "#666");
            })
            .attr("stroke-opacity", 1.0) // Fully opaque highlighted links
            .attr("stroke-width", 3.0); // Thicker highlighted links
        ...
        .on("mouseover", (event, d) => {
            ...
            // Apply scaling transform on hover
            nodeElement.transition("hover_scale").duration(100)
        ...
        .on("mouseout", (event, d_unknown) => {
            ...
            // Revert scale, respecting selected state scale
            nodeElement.transition("hover_scale").duration(150)
                 .attr("transform", `translate(${d.x ?? 0},${d.y ?? 0}) scale(${d.id === selectedNodeId ? 1.05 : 1})`);
