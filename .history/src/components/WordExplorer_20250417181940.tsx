...
import { WordNetwork, WordInfo, SearchResult, SearchOptions, EtymologyTree, Statistics, Definition, SearchWordResult, Relation, ApiWordNetwork } from "../types";
  ...
  const fetchWordNetworkData = useCallback(async (word: string, depth: number = 2, breadth: number = 10): Promise<WordNetwork | null> => { // Return WordNetwork | null
    try {
      ...
      const wordNetworkData: WordNetwork = {
        nodes: apiData.nodes.map(node => ({ // Ensure node IDs are numbers if possible
          ...node,
          id: typeof node.id === 'string' && !isNaN(parseInt(node.id)) ? parseInt(node.id) : (node.id || 0), // Convert ID to number or use 0
        })),
        edges: apiData.links.map(link => ({ // Map links to edges
          source: typeof link.source === 'object' ? (link.source.id || 0) : (link.source || 0), // Convert source ID to number
          target: typeof link.target === 'object' ? (link.target.id || 0) : (link.target || 0), // Convert target ID to number
    ...
    try {
      ...
      try {
        ...
        if (wordData && hasMissingOrMinimalRelations(wordData) && !wordData.semantic_network) {
          ...
          wordData = await fetchAndAddSemanticNetwork(
            wordData.lemma, 
            wordData, 
            depth, 
            breadth
