       ...
       nodeSelection
        ...
        .on("mouseover", (event, d) => {
            ...
            nodeElement.raise();
             ...
             d3.selectAll<SVGLineElement, CustomLink>(".link:not(.highlighted)").attr("stroke-opacity", 0.2);
        ...
        .on("mouseout", (event, d_unknown) => {
            ...
            // Revert hover effect for the circle
             ...
             // Restore opacity for all nodes and links - Simplified: restore default appearance
             d3.selectAll<SVGGElement, CustomNode>(".node")
                .style("opacity", 1); // Restore all node opacity
             d3.selectAll<SVGLineElement, CustomLink>(".link") // Select lines
                 // .filter(l => !d3.select(l as any).classed("highlighted")) // No need to filter, click handler resets first
                 .attr("stroke-opacity", 0.5) // Restore default opacity
                 .attr("stroke", theme === "dark" ? "#555" : "#bbb") // Restore default color
                 .attr("stroke-width", 1.5); // Restore default width

             // If a node IS selected, re-apply its specific styles (border etc.) - This part might need adjustment
             if (selectedNodeId) {
                 d3.select<SVGGElement, CustomNode>(".node.selected circle") // Re-select based on class
                    .attr("stroke-width", 2.5);
                 d3.selectAll<SVGLineElement, CustomLink>(".link.highlighted") // Re-select based on class
                     .attr("stroke-opacity", 0.9)
                     .attr("stroke-width", 2.5)
                     .attr("stroke", (l: CustomLink) => { // Re-apply color logic
                         const sourceId = typeof l.source === 'object' ? (l.source as CustomNode).id : l.source as string;
                         const targetId = typeof l.target === 'object' ? (l.target as CustomNode).id : l.target as string;
                         const selectedNode = nodeMap.get(selectedNodeId);
                         const otherNodeId = sourceId === selectedNodeId ? targetId : sourceId;
                         const otherNode = nodeMap.get(otherNodeId);
                         return otherNode ? getNodeColor(otherNode.group) : (theme === "dark" ? "#aaa" : "#666");
                      });
             }
        })
        .on("dblclick", (event, d) => {
