...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({ wordInfo, onWordClick }) => {
  ...
  const renderRelatedWords = () => {
    ...
    if (!wordInfo.relations) {
      console.log("No relations found in wordInfo.relations");
      return (
        <div className="empty-state">
    ...
    // Extract ALL possible relation types to ensure nothing is missed
    const relationTypes = Object.keys(wordInfo.relations);
    console.log("Relation types found:", relationTypes);
    
    const allRelationData: Record<string, string[]> = {};
    
    // Process all relation types to get words
    relationTypes.forEach(type => {
      if (wordInfo.relations![type] && Array.isArray(wordInfo.relations![type])) {
      ...
      } else if (type === 'root' && wordInfo.relations![type] && wordInfo.relations![type].word) {
        // Special case for 'root' which may not be an array
        console.log(`Found root word:`, wordInfo.relations![type].word);
      }
    });
