...
import { WordNetwork, WordInfo, SearchResult, SearchOptions, EtymologyTree, Statistics, Definition, SearchWordResult, Relation, NetworkNode, NetworkLink } from "../types"; // Add NetworkNode, NetworkLink
...
import { 
  ...
  getEtymologyTree,
  fetchWordRelations,
  ApiWordNetwork // Import ApiWordNetwork from here
} from "../api/wordApi";
...
import { hasMissingOrMinimalRelations, fetchAndAddSemanticNetwork } from '../utils/wordUtils';
...
const WordExplorer: React.FC = () => {
  ...
  const fetchWordNetworkData = useCallback(async (word: string, depth: number = 2, breadth: number = 10): Promise<WordNetwork | null> => { // Return WordNetwork | null
    try {
      setIsLoading(true);
      ...
      // fetchPromise now returns ApiWordNetwork
      const fetchPromise = fetchWordNetwork(word, { 
        depth,
        breadth,
      ...
      let apiData: ApiWordNetwork | null = null; // Use ApiWordNetwork here
      try {
        apiData = await fetchPromise;
        ...
        if (error.name === 'AbortError' || (typeof error.message === 'string' && error.message.includes('timeout'))) {
          console.warn(`Network fetch timed out after ${timeoutDuration}ms for word: ${word}`);
          // Create a minimal fallback ApiWordNetwork 
          const fallbackApiNetwork: ApiWordNetwork = {
            nodes: [{ 
              id: `fallback:${word}`, // Use a unique ID format
              label: word, 
              word: word,
              type: 'main' 
            }],
            links: [],
            metadata: { root_word: word }
          };
          apiData = fallbackApiNetwork; // Use the fallback API data
        } else {
          throw error;
        }
      }
      
      // Ensure apiData is not null before proceeding
      if (!apiData) {
        throw new Error('Failed to fetch or create fallback network data');
      }

      // Map ApiWordNetwork to WordNetwork
      const wordNetworkData: WordNetwork = {
        nodes: apiData.nodes.map((node: any): NetworkNode => ({
          ...node,
          id: typeof node.id === 'string' && !isNaN(parseInt(node.id)) ? parseInt(node.id) : (typeof node.id === 'number' ? node.id : 0),
          label: node.label || node.word || '',
          word: node.word || node.label || '',
          type: node.type || 'unknown'
        })),
        // Ensure edges strictly use number IDs for source/target
        edges: apiData.links.map((link: any): { source: number; target: number; type: string } => { 
          const sourceId = typeof link.source === 'object' ? Number(link.source?.id || 0) : Number(link.source || 0);
          const targetId = typeof link.target === 'object' ? Number(link.target?.id || 0) : Number(link.target || 0);
          return {
            source: isNaN(sourceId) ? 0 : sourceId,
            target: isNaN(targetId) ? 0 : targetId,
            type: link.type || 'related'
          };
        }),
        metadata: {
          root_word: apiData.metadata?.root_word || word,
          normalized_lemma: apiData.metadata?.normalized_lemma || word,
          language_code: apiData.metadata?.language_code || 'tl',
          depth: apiData.metadata?.depth ?? depth,
          total_nodes: apiData.metadata?.total_nodes ?? apiData.nodes.length,
          total_edges: apiData.metadata?.total_edges ?? apiData.links.length,
          // Use available depth/breadth params for defaults related to fetch options
          include_affixes: apiData.metadata?.include_affixes ?? true, // Or derive from depth/breadth?
          include_etymology: apiData.metadata?.include_etymology ?? true,
          cluster_threshold: apiData.metadata?.cluster_threshold ?? 0.3
        }
      };

      console.log('Word network data mapped:', wordNetworkData);
      console.log(`Mapped network has ${wordNetworkData.nodes.length} nodes and ${wordNetworkData.edges.length} edges`);
      setWordNetwork(wordNetworkData); // Set the mapped data
      
      // Sync network data with word details if we have wordData
      if (wordData && wordData.id) {
        // Find the main node (root word)
        const mainNode = wordNetworkData.nodes.find(node => 
          node.type === 'main' || node.word === word || node.label === word
        );
        
        if (mainNode) {
          // Create relations from network data (using edges now)
          const incomingRelations: Relation[] = [];
          const outgoingRelations: Relation[] = [];
          
          wordNetworkData.edges.forEach(edge => {
            const sourceNode = wordNetworkData.nodes.find(n => n.id === edge.source);
            const targetNode = wordNetworkData.nodes.find(n => n.id === edge.target);
      ...
      return wordNetworkData; // Return the mapped data
    } catch (error) {
      ...
      // Return a minimal WordNetwork structure on error
      const fallbackWordNetwork: WordNetwork = {
        nodes: [{ 
          id: 0, label: word, word: word, type: 'main' 
        }],
        edges: [],
        // Provide default values for all required metadata fields
        metadata: { 
          root_word: word, 
          normalized_lemma: word, 
          language_code: 'tl', 
          depth: depth, // Use current depth
          total_nodes: 1, 
          total_edges: 0 
        }
      };
      setWordNetwork(fallbackWordNetwork); // Set fallback
      return fallbackWordNetwork;
    } finally {
      setIsLoading(false);
    }
  }, [wordData]); // Added wordData dependency
  ...
  const handleNodeClick = useCallback(async (word: string) => {
    ...
    try {
      ...
      try {
        ...
        // Check if relations are missing or minimal and fetch semantic network if needed
        if (wordData && hasMissingOrMinimalRelations(wordData) && !wordData.semantic_network) {
          console.log('Word has missing or minimal relations, fetching semantic network');
          // Correct arguments for fetchAndAddSemanticNetwork: wordInfo, word, depth
          const enhancedWordData = await fetchAndAddSemanticNetwork(
            wordData, // Pass wordInfo object
            wordData.lemma, // Pass lemma string
            depth // Pass depth
            // Breadth is not used in the function anymore
          );
          // Use the enhanced data if successful
          if (enhancedWordData) {
      ...
      if (!fallbackToSearch && wordData) {
        ...
        try {
          ...
          // fetchWordNetworkData returns WordNetwork | null
          const mappedNetworkData = await fetchWordNetworkData(wordData.lemma, depth, breadth);
          
          // Use the mappedNetworkData (which has .edges)
          if (mappedNetworkData && (!wordData.outgoing_relations?.length && !wordData.incoming_relations?.length)) {
            console.log('[DEBUG] No direct relations found in handleNodeClick, adding semantic network to selectedWordInfo');
            setSelectedWordInfo(prevInfo => {
              if (!prevInfo) return prevInfo;
              // Check mappedNetworkData again inside closure just in case
              if (!mappedNetworkData) return prevInfo;
              return {
                ...prevInfo,
                semantic_network: {
                  nodes: mappedNetworkData.nodes as NetworkNode[], 
                  links: mappedNetworkData.edges.map(edge => ({ // Map edges to links
                    source: edge.source,
                    target: edge.target,
                    type: edge.type
                  })) as NetworkLink[] 
  ...
  const handleNodeSelect = useCallback(async (word: string) => {
    ...
    try {
      ...
      if (wordData) {
        ...
        // We also need to handle the semantic network for relations
        // Check if we need to fetch network data for relations
        if (!wordData.outgoing_relations?.length && !wordData.incoming_relations?.length) {
          try {
            console.log(`[DEBUG] No direct relations in handleNodeSelect, fetching network for: ${wordData.lemma}`);
            fetchWordNetworkData(wordData.lemma, depth, breadth)
              .then(mappedNetworkData => {
                // Check mappedNetworkData is not null
                if (mappedNetworkData && (mappedNetworkData.nodes?.length || mappedNetworkData.edges?.length)) { 
                  console.log(`[DEBUG] Adding semantic network to selectedWordInfo in handleNodeSelect`);
                  setSelectedWordInfo(prevInfo => {
                    if (!prevInfo || !mappedNetworkData) return prevInfo; // Check again
                    return {
                      ...prevInfo,
                      semantic_network: {
                        nodes: mappedNetworkData.nodes as NetworkNode[],
                        links: mappedNetworkData.edges.map(edge => ({ // Map edges
                          source: edge.source,
                          target: edge.target,
                          type: edge.type
                        })) as NetworkLink[]
              ...
              .catch(err => {
                console.error("Error fetching network for relations:", err);
              });
          } catch (networkError) {
            console.error("Error initiating network fetch for relations:", networkError);
          }
        }
  ...
  }, [wordHistory, depth, breadth, fetchWordDetails, fetchEtymologyTree, fetchWordNetworkData]);
  ...
  const handleSearch = useCallback(async (query: string) => {
    ...
    try {
      ...
      if (result && result.words && result.words.length > 0) {
            ...
            if (networkData && networkData.nodes && networkData.edges) { 
              const enhancedWordData = {
                ...
                semantic_network: {
                  ...
                  links: networkData.edges.map(edge => ({ // Map edges
                    source: edge.source,
                    target: edge.target,
                    type: edge.type
                  })) as NetworkLink[]
        ...
        // No results found
        console.log(`[DEBUG] No search results found for "${query}"`);
  ...
  }, [depth, breadth, wordHistory, currentHistoryIndex, fetchWordNetworkData, fetchEtymologyTree]);
        ...
        Promise.all([
          ...
          depth && breadth 
            ? fetchWordNetworkData(randomWord.lemma, depth, breadth)
              .then(mappedNetworkData => {
                // Check mappedNetworkData is not null
                if (mappedNetworkData && (!wordInfo.outgoing_relations?.length && !wordInfo.incoming_relations?.length)) {
                  setSelectedWordInfo(prevInfo => {
                    if (!prevInfo || !mappedNetworkData) return prevInfo; // Check again
                    return {
                      ...prevInfo,
                ...
                // Don't set network state here, fetchWordNetworkData does it
              })
              .catch(err => {
