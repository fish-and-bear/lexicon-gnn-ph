...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({
  ...
  const renderRelationsTab = () => {
    ...
    console.log("WordDetails: Relations Data Check", {
      ...
      definitionRelations: wordInfo.definition_relations || [],
      definitionRelationsInDefs: wordInfo.definitions?.flatMap(def => def.definition_relations || []),
      wordInfoKeys: Object.keys(wordInfo),
      hasWordObj: incoming_relations.length > 0 ? Boolean(incoming_relations[0].source_word) : false
    });
    
    // Add more detailed debug information
    console.log("WordDetails: Full Word Info", {
      id: wordInfo.id,
      lemma: wordInfo.lemma,
      allKeys: Object.keys(wordInfo),
      definitionsLength: wordInfo.definitions?.length || 0,
      definitionSample: wordInfo.definitions && wordInfo.definitions.length > 0 
        ? { id: wordInfo.definitions[0].id, keys: Object.keys(wordInfo.definitions[0]) } 
        : null
    });
    ...
    console.log("Definition relations converted to outgoing format:", definitionRelationsAsOutgoing);
    ...
    // Try to access network data from multiple sources
    // Instead of using window.wordNetwork which causes TypeScript errors
    const networkFromProps = (wordInfo as any).wordNetwork;
    
    // Get semantic network data from available sources
    const semanticNetworkNodes = 
      (networkFromProps?.nodes) || 
      wordInfo.semantic_network?.nodes || 
      [];
      
    const semanticNetworkLinks = 
      (networkFromProps?.edges) || 
      wordInfo.semantic_network?.links || 
      [];
    
    // Debug the semantic network data directly to see what we're working with
    console.log("FULL SEMANTIC NETWORK DATA:", {
      nodes: semanticNetworkNodes,
      links: semanticNetworkLinks,
      wordLemma: wordInfo.lemma,
      hasNodes: semanticNetworkNodes.length > 0,
      hasLinks: semanticNetworkLinks.length > 0,
      semanticNetworkObj: wordInfo.semantic_network,
      networkFromProps
    });
    
    // ALWAYS use the semantic network if available, or fall back to standard relations
    const hasSemanticNetwork = semanticNetworkLinks.length > 0;
    const useSemanticNetwork = hasSemanticNetwork;
    
    if (hasSemanticNetwork) {
      console.log(`Using semantic network data with ${semanticNetworkNodes.length} nodes and ${semanticNetworkLinks.length} links`);
    } else {
      console.log("No semantic network data available, using standard relations");
    }
    ...
    function createRelationsFromNetwork() {
      ...
      // SIMPLIFIED APPROACH: Create relations for all links
      // This ensures we always display relationships regardless of node structure
      semanticNetworkLinks.forEach((link: any) => {
        // Find the connected nodes
        const sourceId = typeof link.source === 'object' ? link.source.id : link.source;
        const targetId = typeof link.target === 'object' ? link.target.id : link.target;
        
        const sourceNode = semanticNetworkNodes.find((n: any) => n.id === sourceId);
        const targetNode = semanticNetworkNodes.find((n: any) => n.id === targetId);
        ...
        // Skip if either node is the main word to avoid duplication
        const isMainWordSource = sourceNode.type === 'main' || 
                              sourceNode.word === mainWord || 
                              sourceNode.label === mainWord;
                              
        const isMainWordTarget = targetNode.type === 'main' || 
                              targetNode.word === mainWord || 
                              targetNode.label === mainWord;
                              
        // Only create relations where one node is the main word and the other is not
        if ((isMainWordSource && !isMainWordTarget) || (!isMainWordSource && isMainWordTarget)) {
          // When sourceNode is the main word, it's an outgoing relation
          // When targetNode is the main word, it's an incoming relation
          relations.push({
            id: `semantic-${sourceNode.id}-${targetNode.id}`,
            relation_type: link.type || 'related',
            degree: targetNode.depth || link.distance || 1,
            direction: isMainWordSource ? 'outgoing' : 'incoming',
            wordObj: isMainWordSource ? {
              id: targetNode.id,
              ...
              id: sourceNode.id,
              lemma: sourceNode.label || sourceNode.word || String(sourceNode.id),
    ...
    // Important: Log the actual semantic relations that were created
    console.log("Semantic relations created:", semanticRelations.length, semanticRelations);
    
    // Always check if we have a visible network even if we couldn't parse relations
    const hasVisibleNetwork = Boolean(networkFromProps && 
                                     ((networkFromProps.nodes && networkFromProps.nodes.length > 0) || 
                                      (networkFromProps.edges && networkFromProps.edges.length > 0)));
    
    // Fix: If we have a semantic network but no semantic relations were created (which would be unusual),
    // let's fall back to a simple approach of considering all connections
    if (useSemanticNetwork && semanticRelations.length === 0 && semanticNetworkLinks.length > 0) {
      console.log("WARNING: Semantic network exists but no relations were created. Using fallback approach.");
      // Create simple relations for all links
      semanticNetworkLinks.forEach((link: any) => {
        const targetNode = semanticNetworkNodes.find(
          (n: any) => n.id === (typeof link.target === 'object' ? link.target.id : link.target)
        );
        if (targetNode && targetNode.label) {
          semanticRelations.push({
            id: `semantic-fallback-${targetNode.id}`,
            relation_type: link.type || 'related',
            degree: 1,
            direction: 'outgoing',
            wordObj: {
      ...
      console.log("Created fallback relations:", semanticRelations.length);
    }
    
    // Fixed: Check if we have semantic network data directly, don't just rely on processed relations
    // If we have semantic network, don't check for standard relations
    const hasStandardRelations = (incoming_relations.length > 0 || combinedOutgoingRelations.length > 0) && !useSemanticNetwork;
    
    // Updated to include hasSemanticNetwork directly in the check
    const hasAnyRelations = hasStandardRelations || semanticRelations.length > 0 || hasSemanticNetwork || hasVisibleNetwork || 
