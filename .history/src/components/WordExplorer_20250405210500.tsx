...
const processRawWordData = (rawData: RawWordData): WordInfo => {
  ...
  const cleanDefinitions = (rawData.definitions || []).map((def: RawDefinition): Definition => {
    // Omit related_definitions during cleaning for now to simplify typing
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const { related_definitions, standardized_pos_id, original_pos, ...restOfDef } = def;

    return {
      // Spread remaining raw definition fields
      ...restOfDef,
      ...
      // related_definitions: undefined, // Explicitly omit
    };
  });
  ...
  const cleanEtymologies = (rawData.etymologies || []).map((ety: RawEtymology): Etymology => {
    ...
    return {
      // Spread remaining raw etymology fields
      ...restOfEty,
      // Overwrite/process specific fields
  ...
  const cleanTemplates = (rawData.templates || []).map((template: WordTemplate): WordTemplate => ({
    ...
    const processRelation = (rel: Relation): Relation => ({
        ...
        // Cast sources to any to bypass potential mismatch with Relation type (string vs string[])
    ...
    const processAffixation = (affix: Affixation): Affixation => ({
        ...affix,
         // Cast sources to any to bypass potential mismatch with Affixation type (string vs string[])
  ...
  const wordInfo: WordInfo = {
    ...
    tags: cleanTags && cleanTags.length > 0 ? (cleanTags as any) : null, // TEMP CAST for WordInfo.tags (string vs string[])
    ...
    // Metadata - Assign directly, cast data_completeness to bypass potential type mismatch
