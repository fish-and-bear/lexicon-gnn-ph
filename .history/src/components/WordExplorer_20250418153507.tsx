...
const WordExplorer: React.FC = () => {
  // --- DEBUG: Restore useTheme hook call, but don't use value yet ---
  const { theme: contextTheme, toggleTheme } = useTheme(); // Call hook, rename theme variable
  const theme = 'light'; // Keep using default theme for now
  ...
  useEffect(() => {
    console.log("Simulating API connection check (without actual API call)...");
    setIsLoading(true);
    setError(null);

    // --- DEBUG: Comment out API call, directly set state ---
    /*
    testApiConnection()
      .then(connected => {
