  ...
  useEffect(() => {
    ...
    console.log('WordDetails: Raw Baybayin data check:', {
      ...
      unicode_points: wordInfo.has_baybayin && wordInfo.baybayin_form ? 
        Array.from(wordInfo.baybayin_form).map(char => `U+${char.codePointAt(0)?.toString(16).toUpperCase().padStart(4, '0')}`).join(' ') : 
        'N/A',
      ...
      const fetchBaybayinConversion = async () => {
        try {
          ...
          if (result && result.baybayin) {
            ...
            const codePoints = Array.from(result.baybayin).map(char => 
