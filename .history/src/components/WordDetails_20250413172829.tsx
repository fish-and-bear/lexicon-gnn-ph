...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({
  ...
  const renderRelationsTab = () => {
    ...
    // Create a workaround for missing relations arrays
    const fixedWordInfo = { ...wordInfo };
    
    // If relation arrays are missing, create empty ones
    if (!fixedWordInfo.incoming_relations) {
      console.warn("incoming_relations array is missing - adding empty array");
      fixedWordInfo.incoming_relations = [];
    }
    
    if (!fixedWordInfo.outgoing_relations) {
      console.warn("outgoing_relations array is missing - adding empty array");
      fixedWordInfo.outgoing_relations = [];
    }
    
    const incoming_relations = fixedWordInfo.incoming_relations || [];
    const outgoing_relations = fixedWordInfo.outgoing_relations || [];
    
    // Add debugging to check relation data
    console.log("Relations data:", {
      wordId: wordInfo.id,
      lemma: wordInfo.lemma,
      hasIncomingArray: Boolean(wordInfo.incoming_relations),
      hasOutgoingArray: Boolean(wordInfo.outgoing_relations),
      incomingLength: incoming_relations.length,
      outgoingLength: outgoing_relations.length,
      incomingSample: incoming_relations.length > 0 ? incoming_relations[0] : null,
      outgoingSample: outgoing_relations.length > 0 ? outgoing_relations[0] : null
    });
    ...
    const renderRelationGroup = (title: string, relations: WordInfo['outgoing_relations'] | WordInfo['incoming_relations'], direction: 'in' | 'out') => {
      ...
      console.log(`Grouped relations for ${title}:`, groupedRelations);
            ...
            <Box key={relType} sx={{ mb: theme.spacing(2) }}>
              ...
              <Stack direction="row" spacing={1} useFlexGap flexWrap="wrap">
                {rels.map((rel, index) => {
                  ...
                  // Fix: Correct the syntax error in the hasSources condition
