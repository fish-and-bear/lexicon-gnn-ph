...
const WordExplorer: React.FC = () => {
  ...
  const handleNodeClick = useCallback(async (word: string) => {
    ...
    try {
      ...
      if (!fallbackToSearch && wordData) {
        ...
        const existingWordIndex = wordHistory.findIndex(w => 
          typeof w === 'object' && 'id' in w && String(w.id) === wordId
        );
        
        if (existingWordIndex !== -1) {
          // If word exists in history, just update the current index
          console.log(`Word already in history at index ${existingWordIndex}, updating currentHistoryIndex`);
          setCurrentHistoryIndex(existingWordIndex);
        } else {
          // If word doesn't exist, add it to history after current position
          // This also truncates any forward history
          ...
          console.log(`Adding word to history, new index: ${newHistory.length - 1}`);
        }
  ...
  const debouncedSearch = useCallback(
    debounce(async (query: string) => {
      ...
      try {
        ...
        if (result && result.words && result.words.length > 0) {
          console.log(`Found ${result.words.length} suggestions for: ${query}`);
          setSearchResults(result.words);
  ...
  // Update input handling for search suggestions
  useEffect(() => {
    if (inputValue) {
          ...
          {showSuggestions && searchResults.length > 0 && (
            <ul className="search-suggestions">
              {searchResults.map((result) => {
                // Check if this is a duplicate lemma that needs differentiating
                const hasDuplicates = searchResults.filter(r => r.lemma === result.lemma).length > 1;
