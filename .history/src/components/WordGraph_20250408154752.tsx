...
interface NodeData {
  ...
  fy?: number | null;
  index?: number;
  // Additional properties needed for display
  label?: string;
  language?: string;
...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const baseNodes = useMemo<CustomNode[]>(() => {
    ...
    const mappedNodes = wordNetwork.nodes.map(node => {
      // Ensure label exists or create a fallback
      const nodeLabel = node.label || node.lemma || `node-${node.id}`;
      
      let calculatedGroup = 'associated';
      if (nodeLabel === mainWord) {
        calculatedGroup = 'main';
      } else {
        const connectingLink = baseLinks.find(link =>
          (link.source === mainWord && link.target === nodeLabel) ||
          (link.source === nodeLabel && link.target === mainWord)
        );
        calculatedGroup = mapRelationshipToGroup(connectingLink?.relationship);
      }
      
      // Count connections for potential sizing later
      const connections = baseLinks.filter(l => l.source === nodeLabel || l.target === nodeLabel).length;

      return {
        id: nodeLabel, // Ensure id is always a string
        word: nodeLabel,
        label: nodeLabel,
        ...
        .on("click", (event, d) => {
          ...
          d3.selectAll<SVGGElement, CustomNode>(".node")
           ...
           if (onNodeClick) {
             onNodeClick({
               id: d.originalId,
               lemma: d.label || d.word || '',
               language_code: d.language || '',
               main: d.group === 'main',
               has_baybayin: d.has_baybayin || false,
               baybayin_form: d.baybayin_form || null
             });
           }
        })
  ...
  useEffect(() => {
    ...
    // Observe the SVG's parent element
