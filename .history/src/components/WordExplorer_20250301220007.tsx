          ...
          <div className="details-content">
            {isLoading ? (
              ...
              <div className="word-details">
                ...
                {/* Idioms Section */}
                {selectedWordInfo?.idioms && selectedWordInfo.idioms.length > 0 && (
                  <div className="idioms-section">
                    <h3>Idioms & Phrases</h3>
                    <div className="idioms-list">
                      {selectedWordInfo.idioms.map((idiom, index) => (
                        <div key={index} className="idiom-card">
                          <div className="idiom-text">
                            {typeof idiom === 'string' ? idiom : idiom.text || idiom.phrase}
                          </div>
                          {typeof idiom !== 'string' && idiom.meaning && (
                            <div className="idiom-meaning">{idiom.meaning}</div>
                          )}
                          {typeof idiom !== 'string' && idiom.example && (
                            <div className="idiom-example">"{idiom.example}"</div>
                          )}
                        </div>
                      ))}
                    </div>
