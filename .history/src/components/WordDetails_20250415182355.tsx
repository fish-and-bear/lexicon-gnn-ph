...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({
  ...
  const renderDefinitionsTab = () => {
    const sourceMetadataContent = renderSourcesInfoTab();
    const definitions = wordInfo.definitions || [];
    const hasDefinitions = definitions.length > 0;

    if (!hasDefinitions && !sourceMetadataContent) {
      // No definitions AND no metadata
      return <Alert severity="info" sx={{ m: 1 }}>No definitions or metadata available.</Alert>;
    }

    // Group definitions if they exist
    ...
    return (
      <Box sx={{ pt: theme.spacing(1) }}>
        {!hasDefinitions && sourceMetadataContent && (
          // Only metadata available
          <Alert severity="info" sx={{ mb: 2 }}>No definitions available for this word.</Alert>
        )}

        {hasDefinitions && Object.entries(definitionsByPos).map(([posName, defs]) => (
          <Box key={posName} sx={{ mb: theme.spacing(3) }}>
            ...
            <List disablePadding>
              {defs.map((def: Definition, index: number) => (
                // --- Start of Definition ListItem --- Keep existing content here
                <ListItem 
                  key={def.id || index} 
                ...
                >
                    ...
                    <Box sx={{ pl: theme.spacing(2), mb: theme.spacing(1) }}>
                      {def.examples.map((example, exIndex) => (
                        <Typography 
                          key={exIndex} 
                          variant="body2" 
                          sx={{ fontStyle: 'italic', color: 'text.secondary', mb: exIndex < def.examples.length - 1 ? 0.5 : 0, position: 'relative', pl: theme.spacing(3), '&:before': { content: '"""', position: 'absolute', left: 0, color: alpha(theme.palette.text.primary, 0.4), fontSize: '1.5rem', lineHeight: 1, top: -5 } }}
                        >
                          {example}
                        </Typography>
                      ))}
                  ...
                  {def.sources && def.sources.length > 0 && (
                    <Stack direction="row" spacing={0.5} useFlexGap flexWrap="wrap" sx={{ mt: theme.spacing(1) }}>
                      {def.sources.map((source, sourceIndex) => (
                        <Chip key={sourceIndex} label={source} size="small" variant="outlined" sx={{ fontSize: '0.7rem', height: 'auto', padding: theme.spacing(0.25, 0), borderColor: alpha(graphColors.related, 0.5), color: graphColors.related, bgcolor: alpha(graphColors.related, 0.05), '& .MuiChip-label': { px: 1, py: 0.25 } }} />
                      ))}
                    </Stack>
                  )}
                  {/* Definition Tags */}
                  {def.tags && def.tags.length > 0 && (
                    <Stack direction="row" spacing={0.5} useFlexGap flexWrap="wrap" sx={{ mt: theme.spacing(0.5) }}>
                      {def.tags.map((tag, tagIndex) => (
                        <Chip key={tagIndex} label={tag} size="small" sx={{ fontSize: '0.7rem', height: 'auto', padding: theme.spacing(0.25, 0), bgcolor: alpha(theme.palette.primary.main, 0.1), color: theme.palette.primary.main, '& .MuiChip-label': { px: 0.75, py: 0.2 } }} />
                      ))}
                    </Stack>
                  )}
                </ListItem>
                // --- End of Definition ListItem ---
              ))}
            </List>
          </Box>
        ))}

        {/* Conditionally render Source/Metadata Info only if content exists */}
  ...
  const renderSourcesInfoTab = () => {
     ...
     if (!hasCredits && !hasSourceInfo && !hasWordMeta && !hasCompleteness && !hasEntryInfo) {
       // Return null instead of an Alert when there's no data
       return null;
     }
     ...
     const renderJsonData = (title: string, data: Record<string, any>) => {
       ...
       return (
         <StyledAccordion defaultExpanded sx={{ mt: 1, mb: 1 }}>
           <StyledAccordionSummary expandIcon={<ExpandMoreIcon />}>
             <Typography variant="subtitle2" sx={{ fontWeight: 500 }}>{title}</Typography>
           </StyledAccordionSummary>
           <StyledAccordionDetails sx={{ pt: 0 }}>
             <Paper variant="outlined" sx={{ p: 1.5, bgcolor: alpha(theme.palette.grey[500], 0.05), maxHeight: 200, overflowY: 'auto' }}>
     ...
     return (
       <Box sx={{ /* Removed padding p: theme.spacing(2) */ }}>
         {/* Credits List */} 
         {hasCredits && (
           <StyledAccordion defaultExpanded>
             <StyledAccordionSummary expandIcon={<ExpandMoreIcon />}>
               <Typography variant="subtitle2" sx={{ fontWeight: 600 }}>Credits / Sources</Typography>
             </StyledAccordionSummary>
             <StyledAccordionDetails>
               <List dense disablePadding>
                 {credits.map((credit, index) => (
                   <ListItem
                     ...
                     sx={{
                       py: 0.5,
                       borderBottom: index < credits.length - 1 ? `1px solid ${alpha(theme.palette.divider, 0.5)}` : 'none'
                     }}
                   >
                     <ListItemText primary={<Typography variant="body2">{credit.credit}</Typography>} />
                   </ListItem>
                 ))}
               </List>
             </StyledAccordionDetails>
         ...
         {hasCompleteness && (
            <StyledAccordion defaultExpanded>
              <StyledAccordionSummary expandIcon={<ExpandMoreIcon />}>
                <Typography variant="subtitle2" sx={{ fontWeight: 600 }}>Data Completeness</Typography>
              </StyledAccordionSummary>
              <StyledAccordionDetails>
                <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: 1 }}>
         ...
         {hasEntryInfo && (
           <StyledAccordion defaultExpanded>
             <StyledAccordionSummary expandIcon={<ExpandMoreIcon />}>
               <Typography variant="subtitle2" sx={{ fontWeight: 600 }}>Entry Information</Typography>
             </StyledAccordionSummary>
             <StyledAccordionDetails>
               <Stack spacing={1}>
                 {wordInfo.created_at && (
