...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  // Update setupZoom to include better initial centering
  const setupZoom = useCallback((svg: d3.Selection<SVGSVGElement, unknown, null, undefined>, g: d3.Selection<SVGGElement, unknown, null, undefined>) => {
    ...
    // Calculate center transform and apply it
  ...
  // Improve the simulation to better handle centering
  const setupSimulation = useCallback((nodes: CustomNode[], links: CustomLink[]) => {
    ...
    // Create a new simulation with sensible forces
    const sim = d3
      ...
      .force('center', d3.forceCenter(0, 0)) // Center at 0,0 since we'll translate the whole group
      .force('collision', d3.forceCollide().radius(35)) // Slightly larger collision radius
      .on('tick', () => {
    ...
    if (mainWordNode) {
      ...
      // Keep main node fixed for longer to allow graph to stabilize around it
      setTimeout(() => {
        if (mainWordNode) {
          mainWordNode.fx = null;
          mainWordNode.fy = null;
        }
      }, 2500); // Longer stabilization time
    }
  ...
  // Create a simple and robust drag behavior for nodes
  const createDragBehavior = useCallback(() => {
    return d3.drag<SVGGElement, CustomNode>()
      ...
      .on('drag', (event, d) => {
        ...
        // Let the simulation handle the visual update in the tick function
      })
      .on('end', (event, d) => {
        if (!event.active && simulationObject) {
  ...
  // Now define setupNodeInteractions after its dependencies
    ...
    try {
      ...
      const centerOnMainWord = () => {
        ...
        if (mainNode && mainNode.x && mainNode.y && zoom) {
          // Only center if the main node has moved significantly from the center
          const dist = Math.sqrt(mainNode.x * mainNode.x + mainNode.y * mainNode.y);
          if (dist > 50) { // Only recenter if it's drifted significantly
      ...
      setTimeout(centerOnMainWord, 300);
