...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  // Add the getNodeRadius function
        ...
        d3.selectAll(".link")
          .each(function(linkData: any) {
            ...
            if (source === d.id || target === d.id) {
              ...
              d3.select(`.node[data-id="${connectedId}"]`)
  ...
  // Add the setupSimulation function that was missing
  const setupSimulation = useCallback((nodes: CustomNode[], links: CustomLink[]) => {
    ...
    simulation.on("tick", () => {
      ...
      d3.select(svgRef.current).selectAll<SVGGElement, CustomNode>(".node")
        .attr("transform", d => `translate(${d.x || 0}, ${d.y || 0})`);

      d3.select(svgRef.current).selectAll<SVGLineElement, CustomLink>(".link")
