...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({
  ...
  const renderHeader = () => {
    // Find various pronunciation types
    const ipaPronunciation = wordInfo.pronunciations?.find(p => p.type === 'IPA' || p.type === 'ipa');
    const respellingPronunciation = wordInfo.pronunciations?.find(p => p.type === 'respelling');
    const phonemicPronunciation = wordInfo.pronunciations?.find(p => p.type === 'phonemic');
    const xSampaPronunciation = wordInfo.pronunciations?.find(p => p.type === 'x-sampa');
    const pinyinPronunciation = wordInfo.pronunciations?.find(p => p.type === 'pinyin');
    const jyutpingPronunciation = wordInfo.pronunciations?.find(p => p.type === 'jyutping');
    const romajiPronunciation = wordInfo.pronunciations?.find(p => p.type === 'romaji');
    ...
    return (
      ...
      <Box sx={{ 
        ...
        <Stack spacing={1.5} sx={{ mb: theme.spacing(2) }}>
          ...
          {/* Other Pronunciation Types */}
          {(respellingPronunciation || phonemicPronunciation || xSampaPronunciation || 
            pinyinPronunciation || jyutpingPronunciation || romajiPronunciation) && (
            <Box sx={{ mt: 1 }}>
              <Stack direction="row" spacing={2} flexWrap="wrap">
                {respellingPronunciation && (
                  <Box>
                    <Typography variant="caption" sx={{ color: alpha(headerTextColor, 0.7), fontWeight: 500 }}>
                      Respelling
                    </Typography>
                    <Typography variant="body2" sx={{ fontWeight: 500 }}>
                      {respellingPronunciation.value}
                    </Typography>
                  </Box>
                )}
                
                {phonemicPronunciation && (
                  <Box>
                    <Typography variant="caption" sx={{ color: alpha(headerTextColor, 0.7), fontWeight: 500 }}>
                      Phonemic
                    </Typography>
                    <Typography variant="body2" sx={{ fontWeight: 500 }}>
                      {phonemicPronunciation.value}
                    </Typography>
                  </Box>
                )}
        ...
        <Stack spacing={2} sx={{ my: theme.spacing(2) }}>
