...
const WordExplorer: React.FC = () => {
  ...
  const handleRandomWord = useCallback(async () => {
    setError(null);
    setIsLoading(true);
    // Reset states immediately before fetching
    setWordData(null);
    setWordNetwork(null);
    setSelectedNode(null);
    setEtymologyTree(null);
    // Clear history related to previous word - start fresh for random
    // setWordHistory([]); 
    ...
    try {
      ...
      // Fetch related data, allowing individual fetches to fail gracefully
      console.log(`Fetching related data for ${wordInfo.lemma} (ID: ${wordInfo.id})...`);
      const [networkData, etymologyData] = await Promise.all([
        fetchWordNetworkData(wordInfo.lemma, depth, breadth)
          .catch(err => {
             console.error(`Error fetching network data for ${wordInfo.lemma}:`, err);
             // Optionally set a partial error message here if needed
        ...
        wordInfo.id
          ? fetchEtymologyTree(wordInfo.id)
              .catch(err => {
                console.error(`Error fetching etymology tree for ${wordInfo.lemma} (ID: ${wordInfo.id}):`, err);
                // Optionally set a partial error message here if needed
                // setError("Failed to load etymology tree."); 
                return null; // Return null instead of throwing
              })
          : Promise.resolve(null) // No etymology to fetch if no ID
      ]);
      console.log("Related data fetched (network/etymology potentially null on error).");

      // --- Start State Updates ---
      // Update core word data first
      setWordData(wordInfo);
      setSelectedNode(wordInfo.lemma); // Update selected node for graph highlighting

      // Update history *after* successfully getting the word info
      const historyEntry = { id: wordInfo.id, text: wordInfo.lemma };
      // Append to current history state without overwriting navigation history
      setWordHistory(prevHistory => [
        ...prevHistory.slice(0, currentHistoryIndex + 1),
         historyEntry
       ]);
      // Update index to point to the new random word entry
      setCurrentHistoryIndex(prevIndex => prevIndex + 1); 

      // Update network data if fetch succeeded
      if (networkData && networkData.nodes && networkData.edges) {
         setWordNetwork(networkData);
         console.log("Word network updated.");
      } else {
         setWordNetwork(null); // Explicitly clear if fetch failed or data invalid
         console.log("Word network fetch failed or returned invalid data.");
      }

      // Update etymology data if fetch succeeded
      if (etymologyData) {
         setEtymologyTree(etymologyData);
         console.log("Etymology tree updated.");
      } else {
         setEtymologyTree(null); // Explicitly clear if fetch failed or data invalid
         console.log("Etymology tree fetch failed or returned invalid data.");
      }
      // --- End State Updates ---

      console.log("Random word processing complete.");

    } catch (error) {
      // Catch errors from getRandomWord, invalid data, or state updates
      console.error("Error in handleRandomWord execution:", error);
      setError(error instanceof Error ? error.message : "Failed to get or process random word");
      // Ensure states are reset on error
      setWordData(null);
      setWordNetwork(null);
      setSelectedNode(null);
      setEtymologyTree(null);
      // Optionally reset history if the random fetch failed completely
      // setWordHistory([]);
      // setCurrentHistoryIndex(-1);
    } finally {
      // This block ALWAYS runs, ensuring loading state is reset
      setIsLoading(false);
      console.log("handleRandomWord finished, isLoading set to false.");
    }
  }, [
    ...
    setSelectedNode,
    setEtymologyTree,
    setWordHistory,       // Added
    setCurrentHistoryIndex, // Added
    currentHistoryIndex,  // Added
    setError,
    setIsLoading,         // Added
    getRandomWord,        // Added missing dependency
    getEtymologyTree,     // Added missing dependency (used by fetchEtymologyTree)
  ]);
