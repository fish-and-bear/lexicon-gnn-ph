...
const WordExplorer: React.FC = () => {
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const maxWidth = containerRect.width - 300; // Ensure details panel has at least 300px
    ...
    <div className={`word-explorer ${theme} ${isLoading ? 'loading' : ''}`}>
      ...
      <main ref={mainRef}>
        <div 
          ...
          style={leftPanelWidth ? { width: `${leftPanelWidth}px`, flexShrink: 0 } : undefined}
        >
                    ...
                    <details>
                      ...
                      <ul style={{ margin: 0, paddingLeft: '20px' }}>
                        {wordNetwork.edges.slice(0, 10).map((edge, i) => (
                          <li key={i}>
                            {edge.source} → {edge.target} ({edge.type || 'unknown'})
                          </li>
                        ))}
