...
import { WordGraph, WordDetails } from "./index";
...
function isWordNetworkLists(network: WordNetwork): network is WordNetworkLists {
  return 'main_words' in network && 'root_words' in network;
}
...
const WordExplorer: React.FC = () => {
  ...
  const fetchWordNetworkData = useCallback(async (word: string, depth: number = 2, breadth: number = 10) => {
    try {
      console.log('Fetching network data for:', word, 'with depth:', depth, 'breadth:', breadth);
      ...
      if (!data) {
        console.error('No data received from fetchWordNetwork');
        return null;
      }

      if (isWordNetworkGraph(data)) {
        console.log('Network data is graph type with', data.nodes.length, 'nodes and', data.edges.length, 'edges');
      } else if (isWordNetworkLists(data)) {
        console.log('Network data is lists type');
      }
      ...
      <main>
        <div className="graph-container" style={{ width: '100%', height: '100%', position: 'relative' }}>
          {wordNetwork && mainWord && (
            isWordNetworkGraph(wordNetwork) && wordNetwork.nodes.length > 0 ? (
              <WordGraph
              ...
              <div className="no-graph-message" style={{
                position: 'absolute',
                top: '50%',
                left: '50%',
                ...
                <p>No graph data available for this word.</p>
                <p>Try adjusting the depth and breadth settings.</p>
                <pre style={{ 
                  marginTop: '20px', 
                  textAlign: 'left', 
