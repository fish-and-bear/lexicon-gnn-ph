...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const getNodeColor = useCallback((group: string): string => {
    // Restore original simplified palette
    switch (group) {
      ...
      nodeSelection
        .on("click", (event, d) => {
          ...
          d3.selectAll(".node").classed("selected connected", false)
            ...
            // Revert any scaling first
            .attr("transform", d => `translate(${(d as CustomNode).x ?? 0},${(d as CustomNode).y ?? 0}) scale(1)`)
            .style("opacity", 0.15); // Less extreme dimming
          d3.selectAll<SVGLineElement, CustomLink>(".link")
              .classed("highlighted", false)
              .transition().duration(200) // Remove transition name
              .attr("stroke", theme === "dark" ? "#555" : "#bbb") // Moderate dimmed stroke
              .attr("stroke-opacity", 0.1) // Less extreme dimming
        ...
        .on("mouseover", (event, d) => {
            ...
            nodeElement.select("circle")
               .transition().duration(100) // Remove transition name
               .attr("stroke-width", 2.5)
               // Restore previous hover stroke
               .attr("stroke", d3.color(getNodeColor(d.group))?.brighter(0.8).formatHex() ?? (theme === "dark" ? "#eee" : "#333"));
            nodeElement.raise();
            // Keep gentle dimming on hover
            d3.selectAll<SVGGElement, CustomNode>(".node:not(:hover)").transition().duration(100).style("opacity", 0.6); // Remove transition name
            d3.selectAll<SVGLineElement, CustomLink>(".link").transition().duration(100).attr("stroke-opacity", 0.2); // Remove transition name
        ...
        .on("mouseout", (event, d_unknown) => {
            ...
            // Revert any scaling first
            .attr("transform", d => `translate(${(d as CustomNode).x ?? 0},${(d as CustomNode).y ?? 0}) scale(1)`)
            .style("opacity", 0.15); // Restore moderate dimming
          d3.selectAll<SVGLineElement, CustomLink>(".link")
              .classed("highlighted", false)
              .transition().duration(200) // Remove transition name
              .attr("stroke", theme === "dark" ? "#555" : "#bbb") // Restore moderate dimmed stroke
              .attr("stroke-opacity", 0.1) // Restore moderate dimming
              .attr("stroke-width", 1.5);
          ...
          targetNodeElement.classed("selected", true)
            ...
            .style("opacity", 1)
            .attr("transform", `translate(${d.x ?? 0},${d.y ?? 0}) scale(1)`);
            // REMOVE explicit selected style setting here

             // Restore opacity for all nodes and links
             d3.selectAll<SVGGElement, CustomNode>(".node").transition().duration(150).style("opacity", 1); // Remove transition name
             d3.selectAll<SVGLineElement, CustomLink>(".link").transition().duration(150).attr("stroke-opacity", 0.5); // Remove transition name
        ...
        .on("dblclick", (event, d) => {
             ...
             d3.select(event.currentTarget as Element).select('circle')
                 .transition().duration(150) // Remove transition name
                 .attr("stroke-width", d.pinned ? 3 : (d.id === selectedNodeId ? 2.5 : 1))
                 .attr("stroke-dasharray", d.pinned ? "5,3" : "none")
                 // Restore previous pinned stroke logic
                 .attr("stroke", d.pinned ? getNodeColor(d.group) : (d.id === selectedNodeId ? (d3.color(getNodeColor(d.group))?.brighter(0.8).formatHex() ?? "#eee") : (d3.color(getNodeColor(d.group))?.darker(0.6).formatHex() ?? (theme === "dark" ? "rgba(255,255,255,0.1)" : "rgba(0,0,0,0.1)"))));
        });
