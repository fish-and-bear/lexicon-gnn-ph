...
const WordExplorer: React.FC = () => {
  ...
  const handleNodeClickRef = useRef<(id: string) => void>(null);
    ...
    try {
      ...
      if (randomWordCache.length > 0) {
        ...
        if (newCache.length < 5) {
          fetchRandomWordsForCache().then(words => {
            setRandomWordCache(prev => [...prev, ...words]);
          }).catch(err => {
      ...
      setError(err.message || 'Error fetching random word');
    } finally {
      setIsLoading(false);
    }
  }, [
