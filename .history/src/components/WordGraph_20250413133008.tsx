  ...
  useEffect(() => {
    ...
    // Create a more elegant single-column legend with auto-sizing
      const legendContainer = svg.append("g")
        .attr("class", "legend")
      .attr("transform", `translate(${width - 24 - 160}, 20)`); // Position matches controls-container padding

    // Define refined legend properties
    const legendPadding = 8; // Further reduced padding
    const legendItemHeight = 20; // Smaller item height
    const dotRadius = 4; // Smaller dots
    const textPadding = 14; // Less padding between dot and text
    const categorySpacing = 8; // Less space between categories
    const maxLabelWidth = 90; // Narrower labels
    ...
    const toggleRequiredWidth = toggleTextWidth + 40 + (legendPadding * 2); // Reduced extra space again
    const legendWidth = Math.max(
      maxCategoryWidth + 8, // Minimal padding around category headers
      maxTextWidth + textPadding + 6, // Minimal extra space
      toggleRequiredWidth
    ...
    const legendHeight = (totalRows * legendItemHeight) + 
                        ...
                        (legendPadding * 2) + 
                        45 + // Less extra padding for the title and instructions
                        30; // Less extra space for the checkbox option
    ...
    // Add elegant title with less horizontal space
    legendContainer.append("text")
      .attr("x", legendWidth / 2)
      .attr("y", legendPadding + 7)
      .attr("text-anchor", "middle")
      .attr("font-weight", "600")
      .attr("font-size", "10px") // Even smaller font
    ...
    legendContainer.append("text")
      .attr("x", legendWidth / 2)
      .attr("y", legendPadding + 20) // Closer to title
      .attr("text-anchor", "middle")
      .attr("font-weight", "400")
      .attr("font-size", "8px") // Smaller subtitle
    ...
    categories.forEach((category, categoryIndex) => {
      ...
      const categoryTextElement = legendContainer.append("text")
        ...
        .attr("y", yPos)
        .attr("font-weight", "600")
        .attr("font-size", "10px") // Smaller category headers
      ...
      category.types.forEach(type => {
        ...
        const textElement = entry.append("text")
          .attr("x", textPadding)
            .attr("y", 0)
          .attr("dy", ".25em")
          .attr("font-size", "9px") // Smaller item text
          .attr("font-weight", "500")
    ...
    const toggleTrack = toggleContainer.append("rect")
      .attr("width", 26) // Even smaller toggle
      .attr("height", 12) // Even smaller toggle
      .attr("rx", 6)
      .attr("x", 0)
      .attr("y", -4)
    ...
    const toggleHandle = toggleContainer.append("circle")
      .attr("r", 4) // Smaller handle
      .attr("cx", showDisconnectedNodes ? 18 : 8) // Adjusted positions
      .attr("cy", 2)
    ...
    toggleContainer.on("click", function() {
      ...
      toggleHandle
        .transition()
        .duration(250)
        .attr("cx", newValue ? 18 : 8); // Match the positions from above
    });
