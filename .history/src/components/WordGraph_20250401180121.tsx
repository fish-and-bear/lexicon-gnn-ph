...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const nodes: CustomNode[] = useMemo(() => {
    ...
    // Need links to determine group relative to mainWord
    // Note: This assumes 'links' useMemo runs before this or has stable dependencies
    // We might need to combine node/link processing if dependencies are complex
    const memoizedLinks = links; // Use the links calculated in the other useMemo
    ...
    return wordNetwork.nodes.map(node => {
      ...
      if (node.label === mainWord) {
        ...
        // Find the link connecting this node directly to the main word
      ...
      return {
        ...
        word: node.label, 
        label: node.label,
        group: calculatedGroup, // Use the newly calculated group
        info: node, 
        connections: 0,
        pinned: false
      };
    });
  // Add links and mainWord as dependencies
  }, [wordNetwork, links, mainWord]); 

  const links: CustomLink[] = useMemo(() => {
    console.log("[WordGraph] Raw wordNetwork.edges:", wordNetwork?.edges); // Log raw edges
  ...
  const setupSimulation = useCallback((nodes: CustomNode[], links: CustomLink[]) => {
    try {
      ...
      console.log("[WordGraph] Nodes passed to simulation:", JSON.stringify(nodes.map(n => n.id))); // Log only IDs for brevity
      console.log("[WordGraph] Links passed to simulation:", JSON.stringify(links.map(l => ({s: l.source, t: l.target})))); // Log only source/target for brevity
      // --- DEBUG LOGS END ---
