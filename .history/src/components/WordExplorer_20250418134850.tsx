...
import WordGraph from "./WordGraph"; // Restored WordGraph import
import WordDetails from "./WordDetails"; // Restored WordDetails import
...
// import unidecode from "unidecode"; // Step 4: Comment out utility
import { 
  fetchWordNetwork, 
  fetchWordDetails, 
  searchWords, 
...
// import DOMPurify from 'dompurify'; // Step 4: Comment out utility
// import { debounce } from "lodash"; // Step 4: Comment out utility
import Tabs from '@mui/material/Tabs'; // Step 4: Restore MUI
import Tab from '@mui/material/Tab'; // Step 4: Restore MUI
import Box from '@mui/material/Box'; // Step 4: Restore MUI
import CircularProgress from '@mui/material/CircularProgress'; // Step 4: Restore MUI
import { styled } from '@mui/material/styles'; // Step 4: Restore MUI
import NetworkControls from './NetworkControls';
import { Typography, Button } from "@mui/material"; // Step 4: Restore MUI

// Import Resizable Panels components
import { Panel, PanelGroup, PanelResizeHandle } from "react-resizable-panels";
import useMediaQuery from '@mui/material/useMediaQuery'; // Step 4: Restore MUI
...
const WordExplorer: React.FC = () => {
  ...
  // Function to normalize input (comment out unidecode usage)
  // const normalizeInput = (input: string) => unidecode(input.trim().toLowerCase()); // Step 4: Comment out usage
  const normalizeInput = (input: string) => input.trim().toLowerCase(); // Use basic normalization
  ...
  // Initialize the search function (without debounce)
  useEffect(() => {
    const searchFunction = async (query: string) => {
      ...
      console.log(`[DEBUG] Non-debounced search for: "${query}"`);
      setIsLoadingSuggestions(true);
      try {
        const searchOptions: SearchOptions = { 
          page: 1,
        ...
        // Assuming searchWords is stable (comes from useCallback or context)
        ...
        // setError( error instanceof Error ? error.message : 'Search suggestions failed'); 
      } finally {
        setIsLoadingSuggestions(false);
      }
    };
    debouncedSearchRef.current = searchFunction;
    
    // No cleanup needed for non-debounced version
    return () => {};
  // Dependencies: Include functions/state setters called inside searchFunction
  }, [searchWords, setSearchResults, setShowSuggestions, setIsLoadingSuggestions]); 

  // Wrapper function that calls the search function
  ...
  }, [mainWord, fetchWordNetworkData, normalizeInput]); // Added normalizeInput dependency
  ...
  // Render the search bar with navigation buttons (restore MUI buttons)
  const renderSearchBar = () => {
    return (
      <div className="search-container">
        <div className="nav-buttons">
          <Button // Step 4: Restore MUI Button
            className="nav-button back-button" 
            onClick={handleBack}
            disabled={currentHistoryIndex <= 0}
            title="Go back to previous word"
            variant="contained" // Use MUI Button for consistency
            sx={{ /* Restore MUI styles */ }}
          >
            <span>←</span>
          </Button>
          <Button // Step 4: Restore MUI Button
        ...
        <div className="search-input-container">
          ...
          {isLoadingSuggestions && ( // Step 4: Restore MUI CircularProgress
        ...
        <Button // Step 4: Restore MUI Button
          ...
          startIcon={isLoading ? <CircularProgress size={16} /> : null} // Step 4: Restore MUI CircularProgress
          onClick={() => inputValue.trim() && handleSearch(inputValue)}
          disabled={isLoading || !inputValue.trim()} 
          title="Search for this word"
          sx={(theme) => ({ /* Restore MUI styles */ })}
        >
        ...
        <Button // Step 4: Restore MUI Button
          ...
          startIcon={isRandomLoading ? <CircularProgress size={16} /> : null} // Step 4: Restore MUI CircularProgress
  ...
  const isWideLayout = useMediaQuery('(min-width:769px)'); // Step 4: Restore MUI usage
  ...
  console.log('>>> Full WordExplorer rendering (Utilities commented out) <<<'); // Updated log
                ...
                <div className="graph-container" style={{ width: '100%', height: '100%' }}>
                  <div className="graph-content" style={{ width: '100%', height: '100%' }}>
                    ...
                    {!isLoading && wordNetwork && (
                      <WordGraph // Restored WordGraph usage
                    ...
                    {!isLoading && !wordNetwork && !error && (
                      <div className="empty-graph">Enter a word to explore its network.</div> // Restored message
                    )}
                  </div>
                </div>
              </Panel>
              ...
              <Panel defaultSize={40} minSize={25} style={{ overflow: 'hidden', height: '100%', width: '100%' }}>
                ...
                <div ref={detailsContainerRef} className="details-container" style={{ width: '100%', height: '100%', overflow: 'auto' }}>
                  ...
                  {!isLoading && selectedWordInfo && (
                    <WordDetails // Restored WordDetails usage
                  ...
                  {!isLoading && !selectedWordInfo && (
                    <div className="no-word-selected">Select a word or search to see details.</div> // Restored message
                  )}
