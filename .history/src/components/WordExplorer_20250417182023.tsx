  ...
  const fetchWordNetworkData = useCallback(async (word: string, depth: number = 2, breadth: number = 10): Promise<WordNetwork | null> => { // Return WordNetwork | null
    try {
      ...
      const wordNetworkData: WordNetwork = {
        nodes: apiData.nodes.map((node: any): NetworkNode => ({ // Use NetworkNode type, handle ID conversion
          ...node,
          id: typeof node.id === 'string' && !isNaN(parseInt(node.id)) ? parseInt(node.id) : (typeof node.id === 'number' ? node.id : 0),
          // Ensure other fields match NetworkNode if needed
          label: node.label || node.word || '',
          word: node.word || node.label || '',
          type: node.type || 'unknown'
        })),
        edges: apiData.links.map((link: any): NetworkLink => ({ // Use NetworkLink type
          source: typeof link.source === 'object' ? (link.source.id || 0) : (link.source || 0),
          target: typeof link.target === 'object' ? (link.target.id || 0) : (link.target || 0),
          type: link.type || 'related'
        })),
        // Ensure metadata matches WordNetwork metadata structure
        metadata: {
          ...
          include_affixes: apiData.metadata?.include_affixes ?? options.include_affixes,
          include_etymology: apiData.metadata?.include_etymology ?? options.include_etymology,
          cluster_threshold: apiData.metadata?.cluster_threshold ?? options.cluster_threshold
        }
      };
    ...
    try {
      ...
      if (!fallbackToSearch && wordData) {
        ...
        try {
          ...
          // CRITICAL FIX: Update selectedWordInfo with semantic_network if direct relations are missing
          // This ensures relations will be displayed even when direct relations fields are empty
