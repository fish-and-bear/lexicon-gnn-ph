...
const WordExplorer: React.FC = () => {
  ...
  const handleSearch = useCallback(async (query: string) => {
    ...
    try {
      ...
      if (!isConnected) {
        throw new Error('Backend server is not available. Please ensure the server is running on port 10000.');
      }

      // Fetch word details
      const wordInfo = await fetchWordDetails(query);
      if (!wordInfo) {
        throw new Error(`No word information found for "${query}"`);
      }
      setSelectedWordInfo(wordInfo);

      // Fetch network data
      try {
        const networkData = await fetchWordNetwork(wordInfo.id, depth, breadth);
        if (networkData) {
          setWordNetwork(networkData);
        }
      } catch (networkError) {
        console.error('Error fetching word network:', networkError);
        // Don't throw here - we still want to show word details even if network fails
        setError('Could not load word relationships. The word details are still available.');
      }
      ...
      if (error instanceof Error) {
        if (error.message.includes('Backend server')) {
          ...
          errorMessage = error.response?.data?.message || 'Network request failed. Please try again.';
        } else {
          errorMessage = error.message;
        }
      }
