...
const WordGraphComponent: React.FC<WordGraphProps> = ({
  ...
  const nodes: CustomNode[] = useMemo(() => {
    ...
    return wordNetwork.nodes.map((node: any) => ({ // Use any temporarily if NetworkNode is incorrect
      id: String(node.id),
  ...
  const links: CustomLink[] = useMemo(() => {
    ...
    return wordNetwork.edges.map((edge: any) => ({ // Use any temporarily if edge type is incorrect
  ...
  const getNodeRadius = useCallback((node: CustomNode) => {
    if (node.word === mainWord) return 15;
