...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const setupNodeInteractions = useCallback((
      ...
      nodeSelection.on("mouseenter", function(event, d) {
          // ADD CHECK: Also check if node position is fixed (dragging or pinned)
          if (isDraggingRef.current || d.fx !== null) return;
      ...
      nodeSelection.on("mouseleave", function(event, d) {
          // Check if node position is fixed (dragging or pinned)
          if (isDraggingRef.current || d.fx !== null) return;
          
          // Reset ONLY this node's visual emphasis
          d3.select(this)
            .style("opacity", d.id === mainWord ? 1 : 0.8) // Explicitly reset opacity for this node
            .select("circle")
              .attr("stroke-width", d.id === mainWord ? 2.5 : 1.5) // Use node data 'd' for correct stroke
              .attr("stroke-opacity", 1) // Reset to 1 (fully opaque stroke)
              .attr("filter", null);
          
          // Reset all links (keep this)
          d3.selectAll<SVGLineElement, CustomLink>(".link")
            .attr("stroke-opacity", 0.6)
            .attr("stroke-width", 1.5)
            .attr("stroke", themeMode === "dark" ? "#666" : "#ccc");
          
          // Reset connected node styles (Still reset others, but ensure this node isn't re-affected if timing is weird)
          d3.selectAll<SVGGElement, CustomNode>(".node")
            .filter(n => n.id !== d.id) // Filter out the current node just in case
            .style("opacity", n => n.id === mainWord ? 1 : 0.8) // Reset opacity for others
            .select("circle")
              .attr("stroke-width", n => n.id === mainWord ? 2.5 : 1.5)
              .attr("stroke-opacity", 0.7); // Reset stroke for others
              
          // Reset node labels (keep this)
          d3.selectAll<SVGTextElement, CustomNode>(".node-label")
            .style("opacity", 0.9)
