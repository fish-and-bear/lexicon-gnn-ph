...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({
  ...
  // Add this line to log the received pronunciation data:
  console.log("WordDetails received pronunciations:", JSON.stringify(wordInfo.pronunciations, null, 2)); 
  ...
  const renderHeader = () => {
    ...
    return (
      ...
      <Box sx={{ 
        ...
        {/* Pronunciations Section - Updated to handle all types */}
        <Stack spacing={1.5} sx={{ mb: theme.spacing(2) }}>
            ...
            <Box sx={{ mt: 1 }}>
              <Stack direction="row" spacing={1.5} useFlexGap flexWrap="wrap" alignItems="center">
                {wordInfo.pronunciations.map((pron, index) => {
                  // Skip audio type visually here (handled by button)
                  if (pron.type === 'audio') {
                    return null;
                  }
                  
                  // Determine if type is IPA (case-insensitive check)
                  const isIPA = pron.type?.toLowerCase() === 'ipa';
                  const displayValue = isIPA ? `/${pron.value}/` : pron.value;
                  const displayType = pron.type || 'Pronunciation'; // Default label if type is missing
                  ...
                  return (
                    <Box 
                      ...
                      sx={{ 
                        ...
                        px: 1.5, 
                        py: 0.75, // Slightly more padding
                        bgcolor: isDarkMode ? alpha(headerTextColor, 0.06) : alpha(theme.palette.text.primary, 0.04), // Subtle background
                        borderRadius: '6px', // Slightly more rounded
                        border: `1px solid ${isDarkMode ? alpha(headerTextColor, 0.1) : alpha(theme.palette.divider, 0.8)}` // Subtle border
                      }}
                    >
                      <Typography 
                        variant="caption" 
                        sx={{ 
                          color: alpha(headerTextColor, 0.75), 
                          fontWeight: 500, 
                          mr: 0.75, // Slightly more margin
                          whiteSpace: 'nowrap' // Prevent type label from wrapping awkwardly
                        }}
                      >
                        {displayType}:
                      </Typography>
                      <Typography 
                        variant="body2" 
                        sx={{ 
                          fontStyle: isIPA ? 'italic' : 'normal',
                          fontWeight: 500, // Consistent weight
                          color: headerTextColor, // Use header text color for value too
                          // Handle potential long/corrupt values gracefully
                          wordBreak: 'break-all', 
                          whiteSpace: 'pre-wrap'
                        }}
                      >
             ...
             // Optional: Render something if no pronunciations exist
             <Typography variant="body2" sx={{ color: alpha(headerTextColor, 0.6), fontStyle: 'italic' }}>
