  ...
  const handleNodeClick = useCallback(async (word: string) => {
    ...
    try {
      ...
      try {
        ...
        // Ensure we have relations data
        console.log("Checking for relations data:", 
          wordData.incoming_relations?.length || 0, "incoming,", 
          wordData.outgoing_relations?.length || 0, "outgoing");
        
        // If relations are missing, we'll populate them from the network data
        const hasRelations = 
        ...
        if (!hasRelations) {
          console.log("Relations data missing, will populate from network data");
        }
      } catch (error: any) {
