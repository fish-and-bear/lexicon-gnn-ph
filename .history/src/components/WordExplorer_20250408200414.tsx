...
const WordExplorer: React.FC = () => {
  ...
  // Add a ref for the main container to get its dimensions
  ...
  const handleMouseMove = useCallback((e: MouseEvent) => {
    ...
    const mainElement = detailsContainerRef.current.parentElement;
    if (!mainElement) return;
    
    const mainRect = mainElement.getBoundingClientRect();
    const mainWidth = mainRect.width;
    
    // Calculate new width based on mouse position relative to the main container
    // Ensure the panel width is between 20% and 80% of the main container
    const newWidth = Math.max(
      Math.min(mainWidth * 0.8, mainWidth - (e.clientX - mainRect.left)),
      mainWidth * 0.2
    );
    
    // Apply the new width to the details container
    detailsContainerRef.current.style.width = `${newWidth}px`;
    
    // Update the resizer position
    const resizerElement = mainElement.querySelector('.panel-resizer') as HTMLElement;
    if (resizerElement) {
      resizerElement.style.left = `${mainWidth - newWidth}px`;
    }
  ...
  // Add this useEffect to position the resizer initially and on resize
  useEffect(() => {
    const updateResizerPosition = () => {
      ...
      if (resizerElement && detailsContainerRef.current) {
        // Position the resizer at the boundary between the two panels
        resizerElement.style.left = `${mainRect.width - detailsWidth}px`;
      }
    };
    ...
    // Add window resize listener to update position when window size changes
  ...
  const handleSuggestionClick = useCallback((suggestion: SearchWordResult) => {
    ...
    setShowSuggestions(false);
    loadWordData(suggestion.id);
  }, []);

  // Handle network depth and breadth changes
  ...
  }, [mainWord, fetchWordNetworkData, normalizeInput]);
  ...
  // In useEffect
  useEffect(() => {
    // Get reference to the main container for resizer positioning
    if (detailsContainerRef.current) {
      mainContainerRef.current = detailsContainerRef.current.parentElement;
    }
  }, []);
      ...
      <main>
        <div className="graph-container">
          ...
          <div className="graph-content">
            {isLoading && <Loader message="Loading word data..." />}
            ...
            {!isLoading && !error && wordNetwork && (
              <WordGraph
                wordNetwork={{
                  ...wordNetwork,
                  edges: wordNetwork.edges.map(edge => ({
                    ...edge,
        ...
        {/* Resizer element - positioned absolutely */}
        <div 
        ...
        <div className="details-container" ref={detailsContainerRef}>
          {selectedWordInfo && (
            <WordDetails
              ...
              onEtymologyNodeClick={(node) => {
                console.log("Etymology node clicked:", node);
                if (node && node.word && node.word !== mainWord) {
                  handleSearch(node.word);
                }
              }}
            />
          )}
          ...
          {!selectedWordInfo && !isLoading && (
            <div className="no-word-selected">
              <p>Search for a word to see details</p>
              <button
                onClick={async () => {
                  setIsLoading(true);
                  setError(null);
