...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({
  ...
  const renderRelationsTab = () => {
    ...
    // Get direct access to the graph data from props
    const wordNetworkFromProps = wordNetwork;
    
    console.log("WORD NETWORK FROM PROPS:", wordNetworkFromProps);
    
    // Check if we have visible graph data in the component that might not be fully reflected in the wordInfo
    const hasVisibleNetwork = Boolean(
      wordNetworkFromProps && 
      ((wordNetworkFromProps.nodes && wordNetworkFromProps.nodes.length > 0) && 
       (wordNetworkFromProps.edges && wordNetworkFromProps.edges.length > 0))
    );
    ...
    if (!hasAnyRelations && wordNetworkFromProps) {
