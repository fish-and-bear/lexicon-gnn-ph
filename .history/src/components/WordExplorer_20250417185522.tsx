      ...
      if (data && data.nodes && data.edges) {
        ...
        if (wordData && wordData.id) {
          ...
          if (mainNode) {
            ...
            // Update both wordData and selectedWordInfo with relations
            // This ensures the relations are visible in the UI regardless of how the word was accessed
            if (incomingRelations.length > 0 || outgoingRelations.length > 0) {
              // Update wordData (for consistency)
              ...
              // IMPORTANT FIX: Also update selectedWordInfo with the network relations
              // This ensures relations are visible in the UI regardless of navigation method
              setSelectedWordInfo(prevInfo => {
                if (!prevInfo) return prevInfo;
                return {
                  ...prevInfo,
                  incoming_relations: incomingRelations.length > 0 ? incomingRelations : prevInfo.incoming_relations,
                  outgoing_relations: outgoingRelations.length > 0 ? outgoingRelations : prevInfo.outgoing_relations
                };
              });
            }
          }
          ...
          try {
            ...
            if (networkData && networkData.nodes && networkData.edges) {
              ...
              if (mainNode) {
                    ...
                    if (targetNode.id === mainNode.id) {
                      incomingRelations.push({
                        ...
                        source_word: {
                          id: Number(sourceNode.id) || 0,
                          lemma: sourceNode.word || sourceNode.label,
                          language_code: sourceNode.language_code,
                          has_baybayin: sourceNode.has_baybayin,
                          baybayin_form: sourceNode.baybayin_form
                        }
                      });
                      ...
                      outgoingRelations.push({
                        ...
                        target_word: {
                          id: Number(targetNode.id) || 0,
                          lemma: targetNode.word || targetNode.label,
                          language_code: targetNode.language_code,
                          has_baybayin: targetNode.has_baybayin,
                          baybayin_form: targetNode.baybayin_form
                        }
                      });
                ...
                // Update selectedWordInfo with network-derived relations
                // This ensures relations are visible in the UI
                if (incomingRelations.length > 0 || outgoingRelations.length > 0) {
                  setSelectedWordInfo(prevInfo => {
                    if (!prevInfo) return prevInfo;
                    return {
        ...
        try {
          ...
          fetchWordNetworkData(wordData.lemma, depth, breadth)
            .then(networkData => {
              ...
              // Extract and sync relations from network data to ensure they show in UI
              if (networkData && networkData.nodes && networkData.edges) {
                const mainNode = networkData.nodes.find(node => 
                  node.type === 'main' || node.word === wordData.lemma || node.label === wordData.lemma
                );
                ...
                if (!wordInfo.outgoing_relations?.length && !wordInfo.incoming_relations?.length) {
                  ...
                  if (networkData && networkData.nodes && networkData.edges) {
                    ...
                    if (mainNode) {
                      ...
                      // Update selectedWordInfo with the extracted relations
                      if (incomingRelations.length > 0 || outgoingRelations.length > 0) {
                        setSelectedWordInfo(prevInfo => {
                          if (!prevInfo) return prevInfo;
                          return {
