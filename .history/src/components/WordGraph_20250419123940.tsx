...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  // Add handler for toggling relationship filters
  const handleToggleRelationshipFilter = useCallback((relType: string) => {
    const lowerRelType = relType.toLowerCase();
    setFilteredRelationships(prev => {
      // If already in the array, remove it (unfilter)
      if (prev.includes(lowerRelType)) {
        return prev.filter(type => type !== lowerRelType);
      } 
      // Otherwise add it to the array (filter it out)
      return [...prev, lowerRelType];
    });
  }, []);
  ...
  // Define getNodeRadius before it's used in dependency arrays
  ...
  // Now use it in other functions
  const transitionToNewMainWord = useCallback((node: CustomNode) => {
    if (!nodeRef.current || !containerRef.current || !svgRef.current || isTransitioning) return;
    setIsTransitioning(true);
    setTransitionOverlayVisible(true);
    
    // Log node ID formatting for debugging
    const nodeId = node.id.toString();
    console.log(`Double-clicked on node: ${nodeId} (${typeof nodeId})`);
    console.log(`Is numeric: ${!isNaN(Number(nodeId)) && nodeId.trim() !== ''}`);
    
    // Calculate the new center position
    const containerWidth = containerRef.current.clientWidth;
    const containerHeight = containerRef.current.clientHeight;
    const dx = containerWidth / 2 - node.x!;
    const dy = containerHeight / 2 - node.y!;
    
    // Create a transition for the zoom and translation
    d3.select(svgRef.current)
      ...
      .on('end', () => {
        setTimeout(() => {
          ...
          // For numeric IDs, we navigate using the numeric ID
          // For non-numeric IDs, we use the word text itself
          if (!isNaN(Number(nodeId)) && nodeId.trim() !== '') {
            console.log(`Navigating to ID: ${nodeId}`);
            onNodeDoubleClick(Number(nodeId));
          } else {
            console.log(`Navigating to text: ${node.word}`);
            onNodeDoubleClick(node.word);
          }
        }, 100);
      });
  ...
  }, [isTransitioning, onNodeDoubleClick, getNodeRadius]);
  ...
  const setupNodeInteractions = useCallback((
      ...
      nodeSelection
        .on("click", (event, d) => {
          ...
          // It's a single click - set timer to handle after delay
  ...
  }, [selectedNodeId, onNodeClick, getNodeRadius, getNodeColor, theme, nodeMap, onNodeSelect, handleNodeDoubleClick, tooltipTimeoutId]);
...
const getRelationshipTypeLabel = (type: string): { group: string, label: string } => {
  ...
  return (
    <div className="wordgraph-container" style={{ height: "100%", width: "100%", position: "relative" }}>
      <svg
        ref={svgRef}
        width="100%"
        height="100%"
        className={`wordgraph-svg ${theme === "dark" ? "dark-theme" : "light-theme"}`}
      >
        <defs>
          <marker
            id="arrowhead"
