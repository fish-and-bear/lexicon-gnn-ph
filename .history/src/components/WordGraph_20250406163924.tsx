...
const WordGraph: React.FC<WordGraphProps> = ({
      ...
      try {
        ...
        if (color) {
          if (colorIntensity > 100) {
            ...
            const brightenFactor = (colorIntensity - 100) / 50;
            return color.brighter(brightenFactor).formatHex();
          } else {
            // Decrease brightness for values < 100%
            const darkenFactor = (100 - colorIntensity) / 100;
  ...
  useEffect(() => {
    ...
    const containerWidth = width || 300; // Fallback to 300 if width prop is not provided
  ...
  }, [filteredNodes, filteredLinks, getNodeColor, getNodeRadius, mainWord, theme, onNodeClick, width]);
