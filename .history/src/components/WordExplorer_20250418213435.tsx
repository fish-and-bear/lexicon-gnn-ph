...
import { WordNetwork, WordInfo, SearchOptions, EtymologyTree, Statistics, Definition, SearchWordResult, Relation } from "../types";
...
const WordExplorer: React.FC = () => {
  ...
  const [networkBreadth, setNetworkBreadth] = useState(10);
  const [stats, setStats] = useState<Statistics | null>(null);
  const [isLoadingStats, setIsLoadingStats] = useState(false);
  const [statsError, setStatsError] = useState<string | null>(null);
  const [selectedNode, setSelectedNode] = useState<string | null>(null);
  const [affixes, setAffixes] = useState<any[]>([]);
  const [isLoadingAffixes, setIsLoadingAffixes] = useState(false);
  const [affixesError, setAffixesError] = useState<string | null>(null);
