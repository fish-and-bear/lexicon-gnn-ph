...
import { WordNetworkResponse as WordNetwork, WordNetworkNode, WordNetworkEdge } from "../types";
...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const baseLinks = useMemo<{ source: string; target: string; relationship: string }[]>(() => {
    ...
    return wordNetwork.edges
      .map((edge: WordNetworkEdge) => {
        const sourceNode = wordNetwork.nodes.find((n: WordNetworkNode) => n.id === edge.source);
        const targetNode = wordNetwork.nodes.find((n: WordNetworkNode) => n.id === edge.target);

        if (!sourceNode || !targetNode || !sourceNode.label || !targetNode.label) {
          console.warn(`Could not find nodes or labels for edge: ${edge.source} -> ${edge.target}`);
          return null;
        }
  ...
  const baseNodes = useMemo<CustomNode[]>(() => {
    ...
    const mappedNodes = wordNetwork.nodes.map((node: WordNetworkNode): CustomNode => {
      ...
      const connections = baseLinks.filter((l: { source: string; target: string; relationship: string; }) => l.source === node.label || l.target === node.label).length;
  ...
  }, [baseNodes, baseLinks, mainWord, depth]);
