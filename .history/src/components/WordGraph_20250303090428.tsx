...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const getNodeColor = useCallback((node: CustomNode | string): string => {
    // If node is a CustomNode object, get the group from it
  ...
  const createDragBehavior = useCallback((simulation: d3.Simulation<CustomNode, CustomLink>) => {
    ...
    return d3.drag<SVGGElement, CustomNode>()
      .on("start", (event, d) => {
        ...
        // Make sure we have a valid DOM element before trying to use classed
        const element = event.sourceEvent.currentTarget;
        if (element && element instanceof Element) {
          // Find the closest node element
          let nodeElement = element;
          while (nodeElement && !nodeElement.classList.contains('node') && nodeElement.parentElement) {
      ...
      .on("end", (event, d) => {
        ...
        }, 200); // Slightly longer timeout to ensure no accidental clicks
      });
  }, []);
  ...
  const createNodes = useCallback((g: d3.Selection<SVGGElement, unknown, null, undefined>, simulation: d3.Simulation<CustomNode, CustomLink> | null) => {
    ...
    // Add text labels, positioned BELOW the nodes
    nodeGroups.append("text")
      .attr("dy", d => getNodeRadius(d) + 14) // Position text below node
      .attr("text-anchor", "middle")
      .text(d => d.word)
      .attr("font-size", d => `${Math.min(14, 10 + getNodeRadius(d) / 2)}px`)
  ...
  // Fix the updateGraph function return type to properly handle cleanup
  const updateGraph = useCallback((): (() => void) | void => {
    try {
      if (!svgRef.current || !containerRef.current || !isValidNetwork) return;
      ...
      const simulation = d3.forceSimulation(filteredNodes)
        ...
        .force("collide", d3.forceCollide().radius(d => getNodeRadius(d as CustomNode) * 1.5));
      
      // Create simple drag behavior
      ...
      // Fix main word at center initially
      const mainNode = filteredNodes.find(n => n.id === mainWord);
      if (mainNode) {
        mainNode.fx = 0;
        mainNode.fy = 0;
        // Release after stabilization
        setTimeout(() => {
      ...
      setError("Failed to update graph. Please try again.");
      return undefined;
    }
  }, [
    svgRef, 
  ...
  useEffect(() => {
    ...
    const cleanup: (() => void) | void = updateGraph();
  ...
  // Update the useEffect that handles mainWord changes to make transitions smoother
  useEffect(() => {
    setSelectedNodeId(mainWord);
    
    // When main word changes, initiate a smooth transition
    if (svgRef.current && prevMainWordRef.current && prevMainWordRef.current !== mainWord) {
      isTransitioningRef.current = true;
      
      // Add a loading state while transitioning
      setIsLoading(true);
    ...
    // Store current main word for future reference
    prevMainWordRef.current = mainWord;
  }, [mainWord, updateGraph]);
  ...
  return (
    <div className="word-graph-container" style={{ width: '100%', height: '100%', position: 'relative' }}>
      ...
      <div className="controls-container" style={{
        ...
        alignItems: 'center',
        padding: '15px',
        backgroundColor: theme === 'dark' ? 'rgba(30, 30, 30, 0.9)' : 'rgba(240, 240, 240, 0.9)',
        borderRadius: '12px',
