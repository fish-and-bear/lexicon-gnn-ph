...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const mapRelationshipToGroup = useCallback((relationship?: string): string => {
    ...
    if (relationship.includes("synonym")) return "synonym";
    if (relationship.includes("antonym")) return "antonym";
    if (relationship.includes("hypernym")) return "hypernym";
    if (relationship.includes("hyponym")) return "hyponym";
    if (relationship.includes("root")) return "root";
    if (relationship.includes("derived")) return "derived";
    if (relationship.includes("related")) return "related";
  ...
  const getNodeColor = useCallback((group: string): string => {
    ...
    return colorMap[group] || colorMap.associated;
  }, []);
  ...
  // Create a more direct mapping from API data to D3 nodes/links
  const filteredNodes = useMemo<CustomNode[]>(() => {
    if (!mainWord || !rawNodes || rawNodes.length === 0) {
      console.log("Cannot create nodes - missing data", { mainWord, rawNodesLength: rawNodes?.length || 0 });
      return [];
    }
    
    // Directly map API nodes to D3 nodes without complex filtering
    return rawNodes.map(node => {
      // Determine if this is the main word
      const isMainWord = node.lemma === mainWord;
      
      // Determine node group (main node or first-level connection)
      let nodeGroup = 'associated';
      if (isMainWord) {
        nodeGroup = 'main';
      } else {
        // Check if there's a direct connection to the main word
        const mainWordNode = rawNodes.find(n => n.lemma === mainWord);
        if (mainWordNode) {
          const hasDirectConnection = rawLinks.some(link => 
            (link.source === mainWordNode.id && link.target === node.id) ||
            (link.source === node.id && link.target === mainWordNode.id)
          );
          
          if (hasDirectConnection) {
            const connectionType = rawLinks.find(link => 
              (link.source === mainWordNode.id && link.target === node.id) ||
              (link.source === node.id && link.target === mainWordNode.id)
            )?.type || '';
            
            nodeGroup = mapRelationshipToGroup(connectionType);
          }
        }
      }
      
      return {
        id: node.id.toString(),  // Use numeric ID as string
        word: node.lemma,
        label: node.lemma,
        group: nodeGroup,
        connections: rawLinks.filter(link => 
  ...
  }, [rawNodes, rawLinks, mainWord, mapRelationshipToGroup]);
  
  // Create links directly from API data
  const filteredLinks = useMemo<CustomLink[]>(() => {
    if (!rawLinks || rawLinks.length === 0 || !rawNodes || rawNodes.length === 0) {
      console.log("Cannot create links - missing data", { 
        rawLinksLength: rawLinks?.length || 0,
        rawNodesLength: rawNodes?.length || 0
      });
      return [];
    }
    
    // Map from node ID to node object for quick lookups
    const idToNodeMap = new Map(filteredNodes.map(node => [parseInt(node.id), node]));
    
    return rawLinks.map(link => {
      const sourceNode = idToNodeMap.get(link.source);
      const targetNode = idToNodeMap.get(link.target);
  ...
  }, [rawLinks, rawNodes, filteredNodes]);
  ...
  // Move renderFallbackGraph before the main rendering effect
  ...
  // Define drag and tick functions outside the useEffect
  const dragStarted = useCallback((event: any, d: CustomNode) => {
  ...
  const dragged = useCallback((event: any, d: CustomNode) => {
    d.fx = event.x;
    d.fy = event.y;
  }, []);

  const dragEnded = useCallback((event: any, d: CustomNode) => {
  ...
  const tickFunction = useCallback(() => {
    if (!svgRef.current) return;
    
    const svg = d3.select(svgRef.current);
    const links = svg.selectAll<SVGLineElement, CustomLink>(".link");
    const nodes = svg.selectAll<SVGGElement, CustomNode>(".node");
  ...
  useEffect(() => {
    ...
    if (filteredNodes.length === 0) {
      console.warn("No filtered nodes available, showing fallback graph");
      renderFallbackGraph();
      return;
    }
    ...
    try {
      ...
      setError(`Error rendering graph: ${e instanceof Error ? e.message : 'Unknown error'}`);
      renderFallbackGraph();
    }
  ...
  }, [filteredNodes, filteredLinks, getNodeColor, getNodeRadius, mainWord, theme, onNodeClick, centerOnMainWord, depth, breadth, renderFallbackGraph, dragStarted, dragged, dragEnded, tickFunction]);
  ...
  // Helper function to handle source/target which could be string or CustomNode
  const getNodeId = useCallback((node: string | CustomNode): string => {
    if (typeof node === 'string') return node;
    return node.id;
  }, []);
  
  // Update findHiddenConnections to handle source/target which could be string or CustomNode
  const findHiddenConnections = useCallback((sourceId: string, depth = 1): string[] => {
    if (depth <= 0 || !sourceId) return [];
    
    // Create a map for quick lookups
    const idToNodeMap = new Map(filteredNodes.map(n => [n.id, n]));
    const hiddenConnections: string[] = [];
    
    // Find all direct connections to this node
    const directConnections = filteredLinks.filter(link => 
      getNodeId(link.source) === sourceId || getNodeId(link.target) === sourceId
    ).map(link => getNodeId(link.source) === sourceId ? getNodeId(link.target) : getNodeId(link.source));
    
    // Then for each connection, find its connections (excluding the original source)
    directConnections.forEach(targetId => {
      const secondaryConnections = filteredLinks.filter(link => 
        (getNodeId(link.source) === targetId || getNodeId(link.target) === targetId) && 
        getNodeId(link.source) !== sourceId && 
        getNodeId(link.target) !== sourceId
      ).map(link => getNodeId(link.source) === targetId ? getNodeId(link.target) : getNodeId(link.source));
      
      // Add to the hidden connections list
      secondaryConnections.forEach(id => {
        if (!hiddenConnections.includes(id)) {
          hiddenConnections.push(id);
        }
      });
    });
    
    return hiddenConnections;
  }, [filteredLinks, filteredNodes, getNodeId]);

  // Create a memoized map of nodes for lookups
  const idToNodeMap = useMemo(() => {
    return new Map(filteredNodes.map(node => [node.id, node]));
  }, [filteredNodes]);

  // Function to update hover state when user hovers over a node
  const updateNodeHover = useCallback((d: CustomNode | null) => {
    const uniqueId = d ? `${d.id}_${Date.now()}` : null;
    ...
    if (!d || !showTooltip || isDraggingRef.current) {
      setHoveredNode(null);
      return;
    }
    ...
    if (d) {
      ...
      // Find additional info for tooltip
      if (filteredLinks && filteredLinks.length > 0) {
        const connectedLinks = filteredLinks.filter(link => 
          getNodeId(link.source) === d.id || getNodeId(link.target) === d.id
        );
        
        connectedLinks.forEach(link => {
          const sourceId = getNodeId(link.source);
          const targetId = getNodeId(link.target);
          const neighbourNode = sourceId === d.id ? idToNodeMap.get(targetId) : idToNodeMap.get(sourceId);
          // Add additional info to tooltip if needed
        });
      }
    }
  }, [filteredLinks, showTooltip, tooltipTimeoutId, idToNodeMap, getNodeId]);

  // Function to handle node click
  const handleNodeClick = useCallback((d: CustomNode) => {
    // Only handle click if we're not dragging
    if (isDraggingRef.current) return;
    
    // Detect double-click by measuring time between clicks
    const now = Date.now();
    if (now - lastClickTimeRef.current < 300 && selectedNodeId === d.id) {
      // Double click - reset selected node
      setSelectedNodeId(null);
    } else {
      // Single click - set selected node
      setSelectedNodeId(d.id);
      
      // If it's not the main word, notify parent
      if (d.id !== mainWord) {
        onNodeClick(d.word);
      }
    }
    lastClickTimeRef.current = now;
  }, [selectedNodeId, onNodeClick, mainWord]);
