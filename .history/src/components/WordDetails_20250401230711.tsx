    ...
    return (
      <Box sx={{ mb: 3, borderBottom: 1, borderColor: 'divider', pb: 2 }}>
        <Stack direction="row" spacing={1} alignItems="center" sx={{ mb: 1 }}>
           ...
           {hasAudio && (
             <IconButton onClick={playAudio} size="small" title={isAudioPlaying ? "Stop Audio" : "Play Audio"}>
               {isAudioPlaying ? <StopCircleIcon fontSize="inherit"/> : <VolumeUpIcon fontSize="inherit" />}
             </IconButton>
           )}
        </Stack>
        {ipaPronunciation && (
