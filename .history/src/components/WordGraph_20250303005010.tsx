...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  // Update setupZoom to include initial centering
  ...
  const setupSimulation = useCallback((nodes: CustomNode[], links: CustomLink[]) => {
      ...
      .on('tick', () => {
        ...
        d3.select(svgRef.current)
          ...
          .attr('transform', d => {
            ...
            const margin = 20;
            const x = Math.max(margin, Math.min(containerWidth - margin, d.x || centerX));
            const y = Math.max(margin, Math.min(containerHeight - margin, d.y || centerY));
        ...
        d3.select(svgRef.current)
          ...
          .attr('x1', d => {
            ...
            return source ? source.x || centerX : centerX;
          })
          .attr('y1', d => {
            const source = typeof d.source === 'object' ? d.source : 
              nodes.find(n => n.id === d.source);
            return source ? source.y || centerY : centerY;
          })
          .attr('x2', d => {
            const target = typeof d.target === 'object' ? d.target : 
              nodes.find(n => n.id === d.target);
            return target ? target.x || centerX : centerX;
          })
          .attr('y2', d => {
            const target = typeof d.target === 'object' ? d.target : 
              nodes.find(n => n.id === d.target);
            return target ? target.y || centerY : centerY;
          });
      });
    ...
    if (mainWordNode) {
      mainWordNode.fx = centerX;
      mainWordNode.fy = centerY;
      
      // Release the fixed position after a short delay to allow natural layout
      setTimeout(() => {
        if (mainWordNode) {
  ...
  const updateGraph = useCallback(() => {
    ...
    try {
      ...
      // Note: We don't need this tick handler since we've defined it in setupSimulation
      // But we'll keep it to ensure backward compatibility
      sim.on("tick", () => {
        link
