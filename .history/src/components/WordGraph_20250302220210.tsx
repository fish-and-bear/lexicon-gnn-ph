...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const getNodeRelation = useCallback((word: string, node: NetworkNode): string => {
    ...
    // Direct node type check - more extensive check for all possible types
    if (node.type === 'root') return "root";
    if (node.type === 'root_of') return "root_of";
    if (node.type === 'synonym') return "synonym";
    if (node.type === 'antonym') return "antonym";
    if (node.type === 'derived' || node.type === 'derived_from') return "derived";
    if (node.type === 'variant') return "variant";
    if (node.type === 'related') return "related";
    if (node.type === 'kaugnay') return "kaugnay";
    if (node.type === 'component_of') return "component_of";
    if (node.type === 'cognate') return "cognate";
    if (node.type === 'etymology') return "etymology";
    if (node.type === 'associated') return "associated";
    ...
    // Check if the node is in any of the clusters
    if (wordNetwork.clusters) {
      for (const [clusterType, nodes] of Object.entries(wordNetwork.clusters)) {
        if (Array.isArray(nodes) && nodes.some(n => {
          // Check if n is an object with a word property
          return typeof n === 'object' && n !== null && 'word' in n && n.word === word;
        })) {
          switch (clusterType.toLowerCase()) {
            case 'synonyms': return "synonym";
            case 'antonyms': return "antonym";
  ...
  const getNodeColor = useCallback((group: string): string => {
    switch (group) {
      ...
      case "related":
        return "#6c757d"; // Gray
      case "kaugnay":
        return "#e9c46a"; // Light gold (was brown-orange)
      case "component_of":
        return "#ffd166"; // Lighter amber (was #ff9f1c)
      case "cognate":
        return "#9d4edd"; // Lighter violet (was #7209b7)
      case "etymology":
        return "#f72585"; // Bright pink
      case "associated":
  ...
  const renderLegend = () => {
    const relationTypes = [
      { type: 'main', label: 'Main Word', color: getNodeColor('main') },
      { type: 'root', label: 'Root Word', color: getNodeColor('root') },
      { type: 'root_of', label: 'Root Of', color: getNodeColor('root_of') },
      { type: 'synonym', label: 'Synonym', color: getNodeColor('synonym') },
      { type: 'antonym', label: 'Antonym', color: getNodeColor('antonym') },
      { type: 'derived', label: 'Derived', color: getNodeColor('derived') },
      { type: 'variant', label: 'Variant', color: getNodeColor('variant') },
      { type: 'related', label: 'Related', color: getNodeColor('related') },
      { type: 'kaugnay', label: 'Kaugnay', color: getNodeColor('kaugnay') },
      { type: 'component_of', label: 'Component Of', color: getNodeColor('component_of') },
      { type: 'cognate', label: 'Cognate', color: getNodeColor('cognate') },
      { type: 'etymology', label: 'Etymology', color: getNodeColor('etymology') },
      { type: 'associated', label: 'Associated', color: getNodeColor('associated') },
      { type: 'other', label: 'Other', color: getNodeColor('other') },
    ];

    return (
      <div className="graph-legend">
        <div className="legend-title">Word Relationships</div>
