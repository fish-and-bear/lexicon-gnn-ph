...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  // Utility function to get proper screen coordinates from SVG coordinates
  const getSvgToScreenCoordinates = useCallback((x: number, y: number): {x: number, y: number} => {
  ...
  const setupSimulation = useCallback((nodes: CustomNode[], links: CustomLink[]) => {
    try {
      ...
      const simulation = d3.forceSimulation<CustomNode>()
        .nodes(nodes)
        .alphaDecay(0.01)      // Lower for more gradual settling
        .velocityDecay(0.45)   // Lower for more momentum
        .force("link", d3.forceLink<CustomNode, CustomLink>()
          ...
          .distance(link => {
            ...
            switch (link.relationship) {
              ...
              case "derived": return 120;
              default: return 130;
            }
          })
          .strength(0.5))     // Moderate link strength
        .force("charge", d3.forceManyBody<CustomNode>()
          .strength(d => d.id === mainWord ? -300 : -150)  // Stronger repulsion for main word
          .distanceMax(350)   // Increased distance for better spread
          .theta(0.8))        // Accuracy/performance tradeoff
        .force("collide", d3.forceCollide<CustomNode>()
          .radius(d => getNodeRadius(d) * 1.2) // Add more padding
          .strength(0.7)
          .iterations(3))     // More iterations for better collision detection
        .force("center", d3.forceCenter(0, 0).strength(0.1)) // Gentle centering
        .force("x", d3.forceX(0).strength(0.05)) // Gentle force toward center
        .force("y", d3.forceY(0).strength(0.05));

      // Pin the main word more firmly
      const mainWordNode = nodes.find(n => n.id === mainWord);
      if (mainWordNode) {
        mainWordNode.fx = 0;
        mainWordNode.fy = 0;
        mainWordNode.pinned = true;
      }
      ...
      // Add an initial alpha decay schedule for better early positioning
      simulation.alpha(1).alphaTarget(0).restart();
      
      // Additional heat schedule for better initial layout
      setTimeout(() => {
  ...
  const createDragBehavior = useCallback((simulation: d3.Simulation<CustomNode, CustomLink>) => {
    ...
    return d3.drag<SVGGElement, CustomNode>()
      .on("start", (event, d) => {
        try {
          // Don't drag the main word if it's pinned
          if (d.id === mainWord && d.pinned) return;
          ...
          // Get the exact position from the event, not the data
          const draggedNode = d3.select(event.sourceEvent.currentTarget)
            .classed("dragging", true);
          
          // Bring the node to the front while dragging for better visibility
          const parent = d3.select(event.sourceEvent.currentTarget.parentNode);
          parent.node()?.appendChild(event.sourceEvent.currentTarget);
          
          // Visual feedback for dragging
          draggedNode.select("circle")
            .transition()
            .duration(100)
            .attr("r", getNodeRadius(d) * 1.1)
            .attr("stroke-width", 2);
          
          // Highlight connected links with a cleaner approach
          d3.selectAll(".link").each(function() {
            const linkData = d3.select(this).datum() as CustomLink;
            if (!linkData) return;
      ...
      .on("drag", (event, d) => {
        try {
          ...
          // Instead of letting simulation update, we can directly update for immediate feedback
          const node = d3.select(event.sourceEvent.currentTarget)
            .attr("transform", `translate(${event.x}, ${event.y})`);
          
          // Update links connected to this node for immediate feedback
          d3.selectAll(".connected-link").each(function() {
            const linkData = d3.select(this).datum() as CustomLink;
            if (!linkData) return;
            
            if (typeof linkData.source === 'object' && linkData.source?.id === d.id) {
              linkData.source.x = event.x;
              linkData.source.y = event.y;
            }
            
            if (typeof linkData.target === 'object' && linkData.target?.id === d.id) {
              linkData.target.x = event.x;
              linkData.target.y = event.y;
            }
            
            // Update the link position directly
            d3.select(this)
              .attr("x1", () => typeof linkData.source === 'object' ? linkData.source.x || 0 : 0)
              .attr("y1", () => typeof linkData.source === 'object' ? linkData.source.y || 0 : 0)
              .attr("x2", () => typeof linkData.target === 'object' ? linkData.target.x || 0 : 0)
              .attr("y2", () => typeof linkData.target === 'object' ? linkData.target.y || 0 : 0);
          });
        } catch (error) {
      ...
      .on("end", (event, d) => {
        try {
          // Don't affect the main word if it's pinned
          if (d.id === mainWord && d.pinned) return;
          
          // Gradually return to simulation
  ...
  const renderTooltip = useCallback(() => {
    try {
      ...
      // Get accurate screen coordinates using SVG transform
      const screenCoords = getSvgToScreenCoordinates(hoveredNode.x, hoveredNode.y);
      
      // Calculate position relative to the container instead of absolute positioning
      const containerRect = svgRef.current?.getBoundingClientRect();
      if (!containerRect) return null;
      
      const tooltipX = screenCoords.x - containerRect.left + 15;
      const tooltipY = screenCoords.y - containerRect.top + 15;
  ...
  }, [hoveredNode, wordNetwork, theme, getNodeColor, getSvgToScreenCoordinates]);
