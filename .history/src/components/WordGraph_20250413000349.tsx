...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const getNodeColor = useCallback((group: string): string => {
    switch (group) {
      ...
      case "root": return "#e63946"; // Root and derived now share this color
      case "synonym": return "#457b9d";
      case "antonym": return "#f77f00";
      case "variant": return "#f4a261";
      case "related": return "#fcbf49"; // Related and associated now share this color
      case "taxonomic": return "#8338ec";
      case "part_whole": return "#3a86ff"; // Part/Whole and Component now share this color
      case "usage": return "#0ead69";
      case "etymology": return "#3d5a80";
      case "cognate": return "#9381ff";
      default: return "#adb5bd"; // Fallback color
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const groupedRelationTypes = new Map<string, {type: string, label: string}[]>();
    
    const allRelationGroups = [
        { type: 'main', label: 'Main Word' },
        { type: 'root', label: 'Root/Derived' },
        ...
        { type: 'part_whole', label: 'Components/Parts' },
    ...
    allRelationGroups.forEach(item => {
      const { group, label } = getRelationshipTypeLabel(item.type);
      
      // Create unique key from group and label to avoid duplicates
      const uniqueKey = `${group}-${label}`;
      ...
      if (!groupedRelationTypes.has(group)) {
        groupedRelationTypes.set(group, []);
      }
      groupedRelationTypes.get(group)!.push({ type: item.type, label });
    });
    
    // Calculate legend dimensions
    const categorySpacing = 25; // Space between categories
    ...
    let totalItems = 0;
    groupedRelationTypes.forEach((items, group) => {
      // Add 1 for the category header + all items in the category
      totalItems += 1 + items.length;
    });
    
    // Calculate total height with category spacing
    const totalHeight = (totalItems * legendItemHeight) + 
                        ((groupedRelationTypes.size - 1) * categorySpacing) + 
                        (legendPadding * 2) + 10;
    ...
    // Render the grouped legend
    let yOffset = legendPadding + 25;
    
    Array.from(groupedRelationTypes.entries()).forEach(([group, items]) => {
      // Add category header
      legendContainer.append("text")
      ...
      items.forEach(item => {
        const legendEntry = legendContainer.append("g")
          ...
          .attr("transform", `translate(${legendPadding + 8}, ${yOffset})`);
      ...
      yOffset += categorySpacing - legendItemHeight;
    });
