        ...
        entry.append("rect")
          ...
          .on("click", function(this: SVGRectElement) {
            ...
            setFilteredRelationships(prev => {
              ...
              // Directly apply filters to DOM to ensure immediate visual update
              const allNodes = d3.selectAll<SVGGElement, CustomNode>(".node");
              const allLinks = d3.selectAll<SVGLineElement, CustomLink>(".link");
              ...
              allNodes.style("opacity", d => {
                ...
                if (d.id === mainWord) return null;
                // Dim nodes that match filtered types
                return newFilters.includes(d.group.toLowerCase()) ? 0.15 : null;
              })
              .style("pointer-events", d => {
                if (newFilters.length === 0) return "all";
                if (d.id === mainWord) return "all";
                return newFilters.includes(d.group.toLowerCase()) ? "none" : "all";
              });
