      ...
      <div className={`relations-container ${theme}`}>
        <div className="section-header">
          <h3 className="section-title">
            ...
            Related Words
            <span className="relation-count">{totalRelationCount}</span>
          </h3>
        </div>
            ...
            <div className="relation-groups">
              ...
              {relationGroups.find(group => group.type === 'root') && (
                <div className="relation-group">
                  <div 
                    className={`relation-group-header ${expandedGroups.includes('root') ? 'expanded' : ''}`}
                    onClick={() => toggleGroup('root')}
                  >
                    <span className="relation-group-icon">
                      <span className="material-icons">account_tree</span>
                    </span>
                    <span className="relation-group-label">Root Word</span>
                    <span className="relation-group-count">{allRelationData['root']?.length || 0}</span>
                    <span className="expand-icon">
                      <span className="material-icons">
                        {expandedGroups.includes('root') ? 'expand_less' : 'expand_more'}
                      </span>
                    </span>
                  </div>
                  {expandedGroups.includes('root') && (
                    <div className="relation-group-content">
                      <p className="relation-description">The etymological source of this word</p>
              ...
              {relationGroups
                ...
                .map(group => (
                <div key={group.type} className="relation-group">
                  <div 
                    className={`relation-group-header ${expandedGroups.includes(group.type) ? 'expanded' : ''} ${theme}`}
                    onClick={() => toggleGroup(group.type)}
                  >
                    <span className="relation-group-icon">
                      <span className="material-icons">{group.icon}</span>
                    </span>
                    <span className="relation-group-label">{group.label}</span>
                    <span className="relation-group-count">{group.words.length}</span>
                    <span className="expand-icon">
                      <span className="material-icons">
                        {expandedGroups.includes(group.type) ? 'expand_less' : 'expand_more'}
                      </span>
                    </span>
                  </div>
