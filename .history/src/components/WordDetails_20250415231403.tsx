...
const TabPanel = (props: TabPanelProps) => {
  ...
  return (
    ...
    >
      {value === index && (
        <Box sx={{ pt: theme.spacing(2), height: '100%' }}>
          {children}
        </Box>
      )}
    </div>
...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({
  ...
  const [convertedBaybayin, setConvertedBaybayin] = useState<string | null>(null);
  const [isConvertingBaybayin, setIsConvertingBaybayin] = useState<boolean>(false);
  ...
  // Effect to convert to baybayin if not available
  useEffect(() => {
    // Debug output of the raw Baybayin data
    console.log('WordDetails: Raw Baybayin data check:', {
      lemma: wordInfo.lemma,
      has_baybayin: wordInfo.has_baybayin,
      baybayin_form: wordInfo.baybayin_form,
      unicode_points: wordInfo.has_baybayin && wordInfo.baybayin_form ? 
        wordInfo.baybayin_form.split('').map((char: string) => `U+${char.charCodeAt(0).toString(16).toUpperCase().padStart(4, '0')}`).join(' ') : 
        'N/A',
      isConvertingBaybayin,
      convertedBaybayin
    });
    
    // Check if word already has database-stored baybayin and clear any conversion state
    if (wordInfo.has_baybayin && wordInfo.baybayin_form) {
      console.log('WordDetails: Using database-stored Baybayin:', wordInfo.baybayin_form);
      // Always reset conversion state when we have database baybayin
      setConvertedBaybayin(null);
      setIsConvertingBaybayin(false);
      return; // Exit early, no need to convert
    }
    
    // Reset conversion state when word changes
    if (wordInfo.lemma !== prevWordRef.current) {
      console.log(`WordDetails: Word changed from "${prevWordRef.current}" to "${wordInfo.lemma}", resetting Baybayin state`);
      setConvertedBaybayin(null);
      setIsConvertingBaybayin(false);
      prevWordRef.current = wordInfo.lemma;
    }
    
    // Only attempt conversion if there's no baybayin from database, we have a valid word, and we're not already converting
    if (!wordInfo.has_baybayin && wordInfo.lemma && !isConvertingBaybayin && convertedBaybayin === null) {
      console.log('WordDetails: Will attempt Baybayin conversion since no Baybayin in database');
      
      const fetchBaybayinConversion = async () => {
        try {
          setIsConvertingBaybayin(true);
          console.log('WordDetails: Starting Baybayin conversion...');
          
          // Use the appropriate language code from the word or default to 'fil'
          const langCode = wordInfo.language_code === 'tl' ? 'fil' : (wordInfo.language_code || 'fil');
          
          console.log(`WordDetails: Converting "${wordInfo.lemma}" to baybayin using language: ${langCode}`);
          const result = await convertToBaybayin(wordInfo.lemma, langCode);
          
          console.log('WordDetails: Conversion API response:', result);
          
          if (result && result.baybayin) {
            console.log(`WordDetails: Successfully converted to baybayin: ${result.baybayin}`);
            // Display Unicode code points for debugging
            const codePoints = result.baybayin.split('').map((char: string) => 
              `U+${char.charCodeAt(0).toString(16).toUpperCase().padStart(4, '0')}`
            ).join(' ');
            console.log(`WordDetails: Baybayin Unicode code points: ${codePoints}`);
            setConvertedBaybayin(result.baybayin);
          } else {
            console.warn('WordDetails: Conversion returned without baybayin text', result);
            setConvertedBaybayin(null);
          }
        } catch (error) {
          console.error('WordDetails: Error converting to baybayin:', error);
          setConvertedBaybayin(null);
        } finally {
          setIsConvertingBaybayin(false);
        }
      ...
      const timer = setTimeout(() => {
        fetchBaybayinConversion();
      }, 100);
      
      return () => clearTimeout(timer);
    }
  }, [wordInfo.lemma, wordInfo.has_baybayin, wordInfo.baybayin_form, wordInfo.language_code]); 
  ...
  const renderHeader = () => {
    ...
    // Softer, more elegant background colors
    ...
    return (
      ...
      <Box sx={{ 
        ...
        {/* Baybayin - Show either the original or converted form */}
        {(wordInfo.has_baybayin && wordInfo.baybayin_form) || convertedBaybayin ? (
          <Box sx={{ my: theme.spacing(2) }}>
            <Typography variant="caption" sx={{ color: alpha(headerTextColor, 0.75), display: 'block', mb: 0.5 }}>
              {wordInfo.has_baybayin ? "Baybayin Script (From Database)" : "Converted to Baybayin"}
            </Typography>
            <div 
            ...
            >
              {wordInfo.has_baybayin && wordInfo.baybayin_form ? wordInfo.baybayin_form : convertedBaybayin}
            </div>
            {!wordInfo.has_baybayin && convertedBaybayin && (
              <Typography variant="caption" sx={{ display: 'block', mt: 0.5, fontStyle: 'italic', color: alpha(headerTextColor, 0.6) }}>
                Auto-generated via API • May not be accurate
              </Typography>
            )}
          </Box>
        ) : isConvertingBaybayin ? (
          <Box sx={{ my: theme.spacing(2) }}>
            <Typography variant="caption" sx={{ color: alpha(headerTextColor, 0.75), display: 'block', mb: 0.5 }}>
              Converting to Baybayin...
            </Typography>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, minHeight: '48px', p: 1 }}>
              <CircularProgress size={20} thickness={4} sx={{ color: alpha(headerTextColor, 0.7) }} />
              <Typography variant="body2" sx={{ color: alpha(headerTextColor, 0.7) }}>
                Processing...
              </Typography>
            </Box>
          </Box>
          ...
          {wordInfo.language_code && (
            <Box>
              ...
              <Typography 
                ...
                sx={{ 
                  fontWeight: 500,
                  color: isDarkMode ? headerTextColor : theme.palette.text.primary
                }}
              >
  ...
  const renderExamples = () => {
    return (
      <Typography variant="body1">Examples content will go here</Typography>
    );
  };
  
  const renderSemanticNetwork = () => {
    return (
      <Typography variant="body1">Semantic network content will go here</Typography>
    );
  };
  
  const renderHistory = () => {
    return (
      <Typography variant="body1">History content will go here</Typography>
    );
  };
  
  const renderDebug = () => {
    return (
      <Typography variant="body1">Debug content will go here</Typography>
    );
  };
  ...
  const renderSidebar = () => {
    return (
      <Box sx={{ p: 2 }}>
        <Typography variant="subtitle2">Related Words</Typography>
      </Box>
    );
  };
    ...
    >
      ...
      {isLoading ? (
        ...
        >
          ...
          <Typography variant="body1">Loading word details...</Typography>
        </Paper>
      ) : null}
      ...
      {error ? (
        ...
        >
          ...
          <Typography variant="body1" align="center" sx={{ mb: 2, maxWidth: '600px' }}>
            {error}
          </Typography>
          {onRetry && (
            <Button 
      ...
      {!isLoading && !error && wordInfo ? (
        <>
          ...
          <Box sx={{ 
            ...
            <Box sx={{ 
                ...
                <Tabs 
                  ...
                  sx={{
                    '& .MuiTab-root': {
                      ...
                      fontWeight: 500,
                      textTransform: 'none',
                      color: isDarkMode ? alpha('#fff', 0.7) : alpha('#000', 0.7),
                      '&.Mui-selected': {
                        color: isDarkMode ? graphColors.main : theme.palette.primary.main,
                        fontWeight: 600
                      }
                    },
                    '& .MuiTabs-indicator': {
                      backgroundColor: isDarkMode ? graphColors.main : theme.palette.primary.main,
                      height: 3
                    }
                  }}
                >
                  <Tab label="Definitions" id="word-tab-0" aria-controls="word-tabpanel-0" />
                  <Tab label="Examples" id="word-tab-1" aria-controls="word-tabpanel-1" />
                  <Tab label="Etymologies" id="word-tab-2" aria-controls="word-tabpanel-2" />
                  <Tab label="Tree View" id="word-tab-3" aria-controls="word-tabpanel-3" />
                  <Tab label="Semantic Network" id="word-tab-4" aria-controls="word-tabpanel-4" />
                  <Tab label="History" id="word-tab-5" aria-controls="word-tabpanel-5" />
                  <Tab label="Debug" id="word-tab-6" aria-controls="word-tabpanel-6" />
                </Tabs>
              </Box>
              ...
              >
                ...
                <TabPanel value={tabValue} index={3}>
                  <Typography>EtymologyTree would be rendered here</Typography>
                </TabPanel>
            ...
            {/* Side drawer with relations/keywords */}
            <Drawer
              sx={{
                width: drawerWidth,
                flexShrink: 0,
