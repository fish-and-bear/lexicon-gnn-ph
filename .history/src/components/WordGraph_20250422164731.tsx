...
// Define drawDesktopLegend function outside the component
// (Place this after imports and before the WordGraph component definition)

// Helper function to draw the static desktop legend
const drawDesktopLegend = (
  svg: d3.Selection<SVGSVGElement, unknown, null, undefined>,
  width: number,
  height: number,
  muiTheme: Theme, // Pass MUI theme
  getUniqueRelationshipGroups: () => RelationshipGroups, // Pass necessary functions
  filteredRelationships: string[],
  handleToggleRelationshipFilter: (types: string | string[]) => void
) => {
...
const WordGraph: React.FC<WordGraphProps> = ({
      ...
      resizeTimer = window.setTimeout(() => {
        if (svgRef.current) {
          ...
          if (containerRect) {
            ...
            const svg = d3.select(svgRef.current) // Get SVG selection
              .attr("width", width)
              .attr("height", height);
  ...
  useEffect(() => {
    ...
    if (!isMobile) {
      // REMOVE the existing legend rendering logic from here
      // CALL the extracted function instead
      drawDesktopLegend(
        svg, 
        width, 
        height, 
