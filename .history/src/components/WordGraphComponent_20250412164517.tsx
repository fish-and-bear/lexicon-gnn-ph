...
const WordGraphComponent: React.FC<WordGraphProps> = ({
  ...
  // Update the depth change handler
  const handleDepthChange = useCallback((newDepth: number) => {
    setDepth(newDepth);
    onNetworkChange(newDepth, breadth).catch(error => {
      console.error("Error updating network:", error);
    });
  }, [breadth, onNetworkChange]);

  // Update the breadth change handler
  const handleBreadthChange = useCallback((newBreadth: number) => {
    setBreadth(newBreadth);
    onNetworkChange(depth, newBreadth).catch(error => {
      console.error("Error updating network:", error);
    });
  ...
  // Update render empty state function
  ...
  }, [depth, breadth, theme, wordNetwork, handleDepthChange, handleBreadthChange]);
          ...
          <div className="controls-container" style={{
            ...
            <button 
              ...
              style={{
                ...
                borderRadius: '4px',
                border: 'none',
                backgroundColor: 'var(--primary-color, #1d3557)',
                color: 'white',
                cursor: 'pointer'
              }}
            >
