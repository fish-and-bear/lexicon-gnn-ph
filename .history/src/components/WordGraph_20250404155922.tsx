  ...
  useEffect(() => {
    ...
    const handleNodeClick = (event: MouseEvent, d: CustomNode) => {
      ...
      if (now - lastClickTimeRef.current < doubleClickThreshold) {
        ...
        d3.select(event.currentTarget).attr("stroke", d.pinned ? "#ff0000" : (theme === 'dark' ? "#fff" : "#333"));
      } else {
    ...
    const dragstarted = (event: d3.D3DragEvent<SVGCircleElement, CustomNode, any>, d: CustomNode) => {
      isDraggingRef.current = true;
      if (!event.active && simulationRef.current) simulationRef.current.alphaTarget(0.3).restart();
      d.fx = d.x;
      d.fy = d.y;
      // Use currentTarget instead of target and cast
      d3.select(event.sourceEvent.currentTarget as SVGCircleElement).raise().attr("stroke", "black"); 
    };
    ...
    const dragended = (event: d3.D3DragEvent<SVGCircleElement, CustomNode, any>, d: CustomNode) => {
      ...
      // Use currentTarget instead of target and cast
      d3.select(event.sourceEvent.currentTarget as SVGCircleElement).attr("stroke", d.pinned ? "#ff0000" : (theme === 'dark' ? "#fff" : "#333"));
