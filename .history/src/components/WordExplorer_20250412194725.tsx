...
const WordExplorer: React.FC = () => {
  ...
  // Add ref for handleNodeClick to avoid circular dependency
  const handleNodeClickRef = useRef<(id: string) => void>();

  // Add an effect to handle searching and then clicking on the first result
  useEffect(() => {
    if (searchResults.length > 0 && !showSuggestions && handleNodeClickRef.current) {
      // Select the first result automatically
      handleNodeClickRef.current(searchResults[0].id.toString());
    }
  ...
  // Store the handleNodeClick function in the ref
  useEffect(() => {
