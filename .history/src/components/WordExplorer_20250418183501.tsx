...
const WordExplorer: React.FC = () => {
  ...
  const fetchWordNetworkData = useCallback(async (word: string, currentDepth: number, currentBreadth: number) => {
    try {
      setIsLoading(true);
  ...
  const fetchDetails = useCallback(async (wordIdentifier: string | number) => {
    // Ensure wordIdentifier is a string before proceeding
    ...
    try {
      ...
      // Fetch word details using fetchWordDetails
      const data = await fetchWordDetails(normalized);
      console.log('Word details received:', data);

      setSelectedWordInfo(data);
      setWordData(data); // Also update wordData state

      // Trigger etymology tree fetch if the word ID is available
      if (data.id) {
        fetchEtymologyTreeData(data.id);
      } else {
        setEtymologyTree(null); // Clear tree if no ID
      }

      // Update history only if the word is different from the last entry
      // This condition prevents adding the same word consecutively
      const lastHistoryEntry = wordHistory[currentHistoryIndex];
      let shouldAddToHistory = true;
      if (lastHistoryEntry) {
          if (typeof lastHistoryEntry === 'string' && lastHistoryEntry === data.lemma) {
              shouldAddToHistory = false;
          } else if (typeof lastHistoryEntry === 'object' && lastHistoryEntry.id === data.id) {
              shouldAddToHistory = false;
          }
      }
      ...
      // Clear suggestions after selecting a word
  ...
  const debouncedSearch = useCallback(
    debounce(async (query: string) => {
      ...
      try {
        // Call the searchWords API function
  ...
  const handleSearchSubmit = useCallback((e?: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!normalized) return; // Don't search if input is empty after normalization

    console.log(`Search submitted for: ${normalized}`);

    setMainWord(normalized); // Set the main word for the graph
    fetchWordNetworkData(normalized, depth, breadth); // Fetch network data
    fetchDetails(normalized); // Fetch details for the word

    // Clear suggestions after search submission
    setShowSuggestions(false);
    setSearchResults([]);
  ...
  const handleNodeSelect = useCallback((nodeId: string | number | null) => {
    if (nodeId !== null) {
      ...
      // Attempt to find the selected node in the network data
      const selectedNode = wordNetwork?.nodes.find(n => String(n.id) === String(nodeId));

      if (selectedNode) {
        console.log('Selected node details:', selectedNode);
        // Fetch details using the lemma from the node data
        fetchDetails(selectedNode.id || selectedNode.lemma);
        setInputValue(selectedNode.lemma); // Update input field
      } else {
        console.warn(`Node with ID ${nodeId} not found in current network data. Fetching by ID.`);
        // Fallback: If node not found (e.g., graph data is stale), fetch by ID
        fetchDetails(nodeId);
        // Optionally try to update input value if ID fetch is successful, but might be tricky
      }

    } else {
      console.log('Node deselected or click outside nodes.');
      // Optionally clear details or revert to main word details
      // setSelectedWordInfo(null);
    }
  ...
  const handleGoBack = useCallback(() => {
    if (currentHistoryIndex > 0) {
      ...
      // Update input value to reflect the navigated word
      const lemma = typeof previousEntry === 'object' ? previousEntry.text : previousEntry;
      setInputValue(lemma);
      setMainWord(lemma); // Also update the graph's main word
      fetchWordNetworkData(lemma, depth, breadth); // Refetch network for context
    }
  ...
  const fetchRandom = useCallback(async () => {
    ...
    try {
      ...
      if (wordInfo && wordInfo.lemma) {
        ...
        fetchWordNetworkData(wordInfo.lemma, depth, breadth); // Pass depth and breadth
        setSelectedWordInfo(wordInfo); // Set details directly from random fetch
        setWordData(wordInfo); // Update wordData state

        // Update history - logic moved inside fetchDetails, but needed here too for random
        const newHistory = [...wordHistory.slice(0, currentHistoryIndex + 1), { id: wordInfo.id, text: wordInfo.lemma }];
        setWordHistory(newHistory);
        setCurrentHistoryIndex(newHistory.length - 1);


        // Fetch etymology tree if ID exists
        if (wordInfo.id) {
      ...
      // Set a timeout to re-enable the button after a delay (e.g., 1 second)
      randomWordTimeoutRef.current = setTimeout(() => {
        setIsRandomLoading(false);
        randomWordTimeoutRef.current = null; // Clear the ref after timeout finishes
      }, 1000);
    }
  }, [depth, breadth, fetchWordNetworkData, getEtymologyTreeData, isRandomLoading, wordHistory, currentHistoryIndex]);
  ...
  // Function to handle resetting the circuit breaker
  const handleResetCircuitBreaker = () => {
    resetCircuitBreaker();
    setApiConnected(null); // Re-test connection state
    ...
    }, 1000); // Wait 1 second before re-testing
  };
