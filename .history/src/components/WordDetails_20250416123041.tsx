...
interface WordDetailsProps {
  ...
  containerWidth?: number | null; // Make optional and accept null
}
...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({
  ...
  console.log("[WordDetails] Rendering with containerWidth:", containerWidth); // Log received prop
  ...
  const renderHeader = () => {
    ...
    return (
      ...
      <Box sx={{ 
        ...
        {/* Lemma and Audio Button - Reverting to Stack with precise flex control */}
        <Stack direction="row" spacing={1} alignItems="flex-start" flexWrap="nowrap" sx={{ mb: theme.spacing(1.5), width: '100%' }}>
          {/* Lemma Typography - Simplify styles, ensure block/inline-block */}
          <Typography 
            variant="h3" 
            component="h1" 
            className="word-details-header-lemma" 
            sx={{ 
              // Core flex properties for wrapping:
              flexGrow: 1, 
              minWidth: 0, 
              // Core text wrapping properties:
              ...
              fontSize: containerWidth 
                ? containerWidth < 350 ? '1.5rem' // Very narrow panel
                : containerWidth < 500 ? '1.75rem' // Narrow panel
                : '2.125rem' // Default (h3)
                : '2.125rem', // Fallback if width not yet measured
        ...
        {Object.entries(definitionsByPosThenSource).map(([posName, defsBySource]) => (
          <Box key={posName} sx={{ mb: theme.spacing(3) }}>
            ...
            {Object.entries(defsBySource).map(([sourceName, defs]) => (
              <Box key={sourceName} sx={{ mb: theme.spacing(2) }}> {/* Add margin between source groups */}
                <List disablePadding>
  ...
  return (
    <Paper elevation={2} square sx={{ display: 'flex', flexDirection: 'column', height: '100%', width: '100%', bgcolor: 'background.paper' }}>
