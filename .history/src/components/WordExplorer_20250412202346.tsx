...
const WordExplorer: React.FC = () => {
  ...
  const handleNodeClick = useCallback(async (word: string) => {
    ...
    try {
      ...
      try {
        ...
        if (error.message.includes('not found') || 
          ...
          if (searchResults.length > 0) {
            ...
            wordData = await fetchWordDetails(searchResults[0].id.toString().startsWith('id:') ? 
      ...
      if (!wordHistory.find(w => w.id === wordData.id)) {
        const newHistory = [{ id: wordData.id, text: wordData.lemma }, ...wordHistory].slice(0, 20);
        setWordHistory(newHistory);
      }

      // Update network data if necessary
      if (wordData.id !== networkWordId) {
        setNetworkDepth(DEFAULT_NETWORK_DEPTH);
        setNetworkBreadth(DEFAULT_NETWORK_BREADTH);
        setNetworkWordId(wordData.id);
      }

      // Fetch the etymology tree for the word in the background
      try {
        fetchEtymologyTree(wordData.id.toString().startsWith('id:') ? 
  ...
  }, [wordHistory, networkWordId, setNetworkDepth, setNetworkBreadth, setNetworkWordId]);
