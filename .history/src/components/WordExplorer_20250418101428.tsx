...
const WordExplorer: React.FC = () => {
  ...
  const fetchRandomWordsForCache = useCallback(async (count: number = RANDOM_CACHE_SIZE): Promise<any[]> => {
    ...
    try {
      ...
      const validResults = results.filter(result => 
        result !== null && result.lemma && typeof result.lemma === 'string'
      );
      if (validResults.length === 0) {
        console.warn('No valid random words found for cache.');
        // Removed automatic retry to simplify
        return randomWordCache;
      } else {
        const existingWordMap = new Map(randomWordCache.map(w => [w?.lemma?.toLowerCase(), w]));
        const uniqueNewWords = validResults.filter(word => !existingWordMap.has(word.lemma.toLowerCase()));
        let combinedCache = [...randomWordCache];
        if (uniqueNewWords.length > 0) {
          combinedCache = [...randomWordCache, ...uniqueNewWords];
        }
        // Removed cache clearing logic for simplification in this context
        const finalCache = shuffleArray(combinedCache).slice(0, RANDOM_CACHE_SIZE * 3);
        setRandomWordCache(finalCache);
        randomWordCacheRef.current = finalCache;
        console.log(`Added ${uniqueNewWords.length} unique words to cache (total: ${finalCache.length})`);
        return finalCache;
      }
    } catch (error) {
  ...
  }, [isRefreshingCache, randomWordCache]); // Keep dependency
  ...
  }, []); // Assuming getPartsOfSpeech API function is stable

  // Test API connectivity and fetch initial data on mount
  useEffect(() => {
    console.log("Starting initial data fetch sequence...");
    let isMounted = true;
    let cachePrefetchTimer: NodeJS.Timeout | null = null;
    ...
    const initialLoadSequence = async () => {
      try {
        ...
        if (connected) {
          ...
          // 2. Fetch Parts of Speech and Statistics (concurrently)
          await Promise.all([
            (async () => {
              const posData = await getPartsOfSpeech();
              if (isMounted) setPartsOfSpeech(posData);
            })(),
            fetchStatistics() // Already wrapped in useCallback
          ]);

          // 3. Prefetch Random Words if cache is empty (deferred)
          if (isMounted && randomWordCache.length === 0) {
            console.log("Initial random cache empty, scheduling prefetch...");
            cachePrefetchTimer = setTimeout(() => {
              if (isMounted) {
                fetchRandomWordsForCache(RANDOM_CACHE_SIZE); // Fetch standard size
              }
            }, 2000); // Wait 2 seconds after initial data
          }
        } else {
          console.error("API Connection Failed on Initial Load");
          if (isMounted) setError("Failed to connect to backend API. Ensure server is running.");
        }
      } catch (error) {
        if (isMounted) {
          console.error("Error during initial load sequence:", error);
          setError("Error loading initial application data.");
          setApiConnected(false); // Assume connection failed if error occurs here
        }
      }
    };

    // Defer the entire sequence slightly
    const initialLoadTimer = setTimeout(initialLoadSequence, 500);

    // Cleanup function
    return () => {
      isMounted = false;
      clearTimeout(initialLoadTimer);
      if (cachePrefetchTimer) {
        clearTimeout(cachePrefetchTimer);
      }
    };

  // Dependencies: Include stable callbacks and state length used in conditional logic.
  }, [fetchStatistics, fetchRandomWordsForCache, randomWordCache.length]); // Removed getPartsOfSpeech as it's called directly
