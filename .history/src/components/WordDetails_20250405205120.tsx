...
const safeParse = (input: string | null | undefined, returnType: 'array' | 'object' = 'array'): any[] | Record<string, any> => {
  if (!input) {
...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({
  ...
  const renderHeader = () => {
    ...
    // Parse tags safely
    const tags = safeParse(wordInfo.tags, 'array') as string[];
    ...
    return (
      ...
      <List disablePadding sx={{ pt: theme.spacing(1) }}>
        {wordInfo.definitions.map((def: Definition, index: number) => {
            ...
            const examples = def.examples; // Already cleaned to string[] in WordInfo type generation (assuming cleaning happens before passing prop)
    ...
    return (
        <List dense sx={{ py: 1 }}>
            {wordInfo.forms.map((form: WordForm, index: number) => {
                const tags = safeParse(form.tags, 'object') as Record<string, any>; // Parse tags object
                return (
      ...
      return (
          <List dense sx={{ py: 1 }}>
              {wordInfo.templates.map((template: WordTemplate, index: number) => {
                  const args = safeParse(template.args, 'object') as Record<string, any>; // Parse args object
                  return (
