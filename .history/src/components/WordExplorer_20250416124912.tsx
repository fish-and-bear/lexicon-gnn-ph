...
const WordExplorer: React.FC = () => {
  ...
  return (
    <div className={`word-explorer ${theme} ${isLoading ? 'loading' : ''}`}>
      ...
      <main>
        {isWideLayout ? (
          <PanelGroup direction="horizontal" autoSaveId="wordExplorerLayout" style={{ width: '100%', height: '100%', display: 'flex', flex: 1 }}>
            <Panel defaultSize={60} minSize={30} order={1} style={{ display: 'flex', overflow: 'hidden' }}>
              {/* Graph Container Content */}
              <div className="graph-container" style={{ height: '100%', width: '100%', display: 'flex', flexDirection: 'column' }}>
                <div className="graph-content" style={{ flex: 1, overflow: 'hidden' }}>
            ...
            <PanelResizeHandle style={{ width: '4px', cursor: 'col-resize' }} />
            <Panel defaultSize={40} minSize={25} order={2} style={{ display: 'flex', overflow: 'hidden' }}>
              {/* Details Container Content - Ensure full width */}
              <div ref={detailsContainerRef} className="details-container" style={{ height: '100%', width: '100%', overflow: 'auto' }}>
          ...
          // Mobile/Stacked Layout - Also pass width here, though it might be full screen width
          <>
            <div className="graph-container" style={{ width: '100%', height: '50%', display: 'flex', flexDirection: 'column', minHeight: '300px' }}>
              {/* Graph content (same as above) */}
              <div className="graph-content" style={{ flex: 1, overflow: 'hidden' }}>
                {isLoading && <div className="loading">Loading Network...</div>}
                {!isLoading && wordNetwork && (
                  <WordGraph
            ...
            <div ref={detailsContainerRef} className="details-container" style={{ width: '100%', height: '50%', overflow: 'auto', flex: 1, minHeight: '300px' }}>
              {isLoading && <div className="loading-spinner">Loading Details...</div>} 
              {!isLoading && selectedWordInfo && (
                <WordDetails 
                  wordInfo={selectedWordInfo} 
