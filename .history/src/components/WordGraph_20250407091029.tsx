...
interface LinkData {
  id: string;
  source: number; // Use number for ID matching
  target: number; // Use number for ID matching
  type: string;
}
...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  useEffect(() => {
    ...
    if (!simulationRef.current) {
        simulationRef.current = d3.forceSimulation<NodeData, LinkData>(nodes)
            .force('link', d3.forceLink<NodeData, LinkData>(links)
                               .id(d => d.id)
                               .distance(link => link.source === mainNodeId || link.target === mainNodeId ? 100 : 70) // Shorter links for non-main
                               .strength(0.6))
            .force('charge', d3.forceManyBody().strength(-200)) // Increased repulsion
            .force('center', d3.forceCenter(width / 2, height / 2))
            .force('collision', d3.forceCollide().radius(d => (d.id === mainNodeId ? 30 : 20))); // Collision based on size
    } else {
    ...
    const link = g.append('g')
      ...
      .data(links)
      .enter().append('line')
        .attr('class', d => `link ${d.type}`) // Add class based on link type for color-coding
    ...
    node.append('circle')
        .attr('r', d => d.id === mainNodeId ? 18 : 12) // Larger radius for main node
        .attr('fill', d => {
