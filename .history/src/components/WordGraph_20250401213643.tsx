  ...
  const setupSimulation = useCallback((nodes: CustomNode[], links: CustomLink[]) => {
      simulationRef.current = d3.forceSimulation<CustomNode>(nodes)
        ...
        .force("link", d3.forceLink<CustomNode, CustomLink>()
          ...
          .distance(110)
          .strength(0.4))
        .force("charge", d3.forceManyBody<CustomNode>().strength(-180).distanceMax(300))
        .force("collide", d3.forceCollide<CustomNode>().radius(d => getNodeRadius(d) + 5).strength(0.8))
      ...
      nodeSelection: d3.Selection<d3.BaseType, CustomNode, SVGGElement, unknown>
       ...
       nodeSelection
        .on("click", (event, d) => {
          ...
          // Dim everything significantly first (Instantaneous)
          ...
          // Highlight selected node (Instantaneous)
          const targetNodeElement = d3.select(event.currentTarget as Element);
          targetNodeElement.classed("selected", true)
            .style("opacity", 1);
            // Keep potential style changes like border width if needed, applied instantly
            // .select("circle").attr("stroke-width", 2.5);

           // Highlight connected nodes (Instantaneous)
           d3.selectAll<SVGGElement, CustomNode>(".node")
             .filter(n => connectedIds.has(n.id))
             .classed("connected", true) // Add connected class
             .style("opacity", 1); // Ensure all connected nodes are opaque

           // Highlight connected links (Instantaneous)
        ...
        .on("mouseover", (event, d) => {
            ...
            /* --- TEMP: Disable visual hover effects ---
            ...
            // Set timeout to show tooltip (Keep this)
        ...
        .on("mouseout", (event, d_unknown) => {
            if (isDraggingRef.current) return;
            // Clear tooltip timeout and hide tooltip immediately (Keep this)
            if (tooltipTimeoutId) clearTimeout(tooltipTimeoutId);
            setHoveredNode(null);
            ...
            // Revert hover effect for the circle
             nodeElement.select<SVGCircleElement>("circle")
                ...
                .attr("stroke", (n: CustomNode) => n.pinned ? getNodeColor(n.group) : (d3.color(getNodeColor(n.group))?.darker(0.6).formatHex() ?? (theme === "dark" ? "rgba(255,255,255,0.1)" : "rgba(0,0,0,0.1)")));
