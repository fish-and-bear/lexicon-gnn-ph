...
interface WordDetailsProps {
  ...
  wordNetwork?: any; // Optional prop for the word network data
}
...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({
                ...
                <List disablePadding>
                  {defs.map((def: Definition, index: number) => {
                    ...
                    >
                      ...
                      {/* Definition Relations - Display related words linked from this definition */}
                      {def.definition_relations && def.definition_relations.length > 0 && (
                        <Box sx={{ mt: theme.spacing(1), width: '100%' }}>
                          ...
                          <Stack direction="row" spacing={1} useFlexGap flexWrap="wrap">
                            {def.definition_relations.map((rel: DefinitionRelation, relIndex: number) => {
                              // Skip if no related word
                              if (!rel.related_word) return null;
                              ...
                              return (
                                <Chip
                                  key={relIndex}
                                  label={`${formatRelationType(relType)}: ${rel.related_word.lemma}`}
                                  onClick={() => rel.related_word && onWordLinkClick(rel.related_word.lemma)}
                                  size="small"
                                  sx={{
  ...
  const renderRelationsTab = () => {
    ...
    // Debug logs for relations data
    console.log("WordDetails: Relations Data Check", {
      incomingRelations: incoming_relations,
      outgoingRelations: outgoing_relations,
      incomingLength: incoming_relations.length,
      outgoingLength: outgoing_relations.length,
      semanticNetwork: Boolean(wordInfo.semantic_network),
      semanticNetworkNodes: wordInfo.semantic_network?.nodes?.length || 0,
      semanticNetworkLinks: wordInfo.semantic_network?.links?.length || 0,
      wordNetworkProp: Boolean(wordNetwork)
    });
    
    // DIRECT VISUAL FALLBACK - CRITICAL FIX
    // If we have semantic network data but no relations, show the network nodes directly
    const hasVisibleSemanticNetwork = (
      (wordInfo.semantic_network?.nodes && wordInfo.semantic_network.nodes.length > 0) ||
      (wordNetwork?.nodes && wordNetwork.nodes.length > 0)
    );
    
    if (hasVisibleSemanticNetwork) {
      // Use semantic network data from either source
      const networkNodes = wordInfo.semantic_network?.nodes || wordNetwork?.nodes || [];
      const mainWordId = networkNodes.find((n: any) => 
        n.type === 'main' || 
        n.word === wordInfo.lemma || 
        n.label === wordInfo.lemma
      )?.id;
      
      if (mainWordId) {
        // Create simple relation elements directly
        const otherNodes = networkNodes.filter((n: any) => {
          // Filter out main node and nodes with no label
          if (!n.label && !n.word) return false;
          if (n.id === mainWordId) return false;
          return true;
        });
        
        if (otherNodes.length > 0) {
          // Create a visual grouping of all nodes as "Visual Network Connections"
          return (
            <Box sx={{ p: 2 }}>
              <Box sx={{ mb: 3 }}>
                ...
                <Typography variant="body2" sx={{ mb: 2 }}>
                  The following words are connected to "{wordInfo.lemma}" in the semantic network:
                </Typography>
                <Stack 
                  direction="row" 
                  spacing={1} 
                  ...
                  sx={{ mt: 2 }}
                >
                  {otherNodes.map((node: { id: number | string, label?: string, word?: string }) => {
                    // Get a valid string for the onClick handler
                    const wordText = (node.label || node.word || String(node.id));
                    
                    return (
                      <Chip
                        key={node.id}
                        label={wordText}
                        onClick={() => onWordLinkClick(wordText)}
                        variant="outlined"
                        sx={{
                          borderColor: alpha(relationColors.related, 0.5),
                          color: relationColors.related,
                          fontSize: '0.75rem',
                          height: 'auto',
                          padding: theme.spacing(0.25, 0),
                          bgcolor: alpha(relationColors.related, 0.05),
                          my: 0.5,
                          '& .MuiChip-label': { 
                            px: 1, 
                            py: 0.25, 
                          ...
                          '&:hover': {
                            backgroundColor: alpha(relationColors.related, 0.1),
                            borderColor: relationColors.related
                          }
                        }}
                      />
                    );
    ...
    // Get definition relations (either directly from wordInfo or from definitions)
    const definitionRelations = [
      ...(wordInfo.definition_relations || []),
      ...(wordInfo.definitions?.flatMap(def => def.definition_relations || []) || [])
    ];
    
    // Convert definition relations to outgoing relations format for proper display
    const definitionRelationsAsOutgoing = definitionRelations.map(defRel => ({
      id: defRel.id || 0,
      relation_type: defRel.relation_type || 'related',
      target_word: defRel.related_word
    }));
    
    // Combine all relations
    const combinedOutgoingRelations = [
      ...outgoing_relations,
      ...definitionRelationsAsOutgoing
    ];
    
    // Get semantic network data from all available sources
    const semanticNetworkNodes = 
      (wordNetwork?.nodes) || 
      wordInfo.semantic_network?.nodes || 
      [];
      
    const semanticNetworkLinks = 
      (wordNetwork?.edges) || 
      wordInfo.semantic_network?.links || 
      [];
    
    // Debug the semantic network data directly
    console.log("SEMANTIC NETWORK DATA AVAILABLE:", {
      wordNetwork: wordNetwork,
      semantic_network: wordInfo.semantic_network,
      fromProps: Boolean(wordNetwork && wordNetwork.nodes && wordNetwork.nodes.length > 0),
      fromWordInfo: Boolean(wordInfo.semantic_network && wordInfo.semantic_network.nodes && wordInfo.semantic_network.nodes.length > 0),
      nodes: semanticNetworkNodes.length,
      links: semanticNetworkLinks.length,
    });
    
    // ALWAYS use the semantic network if available, or fall back to standard relations
    const hasSemanticNetwork = semanticNetworkLinks.length > 0 && semanticNetworkNodes.length > 0;
    const useSemanticNetwork = hasSemanticNetwork;
    
    function createRelationsFromNetwork() {
      console.log("Creating relations from semantic network data");
      const mainWord = wordInfo.lemma;
      const relations: any[] = [];
      
      // Get the main node ID
      const mainNodeId = semanticNetworkNodes.find(
        (n: any) => n.main === true || n.type === 'main' || n.label === mainWord || n.word === mainWord
      )?.id;
      
      if (!mainNodeId) {
        console.warn("No main node found in semantic network");
        return [];
      }
      
      // Process all links in the semantic network
      semanticNetworkLinks.forEach((link: any) => {
        // Get the source and target IDs
        const sourceId = typeof link.source === 'object' ? link.source.id : link.source;
        const targetId = typeof link.target === 'object' ? link.target.id : link.target;
        
        // Find the nodes
        const sourceNode = semanticNetworkNodes.find((n: any) => String(n.id) === String(sourceId));
        const targetNode = semanticNetworkNodes.find((n: any) => String(n.id) === String(targetId));
        ...
        // If this link connects to the main node, create a relation
        if (String(sourceId) === String(mainNodeId)) {
          // This is an outgoing relation from the main word
          relations.push({
            id: `semantic-out-${sourceId}-${targetId}`,
            relation_type: link.type || 'related',
            degree: targetNode.depth || 1,
            direction: 'outgoing',
            wordObj: {
              id: targetId,
              lemma: targetNode.label || targetNode.word || String(targetId),
              has_baybayin: targetNode.has_baybayin || false,
              baybayin_form: targetNode.baybayin_form || null
            }
          });
        } else if (String(targetId) === String(mainNodeId)) {
          // This is an incoming relation to the main word
          relations.push({
            id: `semantic-in-${sourceId}-${targetId}`,
            relation_type: link.type || 'related',
            degree: sourceNode.depth || 1,
            direction: 'incoming',
            wordObj: {
              id: sourceId,
              lemma: sourceNode.label || sourceNode.word || String(sourceId),
              has_baybayin: sourceNode.has_baybayin || false,
              baybayin_form: sourceNode.baybayin_form || null
            }
          });
        }
      });
      
      console.log(`Created ${relations.length} relations from semantic network`);
      return relations;
    }
    
    // Create relations from semantic network if available, otherwise use standard relations
    const semanticRelations = useSemanticNetwork ? createRelationsFromNetwork() : [];
    
    // If we have a semantic network but couldn't create relations, try a fallback approach
    if (useSemanticNetwork && semanticRelations.length === 0) {
      console.warn("No relations created from semantic network. Trying fallback approach.");
      
      // Simple fallback: for each node in the network (except the main node), create a relation
      semanticNetworkNodes.forEach((node: any) => {
        const nodeIsMain = node.main === true || node.type === 'main' || node.label === wordInfo.lemma || node.word === wordInfo.lemma;
        
        if (!nodeIsMain) {
          semanticRelations.push({
            id: `fallback-${node.id}`,
            relation_type: 'related', // Default to 'related'
            degree: node.depth || 1,
            direction: 'outgoing',
            wordObj: {
              id: node.id,
              lemma: node.label || node.word || String(node.id),
      ...
      console.log(`Created ${semanticRelations.length} relations with fallback approach`);
    }
    
    const hasStandardRelations = (incoming_relations.length > 0 || combinedOutgoingRelations.length > 0) && !useSemanticNetwork;
    
    // Check if we have any relations at all
    const hasAnyRelations = 
      hasStandardRelations || 
      semanticRelations.length > 0 || 
      hasSemanticNetwork || 
      hasVisibleSemanticNetwork || 
      (wordInfo.root_affixations && wordInfo.root_affixations.length > 0) || 
      (wordInfo.affixed_affixations && wordInfo.affixed_affixations.length > 0);
    
    console.log("Has any relations check:", {
      hasStandardRelations,
      semanticRelationsLength: semanticRelations.length,
      hasSemanticNetwork,
      hasVisibleSemanticNetwork,
      rootAffixations: Boolean(wordInfo.root_affixations && wordInfo.root_affixations.length > 0),
      affixedAffixations: Boolean(wordInfo.affixed_affixations && wordInfo.affixed_affixations.length > 0),
      hasAnyRelations
    });
    
    // For testing, if graph is visible but no relations are found, don't show "No relations"
    if (!hasAnyRelations && wordNetwork) {
      // If we have a graph visible, don't show "No relations"
      return (
        <Box sx={{ p: 2 }}>
          <Typography variant="body1">
            Relations are displayed in the graph visualization.
          </Typography>
        </Box>
      );
    }
    ...
    // Get relations based on source - use semantic relations if available
    const allRelations = useSemanticNetwork ? semanticRelations : [...incoming_relations, ...combinedOutgoingRelations];
    
    // Add degree information for regular relations and ensure wordObj exists
    const relationsWithDegree = allRelations.map(rel => ({
      ...
      direction: rel.source_word ? 'incoming' : (rel.target_word ? 'outgoing' : rel.direction || 'outgoing')
    }));
    ...
    // Sort relations by direction and type for better organization
    const sortedRelations = [...filteredRelations].sort((a, b) => {
      // First by direction - outgoing first
      if (a.direction !== b.direction) {
        return a.direction === 'outgoing' ? -1 : 1;
      }
      // Then by relation type
    ...
    const nonEmptyCategories = relationCategories.filter(
      ...
      <Box sx={{ pt: theme.spacing(1), width: '100%', maxWidth: '100%', overflow: 'hidden' }}>
        ...
        {/* Show error banner if there's a server error */}
        {typeof wordInfo.server_error === 'string' && (
