...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const handleToggleRelationshipFilter = useCallback((relationship: string) => {
    ...
    setFilteredRelationships(prevFilters => {
      if (prevFilters.includes(relationshipLower)) {
        console.log("Removing filter:", relationshipLower);
        const newFilters = prevFilters.filter(f => f !== relationshipLower);
        console.log("New filters:", newFilters);
        return newFilters;
      } else {
        console.log("Adding filter:", relationshipLower);
        const newFilters = [...prevFilters, relationshipLower];
        console.log("New filters:", newFilters);
        return newFilters;
      }
    });
  ...
  // Apply visibility filters whenever filteredRelationships changes
  useEffect(() => {
    // Force a complete graph rebuild
    setForceUpdate(prev => prev + 1);
    
    // Don't apply gradual visibility transitions during rebuild
    if (!svgRef.current) return;
    
    console.log("Applying filters to graph:", filteredRelationships);
    ...
    svg.selectAll(".node")
      .each(function(d: any) {
        ...
        // Main word should never be filtered out
        if (nodeData.id === mainWord) {
          node.style("opacity", 1)
               .style("pointer-events", "all");
          return;
        }
        
        const nodeRelationship = nodeData.group.toLowerCase();
        const isFiltered = filteredRelationships.includes(nodeRelationship);
        
        node.transition()
          .duration(300)
          .style("opacity", isFiltered ? 0 : 1)
          .style("pointer-events", isFiltered ? "none" : "all");
      });
    ...
    svg.selectAll(".link")
      .each(function(d: any) {
        ...
        const linkData = d;
        const linkRelationship = linkData.type?.toLowerCase();
        
        if (!linkRelationship) {
          console.log("Link has no type:", linkData);
          return;
        }
        
        const isFiltered = filteredRelationships.includes(linkRelationship);
        
        link.transition()
          .duration(300)
          .style("opacity", isFiltered ? 0 : 0.6)
          .style("pointer-events", isFiltered ? "none" : "all");
      });
  }, [filteredRelationships, mainWord, svgRef]);
