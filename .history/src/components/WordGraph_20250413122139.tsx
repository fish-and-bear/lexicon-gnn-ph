...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const filteredNodes = useMemo<CustomNode[]>(() => {
    ...
    console.log("[FILTER DEBUG] Calculating filteredNodes with filters:", filteredRelationships);
    ...
    // BFS traversal to collect nodes within depth/breadth limits
    while (queue.length > 0) {
      ...
      // Organize connected nodes by group for consistent breadth application
    ...
    // Step 2: Create initial node list with all nodes connected by depth/breadth
    const nodesConnectedByDepth = baseNodes.filter(node => connectedNodeIds.has(node.id));
    
    console.log(`[FILTER DEBUG] Depth/breadth filtering: ${nodesConnectedByDepth.length}/${baseNodes.length} nodes`);
    
    // Step 3: Apply relationship type filtering
    // If no filters active, return all connected nodes
    if (filteredRelationships.length === 0) {
      console.log("[FILTER DEBUG] No relationship filters active");
      return nodesConnectedByDepth;
    }
    
    // FIXED APPROACH: Filter out ONLY nodes whose relationship type matches filtered types
    const finalNodes = nodesConnectedByDepth.filter(node => {
      // Always include the main word
      if (node.id === mainWord) return true;
      
      // If group is not in filtered relationships, keep the node
      const isFiltered = filteredRelationships.includes(node.group.toLowerCase());
      
      // Keep nodes NOT in the filtered list
      return !isFiltered;
    });
    
    console.log(`[FILTER DEBUG] After relationship filtering: ${finalNodes.length}/${nodesConnectedByDepth.length} nodes visible`);
    // Log which node types are being filtered
    const groupCounts: Record<string, { total: number, filtered: number }> = {};
    nodesConnectedByDepth.forEach(node => {
      const group = node.group.toLowerCase();
      if (!groupCounts[group]) groupCounts[group] = { total: 0, filtered: 0 };
      groupCounts[group].total++;
      
      if (filteredRelationships.includes(group) && node.id !== mainWord) {
        groupCounts[group].filtered++;
      }
    });
    console.log("[FILTER DEBUG] Node counts by group:", groupCounts);
  ...
  // Completely redesigned toggle filter handler - much simpler
  const handleToggleRelationshipFilter = useCallback((relationship: string) => {
    ...
    // Force a complete rebuild of the graph with new filter settings
    setForceUpdate(prev => prev + 1);
  }, []);
  ...
  useEffect(() => {
    ...
    // First filter out links where either source or target is not in filteredNodes
    const nodesInDepthLinks = baseLinks.filter(link => {
    ...
    // Then filter out links with relationship types that are being filtered
    const currentFilteredLinks = nodesInDepthLinks.filter(link => {
      // If no relationship filters active, keep all links
      if (filteredRelationships.length === 0) return true;
      
      // Otherwise, check if this link's relationship is being filtered
      const relationship = link.relationship.toLowerCase();
      const isFiltered = filteredRelationships.includes(relationship);
      
      // Keep links NOT in the filtered relationships list
      return !isFiltered;
    }) as CustomLink[];
