...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({
  ...
  const renderRelationsTab = () => {
    ...
    // Create a more elegant relation card component
    const RelationCard = ({ relation }: { relation: any }) => {
      if (!relation.wordObj) return null;
        ...
        <Box
          ...
          sx={{
            ...
            '&:hover': {
              ...
              boxShadow: `0 2px 8px ${alpha(relColor, 0.15)}`,
    ...
    return (
      <Box sx={{ pt: theme.spacing(1) }}>
        ...
        >
          ...
          >
            {uniqueRelatedWords} related {uniqueRelatedWords === 1 ? 'word' : 'words'} across {Object.keys(relationsByType).length} relationship {Object.keys(relationsByType).length === 1 ? 'type' : 'types'}
        ...
        {Object.entries(relationsByType).map(([relType, relations]) => {
          const relColor = relationColors[relType] || relationColors.other;
          
          return (
            <Box key={relType} sx={{ mb: theme.spacing(4) }}>
              <Typography 
                variant="subtitle1" 
                sx={{ 
                  fontWeight: 600,
                  ...
                  borderBottom: `1px solid ${alpha(relColor, 0.2)}`
                }}
              >
                {formatRelationType(relType)}
                <Typography 
              ...
              >
                {relations.map((relation, index) => (
                  <RelationCard 
                    key={`${relation.id || index}-${relation.wordObj?.id || index}`} 
                    relation={relation} 
                  />
                ))}
              </Box>
