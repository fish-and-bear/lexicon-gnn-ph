...
const WordGraph: React.FC<WordGraphProps> = ({
  ...
  const getNodeColor = (node: CustomNode): string => {
    if (node.id === mainWord) return theme === 'dark' ? '#ffcc80' : '#ff9800'; // Main word
    
    if (!node.info || !node.info.type) return theme === 'dark' ? '#90caf9' : '#2196f3'; // Default
    
    // Customize colors based on node type
    switch (node.info.type) {
      case 'noun': return theme === 'dark' ? '#80deea' : '#00bcd4';
      case 'verb': return theme === 'dark' ? '#a5d6a7' : '#4caf50';
      case 'adjective': return theme === 'dark' ? '#ce93d8' : '#9c27b0';
      case 'adverb': return theme === 'dark' ? '#ffab91' : '#ff5722';
      default: return theme === 'dark' ? '#90caf9' : '#2196f3';
    }
  };
