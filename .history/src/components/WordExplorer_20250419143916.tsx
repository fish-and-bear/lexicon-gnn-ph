...
import { KeyboardArrowLeftIcon, KeyboardArrowRightIcon, ClearIcon, SearchIcon } from "@mui/icons-material";
...
const WordExplorer: React.FC = () => {
    ...
    try {
      ...
      if (result && result.words && result.words.length > 0) {
        ...
        try {
          ...
          if (!hasSemanticNetwork) {
            console.log("Semantic network data missing from search result, will load from network data");
          }
    ...
    if (currentHistoryIndex > 0) {
      ...
      fetchWordDetails(wordId)
        .then(wordData => {
          ...
          // After setting word data, fetch network data
          return fetchWordNetworkData(wordText, depth, breadth)
            .then(networkData => {
              setWordNetwork(networkData);
              ...
              if (wordData.id) {
                ...
                const etymologyId = etymologyIdString.startsWith('id:') ? 
                  parseInt(etymologyIdString.substring(3), 10) : 
                  wordData.id;
                return fetchEtymologyTree(etymologyId)
                  .then(tree => {
                    setEtymologyTree(tree);
                  })
                  .catch(err => {
  ...
  const debouncedSearch = useCallback(
    debounce(async (input: string) => {
      ...
      try {
        const result = await searchWords(input, {
          page: 1,
          per_page: 15,
          mode: 'all',
          sort: 'relevance',
        ...
        if (result && result.words) {
          console.log(`Found ${result.words.length} suggestions for "${input}"`);
          setSearchResults(result.words);
        } else {
          setSearchResults([]);
        }
  ...
  // Find similar words and highlight their differences
  const findSimilarWords = (words: SearchWordResult[]) => {
    // Group words by similar lemmas (case insensitive)
    const groups: Record<string, SearchWordResult[]> = {};
    
    words.forEach(word => {
      const normalized = word.lemma.toLowerCase();
    ...
    // Process each group to highlight differences
    const results = words.map(word => {
      const normalized = word.lemma.toLowerCase();
      const similar = groups[normalized];
      
      // If there are multiple similar words, mark them
      let hasSimilar = similar && similar.length > 1;
  ...
  // Helper function to format language information for search results
  const formatSearchResult = (result: SearchWordResult) => {
    // Extract language code
    let languageInfo = '';
    
    // Check if language_code property exists
    if (result.language_code) {
      // Format common language codes
      switch(result.language_code.toLowerCase()) {
        case 'tgl':
        case 'fil':
        ...
        case 'es':
          ...
          break;
        default:
          languageInfo = result.language_code;
      }
    }
      ...
      <div className="search-container">
        <div className="search-bar">
          <div className="history-buttons">
            <IconButton 
              className={`history-button ${wordHistory.length <= 1 || currentHistoryIndex <= 0 ? 'disabled' : ''}`}
              onClick={handleBack}
            ...
            <IconButton
              className={`history-button ${wordHistory.length <= 1 || currentHistoryIndex >= wordHistory.length - 1 ? 'disabled' : ''}`}
              onClick={handleForward}
              disabled={wordHistory.length <= 1 || currentHistoryIndex >= wordHistory.length - 1}
              size="small"
            >
        ...
        {showSuggestions && searchResults.length > 0 && (
          <ul className="search-suggestions">
            {findSimilarWords(searchResults).map((result, index) => {
              // Extract language information
              const languageInfo = formatSearchResult(result);
              
              // Get part of speech if available
              const posInfo = (result as any).pos || '';
              
              // Get short definition if available
              const definition = (result as any).definition || '';
              
              return (
                <li 
                  key={`${result.id}-${index}`}
                  onClick={() => {
                    handleSearch(result.lemma);
                    setInputValue(result.lemma);
                    setShowSuggestions(false);
                  }}
                >
                  <div className="suggestion-header">
                    <span className="suggestion-text">{result.lemma}</span>
                    {languageInfo && (
                      <span className="suggestion-language">{languageInfo}</span>
                    )}
                    {posInfo && (
                      <span className="suggestion-pos">{posInfo}</span>
                    )}
                  </div>
                  {definition && (
