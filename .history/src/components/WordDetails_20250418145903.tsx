...
interface WordDetailsProps {
  wordInfo: WordInfo;
  etymologyTree?: any; // Made optional for testing
  isLoadingEtymology?: boolean; // Made optional
  etymologyError?: string | null; // Made optional
  onWordLinkClick: (word: string | number) => void; // Allow number for IDs
  onEtymologyNodeClick?: (node: any) => void; // Made optional
}
...
const WordDetails: React.FC<WordDetailsProps> = React.memo(({
  ...
  const renderWordLink = (word: RelatedWord | { lemma: string, id?: number | string } | string, key?: React.Key): React.ReactNode => {
    ...
    // Ensure id is defined before calling onWordLinkClick
    const handleClick = () => {
        if (id !== undefined) {
            onWordLinkClick(id);
        }
      ...
      <Box>
        ...
        {wordInfo.tags && (
          <Stack direction="row" spacing={1} sx={{ mt: 1, flexWrap: 'wrap' }}>
            ...
            {(typeof wordInfo.tags === 'string' ? wordInfo.tags : '').split(',').map((tag, index) => (
              tag.trim() && <Chip key={index} label={tag.trim()} size="small" sx={{ bgcolor: 'var(--chip-bg-color)', color: 'var(--chip-text-color)' }} />
            ))}
          </Stack>
        )}
      </Box>
  ...
  // --- DEBUG: Correctly Comment out other render functions --- 
  /* 
  ...
  */
  ...
  const availableTabs = useMemo(() => {
    const tabs: Array<{ label: string; value: string; content: () => React.ReactNode; disabled: boolean }> = [
      { label: 'Definitions', value: 'definitions', content: renderDefinitionsTab, disabled: !wordInfo?.definitions?.length },
      // --- DEBUG: Comment out other tabs --- 
      /* 
      { label: 'Etymology', value: 'etymology', content: renderEtymologyTab, disabled: !wordInfo?.etymologies?.length && !etymologyTree },
      { label: 'Relations', value: 'relations', content: renderRelationsTab, disabled: !wordInfo?.incoming_relations?.length && !wordInfo?.outgoing_relations?.length && !wordInfo?.semantic_network },
      { label: 'Forms/Templates', value: 'forms', content: renderFormsAndTemplatesTab, disabled: !wordInfo?.forms?.length && !wordInfo?.templates?.length },
      { label: 'Sources/Meta', value: 'sources', content: renderSourcesInfoTab, disabled: false },
      */
      // --- END DEBUG --- 
    ];
    // Filter out explicitly disabled tabs
    let enabledTabs = tabs.filter(tab => !tab.disabled);
    
    // If no tabs are enabled (e.g., word has no definitions), create a fallback message tab
    if (enabledTabs.length === 0 && wordInfo) { // Check if wordInfo exists before showing fallback
       enabledTabs = [{
         label: 'Info', 
         value: 'info',
         content: () => <Typography sx={{p: 2}}>No specific details found for this word in the available categories.</Typography>,
         disabled: false
       }];
    } else if (!wordInfo) {
       // Handle the case where wordInfo itself is null (e.g., during initial load or error)
       enabledTabs = [{
          label: 'Loading', 
          value: 'loading',
          content: () => <CircularProgress sx={{display: 'block', margin: '2rem auto'}}/>,
          disabled: false
       }];
    }

    return enabledTabs;
    // Update dependencies as tabs are re-enabled
  }, [wordInfo]); 

  // Reset tab if current activeTab is no longer available or word changes
  useEffect(() => {
    // If wordInfo changes OR the available tabs change...
    if (availableTabs.length > 0) {
      // Check if the current activeTab is still valid among the available tabs
      const activeTabExists = availableTabs.some(tab => tab.value === activeTab);
      // If the active tab is no longer valid, or if it's the initial load/error state, reset to the first available tab
      if (!activeTabExists || activeTab === 'loading' || activeTab === 'info') {
        setActiveTab(availableTabs[0].value);
      }
    } else if (!wordInfo) {
        // Explicitly set to loading if wordInfo is null
        setActiveTab('loading');
    }
    // Key dependency: wordInfo.id ensures this runs when the word itself changes
  ...
  return (
    ...
    >
      ...
      <Box sx={{ flexGrow: 1, display: 'flex', flexDirection: isWideScreen ? 'row' : 'column', overflow: 'hidden' }}>
        ...
        >
          {/* Ensure availableTabs is not empty before mapping */} 
