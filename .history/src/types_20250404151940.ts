...
export interface RawEtymology {
  ...
  components?: EtymologyComponent[] | null;
...
export interface RawWordComprehensiveData {
  ...
  tags?: string | undefined; // String from backend
  data_hash?: string;
  search_text?: string;
  created_at?: string;
  updated_at?: string;
...
export interface WordInfo {
  ...
  root_word_id?: number | null;
  preferred_spelling?: string | null;
  tags?: string | null; // String from backend
  ...
  // REMOVED Fields not in comprehensive response (matching Raw type removals)
}

// Search Result Types
export interface SearchWordResult extends BasicWord {
  definitions?: Array<{ id: number; definition_text: string; standardized_pos: PartOfSpeech | null }>;
}

export interface SearchResult {
  words: SearchWordResult[];
  total: number;
  query: string; // The original query term
  filters: SearchOptions; // The filters applied, echo'd back from backend
}
...
export interface Statistics {
  ...
  total_affixations?: number;
  words_with_examples?: number; // If backend adds these
  words_with_etymology?: number; // If backend adds these
  words_with_relations?: number; // If backend adds these
  words_with_baybayin?: number; // If backend adds these
  words_by_language?: Record<string, number>;
  words_by_pos?: Record<string, number>;
  verification_stats?: Record<string, number>; // If backend adds these
  quality_distribution?: Record<string, number>; // If backend adds these
  update_frequency?: Record<string, number>; // If backend adds these
}

export interface WordNetworkOptions {
  max_depth?: number; // Match backend parameter name
  bidirectional?: boolean; // Match backend parameter name
}
...
// ADD EtymologyComponent type based on backend helper
export interface EtymologyComponent {
  original_text: string; // The full matched component text
  word_part?: string; // The identified word part
  language?: string; // The identified language code
  meaning?: string; // Any inferred meaning
  // Add other fields if the backend helper provides more
}

// ADD Relationship Type Info based on backend endpoint
