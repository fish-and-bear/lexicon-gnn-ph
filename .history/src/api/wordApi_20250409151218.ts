...
export async function fetchWordNetwork(
  ...
  while (retryCount <= maxRetries) {
    try {
      ...
      // Actual API request - using proper parameters matching the backend API
      const requestPromise = api.get(`/words/${encodedWord}/semantic_network`, { 
        params: { 
          depth: sanitizedDepth,
          breadth: 20, // Default breadth parameter expected by the backend API
          relation_types: include_affixes ? null : 'synonym,antonym,hypernym,hyponym'
        }
      });
      ...
      // Backend returns nodes, links (not edges), and stats
      ...
      // Map links to edges
      const edges = networkData.links.map((e: any) => {
        return {
          source: e.source,
          target: e.target,
          type: e.type || 'related',
      ...
      // Create metadata from stats or construct default
      const metadata = {
