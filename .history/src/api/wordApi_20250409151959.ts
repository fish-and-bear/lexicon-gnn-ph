...
export async function fetchWordNetwork(
  ...
  while (retryCount <= maxRetries) {
    try {
      ...
      // Actual API request with correct parameters for the backend
      const requestPromise = api.get(`/words/${encodedWord}/semantic_network`, { 
        params: { 
          depth: sanitizedDepth,
          breadth: 20  // Default breadth parameter expected by the backend
        }
      });
      ...
      // Process the backend API response format
      ...
      // Check both possible formats - backend may return nodes+links or nodes+edges
      const hasNodes = Array.isArray(networkData.nodes);
      const hasLinks = Array.isArray(networkData.links);
      const hasEdges = Array.isArray(networkData.edges);
      
      if (!hasNodes || (!hasLinks && !hasEdges)) {
      ...
      const nodes = (networkData.nodes || []).map((n: any) => {
        return {
          id: n.id, 
          label: n.label || n.lemma || String(n.id), 
          word: n.label || n.lemma || String(n.id),
          language: n.language || n.language_code || 'tl',
          type: n.type || (n.main ? 'main' : 'related'),
      ...
      // Process edges - handle both 'links' and 'edges' field names
      const edgeData = hasLinks ? networkData.links : (hasEdges ? networkData.edges : []);
      const edges = edgeData.map((e: any) => {
        return {
          source: e.source || e.from,
          target: e.target || e.to,
          type: e.type || e.directed || 'related',
        };
      });
      
      // Process metadata - could be in stats or directly in the response
      ...
      // Construct final object in the format expected by the frontend
  ...
  let userMessage = `No response received from server in fetching word network for '${sanitizedWord}'. Check network connection and backend status.`;
  
  if (lastError && typeof lastError.message === 'string') {
    if (lastError.message.includes('404')) {
      userMessage = `Word '${sanitizedWord}' not found in the database.`;
    } else if (lastError.message.includes('CORS')) {
      userMessage = `CORS error fetching network for '${sanitizedWord}'. Ensure backend CORS settings are correct.`;
    }
  }
