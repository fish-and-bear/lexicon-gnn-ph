...
function normalizeWordData(rawData: any): WordInfo {
  ...
  if (wordData.definitions && Array.isArray(wordData.definitions)) {
    normalizedWord.definitions = wordData.definitions.map((def: any): Definition => ({
      ...
      examples: Array.isArray(def.examples) ? def.examples : splitSemicolonSeparated(def.examples), 
...
export async function fetchWordDetails(word: string): Promise<WordInfo> {
  ...
  try {
    ...
    try {
      ...
      // Try to extract any valid definitions
        ...
        try {
          minimalWord.definitions = response.data.definitions.map((def: any) => ({
            ...
            examples: Array.isArray(def.examples) ? def.examples : 
                      (typeof def.examples === 'string' ? def.examples.split(';').map(s => s.trim()) : []),
            usage_notes: [],
            tags: [],
            sources: [],
            part_of_speech: null
...
export async function searchWords(
  ...
  try {
    // Clear and sanitize search options for API
    const apiOptions: Record<string, any> = { ...searchOptions };
    
    // Make sure q is set and sanitized
    apiOptions.q = normalizedQuery;
    
    // Remove any undefined or null values
    Object.keys(apiOptions).forEach(key => {
      if (apiOptions[key] === undefined || apiOptions[key] === null) {
        delete apiOptions[key];
      }
    });
    
    // Convert camelCase keys to snake_case for API
    if (apiOptions.perPage !== undefined) {
      apiOptions.per_page = apiOptions.perPage;
      delete apiOptions.perPage;
    }
    
    // Add fixed parameters for more robust handling
    apiOptions.include_metadata = true;
    ...
    console.log('Search API raw response:', responseData);
    
    // Create a base response object
    const formattedResponse: SearchResult = {
      query: normalizedQuery,
      count: responseData.count || 0,
      total: responseData.total || responseData.count || 0,
      offset: responseData.offset || 0,
      limit: responseData.limit || apiOptions.limit || 20,
      page: responseData.page || apiOptions.page || 1,
      perPage: responseData.per_page || apiOptions.per_page || 20,
      mode: responseData.mode || apiOptions.mode || 'all',
      language: responseData.language || apiOptions.language
    };
    ...
    if (Array.isArray(responseData.results) && responseData.results.length > 0) {
      ...
      // Extract just word objects for older code that expects them
      formattedResponse.words = responseData.results.map((item: any) => ({
        id: item.id,
        lemma: item.lemma || '',
        normalized_lemma: item.normalized_lemma || item.lemma || '',
        language_code: item.language_code || 'tl',
        has_baybayin: item.has_baybayin || false,
        baybayin_form: item.baybayin_form || null,
        romanized_form: item.romanized_form || null,
        definitions: Array.isArray(item.definitions) ? 
          item.definitions.map((def: any) => ({
            id: def.id,
            definition_text: def.definition_text || '',
            part_of_speech: def.part_of_speech || def.original_pos || null
          })) : []
      }));
    } 
    // Handle old API response format
    else if (Array.isArray(responseData.words) && responseData.words.length > 0) {
      formattedResponse.words = responseData.words;
      formattedResponse.results = responseData.words;
    } 
    // If count > 0 but no results were included, try a fallback request without include_full
    else if (responseData.count > 0 && (!Array.isArray(responseData.results) || responseData.results.length === 0) && apiOptions.include_full === true) {
      console.log('Search API returned count but no results. Making fallback request without include_full');
      
      // Create fallback options
      const fallbackOptions = { ...apiOptions, include_full: false };
      
      try {
        const fallbackResponse = await api.get('/search', { params: fallbackOptions });
        if (fallbackResponse.status === 200 && fallbackResponse.data) {
          ...
          if (Array.isArray(fallbackData.results) && fallbackData.results.length > 0) {
            formattedResponse.results = fallbackData.results;
            formattedResponse.words = fallbackData.results.map((item: any) => ({
              id: item.id,
              lemma: item.lemma || '',
              normalized_lemma: item.normalized_lemma || item.lemma || '',
              language_code: item.language_code || 'tl',
          ...
          } else if (Array.isArray(fallbackData.words) && fallbackData.words.length > 0) {
            formattedResponse.words = fallbackData.words;
            formattedResponse.results = fallbackData.words;
          }
        }
      } catch (fallbackError) {
        console.error('Error in fallback search request:', fallbackError);
        // Continue with original (empty) results if fallback fails
      }
    }
    // Empty results
    else {
      console.warn('Unexpected search response format, no results/words array found:', responseData);
      formattedResponse.results = [];
      formattedResponse.words = [];
    }
    ...
    // Failure already recorded by interceptor if not bypassed
    // Throw a more specific error using handleApiError
    await handleApiError(error, `searching for '${normalizedQuery}'`);
    // The line below won't be reached due to handleApiError throwing
    throw new Error(`Search error for '${normalizedQuery}'`); // Fallback if handleApiError doesn't throw as expected
  }
}
