...
export const fetchWordNetwork = async (
  ...
  try {
    circuitBreaker.checkState(); // Check if circuit is open
    ...
    const response = await cacheableAxios.get<WordNetworkResponse>(url);
    
    if (response.status !== 200) {
      throw new Error(`Failed to fetch word network: ${response.statusText}`);
    }
    
    console.log(`[API] Network response received with status ${response.status}`);
    
    // Ensure the response has the expected structure
    const data = response.data;
    
    if (!data || !data.nodes) {
      console.error('[API] Invalid network data format:', data);
      throw new Error('Invalid network data format returned from API');
    }
    
    // Fix common issues with API response
    const processedData: WordNetworkResponse = {
      ...
      stats: data.stats || {
        ...
        edge_count: (data.edges || data.links || []).length,
        depth,
        breadth
      }
    };
    ...
    console.log(`[API] Processed network: ${processedData.nodes.length} nodes, ${processedData.edges?.length || 0} edges`);
    return processedData;
  } catch (error) {
    if (error instanceof CircuitBreakerError) {
      console.error('[API] Circuit breaker open, network request rejected');
      throw error;
    }
    
    if (axios.isAxiosError(error)) {
      if (error.response?.status === 404) {
        console.warn(`[API] No network data found for word "${word}"`);
        // Return minimal network with just the main word
        return {
          nodes: [{ id: 1, lemma: word, language_code: 'tl' }],
          edges: [],
          stats: { node_count: 1, edge_count: 0, depth, breadth }
        };
      }
      
      if (error.response) {
        circuitBreaker.recordFailure();
        console.error(`[API] Network request failed with status ${error.response.status}:`, error.response.data);
        throw new Error(`Network request failed: ${error.response.status} ${error.response.statusText}`);
      } else if (error.request) {
        circuitBreaker.recordFailure();
        console.error('[API] Network request failed with no response:', error.request);
        throw new Error('No response received from server. Check your connection.');
      }
    }
    
    circuitBreaker.recordFailure();
    console.error('[API] Unexpected error fetching word network:', error);
    throw error;
  }
};
...
export async function fetchWordDetails(word: string): Promise<WordInfo> {
  ...
  try {
    // First try direct word lookup
    let endpoint = `/words/${encodeURIComponent(normalizedWord)}`;
    ...
    try {
      ...
      if (response.status === 200 && response.data) {
        console.log("Direct word lookup successful");
        const normalizedData = normalizeWordData(response.data);
        setCachedData(cacheKey, response.data);
        return normalizedData;
      }
    } catch (directLookupError) {
      console.log(`Direct lookup for ${normalizedWord} failed, falling back to search:`, directLookupError);
      // Continue to fallback - don't rethrow yet
    }
    
    // Fallback: Try search if direct lookup failed
    console.log(`Falling back to search for: ${normalizedWord}`);
    ...
    if (searchResponse.status === 200 && searchResponse.data) {
      ...
      // If we reached here, search didn't find usable results
      throw new Error(`No matching words found for "${normalizedWord}"`);
    }

    // If we reach here, both direct lookup and search failed
    throw new Error(`Failed to find word "${normalizedWord}"`);

  } catch (error: unknown) {
    // Failure is recorded by the interceptor
...
export async function testApiConnection(): Promise<boolean> {
  try {
    // First try the test endpoint directly - this is the most reliable test
    console.log("Testing API connection with /test endpoint");
    const response = await api.get('/test', { timeout: 3000 }); // Short timeout
    if (response.status === 200) {
      console.log("API /test endpoint is available and responding");
      
      // Try a simple search as secondary test to verify search functionality
      try {
        console.log("Testing search endpoint with simple query");
        const searchResponse = await api.get('/search', { 
        ...
        if (searchResponse.status === 200) {
          console.log("Search endpoint is working properly");
          return true;
        } else {
          console.warn("Search endpoint returned unusual status:", searchResponse.status);
          // Even if search is weird, /test worked, so API is technically available
          return true;
        }
      } catch (searchError) {
        console.warn("Search endpoint test failed, but /test is working:", searchError);
        // API is still considered available if /test works, even if search has issues
        return true;
      }
    }
    return false;
    ...
    try {
      ...
      // Last resort: try direct fetch to localhost:10000
      try {
        console.log("Trying last resort direct fetch to localhost:10000");
        const lastResortResponse = await fetch('http://localhost:10000/api/v2/test', {
          method: 'GET',
        ...
        if (lastResortResponse.ok) {
          console.log("Direct fetch to localhost:10000 succeeded - saving as endpoint");
          localStorage.setItem('successful_api_endpoint', 'http://localhost:10000');
          return true;
        }
      } catch (lastResortError) {
        console.error("Last resort connection attempt failed:", lastResortError);
      }
