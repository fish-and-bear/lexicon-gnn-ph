  ...
  try {
    ...
    const wordInfo: WordInfo = {
      ...
      definitions: (data.definitions || []).map((def: any) => ({
        ...
        sources: Array.isArray(def.sources) ? def.sources : (def.sources ? def.sources.split(',').map(s => s.trim()) : []),
      ...
      etymologies: (data.etymologies || []).map((etym: any) => ({
        ...
        sources: Array.isArray(etym.sources) ? etym.sources : (etym.sources ? etym.sources.split(',').map(s => s.trim()) : []),
        confidence_level: etym.confidence_level || 'medium',
        verification_status: etym.verification_status || 'unverified',
