...
export async function fetchWordNetwork(word: string, options: NetworkOptions = {}, signal?: AbortSignal): Promise<ApiWordNetwork> {
  ...
  const url = `/words/${encodeURIComponent(sanitizedWord)}/semantic_network`;
  // Removed caching logic - rely on react-query
  try {
    // Correctly pass signal in the config object
    const response = await api.get<ApiWordNetwork>(url, {
      params: options,
      signal: signal,
    });

    // Basic validation, return raw data for react-query to handle
    if (!response.data || !response.data?.nodes || !response.data?.links || !response.data?.metadata) {
       // Return a minimal valid structure on error or empty data
       console.warn(`Received incomplete network data for ${sanitizedWord}`);
       return {
         nodes: [],
         links: [],
         metadata: { root_word: sanitizedWord, nodes:[], links:[], metadata:{root_word: sanitizedWord} } as ApiWordNetwork['metadata']
       };
    }
    return response.data;
  } catch (error: any) {
     // If it's an abort error (timeout or manual), return minimal structure
    if (error.name === 'AbortError' || axios.isCancel(error)) {
      console.warn(`Network fetch aborted for ${sanitizedWord}: ${error.message}`);
      return {
        nodes: [],
        links: [],
        metadata: { root_word: sanitizedWord, nodes:[], links:[], metadata:{root_word: sanitizedWord} } as ApiWordNetwork['metadata']
    ...
    // For other errors, log and return minimal structure to prevent crash
    console.error(`Error fetching word network for ${sanitizedWord}`, error);
    return {
      nodes: [],
      links: [],
      metadata: { root_word: sanitizedWord, nodes:[], links:[], metadata:{root_word: sanitizedWord} } as ApiWordNetwork['metadata']
    };
     // Or uncomment to let handleApiError manage it (which might throw)
     // return handleApiError(error, `fetching word network for ${sanitizedWord}`);
  }
}
...
export async function fetchWordDetails(word: string): Promise<WordInfo> {
  ...
  try {
    ...
    // Use the original word identifier in error context
    return handleApiError(error, `fetching word details for ${word}`);
  }
}
...
export async function searchWords(query: string, options: SearchOptions): Promise<SearchResult> {
  ...
  // Prepare params, mapping frontend options (like page/per_page) to backend (like limit/offset) if necessary
  const apiParams: Record<string, any> = {
    ...options,
    q: sanitizedQuery,
  };
  // Example: Map page/per_page to limit/offset if backend expects that
  if (options.page && options.per_page) {
    apiParams.limit = options.per_page;
    apiParams.offset = (options.page - 1) * options.per_page;
    delete apiParams.page;
    delete apiParams.per_page;
  } else {
     // Use limit/offset directly if provided, or set defaults
     apiParams.limit = options.limit ?? 50;
     apiParams.offset = options.offset ?? 0;
  }

  // Removed caching logic - rely on react-query
  try {
    const response = await api.get<SearchResult>('/search', { params: apiParams });
    // Return raw data, assuming backend returns SearchResult structure
    // Add basic validation if needed
    if (!response.data || !response.data?.results) {
        console.warn(`Received incomplete search results for query "${sanitizedQuery}"`);
        return { count: 0, results: [], filters: options } as SearchResult; // Return empty result
    }
    return response.data;
  } catch (error) {
    return handleApiError(error, `searching words for "${sanitizedQuery}"`);
  }
}
...
export async function advancedSearch(query: string, options: SearchOptions): Promise<SearchResult> {
  const sanitizedQuery = sanitizeInput(query);
  const apiParams: Record<string, any> = { ...options, q: sanitizedQuery };
   // Map page/per_page to limit/offset if necessary (similar to searchWords)
  if (options.page && options.per_page) {
       apiParams.limit = options.per_page;
       apiParams.offset = (options.page - 1) * options.per_page;
       delete apiParams.page;
  ...
  try {
    const response = await api.get<SearchResult>('/search/advanced', { params: apiParams });
    if (!response.data || !response.data?.results) {
       console.warn(`Received incomplete advanced search results for query "${sanitizedQuery}"`);
       return { count: 0, results: [], filters: options } as SearchResult; // Return empty result
    }
    return response.data;
  } catch (error) {
    return handleApiError(error, `advanced searching for "${sanitizedQuery}"`);
  }
}
...
export async function getEtymologyTree(
  ...
  try {
    ...
    if (!response.data /* add more checks based on EtymologyTree structure */) {
        ...
        return { word: '', components: [], component_words: {}, metadata: {} } as EtymologyTree;
    }
    return response.data;
  } catch (error) {
    return handleApiError(error, `fetching etymology tree for word ID ${wordId}`);
  }
}
...
export async function fetchWordRelations(word: string): Promise<{
  ...
  try {
    ...
    // Consider returning empty object on error vs throwing
    console.error(`Error fetching relations for ${word}`, error);
    return { outgoing_relations: [], incoming_relations: [] };
    // return handleApiError(error, `fetching relations for ${word}`);
  }
}
...
export const searchBaybayin = async (query: string, options: { 
  ...
  try {
    ...
    return handleApiError(error, `searching baybayin for "${sanitizedQuery}"`);
  }
};
...
export const convertToBaybayin = async (text: string, language: string = 'fil'): Promise<any> => {
  ...
  try {
    ...
    return handleApiError(error, 'converting text to baybayin');
  }
};