...
export async function fetchWordNetwork(
  ...
  while (retryCount <= maxRetries) {
    try {
      ...
      console.log(`Attempting to fetch word network for ${encodedWord} (attempt ${retryCount + 1}/${maxRetries + 1})`);
      
      // Create a promise that will resolve with the API response or reject after timeout
      const timeoutPromise = new Promise<never>((_, reject) => {
        setTimeout(() => reject(new Error("Request timeout")), 15000); // 15 second timeout
      });
      
      // Actual API request - use a simpler structure that the backend expects
      const requestPromise = api.get(`/words/${encodedWord}/semantic_network`, { 
        params: { 
          max_depth: sanitizedDepth,
          include_affixes: include_affixes ? 1 : 0,
          include_etymology: include_etymology ? 1 : 0,
      ...
      // Check if the response is already in the format we expect
      if (networkData && networkData.nodes && networkData.edges) {
        processedData = networkData;
      } 
      // Check if the response has data property
      else if (networkData && networkData.data && networkData.data.nodes && networkData.data.edges) {
        processedData = networkData.data;
      }
      // Handle other possible formats
      else if (networkData && typeof networkData === 'object') {
        // Try to extract nodes and edges from whatever structure we received
        const nodes = networkData.nodes || networkData.vertices || [];
        const edges = networkData.edges || networkData.links || [];
        ...
        throw new Error('Invalid network data format received from API');
      }

      // Basic validation of the processed data
      if (!processedData || !Array.isArray(processedData.nodes) || !Array.isArray(processedData.edges)) {
        console.error('Invalid network data after processing:', processedData);
        throw new Error('Invalid network data structure received from API.');
      }
      
      // Map the nodes to ensure they have all required properties
      const nodes = processedData.nodes.map((n: any) => {
        // Ensure all required properties exist with fallbacks
        return {
          id: n.id, 
          label: n.label || n.lemma || n.word || String(n.id), 
          word: n.lemma || n.label || n.word || String(n.id),
          language: n.language || n.language_code || 'tl',
          type: n.type || n.node_type || 'unknown',
          depth: n.depth !== undefined ? n.depth : 0,
          definitions: n.definitions || [],
          path: n.path || [],
          has_baybayin: !!n.has_baybayin,
          baybayin_form: n.baybayin_form || null
        };
      });
      
      // Map the edges to ensure they have all required properties
      const edges = processedData.edges.map((e: any) => {
        return {
          source: e.source || e.from || e.source_id,
          target: e.target || e.to || e.target_id,
          type: e.type || e.relation_type || 'unknown',
        };
      });
      ...
      // Construct the final WordNetwork object
      const result: WordNetwork = {
        nodes,
        edges,
        metadata
      };
      ...
      console.error(`Error fetching word network (attempt ${retryCount + 1}/${maxRetries + 1}):`, error);
      ...
      // If it's not a network error or we've used all retries, break out of the loop
  ...
  const userMessage = `No response received from server in fetching word network for '${sanitizedWord}'. Check network connection and backend status.`;
...
export async function testApiConnection(): Promise<boolean> {
  try {
    // Log detailed info about the API configuration
    console.log("Testing API connection with the following configuration:");
    console.log("- CONFIG.baseURL:", CONFIG.baseURL);
    console.log("- Current api.defaults.baseURL:", api.defaults.baseURL);
    console.log("- Saved endpoint:", localStorage.getItem('successful_api_endpoint'));
    
    // Explicitly check port 10000 endpoint first
    const testUrl = 'http://localhost:10000/api/v2/test';
    console.log(`Testing direct connection to primary endpoint: ${testUrl}`);
    ...
    try {
      ...
      if (response.ok) {
        console.log(`✅ Primary API endpoint ${testUrl} is available and responding`);
        localStorage.setItem('successful_api_endpoint', 'http://localhost:10000/api/v2');
        localStorage.setItem('successful_api_endpoint_timestamp', Date.now().toString());
        
        // Update axios defaults to use this endpoint
        api.defaults.baseURL = 'http://localhost:10000/api/v2';
        
        circuitBreaker.recordSuccess();
        return true;
      } else {
        console.log(`❌ Primary API endpoint ${testUrl} returned status ${response.status}`);
      }
    } catch (fetchError) {
      console.warn(`❌ Direct fetch to primary endpoint failed:`, fetchError);
    }
    
    // Try multiple fallback endpoints if primary fails
    const endpoints = [
      '/test',                // Standard API test endpoint
      '/health',              // Health check endpoint if available
      '/random'               // Try the random endpoint as fallback
    ];
    
    // Try other potential base URLs if main one fails
    const baseUrls = [
      'http://localhost:10000/api/v2',
      'http://localhost:5000/api/v2',
      'http://127.0.0.1:10000/api/v2',
      'http://127.0.0.1:5000/api/v2'
    ];
    
    // Try different base URLs with the endpoints
    for (const baseUrl of baseUrls) {
      for (const endpoint of endpoints) {
        try {
          const fullUrl = `${baseUrl}${endpoint.startsWith('/') ? endpoint : '/' + endpoint}`;
          console.log(`Testing fallback endpoint: ${fullUrl}`);
          ...
          if (response.ok) {
            console.log(`✅ Fallback API endpoint ${fullUrl} is available and responding`);
            
            // Save the successful base URL to localStorage
            ...
            console.log(`❌ Fallback API endpoint ${fullUrl} returned status ${response.status}`);
          }
        } catch (fetchError) {
          console.warn(`❌ Direct fetch to ${baseUrl}${endpoint} failed:`, fetchError);
        }
      }
    }
    
    // Try with axios as a final fallback
    console.log("Falling back to axios request for API connection test");
    try {
      // Update axios timeout specifically for this call
      const axiosResponse = await api.get('/test', { timeout: 3000 });
      const isConnected = axiosResponse.status === 200;
      console.log(`Axios API test ${isConnected ? 'succeeded' : 'failed'}`);
    ...
    const errorMsg = "Cannot connect to the API server. Please ensure the backend server is running on port 10000 and CORS is properly configured.";
    console.error(errorMsg);
    
    circuitBreaker.recordFailure();
    return false;
  } catch (error) {
    console.error('API connection testing failed with unexpected error:', error);
    circuitBreaker.recordFailure();
    return false;
  }
}
...
export async function getRandomWord(): Promise<WordInfo> {
  ...
  try {
    ...
    // Construct the full random word URL
