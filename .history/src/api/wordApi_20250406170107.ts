...
export async function fetchWordNetwork(
  ...
  const sanitizedDepth = Math.min(Math.max(1, depth), 4); // Increase max allowed depth to 4
  const sanitizedBreadth = Math.min(Math.max(1, breadth), 40); // Add breadth validation
  ...
  try {
    ...
    const params: Record<string, any> = {
      ...
      breadth: sanitizedBreadth  // Include breadth parameter in API request
    };
