  ...
  try {
    ...
    const result: WordNetwork = {
        ...
        nodes: networkData.nodes.map((n: any) => ({ 
            ...
            group: n.group || n.language || 'unknown' // Example grouping
        })),
        edges: networkData.edges.map((e: any) => ({ 
            source: e.source, 
            target: e.target, 
            type: e.type || 'related' // Fallback type
        })),
        // Clusters might not be directly available from this endpoint
        clusters: { 
            etymology: [], 
            affixes: [], 
            synonyms: [], 
        ...
        metadata: networkData.metadata || { // Use metadata from response or create default
            word_id: networkData.metadata?.word_id || 0, // Assuming word_id is in metadata
            word: sanitizedWord,
            max_depth: sanitizedDepth,
...
function normalizeWordData(rawData: any): WordInfo {
  ...
  // Normalize Definitions - Use explicit RawDefinition type
  if (wordData.definitions && Array.isArray(wordData.definitions)) {
    normalizedWord.definitions = wordData.definitions.map((def: RawDefinition): Definition => ({
      ...
      created_at: def.created_at || undefined, // Use undefined for optional fields
      updated_at: def.updated_at || undefined,
  ...
  // Normalize Etymologies - Use explicit RawEtymology type
  if (wordData.etymologies && Array.isArray(wordData.etymologies)) {
    normalizedWord.etymologies = wordData.etymologies.map((etym: RawEtymology): Etymology => ({
      ...
      created_at: etym.created_at || undefined, // Use undefined for optional fields
      updated_at: etym.updated_at || undefined,
      // Fields required by the cleaned Etymology type
      text: etym.etymology_text || '', 
      languages: splitCommaSeparated(etym.language_codes), 
      components: splitCommaSeparated(etym.components), 
  ...
  // Normalize Pronunciations - Use explicit Pronunciation type
  if (wordData.pronunciations && Array.isArray(wordData.pronunciations)) {
    normalizedWord.pronunciations = wordData.pronunciations.map((pron: Pronunciation): Pronunciation => ({
      // Pronunciation type is already clean, just pass through
      id: pron.id,
      type: pron.type || '',
      value: pron.value || '',
      tags: pron.tags || null, 
      sources: pron.sources || null, 
      created_at: pron.created_at || undefined,
      updated_at: pron.updated_at || undefined,
    }));
  }

  // Normalize Credits - Use explicit Credit type
  if (wordData.credits && Array.isArray(wordData.credits)) {
    normalizedWord.credits = wordData.credits.map((cred: Credit): Credit => ({
      id: cred.id,
      credit: cred.credit || '',
      created_at: cred.created_at || undefined,
      updated_at: cred.updated_at || undefined,
    }));
  }
  ...
  // Normalize Derived Words - Use explicit BasicWord type (from Raw)
  if (wordData.derived_words && Array.isArray(wordData.derived_words)) {
    normalizedWord.derived_words = wordData.derived_words.map((dw: BasicWord): RelatedWord => ({
      id: dw.id,
      lemma: dw.lemma || '',
  ...
  // Normalize Outgoing Relations - Use explicit Relation type
  if (wordData.outgoing_relations && Array.isArray(wordData.outgoing_relations)) {
    normalizedWord.outgoing_relations = wordData.outgoing_relations.map((rel: Relation): Relation => ({
        // Relation type doesn't require much cleaning for WordInfo
        ...rel, // Spread existing properties
        // Ensure nested words are of the correct type (BasicWord)
        target_word: rel.target_word ? { ...rel.target_word } : undefined, 
        source_word: rel.source_word ? { ...rel.source_word } : undefined, 
    }));
  }

  // Normalize Incoming Relations - Use explicit Relation type
  if (wordData.incoming_relations && Array.isArray(wordData.incoming_relations)) {
    normalizedWord.incoming_relations = wordData.incoming_relations.map((rel: Relation): Relation => ({
        ...rel, 
        target_word: rel.target_word ? { ...rel.target_word } : undefined, 
        source_word: rel.source_word ? { ...rel.source_word } : undefined, 
    }));
  ...
  // Normalize Root Affixations - Use explicit Affixation type
  if (wordData.root_affixations && Array.isArray(wordData.root_affixations)) {
    normalizedWord.root_affixations = wordData.root_affixations.map((aff: Affixation): Affixation => ({
        ...aff,
  ...
  // Normalize Affixed Affixations - Use explicit Affixation type
  if (wordData.affixed_affixations && Array.isArray(wordData.affixed_affixations)) {
    normalizedWord.affixed_affixations = wordData.affixed_affixations.map((aff: Affixation): Affixation => ({
      ...aff,
      affixed_word: aff.affixed_word ? { ...aff.affixed_word } : undefined,
      root_word: aff.root_word ? { ...aff.root_word } : undefined,
    }));
...
export async function getRandomWord(): Promise<WordInfo> {
  ...
  try {
    // Assuming an endpoint like /words/random exists
