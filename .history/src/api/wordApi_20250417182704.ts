...
export async function fetchWordNetwork(word: string, options: NetworkOptions = {}, signal?: AbortSignal): Promise<ImportedWordNetwork> {
  ...
  try {
    ...
    const url = `${CONFIG.baseURL}/words/${encodeURIComponent(word)}/network?${params.toString()}`;
    ...
    if (error instanceof Error) {
      if (error.name === 'AbortError') {
        throw new Error(`Network request timed out when fetching network for word: ${word}`);
      }
    }
    await handleApiError(error, 'fetching word network');
    throw new Error(`Failed to fetch word network for ${word}.`);
  }
}
