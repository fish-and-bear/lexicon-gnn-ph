...
function normalizeWordData(rawData: any): WordInfo {
  ...
  console.log("Normalizing word data for:", wordData.lemma, `(ID: ${wordData.id})`);

  // Log the raw relation data RECEIVED by this function
  console.log("Raw data passed to normalizeWordData:", {
    incoming_relations: wordData.incoming_relations,
    outgoing_relations: wordData.outgoing_relations,
    definition_relations_top_level: wordData.definition_relations,
    definitions_with_relations_count: wordData.definitions?.filter((def: any) => def.definition_relations?.length > 0).length
  });
  ...
  const normalizedWord: WordInfo = {
    ...
    root_affixations: [],
    affixed_affixations: [],
    definition_relations: [], // Initialize definition_relations
  ...
  // Process definition_relations if present in the API response
  if (wordData.definition_relations && Array.isArray(wordData.definition_relations)) {
    console.log(`Processing ${wordData.definition_relations.length} definition relations`);
    normalizedWord.definition_relations = wordData.definition_relations.map((rel: any) => ({
      id: rel.id || 0,
      definition_id: rel.definition_id || 0,
      word_id: rel.word_id || 0,
      relation_type: rel.relation_type || 'related',
      relation_data: rel.relation_data || {},
      related_word: rel.related_word ? {
        id: rel.related_word.id || 0,
        lemma: rel.related_word.lemma || 'Unknown',
        language_code: rel.related_word.language_code,
        has_baybayin: rel.related_word.has_baybayin || false,
        baybayin_form: rel.related_word.baybayin_form || null
      } : undefined
    }));
  } else {
    console.warn("No definition relations found in word data");
  }

  // Also process definition_relations inside definitions if they're there
  if (normalizedWord.definitions && normalizedWord.definitions.length > 0) {
    normalizedWord.definitions = normalizedWord.definitions.map(def => {
      // Check if this definition has definition_relations
      if (def.definition_relations && Array.isArray(def.definition_relations)) {
        console.log(`Found ${def.definition_relations.length} definition relations in definition ${def.id}`);
      } else if ((def as any).definition_relations && Array.isArray((def as any).definition_relations)) {
        // Handle case where definition_relations might be in the raw data but not properly typed
        def.definition_relations = (def as any).definition_relations.map((rel: any) => ({
          id: rel.id || 0,
          definition_id: rel.definition_id || def.id || 0,
          word_id: rel.word_id || 0,
          relation_type: rel.relation_type || 'related',
          relation_data: rel.relation_data || {},
          related_word: rel.related_word ? {
...
export async function fetchWordDetails(word: string): Promise<WordInfo> {
  ...
  try {
    ...
    console.log(`Making API request to ${endpoint} with parameters:`, {
    ...
    // Add detailed debug log for API response data
    console.log("API response data:", {
      id: response.data.id,
      lemma: response.data.lemma,
      incomingRelations: response.data.incoming_relations?.length || 0,
      outgoingRelations: response.data.outgoing_relations?.length || 0,
      definitionRelations: response.data.definition_relations?.length || 0,
      definitionsCount: response.data.definitions?.length || 0,
      hasDefinitionsWithRelations: response.data.definitions?.some((d: any) => d.definition_relations?.length > 0) || false,
