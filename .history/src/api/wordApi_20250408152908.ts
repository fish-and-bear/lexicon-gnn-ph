...
const CONFIG = CONFIG_OPTIONS[ENV as keyof typeof CONFIG_OPTIONS];
...
export const fetchWordNetwork = async (
  ...
  try {
    ...
    // Use direct fetch for better control over request
    ...
    const response: WordNetworkResponse = {
      ...
      main_word: data.main_word || word,
      main_word_id: data.main_word_id || (data.nodes[0]?.id || 0),
      stats: {
    ...
    console.log("[API] Successfully fetched word network with nodes:", response.nodes.length);
    ...
    throw new Error(error instanceof Error ? error.message : 'Failed to fetch word network');
  }
};
...
export async function getRandomWord(): Promise<WordInfo> {
  ...
  try {
    ...
    if (response.status !== 200) {
      console.error(`API returned status ${response.status} for random word`);
    ...
    // The API might return either a word ID or the full word data
    if (response.data && response.data.id) {
      // If we have a full word, normalize it
      return normalizeWordData(response.data);
    } else if (response.data && response.data.word_id) {
      // If we just have an ID, fetch the full details
      console.log(`Fetching details for random word ID: ${response.data.word_id}`);
      return await fetchWordDetails(response.data.word_id.toString());
    } else {
      throw new Error('Invalid random word response format');
    }
  } catch (error) {
    console.error("Error in getRandomWord:", error);
    await handleApiError(error, 'fetching random word');
    throw new Error('Failed to fetch random word. Please try again later.');
  }
}
