...
function normalizeWordData(rawData: any): WordInfo {
  ...
  if (wordData.outgoing_relations && Array.isArray(wordData.outgoing_relations)) {
    // >>> START DEBUG LOGGING <<<
    console.log(`[Normalize Debug] Processing ${wordData.outgoing_relations.length} raw outgoing relations. Sample:`, wordData.outgoing_relations[0]);
    // >>> END DEBUG LOGGING <<<
    ...
    // >>> START DEBUG LOGGING <<<
    console.log(`[Normalize Debug] Normalized ${normalizedWord.outgoing_relations.length} outgoing relations. Sample:`, normalizedWord.outgoing_relations[0]);
    // >>> END DEBUG LOGGING <<<
  } else {
    console.warn("[Normalize Debug] No outgoing relations found in raw word data");
  ...
  if (wordData.incoming_relations && Array.isArray(wordData.incoming_relations)) {
    // >>> START DEBUG LOGGING <<<
    console.log(`[Normalize Debug] Processing ${wordData.incoming_relations.length} raw incoming relations. Sample:`, wordData.incoming_relations[0]);
    // >>> END DEBUG LOGGING <<<
    ...
    // >>> START DEBUG LOGGING <<<
    console.log(`[Normalize Debug] Normalized ${normalizedWord.incoming_relations.length} incoming relations. Sample:`, normalizedWord.incoming_relations[0]);
    // >>> END DEBUG LOGGING <<<
  } else {
    console.warn("[Normalize Debug] No incoming relations found in raw word data");
  ...
  // >>> START DEBUG LOGGING <<<
  // Normalize Forms (Assuming structure based on WordInfo type)
  if (wordData.forms && Array.isArray(wordData.forms)) {
      console.log(`[Normalize Debug] Processing ${wordData.forms.length} raw forms. Sample:`, wordData.forms[0]);
      // FIX: Map raw form data to WordForm structure
      normalizedWord.forms = wordData.forms.map((form: any): WordForm => ({
          id: form.id,
          // word_id: form.word_id, // REMOVED: Not in WordForm type
          form: form.form_value || form.form || '', // Match WordForm.form property
          form_type: form.form_type || 'unknown', // Keep if backend provides
          tags: form.tags || null,
          form_metadata: form.form_metadata || null, // Keep if backend provides
          is_canonical: form.is_canonical || false,
          is_primary: form.is_primary || false,
          // Add created_at/updated_at if they exist in WordForm and are provided
          created_at: form.created_at || null,
          updated_at: form.updated_at || null,
      }));
      console.log(`[Normalize Debug] Normalized ${normalizedWord.forms.length} forms. Sample:`, normalizedWord.forms[0]);
  } else {
      console.warn("[Normalize Debug] No forms found in raw word data");
      normalizedWord.forms = [];
  }

  // Normalize Templates (Assuming structure based on WordInfo type)
   if (wordData.templates && Array.isArray(wordData.templates)) {
       console.log(`[Normalize Debug] Processing ${wordData.templates.length} raw templates. Sample:`, wordData.templates[0]);
       // FIX: Map raw template data to WordTemplate structure
       normalizedWord.templates = wordData.templates.map((template: any): WordTemplate => ({
           id: template.id,
           // word_id: template.word_id, // REMOVED: Not in WordTemplate type
           template_name: template.template_name || 'unknown',
           template_structure: template.template_structure || null, // Match WordTemplate property
           tags: template.tags || null,
           template_metadata: template.template_metadata || null, // Keep if backend provides
           // Map args and expansion if they exist in WordTemplate and are provided
           args: template.args || null, 
           expansion: template.expansion || null,
           created_at: template.created_at || null,
           updated_at: template.updated_at || null,
       }));
       console.log(`[Normalize Debug] Normalized ${normalizedWord.templates.length} templates. Sample:`, normalizedWord.templates[0]);
   } else {
       console.warn("[Normalize Debug] No templates found in raw word data");
       normalizedWord.templates = [];
   }
  // >>> END DEBUG LOGGING <<<
...
// Define options interface for fetchWordDetails
interface WordDetailsOptions {
...
export async function fetchWordDetails(
  ...
  if (isIdRequest) {
    ...
    cacheKey = `cache:wordDetails:id:${wordId}`; // Use ID-specific cache key
    ...
    cacheKey = `cache:wordDetails:${normalizedWord}`;
  }

  // Add options to cache key if provided
  if (options && cacheKey) {
  ...
  if (cacheKey) {
      ...
      if (cachedData) {
        console.log(`Cache hit for word details: ${word} with options: ${JSON.stringify(options)}`);
        try {
          return normalizeWordData(cachedData); 
        } catch (e) {
          console.warn('Error normalizing cached data, fetching fresh:', e);
          clearCache(); // Clear cache if normalization fails
        }
      }
      console.log(`Cache miss for word details: ${word} with options: ${JSON.stringify(options)}. Fetching from API...`);
  }
  ...
  try {
    ...
    // Construct parameters based on options or default to complete=true
    const params: Record<string, any> = {};
    if (options && Object.keys(options).length > 0) {
        // Map options to API parameters
        if (options.complete !== undefined) params.complete = options.complete;
        if (options.include_definitions !== undefined) params.include_definitions = options.include_definitions;
        if (options.include_pronunciations !== undefined) params.include_pronunciation = options.include_pronunciations;
        if (options.include_etymology !== undefined) params.include_etymologies = options.include_etymology;
        if (options.include_relations !== undefined) params.include_relations = options.include_relations;
        if (options.include_forms !== undefined) params.include_forms = options.include_forms;
        if (options.include_templates !== undefined) params.include_templates = options.include_templates;
        if (options.include_idioms !== undefined) params.include_idioms = options.include_idioms;
        if (options.include_root !== undefined) params.include_root = options.include_root;
        if (options.include_derived !== undefined) params.include_derived = options.include_derived;
        if (options.include_affixations !== undefined) params.include_affixations = options.include_affixations;
        if (options.include_definition_relations !== undefined) params.include_definition_relations = options.include_definition_relations;
        if (options.include_credits !== undefined) params.include_credits = options.include_credits;
        if (options.include_metadata !== undefined) params.include_metadata = options.include_metadata;
    } else {
        // Default: fetch everything if no specific options are given
