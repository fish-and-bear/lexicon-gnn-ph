...
function normalizeWordData(rawData: any): WordInfo {
  ...
  // Normalize Forms (Making checks more robust)
  if (wordData.forms && Array.isArray(wordData.forms)) {
      ...
      normalizedWord.forms = wordData.forms.map((form: any): WordForm => ({
          id: form.id,
          form: form.form_value || form.form || '',
          tags: form.tags || null,
      ...
      console.warn("[Normalize Debug] No valid forms array found in raw word data. Setting to empty.");
      normalizedWord.forms = []; // Ensure it's an empty array if input is invalid/missing
  }

  // Normalize Templates (Making checks more robust)
   if (wordData.templates && Array.isArray(wordData.templates)) {
       ...
       console.warn("[Normalize Debug] No valid templates array found in raw word data. Setting to empty.");
       normalizedWord.templates = []; // Ensure it's an empty array if input is invalid/missing
   }
  // >>> END DEBUG LOGGING <<<
