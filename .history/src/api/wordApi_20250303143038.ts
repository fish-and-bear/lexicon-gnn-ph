...
export async function fetchWordNetwork(word: string, options?: WordNetworkOptions): Promise<WordNetworkGraph> {
  try {
    ...
    const transformedData: WordNetworkGraph = {
      ...
      edges: data.edges || [],
      clusters: {
        etymology: data.clusters?.etymology || [],
        affixes: data.clusters?.affixes || [],
        synonyms: data.clusters?.synonyms || [],
        antonyms: data.clusters?.antonyms || [],
        variants: data.clusters?.variants || [],
        root_words: data.clusters?.root_words || [],
        derived_words: data.clusters?.derived_words || []
      },
      metadata: {
        root_word: data.metadata?.root_word || word,
        max_depth: data.metadata?.max_depth || 1,
        total_nodes: data.metadata?.total_nodes || 0,
        total_edges: data.metadata?.total_edges || 0,
        cluster_count: data.metadata?.cluster_count || 0
      }
    };
