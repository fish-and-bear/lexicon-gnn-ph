...
export async function fetchWordNetwork(word: string, options?: WordNetworkOptions): Promise<WordNetworkGraph> {
  try {
    ...
    const response = await api.get(`/word-network/${encodeURIComponent(sanitizedWord)}`, {
      params: {
    ...
    const transformedData: WordNetworkGraph = {
      ...
      nodes: (rawData.nodes || []).map((node: any) => ({
        ...
        word: String(node.word || node.lemma || ''),
        normalized_lemma: String(node.normalized_lemma || node.word || ''),
        language: String(node.language || 'tl'),
        has_baybayin: Boolean(node.has_baybayin),
        baybayin_form: node.baybayin_form || null,
        type: String(node.type || node.relation_type || 'other'),
      ...
      edges: (rawData.edges || []).map((edge: any) => ({
        ...
        type: String(edge.type || edge.relation_type || 'other'),
        weight: Number(edge.weight || 1)
      })),
      clusters: {
        etymology: Array.isArray(rawData.clusters?.etymology) ? rawData.clusters.etymology : [],
        affixes: Array.isArray(rawData.clusters?.affixes) ? rawData.clusters.affixes : [],
        synonyms: Array.isArray(rawData.clusters?.synonyms) ? rawData.clusters.synonyms : [],
        antonyms: Array.isArray(rawData.clusters?.antonyms) ? rawData.clusters.antonyms : [],
        variants: Array.isArray(rawData.clusters?.variants) ? rawData.clusters.variants : [],
        root_words: Array.isArray(rawData.clusters?.root_words) ? rawData.clusters.root_words : [],
        derived_words: Array.isArray(rawData.clusters?.derived_words) ? rawData.clusters.derived_words : []
      },
      metadata: {
        root_word: String(rawData.metadata?.root_word || word),
        max_depth: Number(rawData.metadata?.max_depth || options?.depth || 2),
        total_nodes: Number(rawData.metadata?.total_nodes || 0),
        total_edges: Number(rawData.metadata?.total_edges || 0),
        cluster_count: Number(rawData.metadata?.cluster_count || 0)
      }
    };
    ...
    if (!transformedData.nodes.length) {
      console.warn('No nodes found in the network data');
    }
    if (!transformedData.edges.length) {
      console.warn('No edges found in the network data');
    }
