...
export const fetchWordNetwork = async (
  ...
  try {
    ...
    try {
      ...
      console.warn(`Direct fetch failed: ${fetchError}. Falling back to axios.`);
      // If direct fetch fails, fall back to axios
...
export async function searchWords(
  ...
  try {
    ...
    // Try direct fetch first for more reliability
    try {
    ...
    // Return empty but valid result structure on error
    return {
      query: normalizedQuery,
      count: 0,
      total: 0,
      ...
      words: [],
      page: 1,
      perPage: searchOptions.limit || 20
    };
  }
}
