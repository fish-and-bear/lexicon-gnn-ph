...
export const fetchWordNetwork = async (
  ...
  try {
    ...
    // Normalize edges vs links naming
    ...
    // Try fallback using axios
    try {
      console.log("[API] Attempting fallback with axios...");
      const axiosResponse = await api.get(`/words/${encodeURIComponent(word)}/semantic_network`, {
        params: {
          depth,
          breadth,
      ...
      throw new Error(`Failed to fetch word network: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
};
...
export async function testApiConnection(): Promise<boolean> {
  try {
    ...
    const healthUrl = `http://localhost:10000/health`;
...
export async function getRandomWord(): Promise<WordInfo> {
  ...
  try {
    ...
    if (!data || !data.lemma) {
      throw new Error('Invalid random word data received');
    }
    ...
    try {
      const axiosResponse = await api.get('/random', { timeout: 10000 });
      if (axiosResponse.status === 200 && axiosResponse.data) {
        circuitBreaker.recordSuccess();
        return normalizeWordData(axiosResponse.data);
      }
      throw new Error('Failed to get random word from fallback');
    } catch (axiosError) {
      console.error('Axios fallback failed:', axiosError);
      throw new Error('No response received from server in fetching random word. Check network connection and backend status.');
    }
  }
}
