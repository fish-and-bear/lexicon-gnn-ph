...
export async function fetchWordNetwork(
  ...
  try {
    ...
    const params = new URLSearchParams({
      depth: sanitizedDepth.toString(),
      breadth: sanitizedBreadth.toString(),
      include_affixes: include_affixes.toString(),
      include_etymology: include_etymology.toString(),
      cluster_threshold: cluster_threshold.toString()
    });

    const fullUrl = `${CONFIG.baseURL}${endpoint}?${params.toString()}`;
    console.log(`Fetching word network from: ${fullUrl}`);
      ...
      if (axios.isAxiosError(axiosError)) {
        ...
        if (axiosError.response) {
          ...
          if (statusCode === 404) {
            ...
            throw new Error(`Server error: ${errorData?.error || 'Unknown internal server error'}`);
          } else {
            throw new Error(`API error (${statusCode}): ${errorData?.error || axiosError.message}`);
          }
        }
      }
