...
export async function getRandomWord(): Promise<WordInfo> {
  // DEBUG TRACKING - Add timestamp and call counter
  const timestamp = new Date().toISOString();
  console.log(`%c🔍 RANDOM ENDPOINT CALLED at ${timestamp}`, 'background: #ff6b6b; color: white; font-weight: bold; padding: 2px 5px; border-radius: 3px;');
  console.trace('RANDOM ENDPOINT CALL STACK TRACE'); // Show stack trace to identify caller
