...
export async function fetchWordNetwork(word: string, options: NetworkOptions = {}, signal?: AbortSignal): Promise<ApiWordNetwork> {
  ...
  if (cached) {
    // Ensure cached data matches the expected structure
  ...
  try {
    ...
    // Pass signal correctly in the Axios request config
    const response = await api.get<ApiWordNetwork>(`/words/${sanitizeInput(word)}/network`, { params, signal }); 
    ...
    if (response.data && response.data.nodes && response.data.links && response.data.metadata) {
      ...
      console.error(`Invalid data structure received from /network endpoint for word "${word}"`);
      // Return a minimal valid ApiWordNetwork structure instead of {}
      return minimalApiWordNetwork(word); 
    }
  } catch (error) {
    if (axios.isCancel(error)) {
      console.log('Request canceled:', error.message);
      // Return a minimal valid ApiWordNetwork structure on cancellation
      return minimalApiWordNetwork(word); 
    }
    console.error(`Error fetching word network for "${word}":`, error);
    // Return a minimal valid ApiWordNetwork structure on error
    return minimalApiWordNetwork(word); 
  }
}
