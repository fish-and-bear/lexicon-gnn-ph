...
export async function searchWords(query: string, options: SearchOptions): Promise<SearchResult> {
  ...
  try {
    ...
    if (axios.isAxiosError(error) && error.response?.status === 500) {
      ...
      // Handle database schema error related to completeness_score
      if (errorMessage.includes('column w.completeness_score does not exist')) {
        console.error('Database schema error: completeness_score column missing');
        ...
        const errorResult: SearchResult = {
          ...
          total: 0,
          query: query,
          error: 'Database schema error: The database is missing required columns. Please contact the administrator.'
        };
