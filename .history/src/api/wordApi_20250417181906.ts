...
export async function fetchWordNetwork(word: string, options: NetworkOptions = {}, signal?: AbortSignal): Promise<ApiWordNetwork> {
  ...
  try {
    ...
    // Pass signal correctly in the Axios request config & use correct endpoint
    const response = await api.get<ApiWordNetwork>(`/words/${sanitizeInput(word)}/semantic_network`, { params, signal }); 
