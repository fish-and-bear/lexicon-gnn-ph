...
async function handleApiError(error: unknown, context: string): Promise<never> {
  ...
  if (apiError.isAxiosError) {
    ...
    const errorMessage = apiError.response.data?.error || apiError.response.statusText;
    const errorId = apiError.response.data?.error_id;
    ...
    switch (status) {
      ...
      default:
        throw new Error(
          `Error during ${context}: ${errorMessage}${errorId ? ` (ID: ${errorId})` : ''}`
        );
    }
  }
...
export async function getPartsOfSpeech(): Promise<PartOfSpeech[]> {
  try {
    ...
    const response = await api.get<{ parts_of_speech: PartOfSpeech[] }>('/parts-of-speech');
    const data = response.data.parts_of_speech;
...
export async function getEtymologyTree(
  ...
  try {
    ...
    if (!data.word || !data.tree || !data.metadata) {
...
export async function getStatistics(): Promise<Statistics> {
  try {
    ...
    const response = await api.get<{ data: Statistics }>('/statistics');
