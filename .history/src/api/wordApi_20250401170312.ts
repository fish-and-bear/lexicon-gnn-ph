...
// Helper function to split strings by semicolon, trimming whitespace - REVISED
function splitSemicolonSeparated(value: string | string[] | undefined | null): string[] {
  if (Array.isArray(value)) {
    // If it's already an array, filter out non-strings and return
...
// Helper function to split strings by comma, trimming whitespace - REVISED
function splitCommaSeparated(value: string | string[] | undefined | null): string[] {
  if (Array.isArray(value)) {
    // If it's already an array, filter out non-strings and return
    return value.filter((item): item is string => typeof item === 'string');
  }
...
function normalizeWordData(rawData: any): WordInfo {
  ...
  if (wordData.definitions && Array.isArray(wordData.definitions)) {
    normalizedWord.definitions = wordData.definitions.map((def: any): Definition => ({
      ...
      text: def.definition_text || '', // Map definition_text to text
      part_of_speech: def.standardized_pos || null, // Map standardized_pos to part_of_speech
      ...
      relations: [], // Add missing required 'relations' field (empty array as default)
  ...
  if (wordData.etymologies && Array.isArray(wordData.etymologies)) {
    normalizedWord.etymologies = wordData.etymologies.map((etym: any): Etymology => ({
      ...
      text: etym.etymology_text || '', // Map etymology_text to text
      languages: splitCommaSeparated(etym.language_codes), // Map parsed language_codes to languages
      // Attempt to parse components, default to empty array
      components: Array.isArray(etym.components) ? etym.components : splitCommaSeparated(etym.components), 
      sources: splitCommaSeparated(etym.sources), // Ensure sources is string[]
