...
export async function fetchWordNetwork(
  ...
  try {
    ...
    if (!networkData.clusters) {
      console.error('Network data has no clusters property:', networkData);
      throw new Error('Invalid network data: missing clusters property');
    }
    
    if (!networkData.metadata) {
      console.error('Network data has no metadata property:', networkData);
      throw new Error('Invalid network data: missing metadata property');
    }
    ...
    // If we have data property, use that instead (new API format)
    if (networkData.data && typeof networkData.data === 'object') {
      console.log('Adapting API response format: using data property');
      networkData = networkData.data as WordNetwork;
    }
