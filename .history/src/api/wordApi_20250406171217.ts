...
export async function fetchWordNetwork(
  ...
  const {
    ...
    breadth = 15,
    timeout = 30000 // 30 seconds default timeout for larger networks
  } = options;
  
  // Allow up to depth 5, but warn about performance for very deep searches
  const sanitizedDepth = Math.min(Math.max(1, depth), 5);
  // Allow up to 100 breadth, but warn about performance for very wide searches
  const sanitizedBreadth = Math.min(Math.max(1, breadth), 100);
  
  const cacheKey = `cache:wordNetwork:${sanitizedWord}-${sanitizedDepth}-${relation_types.join(',')}-${sanitizedBreadth}`;
  ...
  try {
    ...
    console.log(`Fetching semantic network for "${sanitizedWord}" with depth=${sanitizedDepth}, breadth=${sanitizedBreadth}`);
    
    // Use timeout for larger networks
    const response = await api.get(endpoint, { 
      params,
      timeout: sanitizedDepth > 3 ? timeout : undefined // Only add timeout for deeper searches
    });
    ...
    setCachedData(cacheKey, response.data, 60 * 15); // Cache for 15 minutes
