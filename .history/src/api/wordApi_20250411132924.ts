...
export async function fetchWordNetwork(
  ...
  try {
    ...
    console.log(`Fetching word network from ${api.defaults.baseURL}${endpoint}`, params);
    ...
    try {
      ...
      // Check if we got a response before trying to parse it
      if (!response.ok) {
      ...
      if (!networkData.nodes || !networkData.links) {
        throw new Error('Missing required network data properties');
      }
      ...
      // Only throw connection error if it's actually a connection issue
      ...
      // For other errors, try axios as fallback
    ...
    // Only show backend connection error for actual connection issues
