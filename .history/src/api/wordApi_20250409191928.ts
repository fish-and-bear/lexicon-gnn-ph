...
export async function fetchWordNetwork(
  ...
  const cacheKey = `cache:wordNetwork:${sanitizedWord}-${sanitizedDepth}-${sanitizedBreadth}-${include_affixes}-${include_etymology}`;
  ...
  try {
    ...
    console.log(`Attempting to fetch word network from ${api.defaults.baseURL}/words/${encodedWord}/semantic_network`);
    ...
    // First try with direct fetch to diagnose any CORS or network issues
    try {
      const response = await fetch(`${api.defaults.baseURL}/words/${encodedWord}/semantic_network?depth=${sanitizedDepth}&breadth=${sanitizedBreadth}`, {
        method: 'GET',
        headers: {
          'Accept': 'application/json',
          'Content-Type': 'application/json',
      ...
      // Convert the response to match our WordNetwork type
      const wordNetwork: WordNetwork = {
        nodes: networkData.nodes.map((n: any) => ({
          id: n.id,
          label: n.label || n.word || String(n.id),
          word: n.word || n.label || String(n.id),
          language: n.language || 'tl',
          type: n.type || (n.main ? 'main' : 'related'),
          depth: n.depth || 0,
          has_baybayin: n.has_baybayin || false,
          baybayin_form: n.baybayin_form || null,
          normalized_lemma: n.normalized_lemma || n.label || String(n.id)
        })),
        edges: networkData.links.map((e: any) => ({
          source: e.source,
          target: e.target,
          type: e.type || 'related',
        ...
        metadata: networkData.metadata || {
          ...
          total_edges: networkData.links.length
        }
      };

      // Cache the result
      setCachedData(cacheKey, wordNetwork);
      return wordNetwork;
      ...
      const response = await api.get(`/words/${encodedWord}/semantic_network`, {
        params: {
          depth: sanitizedDepth,
          breadth: sanitizedBreadth,
          include_affixes,
      ...
      const wordNetwork: WordNetwork = {
        ...
        edges: response.data.links.map((e: any) => ({
          source: e.source,
          target: e.target,
          type: e.type || 'related',
          directed: e.directed || false
        ...
        metadata: response.data.metadata || {
          ...
          language_code: 'tl',
          depth: sanitizedDepth,
          total_nodes: response.data.nodes.length,
          total_edges: response.data.links.length
        }
      };
    ...
    // Check if the error is due to the backend not running
    if (error instanceof Error && (
      ...
      throw new Error(
        'Cannot connect to the backend server. Please ensure the backend server is running on port 10000.'
      );
    }
    throw error;
  }
