...
class PersistentCircuitBreaker {
  ...
  canMakeRequest(): boolean {
    // Only clear cache occasionally, not on every request
    if (Math.random() < 0.05) { // 5% chance to clear old cache
      clearOldCache();
    }
...
api.interceptors.request.use(
  (config) => {
    // Modify headers directly instead of reassigning to fix type error
    if (config.headers) {
