...
function normalizeWordData(data: any): WordInfo {
  ...
  if (wordData.definitions && Array.isArray(wordData.definitions)) {
    normalizedWord.definitions = wordData.definitions.map((def: any) => ({
      ...
      tags: def.tags || [],
      sources: def.sources || [],
      relations: (def.relations || []).map(rel => ({ ...rel, word: rel.word || '', sources: toArray(rel.sources) })) // Ensure word exists and sources is array
    }));
...
export async function fetchWordDetails(
  ...
  try {
    ...
    const cleanDefinitions: Definition[] = (rawData.definitions || []).map((def: RawDefinition): Definition => ({
      ...
      text: def.definition_text || def.text || '', // Ensure text exists
      part_of_speech: def.standardized_pos || null,
      examples: toArray(def.examples), // Use helper
      usage_notes: toArray(def.usage_notes), // Use helper
      tags: toArray(def.tags), // Use helper
      sources: toArray(def.sources), // Use helper
      relations: (def.relations || []).map(rel => ({ ...rel, word: rel.word || '', sources: toArray(rel.sources) })) // Ensure word exists and sources is array
    }));
    ...
    const wordInfo: WordInfo = {
      // Core fields (ensure non-nullable)
      id: rawData.id,
      lemma: rawData.lemma,
      ...
      source_info: rawData.source_info, // Keep as is for now
      
      // Transformed fields
      tags: toArray(rawData.tags), // Use helper
      pronunciation: cleanPronunciation,
      definitions: cleanDefinitions,
      etymologies: cleanEtymologies,
      credits: rawData.credits || [], // Assume credits are clean
      root_word: rawData.root_word,
      derived_words: rawData.derived_words || [],
      all_relations: cleanRelations,
      affixations: cleanAffixations,
      data_completeness: rawData.data_completeness,
      relation_summary: rawData.relation_summary,
      
      // TODO: Build the 'relations: Relations' object if needed 
      // This requires mapping cleanRelations back into the specific categories
      // For now, providing 'all_relations' might be sufficient for many use cases
      relations: {},
      
      // TODO: Transform idioms if needed (currently assuming clean)
      idioms: (rawData.idioms || []).map(idiom => typeof idiom === 'string' ? { text: idiom } : idiom),
      
      // Other potential fields (ensure they are optional in type)
