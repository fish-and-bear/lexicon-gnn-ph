...
export function resetCircuitBreaker() {
  ...
  try {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (key && (key.startsWith('cache:') || key.includes('circuit'))) {
...
export async function testApiConnection(): Promise<boolean> {
  ...
  // Create a new axios instance specifically for testing
  ...
  for (const endpoint of possibleEndpoints) {
    try {
      ...
      const response = await testAxios.get(endpoint, {
        headers: {
      ...
      if (response.status >= 200 && response.status < 300) {
        ...
        // Update the axios baseURL to use the successful endpoint
        if (endpoint.includes('/api/v2')) {
          api.defaults.baseURL = endpoint;
        } else {
          api.defaults.baseURL = `${endpoint}/api/v2`;
        }
      ...
      if (axios.isAxiosError(error)) {
        console.error(`API connection test error for ${endpoint}:`, {
          message: error.message,
          code: error.code,
          response: error.response ? {
            status: error.response.status,
