...
export async function fetchWordNetwork(
  ...
  while (retryCount <= maxRetries) {
    try {
      ...
      // Direct fetch to bypass potential Axios issues
      const response = await fetch(`${api.defaults.baseURL}/words/${encodedWord}/semantic_network?depth=${sanitizedDepth}&breadth=20`, {
        method: 'GET',
        headers: {
          'Accept': 'application/json',
          'Content-Type': 'application/json',
      ...
      console.log('Network data response:', networkData);

      if (!networkData) {
        throw new Error('Empty response received from API');
      }
      
      // Backend return format is { nodes: [...], links: [...], stats: {...} }
      // We need to convert 'links' to 'edges' for our frontend
      if (!networkData.nodes) {
        console.error('Response is missing nodes:', networkData);
      ...
      // Convert links to edges (backend uses 'links' but frontend expects 'edges')
      let edges: any[] = [];
      
      // Handle both 'links' (from backend) and 'edges' (fallback) field names
      if (Array.isArray(networkData.links)) {
        edges = networkData.links.map((link: any) => ({
          source: link.source,
          target: link.target,
          type: link.type || 'related'
        }));
      } else if (Array.isArray(networkData.edges)) {
      ...
      // Final result in the format expected by the frontend
