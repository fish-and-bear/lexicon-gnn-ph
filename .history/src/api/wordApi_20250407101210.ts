...
export const API_BASE_URL = 'https://fil-relex.onrender.com/api/v2';

const CONFIG = {
  development: {
    baseURL: 'https://fil-relex.onrender.com/api/v2',
    timeout: 10000,
    retries: 3,
    failureThreshold: 10,
    resetTimeout: 15000,
...
export const fetchWordNetwork = async (
  ...
  try {
    ...
    const baseUrl = "https://fil-relex.onrender.com/api/v2";
    ...
    try {
      ...
      if (directResponse.ok) {
        console.log("[API] Direct fetch network request succeeded");
        ...
        data.edges = data.edges.map(edge => ({
          ...
          type: edge.type || (edge as any).relationship || 'unknown'
        }));
        
        return data;
      } else {
        console.warn("[API] Direct fetch failed, falling back to axios");
      }
    } catch (directFetchError) {
      console.warn("[API] Direct fetch error, falling back to axios:", directFetchError);
    }
    
    // Fallback to axios if direct fetch fails
    ...
    console.log("[API] Word network response:", response.data);
    
    // Transform response if needed
    const data = response.data;
    
    // Make sure we have nodes and edges
    if (!data.nodes) data.nodes = [];
    if (!data.edges && data.links) {
      // Handle case where backend returns links instead of edges
      data.edges = data.links;
    } else if (!data.edges) {
      data.edges = [];
    }
    
    // Normalize edges format - ensure source and target are formatted correctly
    data.edges = data.edges.map(edge => {
      // Handle cases where source or target might be objects or primitives
      return {
        ...edge,
        source: typeof edge.source === 'object' ? edge.source.id : edge.source,
        target: typeof edge.target === 'object' ? edge.target.id : edge.target,
    ...
    console.error("[API] Error fetching word network:", error);
