...
import { getCache, setCache, clearCache, clearOldCache } from '../utils/caching';
...
async function handleApiError(error: unknown, context: string): Promise<never> {
  ...
  if (isApiError(error)) {
    ...
    if (error.response) {
      errorMessage = `API request failed with status ${error.response.status} (${error.response.statusText}) in ${context}.`;
     ...
     errorDetails.code = error.code; // Include Axios error code if available
  } else if (error instanceof Error) {
    errorMessage = `Error in ${context}: ${error.message}`;
  }

  // Throw a new error with a consistent structure if needed, or rethrow
  // For now, just rethrowing the original error after logging
  throw error; 
}
...
export async function fetchWordNetwork(
  ...
  const cachedData = getCache<WordNetwork>(cacheKey);
  if (cachedData) {
...
export async function fetchWordDetails(word: string): Promise<WordInfo> {
  ...
  try {
    ...
    setCache(cacheKey, response.data); // Cache the raw data
...
export async function searchWords(query: string, options: SearchOptions): Promise<SearchResult> {
  ...
  const cachedData = getCache<SearchResult>(cacheKey);
...
export async function getPartsOfSpeech(): Promise<PartOfSpeech[]> {
  ...
  const cachedData = getCache<PartOfSpeech[]>(cacheKey);
...
export async function getEtymologyTree(
  ...
  const cachedData = getCache<EtymologyTree>(cacheKey);
  if (cachedData) return cachedData;
...
export async function getStatistics(): Promise<Statistics> {
  ...
  const cachedData = getCache<Statistics>(cacheKey, 5 * 60 * 1000); // Cache for 5 mins
  if (cachedData) return cachedData;
