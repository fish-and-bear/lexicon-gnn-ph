...
export async function searchWords(queryOrOptions: string | SearchOptions, options?: SearchOptions): Promise<SearchResult> {
  ...
  try {
    ...
    // Increase timeout for search requests
    const response = await api.get('/search', { 
      params: apiParams,
      timeout: CONFIG.timeout * 4, // Quadruple the default timeout for search (was *2)
      maxContentLength: 1024 * 1024 * 10, // Allow up to 10MB response size for large result sets
    ...
    const searchResult: SearchResult = {
      words: (data.results || data.words || []).map((result: any): SearchWordResult => ({
        id: result.id,
        lemma: result.lemma,
      ...
      page: searchOptions.page || 1,
      perPage: searchOptions.per_page || searchOptions.limit || (data.results?.length || 0), 
