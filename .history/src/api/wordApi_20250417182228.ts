...
export async function fetchWordNetwork(word: string, options: NetworkOptions = {}, signal?: AbortSignal): Promise<ApiWordNetwork> {
  ...
  try {
    ...
    if (signal) {
      // @ts-ignore // Suppress TS error for signal property, might be env specific
    ...
    if (axios.isCancel(error)) {
      ...
      // Ensure error is also an Error instance before accessing message
      let cancelMessage = 'Request canceled';
      if (error instanceof Error) {
         cancelMessage = error.message;
      }
      console.log('Request canceled:', cancelMessage);
      return minimalApiWordNetwork(word); 
    }
    // Handle other errors
