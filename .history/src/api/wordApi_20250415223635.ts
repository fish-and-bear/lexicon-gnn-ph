...
export const convertToBaybayin = async (text: string, language_code: string = 'fil'): Promise<any> => {
  try {
    console.log(`API: Starting Baybayin conversion for "${text}" with language "${language_code}"`);
    ...
    // Debug the response
    console.log('API: Baybayin conversion response:', response.data);
    
    // If response exists but doesn't have baybayin property, add debugging
    if (response.data && !response.data.baybayin) {
      console.warn('API: Baybayin conversion response missing baybayin property:', response.data);
    }
    
    // Ensure proper Unicode encoding in the response
    if (response.data && response.data.baybayin) {
      try {
        // Log Unicode code points for debugging
        const codePoints = [...response.data.baybayin].map(char => 
          char.codePointAt(0)?.toString(16).toUpperCase()).join(' ');
        console.log(`API: Baybayin Unicode code points: ${codePoints}`);
      } catch (e) {
        console.error('API: Error logging code points:', e);
      }
    }
    
    return response.data;
  } catch (error) {
    console.error('API: Error in convertToBaybayin:', error);
