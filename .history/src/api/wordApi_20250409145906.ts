...
export async function testApiConnection(): Promise<boolean> {
  try {
    ...
    const endpoints = [
      ...
      '/api/v2/test',         // In case the base URL doesn't include /api/v2
      '/random'               // Try the random endpoint as fallback
    ];
    
    // Try all endpoints with direct fetch first
    for (const endpoint of endpoints) {
      try {
        const testUrl = `${CONFIG.baseURL}${endpoint.startsWith('/api') ? endpoint.substring(7) : endpoint}`;
        console.log(`Testing endpoint with direct fetch: ${testUrl}`);
        ...
        if (response.ok) {
          console.log(`✅ API endpoint ${testUrl} is available and responding`);
          
          // Save the successful base URL to localStorage
          const baseUrl = testUrl.substring(0, testUrl.indexOf(endpoint));
          ...
          console.log(`❌ API endpoint ${testUrl} returned status ${response.status}`);
        }
      } catch (fetchError) {
        console.warn(`❌ Direct fetch to ${endpoint} failed:`, fetchError);
      }
    }
