 ...
 */
export const convertToBaybayin = async (text: string, language_code: string = 'fil'): Promise<any> => {
  try {
    const response = await api.post('/baybayin/convert', {
      text,
      language_code
    });
    return response.data;
...
export const getQualityAssessment = async (filters: any) => {
  try {
    ...
    if (filters.pos) {
      queryParams.append('pos', filters.pos);
    }
    
    if (filters.min_completeness !== undefined) {
      queryParams.append('min_completeness', filters.min_completeness.toString());
    }
    
    if (filters.max_completeness !== undefined) {
      queryParams.append('max_completeness', filters.max_completeness.toString());
    }
    
    if (filters.include_issues !== undefined) {
      queryParams.append('include_issues', filters.include_issues.toString());
    }
    
    if (filters.issue_severity) {
      queryParams.append('issue_severity', filters.issue_severity);
    }
    
    if (filters.max_results) {
      queryParams.append('max_results', filters.max_results.toString());
    }
    
    const url = `/quality_assessment?${queryParams.toString()}`;
