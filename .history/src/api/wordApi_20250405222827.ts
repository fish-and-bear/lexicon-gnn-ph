...
api.interceptors.request.use(
  (config) => {
    logInfo(`Sending request to ${config.url}`);
    config.headers = {
      ...config.headers,
      'Origin': window.location.origin,
...
api.interceptors.response.use(
  (response) => {
    logInfo(`Received response from ${response.config.url}`);
...
export interface WordNetworkOptions {
  ...
  // cluster_threshold?: number; // Removed as unused in current implementation
}
...
export async function fetchWordNetwork(
  ...
  const sanitizedWord = word.toLowerCase(); // Use simple normalization
    const {
      ...
      // cluster_threshold = 0.3 // Destructuring removed as unused
    } = options;
...
// Helper function to safely parse JSON strings - COMMENTED OUT AS UNUSED
