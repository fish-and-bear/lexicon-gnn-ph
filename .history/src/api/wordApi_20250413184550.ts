...
export async function fetchWordDetails(word: string): Promise<WordInfo> {
  ...
  try {
    ...
    // Create a responseData variable to hold any partial data we might have
    let partialResponseData: any = null;
    ...
    if (axios.isAxiosError(error) && error.response?.status === 500) {
      ...
      if (error.response?.data && typeof error.response.data === 'object' && error.response.data.id) {
        partialResponseData = error.response.data;
      }
      
      // NEW: Check for relation_data column error
      if (errorMessage.includes('column r.relation_data does not exist')) {
        console.error('Missing relation_data column in database detected');
        // Return a partial word object with the basics but empty relations
        if (partialResponseData) {
          ...
          return normalizeWordData(partialData);
        }
        // If we can't recover the response data, throw a more specific error
        throw new Error(`Database schema error: Word found but relations couldn't be loaded due to a missing column.`);
      }
