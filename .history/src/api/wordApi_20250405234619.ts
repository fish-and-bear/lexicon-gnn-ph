...
export async function searchWords(
  ...
  try {
    ...
    // Check if we have a words array from an older API format
    else if (Array.isArray(data.words)) {
      console.log(`Mapping ${data.words.length} results from 'words' array`);
      searchResult.words = data.words.map((result: any): SearchWordResult => ({
        id: result.id,
        lemma: result.lemma,
        normalized_lemma: result.normalized_lemma || result.lemma,
        language_code: result.language_code || 'tl',
