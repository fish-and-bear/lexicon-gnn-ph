...
export async function fetchWordNetwork(
  ...
  try {
    ...
    console.log(`Fetching word network for ${encodedWord} with depth=${sanitizedDepth}, breadth=${sanitizedBreadth}`);
    ...
    // Cache the result
    setCachedData(cacheKey, wordNetwork, 300); // Cache for 5 minutes
    return wordNetwork;

  } catch (error) {
    return handleApiError(error, `fetching word network for "${word}"`);
  }
}
