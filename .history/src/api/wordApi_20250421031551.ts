 ...
 */
export const fetchSuggestions = async (query: string, limit: number = 10): Promise<string[]> => {
  if (!query || query.length < 2) {
    return []; // Don't fetch for empty or very short queries
  }
  try {
    // Assuming your API is mounted at /api/v2 based on routes.py setup
    ...
    // Example: if (axios.isAxiosError(error) && error.response?.status === 404) { ... }
