...
export async function testApiConnection(): Promise<boolean> {
  try {
    // First try the test endpoint directly with fetch - this is the most reliable test
    console.log("Testing API connection with direct fetch to /test endpoint");
    ...
    try {
      ...
      console.warn("Direct fetch test failed:", fetchError);
    }
    
    // Try axios as fallback
    console.log("Falling back to axios for API connection test");
    const response = await api.get('/test', { timeout: 3000 }); // Short timeout
    if (response.status === 200) {
      console.log("API /test endpoint is available and responding via axios");
      return true;
    }
    ...
    try {
      // Try direct fetch to localhost:5000 as fallback
      console.log("Testing direct connection to localhost:5000");
      const directResponse = await fetch('http://localhost:5000/api/v2/test', { 
        method: 'GET',
      ...
      if (directResponse.ok) {
        console.log("Direct connection to localhost:5000 succeeded");
        localStorage.setItem('successful_api_endpoint', 'http://localhost:5000');
        return true;
      }
    } catch (directError) {
