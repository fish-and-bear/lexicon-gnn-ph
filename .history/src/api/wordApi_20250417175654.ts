...
export async function fetchWordNetwork(word: string, options: NetworkOptions = {}, signal?: AbortSignal): Promise<ImportedWordNetwork> {
  ...
  try {
    ...
    if (!response.data || !response.data.nodes || !response.data.links) {
       ...
       return { nodes: [], links: [], metadata: { root_word: sanitizedWord } };
    }
    ...
    // For other errors, let handleApiError manage it (which might throw)
    // Or return minimal structure to prevent full crash if needed
    // return handleApiError(error, `fetching word network for ${sanitizedWord}`);
     ...
     return { nodes: [], links: [], metadata: { root_word: sanitizedWord } }; // Fallback
  }
}
...
export async function fetchWordDetails(word: string): Promise<WordInfo> {
  const sanitizedWord = sanitizeInput(word);
  const url = `/words/${encodeURIComponent(sanitizedWord)}`;
  // Removed caching logic - rely on react-query
  try {
    const response = await api.get(url);
    if (!response.data) {
    ...
    return handleApiError(error, `fetching word details for ${sanitizedWord}`);
  }
}
  ...
  try {
    const response = await api.get('/search', {
      params: {
        ...
        include_pronunciations: options.include_pronunciations,
        include_etymologies: options.include_etymologies,
        ...
        pos: options.pos,
        has_baybayin: options.has_baybayin,
...
export async function advancedSearch(query: string, options: SearchOptions): Promise<SearchResult> {
  ...
  try {
    const response = await api.get('/search/advanced', { params: { q: sanitizedQuery, ...options } });
    return response.data;
  } catch (error) {
...
export async function getEtymologyTree(
  ...
  // Removed caching logic
  try {
    const response = await api.get(`/words/${wordId}/etymology/tree`, { params: { depth: maxDepth } });
    return response.data;
  } catch (error) {
...
export async function fetchWordRelations(word: string): Promise<{
  ...
  const sanitizedWord = sanitizeInput(word);
  const url = `/words/${encodeURIComponent(sanitizedWord)}/relations`; // Assuming this endpoint exists
  ...
  try {
    ...
    console.error(`Error fetching relations for ${sanitizedWord}`, error);
    return { outgoing_relations: [], incoming_relations: [] };
    // return handleApiError(error, `fetching relations for ${sanitizedWord}`);
  }
}
