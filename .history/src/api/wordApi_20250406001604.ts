...
api.interceptors.response.use(
  ...
  (error) => {
    ...
    // Special handling for search requests - don't record all failures
    if (isSearchRequest) {
      console.log('Search request encountered an error - evaluating if it should affect circuit breaker');
      
      // For search requests, we're more lenient with circuit breaker tracking
      if (isNetworkError) {
        console.log('Network error for search request - not recording as circuit breaker failure');
        // Don't record network errors for search requests as failures
      } else if (error.response?.status === 404) {
        console.log('404 response for search request - not recording as circuit breaker failure');
        // Don't record 404s for search as failures (could be valid "not found")
      } else {
        // For server errors (500s), we do record the failure
        console.log('Server error for search request - recording circuit breaker failure');
        circuitBreaker.recordFailure();
      }
    } else {
      // For non-search requests, record all failures
      circuitBreaker.recordFailure();
    }
...
export async function directSearch(query: string): Promise<SearchResult> {
  console.log(`Performing direct search for "${query}" (bypassing all circuit breaker and caching)`);
