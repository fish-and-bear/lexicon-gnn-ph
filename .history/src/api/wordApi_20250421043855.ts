...
export async function fetchWordNetwork(
  ...
  const { depth = 2, breadth = 10, include_affixes = true, include_etymology = true, relation_types = [] } = options;
  console.log(`[API] Fetching word network for: '${word}', Depth: ${depth}, Breadth: ${breadth}`);

  // Determine the correct endpoint based on whether 'word' is an ID or lemma
  let endpoint;
  let identifierForCache;
  if (word.startsWith("id:")) {
    // If it's an ID, use it directly in the path. The backend route handles the "id:" prefix.
    endpoint = `/words/${word}/semantic_network`;
    identifierForCache = word; // Use "id:123" as part of the cache key
  } else {
    // If it's a lemma, encode it for the URL path
    endpoint = `/words/${encodeURIComponent(word)}/semantic_network`;
    identifierForCache = encodeURIComponent(word); // Use encoded lemma for cache key
  }

  const params = new URLSearchParams({
    depth: depth.toString(),
    breadth: breadth.toString(),
    include_affixes: include_affixes.toString(),
    include_etymology: include_etymology.toString(),
    ...(relation_types.length > 0 && { relation_types: relation_types.join(',') })
  });

  // Construct a cache key that includes the identifier and relevant options
  const cacheKey = `wordNetwork-${identifierForCache}-d${depth}-b${breadth}-aff${include_affixes}-ety${include_etymology}-rel${relation_types.join('_')}`;
  
  // Check cache first
  const cachedData = await getCachedData<ImportedWordNetwork>(cacheKey);
  if (cachedData) {
    console.log(`[API Cache] Cache hit for word network: ${cacheKey}`);
    return cachedData;
  }
  
  console.log(`[API Cache] Cache miss for word network: ${cacheKey}`);

  try {
    const url = `${endpoint}?${params.toString()}`;
    console.log(`[API] Calling URL: ${url}`);
    const response = await api.get<LocalWordNetwork>(url);
    const data = response.data;
    
    // --- Data Transformation --- 
    // Transform node and edge IDs to match expected numeric format for D3/WordGraph
    // Map backend string IDs (which are numeric strings) to numbers
    const nodeMap = new Map<string, number>(data.nodes.map((node, index) => [node.id, Number(node.id) || index]));
    
    const transformedNodes = data.nodes.map(node => ({
      ...node,
      id: Number(node.id) || 0, // Ensure ID is numeric
    }));
    ...
    const transformedEdges = data.edges.map(edge => {
        ...
        if (isNaN(sourceNum) || isNaN(targetNum)) {
            console.warn(`Invalid edge found with non-numeric IDs: ${edge.source} -> ${edge.target}`);
    ...
    }).filter((edge): edge is { source: number; target: number; type: string } => edge !== null);
    
    // Construct the final WordNetwork object with numeric IDs
    const finalNetwork: ImportedWordNetwork = {
        nodes: transformedNodes,
        edges: transformedEdges,
        metadata: {
            root_word: data.metadata.root_word || word, // Fallback to input word if needed
            normalized_lemma: data.metadata.normalized_lemma,
            language_code: data.metadata.language_code,
            depth: data.metadata.depth || depth,
            total_nodes: transformedNodes.length,
            total_edges: transformedEdges.length,
            query_time: data.metadata.query_time ?? undefined, // Handle potential null
            filters_applied: data.metadata.filters_applied
        }
    };
    
    console.log(`[API] Fetched word network for '${word}': ${finalNetwork.nodes.length} nodes, ${finalNetwork.edges.length} edges.`);
    
    // Store in cache
    await setCachedData(cacheKey, finalNetwork);
