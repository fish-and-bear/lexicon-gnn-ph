...
// Error handling function - REVISED for type safety
async function handleApiError(error: unknown, context: string): Promise<never> {
  ...
  if (isApiError(error)) {
    ...
    if (error.response) {
      // Ensure details assigned to errorMessage are strings
      ...
      // Add specific messages for known statuses
      if (error.response.status === 404) {
      ...
      // Safely attempt to get a message from details
  ...
  // Log the final error message before throwing
