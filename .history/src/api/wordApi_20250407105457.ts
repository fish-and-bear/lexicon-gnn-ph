...
export async function fetchWordDetails(word: string): Promise<WordInfo> {
  ...
  try {
    ...
    // Try direct fetch for search first
    try {
      ...
      console.warn(`Direct fetch search failed, error:`, searchFetchError);
    }
    ...
    } catch (axiosSearchError) {
      console.error(`Axios search failed, error:`, axiosSearchError);
    }
