...
export async function fetchWordDetails(
  ...
  try {
    ...
    const cleanDefinitions: Definition[] = (rawData.definitions || []).map((def: RawDefinition): Definition => ({
      ...
      part_of_speech: def.standardized_pos || null,
      examples: toArray(def.examples), 
      usage_notes: toArray(def.usage_notes), 
      tags: toArray(def.tags), // <--- Explicitly include tags
      sources: toArray(def.sources), 
      relations: (def.relations || []).map((rel: { id: number; type: string; word?: string; sources?: string | string[] }) => ({ 
        id: rel.id,
        type: rel.type,
        word: rel.word || '', 
    ...
    const wordInfo: WordInfo = {
      ...
      // Build the structured relations object
      relations: buildRelationsObject(cleanRelations), // Use a helper function
