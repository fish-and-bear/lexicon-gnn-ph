...
import { fetchWordNetwork, fetchWordSemanticNetwork } from '../api/wordApi';
...
export const fetchAndAddSemanticNetwork = async (
  ...
  try {
    ...
    // Use params compatible with fetchWordSemanticNetwork signature
    // Assuming it primarily needs root_word, language_code and optional params
    const params = {
      root_word: word,
      // Ensure language_code is passed if available and required/useful
      ...(wordInfo?.language_code && { language_code: wordInfo.language_code }), 
      depth: depth, // Pass depth
      // Optional: Pass other known/required params if available
      // breadth: breadth, // Pass breadth if the API supports it
    };

    // Fetch semantic network data
    const networkData = await fetchWordSemanticNetwork(params); // Use refined params
    ...
    // Explicitly include 'links' from networkData
    const semanticNetworkData: WordNetwork = {
        nodes: networkData.nodes,
        links: networkData.links, // Added missing links property
