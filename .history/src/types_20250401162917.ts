...
export interface RawDefinition {
  ...
  examples?: string | string[]; // Can be string or array
  usage_notes?: string | string[]; // Can be string or array
...
export interface RawEtymology {
  ...
  components?: string | string[]; // Can be string or array
  etymology_structure?: string;
  language_codes?: string | string[]; // Can be string or array
  sources?: string | string[]; // Can be string or array
...
export interface RelatedWord {
  word: string;
  // Removed sources from here as it's often not present/consistent in this simplified type
}
...
export interface WordInfo {
  id: number;
  lemma: string;
  normalized_lemma: string; // Ensure normalized_lemma is present
  language_code: string; // Ensure language_code is present
  preferred_spelling?: string | null;
  tags: string[]; // Always array
  has_baybayin: boolean; // Ensure boolean
  baybayin_form?: string | null;
  romanized_form?: string | null;
  pronunciation: CleanPronunciation | null; // Use cleaned pronunciation type
  source_info?: Record<string, any>; // Kept as is for now
  data_hash?: string;
  // Fields below might not always be available depending on source data quality
  ...
  // Use cleaned types for nested arrays
  definitions: Definition[];
  etymologies: Etymology[];
  relations: Relations; // This structure needs careful handling in transformation
  idioms: Idiom[]; // Assuming idioms are clean or transformation is simple
  // Add other potentially useful transformed fields if needed
  credits?: Credit[];
  root_word?: BasicWord | null;
  derived_words?: BasicWord[];
  all_relations?: CleanRelation[]; // Combined list of incoming/outgoing relations
  affixations?: CleanAffixation[]; // Combined list of affixations
  data_completeness?: Record<string, boolean>;
  relation_summary?: Record<string, number>;
}
