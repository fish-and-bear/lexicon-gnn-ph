...
def with_transaction(commit=True):
    ...
    def decorator(func):
        ...
        def wrapper(cur, *args, **kwargs):
            ...
            try:
                # Check transaction status without using set_session
                cur.execute("SELECT txid_current() IS NOT NULL")
                in_transaction = cur.fetchone()[0]
            except Exception:
                # If we can't execute the query, assume no transaction
                pass
            
            # Only set autocommit if we're not in a transaction
            if not in_transaction and original_autocommit:
                try:
                    conn.autocommit = False
                except Exception as e:
                    logger.warning(f"Failed to set autocommit in {func.__name__}: {str(e)}")
            ...
            try:
                if in_transaction:
                    ...
                    try:
                        ...
                        logger.warning(f"Failed to begin transaction in {func.__name__}: {str(e)}")
                ...
                # Restore original autocommit state only if we changed it
                if not in_transaction and original_autocommit:
                    try:
                        conn.autocommit = original_autocommit
                    except Exception as e:
                        logger.warning(f"Failed to restore autocommit in {func.__name__}: {str(e)}")
            ...
            for source in source_list:
                try:
                    ...
                    if source == "root-words":
                        ...
                        kaikki_filenames = ["kaikki.jsonl", "kaikki.jsonl"]
