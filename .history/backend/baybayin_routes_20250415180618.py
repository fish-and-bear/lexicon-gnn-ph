...
def search_baybayin():
    ...
    try:
        ...
        count_result = db.session.execute(sql_text(count_sql), params).scalar()
        ...
        query_result = db.session.execute(sql_text(sql), params)
...
def get_baybayin_statistics():
    ...
    try:
        ...
        # Execute all queries
        overview = db.session.execute(sql_text(sql_overview)).fetchone()
        by_language = db.session.execute(sql_text(sql_by_language)).fetchall()
        char_frequency = db.session.execute(sql_text(sql_char_frequency)).fetchall()
        completeness = db.session.execute(sql_text(sql_completeness)).fetchone()
