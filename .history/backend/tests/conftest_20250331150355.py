...
from backend.models import db, init_app as init_models
from backend.models import PartOfSpeech, Word, Definition, Etymology, Relation
...
# Create a test app and initialize models
test_app = Flask(__name__)
test_app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///:memory:'
test_app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
test_app.config['TESTING'] = True
db.init_app(test_app)
init_models(test_app)

with test_app.app_context():
    db.create_all()

# Now import the app after models are initialized
...
def _setup_test_data():
    ...
    try:
        ...
        db.session.add_all([aso, tuta, tahol])
        db.session.commit()
        ...
        db.session.add_all([aso_def, tuta_def, tahol_def])
        db.session.commit()
        ...
        aso_etym = Etymology(
            ...
            language_codes=['poz'],
            components=[{'text': '*asu', 'language': 'poz', 'meaning': 'dog'}],
        ...
        tuta_etym = Etymology(
            ...
            language_codes=['poz'],
            components=[{'text': '*tuta', 'language': 'poz', 'meaning': 'young animal'}],
            confidence_score=0.8
        )

        db.session.add_all([aso_etym, tuta_etym])
        db.session.commit()
        ...
        aso_tuta = Relation(
            ...
            target_word=tuta,
            type='hypernym',
            relation_metadata={'confidence_score': 0.9}
        )
        ...
        aso_tahol = Relation(
            ...
            target_word=tahol,
            type='related',
            relation_metadata={'confidence_score': 0.85}
        )

        db.session.add_all([aso_tuta, aso_tahol])
        db.session.commit()
