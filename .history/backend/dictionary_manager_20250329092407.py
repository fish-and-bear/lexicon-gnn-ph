...
def with_transaction(commit=True):
    """
    Decorator for database operations that ensures they run in a transaction.
    ...
    def decorator(func):
        ...
        def wrapper(cur, *args, **kwargs):
            ...
            if not in_transaction:
                try:
                    # Don't use set_session inside a transaction, just set autocommit directly
            ...
            try:
                ...
                # Only handle transaction if we started it
                ...
                if started_transaction:
                    try:
                        ...
                        logger.error(f"Failed to restore autocommit: {e}")
