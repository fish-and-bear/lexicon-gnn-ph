...
from sqlalchemy import or_, and_, func, desc, text, distinct, cast, not_, case, exists
    ...
    try:
        ...
        if oldest_created and newest_created:
            ...
            year_distribution = db.session.query(
                extract('year', Word.created_at), func.count(Word.id)
            ).filter(
                Word.id.in_(query.with_entities(Word.id))
            ).group_by(extract('year', Word.created_at)).all()
