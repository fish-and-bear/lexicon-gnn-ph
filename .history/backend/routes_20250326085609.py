    ...
    try:
        ...
        try:
            ...
            if base_query is None:
                return error_response("Search query construction failed", 500, error_code="ERR_SEARCH_CONSTRUCTION")
            
            # Limit results and eager load relationships
            base_query = base_query.options(
                joinedload(Word.definitions).joinedload(Definition.standardized_pos) if include_definitions else None,
                joinedload(Word.etymologies) if include_etymology else None
            )
