                ...
                try:
                    ...
                    if prons:
                        ...
                        for pron in prons:
                            ...
                            # Print the basic pronunciation
                            console.print(f"• {value}" + (f" ({type_})" if type_ else ""))
                            
                            # Handle tags safely
                            try:
                                if p_tags and isinstance(p_tags, str) and p_tags.strip():
                                    p_tags_list = json.loads(p_tags)
                                    if p_tags_list and isinstance(p_tags_list, list):
                                        console.print(f"  [dim]Tags:[/] {', '.join(p_tags_list)}")
                            except json.JSONDecodeError:
                                # If tags can't be parsed, just skip them silently
                                pass
                            except Exception as e:
                                # Log other errors but don't show them to the user
                                logger.warning(f"Error with pronunciation tags: {str(e)}")
                            
                            # Handle sources safely
                            try:
                                if p_sources and isinstance(p_sources, str) and p_sources.strip():
                                    p_sources_list = json.loads(p_sources)
                                    if p_sources_list and isinstance(p_sources_list, list):
                                        console.print(f"  [dim]Sources:[/] {', '.join(p_sources_list)}")
                            except json.JSONDecodeError:
                                # If sources can't be parsed, just skip them silently
                                pass
                            except Exception as e:
                                # Log other errors but don't show them to the user
                                logger.warning(f"Error with pronunciation sources: {str(e)}")
