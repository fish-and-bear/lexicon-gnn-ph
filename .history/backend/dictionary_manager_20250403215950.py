...
def insert_pronunciation(cur, word_id: int, pronunciation_data: Union[str, Dict],
    ...
    try:
        ...
        if not value:
            logger.warning(f"Empty pronunciation value for word ID {word_id} (source '{source_identifier}'). Skipping pronounicitaion insertion.")
            return None
...
def process_marayum_json(cur, filename: str, source_identifier: Optional[str] = None) -> Tuple[int, int]:
    ...
    if not effective_source_identifier:
        ...
        effective_source_identifier = SourceStandardization.standardize_sources(os.path.basename(filename))
        logger.warning(f"No source_identifier passed to process_marayum_json for {filename}, calculated '{effective_source_identifier}' as fallback.")

    # Log using the determined identifier
    logger.info(f"Processing Marayum file: {filename} with source: {effective_source_identifier}") # <-- Use effective_source_identifier
    ...
    with tqdm(total=entries_in_file, desc=f"Processing {effective_source_identifier}", unit="entry") as pbar: # <-- Use effective_source_identifier
        for entry_index, entry in enumerate(data):
            ...
            try:
                ...
                word_metadata = {'marayum_source': effective_source_identifier, 'language_name': language_name, 'processed_timestamp': datetime.now().isoformat()} # <-- Use effective_source_identifier
                try:
                    word_id = get_or_create_word_id(
                ...
                if not word_id: raise ValueError(f"Failed to get/create word ID for '{headword}' ({language_code}) from {effective_source_identifier}") # <-- Use effective_source_identifier
                ...
                if isinstance(pronunciations, list):
                     for pron in pronunciations:
                         if isinstance(pron, str) and pron.strip():
                             try:
                                 pron_id = insert_pronunciation(cur, word_id, pron.strip(), source_identifier=effective_source_identifier) # <-- Use effective_source_identifier
                                 if pron_id: pronunciations_added += 1
                             except Exception as e: logger.warning(f"Failed pronunciation insert '{pron}' for '{headword}' from {effective_source_identifier}: {e}") # <-- Use effective_source_identifier
                ...
                if isinstance(etymology_text_raw, str) and etymology_text_raw.strip():
                     ...
                     try:
                         ety_id = insert_etymology(cur, word_id, etymology_text, source_identifier=effective_source_identifier) # <-- Use effective_source_identifier
                         if ety_id: etymologies_added += 1
                     except Exception as e: logger.warning(f"Failed etymology insert for '{headword}' from {effective_source_identifier}: {e}") # <-- Use effective_source_identifier
                                             ...
                                             for sense_rel_word in sense[sense_rel_key]:
                                                 if isinstance(sense_rel_word, str) and sense_rel_word.strip():
                                                     ...
                                                     if sense_rel_word_clean != headword:
                                                         try:
                                                             sense_rel_id = get_or_create_word_id(cur, sense_rel_word_clean, language_code, source_identifier=effective_source_identifier) # <-- Use effective_source_identifier
                                                             if sense_rel_id:
                                                                 sense_rel_metadata = {'source': effective_source_identifier, 'definition_id': definition_id, 'confidence': 70} # <-- Use effective_source_identifier
                                                             ...
                                                             logger.warning(f"Error adding sense {sense_rel_key} '{sense_rel_word_clean}' for '{headword}' from {effective_source_identifier}: {rel_e}") # <-- Use effective_source_identifier
                         ...
                         for related_word_str in related_words_list:
                             if isinstance(related_word_str, str) and related_word_str.strip():
                                 ...
                                 if related_word_clean != headword: # Avoid self-relation
                                     try:
                                         related_id = get_or_create_word_id(cur, related_word_clean, language_code, source_identifier=effective_source_identifier) # <-- Use effective_source_identifier
                                         if related_id:
                                             top_rel_metadata = {'source': effective_source_identifier, 'confidence': 70} # <-- Use effective_source_identifier
                                     ...
                                     except Exception as top_rel_e: logger.warning(f"Error adding top-level {rel_key} '{related_word_clean}' for '{headword}' from {effective_source_identifier}: {top_rel_e}") # <-- Use effective_source_identifier
...
def process_marayum_directory(cur, directory_path: str) -> None:
    ...
    for json_file in json_files:
        try:
            ...
            processed, errors = process_marayum_json(cur, json_file, source_identifier=marayum_source_id) # <-- MODIFIED CALL
