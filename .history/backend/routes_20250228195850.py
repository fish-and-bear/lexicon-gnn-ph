...
from backend.models import Word, Definition, Etymology, Relation, DefinitionRelation, Affixation, PartsOfSpeech, db
...
from backend.security import validate_json_request, validate_query_params
from backend.caching import multi_level_cache, cache, invalidate_cache_prefix
...
def init_rate_limiter(app):
    ...
    This function sets up the Redis connection used for API rate limiting,
    ...
    redis_url = app.config.get('REDIS_URL', 'redis://redis:6379/0')
        ...
        # Test connection
        redis_client.ping()
        logger.info("Successfully connected to Redis rate limiter", redis_url=redis_url)
    except redis.ConnectionError as e:
        logger.warning(
            "Failed to connect to Redis for rate limiting",
            error=str(e),
            redis_url=redis_url,
            fallback="Rate limiting will be disabled"
        )
        redis_client = None
    except Exception as e:
        logger.error(
            "Unexpected error when connecting to Redis",
            error=str(e),
            redis_url=redis_url,
...
def check_rate_limit(key: str, limit: int = 30, window: int = 60) -> Tuple[bool, Optional[int]]:
    ...
    rate limiting without race conditions.
    
    Args:
        key: The rate limit key (usually IP or API key based)
    ...
    """
    # Skip rate limiting if Redis is not available or if in testing mode
    ...
    try:
        ...
        # Calculate remaining requests and whether this request is allowed
...
def log_request_info():
    ...
    Captures HTTP method, path, query parameters, headers, and client IP.
...
def before_request():
    """
    Execute pre-request processing.
    ...
    # Check rate limits for non-static endpoints
...
def after_request(response):
    """
    Execute post-request processing.
    
    - Logs response details
    - Adds common response headers
...
def handle_exception(e):
    ...
    Captures all unhandled exceptions, logs detailed information,
...
def error_response(message: str, status_code: int = 500, errors: dict = None, error_code: str = None) -> tuple:
    """
    Generate detailed error responses with comprehensive request context.
...
def success_response(data: Any, message: str = None, meta: dict = None) -> tuple:
    """
    Generate consistent success responses with metadata.
...
def normalize_word(text: str) -> str:
    ...
    Removes diacritics, converts to lowercase, and applies
    standardized formatting for reliable search and comparison.
...
def parse_components_field(components_data: str) -> List[str]:
    ...
    Handles both JSON arrays and delimited text formats with
    proper error handling and fallbacks.
    
    Args:
        components_data: The raw components data
...
def get_word_details(word_entry, include_definitions=True, include_relations=True, include_etymology=True, include_metadata=True):
    """
    Generate comprehensive word details including all relationships and metadata.
    
    Args:
        word_entry: The Word model instance
        include_definitions: Whether to include definitions
        include_relations: Whether to include word relationships
        include_etymology: Whether to include etymology information
        include_metadata: Whether to include metadata fields
        
    Returns:
        Dictionary containing all word information
    """
    if not word_entry:
        return None
    ...
    if include_definitions and hasattr(word_entry, 'definitions'):
        ...
        for d in word_entry.definitions:
            # Format examples and usage notes
            examples = []
            if d.examples:
                try:
                    # Try parsing as JSON
                    ex_data = json.loads(d.examples)
                    if isinstance(ex_data, list):
                        examples = ex_data
                    else:
                        examples = [str(ex_data)]
            ...
            if d.usage_notes:
                try:
                    # Try parsing as JSON
                    notes_data = json.loads(d.usage_notes)
                    if isinstance(notes_data, list):
                        usage_notes = notes_data
                    else:
                        usage_notes = [str(notes_data)]
                except (json.JSONDecodeError, TypeError):
                    # Fallback to line splitting
    ...
    # Add etymology information if requested
    ...
    if include_relations:
        ...
        # Process word relations from this word to others
...
def get_words():
    """
    Get a paginated list of words with comprehensive filtering options.
    
    Query Parameters:
        page (int): Page number, starting from 1
        per_page (int): Items per page (max 100)
        search (str): Search term for filtering words
        language (str): Language code filter (tl, ceb)
        pos (str): Part of speech filter
        has_baybayin (bool): Filter for words with Baybayin script
        has_etymology (bool): Filter for words with etymology information
    
    Returns:
        JSON response with word list and pagination metadata
    """
    try:
...
class WordQuerySchema(Schema):
    """Schema for validating word detail query parameters."""
...
def get_word(word, **params):
    ...
    try:
        # Use validated params from decorator
        ...
        # Cache the response
        cache_key = f"word:{word}:{language_code}:{include_definitions}:{include_relations}:{include_etymology}:{include_metadata}"
        cached_response = cache.get(cache_key)
        if cached_response:
            return cached_response

        # Define needed eager loading based on requested data
        query_options = []
        
        # Always load the core word data
...
def search_words():
    ...
    try:
        ...
        for row in results:
            ...
            # Process definitions
            definitions = []
            if row.definitions and row.definitions[0]:
                definitions = [d for d in row.definitions if d]
                
            # Process parts of speech
            pos_codes = []
            if row.pos_codes and row.pos_codes[0]:
                pos_codes = [p for p in row.pos_codes if p]
            ...
            search_results.append({
                ...
                "definitions": definitions[:3] if definitions else [],  # Limit to first 3 definitions
                    "counts": {
...
def get_parts_of_speech():
    ...
    try:
        pos_list = PartsOfSpeech.query.order_by(PartsOfSpeech.code).all()
        results = []
        for pos in pos_list:
            results.append({
                    "code": pos.code,
...
@multi_level_cache(prefix="random_word", ttl=60)  # Short cache time for random
def get_random_word():
    ...
    try:
        ...
        # We use func.random() for PostgreSQL, which is more efficient than ORDER BY RAND()
...
# Add comprehensive validation schemas for additional endpoints
...
# Helper function for rate limit exceeded responses
