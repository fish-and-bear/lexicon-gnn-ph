...
def display_leaderboard(cur, console):
    ...
    try:
        ...
        # Check if metadata column exists and contains POS information
        try:
            cur.execute(
                """
            ...
            # Metadata column might not exist or not be JSONB
            pass
    ...
    try:
        # Check if metadata column exists in definitions table
        ...
        if def_results:
            ...
            # Check if we have metadata with POS info
            if metadata_exists:
                # Display a special note about POS metadata
                with_metadata = sum(1 for row in def_results if len(row) > 8 and row[8] > 0)
                if with_metadata > 0:
                    console.print("[bold yellow]Extended POS Information[/]")
                    console.print(f"Found {with_metadata} sources with enhanced POS metadata.")
                    console.print("[dim]The standardize_entry_pos function has been updated to better handle KWF dictionary format.[/]")
                    console.print()
    except Exception as e:
