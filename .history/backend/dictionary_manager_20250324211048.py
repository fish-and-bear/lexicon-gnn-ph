...
def get_or_create_word_id(cur, lemma: str, language_code: str = "tl", **kwargs) -> int:
    try:
        ...
        word_id = get_or_create_word_id_base(cur, lemma, language_code, **kwargs)
        
        # Double-check that the word was properly created and can be found in the database
        if word_id:
            cur.execute("SELECT id FROM words WHERE id = %s", (word_id,))
            if not cur.fetchone():
                logger.warning(f"Word ID {word_id} for '{lemma}' was created but not found in database. Retrying...")
                # Try to commit the transaction explicitly in case it wasn't committed
                try:
                ...
                cur.execute("SELECT id FROM words WHERE id = %s", (word_id,))
                if not cur.fetchone():
                    logger.error(f"Failed to create word '{lemma}' in database after explicit commit. Creating again.")
                    # Last resort: try creating the word again with a clean query
        ...
        logger.error(f"Error in get_or_create_word_id for '{lemma}': {str(e)}")
        try:
...
def insert_definition(cur, word_id: int, definition_text: str, part_of_speech: str = "",
    ...
    Checks for duplicate definitions using (word_id, definition_text, standardized_pos_id)
    to avoid violating the unique constraint.
    """
    try:
        # Skip definitions that are only Baybayin notices.
        ...
        cur.execute("SELECT id FROM words WHERE id = %s", (word_id,))
        if not cur.fetchone():
            logger.error(f"Word ID {word_id} does not exist.")
            return None

        # Get standardized part-of-speech ID
        std_pos_id = get_standardized_pos_id(cur, part_of_speech)

        # Check for duplicates using the unique key (word_id, definition_text, standardized_pos_id)
        cur.execute("""
        ...
        # Append category info to usage_notes if available.
        if category:
        ...
        logger.warning(f"Duplicate definition detected for word ID {word_id} with text: {definition_text[:50]}...")
        cur.connection.rollback()
        return None
    except Exception as e:
