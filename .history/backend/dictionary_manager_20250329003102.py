...
def migrate_relationships_to_new_system(cur):
    ...
    # Note: POS_MAPPING dictionary is now defined at module level
...
def process_root_words_cleaned(cur, filename: str):
    ...
    for root_entry in tqdm(data, desc="Processing root words"):
        try:
            # Skip entries without a root word
            if not root_entry.get('root_word'):
                skipped += 1
                continue
            
            # Get the root word ID
            root_word = root_entry['root_word']
