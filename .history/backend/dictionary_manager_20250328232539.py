...
def main():
    ...
    parser = argparse.ArgumentParser(description="Filipino Dictionary Management Tool")
    ...
    lookup_parser.add_argument("--lang", "-l", default="tl", help="Language code (tl/ceb)")
    
    # Update command
    update_parser = subparsers.add_parser("update", help="Update dictionary with new data")
    update_parser.add_argument("--file", "-f", required=True, help="JSON file with new data")
    
    # Leaderboard command
    leaderboard_parser = subparsers.add_parser("leaderboard", help="Show contributor leaderboard")
    
    # Stats command
    stats_parser = subparsers.add_parser("stats", help="Show dictionary statistics")
    
    # Explore command
    explore_parser = subparsers.add_parser("explore", help="Explore dictionary interactively")
    
    # Cleanup command
    cleanup_parser = subparsers.add_parser("cleanup", help="Clean up database issues")
    
    # Purge command
    purge_parser = subparsers.add_parser("purge", help="Purge database tables")
    
    # Help command (custom help display)
    ...
    if args.command == "migrate":
        ...
        try:
            ...
            setup_extensions(conn.cursor())
            # Create or update tables
            create_or_update_tables(conn.cursor())
        ...
        with DBConnection() as cursor:
            verify_baybayin_data(cursor)
            print("Verification completed.")
            
    elif args.command == "lookup":
        word = args.word
        lang = args.lang
        print(f"Looking up '{word}' (language: {lang})...")
        # Implement lookup functionality
    
    elif args.command == "leaderboard":
        with DBConnection() as cursor:
            console = Console()
            display_leaderboard(cursor, console)
    
    elif args.command == "help":
        display_help()
        
    elif args.command == "purge":
        confirm = input("Are you sure you want to purge all database tables? This cannot be undone. (yes/no): ").strip().lower()
        if confirm == "yes":
            with DBConnection() as cursor:
                purge_database_tables(cursor)
...
def display_help():
    ...
    console = Console()
    console.print("\n[bold cyan]Filipino Dictionary Manager[/bold cyan]")
    console.print("\nA comprehensive tool for managing Filipino dictionary data with support for")
    console.print("Baybayin script, etymologies, and relations.\n")
    
    console.print("[bold]Available commands:[/bold]")
    commands = [
        ("migrate", "Create or update database schema"),
        ("test", "Run tests to verify database functionality"),
        ("verify", "Verify consistency of database entries"),
        ("lookup <word>", "Look up a word in the dictionary"),
        ("update --file <file>", "Update dictionary with new data from JSON file"),
        ("leaderboard", "Show contributor leaderboard"),
        ("stats", "Show dictionary statistics"),
        ("explore", "Interactive dictionary exploration mode"),
        ("cleanup", "Clean up database inconsistencies"),
        ("purge", "Purge all database tables (use with caution!)"),
        ("help", "Show this help information")
    ]
    
    table = Table(show_header=False, box=box.SIMPLE)
    for cmd, desc in commands:
        table.add_row(f"[yellow]{cmd}[/yellow]", desc)
    console.print(table)
    ...
    examples = [
        "python dictionary_manager.py migrate",
        "python dictionary_manager.py lookup kamandag",
        "python dictionary_manager.py update --file new_words.json",
        "python dictionary_manager.py leaderboard"
    ]
    for ex in examples:
...
def purge_database_tables(cur):
    """
    Purge all database tables, removing all dictionary data.
    This is a destructive operation and should be used with caution.
    """
    logger.warning("Purging all database tables...")
    try:
        # Drop all tables in proper order to respect foreign key constraints
        cur.execute("""
            DROP TABLE IF EXISTS 
            ...
            CASCADE;
        """)
        logger.info("All tables have been purged successfully.")
        return True
    except Exception as e:
        logger.error(f"Error purging database tables: {str(e)}")
        raise

@with_transaction(commit=True)
def display_leaderboard(cur, console=None):
    """Display a leaderboard of top contributors based on definition counts."""
    if console is None:
        console = Console()
        
    try:
        # Query to get standardized source names and definition counts
        cur.execute("""
            SELECT 
                CASE
                    ...
                    WHEN LOWER(TRIM(sources)) ILIKE '%kwf_dictionary%' OR 
                         LOWER(TRIM(sources)) ILIKE '%kwf diksiyonaryo%' THEN 'KWF Diksiyonaryo ng Wikang Filipino'
                    WHEN LOWER(TRIM(sources)) ILIKE '%tagalog-words%' THEN 'diksiyonaryo.ph'
                    WHEN LOWER(TRIM(sources)) ILIKE '%root_words_with_associated%' THEN 'tagalog.com'
                    ELSE sources 
                END AS source_name,
        ...
        table.add_column("Definitions", style="magenta", justify="right")
        
        # Add data to the table
        for i, row in enumerate(results, 1):
            source_name = row[0]
            count = row[1]
            table.add_row(f"{i}", source_name, f"{count:,}")
        ...
        console.print(f"\nTotal definitions in database: [bold]{total_count:,}[/bold]")
