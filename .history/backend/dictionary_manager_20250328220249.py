...
class SourceStandardization:
    """Central class for source standardization and management"""
    
    # Source mapping for standardization
    SOURCE_MAPPING = {
        # Common variants and abbreviations
        'kaikki': 'kaikki.org', 'kaikki-dict': 'kaikki.org',
        'kwf': 'KWF', 'kwf-dict': 'KWF', 'komisyon': 'KWF',
        'english-tagalog': 'eng-tgl', 'eng-tag': 'eng-tgl',
        'tagalog-dictionary': 'tagalog-dict', 'tag-dict': 'tagalog-dict',
        'up-diksiyunaryong': 'UP', 'up-dict': 'UP',
    ...
    SOURCE_DISPLAY = {
        'kaikki.org': 'Kaikki.org Dictionary',
        'KWF': 'Komisyon sa Wikang Filipino',
        'eng-tgl': 'English-Tagalog Dictionary',
        'tagalog-dict': 'Tagalog Dictionary',
        'UP': 'UP Diksiyunaryong Filipino',
        'Wikidata': 'Wikidata',
    ...
    @classmethod
    def standardize_sources(cls, source: str) -> str:
        """
        Standardize source string to consistent format.
        
        Args:
            source: Source string to standardize
            
        Returns:
            Standardized source string
        """
        if not source:
            return "unknown"
        
        # Split by commas and standardize each source
        sources = [s.strip().lower() for s in source.split(',')]
        standardized = []
        
        for src in sources:
            if src in cls.SOURCE_MAPPING:
                standardized.append(cls.SOURCE_MAPPING[src])
            else:
                # Keep as is if not in mapping
                standardized.append(src)
    ...
    def get_display_name(cls, source: str) -> str:
        """
        Get a display name for a standardized source.
        
        Args:
            source: Standardized source string
        ...
        # If it's a single source, get its display name
        if source in cls.SOURCE_DISPLAY:
            return cls.SOURCE_DISPLAY[source]
        
        # For multiple sources, get display names for each
        if ',' in source:
            sources = [s.strip() for s in source.split(',')]
            displays = []
            for src in sources:
                if src in cls.SOURCE_DISPLAY:
                    displays.append(cls.SOURCE_DISPLAY[src])
                else:
                    displays.append(src)
            return ", ".join(displays)
        
        # Default case: return the source as is
        return source
        
    @classmethod
    def get_standardized_source_sql(cls) -> str:
        """
        Get SQL CASE statement for standardizing sources in queries.
        
        Returns:
            SQL CASE statement for source standardization
        """
        cases = []
        for variant, standard in cls.SOURCE_MAPPING.items():
            cases.append(f"WHEN sources ILIKE '%{variant}%' THEN '{standard}'")
        
        sql = f"""
            CASE 
                {" ".join(cases)}
                WHEN sources IS NULL OR sources = '' THEN 'unknown'
                ELSE sources
            END
        """
        return sql
...
def get_standardized_source_sql() -> str:
    return SourceStandardization.get_standardized_source_sql()
...
def process_kaikki_jsonl(cur, filename):
    """Process a Kaikki.org JSONL file line by line."""
    # Read the JSONL file line by line
    with open(filename, 'r', encoding='utf-8') as f:
        for line in f:
            try:
                entry = json.loads(line)
                process_entry(cur, entry, filename)
...
@with_transaction(commit=True)
def migrate_relationships_to_new_system(cur):
    """
    Migrate existing relationships to the new RelationshipType system with metadata.
    
    This updates all existing relationships to:
    1. Standardize relation_type values using RelationshipType
    2. Add strength values to metadata
    3. Ensure bidirectional relationships are properly established
    4. Fix any inconsistencies
    
    Returns:
        int: The number of relationships updated
    """
    logger.info("Starting relationship migration to new system")
    updated_count = 0
    
    # First, ensure the metadata column exists
    try:
        cur.execute("ALTER TABLE relations ADD COLUMN IF NOT EXISTS metadata JSONB DEFAULT '{}'::jsonb")
        cur.execute("ALTER TABLE relations ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP")
    except Exception as e:
        logger.warning(f"Error adding metadata column: {e}")
    
    # Get all existing relationships
    cur.execute("SELECT id, from_word_id, to_word_id, relation_type, sources FROM relations")
    relations = cur.fetchall()
    
    rel_manager = RelationshipManager(cur)
    
    # Process each relationship
    for rel_id, from_id, to_id, rel_type_str, sources in relations:
        # Convert to RelationshipType
        rel_type = RelationshipType.from_string(rel_type_str)
        
        # Add metadata with default strength
        metadata = {
            'strength': rel_type.strength,
            'migrated': True,
            'original_type': rel_type_str
        }
        ...
        cur.execute("""
            ...
            SET relation_type = %s,
                ...
                updated_at = CURRENT_TIMESTAMP
            WHERE id = %s
        """, (str(rel_type.rel_value), json.dumps(metadata), rel_id))
        updated_count += 1
        
        # Check if bidirectional and ensure inverse exists
        if rel_type.bidirectional:
            # Check if inverse exists
            cur.execute("""
                SELECT id FROM relations
                WHERE from_word_id = %s AND to_word_id = %s AND relation_type = %s
            """, (to_id, from_id, str(rel_type.rel_value)))
            ...
            if not cur.fetchone():
                ...
                rel_manager.add_relationship(
                    ...
                    sources=sources,
                    metadata={'strength': rel_type.strength, 'auto_created': True}
                )
                updated_count += 1
        elif rel_type.inverse:
            ...
            cur.execute("""
                SELECT id FROM relations
                WHERE from_word_id = %s AND to_word_id = %s AND relation_type = %s
            """, (to_id, from_id, str(inverse_type.rel_value)))
            ...
            if not cur.fetchone():
                ...
                rel_manager.add_relationship(
                    ...
                    sources=sources,
                    metadata={'strength': inverse_type.strength, 'auto_created': True}
                )
                updated_count += 1
    
    # Fix any inconsistencies
    issues = rel_manager.analyze_relationship_consistency()
    fixed = rel_manager.repair_relationship_issues(issues)
    
    logger.info(f"Relationship migration complete. Updated {updated_count} relationships. Fixed {fixed} issues.")
    return updated_count

def standardize_entry_pos(pos_str: str) -> str:
    """Standardize part of speech string from data entries"""
    if not pos_str:
        return ""
...
def process_root_words_cleaned(cur, filename: str):
    """Process the root words data file (with associated words)."""
    logger.info(f"Processing root words data from {filename}")
    with open(filename, 'r', encoding='utf-8') as f:
        root_words_data = json.load(f)
    
    processed = 0
    skipped = 0
    errors = 0
    total = len(root_words_data)
    
    for root_entry in root_words_data:
        try:
            root_word = root_entry.get("root_word", "").strip()
            if not root_word:
                skipped += 1
                continue
                
            # Create or get the root word ID
            root_id = get_or_create_word_id(cur, root_word, language_code="tl")
            
            # Process associated words
            associated_words = root_entry.get("associated_words", [])
            for associated in associated_words:
                if not isinstance(associated, str) or not associated.strip():
                    continue
                    
                associated_id = get_or_create_word_id(cur, associated.strip(), language_code="tl")
                
                # Create relationship: associated word is derived from root word
                insert_relation(
                    cur, 
                    associated_id, 
                    root_id, 
            ...
            errors += 1
            logger.error(f"Error processing root word {root_entry.get('root_word', 'unknown')}: {str(e)}")
...
# -------------------------------------------------------------------
class PartOfSpeechStandardization:
    """Central class for part of speech standardization"""
    ...
    @classmethod
    def get_standard_code(cls, pos_string: str) -> str:
        """
        Convert a part of speech string to a standardized code.
        ...
        # Check for direct match in mapping
        if pos_key in cls.MAPPING:
            return cls.MAPPING[pos_key]
        
        # Check for partial matches (e.g., "common noun" -> "noun")
        for key, code in cls.MAPPING.items():
            if key in pos_key:
                return code
    ...
    @classmethod
    def standardize(cls, pos_str: str) -> str:
        """
        Standardize part of speech string from data entries.
        
        Args:
            pos_str: The part of speech string to standardize
            
        Returns:
            Standardized part of speech code
        """
        if not pos_str:
            return ""
        return cls.get_standard_code(pos_str)
    
    @classmethod
    def get_pos_id(cls, cur, pos_string: str) -> int:
        """
        Get the database ID for a standardized part of speech.
        
        Args:
            cur: Database cursor
            pos_string: Part of speech string
            
        Returns:
            ID of the standardized part of speech
        """
        if not pos_string:
            return cls.get_uncategorized_pos_id(cur)
        pos_key = pos_string.lower().strip()
        code = cls.get_standard_code(pos_key)
        cur.execute("SELECT id FROM parts_of_speech WHERE code = %s", (code,))
        result = cur.fetchone()
        return result[0] if result else cls.get_uncategorized_pos_id(cur)
    
    @classmethod
    def get_uncategorized_pos_id(cls, cur) -> int:
        """
        Get the database ID for the uncategorized part of speech.
        
        Args:
            cur: Database cursor
            
        Returns:
            ID of the uncategorized part of speech
        """
        cur.execute("SELECT id FROM parts_of_speech WHERE code = 'unc'")
        result = cur.fetchone()
        if result:
...
# Function aliases for backward compatibility
def get_standard_code(pos_string: str) -> str:
    return PartOfSpeechStandardization.get_standard_code(pos_string)

def standardize_entry_pos(pos_str: str) -> str:
    return PartOfSpeechStandardization.standardize(pos_str)

# -------------------------------------------------------------------
# Text Standardization
# -------------------------------------------------------------------
class TextStandardization:
    """Central class for text standardization functions"""
    
    @classmethod
    def normalize_lemma(cls, text: str) -> str:
        """
        Normalize lemma text by removing diacritics and converting to lowercase.
    ...
    @classmethod
    def clean_baybayin_lemma(cls, lemma: str) -> str:
        """
        Clean a Baybayin lemma by removing prefixes like "Baybayin spelling of".
        
        Args:
            lemma: The Baybayin text to clean
            
        Returns:
            Cleaned Baybayin text
        """
        if not lemma:
            return ""
        prefix = "Baybayin spelling of"
    ...
    def has_diacritics(cls, text: str) -> bool:
        ...
        if not text:
            return False
        normalized = cls.normalize_lemma(text)
        return text != normalized
    
    @classmethod
    def format_word_display(cls, word: str, show_baybayin: bool = True) -> str:
        """
        Format a word for display, optionally including Baybayin notation.
        
        Args:
            word: The word to format
            show_baybayin: Whether to include Baybayin notation if present
            
        Returns:
            Formatted word for display
        """
        if not word:
            return ""
        has_bb = any(0x1700 <= ord(c) <= 0x171F for c in word)
