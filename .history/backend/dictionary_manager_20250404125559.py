...
# Define standard part of speech mappings with full Filipino words
POS_MAPPING = {
    # Nouns - Pangngalan
    "noun": "pangngalan",
    ...
    "conj": "pangatnig",
    "katnig": "pangatnig",
    # Interjections - Padamdam
    "interjection": "padamdam",
    "pandamdam": "padamdam",
    "padamdam": "padamdam",
    "intj": "padamdam",
    "damdam": "padamdam",
    ...
    "pantulong": "pantulong",
    "aux": "pantulong",
    # Additional Filipino-specific POS mappings
    "pangalanang pantangi": "pangngalang pantangi",  # Proper noun
    "pangalanang pambalana": "pangngalang pambalana",  # Common noun
    "pang-uri sa panlagay": "pang-uri",  # Predicate adjective
    "pang-uri sa panlapi": "pang-uri",  # Affixal adjective
    "panghalip panao": "panghalip panao",  # Personal pronoun
    "panghalip pamatlig": "panghalip pamatlig",  # Demonstrative pronoun
    "panghalip pananong": "panghalip pananong",  # Interrogative pronoun
    "panghalip paari": "panghalip paari",  # Possessive pronoun
    "pang-abay na pamaraan": "pang-abay na pamaraan",  # Manner adverb
    "pang-abay na panlunan": "pang-abay na panlunan",  # Locative adverb
    "pang-abay na pamanahon": "pang-abay na pamanahon",  # Temporal adverb
    "pang-abay na pang-uring": "pang-abay na pang-uring",  # Qualifying adverb
    "punto": "bantas",  # Punctuation
    "antas na paghahambing": "pang-uri",  # Comparative
    "katumbas na salita": "pangngalan",  # Equivalent term  
    "kahulugan": "pangngalan",  # Definition
    "kabaliktaran": "pangngalan",  # Antonym
    "aspekto": "pangngalan",  # Aspect
    "kasarian": "pangngalan",  # Gender
    "karamihan": "pamilang",  # Number (grammatical)
    "anyo": "pangngalan",  # Form
    "kayarian": "pangngalan",  # Structure
    "gamit": "pangngalan",  # Usage
    "pagkakaugnay": "pangngalan",  # Relation
    # Add KWF-specific terms
    "salitang ugat": "pangngalan",  # Root word
    "salitang hango": "pangngalan",  # Derived word
    "pulutong": "pangngalan",  # Group/category  
    "sangkap": "pangngalan",  # Component
    "uri": "pangngalan",  # Type
    "simbolo": "pangngalan",  # Symbol
    "dinidiinang pantig": "pangngalan",  # Stressed syllable
    "pagbabago": "pangngalan",  # Variation
    "palatandaan": "pangngalan",  # Marker
    "pahiwatig": "pangngalan",  # Indication
    "tugma": "pangngalan",  # Rhyme
    "kahulugan": "pangngalan",  # Gloss
    "salitang hiram": "pangngalan",  # Loan word
    "damdamin": "pangngalan",  # Emotion
    # Direct KWF dictionary mappings - preserve their capitalization as needed
    "Pangngalan": "pangngalan",
    "Pandiwa": "pandiwa",
    "Pang-uri": "pang-uri",
    "Pang-abay": "pang-abay",
    ...
    "Panlapi": "panlapi",
    "Pang-angkop": "pang-angkop",
    "Pangkayarian": "hulapi",  # Structure marker
    
    # Add mappings for the context-based descriptions
    "sa sanga ng punongkahoy": "pangngalan",  # These are typically describing nouns
    "sa anumang bukás na bahagi": "pangngalan",
    "sa isang tao": "pangngalan",
    "sa isang sakít na": "pangngalan",
    "sa isang rabáw": "pangngalan",
    "sa isang sasakyang-dagat": "pangngalan",
    "sa isang serbisyo, binili, o inutang": "pangngalan",
    "sa isang súgat": "pangngalan",
    "kung sa bakal": "pangngalan",
    "sa pananim": "pangngalan",
    "sa isang pook": "pangngalan",
    "sa isang materyal": "pangngalan",
    "sa dalawang tao": "pangngalan",
    "sa mga pananim": "pangngalan",
    "sa isang likido": "pangngalan",
    "sa isang salita": "pangngalan",
    "sa isang kilos": "pangngalan",
    "sa kagamitan": "pangngalan",
    "sa gamot": "pangngalan",
    "sa isang halaga": "pangngalan",
    "sa buhok": "pangngalan",
    "sa lupa": "pangngalan",
    "sa damit": "pangngalan",
    "sa isang sawsáwan": "pangngalan",
    "sa sakít": "pangngalan",
    "sa pagluluto": "pangngalan",
    "kung sa batok o ilong": "pangngalan",
    "unknown": "hindi matukoy"  # Map unknown to "hindi matukoy" (uncategorized)
}
...
def get_standard_code(pos_string: str) -> str:
    """
    Convert a part of speech string to a standardized full Filipino word.

    Args:
        pos_string: The part of speech string to standardize

    Returns:
        A standardized full Filipino word for the part of speech
    """
    if not pos_string:
        return "hindi matukoy"  # Uncategorized in Filipino
    ...
    # Check for 'Sa' prefix pattern which is common in the contextual descriptions
    if pos_key.startswith("sa ") or pos_key.startswith("kung sa "):
        return "pangngalan"  # These are typically noun usage contexts

    # Check for partial matches (e.g., "common noun" -> "pangngalan")
    for key, value in LOWERCASE_POS_MAPPING.items():
        if key in pos_key:
            return value

    # Log the unmapped POS for future improvements
    print(f"WARNING: Unmapped POS: '{pos_string}' -> mapped to 'hindi matukoy'")
    
    # Default to uncategorized if no match found
    return "hindi matukoy"


def standardize_entry_pos(pos_str):
    """Standardize part-of-speech string in dictionary entries to full Filipino words."""
    if not pos_str:
        return "hindi matukoy"  # Default to uncategorized in Filipino
        
    # Handle lists/arrays of POS values (KWF dictionary often has POS as array)
    if isinstance(pos_str, list) and pos_str:
        # Take the first non-empty item in the list
        for pos_item in pos_str:
            if pos_item and isinstance(pos_item, str) and pos_item.strip():
                std_pos = get_standard_code(pos_item)
                if std_pos != "hindi matukoy":  # If we found a valid value, use it
                    return std_pos
        return "hindi matukoy"
        
    # Use the existing get_standard_pos_filipino function
    return get_standard_code(pos_str)


# Function to extract POS information for debugging purposes
def analyze_pos_data(data_file, sample_size=1000):
    """
    Analyze the POS information in the data file and report statistics.
    """
    import json
    import random
    ...
    sample_keys = random.sample(list(data.keys()), min(sample_size, len(data)))
    
    for key in sample_keys:
        entry = data[key]
        if "definitions" in entry and isinstance(entry["definitions"], dict):
            for pos, defs in entry["definitions"].items():
                if pos:
                    pos_types.append(pos)
                    # Check how our standardization works
                    std_pos = standardize_entry_pos(pos)
                    pos_mapping_results.append((pos, std_pos))
    
    # Count frequencies
    pos_counts = Counter(pos_types)
    mapping_counts = Counter(pos_mapping_results)
    
    print(f"Natagpuan ang {len(pos_counts)} magkakaibang uri ng part of speech sa sample")
    print("\nTop 20 na uri ng part of speech:")
    for pos, count in pos_counts.most_common(20):
        print(f"  {pos}: {count}")
    
    print("\nTop 20 na POS mappings:")
    for (orig, mapped), count in mapping_counts.most_common(20):
        print(f"  '{orig}' -> '{mapped}': {count}")
    
    # Report unmapped POS types
    unmapped = [orig for (orig, mapped), _ in mapping_counts.items() if mapped == "hindi matukoy"]
    if unmapped:
        print(f"\nNatagpuan ang {len(unmapped)} hindi matukoy na uri ng part of speech:")
        for pos in sorted(unmapped)[:20]:  # Show only first 20
            print(f"  '{pos}'")
            
    return pos_counts, mapping_counts


# Optional: Create a function to store both standardized POS and original POS
def process_pos_with_metadata(pos_str):
    """
    Process part of speech, storing both standardized and original values.
    
    Returns:
        A tuple of (standardized_pos, metadata_dict)
    """
    standard_pos = standardize_entry_pos(pos_str)
    
    # Create metadata dictionary to preserve the original value
    metadata = {
        "orihinal_na_pos": pos_str,
        "pamantayang_pos": standard_pos
    }
    
    # For array values, store the full array
    if isinstance(pos_str, list) and pos_str:
        metadata["orihinal_na_pos_array"] = pos_str
    
    return standard_pos, metadata
# -------------------------------------------------------------------
...
def process_kwf_dictionary(cur, filename: str):
    ...
    stats = {
        ...
        "source_updates": 0,
        "etymologies_added": 0,
        "pos_types_processed": set(),  # Track unique POS types
    }
    ...
    try:
        ...
        with open(filename, "r", encoding="utf-8") as f:
            try:
                ...
                # Ensure rollback on critical file error
                if not conn.closed:
                    conn.rollback()
        ...
        with tqdm(
            ...
            for entry_index, word, entry in entries_iterator:
                ...
                try:
                    ...
                    # Store part_of_speech at word level if present
                    if "part_of_speech" in entry:
                        word_metadata_dict["part_of_speech_raw"] = json.dumps(entry["part_of_speech"])
                    ...
                    # Add original structure information
                    word_metadata_dict["kwf_structure"] = json.dumps({
                        "has_part_of_speech_array": isinstance(entry.get("part_of_speech"), list),
                        "definition_count": sum(len(defs) for defs in entry.get("definitions", {}).values()),
                        "has_etymology": bool(etymology_list),
                        "has_related_terms": bool(entry.get("related")),
                    ...
                    if isinstance(definitions_by_pos, dict):
                        for raw_pos, definitions_list in definitions_by_pos.items():
                            ...
                            # Track unique POS types for statistics
                            if raw_pos:
                                if isinstance(raw_pos, list):
                                    for pos_item in raw_pos:
                                        if pos_item:
                                            stats["pos_types_processed"].add(str(pos_item))
                                else:
                                    stats["pos_types_processed"].add(str(raw_pos))

                            # Improved POS standardization handling
                            standardized_pos_str = standardize_entry_pos(raw_pos) if raw_pos else None
                            
                            # Debug logging for POS processing
                            if raw_pos:
                                if isinstance(raw_pos, list):
                                    logger.debug(f"POS array for '{formatted_word}': {raw_pos} → standard: {standardized_pos_str}")
                                else:
                                    logger.debug(f"POS for '{formatted_word}': {raw_pos} → standard: {standardized_pos_str}")
                            ...
                            for def_idx, def_item in enumerate(definitions_list):
                                ...
                                # Enhanced metadata to preserve more information
                                def_metadata_dict = {
                                    "source_file": raw_source_identifier,
                                    "original_pos": raw_pos,
                                }
                                
                                # Store the full POS array if applicable
                                if isinstance(raw_pos, list):
                                    def_metadata_dict["original_pos_full"] = json.dumps(raw_pos)
                                    # Map all POS values to standard codes
                                    all_pos_standards = [get_standard_code(pos) for pos in raw_pos if pos]
                                    if all_pos_standards:
                                        def_metadata_dict["all_pos_standards"] = ",".join(all_pos_standards)
                                
                                if standardized_pos_str:
                                    def_metadata_dict["standardized_pos"] = standardized_pos_str
                                ...
                                def_metadata_dict["kwf_structure"] = json.dumps({
                                    "has_categories": bool(tags_list),
                                    "has_note": bool(usage_notes),
                                    "has_synonyms": bool(def_item.get("synonyms")),
                                    "has_antonyms": bool(def_item.get("antonyms")),
                                    "has_cross_references": bool(def_item.get("cross_references")),
                                    "has_example_sets": bool(def_item.get("example_sets"))
                                })
                                
                                def_metadata_json = json.dumps(def_metadata_dict)
                                ...
                                if normalized_examples:
                                    try:
                                        ...
                                        logger.warning(f"Could not serialize examples for '{formatted_word}', def {def_idx}")

                                # Insert the definition with enhanced metadata
                                if definition_text:
                                    try:
                                        # Check the signature of insert_definition
                                        # If it doesn't accept metadata, remove it from the function call
                                        try:
                                            definition_id = insert_definition(
                                                ...
                                                tags=tags
                                                # Remove metadata parameter as it's not supported
                                            )
                                        except TypeError as e:
                                            # Fallback with just required parameters if necessary
                                            if "unexpected keyword argument" in str(e):
                                                logger.warning(f"Falling back to basic insert_definition call for '{formatted_word}': {e}")
                                                definition_id = insert_definition(
                                                    cur,
                                                    word_id,
                                                    definition_text,
                                                    source_identifier=source_identifier,
                                                    part_of_speech=standardized_pos_str
                                                )
                                            else:
                                                raise
                                        if definition_id:
                                            ...
                                            if isinstance(synonyms_list, list):
                                                for syn_idx, syn in enumerate(synonyms_list):
                                                    ...
                                                    if isinstance(syn, str) and syn.strip() and syn.strip() != formatted_word:
                                                        ...
                                                        if not syn_word_clean or syn_word_clean == formatted_word:
                                                        ...
                                                        try:
                                                            ...
                                                            logger.warning(f"Error processing synonym '{syn_word_clean}' for '{formatted_word}': {syn_error}")
                                            ...
                                            if isinstance(antonyms_list, list):
                                                for ant_idx, ant in enumerate(antonyms_list):
                                                    if isinstance(ant, str) and ant.strip() and ant.strip() != formatted_word:
                                                        ...
                                                        if not ant_word_clean or ant_word_clean == formatted_word:
                                                            continue  # Skip if empty after cleaning or self-reference
                                                        try:
                                                            ...
                                                            logger.warning(f"Error processing antonym '{ant_word_clean}' for '{formatted_word}': {ant_error}")
                                            ...
                                            if isinstance(cross_refs, list):
                                                for ref_idx, ref in enumerate(cross_refs):
                                                    ...
                                                    if isinstance(ref_word, str) and ref_word.strip() and ref_word.strip() != formatted_word:
                                                        ...
                                                        if not ref_word_clean or ref_word_clean == formatted_word:
                                                            continue  # Skip if empty after cleaning or self-reference
                                                        try:
                                    ...
                                    try:
                                        ...
                                        if related_id:
                                            ...
                                            if rel_type_enum.bidirectional:
                                                # Check if inverse already added to avoid double counting
                ...
                # --- Error Handling for a Single Entry ---
                    ...
                    try:
                        ...
                        logger.critical(f"CRITICAL: Failed rollback to savepoint {savepoint_name}: {rb_err}. Attempting full tx rollback.", exc_info=True)
                        if not conn.closed:
                            conn.rollback()
                        # Don't re-raise here to continue processing other entries
    ...
    # --- Outer Exception Handling ---
        ...
        try:
            ...
            if conn and not conn.closed:
                ...
                logger.info("Unable to determine connection state in outer exception.")
        except Exception as rollback_error:
    ...
    # --- Final Logging ---
        ...
        if "pos_types_processed" in stats and isinstance(stats["pos_types_processed"], set):
            stats["pos_types_processed"] = list(stats["pos_types_processed"])
            
        try:
            # Ensure stats is JSON serializable
            stats_json = json.dumps(stats, indent=2)
            logger.info(f"Statistics: {stats_json}")
            
            if error_types:
                error_types_json = json.dumps(error_types, indent=2)
                logger.info(f"Error types encountered: {error_types_json}")
        except TypeError as e:
            logger.error(f"Error serializing statistics: {e}")
            logger.info(f"Processed entries: {stats.get('processed_entries', 0)}")
...
def display_leaderboard(cur, console):
    ...
    try:
        ...
        # Get both standardized and original POS stats
        cur.execute(
            "SELECT COUNT(DISTINCT standardized_pos_id) FROM definitions WHERE standardized_pos_id IS NOT NULL"
        )
        total_std_pos = cur.fetchone()[0]
        
        cur.execute(
            "SELECT COUNT(DISTINCT part_of_speech) FROM definitions WHERE part_of_speech IS NOT NULL"
        )
        total_raw_pos = cur.fetchone()[0]
        
        # Count metadata with original POS information
        cur.execute(
            """
            SELECT COUNT(*) FROM definitions 
            WHERE metadata IS NOT NULL AND 
                  metadata::jsonb ? 'original_pos'
            """
        )
        meta_pos_count = cur.fetchone()[0]
        
        pos_display = f"{total_std_pos}"
        if total_raw_pos > total_std_pos:
            pos_display = f"{total_std_pos} std / {total_raw_pos} raw"
            
        overall_stats_table.add_row("Unique Parts of Speech", pos_display)
        if meta_pos_count > 0:
            overall_stats_table.add_row("Definitions w/ POS Metadata", f"{meta_pos_count:,}")
    ...
    try:
        # Enhanced SQL query to get both standardized and raw POS counts, plus metadata presence
        cur.execute(
            """
            WITH source_stats AS (
                SELECT
                    ...
                    COUNT(DISTINCT standardized_pos_id) AS std_pos_count,
                    COUNT(DISTINCT part_of_speech) AS raw_pos_count,
                    COUNT(CASE WHEN usage_notes IS NOT NULL AND usage_notes != '' THEN 1 END) AS with_notes,
                    COUNT(CASE WHEN metadata IS NOT NULL AND metadata::jsonb ? 'original_pos' THEN 1 END) AS with_pos_metadata,
                    COUNT(CASE WHEN metadata IS NOT NULL AND metadata::jsonb ? 'original_pos_full' THEN 1 END) AS with_full_pos_metadata
                FROM definitions
                GROUP BY source_name
            )
            SELECT
        ...
        if def_results:
            ...
            # Track sources with significant POS disparities
            pos_disparities = []
            ...
            for row in def_results:
                ...
                std_pos = row[4] if len(row) > 4 else 0
                raw_pos = row[5] if len(row) > 5 else 0
                notes = row[6] if len(row) > 6 else 0
                pos_meta = row[7] if len(row) > 7 else 0
                full_pos_meta = row[8] if len(row) > 8 else 0
                ex_pct = row[9] if len(row) > 9 else 0.0
                notes_pct = row[10] if len(row) > 10 else 0.0

                # Check for POS disparities
                if raw_pos and std_pos and raw_pos > std_pos * 1.2:  # 20% more raw than standardized
                    pos_disparities.append((source, std_pos, raw_pos, pos_meta, full_pos_meta))
                
                # Create POS display value
                pos_display = str(std_pos)
                if raw_pos and std_pos and raw_pos != std_pos:
                    std_rate = round(100.0 * std_pos / raw_pos, 1) if raw_pos else 0
                    if std_rate < 90:  # Only show dual display if there's a meaningful difference
                        pos_display = f"{std_pos}/{raw_pos}"
            ...
            # Add POS disparity report if significant disparities exist
            if pos_disparities:
                console.print("\n[bold yellow]Part of Speech Standardization Report[/]")
                pos_detail_table = Table(box=box.SIMPLE)
                pos_detail_table.add_column("Source", style="yellow")
                pos_detail_table.add_column("Std POS", justify="right", style="green")
                pos_detail_table.add_column("Raw POS", justify="right", style="cyan")
                pos_detail_table.add_column("POS Metadata", justify="right", style="magenta")
                pos_detail_table.add_column("Std Rate", justify="right", style="dim")
                
                for src, std, raw, meta, full_meta in pos_disparities:
                    std_rate = round(100.0 * std / raw, 1) if raw else 0
                    meta_display = f"{meta:,}"
                    if full_meta > 0:
                        meta_display += f" ({full_meta} w/arrays)"
                    
                    pos_detail_table.add_row(
                        src,
                        f"{std}",
                        f"{raw}",
                        meta_display,
                ...
                console.print(pos_detail_table)
                console.print("\n[dim]Note: A low standardization rate may indicate POS types in the source not yet mapped in standardize_entry_pos function.[/]\n")
