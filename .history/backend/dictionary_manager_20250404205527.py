...
def process_kaikki_jsonl(cur, filename: str):
    """Process Kaikki.org dictionary entries with optimized batch processing."""
    # Initialize statistics
    entry_stats = {
    ...
    BATCH_SIZE = 500  # Process entries in batches of 500
    COMMIT_FREQUENCY = 2000  # Commit every 2000 entries
    ...
    conn = cur.connection  # Get connection for transaction management
    
    # Initialize caches for word lookups and other frequently accessed data
    word_cache = {}  # lemma+lang_code -> word_id
    pos_cache = {}   # code -> pos_id
    ...
    RELATIONSHIP_TYPE_MAPPING = {  # Define mapping locally
    ...
    def get_cached_word_id(lemma, language_code):
        """Get word ID from cache or None if not found."""
        cache_key = f"{lemma.lower()}|{language_code}"
        return word_cache.get(cache_key)
    
    def add_to_word_cache(lemma, language_code, word_id):
        """Add word ID to cache."""
        cache_key = f"{lemma.lower()}|{language_code}"
        word_cache[cache_key] = word_id
    
    def batch_get_or_create_words():
        """Process the word batch in bulk."""
        if not word_batch:
            return {}
            
        # Prepare data for bulk lookup
        values_list = []
        for item in word_batch:
            normalized = normalize_lemma(item['lemma'])
            values_list.append((item['lemma'], normalized, item['language_code']))
            
        # Create a temporary table for the batch lookup
        try:
            cur.execute("CREATE TEMP TABLE word_batch (lemma TEXT, normalized_lemma TEXT, language_code TEXT) ON COMMIT DROP")
            execute_batch(cur, 
                "INSERT INTO word_batch (lemma, normalized_lemma, language_code) VALUES (%s, %s, %s)", 
                values_list)
            
            # Perform a single batch lookup
            cur.execute("""
                SELECT w.id, w.lemma, w.language_code 
                FROM words w
                JOIN word_batch wb ON 
                    (w.normalized_lemma = wb.normalized_lemma AND w.language_code = wb.language_code)
            """)
            existing_words = {f"{row[1].lower()}|{row[2]}": row[0] for row in cur.fetchall()}
            
            # Prepare batch inserts for words that don't exist
            words_to_create = []
            result_mapping = {}
            
            for item in word_batch:
                cache_key = f"{item['lemma'].lower()}|{item['language_code']}"
                if cache_key in existing_words:
                    result_mapping[id(item)] = existing_words[cache_key]
                    add_to_word_cache(item['lemma'], item['language_code'], existing_words[cache_key])
                else:
                    # Build values for insert
                    normalized = normalize_lemma(item['lemma'])
                    values = (
                        item['lemma'], normalized, item['language_code'],
                        item.get('has_baybayin', False), item.get('baybayin_form'), 
                        item.get('romanized_form'), item.get('badlit_form'),
                        item.get('is_proper_noun', False), item.get('is_abbreviation', False),
                        item.get('is_initialism', False), item.get('tags'),
                        Json(item.get('word_metadata', {})) if item.get('word_metadata') else None,
                        Json(item.get('hyphenation', [])) if item.get('hyphenation') else None
                    )
                    words_to_create.append((item, values))
            ...
            if words_to_create:
                query = """
                    INSERT INTO words (
                        ...
                        is_proper_noun, is_abbreviation, is_initialism, tags,
                        word_metadata, hyphenation
                    ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                    RETURNING id, lemma, language_code
                """
                
                # Execute the batch insert and collect results
                for item, values in words_to_create:
                    try:
                        cur.execute(query, values)
                        row = cur.fetchone()
                        if row:
                            word_id, lemma, lang_code = row
                            result_mapping[id(item)] = word_id
                            add_to_word_cache(lemma, lang_code, word_id)
                    except Exception as e:
                        logger.error(f"Error inserting word '{item['lemma']}': {e}")
            
            # Drop the temp table
            cur.execute("DROP TABLE word_batch")
            return result_mapping
            
        except Exception as e:
            logger.error(f"Error in batch word processing: {e}")
            try:
                cur.execute("DROP TABLE IF EXISTS word_batch")
            except:
                pass
            return {}
    
    def batch_insert_definitions():
        """Process definition batch in bulk."""
        if not definition_batch:
            return {}
            
        result_ids = {}
        values_list = []
        
        for item in definition_batch:
            word_id = item.get('word_id')
            definition_text = item.get('definition_text')
            if not word_id or not definition_text:
                continue
                
            # Standardize part of speech
            pos_str = item.get('part_of_speech')
            pos_code = get_standard_code(pos_str) if pos_str else 'unc'
            
            # Get ID for standardized POS
            pos_id = None
            if pos_code in pos_cache:
                pos_id = pos_cache[pos_code]
            else:
                try:
                    cur.execute("SELECT id FROM parts_of_speech WHERE code = %s", (pos_code,))
                    pos_row = cur.fetchone()
                    if pos_row:
                        pos_id = pos_row[0]
                        pos_cache[pos_code] = pos_id
                except Exception as e:
                    logger.error(f"Error getting POS ID for '{pos_code}': {e}")
            
            values = (
                word_id, definition_text, item.get('original_pos'),
                pos_id, item.get('examples'), item.get('usage_notes'),
                item.get('tags'), item.get('sources'), 
                Json(item.get('metadata', {})) if item.get('metadata') else None
            )
        ...
        query = """
            INSERT INTO definitions (
                word_id, definition_text, original_pos, 
                standardized_pos_id, examples, usage_notes, 
                tags, sources, metadata
            ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)
            ON CONFLICT (word_id, definition_text) DO UPDATE SET
                updated_at = CURRENT_TIMESTAMP,
                examples = COALESCE(definitions.examples, EXCLUDED.examples),
                usage_notes = COALESCE(definitions.usage_notes, EXCLUDED.usage_notes),
                metadata = COALESCE(definitions.metadata, '{}'::jsonb) || EXCLUDED.metadata
            RETURNING id
        """
        
        for item, values in values_list:
            try:
                cur.execute(query, values)
                row = cur.fetchone()
                if row:
                    result_ids[id(item)] = row[0]
            except Exception as e:
                logger.error(f"Error inserting definition for word {values[0]}: {e}")
        
        return result_ids
    
    def batch_insert_pronunciations():
        """Process pronunciation batch in bulk."""
        if not pronunciation_batch:
            return {}
            
        result_ids = {}
        values_list = []
        
        for item in pronunciation_batch:
            word_id = item.get('word_id')
            pron_type = item.get('type')
            pron_value = item.get('value')
            if not word_id or not pron_type or not pron_value:
                continue
                
            values = (
                word_id, pron_type, pron_value,
                Json(item.get('tags', {})) if item.get('tags') else None,
                Json(item.get('metadata', {})) if item.get('metadata') else None,
                item.get('sources')
            )
            values_list.append((item, values))
        
        # Batch insert pronunciations
        query = """
            INSERT INTO pronunciations (
                word_id, type, value, tags, pronunciation_metadata, sources
            ) VALUES (%s, %s, %s, %s, %s, %s)
            ON CONFLICT (word_id, type, value) DO UPDATE SET
                updated_at = CURRENT_TIMESTAMP,
                tags = COALESCE(pronunciations.tags, '{}'::jsonb) || EXCLUDED.tags,
                pronunciation_metadata = COALESCE(pronunciations.pronunciation_metadata, '{}'::jsonb) || EXCLUDED.pronunciation_metadata
            RETURNING id
        """
        ...
        for item, values in values_list:
            try:
                ...
                logger.error(f"Error inserting pronunciation for word {values[0]}: {e}")
        
        return result_ids
    
    def batch_insert_etymologies():
        """Process etymology batch in bulk."""
        if not etymology_batch:
            return {}
            
        result_ids = {}
        values_list = []
        
        for item in etymology_batch:
            word_id = item.get('word_id')
            etymology_text = item.get('etymology_text')
            if not word_id or not etymology_text:
                continue
                
            # Extract components and language codes if needed
            normalized_components = None
            if "components" in item:
                normalized_components = json.dumps(item["components"])
            elif etymology_text:
                try:
                    components = extract_etymology_components(etymology_text)
                    if components:
                        normalized_components = json.dumps(components)
                except Exception as e:
                    logger.debug(f"Error extracting components from etymology text: {e}")
            
            values = (
                word_id, etymology_text, normalized_components,
                item.get('etymology_structure'), item.get('language_codes'),
                item.get('sources')
            )
            values_list.append((item, values))
        
        # Batch insert etymologies
        query = """
            INSERT INTO etymologies (
                word_id, etymology_text, normalized_components,
                etymology_structure, language_codes, sources
            ) VALUES (%s, %s, %s, %s, %s, %s)
            ON CONFLICT (word_id, etymology_text) DO UPDATE SET
                updated_at = CURRENT_TIMESTAMP,
        ...
        for item, values in values_list:
            try:
                ...
                logger.error(f"Error inserting etymology for word {values[0]}: {e}")
        
        return result_ids
    
    def batch_insert_relations():
        """Process relation batch in bulk."""
        ...
        query = """
            INSERT INTO relations (
                from_word_id, to_word_id, relation_type, metadata, sources
            ) VALUES (%s, %s, %s, %s, %s)
            ON CONFLICT (from_word_id, to_word_id, relation_type) DO UPDATE SET
                updated_at = CURRENT_TIMESTAMP,
                metadata = COALESCE(relations.metadata, '{}'::jsonb) || EXCLUDED.metadata
            RETURNING id
        """
        ...
        for item, values in values_list:
            try:
                ...
                logger.error(f"Error inserting relation {values[0]}->{values[1]}: {e}")
        
        return result_ids
    
    def batch_insert_forms():
        """Process word form batch in bulk."""
        if not form_batch:
            return {}
        ...
        query = """
            INSERT INTO word_forms (
                word_id, form, is_canonical, is_primary, tags
            ) VALUES (%s, %s, %s, %s, %s)
        ...
        for item, values in values_list:
            try:
                ...
                logger.error(f"Error inserting word form '{values[1]}' for word {values[0]}: {e}")
        
        return result_ids
    
    def process_entry_batch(entries, source_identifier):
        """Process a batch of entries."""
        global word_batch, definition_batch, pronunciation_batch, etymology_batch, relation_batch, form_batch
        ...
        # Process each entry and collect data for batch operations
        local_stats = {k: 0 for k in entry_stats.keys()}
        entry_results = []
        
        # First pass: extract words and build batches
        for entry in entries:
            if "word" not in entry or not entry["word"]:
                entry_results.append((None, "No word field"))
                continue
            ...
            # Check if word exists in cache first
            word_id = get_cached_word_id(word, language_code)
            ...
            if not word_id:
                ...
                if baybayin_form and not baybayin_romanized and local_romanizer and local_romanizer.validate_text(baybayin_form):
                    try:
                        romanized_form = local_romanizer.romanize(baybayin_form)
                    except Exception:
                        pass
                ...
                word_batch.append(word_item)
                
                # Store the entry with its word data for second pass
                entry_results.append((entry, word_item, None))
            else:
                # Word already exists in cache
                entry_results.append((entry, {'word_id': word_id}, None))
        
        # Process word batch
        word_id_mapping = batch_get_or_create_words()
        
        # Second pass: process related data with word IDs
        for i, (entry, word_data, error) in enumerate(entry_results):
            if error or not entry:
                continue
                
            word_id = word_data.get('word_id')
            if not word_id and id(word_data) in word_id_mapping:
                word_id = word_id_mapping[id(word_data)]
                
            if not word_id:
                entry_results[i] = (entry, word_data, "Failed to get/create word ID")
                continue
                
            # Store word_id for reference
            ...
            try:
                ...
                if "sounds" in entry and isinstance(entry["sounds"], list):
                    for sound in entry.get("sounds", []):
                        ...
                        if pron_type and pron_value:
                            ...
                            pron_data["word_id"] = word_id
                            pronunciation_batch.append(pron_data)
                ...
                if "forms" in entry and isinstance(entry["forms"], list):
                    for form_data in entry["forms"]:
                        ...
                        form_batch.append(form_item)
                        local_stats["forms"] += 1
                
                # Process etymology
                if "etymology_text" in entry and entry["etymology_text"]:
                    etymology_text = clean_html(entry["etymology_text"].strip())
                    ...
                    etymology_batch.append(etymology_item)
                    local_stats["etymologies"] += 1
                    
                    # Process etymology relations from templates
                    if "etymology_templates" in entry and isinstance(entry["etymology_templates"], list):
                        for template in entry["etymology_templates"]:
                            if not isinstance(template, dict) or "name" not in template:
                                continue
                            ...
                            if template_name in ["derived", "borrowing", "derived from", "borrowed from", "bor", "der", "bor+"]:
                                ...
                                if source_word and isinstance(source_word, str):
                                    ...
                                    if source_word_clean and source_word_clean.lower() != entry["word"].lower():
                                        source_word_lang = source_lang if source_lang else language_code
                                        
                                        # Check cache for related word
                                        related_word_id = get_cached_word_id(source_word_clean, source_word_lang)
                                        
                                        if not related_word_id:
                                            # Add to word batch for later processing
                                            related_word_item = {
                                                'lemma': source_word_clean,
                                                'language_code': source_word_lang
                                            }
                                            word_batch.append(related_word_item)
                                            
                                            # Placeholder for relation to be processed later
                                            relation_item = {
                                                'from_id': word_id,
                                                'to_word_lemma': source_word_clean,
                                                'to_word_lang': source_word_lang,
                                                'relation_type': rel_type.rel_value,
                                                'metadata': {
                                                    "from_etymology": True,
                                            ...
                                            relation_batch.append(relation_item)
                                        else:
                                            # Related word exists, add direct relation
                                            relation_item = {
                                                'from_id': word_id,
                                                'to_id': related_word_id,
                                                'relation_type': rel_type.rel_value,
                                                'metadata': {
                                                    "from_etymology": True,
                                                    "template": template_name,
                                            ...
                                            if inverse_rel:
                                                inverse_relation = {
                                                    'from_id': related_word_id,
                                                    'to_id': word_id,
                                                    'relation_type': inverse_rel.rel_value,
                                                    'metadata': {
                                                        "from_etymology": True,
                                                        "template": template_name,
                                                        "confidence": inverse_rel.strength
                                                    }
                                                }
                                                relation_batch.append(inverse_relation)
                                                
                                            local_stats["ety_relations"] += 1
                                            local_stats["relations"] += 1
                
                # Process definitions and senses
                if "senses" in entry and isinstance(entry["senses"], list):
                    for sense_idx, sense in enumerate(entry["senses"]):
                        ...
                        # Get POS
                        sense_pos_str = sense.get("pos") or entry.get("pos", "unc")
                        
                        # Add to definitions batch
                        definition_item = {
                            'word_id': word_id,
                            'definition_text': definition_text,
                            'original_pos': sense_pos_str,
                        ...
                        definition_batch.append(definition_item)
                        local_stats["definitions"] += 1
                
                # Update entry with status
                entry_results[i] = (entry, word_data, None)
                
            except Exception as e:
                # Update entry with error
                entry_results[i] = (entry, word_data, str(e))
        
        # Process batch operations
        word_id_mapping = batch_get_or_create_words()  # Refresh for any new words
        def_id_mapping = batch_insert_definitions()
        pron_id_mapping = batch_insert_pronunciations()
        ety_id_mapping = batch_insert_etymologies()
        batch_insert_forms()
        
        # Process pending relations with resolved word IDs
        updated_relation_batch = []
        for rel in relation_batch:
            if 'to_word_lemma' in rel:
                # Get target word ID from cache
                to_id = get_cached_word_id(rel['to_word_lemma'], rel['to_word_lang'])
                if to_id:
                    rel['to_id'] = to_id
                    del rel['to_word_lemma']
                    del rel['to_word_lang']
                    updated_relation_batch.append(rel)
                    
                    # Add inverse relation if needed
                    if rel.get('add_inverse'):
                        rel_type = RelationshipType.from_value(rel['relation_type'])
                        inverse_rel = rel_type.get_inverse()
                        if inverse_rel:
                            inverse_relation = {
                                'from_id': to_id,
                            ...
                            updated_relation_batch.append(inverse_relation)
            else:
                updated_relation_batch.append(rel)
                
        relation_batch = updated_relation_batch
        batch_insert_relations()
        
        # Update global stats
        for key in entry_stats:
            entry_stats[key] += local_stats[key]
            
        # Return processed entries status
        return [
            (entry["word"] if entry else None, word_data.get('word_id'), error) 
            for entry, word_data, error in entry_results
        ]
    ...
    try:
        with open(filename, "r", encoding="utf-8") as f:
            ...
            # Single transaction for entire process
            cur.execute("BEGIN")
            
            for line in f:
                entry_count += 1
                entry_since_commit += 1
                if progress_bar:
                ...
                try:
                    ...
                    if len(batch_entries) >= BATCH_SIZE:
                        batch_results = process_entry_batch(batch_entries, source_identifier)
                        
                        # Update statistics
                        for word, word_id, error in batch_results:
                            if word_id:
                                if error:
                                    stats["processed_with_errors"] += 1
                                    err_key = f"EntryError: {str(error)[:100]}"
                                    error_summary[err_key] = error_summary.get(err_key, 0) + 1
                                else:
                                    stats["processed_ok"] += 1
                            else:
                                stats["failed_entries"] += 1
                                reason = error or "Unknown critical failure"
                        ...
                        # Commit periodically
                        if entry_since_commit >= COMMIT_FREQUENCY:
                            cur.execute("COMMIT")
                            cur.execute("BEGIN")
                            entry_since_commit = 0
                            logger.info(f"Committed transaction at entry {entry_count}")
            ...
            # Final commit handled by transaction decorator
        ...
        raise e  # Re-raise for outer transaction handling
