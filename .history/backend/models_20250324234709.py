...
class Word(db.Model):
    ...
    __table_args__ = (
        ...
        # Adding the constraints from dictionary_manager.py
        DDL("""
...
class Etymology(db.Model):
    ...
    __table_args__ = (
        ...
        DDL("CREATE INDEX IF NOT EXISTS idx_etymologies_langs ON etymologies USING gin(to_tsvector('simple', language_codes))")
    )
...
class DefinitionRelation(db.Model):
    ...
    def validate_relation_type(self, key, value):
        """Validate relation type."""
        if value not in ['synonym', 'antonym', 'variant', 'example', 'see_also', 'usage']:
            raise ValueError("Invalid relation type. Must be one of: synonym, antonym, variant, example, see_also, usage")
        return value
...
def create_constraints(target, connection, **kw):
    """Create any necessary constraints for database compatibility."""
    # Ensure the Baybayin constraints are properly set
    connection.execute(text("""
    ...
    # Ensure the TSVECTOR search_text column and index exist
    connection.execute(text("""
        DO $$ 
        BEGIN
            IF NOT EXISTS (
...
def init_app(app):
    ...
    with app.app_context():
        ...
        db.create_all()
        # Execute schema compatibility checks
