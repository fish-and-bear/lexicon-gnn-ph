...
def process_tagalog_words(cur, filename: str):
    ...
    try:
        ...
        try:
            ...
            # Ensure we're in autocommit mode for this section
                ...
                if isinstance(data, dict):
                    for word, entry in data.items():
                        try:
                            ...
                            # Process baybayin form if appropriate
                            if validate_for_baybayin(word):
                                baybayin_form = transliterate_to_baybayin(word)
                                if baybayin_form and validate_baybayin_entry(baybayin_form, word):
                                    process_baybayin_data(cur, word_id, baybayin_form, word)
                                ...
                                for definition in definitions:
                                    if definition:
                                        insert_definition(
                                            ...
                                            usage_notes=entry.get('pronunciation'),
                                            sources=source
                                        )
                            ...
                            # Process derivatives
                            derivatives = entry.get('derivative')
                            if derivatives:
                                # Create a savepoint for this operation
                                savepoint_name = f"sp_derivatives_{time.time_ns()}"
                                try:
                                    # Process derivatives
                                    if isinstance(derivatives, str):
                                        for derivative in derivatives.split(','):
                                            derivative = derivative.strip()
                                            if derivative:
                                                der_id = get_or_create_word_id(cur, derivative, language_code)
                                                if der_id:
                                                    insert_relation(
                                                        cur,
                                                        word_id,
                                    ...
                                    logger.error(f"Error processing derivatives for {word}: {str(e)}")
                                    
                            # Process definition relations
                            try:
                                for definition in definitions:
                                    if definition:
                                        process_definition_relations(cur, word_id, definition, source)
                            except Exception as e:
                                logger.error(f"Error processing definition relations for {word}: {str(e)}")
                    ...
                    # Process list-style entries
                    for entry in data:
                        try:
                            word = entry.get('word', '')
                            if not word or (word, language_code) not in word_ids:
                                pbar.update(1)
                                continue
                            ...
                            logger.error(f"Error processing word {entry.get('word', '')}: {str(e)}")
                        finally:
                            pbar.update(1)
            ...
            try:
                ...
                logger.warning(f"Failed to restore autocommit mode: {str(e)}")
...
def process_root_words_cleaned(cur, filename: str):
    """
    Process root-words-cleaned.json with structure:
    [
        {
            "root_word": "...",
            "language_code": "tl",
            "pos": "...",
            "definition": "...",
            "associated_words": ["word1", "word2", ...]
        },
        ...
    ]
    """
    logger.info(f"Processing root words from: {filename}")
    source = SourceStandardization.standardize_sources('root-words-cleaned.json')
    ...
    try:
        ...
        if not isinstance(data, list):
            logger.error(f"Invalid data format in {filename}. Expected list, got {type(data)}")
            return
            
        with tqdm(total=len(data), desc="Processing root words") as pbar:
            for entry in data:
                try:
                    ...
                    pos = entry.get('pos', '')
                    definition = entry.get('definition', '')
                    associated_words = entry.get('associated_words', [])
                    
                    if not root_word:
                        logger.warning("Skipping entry without root_word")
                        pbar.update(1)
                        continue
                    
                    # Create the root word
                    root_word_id = get_or_create_word_id(cur, root_word, language_code)
                    if not root_word_id:
                        logger.error(f"Failed to create root word: {root_word}")
                        pbar.update(1)
                        continue
                        
                    # Add the definition if provided
                    if definition:
                        insert_definition(
                            cur,
                            root_word_id,
                    ...
                    for associated_word in associated_words:
                        try:
                            associated_id = get_or_create_word_id(cur, associated_word, language_code)
                            if associated_id:
                                # Update the root word reference
                                cur.execute(
                                    "UPDATE words SET root_word_id = %s WHERE id = %s AND (root_word_id IS NULL OR root_word_id = %s)",
                                    (root_word_id, associated_id, associated_id)
                                )
                            ...
                            logger.error(f"Error processing associated word {associated_word} for root {root_word}: {str(e)}")
                            
                except Exception as e:
                    logger.error(f"Error processing root word entry: {str(e)}")
                finally:
                    pbar.update(1)
    ...
    def process_entry(cur, entry: Dict):
        ...
        try:
            # Begin individual transaction for this entry
            conn = cur.connection
            ...
            logger.error(f"Error processing Kaikki entry: {word}. Error: {str(e)}")
