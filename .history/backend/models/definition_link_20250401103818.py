...
class DefinitionLink(BaseModel):
    ...
    # Relationships
    definition = db.relationship('Definition', backref=db.backref('links', lazy='dynamic', cascade='all, delete-orphan'))
    
    __table_args__ = (
        db.UniqueConstraint('definition_id', 'link_text', 'link_target', name='definition_links_definition_id_link_text_link_target_key'),
    )
    ...
    def __repr__(self):
        return f'<DefinitionLink {self.id}: {self.link_text[:30]}...>'
