...
class DefinitionLink(BaseModel, BasicColumnsMixin):
    ...
    def target_url(self):
        ...
        return self.link_metadata.get('target_url', '') if hasattr(self, 'link_metadata') and self.link_metadata else ''
    ...
    # Property for the missing target_url column
    @property
    def target_url(self):
        """Provide compatibility for the missing target_url column."""
        # Try to get from link_metadata
        if self.link_metadata and 'url' in self.link_metadata:
            return self.link_metadata['url']
        # Try to get from tags
        if self.tags and 'url' in self.tags:
    ...
    __table_args__ = (
        db.UniqueConstraint('definition_id', 'link_text', 'target_url', name='definition_links_unique'),
