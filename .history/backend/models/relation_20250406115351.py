...
class Relation(BaseModel, BasicColumnsMixin):
    ...
    try:
        relation_data = db.Column(JSONB, default=lambda: {})
    except:
    ...
    __table_args__ = (
        ...
        # db.Index('idx_relations_metadata', 'relation_data', postgresql_using='gin'),
        # db.Index('idx_relations_metadata_strength', 'relation_data', postgresql_using='gin', postgresql_ops={'relation_data': 'jsonb_path_ops'})
    )
    ...
    def create_inverse(self) -> 'Relation':
        ...
        return Relation(
            ...
            relation_data=self.relation_data.copy() if self.relation_data else {}
        )
    ...
    def relation_data(self):
        ...
        if hasattr(self, '_relation_data'):
