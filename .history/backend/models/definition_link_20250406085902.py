...
class DefinitionLink(BaseModel, BasicColumnsMixin):
    ...
    link_type = db.Column(db.Text, nullable=False)
    ...
    __table_args__ = (
        db.UniqueConstraint('definition_id', 'link_type', 'target_url', name='definition_links_unique'),
        db.Index('idx_def_links_def', 'definition_id'),
        db.Index('idx_def_links_type_trgm', 'link_type', postgresql_using='gin', postgresql_ops={'link_type': 'gin_trgm_ops'}),
        db.Index('idx_def_links_target', 'target_url')
    )
    
    @validates('link_type')
    def validate_link_type(self, key, value):
        """Validate link type."""
        if not value:
            raise ValueError("Link type cannot be empty")
        if not isinstance(value, str):
            raise ValueError("Link type must be a string")
        value = value.strip()
        if not value:
            raise ValueError("Link type cannot be empty after stripping")
        return value
    ...
    def __repr__(self):
        return f'<DefinitionLink {self.id}: {self.link_type} -> {self.target_url} for def {self.definition_id}>'
