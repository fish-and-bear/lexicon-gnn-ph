...
class Word(BaseModel, TextSearchMixin, GINIndexMixin, TrigramSearchMixin):
    ...
    @completeness_score.expression
    def completeness_score(cls):
        """SQL expression version for completeness_score to use in queries."""
        from sqlalchemy import case, select, func, exists
        from sqlalchemy.sql.expression import cast
        from sqlalchemy import Float, Integer
        
        # Use a constant default score for SQL queries instead of the dynamic calculation
        # This avoids the issue with SQLAlchemy trying to evaluate Python functions in SQL
        return cast(0.5, Float)
    ...
    etymologies = db.relationship('Etymology', 
                                ...
                                primaryjoin="Word.id == Etymology.word_id",
    ...
    outgoing_relations = db.relationship('Relation', 
                                       ...
                                       query_class=db.Query,
                                       # Explicitly specify which columns to load to avoid missing columns
                                       primaryjoin="Word.id == Relation.from_word_id")
    incoming_relations = db.relationship('Relation', 
                                       ...
                                       query_class=db.Query,
                                       # Explicitly specify which columns to load to avoid missing columns
                                       primaryjoin="Word.id == Relation.to_word_id")
    ...
    root_affixations = db.relationship('Affixation', 
                                     ...
                                     query_class=db.Query,
                                     # Explicitly specify which columns to load to avoid missing columns
                                     primaryjoin="Word.id == Affixation.root_word_id")
    affixed_affixations = db.relationship('Affixation', 
                                        ...
                                        query_class=db.Query,
                                        # Explicitly specify which columns to load to avoid missing columns
                                        primaryjoin="Word.id == Affixation.affixed_word_id")
    ...
    def __init__(self, **kwargs):
        ...
        # Don't set is_root directly since it's a hybrid property
        # It will be automatically calculated based on root_word_id
    ...
    def validate_baybayin_form(self, key: str, value: Optional[str]) -> Optional[str]:
        ...
        # Check if the string contains only valid Baybayin characters (U+1700 to U+171F) and spaces
        if not all(c == ' ' or ('\u1700' <= c <= '\u171F') for c in value):
