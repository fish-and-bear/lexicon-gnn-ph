...
class Definition(BaseModel, BasicColumnsMixin):
    ...
    def popularity_score(self):
        ...
        if self.definition_metadata and 'popularity_score' in self.definition_metadata:
    ...
    def popularity_score(self, value):
        ...
        if not self.definition_metadata:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        # Use get_popularity_score if popularity_score doesn't exist
        popularity = self.popularity_score if hasattr(self, 'popularity_score') else self.get_popularity_score
    ...
    def increment_popularity(self, amount: float = 0.1) -> None:
        ...
        if hasattr(self, 'popularity_score'):
            ...
            # Use the property if column doesn't exist
            score = self.get_popularity_score + amount
            if score > 100.0:
                score = 100.0
            self.get_popularity_score = score
    ...
    # Add property for popularity_score compatibility
    @property
    def get_popularity_score(self):
        """Provide default popularity score when the column doesn't exist."""
        # Check if this is an actual column or if we need to provide a default
        if hasattr(self, 'popularity_score'):
            return self.popularity_score
        # Return a default value if the column doesn't exist
        return getattr(self, '_popularity_score', 0.0)
        
    @get_popularity_score.setter
    def get_popularity_score(self, value):
        """Set popularity score when the column doesn't exist."""
        try:
            self._popularity_score = float(value)
    ...
    def get_most_popular_definitions(cls, limit: int = 10) -> List['Definition']:
        ...
        try:
            # Try a safer approach that doesn't depend on the column
            return cls.query.order_by(
                db.desc(db.func.coalesce(
                    cls.definition_metadata['popularity_score'].astext.cast(db.Float),
                    db.literal(0.0)
                ))
            ).limit(limit).all()
        except Exception as e:
            # Fallback to just getting the most recent
            current_app.logger.error(f"Error getting popular definitions: {e}")
