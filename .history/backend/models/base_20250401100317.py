...
def check_extensions(session) -> bool:
    ...
    if is_feature_initialized('extensions'):
        return True
...
def setup_extensions(session):
    ...
    with _init_lock:
        if check_extensions(session):
            logger.info("All required extensions are already installed")
            return
...
def check_tables_exist(session) -> bool:
    ...
    if is_feature_initialized('tables'):
        return True
    ...
    try:
        ...
        mark_feature_initialized('tables')
        return True
    except Exception as e:
...
def setup_timestamp_trigger(session):
    ...
    with _init_lock:
        if is_feature_initialized('timestamp_trigger'):
            logger.info("Timestamp triggers already set up")
            return
...
def setup_text_search(session):
    ...
    with _init_lock:
        if is_feature_initialized('text_search'):
            logger.info("Text search configuration already set up")
            return
...
def setup_indexes(session):
    ...
    with _init_lock:
        if is_feature_initialized('indexes'):
            logger.info("Database indexes already set up")
            return
...
def setup_search_trigger(session):
    ...
    with _init_lock:
        if is_feature_initialized('search_trigger'):
            logger.info("Search vector trigger already set up")
            return
...
def setup_parts_of_speech(session):
    ...
    with _init_lock:
        if is_feature_initialized('parts_of_speech'):
            logger.info("Parts of speech already set up")
            return
