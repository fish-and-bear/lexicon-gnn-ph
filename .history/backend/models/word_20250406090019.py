...
class Word(BaseModel, TextSearchMixin, GINIndexMixin, TrigramSearchMixin):
    ...
    verification_date = db.Column(db.DateTime(timezone=True))
    ...
    @validates('verified')
    def validate_verified(self, key: str, value: Optional[bool]) -> bool:
        """Validate verified field."""
        if value is None:
            return False
        if not isinstance(value, bool):
            raise ValueError("Verified must be a boolean")
        if value and not hasattr(self, 'verification_date'):
            self.verification_date = datetime.now()
        self._is_modified = True
        return value
        
    @validates('verification_notes')
    def validate_verification_notes(self, key: str, value: Optional[str]) -> Optional[str]:
        """Validate verification notes."""
        if value is None:
            return None
        if not isinstance(value, str):
            raise ValueError("Verification notes must be a string")
        value = value.strip()
        if not value:
            return None
        self._is_modified = True
    ...
    def mark_as_verified(self, notes: Optional[str] = None) -> None:
        """Mark word as verified with optional notes."""
        self.verified = True
        self.verification_date = datetime.now()
        if notes:
