...
from language_utils import LanguageSystem, BaybayinCharType, BaybayinChar
...
def process_tagalog_words(cur, filename: str):
    ...
    with open(filename, 'r', encoding='utf-8') as f:
        ...
        for entry in data:
            ...
            # Insert word
            cur.execute("""
                INSERT INTO words (word, language_code)
                VALUES (%s, 'tl')
                RETURNING id
            """, (word,))
            word_id = cur.fetchone()[0]
            ...
            if definition:
                cur.execute("""
                    INSERT INTO definitions (word_id, definition_text, pos_code, pos_name, sources)
                    VALUES (%s, %s, %s, %s, %s)
                """, (word_id, definition, pos, pos, ['tagalog-words.json']))
        ...
        for entry in data:
            ...
            if definition:
                cur.execute("""
                    INSERT INTO definitions (word_id, definition_text, pos_code, pos_name, sources)
                    VALUES (%s, %s, %s, %s, %s)
                """, (word_id, definition, pos, pos, ['root_words_with_associated_words_cleaned.json']))
    ...
    with open(filename, 'r', encoding='utf-8') as f:
        ...
        for entry in data:
            ...
            for definition in definitions:
                cur.execute("""
                    INSERT INTO definitions (word_id, definition_text, sources)
                    VALUES (%s, %s, %s)
                """, (word_id, definition, ['kwf_dictionary.json']))

def process_kaikki_jsonl_new(cur, filename: str):
    """Process Kaikki.org dictionary from JSONL file."""
    with open(filename, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            for definition in definitions:
                ...
                if gloss:
                    cur.execute("""
                        INSERT INTO definitions (word_id, definition_text, pos_code, pos_name, sources)
                        VALUES (%s, %s, %s, %s, %s)
                    """, (word_id, gloss, pos, pos, [os.path.basename(filename)]))
...
def store_processed_entry(cur, word_id: int, processed: Dict) -> None:
    ...
    try:
        ...
        for definition in processed['data'].get('definitions', []):
            cur.execute("""
                INSERT INTO definitions (word_id, definition_text, pos_code, pos_name, category, sources)
                VALUES (%s, %s, %s, %s, %s, %s)
            """, (
                word_id,
                definition['text'],
                definition.get('pos', {}).get('code'),
                definition.get('pos', {}).get('name'),
                definition.get('domain'),
                get_standardized_source(processed['source'])
            ))
        ...
        if 'metadata' in processed['data']:
            ...
            if 'etymology' in metadata:
                etymology_text = metadata['etymology']
                components = []
                language_codes = []
                
                # Extract components and language codes from etymology text
                if isinstance(etymology_text, dict):
                    components = etymology_text.get('components', [])
                    language_codes = etymology_text.get('language_codes', [])
                ...
                """, (
                    word_id,
                    etymology_text if isinstance(etymology_text, str) else json.dumps(etymology_text),
            ...
            if 'pronunciation' in metadata:
                ...
                if isinstance(pron_data, str):
                    pron_data = {'text': pron_data}
                
                cur.execute("""
                    UPDATE words 
                    SET pronunciation_text = %s,
                        pronunciation_data = %s
                    WHERE id = %s
                """, (
                    pron_data['text'],
                    json.dumps(pron_data),
                    word_id
                ))
            ...
            for rel_type, related in processed['data']['related_words'].items():
                ...
                for rel_word in related:
                    ...
                    cur.execute("""
                        INSERT INTO words (word, language_code)
                        VALUES (%s, %s)
                        ON CONFLICT (word, language_code) DO UPDATE
                        SET updated_at = CURRENT_TIMESTAMP
                        RETURNING id
                    """, (rel_word, processed.get('language_code', 'tl')))
                    related_id = cur.fetchone()[0]
                    
                    # Store relation
                    cur.execute("""
                        INSERT INTO word_relations (word_id, related_word_id, relation_type, sources)
                        VALUES (%s, %s, %s, %s)
                        ON CONFLICT (word_id, related_word_id, relation_type) DO UPDATE
                        SET sources = %s,
                            updated_at = CURRENT_TIMESTAMP
                    """, (
                        word_id,
        ...
        logger.error(f"Error in store_processed_entry: {str(e)}")
        raise
