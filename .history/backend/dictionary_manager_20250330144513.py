...
def display_leaderboard(cur, console):
    ...
    try:
        ...
        cur.execute("""
            WITH source_stats AS (
                SELECT 
                    ...
                    COUNT(CASE WHEN usage_notes IS NOT NULL THEN 1 END) AS with_notes
                FROM definitions
                GROUP BY source_name
            )
            SELECT 
        ...
        try:
            ...
            console.print("[yellow]Could not generate etymology statistics[/]")
        ...
        try:
            cur.execute("""
                WITH baybayin_stats AS (
                    SELECT 
                        CASE
                            WHEN LOWER(TRIM(source_info::text)) LIKE '%project marayum%' THEN 'Project Marayum'
                            WHEN LOWER(TRIM(source_info::text)) LIKE '%kaikki%' THEN 'kaikki.org'
                            WHEN LOWER(TRIM(source_info::text)) LIKE '%kwf%' THEN 'KWF Diksiyonaryo'
                            ELSE 'Unknown'
                        END AS source_name,
            ...
            console.print("[yellow]Could not generate Baybayin statistics[/]")
        ...
        try:
            ...
            console.print("[yellow]Could not generate relationship statistics[/]")
