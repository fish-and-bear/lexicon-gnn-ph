    ...
    try:
        ...
        while attempts < max_attempts and word is None:
            ...
            try:
                ...
                # Execute query
                result = db.session.execute(text(sql_query), {"language": language, "pos_code": pos_code, 
                                                             "min_defs": min_definitions}).fetchone()
                if not result:
                ...
                word = _fetch_word_details(result.id,
                    result.id,
                    include_definitions=True,
            include_etymologies=True,
            include_pronunciations=True,
        ...
        if word is None:
            ...
            return jsonify({"error": "Could not find a valid random word. Please try again."}), 500
            
        # Serialize the word using schema
        schema = WordSchema()
