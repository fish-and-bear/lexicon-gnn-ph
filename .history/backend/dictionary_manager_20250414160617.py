...
def process_tagalog_words(cur, filename: str, add_etymology: bool = True): # `cur` argument is unused due to helper decorator, but kept for consistent interface if called directly elsewhere
    ...
    try:
        ...
        if not isinstance(data, dict):
            logger.error(f"JSON file must contain a dictionary, got {type(data).__name__}")
            stats["errors"] += 1 # Count as one major error
            return stats

        total_entries = len(data)
        logger.info(f"Found {total_entries} word entries in {filename}")
        if total_entries == 0:
            logger.info("No entries found in file. Nothing to process.")
            return stats

        # Process each entry
        language_code = "tl" # Assume Tagalog for this file
        with tqdm(total=total_entries, desc=f"Processing {source_identifier}") as pbar:
