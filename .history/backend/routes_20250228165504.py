...
def success_response(data: Any, message: str = None, meta: dict = None) -> tuple:
    ...
    headers = {
        'Content-Type': 'application/json',
        'X-Response-Time': str(int((datetime.now(UTC) - g.start_time).total_seconds() * 1000)) if hasattr(g, 'start_time') else None
    }
...
def after_request(response):
    ...
    if hasattr(g, 'start_time'):
        response_time = int((datetime.now(UTC) - g.start_time).total_seconds() * 1000)
...
def get_word(word, **params):
    ...
    try:
        ...
        response = jsonify(get_word_details(
            word_entry,
    ...
    try:
            ...
            WITH RECURSIVE search_results AS (
                SELECT 
                    ...
                    GREATEST(
                        ...
                        END,
                        ts_rank_cd(
                            setweight(to_tsvector('simple', w.lemma), 'A') ||
                            setweight(to_tsvector('simple', COALESCE(w.preferred_spelling, '')), 'B') ||
                            setweight(to_tsvector('simple', COALESCE(w.normalized_lemma, '')), 'C'),
        ...
        response_data = {
            "meta": {
                ...
                "execution_time_ms": int((datetime.now(UTC) - g.start_time).total_seconds() * 1000)
            },
            "data": [
                {
                    "word": row.lemma,
                    ...
                    "parts_of_speech": row.pos_codes if row.pos_codes and row.pos_codes[0] else [],
                    "definitions": row.definitions if row.definitions and row.definitions[0] else [],
                    "counts": {
...
@multi_level_cache(prefix="random_word")
def get_random_word():
...
def get_word_relationship_graph(word, **params):
    ...
    try:
        # Check rate limit
        rate_limit_key = f"rate_limit:{request.remote_addr}:word_relationships"
...
def get_etymology_tree(word, **params):
    ...
    try:
        # Check rate limit
        rate_limit_key = f"rate_limit:{request.remote_addr}:etymology_tree"
        if not check_rate_limit(rate_limit_key):
            return error_response("Rate limit exceeded. Please try again later.", 429)
...
def rate_limit_exceeded_response(window: int = 60) -> tuple:
    ...
    return jsonify({
        ...
        'X-RateLimit-Reset': str(int(datetime.now(UTC).timestamp()) + window)
    }
