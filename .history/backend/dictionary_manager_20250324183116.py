...
def with_transaction(commit=True):
    """
    Decorator for managing database transactions using savepoints.
    
    Args:
        commit: Whether to automatically commit the transaction after function execution
        
    Returns:
        Decorated function that handles transaction management
    """
    def decorator(func):
        ...
        def wrapper(cur, *args, **kwargs):
            ...
            try:
                ...
                if savepoint_created and commit:
                    try:
                ...
                # Attempt to rollback to the savepoint
                if savepoint_created:
                    try:
                        cur.execute(f"ROLLBACK TO SAVEPOINT {savepoint_name}")
                    except Exception as rollback_e:
                        logger.warning(f"Failed to rollback to savepoint in {func.__name__}: {str(rollback_e)}")
                
                # Log the error, but not for UniqueViolation which is expected in many cases
...
def get_or_create_word_id(cur, lemma: str, language_code: str = "tl", **kwargs) -> int:
    """
    Get or create a word entry in the database.
    ...
    Returns:
        ID of the word in the database
    """
    try:
        ...
        # Verify the word was properly created
        if word_id:
            cur.execute("SELECT id FROM words WHERE id = %s", (word_id,))
            if not cur.fetchone():
                logger.warning(f"Word ID {word_id} was returned but not found in database! Attempting explicit commit.")
                try:
                    # Try an explicit commit
                    cur.connection.commit()
                    
                    # Check again after commit
                    cur.execute("SELECT id FROM words WHERE id = %s", (word_id,))
                    if not cur.fetchone():
                        # Still not found, try to create it again
                        logger.error(f"Word '{lemma}' (ID: {word_id}) still not found after commit. Creating again.")
                        normalized = normalize_lemma(lemma)
                        cur.execute("""
                            ...
                            RETURNING id
                        """, (lemma, normalized, language_code, lemma + " " + normalized))
                        word_id = cur.fetchone()[0]
                        cur.connection.commit()
                except Exception as e:
                    logger.error(f"Error during recovery attempt for word '{lemma}': {str(e)}")
        
        return word_id
    except Exception as e:
        logger.error(f"Error in get_or_create_word_id for lemma '{lemma}': {str(e)}")
        raise
...
def batch_get_or_create_word_ids(cur, entries: List[Tuple[str, str]], batch_size: int = 1000) -> Dict[Tuple[str, str], int]:
    ...
    try:
        # First pass: Get all existing words from the database
        placeholders = []
        params = []
        ...
        for i in range(0, len(entries), batch_size):
            ...
            for _, norm, lang in normalized_entries:
                batch_placeholders.append("(%s, %s)")
                params.extend([norm, lang])
            
            placeholders.extend(batch_placeholders)
        
        # Execute a single query to get all existing words
        if placeholders:
            query = f"""
                SELECT w.lemma, w.language_code, w.id, w.normalized_lemma 
                FROM words w
                WHERE (w.normalized_lemma, w.language_code) IN ({', '.join(placeholders)})
            """
            ...
            try:
                ...
                # Build a mapping of (normalized_lemma, language_code) to word_id
                existing_by_norm = {(norm, lang): wid for _, lang, wid, norm in rows}
                
                # Also map the original (lemma, language_code) pairs
                for lemma, lang, wid, norm in rows:
                    result[(lemma, lang)] = wid
                
            except Exception as e:
                logger.error(f"Error fetching existing words: {str(e)}")
        
        # Second pass: Insert any words that weren't found
        for i in range(0, len(entries), batch_size):
            batch = entries[i:i + batch_size]
            batch = list(dict.fromkeys(batch))  # Remove duplicates
            ...
            for lemma, lang_code in batch:
                if (lemma, lang_code) not in result:
                    # Check if the normalized form exists
                    normalized = normalize_lemma(lemma)
                    if (normalized, lang_code) in existing_by_norm:
                        # Use the existing word ID
                        result[(lemma, lang_code)] = existing_by_norm[(normalized, lang_code)]
                    else:
                        to_insert.append((lemma, normalized, lang_code))
            
            # Insert missing entries with individual transactions to ensure they're committed
            for lemma, normalized, lang_code in to_insert:
                try:
                    # Start a transaction for this word
                    cur.execute("BEGIN")
                    ...
                    # Commit this transaction to ensure the word is saved
                    cur.execute("COMMIT")
                    
                    # Verify the word was properly inserted
                    cur.execute("SELECT id FROM words WHERE id = %s", (word_id,))
                    if cur.fetchone():
                        result[(lemma, lang_code)] = word_id
                        existing_by_norm[(normalized, lang_code)] = word_id
                    else:
                        logger.error(f"Failed to insert word '{lemma}' (ID: {word_id})")
            ...
            for word, entry in entries:
                ...
                try:
                    ...
                    if romanizer.is_baybayin(word):
                        ...
                        if validate_baybayin_entry(baybayin_form, word):
                            has_baybayin = True
