            ...
            with conn.cursor() as cur:
                ...
                cur.execute("""
                    SELECT w.id, w.lemma, w.language_code, w.has_baybayin, w.baybayin_form, 
                           ...
                           w.tags, w.source_info, w.root_word_id
                    FROM words w
                ...
                word_id, lemma, lang_code, has_baybayin, baybayin_form, romanized_form, \
                preferred_spelling, pronunciation_data, idioms, tags, source_info, root_word_id = word
                
                # Format output
                console = Console()

                # Get all sources from definitions and etymologies
                cur.execute("""
                    SELECT DISTINCT sources FROM definitions WHERE word_id = %s
                    UNION
                    SELECT DISTINCT sources FROM etymologies WHERE word_id = %s
                """, (word_id, word_id))
                
                all_sources = set()
                for row in cur.fetchall():
                    if row[0]:
                        all_sources.update(s.strip() for s in row[0].split(','))

                # Add sources from tags and source_info
                if tags:
                    all_sources.update(t.strip() for t in tags.split(','))
                if source_info and isinstance(source_info, dict):
                    all_sources.update(source_info.get('sources', []))
                ...
                for source in sorted(all_sources):
                    ...
                    if any(kw in display_name.lower() for kw in ['dictionary', 'diksiyonaryo']):
                        dictionaries.append(display_name)
                    else:
                        online_resources.append(display_name)
                ...
                # Display word information panel
                console.print("\n" + " " * 48 + "Word Information")
                ...
                if definitions:
                    ...
                    current_pos = None
                    for pos, def_text, examples, usage_notes, sources in definitions:
                        # Format examples
                        formatted_examples = ""
                        if examples:
                            try:
