...
class DefinitionSchema(Schema):
    ...
    standardized_pos = fields.Nested("StandardizedPosSchema", dump_only=True)
    notes = fields.String()
        ...
        if many:
            for item in data:
                if item.get('standardized_pos'):
                    ...
                    item['standardized_pos_name'] = pos.get('name_en', pos.get('name'))
        else:
            if data.get('standardized_pos'):
                pos = data['standardized_pos']
                data['standardized_pos_code'] = pos.get('code')
                data['standardized_pos_name'] = pos.get('name_en', pos.get('name'))
        return data

class StandardizedPosSchema(Schema):
    ...
    name_tl = fields.String(validate=Length(max=100))
    name = fields.String(dump_only=True)  # Compatibility field
    description = fields.String()
    
    @post_dump
    def add_compatibility_fields(self, data, **kwargs):
        """Add name field for compatibility with clients expecting the old schema."""
        if 'name_en' in data and not data.get('name'):
