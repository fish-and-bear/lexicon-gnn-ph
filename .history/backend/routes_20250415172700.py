...
class DefinitionSchema(BaseSchema):
    ...
    standardized_pos = fields.Nested('LocalPosSchema', dump_default=None)
...
def database_error(error):
    ...
    db.session.rollback()  # Explicitly rollback the session on database errors
...
def handle_exception(error):
    ...
    db.session.rollback()  # Roll back on any exception
    API_ERRORS.labels(endpoint=request.endpoint or '', error_type=type(error).__name__).inc()
    logger.error(f"Error in {request.endpoint or 'unknown'}: {str(error)}", exc_info=True)
