...
class Word(db.Model):
    ...
    pronunciation_data = db.Column(db.JSON)  # Legacy field for backward compatibility
    ...
    pronunciations = db.relationship('Pronunciation', back_populates='word', lazy='joined')
    
    # Affixation relationships
    affixations_as_root = db.relationship('Affixation', 
                                        foreign_keys='Affixation.root_word_id',
                                        back_populates='root_word',
                                        lazy='joined')
    affixations_as_affixed = db.relationship('Affixation',
...
class Etymology(db.Model):
    ...
    components = db.Column(db.JSON)  # List of etymological components
    normalized_components = db.Column(db.Text)  # Normalized components for search
    language_codes = db.Column(db.JSON)  # List of language codes
    structure = db.Column(db.JSON)  # Etymology structure
    ...
    # Relationships
    word = db.relationship('Word', back_populates='etymologies', lazy='joined')
    ...
    def update_normalized_components(self):
        """Update normalized_components field from components."""
        if not self.components:
            self.normalized_components = ''
            return
        
        # Convert components to list if it's a string
        if isinstance(self.components, str):
            try:
                components = json.loads(self.components)
            except json.JSONDecodeError:
                components = [comp.strip() for comp in self.components.split(',') if comp.strip()]
        else:
            components = self.components
        
        # Normalize each component and join with spaces
        normalized = ' '.join(normalize_text(comp) for comp in components)
        self.normalized_components = normalized
...
class Affixation(db.Model):
    ...
    position = db.Column(db.Integer)  # position for infixes
    derived_form = db.Column(db.String(255), nullable=False)
    meaning = db.Column(db.Text)
    notes = db.Column(db.Text)
    meta_info = db.Column(db.JSON)  # additional metadata about the affixation
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    ...
    def validate_affix_type(self, key, value):
        """Validate affix type."""
        valid_types = ['prefix', 'suffix', 'infix', 'circumfix']
        if value not in valid_types:
            raise ValueError(f"Invalid affix type. Must be one of: {', '.join(valid_types)}")
        return value
    
    @validates('position')
    def validate_position(self, key, value):
        """Validate position for infixes."""
        if self.affix_type == 'infix' and value is None:
            raise ValueError("Position is required for infixes")
        return value
...
def init_app(app):
    ...
    with app.app_context():
        ...
        # Set up database-specific features
        with db.engine.connect() as connection:
            for ddl in get_index_ddl(connection):
        ...
        event.listen(Etymology, 'before_insert', Etymology.update_normalized_components)
        event.listen(Etymology, 'before_update', Etymology.update_normalized_components)
