...
@with_transaction(commit=True)
def insert_relation(cur, from_word_id: int, to_word_id: int, relation_type: str, 
    ...
    Args:
        ...
        metadata: Additional metadata about the relationship (e.g., strength score, tags)
    """
    try:
        ...
        cur.execute("""
            ...
            DO UPDATE SET
                ...
                metadata = COALESCE(
                    CASE 
                        WHEN relations.metadata IS NULL THEN EXCLUDED.metadata
                        WHEN EXCLUDED.metadata IS NULL THEN relations.metadata
                        ELSE jsonb_merge(relations.metadata, EXCLUDED.metadata)
                    END,
                    relations.metadata,
                    EXCLUDED.metadata,
                    '{}'::jsonb
                )
        """, (from_word_id, to_word_id, relation_type, sources, metadata_json))
    except Exception as e:
        logger.error(f"Error in insert_relation from {from_word_id} to {to_word_id}: {e}")
        
    # Define jsonb_merge function if it doesn't exist
    try:
        cur.execute("""
        ...
        logger.warning(f"Could not create jsonb_merge function: {e}")
        # If we can't create the function, we'll use a simpler update approach
        cur.execute("""
            UPDATE relations 
            SET metadata = COALESCE(metadata, '{}'::jsonb) || %s::jsonb
            WHERE from_word_id = %s AND to_word_id = %s AND relation_type = %s
            AND %s::jsonb IS NOT NULL
        """, (metadata_json, from_word_id, to_word_id, relation_type, metadata_json))
...
def process_kwf_dictionary(cur, filename: str):
    """
    Process the KWF Dictionary JSON file and store its entries in the database.
    ...
    for word, entry in data.items():
        if not isinstance(entry, dict):
            continue
        # Use the 'original' value if available; otherwise, use the key.
        original_word = entry.get('original', word)
        word_id = get_or_create_word_id(cur, original_word, language_code='tl')
        definitions = entry.get('definitions', {})
        if isinstance(definitions, dict):
            for pos, def_list in definitions.items():
                # pos is the part of speech (e.g., "Pangatnig")
...
def process_kaikki_jsonl(cur, filename: str):
    ...
    def process_relations(cur, from_word_id: int, relations_dict: Dict[str, List[str]], lang_code: str, source: str):
        ...
        for raw_key, related_list in relations_dict.items():
            ...
            for rel_word_lemma in related_list:
