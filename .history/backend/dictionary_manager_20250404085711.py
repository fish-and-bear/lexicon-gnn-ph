...
@with_transaction(commit=True) # Changed commit to True
def process_root_words_cleaned(cur, filename: str):
    ...
    Relies on the @with_transaction decorator for transaction management.
    ...
    # conn = cur.connection # No longer needed for manual commit/rollback
    ...
    if isinstance(data, list):  # Original list format
        ...
        if not all(isinstance(v, dict) for v in data.values()):
            ...
            raise TypeError(f"Invalid dictionary value format in {filename}") # Raise error

        def dict_iterator(d):
            for i, (key, value) in enumerate(d.items()):
                # Ensure value is a dict before copying
                if not isinstance(value, dict):
                     logger.warning(f"Skipping non-dict value for key '{key}' in {filename}")
                     continue
                entry_data = value.copy()
                entry_data["root_word"] = key # Add root_word key to the details dict
                yield i, entry_data
    ...
    if total_roots == 0:
        ...
        return stats # Return empty stats, decorator handles commit

    # --- Process Entries ---
    # commit_interval = 500 # Removed periodic commit logic
    with tqdm(
        total=total_roots, desc=f"Processing {source_identifier}"
    ) as pbar:
        for entry_index, root_word_entry in entries_iterator:
            # savepoint_name = f"tagalogcom_root_cleaned_{entry_index}" # Removed savepoint logic
            root_word = None  # Initialize for error logging

            try:
                # cur.execute(f"SAVEPOINT {savepoint_name}") # Removed savepoint logic
                ...
                if not isinstance(root_word_entry, dict):
                    ...
                    stats["skipped"] += 1
                    # cur.execute(f"RELEASE SAVEPOINT {savepoint_name}") # Removed savepoint logic
                    pbar.update(1)
                    continue
                ...
                # --- Root Word Creation (Error here should rollback via decorator) ---
                word_id = get_or_create_word_id(
                    cur,
                    root_word,
                    language_code=language_code,
                ...
                if not word_id or word_id <= 0:
                    # Let get_or_create_word_id raise error or log it, then raise here
                ...
                stats["roots_processed"] += 1 # Changed key
                ...
                if isinstance(definitions, list):
                    for def_idx, definition_item in enumerate(definitions):
                        ...
                        if definition_text:
                            # Error inside insert_definition should rollback via decorator
                            ...
                            if def_id:
                                ...
                                # This case implies insert_definition failed internally but didn't raise
                                # which might indicate a logic issue in insert_definition itself.
                                logger.warning(f"insert_definition returned None/0 for root '{root_word}', def {def_idx}.")
                                # Optionally, raise an error here to ensure transaction rollback if this is unexpected
                                # raise ValueError("insert_definition failed unexpectedly.")
                ...
                if isinstance(associated_data, dict):
                    def assoc_dict_iterator(d):
                        for i, (key, value) in enumerate(d.items()):
                             # Ensure value is a dict before yielding
                             if isinstance(value, dict):
                                 yield i, key.strip(), value
                             else:
                                 logger.warning(f"Skipping non-dict value for associated word key '{key}' in root '{root_word}'")
                    ...
                    def assoc_list_iterator(l):
                        for i, item in enumerate(l):
                            ...
                            if isinstance(item, str): word_str = item.strip()
                                ...
                                details = item
                            # Ensure word_str is valid before yielding
                    ...
                    if assoc_word and assoc_word != root_word:
                        ...
                        if assoc_word_id:
                            ...
                            if rel_id:
                                ...
                                logger.debug(f"DERIVED_FROM relation {assoc_word_id} -> {word_id} might already exist or failed internally.")
                            ...
                            if assoc_def_text:
                                ...
                                if assoc_def_id:
                                    ...
                                    logger.warning(f"insert_definition returned None/0 for associated word '{assoc_word}'.")
                ...
                # Reraise the exception to trigger rollback by the decorator
                # This ensures the faulty entry doesn't commit and the whole transaction fails
                raise entry_e

            finally:
                # This will always run, even if an error occurred and was raised
                pbar.update(1)

    # --- Final Commit --- Removed ---
    # try: ... conn.commit() ...
    # The decorator @with_transaction(commit=True) handles the final commit or rollback

    if error_types:
        logger.warning(f"Error summary for {filename}: {json.dumps(error_types, indent=2)}")
    if stats["roots_processed"] == 0 and total_roots > 0: # Use correct key
        logger.warning(f"No root entries were successfully processed from {filename} despite finding {total_roots} entries.")

    # If the loop completes without any exceptions being raised,
    # the @with_transaction decorator will commit the transaction.
    # If any exception was raised inside the loop, the decorator will roll it back.
    return stats
