...
def insert_affixation(
    ...
    Args:
        cur: Database cursor.
        word_id: ID of the word.
        pronunciation_data: Pronunciation data (string or dictionary).
        source_identifier: Identifier for the data source (e.g., filename). MANDATORY.
    ...
    try:
        ...
        if isinstance(pronunciation_data, str):
            # Handle string input
            params['type'] = 'ipa'
            params['value'] = pronunciation_data
        elif isinstance(pronunciation_data, dict):
            # Handle dictionary input
            params['type'] = pronunciation_data.get('type', 'ipa')
            params['value'] = pronunciation_data.get('value')
            if not params['value']:
                logger.warning(f"Skipping pronunciation insert for word ID {word_id} (source '{source_identifier}'): Missing value.")
                return None
        else:
            logger.warning(f"Skipping pronunciation insert for word ID {word_id} (source '{source_identifier}'): Invalid data type.")
            return None

        cur.execute("""
            INSERT INTO pronunciations (word_id, type, value, sources)
            VALUES (%(word_id)s, %(type)s, %(value)s, %(sources)s)
            ON CONFLICT (word_id, type, value) -- Conflict on exact triplet
            DO UPDATE SET
                -- Overwrite sources: Last write wins for this pronunciation record
                sources = COALESCE(pronunciations.sources, '[]'::jsonb) || %(sources)s,
                updated_at = CURRENT_TIMESTAMP
            RETURNING id
        """, params)
        pronunciation_id = cur.fetchone()[0]
        logger.debug(f"Inserted/Updated pronunciation (ID: {pronunciation_id}) for word ID {word_id} from source '{source_identifier}'.")
        return pronunciation_id

    except psycopg2.IntegrityError as e:
        # Likely due to non-existent word_id (FK constraint violation)
        logger.error(f"Integrity error inserting pronunciation for word ID {word_id} from '{source_identifier}'. Word ID might not exist. Error: {e.pgcode} {e.pgerror}")
        return None
    except psycopg2.Error as e:
        logger.error(f"Database error inserting pronunciation for word ID {word_id} from '{source_identifier}': {e.pgcode} {e.pgerror}")
        return None
    except Exception as e:
        logger.error(f"Unexpected error inserting pronunciation for word ID {word_id} from '{source_identifier}': {e}", exc_info=True)
        return None
...
@with_transaction(commit=True)
def insert_credit(cur, word_id: int, credit_data: Dict, source_identifier: str) -> Optional[int]:
    """
    Insert credit data for a word.
    Uses ON CONFLICT to update existing credits based on (word_id, author_name, author_url),
    applying a 'last write wins' strategy for the 'sources' field.
    ...
    try:
        ...
        params = {
            'word_id': word_id,
            'author_name': credit_data.get('author_name', ''),
            'author_url': credit_data.get('author_url', ''),
            'sources': source_identifier
        }

        cur.execute("""
            INSERT INTO credits (word_id, author_name, author_url, sources)
            VALUES (%(word_id)s, %(author_name)s, %(author_url)s, %(sources)s)
            ON CONFLICT (word_id, author_name, author_url) -- Conflict on exact triplet
            DO UPDATE SET
                -- Overwrite sources: Last write wins for this credit record
                sources = COALESCE(credits.sources, '[]'::jsonb) || %(sources)s,
                updated_at = CURRENT_TIMESTAMP
            RETURNING id
        """, params)
        credit_id = cur.fetchone()[0]
        logger.debug(f"Inserted/Updated credit (ID: {credit_id}) for word ID {word_id} from source '{source_identifier}'.")
        return credit_id

    except psycopg2.IntegrityError as e:
        # Likely due to non-existent word_id (FK constraint violation)
        logger.error(f"Integrity error inserting credit for word ID {word_id} from '{source_identifier}'. Word ID might not exist. Error: {e.pgcode} {e.pgerror}")
        return None
    except psycopg2.Error as e:
        logger.error(f"Database error inserting credit for word ID {word_id} from '{source_identifier}': {e.pgcode} {e.pgerror}")
        return None
    except Exception as e:
        logger.error(f"Unexpected error inserting credit for word ID {word_id} from '{source_identifier}': {e}", exc_info=True)
        return None
...
@with_transaction(commit=True)
def insert_definition(cur, word_id: int, definition_text: str, part_of_speech: str = None,
                     examples: str = None, usage_notes: str = None, tags: str = None,
                     source_identifier: str = None) -> Optional[int]:
    """
    Insert a definition for a word.
    Uses ON CONFLICT to update existing definitions based on (word_id, definition_text),
    applying a 'last write wins' strategy for the 'sources' field.
    ...
    try:
        ...
        cur.execute("""
            INSERT INTO definitions (word_id, definition_text, part_of_speech, examples, usage_notes, tags, sources)
            VALUES (%(word_id)s, %(definition_text)s, %(part_of_speech)s, %(examples)s, %(usage_notes)s, %(tags)s, %(sources)s)
            ON CONFLICT (word_id, definition_text) -- Conflict on exact pair
            DO UPDATE SET
                -- Overwrite sources: Last write wins for this definition record
                sources = COALESCE(definitions.sources, '[]'::jsonb) || %(sources)s,
                updated_at = CURRENT_TIMESTAMP
            RETURNING id
        """, params)
        definition_id = cur.fetchone()[0]
        logger.debug(f"Inserted/Updated definition (ID: {definition_id}) for word ID {word_id} from source '{source_identifier}'.")
        return definition_id

    except psycopg2.IntegrityError as e:
        # Likely due to non-existent word_id (FK constraint violation)
        logger.error(f"Integrity error inserting definition for word ID {word_id} from '{source_identifier}'. Word ID might not exist. Error: {e.pgcode} {e.pgerror}")
        return None
    except psycopg2.Error as e:
        logger.error(f"Database error inserting definition for word ID {word_id} from '{source_identifier}': {e.pgcode} {e.pgerror}")
        return None
    except Exception as e:
        logger.error(f"Unexpected error inserting definition for word ID {word_id} from '{source_identifier}': {e}", exc_info=True)
        return None
...
@with_transaction(commit=True)
def insert_relation(cur, from_word_id: int, to_word_id: int,
                    relation_type: Union[RelationshipType, str],
                    source_identifier: str, # MANDATORY
                    metadata: Optional[Dict] = None
                   ) -> Optional[int]:
    ...
    Insert a relationship between two words. Uses ON CONFLICT to update existing relations
    based on (from_word_id, to_word_id, relation_type), applying a 'last write wins'
    strategy for the 'sources' field.

    Args:
        cur: Database cursor.
        from_word_id: ID of the source word.
        to_word_id: ID of the target word.
        relation_type: Type of relationship (e.g., 'synonym', 'antonym', 'related').
        source_identifier: Identifier for the data source (e.g., filename). MANDATORY.
        metadata: Additional metadata for the relationship (optional).

    Returns:
        The ID of the inserted/updated relationship record, or None if failed.
    """
    if not source_identifier:
        logger.error(f"CRITICAL: Skipping relation insert for words {from_word_id} -> {to_word_id}: Missing MANDATORY source identifier.")
        return None
    ...
    try:
        ...
        params = {
            ...
            'relation_type': relation_type.rel_value if isinstance(relation_type, RelationshipType) else relation_type,
            'sources': source_identifier,
            'metadata': json.dumps(metadata) if metadata else None
        }

        cur.execute("""
            INSERT INTO relationships (from_word_id, to_word_id, relation_type, sources, metadata)
            VALUES (%(from_word_id)s, %(to_word_id)s, %(relation_type)s, %(sources)s, %(metadata)s)
            ON CONFLICT (from_word_id, to_word_id, relation_type) -- Conflict on exact triplet
            DO UPDATE SET
                -- Overwrite sources: Last write wins for this relationship record
                sources = COALESCE(relationships.sources, '[]'::jsonb) || %(sources)s,
                updated_at = CURRENT_TIMESTAMP
            RETURNING id
        """, params)
        relation_id = cur.fetchone()[0]
        logger.debug(f"Inserted/Updated relationship (ID: {relation_id}) {from_word_id}(from) -> {to_word_id}(to) [{relation_type}] from source '{source_identifier}'.")
        return relation_id

    except psycopg2.IntegrityError as e:
        # Likely due to non-existent from_word_id or to_word_id (FK constraint violation)
        logger.error(f"Integrity error inserting relationship {from_word_id}->{to_word_id} ({relation_type}) from '{source_identifier}'. Word ID might not exist. Error: {e.pgcode} {e.pgerror}")
        return None
    except psycopg2.Error as e:
        logger.error(f"Database error inserting relationship {from_word_id}->{to_word_id} ({relation_type}) from '{source_identifier}': {e.pgcode} {e.pgerror}")
        return None
    except Exception as e:
        logger.error(f"Unexpected error inserting relationship {from_word_id}->{to_word_id} ({relation_type}) from '{source_identifier}': {e}", exc_info=True)
        return None


@with_transaction(commit=True)
def insert_affixation(cur, root_id: int, affixed_id: int, affix_type: str,
                      source_identifier: str # MANDATORY
                     ) -> Optional[int]:
    """
    ...
    try:
        ...
        cur.execute("""
            ...
            DO UPDATE SET
                ...
                sources = COALESCE(affixations.sources, '[]'::jsonb) || %(sources)s,
                updated_at = CURRENT_TIMESTAMP
            RETURNING id
        """, params)
