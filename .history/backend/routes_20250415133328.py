...
def search():
    ...
    try:
        ...
        if exclude_baybayin:
            search_conditions.append("(w.has_baybayin = FALSE OR w.has_baybayin IS NULL)")
...
def get_random_word():
    ...
    try:
        ...
        word_result = db.session.execute(text(query), params).fetchone()
        ...
        full_word = _fetch_word_details(
            ...
            include_pronunciations=True,
            include_credits=True,
            include_relations=False, # Skip relations to speed up the response
            include_affixations=False, # Skip affixations to speed up the response
            include_root=True,
            include_derived=False, # Skip derived words to speed up the response
            include_forms=True,
            include_templates=True,
...
def _fetch_word_details(word_id,
    ...
    # Use a separate session for each major operation to avoid transaction issues
    with db.session.begin() as session:
        try:
            ...
            WHERE id = :id
            """
            word_result = session.execute(text(sql_word), {"id": word_id}).fetchone()
            ...
            logger.error(f"Error fetching basic word data for word_id={word_id}: {e}", exc_info=True)
            raise e
            
    # Initialize empty arrays for all relations to avoid None values
    word.definitions = []
    word.etymologies = []
    word.pronunciations = []
    word.credits = []
    ...
    # Use a new session for each relationship to avoid transaction issues

    # Load definitions if requested
    if include_definitions and word:
        with db.session.begin() as session:
            try:
                sql_defs = """
                ...
                """
                defs_result = session.execute(text(sql_defs), {"word_id": word_id}).fetchall()
                ...
                if pos_ids:
                    ...
                    pos_results = session.execute(text(sql_pos), {"ids": pos_ids}).fetchall()
                ...
                logger.error(f"Error loading definitions for word {word_id}: {e}", exc_info=True)
                # Continue with other sections instead of failing completely
                session.rollback()

    # Load etymologies if requested
    if include_etymologies and word:
        with db.session.begin() as session:
            try:
                sql_etym = """
                ...
                WHERE word_id = :word_id
                """
                etym_results = session.execute(text(sql_etym), {"word_id": word_id}).fetchall()
                
                for e in etym_results:
                    etymology = Etymology()
                    for key in e.keys():
                ...
                logger.error(f"Error loading etymologies for word {word_id}: {e}", exc_info=True)
                session.rollback()

    # Load pronunciations if requested
    if include_pronunciations and word:
        with db.session.begin() as session:
            try:
                sql_pron = """
                ...
                WHERE word_id = :word_id
                """
                pron_results = session.execute(text(sql_pron), {"word_id": word_id}).fetchall()
                
                for p in pron_results:
                    ...
                    for key in p.keys():
                        if hasattr(pronunciation, key):
                            try:
                                ...
                                logger.warning(f"Error setting pronunciation attr {key} for word {word_id}: {attr_e}")
                    pronunciation.word_id = word_id
                    word.pronunciations.append(pronunciation)
            except Exception as e:
                logger.error(f"Error loading pronunciations for word {word_id}: {e}", exc_info=True)
                session.rollback()

    # Load credits if requested
    if include_credits and word:
        with db.session.begin() as session:
            try:
                sql_cred = "SELECT id, credit FROM credits WHERE word_id = :word_id"
                cred_results = session.execute(text(sql_cred), {"word_id": word_id}).fetchall()
                ...
                for c in cred_results:
                    ...
                    for key in c.keys():
                        if hasattr(credit, key):
                            try:
                                setattr(credit, key, c[key])
                            except Exception as attr_e:
                                logger.warning(f"Error setting credit attr {key} for word {word_id}: {attr_e}")
                    credit.word_id = word_id
                    word.credits.append(credit)
            except Exception as e:
                logger.error(f"Error loading credits for word {word_id}: {e}", exc_info=True)
                session.rollback()

    # --- Load root_word if requested ---
    if include_root and word and word.root_word_id:
        with db.session.begin() as session:
            try:
                sql_root = """
                ...
                """
                root_result = session.execute(text(sql_root), {"root_id": word.root_word_id}).fetchone()
                
                if root_result:
                    root_word = Word()
                    for key in root_result.keys():
                        if hasattr(root_word, key):
                            try:
                                setattr(root_word, key, root_result[key])
                            except Exception as attr_e:
                                logger.warning(f"Error setting root_word attr {key} for word {word_id}: {attr_e}")
                    word.root_word = root_word
            except Exception as e:
                logger.error(f"Error loading root word for word {word_id}: {e}", exc_info=True)
                session.rollback()

    # --- Load derived words if requested ---
    if include_derived and word:
        with db.session.begin() as session:
            try:
                sql_derived = """
                SELECT id, lemma, language_code, has_baybayin, baybayin_form
                ...
                """
                derived_results = session.execute(text(sql_derived), {"word_id": word_id}).fetchall()
                
                for d in derived_results:
                    derived_word = Word()
                    for key in d.keys():
                        if hasattr(derived_word, key):
                            try:
                                setattr(derived_word, key, d[key])
                            except Exception as attr_e:
                                logger.warning(f"Error setting derived_word attr {key} for word {word_id}: {attr_e}")
                    derived_word.root_word_id = word_id
                    word.derived_words.append(derived_word)
            except Exception as e:
                logger.error(f"Error loading derived words for word {word_id}: {e}", exc_info=True)
                session.rollback()
                
    # --- Load outgoing relations if requested ---
    if include_relations and word:
        with db.session.begin() as session:
            try:
                sql_out_rel = """
                ...
                WHERE r.from_word_id = :word_id
                """
                out_rel_results = session.execute(text(sql_out_rel), {"word_id": word_id}).fetchall()
                
                for r in out_rel_results:
                    relation = Relation()
                    relation.id = r.id
                    relation.from_word_id = r.from_word_id
                    relation.to_word_id = r.to_word_id
                    relation.relation_type = r.relation_type
                    relation.relation_data = r.relation_data if hasattr(r, 'relation_data') else None
                ...
                logger.error(f"Error loading outgoing relations for word {word_id}: {e}", exc_info=True)
                session.rollback()
                
    # --- Load incoming relations if requested ---
    if include_relations and word:
        with db.session.begin() as session:
            try:
                sql_in_rel = """
                ...
                WHERE r.to_word_id = :word_id
                """
                in_rel_results = session.execute(text(sql_in_rel), {"word_id": word_id}).fetchall()
                
                for r in in_rel_results:
                    relation = Relation()
                    relation.id = r.id
                    relation.from_word_id = r.from_word_id
                    relation.to_word_id = r.to_word_id
                ...
                logger.error(f"Error loading incoming relations for word {word_id}: {e}", exc_info=True)
                session.rollback()
                
    # --- Load affixations (root) if requested ---
    if include_affixations and word:
        with db.session.begin() as session:
            try:
                sql_root_aff = """
                ...
                """
                root_aff_results = session.execute(text(sql_root_aff), {"word_id": word_id}).fetchall()
                
                for a in root_aff_results:
                    affixation = Affixation()
                    affixation.id = a.id
                    affixation.root_word_id = word_id
                    affixation.affixed_word_id = a.affixed_word_id
                    affixation.affix_type = a.affix_type
                    affixation.sources = a.sources
                    
                    # Create affixed word
                    affixed_word = Word()
                    affixed_word.id = a.affixed_word_id
                    affixed_word.lemma = a.affixed_word_lemma
                    affixed_word.language_code = a.affixed_word_language_code
                    
                    affixation.affixed_word = affixed_word
                    word.root_affixations.append(affixation)
            except Exception as e:
                logger.error(f"Error loading root affixations for word {word_id}: {e}", exc_info=True)
                session.rollback()
                
    # --- Load affixations (affixed) if requested ---
    if include_affixations and word:
        with db.session.begin() as session:
            try:
                sql_affixed_aff = """
                SELECT af.id, af.affix_type, af.sources,
                       w.id as root_word_id, w.lemma as root_word_lemma, w.language_code as root_word_language_code
                FROM affixations af
                JOIN words w ON af.root_word_id = w.id
                WHERE af.affixed_word_id = :word_id
                """
                affixed_aff_results = session.execute(text(sql_affixed_aff), {"word_id": word_id}).fetchall()
                
                for a in affixed_aff_results:
                    affixation = Affixation()
                    affixation.id = a.id
                    affixation.root_word_id = a.root_word_id
                    affixation.affixed_word_id = word_id
                    affixation.affix_type = a.affix_type
                    affixation.sources = a.sources
                    
                    # Create root word
                    root_word = Word()
                    root_word.id = a.root_word_id
                    root_word.lemma = a.root_word_lemma
                    root_word.language_code = a.root_word_language_code
                    
                    affixation.root_word = root_word
                    word.affixed_affixations.append(affixation)
            except Exception as e:
                logger.error(f"Error loading affixed affixations for word {word_id}: {e}", exc_info=True)
                session.rollback()
                
    # --- Load forms if requested ---
    if include_forms and word:
        with db.session.begin() as session:
            try:
                sql_forms = "SELECT id, form, tags, is_canonical, is_primary FROM word_forms WHERE word_id = :word_id"
                form_results = session.execute(text(sql_forms), {"word_id": word_id}).fetchall()
                ...
                for f in form_results:
                    ...
                    for key in f.keys():
                        if hasattr(form, key):
                            try:
                                setattr(form, key, f[key])
                            except Exception as attr_e:
                                logger.warning(f"Error setting form attr {key} for word {word_id}: {attr_e}")
                    form.word_id = word_id
                    word.forms.append(form)
            except Exception as e:
                logger.error(f"Error loading forms for word {word_id}: {e}", exc_info=True)
                session.rollback()
                
    # --- Load templates if requested ---
    if include_templates and word:
        with db.session.begin() as session:
            try:
                sql_templates = "SELECT id, template_name, args, expansion FROM word_templates WHERE word_id = :word_id"
                template_results = session.execute(text(sql_templates), {"word_id": word_id}).fetchall()
                ...
                for t in template_results:
                    ...
                    for key in t.keys():
                        if hasattr(template, key):
                            try:
                                setattr(template, key, t[key])
                            except Exception as attr_e:
                                logger.warning(f"Error setting template attr {key} for word {word_id}: {attr_e}")
                    template.word_id = word_id
                    word.templates.append(template)
            except Exception as e:
                logger.error(f"Error loading templates for word {word_id}: {e}", exc_info=True)
                session.rollback()

    # Cache the result if successful
    if word and cache_client and cache_key:
        try:
            cache_client.set(cache_key, pickle.dumps(word))
        except Exception as e:
