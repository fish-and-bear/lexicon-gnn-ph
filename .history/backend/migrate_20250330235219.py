...
def verify_schema_compatibility():
    ...
    try:
        with engine.begin() as conn:
            ...
            # Rename metadata column to word_metadata if it exists
            logger.info("Checking metadata column...")
            try:
                conn.execute(text("""
                    DO $$ 
                    BEGIN
                        IF EXISTS (
                            ...
                            WHERE table_name = 'words' AND column_name = 'metadata'
                        ) THEN
                            ALTER TABLE words RENAME COLUMN metadata TO word_metadata;
                            logger.info("Renamed metadata column to word_metadata");
                        END IF;
                    END $$;
                """))
                logger.info("metadata column check complete")
            except Exception as e:
                logger.error(f"Error checking metadata column: {e}")
                return False
            ...
            try:
                ...
                """))
                logger.info("Baybayin constraints updated successfully")
            except Exception as e:
                logger.error(f"Error updating Baybayin constraints: {e}")
                return False
            ...
            try:
                ...
                """))
                logger.info("search_text values updated")
            except Exception as e:
                logger.error(f"Error configuring search_text: {e}")
                return False
