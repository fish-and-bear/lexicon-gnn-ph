...
class SearchSchema(Schema):
    ...
    mode = fields.Str(validate=validate.OneOf([
        'all', 'exact', 'baybayin', 'fuzzy', 'etymology',
    ...
    ]), dump_default='relevance')
    order = fields.Str(validate=validate.OneOf(['asc', 'desc']), dump_default='desc')
...
class WordSchema(Schema):
    ...
    lemma = fields.Str(required=True, validate=validate.Length(min=1, max=255))
    language_code = fields.Str(required=True, validate=validate.Length(max=16))
    normalized_lemma = fields.Str()
    has_baybayin = fields.Bool()
    baybayin_form = fields.Str(validate=validate.Length(max=255))
    romanized_form = fields.Str(validate=validate.Length(max=255))
    root_word_id = fields.Int()
    preferred_spelling = fields.Str(validate=validate.Length(max=255))
    tags = fields.Str()
    idioms = fields.Dict()
    pronunciation_data = fields.Dict()
    source_info = fields.Dict()
    word_metadata = fields.Dict()
    data_hash = fields.Str()
    search_text = fields.Str()
    badlit_form = fields.Str()
    ...
    pronunciations = fields.List(fields.Dict(), validate=validate.Length(max=10))
    credits = fields.List(fields.Dict(), validate=validate.Length(max=10))

class WordQuerySchema(Schema):
    """Schema for word query parameters."""
    language_code = fields.Str(validate=validate.Length(max=16))
    include_definitions = fields.Bool(default=True)
    include_relations = fields.Bool(default=True)
    include_etymology = fields.Bool(default=True)
    include_pronunciation = fields.Bool(default=True)
...
class SearchQuerySchema(Schema):
    ...
    language = fields.Str(validate=validate.OneOf(['tl', 'ceb']), dump_default='tl')
    include_baybayin = fields.Bool(default=True)
    min_similarity = fields.Float(validate=validate.Range(min=0.0, max=1.0), default=0.3)
    mode = fields.Str(validate=validate.OneOf(['all', 'exact', 'phonetic', 'baybayin']), dump_default='all')
    sort = fields.Str(validate=validate.OneOf(['relevance', 'alphabetical', 'created', 'updated']), dump_default='relevance')
    order = fields.Str(validate=validate.OneOf(['asc', 'desc']), dump_default='desc')
...
def validate_relation_type(rel_type: str) -> bool:
    ...
    valid_types = [
        'synonym', 'antonym', 'variant', 'derived_from', 'root_of',
        'related', 'hypernym', 'hyponym', 'meronym', 'holonym',
        'regional_variant', 'spelling_variant'
    ]
    return rel_type in valid_types

def validate_affix_type(affix_type: str) -> bool:
    """Validate affix type."""
    valid_types = ['prefix', 'infix', 'suffix', 'circumfix']
...
def validate_sources(sources: str) -> bool:
    ...
    return isinstance(sources, str) and sources.strip()
...
def validate_metadata(metadata: Dict[str, Any]) -> bool:
    ...
    required_fields = ['source_info', 'word_metadata']
...
def validate_etymology_data(data: Dict[str, Any]) -> bool:
    ...
    required_fields = ['etymology_text', 'language_codes']
...
def validate_definition_data(data: Dict[str, Any]) -> bool:
    ...
    required_fields = ['definition_text', 'original_pos']
    if not all(field in data for field in required_fields):
        return False
    
    # Validate POS code if present
    if 'original_pos' in data and not validate_pos_code(data['original_pos']):
        return False
