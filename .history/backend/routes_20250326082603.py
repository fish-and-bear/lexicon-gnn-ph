...
from sqlalchemy import or_, and_, func, desc, text, event, not_
...
def search_words(**params):
    ...
    try:
        ...
        # Try using the advanced search with PostgreSQL extensions
        try:
            # Use SQL for optimized search with complex ranking
            sql = text("""
                WITH search_results AS (
                    SELECT 
                        ...
                        array_agg(DISTINCT d.id) OVER (PARTITION BY w.id) as definition_ids,
                    ...
                    AND (
                        CASE 
                            ...
                            WHEN :mode = 'baybayin' THEN w.has_baybayin = true AND w.baybayin_form IS NOT NULL AND w.baybayin_form LIKE :query || '%'
                            WHEN :mode = 'fuzzy' THEN similarity(w.normalized_lemma, :normalized_query) > :min_similarity
                            ELSE (
                                ...
                                OR (w.has_baybayin = true AND w.baybayin_form IS NOT NULL AND w.baybayin_form LIKE :query || '%')
                                OR w.preferred_spelling = :query
                                OR w.lemma ILIKE '%' || :query || '%'
                ...
                ORDER BY 
                    ...
                    CASE 
                        ...
                        WHEN :sort = 'created' THEN extract(epoch from sr.created_at)
                        WHEN :sort = 'updated' THEN extract(epoch from sr.updated_at)
                    END DESC
                LIMIT :limit
            """)
            ...
            # Fallback to simpler query without PostgreSQL specific functions
            logger.warning(f"Falling back to simple search due to error: {str(e)}")
            
            # Construct a simpler WHERE clause based on the mode
            if mode == 'exact':
                where_clause = "w.normalized_lemma = :normalized_query"
            elif mode == 'baybayin':
                where_clause = "w.has_baybayin = true AND w.baybayin_form IS NOT NULL AND w.baybayin_form LIKE :query || '%'"
            else:
                # Default to basic LIKE search
                where_clause = "w.lemma ILIKE '%' || :query || '%' OR w.normalized_lemma ILIKE '%' || :normalized_query || '%'"
                if include_baybayin:
                    where_clause += " OR (w.has_baybayin = true AND w.baybayin_form IS NOT NULL AND w.baybayin_form LIKE :query || '%')"
            
            # Construct simple query without advanced PostgreSQL functions
            simple_sql = text(f"""
                SELECT 
                    w.id,
                    w.lemma,
                    ...
                    EXISTS(SELECT 1 FROM etymologies e WHERE e.word_id = w.id) as has_etymology,
                    EXISTS(SELECT 1 FROM relations r WHERE r.from_word_id = w.id OR r.to_word_id = w.id) as has_relations,
                    ARRAY(SELECT d.id FROM definitions d WHERE d.word_id = w.id) as definition_ids,
                    ARRAY(SELECT d.definition_text FROM definitions d WHERE d.word_id = w.id) as definitions,
                    ARRAY(SELECT DISTINCT p.code FROM definitions d JOIN parts_of_speech p ON d.standardized_pos_id = p.id WHERE d.word_id = w.id) as pos_codes,
                ...
                WHERE w.language_code = :language
                AND ({where_clause})
                AND (:pos IS NULL OR EXISTS(
                    SELECT 1 FROM definitions d 
                    JOIN parts_of_speech p ON d.standardized_pos_id = p.id 
                    WHERE d.word_id = w.id AND p.code = :pos
                ))
                ORDER BY 
                    CASE 
                        WHEN :sort = 'alphabetical' THEN w.lemma
                        WHEN :sort = 'created' THEN w.created_at::text
                        WHEN :sort = 'updated' THEN w.updated_at::text
                        ELSE w.lemma
                    END {('DESC' if order == 'desc' else 'ASC')}
                LIMIT :limit
            """)
            
            # Execute the simpler query
            results = db.session.execute(
                simple_sql,
                {
                    "query": query,
        ...
        for row in results:
            ...
            if include_definitions and definitions:
                result["definitions"] = definitions[:3]  # Limit to 3 definitions for brevity
                
            # Add etymology if requested
            if include_etymology and row.has_etymology and row.id:
                etymologies = Etymology.query.filter(Etymology.word_id == row.id).all()
                if etymologies:
                    result["etymology"] = {
                        "text": etymologies[0].etymology_text,
                        "sources": etymologies[0].get_sources_list()
                    }
...
def get_baybayin_words(**params):
    """Get words with Baybayin script with proper handling of has_baybayin flag."""
    ...
    try:
        # Enhanced query: Use has_baybayin flag for more reliable data
        query = db.session.query(Word).filter(
            Word.has_baybayin == True,  # This is now the primary indicator
            Word.baybayin_form.isnot(None),  # Double-check that baybayin_form exists
        ...
        if not words:
            return success_response(
                ...
                message="No Baybayin words found"
            )
        ...
        result = [
            {
                ...
                "romanized_form": word.romanized_form or get_romanized_text(word.baybayin_form),
        ...
        logger.error(f"Error retrieving Baybayin words: {str(e)}")
        return error_response(f"Error retrieving Baybayin words: {str(e)}", 500)
...
def get_affixes():
    ...
    try:
        ...
        # Build query for affix statistics
        sql = text("""
            WITH affix_stats AS (
                SELECT 
                    a.affix_type,
                    ...
                    json_agg(DISTINCT regexp_replace(w.lemma, '^.*, '\\1')) FILTER (WHERE w.id = a.root_word_id) as sample_roots,
                    json_agg(DISTINCT regexp_replace(w2.lemma, '^.*, '\\1')) FILTER (WHERE w2.id = a.affixed_word_id) as sample_affixed
                FROM affixations a
            ...
            SELECT 
                ...
                root_words_count,
                affixed_words_count,
                (SELECT json_agg(sample) FROM (
                    SELECT DISTINCT r.jsonb_array_elements_text as sample
                    FROM affix_stats s2
                    CROSS JOIN LATERAL jsonb_array_elements_text(s2.sample_roots) AS r
                    WHERE s.affix_type = s2.affix_type
                    LIMIT 5
                ) t) as sample_roots,
                (SELECT json_agg(sample) FROM (
                    SELECT DISTINCT a.jsonb_array_elements_text as sample
                    FROM affix_stats s2
                    CROSS JOIN LATERAL jsonb_array_elements_text(s2.sample_affixed) AS a
                    WHERE s.affix_type = s2.affix_type
                    LIMIT 5
                ) t) as sample_affixed
            FROM affix_stats s
...
def get_relations():
    ...
    try:
        ...
        # Build query for relation statistics
        sql = text("""
            WITH relation_stats AS (
                SELECT 
                    ...
                    json_agg(DISTINCT regexp_replace(w.lemma, '^.*, '\\1')) FILTER (WHERE w.id = r.from_word_id) as sample_from,
                    json_agg(DISTINCT regexp_replace(w2.lemma, '^.*, '\\1')) FILTER (WHERE w2.id = r.to_word_id) as sample_to
                FROM relations r
            ...
            SELECT 
                ...
                to_words_count,
                (SELECT json_agg(sample) FROM (
                    SELECT DISTINCT f.jsonb_array_elements_text as sample
                    FROM relation_stats s2
                    CROSS JOIN LATERAL jsonb_array_elements_text(s2.sample_from) AS f
                    WHERE s.relation_type = s2.relation_type
                    LIMIT 5
                ) t) as sample_from,
                (SELECT json_agg(sample) FROM (
                    SELECT DISTINCT t.jsonb_array_elements_text as sample
                    FROM relation_stats s2
                    CROSS JOIN LATERAL jsonb_array_elements_text(s2.sample_to) AS t
                    WHERE s.relation_type = s2.relation_type
                    LIMIT 5
                ) t) as sample_to
            FROM relation_stats s
        """)
...
def get_relation_description(relation_type: str) -> str:
    ...
    descriptions = {
        ...
        "related": "Words with a semantic connection",
        "cognate": "Words sharing the same linguistic origin",
        "root_of": "Original or primitive form of a word"
    }
    return descriptions.get(relation_type, "")
