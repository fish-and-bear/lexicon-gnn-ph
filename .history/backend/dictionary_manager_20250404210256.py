...
def process_kaikki_jsonl(cur, filename: str):
    ...
    def process_single_entry(entry):
        ...
        try:
            ...
            romanized_form = None
            if baybayin_form and not baybayin_romanized and romanizer and romanizer.validate_text(baybayin_form):
                try:
                    ...
                    try:
                        ...
                        if not is_canonical and form_text != word and form_text not in canonical_forms:
                            ...
                            if tags and isinstance(tags, list):
                                ...
                                if any(t in ["standard spelling", "preferred", "standard form", "canonical"] for t in tags):
                                    rel_type, metadata["confidence"] = RelationshipType.SPELLING_VARIANT, 95
                                elif any(t in ["alternative spelling", "alternate spelling", "alt form"] for t in tags):
                                    rel_type, metadata["confidence"] = RelationshipType.SPELLING_VARIANT, 90
                                elif any(t in ["regional", "dialectal", "dialect"] for t in tags):
            ...
            return word_id if word_id else None, f"Unhandled exception: {str(e)}"
        ...
        with open(filename, "r", encoding="utf-8") as f:
            ...
            for line in f:
                ...
                try:
                    ...
                    logger.error(f"Unhandled error at line {entry_count}: {str(e)}")
                    stats["failed_entries"] += 1
