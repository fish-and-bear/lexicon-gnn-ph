...
def migrate_data(args):
    ...
    if hasattr(args, 'file') and args.file:
        ...
        if "kwf" in filename.lower() or "diksiyonaryo" in filename.lower():
            handler = process_kwf_dictionary
        elif "marayum" in filename.lower() and os.path.isdir(filename):
            handler = process_marayum_directory
        elif "root_words" in filename.lower():
            handler = process_root_words_cleaned
        elif "tagalog-words" in filename.lower():
            handler = process_tagalog_words
        elif file_ext == ".jsonl":
            if "ceb" in filename.lower():
                handler = process_kaikki_jsonl
            else:
                handler = process_kaikki_jsonl
        
        if not handler:
            logger.error(f"Cannot determine appropriate handler for file: {filename}")
            return
    ...
    # Confirm with user if not using --force flag
    if not hasattr(args, 'force') or not args.force:
        required_files = [s["file"] for s in sources if s["required"]]
        if required_files:
            console = Console()
            console.print("\n[bold yellow]Warning:[/] This will process the following required sources:")
            for file in required_files:
                console.print(f"  - [cyan]{file}[/]")
            console.print("Any existing data for these sources will be updated.")
            confirmation = input("\nType 'YES' to continue: ")
            if confirmation != "YES":
                console.print("[yellow]Migration cancelled by user.[/]")
                return
    ...
    try:
        ...
        console.print("\n[bold blue]Database Setup[/]")
        with console.status("[bold green]Setting up database schema...[/]") as status:
            create_or_update_tables(conn)
            console.print("[green]✓[/] Schema setup complete")
        ...
        # Process each source
        console.print("\n[bold blue]Processing Data Sources[/]")
        ...
        for source_idx, source in enumerate(sources):
            ...
            if not filename:
                file_type = 'directory' if source.get('is_directory', False) else 'file'
                msg = f"[yellow]⚠[/] {source['name']}: {file_type} not found ({source['file']})"
                console.print(msg)
                if source["required"]:
                    logger.error(f"Required {file_type} not found: {source['file']}")
                    failed_sources += 1
                    ...
                    skipped_sources += 1
                    logger.warning(f"Optional {file_type} not found: {source['file']}")
                    continue
            
            progress_message = f"[{source_idx+1}/{total_sources}] Processing {source['name']}..."
            try:
                with console.status(f"[bold green]{progress_message}[/]") as status:
                    source["handler"](cur, filename)
                    processed_sources += 1
                    console.print(f"[green]✓[/] {source['name']}: Processed successfully")
            except Exception as e:
                error_msg = str(e)
                console.print(f"[red]✗[/] {source['name']}: Failed - {error_msg}")
                logger.error(f"Error processing {source['name']}: {error_msg}")
                failed_sources += 1
                if source["required"] and not (hasattr(args, 'force') and args.force):
                    raise
        
        # Summary
        conn.commit()
        console.print("\n[bold blue]Migration Summary[/]")
        console.print(f"Total sources: {total_sources}")
        console.print(f"[green]Successfully processed: {processed_sources}[/]")
        if skipped_sources > 0:
            console.print(f"[yellow]Skipped: {skipped_sources}[/]")
        if failed_sources > 0:
            console.print(f"[red]Failed: {failed_sources}[/]")
        
        console.print("\n[bold green]Data migration completed successfully![/]")
        ...
        console.print(f"\n[bold red]Migration failed:[/] {error_msg}")
        raise
    finally:
        if cur:
            cur.close()
