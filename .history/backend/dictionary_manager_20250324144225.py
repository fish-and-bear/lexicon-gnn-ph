...
def batch_get_or_create_word_ids(cur, entries: List[Tuple[str, str]], batch_size: int = 1000) -> Dict[Tuple[str, str], int]:
    ...
    for i in range(0, len(entries), batch_size):
        ...
        to_insert = [(lemma, norm, lang) for (lemma, _, lang), (_, norm, _) in zip(batch, normalized_entries)
        ...
        if to_insert:
            ...
            try:
                ...
                # Commit the transaction to ensure words are saved
                cur.execute("COMMIT")
            except Exception as e:
                cur.execute("ROLLBACK")
                logger.error(f"Error in batch word insertion: {str(e)}")
...
def process_tagalog_words(cur, filename: str):
    ...
    try:
        ...
        # Batch insert words
        try:
            word_ids = batch_get_or_create_word_ids(cur, word_entries)
            # Explicitly commit to ensure word IDs are persisted
            cur.connection.commit() 
            logger.info(f"Successfully inserted/updated {len(word_ids)} words")
        except Exception as e:
            cur.connection.rollback()
            logger.error(f"Failed to batch insert words: {str(e)}")
            raise

        # Verify all entries have word IDs
        missing_entries = []
        for word, lang in word_entries:
            if (word, lang) not in word_ids:
                missing_entries.append(word)
        
        if missing_entries:
            logger.warning(f"Missing word IDs for {len(missing_entries)} entries. First 10: {missing_entries[:10]}")
                ...
                try:
                    ...
                    for def_text in definitions:
                        ...
                        try:
                            ...
                            # Continue with other definitions rather than aborting the whole word
