...
def _process_single_tagalog_word_entry(cur, entry_data: dict, word_key: str, source_identifier: str, language_code: str, add_etymology: bool, word_id_cache: dict, stats: dict, error_types: dict, table_column_map: dict, relations_batch: list, required_columns: dict) -> bool: # Added cur and required_columns
    ...
    """
    item_word = entry_data.get("word", word_key).strip()
    if not item_word:
        item_word = word_key.strip()

    if not item_word:
        logger.warning(f"Skipping entry with key '{word_key}': empty word")
        stats["skipped"] += 1
    ...
    try:
        ...
        # Ensure the get_or_create_word_id being called here IS the refactored one
        # that does NOT strip numbers from the lemma argument.
        word_id = get_or_create_word_id(
            cur,
        ...
        pronunciation = entry_data.get("pronunciation", "").strip()
        ...
        if pronunciation and "pronunciations" in required_columns and set(required_columns["pronunciations"]).issubset(table_column_map.get("pronunciations", set())):
            try:
                # Use insert_pronunciation which handles ON CONFLICT
        ...
        if add_etymology and "etymology" in entry_data and entry_data["etymology"] and "etymologies" in required_columns and set(required_columns["etymologies"]).issubset(table_column_map.get("etymologies", set())):
            etymology = entry_data["etymology"]
            if isinstance(etymology, str) and etymology.strip():
                try:
                    # Use insert_etymology which handles ON CONFLICT
                    # Assuming insert_etymology exists and takes (cur, word_id, text, source)
                    etym_id = insert_etymology(cur, word_id, etymology.strip(), source_identifier)
                    if etym_id:
        ...
        elif add_etymology and "etymology" in entry_data and entry_data["etymology"]:
        ...
        # Check if required definition columns exist
        if isinstance(definitions, dict) and "definitions" in required_columns and set(required_columns["definitions"]).issubset(table_column_map.get("definitions", set())):
            for pos, def_list in definitions.items():
                ...
                for def_item in def_list:
                    ...
                    try:
                        ...
                        # Assuming insert_definition exists and takes relevant args
                        # Make sure its signature includes 'examples' parameter for the TEXT column
                        def_id = insert_definition(
                            cur,
                            word_id,
                            definition_text,
                            source_identifier=source_identifier,
                            part_of_speech=pos, # insert_definition needs to handle POS mapping
                            examples=examples_text # Pass formatted example string
                            ...
                            if synonyms and isinstance(synonyms, list):
                                for syn in synonyms:
                                    if isinstance(syn, str) and syn.strip():
                                        relations_batch.append({
                                            'word_id': word_id,
                                            'related_word': syn.strip(),
                                            'rel_type': RelationshipType.SYNONYM, # Use enum
                                            'def_id': def_id, # Link to specific definition
                                            'source_identifier': source_identifier
                                        })
                            if antonyms and isinstance(antonyms, list):
                                for ant in antonyms:
                                    if isinstance(ant, str) and ant.strip():
                                        relations_batch.append({
                                            'word_id': word_id,
                                            'related_word': ant.strip(),
                                            'rel_type': RelationshipType.ANTONYM, # Use enum
                                            'def_id': def_id, # Link to specific definition
                                            'source_identifier': source_identifier
                                        })
                        else:
                             # Log if insert_definition indicates failure (e.g., returns None)
                             logger.warning(f"Non-critical: Failed to insert definition for '{item_word}', POS '{pos}'. Definition text: {definition_text[:50]}...")
                             error_types[f"DefinitionInsertFailure"] = error_types.get(f"DefinitionInsertFailure", 0) + 1

                    except Exception as e:
                        logger.warning(f"Non-critical: Error adding definition for word '{item_word}', POS '{pos}': {e}", exc_info=True) # Added exc_info
                        error_types[f"DefinitionError: {type(e).__name__}"] = error_types.get(f"DefinitionError: {type(e).__name__}", 0) + 1
        elif isinstance(definitions, dict) and definitions:
             logger.debug(f"Skipping definitions for '{item_word}' due to missing required columns in 'definitions' table.")

        # REMOVED domain processing section - handled via word_metadata now

        # Process capitalized word variant (add to batch)
        if item_word and len(item_word) > 1 and not item_word[0].isupper() and item_word[0].isalpha():
            try:
        ...
        # Catch critical errors (like word creation failure) or unexpected errors
...
# with this corrected version:
def process_tagalog_words(cur, filename: str, add_etymology: bool = True): # `cur` argument is unused due to helper decorator, but kept for consistent interface if called directly elsewhere
    """
    Process a JSON file containing Tagalog words and their definitions using entry-level transactions.

    Args:
        cur: Database cursor (passed but generally unused due to helper's decorator)
    ...
    stats = {
        ...
        "etymologies_added": 0,
        "relations_added": 0,
        "errors": 0, # Counts entries that failed critically
    }
    error_types = {} # Track specific error types encountered

    # Define source identifier based on filename
    source_identifier = standardize_source_identifier(os.path.basename(filename))
    if not source_identifier:
        source_identifier = "tagalog_json" # Fallback
    logger.info(f"Using standardized source identifier: '{source_identifier}'")

    # Define required columns based on the ACTUAL schema and usage
    # Ensure these names match your TABLE_CREATION_SQL
    required_columns = {
        "words": {"id", "lemma", "normalized_lemma", "language_code", "source_info", "word_metadata"}, # Used by get_or_create_word_id
        "pronunciations": {"id", "word_id", "value", "sources"}, # Used by insert_pronunciation
        "etymologies": {"id", "word_id", "etymology_text", "sources"}, # Used by insert_etymology
        "definitions": {"id", "word_id", "definition_text", "standardized_pos_id", "sources", "examples"}, # Used by insert_definition
        "relations": {"id", "from_word_id", "to_word_id", "relation_type", "sources", "metadata"}, # Used by insert_relation
        "parts_of_speech": {"id", "code"}, # Needed by get_standardized_pos_id (used internally by insert_definition)
        # 'word_forms' might be needed if processing those
        # 'examples' table does not exist per schema
        # 'word_tags' table does not exist per schema
    }
    table_column_map = {}
    try:
        # Use a temporary connection/cursor for schema check
        temp_conn = get_connection() # Ensure this calls the corrected get_connection
        with temp_conn.cursor() as temp_cur:
            all_tables = list(required_columns.keys())
            # Fetch columns for all required tables at once if possible
            temp_cur.execute("""
            ...
            for table_name, column_name in fetched_columns:
                if table_name in table_column_map:
                    table_column_map[table_name].add(column_name)

            # Perform checks
            for table, req_cols_set in required_columns.items():
                 actual_cols = table_column_map.get(table, set())
                 missing_columns = req_cols_set - actual_cols
                 if missing_columns:
                     logger.warning(f"Schema Check: Table '{table}' is missing required columns used by this processor: {', '.join(missing_columns)}. Some features might not work.")
        temp_conn.close() # Close the temporary connection
    except Exception as schema_err:
        logger.error(f"Failed to perform initial schema check: {schema_err}. Proceeding with caution.")
        # Initialize table_column_map with dummy values to avoid crashing later checks
        for table in required_columns:
            table_column_map[table] = set() # Assume no columns exist if check fails

    # Cache word IDs to avoid repeated lookups across entries and relation processing
    # Key: normalized_lemma|language_code
    word_id_cache = {}
    relations_batch = [] # Collect relations to process later

    try:
        # Use iterative JSON parsing for large files to save memory
        import ijson # Consider adding ijson to requirements.txt
        logger.info(f"Loading {filename} using iterative parser...")
        entries_iterator = None
        try:
            with open(filename, "r", encoding="utf-8") as f:
                # Assuming top level is a dictionary; 'item' iterates over key-value pairs
                entries_iterator = ijson.kvitems(f, '')
                total_entries = -1 # Cannot easily get total count upfront with ijson
                logger.info(f"Iterating through entries in {filename} (total count unknown beforehand)")
        except FileNotFoundError:
             logger.error(f"File not found: {filename}")
             return stats
        ...
        # Process each entry
        language_code = "tl" # Assume Tagalog for this file
        # Cannot use tqdm easily without total_entries, log progress periodically
        ...
        for entry_index, (word_key, entry_data) in enumerate(entries_iterator):
            ...
            # Call the helper function which handles its own transaction
            try:
                # The decorator provides the cursor `cur`
                success = _process_single_tagalog_word_entry(
                    # cur is injected by decorator
                    entry_data,
                    word_key,
                    source_identifier,
                    language_code,
                    ...
                    table_column_map,
                    relations_batch,
                    required_columns # Pass corrected required_columns
                )
                if success:
                    stats["entries_processed"] += 1 # Count successful attempts
                # Error counting is handled inside the helper for critical errors

            except Exception as helper_call_err:
                 # Catch unexpected errors calling the helper itself
                 logger.error(f"FATAL: Unexpected error calling processing function for entry {entry_index} ('{word_key}'): {helper_call_err}", exc_info=True)
                 stats["errors"] += 1 # Count as critical failure
            ...
            if (entry_index + 1) % processed_count_log_interval == 0:
                 logger.info(f"Processed {entry_index + 1} entries...")


        # Process the collected relations batch AFTER the main loop
        logger.info(f"Processing {len(relations_batch)} collected relations...")
        if relations_batch:
            try:
                # --- Use adapted process_relations_batch with @with_db_transaction ---
                @with_db_transaction # Correct decorator
                def _process_relations_batch_transactional(cur, batch, stats_dict, cache): # Added cur
                    processed_count = 0
                    failed_count = 0
                    # Use normalize_lemma for cache lookups
                    lang_code_for_related = "tl" # Assume related words are also Tagalog

                    for relation_info in batch:
                         try:
                              from_id = relation_info['word_id']
                              related_word_lemma = relation_info['related_word']
                              rel_type = relation_info['rel_type'] # Enum expected
                              src_id = relation_info['source_identifier']
                              metadata = relation_info.get('metadata') # Should be dict or None
                              def_id = relation_info.get('def_id') # Optional definition link

                              if not related_word_lemma: continue # Skip empty related words

                              # Normalize related word for cache lookup/creation
                              ...
                              if not related_word_id:
                                   # Create the related word if it doesn't exist
                                   # Use minimal info, source is the current relation's source
                                   ...
                                   if related_word_id:
                                        ...
                                        # If creation failed, we cannot add the relation
                                        logger.warning(f"Failed to get/create related word '{related_word_lemma}' for relation. Skipping.")
                                        failed_count += 1
                                        continue # Skip this relation

                              if related_word_id and related_word_id != from_id:
                                   # Insert relation
                                   # Assuming insert_relation handles metadata correctly
                                   rel_id = insert_relation(cur, from_id, related_word_id, rel_type, src_id, metadata)
                                   if rel_id:
                                        processed_count += 1
                                        stats_dict["relations_added"] += 1 # Update stats

                                        # Insert definition relation if applicable
                                        if def_id:
                                            try:
                                                # Assuming insert_definition_relation exists
                                                insert_definition_relation(cur, def_id, related_word_id, rel_type, src_id)
                                            except Exception as def_rel_err:
                                                 logger.warning(f"Failed inserting definition relation (def_id={def_id}, word_id={related_word_id}, type={rel_type}): {def_rel_err}")


                                        # Add inverse/bidirectional if needed
                                       ...
                                       # insert_relation might return None on conflict or error
                                       logger.debug(f"Relation insert skipped/failed for {from_id} -> {related_word_id} ({rel_type})")
                                       # Not necessarily an error if ON CONFLICT DO NOTHING
                                       # failed_count += 1 # Optionally count conflicts as failures

                         except Exception as rel_proc_err:
                              logger.warning(f"Failed processing relation batch item: {relation_info} - Error: {rel_proc_err}", exc_info=True)
                              failed_count += 1
                              # Decide if this is critical or not for the batch

                    logger.info(f"Processed {processed_count} relations in batch. {failed_count} failures/skips.")
                    return stats_dict # Return updated stats

                # Call the transactional batch processor
                # The decorator provides the cursor `cur`
                stats = _process_relations_batch_transactional(relations_batch, stats, word_id_cache)

            except Exception as final_rel_err:
                logger.error(f"CRITICAL error processing final relations batch: {final_rel_err}", exc_info=True)
                # Decide how to handle this - potentially mark relations as failed
                stats["errors"] += len(relations_batch) # Count all relations in batch as errors? Or just log?
                error_types["RelationsBatchError"] = error_types.get("RelationsBatchError", 0) + 1

    except Exception as e:
        # Catch errors during file loading or main loop setup
        logger.error(f"Critical error processing file {filename}: {e}", exc_info=True)
        stats["errors"] += 1 # Count as one major error
        error_types[f"CriticalFileError: {type(e).__name__}"] = error_types.get(f"CriticalFileError: {type(e).__name__}", 0) + 1
        # No need to rollback here, as transactions are per-entry or per-batch

    # Log final statistics
    logger.info(f"Finished processing {filename}")
    logger.info(f"  Entries processed successfully: {stats['entries_processed']}")
    logger.info(f"  Entries skipped (invalid format/empty word): {stats['skipped']}")
