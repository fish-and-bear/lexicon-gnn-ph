...
def process_kwf_dictionary(cur, filename: str):
    ...
    for word, entry in tqdm(data.items(), desc="Processing KWF Dictionary"):
        try:
                ...
                for pos, def_list in definitions.items():
                    ...
                    for def_entry in def_list:
                        try:
                            ...
                            # Collect example sets - FIX: Process examples properly
                            examples_text = ""
                                ...
                                if 'examples' in example_set and isinstance(example_set['examples'], list):
                                    ...
                                    for ex in example_set['examples']:
                                        if isinstance(ex, str):
                                            example_strings.append(ex)
                                        elif isinstance(ex, dict) and 'text' in ex:
                                            example_strings.append(ex['text'])
                                        elif isinstance(ex, dict) and 'value' in ex:
                                            example_strings.append(ex['value'])
                            ...
                            try:
                                ...
                                if examples_text:
                                    stats["examples_added"] += len(examples_text.split("\n"))
                                ...
                                for synonym in synonyms:
                                    ...
                                    # Handle both string synonyms and dictionary synonyms with 'term' key
                                    synonym_term = None
                                    if isinstance(synonym, str):
                                        synonym_term = synonym.strip()
                                    elif isinstance(synonym, dict) and 'term' in synonym:
                                        if isinstance(synonym['term'], str):
                                            synonym_term = synonym['term'].strip()
                                        else:
                                            logger.warning(f"Skipping non-string term in synonym dictionary for '{formatted_word}': {synonym}")
                                            continue
                                    else:
                                        logger.warning(f"Skipping invalid synonym format for '{formatted_word}': {synonym}")
                                        continue
                                    
                                    # Skip if we couldn't extract a valid term
                                    if not synonym_term:
                                        continue
                                        
                                    # Extract the actual term if it's wrapped in HTML tags
                                    synonym_term = re.sub(r'<[^>]+>', '', synonym_term).strip()
                                    if not synonym_term:
                                        continue
                                ...
                                for antonym in antonyms:
                                    ...
                                    # Handle both string antonyms and dictionary antonyms with 'term' key
                                    antonym_term = None
                                    if isinstance(antonym, str):
                                        antonym_term = antonym.strip()
                                    elif isinstance(antonym, dict) and 'term' in antonym:
                                        if isinstance(antonym['term'], str):
                                            antonym_term = antonym['term'].strip()
                                        else:
                                            logger.warning(f"Skipping non-string term in antonym dictionary for '{formatted_word}': {antonym}")
                                            continue
                                    else:
                                        logger.warning(f"Skipping invalid antonym format for '{formatted_word}': {antonym}")
                                        continue
                                    ...
                                    # Extract the actual term if it's wrapped in HTML tags
                                    antonym_term = re.sub(r'<[^>]+>', '', antonym_term).strip()
                                    if not antonym_term:
                                        continue
                                    
                                    logger.debug(f"Processing antonym '{antonym_term}' for '{formatted_word}'")
                                    try:
                                        ...
                                        logger.error(f"Error processing antonym '{antonym_term}' for '{formatted_word}': {str(e)}")
                                        error_types[str(e)] = error_types.get(str(e), 0) + 1
                                ...
                                for cross_ref in cross_refs:
                                    ...
                                    # Handle both string cross_refs and dictionary cross_refs with 'term' key
                                    ref_term = None
                                    if isinstance(cross_ref, str):
                                        ref_term = cross_ref.strip()
                                    elif isinstance(cross_ref, dict) and 'term' in cross_ref:
                                        if isinstance(cross_ref['term'], str):
                                            ref_term = cross_ref['term'].strip()
                                        else:
                                            logger.warning(f"Skipping non-string term in cross-reference dictionary for '{formatted_word}': {cross_ref}")
                                            continue
                                    else:
                                        logger.warning(f"Skipping invalid cross-reference format for '{formatted_word}': {cross_ref}")
                                        continue
                                    ...
                                    # Extract the actual term if it's wrapped in HTML tags
                                    ref_term = re.sub(r'<[^>]+>', '', ref_term).strip()
                                    if not ref_term:
                                        continue
                                    ...
                                    try:
                                        ...
                                        metadata = {
                                            ...
                                            'link': cross_ref.get('link', '') if isinstance(cross_ref, dict) else '',
                                            'broken': cross_ref.get('broken', False) if isinstance(cross_ref, dict) else False
                                        }
                                        insert_relation(
                                            cur, 
                                            word_id, 
                            ...
                            see_refs = def_entry.get('see')
                            if see_refs:
                                logger.debug(f"Processing 'see' references for '{formatted_word}'")
                                
                                # Handle both string see_refs and list of see_refs
                                if isinstance(see_refs, str):
                                    see_refs = [see_refs]
                                elif isinstance(see_refs, dict):
                                    see_refs = [see_refs]
                                
                                if not isinstance(see_refs, list):
                                    logger.warning(f"Skipping invalid 'see' references for '{formatted_word}': not a list or string")
                                    continue
                                ...
                                for see_ref in see_refs:
                                    ...
                                    # Handle both string see_refs and dictionary see_refs with 'term' key
                                    see_term = None
                                    if isinstance(see_ref, str):
                                        see_term = see_ref.strip()
                                    elif isinstance(see_ref, dict) and 'term' in see_ref:
                                        if isinstance(see_ref['term'], str):
                                            see_term = see_ref['term'].strip()
                                        else:
                                            logger.warning(f"Skipping non-string term in see-reference dictionary for '{formatted_word}': {see_ref}")
                                            continue
                                    else:
                                        logger.warning(f"Skipping invalid see-reference format for '{formatted_word}': {see_ref}")
                                        continue
                                    ...
                                    # Extract the actual term if it's wrapped in HTML tags
                                    see_term = re.sub(r'<[^>]+>', '', see_term).strip()
                                    if not see_term:
                                        continue
                                    
                                    logger.debug(f"Processing 'see' reference '{see_term}' for '{formatted_word}'")    
                                    try:
                                        # Create the related word
                                        related_id = get_or_create_word_id(cur, see_term, language_code='tl')
                                        
                                        # Add metadata for the relationship
                                        metadata = {
                                            'see_context': def_entry.get('see_context'),
                                            'relationship_type': 'see_reference',
                                            'link': see_ref.get('link', '') if isinstance(see_ref, dict) else '',
                                            'broken': see_ref.get('broken', False) if isinstance(see_ref, dict) else False
                                        }
                                        ...
                                        logger.error(f"Error processing 'see' reference '{see_term}' for '{formatted_word}': {str(e)}")
                                        error_types[str(e)] = error_types.get(str(e), 0) + 1
            ...
            # Process affixation information - FIX: Handle different affixation formats properly
            if 'affixation' in entry and isinstance(entry['affixation'], list):
                ...
                for affix_entry in entry['affixation']:
                    try:
                        # Handle both string and dictionary affixation entries
                        if isinstance(affix_entry, str):
                            # Simple string format - just a term
                            affix_term = affix_entry.strip()
                            affix_type = 'unknown'
                        elif isinstance(affix_entry, dict):
                            # Check for 'term' field first (legacy format)
                            if 'term' in affix_entry:
                                if isinstance(affix_entry['term'], str):
                                    affix_term = affix_entry['term'].strip()
                                    affix_type = affix_entry.get('type', 'unknown')
                                else:
                                    logger.warning(f"Skipping non-string term in affixation dictionary for '{formatted_word}': {affix_entry}")
                                    continue
                            # Handle newer format with 'form' and 'types' or 'type' fields
                            elif 'form' in affix_entry:
                                # Process each form with its corresponding type
                                if isinstance(affix_entry['form'], list):
                                    forms = affix_entry['form']
                                    
                                    # Get types - could be a list or a single value
                                    if 'types' in affix_entry and isinstance(affix_entry['types'], list):
                                        types = affix_entry['types']
                                    elif 'type' in affix_entry:
                                        # Single type for all forms
                                        if isinstance(affix_entry['type'], str):
                                            types = [affix_entry['type']] * len(forms)
                                        else:
                                            logger.warning(f"Skipping invalid affixation type for '{formatted_word}': {affix_entry['type']}")
                                            continue
                                    else:
                                        # Default to 'unknown' for all forms if no type specified
                                        types = ['unknown'] * len(forms)
                                    
                                    # Process each form/type pair
                                    for i, form in enumerate(forms):
                                        if not isinstance(form, str) or not form.strip():
                                            continue
                                            
                                        # Get corresponding type or use default
                                        form_type = types[i] if i < len(types) else 'unknown'
                                        
                                        # Create the affixed word
                                        try:
                                            affixed_id = get_or_create_word_id(cur, form.strip(), language_code='tl')
                                            
                                            # Determine relation direction based on type
                                            if form_type.lower() in ('root', 'salita-ugat', 'ugat'):
                                                # DERIVED_FROM: Current word is derived from the affixed term
                                                insert_relation(
                                                    cur,
                                                    word_id,  # from (current word)
                                                    affixed_id,  # to (root word)
                                                    RelationshipType.DERIVED_FROM,
                                                    sources=standardized_source
                                                )
                                            else:
                                                # ROOT_OF: Current word is the root of the affixed term
                                                insert_relation(
                                                    cur,
                                                    word_id,  # from (current word)
                                                    affixed_id,  # to (derived word)
                                            ...
                                            logger.error(f"Error processing affixation form '{form}' for '{formatted_word}': {str(e)}")
                                            error_types[str(e)] = error_types.get(str(e), 0) + 1
                                    
                                    # Skip further processing since we've handled all forms
                                    continue
                                else:
                                    # Single form
                                    affix_term = affix_entry['form'].strip() if isinstance(affix_entry['form'], str) else None
                                    if 'types' in affix_entry and isinstance(affix_entry['types'], list) and affix_entry['types']:
                                        affix_type = affix_entry['types'][0]  # Use first type
                                    elif 'type' in affix_entry:
                                        affix_type = affix_entry['type']
                                    else:
                                        affix_type = 'unknown'
                            else:
                                logger.warning(f"Skipping invalid affixation format for '{formatted_word}': {affix_entry}")
                                continue
                        else:
                            logger.warning(f"Skipping invalid affixation format for '{formatted_word}': {affix_entry}")
                            continue
                        ...
                        # Extract the actual term if it's wrapped in HTML tags
                        affix_term = re.sub(r'<[^>]+>', '', affix_term).strip()
                        if not affix_term:
                            continue
                        ...
                        relation_type = RelationshipType.DERIVED_FROM if affix_type.lower() in ('root', 'salita-ugat', 'ugat') else RelationshipType.ROOT_OF
                        
                        # For DERIVED_FROM, the formatted_word is derived from affix_term
                        # For ROOT_OF, the affix_term is derived from formatted_word
                        from_id = word_id if relation_type == RelationshipType.ROOT_OF else affix_id
                        to_id = affix_id if relation_type == RelationshipType.ROOT_OF else word_id
                        ...
                        logger.error(f"Error processing affixation for '{formatted_word}': {str(e)}")
                        error_types[str(e)] = error_types.get(str(e), 0) + 1
            ...
            if 'related' in entry and isinstance(entry['related'], dict):
                ...
                for rel_type, rel_terms in entry['related'].items():
                    ...
                    std_rel_type = RelationshipType.RELATED  # Default
                    if rel_type.lower() in ('synonym', 'kasingkahulugan'):
                        std_rel_type = RelationshipType.SYNONYM
                    elif rel_type.lower() in ('antonym', 'kasalungat'):
                        std_rel_type = RelationshipType.ANTONYM
                    
                    for rel_term_entry in rel_terms:
                        if rel_term_entry is None:
                            continue
                            
                        # Handle both string rel_terms and dictionary rel_terms with 'term' key
                        rel_term = None
                        if isinstance(rel_term_entry, str):
                            rel_term = rel_term_entry.strip()
                        elif isinstance(rel_term_entry, dict) and 'term' in rel_term_entry:
                            if isinstance(rel_term_entry['term'], str):
                                rel_term = rel_term_entry['term'].strip()
                            else:
                                logger.warning(f"Skipping non-string term in related term dictionary for '{formatted_word}': {rel_term_entry}")
                                continue
                        else:
                            logger.warning(f"Skipping invalid related term format for '{formatted_word}': {rel_term_entry}")
                            continue
                        ...
                        # Extract the actual term if it's wrapped in HTML tags
                        rel_term = re.sub(r'<[^>]+>', '', rel_term).strip()
                        if not rel_term:
                            continue
                            
                        logger.debug(f"Processing related term '{rel_term}' (type: {std_rel_type}) for '{formatted_word}'")
                        try:
                            rel_id = get_or_create_word_id(cur, rel_term, language_code='tl')
                            
                            # Add metadata for the relationship
                            metadata = {
                                'original_type': rel_type,
                                'link': rel_term_entry.get('link', '') if isinstance(rel_term_entry, dict) else '',
                                'broken': rel_term_entry.get('broken', False) if isinstance(rel_term_entry, dict) else False
                            }
                            ...
                            logger.error(f"Error processing related term '{rel_term}' for '{formatted_word}': {str(e)}")
                            error_types[str(e)] = error_types.get(str(e), 0) + 1
