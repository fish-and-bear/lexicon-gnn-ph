...
def insert_definition(
    ...
    Returns:
        The ID of the inserted/updated definition record, or None if failed and error handled internally.

    Raises:
        psycopg2.Error: If a database error occurs that isn't handled internally.
    ...
    # Ensure definition_text is a string before stripping
    if not isinstance(definition_text, str):
         logger.warning(
             f"Invalid definition_text type ({type(definition_text)}) for word ID {word_id}. Skipping."
         )
         return None
    ...
    try:
        ...
        # Ensure 'examples' is None or a Json object/string suitable for DB
        # If it's passed as Json object, it's fine. If string, DB will handle. If other, set None.
        ...
        cur.execute(
            ...
            DO UPDATE SET
                ...
                -- Update sources: If new source is different and not already present? Simple overwrite for now.
                sources = EXCLUDED.sources,
            ...
            """,
            {
                ...
                "examples": examples, # Pass string or Json object or None
        ...
        logger.error(
            ...
            # Avoid logging full traceback for common constraint violations, maybe?
            # exc_info=True,
        )
        # Re-raise the exception so the calling function's transaction/savepoint logic can handle it
        raise e
    except Exception as e:
        logger.error(
    ...
    # This return is unlikely to be hit if errors are raised, but included for completeness
    # return None
@with_transaction(commit=True)
def insert_relation(
    cur,
