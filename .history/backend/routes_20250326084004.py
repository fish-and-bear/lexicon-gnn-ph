...
def search_words(**params):
    ...
    try:
        ...
        if mode == 'exact':
            ...
            # Default full text and pattern search
            base_query = base_query.filter(
                or_(
        ...
        if sort == 'alphabetical':
            ...
            if order == 'asc':
                # Less relevant first - exact matches last
                base_query = base_query.order_by(
                    Word.normalized_lemma.notlike(f'{normalized_query}%').asc(),
                    Word.lemma.asc()
                )
            else:
                # Most relevant first - exact matches first
                base_query = base_query.order_by(
