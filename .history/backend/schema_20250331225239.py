...
class Query(graphene.ObjectType):
    ...
    def resolve_get_word(self, info, word, language=None):
        """Resolve get_word query."""
        if not word:
            raise ValueError("Invalid word")
        query = WordModel.query.filter(WordModel.normalized_lemma == normalize_lemma(word))
        if language:
            query = query.filter(WordModel.language_code == language)
        return query.first()
    
    def resolve_baybayin_words(self, info, text):
        """Resolve baybayin_words query."""
        if not text:
            return []
    ...
    def resolve_get_word_network(self, info, word):
        """Resolve get_word_network query."""
        if not word:
            return None
