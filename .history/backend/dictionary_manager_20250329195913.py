...
def process_kaikki_jsonl(cur, filename: str):
        ...
        if 'alt_of' in entry:
            for alt_of in entry.get('alt_of', []):
                try:
                    ...
                    elif isinstance(alt_of, dict) and 'word' in alt_of:
                        alt_word = alt_of['word']
                        alt_meta = {}
                    else:
                        continue
                    ...
                    logger.error(f"Error processing alt_of relationship for word ID {word_id}: {str(e)}")
    ...
    def process_categories(cur, definition_id: int, categories: List[Dict]):
        ...
        for category in categories:
            ...
            try:
                ...
                logger.error(f"Error processing category for definition ID {definition_id}: {str(e)}")
    ...
    def process_head_templates(cur, word_id: int, templates: List[Dict]):
        ...
        for template in templates:
            try:
                ...
                logger.error(f"Error processing head template for word ID {word_id}: {str(e)}")
    ...
    def process_links(cur, definition_id: int, links: List[List[str]]):
        ...
        for link in links:
            try:
                ...
                logger.error(f"Error processing link for definition ID {definition_id}: {str(e)}")
                ...
                for template in etymology_templates:
                    ...
                    if template_name in ('bor', 'bor+') and '2' in args and '3' in args:
                        ...
                        try:
                            ...
                            # Create borrowed_from relationship - use the mapped standard type
                            insert_relation(
                                cur,
                                word_id, 
                                source_word_id, 
                    ...
                    # Process cognates
                        ...
                        try:    
                            ...
                            # Create cognate relationship - use the mapped standard type
                            insert_relation(
                                cur,
                                word_id, 
                                cog_word_id, 
    ...
    def process_entry(cur, entry: Dict, conn=None, entry_index=0):
        ...
        savepoint_name = f"sp_entry_{abs(entry_index)}"  # Use abs to avoid negative numbers
        ...
        try:
            ...
            if 'senses' in entry:
                for sense in entry['senses']:
                    ...
                    # Insert the definition - WITHOUT the metadata parameter which is causing issues
                    try:
                        definition_id = insert_definition(
                            cur, 
                            word_id, 
    ...
    try:
        with open(filename, 'r', encoding='utf-8') as f:
                ...
                try:
                    ...
                    if len(current_batch) >= batch_size:
                        ...
                        for entry_tuple in current_batch:
                            ...
                            result = process_entry(cur, entry_data, conn, idx)
                            if result:
                        ...
                        if entries_since_commit >= commit_interval:
                            try:
                                ...
                                # Don't adjust entry counts - they weren't successfully committed
            ...
            try:
                conn.commit()
                logger.info(f"Final commit complete. Processed {stats['processed_entries']} entries in total.")
            except Exception as commit_error:
                logger.error(f"Error in final commit: {str(commit_error)}")
                conn.rollback()
                logger.warning("Rolled back final changes due to commit error.")
