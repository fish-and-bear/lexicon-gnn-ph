...
def rate_limit_endpoints():
    """Apply rate limits to endpoints after blueprint registration."""
    if limiter and current_app:
        for endpoint, func in bp.view_functions.items():
            if endpoint.endswith(('word_relationships', 'etymology_tree')):
                limiter.limit(
                    "30 per minute",
