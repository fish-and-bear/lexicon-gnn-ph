                        ...
                        for sense_idx, sense in enumerate(senses):
                            ...
                            try:
                                ...
                                definition_id = insert_definition(
                                    ...
                                    source_identifier=source_identifier, # Pass mandatory source
                                    part_of_speech=standardized_pos_id, # Fix: Rename argument to match definition
        ...
        with open(filename, 'r', encoding='utf-8') as f:
            ...
            for line in f:
                ...
                try:
                    ...
                    # --- Fallback Logic (Consider if needed) ---
                    # If standard processing failed critically (e.g., word creation) or had errors,
                    # decide whether to try the fallback.
                    use_fallback = False
                    if not word_id: # Critical failure in process_entry
                         use_fallback = True # Maybe try fallback? Or just mark as failed?
                         logger.warning(f"Entry {entry_count} ('{entry.get('word', 'N/A')}') failed standard processing critically. Reason: {entry_error}. Trying fallback.")
                    # elif entry_error: # Non-critical error occurred
                    #     logger.info(f"Entry {entry_count} ('{entry.get('word', 'N/A')}') had non-critical errors: {entry_error}. Fallback not triggered.")


                    # Optional: Trigger fallback specifically for Baybayin issues if standard failed
                    # if not word_id and 'forms' in entry:
                    #      if any('Baybayin' in form.get('tags', []) for form in entry.get('forms', []) if isinstance(form, dict)):
                    #            use_fallback = True


                    if use_fallback:
                        # --- Try Fallback ---
                        stats["fallback_entries_used"] += 1
                        fallback_word_id, fallback_error = process_entry_without_baybayin(cur, entry, source_identifier)

                        if fallback_word_id:
                            stats["processed_with_errors"] += 1 # Count as processed, but note it was fallback
                            # Record the fallback error type if any
                            if fallback_error:
                                err_key = f"FallbackError: {fallback_error}"
                                error_summary[err_key] = error_summary.get(err_key, 0) + 1
                            # Commit the successful fallback
                            cur.execute(f"RELEASE SAVEPOINT {savepoint_name}")
                        else:
                            stats["failed_entries"] += 1 # Fallback also failed
                            err_key = f"FallbackFailed: {fallback_error or 'Unknown fallback failure'}"
                            error_summary[err_key] = error_summary.get(err_key, 0) + 1
                            # Rollback the failed fallback attempt
                            cur.execute(f"ROLLBACK TO SAVEPOINT {savepoint_name}")

                    elif word_id: # Standard processing succeeded
                         if entry_error:
