...
def process_tagalog_words(cur, filename: str):
    ...
    try:
        ...
        # First, create all words with individual transactions to ensure they exist
        logger.info("Creating word entries...")
        word_ids = {}
        with tqdm(total=len(word_entries), desc="Creating words") as pbar:
            for lemma, lang_code in word_entries:
                try:
                    # Create each word in its own transaction to ensure it gets committed
                    conn = cur.connection
                    cur.execute("BEGIN")
                    ...
                    # Store the word ID
                    word_ids[(lemma, lang_code)] = word_id
                    
                except Exception as e:
                    logger.error(f"Error creating word {lemma}: {str(e)}")
                    try:
                        cur.execute("ROLLBACK")
                    except Exception:
                        pass  # Ignore rollback errors
        ...
        logger.info(f"Created/found {len(word_ids)} words")
        ...
        with tqdm(total=total_entries, desc="Processing Tagalog words") as pbar:
            ...
            for word, entry in entries:
                word_id = word_ids.get((word, language_code))
                if not word_id:
                ...
                try:
                    ...
                    # Double-check that the word exists in the database
                    cur.execute("SELECT id FROM words WHERE id = %s", (word_id,))
                    if not cur.fetchone():
                        logger.error(f"Word ID {word_id} for '{word}' does not exist in database. Skipping.")
                        cur.execute("ROLLBACK")
                        pbar.update(1)
                        continue
                    ...
                    if romanizer.is_baybayin(word):
                        ...
                        candidate = transliterate_to_baybayin(word)
                        if validate_baybayin_entry(candidate, word):
                            has_baybayin = True
                            baybayin_form = candidate
                            romanized_form = romanizer.romanize(candidate)
                        else:
                            baybayin_form = None  # Clear baybayin_form if candidate is invalid

                    # Update word with pronunciation and Baybayin
                    cur.execute("""
                        UPDATE words 
                        SET has_baybayin = %s,
                            baybayin_form = %s,
                            ...
                            updated_at = CURRENT_TIMESTAMP
                        WHERE id = %s
                    """, (has_baybayin, baybayin_form, romanized_form, json.dumps({"hyphenation": pronunciation}), word_id))
                    ...
                    for def_text in definitions:
                        ...
                        try:
                            # Insert definition with extra check for word existence
