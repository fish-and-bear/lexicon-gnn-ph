...
from source_standardization import extract_etymology_components as source_extract_etymology_components
...
def extract_etymology_components(etymology_text):
    ...
    return source_extract_etymology_components(etymology_text)
...
@with_transaction(commit=False)  # Using commit=True for this function as per the original code
def process_tagalog_words(cur, filename: str):
    ...
    Uses the @with_transaction(commit=False) decorator, meaning each entry attempt
    ...
    # Note: conn = cur.connection is not needed here because @with_transaction(commit=False)
...
@with_transaction(commit=False)  # Changed to commit=True to follow process_tagalog_words pattern
