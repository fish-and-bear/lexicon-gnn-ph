...
def get_semantic_network(word: str):
    ...
    try:
        ...
        # Find the word entry using direct SQL to avoid ORM issues
        try:
            word_id = int(word)
        ...
        def process_word(word_id, current_depth=0):
            ...
            # Get relations using direct SQL instead of ORM to avoid relation loading issues
            sql_out = """
            SELECT r.id, r.relation_type, r.to_word_id, w.lemma, w.language_code
            FROM relations r
            JOIN words w ON r.to_word_id = w.id
            WHERE r.from_word_id = :word_id
            """
            outgoing = db.session.execute(text(sql_out), {"word_id": word_id}).fetchall()
            
            # Add current word as node (use results from direct query)
            word_info = db.session.execute(text("SELECT id, lemma, language_code FROM words WHERE id = :id"), {"id": word_id}).fetchone()
            add_node(word_id, word_info.lemma, word_info.language_code)
            ...
            for relation in outgoing:
                ...
                add_node(target_id, relation.lemma, relation.language_code)
            ...
            sql_in = """
            SELECT r.id, r.relation_type, r.from_word_id, w.lemma, w.language_code
            FROM relations r
            JOIN words w ON r.from_word_id = w.id
            WHERE r.to_word_id = :word_id
            """
            incoming = db.session.execute(text(sql_in), {"word_id": word_id}).fetchall()
            ...
            for relation in incoming:
                ...
                add_node(source_id, relation.lemma, relation.language_code)
