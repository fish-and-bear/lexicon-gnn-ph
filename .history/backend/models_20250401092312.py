...
from sqlalchemy.orm import relationship, validates, backref, declarative_base
...
class Relation(db.Model):
    ...
    sources = Column(Text)
    relation_metadata = Column(JSONB, default=lambda: {})  # Renamed from metadata to relation_metadata
    created_at = Column(DateTime, server_default=func.now())
    updated_at = Column(DateTime, server_default=func.now(), onupdate=func.now())
    ...
    def to_dict(self):
        ...
        return {
            ...
            'sources': self.sources,
            'metadata': self.relation_metadata,  # Keep the output name as metadata for API consistency
            'created_at': self.created_at.isoformat() if self.created_at else None,
            'updated_at': self.updated_at.isoformat() if self.updated_at else None,
        }
...
class Pronunciation(db.Model):
    ...
    tags = Column(JSONB)
    pronunciation_metadata = Column(JSONB)  # Renamed from metadata to pronunciation_metadata
    sources = Column(Text)
    created_at = Column(DateTime, server_default=func.now())
    updated_at = Column(DateTime, server_default=func.now(), onupdate=func.now())
    ...
    def to_dict(self):
        ...
        return {
            ...
            'value': self.value,
            'tags': self.tags,
            'metadata': self.pronunciation_metadata,  # Keep the output name as metadata for API consistency
            'sources': self.sources,
            'created_at': self.created_at.isoformat() if self.created_at else None,
            'updated_at': self.updated_at.isoformat() if self.updated_at else None,
        }
