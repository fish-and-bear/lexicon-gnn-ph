...
class RelationshipTypeEnum(Enum):
    ...
    # Generate enum values from RelationshipType
    @classmethod
    def _values(cls):
        return [(t.name, t.value) for t in RelationshipType]

class RelationshipCategoryEnum(Enum):
    class Meta:
        name = 'RelationshipCategory'
    
    # Generate enum values from RelationshipCategory
    @classmethod
    def _values(cls):
        return [(t.name, t.value) for t in RelationshipCategory]
...
class RelationType(SQLAlchemyObjectType):
    ...
    relation_type = Field(RelationshipTypeEnum)
    category = Field(RelationshipCategoryEnum)
    ...
    def resolve_relation_type(self, info):
        return RelationshipType.from_string(self.relation_type).value
    
    def resolve_category(self, info):
        return RelationshipType.from_string(self.relation_type).category.value
    
    def resolve_word(self, info):
        return self.related_word
    
    def resolve_bidirectional(self, info):
        return RelationshipType.from_string(self.relation_type).bidirectional
    
    def resolve_strength(self, info):
        return RelationshipType.from_string(self.relation_type).strength
