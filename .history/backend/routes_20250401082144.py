...
from backend.database import cached_query
import json
...
def search_words():
    ...
    # Build base query with all relationships loaded
    ...
    if params['mode'] == 'exact':
        query = query.filter(Word.normalized_lemma == normalize_lemma(search_text))
        ...
        query = query.filter(
            or_(
                Word.normalized_lemma.ilike(f"%{normalize_lemma(search_text)}%"),
