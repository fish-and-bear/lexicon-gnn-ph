...
def process_marayum_json(cur, filename: str):
    ...
    try:
        ...
        if not isinstance(data, dict) or 'words' not in data:
            logger.error(f"Invalid Marayum dictionary format in {filename}")
            return 0, 0
        ...
        # Get language code from base_language or filename
        if base_language:
            # Convert language name to code (e.g., "Chavacano" -> "chv")
            language_code = base_language.lower()[:3]
        else:
            # Try to get from filename (e.g., "chavacano-english_processed.json" -> "chv")
            basename = os.path.basename(filename)
            language_code = basename.split('-')[0].lower()[:3]
        ...
        for entry in entries:
            try:
                ...
                if not word_id:
                    logger.error(f"Failed to create word entry for '{word}'")
                    stats["error_entries"] += 1
                    continue
                ...
                # Add see_also relations
                see_also = entry.get('see_also', [])
                for ref in see_also:
                    try:
                        ref_word = ref.strip() if isinstance(ref, str) else ref.get('word', '').strip()
                        if ref_word:
                            ref_id = get_or_create_word_id(cur, ref_word, language_code=language_code)
                            if ref_id and ref_id != word_id:  # Avoid self-references
                                if insert_relation(
                                    cur,
                                    word_id,
                                    ref_id,
                        ...
                        logger.warning(f"Error adding see_also relation for '{word}': {str(e)}")
        ...
        logger.error(f"Error processing Marayum dictionary file {filename}: {str(e)}")
        return 0, 1
