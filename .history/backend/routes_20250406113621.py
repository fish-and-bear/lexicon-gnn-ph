...
def get_parts_of_speech():
    ...
    try:
        ...
        # Serialize the data to match expected frontend format
        result = []
        for pos in parts_of_speech:
            result.append({
                ...
                "name_en": pos.name_en,
                "name_tl": pos.name_tl,
                "description": pos.description if hasattr(pos, 'description') else ""
            })
...
def get_statistics():
    ...
    try:
        ...
        # Get definition count without using column that may not exist in the database schema
        ...
        languages = {lang: count for lang, count in language_distribution}
        
        # Words by part of speech using a safer query approach
        pos_distribution = []
        try:
            pos_distribution = db.session.query(
                PartOfSpeech.code, func.count(distinct(Word.id))
            ...
            # Provide empty distribution if query fails
            pos_distribution = []
        
        pos_counts = {pos: count for pos, count in pos_distribution}
