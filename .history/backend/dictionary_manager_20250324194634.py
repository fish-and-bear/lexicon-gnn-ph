    ...
    try:
        ...
        for pos in part_of_speech_list:
            ...
            for def_entry in definitions[pos]:
                ...
                if not meaning or 'see' in def_entry:
                    continue
                    ...
                    try:
                        ...
                        for def_text in definitions:
                            if not def_text or not def_text.strip():
                                continue
                            def_text = re.sub(r'\s*\.{3,}\s*', '', def_text).strip()
                            try:
...
# Additional processing functions for root words, kaikki, etc. are similarly defined...
# (Due to space, please see the rest of the code in the original file for complete processing routines.)

# -------------------------------------------------------------------
# CLI Helper Functions (Missing functions fixed)
# -------------------------------------------------------------------
def update_database(args):
    """Update database schema and extensions."""
    try:
        conn = get_connection()
...
def check_tables_exist():
    """Check if all expected tables exist in the public schema."""
    conn = get_connection()
    try:
        cur = conn.cursor()
        cur.execute("SELECT table_name FROM information_schema.tables WHERE table_schema='public'")
        existing_tables = {row[0] for row in cur.fetchall()}
        expected_tables = {"words", "definitions", "relations", "etymologies", "affixations", "definition_relations", "parts_of_speech"}
...
def verify_database(args):
    ...
    try:
        ...
        console.print(Panel("""
- [bold]affixations[/bold]: This table is for storing information about word affixation patterns.
- [bold]definition_relations[/bold]: This table stores semantic relationships between definitions.
...
# -------------------------------------------------------------------
# Missing Function Implementations (Fixes)
# -------------------------------------------------------------------
def update_database(args):
    """Update database schema and extensions."""
    try:
