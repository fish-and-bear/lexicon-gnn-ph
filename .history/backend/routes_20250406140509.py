...
def get_random_word():
    ...
    try:
        ...
        # Base query - select ID directly using SQL rather than the ORM
        query = db.session.query(Word.id)
        
        # Add filters
        if language:
            query = query.filter(Word.language_code == language)
        if pos_code:
            query = query.join(Word.definitions).filter(Definition.standardized_pos_id == pos_code)
        if has_etymology:
            query = query.join(Word.etymologies)
        if has_definitions or min_definitions > 0:
            # Ensure definitions are joined
        ...
        if has_pronunciation:
            query = query.join(Word.pronunciations)
        if has_forms:
            query = query.join(Word.forms)
        if has_templates:
            query = query.join(Word.templates)
        if has_definition_relations:
            query = query.join(Word.definition_relations)
            
        # Get a list of IDs that match our criteria
        eligible_ids = [id[0] for id in query.all()]
        ...
        # Select a random ID from the eligible ones
        import random
        random_id = random.choice(eligible_ids)
        ...
        # Use WordSchema to serialize the word
        schema = WordSchema()
        result = schema.dump(word_with_details)
        
        # Add data completeness (might be redundant if already in schema)
        if hasattr(word_with_details, 'data_completeness'):
            result['data_completeness'] = word_with_details.data_completeness
