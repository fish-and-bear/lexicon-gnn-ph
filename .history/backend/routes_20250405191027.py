...
@cached_query(timeout=86400, key_prefix="languages") # Cache for 1 day
def get_languages():
    """Get a list of all languages referenced in the dictionary."""
    try:
        API_REQUESTS.labels(endpoint="get_languages", method="GET").inc()
    start_time = time.time()
    
        # Query all languages, order by English name
        languages = Language.query.order_by(Language.name_en).all()
        ...
        request_time = time.time() - start_time
        REQUEST_LATENCY.labels(endpoint="get_languages").observe(request_time)
        
        return jsonify(result)
    except Exception as e:
        API_ERRORS.labels(endpoint="get_languages", error_type=type(e).__name__).inc()
