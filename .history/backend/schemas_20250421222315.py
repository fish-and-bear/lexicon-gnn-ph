...
class MetadataField(fields.Dict):
    ...
    def _serialize(self, value, attr, obj, **kwargs):
        if value is None:
            return {}
        # Ensure the value is a dictionary before serializing
        if not isinstance(value, dict):
            logger = logging.getLogger(__name__) # Get logger instance
            logger.warning(f"Invalid type for MetadataField '{attr}' on object {obj}: Expected dict, got {type(value)}. Value: {value}. Returning empty dict.")
            return {}
