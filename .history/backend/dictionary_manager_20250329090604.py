...
class RelationshipManager:
    ...
    def add_relationship(self, from_word_id: int, to_word_id: int, 
        ...
        if isinstance(rel_type, RelationshipType):
            ...
            # Convert category to string to avoid PostgreSQL adaptation issues
            if rel_type.category:
                metadata['category'] = rel_type.category.value
        else:
        ...
        try:
            ...
            normalized_sources = ", ".join(sorted(set(filter(None, sources.split(", "))))) if sources else ""
            ...
            if existing:
                ...
                if existing_metadata is None:
                    ...
                    logger.warning(f"Unexpected metadata format: {type(existing_metadata)}")
                    existing_metadata = {}
                    
                # Merge sources
                if existing_sources and normalized_sources:
                    combined_sources = ", ".join(sorted(set(existing_sources.split(", ") + normalized_sources.split(", "))))
                else:
                    combined_sources = normalized_sources or existing_sources or ""
                ...
                self.cursor.execute("""
                    ...
                    VALUES (%s, %s, %s, %s, %s)
                    RETURNING id
                """, (from_word_id, to_word_id, rel_type_str, normalized_sources, json.dumps(metadata)))
            ...
            if isinstance(rel_type, RelationshipType):
                if rel_type.bidirectional:
                    ...
                    self._ensure_bidirectional_exists(to_word_id, from_word_id, rel_type, normalized_sources, metadata)
                elif rel_type.inverse:
                    # Create inverse relationship
                    inverse_type = rel_type.get_inverse()
                    self._ensure_bidirectional_exists(to_word_id, from_word_id, inverse_type, normalized_sources, metadata)
    ...
    def _ensure_bidirectional_exists(self, from_word_id: int, to_word_id: int, 
        ...
        # Ensure metadata includes category as string value
        if metadata is None:
            metadata = {}
            
        # Make sure we're storing category as a string, not the enum
        if rel_type.category:
            metadata['category'] = rel_type.category.value
            
        try:
            # Normalize sources to avoid duplication
            normalized_sources = ", ".join(sorted(set(filter(None, sources.split(", "))))) if sources else ""
            ...
            if existing:
                ...
                if normalized_sources:
                    if existing_sources != normalized_sources:
                        combined_sources = ", ".join(sorted(set(
                            (existing_sources.split(", ") if existing_sources else []) + 
                            (normalized_sources.split(", ") if normalized_sources else [])
                        )))
                        if combined_sources != existing_sources:
                            update_needed = True
                ...
                if update_needed:
                    ...
                    combined_sources = ", ".join(sorted(set(
                        filter(None, (existing_sources.split(", ") if existing_sources else []) + 
                            (normalized_sources.split(", ") if normalized_sources else []))
                    ))) if existing_sources or normalized_sources else ""
                ...
                """, (from_word_id, to_word_id, rel_type_str, normalized_sources, json.dumps(metadata or {})))
    ...
    def batch_add_relationships(self, relationships: List[Dict]) -> Tuple[int, int]:
        ...
        try:
            ...
            for rel in relationships:
                try:
                    ...
                    if isinstance(rel_type, RelationshipType):
                        ...
                        # Convert RelationshipCategory to string in metadata for consistency
                        if rel_type.category:
                            metadata['category'] = rel_type.category.value
                        ...
                        # Track bidirectional relationships for later processing
                        if rel_type.bidirectional:
                    ...
                    if existing:
                        ...
                        updates.append((rel_id, combined_sources, combined_metadata))
                    else:
                        # Add to insert list
                        inserts.append((from_id, to_id, rel_type_str, normalized_sources, metadata))
                    
                    success_count += 1
                    
                except KeyError as e:
                    logger.error(f"Missing required key in relationship data: {e}")
                    error_count += 1
                except Exception as e:
            ...
            if updates:
                ...
                for rel_id, sources, metadata in updates:
                    update_values.append(f"(%s, %s, %s)")
                    update_params.extend([rel_id, sources, json.dumps(metadata)])
                
                self.cursor.execute(f"""
                    WITH updates(id, sources, metadata) AS (
            ...
            if inserts:
                ...
                for from_id, to_id, rel_type, sources, metadata in inserts:
                    insert_values.append(f"(%s, %s, %s, %s, %s)")
                    insert_params.extend([from_id, to_id, rel_type, sources, json.dumps(metadata)])
                
                # Fixed SQL query with proper closing parenthesis
                self.cursor.execute(f"""
                    INSERT INTO relations (from_word_id, to_word_id, relation_type, sources, metadata)
                    ...
                    DO UPDATE SET
                        sources = array_to_string(ARRAY(
                            SELECT DISTINCT unnest(array_cat(
                                string_to_array(relations.sources, ', '),
            ...
            # Process bidirectional relationships
            if bidirectional_rels:
                # Call this method recursively but without bidirectional handling to avoid infinite recursion
                # We've already set up the inverse relationships correctly in the list
                success_count_bi, error_count_bi = self.batch_add_relationships(bidirectional_rels)
                # Don't double count bidirectional relationships in the success count
                # as they're implicit from the original relationships
    ...
    def get_related_words(self, word_id: int, 
        ...
        if category and isinstance(category, RelationshipCategory):
        ...
        if relationship_types:
            ...
            for rt in relationship_types:
                if isinstance(rt, RelationshipType):
                    type_values.append(rt.rel_value)
                elif isinstance(rt, str):
                    # Try to convert string to RelationshipType
                    try:
                        rt_enum = RelationshipType.from_string(rt)
                        type_values.append(rt_enum.rel_value)
                    except ValueError:
                        # Use the string directly if not a known enum value
                        type_values.append(rt)
                else:
                    logger.warning(f"Unsupported relationship type format: {type(rt)}")
            ...
            # Filter by category using the metadata
            rel_type_filter = "AND (r.metadata->>'category')::text = %s"
        ...
        try:
            ...
            for row in self.cursor.fetchall():
                ...
                if include_metadata and len(row) > 5:
                    metadata = row[5]
                    if metadata:
                ...
                seen_word_ids.add(row[0])
            
            # Include transitive relationships if requested
            if transitive and relationship_types:
                transitive_results = []
                
                for rt in relationship_types:
                    if (isinstance(rt, RelationshipType) and rt.transitive) or \
                       (isinstance(rt, str) and any(t.rel_value == rt and t.transitive for t in RelationshipType)):
                        # Get transitive relationships
                        if isinstance(rt, str):
                            try:
                                rt_enum = RelationshipType.from_string(rt)
                                transitive_results.extend(self.get_transitive_relationships(word_id, rt_enum, seen_word_ids))
                            except ValueError:
                                pass
                        else:
                            transitive_results.extend(self.get_transitive_relationships(word_id, rt, seen_word_ids))
            ...
            logger.error(f"Error getting related words for word ID {word_id}: {e}")
            return []
    
    def get_transitive_relationships(self, word_id: int, rel_type: RelationshipType, 
                                    seen_word_ids: Set[int]) -> List[Dict]:
        """
        Get transitive relationships for a given word and relationship type.
        For example, if A is a hypernym of B, and B is a hypernym of C,
        then A is also a hypernym of C (transitive property).
        ...
        # Find direct relationships of the specified type
        self.cursor.execute("""
            SELECT r.to_word_id, w.lemma, w.language_code, r.sources
            FROM relations r
            JOIN words w ON r.to_word_id = w.id
            WHERE r.from_word_id = %s AND r.relation_type = %s
        """, (word_id, rel_type_str))
        
        direct_relations = self.cursor.fetchall()
        transitive_results = []
        
        # For each directly related word, recursively find its relationships
        for row in direct_relations:
            related_id, lemma, lang_code, sources = row
            
            # Skip if we've already seen this word ID to avoid cycles
            if related_id in seen_word_ids:
                continue
                
            # Add this word ID to the seen set
            seen_word_ids.add(related_id)
            
            # Recursive call to find transitive relationships
            indirect_relations = self.get_transitive_relationships(related_id, rel_type, seen_word_ids)
            
            # Add indirect relations to results
            for indirect in indirect_relations:
                if indirect['word_id'] not in seen_word_ids and indirect['word_id'] != word_id:
                    indirect['relation_type'] = f"transitive_{rel_type_str}"
                    transitive_results.append(indirect)
                    seen_word_ids.add(indirect['word_id'])
    ...
    def find_relationship_paths(self, from_word_id: int, to_word_id: int, 
        ...
        """
        Find all paths of relationships between two words.
        This is essentially a breadth-first search through the relationship graph.
        ...
        Args:
            ...
            prefer_categories: List of relationship categories to prefer in results
        ...
        if relationship_types:
            ...
            for rt in relationship_types:
                if isinstance(rt, RelationshipType):
                    rel_type_strs.append(rt.rel_value)
                elif isinstance(rt, str):
                    rel_type_strs.append(rt)
        ...
        if prefer_categories:
            ...
            for cat in prefer_categories:
                if isinstance(cat, RelationshipCategory):
                    category_strs.append(cat.value)
                elif isinstance(cat, str):
                    category_strs.append(cat)
        
        # Queue for BFS
        queue = deque([[(from_word_id, None)]])
        found_paths = []
        
        # Set to track visited nodes to avoid cycles
        visited = set([from_word_id])
        
        # BFS
        while queue and len(found_paths) < 10:  # Limit to 10 paths for practical reasons
            path = queue.popleft()
            
            if len(path) > max_depth:
                continue
                
            node, relation = path[-1]
            ...
            if node == to_word_id:
                ...
                for i in range(1, len(path)):
                    ...
                    curr_node, relation = path[i]
                    result_path.append(relation)
                found_paths.append(result_path)
                continue
            
            # Get related words
            self.cursor.execute("""
                SELECT r.to_word_id, w.lemma, w.language_code, r.relation_type, r.sources, r.metadata
                FROM relations r
                JOIN words w ON r.to_word_id = w.id
                WHERE r.from_word_id = %s
            ...
            for rel_row in related:
                rel_to_id, lemma, lang_code, rel_type, sources, metadata = rel_row
                
                # Skip if we've already visited this node in this path
                if rel_to_id in visited:
                    continue
                    
                # Skip if we're only considering specific relationship types
                if rel_type_strs and rel_type not in rel_type_strs:
                    continue
                ...
                if metadata:
                    if isinstance(metadata, str):
                        try:
                            rel_info['metadata'] = json.loads(metadata)
                        except json.JSONDecodeError:
                            rel_info['metadata'] = {}
                    else:
                        rel_info['metadata'] = metadata
                
                # Create new path
                new_path = path.copy()
                new_path.append((rel_to_id, rel_info))
                
                # Mark as visited to avoid cycles
                visited.add(rel_to_id)
                
                # Add to queue
                queue.append(new_path)
        
        # Sort paths by score if categories are preferred
        if category_strs and found_paths:
            # Define scoring function based on preferred categories
            def path_score(path):
                # Calculate weighted score based on relationship types and strengths
                score = 0
                for relation in path:
                    # Base score is path length (shorter is better)
                    base_score = -1
                    
                    # Bonus for preferred categories
                    category_score = self._get_category_preference_score(relation, category_strs)
                    
                    # Bonus for relationship strength if available
                    strength_score = 0
                    if 'metadata' in relation and relation['metadata']:
                        strength = relation['metadata'].get('strength', 0)
                        strength_score = strength / 100.0  # Normalize to 0-1 range
                    
                    # Combined score
                    score += base_score + (category_score * 2) + strength_score
    ...
    def _get_category_preference_score(self, relation, preferred_categories):
        """Helper method to score relations based on preferred categories"""
        ...
        category = None
        if 'metadata' in relation and relation['metadata']:
            category = relation['metadata'].get('category')
        
        # If no metadata category, try to infer from relation_type
        if not category and 'relation_type' in relation:
            rel_type = relation['relation_type']
            for rt in RelationshipType:
                if rt.rel_value == rel_type:
        ...
        if category in preferred_categories:
            ...
            return len(preferred_categories) - preferred_categories.index(category)
...
def get_or_create_word_id(cur, lemma: str, language_code: str = "tl", check_exists: bool = False, **kwargs) -> int:
    ...
    if result:
        ...
        # If check_exists is True, we skip updates to existing entries
        if check_exists:
            logger.debug(f"Skipping existing word: {lemma}")
            return None
...
def insert_relation(
    ...
    """Insert a relationship between two words."""
    if from_word_id == to_word_id:
        logger.warning(f"Skipping self-relationship for word ID {from_word_id}")
        return None
        
    # Handle RelationshipType enum
    if isinstance(relation_type, RelationshipType):
        # Extract just the string value, not the entire tuple
        rel_type_str = relation_type.rel_value
        
        # Add category to metadata
        if metadata is None:
            metadata = {}
        if relation_type.category:
            # Store category as string value, not the enum object
            metadata['category'] = relation_type.category.value
            
        # Add default strength if not specified
        if 'strength' not in metadata:
            metadata['strength'] = relation_type.strength
    else:
    ...
    try:
        ...
        if isinstance(relation_type, RelationshipType):
            ...
            if relation_type.bidirectional:
                ...
                relationship_manager._ensure_bidirectional_exists(
                    to_word_id, from_word_id, relation_type, normalized_sources, metadata)
            elif relation_type.inverse:
                # Create inverse relationship
                inverse_type = relation_type.get_inverse()
                relationship_manager._ensure_bidirectional_exists(
                    to_word_id, from_word_id, inverse_type, normalized_sources, metadata)
            
        return rel_result
        
    except Exception as e:
        logger.error(f"Error inserting relation {from_word_id} -> {to_word_id} ({relation_type}): {str(e)}")
        return None
@with_transaction(commit=True)
...
def insert_etymology(
    ...
    try:
        ...
        cur.execute(
            ...
            """,
            (word_id, etymology_text, normalized_components, etymology_structure, language_codes, normalized_sources)
        )
    except Exception as e:
...
def process_marayum_json(cur, filename: str):
    ...
    for entry in words_array:
        try:
            ...
            errors += 1
            logger.error(f"Error processing Marayum entry '{entry.get('word', 'unknown')}': {str(e)}")
                
    logger.info(f"Completed processing {dict_name}: {processed} entries processed, {errors} errors")
