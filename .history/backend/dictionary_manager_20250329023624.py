...
def process_marayum_json(cur, filename: str):
    ...
    try:
        ...
        # Map the base language to our language codes
        lang_code = LanguageCode.from_name(base_lang) if base_lang else None
        if not lang_code:
            # Try to determine from filename
            if 'cebuano' in dict_name.lower():
                lang_code = LanguageCode.CEBUANO
            elif 'waray' in dict_name.lower():
                lang_code = LanguageCode.WARAY
            elif 'hiligaynon' in dict_name.lower() or 'ilonggo' in dict_name.lower():
                lang_code = LanguageCode.HILIGAYNON
            elif 'ilocano' in dict_name.lower():
                lang_code = LanguageCode.ILOCANO
            elif 'kapampangan' in dict_name.lower():
                lang_code = LanguageCode.KAPAMPANGAN
            elif 'asi' in dict_name.lower():
                lang_code = LanguageCode.ASI
            else:
                # Default to Tagalog if we can't determine
                lang_code = LanguageCode.TAGALOG
                logger.warning(f"Could not determine language for {dict_name}, defaulting to Tagalog")
        ...
        try:
                ...
                if isinstance(definition, dict):
                    ...
                    if def_text:
                        insert_definition(
                            ...
                            word_id,
                            def_text,
                            part_of_speech=standardize_pos(def_pos) or standardized_pos,
            ...
            for ref in references:
                if isinstance(ref, dict) and 'word' in ref and ref['word']:
                    ...
                    related_word_id = get_or_insert_word(
                        cur, 
                        rel_word, 
                        language_code=lang_code.value, 
                        sources=source
