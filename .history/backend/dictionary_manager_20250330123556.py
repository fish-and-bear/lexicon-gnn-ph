...
def insert_pronunciation(cur, word_id: int, pronunciation_data: Union[str, Dict], sources: str = "") -> Optional[int]:
    ...
    Args:
        ...
        pronunciation_data: Either a string (IPA pronunciation) or a dict with keys:
            - type: str (default: 'ipa')
            - value: str
            - tags: List[str] (optional)
            - metadata: Dict (optional)
            - sources: str (optional)
        sources: Source of the pronunciation data (if not in dict)
    
    Returns:
        Optional[int]: ID of the inserted/updated pronunciation record
    """
    try:
        # Validate word_id
        ...
        logger.error(f"Database error inserting pronunciation for word ID {word_id}: {str(e)}")
        raise
    except Exception as e:
...
def insert_credit(cur, word_id: int, credit_data: Union[str, Dict], sources: str = "") -> Optional[int]:
    ...
    Args:
        cur: Database cursor
        word_id: ID of the word
        credit_data: Either a string (credit text) or a dict with keys:
            - text: str
            - sources: str (optional)
        sources: Source of the credit data (if not in dict)
    
    Returns:
        Optional[int]: ID of the inserted/updated credit record
    """
    try:
        # Validate word_id
        cur.execute("SELECT 1 FROM words WHERE id = %s", (word_id,))
        ...
        logger.error(f"Database error inserting credit for word ID {word_id}: {str(e)}")
        raise
    except Exception as e:
