...
def process_entry(cur, entry: Dict, source_identifier: str) -> Optional[int]:
    ...
    Args:
        ...
        source_identifier: Source identifier (e.g., filename)
    ...
    try:
        ...
        cur.execute("""
            ...
            DO UPDATE SET
                ...
                END
            RETURNING id
        """, (word, normalize_lemma(word), language_code, source_identifier))
        ...
        if "pronunciations" in entry:
            ...
            if isinstance(prons, list):
                for pron in prons:
                    try:
                        insert_pronunciation(cur, word_id, pron, source_identifier)
                    except Exception as e:
                        logger.warning(f"Failed to insert pronunciation for '{word}': {str(e)}")
            elif isinstance(prons, (str, dict)):
                try:
                    insert_pronunciation(cur, word_id, prons, source_identifier)
                except Exception as e:
                    logger.warning(f"Failed to insert pronunciation for '{word}': {str(e)}")
        ...
        if "credits" in entry:
            ...
            if isinstance(credits, list):
                for credit in credits:
                    try:
                        insert_credit(cur, word_id, credit, source_identifier)
                    except Exception as e:
                        logger.warning(f"Failed to insert credit for '{word}': {str(e)}")
            elif isinstance(credits, (str, dict)):
                try:
                    insert_credit(cur, word_id, credits, source_identifier)
                except Exception as e:
                    logger.warning(f"Failed to insert credit for '{word}': {str(e)}")
