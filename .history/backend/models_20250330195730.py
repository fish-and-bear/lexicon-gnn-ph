...
class Word(db.Model):
    ...
    alternative_spellings = Column(JSONB, default=list)
    syllable_count = Column(Integer)
    pronunciation_data = Column(JSONB)
    pronunciation_guide = Column(Text)
    stress_pattern = Column(String(32))
    formality_level = Column(String(32))
    usage_frequency = Column(Float)
    geographic_region = Column(String(64))
    time_period = Column(String(64))
    cultural_notes = Column(Text)
    grammatical_categories = Column(JSONB, default=list)
    semantic_domains = Column(JSONB, default=list)
    etymology_confidence = Column(Float)
    ...
    __table_args__ = (
        ...
        Index('idx_words_stress', 'stress_pattern'),
        Index('idx_words_region', 'geographic_region'),
        Index('idx_words_period', 'time_period'),
        Index('idx_words_formality', 'formality_level'),
        Index('idx_words_frequency', 'usage_frequency'),
    ...
    def validate_language_code(self, key, code):
        ...
        valid_codes = [
            # Primary Philippine Languages
            'tl', 'ceb', 'hil', 'war', 'pam', 'bik', 'ilo', 'pag',
            # Historical/Root Languages
            'psp', 'pph', 'pan',
            # Foreign Language Influences
            'en', 'es', 'spl', 'san', 'may', 'ara', 'cmn', 'nan', 'jpn'
        ]
