...
def init_app(app):
    ...
    with app.app_context():
        ...
        if is_testing_db(db.engine):
            ...
            db.session.execute(text("CREATE INDEX IF NOT EXISTS idx_words_search ON words(search_text)"))
            db.session.execute(text("CREATE INDEX IF NOT EXISTS idx_words_normalized ON words(normalized_lemma)"))
            db.session.execute(text("CREATE INDEX IF NOT EXISTS idx_words_baybayin ON words(baybayin_form) WHERE has_baybayin = 1"))
            db.session.execute(text("CREATE INDEX IF NOT EXISTS idx_etymologies_langs ON etymologies(language_codes)"))
            db.session.execute(text("CREATE INDEX IF NOT EXISTS idx_etymologies_components ON etymologies(normalized_components)"))
