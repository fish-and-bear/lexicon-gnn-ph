...
class RelationshipManager:
    ...
    def _ensure_bidirectional_exists(self, from_word_id: int, to_word_id: int, 
        ...
        """
        Ensure a bidirectional or inverse relationship exists with the EXACT same information.
        ...
        # Ensure metadata includes category as string
        if metadata is None:
            metadata = {}
        if 'category' not in metadata and rel_type.category:
            metadata['category'] = rel_type.category.value
        
        # Create a savepoint so we can roll back just this operation if needed
        savepoint_name = f"sp_ensure_bidirectional_{from_word_id}_{to_word_id}"
        
        try:
            # Create a savepoint for this operation
            self.cursor.execute(f"SAVEPOINT {savepoint_name}")
            
            # Check if relationship already exists with EXACT SAME information
            self.cursor.execute("""
                SELECT id, sources, metadata
                FROM relations 
                WHERE from_word_id = %s AND to_word_id = %s AND relation_type = %s
            ...
            existing = self.cursor.fetchone()
            if existing:
                # We found an existing relationship, check if it has the exact same info
                rel_id, existing_sources, existing_metadata = existing
                ...
                # Check if sources are different (need update)
                sources_need_update = False
                if normalized_sources and existing_sources != normalized_sources:
                    # Sources are different, need to merge
                    combined_sources = ", ".join(sorted(set(existing_sources.split(", ") + normalized_sources.split(", "))))
                    sources_need_update = combined_sources != existing_sources
                ...
                # Update if needed
                if sources_need_update or metadata_needs_update:
                    combined_metadata = {**existing_metadata, **metadata}
                    combined_sources = ", ".join(sorted(set(existing_sources.split(", ") + normalized_sources.split(", ")))) if existing_sources else normalized_sources
            ...
            try:
                ...
                logger.error(f"Failed to rollback to savepoint {savepoint_name}")
            return False
