        ...
        sql = text("""
            WITH affix_stats AS (
                SELECT 
                    ...
                    json_agg(DISTINCT w.lemma)::jsonb FILTER (WHERE w.id = a.root_word_id) as sample_roots,
                    json_agg(DISTINCT w2.lemma)::jsonb FILTER (WHERE w2.id = a.affixed_word_id) as sample_affixed
                FROM affixations a
            ...
            SELECT 
                ...
                (SELECT json_agg(sample) FROM (
                    ...
                    FROM affix_stats s2,
                    jsonb_array_elements_text(COALESCE(s2.sample_roots, '[]'::jsonb)) AS element
                ...
                (SELECT json_agg(sample) FROM (
                    SELECT DISTINCT element as sample
                    FROM affix_stats s2,
                    jsonb_array_elements_text(COALESCE(s2.sample_affixed, '[]'::jsonb)) AS element
                    WHERE s.affix_type = s2.affix_type
                    LIMIT 5
                ) t) as sample_affixed
            FROM affix_stats s
