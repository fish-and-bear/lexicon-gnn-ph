...
def process_kaikki_jsonl(cur, filename: str):
    ...
    def process_etymology(cur, word_id: int, etymology_text: str, etymology_templates: List[Dict] = None):
        ...
        # Determine column name based on language codes
        # Check if the "source_language_codes" column exists
        cur.execute("""
            SELECT EXISTS (
                SELECT FROM information_schema.columns 
    ...
    # Process a single entry - simplified error handling
    ...
    try:
        with open(filename, 'r', encoding='utf-8') as f:
            # Process entries individually with new transaction for each entry
            entry_count = 0
            ...
            for line in f:
                try:
                    ...
                    try:
                        conn.rollback()  # Make sure we're in a clean transaction state
                    except Exception as rollback_error:
                    ...
                    if result:
                        ...
                        try:
                            conn.commit()
                            # Only log every 100 entries to reduce log spam
                            if stats["processed_entries"] % 100 == 0:
                                logger.info(f"Processed {stats['processed_entries']} entries so far.")
                        except Exception as commit_error:
                            logger.error(f"Error committing entry: {str(commit_error)}")
                            conn.rollback()
                            logger.info("Rolled back entry due to commit error.")
                            # Adjust stats - entry wasn't successfully committed
                            stats["error_entries"] += 1
                            stats["processed_entries"] -= 1
                    else:
                        stats["error_entries"] += 1
                        # Rollback this entry's transaction
                        try:
                            conn.rollback()
                        except Exception as rollback_error:
                            logger.error(f"Error during rollback: {str(rollback_error)}")
