...
def process_kaikki_jsonl(cur, filename: str):
    ...
    def process_entry(cur, entry: Dict):
        ...
        try:
            ...
            # Insert relationships with robust mapping
...
def migrate_data(args):
    ...
    try:
        try:
            ...
            console.print(f"2. Create a database named '{DB_NAME}' if it doesn't exist")
            console.print("   You can create it with: [bold]createdb {DB_NAME}[/]")
            console.print("3. Set correct environment variables if needed")
            return
...
def lookup_word(args):
    ...
    try:
        ...
        (word_id, lemma, language_code, has_baybayin, baybayin_form,
         preferred_spelling, pronunciation_data, idioms, def_sources,
        ...
        if has_baybayin:
            ...
            basic_tree.add(f"Romanized: {baybayin_form and baybayin_form or '' if romanized_form is None else romanized_form}")
        if root_word_id:
        ...
        if def_sources:
            for s in def_sources:
                sources_set.update(s.strip() for s in s.split(','))
        if ety_sources:
...
def display_leaderboard(cur, console):
    """
    Display a comprehensive leaderboard of dictionary contributors.
    The leaderboard shows the top 10 sources by definitions count and unique words,
    as well as the most recently active sources in the past 30 days.
    """
    try:
        console.print("\n[bold cyan]📊 Top Contributors Leaderboard[/]", justify="center")
        
        # Top Contributors: by total definitions and unique word count
        cur.execute("""
            SELECT 
                 CASE
                      WHEN sources = 'kaikki-ceb.jsonl' THEN 'kaikki.org (Cebuano)'
            ...
            LIMIT 10
        """)
        top_contrib = cur.fetchall()
        leaderboard_table = Table(title="Top Contributors", box=box.ROUNDED, border_style="cyan")
        leaderboard_table.add_column("Rank", justify="center", style="bold", width=6)
        leaderboard_table.add_column("Source", style="yellow")
        leaderboard_table.add_column("Definitions", justify="right", style="green")
        leaderboard_table.add_column("Unique Words", justify="right", style="blue")
        medals = ["🥇", "🥈", "🥉"]
        for idx, (source, def_count, unique_count) in enumerate(top_contrib, start=1):
            medal = medals[idx-1] if idx <= len(medals) else ""
            leaderboard_table.add_row(str(idx), f"{medal} {source}", f"{def_count:,}", f"{unique_count:,}")
        console.print(leaderboard_table)
        
        # Recently Active Sources (past 30 days)
        cur.execute("""
            SELECT 
                 CASE
                      WHEN sources = 'kaikki-ceb.jsonl' THEN 'kaikki.org (Cebuano)'
        ...
        if recent_active:
            ...
            for source, last_act, count in recent_active:
                last_act_str = last_act.strftime("%Y-%m-%d %H:%M")
                recent_table.add_row(source, last_act_str, f"{count:,}")
            console.print("\n")
            console.print(recent_table)
        else:
            console.print("\n[italic yellow]No recent activity data available.[/]")
    except Exception as e:
        logger.error(f"Error displaying leaderboard: {str(e)}")
        console.print(f"[red]Error: {str(e)}[/]")def purge_database(args):
