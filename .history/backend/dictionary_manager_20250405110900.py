    ...
    try:
        ...
        for idx, result in enumerate(results):
            try:
                ...
                if tags:
                    try:
                        tags_list = json.loads(tags) if isinstance(tags, str) and tags.strip() else tags
                        if tags_list:
                ...
                if sources:
                    try:
                        sources_list = json.loads(sources) if isinstance(sources, str) and sources.strip() else sources
                        if sources_list:
                ...
                try:
                    ...
                    if prons:
                        ...
                        for pron in prons:
                            ...
                            if p_tags:
                                try:
                                    p_tags_list = json.loads(p_tags) if isinstance(p_tags, str) and p_tags.strip() else p_tags
                                    if p_tags_list:
                            ...
                            if p_sources:
                                try:
                                    p_sources_list = json.loads(p_sources) if isinstance(p_sources, str) and p_sources.strip() else p_sources
                                    if p_sources_list:
                ...
                try:
                    ...
                    if etyms:
                        console.print("\n[bold cyan]Etymology:[/]")
                        for etym in etyms:
                            etym_text, normalized_comps, lang_codes, etym_sources = etym
                            etym_panel = Panel(etym_text, title="Etymology")
                            console.print(etym_panel)
                            
                            # Display etymology language codes if available
                            if lang_codes:
                                try:
                                    lang_codes_list = json.loads(lang_codes) if isinstance(lang_codes, str) and lang_codes.strip() else lang_codes
                                    if lang_codes_list:
                                        console.print(f"[dim]Languages:[/] {', '.join(lang_codes_list)}")
                                except Exception as e:
                                    logger.warning(f"Error parsing etymology language codes: {str(e)}")
                                    
                            # Display etymology sources if available
                            if etym_sources:
                                try:
                                    etym_sources_list = json.loads(etym_sources) if isinstance(etym_sources, str) and etym_sources.strip() else etym_sources
                                    if etym_sources_list:
                                        console.print(f"[dim]Sources:[/] {', '.join(etym_sources_list)}")
                                except Exception as e:
                                    logger.warning(f"Error parsing etymology sources: {str(e)}")
