...
def cached_query(timeout: int = 3600, key_prefix: str = ""):
    ...
    def decorator(f):
        ...
        def wrapper(*args, **kwargs):
            ...
            # If false, skip all redis client access and caching logic.
            if not REDIS_ENABLED:
                logger.debug("Redis disabled via REDIS_ENABLED=false, skipping cache.")
                return f(*args, **kwargs)
            
            # Now that we know REDIS_ENABLED is true, check if client exists
            if redis_client is None:
                 logger.debug("Redis enabled but client is None (connection failed?), skipping cache.")
                 return f(*args, **kwargs)
                 
            # --- Rest of caching logic proceeds only if REDIS_ENABLED is true AND redis_client exists ---
