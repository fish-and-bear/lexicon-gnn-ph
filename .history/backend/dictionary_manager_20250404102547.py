...
# Removed @with_transaction(commit=True) decorator
        ...
        for entry_index, root_word_entry in entries_iterator:
            ...
            try:
                ...
                if not isinstance(root_word_entry, dict):
                    ...
                    stats["skipped"] += 1
                    cur.execute(f"RELEASE SAVEPOINT {savepoint_name}") # Release if skipped
                    pbar.update(1)
                    continue
                ...
                # --- Root Word Creation ---
                # If this fails, the exception will be caught below and rolled back to savepoint
                word_id = get_or_create_word_id(
                    cur,
                    root_word,
                    language_code=language_code,
                ...
                if isinstance(definitions, list):
                    for def_idx, definition_item in enumerate(definitions):
                        # (Keep the inner definition processing logic as before)
                        ...
                        if definition_text:
                            # If insert_definition fails, it should raise an exception
                            # which will be caught by the outer try/except for the entry
                            ...
                            if def_id:
                                stats["definitions_added"] += 1
                            # Removed the 'else' block that warned about None return

                # --- Process associated words ---
                associated_data = root_word_entry.get("associated_words", {})
                associated_items_iterator = None
                # <<< START DEBUG LOGGING >>>
                logger.debug(f"Root '{root_word}': Found associated_data type: {type(associated_data)}, content snippet: {str(associated_data)[:100]}...") # <<< ADDED DEBUG LOG
                ...
                if isinstance(associated_data, dict):
                    ...
                    logger.debug(f"Root '{root_word}': Using dictionary iterator for associated words.") # <<< ADDED DEBUG LOG
                    ...
                    logger.debug(f"Root '{root_word}': Using list iterator for associated words.") # <<< ADDED DEBUG LOG
                else:
                    logger.warning(f"Unexpected format for 'associated_words' for root '{root_word}'. Skipping.")
                    associated_items_iterator = iter([])

                processed_assoc_count_for_root = 0 # <<< ADDED Counter for logging
                for assoc_idx, assoc_word, assoc_details in associated_items_iterator:
                    logger.debug(f"Root '{root_word}': Processing associated item #{assoc_idx}: word='{assoc_word}', details='{str(assoc_details)[:50]}...'") # <<< ADDED DEBUG LOG
                    # <<< END DEBUG LOGGING >>>
                    if assoc_word and assoc_word != root_word:
                        stats["associated_processed"] += 1
                        processed_assoc_count_for_root += 1 # <<< Increment local counter
                        assoc_word_id = None

                        # If get_or_create_word_id fails, it raises exception
                        ...
                        if assoc_word_id:
                            ...
                            if rel_id: stats["relations_added"] += 1
                            ...
                            if assoc_def_text:
                                ...
                                if assoc_def_id: stats["definitions_added"] += 1

                if processed_assoc_count_for_root > 0: # <<< ADDED Summary log
                ...
                # NOTE: Removed the `raise entry_e` from here.
                # The rollback handles the error for this entry, allowing the loop to continue.
