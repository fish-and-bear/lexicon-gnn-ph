...
def build_random_word_query(language, pos_code, has_etymology, has_definitions, has_baybayin, min_definitions):
    ...
    # Track joining tables to avoid duplicates
    joined_tables = {'words': True}
    conditions = []
    
    # Add joins based on filters
    if pos_code:
        if 'definitions' not in joined_tables:
            sql_query += " LEFT JOIN definitions d ON d.word_id = w.id"
            joined_tables['definitions'] = True
        
        sql_query += " JOIN parts_of_speech pos ON d.standardized_pos_id = pos.id"
        joined_tables['parts_of_speech'] = True
    ...
    if has_etymology:
        sql_query += " LEFT JOIN etymologies e ON e.word_id = w.id"
        joined_tables['etymologies'] = True
    ...
    if has_baybayin:
        # Ensure Baybayin form is valid (no empty string or only spaces)
        conditions.append("w.has_baybayin = TRUE AND w.baybayin_form IS NOT NULL AND TRIM(w.baybayin_form) != ''")
    else:
        # When not specifically requesting Baybayin words, prioritize words without Baybayin to avoid validation issues
