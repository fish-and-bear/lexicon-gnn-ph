        ...
        try:
            ...
            if conn:
                try:
                    ...
                    # If the savepoint rollback fails, try a full rollback and reconnect
                    try:
                        conn.rollback()
                        # After a full rollback, need to restart the transaction
                        conn.commit()
                        logger.warning("Performed full transaction rollback - restarting transaction")
                    except Exception as conn_error:
                        logger.error(f"Failed to roll back transaction: {str(conn_error)}")
            return None
        ...
        with open(filename, 'r', encoding='utf-8') as f:
            for line_number, line in enumerate(f, 1):
                try:
                    ...
                    # Pass both cursor and connection for proper transaction handling
                    # Also pass the line number for unique savepoint naming
                    result = process_entry(cur, entry, conn, line_number)
                    ...
                    # Also reset batch_count to avoid savepoint conflicts
                    if batch_count >= 100:
                        conn.commit()
                    ...
                    try:
                        ...
                        conn.commit()
                        batch_count = 0
                        logger.warning("Recovered from error with full transaction rollback")
                    except Exception as rollback_error:
            ...
            if batch_count > 0:
                conn.commit()
                logger.info(f"Final transaction commit with {batch_count} entries")
