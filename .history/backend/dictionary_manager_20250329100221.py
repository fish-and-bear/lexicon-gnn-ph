...
def insert_relation(
    ...
    Returns:
        int: The ID of the inserted relation, or None if insertion failed
    """
    try:
        # Validate IDs
        cur.execute("SELECT EXISTS(SELECT 1 FROM words WHERE id = %s)", (from_word_id,))
        if not cur.fetchone()[0]:
            logger.warning(f"From word ID {from_word_id} does not exist")
            return None
            
        cur.execute("SELECT EXISTS(SELECT 1 FROM words WHERE id = %s)", (to_word_id,))
        if not cur.fetchone()[0]:
            logger.warning(f"To word ID {to_word_id} does not exist")
            return None
        ...
        if isinstance(relation_type, RelationshipType):
            ...
            rel_type_str = str(relation_type)
        
        # Truncate relation_type if it's too long (varchar 64 limit)
        if len(rel_type_str) > 64:
            logger.warning(f"Truncating relation type that exceeds 64 characters: '{rel_type_str[:20]}...'")
            # Store the original value in metadata
            if metadata is None:
                metadata = {}
            metadata['original_relation_type'] = rel_type_str
            rel_type_str = rel_type_str[:64]
        ...
        if metadata:
            try:
                ...
                logger.warning(f"Failed to serialize metadata to JSON: {e}. Using empty metadata.")
                metadata_json = '{}'
                
        # Use parameterized query with ON CONFLICT handling
        cur.execute("""
            INSERT INTO relations 
            (from_word_id, to_word_id, relation_type, sources, metadata, created_at, updated_at)
            ...
            ON CONFLICT (from_word_id, to_word_id, relation_type) DO UPDATE SET
                ...
                END,
                metadata = COALESCE(relations.metadata, '{}'::jsonb) || COALESCE(EXCLUDED.metadata, '{}'::jsonb),
                updated_at = NOW()
            RETURNING id
        """, (from_word_id, to_word_id, rel_type_str, sources, metadata_json))
        ...
        logger.error(f"Database error in insert_relation: {e}")
        return None
    except Exception as e:
        logger.error(f"Unexpected error in insert_relation: {e}")
        return None
