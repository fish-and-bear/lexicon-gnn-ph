...
def search_words(**params):
    """
    Search words with enhanced filters and metadata.
    """
    query = params['q']
    limit = params['limit']
    pos = params.get('pos')
    ...
    include_badlit = params.get('include_badlit', True)  # New parameter
    ...
    if pos:
        base_query = base_query.join(Word.definitions).join(Definition.standardized_pos).\
            filter(PartOfSpeech.code == pos)
    
    # Add search conditions based on mode
    search_conditions = []
    
    if mode in ('all', 'exact'):
        search_conditions.extend([
            Word.lemma.ilike(f"%{query}%"),
            Word.normalized_lemma.ilike(f"%{query}%")
        ])
    
    if mode in ('all', 'baybayin') and include_baybayin:
        search_conditions.append(Word.baybayin_form.ilike(f"%{query}%"))
        
    if mode in ('all', 'badlit') and include_badlit:  # New condition
        search_conditions.append(Word.badlit_form.ilike(f"%{query}%"))
    
    if mode in ('all', 'phonetic'):
        # Add phonetic search using pronunciation_data
        search_conditions.append(
            Word.pronunciation_data.op('->>')('ipa').ilike(f"%{query}%")
        )
    ...
    if sort == 'relevance':
        # Use ts_rank for full text search ranking
            ...
            func.ts_rank(Word.search_text, func.to_tsquery('simple', query)).desc()
                        )
    elif sort == 'alphabetical':
                        base_query = base_query.order_by(
            Word.lemma.desc() if order == 'desc' else Word.lemma
        )
    elif sort in ('created', 'updated'):
        sort_col = Word.created_at if sort == 'created' else Word.updated_at
                            base_query = base_query.order_by(
    ...
    for word in results:
        ...
        word_data.update({
            ...
            "is_abbreviation": word.is_abbreviation,
            "is_initialism": word.is_initialism,
            "badlit_form": word.badlit_form,
...
# Add new endpoints for Baybayin/Badlit handling
@bp.route("/api/v2/words/<path:word>/badlit", methods=["GET"])
@cached(prefix="word_badlit", ttl=3600)
def get_word_badlit(word):
    """Get Badlit form for a word."""
    ...
    return success_response({
        "word": word_obj.lemma,
        "badlit_form": word_obj.badlit_form,
        "hyphenation": word_obj.hyphenation
    })
