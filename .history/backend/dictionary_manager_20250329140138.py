                                ...
                                for example_set in def_entry['example_sets']:
                                    if isinstance(example_set, dict) and 'examples' in example_set and isinstance(example_set['examples'], list):
                                        ...
                                        if example_strings:
                                            ...
                                            stats["examples_added"] += len(example_strings)
                            ...
                            try:
                                ...
                                if definition_id:
                                    ...
                                    if def_entry.get('see') and isinstance(def_entry['see'], str):
                                        ...
                                        try:
                                            see_id = get_or_create_word_id(cur, see_reference, language_code='tl', check_exists=True)
                                            if see_id:
                                    ...
                                    if 'cross_references' in def_entry and isinstance(def_entry['cross_references'], list):
                                        for cross_ref in def_entry['cross_references']:
                                            ...
                                            if cross_ref_term and len(cross_ref_term) <= 255:
                                                try:
                                                    cross_ref_id = get_or_create_word_id(cur, cross_ref_term, language_code='tl', check_exists=True)
                                                    if cross_ref_id:
                                                        insert_definition_relation(
                                                            cur, 
                                                            definition_id,
                                        ...
                                        for synonym in def_entry['synonyms']:
                                            ...
                                            if synonym_term and len(synonym_term) <= 255:
                                                ...
                                                try:
                                                    ...
                                                    synonym_id = get_or_create_word_id(cur, synonym_term, language_code='tl', check_exists=True)
                                                    if synonym_id is None:
                                                        # Term doesn't exist, create it with proper error handling
                                                        try:
                                                            ...
                                                            if synonym_id:
                                                                ...
                                                                logger.warning(f"Failed to create term: {synonym_term}")
                                                        except Exception as inner_e:
                                                            logger.error(f"Error creating synonym term '{synonym_term}': {str(inner_e)}")
                                                            error_types[str(inner_e)] = error_types.get(str(inner_e), 0) + 1
                                                            continue
                                                    else:
                                                        # Term exists, create relation
                                                        insert_relation(
                                                            cur, 
                                                            word_id, 
                                                            synonym_id, 
                ...
                if 'related' in entry and isinstance(entry['related'], dict):
                    ...
                    for rel_type, rel_items in entry['related'].items():
                        ...
                        for rel_item in rel_items:
                            ...
                            if related_term and len(related_term) <= 255:
                                try:
                                    # Check if the related term exists first
                                    related_id = get_or_create_word_id(cur, related_term, language_code='tl', check_exists=True)
                                    if related_id is None:
                                        # Related term doesn't exist, create it
                                        try:
                                            related_id = get_or_create_word_id(cur, related_term, language_code='tl')
                                        except Exception as inner_e:
                                            logger.error(f"Error creating related term '{related_term}': {str(inner_e)}")
                                            error_types[str(inner_e)] = error_types.get(str(inner_e), 0) + 1
                                            continue
                                            
                                    # Now create the relation if we have both IDs
