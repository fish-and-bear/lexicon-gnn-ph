...
def search_words():
    ...
    # Apply filters based on search mode with optimized querying
    search_text = params['q'].lower()
    normalized_search = normalize_lemma(search_text)
    
    if params['mode'] == 'exact':
        query = query.filter(Word.normalized_lemma == normalized_search)
    elif params['mode'] == 'baybayin':
        query = query.filter(
            and_(
                Word.has_baybayin == True,
                Word.baybayin_form.ilike(f"%{search_text}%")
            )
        )
    elif params['mode'] == 'phonetic':
        query = query.filter(
            or_(
                func.similarity(Word.lemma, search_text) >= 0.3,
                func.similarity(Word.normalized_lemma, normalized_search) >= 0.3
            )
        )
    else:  # Default to 'all' mode with optimized full-text search
        base_conditions = [
            Word.normalized_lemma.ilike(f"%{normalized_search}%"),
            Word.lemma.ilike(f"%{search_text}%"),
            func.similarity(Word.lemma, search_text) >= 0.3,
            func.similarity(Word.normalized_lemma, normalized_search) >= 0.3
        ]
        
        # Only add full-text search if the column exists and we're not in a test database
        if not is_testing_db(db.get_engine()):
            try:
                base_conditions.append(Word.search_text.match(search_text))
            except Exception:
                pass  # Ignore if search_text column doesn't exist
                
        query = query.filter(or_(*base_conditions))

    # Apply additional filters with optimized joins
    ...
    # Apply sorting with optimized ordering
    ...
    else:  # Default to relevance with optimized ranking
        if not is_testing_db(db.get_engine()):
            try:
                query = query.order_by(
                    func.ts_rank_cd(Word.search_text, func.to_tsquery('filipino', search_text)).desc(),
                    func.similarity(Word.lemma, search_text).desc(),
                    func.similarity(Word.normalized_lemma, normalized_search).desc()
                )
            except Exception:
                # Fallback to basic ordering if full-text search is not available
                query = query.order_by(
                    func.similarity(Word.lemma, search_text).desc(),
                    func.similarity(Word.normalized_lemma, normalized_search).desc()
                )
        else:
            query = query.order_by(Word.lemma)

    # Get total count before pagination with optimized counting
    total_count = query.count()

    # Apply pagination
    query = query.offset(params['offset']).limit(params['limit'])

    # Execute query and format results with optimized serialization
    results = query.all()
    schema = WordSchema(many=True)
    words = schema.dump(results)

    # Generate facets and suggestions with optimized aggregation
