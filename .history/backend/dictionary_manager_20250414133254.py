...
@with_db_transaction(commit=True)
    def _process_single_tagalog_word_entry(cur, entry_data: dict, word_key: str, source_identifier: str, language_code: str, add_etymology: bool, word_id_cache: dict, stats: dict, error_types: dict, table_column_map: dict, relations_batch: list, required_columns: dict) -> bool:    """
    ...
    try:
        ...
        word_id_cache[item_word] = word_id # Update cache immediately

        # Process pronunciation
        pronunciation = entry_data.get("pronunciation", "").strip()
        if pronunciation and set(required_columns["pronunciations"]).issubset(table_column_map["pronunciations"]):
            try:
        ...
        # Process etymology
        if add_etymology and "etymology" in entry_data and entry_data["etymology"] and set(required_columns["etymologies"]).issubset(table_column_map["etymologies"]):
        ...
        if isinstance(definitions, dict) and set(required_columns["definitions"]).issubset(table_column_map["definitions"]):
            for pos, def_list in definitions.items():
                ...
                for def_item in def_list:
                    ...
                    try:
                        # Process examples into JSON format if needed for insert_definition
                        examples_processed = []
                        if examples and isinstance(examples, list) and set(required_columns["examples"]).issubset(table_column_map["examples"]):
                             for example in examples:
                                 if isinstance(example, str) and example.strip():
                                     examples_processed.append({"text": example.strip()})
                                     # Note: Example insertion might need its own table/logic if required
                        ...
                        def_id = insert_definition(
                            ...
                            # Add other fields like usage_notes, tags, metadata if needed
                        )
                        if def_id:
                        ...
                        logger.warning(f"Non-critical: Error adding definition for word '{item_word}', POS '{pos}': {e}")
                        error_types[f"DefinitionError: {type(e).__name__}"] = error_types.get(f"DefinitionError: {type(e).__name__}", 0) + 1

        # Process domains as tags
        domains = entry_data.get("domains", [])
        if domains and isinstance(domains, list) and set(required_columns["word_tags"]).issubset(table_column_map["word_tags"]):
            for domain in domains:
                if isinstance(domain, str) and domain.strip():
                    try:
                        # Assuming word_tags table exists with word_id, tag, source_identifier
                        cur.execute(
                            """
                            INSERT INTO word_tags (word_id, tag, source_identifier)
                            VALUES (%s, %s, %s)
                            ON CONFLICT (word_id, tag) DO NOTHING
                            """,
                            (word_id, domain.strip(), source_identifier),
                        )
                        # Optionally increment a stats counter for tags
                    except Exception as e:
                        logger.warning(f"Non-critical: Error adding domain tag '{domain}' for word '{item_word}': {e}")
                        error_types[f"DomainTagError: {type(e).__name__}"] = error_types.get(f"DomainTagError: {type(e).__name__}", 0) + 1
...
# Modify the main process_tagalog_words function
def process_tagalog_words(cur, filename: str, add_etymology: bool = True):
    """
    Process a JSON file containing Tagalog words and their definitions using entry-level transactions.

    Args:
        cur: Database cursor (passed but not directly used due to decorator)
    ...
    # Check table structure once before starting (optional but good practice)
    required_columns = {
        "words": ["id", "lemma", "normalized_lemma", "language_code", "source_info"], # Add columns needed by get_or_create_word_id
        "pronunciations": ["id", "word_id", "type", "value", "sources"], # Columns for insert_pronunciation
        "etymologies": ["id", "word_id", "etymology_text", "sources"], # Columns for insert_etymology
        "definitions": ["id", "word_id", "definition_text", "standardized_pos_id", "sources"], # Columns for insert_definition
        "examples": ["id", "definition_id", "example"], # Simplified for this example
        "word_tags": ["id", "word_id", "tag", "source_identifier"],
        "relations": ["id", "from_word_id", "to_word_id", "relation_type", "sources"], # Columns for insert_relation
        "parts_of_speech": ["id", "code"], # Needed by get_standardized_pos_id
        "word_forms": ["id", "word_id", "form"] # Simplified example
    }
    table_column_map = {}
    try:
        # Use a temporary cursor for schema check if the main 'cur' might be invalid
        # Or better, perform this check outside this function, perhaps in migrate_data
        temp_conn = get_connection()
        with temp_conn.cursor() as temp_cur:
            for table, columns in required_columns.items():
                temp_cur.execute(f"SELECT column_name FROM information_schema.columns WHERE table_name = %s", (table,))
                table_column_map[table] = {row[0] for row in temp_cur.fetchall()}
                missing_columns = set(columns) - table_column_map[table]
                if missing_columns:
                     logger.warning(f"Schema Check: Table '{table}' is missing required columns: {', '.join(missing_columns)}. Some features might not work.")
        temp_conn.close()
    ...
    try:
        ...
        with tqdm(total=total_entries, desc=f"Processing {source_identifier}") as pbar:
            for entry_index, (word_key, entry_data) in enumerate(data.items()):
                ...
                # Pass copies of mutable objects if necessary (stats, error_types, relations_batch)
                # or update them based on return values if helper returns stats.
                # For simplicity here, we pass them directly, assuming helper modifies them.
                try:
                    # The helper function now requires a cursor argument because the decorator provides one
                    # We don't pass `cur` from here. The decorator handles it.
                    success = _process_single_tagalog_word_entry(
                        entry_data, # Remove cur argument
                        word_key,
                        source_identifier,
                        language_code,
                        add_etymology,
        ...
        if relations_batch:
            try:
                # Use a separate transaction for processing the relations batch
                # process_relations_batch needs to be adapted or called differently
                # Option 1: Adapt process_relations_batch to use @with_transaction
                # Option 2: Call it here and manage transaction manually (less ideal)
                # Let's assume process_relations_batch is adapted with @with_transaction

                # --- Simulate adapting process_relations_batch ---
                @with_db_transaction(commit=True)
                def _process_relations_batch_transactional(cur, batch, stats_dict, cache):
                    # Original logic of process_relations_batch, using the passed cursor
                    # ... (implementation based on original process_relations_batch) ...
                    # For now, just log and count
                    processed_count = 0
                    for relation_info in batch:
                         try:
                              # Simulate getting IDs and inserting (replace with actual logic)
                              from_id = relation_info['word_id']
                              related_word = relation_info['related_word']
                              rel_type = relation_info['rel_type'] # Now an enum
                              src_id = relation_info['source_identifier']
                              metadata = relation_info.get('metadata')

                              # Get/Create related word ID (using cache)
                              rel_word_cache_key = f"{related_word.lower()}|{language_code}" # Assuming same lang
                              related_word_id = cache.get(rel_word_cache_key)
                              if not related_word_id:
                                   # Note: get_or_create needs a cursor
                                   related_word_id = get_or_create_word_id(cur, related_word, language_code, src_id)
                                   if related_word_id:
                                        cache[rel_word_cache_key] = related_word_id

                              if related_word_id and related_word_id != from_id:
                                   # Insert relation (needs cursor)
                                   rel_id = insert_relation(cur, from_id, related_word_id, rel_type, src_id, metadata)
                                   if rel_id:
                                        processed_count += 1
                                        stats_dict["relations_added"] += 1 # Update stats
                                        # Add inverse/bidirectional if needed (using insert_relation)
                              ...
                              logger.warning(f"Failed processing relation: {relation_info} - Error: {rel_proc_err}")
                              # Decide if this is critical or not for the batch

                    logger.info(f"Processed {processed_count} relations in batch.")
                    return stats_dict # Return updated stats

                # Call the transactional batch processor
                stats = _process_relations_batch_transactional(relations_batch, stats, word_id_cache) # Remove cur
                ...
                stats["errors"] += len(relations_batch) # Count all relations in batch as errors
