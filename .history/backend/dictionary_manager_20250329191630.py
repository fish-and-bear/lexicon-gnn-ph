...
def process_kaikki_jsonl(cur, filename: str):
    ...
    RELATIONSHIP_TYPE_MAPPING = {
        'borrowed_from': 'DERIVED_FROM',  # Map borrowed_from to DERIVED_FROM
        'cognate_with': 'RELATED',        # Map cognate_with to RELATED
    }
            ...
            for form in entry.get('forms', []):
                ...
                if 'tags' in form:
                    ...
                    if any(tag in ['standard spelling', 'preferred', 'standard form'] for tag in tags):
                        rel_type = RelationshipType.SPELLING_VARIANT.value
                        ...
                        rel_type = RelationshipType.REGIONAL_VARIANT.value
                    ...
                    if template_name in ('bor', 'bor+') and '2' in args and '3' in args:
                        ...
                        try:
                            ...
                            insert_relation(
                                ...
                                word_id, 
                                source_word_id, 
                                get_standard_relationship_type('borrowed_from'), 
                    ...
                    # Process cognates
                        ...
                        try:    
                            ...
                            insert_relation(
                                ...
                                word_id, 
                                cog_word_id, 
                                get_standard_relationship_type('cognate_with'), 
                                sources=SourceStandardization.standardize_sources('kaikki.jsonl')
                            )
                        except Exception as e:
    ...
    # Main process entry function with safer transaction handling
    def process_entry(cur, entry: Dict, conn=None, entry_index=0):
        """Process a single dictionary entry."""
        if 'word' not in entry:
            logger.warning("Skipping entry without 'word' field")
            return None
            
        # Create a savepoint for this entry using entry_index instead of hash
        savepoint_name = f"sp_entry_{entry_index}"
        ...
        try:
            ...
            if 'senses' in entry:
                for sense in entry['senses']:
                    ...
                    # Check if insert_definition accepts metadata parameter
                    try:
                        # Try to inspect if the function has a metadata parameter
                        import inspect
                        sig = inspect.signature(insert_definition)
                        has_metadata_param = 'metadata' in sig.parameters
                    except Exception:
                        # If we can't check, assume it doesn't have the parameter
                        has_metadata_param = False
                    
                    # Insert the definition without metadata parameter
                    definition_id = insert_definition(
                        cur, 
                        word_id, 
                        definition_text, 
                    ...
                    if definition_id and metadata_dict and not has_metadata_param:
                        try:
            ...
            # Commit the savepoint and release it
            cur.execute(f"RELEASE SAVEPOINT {savepoint_name}")
            return word_id
        except Exception as e:
            # Log the error and rollback to the savepoint
            logger.error(f"Error processing entry {entry_index} ({entry.get('word', 'unknown')}): {str(e)}")
            try:
                # Rollback to the savepoint
                cur.execute(f"ROLLBACK TO SAVEPOINT {savepoint_name}")
                logger.debug(f"Rolled back transaction to savepoint {savepoint_name}")
            except Exception as rollback_error:
        ...
        with open(filename, 'r', encoding='utf-8') as f:
            ...
            for line in f:
                try:
                    ...
                    current_batch.append((entry, entry_count))
                    entry_count += 1
                    ...
                    if len(current_batch) >= batch_size:
                        ...
                        for entry_tuple in current_batch:
                            entry_data, idx = entry_tuple
                            if process_entry(cur, entry_data, conn, idx):
                                processed_in_batch += 1
                            else:
                                errors_in_batch += 1
                        
                        # Update stats
                        stats["processed_entries"] += processed_in_batch
                        stats["error_entries"] += errors_in_batch
                        
                        # Commit the batch if any entries were processed successfully
                        if processed_in_batch > 0:
                            try:
                                conn.commit()
                                logger.info(f"Committed batch. Processed {stats['processed_entries']} entries so far.")
                            except Exception as commit_error:
                                logger.error(f"Error committing batch: {str(commit_error)}")
                                conn.rollback()
                                logger.info("Rolled back batch due to commit error.")
                                # Revert the stats for this batch
                                stats["processed_entries"] -= processed_in_batch
                                stats["error_entries"] += processed_in_batch
                        else:
                            # If no entries were processed successfully, rollback
                            conn.rollback()
                            logger.warning(f"Rolled back batch - all {len(current_batch)} entries failed.")
            ...
            if current_batch:
                ...
                # Commit the final batch if any entries were processed successfully
                if processed_in_batch > 0:
                    try:
                        conn.commit()
                        logger.info(f"Committed final batch. Processed {stats['processed_entries']} entries in total.")
                    except Exception as commit_error:
                        logger.error(f"Error committing final batch: {str(commit_error)}")
                        conn.rollback()
                        logger.info("Rolled back final batch due to commit error.")
                        # Revert the stats for this batch
                        stats["processed_entries"] -= processed_in_batch
                        stats["error_entries"] += processed_in_batch
                else:
                    # If no entries were processed successfully, rollback
                    conn.rollback()
                    logger.warning(f"Rolled back final batch - all {len(current_batch)} entries failed.")
        
        logger.info(f"Kaikki processing complete. Stats: {stats}")
        return stats
    except Exception as e:
