...
class RelationshipAnalyzer:
    """Analyzes and processes word relationships."""
    ...
    def __init__(self):
        self.relationship_types = {
            ...
            'hypernym': {
                'weight': 0.9,
                'bidirectional': False,
                'description': 'More general term that includes the word'
            },
            'hyponym': {
                'weight': 0.9,
                'bidirectional': False,
                'description': 'More specific term included in the word'
            },
            'holonym': {
                'weight': 0.8,
                'bidirectional': False,
                'description': 'Term representing the whole of which the word is a part'
            },
            'meronym': {
                'weight': 0.8,
                'bidirectional': False,
                'description': 'Term representing a part of the word'
            },
            'derived': {
                'weight': 0.9,
                'bidirectional': False,
                'description': 'Word derived from another word'
            },
            'root': {
                'weight': 1.0,
                'bidirectional': False,
                'description': 'Root word from which others are derived'
            },
            'related': {
                'weight': 0.7,
                'bidirectional': True,
                'description': 'Generally related words'
            },
            'occurs_with': {
                'weight': 0.5,
                'bidirectional': True,
                'description': 'Words that commonly occur together'
            },
            'translates_to': {
                'weight': 0.8,
                'bidirectional': False,
            ...
            'inherited': {
                'weight': 1.0,
                'bidirectional': False,
                'description': 'Word inherited from an ancestor language'
            },
            'borrowed': {
                'weight': 0.9,
                'bidirectional': False,
                'description': 'Word borrowed from another language'
            },
            'cognate': {
                'weight': 0.8,
                'bidirectional': True,
                'description': 'Words sharing a common ancestor'
            }
        }
        
        self.semantic_patterns = {
            r'same as (\w+)': 'synonym',
            r'opposite of (\w+)': 'antonym',
            r'type of (\w+)': 'hyponym',
            r'derived from (\w+)': 'derived',
            r'root word: (\w+)': 'root',
            r'related to (\w+)': 'related',
            r'part of (\w+)': 'meronym',
            r'contains (\w+)': 'holonym',
            r'borrowed from (\w+)': 'borrowed',
            r'cognate with (\w+)': 'cognate'
        }
    
    def analyze_relationship(self, word1: str, word2: str, context: Optional[str] = None) -> Dict:
        """
        Analyze the relationship between two words.
        ...
        Returns:
            Dict containing relationship type and confidence score
        """
        # Normalize words
        ...
        similarity = self._calculate_similarity(w1, w2)
        
        # Determine relationship type
        rel_type = self._determine_relationship_type(w1, w2, context)
    ...
    def _determine_relationship_type(
        self, word1: str, word2: str, context: Optional[str]
    ) -> str:
        """Determine the type of relationship between words."""
        if not context:
            return 'related'
            
        # Check semantic patterns
        for pattern, rel_type in self.semantic_patterns.items():
            if re.search(pattern, context, re.IGNORECASE):
                return rel_type
        
        # Default to related if no pattern matches
        return 'related'
    
    def _calculate_confidence(
        self, rel_type: str, similarity: float, has_context: bool
    ) -> float:
        """Calculate confidence score for a relationship."""
        # Get base weight for relationship type
        base_weight = self.relationship_types[rel_type]['weight']
        
        # Adjust based on similarity
        similarity_factor = similarity * 0.3
        
        # Adjust based on context
        context_factor = 0.2 if has_context else 0
        
        # Calculate final confidence
        confidence = base_weight * (0.5 + similarity_factor + context_factor)
    ...
    def is_bidirectional(self, rel_type: str) -> bool:
        """Check if a relationship type is bidirectional."""
        return self.relationship_types.get(rel_type, {}).get('bidirectional', False)
    
    def get_weight(self, rel_type: str) -> float:
        """Get the weight for a relationship type."""
        return self.relationship_types.get(rel_type, {}).get('weight', 0.5)
