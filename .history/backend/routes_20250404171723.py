...
def get_word(word: str):
    ...
    try:
        ...
        if include_pronunciations:
            # Get pronunciations using the database column name 'metadata'
            ...
            for pron in pronunciations:
                result["pronunciations"].append({
                    ...
                    "value": pron.value,
                    "tags": pron.tags,
                    "metadata": pron.metadata  # Keep as 'metadata' in API for consistency
                })
...
def search():
    ...
    try:
        ...
        # Search by exact match first (most efficient)
        exact_matches = []
        normalized_query = normalize_lemma(query)
        
        # Check for Baybayin query and romanize if needed
        ...
        if has_baybayin:
            ...
            # Add Baybayin search condition
            baybayin_filter = Word.baybayin_form.ilike(f"%{query}%")
            # Also search with romanized form
            if romanized_query:
        ...
        if mode == "exact":
            base_query = base_query.filter(
                or_(
                    func.lower(Word.lemma) == func.lower(query),
                    Word.normalized_lemma == normalized_query
                )
            )
            
            # Add Baybayin exact match if needed
            if has_baybayin and baybayin_filter:
                base_query = base_query.filter(
                    or_(
                        func.lower(Word.lemma) == func.lower(query),
                        Word.normalized_lemma == normalized_query,
                        Word.baybayin_form == query
                    )
                )
        
        elif mode == "baybayin":
            # Only search in Baybayin fields
            if has_baybayin and baybayin_filter:
                base_query = base_query.filter(
                    or_(
                ...
                # If query is not Baybayin, search for words with Baybayin forms
                # that might match when romanized
                base_query = base_query.filter(Word.has_baybayin == True)
                base_query = base_query.filter(
                    or_(
                        func.lower(Word.lemma).ilike(f"%{query}%"),
            ...
            base_query = base_query.filter(
                or_(
                    Word.lemma.op("<->")(query) < (1.0 - similarity_threshold),
                    Word.normalized_lemma.op("<->")(normalized_query) < (1.0 - similarity_threshold)
                )
            )
            ...
            base_query = base_query.order_by(
                func.least(
                    Word.lemma.op("<->")(query),
                    Word.normalized_lemma.op("<->")(normalized_query)
                )
            )
            
        elif mode == "root":
            # Find root words only
            base_query = base_query.filter(
            ...
            # Find affixed words only
            base_query = base_query.filter(
                Word.root_word_id.isnot(None)
            ).filter(
                or_(
                    func.lower(Word.lemma).ilike(f"%{query}%"),
                    Word.normalized_lemma.ilike(f"%{normalized_query}%")
            ...
            # Search in etymologies
            etymology_query = db.session.query(Etymology.word_id).filter(
                or_(
            ...
            base_query = base_query.filter(
                or_(
                    func.lower(Word.lemma).ilike(f"%{query}%"),
                    Word.normalized_lemma.ilike(f"%{normalized_query}%"),
                    Word.baybayin_form.ilike(f"%{query}%") if has_baybayin else False,
                    Word.romanized_form.ilike(f"%{query}%")
                )
            )
        ...
        if pos:
            # Get definition IDs with the specified part of speech
            pos_definition_query = db.session.query(Definition.word_id).join(
            ...
            base_query = base_query.filter(
                Word.id.in_(pos_definition_query)
            )
        ...
        # Apply pagination
        words = base_query.order_by(Word.lemma).offset(offset).limit(limit).all()
        ...
        if include_full and word_ids:
            # Load full word data with relationships
            ...
            pronunciations_query = db.session.query(
                ...
                Pronunciation.value,
                Pronunciation.tags,
                Pronunciation.pronunciation_metadata.label('metadata'),  # Use model attribute, label as 'metadata'
