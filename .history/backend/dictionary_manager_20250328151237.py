...
def standardize_pos(pos: str) -> str:
    """
    Normalize the part-of-speech string based on a mapping.
    Returns the standardized Tagalog POS string.
    """
    if not pos:
        return ""
...
def get_or_create_word_id(cur, lemma: str, language_code: str = "tl", **kwargs) -> int:
    """
    Retrieves the word's ID (using a normalized lemma) or creates a new entry.
    """
    try:
        ...
        cur.execute("""
            ...
            RETURNING id
        """, (lemma, normalized, language_code, search_text))
        return cur.fetchone()[0]
    except Exception as e:
        logger.error(f"Error in get_or_create_word_id for '{lemma}': {e}")
        raise
...
def get_or_create_word_id(cur, lemma: str, language_code: str = "tl", **kwargs) -> int:
    """
    Retrieves the word's ID (using a normalized lemma) or creates a new entry.
    (Any extra keyword arguments—for example for Baybayin forms—can be added here.)
    """
    try:
        # Normalize the lemma (here simply lower-case and strip; extend as needed)
        normalized = lemma.lower().strip()
        cur.execute("""
            SELECT id FROM words
            WHERE normalized_lemma = %s AND language_code = %s
        ...
        if row:
            return row[0]
        # Prepare search_text by concatenating the original and normalized forms
        search_text = f"{lemma} {normalized}"
        cur.execute("""
            INSERT INTO words (lemma, normalized_lemma, language_code, search_text)
            VALUES (%s, %s, %s, to_tsvector('simple', %s))
    ...
    try:
        for i in range(0, len(entries), batch_size):
            ...
            for lemma, norm, lang in to_insert:
                try:
                    # Start a new transaction for each word
                    cur.execute("BEGIN")
                    ...
                    # Commit this word's transaction
                    cur.execute("COMMIT")
        ...
        logger.error(f"Error in batch_get_or_create_word_ids: {str(e)}")
        try:
            cur.connection.rollback()
        except Exception:
            pass  # Ignore rollback errors
...
def process_kwf_dictionary(cur, filename: str):
    ...
    for word, entry in data.items():
        ...
        if isinstance(definitions, dict):
            for pos, def_list in definitions.items():
                ...
                for def_entry in def_list:
                    ...
                    # Process regular definition
                    meaning = def_entry.get('meaning')
                    if meaning:
                        # Extract examples
                        examples = []
                        for example_set in def_entry.get('example_sets', []):
                            if isinstance(example_set, dict) and 'examples' in example_set:
                                for example in example_set['examples']:
                                    if isinstance(example, dict) and 'text' in example:
                                        examples.append(example['text'])
                        
                        # Clean HTML from meaning
                        plain_meaning = re.sub(r'<[^>]+>', ' ', meaning).strip()
                        
                        # Extract tags from categories
                        tags = None
                        if def_entry.get('categories'):
                            tags = {'categories': def_entry.get('categories')}
                            
                            # Add numbered definition marker if present
                            if def_entry.get('number') is not None:
                                tags['number'] = def_entry.get('number')
                        ...
                        # Process synonyms
                        if def_entry.get('synonyms'):
                            for syn in def_entry.get('synonyms'):
                                if isinstance(syn, dict) and 'term' in syn:
                                    # Clean HTML tags
                                    syn_term = re.sub(r'<[^>]+>', '', syn['term']).strip()
                                    if not syn_term:
                                        continue
                                        
                                    syn_id = get_or_create_word_id(cur, syn_term, language_code='tl')
                                    
                                    # Add metadata
                                    metadata = {
                                        'strength': 1.0,  # Strong relationship
                                        'context': syn.get('context', []),
                                        'html': syn['term'],  # Keep original HTML
                                        'definition_id': definition_id  # Link to specific definition
                                    }
                                    
                                    # Create bidirectional synonym relationship
                                    insert_relation(cur, word_id, syn_id, 'synonym', sources=standardized_source, metadata=metadata)
                                    insert_relation(cur, syn_id, word_id, 'synonym', sources=standardized_source, metadata=metadata)
                        
                        # Process cross-references
                        if def_entry.get('cross_references'):
                            for xref in def_entry.get('cross_references'):
                                if isinstance(xref, dict) and 'term' in xref:
                                    ...
                                    xref_id = get_or_create_word_id(cur, xref_term, language_code='tl')
                                    
                                    # Add metadata
                                    metadata = {
                                        'link': xref.get('link', ''),
                                        'broken': xref.get('broken', False),
                                        'definition_id': definition_id  # Link to specific definition
                                    }
                                    
                                    # Create bidirectional cross-reference relationship
                                    insert_relation(cur, word_id, xref_id, 'related', sources=standardized_source, metadata=metadata)
                                    insert_relation(cur, xref_id, word_id, 'related', sources=standardized_source, metadata=metadata)
                        
                        # Process antonyms
                        if def_entry.get('antonyms'):
                            for ant in def_entry.get('antonyms'):
                                if isinstance(ant, dict) and 'term' in ant:
                                    # Clean HTML tags
                                    ant_term = re.sub(r'<[^>]+>', '', ant['term']).strip()
                                    if not ant_term:
                                        continue
                                        
                                    ant_id = get_or_create_word_id(cur, ant_term, language_code='tl')
                                    
                                    # Add metadata
                                    metadata = {
                                        'context': ant.get('context', []),
                                        'html': ant['term'],  # Keep original HTML
                                        'definition_id': definition_id  # Link to specific definition
                                    }
                                    
                                    # Create bidirectional antonym relationship
                                    insert_relation(cur, word_id, ant_id, 'antonym', sources=standardized_source, metadata=metadata)
                                    insert_relation(cur, ant_id, word_id, 'antonym', sources=standardized_source, metadata=metadata)
        
        # Process related terms from the 'related' section
        if 'related' in entry and isinstance(entry['related'], dict):
            # Process examples (if not already captured in definitions)
            if 'examples' in entry['related'] and entry['related']['examples']:
                examples = entry['related']['examples']
                # We could store these as additional examples or tag them differently
                
            # Process related terms
            if 'related_terms' in entry['related'] and entry['related']['related_terms']:
                for rel_term in entry['related']['related_terms']:
                    if isinstance(rel_term, dict) and 'term' in rel_term:
                        # Clean HTML tags
                        term = re.sub(r'<[^>]+>', '', rel_term['term']).strip()
                        if not term:
                            continue
                        ...
                        metadata = {
                            ...
                            'relationship_source': 'related_terms_section'
                        }
                        
                        # Create bidirectional relationship
                        insert_relation(cur, word_id, rel_id, 'related', sources=standardized_source, metadata=metadata)
                        insert_relation(cur, rel_id, word_id, 'related', sources=standardized_source, metadata=metadata)
            
            # Process antonyms
            if 'antonyms' in entry['related'] and entry['related']['antonyms']:
                for ant_term in entry['related']['antonyms']:
                    if isinstance(ant_term, dict) and 'term' in ant_term:
                        # Clean HTML tags
                        term = re.sub(r'<[^>]+>', '', ant_term['term']).strip()
                        if not term:
                            continue
                            
                        ant_id = get_or_create_word_id(cur, term, language_code='tl')
                        
                        # Metadata for the relationship
                        metadata = {
                            'link': ant_term.get('link', ''),
                            'broken': ant_term.get('broken', False),
                            'relationship_source': 'antonyms_section'
                        }
        ...
        # Process affixation forms
        for affix_entry in entry.get('affixation', []):
            forms = affix_entry.get('form', [])
            types = affix_entry.get('types', [])
            
            for i, form in enumerate(forms):
                # Get the affixation type
                affix_type = types[i] if i < len(types) else "Unknown"
                
                # Create related word
                affixed_id = get_or_create_word_id(cur, form, language_code='tl')
                
                # Insert affixation relationship
                metadata = {
                    'affix_type_exact': affix_type,
                    'note': affix_entry.get('note')
                }
                
                # Map affixation types to standard types
                mapped_type = 'prefix'
                if affix_type.lower() == 'prefix':
                    mapped_type = 'prefix'
                elif affix_type.lower() == 'suffix':
                    mapped_type = 'suffix'
                elif affix_type.lower() == 'infix':
                    mapped_type = 'infix'
                else:
                    mapped_type = 'derivative'
                
                insert_affixation(
                    cur,
                    root_id=word_id,
                    affixed_id=affixed_id,
                    affix_type=mapped_type,
                    sources=standardized_source
                )
        ...
        # Process idioms
        for idiom_entry in entry.get('idioms', []):
            if not isinstance(idiom_entry, dict) or 'idiom' not in idiom_entry:
                continue
            ...
            # Create the idiom as a separate word
            idiom_id = get_or_create_word_id(cur, idiom_text, language_code='tl')
            
            # Get meaning
            meaning = ""
            if 'meaning' in idiom_entry and idiom_entry['meaning']:
                meaning = idiom_entry['meaning']
            elif 'meanings' in idiom_entry and idiom_entry['meanings']:
                for m in idiom_entry['meanings']:
                    if meaning:
                        meaning += " / "
                    meaning += m
            
            # Insert meaning as definition if available
            if meaning:
                insert_definition(
                    cur,
                    idiom_id,
            ...
            # Create relationship between word and idiom
            metadata = {
                'relationship_type': 'idiom',
                'source': 'kwf_idiom_section'
            }
            
            insert_relation(cur, word_id, idiom_id, 'idiom', sources=standardized_source, metadata=metadata)
    ...
    try:
        ...
        for lemma, entry_data in tqdm(data.items(), desc="Processing Tagalog words"):
            try:
                ...
                if 'senses' in entry_data and entry_data['senses']:
                    for sense in entry_data['senses']:
                        ...
                        definition_id = insert_definition(
                            ...
                            examples=json.dumps(examples) if examples else None,
                            tags=", ".join(sense_tags) if sense_tags else None,
                            sources=source
                        )
                        ...
                        if definition_id:
                            ...
                            # Process sense-level etymology if present
                            if 'etymology' in sense and sense['etymology'] and isinstance(sense['etymology'], dict):
                                sense_etymology = sense['etymology']
                                sense_etymology_text = sense_etymology.get('raw', '')
                                
                                # Extract structured etymology data for the sense
                                sense_etymology_structure = {
                                ...
                                if sense_etymology_text:
                                    sense_language_codes = ", ".join(sense_etymology.get('languages', []))
                                    
                                    # Store the etymology info in the definition's metadata
                                    etymology_metadata = {
                                        'text': sense_etymology_text,
                                        'structure': sense_etymology_structure,
                                        'language_codes': sense_language_codes
                                    }
                                    ...
                                    cur.execute("""
                                        ...
                                        WHERE id = %s
                                    """, (json.dumps(etymology_metadata), definition_id))
                            
                            # Process synonyms for this sense
                            if 'synonyms' in sense and sense['synonyms']:
                                for synonym in sense['synonyms']:
                                    ...
                                    # Create bidirectional synonym relationship
                                    metadata = {'source': 'tagalog-words.json', 'sense_id': definition_id}
                                    
                                    # From current word to synonym
                                    insert_relation(cur, word_id, synonym_id, "synonym", sources=source, metadata=metadata)
                                    # From synonym to current word (unless it's the same word)
                                    if word_id != synonym_id:
                                        insert_relation(cur, synonym_id, word_id, "synonym", sources=source, metadata=metadata)
                                        synonyms_added += 2
                                    else:
                                        synonyms_added += 1
                                    
                                    # Also link this synonym to the specific definition
                                    insert_definition_relation(cur, definition_id, synonym_id, "synonym", sources=source)
                            
                            # Process references for this sense
                            if 'references' in sense and sense['references']:
                                for reference in sense['references']:
                                    if not reference:
                                        continue
                                    
                                    # Get or create the reference word
                                    reference_id = get_or_create_word_id(cur, reference, language_code=language_code)
                                    
                                    # Create "see_also" relationship
                                    metadata = {'source': 'tagalog-words.json', 'sense_id': definition_id}
                                    
                                    # From current word to reference
                                    insert_relation(cur, word_id, reference_id, "see_also", sources=source, metadata=metadata)
                                    references_added += 1
                                    
                                    # Also link this reference to the specific definition
                                    insert_definition_relation(cur, definition_id, reference_id, "see_also", sources=source)
                            
                            # Process variants for this sense
                            if 'variants' in sense and sense['variants']:
                                for variant in sense['variants']:
                                    if not variant:
                                        continue
                                    
                                    # Check if the variant contains a reference to another term (like "term Cf OTHER")
                                    cf_match = re.search(r'(.*?)\s+Cf\s+(.*)', variant)
                                    if cf_match:
                                        variant_term = cf_match.group(1).strip()
                                        reference_term = cf_match.group(2).strip()
                                        
                                        # Process the variant term
                                        variant_id = get_or_create_word_id(cur, variant_term, language_code=language_code)
                                        
                                        # Create variant relationship
                                        metadata = {'source': 'tagalog-words.json', 'sense_id': definition_id}
                                        
                                        # From current word to variant
                                        insert_relation(cur, word_id, variant_id, "variant", sources=source, metadata=metadata)
                                        # From variant to current word
                                        insert_relation(cur, variant_id, word_id, "variant", sources=source, metadata=metadata)
                                        variants_added += 2
                                        
                                        # Also process the referenced term if it exists
                                        if reference_term:
                                            ref_id = get_or_create_word_id(cur, reference_term, language_code=language_code)
                                            
                                            # Create relationship from variant to reference
                                            insert_relation(cur, variant_id, ref_id, "see_also", sources=source)
                                            # And from reference to variant
                                            insert_relation(cur, ref_id, variant_id, "see_also", sources=source)
                                            references_added += 2
                                    else:
                                        # Regular variant processing
                ...
                # Process derivatives if present
                if 'derivative' in entry_data and entry_data['derivative']:
                    derivatives = [d.strip() for d in entry_data['derivative'].split(',')]
                    for derivative in derivatives:
                        if not derivative:
                            continue
                        
                        # Skip derivatives that contain special characters for root markers
                        if '·' in derivative or '⋅' in derivative:
                            # These would need special handling - possibly via a dedicated function
                            continue
                            
                        derivative_id = get_or_create_word_id(cur, derivative, language_code=language_code)
                        
                        # Create bidirectional relationship
                        metadata = {'source': 'tagalog-words.json', 'type': 'derivative'}
                        
                        # From derivative to root
                        insert_relation(cur, derivative_id, word_id, "derived_from", sources=source, metadata=metadata)
                        # From root to derivative 
                        insert_relation(cur, word_id, derivative_id, "base_of", sources=source, metadata=metadata)
                        relations_added += 2
                
                processed_entries += 1
                
            except Exception as e:
                logger.error(f"Error processing entry for '{lemma}': {str(e)}")
                skipped_entries += 1
                continue
        
        logger.info(f"Processed {processed_entries} entries ({skipped_entries} skipped)")
        logger.info(f"Added {definitions_added} definitions")
        logger.info(f"Added {relations_added} root/derivative relations")
        logger.info(f"Added {synonyms_added} synonym relations")
        logger.info(f"Added {references_added} reference relations")
        logger.info(f"Added {variants_added} variant relations")
        ...
        logger.error(f"Error processing Tagalog words: {str(e)}")
...
def process_root_words_cleaned(cur, filename: str):
    """
    Processes root words from a JSON file with structure like:
    
      {
        "root_word1": {
            "variant1": {"type": "noun", "definition": "definition text..."},
            "variant2": {"type": "adjective", "definition": "definition text..."},
            ...
        },
        "root_word2": {
            "variantA": {"type": "verb", "definition": "definition text..."},
            ...
        },
        ...
      }
    
    For each base word we:
      - Get/create the base word entry.
      - For each variant:
          • If the variant is identical to the base, insert the definition into the base word.
          • Otherwise, create/retrieve the variant's word entry, insert its definition,
            and insert two relations: one from the variant to the base ("derived") and
            one from the base to the variant ("base_of").
    """
    logger.info(f"Processing root words from: {filename}")
    with open(filename, 'r', encoding='utf-8') as f:
        data = json.load(f)
        total_entries = len(data)
        logger.info(f"Found {total_entries} root word entries")
        
        # First create a list of all root words and variants
        all_words = []
        ...
        # Create word entries with progress tracking
        unique_words = list(set(all_words))
        logger.info(f"Creating {len(unique_words)} unique word entries...")
        with tqdm(total=len(unique_words), desc="Creating word entries") as pbar:
            for word in unique_words:
                try:
                    get_or_create_word_id(cur, word, language_code='tl')
                except Exception as e:
                    logger.error(f"Error creating word entry for '{word}': {str(e)}")
                finally:
                    pbar.update(1)
        
        # Process root words and their variants with progress tracking
        with tqdm(total=total_entries, desc="Processing root words") as pbar:
            for base_word, variants in data.items():
                if not isinstance(variants, dict):
                    logger.warning(f"Skipping invalid entry for root word {base_word}: expected dictionary for variants")
                    pbar.update(1)
                    continue

                try:
                    # Create or retrieve base word ID.
                    base_word_id = get_or_create_word_id(cur, base_word, language_code='tl')
                    
                    variant_count = 0
                    for variant, details in variants.items():
                        if not isinstance(details, dict):
                            logger.warning(f"Skipping invalid variant {variant} for root word {base_word}: details not a dict")
                            continue
                        ...
                        if not definition_text:
                            ...
                            continue
                        # Remove ellipsis (three or more dots) and trim.
                        definition_text = re.sub(r'\.{3,}', '', definition_text).strip()
                        pos = details.get("type", "")
                        # Use standardized part-of-speech (e.g. using our POS mapping function)
                        std_pos = standardize_pos(pos)
                        if variant == base_word:
                            insert_definition(
                                cur,
                            ...
                            try:
                                ...
                                # Insert relation: variant is derived from base.
                                insert_relation(
                                    cur,
                                    variant_id,
                                    base_word_id,
                                ...
                                # And inverse: base is the source for the variant.
                                insert_relation(
                                    cur,
                                    base_word_id,
                                    variant_id,
                                ...
                                logger.error(f"Error processing variant {variant} of root word {base_word}: {e}")
                    
                    logger.debug(f"Processed root word '{base_word}' with {variant_count} variants")
                except Exception as e:
                    logger.error(f"Error processing root word {base_word}: {e}")
                finally:
                    pbar.update(1)
...
def process_kaikki_jsonl(cur, filename: str):
    """
    Process Kaikki.org dictionary entries (both Tagalog and Cebuano) with
    ...
    def process_direct_relations(cur, word_id, entry, lang_code, source):
        """
        Process the direct relationship arrays in Kaikki entries: 
        synonyms, antonyms, derived, and related.
        """
    ...
    def process_relations(cur, from_word_id: int, relations_dict: Dict[str, List[str]], lang_code: str, source: str):
        ...
        then insert from_word_id -> to_word_id, plus inverse if bidirectional.
        """
    ...
    def extract_sense_relations(cur, word_id, sense, lang_code, source):
        """
        Extract relationships from within individual senses
        """
            ...
            for definition in definitions:
                ...
                if text:
                    ...
                    insert_definition(
                        ...
                        tags=json.dumps(tags) if tags else None,
                        sources=SourceStandardization.standardize_sources(os.path.basename(filename))
                    )
    ...
    with open(filename, 'r', encoding='utf-8') as f:
        for line in f:
            try:
                ...
                logger.error(f"JSON decode error on line: {str(je)}")
            except Exception as ex:
                logger.error(f"General error processing line: {str(ex)}")
