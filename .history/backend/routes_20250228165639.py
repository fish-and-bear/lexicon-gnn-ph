...
import re
from backend.caching import multi_level_cache, cache
...
def init_rate_limiter(app):
    ...
    try:
        ...
        logger.warning(f"Failed to connect to Redis: {str(e)}. Rate limiting will be disabled.")
        redis_client = None
...
def check_rate_limit(key: str, limit: int = 30, window: int = 60) -> Tuple[bool, Optional[int]]:
    ...
    try:
        ...
        logger.error(f"Redis error during rate limit check: {str(e)}")
        return True, None
    except Exception as e:
...
def after_request(response):
    ...
    if hasattr(g, 'start_time'):
        response_time = int((datetime.now(timezone.utc) - g.start_time).total_seconds() * 1000)
        ...
        result["data"]["definitions"] = [
            {
                ...
                "part_of_speech": {
                    ...
                    "description": d.standardized_pos.description if d.standardized_pos else None
                } if d.standardized_pos else None,
                "examples": parse_examples_field(d.examples),
                "usage_notes": parse_examples_field(d.usage_notes),
...
def parse_examples_field(field_data):
    """Parse examples or usage notes fields that could be JSON or plain text."""
    if not field_data:
        return []
    ...
    try:
        ...
        return [line.strip() for line in field_data.split('\n') if line.strip()]

def parse_components_field(components_data):
    """Parse etymology components that could be JSON or delimited text."""
    ...
    try:
        ...
        # If not valid JSON, split by comma or semicolon
            ...
            [
                {
                    ...
                    "pos_list": list(set(d.standardized_pos.code for d in w.definitions if d.standardized_pos))
                }
                for w in words
            ],
            meta={
