...
@with_transaction(commit=True)
def process_kwf_dictionary(cur, filename: str, progress: Progress, task_id: TaskID):
    """
    ...
    try:
        ...
        # --- Process each entry ---
        progress.update(task_id, total=stats["total_entries"])
        for entry_index, word, entry in entries_iterator:
            ...
            try:
                        ...
                        for def_idx, def_item in enumerate(definitions_list):
                            ...
                            if normalized_examples:
                             ...
                             if definition_text:
                                 try:
                                     definition_id = insert_definition(
                                         cur, word_id, definition_text, source_identifier,
                                         examples=examples_json,
                                         usage_notes=usage_notes,
                                         tags=tags,
                                         metadata=def_metadata_dict
                                     )
                                     if definition_id: stats["definitions_added"] += 1
                                 except Exception as def_error:
                                     logger.warning(f"Error inserting definition for '{formatted_word}': {definition_text[:50]}... Error: {def_error}")
                                     error_types[f"DefinitionError: {def_error}"] = error_types.get(f"DefinitionError: {def_error}", 0) + 1

                # --- Process Relations ---
                # KWF structure: "related_terms": { "relation_type": [ {term details}, ... ], ... }
                related_terms = entry.get("related_terms", {})
                if isinstance(related_terms, dict):
                    for rel_type, rel_terms_list in related_terms.items():
                        if not isinstance(rel_terms_list, list): continue

                        for rel_idx, rel_term_data in enumerate(rel_terms_list):
                            # Check if rel_term_data is a dictionary
                            if not isinstance(rel_term_data, dict):
                                logger.warning(f"Skipping invalid related term item (not a dict) for '{formatted_word}', relation type '{rel_type}', index {rel_idx}")
                                continue

                            rel_term = rel_term_data.get("term", "").strip()
                            if not rel_term: continue # Skip if no related term

                            rel_type_mapped = RELATION_TYPE_MAPPING.get(rel_type, RelationshipType.RELATED)
                            rel_metadata = {"source_file": raw_source_identifier, "original_relation_type": rel_type}
                            rel_notes = rel_term_data.get("note") # KWF uses "note"
                            if rel_notes: rel_metadata["notes"] = rel_notes

                            # Insert the relation
                            try:
                                rel_id = insert_relation(
                                    cur, word_id, rel_term, rel_type_mapped, source_identifier,
                                    metadata=rel_metadata
                                )
                                if rel_id: stats["relations_added"] += 1
                            except Exception as rel_error:
                                logger.warning(f"Error inserting relation for '{formatted_word}': {rel_term[:50]}... Error: {rel_error}")
                                error_types[f"RelationError: {rel_error}"] = error_types.get(f"RelationError: {rel_error}", 0) + 1
                ...
                try:
                    ...
                    logger.error(f"Error committing entry '{formatted_word}': {commit_error}")
                    stats["error_entries"] += 1
                    error_types[f"CommitError: {commit_error}"] = error_types.get(f"CommitError: {commit_error}", 0) + 1
                    progress.update(task_id, advance=1)

            except Exception as entry_error:
                logger.error(f"Error processing entry '{word}': {entry_error}", exc_info=True)
                stats["error_entries"] += 1
                error_types[f"EntryError: {entry_error}"] = error_types.get(f"EntryError: {entry_error}", 0) + 1
                try:
                    cur.execute(f"ROLLBACK TO SAVEPOINT {savepoint_name}")
                except Exception as rollback_error:
                    logger.error(f"Failed to rollback to savepoint {savepoint_name} for entry '{word}': {rollback_error}")
                progress.update(task_id, advance=1)
        ...
        try:
            ...
            logger.error(f"Error committing final transaction: {final_commit_error}")
            stats["error_entries"] += 1
            error_types[f"FinalCommitError: {final_commit_error}"] = error_types.get(f"FinalCommitError: {final_commit_error}", 0) + 1
            conn.rollback()
