...
CREATE TABLE IF NOT EXISTS words (
    ...
    data_hash TEXT,
    -- Columns previously added via ALTER TABLE are now defined directly:
    badlit_form TEXT,
    hyphenation JSONB,
    is_proper_noun BOOLEAN DEFAULT FALSE,
    is_abbreviation BOOLEAN DEFAULT FALSE,
    is_initialism BOOLEAN DEFAULT FALSE,
    search_text TSVECTOR, -- Added search_text directly
    -- End added columns
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
...
-- Removed DO block for adding search_text, handled above now.
-- Consider a separate step or trigger for *populating* search_text if needed on existing data.
...
CREATE TABLE IF NOT EXISTS definitions (
    ...
    usage_notes TEXT,
    tags TEXT,
    sources TEXT, -- Kept as possibly NULL, adjust if needed
    metadata JSONB, -- Defined directly here
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS etymologies (
    ...
    language_codes TEXT,
    sources TEXT, -- Kept as possibly NULL
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
...
-- REMOVED the DO block that added columns, as they are now in CREATE TABLE statements.
...
-- Update existing definition_relations rows to have a value for updated_at (already present, but good practice)
-- UPDATE definition_relations SET updated_at = created_at WHERE updated_at IS NULL;
-- Commented out the final UPDATE as it was primarily for the ALTER TABLE scenario; less critical now.
...
@with_transaction(commit=False) # Keep commit=False
def process_kaikki_jsonl(cur, filename: str):
    ...
    raw_source_identifier = os.path.basename(filename) # Keep raw version for metadata
    ...
    conn = cur.connection # Get connection for savepoint management

    # --- Start Helper Function Definitions (Nested for clarity) ---

    RELATIONSHIP_TYPE_MAPPING = { # Define mapping locally if not global
        "borrowed_from": RelationshipType.DERIVED_FROM.rel_value,
        "cognate_with": RelationshipType.RELATED.rel_value,
    }

    def get_standard_relationship_type(rel_type):
         if rel_type in RELATIONSHIP_TYPE_MAPPING: return RELATIONSHIP_TYPE_MAPPING[rel_type]
         try: RelationshipType(rel_type); return rel_type
         except ValueError:
             for rel in RelationshipType:
                 if rel.rel_value == rel_type: return rel_type
         logger.warning(f"Unknown relationship type: {rel_type}, using RELATED as fallback")
         return RelationshipType.RELATED.rel_value

    def extract_baybayin_info(entry: Dict) -> Tuple[Optional[str], Optional[str]]:
        return extract_script_info(entry, "Baybayin", "Baybayin") # Assumes extract_script_info is defined

    def extract_badlit_info(entry: Dict) -> Tuple[Optional[str], Optional[str]]:
        return extract_script_info(entry, "Badlit", "Badlit") # Assumes extract_script_info is defined
    ...
    def standardize_entry_pos(pos_str: str) -> str:
        if not pos_str: return "unc"
        pos_key = pos_str.lower().strip()
        return get_standard_code(pos_key) # Assumes get_standard_code is defined globally

    def process_pronunciation(cur, word_id: int, entry: Dict, source_identifier: str) -> int:
         count = 0
         if "sounds" not in entry: return count
         for sound in entry.get("sounds", []):
              if not isinstance(sound, dict): continue
              pron_data = sound.copy(); pron_type, pron_value = None, None
              if "ipa" in sound and sound["ipa"]: pron_type, pron_value = "ipa", sound["ipa"]
              elif "enpr" in sound and sound["enpr"]: pron_type, pron_value = "enpr", sound["enpr"]
              elif "rhymes" in sound and sound["rhymes"]: pron_type, pron_value = "rhyme", sound["rhymes"]
              elif "audio" in sound and sound["audio"]: pron_type, pron_value = "audio", sound["audio"]
              else: continue
              if pron_type and pron_value:
                  try:
                      pron_data["type"], pron_data["value"] = pron_type, pron_value
                      pron_id = insert_pronunciation(cur, word_id, pron_data, source_identifier) # Assumes insert_pronunciation exists
                      if pron_id: count += 1
                  except Exception as e: logger.error(f"Error inserting pronunciation for word ID {word_id} (Type: {pron_type}): {str(e)}")
         return count

    def process_etymology(cur, word_id: int, etymology_text: str, source_identifier: str, etymology_templates: List[Dict] = None, word_lemma: str = None, language_code: str = None) -> Tuple[Optional[int], int]:
         if not etymology_text or not isinstance(etymology_text, str): return None, 0
         ety_id, relations_added = None, 0; language_codes = None
         if etymology_templates and isinstance(etymology_templates, list):
              languages = set()
              for template in etymology_templates:
                   if not isinstance(template, dict) or "name" not in template: continue
                   template_name = template["name"].lower(); args = template.get("args", {})
                   if template_name in ["derived", "borrowing", "derived from", "borrowed from", "bor", "der", "bor+"]:
                       rel_type = RelationshipType.DERIVED_FROM; source_lang = args.get("1", "") or args.get("2", ""); source_word = args.get("2", "") or args.get("3", "")
                       if source_word and isinstance(source_word, str):
                           try:
                               source_word_clean = clean_html(source_word)
                               if source_word_clean and source_word_clean.lower() != word_lemma.lower():
                                    source_word_lang = source_lang if source_lang else language_code
                                    related_word_id = get_or_create_word_id(cur, source_word_clean, source_word_lang, source_identifier)
                                    if related_word_id and related_word_id != word_id:
                                        metadata = {"from_etymology": True, "template": template_name, "confidence": rel_type.strength}
                                        rel_id = insert_relation(cur, word_id, related_word_id, rel_type, source_identifier, metadata) # Assumes insert_relation exists
                                        if rel_id: relations_added += 1
                                        inverse_rel = rel_type.get_inverse()
                                        if inverse_rel: insert_relation(cur, related_word_id, word_id, inverse_rel, source_identifier, metadata)
                           except Exception as rel_err: logger.warning(f"Error creating etymology relation from template for word ID {word_id}: {rel_err}")
                   for key, value in args.items():
                       if isinstance(value, str) and 1 < len(value) <= 3 and value.isalpha(): languages.add(value.lower())
                       elif isinstance(key, str) and 1 < len(key) <= 3 and key.isalpha(): languages.add(key.lower())
              if languages: language_codes = ",".join(sorted(list(languages)))
         try:
             cleaned_text = clean_html(etymology_text.strip())
             ety_id = insert_etymology(cur, word_id=word_id, etymology_text=cleaned_text, source_identifier=source_identifier, language_codes=language_codes) # Assumes insert_etymology exists
             return ety_id, relations_added
         except Exception as e: logger.error(f"Error saving etymology for word ID {word_id} from source '{source_identifier}': {str(e)}"); return None, relations_added

    def process_sense_relationships(cur, word_id: int, sense: Dict, language_code: str, source_identifier: str, word_lemma: str = None) -> int:
         relations_added = 0
         relation_types = { "synonyms": RelationshipType.SYNONYM, "antonyms": RelationshipType.ANTONYM, "hypernyms": RelationshipType.HYPERNYM, "hyponyms": RelationshipType.HYPONYM, "holonyms": RelationshipType.HOLONYM, "meronyms": RelationshipType.MERONYM, "derived": RelationshipType.ROOT_OF, "related": RelationshipType.RELATED, "coordinate_terms": RelationshipType.RELATED, "see_also": RelationshipType.SEE_ALSO }
         for rel_key, rel_enum in relation_types.items():
              if rel_key in sense and isinstance(sense[rel_key], list):
                  for item in sense[rel_key]:
                      related_word, metadata = None, {"from_sense": True, "confidence": rel_enum.strength}
                      if isinstance(item, dict) and "word" in item: related_word = item["word"]; # Extract metadata if needed
                      elif isinstance(item, str): related_word = item
                      else: continue
                      related_word_clean = clean_html(related_word);
                      if not related_word_clean or (word_lemma and related_word_clean.lower() == word_lemma.lower()): continue
                      try:
                           related_word_id = get_or_create_word_id(cur, related_word_clean, language_code, source_identifier=source_identifier)
                           if related_word_id == word_id: continue
                           from_id, to_id, current_rel_enum = word_id, related_word_id, rel_enum
                           if rel_key == "hyponyms": from_id, to_id, current_rel_enum = related_word_id, word_id, RelationshipType.HYPONYM
                           elif rel_key == "hypernyms": current_rel_enum = RelationshipType.HYPONYM
                           rel_id = insert_relation(cur, from_id, to_id, current_rel_enum, source_identifier, metadata)
                           if rel_id: relations_added += 1
                           if current_rel_enum.bidirectional: insert_relation(cur, to_id, from_id, current_rel_enum, source_identifier, metadata)
                           else:
                               inverse_rel = current_rel_enum.get_inverse()
                               if inverse_rel: insert_relation(cur, to_id, from_id, inverse_rel, source_identifier, metadata)
                      except Exception as e: logger.error(f"Error processing {rel_key} relation for word ID {word_id} to '{related_word_clean}': {str(e)}")
         return relations_added

    def process_forms_storage(cur, word_id: int, entry: Dict, source_identifier: str) -> int:
         count = 0
         if "forms" in entry and isinstance(entry["forms"], list):
             for form_data in entry["forms"]:
                 if not isinstance(form_data, dict) or not form_data.get("form"): continue
                 form_text, form_tags = form_data["form"], form_data.get("tags", [])
                 if "Baybayin" in form_tags or "Badlit" in form_tags: continue
                 tags_json = Json(form_tags) if form_tags else None
                 is_canonical, is_primary = "canonical" in form_tags, "primary" in form_tags
                 try:
                     cur.execute("""INSERT INTO word_forms (word_id, form, is_canonical, is_primary, tags) VALUES (%s, %s, %s, %s, %s) ON CONFLICT (word_id, form) DO UPDATE SET is_canonical = EXCLUDED.is_canonical, is_primary = EXCLUDED.is_primary, tags = COALESCE(word_forms.tags, '{}'::jsonb) || %s::jsonb, updated_at = CURRENT_TIMESTAMP""", (word_id, form_text, is_canonical, is_primary, tags_json, tags_json))
                     if cur.rowcount > 0: count += 1
                 except Exception as e: logger.warning(f"Error inserting word form '{form_text}' for {word_id}: {e}")
         return count

    def process_form_relationships(cur, word_id: int, entry: Dict, language_code: str, source_identifier: str) -> int:
         form_rels_added = 0
         if "forms" not in entry or not isinstance(entry["forms"], list): return form_rels_added
         canonical_forms = set(extract_canonical_forms(entry))
         for form_data in entry["forms"]:
              if not form_data or not isinstance(form_data, dict): continue
              form_word = form_data.get("form", "")
              if not form_word or not isinstance(form_word, str) or form_word == entry.get("word") or form_word in canonical_forms: continue
              try:
                  rel_type, metadata = RelationshipType.VARIANT, {"from_forms": True, "confidence": RelationshipType.VARIANT.strength}
                  tags = form_data.get("tags", [])
                  if tags and isinstance(tags, list):
                      metadata["tags"] = ",".join(tags)
                      if any(t in ["standard spelling", "preferred", "standard form", "canonical"] for t in tags): rel_type, metadata["confidence"] = RelationshipType.SPELLING_VARIANT, 95
                      elif any(t in ["alternative spelling", "alternate spelling", "alt form"] for t in tags): rel_type, metadata["confidence"] = RelationshipType.SPELLING_VARIANT, 90
                      elif any(t in ["regional", "dialectal", "dialect"] for t in tags): rel_type, metadata["confidence"] = RelationshipType.REGIONAL_VARIANT, 85
                      elif "obsolete" in tags: metadata["confidence"] = 70
                  if "qualifier" in form_data: metadata["qualifier"] = form_data["qualifier"]
                  form_word_id = get_or_create_word_id(cur, form_word, language_code, source_identifier=source_identifier)
                  if form_word_id == word_id: continue
                  rel_id = insert_relation(cur, word_id, form_word_id, rel_type, source_identifier, metadata)
                  if rel_id: form_rels_added += 1
                  if rel_type.bidirectional: insert_relation(cur, form_word_id, word_id, rel_type, source_identifier, metadata)
                  else:
                      inverse_rel = rel_type.get_inverse();
                      if inverse_rel: insert_relation(cur, form_word_id, word_id, inverse_rel, source_identifier, metadata)
              except Exception as e: logger.error(f"Error processing form relationship for word ID {word_id} to form '{form_word}': {str(e)}")
         return form_rels_added

    def process_head_templates(cur, word_id: int, templates: List[Dict]) -> int:
          count = 0
          if not isinstance(templates, list): return count
          for template in templates:
               if not isinstance(template, dict) or "name" not in template: continue
               template_name, args, expansion = template["name"], template.get("args"), template.get("expansion")
               try:
                   cur.execute("""INSERT INTO word_templates (word_id, template_name, args, expansion) VALUES (%s, %s, %s, %s) ON CONFLICT (word_id, template_name) DO UPDATE SET args = EXCLUDED.args, expansion = EXCLUDED.expansion, updated_at = CURRENT_TIMESTAMP""", (word_id, template_name, Json(args) if args else None, expansion,))
                   if cur.rowcount > 0: count += 1
               except Exception as e: logger.error(f"Error inserting/updating template '{template_name}' for word ID {word_id}: {str(e)}")
          return count

    def process_links(cur, definition_id: int, links: List[List[str]]) -> int:
         count = 0
         if not isinstance(links, list): return count
         for link in links:
             if not isinstance(link, list) or len(link) < 2: continue
             link_text, link_target = link[0], link[1]
             if not link_text or not link_target: continue
             is_wikipedia = ("wikipedia.org" in link_target.lower() or "w:" in link_text or "W:" in link_text)
             try:
                 cur.execute("""INSERT INTO definition_links (definition_id, link_text, link_target, is_wikipedia) VALUES (%s, %s, %s, %s) ON CONFLICT (definition_id, link_text, link_target) DO NOTHING""", (definition_id, link_text, link_target, is_wikipedia))
                 if cur.rowcount > 0: count += 1
             except Exception as e: logger.error(f"Error inserting link '{link_text}' -> '{link_target}' for definition ID {definition_id}: {str(e)}")
         return count

    def process_categories(cur, definition_id: int, categories: List[Dict]) -> int:
          count = 0
          if not isinstance(categories, list): return count
          for category in categories:
               if not isinstance(category, dict) or "name" not in category: continue
               category_name, category_kind, parents = category["name"], category.get("kind"), category.get("parents")
               try:
                   cur.execute("""INSERT INTO definition_categories (definition_id, category_name, category_kind, parents) VALUES (%s, %s, %s, %s) ON CONFLICT (definition_id, category_name) DO NOTHING""", (definition_id, category_name, category_kind, Json(parents) if parents else None))
                   if cur.rowcount > 0: count += 1
               except Exception as e: logger.error(f"Error inserting category '{category_name}' for definition ID {definition_id}: {str(e)}")
          return count

    # Process a single entry
    def process_entry(cur, entry: Dict, source_identifier: str) -> Tuple[Optional[int], Optional[str]]:
        """Process a single dictionary entry with improved resilience."""
        if "word" not in entry or not entry["word"]: return None, "No word field"
        word, word_id, error_messages = entry["word"], None, []
        local_stats = {k: 0 for k in entry_stats.keys()}
        try:
             pos = entry.get("pos", "unc"); language_code = entry.get("lang_code", DEFAULT_LANGUAGE_CODE)
             if not language_code or len(language_code) > 10: language_code = DEFAULT_LANGUAGE_CODE; logger.warning(f"Invalid/missing lang_code for '{word}', defaulting to '{DEFAULT_LANGUAGE_CODE}'.")
             baybayin_form, baybayin_romanized = extract_baybayin_info(entry); badlit_form, badlit_romanized = extract_badlit_info(entry)
             romanized_form = None
             # Use globals()['romanizer'] carefully or pass romanizer instance
             local_romanizer = globals().get('romanizer')
             if baybayin_form and not baybayin_romanized and local_romanizer and local_romanizer.validate_text(baybayin_form):
                 try: romanized_form = local_romanizer.romanize(baybayin_form); logger.debug(f"Generated romanization '{romanized_form}' for Baybayin '{baybayin_form}'")
                 except Exception as rom_err: logger.warning(f"Error generating romanization for '{word}' (Baybayin: {baybayin_form}): {rom_err}")

             if not romanized_form and baybayin_romanized: romanized_form = baybayin_romanized
             elif not romanized_form and badlit_romanized: romanized_form = badlit_romanized
             if baybayin_form or badlit_form: local_stats["scripts"] += 1
             is_proper_noun = entry.get("proper", False) or pos in ["prop", "proper noun", "name"]; is_abbreviation = pos in ["abbrev", "abbreviation"]; is_initialism = pos in ["init", "initialism", "acronym"]
             ...
             hyphenation = entry.get("hyphenation"); hyphenation_json = Json(hyphenation) if hyphenation and isinstance(hyphenation, list) else None
             word_tags_str = ",".join(tags_list) if tags_list else None
             word_metadata = {"source_file": os.path.basename(filename)} # Use raw filename here
             try:
                 word_id = get_or_create_word_id(cur, lemma=word, language_code=language_code, source_identifier=source_identifier, has_baybayin=bool(baybayin_form), baybayin_form=baybayin_form, romanized_form=romanized_form, badlit_form=badlit_form, hyphenation=hyphenation, is_proper_noun=is_proper_noun, is_abbreviation=is_abbreviation, is_initialism=is_initialism, tags=word_tags_str, word_metadata=word_metadata) # Pass dict directly now
                 if not word_id: raise ValueError("get_or_create_word_id returned None unexpectedly")
             except Exception as word_create_err: logger.error(f"CRITICAL FAILURE: Could not get/create word_id for '{word}' ({language_code}). Error: {word_create_err}. Skipping entry."); return None, f"Word creation failed: {word_create_err}"
             try: local_stats["pronunciations"] += process_pronunciation(cur, word_id, entry, source_identifier)
             except Exception as e: msg=f"Failed processing pronunciation for word ID {word_id}: {e}"; logger.warning(msg); error_messages.append(msg)
             try: local_stats["forms"] += process_forms_storage(cur, word_id, entry, source_identifier); local_stats["form_relations"] += process_form_relationships(cur, word_id, entry, language_code, source_identifier)
             except Exception as e: msg=f"Failed processing form relationships for word ID {word_id}: {e}"; logger.warning(msg); error_messages.append(msg)
             try:
                 if "etymology_text" in entry:
                     ety_id, ety_rels = process_etymology(cur, word_id, entry["etymology_text"], source_identifier, entry.get("etymology_templates"), word, language_code)
                     if ety_id: local_stats["etymologies"] += 1
                     if ety_rels: local_stats["ety_relations"] += ety_rels; local_stats["relations"] += ety_rels
             except Exception as e: msg=f"Failed processing etymology for word ID {word_id}: {e}"; logger.warning(msg); error_messages.append(msg)
             try:
                 if "head_templates" in entry and entry["head_templates"]: local_stats["templates"] += process_head_templates(cur, word_id, entry["head_templates"])
             except Exception as e: msg=f"Failed processing head templates for word ID {word_id}: {e}"; logger.warning(msg); error_messages.append(msg)
             ...
             if "senses" in entry and isinstance(entry["senses"], list):
                 for sense_idx, sense in enumerate(entry["senses"]):
                      ...
                      try:
                          ...
                          # Example processing
                          examples, examples_str = [], None
                          if "examples" in sense and isinstance(sense["examples"], list):
                              for example in sense["examples"]:
                                   ex_text = None
                                   if isinstance(example, str): ex_text = clean_html(example)
                                   elif isinstance(example, dict): ex_text = clean_html(example.get("text")) # Add ref/tr logic if needed
                                   if ex_text: examples.append(ex_text); local_stats["examples"] += 1
                          examples_str = "\n".join(examples) if examples else None
                          # Tags/Labels processing
                          sense_tags = sense.get("tags", []); sense_labels = sense.get("labels", [])
                          ...
                          # Usage notes processing
                          usage_notes = None;
                          if "usage_notes" in sense and sense["usage_notes"]: usage_notes = clean_html(sense["usage_notes"])
                          # Metadata processing
                          def_metadata = {}
                          for key in ["form_of", "raw_glosses", "topics", "taxonomy", "qualifier"]:
                               if key in sense and sense[key]: def_metadata[key] = sense[key]
                          # POS determination
                          sense_pos_str = sense.get("pos") or pos
                          # Insert Definition call (Ensure insert_definition is fixed!)
                          try:
                              definition_id = insert_definition(
                                  cur, word_id, definition_text,
                                  source_identifier=source_identifier,
                                  part_of_speech=sense_pos_str, # Pass original POS string
                                  examples=Json(examples) if examples else None, # Pass list wrapped in Json
                                  usage_notes=usage_notes,
                                  tags=tags_str,
                                  metadata=Json(def_metadata) if def_metadata else None # Pass dict wrapped in Json
                              )
                              if definition_id:
                                   sense_processed_count += 1; local_stats["definitions"] += 1
                                   # Process sub-data (categories, links, relations)
                                   try: local_stats["categories"] += process_categories(cur, definition_id, sense.get("categories", []))
                                   except Exception as e: msg=f"Failed processing categories for def ID {definition_id}: {e}"; logger.warning(msg); error_messages.append(msg)
                                   try: local_stats["links"] += process_links(cur, definition_id, sense.get("links", []))
                                   except Exception as e: msg=f"Failed processing links for def ID {definition_id}: {e}"; logger.warning(msg); error_messages.append(msg)
                                   try:
                                        sense_rels = process_sense_relationships(cur, word_id, sense, language_code, source_identifier, word)
                                        if sense_rels: local_stats["sense_relations"] += sense_rels; local_stats["relations"] += sense_rels
                                   except Exception as e: msg=f"Failed processing sense relationships for word ID {word_id}, def ID {definition_id}: {e}"; logger.warning(msg); error_messages.append(msg)
                                   # Check superseded spelling
                                   try: # Check superseded spelling
                                      superseded_match = re.search(r"Superseded.*?spelling of\s+([^\s\.,(]+)", definition_text, re.IGNORECASE)
                                      if superseded_match:
                                          correct_spelling = superseded_match.group(1).strip("()");
                                          if correct_spelling and correct_spelling.lower() != word.lower():
                                              try:
                                                  correct_word_id = get_or_create_word_id(cur, correct_spelling, language_code, source_identifier=source_identifier)
                                                  if correct_word_id != word_id:
                                                      meta = {"tag": "superseded", "definition_id": definition_id}
                                                      rel_id = insert_relation(cur, word_id, correct_word_id, RelationshipType.SPELLING_VARIANT, source_identifier, meta)
                                                      if rel_id: local_stats["relations"] += 1; logger.debug(f"Linked superseded spelling '{word}' to '{correct_spelling}'")
                                              except Exception as e: logger.warning(f"Error linking superseded spelling '{word}' to '{correct_spelling}': {e}")
                                   except Exception as e: msg=f"Failed checking superseded spelling in def ID {definition_id}: {e}"; logger.warning(msg); error_messages.append(msg)
                              else: logger.warning(f"insert_definition failed for '{word}' (ID: {word_id}), Def {sense_idx+1}. Check internal logs."); definition_id = None
                          except psycopg2.errors.UniqueViolation: logger.debug(f"Definition already exists for word ID {word_id}, sense {sense_idx}"); definition_id = None
                          except Exception as def_error: msg=f"Failed inserting definition for word ID {word_id}, sense {sense_idx}: {def_error}"; logger.warning(msg); error_messages.append(msg); definition_id = None
                      except Exception as sense_proc_err: msg=f"Error processing sense {sense_idx} for word ID {word_id} before definition insert: {sense_proc_err}"; logger.warning(msg); error_messages.append(msg); continue
             if sense_processed_count == 0 and not critical_error_occurred: logger.info(f"No definitions successfully inserted/processed for word '{word}' (ID: {word_id}).")
             try: # Top-level relations
                 ...
                 for rel_key, rel_enum in top_level_rels.items():
                      if rel_key in entry and isinstance(entry[rel_key], list):
                          for item in entry[rel_key]:
                              if isinstance(item, dict) and "word" in item: related_word = item["word"]
                              elif isinstance(item, str): related_word = item
                              else: continue
                              related_word_clean = clean_html(related_word);
                              if not related_word_clean or related_word_clean.lower() == word.lower(): continue
                              try:
                                  related_word_id = get_or_create_word_id(cur, related_word_clean, language_code, source_identifier=source_identifier)
                                  if related_word_id == word_id: continue
                                  metadata = {"confidence": rel_enum.strength, "from_sense": False}; # Add metadata from dict if exists
                                  if isinstance(item, dict) and "tags" in item: metadata["tags"] = ",".join(item["tags"])
                                  rel_id = insert_relation(cur, word_id, related_word_id, rel_enum, source_identifier, metadata)
                                  if rel_id: local_stats["relations"] += 1
                              ...
                              except Exception as top_rel_err_inner: msg=f"Failed processing top-level {rel_key} relation for word ID {word_id} -> '{related_word_clean}': {top_rel_err_inner}"; logger.warning(msg); error_messages.append(msg)
             except Exception as e: msg=f"Failed processing top-level relationships section for word ID {word_id}: {e}"; logger.warning(msg); error_messages.append(msg)
             for key, value in local_stats.items(): entry_stats[key] += value
             return word_id, "; ".join(error_messages) if error_messages else None
        except Exception as e: logger.error(f"UNEXPECTED EXCEPTION during processing entry for word '{word}' (ID: {word_id}): {str(e)}", exc_info=True); return word_id if word_id else None, f"Unhandled entry exception: {e}"


    # Main function processing logic
    stats = {"total_entries": (total_lines if total_lines != -1 else 0), "processed_ok": 0, "processed_with_errors": 0, "failed_entries": 0, "skipped_json_errors": 0}
    error_summary = {}
    romanizer = None # Initialize romanizer variable
    try:
        # Check if the class is defined before initializing
    ...
    if conn.closed: logger.error("Database connection is closed before starting Kaikki processing loop."); stats["failed_entries"] = stats["total_entries"]; return stats
    ...
    try:
        with open(filename, "r", encoding="utf-8") as f:
            ...
            for line in f:
                ...
                try:
                    ...
                    stats["failed_entries"] = (stats["total_entries"] - entry_count + 1);
                    if progress_bar: progress_bar.close()
                    # *** IMPORTANT: Need to rollback the main transaction if savepoint fails severely ***
                    try: conn.rollback(); logger.info("Rolled back main transaction due to savepoint error.")
                    except Exception as rb_err: logger.error(f"Failed to rollback main transaction after savepoint error: {rb_err}")
                    return stats # Stop processing

                try:
                    entry = json.loads(line.strip())
                    word_id, entry_errors = process_entry(cur, entry, source_identifier)

                    if word_id:
                        if entry_errors:
                            stats["processed_with_errors"] += 1; first_error = entry_errors.split(";")[0][:100]; err_key = f"EntryError: {first_error}"; error_summary[err_key] = error_summary.get(err_key, 0) + 1
                            logger.debug(f"Entry {entry_count} ('{entry.get('word', 'N/A')}') processed with errors: {entry_errors}")
                        else:
                            stats["processed_ok"] += 1
                        cur.execute(f"RELEASE SAVEPOINT {savepoint_name}")
                    else:
                        stats["failed_entries"] += 1; reason = entry_errors or "Unknown critical failure"; err_key = f"EntryFailedCritically: {reason[:100]}"; error_summary[err_key] = error_summary.get(err_key, 0) + 1
                        logger.warning(f"Entry {entry_count} ('{entry.get('word', 'N/A')}') failed critical processing step: {reason}")
                        cur.execute(f"ROLLBACK TO SAVEPOINT {savepoint_name}")

                except json.JSONDecodeError:
                    logger.warning(f"Invalid JSON on line {entry_count}"); stats["skipped_json_errors"] += 1
                    try: cur.execute(f"ROLLBACK TO SAVEPOINT {savepoint_name}")
                    except Exception: pass # Ignore error if savepoint didn't exist or transaction aborted
                except Exception as e:
                    logger.error(f"Unhandled error in main loop at line {entry_count}: {str(e)}", exc_info=True)
                    stats["failed_entries"] += 1; err_key = f"LoopException: {type(e).__name__}"; error_summary[err_key] = error_summary.get(err_key, 0) + 1
                    try: cur.execute(f"ROLLBACK TO SAVEPOINT {savepoint_name}")
                    except Exception as rb_err: logger.error(f"Error rolling back savepoint {savepoint_name} after loop exception: {rb_err}")
        ...
        for key, value in entry_stats.items(): logger.info(f"  {key.replace('_', ' ').title()}: {value}")
        if error_summary: logger.warning(f"Error summary for {filename}: {json.dumps(error_summary, indent=2)}")
        ...
        return {
            ...
            "detail_stats": entry_stats # Include detailed stats
        }

    except FileNotFoundError:
        logger.error(f"File not found error during main processing block: {filename}")
        return {"total_entries": stats.get("total_entries", 0), "processed_entries": 0, "error_entries": 1, "skipped_entries": 0, "detail_stats": entry_stats}
    except Exception as e:
        logger.error(f"Fatal error during processing of {filename}: {str(e)}", exc_info=True)
        # Outer transaction handled by migrate_data decorator or context
        stats["failed_entries"] = (stats.get("total_entries", 0) - stats["processed_ok"] - stats["processed_with_errors"] - stats["skipped_json_errors"]) if stats.get("total_entries", 0) > 0 else 1
        # Raise the exception to ensure the outer context knows about the failure and rolls back
        raise e # Re-raise the fatal error

    # Note: This final return is unlikely to be reached if an exception occurs and is raised
    # return { ... } # Included for completeness, but raising error is better


# --- End of process_kaikki_jsonl function ---
