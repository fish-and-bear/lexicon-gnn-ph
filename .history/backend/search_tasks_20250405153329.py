...
# Thread-local storage for tasks
...
def log_search_query(
    ...
    """
    Log a search query, either synchronously or asynchronously.
    
    Args:
        query: The search query text
        language: Optional language filter
        word_id: Optional word ID if a specific word was selected
        query_type: Type of query (search, suggestion, selection)
    """
    try:
        # Basic logging only - in a real app this would store to database
        logger.info(f"Search query: {query} ({query_type}), language: {language}, word_id: {word_id}")
        
        # In a production system, we'd save this data for analytics
        ...
        # db.session.execute("""
        #     INSERT INTO search_logs (query, language, word_id, query_type, timestamp)
        #     VALUES (:query, :language, :word_id, :type, :timestamp)
        # """, search_data)
        # db.session.commit()
        
    except Exception as e:
        logger.error(f"Error logging search query: {e}")
        # Don't re-raise - this is a background task that shouldn't break the user experience
