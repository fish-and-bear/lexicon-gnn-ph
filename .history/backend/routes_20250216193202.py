...
def handle_error(error):
    ...
    logger.error(f"Unhandled error: {error_info}", exc_info=True)
    return error_response("An unexpected error occurred", error_info=error_info)
...
@bp.route("/api/v2/words/<path:word>/related", methods=["GET"])
def get_word_relationships_with_limit(word):
    return get_word_relationships(word)

@limiter.limit("30 per minute")
@bp.route("/api/v2/words/<path:word>/etymology-tree", methods=["GET"])
def get_etymology_tree_with_limit(word):
    return get_etymology_tree(word)

# Add query optimization hints
def optimize_query(query):
    """Add optimization hints to complex queries."""
    return query.execution_options(
        postgresql_hint="SET enable_seqscan = off;",
        postgresql_hint="SET random_page_cost = 1.1;",
        postgresql_hint="SET effective_cache_size = '4GB';"
    )
