...
def process_kwf_dictionary(cur, filename: str):
    ...
    """
    # ... (Standardize source, setup stats, error_types - remains the same) ...

    # Get the connection object for transaction management
    conn = cur.connection

    try:
        # ... (Read file, check format, prepare iterator - remains the same) ...
        ...
        with tqdm(
            ...
            for entry_index, word, entry in entries_iterator:
                # ... (Skip metadata, invalid entries - remains the same) ...
                ...
                try:
                    ...
                    # ... (Handle savepoint error - remains the same) ...
                    continue

                try:
                    # ... (Extract formatted_word, language_code, baybayin, metadata - remains the same) ...
                    ...
                    # ... (Process Pronunciation, Etymology, Definitions by POS, Affixes, Relations - all internal logic remains the same) ...
                    # Ensure all calls within this block use the `cur` object passed in.
                    ...
                    # REMOVED Periodic Commit - No commit inside the loop

                # --- Error Handling for a Single Entry ---
                except (psycopg2.Error, ValueError, KeyError, Exception) as entry_error:
                    # ... (Error logging remains the same) ...
                    # Rollback to savepoint on error
                    try:
                        cur.execute(f"ROLLBACK TO SAVEPOINT {savepoint_name}")
                    except Exception as rb_err:
                        # ... (Critical rollback error handling - remains the same, includes full conn.rollback() and re-raise) ...
                        conn.rollback()
                        raise entry_error from rb_err

                finally:
                    pbar.update(1)

        # --- Final Commit (Outside the loop) ---
        # Only commit if the loop completes without critical errors that triggered a full rollback
        conn.commit() # ADDED final commit here
        logger.info(f"Final transaction commit successful for {filename} processing.")

    # --- Outer Exception Handling ---
    except Exception as outer_e:
        # ... (Outer error logging remains the same) ...
        # Ensure rollback on outer errors
        try:
        ...
        # Optionally re-raise outer_e if needed
...
def batch_get_or_create_word_ids(
    ...
    Ensures source_info is consistently updated using the helper function.
    ...
    """
    result = {}
    processed_keys = set() # Track keys processed in this batch
    ...
    if source:
        try:
            ...
            if not standardized_source:
                standardized_source = source # Fallback
                logger.debug(f"Source standardization failed for '{source}', using original")
        except Exception as e:
            logger.warning(f"Error standardizing source '{source}': {e}")
            standardized_source = source # Fallback

    for i in range(0, len(entries), batch_size):
        batch = entries[i : i + batch_size]
        # Remove duplicates within the current batch, keep first occurrence
        unique_batch_keys = []
        batch_keys_in_order = []
        for entry_key in batch:
            if entry_key not in unique_batch_keys:
                unique_batch_keys.append(entry_key)
            batch_keys_in_order.append(entry_key) # Keep original order for later processing if needed


        if not unique_batch_keys:
            continue  # Empty batch, skip

        # Prepare placeholders for fetching existing words
        normalized_map = {key: normalize_lemma(key[0]) for key in unique_batch_keys}
        placeholders = []
        flat_params = []
        for lemma, lang_code in unique_batch_keys:
            norm = normalized_map[(lemma, lang_code)]
            placeholders.append("(%s, %s)")
            flat_params.extend([norm, lang_code])

        # Fetch existing words for this batch
        existing = {} # Map (lemma, lang_code) -> (id, current_source_info)
        if placeholders:
            query = f"""
                SELECT lemma, language_code, id, source_info
                FROM words
                WHERE (normalized_lemma, language_code) IN ({', '.join(placeholders)})
            """
            try:
                cur.execute(query, flat_params)
                # Store results keyed by original (lemma, lang_code) for easier lookup
                raw_existing = cur.fetchall()
                # Need to map back from normalized form result to original lemma
                norm_to_orig = {(normalize_lemma(key[0]), key[1]): key for key in unique_batch_keys}
                for fetched_lemma, lang, word_id, current_source_info in raw_existing:
                     # Find the original key (lemma, lang_code) corresponding to this fetched result
                     # This assumes normalized_lemma + language_code is unique
                     original_key = norm_to_orig.get((normalize_lemma(fetched_lemma), lang))
                     if original_key:
                         existing[original_key] = (word_id, current_source_info)
                     else:
                        # This case might happen if the fetched lemma differs from the input lemma
                        # even after normalization (e.g., slight data drift). Log and handle.
                        logger.warning(f"Could not map fetched existing word '{fetched_lemma}' ({lang}) back to original batch key.")
                        # Try direct lookup in batch if needed, or skip updating this one.
                        # For simplicity, we might skip update if mapping fails.

            except Exception as e:
                logger.error(f"Error fetching existing words in batch: {str(e)}")
                # Continue with an empty 'existing' dict, will attempt to insert all

        # Update source info for existing words if source was provided
        updated_ids = set()
        if standardized_source and existing:
            for (lemma, lang), (word_id, current_source_info) in existing.items():
                try:
                    updated_source_json = update_word_source_info(
                        current_source_info, standardized_source
                    )
                    # Compare the new JSON string with the old dict/list representation
                    needs_update = False
                    try:
                        # Convert fetched JSONB/dict back to string for comparison if needed,
                        # or parse the new string to compare dicts. Comparing dicts is safer.
                        updated_source_dict = json.loads(updated_source_json)
                        if updated_source_dict != (current_source_info or {}):
                             needs_update = True
                    except (json.JSONDecodeError, TypeError):
                         # If parsing fails or comparison error, assume update is needed just in case
                         needs_update = True
                    ...
                    if needs_update:
                        ...
                        # logger.debug(f"Updated source_info for existing word '{lemma}' ({lang}, ID: {word_id})")
                except Exception as e:
                    logger.warning(f"Error updating source for existing word '{lemma}' (ID: {word_id}): {e}")

            if updated_ids:
                 logger.info(f"Updated source info for {len(updated_ids)} existing words in this batch from source '{standardized_source}'.")


        # Identify entries that need to be inserted
        to_insert = []
        for lemma, lang in unique_batch_keys: # Iterate over unique keys for insertion
            if (lemma, lang) not in existing:
                norm = normalized_map[(lemma, lang)]
                to_insert.append((lemma, norm, lang))

        # Insert missing entries
        # Using ON CONFLICT DO UPDATE is generally safer and handles race conditions
        # if multiple processes run concurrently (though less likely with batching).
        if to_insert:
            insert_values = []
            for lemma, norm, lang in to_insert:
                initial_source_json = update_word_source_info(None, standardized_source)
                search_text = f"{lemma} {norm}" # Base for tsvector
                # Ensure values are properly formatted, especially JSON
                insert_values.append((lemma, norm, lang, initial_source_json, search_text))

            # Use execute_values for potentially better performance with psycopg2,
            # or build a single INSERT statement with multiple VALUES clauses.
            # Using ON CONFLICT DO UPDATE to handle potential races or re-runs.
            # The DO UPDATE part also updates source_info if the row somehow exists already.
            try:
                 insert_query = """
                     INSERT INTO words (lemma, normalized_lemma, language_code, source_info, search_text)
                     VALUES %s
                     ON CONFLICT (normalized_lemma, language_code)
                     DO UPDATE SET
                         lemma = EXCLUDED.lemma, -- Keep the potentially newer lemma spelling
                         source_info = words.source_info || EXCLUDED.source_info, -- Attempt to merge JSONB sources
                         search_text = to_tsvector('simple', EXCLUDED.lemma || ' ' || EXCLUDED.normalized_lemma),
                         updated_at = CURRENT_TIMESTAMP
                    RETURNING lemma, language_code, id, source_info -- Return needed info to update result dict
                 """
                 # Note: Merging JSONB with || might require careful handling depending on desired merge logic.
                 # Consider calling update_word_source_info again in the UPDATE part if complex merging needed.

                 # Psycopg2 Extras for efficient batch insert
                 ...
                 execute_values(cur,
                     ...
                     DO UPDATE SET
                         ...
                         -- Here we might need a smarter merge than just ||
                         -- Fetching and using update_word_source_info in update is safer but slower.
                         -- For simplicity, we overwrite with the new source info for now, assuming batch source is definitive.
                         source_info = EXCLUDED.source_info,
                         search_text = to_tsvector('simple', EXCLUDED.lemma || ' ' || EXCLUDED.normalized_lemma),
                         updated_at = CURRENT_TIMESTAMP
                     RETURNING lemma, language_code, id, source_info
                     """,
                     insert_values,
                     template="(%s, %s, %s, %s::jsonb, to_tsvector('simple', %s))", # Cast source_info to jsonb
                     fetch=True # Fetch the RETURNING results
                 )

                 inserted_results = cur.fetchall()
                 logger.info(f"Inserted {len(inserted_results)} new words in this batch from source '{standardized_source}'.")

                 # Update the 'existing' dict with newly inserted words
                 # Map back using normalized form + lang code to original key
                 for fetched_lemma, lang, word_id, final_source_info in inserted_results:
                    # Find the original key
                    original_key = norm_to_orig.get((normalize_lemma(fetched_lemma), lang))
                    if original_key:
                        existing[original_key] = (word_id, final_source_info)
                    else:
                        logger.warning(f"Could not map inserted word '{fetched_lemma}' ({lang}) back to original batch key.")

            except Exception as e:
                 logger.error(f"Error during batch insert/update: {str(e)}", exc_info=True)
                 # If batch insert fails, we might lose track of which specific words failed.
                 # Consider reverting to individual inserts in case of batch failure for robustness.

        # Update the main result dictionary with IDs from this batch
        # Only add keys that were part of the *original* batch_keys_in_order requested
        for key in batch_keys_in_order:
             if key in existing and key not in processed_keys:
                 result[key] = existing[key][0] # Store only the ID
                 processed_keys.add(key) # Mark as processed to avoid duplicates if key appears in later batches
             elif key not in existing and key not in processed_keys:
                 # This means the word was not found and could not be inserted/updated. Log it.
                 logger.error(f"Failed to obtain word ID for batch entry: {key} from source '{standardized_source}'")
                ...
                try:
                            ...
                            for rel_idx, rel_item in enumerate(rel_items):
                                ...
                                if (
                                    ...
                                    try:
                                        ...
                                        logger.warning(
                                            f"Error processing top-level related term '{related_term}' (type '{rel_type_raw}') for '{formatted_word}': {e}"
                                        )
                    ...
                    if stats["processed_entries"] % 500 == 0:
                        try:
                            conn.commit()
                            logger.info(
                                f"Processed {stats['processed_entries']} entries, committed batch for {filename}"
                            )
                        except Exception as commit_batch_err:
                            logger.error(
                                f"Error committing batch at entry {entry_index} for {filename}: {commit_batch_err}. Rolling back batch..."
                            )
                            conn.rollback()
...
def process_kwf_dictionary(cur, filename: str):
    ...
    # --- Outer Exception Handling ---
        ...
        try:
            ...
            ):  # Check if not already closed/rolled back
                logger.warning("Rolling back transaction due to outer exception.")
                conn.rollback()
            else:
                logger.info(
                    "Transaction already closed or rolled back in outer exception."
                )
        except Exception as rollback_error:
            logger.error(
...
def process_marayum_json(cur, filename: str, source_identifier: Optional[str] = None) -> Tuple[int, int]:
    ...
    if base_language_name:
        ...
        # If language_code is 'unk' after the call, the warning was logged inside get_language_code
    else:
        logger.warning(f"No 'base_language' found in dictionary_info for {filename}. Defaulting language code to 'unk'.")
        language_code = "unk" # Explicitly set default

    # If language code is still 'unk', consider if processing should stop or continue
