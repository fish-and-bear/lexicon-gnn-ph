                                            ...
                                            if isinstance(examples_in_set, list):
                                                for ex_idx, ex in enumerate(examples_in_set):
                                                    if isinstance(ex, dict) and "text" in ex and ex["text"].strip():
                                                        # Remove HTML from example text
                                                        clean_text = re.sub(r"<[^>]+>", "", ex["text"].strip()).strip()
                                                        if clean_text:
                                                            ex_obj = {
                                                                "text": clean_text,  # Store clean text only
                                                                "source": source_identifier,
                                                                "set_label": set_label,
                                                                "set_index": set_idx,
                                                                "index_in_set": ex_idx,
                                                                # Don't store html field
                                                            }
                                ...
                                if definition_text:
                                    try:
                                        ...
                                        if definition_id:
                                                ...
                                                for syn_idx, syn in enumerate(synonyms_list):
                                                    ...
                                                    if isinstance(syn, str) and syn.strip() and syn.strip() != formatted_word:
                                                        ...
                                                        try:
                                                            ...
                                                            if syn_id:
                                                                # Store only necessary metadata, without HTML/link info
                                                                syn_metadata = {
                                                                    "source": source_identifier,
                                                                    "confidence": 90,
                                            ...
                                            if isinstance(antonyms_list, list):
                                                for ant_idx, ant in enumerate(antonyms_list):
                                                    if isinstance(ant, str) and ant.strip() and ant.strip() != formatted_word:
                                                        # Strip HTML tags
                                                        ant_word_clean = re.sub(r"<[^>]+>", "", ant.strip()).strip()
                                            ...
                                            if isinstance(cross_refs, list):
                                                for ref_idx, ref in enumerate(cross_refs):
                                                    ...
                                                    if isinstance(ref, dict):
                                                        ...
                                                        # Don't store the link - we're only keeping clean text

                                                    if isinstance(ref_word, str) and ref_word.strip() and ref_word.strip() != formatted_word:
                                                        # Strip HTML tags
                                                        ref_word_clean = re.sub(r"<[^>]+>", "", ref_word.strip()).strip()
                                                        ...
                                                        try:
                                                            ...
                                                            if ref_id:
                                                                ...
                                                                ref_metadata = {
                                                                    ...
                                                                    "definition_id": definition_id,
                                                                    "index": ref_idx,
                                                                    # "link" field removed to avoid redundancy
                                                                }
                                            ...
                                            # --- Process example_sets - clean HTML from examples too
                                            example_sets = def_item.get("example_sets", [])
                                            normalized_examples = []
                                            if isinstance(example_sets, list):
                                                for set_idx, ex_set in enumerate(example_sets):
                                                        ...
                                                        if isinstance(examples_in_set, list):
                                                            for ex_idx, ex in enumerate(examples_in_set):
                                                                if isinstance(ex, dict) and "text" in ex and ex["text"].strip():
                                                                    ...
                                                                    if clean_text:
                                                                        ex_obj = {
                                                                            "text": clean_text,  # Store clean text
                                                                            "source": source_identifier,
                                                                            "set_label": set_label,
                                                                            "set_index": set_idx,
                                                                            "index_in_set": ex_idx,
                                            ...
                                            if isinstance(related_data, dict):
                                                ...
                                                for rel_type_raw, rel_items in related_data.items():
                                                    ...
                                                    for rel_idx, rel_item in enumerate(rel_items):
                                                        related_term = None
                                                        # Only keep metadata about the relationship type and source, not redundant HTML/link
                                                        item_metadata = {}  
                                                        ...
                                                        if isinstance(rel_item, dict) and "term" in rel_item:
                                                            ...
                                                            # Skip storing other metadata like 'link'
                                                        ...
                                                        if related_term_clean and len(related_term_clean) <= 255 and related_term_clean != formatted_word:
                                                            try:
                                                                ...
                                                                if related_id:
                                                                    ...
                                                                    # Don't include item_metadata which may contain links/HTML
