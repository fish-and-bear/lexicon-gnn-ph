...
def with_transaction(commit=True):
    """
    Decorator for managing database transactions with savepoints.
    It ensures that a transaction block is active by setting autocommit to False if needed.
    """
    def decorator(func):
        @functools.wraps(func)
        def wrapper(cur, *args, **kwargs):
            conn = cur.connection
            # Ensure a transaction block is active
            if conn.autocommit:
            ...
            try:
                ...
                # If creating the savepoint fails, attempt to explicitly begin a transaction
                try:
                    cur.execute("BEGIN")
                    cur.execute(f"SAVEPOINT {savepoint_name}")
                except Exception as e2:
                    logger.warning(f"Could not create savepoint {savepoint_name}: {e2}")
            try:
                ...
                try:
                    ...
                    logger.warning(f"Could not rollback to savepoint {savepoint_name}: {e}")
                raise ex
        return wrapper
    return decorator
# -------------------------------------------------------------------
