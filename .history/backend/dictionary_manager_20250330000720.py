...
def process_marayum_directory(cur, directory_path: str):
    ...
    print(f"DEBUG: Processing Marayum dictionaries from directory: {directory_path}")
    ...
    if not os.path.exists(directory_path):
        ...
        print(f"DEBUG: Directory exists: {directory_path}")
    
    # Find all JSON files in the directory
    json_pattern = os.path.join(directory_path, "*.json")
    print(f"DEBUG: Looking for files with pattern: {json_pattern}")
    ...
    for json_file in json_files:
        ...
        try:
            ...
            total_errors += errors
            print(f"DEBUG: Processed {processed} entries with {errors} errors from {json_file}")
        except Exception as e:
            print(f"DEBUG: Error processing file {json_file}: {str(e)}")
            logger.error(f"Error processing Marayum dictionary file {json_file}: {str(e)}")
            total_errors += 1
    
    print(f"DEBUG: Completed processing with {total_processed} total entries and {total_errors} total errors")
