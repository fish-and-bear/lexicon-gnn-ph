...
class EtymologySchema(BaseSchema):
    ...
    etymology_data = fields.Dict()  # Field already renamed to match property in model
    word = fields.Nested('WordSchema', only=('id', 'lemma', 'language_code'))
...
def get_random_word():
    ...
    try:
        ...
        # Don't filter by completeness_score in SQL since it's a hybrid property
        # We'll apply this filter after getting the objects
        ...
        # If we need to filter by completeness score, fetch the words and filter in Python
        # This is more efficient than loading all words if completeness filtering isn't needed
        if min_completeness > 0:
            # Get the actual Word objects to check completeness scores
            eligible_words = []
            # Process in batches to avoid excessive memory usage
            batch_size = 100
            for i in range(0, len(eligible_ids), batch_size):
                batch_ids = eligible_ids[i:i+batch_size]
                words_batch = Word.query.filter(Word.id.in_(batch_ids)).all()
                for word in words_batch:
                    if hasattr(word, 'completeness_score') and word.completeness_score >= min_completeness:
                        eligible_words.append(word)
                        
            if not eligible_words:
                logger.info("No words match the completeness criteria.")
                return jsonify({"error": "No words match the completeness criteria"}), 404
                
            # Choose a random word from the filtered list
            random_word = random.choice(eligible_words)
            random_word_id = random_word.id
        else:
...
def _fetch_word_details(word_id, 
    ...
    try:
        ...
        SELECT id, lemma, normalized_lemma, language_code, has_baybayin, baybayin_form,
               ...
               is_abbreviation, is_initialism, is_root
        FROM words 
        WHERE id = :id
        """
        word_result = db.session.execute(text(sql_word), {"id": word_id}).fetchone()
        ...
        if include_etymologies:
            ...
            SELECT id, etymology_text, normalized_components, etymology_structure,
                  language_codes, etymology_data, sources
            FROM etymologies
            WHERE word_id = :word_id
            """
        ...
        # Load additional relationships as needed for more complete word details
        if include_affixations:
            # TODO: Add loading affixations
        ...
        if include_definition_relations:
            # TODO: Add loading definition relations
