...
def process_kaikki_jsonl(cur, filename: str):
    """
    Process the Kaikki JSONL file and store its entries in the database.

    The file is expected to be a JSON Lines file where each line is a dictionary
    representing a dictionary entry. The structure of the dictionary is as follows:

    {
        ...
        "hyphenation": "hyphenation_pattern",
        "senses": [
            {
                ...
                "raw_glosses": "raw_glosses_text",
                "qualifier": "qualifier_text",
                "wikipedia": ["wiki_link1", "wiki_link2"],
                "topics": ["topic1", "topic2"],
                "translations": {
                    "language_code": ["translation1", "translation2"]
                },
                "derived": ["derived_word1", "derived_word2"],
    ...
    This function uses the standardized source name "kaikki.org" (via SourceStandardization)
    when inserting definitions.
    ...
    """
    standardized_source = SourceStandardization.standardize_sources(filename)
    total_entries = 0
    error_count = 0
    ...
    with open(filename, 'r', encoding='utf-8') as f:
        for line in f:
            try:
                ...
                process_entry(cur, entry, standardized_source)
            except Exception as e:
                error_count += 1
...
def process_entry(cur, entry: Dict[str, Any], standardized_source: str):
    """
    Process a single dictionary entry and store its data in the database.
    """
    word = entry.get('word', '')
    lang = entry.get('lang', '')
    ...
    # Get or create the word ID
    word_id = get_or_create_word_id(cur, word, language_code=lang_code)
    
    # Process forms
    if 'forms' in entry:
        process_forms(cur, word_id, entry['forms'])
    
    # Process etymology
    if 'etymology_text' in entry:
        etymology_text = entry['etymology_text']
        etymology_templates = entry.get('etymology_templates', {})
        process_etymology(cur, word_id, etymology_text, etymology_templates)
    
    # Process sounds
    if 'sounds' in entry:
        process_sounds(cur, word_id, entry['sounds'])
    
    # Process hyphenation
    if 'hyphenation' in entry:
        hyphenation = entry['hyphenation']
        process_hyphenation(cur, word_id, hyphenation)
    
    # Process senses
    if 'senses' in entry:
        process_senses(cur, word_id, entry['senses'], pos, standardized_source)
    
    # Process related words
    if 'related' in entry:
        process_related_words(cur, word_id, entry['related'])
    ...
    if 'other_sections' in entry:
        process_other_sections(cur, word_id, entry['other_sections'])

def process_forms(cur, word_id: int, forms: List[Dict[str, Any]]):
    """
    Process the 'forms' section of a dictionary entry.
    """
    for form_entry in forms:
        form = form_entry.get('form', '')
        tags = form_entry.get('tags', [])
        if 'Baybayin' in tags:
            process_baybayin_form(cur, word_id, form)

def process_baybayin_form(cur, word_id: int, baybayin_form: str):
    """
    Process a Baybayin form for a word.
    """
    romanized_form = get_romanized_text(baybayin_form)
    process_baybayin_data(cur, word_id, baybayin_form, romanized_form)

def process_etymology(cur, word_id: int, etymology_text: str, etymology_templates: Dict[str, Any]):
    """
    Process the etymology section of a dictionary entry.
    """
    etymology_structure = {
        'raw_data': etymology_text,
        'templates': etymology_templates
    }
    insert_etymology(
        cur,
        word_id,
...
def process_sounds(cur, word_id: int, sounds: List[Dict[str, Any]]):
    """
    Process the 'sounds' section of a dictionary entry.
    """
    for sound_entry in sounds:
        ipa = sound_entry.get('ipa', '')
        tags = sound_entry.get('tags', [])
        process_ipa(cur, word_id, ipa, tags)

def process_ipa(cur, word_id: int, ipa: str, tags: List[str]):
    """
    Process an IPA pronunciation for a word.
    """
    cur.execute("""
        INSERT INTO pronunciations (word_id, ipa, tags)
        VALUES (%s, %s, %s)
        ON CONFLICT (word_id, ipa) DO NOTHING
    """, (word_id, ipa, ', '.join(tags)))

def process_hyphenation(cur, word_id: int, hyphenation: str):
    """
    Process the hyphenation pattern for a word.
    """
    cur.execute("""
        UPDATE words
        SET hyphenation = %s
...
def process_senses(cur, word_id: int, senses: List[Dict[str, Any]], pos: str, standardized_source: str):
    """
    Process the 'senses' section of a dictionary entry.
    """
    for sense in senses:
        process_sense(cur, word_id, sense, pos, standardized_source)

def process_sense(cur, word_id: int, sense: Dict[str, Any], pos: str, standardized_source: str):
    """
    Process a single sense entry.
    """
    ...
    if raw_glosses:
        definition_text = f"{definition_text} ({raw_glosses})"
    if qualifier:
        definition_text = f"{definition_text} [{qualifier}]"
    
    # Process examples
    examples_json = json.dumps(examples)
    
    # Process usage notes
    usage_notes = extract_usage_notes(definition_text)
    ...
    if definition_id:
        ...
        if categories:
            process_categories(cur, definition_id, categories)
        
        # Process links
        if links:
            process_links(cur, definition_id, links)
        
        # Process Wikipedia links
        if wikipedia:
            process_wikipedia_links(cur, definition_id, wikipedia)
        
        # Process topics
        if topics:
            process_topics(cur, definition_id, topics)
        
        # Process translations
        if translations:
            process_translations(cur, definition_id, translations)
        
        # Process derived words
        if derived:
            process_derived_words(cur, word_id, derived)
        
        # Process synonyms
        if synonyms:
            process_synonyms(cur, word_id, synonyms)
        
        # Process antonyms
        if antonyms:
            process_antonyms(cur, word_id, antonyms)
        
        # Process related words
        if related:
            process_related_words(cur, word_id, related)
        
        # Process tags
        if tags:
            process_tags(cur, definition_id, tags)

def process_categories(cur, definition_id: int, categories: List[str]):
    """
    Process the categories for a definition.
    """
    for category in categories:
        cur.execute("""
            INSERT INTO definition_categories (definition_id, category)
            VALUES (%s, %s)
            ON CONFLICT DO NOTHING
        """, (definition_id, category))

def process_links(cur, definition_id: int, links: List[Dict[str, str]]):
    """
    Process the links for a definition.
    """
    for link in links:
        target = link.get('target', '')
        rel = link.get('rel', '')
        process_link(cur, definition_id, target, rel)

def process_link(cur, definition_id: int, target: str, rel: str):
    """
    Process a single link for a definition.
    """
    cur.execute("""
        INSERT INTO definition_links (definition_id, link_type, target)
        VALUES (%s, %s, %s)
        ON CONFLICT DO NOTHING
    """, (definition_id, rel, target))

def process_wikipedia_links(cur, definition_id: int, wikipedia: List[str]):
    """
    Process the Wikipedia links for a definition.
    """
...
def process_topics(cur, definition_id: int, topics: List[str]):
    """
    Process the topics for a definition.
    """
    for topic in topics:
        cur.execute("""
            INSERT INTO definition_topics (definition_id, topic)
            VALUES (%s, %s)
            ON CONFLICT DO NOTHING
        """, (definition_id, topic))

def process_translations(cur, definition_id: int, translations: Dict[str, List[str]]):
    """
    Process the translations for a definition.
    """
    for lang_code, translations_list in translations.items():
        for translation in translations_list:
            cur.execute("""
                INSERT INTO translations (definition_id, language_code, translation)
                VALUES (%s, %s, %s)
                ON CONFLICT DO NOTHING
            """, (definition_id, lang_code, translation))

def process_derived_words(cur, word_id: int, derived: List[str]):
    """
    Process the derived words for a word.
    """
    for derived_word in derived:
        process_derived_word(cur, word_id, derived_word)

def process_derived_word(cur, word_id: int, derived_word: str):
    """
    Process a single derived word for a word.
    """
    derived_id = get_or_create_word_id(cur, derived_word)
    insert_relation(cur, word_id, derived_id, 'derived', sources='kaikki.jsonl')

def process_synonyms(cur, word_id: int, synonyms: List[str]):
    """
    Process the synonyms for a word.
    """
    for synonym in synonyms:
        process_synonym(cur, word_id, synonym)

def process_synonym(cur, word_id: int, synonym: str):
    """
    Process a single synonym for a word.
    """
    synonym_id = get_or_create_word_id(cur, synonym)
    insert_relation(cur, word_id, synonym_id, 'synonym', sources='kaikki.jsonl')

def process_antonyms(cur, word_id: int, antonyms: List[str]):
    """
    Process the antonyms for a word.
    """
    for antonym in antonyms:
        process_antonym(cur, word_id, antonym)

def process_antonym(cur, word_id: int, antonym: str):
    """
    Process a single antonym for a word.
    """
    antonym_id = get_or_create_word_id(cur, antonym)
    insert_relation(cur, word_id, antonym_id, 'antonym', sources='kaikki.jsonl')

def process_related_words(cur, word_id: int, related: Dict[str, List[str]]):
    """
    Process the related words for a word.
    """
    for rel_type, related_words in related.items():
        for related_word in related_words:
            process_related_word(cur, word_id, related_word, rel_type)

def process_related_word(cur, word_id: int, related_word: str, rel_type: str):
    """
    Process a single related word for a word.
    """
    related_id = get_or_create_word_id(cur, related_word)
    insert_relation(cur, word_id, related_id, rel_type, sources='kaikki.jsonl')

def process_tags(cur, definition_id: int, tags: List[str]):
    """
    Process the tags for a definition.
    """
    for tag in tags:
        cur.execute("""
            INSERT INTO definition_tags (definition_id, tag)
            VALUES (%s, %s)
            ON CONFLICT DO NOTHING
        """, (definition_id, tag))

def process_head_templates(cur, word_id: int, head_templates: Dict[str, Any]):
    """
    Process the head templates for a word.
    """
    for template_name, template_data in head_templates.items():
        process_head_template(cur, word_id, template_name, template_data)

def process_head_template(cur, word_id: int, template_name: str, template_data: Dict[str, Any]):
    """
    Process a single head template for a word.
    """
    cur.execute("""
        INSERT INTO word_templates (word_id, template_name, template_data)
        VALUES (%s, %s, %s)
        ON CONFLICT DO NOTHING
    """, (word_id, template_name, json.dumps(template_data)))

def process_other_sections(cur, word_id: int, other_sections: Dict[str, str]):
    """
    Process the other sections for a word.
    """
    for section_name, section_content in other_sections.items():
        process_other_section(cur, word_id, section_name, section_content)

def process_other_section(cur, word_id: int, section_name: str, section_content: str):
    """
    Process a single other section for a word.
    """
    cur.execute("""
        INSERT INTO other_sections (word_id, section_name, section_content)
        VALUES (%s, %s, %s)
        ON CONFLICT DO NOTHING
    """, (word_id, section_name, section_content))

def extract_usage_notes(text: str) -> str:
    """
    Extract usage notes from a definition text.
    """
    usage_notes = []
    for pattern in ['(colloquial)', '(slang)', '(figuratively)', '(alt-of)', '(obsolete)']:
        if pattern in text:
            usage_notes.append(pattern[1:-1])
            text = text.replace(pattern, '')
    return ', '.join(usage_notes)
...
def process_kaikki_jsonl(cur, filename: str):
    ...
    def process_sense_relationships(cur, word_id: int, sense: Dict):
        """Process relationships defined within a sense definition."""
        if not sense:
            return
        ...
        if 'synonyms' in sense:
            for synonym in sense.get('synonyms', []):
                if isinstance(synonym, str):
                    ...
                    syn_word_id = get_or_create_word_id(cur, synonym, language_code)
                    insert_relation(
                        cur, 
                        word_id, 
                        syn_word_id, 
                        RelationshipType.SYNONYM.rel_value,
                        sources=SourceStandardization.standardize_sources(filename),
                        metadata={"strength": 90, "from_sense": True}
                    )
                elif isinstance(synonym, list) and len(synonym) > 0:
                    # List format, first element is the word
                    syn_word = synonym[0]
                    syn_word_id = get_or_create_word_id(cur, syn_word, language_code)
                    
                    # Check for qualifiers or other metadata
                    metadata = {"strength": 90, "from_sense": True}
                    if len(synonym) > 1:
                        metadata["context"] = synonym[1]
        ...
        if 'antonyms' in sense:
            for antonym in sense.get('antonyms', []):
                if isinstance(antonym, str):
                    # Simple string antonym
                    ant_word_id = get_or_create_word_id(cur, antonym, language_code)
                    insert_relation(
                        cur, 
                        word_id, 
                        ant_word_id, 
                        RelationshipType.ANTONYM.rel_value,
                        sources=SourceStandardization.standardize_sources(filename),
                        metadata={"strength": 90, "from_sense": True}
                    )
                elif isinstance(antonym, list) and len(antonym) > 0:
                    # List format, first element is the word
                    ant_word = antonym[0]
                    ant_word_id = get_or_create_word_id(cur, ant_word, language_code)
                    
                    # Check for qualifiers or other metadata
                    metadata = {"strength": 90, "from_sense": True}
                    if len(antonym) > 1:
                        metadata["context"] = antonym[1]
        ...
        # Process hypernyms (broader terms)
        if 'hypernyms' in sense:
            for hypernym in sense.get('hypernyms', []):
                if isinstance(hypernym, str) or (isinstance(hypernym, list) and len(hypernym) > 0):
                    hyper_word = hypernym if isinstance(hypernym, str) else hypernym[0]
                    hyper_word_id = get_or_create_word_id(cur, hyper_word, language_code)
                    
                    # Check for qualifiers or other metadata
                    metadata = {"strength": 85, "from_sense": True}
                    if isinstance(hypernym, list) and len(hypernym) > 1:
                        metadata["context"] = hypernym[1]
                    
                    insert_relation(
                        cur, 
                        word_id, 
                        hyper_word_id, 
                        RelationshipType.HYPERNYM.rel_value,
                        sources=SourceStandardization.standardize_sources(filename),
                        metadata=metadata
                    )
        
        # Process hyponyms (narrower terms)
        if 'hyponyms' in sense:
            for hyponym in sense.get('hyponyms', []):
                if isinstance(hyponym, str) or (isinstance(hyponym, list) and len(hyponym) > 0):
                    hypo_word = hyponym if isinstance(hyponym, str) else hyponym[0]
                    hypo_word_id = get_or_create_word_id(cur, hypo_word, language_code)
                    
                    # Check for qualifiers or other metadata
                    metadata = {"strength": 85, "from_sense": True}
                    if isinstance(hyponym, list) and len(hyponym) > 1:
                        metadata["context"] = hyponym[1]
                    
                    insert_relation(
                        cur, 
                        word_id, 
                        hypo_word_id, 
                        RelationshipType.HYPONYM.rel_value,
                        sources=SourceStandardization.standardize_sources(filename),
                        metadata=metadata
                    )
        
        # Process holonyms (whole of which the term is a part)
        if 'holonyms' in sense:
            for holonym in sense.get('holonyms', []):
                if isinstance(holonym, str) or (isinstance(holonym, list) and len(holonym) > 0):
                    holo_word = holonym if isinstance(holonym, str) else holonym[0]
                    holo_word_id = get_or_create_word_id(cur, holo_word, language_code)
                    
                    # Check for qualifiers or other metadata
                    metadata = {"strength": 80, "from_sense": True}
                    if isinstance(holonym, list) and len(holonym) > 1:
                        metadata["context"] = holonym[1]
                    
                    insert_relation(
                        cur, 
                        word_id, 
                        holo_word_id, 
                        RelationshipType.HOLONYM.rel_value,
                        sources=SourceStandardization.standardize_sources(filename),
                        metadata=metadata
                    )
        
        # Process meronyms (parts of the whole)
        if 'meronyms' in sense:
            for meronym in sense.get('meronyms', []):
                if isinstance(meronym, str) or (isinstance(meronym, list) and len(meronym) > 0):
                    mero_word = meronym if isinstance(meronym, str) else meronym[0]
                    mero_word_id = get_or_create_word_id(cur, mero_word, language_code)
                    
                    # Check for qualifiers or other metadata
                    metadata = {"strength": 80, "from_sense": True}
                    if isinstance(meronym, list) and len(meronym) > 1:
                        metadata["context"] = meronym[1]
                    
                    insert_relation(
                        cur, 
                        word_id, 
                        mero_word_id, 
                        RelationshipType.MERONYM.rel_value,
                        sources=SourceStandardization.standardize_sources(filename),
                        metadata=metadata
                    )
        ...
        if 'derived' in sense:
            for derived in sense.get('derived', []):
                if isinstance(derived, str) or (isinstance(derived, list) and len(derived) > 0):
                    derived_word = derived if isinstance(derived, str) else derived[0]
                    derived_word_id = get_or_create_word_id(cur, derived_word, language_code)
                    
                    # For derived terms, the current word is the root
                    metadata = {"strength": 95, "from_sense": True}
                    if isinstance(derived, list) and len(derived) > 1:
                        metadata["context"] = derived[1]
                    
                    insert_relation(
                        cur, 
                        derived_word_id,  # Derived word points to this word
                        word_id,
                        RelationshipType.DERIVED_FROM.rel_value,
                        sources=SourceStandardization.standardize_sources(filename),
                        metadata=metadata
                    )
        
        # Process "see also" references
        if 'see_also' in sense or 'coordinate_terms' in sense:
            related_terms = []
            related_terms.extend(sense.get('see_also', []))
            related_terms.extend(sense.get('coordinate_terms', []))
            
            for related in related_terms:
                if isinstance(related, str) or (isinstance(related, list) and len(related) > 0):
                    related_word = related if isinstance(related, str) else related[0]
                    related_word_id = get_or_create_word_id(cur, related_word, language_code)
                    
                    # For "see also", use lower strength as it's a weaker relationship
                    metadata = {"strength": 40, "from_sense": True}
                    if isinstance(related, list) and len(related) > 1:
                        metadata["context"] = related[1]
                    
                    insert_relation(
                        cur, 
                        word_id,
                        related_word_id,
                        RelationshipType.SEE_ALSO.rel_value,
                        sources=SourceStandardization.standardize_sources(filename),
                        metadata=metadata
                    )
                    
        # Process any other related terms using the general "related" relationship
        if 'related' in sense:
            for related in sense.get('related', []):
                if isinstance(related, str) or (isinstance(related, list) and len(related) > 0):
                    ...
                    metadata = {"strength": 70, "from_sense": True}
                    if isinstance(related, list) and len(related) > 1:
                        metadata["context"] = related[1]
                    ...
                    insert_relation(
                        ...
                        word_id,
                        related_word_id,
                        RelationshipType.RELATED.rel_value,
                        sources=SourceStandardization.standardize_sources(filename),
                        metadata=metadata
                    )
...
def process_kaikki_jsonl(cur, filename: str):
    ...
    def process_sense_relationships(cur, word_id: int, sense: Dict):
        ...
        if 'synonyms' in sense:
            for synonym in sense['synonyms']:
                if 'word' in synonym:
                    ...
                    syn_word_id = get_or_create_word_id(cur, syn_word)
        ...
        if 'alt_of' in sense:
            for alt in sense['alt_of']:
                if 'word' in alt:
                    alt_word = alt['word']
                    alt_word_id = get_or_create_word_id(cur, alt_word)
                    
                    # Get tags from the sense (e.g., "abbreviation", "initialism")
                    tags = sense.get('tags', [])
                    metadata = {"tags": tags} if tags else None
                    
                    # Determine relationship type based on tags
                    rel_type = 'variant'
                    if tags:
                        if 'abbreviation' in tags:
                            rel_type = 'abbreviation_of'
                        elif 'initialism' in tags:
                            rel_type = 'initialism_of'
                        elif 'obsolete' in tags or 'archaic' in tags:
                            rel_type = 'obsolete_spelling_of'
        ...
        # Process derived terms (existing functionality)
        if 'derived' in sense:
            for derived in sense['derived']:
                if isinstance(derived, dict) and 'word' in derived:
                    der_word = derived['word']
                    der_word_id = get_or_create_word_id(cur, der_word)
                    cur.execute("""
                        INSERT INTO relations (from_word_id, to_word_id, relation_type, sources)
                        VALUES (%s, %s, %s, %s)
                        ON CONFLICT (from_word_id, to_word_id, relation_type) DO NOTHING
        ...
        # Process related terms (existing functionality)
        if 'related' in sense:
            for related in sense['related']:
                if isinstance(related, dict) and 'word' in related:
                    rel_word = related['word']
                    rel_word_id = get_or_create_word_id(cur, rel_word)
                    cur.execute("""
                        INSERT INTO relations (from_word_id, to_word_id, relation_type, sources)
                        VALUES (%s, %s, %s, %s)
                        ON CONFLICT (from_word_id, to_word_id, relation_type) DO NOTHING
    ...
    def process_entry(cur, entry: Dict):
        try:
            word = entry['word']
            language_code = entry.get('lang_code', 'tl')
            pos = entry.get('pos', '')
            ...
            # Check for abbreviation/initialism in senses
            if 'senses' in entry:
                for sense in entry['senses']:
                    if 'tags' in sense:
                        tags = sense.get('tags', [])
                        if 'abbreviation' in tags:
                            is_abbreviation = True
                        if 'initialism' in tags:
                            is_initialism = True
            
            # Get or create word ID with additional metadata
            params = {
                'lemma': word,
                'language_code': language_code,
                'is_proper_noun': is_proper_noun,
            ...
            word_id = get_or_create_word_id(cur, **params)
            
            # Process script forms (Baybayin for Tagalog, Badlit for Cebuano)
            if language_code == 'tl':
                baybayin_form, romanized_form = extract_baybayin_info(entry)
                if baybayin_form:
                    process_baybayin_data(cur, word_id, baybayin_form, romanized_form)
            elif language_code == 'ceb':
                badlit_form, romanized_form = extract_badlit_info(entry)
                if badlit_form:
                    # Store Badlit script information
                    cur.execute("""
                        UPDATE words 
                        SET badlit_form = %s,
                            romanized_form = %s
                        WHERE id = %s
                    """, (badlit_form, romanized_form, word_id))
            
            # Process canonical forms
            if 'forms' in entry:
                for form in entry['forms']:
                    if 'form' in form and 'tags' in form and 'canonical' in form['tags']:
                        is_primary = 'primary' in form['tags']
                        cur.execute("""
            ...
            if 'hyphenation' in entry:
                cur.execute("""
                    ...
                    SET hyphenation = %s
                    WHERE id = %s
                """, (json.dumps(entry['hyphenation']), word_id))
            ...
            if 'senses' in entry:
                for sense in entry['senses']:
                    ...
                    # Join multiple glosses into a single definition
                    definition_text = "; ".join(glosses)
                    ...
                    if 'raw_glosses' in sense:
                        # Extract usage notes from raw glosses that may include parenthetical information
                        raw_glosses = sense['raw_glosses']
                        matches = re.findall(r'\((.*?)\)', raw_glosses)
                        if matches:
                            usage_notes = json.dumps(matches)
                    
                    # Create a metadata object for additional sense information
                    metadata = {}
                    ...
                    if 'topics' in sense and sense['topics']:
                        metadata['topics'] = sense['topics']
                    
                    # Add Wikipedia references if present
                    if 'wikipedia' in sense:
                        metadata['wikipedia'] = sense['wikipedia']
                    
                    # Add sense ID for cross-referencing
                    if 'id' in sense:
                        metadata['sense_id'] = sense['id']
            ...
            if 'etymology_text' in entry:
                etymology_templates = entry.get('etymology_templates', [])
                process_etymology(cur, word_id, entry['etymology_text'], etymology_templates)
            
            # Process relationships at the entry level
            if 'relations' in entry:
                for rel_type, related_words in entry['relations'].items():
                    for rel_word in related_words:
                        if isinstance(rel_word, dict) and 'word' in rel_word:
                            rel_word = rel_word['word']
                        
                        if not isinstance(rel_word, str):
                            continue
            ...
            # Process derived terms at the entry level
            if 'derived' in entry:
                for derived in entry['derived']:
                    if isinstance(derived, dict) and 'word' in derived:
                        der_word = derived['word']
                        der_word_id = get_or_create_word_id(cur, der_word, language_code=language_code)
                        cur.execute("""
                            INSERT INTO relations (from_word_id, to_word_id, relation_type, sources)
                            VALUES (%s, %s, %s, %s)
                            ON CONFLICT (from_word_id, to_word_id, relation_type) DO NOTHING
            ...
            # Process related terms at the entry level
            if 'related' in entry:
