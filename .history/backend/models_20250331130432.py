        ...
        if include_relations:
            result['relations'] = {
                'synonyms': [
                    ...
                    } for r in self.relations_from if r.type == 'synonym'
                ],
                'antonyms': [
                    {
                        'word': r.target_word.lemma,
                        ...
                        'direction': 'outgoing',
                        'metadata': r.metadata or {}
                    } for r in self.relations_from if r.type == 'antonym'
                ],
                'related': [
                    {
                        'word': r.target_word.lemma,
                        ...
                        'direction': 'outgoing',
                        'metadata': r.metadata or {}
                    } for r in self.relations_from if r.type == 'related'
                ],
                'variants': [
                    {
                        'word': r.target_word.lemma,
                        ...
                        'direction': 'outgoing',
                        'metadata': r.metadata or {}
                    } for r in self.relations_from if r.type == 'variant'
                ],
                'affixations': {
                    'as_root': [
                        {
                            ...
                            'direction': 'outgoing',
                            'metadata': r.metadata or {}
                        } for r in self.relations_from if r.type == 'affixation'
                    ],
                    'as_affixed': [
                        {
                            ...
                            'direction': 'incoming',
                            'metadata': r.metadata or {}
                        } for r in self.relations_to if r.type == 'affixation'
                    ]
                }
            }
...
class Definition(db.Model):
    ...
    examples = db.relationship('Example', back_populates='definition', cascade='all, delete-orphan')
...
class Relation(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    source_word_id = db.Column(db.Integer, db.ForeignKey('words.id', ondelete='CASCADE'), nullable=False)
    target_word_id = db.Column(db.Integer, db.ForeignKey('words.id', ondelete='CASCADE'), nullable=False)
    ...
    # Relationships
    source_word = db.relationship('Word', foreign_keys=[source_word_id], back_populates='relations_from')
    target_word = db.relationship('Word', foreign_keys=[target_word_id], back_populates='relations_to')
...
def init_db(app):
    ...
    with app.app_context():
        ...
        try:
            ...
            current_app.logger.warning(f"Some optional extensions could not be installed: {str(e)}")
            db.session.rollback()
        ...
        try:
            ...
            current_app.logger.warning(f"Custom collations could not be created: {str(e)}")
            db.session.rollback()

        # Create text search configuration
        try:
            db.session.execute(text("""
                CREATE TEXT SEARCH CONFIGURATION IF NOT EXISTS simple_tagalog (COPY = simple);
            ...
            current_app.logger.warning(f"Text search configuration could not be created: {str(e)}")
            db.session.rollback()

        current_app.logger.info("Database extensions and language support setup completed")

        # Create all tables
        db.create_all()

        # Register event listeners
        event.listen(Word, 'after_insert', Word._update_search_text)
        event.listen(Word, 'after_update', Word._update_search_text)
