...
def get_or_create_word_id(
    ...
    **kwargs,
    ...
    params = {
        ...
        "tags": tags,
        # Pass Json() wrapped objects or None for JSONB columns
        ...
        # search_text is generated in the query
    }

    # Calculate data hash based on current parameters
    try:
        # Include more fields in hash calculation
    ...
    try:
        ...
        # This avoids the need for check_exists and is safer against race conditions
        cur.execute(
            """
            INSERT INTO words (
            ...
            ON CONFLICT (normalized_lemma, language_code) DO UPDATE SET
                lemma = EXCLUDED.lemma, -- Always update lemma in case of case changes
                has_baybayin = COALESCE(EXCLUDED.has_baybayin, words.has_baybayin), -- Update if new value is not NULL
                baybayin_form = COALESCE(EXCLUDED.baybayin_form, words.baybayin_form),
                romanized_form = COALESCE(EXCLUDED.romanized_form, words.romanized_form),
                ...
                -- Merge source_info JSONB
                source_info = words.source_info || EXCLUDED.source_info,
                -- Update other JSONB fields - prefer new non-null data
                ...
                data_hash = EXCLUDED.data_hash, -- Update hash
                search_text = to_tsvector('simple', EXCLUDED.lemma || ' ' || EXCLUDED.normalized_lemma), -- Update search vector
                updated_at = CURRENT_TIMESTAMP
            RETURNING id
            """,
            params, # Pass the dictionary with Json() objects where needed
        )
        word_id = cur.fetchone()[0]

        if cur.rowcount > 0: # Check if insert or update happened
             # Check if it was an insert or an update based on xmin (internal system column)
             # This check is PostgreSQL specific and might need adjustment
             # A simpler way might be to check if created_at == updated_at after the query,
             # but that requires another SELECT. This approach is common.
            cur.execute("SELECT xmin FROM words WHERE id = %s", (word_id,))
            xmin_val = cur.fetchone()[0]
            # This logic assumes xmin changes on UPDATE but not INSERT in the same transaction
            # It's not perfectly reliable across all scenarios, but gives an indication.
            # A better approach might involve checking timestamps if high precision isn't needed.
            # For simplicity, we log based on the UPSERT nature.
            logger.debug(
                f"Word '{lemma}' ({language_code}) UPSERTED (ID: {word_id}) from source '{standardized_source}'."
            )
        else:
             # This case should ideally not happen with ON CONFLICT...RETURNING id,
             # but added for robustness.
             logger.warning(
                 f"Word '{lemma}' ({language_code}) - ON CONFLICT did not return ID or update rows (ID: {word_id}). State might be unchanged."
            )
        ...
        logger.error(
            ...
            exc_info=False, # Keep log cleaner for this common error
        )
        # Log parameters that might have caused the issue, carefully handling sensitive data if applicable
        # Log only keys and types for potentially large/complex data
    ...
    # --- Propagate Source (Optional - Moved outside UPSERT for clarity) ---
    # Only run if source was provided and word ID is valid
    if standardized_source and word_id:
         try:
             # Check if source already exists to avoid unnecessary updates
             cur.execute("SELECT 1 FROM words WHERE id = %s AND source_info -> 'files' ? %s", (word_id, standardized_source))
             source_exists = cur.fetchone()

             if not source_exists:
                # Update existing word's source info using the helper function's logic implicitly via JSONB concatenation
                # The || operator merges top-level keys, replacing values for existing keys.
                # For the 'files' array, we need jsonb_set to append non-destructively.
                cur.execute("""
                    UPDATE words
                    SET source_info = jsonb_set(
                            COALESCE(source_info, '{}'::jsonb),
                            '{files}',
                            (COALESCE(source_info -> 'files', '[]'::jsonb) || %s::jsonb) - -1, -- Append new source, -1 ensures appending at the end
                            true -- Create 'files' key if not exists
                        )
                        -- Optionally add/update last_updated timestamp for this source
                        || jsonb_build_object('last_updated', jsonb_build_object(%s, %s::text)),
                        updated_at = CURRENT_TIMESTAMP
                    WHERE id = %s
                      AND NOT (source_info -> 'files' ? %s); -- Ensure source isn't already present before updating
                ...
                # Propagate to definitions only if source was newly added to the word
                if cur.rowcount > 0:
                     cur.execute(
                         """
                         UPDATE definitions
...
def process_kaikki_jsonl(cur, filename: str):
    ...
    # Process pronunciation - Improved to track counts and handle different types
    def process_pronunciation(cur, word_id: int, entry: Dict, source_identifier: str) -> int:
        ...
        for sound in entry.get("sounds", []):
            ...
            pron_data = sound.copy()  # Copy to avoid modifying the original
            pron_type = None
            pron_value = None

            # Extract type and value consistently
            if "ipa" in sound and sound["ipa"]:
                pron_type = "ipa"
                pron_value = sound["ipa"]
            elif "enpr" in sound and sound["enpr"]:
                pron_type = "enpr"
                pron_value = sound["enpr"]
                ...
                continue  # Skip if no recognized type

            if pron_type and pron_value:
                try:
                    # Ensure pron_data has consistent type and value fields
                    pron_data["type"] = pron_type
                    pron_data["value"] = pron_value
    ...
    # Process etymology with improved relation extraction
    def process_etymology(
        ...
        """Process etymology text and templates with improved relation handling."""
        if not etymology_text or not isinstance(etymology_text, str):
            return None, 0  # Return (ety_id, relations_count)

        normalized_components = None  # Future enhancement: implement extract_etymology_components
        etymology_structure = None  # Future enhancement: parse structure
        language_codes = None
        ety_id = None
        relations_added = 0
        ...
        try:
            # Extract etymologically related words from templates
            if etymology_templates and word_lemma and language_code:
                for template in etymology_templates:
                    ...
                    # Handle common etymology templates
                    if template_name in ["derived", "borrowing", "derived from", "borrowed from", "bor", "der", "bor+"]:
                        rel_type = RelationshipType.DERIVED_FROM
                        source_lang = args.get("1", "") or args.get("2", "")  # Language code may be in different args
                        source_word = args.get("2", "") or args.get("3", "")  # Source word might be in arg 2 or 3
                        ...
                        if source_word and isinstance(source_word, str):
                            try:
                                ...
                                if source_word_clean and source_word_clean.lower() != word_lemma.lower():
                                    # Create source word with its language if present
                                    source_word_lang = source_lang if source_lang else language_code
                                    related_word_id = get_or_create_word_id(
                                        cur, source_word_clean, source_word_lang, source_identifier
                                    )
                                    ...
                                    if related_word_id and related_word_id != word_id:
                                        ...
                                        # Insert relation: current_word is DERIVED_FROM source_word
                                        rel_id = insert_relation(
                                        ...
                                        # Insert inverse relation automatically
                                ...
                                logger.warning(f"Error creating etymology relation from template for word ID {word_id}: {rel_err}")
                    
                    # Add other template types if needed (cognate, compound, etc.)

            # Call insert_etymology helper function for the text itself
            ...
            return None, relations_added

    # Process sense relationships with improved directionality
    def process_sense_relationships(
        cur, word_id: int, sense: Dict, language_code: str, source_identifier: str, word_lemma: str = None
    ) -> int:
        """Processes various relationships within a sense with proper directionality."""
        relations_added = 0
        ...
        relation_types = {
            ...
            "derived": RelationshipType.ROOT_OF,  # word_id is root_of derived_word
        ...
        for rel_key, rel_enum in relation_types.items():
            if rel_key in sense and isinstance(sense[rel_key], list):
                for item in sense[rel_key]:
                    ...
                    # Clean HTML from related term
                    related_word_clean = clean_html(related_word)
                    if not related_word_clean:
                        continue
                    
                    # Skip self-references
                    if word_lemma and related_word_clean.lower() == word_lemma.lower():
                        continue

                    try:
                        related_word_id = get_or_create_word_id(
                            cur, related_word_clean, language_code, source_identifier=source_identifier
                        )
                        if related_word_id == word_id:
                            continue  # Avoid self-references
                        
                        # Determine primary direction based on relation type
                        from_id, to_id, current_rel_enum = word_id, related_word_id, rel_enum
                        
                        if rel_key == "derived":  # Word is ROOT_OF derived words
                            pass  # Direction is already correct: word_id -> ROOT_OF -> derived_id
                        elif rel_key == "hyponyms":  # Current word is hypernym of related word
                            from_id, to_id = related_word_id, word_id
                            current_rel_enum = RelationshipType.HYPONYM  # HYPONYM_OF direction
                        elif rel_key == "hypernyms":  # Current word is hyponym of related word 
                            current_rel_enum = RelationshipType.HYPONYM  # Use HYPONYM relationship
                        # Similar adjustments could be made for holonym/meronym if needed

                        # Insert primary relationship
                        rel_id = insert_relation(
                            cur, from_id, to_id, current_rel_enum, source_identifier, metadata
                        )
                        
                        if rel_id:
                            relations_added += 1
                        
                        # Handle inverse/bidirectional relationships
                        if current_rel_enum.bidirectional:
                            # For bidirectional, use same enum in opposite direction
                            insert_relation(
                                cur, to_id, from_id, current_rel_enum, source_identifier, metadata
                            )
                        else:
                            ...
                            if inverse_rel:
                                insert_relation(
                                    cur, to_id, from_id, inverse_rel, source_identifier, metadata
                                )
                    except Exception as e:
                        logger.error(f"Error processing {rel_key} relation for word ID {word_id} to '{related_word_clean}': {str(e)}")
        
        return relations_added

    # Store word forms (without creating relationships)
    def process_forms_storage(cur, word_id: int, entry: Dict, source_identifier: str) -> int:
        """Store word forms in word_forms table without creating relationships."""
        count = 0
        if "forms" in entry and isinstance(entry["forms"], list):
            for form_data in entry["forms"]:
                ...
                try:
                    ...
                    logger.warning(f"Error inserting word form '{form_text}' for {word_id}: {e}")
        return count

    # Process form relationships (original function with small improvements)
    ...
    # Process a single entry with enhanced script handling and error tracking
    def process_entry(cur, entry: Dict, source_identifier: str) -> Tuple[Optional[int], Optional[str]]:
        ...
        try:
            ...
            baybayin_form, baybayin_romanized = extract_baybayin_info(entry)
            badlit_form, badlit_romanized = extract_badlit_info(entry)
            
            # Try to generate romanization if missing and we have a romanizer available
            romanized_form = None
            if baybayin_form and not baybayin_romanized:
                try:
                    # Check if romanizer available in the global scope
                    if 'romanizer' in globals():
                        if romanizer.validate_text(baybayin_form):
                            romanized_form = romanizer.romanize(baybayin_form)
                            logger.debug(f"Generated romanization '{romanized_form}' for Baybayin '{baybayin_form}'")
                except Exception as rom_err:
                    logger.warning(f"Error generating romanization for '{word}' (Baybayin: {baybayin_form}): {rom_err}")
            
            # If baybayin has romanization but explicit romanized_form is still None, use it
            if not romanized_form and baybayin_romanized:
                romanized_form = baybayin_romanized
            # If still none but badlit has romanization, use that as fallback
            elif not romanized_form and badlit_romanized:
                romanized_form = badlit_romanized
                
            if baybayin_form or badlit_form:
            ...
            word_metadata = {"source_file": os.path.basename(filename)}
            ...
            try:
                local_stats["pronunciations"] += process_pronunciation(cur, word_id, entry, source_identifier)
            except Exception as e:
            ...
            try:
                local_stats["forms"] += process_forms_storage(cur, word_id, entry, source_identifier)
                local_stats["form_relations"] += process_form_relationships(
            ...
            try:
                if "etymology_text" in entry:
                    ety_id, ety_rels = process_etymology(
                        cur,
                        word_id,
                        ...
                        word, # Pass lemma
                        language_code # Pass language
                    )
                    if ety_id:
                        local_stats["etymologies"] += 1
                    if ety_rels:
                        local_stats["ety_relations"] += ety_rels
                        local_stats["relations"] += ety_rels  # Add to total relations too
            except Exception as e:
            ...
            try:
                if "head_templates" in entry and entry["head_templates"]:
                    local_stats["templates"] += process_head_templates(cur, word_id, entry["head_templates"])
            except Exception as e:
            ...
            if "senses" in entry and isinstance(entry["senses"], list):
                for sense_idx, sense in enumerate(entry["senses"]):
                    ...
                    try:
                        # Get definition text, clean HTML
                        ...
                        # Clean HTML from glosses
                        definition_text = "; ".join(
                            [clean_html(g) for g in glosses if isinstance(g, str)]
                        )
                        if not definition_text:
                            logger.debug(
                        ...
                        if "examples" in sense and isinstance(sense["examples"], list):
                            for example in sense["examples"]:
                                ...
                                if isinstance(example, str):
                                    ...
                                    if ex_text and tr:
                                        ex_text += f" - {clean_html(tr)}"
                                    if ex_text and ref:
                                        ex_text += f" (Ref: {clean_html(ref)})"
                                if ex_text:
                        ...
                        # Extract usage notes and clean HTML
                        usage_notes = None
                        ...
                        if definition_id:
                            try:
                                local_stats["categories"] += process_categories(
                                    cur, definition_id, sense.get("categories", [])
                                )
                            except Exception as e:
                            ...
                            try:
                                local_stats["links"] += process_links(
                                    cur, definition_id, sense.get("links", [])
                                )
                            except Exception as e:
                            ...
                            try:
                                sense_rels = process_sense_relationships(
                                    cur,
                                    word_id,
                                    sense,
                                    language_code,
                                ...
                                if sense_rels:
                                    local_stats["sense_relations"] += sense_rels
                                    local_stats["relations"] += sense_rels  # Add to total relations too
                            except Exception as e:
                                msg = f"Failed processing sense relationships for word ID {word_id}, def ID {definition_id}: {e}"
                                logger.warning(msg)
                                error_messages.append(msg)

                            # --- Process Superseded Spelling ---
                            try:
                                        ...
                                        try:
                                            ...
                                            if correct_word_id != word_id:
                                                ...
                                                if rel_id:
                                                    ...
                                                    logger.debug(f"Linked superseded spelling '{word}' to '{correct_spelling}'")
                                        except Exception as e:
                                            logger.warning(f"Error linking superseded spelling '{word}' to '{correct_spelling}': {e}")
                            except Exception as e:
                                msg = f"Failed checking superseded spelling in def ID {definition_id}: {e}"
                                logger.warning(msg)
                                error_messages.append(msg)
                    ...
                    if rel_key in entry and isinstance(entry[rel_key], list):
                        for item in entry[rel_key]:
                            if isinstance(item, dict) and "word" in item:
                                ...
                                # Clean HTML from related word
                                related_word_clean = clean_html(related_word)
                                if not related_word_clean:
                                    continue
                                
                                # Skip self-references
                                if related_word_clean.lower() == word.lower():
                                    continue
                                ...
                                try:
                                    ...
                                    # Handle bidirectional and inverse relationships
                                    ...
                                    msg = f"Failed processing top-level {rel_key} relation for word ID {word_id} -> '{related_word_clean}': {top_rel_err_inner}"
                                    logger.warning(msg)
            ...
            # Update global stats with local stats from this entry
    ...
    # Initialize romanizer if it seems to be available
    romanizer = None
    try:
        if 'BaybayinRomanizer' in globals():
            romanizer = BaybayinRomanizer()
        ...
        # Continue processing without romanizer
    ...
    try:
        with open(filename, "r", encoding="utf-8") as f:
            ...
            # Set up batch tracking - periodically commit transactions
            batch_size = 100  # Adjust based on your database and entry size
            ...
            for line in f:
                ...
                try:
                    ...
                    # Process the entry using our enhanced function
                ...
                # Periodic commit - every batch_size entries or if processing is slow
                processed_count = stats["processed_ok"] + stats["processed_with_errors"]
                if processed_count > 0 and (processed_count % batch_size == 0 or 
                                          (datetime.now() - batch_start_time).total_seconds() > 300):  # 5 minutes
                    try:
                        conn.commit()
                        logger.info(f"Committed batch after processing {processed_count} entries")
                        batch_start_time = datetime.now()  # Reset timer
                    except Exception as batch_err:
                        logger.error(f"Error committing batch at entry {entry_count}: {batch_err}")
                        # Continue processing, next batch will try again
        ...
        # Log detailed stats about the processed data
        ...
        logger.info(f"  Definitions: {entry_stats['definitions']}")
        logger.info(f"  Total relations: {entry_stats['relations']}")
        logger.info(f"    → From sense: {entry_stats['sense_relations']}")
        logger.info(f"    → From etymology: {entry_stats['ety_relations']}")
        logger.info(f"    → From forms: {entry_stats['form_relations']}")
        logger.info(f"  Etymologies: {entry_stats['etymologies']}")
        logger.info(f"  Pronunciations: {entry_stats['pronunciations']}")
        logger.info(f"  Word forms: {entry_stats['forms']}")
        logger.info(f"  Categories: {entry_stats['categories']}")
        logger.info(f"  Templates: {entry_stats['templates']}")
        logger.info(f"  Links: {entry_stats['links']}")
        logger.info(f"  Scripts (Baybayin/Badlit): {entry_stats['scripts']}")
        logger.info(f"  Examples: {entry_stats['examples']}")
        ...
        # Return stats format compatible with the original function
        return {
            ...
            "detail_stats": entry_stats  # Add detailed stats as an extra field
        }
