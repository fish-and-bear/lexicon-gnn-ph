...
def process_marayum_json(
    ...
    effective_source_identifier = source_identifier  # Use passed identifier if provided
    if not effective_source_identifier:
        # Fallback: Calculate from filename if not passed explicitly
        effective_source_identifier = SourceStandardization.standardize_sources(
            os.path.basename(filename)
        )
        logger.warning(
            f"No source_identifier passed to process_marayum_json for {filename}, calculated '{effective_source_identifier}' as fallback."
        )

    # Log using the determined identifier
    logger.info(
        f"Processing Marayum file: {filename} with source: {effective_source_identifier}"
    )  # <-- Use effective_source_identifier
    ...
    with tqdm(total=entries_in_file, desc=f"Processing {effective_source_identifier}", unit="entry") as pbar:
        for entry_index, entry in enumerate(word_entries_list): # <-- MODIFIED: Iterate over word_entries_list
            # --- Savepoint ---
            ...
            try:
                ...
                logger.error(
                    f"Failed to create savepoint {savepoint_name} for Marayum entry index {entry_index}: {e}. Skipping."
                )
                error_count += 1
                pbar.update(1)
            ...
            try:
                if not isinstance(entry, dict):
                    logger.warning(
                        f"Skipping invalid entry at index {entry_index} in {filename} (not a dict)"
                    )
                    skipped_count += 1
                    cur.execute(
                        f"RELEASE SAVEPOINT {savepoint_name}"
                    )  # Release if skipping early
                    pbar.update(1)
                    continue

                headword = entry.get("headword", "").strip()
                language_name = entry.get(
                    "language", "Unknown"
                )  # Keep original language name
                language_code = get_language_code(language_name)  # Use helper

                if not headword:
                    logger.warning(
                        f"Skipping entry at index {entry_index} in {filename} (no headword)"
                    )
                    skipped_count += 1
                    cur.execute(f"RELEASE SAVEPOINT {savepoint_name}")
                    pbar.update(1)
                ...
                word_metadata = {
                    "marayum_source": effective_source_identifier,
                ...
                try:
                    word_id = get_or_create_word_id(
                        ...
                        headword,
                        language_code=language_code,
                        source_identifier=effective_source_identifier,  # <-- Use effective_source_identifier
                        word_metadata=json.dumps(word_metadata),
                    )
                except TypeError as te:
                    logger.warning(
                        f"TypeError calling get_or_create_word_id for '{headword}', possibly due to word_metadata. Retrying without it. Error: {te}"
                    )
                    word_id = get_or_create_word_id(
                        cur,
                        headword,
                ...
                if not word_id:
                    raise ValueError(
                        f"Failed to get/create word ID for '{headword}' ({language_code}) from {effective_source_identifier}"
                    )  # <-- Use effective_source_identifier
                    ...
                    for pron in pronunciations:
                        if isinstance(pron, str) and pron.strip():
                            try:
                                ...
                                logger.warning(
                                    f"Failed pronunciation insert '{pron}' for '{headword}' from {effective_source_identifier}: {e}"
                                )  # <-- Use effective_source_identifier
                ...
                if isinstance(etymology_text_raw, str) and etymology_text_raw.strip():
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Failed etymology insert for '{headword}' from {effective_source_identifier}: {e}"
                        )  # <-- Use effective_source_identifier
                        ...
                        if definition:
                            ...
                            if examples_list:
                                try:
                                    ...
                                    logger.warning(
                                        f"Could not serialize examples for sense in '{headword}': {examples_list}"
                                    )
                            ...
                            try:
                                definition_id = insert_definition(
                                    ...
                                    definition,
                                    source_identifier=effective_source_identifier,  # <-- Use effective_source_identifier
                                    part_of_speech=standardized_pos_id,
                                    original_pos=pos,
                                    examples=examples_json,
                                )
                                if definition_id:
                                    ...
                                    for (
                                        ...
                                        if sense_rel_key in sense and isinstance(
                                            sense[sense_rel_key], list
                                        ):
                                            for sense_rel_word in sense[sense_rel_key]:
                                                if (
                                                    isinstance(sense_rel_word, str)
                                                    and sense_rel_word.strip()
                                                ):
                                                    sense_rel_word_clean = (
                                                        sense_rel_word.strip()
                                                    )
                                                    if sense_rel_word_clean != headword:
                                                        try:
                                                            sense_rel_id = get_or_create_word_id(
                                                                cur,
                                                                sense_rel_word_clean,
                                                                language_code,
                                                                source_identifier=effective_source_identifier,
                                                            ...
                                                            if sense_rel_id:
                                                                sense_rel_metadata = {
                                                                    "source": effective_source_identifier,
                                                                    "definition_id": definition_id,
                                                                    "confidence": 70,
                                                                }  # <-- Use effective_source_identifier
                                                            ...
                                                            logger.warning(
                                                                f"Error adding sense {sense_rel_key} '{sense_rel_word_clean}' for '{headword}' from {effective_source_identifier}: {rel_e}"
                                                            )  # <-- Use effective_source_identifier

                            except psycopg2.errors.UniqueViolation:
                                logger.debug(
                                    f"Marayum def exists for '{headword}', sense idx {sense_idx}: {definition[:30]}..."
                                )
                            except Exception as def_e:
                                logger.error(
                                    f"Failed def insert for '{headword}', sense idx {sense_idx}: {definition[:30]}... : {def_e}",
                                    exc_info=False,
                                )  # Keep log cleaner
                ...
                for rel_key, rel_type_enum in top_level_relation_map.items():
                    ...
                    if isinstance(related_words_list, list):
                        for related_word_str in related_words_list:
                            if (
                                isinstance(related_word_str, str)
                                and related_word_str.strip()
                            ):
                                related_word_clean = related_word_str.strip()
                                if (
                                    related_word_clean != headword
                                ):  # Avoid self-relation
                                    try:
                                        related_id = get_or_create_word_id(
                                            cur,
                                    ...
                                    except Exception as top_rel_e:
                                        logger.warning(
                                            f"Error adding top-level {rel_key} '{related_word_clean}' for '{headword}' from {effective_source_identifier}: {top_rel_e}"
                                        )  # <-- Use effective_source_identifier
                ...
                logger.error(
                    f"Error processing Marayum entry index {entry_index} ('{entry.get('headword', 'N/A')}') in {filename}: {entry_err}",
                    exc_info=False,
                )  # Keep log cleaner
                error_count += 1
                try:
                    ...
                    logger.critical(
                        f"CRITICAL: Failed to rollback savepoint {savepoint_name} for {filename}: {rb_err}. Attempting full transaction rollback."
                    )
            ...
            if (
                entries_processed_so_far > 0 and entries_processed_so_far % 500 == 0
            ):  # Commit every 500 entries
                try:
                    conn.commit()
                    logger.debug(
                        f"Committed progress for {filename} at entry {entry_index}"
                    )
                except Exception as commit_err:
                    logger.error(
                        f"Error committing progress for {filename} at entry {entry_index}: {commit_err}. Rolling back..."
                    )
                    conn.rollback()
    ...
    try:
        conn.commit()
        logger.info(
            f"Finished processing {filename}. Processed: {processed_count}, Definitions: {definitions_added}, Relations: {relations_added}, Pronunciations: {pronunciations_added}, Etymologies: {etymologies_added}, Skipped: {skipped_count}, Errors: {error_count}"
        )
    except Exception as final_commit_err:
        logger.error(
            f"Error during final commit for {filename}: {final_commit_err}. Rolling back any remaining changes..."
        )
        conn.rollback()
