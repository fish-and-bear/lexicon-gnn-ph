...
class RelationshipType(Enum):
    ...
    SPELLING_VARIANT = "spelling_variant_of"
    OBSOLETE_SPELLING = "obsolete_spelling_of"
    ARCHAIC_FORM = "archaic_form_of"
    ALTERNATIVE_FORM = "alternative_form_of"
    # Special Tagalog-specific relationships
...
def insert_relation(
    cur, from_word_id: int, to_word_id: int, relationship_type: str, 
    bidirectional: bool = False, sources: str = None, metadata: Union[Dict, str] = None
) -> Optional[int]:
    """
    Insert a relation between two words.
    If bidirectional is True, also insert the inverse relation.
    Returns relation ID if successful, None if failed.
    """
    try:
        # Skip self-relations
        ...
        if cur.fetchone()[0] != 2:
            logger.error(f"Cannot create relation - one or both words don't exist: {from_word_id}, {to_word_id}")
            return None
        ...
        cur.execute("""
            SELECT id FROM word_relationships
            WHERE from_word_id = %s AND to_word_id = %s AND relationship_type = %s
        """, (from_word_id, to_word_id, relationship_type))
        
        existing = cur.fetchone()
        if existing:
            # Update existing relation if we have new information
            ...
            if sources:
                # Append to existing sources if they exist
                cur.execute("SELECT sources FROM word_relationships WHERE id = %s", (relation_id,))
                existing_sources = cur.fetchone()[0]
                if existing_sources:
                    sources = f"{existing_sources}, {sources}"
            ...
            if update_parts:
                ...
                update_values.append(relation_id)
            ...
            cur.execute("""
                ...
                (from_word_id, to_word_id, relationship_type, sources, metadata)
                VALUES (%s, %s, %s, %s, %s)
                RETURNING id
            """, (from_word_id, to_word_id, relationship_type, sources, metadata_json))
            
            relation_id = cur.fetchone()[0]
            
        # If bidirectional, insert the inverse relation
        if bidirectional:
            # Determine inverse relationship type
            inverse_type = _get_inverse_relationship_type(relationship_type)
            
            # Check if inverse relation exists
            cur.execute("""
                SELECT id FROM word_relationships
                WHERE from_word_id = %s AND to_word_id = %s AND relationship_type = %s
            """, (to_word_id, from_word_id, inverse_type))
            ...
            if not cur.fetchone():
                ...
                cur.execute("""
                    ...
                    (from_word_id, to_word_id, relationship_type, sources, metadata)
                    VALUES (%s, %s, %s, %s, %s)
                """, (to_word_id, from_word_id, inverse_type, sources, metadata_json))
        
        return relation_id
        
    except Exception as e:
        logger.error(f"Error inserting relation: {str(e)}")
        return None


def _get_inverse_relationship_type(relationship_type: str) -> str:
    """Get the inverse of a relationship type."""
    inverse_map = {
        RelationshipType.ROOT.value: RelationshipType.ROOT_OF.value,
        RelationshipType.ROOT_OF.value: RelationshipType.ROOT.value,
        RelationshipType.DERIVED.value: RelationshipType.DERIVED_OF.value,
        RelationshipType.DERIVED_OF.value: RelationshipType.DERIVED.value,
        RelationshipType.SYNONYM.value: RelationshipType.SYNONYM.value,  # Symmetric
        RelationshipType.VARIANT.value: RelationshipType.VARIANT.value,  # Symmetric
        RelationshipType.RELATED.value: RelationshipType.RELATED.value,  # Symmetric
        RelationshipType.COMPOUND.value: "has_compound",  # Made-up inverse
        RelationshipType.ABBREVIATION.value: "abbreviated_as",  # Made-up inverse
        RelationshipType.INITIALISM.value: "initialized_as",  # Made-up inverse
        RelationshipType.BORROWED.value: "borrowed_by",  # Made-up inverse
        RelationshipType.COGNATE.value: RelationshipType.COGNATE.value,  # Symmetric
        RelationshipType.SPELLING_VARIANT.value: RelationshipType.SPELLING_VARIANT.value,  # Symmetric
        RelationshipType.OBSOLETE_SPELLING.value: "modern_spelling_of",  # Made-up inverse
        RelationshipType.ARCHAIC_FORM.value: "modern_form_of",  # Made-up inverse
        RelationshipType.ALTERNATIVE_FORM.value: RelationshipType.ALTERNATIVE_FORM.value,  # Symmetric
        RelationshipType.ALTERNATE_SPELLING.value: RelationshipType.ALTERNATE_SPELLING.value,  # Symmetric 
        RelationshipType.PRONUNCIATION_SPELLING.value: "standard_spelling_of"  # Made-up inverse
    }
...
def get_or_create_word_id(cur, lemma: str, language_code: str = "tl", **kwargs) -> int:
    """Get the ID of an existing word or create a new one."""
    ...
    preferred_spelling = kwargs.get('preferred_spelling', None)
    ...
    if result:
        # If word exists but we have new attributes, update it
        word_id = result[0]
        
        # Update tags and attributes if provided
        update_fields = []
        update_values = []
        
        if is_proper_noun:
            update_fields.append("is_proper_noun = %s")
            update_values.append(is_proper_noun)
            
        if is_abbreviation:
            update_fields.append("is_abbreviation = %s")
            update_values.append(is_abbreviation)
            
        if is_initialism:
            update_fields.append("is_initialism = %s")
            update_values.append(is_initialism)
        ...
        if preferred_spelling:
            update_fields.append("preferred_spelling = %s")
            update_values.append(preferred_spelling)
    ...
    # Process Baybayin/Badlit script detection
    if language_code == 'tl':
        if any('\u1700' <= c <= '\u171F' for c in lemma):
            has_baybayin = True
            baybayin_form = lemma
            # We'll get romanized form later through processing
...
def insert_definition(
    ...
    """Insert a new definition for a word."""
    try:
        if not text or not text.strip():
            logger.error(f"Cannot insert empty definition for word_id {word_id}")
            return None
            
        # Standardize part of speech
        standardized_pos_id = get_standardized_pos_id(cur, part_of_speech) if part_of_speech else None
        ...
        cur.execute("""
            ...
            WHERE word_id = %s 
              AND text = %s
              AND (standardized_pos_id IS NULL OR standardized_pos_id = %s)
            LIMIT 1
        """, (word_id, text, standardized_pos_id))
        
        existing = cur.fetchone()
        if existing:
            # Update the existing definition if we have new information
            existing_id = existing[0]
            update_parts = []
            update_values = []
            
            if usage_notes:
                update_parts.append("usage_notes = %s")
                update_values.append(usage_notes)
                
            if examples:
                update_parts.append("examples = %s")
                update_values.append(examples)
            ...
            if sources:
                # Append to existing sources if they exist
                cur.execute("SELECT sources FROM definitions WHERE id = %s", (existing_id,))
                existing_sources = cur.fetchone()[0]
                if existing_sources:
                    sources = f"{existing_sources}, {sources}"
                update_parts.append("sources = %s")
        ...
        cur.execute("""
            ...
            (word_id, text, standardized_pos_id, usage_notes, examples, tags, metadata, sources) 
        ...
        logger.error(f"Error inserting definition: {str(e)}")
        return None
...
def process_kaikki_jsonl(cur, filename: str):
    ...
    def extract_badlit_info(entry: Dict) -> Tuple[Optional[str], Optional[str]]:
        """Extract Badlit form and romanization from entry."""
        if 'forms' not in entry:
            return None, None
    ...
    def extract_canonical_forms(entry: Dict) -> List[str]:
        """Extract all canonical forms of a word entry."""
        if 'forms' not in entry:
            return [entry['word']]
        ...
        for form in entry['forms']:
            if 'canonical' in form.get('tags', []):
                canonical_forms.append(form['form'])
        
        # If no canonical forms found, use the main word
        if not canonical_forms:
            canonical_forms.append(entry['word'])
    ...
    def process_sense_relationships(cur, word_id: int, sense: Dict):
        """Process relationships within a sense definition."""
        # Process synonyms
        ...
        if 'alt_of' in sense:
            for alt in sense['alt_of']:
                if 'word' in alt:
                    ...
                    # Get tags from the sense (used for meta info like "abbreviation", "initialism")
                    tags = sense.get('tags', [])
                    metadata = {"tags": tags} if tags else None
                    
                    # Determine relationship type based on tags
                    rel_type = RelationshipType.VARIANT.value
                    if 'alt-of' in tags:
        ...
        # Process derived terms within sense
        if 'derived' in sense:
            ...
            for related in sense['related']:
                if isinstance(related, dict) and 'word' in related:
                    ...
                    insert_relation(
                        ...
                        word_id,
                        rel_word_id,
                        RelationshipType.RELATED.value,
                        sources=SourceStandardization.standardize_sources('kaikki.jsonl')
                    )
    
    def process_categories(cur, definition_id: int, categories: List[Dict]):
        """Process category information for a definition."""
        if not categories:
            return
    ...
    def process_head_templates(cur, word_id: int, templates: List[Dict]):
        """Process head templates information."""
        if not templates:
            return
    ...
    def process_links(cur, definition_id: int, links: List[List[str]]):
        """Process links from a definition to external references."""
        if not links:
            return
    ...
    def process_etymology(cur, word_id: int, etymology_text: str, etymology_templates: List[Dict] = None):
        """Process etymology information with structured data."""
        if not etymology_text:
            return
        ...
        # Store detailed etymology template information
        etymology_structure = None
        ...
        # Create a more structured etymology record
        cur.execute("""
            INSERT INTO etymologies (
                word_id, 
                etymology_text, 
        ...
        # Process relationships based on etymology templates
        if etymology_templates:
            for template in etymology_templates:
                template_name = template.get('name', '')
                args = template.get('args', {})
    ...
    def process_entry(cur, entry: Dict):
        ...
        try:
            ...
            entry_type = entry.get('pos', '')  # Default to POS
            is_proper_noun = (entry_type == 'name')
            ...
            if language_code == 'tl':
                ...
                if badlit_form:
                    # Store Badlit script information similar to Baybayin
                    cur.execute("""
                        UPDATE words 
                        SET badlit_form = %s,
                            romanized_form = %s
            ...
            # Process all canonical forms
            canonical_forms = extract_canonical_forms(entry)
            for idx, form in enumerate(canonical_forms):
                if idx > 0:  # Skip the first one (it's the main entry)
                    is_primary = (idx == 0)
                    cur.execute("""
                        INSERT INTO word_forms (word_id, form, is_canonical, is_primary)
                        VALUES (%s, %s, %s, %s)
                        ON CONFLICT (word_id, form) DO NOTHING
                    """, (word_id, form, True, is_primary))
            ...
            if 'hyphenation' in entry:
                hyphenation = json.dumps(entry['hyphenation'])
                cur.execute("""
                    UPDATE words 
                    SET hyphenation = %s
                    WHERE id = %s
            ...
            if 'senses' in entry:
                for sense in entry['senses']:
                    ...
                    # Extract POS from the main entry since senses often don't have it
                    pos = standardize_entry_pos(entry.get('pos', ''))
                    
                    # Extract examples
                    examples = sense.get('examples', [])
                    
                    # Extract tags like "colloquial", "formal", etc.
                    tags = sense.get('tags', [])
                    tags_str = ", ".join(tags) if tags else None
                    
                    # Extract any usage notes
                    usage_notes = []
                    ...
                    # Extract domain qualifiers
                    qualifier = sense.get('qualifier', None)
                    ...
                    wikipedia_refs = sense.get('wikipedia', [])
                    if wikipedia_refs:
                        metadata['wikipedia'] = wikipedia_refs
                    ...
                    # Process relationships within this sense
            ...
            # Process relations at the entry level
            ...
            # Process derived terms from the main entry
            if 'derived' in entry:
    ...
    try:
        ...
        # Upgrade etymologies table if it exists
        cur.execute("""
            CREATE TABLE IF NOT EXISTS etymologies (
                id SERIAL PRIMARY KEY,
                word_id INTEGER REFERENCES words(id) ON DELETE CASCADE,
        ...
        logger.error(f"Error setting up database tables for Kaikki import: {str(e)}")
        return

    # Update the get_or_create_word_id function to handle new attributes
    # This would be a new function, but we can't create it here.
    # Instead, we'll override the existing usage inline
