...
def get_semantic_network(word: str):
    ...
    try:
        ...
        try:
            ...
            # Use direct SQL to lookup the ID by word
            sql = """
            SELECT id, lemma, language_code FROM words
            WHERE LOWER(lemma) = LOWER(:word) OR LOWER(normalized_lemma) = LOWER(:normalized)
            LIMIT 1
            """
            params = {"word": word, "normalized": normalized}
        ...
        try:
            ...
            # If there's any error with caching, just log and continue without caching
            current_app.logger.warning(f"Cache error (non-critical): {str(e)}")
            pass
        ...
        try:
            ...
            current_app.logger.warning(f"Cache set error (non-critical): {str(e)}")
            pass
