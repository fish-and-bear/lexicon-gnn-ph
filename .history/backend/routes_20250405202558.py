...
class DefinitionSchema(BaseSchema):
    ...
    # Add missing fields from Definition model
    related_definitions = fields.List(fields.Nested('DefinitionSchema', exclude=("word", "related_definitions")))
    # definition_relations? Too complex for standard schema?
...
class LanguageSchema(Schema): # Added schema for Language
    """Schema for Language model."""
    code = fields.Str(required=True)
    name_en = fields.Str()
    name_tl = fields.Str()
...
def _fetch_word_details(
    ...
    # --- Add loading for new relationships ---
    # Always load language as it's fundamental info
    options.append(joinedload(Word.language))
    # Conditionally load forms and templates (could add include_forms/include_templates flags if needed)
    # For now, assume they are always loaded if WordSchema includes them
    options.append(selectinload(Word.forms))
    options.append(selectinload(Word.templates))
    # Add loading for definition relations/related definitions if needed by schema
    # options.append(selectinload(Word.related_definitions))
    # -----------------------------------------
...
def search():
    ...
    try:
        ...
        result = {
            ...
            "mode": mode,
            "language": language,
            "pos": pos, # Include pos in response if provided
            "results": []
        }
        ...
        if include_full:
            ...
            full_words_query = Word.query.options(
                ...
                # Add other relationships if needed by WordSchema and include_full implies them
