...
def api_suggestions():
    ...
    suggestions_data = []
    try:
        # Select distinct lemma and its language code
        # Order by exact match first, then length, then alphabetically
        # Use DISTINCT ON (lemma) to get only one entry per lemma
        # Note: DISTINCT ON requires the ON column(s) to be first in ORDER BY
        sql = text("""
            SELECT DISTINCT ON (lemma) lemma, language_code
            FROM words
            WHERE lemma ILIKE :query
            ORDER BY lemma = :exact_query DESC, length(lemma), lemma
            LIMIT :limit;
        """)
        
        results = db.session.execute(sql, {'query': query + '%', 'exact_query': query, 'limit': limit})
        
        # Format results as list of objects
        suggestions_data = [
            {"lemma": row[0], "language_code": row[1]} 
