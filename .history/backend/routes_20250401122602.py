...
def search_words():
    ...
    try:
        ...
        # Base query with basic text matching
        query = Word.query.options(
            joinedload(Word.definitions).load_only(
                Definition.id, Definition.definition_text, Definition.original_pos,
                Definition.standardized_pos_id, Definition.examples, Definition.usage_notes,
                Definition.tags, Definition.sources
            ).joinedload(Definition.standardized_pos).load_only(
                PartOfSpeech.id, PartOfSpeech.code, PartOfSpeech.name_en,
                PartOfSpeech.name_tl, PartOfSpeech.description
            )
        ).filter(
            or_(
