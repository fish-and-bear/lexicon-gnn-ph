...
def process_tagalog_words(cur, filename: str):
    ...
    try:
        ...
        if isinstance(data, dict):
            ...
            logger.info(f"Found {total_entries} entries in tagalog-words.json")
                ...
                for word, entry in data.items():
                    ...
                    try:
                        ...
                        if etymology:
                            ...
                            # Try to extract language codes from etymology
                            language_codes = ""
                            if "Esp" in etymology:
                                language_codes = "es"  # Spanish
                            elif "Eng" in etymology:
                                language_codes = "en"  # English
                            elif "Ch" in etymology or "Tsino" in etymology:
                                language_codes = "zh"  # Chinese
                            elif "Jap" in etymology:
                                language_codes = "ja"  # Japanese
                            elif "San" in etymology or "Sanskrit" in etymology:
                                language_codes = "sa"  # Sanskrit
                        ...
                        # Try to extract relationships from definitions
                        for definition in definitions:
                            if isinstance(definition, str):
                                # Look for synonyms
                                synonym_matches = re.findall(r'ka(singkahulugan|tulad) ng\s+(\w+)', definition, re.IGNORECASE)
                                for match_type, synonym in synonym_matches:
                                    if synonym:
                                ...
                                # Look for antonyms
                                antonym_matches = re.findall(r'(kasalungat|kabaligtaran) ng\s+(\w+)', definition, re.IGNORECASE)
                                for match_type, antonym in antonym_matches:
                                    if antonym:
                                        ant_id = get_or_create_word_id(cur, antonym, language_code='tl')
                                        insert_relation(
                                            cur,
                                            word_id,
                                            ant_id,
                        ...
                        # Create an equivalent structure for relationship processing
                        relationship_data = {
                            'word': word,
                            'language_code': 'tl',
                            'derivative': derivative,
                            'etymology': etymology,
                            'definitions': [{'text': d} for d in definitions if d]
                        }
                        
                        # Process relationships from other fields
                        process_relationships(cur, word_id, relationship_data, SourceStandardization.standardize_sources('tagalog-words.json'))
                        
                    except Exception as e:
                        logger.error(f"Error processing tagalog word {word}: {str(e)}")
                    
                    pbar.update(1)
        else:
            # Format 2: List of word entries
            total_entries = len(data)
            logger.info(f"Found {total_entries} entries in list-style tagalog-words.json")
            
            with tqdm(total=total_entries, desc="Processing Tagalog words (list format)") as pbar:
                for entry in data:
                    ...
                    try:
                        ...
                        # Create data structure for relationship processing
                        relationship_data = {
                            'word': word,
                            'language_code': 'tl',
                        ...
                        logger.error(f"Error processing tagalog word list entry: {str(e)}")
...
def process_kwf_dictionary(cur, filename: str):
    """
    Process KWF dictionary entries with enhanced relationship extraction.
    
    This function processes the KWF dictionary file which contains rich
    semantic information including synonyms, antonyms, and related terms.
    """
    ...
    try:
        ...
        if not isinstance(data, dict):
            logger.warning(f"KWF dictionary file has unexpected format. Expected dictionary, got {type(data)}")
            return
        ...
        with tqdm(total=total_entries, desc="Processing KWF entries") as pbar:
            for word, entry in data.items():
                if not word or not isinstance(entry, dict):
                    logger.warning(f"Skipping invalid KWF entry: {word}")
                    pbar.update(1)
                    continue
                ...
                try:
                            ...
                            if isinstance(def_list, list):
                                ...
                                for def_item in def_list:
                                    if isinstance(def_item, dict):
                                        # Get definition text
                                        definition = def_item.get('meaning', '')
                                        if definition:
                                            insert_definition(
                                                cur, 
                                                word_id, 
                    ...
                    if 'metadata' in entry and isinstance(entry['metadata'], dict):
                        etymology_data = entry['metadata'].get('etymology', [])
                        if isinstance(etymology_data, list) and etymology_data:
                            for etym in etymology_data:
                                if isinstance(etym, dict):
                                    etym_text = etym.get('text', '')
                                    if etym_text:
                                        source_langs = etym.get('source_language', [])
                                        lang_codes = ", ".join(source_langs) if isinstance(source_langs, list) else ""
                                        components = extract_etymology_components(etym_text)
                                        normalized_components = json.dumps(components) if components else None
                    ...
                    # Process relationships (synonyms, antonyms, related terms, etc.)
                    process_relationships(cur, word_id, entry, SourceStandardization.standardize_sources('kwf_dictionary.json'))
                    
                except Exception as e:
                    logger.error(f"Error processing KWF entry '{word}': {str(e)}")
...
def process_kaikki_jsonl(cur, filename: str):
    ...
    logger.info(f"Processing Kaikki data from: {filename}")
    
    # Determine language code from filename
    language_code = 'ceb' if 'ceb' in filename.lower() else 'tl'
    logger.info(f"Using language code: {language_code}")
    
    def extract_baybayin_info(entry: Dict) -> Tuple[Optional[str], Optional[str]]:
        """Extract Baybayin script and romanized form from entry."""
    ...
    def standardize_entry_pos(pos_str: str) -> str:
        """Standardize part of speech from Kaikki format."""
    ...
    try:
        ...
        logger.info(f"Found {line_count} entries in {filename}")
        
        with open(filename, 'r', encoding='utf-8') as f:
            with tqdm(total=line_count, desc=f"Processing {os.path.basename(filename)}") as pbar:
                for line in f:
                    try:
                        ...
                        # Get language code from entry or use default
                        entry_lang = entry.get('language_code') or entry.get('lang_code', language_code)
                        
                        # Create word entry
                        word_id = get_or_create_word_id(cur, word, language_code=entry_lang)
                        
                        # Process Baybayin form if available
                        baybayin_form, romanized_form = extract_baybayin_info(entry)
                        if baybayin_form:
                            try:
                                process_baybayin_data(cur, word_id, baybayin_form, romanized_form)
                            except Exception as e:
                                logger.warning(f"Invalid Baybayin form for word {word}: {str(e)}")
                        ...
                        if isinstance(definitions, list):
                            for definition in definitions:
                                if isinstance(definition, dict):
                                    # Get definition text from gloss or definition
                                    def_text = definition.get('gloss') or definition.get('definition', '')
                                    if def_text:
                        ...
                        if 'etymology' in entry and entry['etymology']:
                            ...
                            # Extract language codes if available
                            etym_langs = entry.get('etymology_languages', [])
                            lang_codes = ", ".join(etym_langs) if isinstance(etym_langs, list) else ""
                            components = extract_etymology_components(etym_text)
                            normalized_components = json.dumps(components) if components else None
                        ...
                        process_relationships(cur, word_id, entry, SourceStandardization.standardize_sources(os.path.basename(filename)))
                        
                        processed_count += 1
                        
                    except Exception as e:
                        error_count += 1
                        logger.error(f"Error processing entry {pbar.n+1}: {str(e)}")
                    
                    pbar.update(1)
        
        logger.info(f"Completed processing {filename}. Processed {processed_count} entries with {error_count} errors.")
