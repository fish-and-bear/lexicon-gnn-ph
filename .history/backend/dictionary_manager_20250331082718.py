...
def process_marayum_json(cur, filename: str):
    ...
    def clean_word(word: str) -> str:
        """Clean word while preserving special characters and diacritics."""
        return word.strip().replace('\n', ' ').replace('  ', ' ')
    
    def clean_pronunciation(pron: str) -> str:
        """Clean and validate IPA pronunciation."""
        pron = pron.strip().replace('..', '.').replace('  ', ' ')
        if not (pron.startswith('[') and pron.endswith(']')):
            pron = f"[{pron.strip('[]')}]"
        return pron
    
    def clean_credits(credits: str) -> str:
        """Clean and standardize credits format."""
        return (credits.replace('\n', ' ')
                .replace('      ', ' ')
                .replace('  ', ' ')
                .strip())
    
    def get_alphabet_key(word: str) -> str:
        """Get the alphabet key while handling diacritics properly."""
        normalized = ''.join(c for c in unicodedata.normalize('NFD', word)
                           if unicodedata.category(c) != 'Mn')
        return normalized[0].upper() if normalized else ''
    ...
    try:
        ...
        if not isinstance(data, dict):
            logger.error(f"Invalid JSON format in {filename} - expected a dictionary")
            return 0, 0
            
        # Validate required sections
        required_sections = ['dictionary_info', 'statistics', 'words']
        missing_sections = [section for section in required_sections if section not in data]
        if missing_sections:
            logger.error(f"Missing required sections in {filename}: {', '.join(missing_sections)}")
            return 0, 0
            
        # Get dictionary info
        dict_info = data['dictionary_info']
        dict_name = dict_info.get('name', os.path.basename(filename).replace('_processed.json', ''))
        
        # Validate dictionary info
        required_dict_info = ['id', 'name', 'base_language', 'target_language']
        missing_dict_info = [field for field in required_dict_info if not dict_info.get(field)]
        if missing_dict_info:
            logger.error(f"Missing required dictionary info in {filename}: {', '.join(missing_dict_info)}")
            return 0, 0
            
        # Get word counts from multiple sources
        dict_total = dict_info.get('total_words', 0)
        stats_total = data.get('statistics', {}).get('total_words', 0)
        words = data.get('words', [])
        actual_total = len(words) if isinstance(words, list) else 0
        
        # Use the most reliable count
        total_words = actual_total  # Prefer actual count over reported counts
        
        if total_words == 0:
            logger.info(f"Dictionary {dict_name} has no words")
            return 0, 0
            
        if not isinstance(words, list):
            logger.error(f"Invalid words format in {filename} - expected an array")
            return 0, 0
            
        stats["total_entries"] = total_words
        source = os.path.basename(filename).replace('_processed.json', '')
        
        # Get base language code
        base_lang = dict_info['base_language']
        lang_code = base_lang.lower().replace(' ', '-')
        
        # Group entries by base word (removing numbers)
        word_groups = {}
        for entry in words:
            try:
                if not isinstance(entry, dict):
                    logger.warning(f"Invalid entry format in {filename} - skipping")
                ...
                # Remove trailing numbers to get base word
                base_word = re.sub(r'\d+$', '', word).strip()
                if not base_word:
                    stats["entries_skipped"] += 1
                    continue
                
                # Track for alphabet index
                alpha_key = get_alphabet_key(base_word)
                if alpha_key:
                    if alpha_key not in alphabet_index:
                        alphabet_index[alpha_key] = []
                    ...
                    if entry_info not in alphabet_index[alpha_key]:
                        alphabet_index[alpha_key].append(entry_info)
                
                if base_word not in word_groups:
                    word_groups[base_word] = []
                word_groups[base_word].append(entry)
                
            except Exception as e:
                logger.error(f"Error grouping entry in {filename}: {str(e)}")
                stats["errors"] += 1
                continue
        
        # Process each word group
        for base_word, entries in word_groups.items():
            try:
                # Create or get word entry for the base word
                word_id = get_or_create_word_id(
                    cur,
                    base_word,
                    language_code=lang_code,
                    check_exists=True,
                    is_core=any(e.get('is_core', False) for e in entries)
                )
                ...
                # Track if we've consolidated multiple entries
                if len(entries) > 1:
                    stats["words_consolidated"] += 1
                    logger.info(f"Consolidated {len(entries)} entries for word '{base_word}'")
                
                # Process all entries for this word
                all_definitions = []
                all_examples = []
                ...
                for entry in entries:
                    try:
                        ...
                        if credits:
                            credits = clean_credits(credits)
                            if credits:
                                all_credits.add(credits)
                        
                        # Collect etymologies
                        etymology = entry.get('etymology', '').strip()
                        if etymology:
                            all_etymologies.add(etymology)
                                    ...
                                    for ex in examples:
                                        if isinstance(ex, dict):
                                            ...
                                            if text and translation:
                                                def_info['examples'].append({
                                                    'text': text,
                                                    'translation': translation,
                                                    'example_id': ex.get('example_id', 1)
                                                })
                                
                                all_definitions.append(def_info)
                        
                    except Exception as e:
                        logger.error(f"Error processing entry data for '{base_word}' in {filename}: {str(e)}")
                        stats["errors"] += 1
                        continue
                ...
                for pron in all_pronunciations:
                    try:
                        ...
                        logger.warning(f"Error adding pronunciation for '{base_word}' in {filename}: {str(e)}")
                
                for etym in all_etymologies:
                    try:
                        if insert_etymology(
                            cur,
                            word_id,
                        ...
                        logger.warning(f"Error adding etymology for '{base_word}' in {filename}: {str(e)}")
                ...
                for credit in all_credits:
                    try:
                        ...
                        logger.warning(f"Error adding credits for '{base_word}' in {filename}: {str(e)}")
                
                # Add all definitions
                for def_info in all_definitions:
                    try:
                        examples_json = json.dumps(def_info['examples']) if def_info['examples'] else None
                        
                        def_id = insert_definition(
                            cur,
                            word_id,
                            def_info['text'],
                            part_of_speech=def_info['pos'],
                            examples=examples_json,
                            sources=source
                        )
                        ...
                        if def_id:
                            ...
                            process_definition_relations(cur, def_id, def_info['text'], source)
                            
                    except Exception as e:
                        logger.warning(f"Error processing definition for '{base_word}' in {filename}: {str(e)}")
                        stats["errors"] += 1
                        continue
                    ...
                    if isinstance(see_also, list):
                        for ref in see_also:
                            if isinstance(ref, dict):
                                ...
                                if text:
                                    ref_base = re.sub(r'\d+$', '', text)
                                    if ref_base != base_word:
                                        all_refs.add(ref_base)
                ...
                if all_refs:
                    relations_dict = {'see_also': list(all_refs)}
                    try:
                        process_relations(cur, word_id, relations_dict, lang_code, source)
                    except Exception as e:
                        logger.warning(f"Error processing relations for '{base_word}' in {filename}: {str(e)}")
                
                stats["entries_processed"] += 1
                
            except Exception as e:
                logger.error(f"Error processing word group '{base_word}' in {filename}: {str(e)}")
                stats["errors"] += 1
                continue
        
        # Sort alphabet index
        sorted_index = {}
        for key in sorted(alphabet_index.keys()):
            sorted_index[key] = sorted(alphabet_index[key], key=lambda x: x['word'])
        
        # Update the file with the new alphabet index
        data['alphabet_index'] = sorted_index
        with open(filename, 'w', encoding='utf-8') as f:
            json.dump(data, f, indent=2, ensure_ascii=False)
        
        logger.info(f"Processed {stats['entries_processed']} entries from {filename}")
        logger.info(f"Consolidated {stats['words_consolidated']} word groups")
        ...
        return stats["entries_processed"], stats["errors"]
...
def process_marayum_directory(cur, directory_path: str):
    ...
    for json_file in json_files:
        try:
            ...
            try:
                with open(json_file, 'r', encoding='utf-8') as f:
                    try:
                        data = json.load(f)
                        if not isinstance(data, dict):
                            logger.error(f"Invalid JSON structure in {json_file} - not a dictionary")
                            total_files_skipped += 1
                            continue
                            
                        # Check required sections
                        if not all(section in data for section in ['dictionary_info', 'statistics', 'words']):
                            logger.error(f"Missing required sections in {json_file}")
                            total_files_skipped += 1
                            continue
                            
                        # Check if dictionary has any words
                        words = data.get('words', [])
                        if not isinstance(words, list):
                            logger.error(f"Invalid words section in {json_file}")
                            total_files_skipped += 1
                            continue
                            
                        # Get dictionary info for better logging
                        dict_info = data.get('dictionary_info', {})
                        dict_name = dict_info.get('name', dict_name)
                        total_words = len(words)
                        
                        logger.info(f"Found {total_words} words in {dict_name}")
            ...
            try:
                ...
                # Get consolidation stats from the processed file
                try:
                    with open(json_file, 'r', encoding='utf-8') as f:
                        updated_data = json.load(f)
                        if isinstance(updated_data, dict):
                            stats = updated_data.get('statistics', {})
                            if isinstance(stats, dict):
                                consolidated = stats.get('words_consolidated', 0)
                                total_consolidated += consolidated
                                logger.info(f"Consolidated {consolidated} word groups in {dict_name}")
                except Exception:
                    pass  # Ignore errors reading consolidation stats
                
                logger.info(f"Marayum processing complete for {dict_name}: {processed} processed with {errors} errors")
            ...
            logger.error(f"Error accessing file {json_file}: {str(e)}")
            total_errors += 1
            total_files_skipped += 1
            continue
    ...
    if total_processed > 0 or total_errors > 0:
        ...
        logger.info(f"  Total words consolidated: {total_consolidated}")
