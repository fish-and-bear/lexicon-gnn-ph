...
class Word(db.Model):
    ...
    source_relations = relationship('Relation', foreign_keys='Relation.source_word_id', back_populates='source_word', lazy='joined')
    target_relations = relationship('Relation', foreign_keys='Relation.target_word_id', back_populates='target_word', lazy='joined')
    ...
    def search(cls, query_text, language_code=None):
        ...
        if is_testing_db(db.session.get_bind()):
            # SQLite fallback
            ...
            base_query = cls.query.filter(
                db.or_(
                    cls.normalized_lemma.ilike(f'%{query_text}%'),
                    cast(cls.search_text, TSVECTOR).op('@@')(search_query)
                )
        ...
        return base_query.order_by(func.similarity(cls.lemma, query_text).desc())
...
class Relation(db.Model):
    ...
    source_word = db.relationship('Word', foreign_keys=[source_word_id], back_populates='source_relations')
    target_word = db.relationship('Word', foreign_keys=[target_word_id], back_populates='target_relations')
...
class PartOfSpeech(db.Model):
    ...
    # Relationships
    definitions = db.relationship('Definition', back_populates='standardized_pos')
