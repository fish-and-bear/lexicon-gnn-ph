...
def process_marayum_json(cur, filename: str):
    ...
    try:
        ...
        raw_language_code = dict_info.get("base_language", "").lower()
        if not raw_language_code:
            # Try to extract from filename
            filename_parts = os.path.basename(filename).split('-')
            if len(filename_parts) > 0:
                raw_language_code = filename_parts[0].lower()
            else:
                raw_language_code = "tl"  # Default to Tagalog if no language found
        ...
        if len(raw_language_code) > 3 or "-" in raw_language_code or "'" in raw_language_code:
            ...
            language_code = raw_language_code[:3]  # Take first 3 chars
        ...
        for entry_idx, entry in enumerate(entries, 1):
            try:
                ...
                try:
                    ...
                    # Clean pronunciation (remove brackets and normalize)
                    raw_pronunciation = entry.get("pronunciation", "")
                    pronunciation = raw_pronunciation.strip('[]').strip() if raw_pronunciation else ""
                    
                    etymology = entry.get("etymology", "").strip()
                    credits = entry.get("credits", "").strip()
                    comment = entry.get("comment", "").strip()
                    dialect = entry.get("dialect", "").strip()
                    is_core = entry.get("is_core", False)
                    
                    # Combine metadata as tags
                    tags = []
                    if comment:
                    ...
                    if dict_info.get("name"):
                        tags.append(f"dictionary:{dict_info['name']}")
                    ...
                    if pronunciation:
                        try:
                            pron_data = {
                                'type': 'ipa',
                                'value': pronunciation,
                                'metadata': {'original': raw_pronunciation} if raw_pronunciation != pronunciation else None,
                                'sources': source
                            }
                            pron_id = insert_pronunciation(cur, word_id, pron_data)
                            if pron_id:
                    ...
                    if credits:
                        try:
                            ...
                            credit_id = insert_credit(cur, word_id, credit_data)
                            if credit_id:
                    ...
                    if isinstance(definitions, list):
                        for def_item in definitions:
                            ...
                            # Insert definition with standardized POS
                            try:
                                def_id = insert_definition(
                                    cur,
                                    word_id,
                                    definition_text,
                                    part_of_speech=standardize_entry_pos(pos) if pos else "",
                        ...
                        for rel in see_also:
                            try:
                                ...
                                if rel_word:
                                    ...
                                    if rel_id:
                                        cur.execute("SELECT id FROM words WHERE id = %s", (rel_id,))
                                        result = cur.fetchone()
                                        if result:
                                            rel_id = result[0]
                                    
                                    # If not found by ID, get or create by word
                                    if not rel_id:
