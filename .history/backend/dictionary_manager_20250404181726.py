...
from psycopg2.extras import Json # Make sure Json is imported correctly
from typing import Optional, Tuple, Dict, Union, List
from tqdm import tqdm
from bs4 import BeautifulSoup # For robust HTML cleaning
...
# --- Replace the insert_definition function (around line 2924) ---
@with_transaction(commit=True)
def insert_definition(
    ...
    examples: Optional[str] = None, # Should be JSON string or None
    usage_notes: Optional[str] = None,
    tags: Optional[str] = None, # Comma-separated string or None
    # metadata: Optional[Union[dict, str]] = None # Removed, store in usage_notes or dedicated field if needed
) -> Optional[int]:
    """
    ...
    Args:
        ...
        examples: JSON string representation of example sentences (optional).
    ...
    try:
        # Get standardized part of speech ID using the original POS string
        # get_standardized_pos_id now handles standardization internally
        standardized_pos_id = get_standardized_pos_id(cur, part_of_speech)

        # Truncation checks (optional, uncomment if needed)
        # max_def_length = 65535 # Example max length
        # if len(definition_text) > max_def_length:
        #     logger.warning(f"Definition text for word ID {word_id} truncated.")
        #     definition_text = definition_text[:max_def_length]

        # max_examples_length = 65535
        # if examples and len(examples) > max_examples_length:
        #      logger.warning(f"Examples for word ID {word_id} truncated.")
        ...
        cur.execute(
            ...
            DO UPDATE SET
                ...
                original_pos = COALESCE(EXCLUDED.original_pos, definitions.original_pos), -- Update original if new one provided
                updated_at = CURRENT_TIMESTAMP
            RETURNING id
            """,
            {
                ...
                "pos_id": standardized_pos_id, # Use the obtained ID
                "examples": examples, # Pass JSON string or None
                "usage_notes": usage_notes,
                "tags": tags,
                "sources": source_identifier, # Use the mandatory source_identifier
                "original_pos": part_of_speech, # Store the ORIGINAL string here
            },
        )
...
# --- Helper function to clean HTML (place outside process_kaikki_jsonl) ---
def clean_html(raw_html: Optional[str]) -> str:
    """Removes HTML tags from a string."""
    if not raw_html or not isinstance(raw_html, str):
        return ""
...
# --- The Main Processing Function (Modified) ---
@with_transaction(
    commit=False
)  # Keep commit=False assuming called by migrate_data
def process_kaikki_jsonl(cur, filename: str) -> Tuple[int, int]: # Return (processed_count, issues_count)
    """Process Kaikki.org dictionary entries."""
    # --- Initialize statistics (more detailed) ---
    stats = {
        "total_entries": 0, "processed_ok": 0, "processed_with_errors": 0,
        "failed_entries": 0, "skipped_json_errors": 0, "definitions": 0,
        "relations": 0, "etymologies": 0, "pronunciations": 0, "forms": 0,
        "categories": 0, "templates": 0, "links": 0, "scripts": 0,
        "ety_relations": 0, "sense_relations": 0, "form_relations": 0, # Detailed relation counts
        "examples": 0,
    }
    ...
    # --- Schema Check (Unchanged - keep as is in the original) ---
    try:
        cur.execute("""CREATE TABLE IF NOT EXISTS pronunciations (...)""") # Truncated for brevity
        cur.execute("""CREATE TABLE IF NOT EXISTS definition_categories (...)""")
        cur.execute("""CREATE TABLE IF NOT EXISTS word_templates (...)""")
        cur.execute("""CREATE TABLE IF NOT EXISTS word_forms (...)""")
        cur.execute("""CREATE TABLE IF NOT EXISTS definition_links (...)""")
        cur.execute("""DO $$ BEGIN ... END $$;""") # ALTER TABLE statements
        conn.commit()
        logger.info("Checked/committed schema changes for Kaikki processing.")
    except Exception as e:
        logger.error(f"Error during schema setup/check for Kaikki processing: {str(e)}")
        conn.rollback()
        return 0, stats.get("total_entries", 1)

    # --- Instantiate Romanizer ---
    romanizer = BaybayinRomanizer()

    # --- Nested Helper Functions (Modified where needed) ---

    RELATIONSHIP_TYPE_MAPPING = { # Kept inside, update if needed
        "borrowed_from": RelationshipType.DERIVED_FROM.rel_value,
        "cognate_with": RelationshipType.RELATED.rel_value,
    }

    def get_standard_relationship_type(rel_type_raw):
        # (Keep original implementation using RELATIONSHIP_TYPE_MAPPING and RelationshipType enum)
        rel_type = rel_type_raw.lower().strip()
        if rel_type in RELATIONSHIP_TYPE_MAPPING:
            return RELATIONSHIP_TYPE_MAPPING[rel_type]
        try:
            for rel_enum in RelationshipType:
                if rel_type == rel_enum.name.lower() or rel_type == rel_enum.rel_value:
                    return rel_enum.rel_value
            logger.debug(f"Unknown relationship type '{rel_type_raw}', using RELATED.")
            return RelationshipType.RELATED.rel_value
        except Exception:
             logger.warning(f"Error standardizing relationship type '{rel_type_raw}', using RELATED.")
             return RelationshipType.RELATED.rel_value
    ...
    def extract_script_info(entry: Dict, script_tag: str, script_name_in_template: str) -> Tuple[Optional[str], Optional[str]]:
        ...
        if "forms" in entry and isinstance(entry["forms"], list):
            for form_data in entry["forms"]:
                if isinstance(form_data, dict) and script_tag in form_data.get("tags", []):
                    ...
                    if form_text:
                        ...
                        # Basic validation - check for non-Latin chars as proxy
    ...
    def extract_baybayin_info(entry: Dict) -> Tuple[Optional[str], Optional[str]]:
         # Calls the generalized version
         return extract_script_info(entry, "Baybayin", "Baybayin")

    def extract_badlit_info(entry: Dict) -> Tuple[Optional[str], Optional[str]]:
         # Calls the generalized version
         return extract_script_info(entry, "Badlit", "Badlit")
    ...
    def standardize_entry_pos(pos_str: Optional[str]) -> str:
        # (Keep original implementation)
    ...
    def process_pronunciation(cur, word_id: int, entry: Dict, source_identifier: str) -> int:
        ...
        if "sounds" in entry and isinstance(entry["sounds"], list):
            for sound in entry["sounds"]:
                ...
                if "ipa" in sound and sound["ipa"]: pron_data["value"] = sound["ipa"]; pron_data.setdefault("type", "ipa")
                elif "enpr" in sound and sound["enpr"]: pron_data["value"] = sound["enpr"]; pron_data.setdefault("type", "enpr")
                elif "rhymes" in sound and sound["rhymes"]: pron_data["value"] = sound["rhymes"]; pron_data.setdefault("type", "rhyme")
                elif "audio" in sound and sound["audio"]: pron_data["value"] = sound["audio"]; pron_data.setdefault("type", "audio")
                else: continue

                if pron_data.get("value"):
                    try:
                        pron_inserted_id = insert_pronunciation(cur, word_id, pron_data, source_identifier)
                        if pron_inserted_id: count += 1
                    except Exception as e: logger.warning(f"Error inserting pronunc. for word ID {word_id}, type {pron_data.get('type')}: {e}")
        return count

    def process_sense_relationships(cur, word_id: int, sense: Dict, language_code: str, source_identifier: str, word_lemma: str) -> int:
        # (Modified to use RelationshipType enum and handle directionality/inverses, returns count)
        relations_added = 0
        relation_types = {
            "synonyms": RelationshipType.SYNONYM, "antonyms": RelationshipType.ANTONYM,
            "hypernyms": RelationshipType.HYPERNYM, "hyponyms": RelationshipType.HYPONYM,
            "holonyms": RelationshipType.HOLONYM, "meronyms": RelationshipType.MERONYM,
            "derived": RelationshipType.ROOT_OF, # Corrected: word_id is ROOT_OF derived word
            "related": RelationshipType.RELATED, "coordinate_terms": RelationshipType.RELATED,
        ...
        for rel_key, rel_enum in relation_types.items():
            if rel_key in sense and isinstance(sense[rel_key], list):
                for item in sense[rel_key]:
                    related_word = None; metadata = {"from_sense": True}
                    if isinstance(item, dict) and "word" in item:
                        related_word = item["word"]
                        # Extract metadata (cleaned up)
                        for meta_key in ["tags", "qualifier", "_dis1"]:
                             if meta_key in item and item[meta_key]:
                                 # Rename _dis1 for clarity
                                 db_key = "distance" if meta_key == "_dis1" else meta_key
                                 metadata[db_key] = item[meta_key]
                    elif isinstance(item, str): related_word = item
                    else: continue

                    related_word_clean = clean_html(related_word) # Clean HTML from related term
                    if not related_word_clean or related_word_clean.lower() == word_lemma.lower(): continue

                    try:
                        related_word_id = get_or_create_word_id(cur, related_word_clean, language_code, source_identifier)
                        if related_word_id == word_id: continue

                        # Determine primary direction based on the key (esp. for non-bidirectional)
                        from_id, to_id, current_rel_enum = word_id, related_word_id, rel_enum
                        if rel_key == "derived": # word_id -> ROOT_OF -> derived_id
                             pass # Direction is already correct with rel_enum=ROOT_OF
                        elif rel_key == "hyponyms": # hyponym_id -> HYPONYM_OF -> word_id (current word is hypernym)
                             from_id, to_id = related_word_id, word_id
                             current_rel_enum = RelationshipType.HYPONYM # Use HYPONYM relationship
                        elif rel_key == "hypernyms": # word_id -> HYPONYM_OF -> hypernym_id
                             current_rel_enum = RelationshipType.HYPONYM # Use HYPONYM relationship
                        # Add similar logic for meronym/holonym if needed

                        rel_id_1 = insert_relation(cur, from_id, to_id, current_rel_enum, source_identifier, Json(metadata) if metadata else None) # Use Json wrapper for metadata
                        if rel_id_1: relations_added += 1

                        # Handle inverse/bidirectional
                        inverse_rel = current_rel_enum.get_inverse()
                        if current_rel_enum.bidirectional:
                            insert_relation(cur, to_id, from_id, current_rel_enum, source_identifier, Json(metadata) if metadata else None)
                        elif inverse_rel:
                            insert_relation(cur, to_id, from_id, inverse_rel, source_identifier, Json(metadata) if metadata else None)
                    except Exception as e: logger.error(f"Error processing sense relation {rel_key} for {word_id} -> '{related_word_clean}': {e}")
        return relations_added

    def process_forms_storage(cur, word_id: int, entry: Dict, source_identifier: str) -> int:
        # (Renamed from process_form_relationships, focuses only on storing forms)
        count = 0
        if "forms" in entry and isinstance(entry["forms"], list):
            for form_data in entry["forms"]:
                ...
                is_primary = "primary" in form_tags # Check for primary flag if needed

                # Skip storing Baybayin/Badlit forms here if already handled by script extraction
                ...
                try:
                    cur.execute(
                        """INSERT INTO word_forms (word_id, form, is_canonical, is_primary, tags)
                           VALUES (%s, %s, %s, %s, %s)
                           ON CONFLICT (word_id, form) DO UPDATE SET
                           is_canonical = EXCLUDED.is_canonical,
                           is_primary = EXCLUDED.is_primary,
                           tags = COALESCE(word_forms.tags, '{}'::jsonb) || EXCLUDED.tags,
                           updated_at = CURRENT_TIMESTAMP""",
                        (word_id, form_text, is_canonical, is_primary, tags_json)
                    )
    ...
    def process_categories(cur, definition_id: int, categories: List[Dict]) -> int:
        # (Keep original implementation, return count)
        count = 0
            ...
            try:
                cur.execute(
                    """INSERT INTO definition_categories (definition_id, category_name, category_kind, parents)
                       VALUES (%s, %s, %s, %s) ON CONFLICT (definition_id, category_name) DO NOTHING""",
                    (definition_id, category["name"], category.get("kind"), Json(category.get("parents")) if category.get("parents") else None)
                )
                if cur.rowcount > 0: count += 1
            except Exception as e: logger.error(f"Error inserting category '{category.get('name')}' for def ID {definition_id}: {e}")
        return count
    ...
    def process_head_templates(cur, word_id: int, templates: List[Dict]) -> int:
        ...
        for template in templates:
            ...
            try:
                cur.execute(
                    """INSERT INTO word_templates (word_id, template_name, args, expansion) VALUES (%s, %s, %s, %s)
                       ON CONFLICT (word_id, template_name) DO UPDATE SET
                       args = EXCLUDED.args, expansion = EXCLUDED.expansion, updated_at = CURRENT_TIMESTAMP""",
                    (word_id, template["name"], Json(template.get("args")) if template.get("args") else None, template.get("expansion"))
                )
                if cur.rowcount > 0: count += 1
            except Exception as e: logger.error(f"Error inserting template '{template.get('name')}' for word ID {word_id}: {e}")
        return count
            ...
            try:
                cur.execute(
                    """INSERT INTO definition_links (definition_id, link_text, link_target, is_wikipedia)
                       VALUES (%s, %s, %s, %s) ON CONFLICT (definition_id, link_text, link_target) DO NOTHING""",
                    (definition_id, link_text, link_target, is_wikipedia)
                )
                if cur.rowcount > 0: count += 1
            except Exception as e: logger.error(f"Error inserting link for def ID {definition_id}: {e}")
        return count

    # --- process_entry (nested function - Modified) ---
    def process_entry(cur, entry: Dict, source_identifier: str, romanizer: BaybayinRomanizer) -> Tuple[Optional[int], Optional[str]]:
        """Processes a single Kaikki entry, returns (word_id, error_string | None)."""
        ...
        try:
            # --- Basic Info ---
            pos_raw = entry.get("pos", "unk")
            language_code = entry.get("lang_code", DEFAULT_LANGUAGE_CODE)
            if not language_code or len(language_code) > 10: language_code = DEFAULT_LANGUAGE_CODE

            # --- Script Extraction & Romanization ---
            script_tag_found, script_form, script_romanized_explicit, badlit_form = None, None, None, None
            try:
                 # Use the generalized helper (assuming extract_script_info is defined outside or above)
                 script_form_bay, script_romanized_bay = extract_script_info(entry, "Baybayin", "Baybayin")
                 script_form_bad, script_romanized_bad = extract_script_info(entry, "Badlit", "Badlit")

                 # Prioritize (arbitrarily, adjust if needed)
                 if script_form_bay:
                     script_tag_found = "Baybayin"
                     script_form = script_form_bay
                     script_romanized_explicit = script_romanized_bay
                 elif script_form_bad:
                     script_tag_found = "Badlit"
                     script_form = script_form_bad
                     script_romanized_explicit = script_romanized_bad
                     # If using Option B schema for Badlit:
                     # badlit_form = script_form

                 # Generate Romanization if missing
                 script_romanized_final = None
                 if script_form:
                     if script_romanized_explicit:
                         script_romanized_final = script_romanized_explicit
                     else:
                         try:
                             if romanizer.validate_text(script_form): # Basic validation
                                 script_romanized_final = romanizer.romanize(script_form)
                                 logger.debug(f"Generated romanization '{script_romanized_final}' for {script_tag_found} '{script_form}'.")
                             else:
                                 logger.warning(f"Invalid {script_tag_found or 'script'} structure for '{lemma}', cannot romanize: {script_form}")
                         except Exception as rom_err:
                             logger.error(f"Error generating romanization for '{lemma}' ({script_tag_found}: {script_form}): {rom_err}")
                 if script_form: stats["scripts"] += 1

            except Exception as form_err:
                 logger.warning(f"Error during script extraction for '{lemma}': {form_err}")
                 error_messages.append(f"Script extraction error: {form_err}")

            # --- Word Metadata & Tags ---
            word_metadata_to_store = {"source_file": raw_source_identifier}
            if script_tag_found: word_metadata_to_store["script_type"] = script_tag_found
            hyphenation_json = Json(entry["hyphenation"]) if entry.get("hyphenation") else None
            tags_list = entry.get("tags", [])
            word_tags_str = ",".join(t for t in tags_list if isinstance(t, str)) if tags_list else None
            is_proper_noun = (entry.get("proper", False) or pos_raw in ["prop", "proper noun", "name"])
            is_abbreviation = (pos_raw in ["abbrev", "abbreviation"] or any(t in ["abbreviation", "abbrev"] for t in tags_list if isinstance(t, str)))
            is_initialism = (pos_raw in ["init", "initialism", "acronym"] or any(t in ["initialism", "acronym"] for t in tags_list if isinstance(t, str)))

            # --- Get or Create Word ID (Critical) ---
            try:
                word_id = get_or_create_word_id(
                    cur, lemma=lemma, language_code=language_code,
                    has_baybayin=bool(script_form), # Overloaded flag
                    baybayin_form=script_form,      # Stores Baybayin or Badlit
                    romanized_form=script_romanized_final,
                    # badlit_form=badlit_form, # Add if using Option B schema
                ...
                if not word_id: raise ValueError("get_or_create_word_id returned None")
            except Exception as word_err:
                 logger.error(f"CRITICAL FAILURE getting/creating word_id for '{lemma}' ({language_code}): {word_err}")
                 raise word_err # Propagate critical error

            # --- Process other word-level data (collecting non-critical errors) ---
            try: stats["pronunciations"] += process_pronunciation(cur, word_id, entry, source_identifier)
            except Exception as e: error_messages.append(f"Pronunciation error: {e}")

            try:
                 ety_id, rels_added = process_etymology( # Use enhanced helper
                     cur, word_id, entry.get("etymology_text"), source_identifier,
                     entry.get("etymology_templates"), lemma, language_code
                 )
                 if ety_id: stats["etymologies"] += 1
                 if rels_added: stats["ety_relations"] += rels_added # Separate counter
            except Exception as e: error_messages.append(f"Etymology/relation error: {e}")

            try:
                stats["forms"] += process_forms_storage(cur, word_id, entry, source_identifier)
            except Exception as e: error_messages.append(f"Word form storage error: {e}")

            try:
                if entry.get("head_templates"):
                     stats["templates"] += process_head_templates(cur, word_id, entry["head_templates"])
            except Exception as e: error_messages.append(f"Head template error: {e}")
            ...
            if "senses" in entry and isinstance(entry["senses"], list):
                for sense_idx, sense in enumerate(entry["senses"]):
                     ...
                     try:
                         ...
                         definition_text = "; ".join([clean_html(g) for g in glosses if isinstance(g, str) and clean_html(g)])
                         ...
                         if "examples" in sense and isinstance(sense["examples"], list):
                             for ex_item in sense["examples"]:
                                 ex_text_clean = clean_html(ex_item.get("text"))
                                 if ex_text_clean:
                                     ex_obj = {"text": ex_text_clean}
                                     if ex_item.get("translation"): ex_obj["translation"] = ex_item["translation"]
                                     if ex_item.get("ref"): ex_obj["ref"] = ex_item["ref"]
                                     examples_processed.append(ex_obj)
                         examples_json = Json(examples_processed) if examples_processed else None
                         if examples_processed: stats["examples"] += len(examples_processed)

                         sense_tags_list = sense.get("tags", []) + sense.get("labels", [])
                         sense_tags_list.extend([cat.get("name") for cat in sense.get("categories", []) if isinstance(cat, dict) and cat.get("name")])
                         tags_str = ",".join(filter(None, [str(t).strip() for t in sense_tags_list])) if sense_tags_list else None

                         sense_pos_str = sense.get("pos") or pos_raw # Use entry POS as fallback

                         sense_metadata = {}
                         for key in ["form_of", "raw_glosses", "topics", "taxonomy", "qualifier", "usage_notes"]: # Added usage notes here
                             if key in sense and sense[key]: sense_metadata[key] = sense[key]
                         ...
                         try:
                             definition_id = insert_definition(
                                 ...
                                 source_identifier=source_identifier,
                                 part_of_speech=sense_pos_str, # Pass raw POS string
                                 examples=examples_json,
                                 tags=tags_str,
                                 usage_notes=clean_html(sense_metadata.pop("usage_notes", None)), # Extract and clean usage notes
                                 # metadata=Json(sense_metadata) if sense_metadata else None # Pass remaining metadata
                             )
                             if definition_id: stats["definitions"] += 1
                             else: raise ValueError("insert_definition failed")
                         except psycopg2.errors.UniqueViolation:
                              logger.debug(f"Def already exists for word ID {word_id}, sense {sense_idx}")
                              definition_id = None
                         except Exception as def_err:
                              logger.warning(f"Error inserting def for '{lemma}', sense {sense_idx}: {def_err}")
                              error_messages.append(f"Def {sense_idx} insert error: {def_err}")
                              definition_id = None

                         # --- Process Definition Sub-Data ---
                         if definition_id:
                             try: stats["categories"] += process_categories(cur, definition_id, sense.get("categories", []))
                             except Exception as e: error_messages.append(f"Category error (Def {definition_id}): {e}")
                             try: stats["links"] += process_links(cur, definition_id, sense.get("links", []))
                             except Exception as e: error_messages.append(f"Link error (Def {definition_id}): {e}")
                             try:
                                 rels_added = process_sense_relationships(cur, word_id, sense, language_code, source_identifier, lemma)
                                 if rels_added: stats["sense_relations"] += rels_added # Use specific counter
                             except Exception as e: error_messages.append(f"Sense relation error (Def {definition_id}): {e}")

                            # --- Optional: Superseded Spelling Check ---
                             ...
                             if superseded_match:
                                 ...
                                 if correct_spelling and correct_spelling.lower() != lemma.lower():
                                     try:
                                         correct_word_id = get_or_create_word_id(cur, correct_spelling, language_code, source_identifier)
                                         if correct_word_id != word_id:
                                             meta = {"tag": "superseded", "definition_id": definition_id}
                                             rel_id_1 = insert_relation(cur, word_id, correct_word_id, RelationshipType.SPELLING_VARIANT, source_identifier, meta)
                                             rel_id_2 = insert_relation(cur, correct_word_id, word_id, RelationshipType.SPELLING_VARIANT, source_identifier, meta)
                                             if rel_id_1: stats["relations"] += 1; # Increment general relations too
                                             logger.debug(f"Linked superseded spelling '{lemma}' to '{correct_spelling}'")
                                     except Exception as e: logger.warning(f"Error linking superseded spelling '{lemma}' to '{correct_spelling}': {e}")

                     except Exception as sense_proc_err:
                          logger.warning(f"Error processing sense {sense_idx} for word ID {word_id}: {sense_proc_err}")
                          error_messages.append(f"Sense {sense_idx} processing error: {sense_proc_err}")

            # --- Return status for the entry ---
            return word_id, "; ".join(error_messages) if error_messages else None

        except Exception as e:
             # Catch unexpected errors within the main try block of process_entry
             logger.error(f"UNEXPECTED EXCEPTION processing entry for '{word}' (ID: {word_id}): {e}", exc_info=True)
             return word_id if word_id else None, f"Unhandled entry exception: {e}"

    # --- Main Processing Loop (Iterating through file lines) ---
    try:
        with open(filename, "r", encoding="utf-8") as f:
            entry_count = 0
            ...
            for line in f:
                ...
                try:
                    ...
                    logger.critical(f"CRITICAL: Failed savepoint {savepoint_name}. Aborting. Error: {sp_error}")
                    stats["failed_entries"] = stats.get("total_entries", entry_count) - entry_count + 1
                    if progress_bar: progress_bar.close()
                    # Since commit=False, the caller (migrate_data) needs to handle rollback
                    return stats["processed_ok"] + stats["processed_with_errors"], stats["failed_entries"] + stats["skipped_json_errors"] # Return partial stats

                word_id = None
                entry_errors_str = None
                try:
                    entry = json.loads(line.strip())
                    # Call the enhanced process_entry
                    word_id, entry_errors_str = process_entry_kaikki(cur, entry, source_identifier, romanizer)

                    if word_id: # If word_id exists, entry was at least partially processed
                        if entry_errors_str:
                            stats["processed_with_errors"] += 1
                            first_error = entry_errors_str.split(";")[0][:100]
                            err_key = f"EntryNonCritError:{first_error}"
                            error_summary[err_key] = error_summary.get(err_key, 0) + 1
                        else:
                            stats["processed_ok"] += 1
                        cur.execute(f"RELEASE SAVEPOINT {savepoint_name}") # Commit this entry's work
                    else:
                        # Critical failure within process_entry
                        stats["failed_entries"] += 1
                        reason = entry_errors_str or "Unknown critical failure"
                        err_key = f"EntryFailedCritically: {reason[:100]}"
                        error_summary[err_key] = error_summary.get(err_key, 0) + 1
                        cur.execute(f"ROLLBACK TO SAVEPOINT {savepoint_name}") # Rollback this entry

                except json.JSONDecodeError:
                    logger.warning(f"Invalid JSON on line {entry_count} in {filename}")
                    stats["skipped_json_errors"] += 1
                    try: cur.execute(f"ROLLBACK TO SAVEPOINT {savepoint_name}")
                    except Exception: pass
                except Exception as e:
                    logger.error(f"Unhandled error processing line {entry_count} ('{entry.get('word', 'unknown') if entry else 'unknown'}'): {e}", exc_info=True)
                    stats["failed_entries"] += 1
                    error_key = f"LoopException: {type(e).__name__}"
                    error_summary[error_key] = error_summary.get(error_key, 0) + 1
                    try: cur.execute(f"ROLLBACK TO SAVEPOINT {savepoint_name}")
                    except Exception as rb_err:
                         logger.critical(f"CRITICAL: Failed rollback {savepoint_name} after loop error: {rb_err}. Aborting.", exc_info=True)
                         raise e from rb_err # Abort file processing

                finally:
                     # Periodic commit (optional, can be removed if outer transaction handles all)
                     processed_count = stats["processed_ok"] + stats["processed_with_errors"]
                     if processed_count > 0 and processed_count % 1000 == 0:
                         try:
                             logger.info(f"Releasing final savepoint {savepoint_name} before batch commit...")
                             # Ensure savepoint is released *before* commit if relying on them
                             # Note: Releasing might not be needed if commit handles it, depends on DB/psycopg2 version behavior
                             # cur.execute(f"RELEASE SAVEPOINT {savepoint_name}") # This might cause issues if called after rollback
                             conn.commit() # Commit the successful batch
                             logger.info(f"Committed batch after {entry_count} lines processed.")
                         except Exception as batch_commit_err:
                             logger.error(f"Error committing batch at line {entry_count}: {batch_commit_err}. Rolling back...", exc_info=True)
                             try: conn.rollback()
                             except Exception as rb_err: logger.error(f"Rollback after batch commit error failed: {rb_err}")
                             error_summary["BatchCommitError"] = error_summary.get("BatchCommitError", 0) + 1
                             # Continue processing, next entry will start a new transaction context implicitly

                     if progress_bar: progress_bar.update(1)

            if progress_bar: progress_bar.close()

        # --- Final Commit for the file (handled by caller due to commit=False) ---
        logger.info(f"Finished processing loop for {filename}. Final commit handled by caller.")

    except FileNotFoundError:
        logger.error(f"File not found error during main processing block: {filename}")
        stats["failed_entries"] = stats.get("total_entries", 1) # Mark at least one failure
    except Exception as e:
        logger.error(f"Fatal error during processing of {filename}: {str(e)}", exc_info=True)
        # Rollback is handled by the @with_transaction decorator on error
        stats["failed_entries"] = stats.get("total_entries", entry_count) - stats.get("processed_ok", 0) - stats.get("processed_with_errors", 0) - stats.get("skipped_json_errors", 0)

    # --- Final Logging ---
    logger.info(f"Completed processing {filename}:")
    logger.info(f"  Total Lines: {stats.get('total_entries', 'N/A')}")
    logger.info(f"  Processed OK: {stats['processed_ok']}")
    logger.info(f"  Processed w/ Errors: {stats['processed_with_errors']}")
    logger.info(f"  Failed Entries: {stats['failed_entries']}")
    logger.info(f"  Skipped JSON Errors: {stats['skipped_json_errors']}")
    logger.info(f"  -> Definitions Added: {stats['definitions']}")
    logger.info(f"  -> Relations Added (Total): {stats['relations'] + stats['ety_relations']}")
    logger.info(f"     (From Senses: {stats.get('sense_relations', 0)}, From Ety: {stats['ety_relations']})")
    logger.info(f"  -> Etymologies Added: {stats['etymologies']}")
    logger.info(f"  -> Pronunciations Added: {stats['pronunciations']}")
    logger.info(f"  -> Forms Stored: {stats['forms']}")
    logger.info(f"  -> Scripts Found: {stats['scripts']}")
    logger.info(f"  -> Categories Stored: {stats['categories']}")
    logger.info(f"  -> Templates Stored: {stats['templates']}")
    logger.info(f"  -> Links Stored: {stats['links']}")
    if error_summary:
       logger.warning(f"  Error Summary: {json.dumps(error_summary)}")

    total_issues = stats["failed_entries"] + stats["skipped_json_errors"] + stats["processed_with_errors"]
    total_processed = stats["processed_ok"] + stats["processed_with_errors"]
    # Return counts needed by migrate_data
    return total_processed, total_issues
