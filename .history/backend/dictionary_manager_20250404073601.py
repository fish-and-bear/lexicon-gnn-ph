...
# Replace the existing process_kwf_dictionary function (starts around line 4366)
@with_transaction(commit=False) # Ensure commit is False
def process_kwf_dictionary(cur, filename: str):
    ...
    try:
        ...
        if not isinstance(data, dict):
            logger.error(f"Unexpected format in {filename}, expected a dictionary.")
            stats["error_entries"] += 1
            error_types["InvalidFormat"] = error_types.get("InvalidFormat", 0) + 1
            return stats

        if not all(isinstance(v, dict) for v in data.values()):
            logger.error(f"File {filename} values are not all dictionaries.")
            stats["error_entries"] += 1
        ...
        def kwf_dict_iterator(d):
            for i, (word_key, entry_details) in enumerate(d.items()):
                 if word_key != "__metadata__": # Skip metadata entry if present
                    yield i, word_key, entry_details

        entries_iterator = kwf_dict_iterator(data)
        stats["total_entries"] = len(data) - (1 if "__metadata__" in data else 0) # Adjust count if metadata present
        logger.info(f"Found {stats['total_entries']} entries in {filename}")

        dictionary_metadata = data.get("__metadata__", {}) if isinstance(data.get("__metadata__"), dict) else {}
        ...
        RELATION_TYPE_MAPPING = {
            ...
            "related_terms": RelationshipType.RELATED,
        }
        ...
        with tqdm(
            ...
            for entry_index, word, entry in entries_iterator:
                ...
                try:
                    ...
                    logger.error(f"Failed savepoint {savepoint_name} for '{word}': {sp_err}. Skipping.")
                    stats["error_entries"] += 1
                ...
                try:
                    ...
                    baybayin_form, romanized_form, has_baybayin = None, None, False # Adapt if KWF has baybayin

                    word_metadata_dict = entry.get("metadata", {}).copy()
                    word_metadata_dict["source_file"] = raw_source_identifier
                    word_metadata_dict["original_key"] = word
                    if dictionary_metadata: word_metadata_dict["dictionary_info"] = dictionary_metadata
                    pronunciation_list = word_metadata_dict.pop("pronunciation", [])
                    etymology_list = word_metadata_dict.pop("etymology", [])
                    word_metadata_json = json.dumps(word_metadata_dict) if word_metadata_dict else None

                    word_id = get_or_create_word_id(
                        cur, formatted_word, language_code=language_code,
                        has_baybayin=has_baybayin, baybayin_form=baybayin_form, romanized_form=romanized_form,
                        source_identifier=source_identifier, word_metadata=word_metadata_json
                    )
                    if not word_id or word_id <= 0: raise ValueError(f"Failed word ID for '{formatted_word}'")
                    ...
                    if isinstance(etymology_list, list):
                        for ety_idx, ety_item in enumerate(etymology_list):
                             ety_text = None
                             if isinstance(ety_item, str) and ety_item.strip(): ety_text = ety_item.strip()
                             elif isinstance(ety_item, dict) and ety_item.get("text"): ety_text = ety_item["text"].strip()
                             if ety_text:
                                 try:
                                     components = extract_etymology_components(ety_text)
                                     components_json = json.dumps(components) if components else None
                                     lang_codes_list = extract_language_codes(ety_text)
                                     lang_codes_str = ", ".join(lang_codes_list) if language_codes_list else None
                                     ety_db_id = insert_etymology(cur, word_id, ety_text, source_identifier, normalized_components=components_json, language_codes=lang_codes_str)
                                     if ety_db_id: stats["etymologies_added"] += 1
                                 except Exception as ety_error:
                                     logger.warning(f"Etymology error for '{formatted_word}': {ety_text[:50]}... Err: {ety_error}")
                    ...
                    if isinstance(definitions_by_pos, dict):
                        for raw_pos, definitions_list in definitions_by_pos.items():
                            ...
                            for def_idx, def_item in enumerate(definitions_list):
                                if not isinstance(def_item, dict): continue
                                definition_text = def_item.get("meaning", "").strip()
                                if not definition_text: continue
                                examples_json, usage_notes, tags = None, def_item.get("note"), None
                                tags_list = def_item.get("categories", [])
                                            ...
                                            if isinstance(examples_in_set, list):
                                                for ex_idx, ex in enumerate(examples_in_set):
                                                    if isinstance(ex, dict) and "text" in ex and ex["text"].strip():
                                                        normalized_examples.append({"text": ex["text"].strip(), "source": source_identifier, "index_in_set": ex_idx})
                                if normalized_examples:
                                    try:
                                        examples_json = json.dumps(normalized_examples)
                                        stats["examples_added"] += len(normalized_examples)
                                    except TypeError: logger.warning(f"Cannot serialize examples for '{formatted_word}', def {def_idx}")

                                if definition_text:
                                    try:
                                        definition_id = insert_definition(cur, word_id, definition_text, source_identifier=source_identifier,
                                                                          part_of_speech=standardized_pos_str, examples=examples_json,
                                                                          usage_notes=(usage_notes.strip() if usage_notes else None), tags=tags)
                                        if definition_id:
                                            stats["definitions_added"] += 1
                                            # Process synonyms, antonyms, cross_references within definition_id scope...
                                            # (Synonym processing example)
                                            ...
                                            if isinstance(synonyms_list, list):
                                                for syn_idx, syn in enumerate(synonyms_list):
                                                    if isinstance(syn, str) and syn.strip() and syn.strip() != formatted_word:
                                                        syn_word_clean = re.sub(r"<[^>]+>", "", syn.strip()).strip()
                                                        if not syn_word_clean or syn_word_clean == formatted_word: continue
                                                        try:
                                                            ...
                                                            if syn_id:
                                                                ...
                                                                if insert_relation(cur, word_id, syn_id, RelationshipType.SYNONYM, source_identifier, metadata=syn_metadata):
                                                                    stats["synonyms_added"] += 1; stats["relations_added"] += 1
                                                                insert_relation(cur, syn_id, word_id, RelationshipType.SYNONYM, source_identifier, metadata=syn_metadata) # Bidirectional
                                                        except Exception as syn_error: logger.warning(f"Synonym error '{syn_word_clean}' for '{formatted_word}': {syn_error}")
                                            # (Antonym processing similar to synonym)
                                            # (Cross-reference processing similar to synonym, using SEE_ALSO)

                                    except psycopg2.errors.UniqueViolation: logger.debug(f"Definition exists for '{formatted_word}', def {def_idx}")
                                    except Exception as def_error:
                                        logger.warning(f"Definition insert error for '{formatted_word}', def {def_idx}: {def_error}")
                    ...
                    if isinstance(related_data, dict):
                        for rel_type_raw, rel_items in related_data.items():
                            if rel_type_raw.lower() == "examples" or not isinstance(rel_items, list): continue
                            rel_type_enum = RELATION_TYPE_MAPPING.get(rel_type_raw.lower(), RelationshipType.RELATED)
                            for rel_idx, rel_item in enumerate(rel_items):
                                related_term_raw, item_metadata = None, {}
                                if isinstance(rel_item, dict) and "term" in rel_item:
                                    related_term_raw = str(rel_item["term"]).strip()
                                    item_metadata = {k:v for k,v in rel_item.items() if k != "term"}
                                elif isinstance(rel_item, str): related_term_raw = rel_item.strip()
                                related_term_clean = re.sub(r"<[^>]+>", "", related_term_raw).strip() if related_term_raw else None
                                if related_term_clean and len(related_term_clean) <= 255 and related_term_clean != formatted_word:
                                    try:
                                        related_id = get_or_create_word_id(cur, related_term_clean, language_code, source_identifier=source_identifier)
                                        if related_id:
                                            relation_metadata = {"source": source_identifier, "original_type": rel_type_raw, "index": rel_idx, "confidence": 85, **item_metadata}
                                            if insert_relation(cur, word_id, related_id, rel_type_enum, source_identifier, metadata=relation_metadata):
                                                stats["relations_added"] += 1
                                            if rel_type_enum.bidirectional: insert_relation(cur, related_id, word_id, rel_type_enum, source_identifier, metadata=relation_metadata)
                                    except Exception as e:
                                        logger.warning(f"Related term error '{related_term_clean}' for '{formatted_word}': {e}")
                    ...
                    # REMOVED Periodic Commit Logic

                # --- Error Handling for a Single Entry ---
                except (psycopg2.Error, ValueError, KeyError, Exception) as entry_error:
                    error_msg = str(entry_error).split("\n")[0][:100] # Limit length
                    error_type_key = f"{type(entry_error).__name__}: {error_msg}"
                    logger.error(f"Error processing entry '{word}': {entry_error}", exc_info=False) # Less verbose traceback in loop
                    error_types[error_type_key] = error_types.get(error_type_key, 0) + 1
                    stats["error_entries"] += 1
                    try:
                        cur.execute(f"ROLLBACK TO SAVEPOINT {savepoint_name}")
                    except Exception as rb_err:
                        logger.critical(f"CRITICAL: Failed rollback to {savepoint_name}: {rb_err}. Full rollback.", exc_info=True)
                        conn.rollback()
                        raise entry_error from rb_err # Re-raise original error to stop processing
    ...
    # --- Outer Exception Handling ---
        ...
        try:
            if conn and not conn.closed and conn.status == psycopg2.extensions.STATUS_IN_TRANSACTION : # Check if in transaction
                conn.rollback()
                logger.warning("Rolling back transaction due to outer exception.")
            elif conn and not conn.closed:
                 logger.info("Transaction not active or already rolled back in outer exception handler.")
            else:
                 logger.warning("Connection closed in outer exception handler before rollback attempt.")

        except Exception as rollback_error:
            logger.error(f"Failed to roll back transaction in outer exception handler: {rollback_error}")
        # Optionally re-raise outer_e if needed, but returning stats might be safer

    # --- Final Logging ---
    finally:
        logger.info(f"Completed processing KWF Dictionary: {filename}")
        logger.info(f"Statistics: {json.dumps(stats, indent=2)}")
        if error_types: logger.info(f"Error types encountered: {json.dumps(error_types, indent=2)}")
