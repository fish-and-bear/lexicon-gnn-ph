...
def with_transaction(commit=True):
    """Decorator for functions that need to be executed within a database transaction.
    
    Args:
        commit: Whether to commit the transaction on success (True) or 
               leave it to the caller (False)
    """
    def decorator(func):
        @functools.wraps(func)
        def wrapper(cur, *args, **kwargs):
            ...
            # Store the original autocommit state to restore it later
            original_autocommit = conn.autocommit
            
            # Ensure we're not in an aborted transaction state before starting
            if conn.get_transaction_status() == psycopg2.extensions.TRANSACTION_STATUS_INERROR:
                conn.rollback()
                logger.warning("Rolled back aborted transaction before starting new one")
            
            # Make sure autocommit is off for our transaction
