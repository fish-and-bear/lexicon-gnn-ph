...
def init_limiter(app):
    ...
    limiter.limit("200 per day")(bp)
    limiter.limit("50 per hour")(bp)
    limiter.limit("1000 per day", exempt_when=lambda: app.config.get('TESTING'))(app)

def rate_limit(limit_string):
    """Custom rate limit decorator that works with blueprint endpoints."""
    def decorator(f):
        @wraps(f)
