    ...
    try:
        ...
        try:
            ...
            word_entry_initial = Word.query.with_entities(Word.id).filter(
                func.lower(Word.normalized_lemma) == func.lower(normalize_lemma(word)) # Use normalized lemma
            ).first()
            if word_entry_initial:
                word_id = word_entry_initial.id
                logger.debug(f"Found word ID {word_id} by normalized lemma '{normalize_lemma(word)}'.")
            else:
                # Try original lemma as fallback
                word_entry_initial = Word.query.with_entities(Word.id).filter(
                    func.lower(Word.lemma) == func.lower(word)
                ).first()
                if word_entry_initial:
                   word_id = word_entry_initial.id
                   logger.debug(f"Found word ID {word_id} by original lemma '{word}'.")
                else:
