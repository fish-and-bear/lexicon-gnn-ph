...
def api_suggestions():
    ...
    try:
        ...
        # Use text() for safer parameter binding with SQLAlchemy
        sql = text("""
            SELECT lemma
            FROM words
        ...
        # Execute the query using your DB session
        # The exact way to execute depends on your DB setup (SQLAlchemy, psycopg2, etc.)
        # This assumes SQLAlchemy session:
        results = db.session.execute(sql, {'query': query + '%', 'limit': limit})
        suggestions = [row[0] for row in results.fetchall()]

        logger = current_app.logger if current_app else logging.getLogger(__name__)
        logger.debug(f"Suggestions for '{query}': {suggestions}")
        return jsonify(suggestions)

    except Exception as e:
        logger = current_app.logger if current_app else logging.getLogger(__name__)
        logger.error(f"Error fetching suggestions directly in route for query '{query}': {e}", exc_info=True)
