...
def process_kwf_dictionary(cur, filename: str):
    ...
    stats = {
        ...
        "affixes_processed": 0, # Placeholder if needed later
        "relations_added": 0, # Grand total for relations
        "source_updates": 0, # Placeholder if needed later
        "etymologies_added": 0 # Added counter for etymology
    }
    error_types = {}  # Track unique error types for summary

    # Get the connection object for transaction management (savepoints, commit, rollback)
    conn = cur.connection
    ...
    try:
        ...
        with open(filename, 'r', encoding='utf-8') as f:
            try:
                ...
                stats["error_entries"] += 1 # Count file load error
                error_types["JSONDecodeError"] = error_types.get("JSONDecodeError", 0) + 1
                return stats # Return early if file is invalid

        # Initialize statistics based on data format (list or dict)
        entries = []
        dictionary_metadata = {} # To store metadata about the dictionary itself
        if isinstance(data, list):
            stats["total_entries"] = len(data)
            entries = data
            logger.info(f"Found {stats['total_entries']} entries in list format")
        elif isinstance(data, dict):
            entries = data.get("entries", []) # Check for 'entries' key
            stats["total_entries"] = len(entries)
            logger.info(f"Found {stats['total_entries']} entries in dictionary format under 'entries' key")
            # Try to extract dictionary metadata if available in the main dict
        ...
        # Create source metadata object to potentially store with words/definitions
        source_metadata = {
            "name": "KWF Diksiyonaryo ng Wikang Filipino", # Specific name
            "file": raw_source_identifier, # Original filename
            "processed_timestamp": datetime.now().isoformat()
        }
        if dictionary_metadata:
            source_metadata.update(dictionary_metadata) # Add extracted metadata

        # Convert to JSON string for storage (optional, depends on schema)
        # source_metadata_json = json.dumps(source_metadata)

        # Mapping for KWF relation types to standard RelationshipType enum or string values
        # Adjusted based on the original snippet's usage
        RELATION_TYPE_MAPPING = {
            "examples": None,  # Explicitly skip processing 'examples' as relations
            "cognate_with": RelationshipType.RELATED, # Map cognate to RELATED (or custom type if exists)
            "borrowed_from": RelationshipType.DERIVED_FROM, # Map borrowed to DERIVED_FROM
        ...
        with tqdm(total=stats["total_entries"], desc=f"Processing {os.path.basename(filename)}", unit="entry") as pbar:
            for entry_index, entry in enumerate(entries):
                ...
                if not entry or not isinstance(entry, dict):
                    logger.debug(f"Skipping empty or invalid entry at index {entry_index}")
                    stats["skipped_entries"] += 1
                    pbar.update(1)
                    continue

                # Create a savepoint for this entry to allow partial rollback on error
                savepoint_name = f"kwf_entry_{entry_index}"
                try:
                    cur.execute(f"SAVEPOINT {savepoint_name}")
                except Exception as sp_err:
                     logger.error(f"Failed to create savepoint {savepoint_name} for entry index {entry_index}: {sp_err}. Skipping entry.")
                     stats["error_entries"] += 1
                     error_types[f"SavepointError: {sp_err}"] = error_types.get(f"SavepointError: {sp_err}", 0) + 1
                     pbar.update(1)
                     continue # Skip entry if savepoint fails

                word = None # Initialize word variable
                try:
                    # Extract the word - handle different possible keys ('word', 'headWord')
                    if "word" in entry:
                        word = entry["word"]
                    elif "headWord" in entry:
                        word = entry["headWord"]
                    else:
                    ...
                    # Check if word is valid string
                    if not word or not isinstance(word, str):
                        logger.warning(f"Invalid word ('{word}') in entry #{entry_index}, skipping")
                        stats["skipped_entries"] += 1
                        cur.execute(f"RELEASE SAVEPOINT {savepoint_name}") # Release savepoint
                        pbar.update(1)
                        continue

                    formatted_word = word.strip() # Use stripped version
                    if not formatted_word:
                         logger.warning(f"Word in entry #{entry_index} is empty after stripping, skipping")
                         stats["skipped_entries"] += 1
                         cur.execute(f"RELEASE SAVEPOINT {savepoint_name}") # Release savepoint
                         pbar.update(1)
                         continue
                    ...
                    # --- Language Code ---
                    language_code = entry.get("language_code", "tl").strip() or "tl" # Default to 'tl'
                    ...
                    if "baybayin" in entry and entry["baybayin"]:
                        ...
                        if isinstance(baybayin_text, str) and baybayin_text.strip():
                            ...
                            # Validate before processing further
                            if validate_baybayin_entry(baybayin_form_raw):
                                baybayin_form = clean_baybayin_text(baybayin_form_raw) # Store cleaned version
                                if baybayin_form:
                                     has_baybayin = True
                                     # Try to get romanization (of cleaned form)
                                     romanized_form = get_romanized_text(baybayin_form)
                            else:
                                 logger.warning(f"Invalid Baybayin found for '{formatted_word}': {baybayin_form_raw}")


                    # --- Prepare Word Metadata ---
                    # Construct metadata dictionary based on entry fields
                    word_metadata_dict = {}
                    word_metadata_dict["source_file"] = raw_source_identifier # Keep original filename
                    # Add domain/register information if available
                    if "register" in entry: word_metadata_dict["register"] = entry["register"]
                    if "domain" in entry: word_metadata_dict["domain"] = entry["domain"]
                    # Add any other useful metadata from the entry level
                    for meta_key in ["notes", "usage", "origin", "dialect"]:
                        if meta_key in entry and entry[meta_key]:
                            word_metadata_dict[meta_key] = entry[meta_key]
                    # Add extracted dictionary metadata
                    ...
                    word_id = get_or_create_word_id(
                        ...
                        has_baybayin=has_baybayin, # Pass boolean flag
                        baybayin_form=baybayin_form, # Pass cleaned form or None
                        romanized_form=romanized_form, # Pass romanized form or None
                        source_identifier=source_identifier, # Pass mandatory source
                        word_metadata=word_metadata_json # Pass metadata JSON or None
                        # Add other kwargs like is_proper_noun if determinable
                    )

                    if not word_id or word_id <= 0: # Check for valid ID
                        raise ValueError(f"Failed to get or create a valid word ID for '{formatted_word}'")
                    ...
                    if "etymology" in entry and entry["etymology"]:
                        ...
                        if isinstance(etymology_text, str) and etymology_text.strip():
                            etymology_text_clean = etymology_text.strip()
                            try:
                                # Optional: Extract structured components if possible
                                components = extract_etymology_components(etymology_text_clean)
                                components_json = json.dumps(components) if components else None

                                # Optional: Extract language codes mentioned
                                lang_codes_str = None
                                if "from" in etymology_text_clean.lower():
                                     language_codes_list = extract_language_codes(etymology_text_clean)
                                     if language_codes_list:
                                         lang_codes_str = ", ".join(language_codes_list)

                                # Insert etymology, passing mandatory source_identifier
                                ety_id = insert_etymology(
                                    cur,
                                    word_id,
                                    etymology_text_clean,
                                    source_identifier=source_identifier, # Pass mandatory source
                                    normalized_components=components_json, # Pass components JSON or None
                                    language_codes=lang_codes_str # Pass language codes string or None
                                    # Pass etymology_structure if available/needed
                                )
                                if ety_id: stats["etymologies_added"] += 1
                            except Exception as ety_error:
                                logger.warning(f"Error processing etymology for '{formatted_word}': {etymology_text_clean[:50]}... Error: {ety_error}")
                    ...
                    # Check for 'entries' key which seems to hold POS-grouped definitions in KWF format
                    if "entries" in entry and isinstance(entry["entries"], list):
                        for pos_entry in entry["entries"]:
                            if not isinstance(pos_entry, dict): continue

                            # Standardize the part of speech
                            raw_pos = pos_entry.get("partOfSpeech", "") # KWF uses 'partOfSpeech'
                            standardized_pos_str = standardize_entry_pos(raw_pos) if raw_pos else None # Standardize

                            # Process definitions within this POS group
                            definitions_list = pos_entry.get("definitions", [])
                            if isinstance(definitions_list, list):
                                for def_idx, def_item in enumerate(definitions_list):
                                    definition_text = None
                                    examples_json = None
                                    usage_notes = None
                                    tags = None # Placeholder for definition tags if KWF provides them
                                    def_metadata_dict = {"source_file": raw_source_identifier} # Metadata specific to definition
                                    if raw_pos: def_metadata_dict["original_pos"] = raw_pos
                                    ...
                                    if isinstance(def_item, str):
                                        ...
                                        # Extract examples, normalize to list of dicts, convert to JSON
                                            ...
                                            if isinstance(raw_examples, list):
                                                for ex_idx, ex in enumerate(raw_examples):
                                                    ...
                                                    if isinstance(ex, str) and ex.strip():
                                                        ex_obj = {"text": ex.strip(), "source": source_identifier, "index": ex_idx}
                                                    elif isinstance(ex, dict) and "text" in ex and ex["text"].strip():
                                                        ex_obj = {"text": ex["text"].strip(), "source": source_identifier, "index": ex_idx}
                                                        # Add translation if available
                                                        if "translation" in ex: ex_obj["translation"] = ex["translation"]
                                                    if ex_obj: normalized_examples.append(ex_obj)
                                            # Add elif for dict format if KWF uses it
                                        ...
                                        # Extract usage notes
                                        usage_notes = def_item.get("notes", "") or def_item.get("usage", "")
                                        if usage_notes: usage_notes = usage_notes.strip()

                                        # Extract definition-level metadata if present
                                        if "domain" in def_item: def_metadata_dict["domain"] = def_item["domain"]
                                        if "register" in def_item: def_metadata_dict["register"] = def_item["register"]

                                    # Insert the definition if text is valid
                                    if definition_text:
                                        try:
                                            # Pass mandatory source_identifier
                                            # Pass part_of_speech using the standardized string
                                            definition_id = insert_definition(
                                                cur,
                                                word_id,
                                                definition_text,
                                                source_identifier=source_identifier, # Pass mandatory source
                                                part_of_speech=standardized_pos_str, # Use standardized POS string
                                                original_pos=raw_pos, # Pass original POS string if needed by insert_definition
                                                examples=examples_json, # Pass examples JSON or None
                                                usage_notes=usage_notes or None, # Pass usage notes or None
                                                tags=tags # Pass tags string or None
                                                # Removed incorrect metadata= argument
                                            )
                                            if definition_id:
                                                stats["definitions_added"] += 1

                                                # Process relations tied to this definition (synonyms, antonyms, see_also)
                                                # Process Synonyms
                                                if "synonyms" in def_item and isinstance(def_item["synonyms"], list):
                                                    for syn_idx, syn in enumerate(def_item["synonyms"]):
                                                        syn_word = syn.get("word") if isinstance(syn, dict) else syn
                                                        if isinstance(syn_word, str) and syn_word.strip() and syn_word.strip() != formatted_word:
                                                            ...
                                                            try:
                                                                ...
                                                                if syn_id:
                                                                    ...
                                                                    rel_syn_id = insert_relation(cur, word_id, syn_id, RelationshipType.SYNONYM, source_identifier=source_identifier, metadata=syn_metadata)
                                                                    if rel_syn_id: stats["synonyms_added"] += 1; stats["relations_added"] += 1
                                                                    # Handle bidirectional SYNONYM
                                                                    insert_relation(cur, syn_id, word_id, RelationshipType.SYNONYM, source_identifier=source_identifier, metadata=syn_metadata)
                                                            except Exception as syn_error: logger.warning(f"Error processing synonym '{syn_word_clean}' for '{formatted_word}': {syn_error}")

                                                # Process Antonyms
                                                if "antonyms" in def_item and isinstance(def_item["antonyms"], list):
                                                    for ant_idx, ant in enumerate(def_item["antonyms"]):
                                                        ant_word = ant.get("word") if isinstance(ant, dict) else ant
                                                        if isinstance(ant_word, str) and ant_word.strip() and ant_word.strip() != formatted_word:
                                                            ant_word_clean = ant_word.strip()
                                                            try:
                                                                ...
                                                                if ant_id:
                                                                    ...
                                                                    rel_ant_id = insert_relation(cur, word_id, ant_id, RelationshipType.ANTONYM, source_identifier=source_identifier, metadata=ant_metadata)
                                                                    if rel_ant_id: stats["antonyms_added"] += 1; stats["relations_added"] += 1
                                                                    # Handle bidirectional ANTONYM
                                                                    insert_relation(cur, ant_id, word_id, RelationshipType.ANTONYM, source_identifier=source_identifier, metadata=ant_metadata)
                                                            except Exception as ant_error: logger.warning(f"Error processing antonym '{ant_word_clean}' for '{formatted_word}': {ant_error}")

                                                # Process See Also (Cross-references)
                                                if "see_also" in def_item and isinstance(def_item["see_also"], list):
                                                    for ref_idx, ref in enumerate(def_item["see_also"]):
                                                        ref_word = ref.get("word") if isinstance(ref, dict) else ref
                                                        if isinstance(ref_word, str) and ref_word.strip() and ref_word.strip() != formatted_word:
                                                             ...
                                                             try:
                                                                ...
                                                                if ref_id:
                                                                    ref_metadata = {"source": source_identifier, "confidence": 80, "definition_id": definition_id, "index": ref_idx}
                                                                    rel_ref_id = insert_relation(cur, word_id, ref_id, RelationshipType.SEE_ALSO, source_identifier=source_identifier, metadata=ref_metadata)
                                                                    if rel_ref_id: stats["refs_added"] += 1; stats["relations_added"] += 1
                                                                    # Handle bidirectional SEE_ALSO if desired (usually not needed)
                                                             except Exception as ref_error: logger.warning(f"Error processing see_also '{ref_word_clean}' for '{formatted_word}': {ref_error}")

                                        except psycopg2.errors.UniqueViolation:
                                            logger.debug(f"Definition already exists for '{formatted_word}', def {def_idx}: {definition_text[:50]}...")
                                        except Exception as def_error:
                                            logger.warning(f"Error inserting definition for '{formatted_word}', def {def_idx}: {definition_text[:50]}... Error: {def_error}")
                    ...
                    # Check for 'related' key at the entry level
                    if 'related' in entry and isinstance(entry['related'], dict):
                        ...
                        for rel_type_raw, rel_items in entry['related'].items():
                            ...
                            if rel_type_enum is None:
                                # Fallback using normalize_relation_type if not in mapping
                                try:
                                    std_rel_type_str, _, _ = normalize_relation_type(rel_type_raw)
                                    rel_type_enum = RelationshipType.from_string(std_rel_type_str) # Get enum member
                                except (ValueError, KeyError):
                                    logger.warning(f"Unknown top-level relation type '{rel_type_raw}' for '{formatted_word}', treating as RELATED.")
                                    rel_type_enum = RelationshipType.RELATED # Safe fallback
                            ...
                            for rel_idx, rel_item in enumerate(rel_items):
                                ...
                                if isinstance(rel_item, dict) and 'value' in rel_item:
                                    related_term = str(rel_item['value']).strip()
                                    # Extract any additional metadata from the dict item
                                    for meta_key, meta_val in rel_item.items():
                                        if meta_key != 'value': item_metadata[meta_key] = meta_val
                                elif isinstance(rel_item, str):
                                    related_term = rel_item.strip()

                                # Process if term is valid and not too long (DB constraints)
                                if related_term and len(related_term) <= 255 and related_term != formatted_word:
                                    try:
                                        # Get/Create ID for the related term, passing source
                                        related_id = get_or_create_word_id(
                                            cur, related_term, language_code=language_code,
                                            source_identifier=source_identifier # Pass mandatory source
                                        )
                                        if related_id:
                                            # Prepare metadata for the relation itself
                                            relation_metadata = {
                                                "source": source_identifier,
                                                "original_type": rel_type_raw, # Store the original KWF type
                                                "index": rel_idx,
                                                "confidence": 85 # Default confidence
                                            }
                                            relation_metadata.update(item_metadata) # Add item-specific metadata

                                            # Insert the relation, passing source
                                            rel_top_id = insert_relation(
                                                cur, word_id, related_id, # From lemma word TO related term
                                                rel_type_enum, # Use the mapped/normalized enum member
                                                source_identifier=source_identifier, # Pass mandatory source
                                                metadata=relation_metadata # Pass combined metadata
                                            )
                                            if rel_top_id: stats["relations_added"] += 1
                                            # Handle bidirectional relations if applicable
                                            # if rel_type_enum.bidirectional: insert_relation(cur, related_id, word_id, rel_type_enum, source_identifier=source_identifier, metadata=relation_metadata)
                    ...
                    # Release the savepoint if all steps for this entry succeeded
                    cur.execute(f"RELEASE SAVEPOINT {savepoint_name}")
                    stats["processed_entries"] += 1

                    # Commit periodically to avoid excessively large transactions
                    # Check if processed_entries is a multiple of 500
                    if stats["processed_entries"] % 500 == 0:
                        try:
                            ...
                            logger.info(f"Processed {stats['processed_entries']} entries, committed batch for {filename}")
                             ...
                             # Consider how to handle this - maybe stop? For now, log and continue.
                             stats["error_entries"] += 500 # Approximate error count for the failed batch
                ...
                # --- Error Handling for a Single Entry ---
                    ...
                    error_msg = str(db_error).split('\n')[0] # Get primary error message
                    logger.error(f"Database error processing entry '{word or f'index {entry_index}'}': {error_msg}", exc_info=False) # Less verbose log
                    error_types[f"DBError: {error_msg}"] = error_types.get(f"DBError: {error_msg}", 0) + 1
                    stats["error_entries"] += 1
                    try: cur.execute(f"ROLLBACK TO SAVEPOINT {savepoint_name}") # Rollback only this entry
                    except Exception as rb_err: logger.critical(f"CRITICAL: Failed rollback to savepoint {savepoint_name}: {rb_err}. Attempting full tx rollback.", exc_info=True); conn.rollback(); raise db_error from rb_err # If rollback fails, raise

                except ValueError as val_error: # Catch ValueErrors (e.g., from get_or_create_word_id failure)
                    logger.error(f"ValueError processing entry '{word or f'index {entry_index}'}': {val_error}", exc_info=False)
                    ...
                    error_msg = str(e)
                    logger.error(f"Unexpected error processing entry '{word or f'index {entry_index}'}': {error_msg}", exc_info=True) # Log traceback for unexpected
                    error_types[f"UnexpectedError: {error_msg}"] = error_types.get(f"UnexpectedError: {error_msg}", 0) + 1
                    stats["error_entries"] += 1
                    try: cur.execute(f"ROLLBACK TO SAVEPOINT {savepoint_name}") # Rollback entry
                    except Exception as rb_err: logger.critical(f"CRITICAL: Failed rollback to savepoint {savepoint_name}: {rb_err}. Attempting full tx rollback.", exc_info=True); conn.rollback(); raise e from rb_err # Raise if rollback fails

                finally:
                    pbar.update(1) # Ensure progress bar always updates

        # --- Final Commit ---
        # Commit any remaining changes after the loop finishes
        try:
            conn.commit()
            ...
            conn.rollback() # Rollback the last batch

    # --- Outer Exception Handling ---
    except Exception as outer_e:
        # Catch errors occurring outside the main entry processing loop (e.g., file read)
        ...
        stats["error_entries"] += 1 # Count this as a general error
        # Ensure transaction rollback if an outer error occurs
        try:
            if conn and hasattr(conn, 'rollback') and conn.status != psycopg2.extensions.STATUS_IN_TRANSACTION:
                 logger.info("Transaction already closed or rolled back in outer exception.")
            elif conn and hasattr(conn, 'rollback'):
    ...
    # --- Final Logging ---
    finally: # Ensure stats are logged even if errors occurred
                        ...
                        for sense_idx, sense in enumerate(senses):
                            ...
                            try:
                                ...
                                definition_id = insert_definition(
                                    ...
                                    original_pos=pos_str, # Store original POS string(s)
...
# Replace the existing process_root_words_cleaned function (starts around line 4376)
@with_transaction(commit=False)  # Manage transactions manually
def process_root_words_cleaned(cur, filename: str):
    ...
    if not isinstance(data, list): # Check if data is a list of entries
         logger.error(f"File {filename} does not contain a list of root word entries.")
         stats["errors"] += 1
         return stats
    ...
    for entry_index, root_word_entry in enumerate(tqdm(data, total=total_roots, desc=f"Processing {source_identifier}")):
        ...
        try:
            ...
            if not word_id: # Check if word_id was successfully obtained
            ...
            if isinstance(associated_words, list):
                for assoc_idx, assoc_word_entry in enumerate(associated_words):
                    ...
                    if assoc_word and assoc_word != root_word:
                        ...
                        try:
                            ...
                            if assoc_word_id:
                                 ...
                                 rel_id = insert_relation(
                                     cur, assoc_word_id, root_word_id, # from derived TO root
                                     RelationshipType.DERIVED_FROM,
                                     source_identifier=source_identifier, # Pass mandatory source
                                 ...
                                 if rel_id:
                                     stats["relations_added"] += 1
                                 # Optionally add inverse ROOT_OF relation from root to derived
                                 # inv_rel_metadata = {"source": source_identifier, "index": assoc_idx, "confidence": 95}
                                 # inv_rel_id = insert_relation(cur, root_word_id, assoc_word_id, RelationshipType.ROOT_OF, source_identifier=source_identifier, metadata=inv_rel_metadata)
                                 # if inv_rel_id: stats["relations_added"] += 1 # Decide how to count relation pairs
            ...
            logger.error(f"Error processing root entry index {entry_index} ('{root_word_entry.get('root_word', 'N/A')}') in {filename}: {entry_e}", exc_info=False) # Less verbose logs
            stats["errors"] += 1
            try:
...
def process_kaikki_jsonl(cur, filename: str):
    ...
    # Process sense relationships
        # Process sense relationships
    def process_sense_relationships(cur, word_id: int, sense: Dict, source_identifier: str): # Added source_identifier
        # Process synonyms
        if 'synonyms' in sense and isinstance(sense['synonyms'], list):
            for synonym in sense['synonyms']:
                if isinstance(synonym, dict) and 'word' in synonym:
                    ...
                    try:
                         # <<< CORRECTED: Pass source_identifier
                        ...
                        if 'tags' in synonym and isinstance(synonym['tags'], list):
                         ...
                         # <<< CORRECTED: Pass source_identifier variable
                        insert_relation(cur, word_id, syn_id, RelationshipType.SYNONYM.rel_value, source_identifier, metadata)
                    except Exception as e:
                ...
                if isinstance(antonym, dict) and 'word' in antonym:
                    try:
                        ...
                        if 'tags' in antonym and isinstance(antonym['tags'], list):
                         ...
                         # <<< CORRECTED: Pass source_identifier variable
                        insert_relation(cur, word_id, ant_id, RelationshipType.ANTONYM.rel_value, source_identifier, metadata)
                    except Exception as e:
                ...
                if isinstance(hypernym, dict) and 'word' in hypernym:
                    try:
                        ...
                        if 'tags' in hypernym and isinstance(hypernym['tags'], list):
                         ...
                         # <<< CORRECTED: Pass source_identifier variable
                        insert_relation(cur, word_id, hyper_id, RelationshipType.HYPERNYM.rel_value, source_identifier, metadata)
                    except Exception as e:
                ...
                if isinstance(hyponym, dict) and 'word' in hyponym:
                    try:
                        ...
                        if 'tags' in hyponym and isinstance(hyponym['tags'], list):
                         ...
                         # <<< CORRECTED: Pass source_identifier variable
                        insert_relation(cur, word_id, hypo_id, RelationshipType.HYPONYM.rel_value, source_identifier, metadata)
                    except Exception as e:
                ...
                if isinstance(holonym, dict) and 'word' in holonym:
                    try:
                        ...
                        if 'tags' in holonym and isinstance(holonym['tags'], list):
                         ...
                         # <<< CORRECTED: Pass source_identifier variable
                        insert_relation(cur, word_id, holo_id, RelationshipType.HOLONYM.rel_value, source_identifier, metadata)
                    except Exception as e:
                ...
                if isinstance(meronym, dict) and 'word' in meronym:
                    try:
                        ...
                        if 'tags' in meronym and isinstance(meronym['tags'], list):
                         ...
                         # <<< CORRECTED: Pass source_identifier variable
                        insert_relation(cur, word_id, mero_id, RelationshipType.MERONYM.rel_value, source_identifier, metadata)
                    except Exception as e:
                        logger.error(f"Error processing meronym relation for word ID {word_id}: {str(e)}")

        # Process derived terms (This might be redundant if handled in main process_entry, review logic)
                ...
                if isinstance(derived, dict) and 'word' in derived:
                    try:
                        derived_word = derived['word']
                         # <<< CORRECTED: Pass source_identifier
                        derived_id = get_or_create_word_id(cur, derived_word, 'tl', source_identifier=source_identifier)
                        ...
                        logger.error(f"Error processing derived relation (sense level) for word ID {word_id}: {str(e)}")
                ...
                if isinstance(see_also, dict) and 'word' in see_also:
                    try:
                        see_also_word = see_also['word']
                         # <<< CORRECTED: Pass source_identifier
                        see_also_id = get_or_create_word_id(cur, see_also_word, 'tl', source_identifier=source_identifier)
                        metadata = {'confidence': 70}
                        if 'tags' in see_also and isinstance(see_also['tags'], list):
                            metadata['tags'] = ','.join(see_also['tags'])
                         # <<< CORRECTED: Pass source_identifier variable
                        insert_relation(cur, word_id, see_also_id, RelationshipType.SEE_ALSO.rel_value, source_identifier, metadata)
                    except Exception as e:
                        logger.error(f"Error processing see_also relation for word ID {word_id}: {str(e)}")

    def process_form_relationships(cur, word_id: int, entry: Dict, language_code: str, source_identifier: str): # Added source_identifier
    ...
    def process_links(cur, definition_id: int, links: List[List[str]]):
        ...
        # <<< CORRECTED: Signature parameter order fixed
    def process_etymology(cur, word_id: int, etymology_text: str, source_identifier: str, etymology_templates: List[Dict] = None):
        ...
        normalized_components = None # Or implement extract_etymology_components if needed
        etymology_structure = None # Or parse structure if needed
        language_codes = None
        ...
        if etymology_templates:
            ...
            for template in etymology_templates:
                 # Simplified language extraction logic - adapt if needed
                if 'name' in template and 'language' in template['name'].lower():
                    if 'args' in template and template['args']:
                        # Try to find a standard 2-3 letter code
                        potential_langs = [arg for arg in template['args'].values() if isinstance(arg, str) and 1 < len(arg) <= 3]
                        if potential_langs:
                             languages.extend(potential_langs)
            if languages:
                 language_codes = ",".join(sorted(list(set(languages)))) # Comma-separated unique codes

        try:
            # <<< CORRECTED: Call insert_etymology helper function
            insert_etymology(
                cur,
                word_id=word_id,
    ...
    def process_entry(cur, entry: Dict, source_identifier: str):
        ...
        try:
            ...
            word_id = get_or_create_word_id(
                ...
                source_identifier=source_identifier # <<< CORRECTED: Pass source_identifier
            )
            ...
            process_form_relationships(cur, word_id, entry, language_code, source_identifier) # <<< CORRECTED: Pass source_identifier

            # Process etymologies
            if 'etymology_text' in entry:
                 # <<< CORRECTED: Pass source_identifier (note: process_etymology signature also needs fix - see below)
                ...
                for sense in entry['senses']:
                    ...
                    try:
                        definition_id = insert_definition(
                            ...
                            source_identifier=source_identifier # <<< CORRECTED: Pass source_identifier variable
                        )
                    except Exception as e:
                    ...
                    if definition_id:
                        try:
                             # <<< CORRECTED: Pass source_identifier
                            process_sense_relationships(cur, word_id, sense, source_identifier)
                        except Exception as e:
                ...
                for derived in entry['derived']:
                    try:
                        if isinstance(derived, dict) and 'word' in derived:
                            derived_word = derived['word']
                             # <<< CORRECTED: Pass source_identifier
                            derived_id = get_or_create_word_id(cur, derived_word, language_code, source_identifier=source_identifier)
                            metadata = {'confidence': 95}
                            if '_dis1' in derived:
                                metadata['distance'] = derived['_dis1']
                             # <<< CORRECTED: Pass source_identifier variable instead of "kaikki"
                            insert_relation(cur, word_id, derived_id, RelationshipType.ROOT_OF.rel_value, source_identifier, metadata)
                    except Exception as e:
                    ...
                    try:
                        if isinstance(related, dict) and 'word' in related:
                            related_word = related['word']
                             # <<< CORRECTED: Pass source_identifier
                            related_id = get_or_create_word_id(cur, related_word, language_code, source_identifier=source_identifier)
                            metadata = {'confidence': 70}
                            if '_dis1' in related:
        ...
        with open(filename, 'r', encoding='utf-8') as f:
            ...
            for line in f:
                try:
                    ...
                    result = process_entry(cur, entry, filename)
                    ...
                    if not result and 'forms' in entry:
                        for form in entry.get('forms', []):
                            if 'tags' in form and 'Baybayin' in form.get('tags', []):
                                ...
                                logger.info(f"Trying fallback processing for entry with word '{entry.get('word')}' from {filename}")
                                result = process_entry_without_baybayin(cur, entry, filename)
                                if result:
