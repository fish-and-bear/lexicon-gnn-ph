...
class BaybayinRomanizer:
    ...
    def process_syllable(self, chars: List[str]) -> Tuple[str, int]:
        """Process a syllable in Baybayin text."""
        if not chars:
            return '', 0
            
        result = ''
        pos = 0
        char = self.get_char_info(chars[0])
        
        if not char:
            return '', 1
            
        if char.char_type == BaybayinCharType.VOWEL:
            result = char.default_sound
            pos += 1
        elif char.char_type == BaybayinCharType.CONSONANT:
            result = char.default_sound
            pos += 1
...
def process_baybayin_entries(cur):
    """Process entries with Baybayin text."""
    try:
        # Get all entries with Baybayin text
        cur.execute("""
        ...
        for entry in entries:
            try:
                word_id, lemma, baybayin_form, romanized_form = entry
                process_baybayin_data(cur, word_id, baybayin_form, romanized_form)
            except Exception as e:
                logger.error(f"Error processing Baybayin entry {lemma}: {str(e)}")
                continue
    except Exception as e:
        logger.error(f"Error processing Baybayin entries: {str(e)}")
        raise
...
def process_kwf_dictionary(cur, filename: str):
    """Process KWF dictionary entries."""
...
def lookup_word(args):
    ...
    try:
        ...
        cur.execute("""
            SELECT w.id, w.lemma, w.language_code, w.normalized_lemma,
                   w.preferred_spelling, w.tags, w.has_baybayin,
                   w.baybayin_form, w.romanized_form,
                   w.pronunciation_data, w.root_word_id
            FROM words w
            WHERE w.normalized_lemma = %s
            OR w.lemma = %s
        """, (normalize_lemma(args.word), args.word))
        ...
        for result in results:
            word_info = []
            (word_id, lemma, language_code, normalized_lemma,
             preferred_spelling, tags, has_baybayin, baybayin_form,
             romanized_form, pronunciation_data, root_word_id) = result
            
            # Display word and language
            word_info.append(f"\nWord: {format_word_display(lemma, has_baybayin)}")
            word_info.append(f"Language: {language_code.upper()}")
            
            # Display preferred spelling if different
            if preferred_spelling and preferred_spelling != lemma:
                word_info.append(f"Preferred Spelling: {preferred_spelling}")
            
            # Display Baybayin information
            if has_baybayin and baybayin_form:
                word_info.append(f"Baybayin: {baybayin_form}")
                if romanized_form:
                    word_info.append(f"Romanized: {romanized_form}")
            
            # Display pronunciation data
                    ...
                    for sound in pronunciation_data['sounds']:
                        if isinstance(sound, dict):
                            if 'ipa' in sound:
                                ...
                                if 'audio' in sound:
                                    word_info.append(f"Audio: {sound['audio']}")
            
            # Get definitions
            cur.execute("""
                SELECT d.definition_text, p.pos_name, d.examples,
                       d.usage_notes, d.category, d.sources
                FROM definitions d
            ...
            if definitions:
                word_info.append("\nDefinitions:")
                for i, (text, pos, examples, usage_notes, category, sources) in enumerate(definitions, 1):
                    ...
                    if examples:
                        ...
                        if examples:
                            word_info.append("Examples:")
                            for example in examples:
                                word_info.append(f"  • {example}")
                    
                    # Display usage notes
                    if usage_notes:
                        usage_notes = json.loads(usage_notes)
                        if usage_notes:
                            word_info.append("Usage Notes:")
                            for note in usage_notes:
                                word_info.append(f"  • {note}")
                    
                    # Display category
                    if category:
                        word_info.append(f"Category: {category}")
                    ...
                    if sources:
                        ...
                        if source_list:
                            word_info.append(f"Sources: {', '.join(source_list)}")
            ...
            if etymologies:
                word_info.append("\nEtymology:")
                for etymology_text, components, language_codes, sources in etymologies:
                    word_info.append(f"  {etymology_text}")
                    if components:
                        components = json.loads(components)
                        if components:
                            word_info.append("Components:")
                            for component in components:
                                word_info.append(f"  • {component}")
            
            # Display relations
            cur.execute("""
                SELECT r.relation_type, w.lemma, w.language_code,
                       w.has_baybayin, r.sources
                FROM relations r
            ...
            if relations:
                word_info.append("\nRelated Words:")
                for rel_type, rel_lemma, rel_lang, has_baybayin, sources in relations:
                    display_word = format_word_display(rel_lemma, has_baybayin)
                    word_info.append(f"  • {rel_type}: {display_word} ({rel_lang.upper()})")
            ...
            for form in processed['data']['forms']:
                if 'Baybayin' in form.get('tags', []):
                    try:
                        ...
                        logger.error(f"Error processing Baybayin form: {str(e)}")
                        continue
                ...
                for rel_word in related:
                    try:
                        rel_word_id = get_or_create_word_id(
                            cur,
                            rel_word['word'],
                            rel_word.get('language_code', 'tl')
                        )
                        insert_relation(cur, word_id, rel_word_id, rel_type)
                    except Exception as e:
                        logger.error(f"Error processing related word: {str(e)}")
                        continue
    except Exception as e:
