...
class Query(graphene.ObjectType):
    ...
    def resolve_find_related_words(self, info, word_id, relation_type=None,
        ...
        """Resolve find_related_words query."""
        word = WordModel.query.get(word_id)
        if not word:
            return []
        
        if direction == 'outgoing':
            query = RelationModel.query.filter_by(from_word_id=word_id)
        elif direction == 'incoming':
            query = RelationModel.query.filter_by(to_word_id=word_id)
        else:
            query = RelationModel.query.filter(
                or_(
