...
def search():
    ...
    try:
        ...
        SELECT w.id, w.lemma, w.normalized_lemma, w.language_code, 
               w.has_baybayin, w.baybayin_form, w.completeness_score
        FROM words w
        """
        ...
        if sort_field == 'relevance':
            ...
            main_sql += f" ORDER BY w.completeness_score {sort_order}"
        else:
                ...
                if completeness_filter is not None:
                    ...
                    for row in rows:
                        basic_results.append({
                            ...
                            'baybayin_form': row[5],
                            'completeness_score': row[6] if len(row) > 6 else None
                        })
...
def search_baybayin():
    """Search specifically for words with Baybayin script."""
    try:
        ...
        mode = request.args.get('mode', 'all')  # all, exact, contains
        
        # Build search conditions
        conditions = ["has_baybayin = true", "baybayin_form IS NOT NULL", "baybayin_form != ''"]
        params = {
            'limit': limit,
            'offset': offset
        }
        ...
        if q:
            ...
            if mode == 'exact':
                conditions.append("baybayin_form = :query")
                params['query'] = q
            elif mode == 'contains':
                conditions.append("baybayin_form LIKE :query")
                params['query'] = f"%{q}%"
            else:  # Default 'all' mode
                # Try to detect if query contains actual Baybayin characters
                if any(ord(c) >= 0x1700 and ord(c) <= 0x171F for c in q):
                    # Search by Baybayin characters
                    conditions.append("baybayin_form LIKE :query")
                    params['query'] = f"%{q}%"
                else:
                    # Search by Latin script, matching on lemma or normalized fields
                    conditions.append("(lemma LIKE :query OR normalized_lemma LIKE :query)")
                    params['query'] = f"%{q}%"
        ...
        sql = f"""
        SELECT 
            id, lemma, normalized_lemma, language_code, has_baybayin, 
            baybayin_form, root_word_id, is_proper_noun, is_abbreviation
        FROM words
        WHERE {where_clause}
        ORDER BY lemma
        ...
        # Get count of total matches (for pagination)
        count_sql = f"""
        SELECT COUNT(*) FROM words WHERE {where_clause}
        """
        ...
        for r in results:
            formatted_results.append({
                ...
                "has_baybayin": r.has_baybayin,
                "baybayin_form": r.baybayin_form,
...
def convert_to_baybayin():
    """Convert Latin text to Baybayin script using algorithmic conversion."""
    try:
        ...
        # Import BaybayinRomanizer for conversion
        from backend.dictionary_manager import BaybayinRomanizer
        
        # Convert text to Baybayin
        baybayin_text = BaybayinRomanizer.to_baybayin(text)
        ...
        logger.error(f"Error in Baybayin conversion: {str(e)}", exc_info=True)
...
@bp.route("/baybayin/romanize", methods=["POST"])
def romanize_from_baybayin():
    """Convert Baybayin script to Latin text (romanization)."""
    try:
        API_REQUESTS.labels(endpoint="romanize_from_baybayin", method="POST").inc()
        ...
        if not any(ord(c) >= 0x1700 and ord(c) <= 0x171F for c in text):
            return jsonify({
                "error": "Input doesn't appear to contain Baybayin characters",
                "success": False
            }), 400
        ...
        # Convert Baybayin to Latin text
        latin_text = BaybayinRomanizer.to_latin(text)
        ...
        logger.error(f"Error in Baybayin romanization: {str(e)}", exc_info=True)
        API_ERRORS.labels(endpoint="romanize_from_baybayin", error_type=type(e).__name__).inc()
        return jsonify({
            "error": f"An unexpected error occurred: {str(e)}",
            "type": type(e).__name__,
            "success": False
...
@bp.route("/baybayin/characters", methods=["GET"])
@cached_query(timeout=86400)  # Cache for 24 hours as this is static data
def get_baybayin_characters():
    """Get reference information about Baybayin characters."""
    try:
        API_REQUESTS.labels(endpoint="get_baybayin_characters", method="GET").inc()
        
        # Define Baybayin character data
        characters = [
            {
                "unicode": "1700",
                "character": "ᜀ",
                "name": "BAYBAYIN LETTER A",
                "latin_equivalent": "a",
                "description": "Represents the vowel 'a'"
            },
            {
                "unicode": "1701",
                "character": "ᜁ",
                "name": "BAYBAYIN LETTER I",
                "latin_equivalent": "i/e",
                "description": "Represents the vowels 'i' and 'e'"
            },
            {
                "unicode": "1702",
                "character": "ᜂ",
                "name": "BAYBAYIN LETTER U",
                "latin_equivalent": "u/o",
                "description": "Represents the vowels 'u' and 'o'"
            },
            {
                "unicode": "1703",
                "character": "ᜃ",
                "name": "BAYBAYIN LETTER KA",
                "latin_equivalent": "ka",
                "description": "Consonant 'ka'"
            },
            {
                "unicode": "1704",
                "character": "ᜄ",
                "name": "BAYBAYIN LETTER GA",
                "latin_equivalent": "ga",
                "description": "Consonant 'ga'"
            },
            {
                "unicode": "1705",
                "character": "ᜅ",
                "name": "BAYBAYIN LETTER NGA",
                "latin_equivalent": "nga",
                "description": "Consonant 'nga'"
            },
            {
                "unicode": "1706",
                "character": "ᜆ",
                "name": "BAYBAYIN LETTER TA",
                "latin_equivalent": "ta",
                "description": "Consonant 'ta'"
            },
            {
                "unicode": "1707",
                "character": "ᜇ",
                "name": "BAYBAYIN LETTER DA",
                "latin_equivalent": "da",
                "description": "Consonant 'da'"
            },
            {
                "unicode": "1708",
                "character": "ᜈ",
                "name": "BAYBAYIN LETTER NA",
                "latin_equivalent": "na",
                "description": "Consonant 'na'"
            },
            {
                "unicode": "1709",
                "character": "ᜉ",
                "name": "BAYBAYIN LETTER PA",
                "latin_equivalent": "pa",
                "description": "Consonant 'pa'"
            },
            {
                "unicode": "170A",
                "character": "ᜊ",
                "name": "BAYBAYIN LETTER BA",
                "latin_equivalent": "ba",
                "description": "Consonant 'ba'"
            },
            {
                "unicode": "170B",
                "character": "ᜋ",
                "name": "BAYBAYIN LETTER MA",
                "latin_equivalent": "ma",
                "description": "Consonant 'ma'"
            },
            {
                "unicode": "170C",
                "character": "ᜌ",
                "name": "BAYBAYIN LETTER YA",
                "latin_equivalent": "ya",
                "description": "Consonant 'ya'"
            },
            {
                "unicode": "170D",
                "character": "ᜍ",
                "name": "BAYBAYIN LETTER RA",
                "latin_equivalent": "ra",
                "description": "Consonant 'ra'"
            },
            {
                "unicode": "170E",
                "character": "ᜎ",
                "name": "BAYBAYIN LETTER LA",
                "latin_equivalent": "la",
                "description": "Consonant 'la'"
            },
            {
                "unicode": "170F",
                "character": "ᜏ",
                "name": "BAYBAYIN LETTER WA",
                "latin_equivalent": "wa",
                "description": "Consonant 'wa'"
            },
            {
                "unicode": "1710",
                "character": "ᜐ",
                "name": "BAYBAYIN LETTER SA",
                "latin_equivalent": "sa",
                "description": "Consonant 'sa'"
            },
            {
                "unicode": "1711",
                "character": "ᜑ",
                "name": "BAYBAYIN LETTER HA",
                "latin_equivalent": "ha",
                "description": "Consonant 'ha'"
            },
            {
                "unicode": "1712",
                "character": "ᜒ",
                "name": "BAYBAYIN VOWEL SIGN I",
                "latin_equivalent": "i/e",
                "description": "Vowel sign i/e, placed above consonant"
            },
            {
                "unicode": "1713",
                "character": "ᜓ",
                "name": "BAYBAYIN VOWEL SIGN U",
                "latin_equivalent": "u/o",
                "description": "Vowel sign u/o, placed above consonant"
            },
            {
                "unicode": "1714",
                "character": "᜔",
                "name": "BAYBAYIN VIRAMA",
                "latin_equivalent": "",
                "description": "Removes inherent 'a' vowel from consonant"
            }
        ]
        ...
        return jsonify({
            ...
            "reference": "Unicode 14.0.0",
            "usage_notes": "Baybayin is a pre-Spanish Philippine script used primarily for Tagalog"
        })
        
    except Exception as e:
        logger.error(f"Error fetching Baybayin characters: {str(e)}", exc_info=True)
        API_ERRORS.labels(endpoint="get_baybayin_characters", error_type=type(e).__name__).inc()
        return jsonify({
            "error": f"An unexpected error occurred: {str(e)}",
            "type": type(e).__name__
        }), 500
...
def get_baybayin_statistics():
    ...
    try:
        ...
        language = request.args.get('language')
        lang_filter = "WHERE language_code = :language" if language else ""
        ...
        {lang_filter}
        """
        coverage = db.session.execute(text(sql_coverage), params).fetchone()
        
        # 2. Distribution by language
        sql_by_language = """
        SELECT 
            language_code,
            COUNT(*) as total_words,
        ...
        # 3. Character frequency in Baybayin forms
        sql_char_freq = f"""
        WITH chars AS (
            SELECT 
                w.id,
                w.lemma,
                w.baybayin_form,
                UNNEST(string_to_array(w.baybayin_form, NULL)) as char
            FROM words w
            WHERE has_baybayin = true AND baybayin_form IS NOT NULL AND baybayin_form != ''
            {lang_filter[6:] if lang_filter else ""}
        )
        SELECT 
            char,
            COUNT(*) as frequency
        ...
        """
        char_freq = db.session.execute(text(sql_char_freq), params).fetchall()
        
        # 4. Get words recently added with Baybayin
        sql_recent = f"""
        SELECT 
            id, lemma, language_code, baybayin_form, created_at
        FROM words
        ...
        LIMIT 10
        """
        recent = db.session.execute(text(sql_recent), params).fetchall()
        
        # Format results
        result = {
            "coverage": {
                "total_words": coverage.total_words,
                "with_baybayin": coverage.with_baybayin,
                "percentage": float(coverage.percentage) if coverage.percentage is not None else 0
            },
            "by_language": [
                {
...
@cached_query(timeout=300)  # Cache for 5 minutes
@limiter.limit("30 per minute", key_func=get_remote_address)
def advanced_filter():
    """
    Filter words by multiple criteria with advanced options.
    This endpoint focuses on filtering rather than text search.
    ---
    parameters:
      - name: language
        in: query
        type: string
        description: Filter by language code (comma-separated for multiple)
      - name: pos
        in: query
        type: string
        description: Filter by part of speech code (comma-separated for multiple)
      - name: has_baybayin
        in: query
        type: boolean
        description: Filter words that have baybayin script
      - name: has_pronunciation
        in: query
        type: boolean
        description: Filter words with pronunciation entries
      - name: has_etymology
        in: query
        type: boolean
        description: Filter words with etymology entries
      - name: has_examples
        in: query
        type: boolean
        description: Filter words with usage examples
      - name: is_root
        in: query
        type: boolean
        description: Filter root words (true) or derived words (false)
      - name: is_proper_noun
        in: query
        type: boolean
        description: Filter proper nouns
      - name: min_completeness
        in: query
        type: number
        description: Minimum completeness score (0.0-1.0)
      - name: min_definitions
        in: query
        type: integer
        description: Minimum number of definitions
      - name: max_definitions
        in: query
        type: integer
        description: Maximum number of definitions
      - name: relation_type
        in: query
        type: string
        description: Filter by relation type (comma-separated for multiple)
      - name: etymology_source
        in: query
        type: string
        description: Filter by etymology source language
      - name: created_after
        in: query
        type: string
        format: date-time
        description: Filter words created after this date (ISO format)
      - name: created_before
        in: query
        type: string
        format: date-time
        description: Filter words created before this date (ISO format)
      - name: updated_after
        in: query
        type: string
        format: date-time
        description: Filter words updated after this date (ISO format)
      - name: updated_before
        in: query
        type: string
        format: date-time
        description: Filter words updated before this date (ISO format)
      - name: definition_contains
        in: query
        type: string
        description: Filter words with definitions containing this text
      - name: sort
        in: query
        type: string
        default: "alphabetical"
        enum: ["alphabetical", "completeness", "created", "updated"]
    ...
    responses:
      200:
        description: Filtered word list
      400:
    ...
    try:
        ...
        schema = FilterQuerySchema()
        params = schema.load(request.args)
        
        # Set default sort to alphabetical for filter endpoint
        sort_field = params.get('sort', 'alphabetical')
        sort_order = params.get('order', 'asc').upper()
        limit = int(params.get('limit', 50))
        offset = int(params.get('offset', 0))
        include_full = params.get('include_full', False)
        ...
        # Copy parameters to filters object dynamically
        filter_fields = [
            'language', 'pos', 'has_baybayin', 'has_pronunciation', 
            'has_etymology', 'has_examples', 'is_root', 'is_proper_noun',
            'is_abbreviation', 'is_initialism', 'min_completeness',
            'min_definitions', 'max_definitions', 'relation_type',
            'etymology_source', 'created_after', 'created_before',
            'updated_after', 'updated_before', 'definition_contains', 'tag_contains'
        ]
        ...
        # Process filters
        if filters.get('has_baybayin') is not None:
            where_conditions.append("w.has_baybayin = :has_baybayin")
            query_params['has_baybayin'] = filters['has_baybayin']
        ...
        if filters.get('is_proper_noun') is not None:
            where_conditions.append("w.is_proper_noun = :is_proper_noun")
            query_params['is_proper_noun'] = filters['is_proper_noun']
            
        if filters.get('is_abbreviation') is not None:
            where_conditions.append("w.is_abbreviation = :is_abbreviation")
            query_params['is_abbreviation'] = filters['is_abbreviation']
            
        if filters.get('is_initialism') is not None:
            where_conditions.append("w.is_initialism = :is_initialism")
            query_params['is_initialism'] = filters['is_initialism']
        ...
        if filters.get('has_pronunciation') is not None:
            join_clauses.append("LEFT JOIN pronunciations p ON p.word_id = w.id")
            having_conditions.append("COUNT(DISTINCT p.id) > 0" if filters['has_pronunciation'] else "COUNT(DISTINCT p.id) = 0")
            
        if filters.get('has_etymology') is not None:
            join_clauses.append("LEFT JOIN etymologies e ON e.word_id = w.id")
            having_conditions.append("COUNT(DISTINCT e.id) > 0" if filters['has_etymology'] else "COUNT(DISTINCT e.id) = 0")
            
        if filters.get('has_forms') is not None:
            join_clauses.append("LEFT JOIN word_forms wf ON wf.word_id = w.id")
            having_conditions.append("COUNT(DISTINCT wf.id) > 0" if filters['has_forms'] else "COUNT(DISTINCT wf.id) = 0")
        
        # Process POS filter
        if filters.get('pos') is not None:
            pos_codes = filters['pos'].split(',')
            if len(pos_codes) == 1:
                join_clauses.append("JOIN definitions d_pos ON d_pos.word_id = w.id")
                join_clauses.append("JOIN parts_of_speech pos ON d_pos.standardized_pos_id = pos.id")
        ...
        # Process definition count filters
        if filters.get('min_definitions') is not None or filters.get('max_definitions') is not None:
            join_clauses.append("LEFT JOIN definitions d_count ON d_count.word_id = w.id")
            
            if filters.get('min_definitions') is not None:
                having_conditions.append("COUNT(DISTINCT d_count.id) >= :min_definitions")
                query_params['min_definitions'] = int(filters['min_definitions'])
                
            if filters.get('max_definitions') is not None:
                having_conditions.append("COUNT(DISTINCT d_count.id) <= :max_definitions")
                query_params['max_definitions'] = int(filters['max_definitions'])
        
        # Process other content filters
        if filters.get('definition_contains') is not None:
            join_clauses.append("JOIN definitions d_content ON d_content.word_id = w.id")
            where_conditions.append("d_content.definition_text ILIKE :definition_contains")
            query_params['definition_contains'] = f"%{filters['definition_contains']}%"
        
        if filters.get('has_examples') is not None:
            join_clauses.append("JOIN definitions d_examples ON d_examples.word_id = w.id")
            if filters['has_examples']:
                where_conditions.append("(d_examples.examples IS NOT NULL AND d_examples.examples != '[]')")
            else:
        ...
        if filters.get('relation_type') is not None:
            relation_types = filters['relation_type'].split(',')
            if len(relation_types) == 1:
                join_clauses.append("JOIN relations r ON (r.from_word_id = w.id OR r.to_word_id = w.id)")
                where_conditions.append("r.relation_type = :relation_type")
        ...
        if filters.get('etymology_source') is not None:
            join_clauses.append("JOIN etymologies etym ON etym.word_id = w.id")
            where_conditions.append("etym.language_codes ILIKE :etymology_source")
            query_params['etymology_source'] = f"%{filters['etymology_source']}%"
        
        if filters.get('tag_contains') is not None:
            where_conditions.append("w.tags ILIKE :tag_contains")
            query_params['tag_contains'] = f"%{filters['tag_contains']}%"
        
        # Completeness score filter
        if filters.get('min_completeness') is not None:
            # For performance, we'll do a rough filter in SQL
            where_conditions.append("""
            (
                (SELECT COUNT(*) FROM definitions WHERE word_id = w.id) > 0 OR
                (SELECT COUNT(*) FROM etymologies WHERE word_id = w.id) > 0 OR
        ...
        # If count is zero, return empty results
        if result['count'] == 0 and not filters.get('min_completeness'):
            return jsonify(result), 200
...
class FilterQuerySchema(Schema):
    ...
    pos = fields.String(missing=None)
    has_baybayin = fields.Boolean(missing=None)
    has_pronunciation = fields.Boolean(missing=None)
    has_etymology = fields.Boolean(missing=None)
    has_examples = fields.Boolean(missing=None)
    is_root = fields.Boolean(missing=None)
    is_proper_noun = fields.Boolean(missing=None)
    is_abbreviation = fields.Boolean(missing=None)
    is_initialism = fields.Boolean(missing=None)
    min_completeness = fields.Float(missing=None, validate=validate.Range(min=0.0, max=1.0))
    min_definitions = fields.Integer(missing=None, validate=validate.Range(min=1))
    max_definitions = fields.Integer(missing=None, validate=validate.Range(min=1))
    relation_type = fields.String(missing=None)
    etymology_source = fields.String(missing=None)
    ...
    updated_before = fields.DateTime(missing=None)
    definition_contains = fields.String(missing=None)
    tag_contains = fields.String(missing=None)
    sort = fields.String(missing="alphabetical", validate=validate.OneOf(
        ["alphabetical", "completeness", "created", "updated"]))
    ...
    include_full = fields.Boolean(missing=False)
    ...
    def validate_date_ranges(self, data, **kwargs):
        ...
        if data.get('updated_after') and data.get('updated_before'):
            if data['updated_after'] >= data['updated_before']:
                raise ValidationError("updated_after must be before updated_before")
                
    @validates_schema
    def validate_definition_counts(self, data, **kwargs):
        if data.get('min_definitions') and data.get('max_definitions'):
            if data['min_definitions'] > data['max_definitions']:
                raise ValidationError("min_definitions must be less than or equal to max_definitions")

    @post_load
    def cleanup_data(self, data, **kwargs):
        # Convert string 'true'/'false' values to boolean
        boolean_fields = ['has_baybayin', 'has_pronunciation', 'has_etymology', 
                         'has_examples', 'is_root', 'is_proper_noun', 
                         'is_abbreviation', 'is_initialism', 'include_full']
                         
        for field in boolean_fields:
            if isinstance(data.get(field), str):
                if data[field].lower() == 'true':
                    data[field] = True
                elif data[field].lower() == 'false':
                    data[field] = False
