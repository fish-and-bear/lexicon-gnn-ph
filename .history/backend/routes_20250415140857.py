...
def get_semantic_network(word: str):
    """
    Get semantic network of relationships for a word.
    ---
    parameters:
      - name: word
        in: path
        type: string
        required: true
        description: The word to get the network for
      - name: depth
        in: query
        type: integer
        default: 2
        description: How deep to traverse the network (1-3)
      - name: include_affixes
        in: query
        type: boolean
        default: false
        description: Whether to include affixation relationships
      - name: include_etymology
        in: query
        type: boolean
        default: false
        description: Whether to include etymology relationships
      - name: cluster_threshold
        in: query
        type: number
        default: 0.5
        description: Threshold for clustering nodes (0.0-1.0)
    responses:
      200:
    ...
    """
    try:
        API_REQUESTS.labels(endpoint="get_semantic_network", method="GET").inc()
        
        # Create a dedicated session for network operations
        from sqlalchemy.orm import Session
        ...
        with Session(db.engine) as session:
            ...
            depth = min(max(int(request.args.get('depth', 2)), 1), 3)  # Limit depth to 1-3
            include_affixes = request.args.get('include_affixes', 'false').lower() == 'true'
            include_etymology = request.args.get('include_etymology', 'false').lower() == 'true'
            cluster_threshold = min(max(float(request.args.get('cluster_threshold', 0.5)), 0.0), 1.0)
            
            # First query to get the word ID
            sql = """
            SELECT id, lemma, normalized_lemma, language_code 
            FROM words 
            WHERE LOWER(lemma) = LOWER(:word) 
            ORDER BY language_code = 'tl' DESC LIMIT 1
            """
            
            word_result = session.execute(text(sql), {"word": word}).fetchone()
            
            if not word_result:
                # Try with normalized_lemma
                sql = """
                SELECT id, lemma, normalized_lemma, language_code 
                FROM words 
                WHERE LOWER(normalized_lemma) = LOWER(:word)
                ORDER BY language_code = 'tl' DESC LIMIT 1
                """
                ...
                if not word_result:
                    return jsonify({
                        "error": f"Word '{word}' not found",
                        "details": "The requested word does not exist in the dictionary."
                    }), 404
            
            # Begin constructing the network
            central_word_id = word_result.id
            central_word = word_result.lemma
            central_lang = word_result.language_code
            
            # Initialize the network
            # Use networkx-like structure
            nodes = []
            links = []
            visited = set()  # Keep track of visited words
            node_map = {}  # Map word IDs to node indices
            id_to_lemma = {}  # Map IDs to lemmas for edge labeling
            
            # Add the central word as the first node
            nodes.append({
                "id": central_word_id,
                "word": central_word,
                "label": central_word,
                "language": central_lang,
                "type": "main",
                "depth": 0,
                "definitions": []
            })
            
            # Add to visited set and maps
            visited.add(central_word_id)
            node_map[central_word_id] = 0  # Index of the node
            id_to_lemma[central_word_id] = central_word
            
            # Get definitions for the main word to include in tooltip
            def_sql = """
            ...
            def_results = session.execute(text(def_sql), {"word_id": central_word_id}).fetchall()
            main_defs = [row.definition_text for row in def_results]
            nodes[0]["definitions"] = main_defs
            
            # Define recursive processing function
            def process_relations(current_id, current_depth):
                if current_depth >= depth:
                    return
                
                # Skip if we've already processed this node at this depth or earlier
                if current_id in visited:
                    return
                    
                # Process outgoing relations (word -> related words)
                outgoing_sql = """
                SELECT r.id, r.from_word_id, r.to_word_id, r.relation_type,
                       w.lemma as target_lemma, w.language_code as target_lang,
                ...
                outgoing = session.execute(text(outgoing_sql), 
                                          {"word_id": current_id}).fetchall()
                
                for rel in outgoing:
                    # Skip self-relations
                    if rel.to_word_id == central_word_id:
                        continue
                        
                    # Skip etymology relations if not requested
                    if not include_etymology and rel.relation_type in ['etymology', 'cognate']:
                        continue
                        
                    # Add target word as node if not already visited
                    if rel.to_word_id not in visited:
                        ...
                        visited.add(rel.to_word_id)
                        node_map[rel.to_word_id] = node_index
                        id_to_lemma[rel.to_word_id] = rel.target_lemma
                        
                        # Add definitions for this node
                        node_defs = session.execute(
                            text("SELECT definition_text FROM definitions WHERE word_id = :word_id LIMIT 3"),
                            {"word_id": rel.to_word_id}
                        ).fetchall()
                        
                        nodes[node_index]["definitions"] = [d.definition_text for d in node_defs]
                        
                        # Recursively process this node
                        process_relations(rel.to_word_id, current_depth + 1)
                    
                    # Add link
                    from_index = node_map[current_id]
                    to_index = node_map[rel.to_word_id]
                ...
                # Process incoming relations (related words -> word)
                incoming_sql = """
                SELECT r.id, r.from_word_id, r.to_word_id, r.relation_type,
                       w.lemma as source_lemma, w.language_code as source_lang,
                       w.has_baybayin, w.baybayin_form
                FROM relations r
                ...
                incoming = session.execute(text(incoming_sql), 
                                          {"word_id": current_id}).fetchall()
                
                for rel in incoming:
                    # Skip self-relations
                    if rel.from_word_id == central_word_id:
                        continue
                    ...
                    # Add source word as node if not already visited
                    if rel.from_word_id not in visited:
                        node_index = len(nodes)
                        nodes.append({
                            "id": rel.from_word_id,
                        ...
                        visited.add(rel.from_word_id)
                        node_map[rel.from_word_id] = node_index
                        id_to_lemma[rel.from_word_id] = rel.source_lemma
                        
                        # Add definitions for this node
                        node_defs = session.execute(
                            text("SELECT definition_text FROM definitions WHERE word_id = :word_id LIMIT 3"),
                            {"word_id": rel.from_word_id}
                        ).fetchall()
                        
                        nodes[node_index]["definitions"] = [d.definition_text for d in node_defs]
                        
                        # Recursively process this node
                        process_relations(rel.from_word_id, current_depth + 1)
                    
                    # Add link
                    from_index = node_map[rel.from_word_id]
                    to_index = node_map[current_id]
                ...
                if include_affixes:
                    # Root affixations
                    root_affix_sql = """
                    SELECT a.id, a.root_word_id, a.affixed_word_id, a.affix_type,
                           w.lemma as affixed_lemma, w.language_code as affixed_lang,
                           w.has_baybayin, w.baybayin_form
                    FROM affixations a
                    ...
                    root_aff = session.execute(text(root_affix_sql), 
                                              {"word_id": current_id}).fetchall()
                    
                    for aff in root_aff:
                        # Skip self-relations
                        if aff.affixed_word_id == central_word_id:
                            continue
                            
                        # Add affixed word as node if not already visited
                        if aff.affixed_word_id not in visited:
                            node_index = len(nodes)
                            nodes.append({
                                "id": aff.affixed_word_id,
                                "word": aff.affixed_lemma,
                                "label": aff.affixed_lemma,
                                "language": aff.affixed_lang,
                                "type": "derived",
                                "depth": current_depth + 1,
                                "definitions": [],
                            ...
                            visited.add(aff.affixed_word_id)
                            node_map[aff.affixed_word_id] = node_index
                            id_to_lemma[aff.affixed_word_id] = aff.affixed_lemma
                            
                            # Add definitions for this node
                            node_defs = session.execute(
                                text("SELECT definition_text FROM definitions WHERE word_id = :word_id LIMIT 3"),
                                {"word_id": aff.affixed_word_id}
                            ).fetchall()
                            
                            nodes[node_index]["definitions"] = [d.definition_text for d in node_defs]
                            
                            # Recursively process this node
                            process_relations(aff.affixed_word_id, current_depth + 1)
                        
                        # Add link
                        from_index = node_map[current_id]
                        to_index = node_map[aff.affixed_word_id]
                    ...
                    affixed_sql = """
                    SELECT a.id, a.root_word_id, a.affixed_word_id, a.affix_type,
                           w.lemma as root_lemma, w.language_code as root_lang,
                           w.has_baybayin, w.baybayin_form
                    FROM affixations a
                    ...
                    affixed = session.execute(text(affixed_sql), 
                                             {"word_id": current_id}).fetchall()
                    
                    for aff in affixed:
                        # Skip self-relations
                        if aff.root_word_id == central_word_id:
                            continue
                            
                        # Add root word as node if not already visited
                        if aff.root_word_id not in visited:
                            node_index = len(nodes)
                            nodes.append({
                                "id": aff.root_word_id,
                                "word": aff.root_lemma,
                                "label": aff.root_lemma,
                                "language": aff.root_lang,
                                "type": "root",
                                "depth": current_depth + 1,
                                "definitions": [],
                                "has_baybayin": aff.has_baybayin,
                            ...
                            visited.add(aff.root_word_id)
                            node_map[aff.root_word_id] = node_index
                            id_to_lemma[aff.root_word_id] = aff.root_lemma
                            
                            # Add definitions for this node
                            node_defs = session.execute(
                                text("SELECT definition_text FROM definitions WHERE word_id = :word_id LIMIT 3"),
                                {"word_id": aff.root_word_id}
                            ).fetchall()
                            
                            nodes[node_index]["definitions"] = [d.definition_text for d in node_defs]
                            
                            # Recursively process this node
                            process_relations(aff.root_word_id, current_depth + 1)
                        
                        # Add link
                        from_index = node_map[aff.root_word_id]
                        to_index = node_map[current_id]
            ...
            # Start processing from the central word at depth 0
            process_relations(central_word_id, 0)
            ...
            result = {
                ...
                "links": links,
                "metadata": {
                    "root_word": central_word,
                    "normalized_lemma": word_result.normalized_lemma,
                    "language_code": central_lang,
                    "depth": depth,
        ...
        logger.error(f"Error in get_semantic_network for word '{word}': {str(e)}", exc_info=True)
        API_ERRORS.labels(endpoint="get_semantic_network", error_type=type(e).__name__).inc()
        return jsonify({
            "error": f"An unexpected error occurred: {type(e).__name__}",