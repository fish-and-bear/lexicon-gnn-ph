...
def create_argument_parser_cli() -> argparse.ArgumentParser:
    ...
    # Add 'load' command
    load_parser = subparsers.add_parser("load", help="Load data from source files into the database")
...
# -------------------------------------------------------------------
def load_data(db_conn, data_dir: str = 'data') -> None:
    """
    Load dictionary data from various sources into the database.
    ...
    with db_conn, db_conn.cursor() as cur:
        # Process KWF Dictionary data
        kwf_path = os.path.join(data_dir, 'kwf_dictionary.json')
        if os.path.exists(kwf_path):
            try:
                logger.info(f"Loading KWF dictionary from {kwf_path}")
                
                with open(kwf_path, 'r', encoding='utf-8') as f:
                    entries = json.load(f)
                ...
                for i, entry in enumerate(entries):
                    if i % 100 == 0:
                        logger.info(f"Processing KWF entry {i+1}/{total_entries}")
                    
                    try:
                        process_entry(cur, entry, entry_type='kwf', language_code='tl')
                    except Exception as e:
                        logger.error(f"Error processing KWF entry {i+1}: {str(e)}")
                
                logger.info(f"Completed processing KWF dictionary")
            except Exception as e:
                logger.error(f"Error processing KWF dictionary: {str(e)}")
        
        # Process tagalog_words.json data
        tagalog_words_path = os.path.join(data_dir, 'tagalog_words.json')
        if os.path.exists(tagalog_words_path):
            try:
                logger.info(f"Loading tagalog_words from {tagalog_words_path}")
                
                with open(tagalog_words_path, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                
                if isinstance(data, dict):
                    # Format 1: Dictionary with word keys
                    total_entries = len(data)
                    logger.info(f"Found {total_entries} entries in tagalog_words dictionary")
                    
                    for i, (word, entry_data) in enumerate(data.items()):
                        if i % 100 == 0:
                            logger.info(f"Processing tagalog_words entry {i+1}/{total_entries}")
                        
                        try:
                            # Add the word field to match our expected format
                            entry = entry_data.copy()
                            entry['word'] = word
                            
                            process_entry(cur, entry, entry_type='tagalog_words', language_code='tl')
                        except Exception as e:
                            logger.error(f"Error processing tagalog_words entry {word}: {str(e)}")
                else:
                    # Format 2: List of word entries
                    total_entries = len(data)
                    logger.info(f"Found {total_entries} entries in tagalog_words list")
                    ...
                    for i, entry in enumerate(data):
                        ...
                        try:
                            ...
                            logger.error(f"Error processing tagalog_words entry {i+1}: {str(e)}")
                
                logger.info(f"Completed processing tagalog_words")
            except Exception as e:
                logger.error(f"Error processing tagalog_words: {str(e)}")
        
        # Process Kaikki data
        kaikki_path = os.path.join(data_dir, 'kaikki.jsonl')
        if os.path.exists(kaikki_path):
            try:
                logger.info(f"Loading Kaikki data from {kaikki_path}")
                process_kaikki_jsonl(cur, kaikki_path)
                logger.info(f"Completed processing Kaikki data")
            except Exception as e:
                logger.error(f"Error processing Kaikki data: {str(e)}")
            
        # Apply Baybayin processing for all words
        try:
            logger.info("Processing Baybayin forms for all words without existing Baybayin data")
            process_missing_baybayin(cur)
            logger.info("Completed Baybayin processing")
        except Exception as e:
            logger.error(f"Error during Baybayin processing: {str(e)}")

        # Build search index after all data is loaded
        logger.info("Building search index")
        build_search_index(cur)
        logger.info("Search index built successfully")

def load_data_cli(args):
    """CLI wrapper for the load_data function"""
    try:
        from database import get_db_connection
        conn = get_db_connection()
        logger.info(f"Connected to database for data loading")
        
        load_data(conn, args.data_dir)
        
        logger.info(f"Data loading process completed")
    except Exception as e:
...
# Main entry point
# -------------------------------------------------------------------
def create_argument_parser_cli():
    parser = argparse.ArgumentParser(description="Fil-Dictionary Database Management Tool")
