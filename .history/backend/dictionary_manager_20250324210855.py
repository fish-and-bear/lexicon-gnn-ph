...
def process_root_words_cleaned(cur, filename: str):
    ...
    two relations are inserted: one from the variant to the base (indicating the variant 
    is derived) and one from the base to the variant (indicating the base is the source).
    """
    import re
    import json
    ...
    with open(filename, 'r', encoding='utf-8') as f:
        ...
        for base_word, variants in data.items():
            ...
            # Create or retrieve the base word's ID.
            ...
            for variant, details in variants.items():
                ...
                # Retrieve and clean the definition text.
                ...
                # Remove three or more consecutive dots (ellipsis) and trim whitespace.
                definition_text = re.sub(r'\.{3,}', '', definition_text).strip()
                pos = details.get("type", "")

                if variant == base_word:
                    # This is the base word's own definition.
                    insert_definition(
                        cur,
                        base_word_id,
                        definition_text,
                    ...
                    try:
                        # Create or retrieve the variant's word entry.
                        variant_id = get_or_create_word_id(cur, variant, language_code='tl')
                        # Insert the variant's definition.
                        insert_definition(
                            cur,
                            variant_id,
                            definition_text,
                        ...
                        # Insert a relation indicating that the variant is derived from the base.
                        insert_relation(
                            cur,
                            variant_id,
                            base_word_id,
                        ...
                        # Also insert the inverse relation indicating that the base is the source of the variant.
                        insert_relation(
                            cur,
                            base_word_id,
                            variant_id,
