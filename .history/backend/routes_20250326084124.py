...
def search_words(**params):
    ...
    try:
        ...
        if mode == 'exact':
            ...
            # For phonetic search, we'll use a simple LIKE query instead of metaphone
            # This is less accurate but doesn't require PostgreSQL extensions
        ...
        if sort == 'alphabetical':
            ...
            # For relevance sorting without PostgreSQL functions:
            # 1. Exact matches get highest priority
            # 2. Then starts-with matches
            # 3. Then contains matches
            if order == 'asc':
                # For ascending order (less relevant first):
                # - Exact matches last (NOT equal comes first)
                # - Starts-with matches next (NOT like comes first)
                # - Contains matches first
                ...
                # For descending order (most relevant first):
                # - Exact matches first (equal comes first)
                # - Starts-with matches next (like comes first)
        ...
        search_results = []
        for word in words:
            # Calculate a simple relevance score without PostgreSQL functions
            relevance = 0.0
            ...
            if include_definitions and word.definitions:
                word_definitions = [d.definition_text for d in word.definitions[:3]]  # Limit to 3 definitions
            ...
            relation_count = (
                len(word.relations_from) if word.relations_from else 0
            ) + (
                len(word.relations_to) if word.relations_to else 0
            )
            ...
            result = {
                ...
                "has_relations": bool(word.relations_from or word.relations_to),
                "relevance": relevance,
