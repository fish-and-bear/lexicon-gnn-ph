...
class Word(db.Model):
    ...
    def to_dict(self, include_definitions: bool = True, include_etymology: bool = True,
        ...
        if include_metadata:
            result.update({
                "word_metadata": json.loads(self.word_metadata) if isinstance(self.word_metadata, str) else self.word_metadata or {},
                "source_info": json.loads(self.source_info) if isinstance(self.source_info, str) else self.source_info or {},
                "idioms": json.loads(self.idioms) if isinstance(self.idioms, str) else self.idioms or [],
                "pronunciation_data": json.loads(self.pronunciation_data) if isinstance(self.pronunciation_data, str) else self.pronunciation_data or {}
            })
        ...
        if include_etymology:
            result["etymologies"] = [e.to_dict() for e in self.etymologies] if self.etymologies else []
...
class PartOfSpeech(db.Model):
    ...
    def to_dict(self, include_metadata: bool = False) -> Dict[str, Any]:
        ...
        if include_metadata:
            result.update({
                "metadata": json.loads(self.meta_info) if isinstance(self.meta_info, str) else self.meta_info or {},
                "created_at": self.created_at.isoformat() if self.created_at else None,
                "updated_at": self.updated_at.isoformat() if self.updated_at else None
            })
    ...
    def __repr__(self):
        return f"<PartOfSpeech {self.code}>"
...
def init_app(app):
    ...
    with app.app_context():
        ...
        if is_testing_db(db.engine):
            # SQLite setup code...
            pass
        else:
            # PostgreSQL setup code...
            try:
                ...
                db.session.rollback()
                print(f"Warning: Failed to create PostgreSQL extensions and indexes: {e}")
            ...
            try:
                ...
                db.session.rollback()
                print(f"Warning: Failed to create additional indexes: {e}")