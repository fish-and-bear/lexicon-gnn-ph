...
@validate_query_params(AffixesQuerySchema)
def get_affixes(**params):
    """
    Get all available affix types and their usage statistics.
    ...
    Returns:
        List of affix types with usage statistics
    """
    try:
        # Extract validated parameters
        language = params.get("language", "tl")
        affix_type = params.get("type")
        ...
        sql = text("""
            SELECT 
                ...
                (
                    SELECT json_agg(lemma)
                    FROM (
                        SELECT DISTINCT replace(w.lemma, '\\u0027', '''') as lemma
                        FROM affixations a2
                ...
                (
                    SELECT json_agg(lemma)
                    FROM (
                        SELECT DISTINCT replace(w2.lemma, '\\u0027', '''') as lemma
                        FROM affixations a3
        ...
        for row in results:
            ...
            # Properly decode sample words to ensure proper character rendering
...
@validate_query_params(RelationsQuerySchema)
