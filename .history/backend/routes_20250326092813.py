...
def get_relations():
    ...
    try:
        ...
        # Build query for relation statistics with proper casting to JSONB
        sql = text("""
            WITH relation_stats AS (
                SELECT 
                    ...
                    json_agg(DISTINCT w.lemma)::jsonb FILTER (WHERE w.id = r.from_word_id) as sample_from,
                    json_agg(DISTINCT w2.lemma)::jsonb FILTER (WHERE w2.id = r.to_word_id) as sample_to
                FROM relations r
            ...
            SELECT 
                ...
                to_words_count,
                (SELECT json_agg(sample) FROM (
                    SELECT DISTINCT element as sample
                    FROM relation_stats s2,
                    jsonb_array_elements_text(COALESCE(s2.sample_from, '[]'::jsonb)) AS element
                ...
                (SELECT json_agg(sample) FROM (
                    SELECT DISTINCT element as sample
                    FROM relation_stats s2,
                    jsonb_array_elements_text(COALESCE(s2.sample_to, '[]'::jsonb)) AS element
                    WHERE s.relation_type = s2.relation_type
                    LIMIT 5
                ) t) as sample_to
