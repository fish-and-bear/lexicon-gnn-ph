...
def process_entry(cur, entry: Dict, source_identifier: str):
    """
    Process a single entry, extract data, and insert into tables.
    """
    if 'word' not in entry:
        return None # Skip entries without a word

    word = entry['word']
    language_code = entry.get('lang_code', 'tl') # Assume Tagalog if not specified
    pos = entry.get('pos', 'uncategorized') # Get part of speech
    
    # Logging the entry being processed for traceability
    logger.debug(f"Processing entry for word: {word}, Language: {language_code}, POS: {pos}")

    try:
        # Extract Baybayin info using the specialized local helper
        baybayin_form, romanized_form = extract_baybayin_info(entry)
        has_baybayin = baybayin_form is not None
        
        # Clean Baybayin form if present
        if baybayin_form:
            baybayin_form = clean_baybayin_text(baybayin_form)
            
        # Validate Baybayin data
        if has_baybayin:
            if not validate_baybayin_entry(baybayin_form, romanized_form):
                logger.warning(f"Invalid Baybayin data for '{word}'. Form: '{baybayin_form}', Romanized: '{romanized_form}'")
                # Option: Decide whether to skip or insert without Baybayin
                # For now, we'll proceed but log the warning. Revisit if needed.
                # has_baybayin = False # Example: Disable Baybayin if invalid
                # baybayin_form = None
                # romanized_form = None

        # Get or create the word ID, passing Baybayin info
        word_id = get_or_create_word_id(
            cur,
            word,
            language_code=language_code,
            source_identifier=source_identifier, # Pass source here
            has_baybayin=has_baybayin,
            baybayin_form=baybayin_form,
            romanized_form=romanized_form,
            preferred_spelling=entry.get('form'), # Use 'form' if available
            tags=','.join(entry.get('tags', [])) if entry.get('tags') else None
        )

        if not word_id:
            logger.error(f"Failed to get or create word ID for '{word}' from source '{source_identifier}'")
        ...
        if 'senses' in entry and isinstance(entry['senses'], list):
            for sense_index, sense in enumerate(entry['senses']):
                if not sense or not isinstance(sense, dict):
                    continue # Skip invalid senses

                glosses = sense.get('glosses', [])
                if not glosses or not isinstance(glosses, list):
                    continue # Skip senses without glosses

                # Combine all glosses into one definition text
                definition_text = '; '.join(filter(None, glosses))
                if not definition_text:
                    continue

                # Truncate if definition text is excessively long
                max_def_length = 2048
                if len(definition_text) > max_def_length:
                    definition_text = definition_text[:max_def_length] + "..."
                    logger.warning(f"Truncated long definition for word ID {word_id}, sense {sense_index+1}")
                
                # Extract examples associated with this sense
                sense_examples = []
                if 'examples' in sense and isinstance(sense['examples'], list):
                    for ex_data in sense['examples']:
                        if isinstance(ex_data, str):
                            sense_examples.append(ex_data.strip())
                        elif isinstance(ex_data, dict):
                            example_text = ex_data.get('text', '').strip()
                            translation = ex_data.get('translation', ex_data.get('english', '')).strip()
                            if example_text:
                                full_example = example_text
                                if translation:
                                    full_example += f" – {translation}"
                                sense_examples.append(full_example)
                                
                examples_str = '\n'.join(sense_examples) if sense_examples else None
                
                # Extract usage notes
                raw_tags = sense.get('raw_tags', [])
                tags_str = ','.join(filter(None, raw_tags)) if raw_tags else None

                # Use the global standardizer for POS
                standard_pos = standardize_pos(pos) # Use global function
                ...
                definition_id = insert_definition(
                    ...
                    usage_notes=sense.get('notes'), # Assuming notes field exists
                    tags=tags_str
                )
                
                if definition_id:
                     # Process categories associated with the definition
                    if 'categories' in sense and isinstance(sense['categories'], list):
                       process_categories(cur, definition_id, sense['categories'])

                     # Process links associated with the definition
                    if 'links' in sense and isinstance(sense['links'], list):
                        process_links(cur, definition_id, sense['links'])
                        
                    # Process relationships specific to this sense
                    process_sense_relationships(cur, word_id, sense, source_identifier) # Pass source here
                else:
                    logger.warning(f"Failed to insert definition for word ID {word_id}, sense index {sense_index}")


        # Process form-based relationships (applies to the word entry itself)
        process_form_relationships(cur, word_id, entry, language_code, source_identifier) # Pass source here

        # Process etymology information
        etymology_text = entry.get('etymology_text')
        etymology_templates = entry.get('etymology_templates')
        if etymology_text:
            process_etymology(cur, word_id, etymology_text, etymology_templates, source_identifier) # Pass source here

        # Process overall pronunciations (not tied to specific senses)
        process_pronunciation(cur, word_id, entry) # Pronunciation table might not need source
        
         # Process head templates (might relate to inflection or forms)
        ...
        # Process credits/sources of the entry itself
        credits_list = entry.get('sources', []) # Assuming 'sources' holds credit info in Kaikki
        if credits_list and isinstance(credits_list, list):
             # Combine multiple sources/credits into a single string if needed
             credits_text = "; ".join(c.get('name', '') for c in credits_list if isinstance(c, dict) and 'name' in c)
             if credits_text:
                insert_credit(cur, word_id, credits_text, source_identifier=source_identifier) # Pass source here

        return word_id # Return the word_id on success

    except Exception as e:
        logger.error(f"Error processing entry details for word '{word}' (ID: {word_id if 'word_id' in locals() else 'N/A'}) from source '{source_identifier}': {str(e)}")
        # Optionally log traceback for detailed debugging
        # logger.exception(f"Traceback for error processing entry '{word}':")
        return None # Return None on failure

# --------------------------------------------------------------------------
# Main processing loop for Kaikki JSONL file
# --------------------------------------------------------------------------

@with_transaction(commit=False)
def process_kaikki_jsonl(cur, filename: str, source_identifier: str):
    """
    Process a Kaikki JSONL file and insert data into the database.
    """
    # Helper function to extract Baybayin info from Kaikki entry
    def extract_baybayin_info(entry: Dict) -> Tuple[Optional[str], Optional[str]]:
        """
        Extract Baybayin form and romanized form from Kaikki entry.
        """
        baybayin_form = None
        romanized_form = None
        ...
        if 'forms' in entry and isinstance(entry['forms'], list):
            for form in entry['forms']:
                if isinstance(form, dict) and 'tags' in form and 'spelling' in form['tags']:
                    baybayin_form = form['form'].strip()
                    romanized_form = form['tags']['spelling'].strip()
                    break
    ...
    logger.info(f"Processing Kaikki dictionary file: {filename}. Standardized source: '{source_identifier}'")
    
    # Initialize stats more comprehensively
    stats = {
    ...
    conn = cur.connection # Get underlying connection for manual commit/rollback
    
    # Progress bar setup
    # Use tqdm for progress bar if available, otherwise simple counter
    try:
        from tqdm import tqdm
        progress_iterator = tqdm(open(filename, 'r', encoding='utf-8'), total=total_lines, desc=f"Processing {os.path.basename(filename)}", unit=" lines")
    except ImportError:
        logger.info("tqdm not found, using simple counter for progress.")
        progress_iterator = enumerate(open(filename, 'r', encoding='utf-8'), 1)

    processed_count_since_log = 0
    
    for line_num, line in progress_iterator:
        if isinstance(progress_iterator, tqdm): # If using tqdm, line_num is implicitly handled
             pass 
        else: # If using enumerate, line_num needs manual increment if we skip lines
             pass # line_num starts at 1 from enumerate

        # Define a unique savepoint name for this line/entry
        savepoint_name = f"kaikki_entry_{line_num}"

        try:
            # Start a savepoint for this entry
            ...
            if 'word' not in entry or not entry['word']:
                ...
                cur.execute(f"RELEASE SAVEPOINT {savepoint_name}") # Release savepoint for skipped entry
                continue

            # Process the entry using the nested helper, passing the source identifier
            result_word_id = process_entry(cur, entry, source_identifier=source_identifier) 

            if result_word_id:
                stats["processed_ok"] += 1
                processed_count_since_log += 1
                # Release the savepoint after successful processing of the entry
                cur.execute(f"RELEASE SAVEPOINT {savepoint_name}")
            else:
                # An error occurred within process_entry, logged there. Rollback this entry.
                stats["processing_errors"] += 1
                cur.execute(f"ROLLBACK TO SAVEPOINT {savepoint_name}")

            # Commit periodically to avoid holding locks for too long and manage memory
            # Commit every 100 successfully processed entries
            commit_interval = 100 
            if stats["processed_ok"] > 0 and stats["processed_ok"] % commit_interval == 0:
                try:
                    conn.commit()
                    logger.debug(f"Committed transaction after {stats['processed_ok']} processed entries.")
                    # Reset counter after logging batch progress
                    processed_count_since_log = 0 
                except Exception as commit_error:
                    logger.error(f"Critical Error committing transaction after entry {line_num}: {str(commit_error)}. Rolling back.")
                    try:
                         conn.rollback() # Rollback the whole transaction since last commit
                    except Exception as rb_error:
                         logger.critical(f"Failed to rollback after commit error: {rb_error}. Database state might be inconsistent.")
                         # Consider exiting or more drastic error handling here
                    stats["commit_errors"] += 1
                    # Adjust stats: the last batch didn't actually commit successfully
                    stats["processed_ok"] -= processed_count_since_log 
                    stats["processing_errors"] += processed_count_since_log # Assume the whole batch failed
                    processed_count_since_log = 0

        except json.JSONDecodeError as json_err:
            logger.warning(f"Invalid JSON on line {line_num}: {json_err}")
            stats["json_decode_errors"] += 1
            # No transaction started for this line, so just continue
            # No need to rollback to savepoint as it wasn't successfully created or the error is before it matters
        except Exception as e:
            ...
            try:
                ...
                # If savepoint rollback fails, it might indicate a connection issue
                # Log critically, but the outer transaction might still be rolled back later
                logger.error(f"Failed to rollback to savepoint {savepoint_name} for line {line_num}: {rb_err}. Continuing...")

    # Final commit for any remaining processed entries after the loop
    try:
        conn.commit()
        logger.info("Committed final transaction.")
    except Exception as commit_error:
        logger.error(f"Error committing final transaction: {str(commit_error)}. Rolling back.")
        try:
            conn.rollback()
        except Exception as rb_error:
            logger.critical(f"Failed to rollback final transaction: {rb_error}.")
        # Adjust final stats if commit failed
        stats["commit_errors"] += 1
        stats["processed_ok"] -= processed_count_since_log
        stats["processing_errors"] += processed_count_since_log


    # Close progress bar if it's tqdm
    if isinstance(progress_iterator, tqdm):
        progress_iterator.close()

    logger.info(f"Completed processing {filename}. Final Stats: {stats}")
    
    # Return detailed stats
    return stats

# --------------------------------------------------------------------------
# Helper functions for data standardization (now potentially global)
# --------------------------------------------------------------------------
...
def process_kwf_dictionary(cur, filename: str):
    ...
    # Ensure we have a valid source identifier
    if not source_identifier:
    ...
    try:
        ...
        # Extract dictionary metadata if available
        dictionary_metadata = {}
        if isinstance(data, dict):
            # Try to extract dictionary metadata
            for meta_key in ['version', 'publisher', 'date', 'description', 'copyright']:
                if meta_key in data:
                    dictionary_metadata[meta_key] = data[meta_key]
            ...
            for entry_index, entry in enumerate(entries):
                ...
                try:
                    ...
                    if isinstance(entry, dict):
                        # Include dictionary source
                        word_metadata["source"] = source_identifier
                        
                        # Add domain/register information if available
                        if "register" in entry:
                            word_metadata["register"] = entry["register"]
                        if "domain" in entry:
                            word_metadata["domain"] = entry["domain"]
                            
                        # Add any other useful metadata
                        for meta_key in ["notes", "usage", "origin", "dialect"]:
                            if meta_key in entry:
                                word_metadata[meta_key] = entry[meta_key]
                    
                    # Convert metadata to JSON
                    word_metadata_json = json.dumps(word_metadata) if word_metadata else None
                    ...
                    if "etymology" in entry and entry["etymology"]:
                        ...
                        if isinstance(etymology_text, str) and etymology_text.strip():
                            try:
                                ...
                                # Extract language codes from etymology if possible
                                language_codes = None
                                if "from" in etymology_text.lower():
                                    # Extract languages mentioned in etymology
                                    language_patterns = [
                                        r'from\s+(.*?)\s+', 
                                        r'borrowed\s+from\s+(.*?)\s+',
                            ...
                            if "definitions" in pos_entry and isinstance(pos_entry["definitions"], list):
                                for def_item in pos_entry["definitions"]:
                                    if isinstance(def_item, str):
                                        ...
                                        # Extract examples and structure as objects
                                        examples_json = None
                                        
                                        # Handle examples that could be in different formats
                                        examples = def_item.get("examples", [])
                                        if examples:
                                            # Normalize examples to structured objects
                                            normalized_examples = []
                                            ...
                                            if isinstance(examples, list):
                                                ...
                                                for ex_idx, (ex_key, ex_val) in enumerate(examples.items()):
                                                    if isinstance(ex_val, str):
                                                        ...
                                                        # Add translation if available
                                                        if "translation" in ex_val:
                                                            ex_obj["translation"] = ex_val["translation"]
                                                        normalized_examples.append(ex_obj)
                                        ...
                                        if not usage_notes and "usage" in def_item:
                                            usage_notes = def_item["usage"]
                                        
                                        # Add domain/register if available
                                        if "domain" in def_item:
                                            def_metadata["domain"] = def_item["domain"]
                                        if "register" in def_item:
                                            def_metadata["register"] = def_item["register"]
                                        ...
                                        try:
                                                ...
                                                if isinstance(synonyms, list):
                                                    for syn_idx, syn in enumerate(synonyms):
                                                        ...
                                                        if isinstance(syn_word, str) and syn_word.strip():
                                                            try:
                                                                # Create synonym with same source
                                            ...
                                            if "antonyms" in def_item and definition_id:
                                                ...
                                                if isinstance(antonyms, list):
                                                    for ant_idx, ant in enumerate(antonyms):
                                                        ant_word = ant
                                                        ...
                                                        if isinstance(ant_word, str) and ant_word.strip():
                                                            try:
                                                                # Create antonym with same source
                                                                ...
                                                                ant_metadata = {
                                                                    "source": source_identifier,
                                                                    "confidence": 90,  # High confidence for explicit antonyms
                                                                    "definition_id": definition_id,
                                                                    "index": ant_idx
                                                                }
                                            ...
                                            if "see_also" in def_item and definition_id:
                                                ...
                                                if isinstance(see_also_refs, list):
                                                    for ref_idx, ref in enumerate(see_also_refs):
                                                        ref_word = ref
                                                        ...
                                                        if isinstance(ref_word, str) and ref_word.strip():
                                                            try:
                                                                # Create reference with same source
                                                                ...
                                                                ref_metadata = {
                                                                    "source": source_identifier,
                                                                    "confidence": 80,  # Good confidence for cross-references
                                                                    "definition_id": definition_id,
                                                                    "index": ref_idx
                                                                }
                    ...
                    # Process related terms with enhanced metadata
                        ...
                        for rel_type, rel_items in entry['related'].items():
                            ...
                            for rel_idx, rel_item in enumerate(rel_items):
                                ...
                                if isinstance(rel_item, dict) and 'value' in rel_item:
                                    ...
                                    for meta_key in rel_item:
                                        if meta_key != 'value':
                                            rel_metadata[meta_key] = rel_item[meta_key]
                                elif isinstance(rel_item, str):
                                    related_term = rel_item
                                    
                                if related_term and len(related_term) <= 255:
                                    try:
                                        # Create related term with consistent source tracking
                                        ...
                                        rel_metadata.update({
                                            ...
                                            "index": rel_idx,
                                            "confidence": 85  # Good confidence for KWF relations
                                        })
                                        
                                        # Insert the relation with metadata
                                        insert_relation(
                                            cur, 
                                            word_id, 
                                            related_id, 
...
@with_transaction(commit=True) # Changed back to commit=True for the whole function
def process_tagalog_words(cur, filename: str):
    ...
    stats = {
        ...
        "definitions_added": 0,
        "relations_added": 0,
        "synonyms_added": 0, # Track specific relation types
        "references_added": 0,
        "variants_added": 0,
        "derivatives_added": 0,
        ...
        "affixations_added": 0 # Track affixation relationships
    }

    conn = cur.connection # Get connection for potential manual rollback if savepoint fails
    ...
    try:
        ...
        with tqdm(total=stats["total_entries"], desc=f"Processing {source_identifier}", unit="word") as pbar:
            for lemma, entry_data in data.items():
                # --- Savepoint ---
                # FIX: Use abs() for hash to ensure valid SQL identifier
                ...
                try:
                    ...
                    if 'part_of_speech' in entry_data and entry_data['part_of_speech']:
                        word_metadata["part_of_speech_raw"] = entry_data['part_of_speech'] # Store raw POS
                        # Consider adding POS to tags? tags.extend(entry_data['part_of_speech'])
                    if 'pronunciation' in entry_data and entry_data['pronunciation']:
                        ...
                        if 'alternate_pronunciation' in entry_data:
                    ...
                    word_id = get_or_create_word_id(
                        ...
                        source_identifier=source_identifier, # Will update source_info JSON
                        word_metadata=json.dumps(word_metadata), # Pass constructed metadata
                        tags=", ".join(tags) if tags else None # Pass top-level tags
                    )
                    if not word_id: raise ValueError(f"Failed to get/create word ID for '{lemma}'")

                    # --- Process Pronunciations ---
                    if 'pronunciation' in entry_data and entry_data['pronunciation']:
                        pron_obj = {"value": entry_data['pronunciation'], "type": "ipa"}
                    ...
                    if 'alternate_pronunciation' in entry_data and entry_data['alternate_pronunciation']:
                        alt_pron_obj = {"value": entry_data['alternate_pronunciation'], "type": "ipa", "metadata": {"is_alternate": True}}
                    ...
                    if 'derivative' in entry_data and entry_data['derivative']:
                        ...
                        for form_idx, form in enumerate(derivative_forms):
                            form = form.strip()
                            if form:
                                rel_type = RelationshipType.DERIVED_FROM # Base type
                                form_word = form
                                # Check if type info is included (e.g., "verb salamat") - simple split
                                if " " in form:
                                    parts = form.split(' ', 1)
                                    # Basic check if first part looks like a POS tag - refine if needed
                                    if len(parts[0]) < 5 and parts[0] in POS_MAPPING:
                                        # Potentially store this type info in relation metadata?
                                        form_word = parts[1]
                                        # rel_type = RelationshipType.from_string(parts[0]) # Or store type in metadata
                                try:
                                    derivative_id = get_or_create_word_id(cur, form_word, language_code, source_identifier=source_identifier)
                                    if derivative_id and derivative_id != word_id:
                                        rel_metadata = {"source": source_identifier, "raw_text": form, "index": form_idx, "confidence": 85}
                                        rel_id = insert_relation(cur, derivative_id, word_id, rel_type, source_identifier=source_identifier, metadata=rel_metadata)
                                        if rel_id:
                                            stats["derivatives_added"] += 1
                                            stats["relations_added"] += 1
                                except Exception as der_e:
                                    logger.error(f"Error processing derivative '{form}' for word '{lemma}': {der_e}")
                    ...
                    if 'etymology' in entry_data and entry_data['etymology'] and isinstance(entry_data['etymology'], dict):
                        ...
                        if etymology_text:
                            try:
                                etymology_structure = {'source': source_identifier, **etymology_data}
                                language_codes = ", ".join(etymology_data.get('languages', []))
                                # FIX: Correct argument name: source_identifier
                                ety_id = insert_etymology(
                                    cur, word_id, etymology_text,
                                    etymology_structure=json.dumps(etymology_structure),
                                    language_codes=language_codes,
                                    source_identifier=source_identifier # Corrected
                                )
                    ...
                    if 'senses' in entry_data and entry_data['senses']:
                        for sense_idx, sense in enumerate(entry_data['senses']):
                            ...
                            # Determine POS ID
                            pos_list_sense = sense.get('part_of_speech', [])
                            pos_list_entry = entry_data.get('part_of_speech', [])
                            ...
                            pos_str = ", ".join(p for p in flat_pos if p)
                            standardized_pos_id = get_standardized_pos_id(cur, pos_str)
                            ...
                            if isinstance(example_data, dict):
                                if 'examples' in example_data and example_data['examples']:
                                    examples = [{"text": ex, "source": source_identifier, "index": i} for i, ex in enumerate(example_data['examples'])]
                                elif 'raw' in example_data and example_data['raw']:
                                    examples = [{"text": example_data['raw'], "source": source_identifier, "raw": True}]

                            # Prepare Tags
                            sense_tags = sense.get('tags', [])
                            if 'category' in sense and sense['category']:
                                sense_tags.append(f"category:{sense['category']}")
                            tags_str = ", ".join(sense_tags) if sense_tags else None

                            try:
                                # FIX: Removed incorrect 'metadata' argument
                                definition_id = insert_definition(
                                    cur, word_id, definition_text,
                                    standardized_pos_id=standardized_pos_id, # Pass ID
                                    original_pos=pos_str, # Store original string
                                ...
                                if definition_id:
                                    ...
                                    for rel_key, rel_type_enum in [('synonyms', RelationshipType.SYNONYM),
                                                                    ('variants', RelationshipType.VARIANT),
                                                                    ('references', RelationshipType.SEE_ALSO)]:
                                        if rel_key in sense:
                                            for item_idx, item_word in enumerate(sense[rel_key]):
                                                if isinstance(item_word, str) and item_word:
                                                    try:
                                                        item_id = get_or_create_word_id(cur, item_word, language_code, source_identifier=source_identifier)
                                                        if item_id and item_id != word_id:
                                                            rel_meta = {"source": source_identifier, "definition_id": definition_id, "index": item_idx, "confidence": 80} # Adjust confidence per type?
                                                            rel_ins_id = insert_relation(cur, word_id, item_id, rel_type_enum, source_identifier=source_identifier, metadata=rel_meta)
                                                            if rel_ins_id:
                                                                stats[f"{rel_key}_added"] += 1
                                                                stats["relations_added"] += 1
                                                    except Exception as rel_e:
                                                        logger.error(f"Error processing sense relation '{rel_key}'->'{item_word}' for '{lemma}': {rel_e}")
                            except psycopg2.errors.UniqueViolation:
                                logger.debug(f"Def already exists for '{lemma}', sense: {definition_text[:30]}...")
                            except Exception as def_e:
                                logger.error(f"Error inserting definition for '{lemma}': {def_e}")

                    # --- Process Baybayin (Original Snippet Logic) ---
                    if 'baybayin' in entry_data and entry_data['baybayin']:
                        baybayin_text = entry_data['baybayin']
                        # Clean might be too aggressive, let's validate first
                        if isinstance(baybayin_text, str) and baybayin_text:
                           if validate_baybayin_entry(baybayin_text): # Validate before using
                               clean_baybayin = clean_baybayin_text(baybayin_text) # Clean for storage
                               if clean_baybayin:
                                   try:
                                       ...
                                       if cur.rowcount > 0: stats["baybayin_added"] += 1
                                   except Exception as bb_e:
                                       logger.error(f"Error processing baybayin '{baybayin_text}' for '{lemma}': {bb_e}")
                           else:
                               logger.warning(f"Invalid baybayin structure for '{lemma}': {baybayin_text}")

                    # --- Process Affix Forms (Original Snippet Logic) ---
                    if 'affix_forms' in entry_data and entry_data['affix_forms'] and \
                       'affix_types' in entry_data and entry_data['affix_types']:
                        affix_forms = entry_data['affix_forms']
                        affix_types = entry_data['affix_types']
                        for i, form in enumerate(affix_forms):
                            clean_form = form.strip() if isinstance(form, str) else None
                            if clean_form:
                                affix_type = affix_types[i] if i < len(affix_types) and isinstance(affix_types[i], str) else "unknown"
                                try:
                                    affixed_id = get_or_create_word_id(cur, clean_form, language_code, source_identifier=source_identifier)
                                    if affixed_id and affixed_id != word_id:
                                        aff_id = insert_affixation(
                                            cur, root_id=word_id, affixed_id=affixed_id,
                                            affix_type=affix_type, source_identifier=source_identifier
                                        )
                                        if aff_id: stats["affixations_added"] += 1
                                        # Also create relation? (Redundant with affixation table?)
                                        # rel_metadata = {"source": source_identifier, "affix_type": affix_type, "index": i, "confidence": 90}
                                        # insert_relation(cur, word_id, affixed_id, RelationshipType.ROOT_OF, source_identifier=source_identifier, metadata=rel_metadata)
                                        # stats["relations_added"] += 1 # Count if adding relation too
                                except Exception as aff_e:
                                    logger.error(f"Error processing affix '{clean_form}' (type {affix_type}) for '{lemma}': {aff_e}")
                    ...
                    logger.error(f"Error processing entry '{lemma}': {entry_err}", exc_info=True) # Log traceback
                    stats["error_entries"] += 1
                    try:
                        cur.execute(f"ROLLBACK TO SAVEPOINT {savepoint_name}")
                    except Exception as rollback_error:
                        logger.critical(f"CRITICAL: Failed rollback savepoint {savepoint_name} for '{lemma}': {rollback_error}. Rolling back full transaction.")
                        conn.rollback() # Attempt full rollback
                        logger.warning("Performed full transaction rollback due to savepoint failure.")
                        raise entry_err # Re-raise original error to stop processing
                finally:
                    pbar.update(1)
                    # Commit periodically moved outside loop for single commit at end

    except (IOError, json.JSONDecodeError) as file_err:
        logger.error(f"Error reading or parsing file {filename}: {file_err}")
        # Transaction handled by decorator (will rollback)
        raise # Re-raise to indicate failure

    except Exception as e:
        logger.error(f"Unexpected fatal error processing file {filename}: {e}", exc_info=True)
        # Transaction handled by decorator (will rollback)
        raise # Re-raise critical errors

    finally: # Log stats even if errors occurred mid-processing
        ...
        logger.info(f"  Skipped (invalid format): {stats['skipped_entries']}")
        ...
        logger.info(f"    (Deriv: {stats['derivatives_added']}, Syn: {stats['synonyms_added']}, Var: {stats['variants_added']}, Ref: {stats['references_added']})")
...
# <<< End of process_tagalog_words function >>>
...
@with_transaction(commit=False)
def process_entry(cur, entry: Dict, source_identifier: str) -> Optional[int]:
    """
    Process a single dictionary entry (likely from a simpler format).
    Handles basic word creation, pronunciations, and credits.

    Args:
        cur: Database cursor
        entry: Dictionary containing entry data
        source_identifier: Source identifier for the entry
        check_exists: Whether to check if word already exists before creating (Not fully implemented here)
    ...
    # FIX: Use abs() to ensure savepoint name is valid SQL identifier
    savepoint_name = f"entry_{abs(hash(word))}"
    try:
        cur.execute(f"SAVEPOINT {savepoint_name}")
    except Exception as e:
        logger.error(f"Failed to create savepoint {savepoint_name} for word '{word}': {e}. Skipping entry.")
        # Skip this entry if savepoint creation fails to avoid partial data.
        return None
    ...
    try:
        ...
        # Use get_or_create_word_id for consistency and proper source_info handling
        word_id = get_or_create_word_id(
            cur,
            lemma=word,
            language_code=language_code,
            source_identifier=source_identifier # Pass the standardized source
            # Add other relevant kwargs if available in this entry format
        )
        ...
        if "pronunciations" in entry:
            ...
            if isinstance(prons, list):
                for pron in prons:
                    try:
                        # FIX: Pass mandatory source_identifier argument correctly
                        insert_pronunciation(cur, word_id, pron, source_identifier=source_identifier)
                    except Exception as e:
                        logger.warning(f"Failed to insert pronunciation for '{word}': {str(e)}")
            elif isinstance(prons, (str, dict)):
                try:
                    # FIX: Pass mandatory source_identifier argument correctly
                    insert_pronunciation(cur, word_id, prons, source_identifier=source_identifier)
                except Exception as e:
                    logger.warning(f"Failed to insert pronunciation for '{word}': {str(e)}")
        ...
        if "credits" in entry:
            ...
            if isinstance(credits, list):
                for credit in credits:
                    try:
                        # FIX: Pass mandatory source_identifier argument correctly
                        insert_credit(cur, word_id, credit, source_identifier=source_identifier)
                    except Exception as e:
                        logger.warning(f"Failed to insert credit for '{word}': {str(e)}")
            elif isinstance(credits, (str, dict)):
                try:
                    # FIX: Pass mandatory source_identifier argument correctly
                    insert_credit(cur, word_id, credits, source_identifier=source_identifier)
                except Exception as e:
                    logger.warning(f"Failed to insert credit for '{word}': {str(e)}")
        ...
        try:
            ...
            logger.error(f"Failed to roll back to savepoint {savepoint_name} for entry '{word}': {rb_err}")
            # If savepoint rollback fails, re-raise original error to trigger full transaction rollback
            raise e
...
def process_kaikki_jsonl(cur, filename: str):
    ...
    def process_sense_relationships(cur, word_id: int, sense: Dict, source_identifier: str):
        # Process synonyms
        if 'synonyms' in sense and isinstance(sense['synonyms'], list):
            for synonym in sense['synonyms']:
                if isinstance(synonym, dict) and 'word' in synonym:
                    ...
                    try:
                        # Pass source_identifier to get_or_create for related word
                        syn_id = get_or_create_word_id(cur, syn_word, 'tl', source_identifier=source_identifier)
                        metadata = {'confidence': 90} # Default confidence for Kaikki synonym
                        if 'tags' in synonym and isinstance(synonym['tags'], list):
                            metadata['tags'] = ','.join(synonym['tags'])
                        # Use passed source_identifier for the relation
                        if syn_id and syn_id != word_id: # Avoid self-relation
                             insert_relation(cur, word_id, syn_id, RelationshipType.SYNONYM, source_identifier=source_identifier, metadata=metadata)
                    except Exception as e:
                        logger.error(f"Error processing synonym relation ('{syn_word}') for word ID {word_id}: {str(e)}")
            ...
            for antonym in sense['antonyms']:
                if isinstance(antonym, dict) and 'word' in antonym:
                    try:
                        ...
                        # Pass source_identifier to get_or_create for related word
                        ant_id = get_or_create_word_id(cur, ant_word, 'tl', source_identifier=source_identifier)
                        ...
                        if ant_id and ant_id != word_id:
                            insert_relation(cur, word_id, ant_id, RelationshipType.ANTONYM, source_identifier=source_identifier, metadata=metadata)
                    except Exception as e:
                        logger.error(f"Error processing antonym relation ('{ant_word}') for word ID {word_id}: {str(e)}")
            ...
            for hypernym in sense['hypernyms']:
                if isinstance(hypernym, dict) and 'word' in hypernym:
                    try:
                        ...
                        # Pass source_identifier to get_or_create for related word
                        hyper_id = get_or_create_word_id(cur, hyper_word, 'tl', source_identifier=source_identifier)
                        metadata = {'confidence': 85}
                        if 'tags' in hypernym and isinstance(hypernym['tags'], list):
                            metadata['tags'] = ','.join(hypernym['tags'])
                        # Use passed source_identifier for the relation
                        if hyper_id and hyper_id != word_id:
                            insert_relation(cur, word_id, hyper_id, RelationshipType.HYPERNYM, source_identifier=source_identifier, metadata=metadata)
                    except Exception as e:
                        logger.error(f"Error processing hypernym relation ('{hyper_word}') for word ID {word_id}: {str(e)}")
            ...
            for hyponym in sense['hyponyms']:
                if isinstance(hyponym, dict) and 'word' in hyponym:
                    try:
                        ...
                        # Pass source_identifier to get_or_create for related word
                        hypo_id = get_or_create_word_id(cur, hypo_word, 'tl', source_identifier=source_identifier)
                        metadata = {'confidence': 85}
                        if 'tags' in hyponym and isinstance(hyponym['tags'], list):
                            metadata['tags'] = ','.join(hyponym['tags'])
                        # Use passed source_identifier for the relation
                        if hypo_id and hypo_id != word_id:
                            insert_relation(cur, word_id, hypo_id, RelationshipType.HYPONYM, source_identifier=source_identifier, metadata=metadata)
                    except Exception as e:
                        logger.error(f"Error processing hyponym relation ('{hypo_word}') for word ID {word_id}: {str(e)}")
            ...
            for holonym in sense['holonyms']:
                if isinstance(holonym, dict) and 'word' in holonym:
                    try:
                        ...
                        # Pass source_identifier to get_or_create for related word
                        holo_id = get_or_create_word_id(cur, holo_word, 'tl', source_identifier=source_identifier)
                        metadata = {'confidence': 80}
                        if 'tags' in holonym and isinstance(holonym['tags'], list):
                            metadata['tags'] = ','.join(holonym['tags'])
                        # Use passed source_identifier for the relation
                        if holo_id and holo_id != word_id:
                            insert_relation(cur, word_id, holo_id, RelationshipType.HOLONYM, source_identifier=source_identifier, metadata=metadata)
                    except Exception as e:
                        logger.error(f"Error processing holonym relation ('{holo_word}') for word ID {word_id}: {str(e)}")
            ...
            for meronym in sense['meronyms']:
                if isinstance(meronym, dict) and 'word' in meronym:
                    try:
                        ...
                        # Pass source_identifier to get_or_create for related word
                        mero_id = get_or_create_word_id(cur, mero_word, 'tl', source_identifier=source_identifier)
                        metadata = {'confidence': 80}
                        if 'tags' in meronym and isinstance(meronym['tags'], list):
                            metadata['tags'] = ','.join(meronym['tags'])
                        # Use passed source_identifier for the relation
                        if mero_id and mero_id != word_id:
                             insert_relation(cur, word_id, mero_id, RelationshipType.MERONYM, source_identifier=source_identifier, metadata=metadata)
                    except Exception as e:
                        logger.error(f"Error processing meronym relation ('{mero_word}') for word ID {word_id}: {str(e)}")
    ...
    def process_form_relationships(cur, word_id: int, entry: Dict, language_code: str, source_identifier: str):
        ...
        for form_data in entry['forms']:
            if not isinstance(form_data, dict) or 'form' not in form_data:
                continue

            form_word = form_data['form']
            if not form_word or form_word == entry.get('word'): # Skip empty forms or self-references
                continue

            # Determine relationship type from tags (e.g., 'plural', 'feminine', 'diminutive')
            rel_type = RelationshipType.VARIANT # Default to generic variant
            tags = form_data.get('tags', [])
            if isinstance(tags, list):
                if 'plural' in tags:
                    rel_type = RelationshipType.VARIANT # Or a more specific type if defined
                elif 'feminine' in tags:
                     rel_type = RelationshipType.VARIANT
                elif 'diminutive' in tags:
                     rel_type = RelationshipType.VARIANT
                # Add more specific form types if needed
                if 'canonical' in tags:
                    # Canonical forms often represent the main entry, skip relation? Or use EQUALS?
                    continue # Skip relating to canonical for now

            try:
                # Get or create the related form word, passing the source
                ...
                if form_word_id and form_word_id != word_id:
                    metadata = {
                        'confidence': 85, # Default confidence for form relations
                        'tags': ','.join(tags) if tags else None
                    }
                    if 'source' in form_data:
                         metadata['form_source_info'] = form_data['source']

                    # Use passed source_identifier for the relation
                    insert_relation(cur, word_id, form_word_id, rel_type, source_identifier=source_identifier, metadata=metadata)

            except Exception as e:
                logger.error(f"Error processing form relationship ('{form_word}') for word ID {word_id}: {str(e)}")

    # Process etymology
    def process_etymology(cur, word_id: int, etymology_text: str, etymology_templates: List[Dict] = None, source_identifier: str):
        if not etymology_text:
            return

        # Basic normalization/cleaning of etymology text (optional)
        etymology_text = etymology_text.strip()

        # Extract structured info if possible (e.g., from templates)
        # This part is complex and depends heavily on Kaikki's template structure
        normalized_components = None
        etymology_structure = None
        language_codes = None

        if etymology_templates and isinstance(etymology_templates, list):
            # Example: Extract languages mentioned in 'derived', 'borrowed', 'cognate' templates
            langs = set()
            components = []
            try:
                for template in etymology_templates:
                     if isinstance(template, dict):
                        ...
                        if name in ['derived', 'borrowed', 'inherited', 'der', 'bor', 'inh']:
                            lang_code = args.get('1')
                            source_word = args.get('2')
                            if lang_code: langs.add(lang_code)
                            if source_word: components.append(f"{name}:{lang_code}:{source_word}")
                        elif name in ['cognate', 'cog']:
                            lang_code = args.get('1')
                            if lang_code: langs.add(lang_code)
                if langs:
                    language_codes = ",".join(sorted(list(langs)))
                if components:
                     normalized_components = ";".join(components)
                # Store raw templates structure? (Could be large)
                # etymology_structure = json.dumps(etymology_templates)
            except Exception as e:
                 logger.warning(f"Error processing etymology templates for word ID {word_id}: {e}")

        try:
            # Use passed source_identifier when inserting
            insert_etymology(
                cur,
                word_id,
                etymology_text,
            ...
            logger.error(f"Error inserting etymology for word ID {word_id}: {str(e)}")
...
def process_marayum_json(cur, filename: str) -> Tuple[int, int]:
    """Process a single Marayum JSON file."""
    source = SourceStandardization.standardize_sources(os.path.basename(filename))
    logger.info(f"Processing Marayum file: {filename} with source: {source}")
    ...
    language_mapping = get_language_mapping() # Ensure this helper is defined globally

    with tqdm(total=entries_in_file, desc=f"Processing {source}", unit="entry") as pbar:
        for entry_index, entry in enumerate(data):
            # --- Savepoint ---
            # Marayum uses entry_index, which is safe (no negative hashes/hyphens)
            ...
            try:
                if not isinstance(entry, dict):
                     logger.warning(f"Skipping invalid entry at index {entry_index} (not a dict)")
                     skipped_count += 1
                ...
                language_name = entry.get('language', 'Unknown')
                language_code = get_language_code(language_name) # Use helper

                if not headword:
                     logger.warning(f"Skipping entry at index {entry_index} (no headword)")
                     skipped_count += 1
                     cur.execute(f"RELEASE SAVEPOINT {savepoint_name}")
                     pbar.update(1)
                     continue
                ...
                # --- Word Creation ---
                word_metadata = {'marayum_source': source, 'language_name': language_name, 'processed_timestamp': datetime.now().isoformat()}
                # Add other Marayum fields to metadata if needed: e.g., entry.get('id')

                word_id = get_or_create_word_id(
                    cur, headword, language_code=language_code,
                    source_identifier=source, word_metadata=json.dumps(word_metadata)
                )
                if not word_id: raise ValueError(f"Failed to get/create word ID for '{headword}' ({language_code})")

                # --- Process Pronunciations ---
                if 'pronunciations' in entry and entry['pronunciations']:
                     for pron in entry['pronunciations']:
                         if isinstance(pron, str) and pron:
                             try:
                                 insert_pronunciation(cur, word_id, pron, source_identifier=source)
                             except Exception as e: logger.warning(f"Failed pronunciation insert '{pron}' for '{headword}': {e}")

                # --- Process Etymology ---
                if 'etymology' in entry and entry['etymology']:
                     etymology_text = entry['etymology']
                     if isinstance(etymology_text, str) and etymology_text:
                         try:
                             insert_etymology(cur, word_id, etymology_text, source_identifier=source)
                         except Exception as e: logger.warning(f"Failed etymology insert for '{headword}': {e}")
                ...
                if 'senses' in entry and entry['senses']:
                     for sense in entry['senses']:
                         ...
                         if definition:
                             examples_list = [{"text": ex, "source": source} for ex in examples_raw if isinstance(ex, str) and ex]
                             standardized_pos_id = get_standardized_pos_id(cur, standardize_entry_pos(pos)) # Get ID

                             try:
                                 definition_id = insert_definition(
                                     cur, word_id, definition,
                                     standardized_pos_id=standardized_pos_id, original_pos=pos,
                                     examples=json.dumps(examples_list) if examples_list else None,
                                     source_identifier=source # Pass mandatory source
                                     # Add tags/usage_notes if Marayum provides them here
                                 )
                                 # Process sense-level relations if Marayum format includes them
                                 if definition_id and 'synonyms' in sense and sense['synonyms']:
                                     for synonym_word in sense['synonyms']:
                                         if isinstance(synonym_word, str) and synonym_word:
                                             try:
                                                 synonym_id = get_or_create_word_id(cur, synonym_word, language_code, source_identifier=source)
                                                 if synonym_id and synonym_id != word_id:
                                                     rel_metadata = {'source': source, 'definition_id': definition_id, 'confidence': 70}
                                                     insert_relation(cur, word_id, synonym_id, RelationshipType.SYNONYM, source_identifier=source, metadata=rel_metadata)
                                             except Exception as rel_e: logger.warning(f"Error adding synonym '{synonym_word}' for '{headword}': {rel_e}")
                                 # Add blocks for antonyms, related words etc. if provided by Marayum senses

                             except psycopg2.errors.UniqueViolation:
                                logger.debug(f"Marayum def exists for '{headword}', sense: {definition[:30]}...")
                             except Exception as def_e: logger.error(f"Failed def insert for '{headword}', sense: {definition[:30]}... : {def_e}")

                # --- Process Top-Level Relations ---
                relation_keys = {'synonyms', 'antonyms', 'relatedWords'} # Adjust based on Marayum format
                for rel_key in relation_keys:
                     if rel_key in entry and entry[rel_key]:
                         rel_type_enum = None
                         if rel_key == 'synonyms': rel_type_enum = RelationshipType.SYNONYM
                         elif rel_key == 'antonyms': rel_type_enum = RelationshipType.ANTONYM
                         elif rel_key == 'relatedWords': rel_type_enum = RelationshipType.RELATED
                         if rel_type_enum:
                             for related_word_str in entry[rel_key]:
                                 if isinstance(related_word_str, str) and related_word_str:
                                     try:
                                         related_id = get_or_create_word_id(cur, related_word_str, language_code, source_identifier=source)
                                         if related_id and related_id != word_id:
                                             rel_metadata = {'source': source, 'confidence': 70}
                                             insert_relation(cur, word_id, related_id, rel_type_enum, source_identifier=source, metadata=rel_metadata)
                                     except Exception as top_rel_e: logger.warning(f"Error adding top-level relation '{related_word_str}' for '{headword}': {top_rel_e}")
                ...
                logger.error(f"Error processing Marayum entry index {entry_index} ('{entry.get('headword', 'N/A')}') : {entry_err}", exc_info=False) # Keep log cleaner
                try:
                    cur.execute(f"ROLLBACK TO SAVEPOINT {savepoint_name}")
                except Exception as rb_err:
                    logger.critical(f"CRITICAL: Failed to rollback savepoint {savepoint_name}: {rb_err}. Attempting full transaction rollback.")
            ...
            # Commit periodically (e.g., every 500 entries)
            if (processed_count + error_count + skipped_count) % 500 == 0:
                 try:
                     cur.connection.commit()
                     logger.debug(f"Committed progress for {filename} at entry {entry_index}")
                 except Exception as commit_err:
                     logger.error(f"Error committing progress for {filename}: {commit_err}. Rolling back...")
                     cur.connection.rollback()
                     # Continue processing, but acknowledge commit failed

    # Final commit for the file
    try:
        cur.connection.commit()
        logger.info(f"Finished processing {filename}. Processed: {processed_count}, Skipped: {skipped_count}, Errors: {error_count}")
    except Exception as final_commit_err:
        logger.error(f"Error during final commit for {filename}: {final_commit_err}. Rolling back...")
        cur.connection.rollback()
