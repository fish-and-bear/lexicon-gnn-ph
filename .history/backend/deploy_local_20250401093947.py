...
def pre_flight_check() -> bool:
    ...
    try:
        ...
        # Check disk space
        free_space = psutil.disk_usage(str(backend_dir)).free  # Convert Path to string
...
def setup_environment():
    ...
    try:
        ...
        if env_file.exists():
            load_dotenv(str(env_file))  # Convert Path to string
...
def create_api_script():
    ...
    with open(str(api_script), 'w') as f:  # Convert Path to string
        f.write(f'''"""
...
# Add the backend directory to the Python path
backend_dir = str(Path(__file__).parent.absolute())  # Convert Path to string
...
def deploy():
    ...
    try:
        ...
        api_process = subprocess.Popen(
            [sys.executable, str(api_script)],  # Convert Path to string
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            env=os.environ.copy(),
            text=True
