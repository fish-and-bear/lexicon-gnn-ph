...
class RelationshipType(Enum):
    ...
    - value: The string value stored in the database
    ...
    def __init__(self, value, category, bidirectional, inverse, transitive, strength):
        self.value = value
...
class RelationshipManager:
    ...
    def add_relationship(self, from_word_id: int, to_word_id: int, 
        ...
        try:
            ...
            """, (from_word_id, to_word_id, str(rel_type), sources, json.dumps(metadata)))
            ...
            if rel_type.bidirectional:
                self.cursor.execute("""
                    ...
                    DO UPDATE SET 
                        ...
                        END,
                        metadata = COALESCE(relations.metadata, '{}') || EXCLUDED.metadata
                """, (to_word_id, from_word_id, str(rel_type), sources, json.dumps(metadata)))
                ...
                self.cursor.execute("""
                    ...
                    DO UPDATE SET 
                        ...
                        END,
                        metadata = COALESCE(relations.metadata, '{}') || EXCLUDED.metadata
                """, (to_word_id, from_word_id, str(inverse_type), sources, json.dumps(metadata)))
    ...
    for rel_id, from_id, to_id, rel_type_str, sources in relations:
        ...
        cur.execute("""
            ...
            SET relation_type = %s,
                ...
                updated_at = CURRENT_TIMESTAMP
            WHERE id = %s
        """, (str(rel_type), json.dumps(metadata), rel_id))
        updated_count += 1
...
# Add after the display_help_cli function at approximately line 4869
def migrate_relationships_cli(args):
    """Migrate existing relationships to the new RelationshipType system."""
    try:
        conn = get_connection()
        cur = conn.cursor()
        console = Console()
        console.print("[bold blue]Starting relationship migration to new RelationshipType system...[/]")
        
        count = migrate_relationships_to_new_system(cur)
        
        console.print(f"[bold green]Migration complete. Updated {count} relationships.[/]")
    except Exception as e:
        print(f"Error during relationship migration: {str(e)}")
...
def main():
    ...
    elif args.command == "migrate-relationships":
        migrate_relationships_cli(args)
    else:
        parser.print_help()
