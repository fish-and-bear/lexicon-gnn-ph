...
de@with_transaction(commit=True)
def process_entry(cur, entry: Dict, filename=None, check_exists=False):
    ...
    Args:
        ...
        check_exists: Whether to only check if the word exists
        
    Returns:
        The word ID if successful, None if skipped or error
    """
    conn = cur.connection
    ...
    try:
        ...
        if 'etymology' in entry and entry['etymology'] and isinstance(entry['etymology'], str) and entry['etymology'].strip():
