        ...
        def wrapper(cur, *args, **kwargs):
            ...
            try:
                ...
                try:
                    ...
                    logger.warning(f"Failed to create savepoint for {func.__name__}: {str(sp_error)}")
                ...
                if commit:
                    # Release the savepoint first if it was created
                    if savepoint_created:
                        try:
                            cur.execute(f"RELEASE SAVEPOINT {savepoint_name}")
                        except Exception as release_error:
                            logger.warning(f"Failed to release savepoint in {func.__name__}: {str(release_error)}")
                    
                    # For specific functions related to word creation, ensure an explicit commit
                    if func.__name__ in ('get_or_create_word_id', 'get_or_create_word_id_base', 'batch_get_or_create_word_ids'):
                        try:
                            conn.commit()
                            logger.debug(f"Explicitly committed transaction for {func.__name__}")
                        except Exception as commit_error:
                            logger.error(f"Failed to explicitly commit transaction for {func.__name__}: {str(commit_error)}")
                ...
                if savepoint_created:
                    try:
                        ...
                        logger.warning(f"Failed to rollback to savepoint in {func.__name__}: {str(rollback_error)}")
                logger.debug(f"Unique violation in {func.__name__}: {str(e)}")
                raise
                ...
                # For critical functions, try to recover with a full rollback
                if func.__name__ in ('get_or_create_word_id', 'get_or_create_word_id_base', 'batch_get_or_create_word_ids'):
                    logger.warning(f"Transaction failed in {func.__name__}, rolling back entire transaction")
                    try:
                        conn.rollback()
                    except Exception as rollback_error:
...
def insert_definition(cur, word_id: int, definition_text: str, part_of_speech: str = "",
    ...
    try:
        ...
        # Explicitly commit to ensure the definition is saved
        try:
            cur.connection.commit()
        except Exception as commit_error:
            logger.error(f"Failed to explicitly commit definition: {str(commit_error)}")
            
        return def_id
        
    except Exception as e:
        logger.error(f"Error in insert_definition for word_id {word_id}, definition: {definition_text}: {str(e)}")
        try:
            cur.connection.rollback()
        except Exception:
            pass  # Ignore rollback errors
