...
class RelationshipTypeEnum(graphene.Enum):
    ...
    class Meta:
        ...
        description = 'Types of word relationships'

    @classmethod
    def __init_subclass_with_meta__(cls, **options):
        values = OrderedDict()
        for rel_type in RelationshipType:
            values[rel_type.name] = graphene.EnumValue(
                rel_type.value[0],  # Use the string value from the tuple
                description=rel_type.description,
                value=rel_type.name
            )
        options['enum'] = type('RelationshipTypeEnum', (), {'_member_map_': values})
        super().__init_subclass_with_meta__(**options)

class RelationshipCategoryEnum(graphene.Enum):
    """GraphQL enum for relationship categories."""
    class Meta:
        name = 'RelationshipCategory'
        description = 'Categories of word relationships'

    @classmethod
    def __init_subclass_with_meta__(cls, **options):
        values = OrderedDict()
        for category in RelationshipCategory:
            values[category.name] = graphene.EnumValue(
                category.value,
                description=f"Relationships based on {category.value}",
                value=category.name
            )
        options['enum'] = type('RelationshipCategoryEnum', (), {'_member_map_': values})
        super().__init_subclass_with_meta__(**options)

class RelationshipMetadata(graphene.ObjectType):
    """Type for relationship metadata."""
    type = graphene.Field(RelationshipTypeEnum, required=True)
    category = graphene.Field(RelationshipCategoryEnum, required=True)
    bidirectional = graphene.Boolean(required=True)
    inverse = graphene.Field(RelationshipTypeEnum)
    ...
    def from_relationship_type(rel_type: RelationshipType) -> 'RelationshipMetadata':
        ...
        return RelationshipMetadata(
            type=rel_type.name,
            category=rel_type.category.name,
            bidirectional=rel_type.bidirectional,
            inverse=rel_type.inverse if rel_type.inverse else None,
...
class VerificationStatusEnum(graphene.Enum):
    ...
    class Meta:
        ...
        description = 'Status of data verification'

    @classmethod
    def __init_subclass_with_meta__(cls, **options):
        values = OrderedDict([
            ('UNVERIFIED', graphene.EnumValue('unverified', description='Data has not been verified')),
            ('VERIFIED', graphene.EnumValue('verified', description='Data has been verified by an expert')),
            ('NEEDS_REVIEW', graphene.EnumValue('needs_review', description='Data needs review by an expert')),
            ('DISPUTED', graphene.EnumValue('disputed', description='Data accuracy is disputed'))
        ])
        options['enum'] = type('VerificationStatusEnum', (), {'_member_map_': values})
        super().__init_subclass_with_meta__(**options)
...
class MetaInfoType(graphene.ObjectType):
    ...
    verification_status = graphene.Field(VerificationStatusEnum, description='Verification status')
...
class RelationType(SQLAlchemyObjectType):
    ...
    category = graphene.Field(RelationshipCategoryEnum)
    ...
    def resolve_relation_type(self, info):
        """Resolve the relationship type enum value."""
