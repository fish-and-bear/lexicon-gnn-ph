...
def get_semantic_network(word: str):
    ...
    """
    try:
        depth = min(int(request.args.get("depth", 1)), 4)  # Allow higher depth (max 4)
        breadth = min(int(request.args.get("breadth", 10)), 50)  # Max breadth 50
        ...
        word_id = None
        normalized = normalize_word(word)
        word_data = get_word_by_lemma_or_id(normalized)
        ...
        # Check if pre-computed network is already cached for more expensive networks
        cache_key = f"semantic_network:{word_id}:{depth}:{breadth}:{relation_types}"
        if depth > 1 and config.USE_CACHING:
        ...
        outgoing_sql = """
            SELECT /*+ INDEX(r idx_relations_from_word_id) */ 
                   r.id, r.relation_type, r.to_word_id, 
                   w.lemma, w.language_code
            FROM relations r
        ...
        incoming_sql = """
            SELECT /*+ INDEX(r idx_relations_to_word_id) */
                   r.id, r.relation_type, r.from_word_id,
                   w.lemma, w.language_code
            FROM relations r
        ...
        nodes = {word_id: {"id": str(word_id), "label": word_data.lemma, "main": True}}
        edges = {}
        
        # Process outgoing relations
        for rel in outgoing:
            rel_id, rel_type, target_id, target_lemma, target_lang = rel
        ...
        for rel in incoming:
            rel_id, rel_type, source_id, source_lemma, source_lang = rel
        ...
        # Cache the result for expensive queries
        if depth > 1 and config.USE_CACHING:
            cache.set(cache_key, json.dumps(result), timeout=900)  # 15 minutes timeout
...
@cached_query(timeout=900)  # Cache for 15 minutes as this is resource-intensive
def get_semantic_network(word: str):
    """Get semantic network for a word with configurable depth and breadth."""
    try:
        start_time = time.time()
        
        # Get and validate parameters
        depth = request.args.get("depth", "1")
        relation_types_param = request.args.get("relation_types", "")
        breadth_param = request.args.get("breadth", "15")
        
        # Validate depth (allow 1-5)
        try:
            depth = int(depth)
            # Cap depth at 5 for performance reasons
            depth = min(max(depth, 1), 5)
        except ValueError:
            depth = 1
            
        # Validate breadth (allow 1-100)
        try:
            breadth = int(breadth_param)
            # Cap breadth at 100 for performance reasons
            breadth = min(max(breadth, 1), 100)
        except ValueError:
            breadth = 15
        
        # Parse relation types filter
        relation_types = relation_types_param.split(",") if relation_types_param else []
        relation_types = [r.strip() for r in relation_types if r.strip()]
        
        # Find the word entry using direct SQL to avoid ORM overhead
        try:
            word_id = int(word)
            sql = "SELECT id, lemma, normalized_lemma, language_code FROM words WHERE id = :id"
            word_entry = db.session.execute(text(sql), {"id": word_id}).fetchone()
        except ValueError:
            normalized = normalize_lemma(word)
            sql = "SELECT id, lemma, normalized_lemma, language_code FROM words WHERE LOWER(lemma) = LOWER(:word) OR LOWER(normalized_lemma) = LOWER(:normalized)"
            word_entry = db.session.execute(text(sql), {"word": word, "normalized": normalized}).fetchone()
        
        if not word_entry:
            return jsonify({"error": f"Word '{word}' not found"}), 404
        
        # Optimization: Use a more efficient approach for deep networks
        if depth > 3 or breadth > 30:
            logger.info(f"Large network requested for '{word}' with depth={depth}, breadth={breadth}")
            return _get_large_semantic_network(word_entry.id, depth, breadth, relation_types)
        
        # Create network - use optimized data structures
        nodes = {}  # id -> node_data mapping
        edges = []
        processed = set()  # track processed nodes
        
        # Process queue for BFS traversal: (node_id, current_depth)
        process_queue = [(word_entry.id, 0)]
        
        # Add the main word to nodes dictionary
        nodes[word_entry.id] = {
            "id": word_entry.id,
            "lemma": word_entry.lemma,
            "language_code": word_entry.language_code,
            "group": "main"  # Mark the main word with a special group
        }
        
        # Prepare common relation query parts
        relation_condition = ""
        if relation_types:
            placeholders = ", ".join([f":rel_type_{i}" for i in range(len(relation_types))])
            relation_condition = f" AND r.relation_type IN ({placeholders})"
        
        # Process nodes in breadth-first order
        while process_queue:
            current_id, current_depth = process_queue.pop(0)
            
            # Skip if already processed or past max depth
            ...
            FROM relations r
            JOIN words w ON r.to_word_id = w.id
            WHERE r.from_word_id = :word_id{relation_condition}
            ORDER BY 
                CASE 
                    WHEN r.relation_type IN ('synonym', 'antonym') THEN 1
                    WHEN r.relation_type IN ('hypernym', 'hyponym') THEN 2 
            ...
            outgoing_params = {"word_id": current_id, "breadth": breadth}
            if relation_types:
                for i, rel_type in enumerate(relation_types):
                    outgoing_params[f"rel_type_{i}"] = rel_type
            
            outgoing = db.session.execute(text(outgoing_sql), outgoing_params).fetchall()
            
            # Process outgoing relations
            for relation in outgoing:
                target_id = relation.to_word_id
                
                # Determine relationship group
                rel_group = "related"
                if relation.relation_type == "synonym":
                    rel_group = "synonym"
                elif relation.relation_type == "antonym":
                    rel_group = "antonym"
                elif relation.relation_type == "hypernym":
                    rel_group = "taxonomic"
                elif relation.relation_type == "hyponym":
                    rel_group = "taxonomic"
                elif relation.relation_type == "root":
                    rel_group = "root"
                elif relation.relation_type == "derived":
                    rel_group = "derived"
                ...
                if target_id not in nodes:
                    ...
                    # Only process this node further if we're not at max depth
                    if current_depth < depth - 1:
                        process_queue.append((target_id, current_depth + 1))
                
                # Add edge
                edge_id = f"{current_id}-{relation.relation_type}-{target_id}"
                edges.append({
                    "id": edge_id,
                    "source": current_id,
                    "target": target_id,
                    "relationship": relation.relation_type,
                    "metadata": relation.relation_data or {}
                })
            
            # Query incoming relations if not at max depth
            if current_depth < depth - 1:
                incoming_sql = f"""
                SELECT /*+ INDEX(r idx_relations_to_word_id) */ 
                       r.id, r.relation_type, r.from_word_id, 
                ...
                FROM relations r
                JOIN words w ON r.from_word_id = w.id
                WHERE r.to_word_id = :word_id{relation_condition}
                ORDER BY 
                    CASE 
                        WHEN r.relation_type IN ('synonym', 'antonym') THEN 1
                        WHEN r.relation_type IN ('hypernym', 'hyponym') THEN 2 
                ...
                # Prepare query params
                incoming_params = {"word_id": current_id, "breadth": breadth}
                if relation_types:
                    for i, rel_type in enumerate(relation_types):
                        incoming_params[f"rel_type_{i}"] = rel_type
                
                incoming = db.session.execute(text(incoming_sql), incoming_params).fetchall()
                
                # Process incoming relations
                for relation in incoming:
                    source_id = relation.from_word_id
                    ...
                    if source_id not in nodes:
                        ...
                        # Add to queue for processing if not at max depth
                        if current_depth < depth - 1:
                            process_queue.append((source_id, current_depth + 1))
                    
                    # Add edge
                    edge_id = f"{source_id}-{relation.relation_type}-{current_id}"
                    edges.append({
                        "id": edge_id,
                        "source": source_id,
                        "target": current_id,
        ...
        execution_time = time.time() - start_time
        logger.info(f"Generated semantic network for '{word}' with {len(nodes)} nodes, {len(edges)} edges in {execution_time:.2f}s")
...
def _get_large_semantic_network(word_id, depth, breadth, relation_types=None):
    """Optimized method for retrieving large semantic networks efficiently."""
    try:
        start_time = time.time()
        
        # Use single SQL query with recursive CTE for better performance
        cte_condition = ""
        params = {"word_id": word_id, "max_depth": depth, "max_breadth": breadth}
        
        if relation_types:
            placeholders = ", ".join([f":rel_type_{i}" for i in range(len(relation_types))])
            cte_condition = f" AND r.relation_type IN ({placeholders})"
            for i, rel_type in enumerate(relation_types):
                params[f"rel_type_{i}"] = rel_type
        
        # SQL with recursive CTE for efficient traversal
        network_sql = f"""
        WITH RECURSIVE word_network AS (
            -- Anchor member: Start with the main word
            SELECT 
                w.id, w.lemma, w.language_code, 
                0 AS depth,
                'main' AS rel_group,
                w.id::text AS path
            FROM words w
            ...
            -- Recursive member: Add connected words within depth limit
            SELECT 
                w.id, w.lemma, w.language_code,
                wn.depth + 1 AS depth,
                CASE 
                    WHEN r.relation_type = 'synonym' THEN 'synonym'
                    WHEN r.relation_type = 'antonym' THEN 'antonym'
                    WHEN r.relation_type IN ('hypernym', 'hyponym') THEN 'taxonomic'
                    WHEN r.relation_type = 'root' THEN 'root'
                    WHEN r.relation_type = 'derived' THEN 'derived'
                    ELSE 'related'
                END AS rel_group,
                wn.path || ',' || w.id::text AS path
            FROM word_network wn
            JOIN relations r ON (r.from_word_id = wn.id OR r.to_word_id = wn.id)
            JOIN words w ON (
                (r.from_word_id = wn.id AND r.to_word_id = w.id) OR
                (r.to_word_id = wn.id AND r.from_word_id = w.id)
            )
            WHERE 
                wn.depth < :max_depth
                AND w.id::text NOT IN (SELECT unnest(string_to_array(wn.path, ',')))
                {cte_condition}
                -- Limit breadth per node by taking only top N per parent node
                AND (
                    SELECT COUNT(*) 
                    FROM relations r2 
                    WHERE 
                        (r2.from_word_id = wn.id AND r2.to_word_id != w.id) OR
                        (r2.to_word_id = wn.id AND r2.from_word_id != w.id)
                ) < :max_breadth
        ),
        
        -- Get all edges between nodes in the network
        network_edges AS (
            SELECT DISTINCT
                r.from_word_id AS source,
                r.to_word_id AS target,
                r.relation_type,
                r.relation_data
            FROM relations r
            WHERE 
                r.from_word_id IN (SELECT id FROM word_network) AND
                r.to_word_id IN (SELECT id FROM word_network)
                {cte_condition}
        )
        
        -- Return nodes and edges
        SELECT 
            n.id, n.lemma, n.language_code, n.rel_group,
            e.source, e.target, e.relation_type, e.relation_data
        FROM word_network n
        LEFT JOIN network_edges e ON (e.source = n.id OR e.target = n.id)
        ORDER BY n.depth, n.id
        """
        
        # Execute the query
        results = db.session.execute(text(network_sql), params).fetchall()
        ...
        for row in results:
            ...
            if row.source and row.target:
                edge_id = f"{row.source}-{row.relation_type}-{row.target}"
                ...
                if not any(e["id"] == edge_id for e in edges):
                    edges.append({
                        ...
                        "metadata": row.relation_data or {}
                    })
        
        # Calculate execution time
        execution_time = time.time() - start_time
        logger.info(f"Generated large semantic network with {len(nodes)} nodes, {len(edges)} edges in {execution_time:.2f}s")
        ...
        logger.error(f"Error generating large semantic network", error=str(e), exc_info=True)
        return jsonify({"error": str(e)}), 500
