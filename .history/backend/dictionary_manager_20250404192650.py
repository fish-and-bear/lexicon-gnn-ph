...
CREATE TABLE IF NOT EXISTS words (
    ...
    root_word_id INT REFERENCES words(id) ON DELETE SET NULL, -- Changed ON DELETE behavior
    preferred_spelling VARCHAR(255),
    tags TEXT,
    idioms JSONB DEFAULT '[]',
    pronunciation_data JSONB, -- Keep for potential direct storage, though separate table is better
...
-- Add search_text column and populate it if it doesn't exist
DO $$ BEGIN
    IF NOT EXISTS (
        ...
        -- Initial population might take time, consider doing it separately or only for new rows via trigger later
...
-- Add missing columns (idempotent checks) - MOVED FROM KAIKKI PROCESSOR
DO $$
BEGIN
    -- Words table enhancements
    IF NOT EXISTS (SELECT FROM information_schema.columns WHERE table_name = 'words' AND column_name = 'badlit_form') THEN
        ALTER TABLE words ADD COLUMN badlit_form TEXT;
    END IF;
    IF NOT EXISTS (SELECT FROM information_schema.columns WHERE table_name = 'words' AND column_name = 'hyphenation') THEN
        ALTER TABLE words ADD COLUMN hyphenation JSONB;
    END IF;
    IF NOT EXISTS (SELECT FROM information_schema.columns WHERE table_name = 'words' AND column_name = 'is_proper_noun') THEN
        ALTER TABLE words ADD COLUMN is_proper_noun BOOLEAN DEFAULT FALSE;
    END IF;
    IF NOT EXISTS (SELECT FROM information_schema.columns WHERE table_name = 'words' AND column_name = 'is_abbreviation') THEN
        ALTER TABLE words ADD COLUMN is_abbreviation BOOLEAN DEFAULT FALSE;
    END IF;
    IF NOT EXISTS (SELECT FROM information_schema.columns WHERE table_name = 'words' AND column_name = 'is_initialism') THEN
        ALTER TABLE words ADD COLUMN is_initialism BOOLEAN DEFAULT FALSE;
    END IF;

    -- Definitions table enhancements
    IF NOT EXISTS (SELECT FROM information_schema.columns WHERE table_name = 'definitions' AND column_name = 'metadata') THEN
...
CREATE INDEX IF NOT EXISTS idx_words_metadata ON words USING GIN(word_metadata); -- Added index for metadata
...
CREATE TABLE IF NOT EXISTS definitions (
    ...
    original_pos TEXT,
    standardized_pos_id INT REFERENCES parts_of_speech(id) ON DELETE SET NULL, -- Changed ON DELETE behavior
    examples TEXT, -- Consider JSONB if examples become structured
    usage_notes TEXT,
    tags TEXT, -- Consider JSONB if tags become structured
    sources TEXT, -- Changed to allow NULL initially, populate later? Or keep NOT NULL? Kept NOT NULL for now.
    metadata JSONB, -- Added metadata column
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    -- Unique constraint may need adjustment if multiple definitions with same text but different metadata are valid
...
CREATE INDEX IF NOT EXISTS idx_definitions_text ON definitions USING gin(to_tsvector('english', definition_text)); -- Use 'simple' if handling multiple languages
...
CREATE TABLE IF NOT EXISTS etymologies (
    ...
    normalized_components TEXT, -- Consider JSONB if components become structured
    etymology_structure TEXT, -- Consider JSONB if structure becomes complex
    language_codes TEXT, -- Comma-separated list, consider array or separate table for better querying
    sources TEXT, -- Changed to allow NULL initially? Or keep NOT NULL? Kept NOT NULL for now.
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
...
-- NEW TABLES MOVED FROM KAIKKI PROCESSOR:
