...
# Fix the with_transaction decorator to handle transactions properly
def with_transaction(commit=True):
    """
    Decorator that handles transaction management for database functions.
    ...
    def decorator(func):
        ...
        def wrapper(cur, *args, **kwargs):
            ...
            try:
                # This will succeed only if we're not in a transaction
                cur.execute("SELECT 1; ROLLBACK;")
            except psycopg2.Error:
                # We couldn't roll back, which means we're in a transaction
                in_transaction = True
            ...
            try:
                ...
                # Only commit if we started the transaction and commit flag is True
...
def create_or_update_tables(conn):
    ...
    try:
        ...
        DO $$
        BEGIN
            -- Check if source_languages column exists but language_codes doesn't
            IF EXISTS (
                SELECT FROM information_schema.columns 
                WHERE table_name='etymologies' AND column_name='source_languages'
            ) AND NOT EXISTS (
                SELECT FROM information_schema.columns 
                WHERE table_name='etymologies' AND column_name='language_codes'
            ) THEN
                -- Rename the column
                ALTER TABLE etymologies RENAME COLUMN source_languages TO language_codes;
                RAISE NOTICE 'Renamed source_languages column to language_codes';
            
            -- If neither column exists, add language_codes
            ELSIF NOT EXISTS (
                SELECT FROM information_schema.columns 
                WHERE table_name='etymologies' AND column_name='language_codes'
            ) THEN
                -- Add the column
                ALTER TABLE etymologies ADD COLUMN language_codes VARCHAR(128);
                RAISE NOTICE 'Added language_codes column';
            
            -- If both exist (unlikely), keep language_codes and drop source_languages
            ELSIF EXISTS (
                SELECT FROM information_schema.columns 
                WHERE table_name='etymologies' AND column_name='source_languages'
            ) AND EXISTS (
                ...
                UPDATE etymologies 
                SET language_codes = source_languages 
                WHERE language_codes IS NULL AND source_languages IS NOT NULL;
                
                -- Drop the redundant column
                ALTER TABLE etymologies DROP COLUMN source_languages;
                RAISE NOTICE 'Migrated data and dropped source_languages column';
            END IF;
            
            -- Check and increase column sizes if needed for relations
            IF EXISTS (
                SELECT FROM information_schema.columns 
                WHERE table_name='relations' AND column_name='relation_type' 
                AND character_maximum_length < 128
            ) THEN
                -- Increase size of relation_type column
                ALTER TABLE relations ALTER COLUMN relation_type TYPE VARCHAR(128);
                RAISE NOTICE 'Increased size of relation_type column to VARCHAR(128)';
            END IF;
            
            -- Check sources column size
            IF EXISTS (
                SELECT FROM information_schema.columns 
                WHERE table_name='relations' AND column_name='sources' 
                AND character_maximum_length < 255
            ) THEN
                -- Increase size of sources column
                ALTER TABLE relations ALTER COLUMN sources TYPE VARCHAR(255);
                RAISE NOTICE 'Increased size of sources column to VARCHAR(255)';
            END IF;
        END $$;
        """)
        conn.commit()
...
def get_or_create_word_id(cur, lemma: str, language_code: str = "tl", check_exists: bool = False, **kwargs) -> int:
    ...
    # Create a deterministic lock ID from the normalized lemma and language code
    lock_key = hash((normalized_lemma, language_code)) % (2**31)
    ...
    try:
        ...
        if has_baybayin and baybayin_form and not validate_baybayin_entry(baybayin_form, romanized_form):
        ...
        cur.execute("""
            INSERT INTO words (
                ...
                romanized_form, created_at, updated_at
...
def insert_relation(
    ...
    """
    Insert a relationship between two words with maximum compatibility.
    Adapts to existing database schema - works with both 'relations' and 'word_relations' tables
    ...
    Args:
        ...
        sources: Sources of the relationship information
        metadata: Additional metadata about the relationship
        
    Returns:
        ID of the inserted relationship or None if insertion failed
    """
    # Guard against self-relationships
    ...
    if isinstance(relation_type, str):
        try:
            ...
            logger.warning(f"Unknown relationship type: {relation_type}, using RELATED instead")
            relation_type = RelationshipType.RELATED
    
    # Extract the relation value
    ...
    if sources and len(sources) > max_source_length:
        logger.warning(f"Source value too long ({len(sources)}), truncating to {max_source_length} chars")
        sources = sources[:max_source_length]
    
    # Truncate relation_type if too long
    max_rel_type_length = 64  # Using conservative value
    if len(rel_value) > max_rel_type_length:
        logger.warning(f"Relation type too long ({len(rel_value)}), truncating to {max_rel_type_length} chars")
        rel_value = rel_value[:max_rel_type_length]
    ...
    if metadata:
        try:
            ...
            if len(metadata_json) > max_metadata_length:
                logger.warning(f"Metadata too large ({len(metadata_json)}), simplifying")
                # Keep only essential keys or truncate
                simplified_metadata = {}
                for k, v in metadata.items():
                    if isinstance(v, str) and len(v) > 100:
                        simplified_metadata[k] = v[:100] + "..."
                    else:
    ...
    try:
        # First, check which relation table exists and its schema
        table_info = {
            'name': 'relations',  # Default assumption
        ...
        cur.execute("SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'relations')")
        relations_exists = cur.fetchone()[0]
        
        # Check if word_relations table exists
        cur.execute("SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'word_relations')")
        word_relations_exists = cur.fetchone()[0]
        
        # Determine which table to use
        if word_relations_exists:
            table_info['name'] = 'word_relations'
            table_info['exists'] = True
            
            # Check columns in word_relations
            cur.execute("""
                SELECT column_name FROM information_schema.columns 
                WHERE table_name = 'word_relations'
            """)
            columns = [row[0] for row in cur.fetchall()]
            
            table_info['has_bidirectional'] = 'bidirectional' in columns
            table_info['has_metadata'] = 'metadata' in columns
            table_info['has_strength'] = 'strength' in columns
            
        elif relations_exists:
            table_info['name'] = 'relations'
            table_info['exists'] = True
            
            # Check columns in relations
            cur.execute("""
                SELECT column_name FROM information_schema.columns 
        ...
        # If no table exists, create a minimal one
        if not table_info['exists']:
            logger.warning("No relations table found, creating minimal 'relations' table")
            cur.execute("""
            ...
            """)
            cur.execute("CREATE INDEX relations_from_word_id_idx ON relations(from_word_id)")
            cur.execute("CREATE INDEX relations_to_word_id_idx ON relations(to_word_id)")
            
            table_info['name'] = 'relations'
            table_info['exists'] = True
            table_info['has_bidirectional'] = False
            table_info['has_metadata'] = False
            table_info['has_strength'] = False
        
        # Check if relation already exists
        cur.execute(f"""
            SELECT id FROM {table_info['name']} 
        ...
        if existing:
            ...
            if table_info['has_strength']:
                update_parts.append("strength = %s")
                update_values.append(relation_type.strength)
            ...
            # Insert new relation - adapt fields based on what's available
            fields = ["from_word_id", "to_word_id", "relation_type", "sources"]
            values = [from_word_id, to_word_id, rel_value, sources]
            placeholders = ["%s", "%s", "%s", "%s"]
            
            if table_info['has_bidirectional']:
                fields.append("bidirectional")
                values.append(relation_type.bidirectional)
            ...
            if table_info['has_strength']:
                ...
                values.append(relation_type.strength)
                placeholders.append("%s")
            ...
            # For bidirectional relations, try to add inverse if supported
            if relation_type.bidirectional and table_info['has_bidirectional']:
                try:
                    ...
                    if not cur.fetchone():
                        # Swap from/to for inverse relation
                        inv_values = values.copy()
                        inv_values[0] = to_word_id
                        inv_values[1] = from_word_id
                        
                        cur.execute(insert_query, tuple(inv_values))
                        logger.debug(f"Added inverse relation {rel_value} from word ID {to_word_id} to {from_word_id}")
                except Exception as e:
                    logger.error(f"Error adding inverse relation: {str(e)}")
            
            return relation_id
                
    except Exception as e:
        logger.error(f"Database error in insert_relation: {str(e)}")
        return None
...
def process_kaikki_jsonl(cur, filename: str):
    ...
    def process_etymology(cur, word_id: int, etymology_text: str, etymology_templates: List[Dict] = None):
        ...
        # Truncate etymology_text if too long
        max_etymo_length = 4096  # Adjust based on your database column size
        if len(etymology_text) > max_etymo_length:
            logger.warning(f"Etymology text too long ({len(etymology_text)} chars), truncating")
            etymology_text = etymology_text[:max_etymo_length]
        ...
        if etymology_templates:
            try:
                etymology_structure = json.dumps(etymology_templates)
                # Check if structure is too large
                max_structure_length = 4096  # Adjust based on your database column size
                if etymology_structure and len(etymology_structure) > max_structure_length:
                    logger.warning(f"Etymology structure too large ({len(etymology_structure)} chars), simplifying")
                    # Store only essential information
                    simplified = [{'type': t.get('type'), 'name': t.get('name')} for t in etymology_templates]
        ...
        try:
            ...
            """, (
                ...
                json.dumps(normalized_components) if normalized_components else None,
                etymology_structure,
                json.dumps(language_codes) if language_codes else None,
                standardized_source
            ))
            logger.debug(f"Successfully added etymology for word ID {word_id}")
        except Exception as e:
