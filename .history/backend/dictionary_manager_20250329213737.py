...
def process_kaikki_jsonl(cur, filename: str):
    ...
    try:
        with open(filename, 'r', encoding='utf-8') as f:
            # Process entries individually with new transaction per entry
            entry_count = 0
            ...
            for line in f:
                try:
                    ...
                    if not result:
                        logger.debug(f"Trying fallback processing for entry {entry_count} (word: {entry.get('word', 'unknown')})")
