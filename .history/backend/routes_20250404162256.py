...
def search_words():
    ...
    try:
        ...
        # Apply pagination
        # REMOVED selectinload for Word.definitions as it causes InvalidRequestError
        # Definitions will be handled by the schema or loaded lazily if accessed directly.
        # We can still eager load the PartOfSpeech for the definitions if needed, 
        # but let's remove it for now to ensure the query runs.
...
def get_word_comprehensive(word: str):
    ...
    try:
        logger.info(f"Starting comprehensive fetch for: {word}") # Added entry log
        ...
        if not word_obj:
            logger.warning(f"Word not found during comprehensive fetch: {word}")
            return jsonify({
                "error": "Word not found",
                "suggestions": get_word_suggestions(word) # Keep suggestion helper
            }), 404
        
        logger.info(f"Successfully fetched Word object ID: {word_obj.id}")
        
        # --- DIAGNOSTIC STEP: Skip serialization --- 
        # Return minimal data immediately to check if fetching works
        minimal_result = {
            "id": word_obj.id,
            "lemma": word_obj.lemma,
            "message": "Serialization skipped for diagnostics"
        }
        logger.info(f"Returning minimal diagnostic response for word ID: {word_obj.id}")
        return jsonify(minimal_result), 200
        # --- END DIAGNOSTIC STEP ---

        # Original serialization logic (commented out for diagnostics)
        # schema = WordSchema(exclude=("outgoing_relations", "incoming_relations", "pronunciations")) 
        # logger.info(f"Attempting to serialize Word object ID: {word_obj.id}")
        # result = schema.dump(word_obj)
        # logger.info(f"Successfully serialized Word object ID: {word_obj.id}")

        # Relation summary calculation won't work with relations excluded, comment out
    ...
    except Exception as e: # Broad exception catch for diagnostics
        logger.error(f"Unhandled exception in get_word_comprehensive for {word}: {str(e)}", exc_info=True)
        # Ensure session is rolled back on any error
        db.session.rollback()
        return jsonify({"error": "Internal server error during comprehensive fetch", "details": str(e)}), 500
...
def _get_word_details_orm(word_identifier: Union[str, int]) -> Optional[Word]:
    ...
    # Eager loading removed as it causes InvalidRequestError with current model setup.
    # Relationships will be loaded lazily by SQLAlchemy when accessed.
    # query = query.options(
    ...
    # Fetch the word object without eager loading options
