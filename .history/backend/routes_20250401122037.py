...
def search_words():
    ...
    try:
        ...
        # Always include basic text matching
        conditions.extend([
            Word.normalized_lemma.ilike(f'%{search_text}%'),
            Word.lemma.ilike(f'%{search_text}%')
        ])
        
        # Add similarity matching if pg_trgm is available
        try:
            conditions.extend([
                func.similarity(Word.lemma, search_text) >= 0.3,
                func.similarity(Word.normalized_lemma, search_text) >= 0.3
            ])
        except Exception as e:
            current_app.logger.warning(f"Similarity function not available: {str(e)}")
        
        # Add full-text search if search_text column exists
        try:
            if hasattr(Word, 'search_text') and not is_testing_db(db.engine):
                conditions.append(
                    Word.search_text.op('@@')(
                        func.to_tsquery('filipino', search_text)
                    )
                )
        except Exception as e:
            current_app.logger.warning(f"Full-text search not available: {str(e)}")
        ...
        if params.get('pos'):
            query = query.join(Definition).join(PartOfSpeech).filter(
            ...
            if params['sort'] == 'alphabetical':
                ...
                if hasattr(Word, 'search_text') and not is_testing_db(db.engine):
                    query = query.order_by(
                        ...
                        func.similarity(Word.normalized_lemma, search_text).desc()
                    )
                else:
                    query = query.order_by(Word.lemma.asc())
        except Exception as e:
            current_app.logger.warning(f"Sorting error: {str(e)}")
            query = query.order_by(Word.lemma.asc())
