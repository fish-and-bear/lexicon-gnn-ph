...
@with_transaction(commit=False) # Manage commit manually within the function or via caller
def process_kaikki_jsonl(cur, filename: str) -> Tuple[int, int]: # Return (processed_count, issues_count)
    """Process Kaikki.org dictionary entries (JSONL format)."""
    # --- Initial setup (stats, source_id, line count, schema check - KEPT FROM ORIGINAL) ---
    stats = {
    ...
    error_summary = {}
    raw_source_identifier = os.path.basename(filename)
    source_identifier = standardize_source_identifier(raw_source_identifier)
    ...
    conn = cur.connection
    # Schema checks are assumed done by migrate command, but keep the originals for robustness if needed
    try:
        # Add CREATE TABLE IF NOT EXISTS and ALTER TABLE ADD COLUMN IF NOT EXISTS here if desired
        # Keeping them out for brevity as they were in the original function provided in the previous turn
        logger.debug("Schema checks/updates skipped in this version, assuming done by main migration.")
        # conn.commit() # Commit schema changes if any were made
    except Exception as e:
        logger.error(f"Error during schema setup/check for Kaikki processing: {str(e)}")
        conn.rollback()
        return 0, stats.get("total_entries", 1) # Mark all as issues

    # --- Instantiate Romanizer ---
    romanizer = BaybayinRomanizer()

    # --- Nested Helper Functions (kept inside as requested) ---

    # Relationship mapping (can be defined outside if preferred)
    RELATIONSHIP_TYPE_MAPPING = { # Keep or update as needed
    ...
    def get_standard_relationship_type(rel_type_raw):
        ...
        try:
            # Try matching enum value directly or via rel_value attribute
        ...
        except Exception: # Catch potential errors during enum access
    ...
    def extract_script_info(entry: Dict, script_tag: str, script_name_in_template: str) -> Tuple[Optional[str], Optional[str]]:
        """Extracts specific script form (Baybayin/Badlit) and explicit romanization."""
        script_form = None
        romanized = None
            ...
            for form_data in entry["forms"]:
                if isinstance(form_data, dict) and script_tag in form_data.get("tags", []):
                    ...
                    if form_text:
                        ...
                        # Add script-specific validation if needed
                        if cleaned_form:
                             script_form = cleaned_form
                             romanized = form_data.get("romanized") # Get explicit romanization
                             return script_form, romanized # Return first one found
            ...
            for template in entry["head_templates"]:
                if isinstance(template, dict) and "expansion" in template:
                    ...
                    if isinstance(expansion, str):
                        # Use more specific regex capturing valid script characters
                        ...
                        if match:
                            ...
                            if potential_script:
                                ...
                                romanized = None # Romanization usually not in expansion
    ...
    def standardize_entry_pos(pos_str: Optional[str]) -> str:
        # (Keep original implementation using global get_standard_code)
    ...
    def process_pronunciation(cur, word_id: int, entry: Dict, source_identifier: str) -> int:
        # (Keep original implementation, ensure it returns count)
        count = 0
        if "sounds" in entry and isinstance(entry["sounds"], list):
            for sound in entry["sounds"]:
                ...
                else: continue # Skip if no usable value found
                ...
                if pron_data.get("value"):
                    ...
                    except Exception as e: logger.warning(f"Error inserting pronunc. for word ID {word_id}: {e}")
        return count

    def process_sense_relationships(cur, word_id: int, sense: Dict, language_code: str, source_identifier: str, word_lemma: str) -> int:
        # (Keep original implementation, VERIFY 'derived' logic)
        relations_added = 0
        relation_types = {
            ...
            "derived": RelationshipType.ROOT_OF, # Ensure this maps correctly: word_id is ROOT_OF derived word
            ...
            if rel_key in sense and isinstance(sense[rel_key], list):
                for item in sense[rel_key]:
                    ...
                    if isinstance(item, dict) and "word" in item:
                        ...
                        if "tags" in item: metadata["tags"] = item["tags"]
                        if "qualifier" in item: metadata["qualifier"] = item["qualifier"]
                        # Add other metadata extraction...
                    elif isinstance(item, str): related_word = item
                    else: continue

                    related_word_clean = clean_html(related_word) # Clean HTML
                    ...
                    try:
                        ...
                        # Determine direction based on original key for non-bidirectional
                        from_id, to_id, current_rel_enum = word_id, related_word_id, rel_enum
                        if rel_key == "derived": # If the key was 'derived', the current word is the root
                            # Already set rel_enum = ROOT_OF, direction word_id -> derived_id is correct
                            pass
                        # Add other directional logic if needed (e.g., for hypernyms/hyponyms if source format differs)

                        rel_id_1 = insert_relation(cur, from_id, to_id, current_rel_enum, source_identifier, metadata)
                        if rel_id_1: relations_added += 1

                        if current_rel_enum.bidirectional:
                            insert_relation(cur, to_id, from_id, current_rel_enum, source_identifier, metadata)
                        else:
                            inverse_rel = current_rel_enum.get_inverse()
                            if inverse_rel:
                                insert_relation(cur, to_id, from_id, inverse_rel, source_identifier, metadata)
    ...
    def process_forms_storage(cur, word_id: int, entry: Dict, source_identifier: str) -> int:
        """Stores forms in word_forms table."""
        count = 0
        if "forms" in entry and isinstance(entry["forms"], list):
            for form_data in entry["forms"]:
                ...
                try:
                    cur.execute(
                        """INSERT INTO word_forms (word_id, form, is_canonical, tags) VALUES (%s, %s, %s, %s)
                           ON CONFLICT (word_id, form) DO UPDATE SET
                           is_canonical = EXCLUDED.is_canonical,
                           tags = COALESCE(word_forms.tags, '{}'::jsonb) || EXCLUDED.tags,
                           updated_at = CURRENT_TIMESTAMP""",
                        (word_id, form_text, is_canonical, tags_json)
                    )
    ...
    # --- Process a single entry (nested function) ---
    def process_entry(cur, entry: Dict, source_identifier: str, romanizer: BaybayinRomanizer) -> Tuple[Optional[int], Optional[str]]:
        ...
        lemma = word
        word_id = None
        error_messages = [] # Collect non-critical errors during processing
        ...
        try:
            ...
            # --- Script Extraction & Romanization (Enhanced) ---
            script_tag_found, script_form, script_romanized_explicit, badlit_form = None, None, None, None
            try:
                 # Check forms array first
                 forms_count, script_tag_found, script_form, script_romanized_explicit, badlit_form = process_forms(cur, 0, entry, source_identifier) # Dummy word_id 0 for initial check
                 stats["forms"] += forms_count # Add to total forms processed (read/parsed)

                 # Fallback to head_templates
                 if not script_form and "head_templates" in entry and isinstance(entry["head_templates"], list):
                     logger.debug(f"No script in forms for '{lemma}', checking head_templates.")
                     for template in entry["head_templates"]:
                         if isinstance(template, dict) and "expansion" in template:
                             expansion = template.get("expansion", "")
                             if isinstance(expansion, str):
                                 match_bay = re.search(r'Baybayin spelling\s+([\u1700-\u171F\s]+)', expansion, re.IGNORECASE)
                                 match_bad = re.search(r'Badlit spelling\s+([\u1700-\u171F\u1730-\u173F\s]+)', expansion, re.IGNORECASE) # Adjust range if needed

                                 found_script = None; script_tag = None
                                 if match_bad: found_script = match_bad.group(1).strip(); script_tag = "Badlit"
                                 elif match_bay: found_script = match_bay.group(1).strip(); script_tag = "Baybayin"

                                 if found_script and any(not ('a' <= char.lower() <= 'z') for char in found_script):
                                     script_form = found_script
                                     script_tag_found = script_tag
                                     script_romanized_explicit = None # No explicit romanization here
                                     logger.debug(f"Found {script_tag_found} form '{script_form}' in head_template expansion for '{lemma}'.")
                                     break # Found one, stop checking templates

                 # Generate Romanization if needed
                 script_romanized_final = None
                 if script_form:
                     stats["scripts"] += 1 # Count word having a script form
                 ...
                 logger.warning(f"Error during form/script processing for '{lemma}': {form_err}")
                 error_messages.append(f"Form/Script processing error: {form_err}")
            ...
            word_tags_str = ",".join(t for t in tags_list if isinstance(t, str)) if tags_list else None # Handle non-strings just in case
            ...
            try:
                word_id = get_or_create_word_id(
                    ...
                    # Add badlit_form if schema changed (Option B)
                    # badlit_form=badlit_form if script_tag_found == "Badlit" else None,
            ...
            # --- Process other word-level data (non-critical errors collected) ---
            ...
            try:
                 ety_id, rels_added = process_etymology(
                     cur, word_id, entry.get("etymology_text"), source_identifier,
                     entry.get("etymology_templates"), lemma, language_code
                 )
                 if ety_id: stats["etymologies"] += 1
                 if rels_added: stats["relations"] += rels_added # Add count from etymology helper
            except Exception as e: error_messages.append(f"Etymology/relation error: {e}")

            try:
                # Re-process forms to store in word_forms table with correct word_id
                process_forms_storage(cur, word_id, entry, source_identifier) # Use the renamed helper
            ...
            if "senses" in entry and isinstance(entry["senses"], list):
                for sense_idx, sense in enumerate(entry["senses"]):
                     ...
                     try:
                         ...
                         sense_pos_str = sense.get("pos") or pos_raw
                         # Standardize POS for DB insertion
                         standard_pos_id = get_standardized_pos_id(cur, sense_pos_str)
                         ...
                         try:
                             definition_id = insert_definition(
                                 ...
                                 source_identifier=source_identifier,
                                 part_of_speech=sense_pos_str, # Pass raw POS for original_pos column
                                 examples=examples_json,
                                 tags=tags_str,
                                 # Pass metadata if column exists and helper accepts it
                                 # metadata=Json(sense_metadata) if sense_metadata else None
                             )
                         ...
                         if definition_id:
                             ...
                             try:
                                 ...
                                 if rels_added: stats["relations"] += rels_added
                            ...
                            # --- Optional: Superseded Spelling Check ---
                             ...
                             if superseded_match:
                                 correct_spelling = superseded_match.group(1).strip("()") # Get the correct word
            ...
            if not error_messages:
                ...
                first_error = error_messages[0][:100]
                err_key = f"EntryNonCritError:{first_error}"
                error_summary[err_key] = error_summary.get(err_key, 0) + 1
                logger.debug(f"Entry {entry_count} ('{word}') processed with errors: {'; '.join(error_messages)}")

            cur.execute(f"RELEASE SAVEPOINT {savepoint_name}") # Commit entry
            ...
            logger.error(f"Failed processing entry #{entry_count} ('{lemma or 'unknown'}') in {filename}: {entry_err}", exc_info=True)
            stats["failed_entries"] += 1
            error_key = f"EntryCriticalError: {type(entry_err).__name__}"
            error_summary[error_key] = error_summary.get(error_key, 0) + 1
            try: cur.execute(f"ROLLBACK TO SAVEPOINT {savepoint_name}")
            except Exception as rb_err:
                logger.critical(f"CRITICAL: Failed rollback to savepoint {savepoint_name} after entry error: {rb_err}. Aborting file.", exc_info=True)
                raise entry_err from rb_err # Abort file processing
        finally:
            # Periodic commit
            ...
            if processed_count > 0 and processed_count % 1000 == 0:
                try:
                    conn.commit()
                    logger.info(f"Committed batch after {entry_count} lines processed for {filename}")
                except Exception as batch_commit_err:
                     logger.error(f"Error committing batch for {filename} at line {entry_count}: {batch_commit_err}. Rolling back...", exc_info=True)
                     conn.rollback()
                     error_summary["BatchCommitError"] = error_summary.get("BatchCommitError", 0) + 1
                     # Continue processing next entries with savepoints
    ...
    # --- Final Commit ---
    try:
       if not conn.closed: conn.commit()
       logger.info(f"Final commit for {filename} successful.")
    except Exception as final_commit_err:
       logger.error(f"Error during final commit for {filename}: {final_commit_err}. Attempting rollback.", exc_info=True)
       try:
           if not conn.closed: conn.rollback()
       except Exception as rb_final_err:
           logger.error(f"Final rollback failed for {filename}: {rb_final_err}")
       stats["failed_entries"] = stats.get("total_entries", entry_count) - stats["processed_ok"] - stats["processed_with_errors"] - stats["skipped_json_errors"]
    ...
    logger.info(f"  -> Relations Added: {stats['relations']}")
    ...
    if error_summary: logger.warning(f"  Error Summary: {json.dumps(error_summary)}")
