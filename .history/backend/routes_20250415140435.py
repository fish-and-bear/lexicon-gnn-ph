...
def get_word_by_id(word_id: int):
    ...
    try:
        ...
        # Fetch word details with full relationships
...
def get_random_word():
    ...
    try:
        ...
        with Session(db.engine) as session:
            ...
            # Get the word ID from the result
            word_id = word_result.id
        
        # Use a separate method for fetching detailed data with its own session
        def fetch_word_details_safely(word_id):
            ...
            with Session(db.engine) as new_session:
                try:
                    # Use the ORM to query the word and benefit from the session attachment
                    word = new_session.query(Word).filter(Word.id == word_id).first()
                    if not word:
                        return None
                        
                    # Load needed relationships
                    if word:
                        if hasattr(word, 'definitions') and word.definitions is None:
                            word.definitions = []
                        if hasattr(word, 'etymologies') and word.etymologies is None:
                            word.etymologies = []
                        if hasattr(word, 'pronunciations') and word.pronunciations is None:
                            word.pronunciations = []
                        if hasattr(word, 'credits') and word.credits is None:
                            word.credits = []
                        if hasattr(word, 'outgoing_relations') and word.outgoing_relations is None:
                            word.outgoing_relations = []
                        if hasattr(word, 'incoming_relations') and word.incoming_relations is None:
                            word.incoming_relations = []
                        if hasattr(word, 'root_affixations') and word.root_affixations is None:
                            word.root_affixations = []
                        if hasattr(word, 'affixed_affixations') and word.affixed_affixations is None:
                            word.affixed_affixations = []
                        if hasattr(word, 'derived_words') and word.derived_words is None:
                            word.derived_words = []
                        if hasattr(word, 'forms') and word.forms is None:
                            word.forms = []
                        if hasattr(word, 'templates') and word.templates is None:
                            word.templates = []
                    
                    # Manually load related data
                    new_session.execute(text("SELECT 1 FROM definitions WHERE word_id = :word_id LIMIT 1"), {"word_id": word_id})
                    new_session.execute(text("SELECT 1 FROM etymologies WHERE word_id = :word_id LIMIT 1"), {"word_id": word_id})
                    new_session.execute(text("SELECT 1 FROM pronunciations WHERE word_id = :word_id LIMIT 1"), {"word_id": word_id})
                    
                    # Add root word reference if needed
                    if word.root_word_id:
                        root = new_session.query(Word).filter(Word.id == word.root_word_id).first()
                        if root:
                            word.root_word = root
                    
                    # Clone and detach the word
                    new_session.expunge(word)
                    return word
        ...
        # Get word details using the ORM-based function
        full_word = fetch_word_details_safely(word_id)
        
        if not full_word:
            # Fallback to _fetch_word_details if the safe function failed
            full_word = _fetch_word_details(
                word_id,
                include_definitions=True,
                include_etymologies=True,
...
def get_word_affixations(word: str):
    ...
    try:
        ...
        schema = SchemasAffixationSchema(many=True)
        root_affix = schema.dump(word_entry.root_affixations)
        affixed_affix = schema.dump(word_entry.affixed_affixations)
