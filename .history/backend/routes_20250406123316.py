...
def search():
    ...
    try:
        ...
        # Due to schema mismatches, we can't safely load full details
        if include_full:
            logger.warning("include_full=true option disabled due to schema mismatches")
            include_full = False
            result["note"] = "Full details loading disabled due to schema mismatches. Use individual word endpoints for details."
        ...
        # Completeness filter is disabled because completeness_score is a hybrid property
        ...
        count_params = {
            ...
            # min_completeness removed as it's not used in SQL now
        }
        ...
        if sort_field == 'relevance':
            ...
            # Use lemma instead of completeness_score which is a hybrid property, not a database column
            main_sql += f" ORDER BY w.lemma {'DESC' if sort_order == 'desc' else 'ASC'}"
        else:
        ...
        if include_full:
            # Due to schema mismatches, we can't load full details
            # Return simplified results instead
            logger.warning("include_full=true option disabled due to schema mismatches")
            include_full = False
            result["note"] = "Full details loading disabled due to schema mismatches. Use individual word endpoints for details."
            ...
            if search_args.get('include_definitions', True):
                # Limit to columns that exist in the database
                include_options.append(
                    selectinload(Word.definitions).load_only(
                        Definition.id, Definition.word_id, Definition.definition_text,
                        Definition.original_pos, Definition.standardized_pos_id,
                        Definition.usage_notes, Definition.examples, Definition.tags,
                        Definition.sources, Definition.created_at, Definition.updated_at
                    ).selectinload(Definition.standardized_pos)
                )
                
                # Skip loading links due to column mismatch
                # include_options.append(
                #     selectinload(Word.definitions).selectinload(Definition.links).load_only(
                #         'id', 'definition_id', 'link_type', 'target_url', 'display_text',
                #         'is_external', 'created_at', 'updated_at'
                #     )
                # )
                
                include_options.append(
                    selectinload(Word.definitions).selectinload(Definition.categories).load_only(
                        'id', 'definition_id', 'category_name', 'category_kind',
            ...
            if search_args.get('include_definition_relations', False):
                # Don't load definition_relations directly due to schema mismatch
                # We'll include a message explaining this in the response
                result["definition_relations_note"] = "Definition relations loading disabled due to schema mismatch"
                # include_options.append(
                #     selectinload(Word.definition_relations).load_only(
                #         'id', 'definition_id', 'word_id', 'relation_type', 
                #         'sources', 'created_at', 'updated_at'
                #     )
                # )
                # include_options.append(
                #     selectinload(Word.related_definitions).load_only(
                #         Definition.id, Definition.word_id, Definition.definition_text,
                #         Definition.original_pos, Definition.standardized_pos_id,
                #         Definition.usage_notes, Definition.examples, Definition.tags,
                #         Definition.sources, Definition.created_at, Definition.updated_at
            ...
            result["results"] = [
                {
                    ...
                    # Use a default value for completeness_score in search results
