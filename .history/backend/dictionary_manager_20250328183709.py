...
RELATION_MAPPING = {
    ...
    "see_also": {
        "relation_type": "compare_with",  # Standardize to compare_with
        "bidirectional": True,
    ...
    "initial_abbreviation": {
        "relation_type": "initial_abbreviation",
        "bidirectional": True,
        "inverse": "full_form",
        "category": "variant"
    },
    ...
    "misspelling": {
        "relation_type": "misspelling",
        "bidirectional": True,
        "inverse": "correct_spelling",
        "category": "variant"
    },
    "regional": {
        "relation_type": "regional",
        "bidirectional": True,
        "inverse": "standard",
        "category": "variant"
    },
    ...
    "full_form": {
        "relation_type": "full_form",
        "bidirectional": True,
        "inverse": "abbreviation",
        "category": "other"
    },
    "correct_spelling": {
        "relation_type": "correct_spelling",
        "bidirectional": True,
        "inverse": "misspelling",
        "category": "other"
    },
    ...
    "current_spelling": {
        "relation_type": "current_spelling",
        "bidirectional": True,
        "inverse": "obsolete_spelling",
        "category": "other"
    }
}

def normalize_relation_type(relation_type: str) -> Tuple[str, bool, Optional[str]]:
    """
    Normalize a relationship type and return its standardized form,
    bidirectionality, and inverse type.
    """
    relation_info = RELATION_MAPPING.get(relation_type.lower(), {})
    std_relation_type = relation_info.get("relation_type", relation_type.lower())
    is_bidirectional = relation_info.get("bidirectional", False)
    inverse_type = relation_info.get("inverse", None)
    return std_relation_type, is_bidirectional, inverse_type

def detect_variant_type(source_word: str, target_word: str, lang_code: str = "tl") -> Optional[str]:
    """
    Detects the type of variant relationship between two words.
    
    This function analyzes word pairs to determine the specific variant type, such as:
    - abbreviation
    - misspelling
    - regional form
    - etc.
    ...
    Args:
        ...
        target_word: The target/variant word
        lang_code: Language code (default: "tl" for Tagalog)
        
    Returns:
        A string representing the detected variant type or None if not determinable
    """
    if not source_word or not target_word:
        return None
        
    # Normalize words for comparison
    source_norm = normalize_lemma(source_word).strip()
    target_norm = normalize_lemma(target_word).strip()
    ...
    # 1. Initialisms (Philippine -> PH)
    if len(target_norm) <= 5 and all(c.isalpha() for c in target_norm):
        # Check if target consists of initials from source
        words = source_norm.split()
        if len(words) >= 2:
            initials = ''.join(word[0] for word in words if word)
            if target_norm.lower() == initials.lower():
                return "initial_abbreviation"
    
    # 2. Common abbreviation patterns
    if len(target_norm) < len(source_norm):
        # Check if it's a truncated form (first few letters)
        if source_norm.startswith(target_norm) and len(target_norm) >= 3:
            return "abbreviation"
            
        # Check for shortened form (remove vowels)
        source_novowels = ''.join(c for c in source_norm if c.lower() not in 'aeiou')
        target_novowels = ''.join(c for c in target_norm if c.lower() not in 'aeiou')
        if source_novowels == target_novowels:
            return "abbreviation"
            
        # Check for acronym
        words = source_norm.split()
        if len(words) >= 2:
            # True acronym (using first letters)
            acronym = ''.join(word[0] for word in words if word)
            if target_norm.lower() == acronym.lower():
                return "acronym"
                
            # Check for partial acronyms (some letters from each word)
            if len(target_norm) <= 6 and all(any(letter in word.lower() for letter in target_norm.lower()) for word in words):
                return "acronym"
    
    # Simple Levenshtein distance approximation for misspellings
    # If words are similar but with small changes
    if abs(len(source_norm) - len(target_norm)) <= 2:
        diff_count = sum(1 for a, b in zip(source_norm[:min(len(source_norm), len(target_norm))], 
                                           target_norm[:min(len(source_norm), len(target_norm))]) if a != b)
        diff_count += abs(len(source_norm) - len(target_norm))
        
        if diff_count <= 2:
            return "misspelling"
    
    # Regional variant detection (could be enhanced with specific regional patterns)
    # This is a simple approximation - real detection would require more linguistic knowledge
    regional_markers = ["tagalog", "cebuano", "bisaya", "ilokano", "iloko", "ilocano", "waray", "kapampangan", 
                       "hiligaynon", "bikol", "bikolano", "southern", "northern", "metro manila", "manila"]
    
    # Check if any regional markers appear in either word
    for marker in regional_markers:
        if marker in source_norm or marker in target_norm:
            return "regional"
    
    # If no specific type detected, return generic variant
    return "variant"

def create_word_relationship(cur, from_word_id: int, to_word_id: int, relation_type: str, 
                             sources: str = "", metadata: Dict = None):
    """
    Creates a word relationship with automated handling of bidirectionality
    and inverse relationships based on the relation type.
    
    This function simplifies the creation of relationships between words by:
    1. Normalizing the relationship type
    2. Automatically creating inverse relationships for bidirectional types
    3. Ensuring consistent metadata formatting
    
    Args:
        cur: Database cursor
        from_word_id: ID of the source word
        to_word_id: ID of the target word
        relation_type: Type of relationship (will be normalized)
        sources: Comma-separated list of data sources
        metadata: Optional dictionary of additional metadata to store
    """
    # Check for self-referential relationships
    if from_word_id == to_word_id:
        logger.warning(f"Attempted to create self-referential relationship of type {relation_type}")
        return
        
    # Normalize the relationship type
    std_relation_type, is_bidirectional, inverse_type = normalize_relation_type(relation_type)
    
    # Insert the primary relationship
    insert_relation(cur, from_word_id, to_word_id, std_relation_type, sources, metadata)
    
    # For bidirectional relationships, also create the inverse relationship
    if is_bidirectional and inverse_type:
        # Create inverse metadata if needed
        inverse_metadata = None
        if metadata:
            inverse_metadata = metadata.copy()
            if 'direction' in inverse_metadata:
                inverse_metadata['direction'] = 'inverse'
        
        # Insert the inverse relationship
        insert_relation(cur, to_word_id, from_word_id, inverse_type, sources, inverse_metadata)

# Main function and CLI commands follow
def main():
...
@with_transaction(commit=True)
def create_word_relationship(cur, from_word_id: int, to_word_id: int, relation_type: str, sources: str = "", metadata: Dict = None):
    """
    Create a relationship between words, automatically handling bidirectionality based on the relationship type.
    
    This is a higher-level wrapper around insert_relation that:
    1. Standardizes the relation_type using normalize_relation_type
    2. Automatically creates the inverse relationship if the relationship is bidirectional
    3. Ensures consistent metadata format
    ...
    if is_bidirectional and inverse_type:
        # For bidirectional relationships, we might want to share metadata
        # or have slightly different metadata for the inverse
        inverse_metadata = metadata.copy() if metadata else {}
        inverse_metadata["inverse_of"] = std_relation_type
        
        insert_relation(cur, to_word_id, from_word_id, inverse_type, sources, inverse_metadata)

def detect_variant_type(cur, word1: str, word2: str, context: str = None) -> str:
    """
    Detect and classify the type of variant relationship between two words.
    
    This function analyzes word pairs to determine the specific variant type
    (abbreviation, misspelling, regional form, etc.) which can then be used
    to create more specific metadata when creating word relationships.
    
    Args:
        cur: Database cursor
        word1: First word 
        word2: Second word
        context: Optional context string that might provide clues about relationship
        
    Returns:
        Variant subtype (e.g., "abbreviation", "regional", "misspelling", etc.)
    """
    # If one word is substantially shorter than the other, it might be an abbreviation
    if len(word1) <= 3 and len(word2) > 5 and word1.lower() == word2.lower()[:len(word1)]:
        return "abbreviation"
    elif len(word2) <= 3 and len(word1) > 5 and word2.lower() == word1.lower()[:len(word2)]:
        return "abbreviation"
        
    # Check for common abbreviation patterns
    if re.match(r'^[A-Za-z]\.?$', word1) and word2.lower().startswith(word1[0].lower()):
        return "initial_abbreviation"
    elif re.match(r'^[A-Za-z]\.?$', word2) and word1.lower().startswith(word2[0].lower()):
        return "initial_abbreviation"
        
    # Check for acronyms
    if (len(word1) <= 5 and len(word2) > len(word1) * 2 and 
        all(w[0].lower() in word1.lower() for w in word2.split())):
        return "acronym"
    elif (len(word2) <= 5 and len(word1) > len(word2) * 2 and 
          all(w[0].lower() in word2.lower() for w in word1.split())):
        return "acronym"
    
    # Check for misspellings (small edit distance)
    w1_norm = normalize_lemma(word1)
    w2_norm = normalize_lemma(word2)
    
    # Simple Levenshtein distance approximation for detecting possible misspellings
    edit_distance = sum(1 for a, b in zip(w1_norm, w2_norm) if a != b)
    edit_distance += abs(len(w1_norm) - len(w2_norm))
    
    if edit_distance == 1 and len(w1_norm) > 3 and len(w2_norm) > 3:
        return "misspelling"
    elif edit_distance <= 2 and len(w1_norm) > 5 and len(w2_norm) > 5:
        return "misspelling"
        
    # Check for regional variants using context clues
    if context:
        regional_indicators = ['regional', 'dialectal', 'dialect', 'province', 'local', 
                               'rehiyon', 'rehiyonal', 'diyalekto']
        if any(indicator in context.lower() for indicator in regional_indicators):
            return "regional"
            
    # Check database for existing clues from other entries
    try:
        cur.execute("""
            SELECT r.metadata 
            FROM relations r
            ...
            WHERE r.relation_type = 'variant'
            AND (
                (w1.lemma ILIKE %s AND w2.lemma ILIKE %s)
                OR
                (w1.lemma ILIKE %s AND w2.lemma ILIKE %s)
            )
        ...
        for row in results:
            if row[0] and isinstance(row[0], dict) and 'subtype' in row[0]:
                return row[0]['subtype']
    except Exception as e:
        logger.warning(f"Error checking for existing variant types: {e}")
    
    # Default to generic variant if no specific type is determined
    return "variant"

@with_transaction(commit=True)
def create_variant_relationship(cur, from_word_id: int, to_word_id: int, 
                               subtype: str = None, context: str = None, 
                               sources: str = "", metadata: Dict = None):
    """
    Create a variant relationship between words with enhanced variant classification.
    
    This function builds on create_word_relationship but specializes in variant
    relationships with automatic subtype detection and appropriate metadata.
    
    Args:
        cur: Database cursor
        from_word_id: ID of the source word
        to_word_id: ID of the target word
        subtype: Optional explicit variant subtype
        context: Optional context that might help determine variant type
        sources: Source information
        metadata: Optional metadata to include
    """
    if from_word_id == to_word_id:
        logger.warning(f"Attempted to create self-referential variant relationship")
        return
        
    # Get the word forms to help determine variant type if not provided
    if not subtype or context:
        cur.execute("SELECT lemma FROM words WHERE id = %s", (from_word_id,))
        from_word = cur.fetchone()[0] if cur.rowcount > 0 else ""
        
        cur.execute("SELECT lemma FROM words WHERE id = %s", (to_word_id,))
        to_word = cur.fetchone()[0] if cur.rowcount > 0 else ""
        
        if not subtype:
            subtype = detect_variant_type(cur, from_word, to_word, context)
    
    # Prepare metadata
    if metadata is None:
        metadata = {}
    
    # Add variant subtype to metadata
    if subtype and subtype != "variant":
    ...
    # Create the relationship (bidirectional handling is done in create_word_relationship)
    create_word_relationship(cur, from_word_id, to_word_id, "variant", sources, metadata)
...
def extract_sense_relations(cur, word_id, sense, lang_code, source):
    ...
    try:
        ...
        logger.error(f"Error opening or reading file {filename}: {str(ex)}")
        raise
