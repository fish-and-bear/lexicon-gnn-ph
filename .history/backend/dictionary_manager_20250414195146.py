...
# Add 'ijson' to your imports near the top if not already present
# import ijson

# Replace the existing _process_single_tagalog_word_entry function (around line 6531)
# with this corrected version:
@with_db_transaction # Correct decorator
def _process_single_tagalog_word_entry(cur, entry_data: dict, word_key: str, source_identifier: str, language_code: str, add_etymology: bool, word_id_cache: dict, stats: dict, error_types: dict, table_column_map: dict, relations_batch: list, required_columns: dict) -> bool: # Added cur and required_columns
    """
    Processes a single entry from the tagalog-words.json file within its own transaction.
    Returns True if processed successfully (even with minor non-critical errors), False on critical failure.
    """
    item_word = entry_data.get("word", word_key)
    # Ensure item_word is a string before stripping
    if isinstance(item_word, str):
        item_word = item_word.strip()
    else:
        item_word = str(item_word).strip() # Fallback if not string

    if not item_word:
        # Try using the key if the 'word' field was empty or missing
        if isinstance(word_key, str):
             item_word = word_key.strip()
        else:
             item_word = str(word_key).strip()


    if not item_word:
        logger.warning(f"Skipping entry with key '{word_key}': derived word is empty")
        stats["skipped"] += 1
        return True # Not a critical failure

    # Collect domains to add to word metadata
    domains = []
    raw_domains = entry_data.get("domains", [])
    if raw_domains and isinstance(raw_domains, list):
        for domain in raw_domains:
            if isinstance(domain, str) and domain.strip():
                domains.append(domain.strip())

    word_metadata_update = {}
    if domains:
        # Store domains under a specific key in metadata
        word_metadata_update["domains"] = domains

    try:
        # Get or create word entry, passing collected domains
        # CRITICAL: Assumes the get_or_create_word_id being called HAS the number stripping removed.
        word_id = get_or_create_word_id(
            cur,
            item_word, # Pass the original, un-stripped lemma
            language_code=language_code,
            source_identifier=source_identifier,
            word_metadata=word_metadata_update # Pass domains here
        )
        if not word_id:
             # This is critical, cannot proceed without a word_id
             raise ValueError(f"Failed to get or create word ID for '{item_word}'")
        # Use normalized lemma for cache key consistency with get_or_create_word_id logic
        normalized_key = normalize_lemma(item_word)
        word_id_cache[f"{normalized_key}|{language_code}"] = word_id # Update cache immediately
        ...
        if isinstance(pronunciation, str):
             ...
             pronunciation = None # Ignore non-string pronunciations

        # Check if required pronunciation columns exist in the actual table map
        if pronunciation and "pronunciations" in required_columns and set(required_columns["pronunciations"]).issubset(table_column_map.get("pronunciations", set())):
            try:
                # Assuming insert_pronunciation exists and takes (cur, word_id, value, source)
                pron_id = insert_pronunciation(cur, word_id, pronunciation, source_identifier)
                if pron_id:
                    stats["pronunciations_added"] += 1
            except psycopg2.errors.UniqueViolation:
                 logger.debug(f"Pronunciation already exists for word ID {word_id}: '{pronunciation}'. Skipping.")
                 # Optionally handle ON CONFLICT DO NOTHING inside insert_pronunciation
            except Exception as e:
                logger.warning(f"Non-critical: Error adding pronunciation for word '{item_word}': {e}")
                error_types[f"PronunciationError: {type(e).__name__}"] = error_types.get(f"PronunciationError: {type(e).__name__}", 0) + 1
        elif pronunciation:
             logger.debug(f"Skipping pronunciation for '{item_word}' due to missing required columns in 'pronunciations' table.")
        ...
        # Check if required etymology columns exist
        if add_etymology and etymology and "etymologies" in required_columns and set(required_columns["etymologies"]).issubset(table_column_map.get("etymologies", set())):
            try:
                # Assuming insert_etymology exists and takes (cur, word_id, text, source)
                etym_id = insert_etymology(cur, word_id, etymology, source_identifier)
                if etym_id:
                    stats["etymologies_added"] += 1
            except psycopg2.errors.UniqueViolation:
                logger.debug(f"Etymology already exists for word ID {word_id}. Skipping.")
            except Exception as e:
                logger.warning(f"Non-critical: Error adding etymology for word '{item_word}': {e}")
                error_types[f"EtymologyError: {type(e).__name__}"] = error_types.get(f"EtymologyError: {type(e).__name__}", 0) + 1
        elif add_etymology and etymology:
            logger.debug(f"Skipping etymology for '{item_word}' due to missing required columns in 'etymologies' table.")


        # --- Process Cross References (add to batch for later) ---
            ...
            for ref in cross_refs:
                if isinstance(ref, str) and ref.strip():
                    relations_batch.append({
                        'word_id': word_id,
                        'related_word': ref.strip(),
                        'rel_type': RelationshipType.SEE_ALSO, # Use enum
                        'source_identifier': source_identifier
                    })
                elif ref:
                     logger.warning(f"Non-string cross_reference found for '{item_word}': {ref}. Skipping.")

        # --- Process Definitions ---
        definitions_data = entry_data.get("definitions", {})
        # Check if required definition columns exist and data is a dict
        if isinstance(definitions_data, dict) and "definitions" in required_columns and set(required_columns["definitions"]).issubset(table_column_map.get("definitions", set())):
            for pos, def_list in definitions_data.items():
                if not isinstance(def_list, list):
                    logger.warning(f"Expected list for POS '{pos}' definitions in '{item_word}', got {type(def_list)}. Skipping.")
                    continue
                ...
                for def_item in def_list:
                    ...
                    if isinstance(def_item, str):
                        ...
                        if isinstance(definition_text, str):
                            ...
                            definition_text = "" # Ignore non-string definitions

                        raw_examples = def_item.get("examples", [])
                        if raw_examples and isinstance(raw_examples, list):
                             examples_list = [ex.strip() for ex in raw_examples if isinstance(ex, str) and ex.strip()]

                        raw_synonyms = def_item.get("synonyms", [])
                        if isinstance(raw_synonyms, list):
                            synonyms = [syn.strip() for syn in raw_synonyms if isinstance(syn, str) and syn.strip()]

                        raw_antonyms = def_item.get("antonyms", [])
                        if isinstance(raw_antonyms, list):
                             antonyms = [ant.strip() for ant in raw_antonyms if isinstance(ant, str) and ant.strip()]
                    else:
                        logger.warning(f"Unexpected item type in definition list for '{item_word}', POS '{pos}': {type(def_item)}. Skipping.")
                        continue # Skip this definition item

                    if not definition_text:
                        continue # Skip empty definitions

                    # --- Add definition using insert_definition ---
                    try:
                        # Format examples list into a single string for the TEXT column
                        examples_text = "\n".join(examples_list) if examples_list else None

                        # Assuming insert_definition handles POS mapping and takes 'examples' as TEXT
                        def_id = insert_definition(
                            cur,
                            word_id,
                            definition_text,
                            ...
                            # Add other fields like usage_notes, tags, metadata if insert_definition supports them
                        )
                        if def_id:
                            stats["definitions_added"] += 1

                            # --- Add synonyms/antonyms to relations batch ---
                            for syn in synonyms:
                                relations_batch.append({
                                    'word_id': word_id, 'related_word': syn,
                                    'rel_type': RelationshipType.SYNONYM, 'def_id': def_id,
                                    'source_identifier': source_identifier
                                })
                            for ant in antonyms:
                                relations_batch.append({
                                    'word_id': word_id, 'related_word': ant,
                                    'rel_type': RelationshipType.ANTONYM, 'def_id': def_id,
                                    'source_identifier': source_identifier
                                })
                        else:
                             # Log if insert_definition indicates failure (e.g., conflict or other error returning None)
                             logger.debug(f"Non-critical: insert_definition returned no ID for '{item_word}', POS '{pos}'. Definition text: {definition_text[:50]}...")
                             error_types["DefinitionInsertSkippedOrFailed"] = error_types.get("DefinitionInsertSkippedOrFailed", 0) + 1

                    except psycopg2.errors.UniqueViolation:
                         logger.debug(f"Definition already exists for word ID {word_id}, POS '{pos}'. Skipping.")
                    except Exception as e:
                        logger.warning(f"Non-critical: Error adding definition for word '{item_word}', POS '{pos}': {e}", exc_info=True)
                        error_types[f"DefinitionError: {type(e).__name__}"] = error_types.get(f"DefinitionError: {type(e).__name__}", 0) + 1
        elif isinstance(definitions_data, dict) and definitions_data:
             logger.debug(f"Skipping definitions for '{item_word}' due to missing required columns in 'definitions' table.")
        elif definitions_data: # Log if definitions data exists but isn't a dict
              logger.warning(f"Expected 'definitions' to be a dict for word '{item_word}', got {type(definitions_data)}. Skipping definitions.")


        # --- Process capitalized word variant (add to batch) ---
        if isinstance(item_word, str) and len(item_word) > 1 and not item_word[0].isupper() and item_word[0].isalpha():
            try:
                capitalized_word = item_word[0].upper() + item_word[1:]
                # Add to batch for relation processing later
                relations_batch.append({
                    'word_id': word_id,
                    'related_word': capitalized_word,
                    'rel_type': RelationshipType.VARIANT, # Or a specific "capitalized_variant" if desired
                    'metadata': {'is_capitalization': True}, # Add metadata
                    'source_identifier': source_identifier
                })
            except Exception as cap_err:
                logger.warning(f"Non-critical: Error preparing capitalized variant for '{item_word}': {cap_err}")
                error_types[f"CapitalizedVariantError: {type(cap_err).__name__}"] = error_types.get(f"CapitalizedVariantError: {type(cap_err).__name__}", 0) + 1

        return True # Processed successfully (or with non-critical errors)

    except Exception as e:
        # Catch critical errors (like word creation failure) or unexpected errors within this entry's transaction
        logger.error(f"CRITICAL error processing entry for '{item_word}' (key: '{word_key}'): {e}", exc_info=True)
        stats["errors"] += 1
        error_types[f"CriticalEntryError: {type(e).__name__}"] = error_types.get(f"CriticalEntryError: {type(e).__name__}", 0) + 1
        return False # Indicate critical failure for this entry


# Replace the existing process_tagalog_words function (around line 6720)
# with this corrected version:
def process_tagalog_words(cur, filename: str, add_etymology: bool = True): # `cur` argument is unused due to helper decorator
    """
    Process a JSON file containing Tagalog words and their definitions using entry-level transactions
    and iterative JSON parsing.

    Args:
        cur: Database cursor (unused, handled by helper's decorator)
    ...
    """
    logger.info(f"Processing Tagalog words file: {filename} with entry-level transactions.")
    ...
    try:
        # Ensure the get_connection() used here is the corrected one without set_session
        # Or better, ensure get_connection() is intended for non-pooled connections if it exists
        # Assuming get_connection() provides a standalone connection here.
        ...
        # REMOVE release_db_connection(temp_conn) from here if it exists
        ...
        # Ensure the temporary connection is closed, regardless of success/failure
        if temp_conn:
            try:
                temp_conn.close() # <-- Make sure this line uses .close()
    ...
    try:
        ...
        try:
            ...
            logger.error("Module 'ijson' not found. Please install it (`pip install ijson`) to process large JSON files efficiently.")
        ...
        try:
            ...
            # Assuming top level is a dictionary; 'item' iterates over key-value pairs
            entries_iterator = ijson.kvitems(file_handle, '')
            logger.info(f"Iterating through entries in {filename}...")
        except FileNotFoundError:
             logger.error(f"File not found: {filename}")
             if file_handle: file_handle.close()
             return stats
        except Exception as e: # Catch ijson specific errors too
             logger.error(f"Error loading or parsing JSON file {filename}: {e}", exc_info=True)
             if file_handle: file_handle.close()
             stats["errors"] += 1
             error_types[f"CriticalFileError: {type(e).__name__}"] = error_types.get(f"CriticalFileError: {type(e).__name__}", 0) + 1
             return stats

        # Process each entry
        language_code = "tl"
        processed_count_log_interval = 1000 # Log progress every N entries
        ...
        for entry_index, (word_key, entry_data) in enumerate(entries_iterator):
            ...
            # Call the helper function (handles its own transaction)
            try:
                success = _process_single_tagalog_word_entry(
                ...
                # Critical errors inside helper increment stats["errors"]

            except Exception as helper_call_err:
                 # Catch unexpected errors *calling* the helper
                 logger.error(f"FATAL: Error occurred calling processing function for entry {entry_index} ('{word_key}'): {helper_call_err}", exc_info=True)
                 stats["errors"] += 1
                 error_types[f"HelperCallError: {type(helper_call_err).__name__}"] = error_types.get(f"HelperCallError: {type(helper_call_err).__name__}", 0) + 1
                 # Depending on severity, might want to break or continue
                 # For now, continue to next entry

            # Log progress periodically
            current_processed = stats["entries_processed"] + stats["skipped"] + stats["errors"]
            if current_processed % processed_count_log_interval == 0 and current_processed > 0:
                 logger.info(f"Processed ~{current_processed} entries...")
        
        # Ensure file handle is closed
        if file_handle:
             file_handle.close()

        # --- Process Relations Batch ---
        ...
        if relations_batch:
            try:
                ...
                def _process_relations_batch_transactional(cur, batch, stats_dict, cache):
                    processed_count = 0; failed_count = 0
                    lang_code_for_related = "tl"
                         ...
                         try:
                              ...
                              if not related_word_id:
                                   ...
                                   if related_word_id:
                                        ...
                                        logger.warning(f"Failed get/create related word '{related_word_lemma}' for relation. Skipping.")
                              ...
                              if related_word_id and related_word_id != from_id:
                                   ...
                                   if rel_id:
                                        ...
                                        if rel_type.bidirectional: insert_relation(cur, related_word_id, from_id, rel_type, src_id, metadata)
                                        else:
                                             inverse_rel = rel_type.get_inverse()
                                             if inverse_rel: insert_relation(cur, related_word_id, from_id, inverse_rel, src_id, metadata)
                                   # else: logger.debug(...) # Optionally log skips/conflicts

                         except psycopg2.errors.UniqueViolation:
                              logger.debug(f"Relation batch item skipped due to unique constraint: {relation_info}")
                         except Exception as rel_proc_err:
                              logger.warning(f"Failed relation batch item: {relation_info} - Error: {rel_proc_err}", exc_info=True)
                              failed_count += 1

                    logger.info(f"Processed {processed_count} relations in batch. {failed_count} failures/skips.")
                    return stats_dict

                # Call the batch processor
                stats = _process_relations_batch_transactional(relations_batch, stats, word_id_cache)

            except Exception as final_rel_err:
                logger.error(f"CRITICAL error during final relations batch processing: {final_rel_err}", exc_info=True)
                stats["errors"] += 1 # Increment overall error count
                error_types["RelationsBatchError"] = error_types.get("RelationsBatchError", 0) + 1

    except ImportError:
        # Error logged above, just update stats
        stats["errors"] += 1
        error_types["CriticalFileError: ImportError"] = 1
    except Exception as e:
        # Catch other errors during file loading or main loop setup
        logger.error(f"Critical error processing file {filename}: {e}", exc_info=True)
        stats["errors"] += 1
        error_types[f"CriticalFileError: {type(e).__name__}"] = error_types.get(f"CriticalFileError: {type(e).__name__}", 0) + 1
        # Ensure file handle closed if open
    ...
    logger.info(f"  Entries failed (critical errors): {stats['errors']}")
