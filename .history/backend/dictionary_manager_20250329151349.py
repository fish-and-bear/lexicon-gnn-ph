...
def process_kwf_dictionary(cur, filename: str):
    ...
    # Add proper exception handling for file reading
    try:
        ...
        for entry in tqdm(data, desc="Processing KWF Dictionary (list format)"):
            try:
                ...
                if word and len(word) > 255:
                    word_summary = word[:250] + "..."
                    logger.warning(f"Word exceeds maximum length (255): {word_summary}")
                    stats["skipped_entries"] += 1
                    continue
                ...
                try:
                    ...
                    if word_id is None or word_id <= 0:
                        logger.warning(f"Failed to create or retrieve word ID for '{word}'")
                        stats["skipped_entries"] += 1
                        continue
                    ...
                    logger.error(f"Database error processing entry {entry.get('word', 'unknown')}: {error_msg}")
    ...
    # Dictionary that maps custom relationship types to standard ones
    RELATION_TYPE_MAPPING = {
        "examples": None,  # Explicitly mark as not a relation type
        "cognate_with": "related",
        "borrowed_from": "derived_from",
        "loanword_from": "derived_from",
        "source_language": "derived_from",
        "cognates": "related",
        "dialectal_variant": "variant",
        "variant_of": "variant",
        "alternate_form": "variant",
        "regional_variant": "variant",
        "spelling_variant": "variant",
        "informal_form": "variant",
        "formal_equivalent": "related",
        "standard_form": "variant"
    }
    ...
    for word, entry in tqdm(data.items(), desc="Processing KWF Dictionary"):
        ...
        try:
            ...
            # Check if word exceeds maximum length and skip if it does
            if formatted_word and len(formatted_word) > 255:
                word_summary = formatted_word[:250] + "..."
                logger.warning(f"Formatted word exceeds maximum length (255): {word_summary}")
                stats["skipped_entries"] += 1
                continue
                
            logger.debug(f"Processing entry for word: '{word}' (formatted as '{formatted_word}')")
            
            # Start a subtransaction for this word
            try:
                ...
                # Verify that we got a valid word ID
                if word_id is None or word_id <= 0:
                    logger.warning(f"Failed to create or retrieve word ID for '{formatted_word}'")
                    stats["skipped_entries"] += 1
                    continue
                
                # Collect all examples from the entry to add to definitions later
                all_examples = []
                
                # Extract examples from related section if they exist
                if 'related' in entry and isinstance(entry['related'], dict) and 'examples' in entry['related']:
                    example_items = entry['related']['examples']
                    if isinstance(example_items, list):
                        for ex_item in example_items:
                            if isinstance(ex_item, str):
                                all_examples.append(ex_item)
                            elif isinstance(ex_item, dict) and 'value' in ex_item:
                                all_examples.append(ex_item['value'])
                ...
                if 'metadata' in entry:
                    ...
                    if 'etymology' in entry['metadata'] and entry['metadata']['etymology']:
                        ...
                        for etym in entry['metadata']['etymology']:
                            if isinstance(etym, dict) and 'value' in etym:
                                if etymology_text:
                                    etymology_text += ", "
                                etymology_text += etym['value']
                            elif isinstance(etym, str):
                                if etymology_text:
                                    etymology_text += ", "
                                etymology_text += etym
                        ...
                        if etymology_text:
                            ...
                            try:
                                ...
                                language_codes = ""
                                if 'source_language' in entry['metadata'] and entry['metadata']['source_language']:
                                    language_codes = ", ".join([
                                        sl.get('value', '') if isinstance(sl, dict) and 'value' in sl else str(sl)
                                        for sl in entry['metadata']['source_language']
                                        if (isinstance(sl, dict) and 'value' in sl) or isinstance(sl, str)
                                    ])
                                
                                # Double check word_id before inserting
                                ...
                                logger.warning(f"Error inserting etymology for '{formatted_word}': {str(etym_error)}")
                                error_types[str(etym_error)] = error_types.get(str(etym_error), 0) + 1
                ...
                if 'definitions' in entry and isinstance(entry['definitions'], dict):
                    ...
                    for pos, def_list in entry['definitions'].items():
                        if not isinstance(def_list, list):
                        ...
                        for def_entry in def_list:
                            if not isinstance(def_entry, dict):
                                logger.warning(f"Skipping invalid definition entry for '{formatted_word}': not a dictionary")
                                continue
                            ...
                            category_tags = ""
                            if 'categories' in def_entry and isinstance(def_entry['categories'], list):
                                category_tags = ", ".join([
                                    f"category:{cat}" for cat in def_entry['categories']
                                    if isinstance(cat, str)
                                ])
                            
                            # Collect example sets - Process examples properly, handling different formats
                            examples_text = ""
                            def_examples = []
                            
                            # Process example_sets from definition entry
                            if 'example_sets' in def_entry and isinstance(def_entry['example_sets'], list):
                                for example_set in def_entry['example_sets']:
                                    if isinstance(example_set, dict) and 'examples' in example_set and isinstance(example_set['examples'], list):
                                        # Process each example, handling both string and dict formats
                                        for ex in example_set['examples']:
                                            if isinstance(ex, str):
                                                def_examples.append(ex)
                                            elif isinstance(ex, dict):
                                                # First check for common fields in example dictionaries
                                                if 'text' in ex and isinstance(ex['text'], str):
                                                    def_examples.append(ex['text'])
                                                elif 'value' in ex and isinstance(ex['value'], str):
                                                    def_examples.append(ex['value'])
                                                elif 'example' in ex and isinstance(ex['example'], str):
                                                    def_examples.append(ex['example'])
                                                # Extract any other string values if we still don't have text
                                                else:
                                                    # Try to get any string values from the dict
                                                    for key, val in ex.items():
                                                        if isinstance(val, str) and val:
                                                            def_examples.append(f"{key}: {val}")
                                                            break
                            
                            # Add all collected examples for this word
                            all_def_examples = def_examples + all_examples
                            if all_def_examples:
                                examples_text = "\n".join(all_def_examples)
                                stats["examples_added"] += len(all_def_examples)
                                            
                            # Add usage notes
                            usage_notes = def_entry.get('note', '')
                            if not usage_notes and 'notes' in def_entry and isinstance(def_entry['notes'], list):
                                usage_notes = "; ".join([
                                    note.get('value', '') if isinstance(note, dict) and 'value' in note else str(note)
                                    for note in def_entry['notes']
                                    if (isinstance(note, dict) and 'value' in note) or isinstance(note, str)
                                ])
                            ...
                            try:
                                ...
                                if word_id and word_id > 0:
                                    ...
                                    if definition_id:
                                        ...
                                        if def_entry.get('see') and isinstance(def_entry['see'], str):
                                            see_reference = def_entry['see'].strip()
                                            if len(see_reference) <= 255:
                                                try:
                                                    # First verify the definition_id exists
                                                    cur.execute("SELECT EXISTS(SELECT 1 FROM definitions WHERE id = %s)", (definition_id,))
                                                    if not cur.fetchone()[0]:
                                                        logger.warning(f"Definition ID {definition_id} does not exist")
                                                        continue
                                                        
                                                    see_id = get_or_create_word_id(cur, see_reference, language_code='tl')
                                                    if see_id and see_id > 0:
                                                        # Verify the see_id word exists in the database
                                                        cur.execute("SELECT EXISTS(SELECT 1 FROM words WHERE id = %s)", (see_id,))
                                                        if cur.fetchone()[0]:
                                                            ...
                                                            logger.warning(f"See reference word ID {see_id} does not exist in the database")
                                                except Exception as e:
                                                    logger.warning(f"Error processing 'see' reference '{see_reference}' for '{formatted_word}': {str(e)}")
                                                    error_types[str(e)] = error_types.get(str(e), 0) + 1
                                        
                                        # Process cross-references
                                        if 'cross_references' in def_entry and isinstance(def_entry['cross_references'], list):
                                            for cross_ref in def_entry['cross_references']:
                                                cross_ref_term = ""
                                                if isinstance(cross_ref, dict) and 'value' in cross_ref:
                                                    cross_ref_term = cross_ref['value']
                                                elif isinstance(cross_ref, str):
                                                    cross_ref_term = cross_ref
                                                    
                                                if cross_ref_term and len(cross_ref_term) <= 255:
                                                    try:
                                                        # First verify the definition_id exists
                                                        cur.execute("SELECT EXISTS(SELECT 1 FROM definitions WHERE id = %s)", (definition_id,))
                                                        if not cur.fetchone()[0]:
                                                        ...
                                                        cross_ref_id = get_or_create_word_id(cur, cross_ref_term, language_code='tl')
                                                        if cross_ref_id and cross_ref_id > 0:
                                                            # Verify the cross_ref_id word exists in the database
                                                            cur.execute("SELECT EXISTS(SELECT 1 FROM words WHERE id = %s)", (cross_ref_id,))
                                                            if cur.fetchone()[0]:
                                                                try:
                                                                    # Check if definition_relations table exists (not definition_links)
                                                                    insert_definition_relation(
                                                                        cur, 
                                                                        definition_id,
                                                                        cross_ref_id,
                                                                    ...
                                                                    logger.warning(f"Database error creating cross-reference: {str(e)}")
                                                                    error_types[str(e)] = error_types.get(str(e), 0) + 1
                                                            else:
                                                                logger.warning(f"Cross-reference word ID {cross_ref_id} does not exist in the database")
                                                    except Exception as e:
                                                        logger.warning(f"Error processing cross-reference '{cross_ref_term}' for '{formatted_word}': {str(e)}")
                                                        error_types[str(e)] = error_types.get(str(e), 0) + 1
                                    
                                        # Process synonyms
                                        if 'synonyms' in def_entry and isinstance(def_entry['synonyms'], list):
                                            for synonym in def_entry['synonyms']:
                                                synonym_term = ""
                                                if isinstance(synonym, dict) and 'value' in synonym:
                                                    synonym_term = synonym['value']
                                                elif isinstance(synonym, str):
                                                    synonym_term = synonym
                                                    
                                                if synonym_term and len(synonym_term) <= 255:
                                                    logger.debug(f"Processing synonym '{synonym_term}' for '{formatted_word}'")
                                                    try:
                                                        # First check if term exists
                                                        synonym_id = get_or_create_word_id(cur, synonym_term, language_code='tl')
                                                        
                                                        # Verify both word IDs exist in database
                                                        if word_id and word_id > 0 and synonym_id and synonym_id > 0:
                                                            # Verify both words exist in the database
                                                            cur.execute("""
                                                                SELECT 
                                                                    EXISTS(SELECT 1 FROM words WHERE id = %s),
                                                                    EXISTS(SELECT 1 FROM words WHERE id = %s)
                                                            """, (word_id, synonym_id))
                                                            word_exists, synonym_exists = cur.fetchone()
                                                            
                                                            if word_exists and synonym_exists:
                                                                # Create the relation using string type (not Enum) to avoid the tuple error
                                                                insert_relation(
                                                                    cur, 
                                                                    word_id, 
                                                                    synonym_id, 
                                                                ...
                                                                logger.warning(
                                                                    f"Cannot create synonym relation: " +
                                                                    f"{'Word' if not word_exists else 'Synonym'} does not exist"
                                                                )
                                                        else:
                                                            logger.warning(f"Invalid word IDs for synonym relation: from_id={word_id}, to_id={synonym_id}")
                                                    except Exception as e:
                                                        logger.error(f"Error processing synonym '{synonym_term}' for '{formatted_word}': {str(e)}")
                                                        error_types[str(e)] = error_types.get(str(e), 0) + 1
                                        
                                        # Process antonyms
                                        if 'antonyms' in def_entry and isinstance(def_entry['antonyms'], list):
                                            for antonym in def_entry['antonyms']:
                                                antonym_term = ""
                                                if isinstance(antonym, dict) and 'value' in antonym:
                                                    antonym_term = antonym['value']
                                                elif isinstance(antonym, str):
                                                    antonym_term = antonym
                                                    
                                                if antonym_term and len(antonym_term) <= 255:
                                                    try:
                                                        antonym_id = get_or_create_word_id(cur, antonym_term, language_code='tl')
                                                        
                                                        # Verify both word IDs exist in database
                                                        if word_id and word_id > 0 and antonym_id and antonym_id > 0:
                                                            # Verify both words exist in the database
                                                            cur.execute("""
                                                                SELECT 
                                                                    EXISTS(SELECT 1 FROM words WHERE id = %s),
                                                                    EXISTS(SELECT 1 FROM words WHERE id = %s)
                                                            """, (word_id, antonym_id))
                                                            word_exists, antonym_exists = cur.fetchone()
                                                            
                                                            if word_exists and antonym_exists:
                                                                # Create the relation using string type (not Enum) to avoid the tuple error
                                                                insert_relation(
                                                                    cur, 
                                                                    word_id, 
                                                                ...
                                                                logger.warning(
                                                                    f"Cannot create antonym relation: " +
                                                                    f"{'Word' if not word_exists else 'Antonym'} does not exist"
                                                                )
                                                        else:
                                                            logger.warning(f"Invalid word IDs for antonym relation: from_id={word_id}, to_id={antonym_id}")
                                                    except Exception as e:
                                                        logger.warning(f"Error processing antonym '{antonym_term}' for '{formatted_word}': {str(e)}")
                                                        error_types[str(e)] = error_types.get(str(e), 0) + 1
                ...
                # Process affixation relationships
                if 'affixation' in entry and isinstance(entry['affixation'], list):
                    logger.debug(f"Processing affixation for '{formatted_word}'")
                    
                    for affix in entry['affixation']:
                        affix_type = ""
                        related_term = ""
                        
                        if isinstance(affix, dict):
                            affix_type = affix.get('type', '')
                            related_term = affix.get('value', '')
                        elif isinstance(affix, str):
                            # Try to extract affix type and related term from string
                            parts = affix.split(':', 1)
                            if len(parts) == 2:
                                affix_type = parts[0].strip()
                                related_term = parts[1].strip()
                            else:
                                related_term = affix.strip()
                                affix_type = "affixed"
                        ...
                        if related_term and len(related_term) <= 255:
                            try:
                                ...
                                # Verify both word IDs exist in database
                                if word_id and word_id > 0 and related_id and related_id > 0:
                                    # Verify both words exist in the database
                                    cur.execute("""
                                        SELECT 
                                            EXISTS(SELECT 1 FROM words WHERE id = %s),
                                    ...
                                    if word_exists and related_exists:
                                        if affix_type.lower() in ('root', 'ugat'):
                                            insert_relation(
                                                cur, 
                                                related_id, 
                                                word_id, 
                                        ...
                                        if affix_type.lower() in ('root', 'ugat'):
                                            ...
                                            insert_affixation(
                                                ...
                                                word_id,
                                                related_id,
                                                affix_type.lower() if affix_type else "affixed",
                                                sources=standardized_source
                                            )
                                    else:
                                        logger.warning(
                                            f"Cannot create affixation relation: " +
                                            f"{'Word' if not word_exists else 'Related term'} does not exist"
                                        )
                                else:
                                    logger.warning(f"Invalid word IDs for affixation: from_id={word_id}, to_id={related_id}")
                            except Exception as e:
                                logger.warning(f"Error processing affixation '{related_term}' for '{formatted_word}': {str(e)}")
                                error_types[str(e)] = error_types.get(str(e), 0) + 1
                        ...
                        for rel_item in rel_items:
                            ...
                            if related_term and len(related_term) <= 255:
                                try:
                                    ...
                                    # Verify both word IDs exist in database
                                    if word_id and word_id > 0 and related_id and related_id > 0:
                                        # Double-check that both words exist in database
                                        cur.execute("""
                                            SELECT 
                                                EXISTS(SELECT 1 FROM words WHERE id = %s),
                                                EXISTS(SELECT 1 FROM words WHERE id = %s)
                                        ...
                                        logger.warning(f"Invalid word IDs for relation: from_id={word_id}, to_id={related_id}")
                                except Exception as e:
                ...
                # Make sure to roll back this entry's subtransaction
                try:
                    conn.rollback()
                except Exception as rollback_error:
                    logger.error(f"Failed to roll back transaction: {rollback_error}")
            ...
            logger.error(f"Outer exception processing entry '{word}': {str(outer_e)}")
            stats["error_entries"] += 1
...
def process_kaikki_jsonl(cur, filename: str):
    ...
    # Main process entry function (enhanced version with transaction control)
    def process_entry(cur, entry: Dict, conn=None):
        """Process a single dictionary entry."""
        ...
        savepoint_name = f"sp_{hash(str(entry.get('word', '')))}"
        try:
                                ...
                                for wiki in wiki_links:
                                    if isinstance(wiki, str):
                                        try:
                                            ...
                                            logger.warning(f"Error inserting Wikipedia link for '{word}': {str(e)}")
                    
                    # Process relationships
                    process_sense_relationships(cur, word_id, sense)
            
            # Successfully processed - release the savepoint
            cur.execute(f"RELEASE SAVEPOINT {savepoint_name}")
            return word_id
                
        except Exception as e:
            # Roll back only this entry
            logger.error(f"Error processing entry {entry.get('word', 'unknown')}: {str(e)}")
            if conn:
                try:
                    cur.execute(f"ROLLBACK TO SAVEPOINT {savepoint_name}")
                    logger.info(f"Successfully rolled back to savepoint for entry {entry.get('word', 'unknown')}")
                except Exception as rollback_error:
                    logger.error(f"Failed to rollback to savepoint: {str(rollback_error)}")
            return None
    ...
    try:
        with open(filename, 'r', encoding='utf-8') as f:
            for line_number, line in enumerate(f, 1):
                try:
                    ...
                    result = process_entry(cur, entry, conn)
                    ...
                    # Commit periodically to avoid having too large transactions
                    if entries_processed > 0 and entries_processed % 500 == 0:
                        conn.commit()
                        logger.info(f"Processed {entries_processed} entries from {filename}, committed batch")
                        
                except json.JSONDecodeError as e:
                    logger.error(f"Error decoding JSON at line {line_number}: {str(e)}")
                    errors += 1
                except Exception as e:
                    logger.error(f"Error processing line {line_number}: {str(e)}")
                    errors += 1
                    # Try to rollback the connection if something went wrong
                    try:
                        conn.rollback()
                    except Exception as rollback_error:
                        logger.error(f"Failed to roll back transaction after line error: {str(rollback_error)}")
        ...
        try:
            ...
            logger.error(f"Failed to roll back transaction after file error: {str(rollback_error)}")
    ...
    if not json_files:
        ...
        if os.path.isdir(directory_path):
            ...
            if files:
                ...
                logger.info(f"Directory is empty: {directory_path}")
        return 0, 0
...
def migrate_data(args):
    ...
    sources = [
        ...
        # Fix: Changed from glob pattern to directory processing
        {
            "name": "Project Marayum",
            "file": "data/marayum_dictionaries",  # Just point to the directory
            "handler": process_marayum_directory,  # Use a new handler function
            "required": False
        }
    ]
    ...
    if hasattr(args, 'file') and args.file:
        ...
        if filename.endswith('.jsonl'):
            ...
            handler = process_kwf_dictionary
        elif os.path.isdir(filename) and 'marayum' in filename.lower():
