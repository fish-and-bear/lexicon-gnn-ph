...
def get_semantic_network(word: str):
    ...
    try:
        ...
        # Database query for outgoing relations - REMOVED r.relation_data
        outgoing_sql = """
            SELECT /*+ INDEX(r idx_relations_from_word_id) */ 
                   r.id, r.from_word_id, r.to_word_id, r.relation_type, 
        ...
        # Database query for incoming relations - REMOVED r.relation_data
        incoming_sql = """
        ...
        try:
            ...
            logger.error(f"Database error fetching relations for word '{word}': {str(e)}")
            return jsonify({
                "error": "Database error occurred while fetching relations",
                "nodes": [],
                "links": [],
                "metadata": {
        ...
        for rel in outgoing:
            try:
                # Adjusted index since relation_data is removed
                rel_id, _, target_id, rel_type = rel[:4] 
                target_info = {
                    "id": str(target_id),
                ...
                logger.error(f"Error processing outgoing relation: {str(e)}")
                continue
        ...
        for rel in incoming:
            try:
                # Adjusted index since relation_data is removed
                rel_id, source_id, _, rel_type = rel[:4] 
                source_info = {
                    "id": str(source_id),
                ...
                logger.error(f"Error processing incoming relation: {str(e)}")
                continue
        ...
        logger.info(f"Generated semantic network for '{word}' with {len(nodes)} nodes and {len(edges)} edges in {execution_time:.2f}s")
        return jsonify(result)
        ...
        for key in word_result.keys():
            if hasattr(word, key) and key != 'is_root':
                ...
                try:
                    ...
                    logger.warning(f"Type error setting {key} for word {word_id}: {te}, value: {value}")
                ...
                for p_row in pron_result:
                    ...
                    for key in p_row.keys():
                        if hasattr(pronunciation, key):
                           try:
                                setattr(pronunciation, key, p_row[key])
                           except Exception as attr_e:
            ...
            try:
                ...
                for r in out_rel_result:
                    try: # Added inner try-except
                        ...
                        logger.warning(f"Error processing outgoing relation ID {r.id} for word {word_id}: {inner_e}")
