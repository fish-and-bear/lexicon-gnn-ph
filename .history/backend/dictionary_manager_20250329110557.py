...
# -------------------------------------------------------------------
# Batch Processing Functions with Transaction Isolation
# -------------------------------------------------------------------
def batch_process_entries(entries_list, source_name, batch_size=100, check_exists=True):
    """
    Process a list of dictionary entries in batches with proper transaction isolation.
    Each batch gets its own transaction, so if one batch fails, it doesn't affect others.
    
    Args:
        entries_list (list): List of entry dictionaries to process
        source_name (str): Name of the source for logging
        batch_size (int): Number of entries to process in a single transaction
        check_exists (bool): Whether to check if entries already exist
    ...
    logger.info(f"Processing {total_entries} entries from {source_name} in batches of {batch_size}")
    
    # Process in batches
    for i in range(0, total_entries, batch_size):
        batch = entries_list[i:i+batch_size]
        batch_successful, batch_failed = process_entry_batch(batch, i//batch_size+1, check_exists)
        successful += batch_successful
        failed += batch_failed
        
        # Log progress at regular intervals
        if (i // batch_size) % 10 == 0 or i+batch_size >= total_entries:
            logger.info(f"Processed {i+len(batch)}/{total_entries} entries "
                       f"({successful} successful, {failed} failed)")
            
    return successful, failed

@with_transaction(commit=True)
def process_entry_batch(cur, entries_batch, batch_number, check_exists=True):
    """
    Process a single batch of entries within its own transaction.
    If any entry fails, only that entry is skipped, not the whole batch.
    
    Args:
        cur: Database cursor
        entries_batch (list): Batch of entries to process
    ...
    for entry in entries_batch:
        try:
            # Extract core entry information
            word = entry.get('word')
            if not word:
                continue
                
            language_code = entry.get('language_code', 'tl')  # Default to Tagalog
            source = entry.get('source', 'batch_import')
            
            # Process the entry using the existing process_entry function
            word_id = get_or_create_word_id(cur, word, language_code=language_code, sources=source)
            if word_id:
                process_entry(cur, entry, word_id, language_code, source)
                successful += 1
            else:
                logger.error(f"Failed to create or get word_id for '{word}'")
                failed += 1
                
        except Exception as e:
            failed += 1
            logger.error(f"Error processing entry '{entry.get('word', 'unknown')}' in batch {batch_number}: {str(e)}")
            # We don't raise the exception, so other entries in the batch can still be processed
            # The transaction decorator will handle committing successful entries
...
def process_tagalog_words(cur, filename: str):
    ...
    try:
        ...
        # Convert dictionary to list of entries for batch processing
        entries_list = []
        for lemma, entry_data in data.items():
            if entry_data and isinstance(entry_data, dict):
                # Create a copy with the word explicitly set
                entry_copy = entry_data.copy()
                entry_copy['word'] = lemma
                entry_copy['language_code'] = language_code
                entry_copy['source'] = source
                entries_list.append(entry_copy)
        
        # Process entries in batches with transaction isolation
        successful, failed = batch_process_entries(entries_list, filename, check_exists=False)
...
def batch_process_entries(entries, filename=None, check_exists=False):
    """
    Process multiple dictionary entries with transaction isolation.
    
    This function processes each entry in its own transaction to prevent a failure
    in one entry from causing all subsequent entries to fail with a 
    "current transaction is aborted" error.
    
    Args:
        entries: List of dictionary entries to process
        filename: Optional source filename for provenance
        check_exists: Whether to only check if entries exist
    ...
    for entry in entries:
        ...
        # Use a fresh connection for each entry to ensure transaction isolation
        try:
            with DBConnection(autocommit=False) as cur:
                try:
                    result = process_entry(cur, entry, filename, check_exists)
                    if result is not None:
                        successful += 1
                    # The transaction will be committed by the DBConnection context manager
                except Exception as e:
                    # Any exception in processing will be caught here
                    # The transaction will be rolled back by the DBConnection context manager
                    logger.error(f"Error processing entry '{word}': {str(e)}")
                    failed += 1
        except Exception as conn_error:
            # Handle connection-level errors
            logger.error(f"Database connection error while processing '{word}': {str(conn_error)}")
            failed += 1
