...
def process_baybayin_entries(cur):
    ...
    try:
        ...
        logger.info(
            f"Found {len(baybayin_entries)} potential Baybayin entries to process"
        )
        ...
        for entry in baybayin_entries:
            ...
            try:
                ...
                cur.execute(
                    "SELECT baybayin_form, romanized_form FROM words WHERE id = %s",
                    (baybayin_id,),
                )
                ...
                if not baybayin_segments and existing_baybayin:
                    ...
                    if cleaned_existing and re.match(
                        r"^[\u1700-\u171F\s]*$", cleaned_existing
                    ):
                        ...
                        if not existing_romanized:
                            try:
                                ...
                                logger.error(
                                    f"Error updating romanization for word ID {baybayin_id}: {e}"
                                )
                                error_count += 1
                    else:
                        ...
                        skipped_count += 1
                        logger.warning(
                            f"Removed invalid Baybayin form for word ID {baybayin_id}"
                        )
                    ...
                    for segment in baybayin_segments:
                        try:
                            ...
                            logger.warning(
                                f"Error romanizing segment '{segment}' for word ID {baybayin_id}: {e}"
                            )
                    ...
                    if valid_segments:
                        ...
                        logger.warning(
                            f"No valid Baybayin segments in entry {baybayin_id}: {baybayin_lemma}"
                        )
                        cur.execute(
                            """
                            UPDATE words
                ...
                if (processed_count + skipped_count + error_count) % 100 == 0:
                    logger.info(
                        f"Progress: {processed_count} processed, {skipped_count} skipped, {error_count} errors"
                    )
        ...
        logger.info(
            f"Processed {processed_count} Baybayin entries, skipped {skipped_count}, with {error_count} errors"
        )
    ...
    try:
        ...
        for word_id, baybayin_form in cur.fetchall():
            try:
                ...
                logger.warning(
                    f"Error generating romanization for word ID {word_id}: {e}"
                )
        ...
        logger.info(
            f"Baybayin data cleanup completed: {baybayin_count} valid Baybayin entries remain"
        )
...
def check_baybayin_consistency(cur):
    ...
    if missing_rom_count > 0:
        ...
        for word_id, lemma, baybayin in cur.fetchall():
            logger.warning(
                f"Missing romanization for word ID {word_id}: {lemma} / {baybayin}"
            )
    ...
    if inconsistent_count > 0:
        issues.append(
            f"Found {inconsistent_count} entries with inconsistent Baybayin flags"
        )
        ...
        for word_id, lemma, has_flag, baybayin in cur.fetchall():
            state = (
                "marked as Baybayin but missing form"
                if has_flag
                else "has Baybayin form but not flagged"
            )
            logger.warning(
                f"Inconsistent Baybayin flags for word ID {word_id}: {lemma} ({state})"
            )
    ...
    if invalid_chars_count > 0:
        issues.append(
            f"Found {invalid_chars_count} entries with invalid Baybayin characters"
        )
        ...
        for word_id, lemma, baybayin in cur.fetchall():
            logger.warning(
                f"Invalid Baybayin characters in word ID {word_id}: {lemma} / {baybayin}"
            )
    ...
    if missing_baybayin_count > 0:
        issues.append(
            f"Found {missing_baybayin_count} entries with Baybayin characters in lemma but not processed"
        )
...
def get_or_create_word_id(
    ...
    **kwargs,
    ...
    pronunciation_data_json = kwargs.get(
        "pronunciation_data"
    )  # Assume already JSON string or None
    word_metadata_json = kwargs.get(
        "word_metadata", "{}"
    ...
    if source_identifier:
        try:
            ...
            if not standardized_source:
                standardized_source = (
                    source_identifier  # Fallback to original if standardization fails
                )
                logger.debug(
                    f"Source standardization failed for '{source_identifier}', using original"
                )
        except Exception as e:
    ...
    try:
        ...
        if existing_word:
            ...
            current_source_info = existing_word[
                1
            ]  # Fetched as dict/list/etc. from JSONB
            ...
            if standardized_source:
                ...
                updated_source_json = update_word_source_info(
                    current_source_info, standardized_source
                )
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to parse updated source JSON for word ID {word_id}: '{updated_source_json}'"
                    )
                ...
                if updated_source_dict != (current_source_info or {}):
                    ...
                    logger.debug(
                        f"Word '{lemma}' ({language_code}) found (ID: {word_id}). Updated source_info from source '{standardized_source}'."
                    )
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Error propagating source '{standardized_source}' to related records for word ID {word_id}: {prop_e}"
                        )
                else:
                    logger.debug(
                        f"Word '{lemma}' ({language_code}) found (ID: {word_id}). Source_info already includes '{standardized_source}' or no update needed."
                    )
            ...
            if not standardized_source:
                logger.debug(
                    f"Word '{lemma}' ({language_code}) found (ID: {word_id}). No source provided for update."
                )

        else:
            # Word doesn't exist, insert it
            logger.debug(
                f"Word '{lemma}' ({language_code}) not found. Creating new entry from source '{standardized_source}'."
            )
            ...
            try:
                hash_input = (
                    f"{lemma}|{language_code}|{baybayin_form}|{romanized_form}|{tags}"
                )
            ...
            logger.info(
                f"Word '{lemma}' ({language_code}) created (ID: {word_id}) from source '{standardized_source}'."
            )

    except psycopg2.Error as e:
        logger.error(
            f"Database error in get_or_create_word_id for '{lemma}' ({language_code}) from source '{source_identifier}': {e.pgcode} {e.pgerror}",
            exc_info=True,
        )
        raise DatabaseError(
        ...
        logger.error(
            f"Unexpected error in get_or_create_word_id for '{lemma}' ({language_code}) from source '{source_identifier}': {e}",
            exc_info=True,
        )
    ...
    if word_id is None:
        ...
        raise DatabaseError(
            f"Failed to obtain word ID for '{lemma}' ({language_code}) from source '{source_identifier}' after operations."
        )
...
def insert_definition(
    ...
    if not source_identifier:
        logger.error(
            f"CRITICAL: Skipping definition insert for word ID {word_id}: Missing MANDATORY source identifier."
        )
        return None

    if not definition_text:
        logger.warning(
            f"Empty definition text for word ID {word_id} from source '{source_identifier}'. Skipping."
        )
        return None
    ...
    try:
        ...
        logger.debug(
            f"Inserted/Updated definition (ID: {def_id}) for word ID {word_id} from source '{source_identifier}'."
        )
        return def_id
...
def process_marayum_json(
    ...
    max_word_entries: int = 1000000,  # 1 million
    max_definitions_per_word: int = 100,
    max_relations_per_word: int = 100,
    max_pronunciations_per_word: int = 10,
    max_etymologies_per_word: int = 10,
    max_credits_per_word: int = 10,
    max_examples_per_definition: int = 10,
    max_see_also_per_word: int = 10,
    max_metadata_length: int = 500,
    max_metadata_depth: int = 5,
    max_metadata_keys: int = 100,
    max_metadata_values: int = 100,
    max_metadata_value_length: int = 200,
    max_metadata_key_length: int = 100,
    max_metadata_array_length: int = 100,
    max_metadata_object_length: int = 100,
    max_metadata_string_length: int = 200,
    max_metadata_number_length: int = 50,
    max_metadata_boolean_length: int = 10,
    max_metadata_null_length: int = 10,
    max_metadata_other_length: int = 200,
    max_metadata_total_length: int = 1000,
    max_metadata_total_keys: int = 1000,
    max_metadata_total_values: int = 1000,
    max_metadata_total_value_length: int = 2000,
    max_metadata_total_key_length: int = 1000,
    max_metadata_total_array_length: int = 1000,
    max_metadata_total_object_length: int = 1000,
    max_metadata_total_string_length: int = 2000,
    max_metadata_total_number_length: int = 500,
    max_metadata_total_boolean_length: int = 100,
    max_metadata_total_null_length: int = 100,
    max_metadata_total_other_length: int = 2000,
    max_metadata_total_total_length: int = 10000,
    max_metadata_total_total_keys: int = 10000,
    max_metadata_total_total_values: int = 10000,
    max_metadata_total_total_value_length: int = 20000,
    max_metadata_total_total_key_length: int = 10000,
    max_metadata_total_total_array_length: int = 10000,
    max_metadata_total_total_object_length: int = 10000,
    max_metadata_total_total_string_length: int = 20000,
    max_metadata_total_total_number_length: int = 5000,
    max_metadata_total_total_boolean_length: int = 1000,
    max_metadata_total_total_null_length: int = 1000,
    max_metadata_total_total_other_length: int = 20000,
    max_metadata_total_total_total_length: int = 100000,
    max_metadata_total_total_total_keys: int = 100000,
    max_metadata_total_total_total_values: int = 100000,
    max_metadata_total_total_total_value_length: int = 200000,
    max_metadata_total_total_total_key_length: int = 100000,
    max_metadata_total_total_total_array_length: int = 100000,
    max_metadata_total_total_total_object_length: int = 100000,
    max_metadata_total_total_total_string_length: int = 200000,
    max_metadata_total_total_total_number_length: int = 50000,
    max_metadata_total_total_total_boolean_length: int = 10000,
    max_metadata_total_total_total_null_length: int = 10000,
    max_metadata_total_total_total_other_length: int = 200000,
    max_metadata_total_total_total_total_length: int = 1000000,
    max_metadata_total_total_total_total_keys: int = 1000000,
    max_metadata_total_total_total_total_values: int = 1000000,
    max_metadata_total_total_total_total_value_length: int = 2000000,
    max_metadata_total_total_total_total_key_length: int = 1000000,
    max_metadata_total_total_total_total_array_length: int = 1000000,
    max_metadata_total_total_total_total_object_length: int = 1000000,
    max_metadata_total_total_total_total_string_length: int = 2000000,
    max_metadata_total_total_total_total_number_length: int = 500000,
    max_metadata_total_total_total_total_boolean_length: int = 100000,
    max_metadata_total_total_total_total_null_length: int = 100000,
    max_metadata_total_total_total_total_other_length: int = 2000000,
    max_metadata_total_total_total_total_total_length: int = 10000000,
    max_metadata_total_total_total_total_total_keys: int = 10000000,
    max_metadata_total_total_total_total_total_values: int = 10000000,
    max_metadata_total_total_total_total_total_value_length: int = 20000000,
    max_metadata_total_total_total_total_total_key_length: int = 10000000,
    max_metadata_total_total_total_total_total_array_length: int = 10000000,
    max_metadata_total_total_total_total_total_object_length: int = 10000000,
    max_metadata_total_total_total_total_total_string_length: int = 20000000,
    max_metadata_total_total_total_total_total_number_length: int = 5000000,
    max_metadata_total_total_total_total_total_boolean_length: int = 1000000,
    max_metadata_total_total_total_total_total_null_length: int = 1000000,
    max_metadata_total_total_total_total_total_other_length: int = 20000000,
    max_metadata_total_total_total_total_total_total_length: int = 100000000,
    max_metadata_total_total_total_total_total_total_keys: int = 100000000,
    max_metadata_total_total_total_total_total_total_values: int = 100000000,
    max_metadata_total_total_total_total_total_total_value_length: int = 200000000,
    max_metadata_total_total_total_total_total_total_key_length: int = 100000000,
    max_metadata_total_total_total_total_total_total_array_length: int = 100000000,
    max_metadata_total_total_total_total_total_total_object_length: int = 100000000,
    max_metadata_total_total_total_total_total_total_string_length: int = 200000000,
    max_metadata_total_total_total_total_total_total_number_length: int = 50000000,
    max_metadata_total_total_total_total_total_total_boolean_length: int = 10000000,
    max_metadata_total_total_total_total_total_total_null_length: int = 10000000,
    max_metadata_total_total_total_total_total_total_other_length: int = 200000000,
    max_metadata_total_total_total_total_total_total_total_length: int = 1000000000,
    max_metadata_total_total_total_total_total_total_total_keys: int = 1000000000,
    max_metadata_total_total_total_total_total_total_total_values: int = 1000000000,
    max_metadata_total_total_total_total_total_total_total_value_length: int = 2000000000,
    max_metadata_total_total_total_total_total_total_total_key_length: int = 1000000000,
    max_metadata_total_total_total_total_total_total_total_array_length: int = 1000000000,
    max_metadata_total_total_total_total_total_total_total_object_length: int = 1000000000,
    max_metadata_total_total_total_total_total_total_total_string_length: int = 2000000000,
    max_metadata_total_total_total_total_total_total_total_number_length: int = 500000000,
    max_metadata_total_total_total_total_total_total_total_boolean_length: int = 100000000,
    max_metadata_total_total_total_total_total_total_total_null_length: int = 100000000,
    max_metadata_total_total_total_total_total_total_total_other_length: int = 2000000000,
    max_metadata_total_total_total_total_total_total_total_total_length: int = 10000000000,
    max_metadata_total_total_total_total_total_total_total_total_keys: int = 10000000000,
    max_metadata_total_total_total_total_total_total_total_total_values: int = 10000000000,
    max_metadata_total_total_total_total_total_total_total_total_value_length: int = 20000000000,
    max_metadata_total_total_total_total_total_total_total_total_key_length: int = 10000000000,
    max_metadata_total_total_total_total_total_total_total_total_array_length: int = 10000000000,
    max_metadata_total_total_total_total_total_total_total_total_object_length: int = 10000000000,
    max_metadata_total_total_total_total_total_total_total_total_string_length: int = 20000000000,
    max_metadata_total_total_total_total_total_total_total_total_number_length: int = 5000000000,
    max_metadata_total_total_total_total_total_total_total_total_boolean_length: int = 1000000000,
    max_metadata_total_total_total_total_total_total_total_total_null_length: int = 1000000000,
    max_metadata_total_total_total_total_total_total_total_total_other_length: int = 20000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_length: int = 100000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_keys: int = 100000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_values: int = 100000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_value_length: int = 200000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_key_length: int = 100000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_array_length: int = 100000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_object_length: int = 100000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_string_length: int = 200000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_number_length: int = 50000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_boolean_length: int = 10000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_null_length: int = 10000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_other_length: int = 200000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_length: int = 1000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_keys: int = 1000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_values: int = 1000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_value_length: int = 2000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_key_length: int = 1000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_array_length: int = 1000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_object_length: int = 1000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_string_length: int = 2000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_number_length: int = 500000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_boolean_length: int = 100000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_null_length: int = 100000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_other_length: int = 2000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_length: int = 10000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_keys: int = 10000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_values: int = 10000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_value_length: int = 20000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_key_length: int = 10000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_array_length: int = 10000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_object_length: int = 10000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_string_length: int = 20000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_number_length: int = 5000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_boolean_length: int = 1000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_null_length: int = 1000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_other_length: int = 20000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_total_length: int = 100000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_total_keys: int = 100000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_total_values: int = 100000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_total_value_length: int = 200000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_total_key_length: int = 100000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_total_array_length: int = 100000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_total_object_length: int = 100000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_total_string_length: int = 200000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_total_number_length: int = 50000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_total_boolean_length: int = 10000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_total_null_length: int = 10000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_total_other_length: int = 200000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_total_total_length: int = 1000000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_total_total_keys: int = 1000000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_total_total_values: int = 1000000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_total_total_value_length: int = 2000000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_total_total_key_length: int = 1000000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_total_total_array_length: int = 1000000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_total_total_object_length: int = 1000000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_total_total_string_length: int = 2000000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_total_total_number_length: int = 500000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_total_total_boolean_length: int = 100000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_total_total_null_length: int = 100000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_total_total_other_length: int = 2000000000000000,
    max_metadata_total_total_total_total_total_total_total_total_total_total_total_total_total_total_length: int = 10000000000000000,
) -> int:
    """
    Get the ID of a word, creating it if necessary.
    Updates the word's source_info JSONB field with the provided identifier.
...
def process_kwf_dictionary(cur, filename: str):
    ...
    try:
        ...
        with open(filename, "r", encoding="utf-8") as f:
            try:
                ...
                error_types["JSONDecodeError"] = (
                    error_types.get("JSONDecodeError", 0) + 1
                )
                # We are managing transaction manually, so ensure rollback on critical file error
                conn.rollback()
        ...
        if not isinstance(data, dict):
            logger.error(
                f"Unexpected format in {filename}, expected a dictionary mapping words to details."
            )
            stats["error_entries"] += 1
            error_types["InvalidFormat"] = error_types.get("InvalidFormat", 0) + 1
            conn.rollback() # Rollback on format error
            return stats

        # Ensure values are dictionaries (entry details)
        if not all(isinstance(v, dict) for v in data.values()):
            logger.error(
                f"File {filename} is a dictionary, but not all values are dictionaries (entry details). Cannot process."
            )
            stats["error_entries"] += 1
            error_types["InvalidValueFormat"] = (
                error_types.get("InvalidValueFormat", 0) + 1
            )
            conn.rollback() # Rollback on format error
            return stats
        ...
        logger.info(
            f"Found {stats['total_entries']} entries in dictionary format in {filename}"
        )
        ...
        with tqdm(
            ...
            for entry_index, word, entry in entries_iterator:
                ...
                if not word or not isinstance(entry, dict):
                    logger.debug(
                        f"Skipping invalid entry for key '{word}' at index {entry_index}"
                    )
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to create savepoint {savepoint_name} for entry '{word}': {sp_err}. Skipping entry."
                    )
                    stats["error_entries"] += 1
                    error_types[f"SavepointError: {sp_err}"] = (
                        error_types.get(f"SavepointError: {sp_err}", 0) + 1
                    )
                    pbar.update(1)
                    continue
                ...
                try:
                    ...
                    if not formatted_word:
                        logger.warning(
                            f"Entry '{word}' has empty formatted word, skipping"
                        )
                        stats["skipped_entries"] += 1
                    ...
                    language_code = (
                        entry.get("metadata", {}).get("language_code", "tl").strip()
                        or "tl"
                    )
                    ...
                    word_metadata_dict = entry.get(
                        "metadata", {}
                    ).copy()  # Start with metadata section
                    word_metadata_dict["source_file"] = raw_source_identifier
                    word_metadata_dict["original_key"] = (
                        word  # Store the original dictionary key
                    )
                    if dictionary_metadata:
                        word_metadata_dict["dictionary_info"] = dictionary_metadata
                    # Add pronunciation if available
                    pronunciation_list = word_metadata_dict.pop(
                        "pronunciation", []
                    )  # Remove from dict, process separately
                    # Add etymology info if available
                    etymology_list = word_metadata_dict.pop(
                        "etymology", []
                    )  # Remove from dict, process separately
                    # Add source language if available
                    source_lang_list = word_metadata_dict.pop(
                        "source_language", []
                    )  # Remove from dict
                    if source_lang_list:
                        word_metadata_dict["source_language_detail"] = source_lang_list

                    word_metadata_json = (
                        json.dumps(word_metadata_dict) if word_metadata_dict else None
                    )
                    ...
                    if not word_id or word_id <= 0:
                        raise ValueError(
                            f"Failed to get or create a valid word ID for '{formatted_word}' (key: '{word}')"
                        )
                        ...
                        for pron_data in pronunciation_list:
                            ...
                            if isinstance(pron_data, str) and pron_data.strip():
                                insert_pronunciation(
                                    cur, word_id, pron_data.strip(), source_identifier
                                )
                    ...
                    if isinstance(etymology_list, list):
                        for ety_idx, ety_item in enumerate(etymology_list):
                            ...
                            elif isinstance(ety_item, dict) and ety_item.get(
                                "text"
                            ):  # Example structure
                            ...
                            if ety_text:
                                try:
                                    ...
                                    lang_codes_str = None
                                    language_codes_list = extract_language_codes(
                                        ety_text
                                    )
                                    if language_codes_list:
                                        lang_codes_str = ", ".join(language_codes_list)
                                    ...
                                    logger.warning(
                                        f"Error processing etymology for '{formatted_word}': {ety_text[:50]}... Error: {ety_error}"
                                    )
                                    error_types[f"EtymologyError: {ety_error}"] = (
                                        error_types.get(
                                            f"EtymologyError: {ety_error}", 0
                                        )
                                        + 1
                                    )
                    ...
                    if isinstance(definitions_by_pos, dict):
                        for raw_pos, definitions_list in definitions_by_pos.items():
                            ...
                            standardized_pos_str = (
                                standardize_entry_pos(raw_pos) if raw_pos else None
                            )
                            ...
                            for def_idx, def_item in enumerate(definitions_list):
                                ...
                                if not isinstance(def_item, dict):
                                    logger.warning(
                                        f"Skipping invalid definition item (not a dict) for '{formatted_word}', POS '{raw_pos}', index {def_idx}"
                                    )
                                    continue

                                meaning_value = def_item.get("meaning")
                                definition_text = (
                                    meaning_value.strip() if meaning_value else ""
                                )
                                if not definition_text:
                                    continue  # Skip if no definition text

                                examples_json = None
                                usage_notes = def_item.get("note")  # KWF uses "note"
                                tags_list = def_item.get(
                                    "categories", []
                                )  # KWF uses "categories" for tags/domain
                                tags = (
                                    ", ".join(filter(None, map(str.strip, tags_list)))
                                    if tags_list
                                    else None
                                )
                                def_metadata_dict = {
                                    "source_file": raw_source_identifier,
                                    "original_pos": raw_pos,
                                }
                                if standardized_pos_str:
                                    def_metadata_dict["standardized_pos"] = (
                                        standardized_pos_str
                                    )
                                def_num = def_item.get("number")
                                if def_num is not None:
                                    def_metadata_dict["definition_number"] = def_num
                                ...
                                if normalized_examples:
                                    try:
                                        ...
                                        stats["examples_added"] += len(
                                            normalized_examples
                                        )
                                    except TypeError:
                                        logger.warning(
                                            f"Could not serialize examples for '{formatted_word}', def {def_idx}"
                                        )
                                                ...
                                                for syn_idx, syn in enumerate(
                                                    ...
                                                    if (
                                                        ...
                                                        if (
                                                            ...
                                                            continue  # Skip if empty after cleaning or self-reference
                                                        try:
                                                            ...
                                                            logger.warning(
                                                                f"Error processing synonym '{syn_word_clean}' for '{formatted_word}': {syn_error}"
                                                            )

                                            # Antonyms
                                            antonyms_list = def_item.get("antonyms", [])
                                            if isinstance(antonyms_list, list):
                                                for ant_idx, ant in enumerate(
                                                    antonyms_list
                                                ):
                                                    if (
                                                        isinstance(ant, str)
                                                        ...
                                                        try:
                                                            ...
                                                            logger.warning(
                                                                f"Error processing antonym '{ant_word_clean}' for '{formatted_word}': {ant_error}"
                                                            )

                                            # Process cross_references within definition if present
                                            cross_refs = def_item.get(
                                                "cross_references", []
                                            )
                                                        ...
                                                        try:
                                                            ...
                                                            if ref_id:
                                                                ...
                                                                if rel_ref_id:
                                                                    ...
                                                                    logger.warning(
                                                                        f"Error processing cross_reference '{ref_word_clean}' for '{formatted_word}': Relation not inserted."
                                                                    )
                                                            else:
                                                                logger.warning(
                                                                    f"Error processing cross_reference '{ref_word_clean}' for '{formatted_word}': Word ID not found."
                                                                )
                                                        except Exception as ref_error:
                                                            logger.warning(
                                                                f"Error processing cross_reference '{ref_word_clean}' for '{formatted_word}': {ref_error}"
                                                            )

                                    except psycopg2.errors.UniqueViolation:
                                        logger.debug(
                                            f"Definition already exists for '{formatted_word}', POS '{raw_pos}', def {def_idx}: {definition_text[:50]}..."
                                        )
                                    except Exception as def_error:
                                        logger.warning(
                                            f"Error inserting definition for '{formatted_word}', POS '{raw_pos}', def {def_idx}: {definition_text[:50]}... Error: {def_error}"
                                        )
                                        error_types[
                                            f"DefinitionInsertError: {def_error}"
                                        ] = (
                                            error_types.get(
                                                f"DefinitionInsertError: {def_error}", 0
                                            )
                                            + 1
                                        )
                    ...
                    if isinstance(related_data, dict):
                        ...
                        for rel_type_raw, rel_items in related_data.items():
                            ...
                            if rel_type_enum is None:
                                try:
                                    std_rel_type_str, _, _ = normalize_relation_type(
                                        rel_type_raw
                                    )
                                    rel_type_enum = RelationshipType.from_string(
                                        std_rel_type_str
                                    )
                                except (ValueError, KeyError):
                                    logger.warning(
                                        f"Unknown top-level relation type '{rel_type_raw}' for '{formatted_word}', treating as RELATED."
                                    )
                                    rel_type_enum = RelationshipType.RELATED

                            if not isinstance(rel_items, list):
                                logger.warning(
                                    f"Skipping invalid related items for type '{rel_type_raw}' in '{formatted_word}': not a list"
                                )
                                continue
                            ...
                            for rel_idx, rel_item in enumerate(rel_items):
                                ...
                                item_metadata = (
                                    {}
                                )  # Metadata specific to this related item
                                ...
                                if isinstance(rel_item, dict) and "term" in rel_item:
                                    ...
                                    for meta_key, meta_val in rel_item.items():
                                        if meta_key != "term":
                                            item_metadata[meta_key] = meta_val
                                ...
                                if related_term_raw:
                                    # Strip HTML tags before further processing
                                    related_term_clean = re.sub(
                                        r"<[^>]+>", "", related_term_raw
                                    ).strip()

                                if (
                                    related_term_clean
                                    and len(related_term_clean) <= 255
                                    and related_term_clean != formatted_word
                                ):
                                    try:
                                        ...
                                        logger.warning(
                                            f"Error processing top-level related term '{related_term_clean}' (type '{rel_type_raw}') for '{formatted_word}': {e}"
                                        )
                                        error_types[f"RelationError: {e}"] = (
                                            error_types.get(f"RelationError: {e}", 0)
                                            + 1
                                        )
                    ...
                    # ---- REMOVED PERIODIC COMMIT BLOCK ----

                # --- Error Handling for a Single Entry ---
                except psycopg2.Error as db_error:
                    error_msg = str(db_error).split("\n")[0]
                    logger.error(
                        f"Database error processing entry '{word}': {error_msg}",
                        exc_info=False,
                    )
                    error_types[f"DBError: {error_msg}"] = (
                        error_types.get(f"DBError: {error_msg}", 0) + 1
                    )
                    stats["error_entries"] += 1
                    try:
                        ...
                        logger.critical(
                            f"CRITICAL: Failed rollback to savepoint {savepoint_name}: {rb_err}. Attempting full tx rollback.",
                            exc_info=True,
                        )
                        conn.rollback()
                        raise db_error from rb_err

                except ValueError as val_error:
                    logger.error(
                        f"ValueError processing entry '{word}': {val_error}",
                        exc_info=False,
                    )
                    error_types[f"ValueError: {val_error}"] = (
                        error_types.get(f"ValueError: {val_error}", 0) + 1
                    )
                    stats["error_entries"] += 1
                    try:
                        cur.execute(f"ROLLBACK TO SAVEPOINT {savepoint_name}")
                        ...
                        conn.rollback()
                        raise val_error from rb_err

                except Exception as e:
                    error_msg = str(e)
                    logger.error(
                        f"Unexpected error processing entry '{word}': {error_msg}",
                        exc_info=True,
                    )
                    error_types[f"UnexpectedError: {error_msg}"] = (
                        error_types.get(f"UnexpectedError: {error_msg}", 0) + 1
                    )
                    stats["error_entries"] += 1
                    try:
                        cur.execute(f"ROLLBACK TO SAVEPOINT {savepoint_name}")
        ...
        # --- Final Commit (Moved here) ---
        # Commit only if the entire process was successful (no critical errors caused rollback)
        try:
            conn.commit()
            logger.info("Final transaction commit successful for KWF processing.")
        except Exception as commit_error:
            logger.error(
                f"Error during final commit for {filename}: {commit_error}. Rolling back remaining changes..."
            )
            stats["error_entries"] += 1  # Count commit failure as an error
            error_types["FinalCommitError"] = error_types.get("FinalCommitError", 0) + 1
            # Attempt rollback again just in case
            try:
                 if not conn.closed: conn.rollback()
            except Exception as rb_final_err:
                 logger.error(f"Error during final rollback attempt: {rb_final_err}")

    # --- Outer Exception Handling ---
    except Exception as outer_e:
        logger.error(
            f"Outer exception processing KWF dictionary {filename}: {outer_e}",
            exc_info=True,
        )
        error_types[f"OuterError: {outer_e}"] = (
            error_types.get(f"OuterError: {outer_e}", 0) + 1
        )
        stats["error_entries"] += 1
        try:
            ...
            if conn and not conn.closed and conn.status != psycopg2.extensions.STATUS_READY:
                ...
                conn.rollback()
                 ...
                 logger.info("Connection already closed in outer exception handler.")
            else:
                logger.info("Transaction likely already rolled back or committed in outer exception.")
        except Exception as rollback_error:
            logger.error(
                f"Failed to roll back transaction in outer exception handler: {rollback_error}"
            )
...
@with_transaction(
    commit=True
)  # Using commit=True for this function as per the original code
...
def process_root_words_cleaned(cur, filename: str):
    ...
    )  # Or "tagalog.com-RootWords-Cleaned"
    ...
    if isinstance(data, list):  # Original list format
        ...
        if not all(isinstance(v, dict) for v in data.values()):
            logger.error(
                f"File {filename} is a dictionary, but not all values are dictionaries (details). Cannot process."
            )
            stats["errors"] += 1
            return stats
        ...
        def dict_iterator(d):
            for i, (key, value) in enumerate(d.items()):
                ...
                entry_data["root_word"] = (
                    key  # Ensure root_word key exists within the entry data
                )
                yield i, entry_data
    ...
    # --- Process Entries ---
    for entry_index, root_word_entry in tqdm(
        entries_iterator, total=total_roots, desc=f"Processing {source_identifier}"
    ):
        ...
        try:
            ...
            logger.error(
                f"Failed to create savepoint {savepoint_name} for root entry index {entry_index}: {e}. Skipping."
            )
            stats["errors"] += 1
        ...
        try:
            ...
            if not isinstance(root_word_entry, dict):
                logger.warning(
                    f"Skipping invalid entry at index {entry_index} in {filename} (not a dict after processing)"
                )
                stats["skipped"] += 1
            ...
            if not root_word:
                logger.warning(
                    f"Skipping tagalog.com entry at index {entry_index} with empty root word."
                )
                stats["skipped"] += 1
                cur.execute(f"RELEASE SAVEPOINT {savepoint_name}")  # Release savepoint
                continue
            ...
            if (
                not word_id or word_id <= 0
            ):  # Check if word_id was successfully obtained
                raise ValueError(
                    f"Failed to get/create root word ID for tagalog.com entry: {root_word}"
                )
            ...
            if isinstance(definitions, list):
                for def_idx, definition_item in enumerate(definitions):
                    ...
                    examples_json: Optional[str] = (
                        None  # Store examples as JSON string or None
                    )
                    tags: Optional[str] = (
                        None  # Store tags as comma-separated string or None
                    )
                        ...
                        if isinstance(raw_examples, list):
                            ...
                            if examples_list:
                                try:
                                    ...
                                    logger.warning(
                                        f"Could not serialize examples for root '{root_word}', def {def_idx}: {examples_list}"
                                    )
                        elif isinstance(raw_examples, str) and raw_examples.strip():
                            try:
                                examples_json = json.dumps([raw_examples.strip()])
                            except TypeError:
                                logger.warning(
                                    f"Could not serialize example string for root '{root_word}', def {def_idx}: {raw_examples}"
                                )
                    ...
                    if definition_text:
                        try:
                            ...
                            logger.warning(
                                f"Error inserting definition for root '{root_word}', def {def_idx}: {definition_text[:50]}... Error: {def_e}"
                            )
            ...
            associated_data = root_word_entry.get(
                "associated_words", {}
            ...
            if isinstance(associated_data, dict):
                ...
                logger.warning(
                    f"Unexpected format for 'associated_words' for root '{root_word}'. Expected dict or list, got {type(associated_data)}. Skipping."
                )
                ...
                if assoc_word and assoc_word != root_word:
                    ...
                    try:
                        ...
                        if assoc_word_id:
                            ...
                            assoc_pos = assoc_details.get(
                                "type", ""
                            ).strip()  # Get POS from 'type' key
                            if assoc_def_text:
                                try:
                                    ...
                                    if assoc_def_id:
                                        stats[
                                            "definitions_added"
                                        ] += 1  # Count assoc defs too
                                except Exception as assoc_def_e:
                                    logger.warning(
                                        f"Error inserting definition for associated word '{assoc_word}' of root '{root_word}': {assoc_def_e}"
                                    )

                    except Exception as assoc_e:
                        logger.warning(
                            f"Failed to process/relate associated word '{assoc_word}' (index {assoc_idx}) for root '{root_word}': {assoc_e}"
                        )
            ...
            logger.error(
                f"Error processing root entry index {entry_index} ('{root_word or 'N/A'}') in {filename}: {entry_e}",
                exc_info=False,
            )  # Less verbose logs
            stats["errors"] += 1
            try:
                ...
                logger.critical(
                    f"CRITICAL: Failed rollback to savepoint {savepoint_name} after error: {rb_err}. Attempting full transaction rollback."
                )
                conn.rollback()  # Rollback the entire transaction
                logger.warning(
                    "Performed full transaction rollback due to savepoint rollback failure."
                )
    ...
    try:
        conn.commit()
        logger.info(
            f"Finished processing {filename}. Stats: {json.dumps(stats, indent=2)}"
        )
    except Exception as commit_err:
        logger.error(
            f"Error during final commit for {filename}: {commit_err}. Rolling back any uncommitted changes..."
        )
...
@with_transaction(
    commit=False
)  # Changed to commit=False to manage transactions manually
def process_kaikki_jsonl(cur, filename: str):
    ...
    try:
        ...
        conn.commit()
        logger.info(
            "Successfully checked/committed schema changes for Kaikki processing."
        )
    except Exception as e:
    ...
    def get_standard_relationship_type(rel_type):
        ...
        logger.warning(
            f"Unknown relationship type: {rel_type}, using RELATED as fallback"
        )
    ...
    def extract_baybayin_info(entry: Dict) -> Tuple[Optional[str], Optional[str]]:
        ...
        for form in entry["forms"]:
            if (
                ...
                and "tags" in form
                and "Baybayin" in form.get("tags", [])
            ):
                baybayin_form = form.get("form", "")
                romanized = form.get(
                    "romanized"
                )  # Corrected key based on common JSON structure
                ...
                if baybayin_form and any(
                    "\u1700" <= char <= "\u171f" for char in baybayin_form
                ):
        ...
        if "forms" in entry:
            for form in entry["forms"]:
                if (
                    ...
                    and form.get("form")
                    and "tags" in form
                    and "canonical" in form.get("tags", [])
                ):
    ...
    def process_pronunciation(
        cur, word_id: int, entry: Dict, source_identifier: str
    ):  # Added source_identifier
        ...
        for sound in entry.get("sounds", []):
            ...
            if pron_type and pron_value:
                try:
                    ...
                    logger.error(
                        f"Error inserting pronunciation for word ID {word_id} (Type: {pron_type}): {str(e)}"
                    )

    # Process sense relationships
    def process_sense_relationships(
        cur, word_id: int, sense: Dict, language_code: str, source_identifier: str
    ):
            ...
            if rel_key in sense and isinstance(sense[rel_key], list):
                for item in sense[rel_key]:
                    if isinstance(item, dict) and "word" in item:
                        ...
                        if not related_word or not isinstance(related_word, str):
                            continue
                        ...
                        try:
                            ...
                            if (
                                "_dis1" in item
                            ):  # Capture distance if present (from Kaikki?)
                                metadata["distance"] = item["_dis1"]
                            metadata["from_sense"] = (
                                True  # Indicate it came from a sense
                            )
                            ...
                            logger.error(
                                f"Error processing {rel_key} relation for word ID {word_id} to '{related_word}': {str(e)}"
                            )

    def process_form_relationships(
        cur, word_id: int, entry: Dict, language_code: str, source_identifier: str
    ):
        ...
        for form_data in entry["forms"]:
            ...
            try:
                ...
                metadata = {
                    "from_forms": True,
                    "confidence": RelationshipType.VARIANT.strength,
                }
                ...
                if tags and isinstance(tags, list):
                    ...
                    if any(
                        ...
                        tag
                        in ["alternative spelling", "alternate spelling", "alt form"]
                        for tag in tags
                    ):
                        rel_type = RelationshipType.SPELLING_VARIANT
                        metadata["confidence"] = 90
                    elif any(
                        tag in ["regional", "dialectal", "dialect"] for tag in tags
                    ):
                        rel_type = RelationshipType.REGIONAL_VARIANT
                        metadata["confidence"] = 85
                    elif "obsolete" in tags:
                        metadata["confidence"] = (
                            70  # Lower confidence for obsolete variants
                        )
                ...
                form_word_id = get_or_create_word_id(
                    cur, form_word, language_code, source_identifier=source_identifier
                )
                if form_word_id == word_id:
                    continue  # Avoid self-references

                # Insert the relationship (and its inverse/bidirectional counterpart)
                insert_relation(
                    cur, word_id, form_word_id, rel_type, source_identifier, metadata
                )
                ...
                logger.error(
                    f"Error processing form relationship for word ID {word_id} to form '{form_word}': {str(e)}"
                )
    ...
    def process_categories(cur, definition_id: int, categories: List[Dict]):
        ...
        for category in categories:
            ...
            try:
                ...
                logger.error(
                    f"Error inserting category '{category_name}' for definition ID {definition_id}: {str(e)}"
                )
    ...
    def process_head_templates(cur, word_id: int, templates: List[Dict]):
        ...
        for template in templates:
            ...
            try:
                ...
                logger.error(
                    f"Error inserting/updating template '{template_name}' for word ID {word_id}: {str(e)}"
                )
    ...
    def process_links(cur, definition_id: int, links: List[List[str]]):
        ...
        for link in links:
            ...
            is_wikipedia = (
                "wikipedia.org" in link_target.lower()
                or "w:" in link_text
                or "W:" in link_text
            )
            ...
            try:
                ...
                logger.error(
                    f"Error inserting link '{link_text}' -> '{link_target}' for definition ID {definition_id}: {str(e)}"
                )

    def process_etymology(
        cur,
        word_id: int,
        etymology_text: str,
        source_identifier: str,
        etymology_templates: List[Dict] = None,
    ):
        ...
        normalized_components = (
            None  # TODO: Implement extract_etymology_components if needed
        )
        ...
        if etymology_templates and isinstance(etymology_templates, list):
            ...
            for template in etymology_templates:
                if (
                    ...
                    and "args" in template
                    and isinstance(template["args"], dict)
                ):
                    for key, value in template["args"].items():
                        ...
                        # Check if key looks like a language code
                            ...
                            isinstance(key, str) and 1 < len(key) <= 3 and key.isalpha()
                        ):
                            languages.add(key.lower())
            if languages:
                language_codes = ",".join(
                    sorted(list(languages))
                )  # Comma-separated unique codes
        ...
        try:
            ...
            insert_etymology(
                ...
                etymology_structure=(
                    json.dumps(etymology_structure) if etymology_structure else None
                ),
            ...
            logger.error(
                f"Error saving etymology for word ID {word_id} from source '{source_identifier}': {str(e)}"
            )
    ...
    def process_entry(cur, entry: Dict, source_identifier: str):
        ...
        try:
            ...
            if not language_code or len(language_code) > 10:
                ...
                logger.warning(
                    f"Invalid/missing lang_code for '{word}', defaulting to '{DEFAULT_LANGUAGE_CODE}'."
                )
            ...
            try:
                ...
                logger.error(
                    f"CRITICAL FAILURE: Could not get/create word_id for '{word}' ({language_code}). Error: {word_create_err}. Skipping entry."
                )
            ...
            try:
                process_form_relationships(
                    cur, word_id, entry, language_code, source_identifier
                )
            except Exception as e:
            ...
            if "senses" in entry and isinstance(entry["senses"], list):
                for sense_idx, sense in enumerate(entry["senses"]):
                    if not sense or not isinstance(sense, dict):
                        logger.debug(
                            f"Skipping invalid sense item {sense_idx} for word ID {word_id}"
                        )
                        continue

                    definition_id = None  # Reset for each sense
                    try:
                        glosses = sense.get("glosses", []) or sense.get(
                            "raw_glosses", []
                        )
                        if not glosses or not isinstance(glosses, list):
                            logger.debug(
                                f"Skipping sense {sense_idx} for word ID {word_id}: missing/invalid glosses."
                            )
                            continue
                        definition_text = "; ".join(
                            [g for g in glosses if isinstance(g, str)]
                        )
                        if not definition_text:
                            logger.debug(
                                f"Skipping sense {sense_idx} for word ID {word_id}: empty definition text."
                            )
                            continue
                        ...
                        if len(definition_text) > max_def_length:
                            logger.warning(
                                f"Definition {sense_idx} for word ID {word_id} truncated."
                            )
                            definition_text = definition_text[:max_def_length]
                        ...
                        if "examples" in sense and isinstance(sense["examples"], list):
                            for example in sense["examples"]:
                                ...
                                if isinstance(example, str):
                                    ...
                                    ref = example.get("ref")
                                    tr = example.get("translation") or example.get(
                                        "english"
                                    )
                                    if ex_text and tr:
                                        ex_text += f" - {tr}"
                        ...
                        all_tags = (
                            sense_tags if isinstance(sense_tags, list) else []
                        ...
                        try:
                            ...
                            logger.debug(
                                f"Definition already exists for word ID {word_id}, sense {sense_idx}"
                            )
                        ...
                        if definition_id:
                            ...
                            try:
                                if "categories" in sense and sense["categories"]:
                                    process_categories(
                                        cur, definition_id, sense["categories"]
                                    )
                            except Exception as e:
                ...
                if sense_processed_count == 0 and not critical_error_occurred:
                    ...
                    logger.info(
                        f"No definitions successfully inserted/processed for word '{word}' (ID: {word_id}). Senses might be empty or all failed individually."
                    )
                ...
                for rel_key, rel_enum in top_level_rels.items():
                    if rel_key in entry and isinstance(entry[rel_key], list):
                        for item in entry[rel_key]:
                            if isinstance(item, dict) and "word" in item:
                                related_word = item["word"]
                                if not related_word or not isinstance(
                                    related_word, str
                                ):
                                    continue
                                try:
                                    ...
                                    error_messages.append(
                                        msg
                                    )  # Log inner failure but continue loop
            except Exception as e:
            ...
            logger.error(
                f"UNEXPECTED EXCEPTION during processing entry for word '{word}' (ID: {word_id}): {str(e)}",
                exc_info=True,
            )
    ...
    def process_entry_without_baybayin(cur, entry: Dict, source_identifier: str):
        ...
        logger.warning(
            f"Using FALLBACK processing for word: {entry.get('word', 'N/A')}"
        )
        if "word" not in entry or not entry["word"]:
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"FALLBACK CRITICAL: Failed get/create word_id for '{word}'. Error: {word_create_err}"
                )
            ...
            if "senses" in entry and isinstance(entry["senses"], list):
                ...
                for sense_idx, sense in enumerate(entry["senses"]):
                    ...
                    try:
                        ...
                        logger.debug(
                            f"Fallback: Definition already exists for {word_id}, sense {sense_idx}"
                        )
                    except Exception as def_error:
                        logger.warning(
                            f"Fallback: Error inserting definition for {word_id}, sense {sense_idx}: {def_error}"
                        )
                        if not entry_error:
                            entry_error = f"Fallback def insert failed: {def_error}"

                if sense_processed_count == 0 and not entry_error:
                    logger.warning(
                        f"Fallback: No definitions processed for word '{word}' (ID: {word_id})"
                    )
            ...
            logger.error(
                f"FALLBACK UNHANDLED EXCEPTION for word '{word}': {str(e)}",
                exc_info=True,
            )
            try:
                conn.rollback()
            except Exception as rb_err:
                logger.error(f"Fallback rollback error: {rb_err}")
    ...
    stats = {
        ...
        ),  # Use counted lines if available
        "processed_ok": 0,
    ...
    if conn.closed:
        logger.error(
            "Database connection is closed before starting Kaikki processing loop."
        )
    ...
    try:
        with open(filename, "r", encoding="utf-8") as f:
            ...
            for line in f:
                ...
                try:
                    ...
                    logger.error(
                        f"CRITICAL: Failed to create savepoint {savepoint_name}. Aborting processing. Error: {sp_error}"
                    )
                    stats["failed_entries"] = (
                        stats["total_entries"] - entry_count + 1
                ...
                try:
                    ...
                    word_id, entry_errors = process_entry(
                        cur, entry, source_identifier
                    )  # Returns word_id and potential errors

                    if (
                        word_id
                    ):  # Word was created/found, proceed even if non-critical errors occurred
                        if entry_errors:
                            stats["processed_with_errors"] += 1
                            # Log first part of error summary key
                            first_error = entry_errors.split(";")[0][
                                :100
                            ]  # Limit key length
                            err_key = f"EntryError: {first_error}"
                            error_summary[err_key] = error_summary.get(err_key, 0) + 1
                            logger.debug(
                                f"Entry {entry_count} ('{entry.get('word', 'N/A')}') processed with errors: {entry_errors}"
                            )
                        else:
                        ...
                        reason = (
                            entry_errors or "Unknown critical failure"
                        )  # entry_errors should contain the reason
                        err_key = f"EntryFailedCritically: {reason[:100]}"
                        error_summary[err_key] = error_summary.get(err_key, 0) + 1
                        logger.warning(
                            f"Entry {entry_count} ('{entry.get('word', 'N/A')}') failed critical processing step: {reason}"
                        )
                    ...
                    try:
                        ...
                        pass  # Ignore rollback error if savepoint doesn't exist
                except Exception as e:
                    logger.error(
                        f"Unhandled error in main loop at line {entry_count}: {str(e)}",
                        exc_info=True,
                    )
                    stats["failed_entries"] += 1
                    ...
                    try:
                        ...
                        logger.error(
                            f"Error rolling back savepoint {savepoint_name} after loop exception: {rb_err}"
                        )
        ...
        try:
            ...
            logger.error(
                f"Error during final commit after processing {filename}: {final_commit_err}"
            )
        ...
        logger.error(
            f"Fatal error during processing of {filename}: {str(e)}", exc_info=True
        )
        try:
...
def display_leaderboard(cur, console):
    ...
    try:
        ...
        badlit_count_sql = (
            ...
            else "0 AS with_badlit"
        )
        badlit_percentage_sql = (
            "ROUND(100.0 * with_badlit / NULLIF(total_count, 0), 1) as badlit_percentage"
            if has_badlit_column
            else "0.0 as badlit_percentage"
        )
        ...
        try:
            ...
            logger.warning(
                f"Error checking words table columns for source detection: {str(e)}. Falling back to tags only."
            )
        ...
        if baybayin_results:
            baybayin_table = Table(
                title="[bold blue]Baybayin & Related Scripts Contributors[/]",
                box=box.ROUNDED,
            )
            baybayin_table.add_column("Source", style="yellow")
            baybayin_table.add_column(
                "Total Entries", justify="right", style="green"
            )  # Words with potential for scripts
        ...
        console.print(
            f"[red]Error:[/][yellow] Could not generate Baybayin statistics: {str(e)}[/]"
        )
    ...
    try:
        ...
        if (
            cur.connection.closed
            or cur.connection.status != psycopg2.extensions.STATUS_READY
        ):
