...
def _fetch_word_details(word_id,
    ...
    try:
        ...
        if include_relations:
            word.outgoing_relations = [] # Initialize as empty lists
            ...
            try:
                ...
                # Add logging for outgoing relations count and sample
                out_rel_count = len(out_rel_result)
                logger.info(f"RELATIONS DEBUG: Word {word_id} returned {out_rel_count} outgoing relations")
                if out_rel_count > 0:
                    sample_rel = out_rel_result[0]
                    logger.info(f"RELATIONS DEBUG: Sample outgoing relation: id={getattr(sample_rel, 'id', 'N/A')}, type={getattr(sample_rel, 'relation_type', 'N/A')}, to_word={getattr(sample_rel, 'to_word_id', 'N/A')}")
                
                for r in out_rel_result:
                    try: # Inner try-except for individual relation processing
                        relation = Relation()
                        ...
                        logger.debug(f"RELATIONS DEBUG: Added outgoing relation {relation.id} of type {relation.relation_type} to word {word_id}")
                    except Exception as inner_e:
                        logger.warning(f"Error processing outgoing relation ID {getattr(r, 'id', 'UNKNOWN')} for word {word_id}: {inner_e}", exc_info=True)
                ...
                # Add logging for incoming relations count and sample
                in_rel_count = len(in_rel_result)
                logger.info(f"RELATIONS DEBUG: Word {word_id} returned {in_rel_count} incoming relations")
                if in_rel_count > 0:
                    sample_rel = in_rel_result[0]
                    logger.info(f"RELATIONS DEBUG: Sample incoming relation: id={getattr(sample_rel, 'id', 'N/A')}, type={getattr(sample_rel, 'relation_type', 'N/A')}, from_word={getattr(sample_rel, 'from_word_id', 'N/A')}")
                ...
                for r in in_rel_result:
                    try: # Inner try-except for individual relation processing
                        ...
                        source_word.lemma = getattr(r, 'source_lemma', None)
                        source_word.language_code = getattr(r, 'source_language_code', None)
                        source_word.has_baybayin = getattr(r, 'source_has_baybayin', None)
                        
                        # Safe handling for baybayin form in incoming relations too
                        try:
                            baybayin_form = getattr(r, 'source_baybayin_form', None)
                            if baybayin_form and len(baybayin_form.strip()) > 0:
                                source_word.baybayin_form = baybayin_form
                            else:
                                source_word.baybayin_form = None
                                source_word.has_baybayin = False
                        except ValueError:
                            # Invalid baybayin form, set to None
                            source_word.baybayin_form = None
                            source_word.has_baybayin = False
                        ...
                        logger.debug(f"RELATIONS DEBUG: Added incoming relation {relation.id} of type {relation.relation_type} to word {word_id}")
                    except Exception as inner_e:
                        logger.warning(f"Error processing incoming relation ID {getattr(r, 'id', 'UNKNOWN')} for word {word_id}: {inner_e}", exc_info=True)
                        # Continue processing other relations

                # Final check to see if arrays remain empty despite having data
                if out_rel_count > 0 and len(word.outgoing_relations) == 0:
                    logger.error(f"RELATIONS DEBUG: Word {word_id} had {out_rel_count} outgoing relations in query but none were added to the object")
                if in_rel_count > 0 and len(word.incoming_relations) == 0:
                    logger.error(f"RELATIONS DEBUG: Word {word_id} had {in_rel_count} incoming relations in query but none were added to the object")

            except SQLAlchemyError as db_e:
                # Handle database errors specifically
                logger.error(f"Database error loading relations for word {word_id}: {db_e}", exc_info=True)
                # Keep relations as empty lists since we initialized them at the start
            except Exception as e:
                # Handle other unexpected errors during the relation loading block
                logger.error(f"Unexpected error loading relations for word {word_id}: {e}", exc_info=True)
                # Keep relations as empty lists since we initialized them at the start
        else:
            # Ensure attributes exist even if relations are not requested
