...
def cached_query(timeout: int = 3600, key_prefix: str = ""):
    ...
    def decorator(f):
        @wraps(f)
        def wrapper(*args, **kwargs):
            # Remove explicit internal import if it exists
            # Check ONLY the REDIS_ENABLED flag first.
            if not REDIS_ENABLED:
                logger.debug("Redis disabled via REDIS_ENABLED=false, skipping cache.")
                return f(*args, **kwargs)
            
            # Now check if client exists (only if REDIS_ENABLED was true)
            ...
            try:
                ...
                if cached_result:
                    ...
                    # Make sure DB_CACHE_HITS is defined before using
                    if 'DB_CACHE_HITS' in globals():
                        DB_CACHE_HITS.inc()
            ...
            # Make sure DB_CACHE_MISSES is defined before using
            if 'DB_CACHE_MISSES' in globals():
                DB_CACHE_MISSES.inc()
            result = f(*args, **kwargs)
