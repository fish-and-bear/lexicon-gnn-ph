...
from dictionary_manager import (
    ...
    normalize_lemma, extract_etymology_components
)
import json
...
class WordSchema(BaseSchema):
    ...
    language_code = fields.Str(validate=validate.OneOf([
        'tl', 'ceb', 'hil', 'war', 'pam', 'bik', 'ilo', 'pag',  # Primary Philippine
        'psp', 'pph', 'pan',  # Historical/Root
        'en', 'es', 'spl', 'san', 'may', 'ara', 'cmn', 'nan', 'jpn'  # Foreign
    ]))
...
class SearchQuerySchema(Schema):
    ...
    language = fields.Str(validate=validate.OneOf([
        'tl', 'ceb', 'hil', 'war', 'pam', 'bik', 'ilo', 'pag',  # Primary Philippine
        'psp', 'pph', 'pan',  # Historical/Root
        'en', 'es', 'spl', 'san', 'may', 'ara', 'cmn', 'nan', 'jpn'  # Foreign
    ]), default='tl')
...
def get_statistics():
    ...
    stats = {
        ...
        "words_by_pos": db.session.query(
            Definition.standardized_pos,
            func.count(distinct(Word.id))
        ).join(Word).group_by(Definition.standardized_pos).all(),
...
def get_word_etymology(word: str):
    ...
    for etym in word_entry.etymologies:
        etymology_data = {
            ...
            "metadata": etym.metadata
        }

        # Add structured etymology data if available
        if etym.etymology_structure:
            try:
                structure = json.loads(etym.etymology_structure)
                etymology_data.update({
                    "structure": structure,
                    "period": structure.get("period"),
                    "reconstructed": structure.get("reconstructed", False),
                    "uncertain": structure.get("uncertain", False),
    ...
    return jsonify({
        ...
        "etymology_confidence": word_entry.etymology_confidence,
        "verification_status": word_entry.verification_status
    }), 200
