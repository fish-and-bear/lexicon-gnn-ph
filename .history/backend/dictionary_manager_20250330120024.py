...
def create_or_update_tables(cur):
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS words (
            ...
            language_code VARCHAR(10) NOT NULL DEFAULT 'tl',
            root_word_id INTEGER REFERENCES words(id) ON DELETE SET NULL,
            preferred_spelling TEXT,
            tags TEXT[],
            has_baybayin BOOLEAN DEFAULT FALSE,
            baybayin_form TEXT,
            romanized_form TEXT,
    ...
    # Create indexes
    cur.execute("""
        CREATE INDEX IF NOT EXISTS words_normalized_lemma_idx ON words USING gin (normalized_lemma gin_trgm_ops);
        CREATE INDEX IF NOT EXISTS words_lemma_idx ON words USING gin (lemma gin_trgm_ops);
        CREATE INDEX IF NOT EXISTS words_language_code_idx ON words(language_code);
