...
class WordSchema(BaseSchema):
    ...
    credits = fields.List(fields.Nested('backend.routes.CreditSchema')) # Removed exclude=("word",)
...
def get_semantic_network(word: str):
    """
    Generate and return the semantic network for a given word.
    
    This endpoint now handles both lemma strings and 'id:<word_id>' identifiers.
    
    Args:
        word (str): The word lemma or the identifier 'id:<word_id>'.
        
    Query Parameters:
        depth (int): Max traversal depth (default 2, max 4).
        breadth (int): Max number of neighbors per node (default 10, min 5, max 50).
        include_affixes (bool): Include affixation relationships (default True).
        include_etymology (bool): Include etymological relationships (default True).
        cluster_threshold (float): Threshold for clustering (default 0.3).
        relation_types (str): Comma-separated list of relation types to include (optional).
        
    Returns:
        JSON: A dictionary containing nodes, links, and metadata.
    """
    try:
        # Get parameters from query string with validation
        depth = request.args.get('depth', default=2, type=int)
        breadth = request.args.get('breadth', default=10, type=int)
        include_affixes = request.args.get('include_affixes', default=True, type=lambda v: v.lower() == 'true')
        include_etymology = request.args.get('include_etymology', default=True, type=lambda v: v.lower() == 'true')
        cluster_threshold = request.args.get('cluster_threshold', default=0.3, type=float)
        relation_types_str = request.args.get('relation_types', default=None, type=str)
        
        # Validate parameters
        depth = min(max(1, depth), 4)
        breadth = min(max(5, breadth), 50)
        cluster_threshold = max(0.0, min(1.0, cluster_threshold))
        relation_types = relation_types_str.split(',') if relation_types_str else None

        # Determine if the input is an ID or a lemma
        main_word_obj = None
        if word.startswith("id:"):
            try:
                word_id = int(word.split(":")[1])
                # Use the internal helper that returns None on failure instead of 404
                main_word_obj = _get_word_by_id(word_id) 
                if not main_word_obj:
                     return jsonify({"error": f"Word with ID {word_id} not found"}), 404
                print(f"Semantic network request for ID: {word_id} (Lemma: {main_word_obj.lemma})")
            except (ValueError, IndexError):
                return jsonify({"error": "Invalid ID format. Use 'id:<number>'"}), 400
        else:
            normalized_lemma = normalize_query(word)
            # Use the internal helper that returns None on failure instead of 404
            main_word_obj = _get_word_by_lemma(normalized_lemma)
            if not main_word_obj:
                 return jsonify({"error": f"Word with lemma '{word}' not found"}), 404
            print(f"Semantic network request for Lemma: {word} (Normalized: {normalized_lemma}, ID: {main_word_obj.id})")

        # Check if main_word_obj was successfully found
        if not main_word_obj:
            # This case should ideally be handled by the checks above, but as a safeguard:
            return jsonify({"error": f"Word '{word}' could not be found"}), 404

        # Use the found word object's ID for caching and processing
        cache_key = f"network:{main_word_obj.id}-{depth}-{breadth}-{include_affixes}-{include_etymology}-{relation_types_str}"
        cached_network = cache_client.get(cache_key) if cache_client else None # Use cache_client
        if cached_network:
            print(f"Cache hit for semantic network: {cache_key}")
            return jsonify(cached_network)
            
        print(f"Cache miss for semantic network: {cache_key}. Generating...")
        
        start_time = time.time()
        
        # --- Network Generation Logic ---
        # This part remains largely the same, but uses main_word_obj
        
        # Initialize sets and queues
        nodes = {}  # Dictionary to store node details {id: node_data}
        edges = set()  # Set to store edges as tuples (source_id, target_id, type) to avoid duplicates
        queue = deque([(main_word_obj.id, 0)])  # Queue stores (word_id, current_depth)
        visited_ids = {main_word_obj.id} # Keep track of visited word IDs

        # Add the main word node first
        nodes[main_word_obj.id] = {
            "id": main_word_obj.id,
            "label": main_word_obj.lemma,
            "word": main_word_obj.lemma,
            "language": main_word_obj.language_code,
            "type": "main",
            "depth": 0,
            "has_baybayin": main_word_obj.has_baybayin,
            "baybayin_form": main_word_obj.baybayin_form,
            "normalized_lemma": main_word_obj.normalized_lemma,
            "main": True # Explicitly mark the main node
        }

        # Function to process relations and add to queue if criteria met
        def process_relations(current_id, current_depth):
            if current_depth >= depth:
                return

            # Fetch relations for the current word ID using optimized query
            # Use a helper function to avoid repeating query logic
            relations = _fetch_relations_for_network(current_id)

            neighbors_added_at_this_level = 0
            
            # Process outgoing relations first
            for rel in relations.get('outgoing', []):
                if neighbors_added_at_this_level >= breadth: break
                
                target_word = rel.target_word
                if not target_word: continue # Skip if target word is missing
                ...
                if relation_types and rel_type not in relation_types:
                    continue # Skip this relation type

                # Add edge
                edge = (current_id, target_id, rel_type)
                edges.add(edge)
                ...
                if target_id not in nodes:
                    nodes[target_id] = {
                        "id": target_id,
                        "label": target_word.lemma,
                        "word": target_word.lemma,
                        "language": target_word.language_code,
                        "type": rel_type, # Use relation type as node type initially
                        "depth": current_depth + 1,
                        "has_baybayin": target_word.has_baybayin,
                        "baybayin_form": target_word.baybayin_form,
                        "normalized_lemma": target_word.normalized_lemma,
                        "main": False
                    }
                ...
                if target_id not in visited_ids:
                    ...
                    neighbors_added_at_this_level += 1

            # Process incoming relations (only add edges, nodes should be added via outgoing)
            for rel in relations.get('incoming', []):
                source_word = rel.source_word
                if not source_word: continue # Skip if source word is missing
                ...
                edge = (source_id, current_id, rel_type)
                edges.add(edge)
                
                # Add source node if not already added (might happen if breadth limit hit earlier)
                if source_id not in nodes:
                     nodes[source_id] = {
                        "id": source_id,
                        "label": source_word.lemma,
                        "word": source_word.lemma,
                        "language": source_word.language_code,
                        "type": rel_type, # Use relation type
                        "depth": current_depth + 1, # Assign depth based on when it's added
                        "has_baybayin": source_word.has_baybayin,
                        "baybayin_form": source_word.baybayin_form,
                        "normalized_lemma": source_word.normalized_lemma,
                        "main": False
                    }
                
                # Add to queue ONLY if not visited and depth allows
                # This ensures we don't exceed depth by exploring incoming paths too early
                if source_id not in visited_ids:
                     if neighbors_added_at_this_level < breadth:
                         visited_ids.add(source_id)
                         queue.append((source_id, current_depth + 1))
                         neighbors_added_at_this_level += 1
                     else:
                         # If breadth limit hit, add node but don't queue for further exploration
                         visited_ids.add(source_id)
        ...
        while queue:
            ...
            if current_word_id in processed_ids:
                continue
            process_relations(current_word_id, current_depth)
            processed_ids.add(current_word_id)

        end_time = time.time()
        execution_time = end_time - start_time
        print(f"Semantic network generation took {execution_time:.4f} seconds")
        
        # Format nodes and edges for response
        formatted_nodes = list(nodes.values())
        formatted_links = [{"source": s, "target": t, "type": type} for s, t, type in edges if s in nodes and t in nodes]

        network_data = {
            "nodes": formatted_nodes,
            "links": formatted_links,
            "metadata": {
                "root_word": main_word_obj.lemma, # Always use lemma for metadata root_word
                "normalized_lemma": main_word_obj.normalized_lemma,
                "language_code": main_word_obj.language_code,
                "depth": depth,
        ...
        # Cache the generated network
        if cache_client: cache_client.set(cache_key, network_data, timeout=900)
        return jsonify(network_data)

    except SQLAlchemyError as e:
        db.session.rollback()
        print(f"Database error generating semantic network for '{word}': {e}")
        # Log the original exception for detailed debugging
        import traceback
        traceback.print_exc()
        return jsonify({"error": f"Database error generating network for '{word}'.", "details": str(e)}), 500
    except Exception as e:
        # Catch any other unexpected errors during generation
        print(f"Unexpected error generating semantic network for '{word}': {e}")
        import traceback
        traceback.print_exc()
        return jsonify({"error": f"An unexpected error occurred generating the network for '{word}'.", "details": str(e)}), 500

def _fetch_relations_for_network(word_id: int) -> dict:
    """Helper function to fetch relations using an optimized query."""
    try:
        # Fetch outgoing relations
        outgoing = db.session.query(Relation).options(
            selectinload(Relation.target_word) # Load target word efficiently
        ).filter(Relation.from_word_id == word_id).all() # Use from_word_id

        # Fetch incoming relations
        incoming = db.session.query(Relation).options(
            selectinload(Relation.source_word) # Load source word efficiently
        ).filter(Relation.to_word_id == word_id).all() # Use to_word_id

        # Use a simplified schema for network generation to avoid large data transfer
        relation_schema = RelationSchema(many=True, exclude=('source_word.definitions', 'target_word.definitions', 'relation_data')) # Exclude heavy fields
        
        return {
            "outgoing": relation_schema.dump(outgoing),
            "incoming": relation_schema.dump(incoming)
        }
    except SQLAlchemyError as e:
        db.session.rollback()
        print(f"Database error fetching relations for network generation (word_id={word_id}): {e}")
        return {"outgoing": [], "incoming": []} # Return empty on error

# Internal helper to get word by ID without raising 404 immediately
def _get_word_by_id(word_id: int) -> Word | None:
    return db.session.query(Word).filter(Word.id == word_id).first()

# Internal helper to get word by lemma without raising 404 immediately
def _get_word_by_lemma(normalized_lemma: str) -> Word | None:
    # Prioritize exact match on normalized lemma
    word = db.session.query(Word).filter(Word.normalized_lemma == normalized_lemma).first()
    if not word:
        # Fallback: Check non-normalized lemma if needed (less common)
        word = db.session.query(Word).filter(Word.lemma == normalized_lemma).first()
    return word
            ...
            try:
                ...
                if definition_ids:
                    ...
                    try:
                        ...
                        sql_categories = """
                        SELECT id, definition_id, category_name, category_kind, parents 
                        FROM definition_categories 
                        WHERE definition_id = ANY(:ids)
                        """ # Removed category_metadata
                        ...
                        for c_row in category_results:
                            ...
                            # Get parents from the query results
                            # category.category_metadata = c_row.category_metadata if c_row.category_metadata else {} # Removed category_metadata access
                            category.parents = c_row.parents if c_row.parents else []
                            # category.description = category_data.get('description', '') # Removed description access
                    ...
                    try:
                        # Modified SQL to only select columns that definitely exist
                        sql_links = """
                        SELECT id, definition_id, link_text, is_external 
                        FROM definition_links 
                        WHERE definition_id = ANY(:ids)
                        """ # Removed target_url and display_text
                        ...
                        for l_row in link_results:
                            ...
                            # link.target_url = l_row.target_url # Removed target_url access
                            # link.display_text = l_row.display_text # Removed display_text access
            ...
            try:
                ...
                for p_row in pron_result:
                    try:
                        ...
                        pronunciation.type = p_row.type if p_row.type in ['ipa', 'x-sampa', 'pinyin', 'jyutping', 'romaji', 'audio', 'respelling', 'phonemic', 'rhyme'] else 'ipa'
            ...
            try:
                ...
                for r in out_rel_result:
                    try: # Added inner try-except
                        ...
                        # relation.relation_data = getattr(r, 'relation_data', None) # Removed relation_data access
