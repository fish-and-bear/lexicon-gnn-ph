...
def init_app(app):
    ...
    with app.app_context():
        ...
        if is_testing_db(db.engine):
            # SQLite needs separate executions
            ...
            with db.engine.connect() as conn:
                conn.execute(text("CREATE INDEX IF NOT EXISTS idx_words_search ON words(search_text)"))
                conn.execute(text("CREATE INDEX IF NOT EXISTS idx_words_normalized ON words(normalized_lemma)"))
                conn.execute(text("CREATE INDEX IF NOT EXISTS idx_words_baybayin ON words(baybayin_form) WHERE has_baybayin = 1"))
                conn.execute(text("CREATE INDEX IF NOT EXISTS idx_etymologies_langs ON etymologies(language_codes)"))
                conn.execute(text("CREATE INDEX IF NOT EXISTS idx_etymologies_components ON etymologies(normalized_components)"))
                conn.execute(text("""
                    CREATE TRIGGER IF NOT EXISTS update_timestamp
                    AFTER UPDATE ON words
                    FOR EACH ROW
...
def create_constraints(target, engine_or_conn, **kw):
    ...
    if is_testing_db(engine_or_conn):
        ...
        statements = [
            ...
            "CREATE INDEX IF NOT EXISTS idx_words_baybayin ON words(baybayin_form) WHERE has_baybayin = 1",
