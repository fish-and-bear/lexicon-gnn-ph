...
def with_transaction(commit=True):
    """
    Decorator that runs a function inside a transaction block.
    If any error occurs, the entire transaction is rolled back to clear
    the aborted state. On success, it releases the savepoint and commits
    ...
    def decorator(func):
        ...
        def wrapper(cur, *args, **kwargs):
            conn = cur.connection
            # Ensure we are not in autocommit mode.
            if conn.autocommit:
                conn.autocommit = False

            savepoint_name = f"sp_{func.__name__}"
            try:
                # Try to create a savepoint. If we’re in an aborted transaction,
                ...
                try:
                    conn.rollback()  # Roll back the entire transaction to clear the error state.
