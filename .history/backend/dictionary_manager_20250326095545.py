...
def transliterate_to_baybayin(text: str) -> str:
    ...
    # Remove diacritical marks (for words like bagyó)
    ...
    # Updated regex to match all Filipino vowels including e and o
    syllables = re.findall(r'(ng|[kgtdnpmbylswh]?)(a|e|i|o|u)', text)
    ...
    # Handle final consonants with virama
    for match in re.finditer(r'(ng|[kgtdnpmbylswh])(?![aeiou]|$)', text):
        cons = match.group(1)
        if cons in consonants:
            # Find position in result string to add virama
            pos = len(result)
            for i in range(match.start()):
                if i < len(text) and text[i] in 'aeiou':
                    pos -= 1
            if pos < len(result):
                result = result[:pos] + '᜔' + result[pos:]
