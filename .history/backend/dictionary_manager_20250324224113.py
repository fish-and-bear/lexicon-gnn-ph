...
def display_leaderboard(cur, console):
    ...
    Uses ILIKE with wildcards in the CASE statement so that if the sources field
    contains multiple values (e.g. "kwf_dictionary.json, diksiyonaryo.ph"),
    it still correctly maps KWF Dictionary to "KWF Diksiyonaryo ng Wikang Filipino".
    """
    try:
        console.print("\n[bold cyan]📊 Dictionary Contributors Leaderboard[/]", justify="center")
        
        # Modified query using ILIKE and wildcards for source matching
        cur.execute("""
            SELECT 
                 CASE
                      WHEN sources ILIKE '%kaikki-ceb.jsonl%' THEN 'kaikki.org (Cebuano)'
                     WHEN sources ILIKE '%kaikki.jsonl%' THEN 'kaikki.org (Tagalog)'
                     WHEN sources ILIKE '%kwf_dictionary.json%' OR sources ILIKE '%KWF Diksiyonaryo%' THEN 'KWF Diksiyonaryo ng Wikang Filipino'
                     WHEN sources ILIKE '%root_words_with_associated_words_cleaned.json%' THEN 'tagalog.com'
                     WHEN sources ILIKE '%tagalog-words.json%' THEN 'diksiyonaryo.ph'
                     ELSE sources
                 END AS source_name,
                 COUNT(*) AS def_count,
