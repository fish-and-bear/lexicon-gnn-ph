...
def lookup_word(args):
    ...
    try:
        ...
        if not word_data:
            console.print(f"\nNo word found: {args.word}")
            return

        word_id = word_data[0]
        if not word_id:
            console.print(f"\nInvalid word data for: {args.word}")
            return
        ...
        # Basic word info
        word_info.append(f"Word: {word_data[1]}")
        word_info.append(f"Language: {word_data[2].title()}")
        
        # Process sources
        all_sources = set()
        if word_data[9]:  # definition_sources
            for sources in word_data[9]:
                if sources:
                    if isinstance(sources, str):
                        all_sources.update(s.strip() for s in sources.split(','))
                    elif isinstance(sources, list):
                        all_sources.update(s.strip() for s in sources)
        if word_data[10]:  # etymology_sources
            for sources in word_data[10]:
                if sources:
                    if isinstance(sources, str):
                        all_sources.update(s.strip() for s in sources.split(','))
                    elif isinstance(sources, list):
        ...
        for source in all_sources:
            if source:
                if 'dictionary' in source.lower() or 'diksiyonaryo' in source.lower():
                    dictionaries.append(source)
                elif '.org' in source or '.com' in source or 'online' in source.lower():
                    online_resources.append(source)
                else:
                    academic_sources.append(source)
        
        if dictionaries:
            word_info.append(f"Dictionaries: {', '.join(sorted(dictionaries))}")
        if online_resources:
            word_info.append(f"Online Resources: {', '.join(sorted(online_resources))}")
        if academic_sources:
            word_info.append(f"Academic Sources: {', '.join(sorted(academic_sources))}")
        
        # Add idioms if available
        if word_data[8]:  # idioms
            try:
                idioms = word_data[8]
                    ...
                    for idiom in idioms:
                        if isinstance(idiom, dict):
                            ...
                            if idiom_text:
                                ...
                                idiom_text = re.sub(r'[►➞→]', '', idiom_text).strip()
                                idiom_text = ' '.join(idiom_text.split())
                                if idiom_meaning:
                                    idiom_meaning = re.sub(r'[►➞→]', '', idiom_meaning).strip()
                                    idiom_meaning = ' '.join(idiom_meaning.split())
                                    entry = f"• {idiom_text} - {idiom_meaning}"
                                else:
                                    entry = f"• {idiom_text}"
                                ...
                                if examples:
                                    ...
                                    if cleaned_examples:
                                        entry += "\n  Examples:\n  " + "\n  ".join(
                                            f"◦ {ex}" for ex in cleaned_examples
                                        )
                                idiom_entries.append(entry)
                    if idiom_entries:
                        word_info.append("\nIdioms:")
                        word_info.extend(idiom_entries)
            except (json.JSONDecodeError, TypeError) as e:
                logger.warning(f"Error processing idioms: {str(e)}")
        
        # Display word info in a table
        ...
        cur.execute("""
            SELECT d.definition_text, p.name_tl, d.examples, d.usage_notes, d.sources
                FROM definitions d
            LEFT JOIN parts_of_speech p ON d.standardized_pos_id = p.id
                WHERE d.word_id = %s
            ORDER BY p.name_tl NULLS LAST, d.definition_text
        """, (word_id,))
        ...
        if definitions:
            ...
            for definition in definitions:
                def_text, pos, examples, usage_notes, sources = definition
                ...
                if examples:
                    try:
                        ...
                        if isinstance(ex_list, list):
                            formatted_examples = "\n".join(f"• {ex}" for ex in ex_list if ex)
                ...
                if usage_notes:
                    try:
                        notes_list = [note.strip() for note in usage_notes.split('\n') if note.strip()]
                        formatted_notes = "\n".join(f"• {note}" for note in notes_list)
                    except Exception as e:
                        logger.warning(f"Error formatting usage notes: {str(e)}")
                        formatted_notes = usage_notes
                ...
                table.add_row(
                    ...
                    Text(def_text, style="bold"),
                    Text(formatted_examples, style="green") if formatted_examples else "",
                    Text(formatted_notes, style="yellow") if formatted_notes else "",
        ...
        # Get and display etymology
        cur.execute("""
            SELECT DISTINCT ON (e.word_id) 
                e.etymology_text, e.normalized_components, e.language_codes, e.sources
            FROM etymologies e
            WHERE e.word_id = %s
            ORDER BY e.word_id, length(e.etymology_text) DESC
        """, (word_id,))
        
        etymology_data = cur.fetchone()
        if etymology_data:
            etymology_text, components, languages, sources = etymology_data
            
            console.print("\n[bold]Etymology[/]", justify="center")
            table = Table(box=box.ROUNDED, expand=True, show_header=False)
            
            # Display the main etymology text with improved formatting
            if etymology_text:
                # Clean up and normalize the text
                paragraphs = etymology_text.split('\n')
                cleaned_paragraphs = []
                for para in paragraphs:
                    # Remove extra whitespace and normalize
                    para = ' '.join(para.split())
                    # Fix spacing around punctuation
                    para = re.sub(r'\s*([.,])\s*', r'\1 ', para)
                ...
                formatted_text = ' '.join(cleaned_paragraphs)
                # Ensure proper sentence spacing
                formatted_text = re.sub(r'(?<=[.!?])\s+', ' ', formatted_text)
                table.add_row(Text(formatted_text, style="cyan", overflow="fold"))
            
            # Display components if available
                if components:
                table.add_row("")  # Add spacing
                table.add_row(Text("Components:", style="bold yellow"))
                component_list = []
                for comp in components.split(','):
                        comp = comp.strip()
                    if comp:
                        # Try to extract word and meaning if in format "word (meaning)"
                        match = re.match(r'(.*?)\s*\((.*?)\)', comp)
                        if match:
                            word, meaning = match.groups()
                            component_list.append(f'{word.strip()} ("{meaning.strip()}")')
                        else:
                            component_list.append(comp)
                if component_list:
                    table.add_row(Text("• " + ", ".join(component_list), style="cyan"))
            
            # Display languages if available
            if languages:
                table.add_row("")  # Add spacing
                table.add_row(Text("Languages:", style="bold yellow"))
                lang_list = [lang.strip() for lang in languages.split(',') if lang.strip()]
                    if lang_list:
                    table.add_row(Text(", ".join(sorted(lang_list)), style="cyan"))
            
            # Display sources if available
                if sources:
                table.add_row("")  # Add spacing
                table.add_row(Text("Sources:", style="bold yellow"))
                source_list = [src.strip() for src in sources.split(',') if src.strip()]
                    if source_list:
                    table.add_row(Text(", ".join(sorted(source_list)), style="cyan"))
            
            console.print(table)

        # Get and display relations with improved categorization
        cur.execute("""
            WITH etymology_words AS (
                SELECT DISTINCT unnest(string_to_array(COALESCE(e.normalized_components, e.etymology_text), ', ')) as component,
                       e.etymology_text
                FROM etymologies e
                WHERE e.word_id = %s
            ),
            ...
            synonyms AS (
                ...
                AND (r.relation_type ILIKE '%%synonym%%' OR w2.normalized_lemma = %s)
            )
            SELECT DISTINCT r.relation_type, w2.lemma, p.name_tl as pos, w2.id,
                   CASE 
                       WHEN e.component IS NOT NULL THEN 'etymology'
                       WHEN v.lemma IS NOT NULL THEN 'variant'
                       WHEN s.lemma IS NOT NULL THEN 'synonym'
                       WHEN r.relation_type ILIKE '%%antonym%%' THEN 'antonym'
                       WHEN r.relation_type IN ('root_of', 'derived_from') THEN 'root_derived'
                       ELSE 'related'
                   END as derived_type,
                   e.etymology_text
            FROM relations r
            ...
            LEFT JOIN parts_of_speech p ON d.standardized_pos_id = p.id
            LEFT JOIN etymology_words e ON w2.lemma = e.component
            LEFT JOIN word_variants v ON w2.lemma = v.lemma
            LEFT JOIN synonyms s ON w2.lemma = s.lemma
            WHERE r.from_word_id = %s
            ORDER BY derived_type, w2.lemma
        """, (word_id, normalized_word, word_id, word_id, normalized_word, word_id))
        ...
        if relations:
            ...
            for rel_type, rel_word, pos, rel_id, derived_type, etymology_text in relations:
                word_pos = f"{rel_word} ({pos})" if pos else rel_word
                word_key = (rel_word, pos)
                
                if word_key not in seen_words:
                    if derived_type == 'etymology':
                        group = 'Etymology Components'
                    elif derived_type == 'variant':
                        group = 'Variants'
                    elif derived_type == 'synonym':
                        group = 'Synonyms'
                    elif derived_type == 'antonym':
                        group = 'Antonyms'
                    elif derived_type == 'root_derived':
                        group = 'Root/Derived Forms'
                    else:
                        group = 'Related Terms'
                    
                    relation_groups.setdefault(group, set()).add(word_pos)
                    seen_words.add(word_key)
            
            table = Table("Relation Type", "Related Words", 
                         box=box.ROUNDED, expand=True, show_lines=True, padding=(0, 1))
            
            # Define display order for relation types
            order = ['Etymology Components', 'Variants', 'Synonyms', 'Antonyms', 'Root/Derived Forms', 'Related Terms']
            
            for group in order:
                if group in relation_groups:
                    table.add_row(
                        Text(group, style="bold yellow"),
                        Text(", ".join(sorted(relation_groups[group])), style="cyan", overflow="fold")
                    )
            console.print(table)

    except Exception as e:
        logger.error(f"Error during word lookup: {str(e)}", exc_info=True)
        console.print(f"\nAn error occurred while looking up the word: {str(e)}")
    finally:
        if cur:
                cur.close()
        if conn:
