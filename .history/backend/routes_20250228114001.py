...
def init_rate_limiter(app):
    """Initialize Redis client for rate limiting with proper error handling."""
    global redis_client
    try:
...
def check_rate_limit(key: str, limit: int = 30, window: int = 60) -> Tuple[bool, Optional[Dict[str, Any]]]:
    """
    ...
    Returns:
        Tuple[bool, Optional[Dict]]: (is_allowed, rate_limit_info)
        Rate limit info contains current count, limit, remaining, and reset time
    """
    ...
    try:
        ...
        reset_time = int(now - (now % window) + window)
...
def rate_limit_headers(rate_limit_info: Optional[Dict[str, Any]]) -> Dict[str, str]:
    """Generate rate limit headers from rate limit info."""
...
def get_word_relationship_graph(word, **params):
    ...
    try:
        ...
        is_allowed, rate_limit_info = check_rate_limit(rate_limit_key)
        if not is_allowed:
            return error_response("Rate limit exceeded. Please try again later.", 429, rate_limit_headers(rate_limit_info))
