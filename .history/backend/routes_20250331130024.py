...
def get_word_pronunciation(word):
    ...
    try:
        ...
        # Add default pronunciation if none exists
        ...
        return jsonify({
            ...
            'has_pronunciation': bool(word_obj.pronunciation_data)
        }), 200
...
def get_statistics():
    ...
    try:
        stats = {
            ...
            "words_with_etymology": db.session.query(func.count(distinct(Word.id)))
                .join(Etymology, Etymology.word_id == Word.id).scalar(),
            "total_definitions": Definition.query.count(),
            "total_examples": Definition.query.filter(Definition.examples != '[]').count(),
...
def get_semantic_network(word: str):
    ...
    try:
        ...
        def add_node(word_obj):
            if word_obj.id in processed:
                return
            processed.add(word_obj.id)
            
            # Get primary definition
            primary_def = word_obj.definitions[0] if word_obj.definitions else None
            pos = primary_def.standardized_pos.to_dict() if primary_def and primary_def.standardized_pos else None
            ...
            nodes.append({
                ...
                "baybayin_form": word_obj.baybayin_form,
                "pos": pos,
                "definition": primary_def.definition_text if primary_def else None
            })

        # Add initial word and related words
        add_node(word_entry)
        for rel in word_entry.relations_from:
            add_node(rel.to_word)
            edges.append({
