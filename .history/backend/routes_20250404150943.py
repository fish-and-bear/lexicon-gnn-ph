...
class BaseSchema(Schema):
    ...
    #         data['sources'] = self.standardize_sources(data['sources'])
    #     return data
...
class PronunciationType(BaseSchema): # Inherit from BaseSchema
    ...
    tags = fields.Dict()
    # Rename pronunciation_metadata to metadata for consistency
...
class EtymologySchema(BaseSchema):
    ...
    # Helper for parsing examples string (if stored as string)
    # def parse_examples(self, value):
    #     if isinstance(value, str):
    #         return [ex.strip() for ex in value.split('\n') if ex.strip()]
    #     return value
...
def get_word_comprehensive(word: str):
    ...
    try:
        ...
        # Add extracted etymology components manually if needed
        if 'etymologies' in result:
            for etym_data in result['etymologies']:
                 if etym_data.get('etymology_text'):
                     try:
                         components = extract_etymology_components(etym_data['etymology_text'])
                         if isinstance(components, dict) and 'original_text' in components:
                         ...
                         logger.warning(f"Error extracting etymology components during comprehensive serialization: {e}")
                         etym_data["components"] = []
                 else:
                     etym_data["components"] = []
...
def get_random_word():
    ...
    try:
        ...
        # Efficiently get a random word ID using ORM/SQL hybrid
        # Strategy: Get count, pick random offset, fetch one ID
        word_count = db.session.query(func.count(Word.id)).scalar()
        ...
        random_word_entry = db.session.query(Word.id).offset(random_offset).first()

        if not random_word_entry:
             logger.error("Failed to select a random word ID using offset.")
             # Fallback: Get first word? Or error?
             return jsonify({"error": "Failed to select a random word."}), 500

        random_word_id = random_word_entry.id
            ...
            for etym_data in result['etymologies']:
                 if etym_data.get('etymology_text'):
                     try:
                         ...
                         logger.warning(f"Error extracting etymology components during random serialization: {e}")
                         etym_data["components"] = []
                 else:
                      etym_data["components"] = []
