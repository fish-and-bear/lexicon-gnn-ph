...
def extract_components_from_text(etymology_text: str) -> List[str]:
    ...
    if '+' in etymology_text:
        ...
        cleaned_text = re.sub(r'^(From|Derived from|Compound of)\s+', '', etymology_text, flags=re.IGNORECASE)
    ...
    # Pattern for "from X" or "derived from X"
    elif re.search(r'from\s+([^\s,;.]+)', etymology_text, re.IGNORECASE):
        matches = re.findall(r'from\s+([^\s,;.]+)', etymology_text, re.IGNORECASE)
        components.extend([m.strip() for m in matches if m.strip()])
    
    # Pattern for "compound of X and Y"
    elif re.search(r'compound of\s+([^\s,;.]+)\s+and\s+([^\s,;.]+)', etymology_text, re.IGNORECASE):
        matches = re.findall(r'compound of\s+([^\s,;.]+)\s+and\s+([^\s,;.]+)', etymology_text, re.IGNORECASE)
...
def get_baybayin_words(**params):
    ...
    Returns:
        List of words with Baybayin forms
    """
    try:
        # Extract parameters
                ...
                if group_by_language:
                    ...
                    for comp in components:
                        ...
                        if comp_word and comp_word.id not in visited and current_depth < max_depth:
                            comp_tree = build_etymology_tree(comp_word, visited, current_depth + 1)
                            if comp_tree:
                                tree["component_objects"].append(comp_tree)
