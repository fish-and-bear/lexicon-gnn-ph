        ...
        try:
                        ...
                        try:
                            ...
                            try:
                                ...
                                for synonym in synonyms:
                                    ...
                                    if isinstance(synonym_term, str):
                                        # Basic HTML tag removal (for <a> tags in synonyms)
                                ...
                                for antonym in antonyms:
                                    ...
                                    if isinstance(antonym, str):
                                        ...
                                        antonym_term = antonym['term']
                                    else:
                                        logger.warning(f"Skipping invalid antonym format for '{formatted_word}': {antonym}")
                                        continue
                                    
                                    # Extract the actual term if it's wrapped in HTML tags
                                    if isinstance(antonym_term, str):
                                        # Basic HTML tag removal (for <a> tags in antonyms)
                                ...
                                for cross_ref in cross_refs:
                                    ...
                                    # Extract the actual term if it's wrapped in HTML tags
                                    if isinstance(ref_term, str):
                                        # Basic HTML tag removal (for <a> tags in cross references)
                            ...
                            if def_entry.get('see'):
                                ...
                                # Handle both string and list of "see" references
                                ...
                                for see_ref in see_refs:
                                    ...
                                    if isinstance(see_ref, str):
                                        ...
                                        see_term = see_ref['term']
                                    else:
                                        logger.warning(f"Skipping invalid see reference format for '{formatted_word}': {see_ref}")
                                        continue
                                    
                                    # Extract the actual term if it's wrapped in HTML tags
                                    if isinstance(see_term, str):
                                        # Basic HTML tag removal (for <a> tags in see references)
            ...
            if 'affixation' in entry and isinstance(entry['affixation'], list):
                ...
                for affix_entry in entry['affixation']:
                    ...
                    # Extract the actual term if it's wrapped in HTML tags
                    if isinstance(affix_term, str):
                        # Basic HTML tag removal (for <a> tags in affixation terms)
            ...
            if 'related' in entry and isinstance(entry['related'], dict):
                ...
                for rel_type, rel_terms in entry['related'].items():
                    ...
                    for rel_term in rel_terms:
                        # Handle both string related terms and dictionary related terms with 'term' key
                        term = None
                        if isinstance(rel_term, str):
                            term = rel_term
                        elif isinstance(rel_term, dict) and 'term' in rel_term:
                            term = rel_term.get('term')
                        else:
                            logger.warning(f"Skipping invalid related term format for '{formatted_word}': {rel_term}")
                            continue
                        
                        # Extract the actual term if it's wrapped in HTML tags
                        if isinstance(term, str):
                            # Basic HTML tag removal (for <a> tags in related terms)
                        ...
                        try:
                            ...
                            metadata = {
                                ...
                                'link': rel_term.get('link', '') if isinstance(rel_term, dict) else '',
                                'broken': rel_term.get('broken', False) if isinstance(rel_term, dict) else False
                            }
...
def migrate_data(args):
    ...
    sources = [
        ...
        },
        {
            ...
            "file": "marayum_dictionaries/*_processed.json",
            "pattern": True,  # Indicates this is a glob pattern
    ...
    try:
        ...
        with Progress(SpinnerColumn(), TextColumn("[progress.description]{task.description}"), console=console) as progress:
            for source in sources:
                # Check if this is a pattern-based source (like wildcard)
                if source.get("pattern", False):
                    # Look for files matching the pattern in all data directories
                    matching_files = []
                    for data_dir in data_dirs:
                        pattern_path = os.path.join(data_dir, source["file"])
                        import glob
                        matching_files.extend(glob.glob(pattern_path))
                    
                    if not matching_files:
                        if source["required"]:
                            logger.error(f"Required pattern did not match any files: {source['file']}")
                            sys.exit(1)
                        else:
                            logger.warning(f"Optional pattern did not match any files: {source['file']}")
                            continue
                    
                    # Process each matching file
                    for filename in matching_files:
                        task = progress.add_task(f"Processing {source['name']} ({os.path.basename(filename)})...", total=1)
                        try:
                            source["handler"](cur, filename)
                            progress.advance(task)
                        except Exception as e:
                            logger.error(f"Error processing {source['name']} file {filename}: {str(e)}")
                            if source["required"]:
                                raise
                else:
                    # Regular file source (not a pattern)
