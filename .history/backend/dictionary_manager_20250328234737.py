...
# -------------------------------------------------------------------
# DB_CONFIG is defined at the top of the file
...
def get_connection():
    """
    Get a database connection. This function is maintained for backward compatibility.
    For new code, prefer using get_db_connection() which supports connection pooling.
    """
...
def check_extensions():
    ...
    with DBConnection() as cursor:
        ...
        missing = []
        for ext in extensions:
            cursor.execute("SELECT EXISTS (SELECT FROM pg_extension WHERE extname = %s)", (ext,))
            if not cursor.fetchone()[0]:
                missing.append(ext)
        return (False, f"Missing extensions: {', '.join(missing)}") if missing else (True, f"All {len(extensions)} extensions installed")
