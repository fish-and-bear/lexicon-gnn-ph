...
def display_leaderboard(cur, console):
    ...
    try:
        ...
        # Note: We now compare against the standardized value for KWF Dictionary.
        cur.execute("""
            SELECT 
                 CASE
                      WHEN sources = 'kaikki-ceb.jsonl' THEN 'kaikki.org (Cebuano)'
                     WHEN sources = 'kaikki.jsonl' THEN 'kaikki.org (Tagalog)'
                     WHEN sources = 'KWF Diksiyonaryo ng Wikang Filipino' THEN 'KWF Diksiyonaryo ng Wikang Filipino'
                     WHEN sources = 'root_words_with_associated_words_cleaned.json' THEN 'tagalog.com'
                     WHEN sources = 'tagalog-words.json' THEN 'diksiyonaryo.ph'
                     ELSE sources
                 END as source_name,
        ...
        if results:
            ...
            medals = ["🥇", "🥈", "🥉"]
            for idx, (source, def_count, unique_count) in enumerate(results, start=1):
                medal = medals[idx-1] if idx <= len(medals) else ""
                table.add_row(str(idx), f"{medal} {source}", f"{def_count:,}", f"{unique_count:,}")
            console.print(table)
            ...
            if recent_results:
                ...
                for source, last_activity, recent_count in recent_results:
                    last_act_str = last_activity.strftime("%Y-%m-%d %H:%M")
                    recent_table.add_row(source, last_act_str, f"{recent_count:,}")
                console.print("\n")
