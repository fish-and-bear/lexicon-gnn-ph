...
def migrate_relationships_to_new_system(cur):
    ...
    This function:
    1. Reads existing relationships from the relations table
    2. Determines the appropriate RelationshipType for each
    3. Adds metadata including appropriate strength values
    4. Updates the relationships with standardized values
    """
    # The following duplicate functions have been removed:
    # 1. check_tables_exist (lines ~5981-5997)
    # 2. check_extensions (lines ~6002-6021)
    # 3. get_standard_code (lines ~6062-6090)
    # 4. get_standardized_pos_id (lines ~6091-6099)
    # 5. get_uncategorized_pos_id (lines ~6100-6111)
    
    # Define standard part of speech mappings - keeping this dictionary for reference
    POS_MAPPING = {
        # Nouns
    ...
    # Continue with the migrate_relationships_to_new_system implementation
    logger.info("Starting migration of relationships to new system...")
    
    try:
        # Get all relationships
        cur.execute("""
            SELECT id, from_word_id, to_word_id, relation_type, sources, metadata
            FROM relations
        """)
        ...
        # Process each relationship
        for rel_id, from_id, to_id, rel_type, sources, metadata in relationships:
            try:
                # Convert to RelationshipType enum if possible
                try:
                    rel_enum = RelationshipType.from_string(rel_type)
                    rel_value = rel_enum.rel_value
                    default_strength = rel_enum.strength
                except ValueError:
                    rel_value = rel_type
                    default_strength = 50  # Default middle value
                
                # Process metadata
                if metadata is None:
                    new_metadata = {'strength': default_strength}
                else:
                    ...
                    if 'strength' not in new_metadata:
                        new_metadata['strength'] = default_strength
                
                # Update the relationship with standardized values
                cur.execute("""
                    UPDATE relations
                    SET relation_type = %s,
                        metadata = %s
                    WHERE id = %s
                """, (rel_value, json.dumps(new_metadata), rel_id))
                
            except Exception as e:
                logger.error(f"Error processing relationship ID {rel_id}: {e}")
        
        # Ensure bidirectional and inverse relationships exist
        logger.info("Creating missing bidirectional and inverse relationships...")
        cur.execute("""
        ...
        added = 0
        for from_id, to_id, rel_type, sources, metadata in all_relationships:
            try:
                rel_enum = RelationshipType.from_string(rel_type)
                
                # Skip non-bidirectional relationships that already have inverses
                if not rel_enum.bidirectional and not rel_enum.inverse:
                    continue
                
                # Ensure bidirectional relationships exist in both directions
                if rel_enum.bidirectional:
                    ...
                    if not cur.fetchone():
                        if rel_manager.add_relationship(
                            to_id, from_id, rel_enum, sources, 
                            json.loads(metadata) if isinstance(metadata, str) else metadata
                        ):
                            added += 1
                
                # Ensure inverse relationships exist
                elif rel_enum.inverse:
                    inverse_type = rel_enum.get_inverse()
                    ...
                    if not cur.fetchone():
                        if rel_manager.add_relationship(
                            to_id, from_id, inverse_type, sources,
                            json.loads(metadata) if isinstance(metadata, str) else metadata
                        ):
                            added += 1
            
            except Exception as e:
                logger.error(f"Error processing bidirectional/inverse for {from_id}->{to_id} ({rel_type}): {e}")
        
        logger.info(f"Added {added} missing bidirectional/inverse relationships")
        logger.info("Relationship migration completed successfully")
        
    except Exception as e:
        logger.error(f"Error during relationship migration: {e}")
        raise
