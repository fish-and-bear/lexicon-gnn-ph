...
class SourceStandardization:
    ...
    def standardize_sources(sources: str) -> str:
        ...
        for source in source_list:
            if source in SourceStandardization.STANDARD_SOURCES:
                standardized.add(source)
            else:
                # Try to match partial names
                matched = False
                for code, name in SourceStandardization.STANDARD_SOURCES.items():
                    if source in name.lower():
                        standardized.add(code)
                        matched = True
                        break
                if not matched:
                    standardized.add(source)  # Keep original if no match
