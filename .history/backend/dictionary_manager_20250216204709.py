...
def lookup_word(args):
    """Look up word details with improved formatting and data source handling."""
    try:
        ...
        with get_connection() as conn:
            with conn.cursor() as cur:
                # Get word details
                cur.execute("""
                    SELECT w.*, array_agg(DISTINCT d.sources) as definition_sources
                    FROM words w
                ...
                if not word_data:
                    logger.info(f"Word not found: {args.word}")
                    return None

                # Get definitions with proper formatting
                cur.execute("""
                    SELECT 
                        d.definition_text,
                        p.code as pos_code,
                    ...
                    WHERE d.word_id = %s
                    ORDER BY p.code != 'unc', d.confidence_score DESC
                """, (word_data[0],))
                ...
                for def_row in cur.fetchall():
                    # Format examples
                    examples = []
                    if def_row[3]:  # examples
                        try:
                            ex_list = json.loads(def_row[3]) if isinstance(def_row[3], str) else def_row[3]
                            if isinstance(ex_list, list):
                                examples = [ex.strip() for ex in ex_list if ex and ex.strip()]
                        except json.JSONDecodeError:
                            logger.warning(f"Error parsing examples for word {args.word}")
                            examples = [def_row[3]] if def_row[3] else []

                    # Format usage notes
                    usage_notes = []
                    if def_row[4]:  # usage_notes
                        try:
                            if isinstance(def_row[4], str):
                                notes = [note.strip() for note in def_row[4].split('\n')]
                                usage_notes = [note for note in notes if note]
                            elif isinstance(def_row[4], list):
                                usage_notes = [note.strip() for note in def_row[4] if note and note.strip()]
                        except Exception as e:
                            logger.warning(f"Error formatting usage notes: {str(e)}")
                            usage_notes = [def_row[4]] if def_row[4] else []

                    # Format sources with standardization
                    sources = []
                    if def_row[5]:  # sources
                        raw_sources = def_row[5].split(',') if isinstance(def_row[5], str) else def_row[5]
                        sources = [
                            get_standardized_source(source.strip())
                            for source in raw_sources
                            if source and source.strip()
                        ]

                    # Add definition with formatted data
                    definitions.append({
                        'text': def_row[0],  # definition_text
                        'pos': {
                            'code': def_row[1],  # pos_code
                            'name': def_row[2]   # pos_name
                        } if def_row[1] and def_row[2] else None,
                        'examples': examples,
                        'usage_notes': usage_notes,
                        'sources': sources,
                        'confidence_score': def_row[6],  # confidence_score
                        'is_verified': def_row[7]        # is_verified
                    })
                ...
                etymologies = []
                for etym_row in cur.fetchall():
                    etymologies.append({
                        'text': etym_row[0],
                        'components': etym_row[1].split(', ') if etym_row[1] else [],
                        'languages': etym_row[2].split(', ') if etym_row[2] else [],
                        'sources': [
                            get_standardized_source(s.strip())
                            for s in (etym_row[3].split(',') if isinstance(etym_row[3], str) else etym_row[3])
                            if s and s.strip()
                        ],
                        'confidence_level': etym_row[4],
                        'verification_status': etym_row[5],
                        'verification_notes': etym_row[6]
                    })
                ...
                cur.execute("""
                    ...
                    SELECT 
                        ...
                        'to' as direction
                    FROM relations r
                    JOIN words w2 ON r.from_word_id = w2.id
                ...
                for rel_row in cur.fetchall():
                    relation = {
                        ...
                        'sources': [
                            get_standardized_source(s.strip())
                            for s in (rel_row[2].split(',') if isinstance(rel_row[2], str) else rel_row[2])
                            if s and s.strip()
                        ]
                    }
                    relations[rel_row[3]].append(relation)

                # Process Baybayin data if present
                baybayin_data = None
                if word_data[7]:  # has_baybayin
                    baybayin_data = {
                        'has_baybayin': True,
                        'baybayin_form': word_data[8],  # baybayin_form
                        'romanized_form': word_data[9]  # romanized_form
                    }

                # Construct final response
                result = {
                    'lemma': word_data[1],  # lemma
                    'normalized_lemma': word_data[2],  # normalized_lemma
                    'language_code': word_data[3],  # language_code
                    'preferred_spelling': word_data[4],  # preferred_spelling
                    'tags': word_data[5].split(', ') if word_data[5] else [],  # tags
                    'baybayin': baybayin_data,
                    'definitions': definitions,
                    'etymologies': etymologies,
                    'relations': relations,
                    'metadata': {
                        'sources': list(set(
                            source for def_sources in word_data[11] for source in def_sources.split(',')
                            if source and source.strip()
                        )),
                        'created_at': word_data[12].isoformat() if word_data[12] else None,  # created_at
                        'updated_at': word_data[13].isoformat() if word_data[13] else None,  # updated_at
                        'is_verified': word_data[14],  # is_verified
                        'verification_notes': word_data[15],  # verification_notes
                        'data_quality_score': word_data[16]  # data_quality_score
                    }
                }
        ...
        logger.error(f"Error in lookup_word: {str(e)}")
        raise

def get_standardized_source(source: str) -> str:
    """Get standardized display name for a source."""
    from source_standardization import SourceStandardization
    return SourceStandardization.get_display_name(source)
