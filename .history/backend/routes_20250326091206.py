...
def get_affixes():
    ...
    try:
        ...
        # Build query for affix statistics - Fix regexp_replace escaping
        sql = text("""
            WITH affix_stats AS (
                SELECT 
                    a.affix_type,
                    ...
                    json_agg(DISTINCT w.lemma) FILTER (WHERE w.id = a.root_word_id) as sample_roots,
                    json_agg(DISTINCT w2.lemma) FILTER (WHERE w2.id = a.affixed_word_id) as sample_affixed
                FROM affixations a
...
def get_relations():
    ...
    try:
        ...
        # Build query for relation statistics - Fix regexp_replace escaping
        sql = text("""
            WITH relation_stats AS (
                SELECT 
                    ...
                    json_agg(DISTINCT w.lemma) FILTER (WHERE w.id = r.from_word_id) as sample_from,
                    json_agg(DISTINCT w2.lemma) FILTER (WHERE w2.id = r.to_word_id) as sample_to
                FROM relations r
