...
# -------------------------------------------------------------------
DB_NAME = os.getenv("POSTGRES_DB", "tagalog_dict")
...
def get_connection_pool(min_connections=1, max_connections=10):
    """Get or initialize a connection pool."""
...
def get_connection():
    ...
    try:
        conn = pool.getconn()
        # Set application name for better tracking in pg_stat_activity
        conn.cursor().execute("SET application_name = 'dictionary_manager'")
        return conn
    except Exception as e:
...
def with_transaction(commit=True):
    ...
    The decorated function must take a cursor as its first argument.
    """
    def decorator(func):
...
class RelationshipManager:
    ...
    def add_relationship(self, from_word_id: int, to_word_id: int, 
        ...
        Args:
            ...
            strength: Confidence/strength score (0-100)
        ...
        try:
            ...
            """, (from_word_id, to_word_id, str(rel_type.rel_value), sources, json.dumps(metadata)))
            ...
            if rel_type.bidirectional:
                self.cursor.execute("""
                    ...
                    DO UPDATE SET 
                        ...
                        END,
                        metadata = COALESCE(relations.metadata, '{}') || EXCLUDED.metadata
                """, (to_word_id, from_word_id, str(rel_type.rel_value), sources, json.dumps(metadata)))
                ...
                self.cursor.execute("""
                    ...
                    DO UPDATE SET 
                        ...
                        END,
                        metadata = COALESCE(relations.metadata, '{}') || EXCLUDED.metadata
                """, (to_word_id, from_word_id, str(inverse_type.rel_value), sources, json.dumps(metadata)))
                
            return True
        except Exception as e:
            logger.error(f"Error adding relationship from {from_word_id} to {to_word_id}: {e}")
            return False
    ...
    def find_relationship_paths(self, from_word_id: int, to_word_id: int, 
        ...
        visited = set()  # Set of visited word IDs
        ...
        try:
            ...
            # Perform BFS
            while queue and len(found_paths) < 10:  # Limit to 10 paths to avoid excessive results
                current_path = queue.popleft()
                current_node = current_path[-1]
                current_word_id = current_node["word_id"]
                
                # Skip if we've visited this word in this path already to avoid cycles
                if current_word_id in [node["word_id"] for node in current_path[:-1]]:
                    continue
                ...
                # Sort by relationship strength if available
                related_words.sort(
                ...
                for related in related_words:
                    ...
                    if related_id == to_word_id:
                        ...
                        # Don't break here - we want to find all paths up to max_depth
                    
                    # If we haven't reached max depth, add to queue for further exploration
                    elif len(new_path) < max_depth + 1:  # +1 because the path includes the starting node
            ...
            def path_score(path):
                # Calculate sum of relationship strengths
                strength_sum = sum(
                    node.get("metadata", {}).get("strength", 50) 
                    for node in path[1:]  # Skip the first node (start)
                    if isinstance(node.get("metadata"), dict)
                )
                # Prefer shorter paths with higher strengths
                return strength_sum / (len(path) - 1) if len(path) > 1 else 0
    ...
    def analyze_relationship_consistency(self) -> List[Dict]:
        """
        Analyze the relationship network for inconsistencies.
        
        Checks for:
        - Missing inverse relationships
        - Conflicting relationships (e.g., both synonym and antonym)
        - Circular derivational chains
        
        Returns:
            List of detected inconsistencies with details
        """
        issues = []
        
        # Check for missing inverse relationships
        self.cursor.execute("""
            SELECT r.from_word_id, r.to_word_id, r.relation_type
            FROM relations r
            WHERE r.relation_type IN (
                SELECT DISTINCT relation_type 
                FROM relations
                WHERE relation_type NOT IN ('synonym', 'antonym', 'related', 'variant', 'spelling_variant')
            )
        """)
        
        for row in self.cursor.fetchall():
            from_id, to_id, rel_type = row
            rel_obj = RelationshipType.from_string(rel_type)
            
            if not rel_obj.bidirectional and rel_obj.inverse:
                inverse_type = rel_obj.get_inverse()
                
                # Check if inverse relationship exists
                self.cursor.execute("""
                    SELECT 1 FROM relations
                    WHERE from_word_id = %s AND to_word_id = %s AND relation_type = %s
                """, (to_id, from_id, str(inverse_type)))
                ...
                if not self.cursor.fetchone():
                    issues.append({
                        ...
                        'to_word_id': to_id,
                        'relation_type': rel_type,
                        'expected_inverse': str(inverse_type)
                    })
        
        # Check for conflicting relationships
        # For example, words that are both synonyms and antonyms
        self.cursor.execute("""
            SELECT r1.from_word_id, r1.to_word_id, r1.relation_type, r2.relation_type
            FROM relations r1
            JOIN relations r2 ON r1.from_word_id = r2.from_word_id AND r1.to_word_id = r2.to_word_id
            WHERE r1.relation_type = 'synonym' AND r2.relation_type = 'antonym'
        """)
        
        for row in self.cursor.fetchall():
            from_id, to_id, rel1, rel2 = row
            issues.append({
    ...
    def repair_relationship_issues(self, issues: List[Dict]) -> int:
        """
        Repair detected relationship inconsistencies.
        
        Args:
            issues: List of issues detected by analyze_relationship_consistency
        ...
        for issue in issues:
            if issue['type'] == 'missing_inverse':
                # Add the missing inverse relationship
                self.cursor.execute("""
                    INSERT INTO relations (from_word_id, to_word_id, relation_type, sources)
                    VALUES (%s, %s, %s, 'auto-repair')
                    ON CONFLICT (from_word_id, to_word_id, relation_type) DO NOTHING
                """, (issue['to_word_id'], issue['from_word_id'], issue['expected_inverse']))
                fixed_count += 1
                
            elif issue['type'] == 'conflicting_relations':
                # Remove the conflicting relationship with lower priority
                # In this case, we keep synonyms and remove antonyms
                self.cursor.execute("""
                    DELETE FROM relations
                    WHERE from_word_id = %s AND to_word_id = %s AND relation_type = 'antonym'
                """, (issue['from_word_id'], issue['to_word_id']))
                fixed_count += 1
...
def standardize_entry_pos(pos_str: str) -> str:
    ...
    if not pos_str:
        return ""
    # Use the existing standardize_pos function but convert to single string format
    result = standardize_pos(pos_str)
    return result
