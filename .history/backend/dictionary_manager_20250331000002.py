...
def process_kaikki_jsonl(cur, filename: str):
    ...
    with open(filename, 'r', encoding='utf-8') as f:
        for line in tqdm(f, total=total_lines, desc="Processing Kaikki entries"):
            try:
                entry = json.loads(line)
                
                # Skip non-Tagalog entries
                ...
                try:
                    ...
                    if processed % 100 == 0:
                        conn.commit()
                        logger.info(f"Progress: {processed}/{total_lines} entries processed")
    ...
    logger.info(f"Kaikki processing complete: {processed} processed, {skipped} skipped, {errors} errors")
    return {
...
def get_or_create_word_id(cur, lemma: str, language_code: str = 'tl') -> int:
    """
    Get the word ID for a given lemma and language code.
    If the word doesn't exist, create a new entry and return the new ID.
    """
    cur.execute(
        "SELECT id FROM words WHERE normalized_lemma = %s AND language_code = %s",
        (normalize_word(lemma), language_code)
    )
    result = cur.fetchone()
    if result:
        return result[0]
        ...
        cur.execute(
            "INSERT INTO words (lemma, normalized_lemma, language_code) VALUES (%s, %s, %s) RETURNING id",
            (lemma, normalize_word(lemma), language_code)
        )
...
def get_word_id(cur, lemma: str, language_code: str = 'tl') -> Optional[int]:
    """Get the word ID for a given lemma and language code."""
    cur.execute(
        "SELECT id FROM words WHERE normalized_lemma = %s AND language_code = %s",
        (normalize_word(lemma), language_code)
...
def get_word_by_id(cur, word_id: int) -> Optional[Dict[str, Any]]:
...
def get_words_by_lemma(cur, lemma: str, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with the given lemma and language code."""
    cur.execute(
        "SELECT * FROM words WHERE normalized_lemma = %s AND language_code = %s",
        (normalize_word(lemma), language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_prefix(cur, prefix: str, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words starting with the given prefix and language code."""
    cur.execute(
        "SELECT * FROM words WHERE normalized_lemma LIKE %s AND language_code = %s",
        (f"{normalize_word(prefix)}%", language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_suffix(cur, suffix: str, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words ending with the given suffix and language code."""
    cur.execute(
        "SELECT * FROM words WHERE normalized_lemma LIKE %s AND language_code = %s",
        (f"%{normalize_word(suffix)}", language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_pattern(cur, pattern: str, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words matching the given pattern and language code."""
    cur.execute(
        "SELECT * FROM words WHERE normalized_lemma LIKE %s AND language_code = %s",
        (f"%{normalize_word(pattern)}%", language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_pos(cur, pos: str, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with the given part of speech and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM definitions WHERE standardized_pos_id = %s) AND language_code = %s",
        (pos, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_source(cur, source: str, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with the given source and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM definitions WHERE sources = %s) AND language_code = %s",
        (source, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_tag(cur, tag: str, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with the given tag and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM definitions WHERE tags = %s) AND language_code = %s",
        (tag, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_example(cur, example: str, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with the given example and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM definitions WHERE examples = %s) AND language_code = %s",
        (example, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_usage_note(cur, usage_note: str, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with the given usage note and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM definitions WHERE usage_notes = %s) AND language_code = %s",
        (usage_note, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_pronunciation(cur, pronunciation: str, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with the given pronunciation and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM pronunciations WHERE value = %s) AND language_code = %s",
        (pronunciation, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_credit(cur, credit: str, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with the given credit and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM credits WHERE credit = %s) AND language_code = %s",
        (credit, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_etymology(cur, etymology: str, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with the given etymology and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM etymologies WHERE etymology_text = %s) AND language_code = %s",
        (etymology, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_relation(cur, relation: str, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with the given relation and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT from_word_id FROM relations WHERE relation_type = %s) AND language_code = %s",
        (relation, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_affixation(cur, affixation: str, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with the given affixation and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT affixed_word_id FROM affixations WHERE affix_type = %s) AND language_code = %s",
        (affixation, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_template(cur, template: str, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with the given template and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM word_templates WHERE template_name = %s) AND language_code = %s",
        (template, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_form(cur, form: str, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with the given form and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM word_forms WHERE form = %s) AND language_code = %s",
        (form, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_category(cur, category: str, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with the given category and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM definition_categories WHERE category_name = %s) AND language_code = %s",
        (category, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_link(cur, link: str, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with the given link and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM definition_links WHERE link_text = %s) AND language_code = %s",
        (link, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_metadata(cur, metadata: str, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with the given metadata and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM definitions WHERE metadata = %s) AND language_code = %s",
        (metadata, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_badlit_form(cur, badlit_form: str, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with the given badlit form and language code."""
    cur.execute(
        "SELECT * FROM words WHERE badlit_form = %s AND language_code = %s",
        (badlit_form, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_hyphenation(cur, hyphenation: str, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with the given hyphenation and language code."""
    cur.execute(
        "SELECT * FROM words WHERE hyphenation = %s AND language_code = %s",
        (hyphenation, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_proper_noun(cur, is_proper_noun: bool, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with the given proper noun status and language code."""
    cur.execute(
        "SELECT * FROM words WHERE is_proper_noun = %s AND language_code = %s",
        (is_proper_noun, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_abbreviation(cur, is_abbreviation: bool, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with the given abbreviation status and language code."""
    cur.execute(
        "SELECT * FROM words WHERE is_abbreviation = %s AND language_code = %s",
        (is_abbreviation, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_initialism(cur, is_initialism: bool, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with the given initialism status and language code."""
    cur.execute(
        "SELECT * FROM words WHERE is_initialism = %s AND language_code = %s",
        (is_initialism, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_baybayin(cur, baybayin_form: str, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with the given Baybayin form and language code."""
    cur.execute(
        "SELECT * FROM words WHERE baybayin_form = %s AND language_code = %s",
        (baybayin_form, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_romanized(cur, romanized_form: str, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with the given romanized form and language code."""
    cur.execute(
        "SELECT * FROM words WHERE romanized_form = %s AND language_code = %s",
        (romanized_form, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_root(cur, root_word_id: int, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with the given root word ID and language code."""
    cur.execute(
        "SELECT * FROM words WHERE root_word_id = %s AND language_code = %s",
        (root_word_id, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_idiom(cur, idiom: str, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with the given idiom and language code."""
    cur.execute(
        "SELECT * FROM words WHERE idioms = %s AND language_code = %s",
        (idiom, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_preferred_spelling(cur, preferred_spelling: str, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with the given preferred spelling and language code."""
    cur.execute(
        "SELECT * FROM words WHERE preferred_spelling = %s AND language_code = %s",
        (preferred_spelling, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_tags(cur, tags: List[str], language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with the given tags and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM definitions WHERE tags = ANY(%s)) AND language_code = %s",
        (tags, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_sources(cur, sources: List[str], language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with the given sources and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM definitions WHERE sources = ANY(%s)) AND language_code = %s",
        (sources, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_definition(cur, definition: str, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with the given definition and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM definitions WHERE definition_text = %s) AND language_code = %s",
        (definition, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_definition_pattern(cur, pattern: str, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with definitions matching the given pattern and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM definitions WHERE definition_text LIKE %s) AND language_code = %s",
        (f"%{pattern}%", language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_definition_relation(cur, relation: str, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with definitions having the given relation and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM definition_relations WHERE relation_type = %s) AND language_code = %s",
        (relation, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_definition_category(cur, category: str, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with definitions in the given category and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM definition_categories WHERE category_name = %s) AND language_code = %s",
        (category, language_code)
    )
...
def get_words_by_definition_link(cur, link: str, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with definitions containing the given link and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM definition_links WHERE link_text = %s) AND language_code = %s",
        (link, language_code)
    )
...
def get_words_by_definition_metadata(cur, metadata: str, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with definitions containing the given metadata and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM definitions WHERE metadata = %s) AND language_code = %s",
        (metadata, language_code)
    )
...
def get_words_by_definition_example(cur, example: str, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with definitions containing the given example and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM definitions WHERE examples = %s) AND language_code = %s",
        (example, language_code)
    )
...
def get_words_by_definition_usage_note(cur, usage_note: str, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with definitions containing the given usage note and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM definitions WHERE usage_notes = %s) AND language_code = %s",
        (usage_note, language_code)
    )
...
def get_words_by_pronunciation_type(cur, pronunciation_type: str, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with pronunciations of the given type and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM pronunciations WHERE type = %s) AND language_code = %s",
        (pronunciation_type, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_pronunciation_tags(cur, tags: List[str], language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with pronunciations having the given tags and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM pronunciations WHERE tags = ANY(%s)) AND language_code = %s",
        (tags, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_pronunciation_metadata(cur, metadata: str, language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with pronunciations containing the given metadata and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM pronunciations WHERE metadata = %s) AND language_code = %s",
        (metadata, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_pronunciation_sources(cur, sources: List[str], language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with pronunciations from the given sources and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM pronunciations WHERE sources = ANY(%s)) AND language_code = %s",
        (sources, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_credit_sources(cur, sources: List[str], language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with credits from the given sources and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM credits WHERE sources = ANY(%s)) AND language_code = %s",
        (sources, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_etymology_sources(cur, sources: List[str], language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with etymologies from the given sources and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM etymologies WHERE sources = ANY(%s)) AND language_code = %s",
        (sources, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_relation_sources(cur, sources: List[str], language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with relations from the given sources and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT from_word_id FROM relations WHERE sources = ANY(%s)) AND language_code = %s",
        (sources, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_affixation_sources(cur, sources: List[str], language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with affixations from the given sources and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT affixed_word_id FROM affixations WHERE sources = ANY(%s)) AND language_code = %s",
        (sources, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_template_sources(cur, sources: List[str], language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with templates from the given sources and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM word_templates WHERE sources = ANY(%s)) AND language_code = %s",
        (sources, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_form_sources(cur, sources: List[str], language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with forms from the given sources and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM word_forms WHERE sources = ANY(%s)) AND language_code = %s",
        (sources, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_category_sources(cur, sources: List[str], language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with categories from the given sources and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM definition_categories WHERE sources = ANY(%s)) AND language_code = %s",
        (sources, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_link_sources(cur, sources: List[str], language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with links from the given sources and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM definition_links WHERE sources = ANY(%s)) AND language_code = %s",
        (sources, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_metadata_sources(cur, sources: List[str], language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with metadata from the given sources and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM definitions WHERE metadata = ANY(%s)) AND language_code = %s",
        (sources, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_badlit_form_sources(cur, sources: List[str], language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with badlit forms from the given sources and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM words WHERE badlit_form = ANY(%s)) AND language_code = %s",
        (sources, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_hyphenation_sources(cur, sources: List[str], language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with hyphenation from the given sources and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM words WHERE hyphenation = ANY(%s)) AND language_code = %s",
        (sources, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_proper_noun_sources(cur, sources: List[str], language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with proper noun status from the given sources and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM words WHERE is_proper_noun = ANY(%s)) AND language_code = %s",
        (sources, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_abbreviation_sources(cur, sources: List[str], language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with abbreviation status from the given sources and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM words WHERE is_abbreviation = ANY(%s)) AND language_code = %s",
        (sources, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_initialism_sources(cur, sources: List[str], language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with initialism status from the given sources and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM words WHERE is_initialism = ANY(%s)) AND language_code = %s",
        (sources, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_baybayin_sources(cur, sources: List[str], language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with Baybayin forms from the given sources and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM words WHERE baybayin_form = ANY(%s)) AND language_code = %s",
        (sources, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_romanized_sources(cur, sources: List[str], language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with romanized forms from the given sources and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM words WHERE romanized_form = ANY(%s)) AND language_code = %s",
        (sources, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_root_sources(cur, sources: List[str], language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with root words from the given sources and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM words WHERE root_word_id = ANY(%s)) AND language_code = %s",
        (sources, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_idiom_sources(cur, sources: List[str], language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with idioms from the given sources and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM words WHERE idioms = ANY(%s)) AND language_code = %s",
        (sources, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_preferred_spelling_sources(cur, sources: List[str], language_code: str = 'tl') -> List[Dict[str, Any]]:
    """Get all words with preferred spellings from the given sources and language code."""
    cur.execute(
        "SELECT * FROM words WHERE id IN (SELECT word_id FROM words WHERE preferred_spelling = ANY(%s)) AND language_code = %s",
        (sources, language_code)
    )
    return [dict(row) for row in cur.fetchall()]

def get_words_by_tags_sources(cur, sources: List[str], language_code: str = 'tl') -> List[Dict[str, Any]]:
#!/usr/bin/env python3
"""
dictionary_manager.py
        ...
        cur.execute("""
            ...
            DO UPDATE SET
                ...
                sources = CASE 
                    ...
                    ELSE pronunciations.sources || ', ' || EXCLUDED.sources
                END,
...
def process_marayum_json(cur, filename: str):
    """Process a Marayum dictionary JSON file."""
    ...
    try:
        ...
        if not isinstance(data, list):
            logger.error("Invalid JSON format - expected a list of entries")
            return stats
            
        stats["total_entries"] = len(data)
        source = os.path.basename(filename).replace('_processed.json', '')
        ...
        for entry in data:
            try:
                ...
                # Remove trailing numbers from word
                word = re.sub(r'\d+$', '', word)
                
                # Get language code from filename
                lang_code = source.split('-')[0] if '-' in source else 'tl'
                
                # Get POS
                pos = entry.get('pos', '').strip().lower()
                if pos:
                    pos = standardize_entry_pos(pos)
                
                # Process pronunciation
                pronunciation = entry.get('pronunciation', '').strip()
                ...
                for def_item in definitions:
                    try:
                        ...
                        if def_id:
                            ...
                            # Process definition relations
                            process_definition_relations(cur, def_id, def_text, source)
                ...
                if see_also:
                    relations_dict = {'see_also': see_also}
                    process_relations(cur, word_id, relations_dict, lang_code, source)
        ...
        logger.info(f"Processed {stats['entries_processed']} entries from {filename}")
        logger.info(f"Added {stats['definitions_added']} definitions")
        logger.info(f"Added {stats['pronunciations_added']} pronunciations")
        logger.info(f"Added {stats['etymologies_added']} etymologies")
        logger.info(f"Added {stats['credits_added']} credits")
...
def process_marayum_directory(cur, directory_path: str):
    ...
    if not os.path.exists(directory_path):
        # Try to create the base directory path if it doesn't exist
        os.makedirs(directory_path, exist_ok=True)
        logger.warning(f"Created Marayum dictionaries directory: {directory_path}")
        return 0, 0
    ...
    for json_file in json_files:
        logger.info(f"Processing Marayum dictionary file: {json_file}")
        try:
            # Extract dictionary name from filename for better logging
            dict_name = os.path.basename(json_file).replace('_processed.json', '')
    ...
    if total_processed > 0 or total_errors > 0:
        logger.info(f"Completed processing Marayum dictionaries: {total_processed} entries processed with {total_errors} errors")
    else:
        logger.warning("No entries were processed from any Marayum dictionary files")
...
def migrate_data(args):
    ...
    sources = [
        ...
        },
        {
            ...
            "file": "marayum_dictionaries/*_processed.json",  # Changed to only match processed files
    ...
    if hasattr(args, 'file') and args.file:
        ...
        source_found = False
        for source in sources:
            if source["file"] == basename or (os.path.isdir(filename) and os.path.basename(source["file"]) == os.path.basename(filename)):
                ...
                if os.path.isabs(source["file"]):
                    ...
                    for data_dir in data_dirs:
                        ...
                        if source.get("is_directory", False):
                            # For Marayum, check if any processed files exist
                            if source["name"] == "Project Marayum":
                                processed_files = glob.glob(potential_path)
                                if processed_files:
