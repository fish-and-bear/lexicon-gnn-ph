...
def create_or_update_tables(conn):
    ...
    try:
        ...
        BEGIN
            ...
            -- Check and increase column sizes if needed for word_relations
            IF EXISTS (
                SELECT FROM information_schema.columns 
                WHERE table_name='word_relations' AND column_name='relation_type' 
                AND character_maximum_length < 128
            ) THEN
                -- Increase size of relation_type column
                ALTER TABLE word_relations ALTER COLUMN relation_type TYPE VARCHAR(128);
            ...
            IF EXISTS (
                SELECT FROM information_schema.columns 
                WHERE table_name='word_relations' AND column_name='sources' 
                AND character_maximum_length < 255
            ) THEN
                -- Increase size of sources column
                ALTER TABLE word_relations ALTER COLUMN sources TYPE VARCHAR(255);
...
def insert_relation(
    ...
    Args:
        ...
        relation_type: Type of relationship (can be a RelationshipType enum or a string)
    ...
    # Extract the relation value and strength
    rel_value = relation_type.rel_value
    strength = relation_type.strength
    
    # Truncate sources if too long (with warning)
    max_source_length = 255  # Assuming this is the column max length
    ...
    if metadata:
        try:
            ...
            # Check length of metadata JSON
            max_metadata_length = 1024  # Adjust based on your DB column size
            if len(metadata_json) > max_metadata_length:
                logger.warning(f"Metadata too large ({len(metadata_json)}), truncating complex values")
                # Simplify metadata if too large by removing the largest values
                while len(metadata_json) > max_metadata_length and metadata:
                    # Find the key with the largest value
                    largest_key = max(metadata.items(), key=lambda x: len(str(x[1])))[0]
                    logger.debug(f"Removing large metadata key: {largest_key}")
                    metadata.pop(largest_key, None)
                    metadata_json = json.dumps(metadata)
                
                if len(metadata_json) > max_metadata_length:
                    logger.warning("Metadata still too large, storing empty metadata")
                    metadata_json = "{}"
        except Exception as e:
        ...
        if existing:
            ...
            cur.execute("""
                ...
                SET sources = %s,
                    ...
                    updated_at = NOW()
                WHERE id = %s
            """, (sources, metadata_json, strength, existing[0]))
            ...
            # If relation is bidirectional, add the inverse relation
            if relation_type.bidirectional:
                try:
                    # Don't create duplicates for bidirectional relations
                    cur.execute("""
                        SELECT id FROM word_relations 
                        WHERE from_word_id = %s AND to_word_id = %s AND relation_type = %s
                    ...
                    if not cur.fetchone():
                        ...
                        logger.debug(f"Inserted inverse bidirectional relation {rel_value} from word ID {to_word_id} to {from_word_id}")
                except Exception as e:
                    logger.error(f"Error inserting inverse relation: {str(e)}")
                    # Continue anyway, the primary relation was added successfully
