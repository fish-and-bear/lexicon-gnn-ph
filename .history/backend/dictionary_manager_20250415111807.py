...
from .models import RelationType # Or adjust path if RelationType is defined elsewhere (e.g., .enums)
...
# ---------------------------------------------------------------------------
#          MODIFIED: Main Processing Function with Batch Commits
# ---------------------------------------------------------------------------
def process_tagalog_words(filename: str, add_etymology: bool = True, batch_commit_size: int = 1000): # REMOVED `cur` argument
    """
    Process a JSON file containing Tagalog words and their definitions using
    manual transaction management with batch commits and iterative JSON parsing.

    Args:
        filename: Path to the JSON file
        add_etymology: Whether to add etymology data
        batch_commit_size: Number of entries to process before committing.

    Returns:
        Dictionary with processing statistics
    """
    logger.info(f"Processing Tagalog words file: {filename} with batch commits (size={batch_commit_size}).")

    # --- Initialize Statistics ---
    stats = {
        "entries_processed": 0, "skipped": 0, "definitions_added": 0,
        "pronunciations_added": 0, "etymologies_added": 0, "relations_added": 0,
        "errors": 0, # Errors during single entry processing (entry skipped)
        "critical_errors": 0 # Errors preventing processing (e.g., file not found, DB connection)
    }
    error_types = {} # Track specific error types encountered
    ...
    # --- Schema Check (Still useful, uses temporary connection) ---
    required_columns = {
        ...
        "definition_relations": {"id", "definition_id", "related_word_id", "relation_type", "sources"}, # If using definition_relations table
    }
    table_column_map = {}
    temp_conn = None # Initialize outside try
    try:
        temp_conn = get_connection() # Use non-pooled connection for quick check if possible
        with temp_conn.cursor() as temp_cur:
            all_tables = list(required_columns.keys())
            # Filter tables based on add_etymology flag if necessary
            ...
            for table, req_cols_set in required_columns.items():
                 # Only check tables we expect based on flags
        ...
        # Assume all columns might be missing if check fails
        for table in required_columns: table_column_map[table] = set()
    finally:
        if temp_conn:
            try:
                temp_conn.close()
                logger.debug("Temporary schema check connection closed.")
            except Exception as close_err:
                logger.error(f"Error closing temporary schema check connection: {close_err}")

    # --- Caches and Batches ---
    word_id_cache = {} # Key: normalized_lemma|language_code
    relations_batch = [] # Collect relations here

    # --- Main Processing Loop with Manual Transactions ---
    conn = None
    cur = None
    processed_in_batch = 0
    total_processed_logged = 0
    processed_count_log_interval = max(1000, batch_commit_size) # Log progress periodically
    ...
    try:
        ...
        try:
            ...
            # --- Get DB Connection & Start First Transaction ---
            conn = get_db_connection() # Get connection from pool
            cur = conn.cursor()
            cur.execute("BEGIN;") # Start manual transaction
            logger.debug("Initial transaction started.")
            ...
            for entry_index, (word_key, entry_data) in enumerate(entries_iterator):
                ...
                try:
                    ...
                    logger.error(f"Failed processing entry {entry_index} ('{word_key}'): {entry_err}", exc_info=False) # exc_info=False for less verbose logs per entry
                    ...
                    # DO NOT rollback here - continue the loop for other entries

                # --- Batch Commit Logic ---
                if processed_in_batch >= batch_commit_size:
                    try:
                        conn.commit() # Commit the current batch
                        logger.debug(f"Committed batch of {processed_in_batch} entries.")
                        cur.execute("BEGIN;") # Start next transaction
                        processed_in_batch = 0 # Reset batch counter
                    except Exception as commit_err:
                         logger.error(f"CRITICAL: Failed to commit batch after entry {entry_index}: {commit_err}", exc_info=True)
                         conn.rollback() # Rollback the failed batch
                         logger.warning("Rolled back failed batch. Trying to restart transaction.")
                         try: cur.execute("BEGIN;") # Try to start a new transaction
                         except Exception as restart_err:
                              logger.critical(f"CRITICAL: Cannot restart transaction after commit failure: {restart_err}. Aborting.", exc_info=True)
                              stats["critical_errors"] += 1; error_types["TransactionRestartFailed"] = 1
                              raise # Re-raise to stop processing

                # Log progress periodically
                current_total_processed = stats["entries_processed"] + stats["skipped"] + stats["errors"]
                if current_total_processed >= total_processed_logged + processed_count_log_interval:
                     logger.info(f"Processed ~{current_total_processed} entries...")
                     total_processed_logged = current_total_processed

            # --- Commit Final Batch ---
            if processed_in_batch > 0:
                try:
                    conn.commit()
                    logger.info(f"Committed final batch of {processed_in_batch} entries.")
                except Exception as final_commit_err:
                     logger.error(f"CRITICAL: Failed to commit final batch: {final_commit_err}", exc_info=True)
                     conn.rollback() # Rollback the failed final batch
                     stats["critical_errors"] += 1 # Count as critical since final data lost
                     error_types["FinalCommitFailed"] = 1
                # ----- These are the required clauses for the outer 'try' -----
                # ----- These are the required clauses for the outer 'try' -----
        except FileNotFoundError:
             logger.error(f"File not found: {filename}")
             stats["critical_errors"] += 1
             error_types["FileNotFound"] = 1
             # No rollback needed if conn wasn't established
        except Exception as e: # Catch other file/DB setup/iteration errors
             ...
             if conn and not conn.closed: # Rollback only if connection exists
                  try: conn.rollback()
                  except Exception as rb_err: logger.error(f"Error during rollback: {rb_err}")
        finally:
            # Ensure resources are closed/released
            if file_handle: file_handle.close()
            if cur: cur.close()
            if conn: release_db_connection(conn) # Release back to pool
            logger.debug("Main processing loop finished, resources released.")
        # ----- End of required clauses -----
        ...
        stats["critical_errors"] += 1
        error_types[f"FileProcessingError: {type(e).__name__}"] = error_types.get(f"FileProcessingError: {type(e).__name__}", 0) + 1
        if conn and not conn.closed: conn.rollback() # Rollback if transaction was open

    finally:
        if file_handle: file_handle.close()
        # Close cursor and connection IF they were opened
        if cur: cur.close()
        if conn: release_db_connection(conn) # Use your pool release function


    # --- Process Relations Batch (Still using transaction for the whole batch, but improved internal error handling) ---
    logger.info(f"Finished iterating entries. Processing {len(relations_batch)} collected relations...")
    if relations_batch:
        try:
            # Define relation processing helper locally or move outside if preferred
            # Applying transaction decorator here is OK for the batch, but we add internal try/except
            @with_db_transaction # Handles transaction for the whole batch
            def _process_relations_batch_transactional(cur, batch, stats_dict, cache, error_types_dict):
                processed_count = 0
                failed_count = 0
                ...
                for relation_info in tqdm(batch, desc="Processing Relations"):
                     try: # <-- ADDED: Try/Except around each relation
                          from_id = relation_info['word_id']
                          related_word_lemma = relation_info['related_word']
                          rel_type = relation_info['rel_type'] # Should be RelationType enum
                          src_id = relation_info['source_identifier']
                          metadata = relation_info.get('metadata')
                          ...
                          if not related_word_id:
                               # Use get_or_create_word_id which should handle its own logic now
                               related_word_id = get_or_create_word_id(cur, related_word_lemma, lang_code_for_related, src_id)
                               if related_word_id:
                                    cache[rel_word_cache_key] = related_word_id # Cache on success
                               else:
                                    logger.warning(f"Failed get/create related word '{related_word_lemma}' for relation. Skipping relation.")
                                    failed_count += 1; continue # Skip this relation

                          # Proceed only if we have a valid related_word_id and it's not self-referential
                          if related_word_id and related_word_id != from_id:
                               # Assume insert_relation uses the passed 'cur'
                               rel_id = insert_relation(cur, from_id, related_word_id, rel_type, src_id, metadata)
                               if rel_id:
                                    processed_count += 1
                                    stats_dict["relations_added"] += 1 # Use the passed stats dict

                                    # Insert definition relation if applicable
                                    if def_id and "definition_relations" in required_columns: # Check if table exists
                                        try: insert_definition_relation(cur, def_id, related_word_id, rel_type, src_id)
                                        except Exception as def_rel_err: logger.warning(f"Failed inserting definition relation: {def_rel_err}") # Log but continue

                                    # Handle bidirectional/inverse relations
                                    # Assume insert_relation handles unique constraints gracefully or logs them
                                    if rel_type.bidirectional:
                                        insert_relation(cur, related_word_id, from_id, rel_type, src_id, metadata)
                                    else:
                                         inverse_rel = rel_type.get_inverse()
                                         if inverse_rel:
                                             insert_relation(cur, related_word_id, from_id, inverse_rel, src_id, metadata)
                               # else: insert_relation likely returned None due to conflict/error (should log internally)

                     except psycopg2.errors.UniqueViolation:
                          logger.debug(f"Relation batch item skipped due to unique constraint: from={from_id}, to={related_word_id}, type={rel_type}")
                          # Don't increment failed_count for expected unique violations
                ...
                # The decorator handles commit/rollback based on *uncaught* exceptions from this function

            # Call the batch processor - it mutates stats and error_types directly
            _process_relations_batch_transactional(relations_batch, stats, word_id_cache, error_types)

        except Exception as final_rel_err:
            logger.error(f"CRITICAL error during final relations batch processing transaction: {final_rel_err}", exc_info=True)
            stats["critical_errors"] += 1 # Increment critical error count
            error_types["RelationsBatchTransactionError"] = error_types.get("RelationsBatchTransactionError", 0) + 1

    # --- Log Final Statistics ---
    logger.info(f"Finished processing {filename}")
    logger.info(f"  Entries processed successfully: {stats['entries_processed']}")
    logger.info(f"  Entries skipped (no lemma, wrong type): {stats['skipped']}")
    logger.info(f"  Entries failed processing (logged errors): {stats['errors']}")
    logger.info(f"  Critical errors (file, DB connection, commit): {stats['critical_errors']}")
