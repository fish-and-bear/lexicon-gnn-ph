...
def process_root_words_cleaned(cur, filename: str):
    ...
    if isinstance(data, list):  # Original list format
        ...
        if not all(isinstance(v, dict) for v in data.values()):
            ...
            stats["errors"] += 1
            error_types["InvalidDictValueFormat"] = error_types.get("InvalidDictValueFormat", 0) + 1
            return stats

        def dict_iterator(d):
            for i, (key, value) in enumerate(d.items()):
                entry_data = value.copy()
                entry_data["root_word"] = key
                yield i, entry_data
    ...
    # --- Process Entries ---
    commit_interval = 500  # Commit every 500 entries
    with tqdm(
        total=total_roots, desc=f"Processing {source_identifier}"
    ) as pbar:
        for entry_index, root_word_entry in entries_iterator:
            ...
            try:
                ...
                if not isinstance(root_word_entry, dict):
                    logger.warning(
                        f"Skipping invalid entry at index {entry_index} (not a dict after processing)"
                    )
                    stats["skipped"] += 1
                    cur.execute(f"RELEASE SAVEPOINT {savepoint_name}")
                    pbar.update(1)
                ...
                try:
                    word_id = get_or_create_word_id(
                        ...
                        language_code=language_code,
                        source_identifier=source_identifier,
                        is_root_word=True, # Optional flag
                    )
                    if not word_id or word_id <= 0:
                    ...
                    logger.error(f"CRITICAL FAILURE: Cannot get/create word ID for root '{root_word}'. Skipping entry. Error: {word_err}")
                    stats["errors"] += 1
                    error_types[f"RootWordCreationError: {type(word_err).__name__}"] = error_types.get(f"RootWordCreationError: {type(word_err).__name__}", 0) + 1
                    cur.execute(f"ROLLBACK TO SAVEPOINT {savepoint_name}")
                    pbar.update(1)
                    continue # Cannot proceed without root word ID
                ...
                if isinstance(definitions, list):
                    for def_idx, definition_item in enumerate(definitions):
                        ...
                        usage_notes: Optional[str] = None # Added for potential future use
                        ...
                        if isinstance(definition_item, str):
                            ...
                            if isinstance(raw_examples, list):
                                ...
                                if examples_list:
                                    ...
                                    except TypeError: logger.warning(f"Could not serialize examples for root '{root_word}', def {def_idx}")
                            elif isinstance(raw_examples, str) and raw_examples.strip():
                                try: examples_json = json.dumps([raw_examples.strip()])
                                except TypeError: logger.warning(f"Could not serialize example string for root '{root_word}', def {def_idx}")

                            # Extract tags
                            raw_tags = definition_item.get("tags")
                            if isinstance(raw_tags, list):
                                tags_list = [str(t).strip() for t in raw_tags if str(t).strip()]
                                if tags_list: tags_str = ", ".join(tags_list)
                            elif isinstance(raw_tags, str) and raw_tags.strip():
                                tags_str = raw_tags.strip()

                            # Extract usage notes (if present in source)
                            raw_notes = definition_item.get("usage_notes") or definition_item.get("notes")
                            if isinstance(raw_notes, str) and raw_notes.strip():
                                usage_notes = raw_notes.strip()

                        # Remove trailing ellipsis "..." from definition text
                        ...
                        if definition_text:
                            try:
                                ...
                                if def_id:
                                    ...
                                    logger.warning(f"insert_definition failed for root '{root_word}' (ID: {word_id}), def {def_idx}. Check internal logs.")
                                    error_key = f"RootDefInsertFailure"
                                    error_types[error_key] = error_types.get(error_key, 0) + 1
                            except Exception as def_e:
                                logger.error(f"Error inserting definition for root '{root_word}', def {def_idx}: {definition_text[:50]}... Error: {def_e}", exc_info=True)
                                error_key = f"RootDefInsertError: {type(def_e).__name__}"
                                error_types[error_key] = error_types.get(error_key, 0) + 1

                # --- Process associated words ---
                associated_data = root_word_entry.get("associated_words", {})
                ...
                if isinstance(associated_data, dict):
                    ...
                    logger.warning(f"Unexpected format for 'associated_words' for root '{root_word}'. Skipping.")
                        ...
                        try:
                            ...
                            if assoc_word_id:
                                ...
                                if rel_id:
                                    ...
                                    logger.debug(f"DERIVED_FROM relation {assoc_word_id} -> {word_id} might already exist.")
                                    error_key = f"DerivedFromRelationInsertFailure" # Maybe count these?
                                    error_types[error_key] = error_types.get(error_key, 0) + 1
                                ...
                                assoc_pos = assoc_details.get("type", "").strip() or None # Use None if empty
                                ...
                                if assoc_def_text:
                                    try:
                                        ...
                                        if assoc_def_id:
                                            ...
                                            logger.warning(f"insert_definition failed for associated word '{assoc_word}' (ID: {assoc_word_id}). Check internal logs.")
                                            error_key = f"AssocDefInsertFailure"
                                            error_types[error_key] = error_types.get(error_key, 0) + 1
                                    except Exception as assoc_def_e:
                                        logger.error(f"Error inserting definition for associated word '{assoc_word}' (ID: {assoc_word_id}): {assoc_def_e}", exc_info=True)
                                        error_key = f"AssocDefInsertError: {type(assoc_def_e).__name__}"
                                        error_types[error_key] = error_types.get(error_key, 0) + 1
                        except Exception as assoc_e:
                            logger.error(f"Failed to process/relate associated word '{assoc_word}' (index {assoc_idx}) for root '{root_word}': {assoc_e}", exc_info=True)
                            stats["errors"] += 1 # Count error in associated word processing
                            error_key = f"AssociatedWordProcessingError: {type(assoc_e).__name__}"
                            error_types[error_key] = error_types.get(error_key, 0) + 1
                            # If assoc_word_id creation failed, we might not have it here.
                            # Decide if this warrants rolling back the whole root entry.
                            # For now, log the error and continue with the root entry.

                # --- Finish Entry Processing ---
                cur.execute(f"RELEASE SAVEPOINT {savepoint_name}")

                # --- Periodic Commit ---
                if (entry_index + 1) % commit_interval == 0:
                    try:
                        conn.commit()
                        logger.info(f"Committed {commit_interval} entries for {filename}. Total processed so far: {entry_index + 1}")
                    except (psycopg2.InterfaceError, psycopg2.OperationalError) as commit_conn_err:
                         logger.error(f"Connection error during periodic commit for {filename} at entry {entry_index}: {commit_conn_err}. Stopping file processing.", exc_info=True)
                         stats["errors"] += (total_roots - (entry_index + 1)) # Count remaining as errors
                         error_types["PeriodicCommitConnectionError"] = error_types.get("PeriodicCommitConnectionError", 0) + 1
                         pbar.update(total_roots - (entry_index + 1)) # Update progress bar
                         return stats # Stop processing this file
                    except Exception as periodic_commit_err:
                        logger.error(f"Error during periodic commit for {filename} at entry {entry_index}: {periodic_commit_err}. Rolling back...", exc_info=True)
                        stats["errors"] += 1 # Count commit error
                        error_types["PeriodicCommitError"] = error_types.get("PeriodicCommitError", 0) + 1
                        try:
                            conn.rollback()
                            logger.info("Transaction rolled back after periodic commit error.")
                        except Exception as rb_err:
                            logger.critical(f"CRITICAL: Failed to rollback after periodic commit error for {filename}: {rb_err}. Stopping file processing.", exc_info=True)
                            stats["errors"] += (total_roots - (entry_index + 1))
                            error_types["PeriodicCommitRollbackError"] = error_types.get("PeriodicCommitRollbackError", 0) + 1
                            pbar.update(total_roots - (entry_index + 1))
                            return stats # Stop processing

            except Exception as entry_e:
                logger.error(
                    f"UNEXPECTED error processing root entry index {entry_index} ('{root_word or 'N/A'}'): {entry_e}",
                    exc_info=True,
                )
                stats["errors"] += 1
                error_key = f"UnexpectedRootEntryError: {type(entry_e).__name__}"
                error_types[error_key] = error_types.get(error_key, 0) + 1
                try:
                    cur.execute(f"ROLLBACK TO SAVEPOINT {savepoint_name}")
                except Exception as rb_err:
                    logger.critical(
                        f"CRITICAL: Failed rollback to savepoint {savepoint_name} after entry error: {rb_err}. Attempting full transaction rollback."
                    )
                    try:
                        conn.rollback()
                        logger.info("Full transaction rolled back due to savepoint rollback failure.")
                    except Exception as full_rb_err:
                        logger.critical(f"CRITICAL: Failed even full transaction rollback: {full_rb_err}. Stopping file processing.")
                        stats["errors"] += (total_roots - (entry_index + 1)) # Count remaining as errors
    ...
    # --- Final Commit ---
    try:
        # Check connection status before final commit
        if conn is None or conn.closed or conn.status != psycopg2.extensions.STATUS_READY:
            logger.error(f"Connection not available or not ready for final commit for {filename}. Data might be inconsistent.")
            stats["errors"] += 1 # Count as error
            error_types["FinalCommitConnectionError"] = error_types.get("FinalCommitConnectionError", 0) + 1
        else:
            conn.commit()
            logger.info(
                f"Finished processing {filename}. Final commit successful. Stats: {json.dumps(stats, indent=2)}"
            )
    except Exception as final_commit_err:
        logger.error(
            f"Error during final commit for {filename}: {final_commit_err}. Rolling back changes...", exc_info=True
        )
        stats["errors"] += 1
        error_types["FinalCommitError"] = error_types.get("FinalCommitError", 0) + 1
        try:
             ...
             logger.error(f"Failed to rollback after final commit error for {filename}: {rb_err}")
             error_types["FinalCommitRollbackError"] = error_types.get("FinalCommitRollbackError", 0) + 1

    if error_types:
        logger.warning(f"Error summary for {filename}: {json.dumps(error_types, indent=2)}")
    if stats["processed"] == 0 and total_roots > 0:
