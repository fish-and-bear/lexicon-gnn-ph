...
def setup_extensions(conn, required_only=False) -> bool:
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute("""
                -- Create Filipino text search configuration
                CREATE TEXT SEARCH CONFIGURATION IF NOT EXISTS filipino (COPY = simple);
                ALTER TEXT SEARCH CONFIGURATION filipino
                    ALTER MAPPING FOR asciiword, word, numword, asciihword, hword, numhword
                    WITH unaccent, simple;
                
                -- Create Baybayin text search configuration
                CREATE TEXT SEARCH CONFIGURATION IF NOT EXISTS baybayin (COPY = simple);
            ...
            cur.execute("""
                ...
                CREATE OR REPLACE FUNCTION word_similarity(text, text) RETURNS float
                LANGUAGE sql IMMUTABLE
                AS $$
                    SELECT similarity(normalize_search_text($1), normalize_search_text($2));
                $$;
