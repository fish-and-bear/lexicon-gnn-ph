...
def lookup_word(args):
    """Look up word details with improved formatting."""
    try:
        ...
        with get_connection() as conn:
            with conn.cursor() as cur:
                # Get word details
                cur.execute("""
                    SELECT w.id, w.lemma, w.language_code, w.baybayin_form, w.romanized_form,
                           w.pronunciation_data, w.has_baybayin
                    FROM words w
                ...
                word_id, lemma, lang_code, baybayin_form, romanized_form, pronunciation, has_baybayin = word
                
                # Get definitions
                cur.execute("""
                    SELECT d.definition_text, d.examples, d.usage_notes, d.sources,
                ...
                # Create header panel for word information
                word_info = []
                word_info.append(f"Word: {lemma}")
                word_info.append(f"Language: {lang_code}")

                # Get unique sources
                all_sources = set()
                for d in definitions:
                    if d[3]:  # sources field
                        all_sources.update(s.strip() for s in d[3].split(','))
                
                # Group sources by type
                dictionaries = []
                online_resources = []
                for source in sorted(all_sources):
                    if "Dictionary" in source or "Diksiyonaryo" in source:
                        dictionaries.append(source)
                    else:
                        online_resources.append(source)

                if dictionaries:
                    word_info.append(f"Dictionaries: {', '.join(dictionaries)}")
                if online_resources:
                    word_info.append(f"Online Resources: {', '.join(online_resources)}")

                # Create and display the word information panel
                console.print("\n" + " " * 48 + "Word Information")
                console.print(Panel("\n".join(word_info), expand=True))
                
                # Display Baybayin if available
                if has_baybayin and baybayin_form:
                    console.print(f"\n[bold cyan]{baybayin_form}[/] [dim]({romanized_form or get_romanized_text(baybayin_form)})[/]")
                
                # Display pronunciation
                if pronunciation:
                    pron_data = json.loads(pronunciation)
                    console.print(f"\n[italic]{pron_data.get('text', '')}[/]")
                ...
                if definitions:
                    ...
                    table.add_column("Definition", style="bold")
                    table.add_column("Examples", style="green")
                    table.add_column("Usage Notes", style="yellow")
                    table.add_column("Sources", style="magenta")
                    
                    current_pos = None
                    for definition in definitions:
                        def_text, examples, usage_notes, sources, pos_code, pos_name = definition
                        ...
                        # Display POS
                        display_pos = pos_name or "Hindi Tiyak"
                ...
                if etymologies:
                    ...
                    for etymology in etymologies:
                        etym_text, components, langs, sources = etymology
                        etymology_content.append(etym_text)
                        
                        if components:
                            try:
                                comp_list = json.loads(components)
                                if comp_list:
                                    etymology_content.append("\nComponents:")
                                    for comp in comp_list:
                                        etymology_content.append(f"- {comp}")
                            except json.JSONDecodeError:
                                pass
                        
                        if langs:
                            etymology_content.append("\nLanguages:")
                            etymology_content.append(langs)
                        
                        if sources:
                            etymology_content.append("\nSources:")
                            etymology_content.append(sources)
                    
                    console.print(Panel("\n".join(etymology_content), expand=True))
                
                # Get and display relations
                cur.execute("""
                    SELECT r.relation_type, w2.lemma, w2.language_code, r.sources
                    FROM relations r
                ...
                if relations:
                    console.print("\n" + " " * 50 + "Related Words")
                    table = Table(box=box.SIMPLE)
                    table.add_column("Relation Type", style="blue")
                    table.add_column("Related Words", style="bold")
                    
                    current_type = None
                    related_words = []
                    
                    for relation in relations:
                        rel_type, rel_word, rel_lang, sources = relation
                        if rel_type != current_type:
                            ...
                            related_words = []
                        related_words.append(f"{rel_word} [{rel_lang}]")
