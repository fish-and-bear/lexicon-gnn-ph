...
for metric in list(REGISTRY._names_to_collectors.values()):
    try:
        if hasattr(metric, 'name') and metric.name in ['server_uptime_seconds', 'health_check_status', 'database_metrics']:
            REGISTRY.unregister(metric)
    except (KeyError, AttributeError):
        pass

# Initialize metrics
SERVER_UPTIME = Histogram('server_uptime_seconds', 'Server uptime in seconds')
HEALTH_CHECK_STATUS = Counter('health_check_status', 'Health check status (1=healthy, 0=unhealthy)')
DB_METRICS = Counter('database_metrics', 'Database metrics', ['metric'])
