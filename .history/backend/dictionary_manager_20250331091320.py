...
def get_language_mapping():
    ...
    language_map = {}
    try:
        # Get all processed dictionary files
        ...
        for json_file in json_files:
            try:
                with open(json_file, 'r', encoding='utf-8') as f:
                    ...
                    if '-' in base_language:
                        # Store both the full name and its parts
    ...
    # Add some common alternative names/spellings
    alternatives = {
        'tagalog': 'tgl',
        'filipino': 'tgl',
        'pilipino': 'tgl',
    ...
    # Only add alternatives if they don't conflict with discovered mappings
    for alt, code in alternatives.items():
        if alt not in language_map:
            language_map[alt] = code
    
    return language_map

def get_language_code(language: str) -> str:
    """Get ISO code for a language name, dynamically mapped from dictionary files."""
    if not hasattr(get_language_code, '_language_map'):
        # Cache the language mapping
        get_language_code._language_map = get_language_mapping()
    ...
    if lang in get_language_code._language_map:
        return get_language_code._language_map[lang]
    
    # Try without region/variant specifiers
    base_lang = lang.split('-')[0]
    if base_lang in get_language_code._language_map:
        return get_language_code._language_map[base_lang]
    
    # If we still don't have a mapping, log it and return the base name
...
def display_language_support():
    """Display all supported languages and their variants."""
    languages = get_supported_languages()
    console = Console()
    
    table = Table(title="Supported Languages")
    table.add_column("Code", style="cyan")
    table.add_column("Primary Name", style="green")
    table.add_column("Variants", style="yellow")
    
    for lang in languages:
        variants = ", ".join(lang['variants']) if lang['variants'] else "-"
        table.add_row(lang['code'], lang['name'], variants)
