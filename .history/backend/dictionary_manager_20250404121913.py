...
POS_MAPPING = {
    ...
    "interjection": "intj",
    "pandamdam": "intj",
    "padamdam": "intj",  # Add this variation
    "intj": "intj",
    "damdam": "intj",
    # Articles
    "article": "art",
    ...
    "Pantukoy": "art",
    "Panlapi": "aff",
    "Pang-angkop": "part",  # Add this one
    "Pangkayarian": "suff",  # Structure marker
    
    # Add mappings for the context-based descriptions
    "sa sanga ng punongkahoy": "n",  # These are typically describing nouns
    ...
    "unknown": "unc"  # Map unknown to uncategorized
}
...
def get_standard_code(pos_string: str) -> str:
    ...
    for key, code in LOWERCASE_POS_MAPPING.items():
        if key in pos_key:
            return code

    # Log the unmapped POS for future improvements
    print(f"WARNING: Unmapped POS: '{pos_string}' -> mapped to 'unc'")
    ...
    if isinstance(pos_str, list) and pos_str:
        ...
        for pos_item in pos_str:
            if pos_item and isinstance(pos_item, str) and pos_item.strip():
                std_code = get_standard_code(pos_item)
                if std_code != "unc":  # If we found a valid code, use it
                    return std_code
        return "unc"
...
def analyze_pos_data(data_file, sample_size=1000):
    ...
    print(f"Found {len(pos_counts)} unique POS types in sample")
    print("\nTop 20 POS types:")
    ...
    unmapped = [orig for (orig, mapped), _ in mapping_counts.items() if mapped == "unc"]
    if unmapped:
        print(f"\nFound {len(unmapped)} unmapped POS types:")
