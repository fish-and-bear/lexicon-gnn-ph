...
from sqlalchemy import or_, and_, func, desc, text, event, not_, case, exists
from models import Word, Definition, Etymology, Relation, DefinitionRelation, Affixation, PartOfSpeech, Pronunciation, db
...
from dictionary_manager import transliterate_to_baybayin, get_romanized_text, get_pronunciation, get_respelling
from default_mappings import LanguageMappings, VALID_CODES
...
class WordQuerySchema(Schema):
    ...
    language_code = fields.Str(validate=validate.OneOf(list(VALID_CODES)), default='tl')
...
class SearchQuerySchema(Schema):
    ...
    language = fields.Str(validate=validate.OneOf(list(VALID_CODES)), default='tl')
    include_baybayin = fields.Bool(default=True)
...
def index():
    ...
    for code in VALID_CODES:
        info = LanguageMappings.get_language_info(code)
        if info:
            languages.append({
                "code": code,
                "name": info.name,
                ...
                "writing_systems": info.writing_systems,
                "endangered": info.endangered,
                "extinct": info.extinct,
                "revitalization_status": info.revitalization_status
            })

    # Group languages by family and region
    by_family = {}
    by_region = {}
    for lang in languages:
        ...
        if family not in by_family:
            by_family[family] = []
        by_family[family].append(lang["code"])
        ...
        if region:
            if region not in by_region:
                by_region[region] = {}
            subregion = lang["subregion"] or "Other"
            if subregion not in by_region[region]:
                by_region[region][subregion] = []
            by_region[region][subregion].append(lang["code"])
    ...
    return jsonify({
        ...
        "languages": {
            ...
            "by_region": by_region,
            "total": len(languages),
            "endangered": len([l for l in languages if l["endangered"]]),
            "extinct": len([l for l in languages if l["extinct"]])
        }
    })
...
def get_word(word, **params):
    ...
    Query Parameters:
        ...
        include_baybayin (bool): Include Baybayin script (default: True)
        include_romanized (bool): Include romanized form (default: True)
    ...
    try:
        ...
        include_romanized = params.get('include_romanized', True)
        language_code = request.args.get('language_code', 'tl')
        
        # Validate language code
        if not LanguageMappings.validate_code(language_code):
            return error_response("Invalid language code", 400)
        
        # Get language info
        language_info = LanguageMappings.get_language_info(language_code)
        if not language_info:
            return error_response("Language not supported", 400)
        ...
        if include_pronunciation:
            query_options.append(joinedload(Word.pronunciations))
            
        # Get word
        word_obj = Word.query.options(*query_options).filter(
            and_(
        ...
        if not word_obj:
            # Try to find similar words in the same language
            ...
            # Get related languages that might have this word
            related_languages = LanguageMappings.get_related_languages(language_code)
            if related_languages:
                # Check for exact word in related languages
                related_matches = Word.query.filter(
                    Word.normalized_lemma == normalize_word(word),
                    Word.language_code.in_(related_languages)
                ).all()
                
                if related_matches:
                    for match in related_matches:
                        match_info = LanguageMappings.get_language_info(match.language_code)
                        suggestions.append({
                            "word": match.lemma,
                            "normalized_lemma": match.normalized_lemma,
                            "language": {
                                "code": match.language_code,
                                "name": match_info.name if match_info else match.language_code
                            },
                            "similarity": 1.0
                        })
        ...
        word_data = {
            ...
            "language": {
                "code": word_obj.language_code,
                "name": language_info.name,
                "family": language_info.family,
                "subfamily": language_info.subfamily,
                "branch": language_info.branch,
                "region": language_info.region,
                "writing_systems": language_info.writing_systems
            },
            "has_baybayin": word_obj.has_baybayin,
            "created_at": word_obj.created_at.isoformat() if word_obj.created_at else None,
            "updated_at": word_obj.updated_at.isoformat() if word_obj.updated_at else None
        }
        
        # Add Baybayin and romanized forms if available
        if include_baybayin and word_obj.has_baybayin:
            word_data["baybayin_form"] = word_obj.baybayin_form
            if include_romanized and word_obj.romanized_form:
                word_data["romanized_form"] = word_obj.romanized_form
                
        # Add definitions
        if include_definitions and word_obj.definitions:
            word_data["definitions"] = []
            for definition in word_obj.definitions:
                def_data = {
                    "id": definition.id,
                    "definition_text": definition.definition_text,
                    "part_of_speech": definition.standardized_pos.code if definition.standardized_pos else None,
                    "pos_name": definition.standardized_pos.name_en if definition.standardized_pos else None
                }
                
                # Add examples if present
                if definition.examples:
                    try:
                        def_data["examples"] = json.loads(definition.examples)
                    except json.JSONDecodeError:
                        def_data["examples"] = [{"text": ex.strip()} for ex in definition.examples.split('\n') if ex.strip()]
                        
                # Add usage notes if present
                if definition.usage_notes:
                    try:
                        def_data["usage_notes"] = json.loads(definition.usage_notes)
                    except json.JSONDecodeError:
                        def_data["usage_notes"] = [note.strip() for note in definition.usage_notes.split('\n') if note.strip()]
                        
                # Add definition relations if requested
                if include_definition_relations and definition.definition_relations:
                    def_data["related_definitions"] = [
                        {
                            "word": rel.word.lemma,
                            "type": rel.relation_type,
                            "metadata": rel.metadata
                        }
                        for rel in definition.definition_relations
                    ]
                    
                # Add sources if requested
                if include_source_info and definition.sources:
                    def_data["sources"] = definition.sources.split(', ')
        ...
        if include_etymology and word_obj.etymologies:
            ...
            for etymology in word_obj.etymologies:
                etym_data = {
                    "etymology_text": etymology.etymology_text,
                    "language_codes": etymology.language_codes.split(',') if etymology.language_codes else []
                }
                
                if etymology.normalized_components:
                    try:
                        etym_data["components"] = json.loads(etymology.normalized_components)
                    except json.JSONDecodeError:
                        etym_data["components"] = [
                            {"text": comp.strip()} 
                            for comp in etymology.normalized_components.split(';')
                            if comp.strip()
                        ]
                        
                if include_source_info and etymology.sources:
                    etym_data["sources"] = etymology.sources.split(', ')
                    
                word_data["etymologies"].append(etym_data)
        ...
        if include_relations:
            ...
            for rel in word_obj.relations_from:
                ...
                target_word = rel.to_word.lemma
                target_lang = LanguageMappings.get_language_info(rel.to_word.language_code)
                
                relation_data = {
                    "word": target_word,
                    "language": {
                        "code": rel.to_word.language_code,
                        "name": target_lang.name if target_lang else rel.to_word.language_code
                    }
                }
                
                if rel_type in ['synonym', 'equals']:
                    relations_data["synonyms"].append(relation_data)
                elif rel_type == 'antonym':
                    relations_data["antonyms"].append(relation_data)
                elif rel_type in ['variant', 'spelling_variant', 'regional_variant']:
                    relations_data["variants"].append(relation_data)
                elif rel_type == 'derived_from':
                    relations_data["derived_from"].append(relation_data)
                elif rel_type == 'hypernym':
                    relations_data["hypernyms"].append(relation_data)
                elif rel_type == 'hyponym':
                    relations_data["hyponyms"].append(relation_data)
                elif rel_type in ['related', 'similar', 'compare_with', 'see_also']:
                    relations_data["related"].append(relation_data)
                    
            # Process incoming relations
            for rel in word_obj.relations_to:
                rel_type = rel.relation_type.lower()
                source_word = rel.from_word.lemma
                source_lang = LanguageMappings.get_language_info(rel.from_word.language_code)
                
                relation_data = {
                    "word": source_word,
                    "language": {
                        "code": rel.from_word.language_code,
                        "name": source_lang.name if source_lang else rel.from_word.language_code
                    }
                }
                
                if rel_type == 'derived_from':
                    relations_data["root_of"].append(relation_data)
                elif rel_type in ['synonym', 'equals', 'variant', 'spelling_variant', 'regional_variant']:
                    if source_word not in relations_data["synonyms"] and source_word not in relations_data["variants"]:
                        relations_data["related"].append(relation_data)
                        
            word_data["relationships"] = relations_data
        ...
        if include_affixes:
            ...
            # Process affixations where this word is the root
            for affixation in word_obj.affixations_as_root:
                affixed_lang = LanguageMappings.get_language_info(affixation.affixed_word.language_code)
                affixes_data["as_root"].append({
                    "affixed_word": affixation.affixed_word.lemma,
                    "language": {
                        "code": affixation.affixed_word.language_code,
                        "name": affixed_lang.name if affixed_lang else affixation.affixed_word.language_code
                    },
                    "affix_type": affixation.affix_type,
                    "sources": affixation.sources.split(', ') if include_source_info and affixation.sources else []
                })
                
            # Process affixations where this word is the affixed form
            for affixation in word_obj.affixations_as_affixed:
                root_lang = LanguageMappings.get_language_info(affixation.root_word.language_code)
                affixes_data["as_affixed"].append({
                    "root_word": affixation.root_word.lemma,
                    "language": {
                        "code": affixation.root_word.language_code,
                        "name": root_lang.name if root_lang else affixation.root_word.language_code
                    },
                    "affix_type": affixation.affix_type,
                    "sources": affixation.sources.split(', ') if include_source_info and affixation.sources else []
                })
                
            word_data["affixations"] = affixes_data
            
        # Add pronunciation
        if include_pronunciation and word_obj.pronunciations:
            word_data["pronunciations"] = []
            for pron in word_obj.pronunciations:
                try:
                ...
                if include_source_info and pron.sources:
                    pron_data["sources"] = pron.sources.split(', ')
                    
                word_data["pronunciations"].append(pron_data)
                
        # Add metadata
        if include_metadata:
            word_data["metadata"] = {
                "has_definitions": bool(word_obj.definitions),
                "has_etymology": bool(word_obj.etymologies),
                "has_pronunciation": bool(word_obj.pronunciations),
                "has_relations": bool(word_obj.relations_from or word_obj.relations_to),
                "has_affixations": bool(word_obj.affixations_as_root or word_obj.affixations_as_affixed),
                "definition_count": len(word_obj.definitions),
                "etymology_count": len(word_obj.etymologies),
                "pronunciation_count": len(word_obj.pronunciations),
                "relation_count": len(word_obj.relations_from) + len(word_obj.relations_to),
                "affixation_count": len(word_obj.affixations_as_root) + len(word_obj.affixations_as_affixed)
            }
            
            if word_obj.tags:
                try:
                    word_data["metadata"]["tags"] = json.loads(word_obj.tags) if isinstance(word_obj.tags, str) else word_obj.tags
                except json.JSONDecodeError:
                    word_data["metadata"]["tags"] = [tag.strip() for tag in word_obj.tags.split(',') if tag.strip()]
                    
        # Add source info
        if include_source_info and word_obj.sources:
            word_data["sources"] = word_obj.sources.split(', ')
        ...
        return success_response(
            ...
            meta={
                ...
                "includes": {
                    ...
                    "baybayin": include_baybayin,
                    "romanized": include_romanized
                }
            }
        )
...
def search_words(**params):
    ...
    Query Parameters:
        ...
        filter_has_baybayin (bool): Filter words with Baybayin script (default: False)
    
    Returns:
        List of matching words with requested details
    """
    try:
        # Extract parameters
        query = params.get('q')
        ...
        filter_has_baybayin = params.get('filter_has_baybayin', False)
        ...
        # Add eager loading based on includes
        if include_definitions:
            base_query = base_query.options(
                joinedload(Word.definitions).joinedload(Definition.standardized_pos)
            )
        if include_etymology:
            base_query = base_query.options(joinedload(Word.etymologies))
        if include_pronunciation:
            base_query = base_query.options(joinedload(Word.pronunciations))
            
        # Apply filters
        filters = []
        
        # Language filter
        filters.append(Word.language_code == language)
            ...
            filters.append(
                exists().where(
                    and_(
                        ...
                        Definition.standardized_pos_id == PartsOfSpeech.id,
                        PartsOfSpeech.code == pos
                    )
                )
            )
        ...
        if filter_has_etymology:
            filters.append(exists().where(Etymology.word_id == Word.id))
        if filter_has_pronunciation:
            filters.append(exists().where(Pronunciation.word_id == Word.id))
        if filter_has_baybayin:
            filters.append(Word.has_baybayin == True)
        ...
        if mode == 'exact':
            ...
            filters.append(
                or_(
                    Word.lemma.op('~*')(f'^{query}'),
                    Word.normalized_lemma.op('~*')(f'^{normalized_query}'),
                    Word.search_text.op('@@')(func.phraseto_tsquery('simple', query))
                )
            )
        ...
        elif mode == 'etymology' and include_etymology:
            filters.append(
                exists().where(
                    and_(
                        Etymology.word_id == Word.id,
            ...
            filters.append(
                or_(
                    func.similarity(Word.lemma, query) > min_similarity,
                    func.similarity(Word.normalized_lemma, normalized_query) > min_similarity,
                    Word.search_text.op('@@')(func.websearch_to_tsquery('simple', query))
                )
            )
        else:  # mode == 'all'
            filters.append(
                or_(
                    ...
                    Word.search_text.op('@@')(func.plainto_tsquery('simple', query)),
                    and_(
                        Word.has_baybayin == True,
                        Word.baybayin_form.isnot(None),
                        Word.baybayin_form.like(f'%{query}%')
                    ...
                    ) if include_etymology else False
                )
            )
        ...
        if sort == 'alphabetical':
            base_query = base_query.order_by(
                Word.lemma.asc() if order == 'asc' else Word.lemma.desc()
            )
        elif sort == 'created':
            base_query = base_query.order_by(
                Word.created_at.asc() if order == 'asc' else Word.created_at.desc()
            )
        elif sort == 'updated':
            base_query = base_query.order_by(
                Word.updated_at.asc() if order == 'asc' else Word.updated_at.desc()
            )
        elif sort == 'etymology':
            ...
            if mode == 'fuzzy':
                ...
                base_query = base_query.order_by(
                    case(
                        (Word.lemma.ilike(query), 1.0),
                        (Word.normalized_lemma == normalized_query, 0.9),
                        (Word.lemma.ilike(f'{query}%'), 0.8),
                        (Word.normalized_lemma.like(f'{normalized_query}%'), 0.7),
                        else_=0.0
                    ).desc()
                )
        ...
        for word in words:
            ...
            if word.has_baybayin and include_baybayin:
                word_data["baybayin_form"] = word.baybayin_form
                word_data["romanized_form"] = word.romanized_form
                
            if include_definitions and word.definitions:
                word_data["definitions"] = [
                    {
                        "definition_text": d.definition_text,
                        "part_of_speech": d.standardized_pos.code if d.standardized_pos else None,
                        "pos_name": d.standardized_pos.name_en if d.standardized_pos else None
                    }
            ...
            if include_etymology and word.etymologies:
                word_data["etymologies"] = [
                    ...
                    for e in word.etymologies
                ]
                
            if include_pronunciation and word.pronunciations:
                word_data["pronunciations"] = [
                    {
                        "type": p.type,
                        "value": p.value
            ...
            if include_source_info and word.sources:
                word_data["sources"] = word.sources.split(', ')
        ...
        return success_response(
            ...
            meta={
                ...
                "filters": {
                    "language": language,
                    "pos": pos,
                    "has_etymology": filter_has_etymology,
                    "has_pronunciation": filter_has_pronunciation,
                    "has_baybayin": filter_has_baybayin,
                    "has_idioms": filter_has_idioms
                }
            }
        )
        ...
        return error_response("Failed to search words")
