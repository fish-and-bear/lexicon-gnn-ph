...
def process_marayum_directory(cur, directory_path: str):
    """Process all processed JSON files in a directory."""
    logger.info(f"Processing Marayum dictionaries from directory: {directory_path}")
    
    # Find all processed JSON files
    json_files = []
    try:
        for file in glob.glob(os.path.join(directory_path, "*_processed.json")):
    ...
    with tqdm(total=len(json_files), desc="Processing Marayum dictionaries", unit="files") as pbar:
        ...
        for file_index, json_file in enumerate(json_files):
            try:
                ...
                pbar.set_description(f"Processing {dict_name}")
                
                # Initialize file-specific statistics
                file_stats = {
                    'processed': 0,
                    'errors': 0,
                    'skipped': 0,
                ...
                with open(json_file, 'r', encoding='utf-8') as f:
                    try:
                        ...
                        # Initialize inner progress bar for entries in this file
                        total_entries = len(data) if isinstance(data, list) else 0
                        if total_entries == 0:
                            logger.info(f"Skipping {dict_name} as it has 0 entries")
                            pbar.update(1)
                            continue
                        
                        with tqdm(total=total_entries, desc=f"Entries in {dict_name}", unit="entries", leave=False) as inner_pbar:
                            ...
                            for entry_index, entry in enumerate(data):
                                try:
                                            ...
                                            if isinstance(definition, str) and definition.strip():
                                                try:
                                                    insert_definition(
                                                        ...
                                                        word_id,
                                                        definition.strip(),
                                                        sources=f"marayum_{dict_name}"
                                                    )
                                                    file_stats['definitions'] += 1
                                                except Exception as def_error:
                                                    logger.error(f"Error inserting definition for '{word}': {str(def_error)}")
                                    
                                    # Process examples
                                    if 'examples' in entry and isinstance(entry['examples'], list):
                                        examples = [ex for ex in entry['examples'] if isinstance(ex, str) and ex.strip()]
                                        if examples:
                                            try:
                                                cur.execute("""
                                                    UPDATE words 
                                                    SET examples = examples || %s::jsonb
                                                    WHERE id = %s
                                                """, (json.dumps(examples), word_id))
                                                file_stats['examples'] += len(examples)
                                            except Exception as ex_error:
                                                logger.error(f"Error adding examples for '{word}': {str(ex_error)}")
                                    
                                    # Process relations
                                    if 'related' in entry and isinstance(entry['related'], dict):
                                        for rel_type, related_words in entry['related'].items():
                                            if isinstance(related_words, list):
                                                for related_word in related_words:
                                                    if isinstance(related_word, str) and related_word.strip():
                                                        try:
                                                            ...
                                                            file_stats['relations'] += 1
                                                        except Exception as rel_error:
                                                            logger.error(f"Error processing relation for '{word}': {str(rel_error)}")
                                    
                                    file_stats['processed'] += 1
                                    
                                    # Update progress periodically
                                    current_time = time.time()
                                    if current_time - last_update_time >= 1.0:  # Update at most once per second
                                        # Calculate processing rate
                                        elapsed_time = current_time - inner_pbar.start_t
                                        rate = file_stats['processed'] / elapsed_time if elapsed_time > 0 else 0
                                        
                                        # Update progress bar description
                                        desc = (f"[{rate:.1f} entries/s] "
                                    ...
                                    logger.error(f"Error processing entry {entry_index} in {dict_name}: {str(entry_error)}")
                                    file_stats['errors'] += 1
                                
                                # Update inner progress bar
                                inner_pbar.update(1)
                            
                            # Update outer progress bar with file stats
                            pbar.set_postfix(
                                processed=total_processed + file_stats['processed'],
                                errors=total_errors + file_stats['errors']
                            )
                        
                    except json.JSONDecodeError as json_error:
                        logger.error(f"Error decoding JSON from {dict_name}: {str(json_error)}")
                        file_stats['errors'] += 1
                ...
                # Log file completion
                logger.info(
                    f"Completed {dict_name}: "
                    f"Processed {file_stats['processed']}, "
                    f"Errors {file_stats['errors']}, "
                    f"Skipped {file_stats['skipped']}, "
                    f"Definitions {file_stats['definitions']}, "
                    f"Examples {file_stats['examples']}, "
                    f"Relations {file_stats['relations']}"
                )
                ...
                logger.error(f"Error processing file {json_file}: {str(file_error)}")
                total_errors += 1
                pbar.update(1)
