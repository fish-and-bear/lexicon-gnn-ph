...
def process_kaikki_jsonl(cur, filename: str):
    ...
    # Process a single entry - changed to use simpler transaction management
    def process_entry(cur, entry: Dict):
        """Process a single dictionary entry with better error handling."""
    ...
    try:
        with open(filename, 'r', encoding='utf-8') as f:
            ...
            for line in f:
                try:
                    ...
                    if len(current_batch) >= batch_size:
                        ...
                        conn.rollback()  # Ensure we're in a clean state
                        
                        processed_in_batch = 0
                        errors_in_batch = 0
                        
                        for entry_data in current_batch:
                            result = process_entry(cur, entry_data)
                            if result:
                        ...
                        try:
                            conn.commit()
                            logger.info(f"Committed batch {entry_count // batch_size}. Processed {stats['processed_entries']} entries so far.")
                        except Exception as commit_error:
                            logger.error(f"Error committing batch: {str(commit_error)}")
                            conn.rollback()
                            logger.info("Rolled back batch due to commit error. Entries will be counted as errors.")
                            # Adjust stats - entries weren't successfully committed
            ...
            if current_batch:
                # Start a fresh transaction for the last batch
                conn.rollback()  # Ensure we're in a clean state
