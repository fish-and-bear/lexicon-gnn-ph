...
def create_app(test_config=None):
    ...
    def bad_request_error(error):
        API_ERRORS.labels(endpoint=request.path, error_type='bad_request').inc()
        return jsonify({
            "error": "Bad request",
            "message": str(error),
            "status_code": 400,
    ...
    def not_found_error(error):
        API_ERRORS.labels(endpoint=request.path, error_type='not_found').inc()
        return jsonify({
            "error": "Not found",
            "status_code": 404,
            "path": request.path
    ...
    def ratelimit_error(error):
        API_ERRORS.labels(endpoint=request.path, error_type='rate_limit').inc()
        return jsonify({
    ...
    def internal_error(error):
        API_ERRORS.labels(endpoint=request.path, error_type='internal').inc()
