    ...
    try:
        stats = {
            ...
            'words_with_examples': db.session.query(
                db.func.count(db.distinct(Definition.word_id))
            ).filter(
            ...
            'words_with_etymology': db.session.query(
                db.func.count(db.distinct(Etymology.word_id))
            ).filter(
                Etymology.verification_status == 'verified'
            ).scalar(),
            'words_with_relations': db.session.query(
                db.func.count(db.distinct(Relation.source_word_id))
            ).scalar()
        }
