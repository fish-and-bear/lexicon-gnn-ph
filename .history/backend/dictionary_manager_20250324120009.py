...
def get_connection():
    ...
    return psycopg2.connect(
        dbname=os.environ.get('POSTGRES_DB', 'dictionary'),
                        ...
                        for definition in definitions:
                            if isinstance(definition, str):
                                ...
                                for match_type, synonym in synonym_matches:
                                    if synonym:
                                        syn_id = get_or_create_word_id(cur, synonym.strip(), language_code='tl')
                                        insert_relation(
                                            cur,
                                            word_id,
                                            syn_id,
                                ...
                                for match_type, antonym in antonym_matches:
                                    if antonym:
                                        ant_id = get_or_create_word_id(cur, antonym.strip(), language_code='tl')
                                        insert_relation(
                                            cur,
                                            word_id,
                                            ant_id,
