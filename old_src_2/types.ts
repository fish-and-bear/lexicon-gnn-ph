...
// ADD BasicDefinition type for references
export interface BasicDefinition {
  id: number;
  text?: string; // Keep it minimal
}
...
export interface Definition
  ...
  tags: string[]; // Add the missing tags field (expect string array after cleaning)
...
export interface Etymology
  extends Omit<
    RawEtymology,
    "components" | "language_codes" | "sources" | "normalized_components"
  > {
  text: string; // Ensure text is always present
...
export interface CleanPronunciation
  extends Omit<Pronunciation, "sources" | "value"> {
...
export interface EtymologyTree {
  ...
  // component_words?: ComponentWord[];
  metadata?: {
    // If we decide to add metadata back
    word: string;
    max_depth: number;
  };
}
...
// Use composition pattern for extending with d3 specific fields if needed
...
// NetworkLink is a more flexible version of edge used in d3 force layout
