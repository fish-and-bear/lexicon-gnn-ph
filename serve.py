    ...
    try:
        ...
        if result.returncode != 0:
            ...
            recreate_result = subprocess.run(
                [sys.executable, "backend/migrate.py", "--recreate"], 
                capture_output=True, 
                text=True
            )
            ...
            # Run dictionary data migration with numeric suffix handling
            dict_migration = subprocess.run(
                [sys.executable, "backend/dictionary_manager.py", "migrate", "--data-dir", data_dir, "--handle-numeric-suffixes"],
                capture_output=True,
                text=True
            )
            ...
            verify = subprocess.run(
                [sys.executable, "backend/dictionary_manager.py", "verify", "--check-numeric-suffixes"],
                capture_output=True,
                text=True
            )
...
if __name__ == "__main__":
    ...
    app.config.update(
        HANDLE_NUMERIC_SUFFIXES=True,  # Enable numeric suffix handling
        FLEXIBLE_LANGUAGE_CODES=True,  # Enable flexible language code handling
