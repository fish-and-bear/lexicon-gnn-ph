...
WORD_WITH_TRAILING_NUMBER_PATTERN = re.compile(r'(\b[a-zA-ZáàâäãåąćçéèêëęíìîïłńñóòôöõøùúûüÿýźżÁÀÂÄÃÅĄĆÇÉÈÊËĘÍÌÎÏŁŃÑÓÒÔÖÕØÙÚÛÜŸÝŹŻŠČĐĎŤŇŽÞÆŒƏƆƵǄǦǮȤΔΦΩк\-]+?)(\d+)\b')

# Additional patterns for complex formats within text
WORD_WITH_TRAILING_NUMBER_DASH_PATTERN = re.compile(r'(\b[a-zA-ZáàâäãåąćçéèêëęíìîïłńñóòôöõøùúûüÿýźżÁÀÂÄÃÅĄĆÇÉÈÊËĘÍÌÎÏŁŃÑÓÒÔÖÕØÙÚÛÜŸÝŹŻŠČĐĎŤŇŽÞÆŒƏƆƵǄǦǮȤΔΦΩк\-]+?)(\d+)-\b')  # e.g., "úngol1-"
WORD_WITH_TRAILING_NUMBER_COMMA_PATTERN = re.compile(r'(\b[a-zA-ZáàâäãåąćçéèêëęíìîïłńñóòôöõøùúûüÿýźżÁÀÂÄÃÅĄĆÇÉÈÊËĘÍÌÎÏŁŃÑÓÒÔÖÕØÙÚÛÜŸÝŹŻŠČĐĎŤŇŽÞÆŒƏƆƵǄǦǮȤΔΦΩк\-]+?),(\d+)\b')  # e.g., "úngol,2"
WORD_WITH_COMPLEX_TRAILING_NUMBER_PATTERN = re.compile(r'(\b[a-zA-ZáàâäãåąćçéèêëęíìîïłńñóòôöõøùúûüÿýźżÁÀÂÄÃÅĄĆÇÉÈÊËĘÍÌÎÏŁŃÑÓÒÔÖÕØÙÚÛÜŸÝŹŻŠČĐĎŤŇŽÞÆŒƏƆƵǄǦǮȤΔΦΩк\-]+?)(\d+)[,-](\d+)\b')  # e.g., "úngol1-2" or "úngol1,2"
...
def clean_word_in_text(text):
    ...
    for word in words:
        ...
        # Process word if it has trailing numbers
        cleaned_word = word
        
        # Handle special case for "úngol1-2" or "úngol1,2" pattern
        complex_match = COMPLEX_TRAILING_NUMBER_PATTERN.match(word)
        if complex_match:
            cleaned_word = complex_match.group(1)
            changes += 1
        # Handle special case for "úngol1-" pattern
        elif TRAILING_NUMBER_WITH_DASH_PATTERN.match(word):
            cleaned_word = TRAILING_NUMBER_WITH_DASH_PATTERN.match(word).group(1)
            changes += 1
        # Handle special case for "úngol,2" pattern
        elif TRAILING_NUMBER_WITH_COMMA_PATTERN.match(word):
            cleaned_word = TRAILING_NUMBER_WITH_COMMA_PATTERN.match(word).group(1)
            changes += 1
        # Handle special case for hyphenated words with trailing numbers
        elif HYPHENATED_NUMBER_PATTERN.match(word):
            cleaned_word = HYPHENATED_NUMBER_PATTERN.match(word).group(1)
            changes += 1
        # Handle regular case for words ending with numbers
        elif TRAILING_NUMBER_PATTERN.match(word):
            cleaned_word = TRAILING_NUMBER_PATTERN.match(word).group(1)
            changes += 1
    ...
    # Additional processing for embedded words with complex patterns
    ...
    processed_text = WORD_WITH_TRAILING_NUMBER_DASH_PATTERN.sub(replace_word, processed_text)
    processed_text = WORD_WITH_TRAILING_NUMBER_COMMA_PATTERN.sub(replace_word, processed_text)
    processed_text = WORD_WITH_COMPLEX_TRAILING_NUMBER_PATTERN.sub(replace_word, processed_text)
...
def remove_trailing_numbers(value, path=""):
    ...
    if isinstance(value, str):
        ...
        # Clean words in any string, not just definitions
