...
from .data_loading import load_pg_connection, fetch_graph_from_postgres, create_toy_graph, save_graph_json
...
def main():
    ...
    parser.add_argument("command", choices=["train", "evaluate", "infer", "ablate", "export", "export_predictions"],
                        help="Command to run")
    ...
    parser.add_argument("--data-source", type=str, default="toy",
                        choices=["postgres", "json", "toy"],
                        help="Data source")
    parser.add_argument("--data-path", type=str, default="toy_graph.json",
    ...
    if args.command == "train":
        ...
        save_graph_json(raw_data, args.data_path)
        print(f"Graph exported to {args.data_path}")
        
    elif args.command == "export_predictions":
        print("\nExporting 100 model-predicted links for manual judgement...")
        import csv
        # Load model
        model = create_model(args.model_type, metadata, in_channels_dict, config).to(device)
        checkpoint = torch.load(args.model_path, map_location=device)
        ...
        output_path = args.model_path.replace(".pt", "_top100_predictions.csv")
        with open(output_path, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(["word1", "word2", "score"])
            for i, j, score in top100:
                writer.writerow([idx2lemma.get(i, str(i)), idx2lemma.get(j, str(j)), score])
    ...
    if psutil:
        ...
        print(f"[Resource] Elapsed time: {elapsed:.2f} seconds | Peak memory: {peak:.2f} MB")
    else:
        print(f"[Resource] Elapsed time: {elapsed:.2f} seconds | (psutil not installed)")
