...
VALID_BAYBAYIN_REGEX = re.compile(r"^[\u1700-\u171F\s\u1735]+$") # Added \u1735 for Philippine Single Punctuation
    ...
    for attempt in range(max_retries):
        try:
            ...
            if conn:
                ...
                # ** NO conn.set_session CALL SHOULD BE HERE **

                # Check if the connection is usable (optional sanity check)
                if conn.closed:
                    logger.warning("Connection pool returned a closed connection. Retrying...")
                    connection_pool.putconn(conn, close=True) # Discard closed connection
                    last_exception = DatabaseConnectionError("Pool returned closed connection")
                    continue # Retry getting a new connection

                return conn # Return the usable connection
            else:
            ...
            # Catch unexpected errors during connection acquisition
            ...
            break # Break on non-operational errors

    logger.error(f"Failed to get database connection after {max_retries} attempts.")
    if isinstance(last_exception, DatabaseConnectionError):
...
POS_MAPPING = {
    ...
    "kabaligtaran": "n", # Antonym (noun context)
...
class RelationshipType(Enum):
    ...
    COGNATE_OF = ("cognate_of", RelationshipCategory.SEMANTIC, True, None, False, 75) # Cognates are related semantically
    DOUBLET_OF = ("doublet_of", RelationshipCategory.DERIVATIONAL, True, None, False, 80) # Doublets share derivation
...
def remove_trailing_numbers(text: str) -> str:
    """
    Remove trailing numbers from a string only when they're directly attached to words.
    Preserves standalone numbers and numbers that are part of phrases with separators.
    
    Args:
        text: The input text to process
        
    Returns:
        The processed text with trailing numbers removed from words only
    
    Examples:
        "suyo1" -> "suyo"
        "alam23" -> "alam"
        "bahay" -> "bahay" (unchanged)
        "2" -> "2" (preserved)
        "1,000" -> "1,000" (preserved)
        "ika-100" -> "ika-100" (preserved)
    """
    if not text:
        return ""
        
    # Check for special cases that should be preserved
    if text.isdigit():  # Standalone numbers like "2"
        return text
        
    if "," in text or "-" in text:  # Numbers with separators like "1,000" or "ika-100"
        return text
        
    # Only match digits directly attached to word characters
    result = re.sub(r'(\w)(\d+)$', r'\1', text)
    
    # Safety check to avoid returning empty strings
    if not result:
        return text
        
    if result != text:
        logger.debug(f"Removed trailing numbers: '{text}' -> '{result}'")
    return result

# Add this function near the start of the file or where other utility functions are defined
def preserve_acronyms_in_parentheses(text: str) -> str:
    """
    Preserve acronyms in parentheses by replacing them with special markers
    that won't be affected by extract_parenthesized_text.
    
    Args:
        text: The input text to process
        
    Returns:
        Text with acronyms in parentheses preserved with markers
    """
    if not text:
        return text
        
    def is_acronym(content):
        """Check if content is an acronym (all uppercase, relatively short)"""
        return (
            re.match(r'^[A-Z0-9\s\-\.]+$', content) and 
            len(content) <= 10 and
    ...
    # Replace acronyms with special markers (working backwards to preserve indices)
    for match in reversed(matches):
        content = match.group(1)
        if is_acronym(content):
            # Add a special marker around the acronym
...
def restore_preserved_acronyms(text: str) -> str:
    ...
    Args:
        text: The input text with markers
        
    Returns:
        Text with acronyms in parentheses restored
    """
    if not text:
        return text
    
    # Find all markers and restore them to parenthesized form
...
def extract_parenthesized_text(text: str) -> Tuple[str, Optional[str]]:
    ...
    Returns:
        A tuple containing:
        - The cleaned text (parentheses removed).
        - The original text if parentheses are embedded within the word.
        - The joined content of parentheses if they are only trailing notes.
        - None if no parentheses are found.

    Examples:
        "hig(ə)pít" -> ("higpít", "hig(ə)pít")  # Embedded
        "bahay (fig.)" -> ("bahay", "fig.")     # Trailing
        "word (note1) (note2)" -> ("word", "note1, note2") # Multiple trailing
        "normal" -> ("normal", None)
        "(note) word" -> ("word", "(note) word") # Embedded (at start)
    """
    if not text:
        return "", None

    original_text = text # Keep original for potential return
    clean_text = text
    parenthesized_parts = []
    found_embedded = False
    found_trailing = False
    matches_to_remove = [] # Store match objects to remove later

    # Find all parenthesized parts
    pattern = r'\(([^)]+)\)'
    ...
    for match in matches:
        ...
        matches_to_remove.append(match) # Store the match object

        # Check if the match is strictly trailing: ends the string AND is preceded by whitespace
        # (or starts the string, but we handle that as embedded)
        is_strictly_trailing = (match.end() == len(text)) and \
                               (match.start() > 0 and text[match.start() - 1].isspace())

        if not is_strictly_trailing:
             # Consider it embedded if it's not strictly trailing
             # This includes cases like "(note) word" or "word(note)" or "word(note)word"
            found_embedded = True
        else:
            found_trailing = True
            # We only care if *all* matches are trailing, so keep checking

    # If we found any embedded match, the outcome is decided
    if found_embedded:
        # Remove all matched parentheses from the text
        # Iterate backwards through matches to avoid index issues during removal
    ...
    elif found_trailing: # Only executed if found_embedded is False (meaning all matches were trailing)
        # Remove all matched parentheses from the text
        for match in reversed(matches_to_remove):
             clean_text = clean_text[:match.start()] + clean_text[match.end():]
        # Clean up extra spaces
        clean_text = ' '.join(clean_text.split())
        # Return only the joined content of the trailing notes
        return clean_text, ", ".join(parenthesized_parts)
    else:
        # This case should not be hit if matches were found, but included for completeness
        # If no matches were found, the initial check returns (text, None)
        return text, None # Should technically not be reachable

# --- End of extract_parenthesized_text ---
...
def validate_schema(cur):
    """
    Validates that the database schema has all required tables and indexes.
    Creates missing indexes if possible.
    
    Args:
        cur: Database cursor
        
    Returns:
        bool: True if schema is valid or was fixed, False if critical issues found
    """
    issues = []
    ...
    required_tables = [
        ...
        "etymologies", "word_relations", "word_tags"
    ]
    ...
    # If any tables are missing, we can't proceed
    if issues:
        for issue in issues:
            logger.error(f"Schema validation failed: {issue}")
        return False
    
    # Check for required indexes and create them if missing
    required_indexes = [
        {"table": "words", "name": "words_lemma_idx", "columns": "lemma"},
        {"table": "words", "name": "words_word_idx", "columns": "word"},
        {"table": "word_relations", "name": "word_relations_source_target_idx", 
         "columns": "word_id, related_word_id, relation_type"}
    ]
    ...
    for index_info in required_indexes:
        try:
            ...
            exists = cur.fetchone()[0]
            if not exists:
                logger.warning(f"Required index {index_info['name']} on {index_info['table']}({index_info['columns']}) does not exist")
                
                # Attempt to create the missing index
                try:
                    logger.info(f"Creating missing index {index_info['name']} on {index_info['table']}({index_info['columns']})")
                    create_stmt = f"CREATE INDEX {index_info['name']} ON {index_info['table']}({index_info['columns']})"
                    cur.execute(create_stmt)
                    logger.info(f"Successfully created index {index_info['name']}")
                except Exception as create_err:
                    logger.error(f"Failed to create index {index_info['name']}: {create_err}")
                    # Don't add this to issues - we'll continue processing without the index
                    # since it's not critical for the migration to work
        except Exception as e:
            logger.warning(f"Error checking index '{index_info['name']}': {e}")
            # Don't add this to issues - we'll continue processing without the index
...
def remove_trailing_numbers(text: str) -> str:
    ...
    Returns:
        The processed text with trailing numbers removed
...
def extract_parenthesized_text(text: str) -> Tuple[str, Optional[str]]:
    ...
    Returns:
        A tuple containing the cleaned text and the extracted parenthesized content
...
def get_or_create_word_id(
    ...
    **kwargs,
    ...
    if not preserve_numbers:
        ...
        # Preserve fractions and numbers with slashes (e.g. "2/3", "1/2")
        elif '/' in lemma and any(c.isdigit() for c in lemma):
            should_preserve = True
        # Preserve numbers with separators
        elif any(c in lemma for c in [',', '-', '–', '.', '/']):
            should_preserve = True
        # Preserve phrases that end with numbers (e.g., "article on 100")
        elif re.search(r'\s\d+$', lemma):
            should_preserve = True

    # Only remove trailing numbers if not preserving
    original_lemma = lemma
    if not should_preserve and not original_lemma.startswith("bitamína "):
        lemma = remove_trailing_numbers(lemma)
        if lemma != original_lemma:
            logger.info(f"Removed trailing number(s) from lemma: '{original_lemma}' → '{lemma}'")
    
    # Safety check - don't allow empty lemmas after processing
    if not lemma:
        logger.warning(f"Processing would result in empty lemma, preserving original: '{original_lemma}'")
        lemma = original_lemma

    # Extract parenthesized text from lemma (e.g., "bahay (fig.)" becomes "bahay" with note "fig.")
    parenthesized_text = None
    lemma, parenthesized_text = extract_parenthesized_text(lemma)
    if parenthesized_text:
        logger.info(f"Extracted parenthesized text from lemma: original='{original_lemma}' → lemma='{lemma}', note='{parenthesized_text}'")

    # Ensure lemma is not empty after all processing
    if not lemma:
        logger.warning(f"All processing resulted in empty lemma, using original: '{original_lemma}'")
        lemma = original_lemma
    ...
    # If word_metadata is None, initialize as empty dict
    if word_metadata is None:
        word_metadata = {}
    
    # Store parenthesized text in word_metadata if found
    if parenthesized_text:
    ...
    try:
        ...
        if hasattr(e, 'pgcode') and e.pgcode:
            logger.error(f"PostgreSQL Error ({e.pgcode}): {e}")
        else:
            logger.error(f"Database error in get_or_create_word_id for '{lemma}': {e}")
        raise DatabaseError(f"Failed to get or create word '{lemma}': {e}")
...
def batch_get_or_create_word_ids(
    ...
    for i in range(0, len(entries), batch_size):
        ...
        # Remove trailing numbers from lemmas before normalizing
        cleaned_batch = []
        for lemma, lang_code in batch:
            original_lemma = lemma
            cleaned_lemma = remove_trailing_numbers(lemma)
            if cleaned_lemma != original_lemma:
                logger.debug(f"Batch processing: Removed trailing number(s) from lemma: '{original_lemma}' → '{cleaned_lemma}'")
            cleaned_batch.append((cleaned_lemma, lang_code))
        
        # First, let's ensure all existing words are fetched from the database
        normalized_entries = [
            (lemma, normalize_lemma(lemma), lang_code) for lemma, lang_code in cleaned_batch
        ]
        placeholders = []
        flat_params = []
    ...
    with tqdm(total=entries_in_file, desc=f"Processing {source_identifier}", unit="entry", leave=False) as pbar:
        for entry_index, (original_key, entry) in enumerate(data.items()):
            ...
            try:
                ...
                # Save the original lemma for logging
                original_lemma = lemma
                ...
                try:
                    ...
                    # After successfully creating the word, check if we need to update the lemma
                    # to restore the acronyms that may have been processed by extract_parenthesized_text
                    restored_lemma = restore_preserved_acronyms(lemma_with_markers)
                    if restored_lemma != lemma_with_markers:
                        # The lemma had acronyms that were preserved with markers, now restore them
...
# --- Helper Functions (Ensure these accept 'cur' as the first argument) ---
# Example: def get_or_create_word_id(cur, lemma, language_code, source_identifier): ...
# Example: def insert_definition(cur, word_id, definition_text, ...): ...
# ... (Your existing helper functions like get_or_create_word_id, insert_definition, etc.) ...
# MAKE SURE ALL HELPERS USED BELOW ACCEPT `cur` AS THE FIRST ARGUMENT

# ---------------------------------------------------------------------------
#          MODIFIED: Single Entry Processing (No Decorator, Takes Cursor)
# ---------------------------------------------------------------------------
def _process_single_tagalog_word_entry(
    ...
    """
    Processes a single word entry using the provided database cursor.
    ...
    Args:
        cur: Active database cursor within a transaction.
        entry_data: Dictionary containing data for one word entry.
        word_key: The original key (lemma) from the JSON.
        source_identifier: Standardized source identifier for this file.
        language_code: Language code (e.g., 'tl').
        add_etymology: Flag to process etymology section.
        word_id_cache: Cache for word IDs {normalized_lemma|lang_code: word_id}.
        stats: Statistics dictionary (mutated).
        error_types: Dictionary for tracking error types (mutated).
        table_column_map: Map of existing table columns for schema checks.
        relations_batch: List to collect relation data (mutated).
        required_columns: Map of required columns for schema checks.

    Returns:
        None: Mutates stats, error_types, relations_batch directly. Raises exceptions on error.
    """
    lemma = entry_data.get("word") or word_key # Use "word" field if present, fallback to key
    if not lemma:
        logger.warning(f"Skipping entry with missing lemma (key: '{word_key}')")
        stats["skipped"] += 1
        return # Must return here, not False, as caller expects exceptions on actual failure

    # --- Initial Word Insertion ---
    # Assume get_or_create_word_id uses the passed 'cur'
    # It should handle normalization and return the ID
    word_id = get_or_create_word_id(cur, lemma, language_code, source_identifier)
    if not word_id:
        # Failure already logged by get_or_create_word_id
        stats["errors"] += 1
        error_types["WordCreationFailed"] = error_types.get("WordCreationFailed", 0) + 1
        raise ValueError(f"Failed to get or create word_id for lemma: {lemma}") # Raise exception

    # Add to cache immediately AFTER successful creation/retrieval
    normalized_lemma_cache_key = normalize_lemma(lemma) # Ensure consistent normalization
    cache_key = f"{normalized_lemma_cache_key}|{language_code}"
    word_id_cache[cache_key] = word_id

    # --- Process Pronunciations ---
    if "pronunciations" in required_columns and "pronunciations" in entry_data:
        pronunciations = entry_data.get("pronunciations", [])
        if isinstance(pronunciations, list):
            # Assume insert_pronunciation uses the passed 'cur'
            for pron_val in pronunciations:
                if pron_val: # Ensure non-empty pronunciation
                    pron_id = insert_pronunciation(cur, word_id, pron_val, source_identifier)
                    if pron_id: stats["pronunciations_added"] += 1
        else:
            logger.warning(f"Expected list for 'pronunciations' in '{lemma}', got {type(pronunciations)}. Skipping.")


    # --- Process Etymology ---
    if add_etymology and "etymology" in required_columns and "etymology_text" in entry_data:
        etymology_text = entry_data.get("etymology_text")
        if etymology_text:
            # Assume insert_etymology uses the passed 'cur'
            ety_id = insert_etymology(cur, word_id, etymology_text, source_identifier)
            if ety_id: stats["etymologies_added"] += 1


    # --- Process Definitions (Senses) ---
    if "definitions" in required_columns and "senses" in entry_data:
        senses = entry_data.get("senses", [])
        if isinstance(senses, list):
            for sense in senses:
                if not isinstance(sense, dict): continue
                definition_text = sense.get("glosses", [None])[0] # Take the first gloss as definition
                pos_tags = sense.get("pos", []) # Get list of POS tags
                examples_list = sense.get("examples", []) # Get list of examples
                raw_tags = sense.get("raw_tags", []) # Get raw tags if available

                if definition_text:
                    # Assume insert_definition uses the passed 'cur'
                    def_id = insert_definition(
                        cur=cur,
                        word_id=word_id,
                        definition_text=definition_text,
                        pos_list=pos_tags,
                        example_list=examples_list,
                        source_identifier=source_identifier,
                        raw_tags=raw_tags # Pass raw tags if helper uses them
                    )
                    if def_id:
                        stats["definitions_added"] += 1
                        # --- Collect Relations from Sense ---
                        if "relations" in required_columns:
                            relations = sense.get("related", [])
                            if isinstance(relations, list):
                                for rel in relations:
                                    if isinstance(rel, dict) and "word" in rel and "type" in rel:
                                        relation_type_str = rel["type"]
                                        related_word_lemma = rel["word"]
                                        # Get the standardized RelationType enum
                                        #rel_type_enum = RelationType.from_string(relation_type_str)
                                        #if not rel_type_enum:
                                        #     logger.warning(f"Unknown relation type '{relation_type_str}' for word '{lemma}'. Skipping relation.")
                                        #     continue

                                        # Collect relation info for batch processing later
                                        relations_batch.append({
                                            "word_id": word_id,
                                            "related_word": related_word_lemma,
                                            "rel_type": relation_type_str,
                                            "source_identifier": source_identifier,
                                            "metadata": {"context": "definition"}, # Optional metadata
                                            "def_id": def_id # Link relation to the definition
                                        })
                            else:
                                logger.warning(f"Expected list for 'related' in sense of '{lemma}', got {type(relations)}. Skipping relations for this sense.")
        else:
            logger.warning(f"Expected list for 'senses' in '{lemma}', got {type(senses)}. Skipping definitions.")


# ---------------------------------------------------------------------------
#          REVISED: Main Processing Function (Accepts Cursor)
# ---------------------------------------------------------------------------
def process_tagalog_words(cur, filename: str, add_etymology: bool = True): # ADDED `cur` argument back
    """
    Process a JSON file containing Tagalog words using the provided cursor
    and iterative JSON parsing. Assumes transaction is managed by the caller.
    ...
    stats = {
        "entries_processed": 0, "skipped": 0, "definitions_added": 0,
        "pronunciations_added": 0, "etymologies_added": 0, "relations_added": 0,
        "errors": 0, # Errors during single entry processing (entry rolled back via savepoint)
        "critical_errors": 0 # Errors preventing processing (e.g., file not found)
    }
    error_types = {} # Track specific error types encountered
    ...
    if not source_identifier:
        source_identifier = "tagalog_json" # Fallback
    logger.info(f"Using standardized source identifier: '{source_identifier}'")

    # --- Schema Check (Uses temporary standalone connection) ---
    # (Schema check logic remains the same as before, using get_connection() and temp_conn.close())
    required_columns = {
        "words": {"id", "lemma", "normalized_lemma", "language_code", "source_info", "word_metadata"},
        "pronunciations": {"id", "word_id", "value", "sources"},
        "etymologies": {"id", "word_id", "etymology_text", "sources"},
        "definitions": {"id", "word_id", "definition_text", "standardized_pos_id", "sources", "examples"},
        "relations": {"id", "from_word_id", "to_word_id", "relation_type", "sources", "metadata"},
        "parts_of_speech": {"id", "code"},
        "definition_relations": {"id", "definition_id", "related_word_id", "relation_type", "sources"},
    }
    table_column_map = {}
    temp_conn = None
    try:
        temp_conn = get_connection() # Get a temporary, non-pooled connection
        with temp_conn.cursor() as temp_cur:
            all_tables = list(required_columns.keys())
            if not add_etymology and "etymologies" in all_tables:
                 all_tables.remove("etymologies")

            temp_cur.execute("SELECT table_name, column_name FROM information_schema.columns WHERE table_schema = 'public' AND table_name = ANY(%s)", (all_tables,))
            fetched_columns = temp_cur.fetchall()
            for table in all_tables: table_column_map[table] = set()
            for table_name, column_name in fetched_columns:
                if table_name in table_column_map: table_column_map[table_name].add(column_name)
            for table, req_cols_set in required_columns.items():
                 if table == "etymologies" and not add_etymology: continue
                 missing_columns = req_cols_set - table_column_map.get(table, set())
                 if missing_columns:
                     logger.warning(f"Schema Check: Table '{table}' missing required columns: {', '.join(missing_columns)}. Some features might not work.")
    except Exception as schema_err:
        logger.error(f"Failed initial schema check: {schema_err}. Proceeding cautiously.")
        for table in required_columns: table_column_map[table] = set()
    finally:
        if temp_conn:
            try: temp_conn.close()
            except Exception as close_err: logger.error(f"Error closing temp schema check connection: {close_err}")

    # --- Caches and Batches ---
    word_id_cache = {} # Key: normalized_lemma|language_code
    relations_batch = [] # Collect relations here

    # --- Main Processing Loop using passed cursor ---
    conn = cur.connection # Get connection from the cursor for savepoint management
    processed_count_logged = 0
    processed_count_log_interval = 1000

    try:
        # Use iterative JSON parsing
        try: import ijson
        except ImportError:
            logger.error("Module 'ijson' not found. Please install it (`pip install ijson`).")
            stats["critical_errors"] += 1
            error_types["ImportError: ijson"] = 1
            # Raise error to indicate file processing failed
            raise RuntimeError("ijson module not found")

        logger.info(f"Loading {filename} using iterative parser ijson...")
        entries_iterator = None
        file_handle = None

        try:
            file_handle = open(filename, "r", encoding="utf-8")
            entries_iterator = ijson.kvitems(file_handle, '') # Assuming top level is dict
            logger.info(f"Iterating through entries in {filename}...")

            # --- Process Each Entry with Savepoints ---
            language_code = "tl"
            for entry_index, (word_key, entry_data) in enumerate(entries_iterator):
                savepoint_name = f"tagalog_entry_{entry_index}"

                if not isinstance(entry_data, dict):
                    logger.warning(f"Skipping entry {entry_index} ('{word_key}') - expected dict, got {type(entry_data)}")
                    stats["skipped"] += 1
                    continue
                ...
                try:
                    ...
                    # Call the helper function - PASSING THE CURSOR
                    # It will raise exceptions on failure
                    _process_single_tagalog_word_entry(
                        cur=cur, # Pass the active cursor
                        entry_data=entry_data, word_key=word_key,
                        source_identifier=source_identifier, language_code=language_code,
                        add_etymology=add_etymology, word_id_cache=word_id_cache,
                        stats=stats, error_types=error_types,
                        table_column_map=table_column_map, relations_batch=relations_batch,
                        required_columns=required_columns
                    )
                    stats["entries_processed"] += 1
                    cur.execute(f"RELEASE SAVEPOINT {savepoint_name};") # Release savepoint on success

                except Exception as entry_err:
                    # Catch errors from _process_single_tagalog_word_entry
                    logger.error(f"Failed processing entry {entry_index} ('{word_key}'): {entry_err}", exc_info=False)
                    stats["errors"] += 1 # Increment specific error count for entries
                    error_type_key = f"EntryProcessingError: {type(entry_err).__name__}"
                    error_types[error_type_key] = error_types.get(error_type_key, 0) + 1
                    # Rollback to savepoint for this entry
                    try:
                        cur.execute(f"ROLLBACK TO SAVEPOINT {savepoint_name};")
                    except Exception as rb_err:
                         logger.critical(f"CRITICAL: Failed rollback to savepoint {savepoint_name} after entry error: {rb_err}. Aborting.", exc_info=True)
                         raise rb_err # Re-raise critical rollback failure

                # Log progress periodically
                current_processed_total = stats["entries_processed"] + stats["skipped"] + stats["errors"]
                if current_processed_total >= processed_count_logged + processed_count_log_interval:
                     logger.info(f"Processed ~{current_processed_total} entries...")
                     processed_count_logged = current_processed_total

            # --- NO Final Commit Here ---
            # Commit/Rollback is handled by the caller (e.g., migrate_data)
        ...
        except Exception as e: # Catch ijson errors or other file processing errors
             logger.error(f"Error loading or iterating through JSON file {filename}: {e}", exc_info=True)
             stats["critical_errors"] += 1
             error_types[f"FileProcessingError: {type(e).__name__}"] = error_types.get(f"FileProcessingError: {type(e).__name__}", 0) + 1
             raise # Re-raise to signal failure to caller

        finally:
            if file_handle: file_handle.close()
            # DO NOT close cursor or connection here - managed by caller
    # ----- ADD THESE CLAUSES for the outer 'try' on line 6647 -----
    except FileNotFoundError:
         logger.error(f"File not found: {filename}")
         stats["critical_errors"] += 1
         error_types["FileNotFound"] = 1
         raise # Re-raise to signal failure to caller
    except RuntimeError as rt_err: # Catch specific raised errors like ijson missing
         # Error already logged, just re-raise
         raise rt_err
    except Exception as e: # Catch other unexpected errors during setup/iteration
         logger.error(f"Error loading or iterating through JSON file {filename}: {e}", exc_info=True)
         stats["critical_errors"] += 1
         error_types[f"FileProcessingError: {type(e).__name__}"] = error_types.get(f"FileProcessingError: {type(e).__name__}", 0) + 1
         raise # Re-raise to signal failure to caller

    # --- Process Relations Batch (using the same passed cursor) ---
    logger.info(f"Finished iterating entries. Processing {len(relations_batch)} collected relations...")
    if relations_batch:
        # Define relation processing helper locally
        # REMOVED @with_db_transaction - uses the main transaction
        def _process_relations_batch_inline(cur, batch, stats_dict, cache, error_types_dict):
            processed_count = 0
            failed_count = 0
            lang_code_for_related = "tl" # Assuming related words are also Tagalog

            # Define relation properties (as discussed before)
            RELATION_PROPERTIES = {
                "synonym": {"bidirectional": True, "strength": 90},
                "antonym": {"bidirectional": True, "strength": 90},
                "related": {"bidirectional": False, "strength": 50}, # Example: related might not be bidirectional
                # Add other types from your data (e.g., hypernym, hyponym)
                "hypernym": {"inverse": "hyponym", "strength": 75},
                "hyponym": {"inverse": "hypernym", "strength": 75},
            }

            for relation_info in tqdm(batch, desc="Processing Relations"):
                 relation_savepoint = f"rel_{relation_info.get('word_id', 'unk')}_{processed_count+failed_count}"
                 try:
                      cur.execute(f"SAVEPOINT {relation_savepoint};") # Savepoint per relation

                      from_id = relation_info['word_id']
                      related_word_lemma = relation_info['related_word']
                      # Use the raw string type stored earlier
                      rel_type_str = relation_info['rel_type'] # This should be the string
                      src_id = relation_info['source_identifier']
                      metadata = relation_info.get('metadata', {}) # Ensure metadata is a dict
                      def_id = relation_info.get('def_id')

                      if not related_word_lemma:
                           logger.debug("Skipping relation with empty related_word lemma.")
                           cur.execute(f"RELEASE SAVEPOINT {relation_savepoint};")
                           continue

                      normalized_related_lemma = normalize_lemma(related_word_lemma)
                      rel_word_cache_key = f"{normalized_related_lemma}|{lang_code_for_related}"
                      related_word_id = cache.get(rel_word_cache_key)

                      if not related_word_id:
                           related_word_id = get_or_create_word_id(cur, related_word_lemma, lang_code_for_related, src_id)
                           if related_word_id:
                                cache[rel_word_cache_key] = related_word_id
                           else:
                                logger.warning(f"Failed get/create related word '{related_word_lemma}'. Skipping relation.")
                                failed_count += 1
                                cur.execute(f"RELEASE SAVEPOINT {relation_savepoint};")
                                continue

                      if related_word_id and related_word_id != from_id:
                           # --- Direct SQL Insert for Relation ---
                           try:
                                # Add default confidence if not present
                                if "confidence" not in metadata:
                                    metadata["confidence"] = RELATION_PROPERTIES.get(rel_type_str, {}).get("strength", 50) # Default confidence

                                # --- Execute the INSERT statement (moved outside the 'if') ---
                                cur.execute(
                                    """
                                    INSERT INTO relations (from_word_id, to_word_id, relation_type, sources, metadata, definition_id)
                                    VALUES (%s, %s, %s, %s, %s, %s)
                                    ON CONFLICT (from_word_id, to_word_id, relation_type, COALESCE(definition_id, -1)) DO NOTHING
                                    """,
                                    (from_id, related_word_id, rel_type_str, Json([src_id]), Json(metadata), def_id)
                                )
                                # --- Stat increments (correct indentation) ---
                                stats_dict["relations_added"] += 1 # Increment even if ON CONFLICT ignores? Check requirement.
                                processed_count += 1 # Count successful attempt

                                # Handle definition_relations if applicable
                                if def_id and "definition_relations" in required_columns:
                                    try: insert_definition_relation(cur, def_id, related_word_id, rel_type_str, src_id)
                                    except Exception as def_rel_err: logger.warning(f"Failed inserting definition relation: {def_rel_err}")

                                # Handle bidirectional/inverse using RELATION_PROPERTIES
                                props = RELATION_PROPERTIES.get(rel_type_str)
                                if props:
                                    inverse_metadata = metadata.copy() # Avoid modifying original metadata
                                    inverse_metadata["is_inverse"] = True # Mark as inverse relation
                                    ...
                                    if props.get("bidirectional"):
                                        cur.execute(
                                            ...
                                            ON CONFLICT (from_word_id, to_word_id, relation_type, COALESCE(definition_id, -1)) DO NOTHING
                                            """,
                                            (related_word_id, from_id, rel_type_str, Json([src_id]), Json(inverse_metadata))
                                        )
                                    else:
                                        inverse_rel_type = props.get("inverse")
                                        if inverse_rel_type:
                                             cur.execute(
                                                 """
                                                 INSERT INTO relations (from_word_id, to_word_id, relation_type, sources, metadata)
                                                 ...
                                                 ON CONFLICT (from_word_id, to_word_id, relation_type, COALESCE(definition_id, -1)) DO NOTHING
                                                 """,
                                                 (related_word_id, from_id, inverse_rel_type, Json([src_id]), Json(inverse_metadata))
                                             )
                           except Exception as insert_err:
                                logger.warning(f"Failed inserting relation/inverse for from={from_id}, to={related_word_id}, type={rel_type_str}: {insert_err}")
                                # Re-raise to trigger rollback TO savepoint for this relation
                                raise insert_err


                 except Exception as rel_proc_err:
                      logger.warning(f"Failed processing relation item: {relation_info} - Error: {rel_proc_err}", exc_info=True)
                      failed_count += 1
                      error_type_key = f"RelationProcessingError: {type(rel_proc_err).__name__}"
                      error_types_dict[error_type_key] = error_types_dict.get(error_type_key, 0) + 1
                      # Rollback this specific relation's savepoint
                      try: cur.execute(f"ROLLBACK TO SAVEPOINT {relation_savepoint};")
                      except Exception as rb_rel_err: logger.error(f"Failed to rollback relation savepoint {relation_savepoint}: {rb_rel_err}")
                 finally:
                      # Release savepoint if it wasn't rolled back
                      try: cur.execute(f"RELEASE SAVEPOINT {relation_savepoint};")
                      except psycopg2.Error: pass # Ignore errors if savepoint doesn't exist (already rolled back)

            logger.info(f"Processed {processed_count} relations in batch. Encountered {failed_count} failures/skips during processing.")
            # No explicit return needed as stats_dict and error_types_dict are mutated

        # Call the inline batch processor
        _process_relations_batch_inline(cur, relations_batch, stats, word_id_cache, error_types)

    # --- Log Final Statistics ---
    logger.info(f"Finished processing {filename}")
    # Log detailed stats...
    logger.info(f"  Entries processed successfully: {stats['entries_processed']}")
    logger.info(f"  Entries skipped (invalid format): {stats['skipped']}")
    logger.info(f"  Entries failed processing (errors): {stats['errors']}")
    logger.info(f"  Critical errors (file read, ijson): {stats['critical_errors']}")
    ...
    logger.info(f"  Relations added (total): {stats['relations_added']}")
    if error_types:
        logger.warning(f"Error Summary ({filename}): {error_types}")

    # Return value expected by migrate_data (processed, issues)
    total_issues = stats["skipped"] + stats["errors"] + stats["critical_errors"]
    return stats["entries_processed"], total_issues



def process_relations_batch(cur, relations_batch, stats, word_id_cache):
    """
    Process a batch of relations efficiently.
    
    Args:
        cur: Database cursor
        relations_batch: List of relation dictionaries to process
        stats: Statistics dictionary to update
        word_id_cache: Cache of word IDs to avoid repeated lookups
    ...
    for relation in relations_batch:
        try:
            ...
            source_identifier = relation['source_identifier']
            def_id = relation.get('def_id')
            
            # Either use provided related_word_id or look up the related_word
            if 'related_word_id' in relation:
                related_word_id = relation['related_word_id']
            else:
                related_word = relation['related_word']
                
                # Skip if related_word is empty or matches the source word
                if not related_word:
                    continue
                
                # Check cache first
                if related_word in word_id_cache:
                    related_word_id = word_id_cache[related_word]
                else:
                    ...
                    if result:
                        related_word_id = result[0]
                        word_id_cache[related_word] = related_word_id
                    else:
            ...
            if def_id:
                cur.execute(
                    ...
                    (word_id, related_word_id, relation_type, definition_id, source_identifier)
                    VALUES (%s, %s, %s, %s, %s)
                    ON CONFLICT (word_id, related_word_id, relation_type, COALESCE(definition_id, -1))
                    DO NOTHING
                    """,
                    (word_id, related_word_id, rel_type, def_id, source_identifier),
                )
            else:
                cur.execute(
                    """
                    INSERT INTO word_relations
                    (word_id, related_word_id, relation_type, source_identifier)
                    VALUES (%s, %s, %s, %s)
                    ON CONFLICT (word_id, related_word_id, relation_type, COALESCE(definition_id, -1))
                    DO NOTHING
                    """,
                    (word_id, related_word_id, rel_type, source_identifier),
                )
            
            stats["relations_added"] += 1
            
        except psycopg2.IntegrityError as e:
            # Unique violation - relation already exists
            if "unique constraint" in str(e).lower():
                pass  # Already handled by ON CONFLICT
            else:
                logger.error(f"Integrity error adding relation: {e}")
        except Exception as e:
                            ...
                            if isinstance(examples_raw, list):
                                for ex_item in examples_raw:
                                    if isinstance(ex_item, dict) and ex_item.get("language", "").lower() == def_lang:
                                        # FIX: Handle None value by providing a default empty string before strip()
                                        example_text = (ex_item.get("example") or "").strip()
                                        if example_text:
...
def extract_script_info(entry, tag_name, script_name):
    """Helper function to extract script information from an entry."""
    if not entry or not isinstance(entry, dict) or "forms" not in entry:
        return None, None
    
    for form in entry.get("forms", []):
        if isinstance(form, dict) and "tags" in form and isinstance(form["tags"], list):
            if tag_name in form["tags"] and "form" in form and form["form"]:
                return form["form"], None  # Return the form and no romanization by default
    ...
    def process_single_entry(entry):
        ...
        #--- BEGIN IMPROVED BAYBAYIN EXTRACTION ---
            ...
            if isinstance(baybayin_form, str) and baybayin_form:
                ...
                # Find all sequences of Baybayin characters (U+1700 to U+171F)
                # Using the direct unicode range ᜀ-ᜟ which covers Baybayin characters
                baybayin_matches = re.findall(r"([ᜀ-ᜟ]+)", baybayin_form)
                if baybayin_matches:
                    # Assume the last sequence found is the intended script
                    extracted_script = baybayin_matches[-1]
                    # Check if this extracted script is different from the original potentially mixed string
                    if extracted_script != baybayin_form:
                         logger.info(f"Extracted Baybayin '{extracted_script}' from mixed string '{baybayin_form}' for word '{word}'.")
                    ...
                    if etymology_templates and isinstance(etymology_templates, list):
                        for template in etymology_templates:
                            ...
                            if template_name == "blend":
                                ...
                                if not components: # Fallback to positional args if numbered ones fail
                                    if isinstance(args, dict) and all(isinstance(k, str) and k.isdigit() for k in args.keys()):
                                        positional_args = [v for k, v in sorted(args.items()) if k != "1"] # Skip language arg "1" if present
                                        components = [arg for arg in positional_args if isinstance(arg, str)]

                                for component_word in components:
                                    component_word_clean = clean_html(component_word)
                                    if component_word_clean and component_word_clean.lower() != word.lower():
                                        ...
                                        if component_cache_key in word_cache:
                                            related_word_id = word_cache[component_cache_key]
                                        else:
                                            related_word_id = get_or_create_word_id(
                                                cur, component_word_clean, component_lang, source_identifier
                                            )
                                            if related_word_id:
                                                word_cache[component_cache_key] = related_word_id
                                        ...
                                        if related_word_id and related_word_id != word_id:
                                            ...
                                            rel_id = insert_relation(
                                                cur, related_word_id, word_id,
                                                RelationshipType.RELATED, # Use RELATED for components
                                                source_identifier, metadata
                                            )
                                            if rel_id:
                                                local_stats["ety_relations"] += 1
                                                local_stats["relations"] += 1

                                                        # --- Additions for inh, cog, doublet ---
                            
                            elif template_name == "inh": # Inherited from proto-language
                                proto_lang = args.get("2", "")
                                proto_word = args.get("3", "")
                                if proto_lang and proto_word and isinstance(proto_word, str):
                                    proto_word_clean = clean_html(proto_word).lstrip('*') # Clean and remove leading asterisk
                                    if proto_word_clean and proto_word_clean.lower() != word.lower():
                                        # Get ID for the proto-word
                                        proto_cache_key = f"{proto_word_clean.lower()}|{proto_lang}"
                                        if proto_cache_key in word_cache:
                                            related_word_id = word_cache[proto_cache_key]
                                        else:
                                            ...
                                            related_word_id = get_or_create_word_id(
                                                cur, proto_word_clean, proto_lang, source_identifier
                                                #, is_proto_word=True # Add flag if needed by get_or_create_word_id
                                            )
                                            if related_word_id:
                                                word_cache[proto_cache_key] = related_word_id
                                        ...
                                        if related_word_id and related_word_id != word_id:
                                            ...
                                            insert_relation(
                                                cur, related_word_id, word_id,
                                                RelationshipType.ROOT_OF,
                                                source_identifier, metadata
                                            )

                            elif template_name == "cog": # Cognate with word in another language
                                cognate_lang = args.get("1", "")
                                cognate_word = args.get("2", "")
                                if cognate_lang and cognate_word and isinstance(cognate_word, str):
                                    cognate_word_clean = clean_html(cognate_word)
                                    if cognate_word_clean and cognate_word_clean.lower() != word.lower():
                                        # Get ID for the cognate word
                                        cognate_cache_key = f"{cognate_word_clean.lower()}|{cognate_lang}"
                                        if cognate_cache_key in word_cache:
                                            related_word_id = word_cache[cognate_cache_key]
                                        else:
                                            related_word_id = get_or_create_word_id(
                                                cur, cognate_word_clean, cognate_lang, source_identifier
                                            )
                                            if related_word_id:
                                                word_cache[cognate_cache_key] = related_word_id
                            ...
                            elif template_name == "doublet": # Doublet within the same language
                                doublet_lang = args.get("1", "") or language_code # Should be same language
                                doublet_word = args.get("2", "")
                                if doublet_word and isinstance(doublet_word, str):
                                    doublet_word_clean = clean_html(doublet_word)
                                    if doublet_word_clean and doublet_word_clean.lower() != word.lower():
                                        # Get ID for the doublet word
                                        doublet_cache_key = f"{doublet_word_clean.lower()}|{doublet_lang}"
                                        if doublet_cache_key in word_cache:
                                            related_word_id = word_cache[doublet_cache_key]
                                        else:
                                            related_word_id = get_or_create_word_id(
                                                cur, doublet_word_clean, doublet_lang, source_identifier
                                            )
                                            if related_word_id:
                                                word_cache[doublet_cache_key] = related_word_id
                            ...
                            if template_name in ["derived", "borrowing", "derived from", "borrowed from", "bor", "der", "bor+"]:
                                ...
                                source_lang = ""
                                source_word = ""
                                if template_name in ["bor", "bor+", "borrowing", "borrowed from"]: # Borrowing templates
                                    source_lang = args.get("2", "") # Language code is usually arg 2
                                    source_word = args.get("3", "") # Source word is usually arg 3
                                    if not source_lang: # Fallback if arg 2 is missing/empty
                                        source_lang = args.get("1", "")
                                elif template_name in ["der", "derived", "derived from"]: # Derivation templates
                                    source_lang = args.get("1", "") # Language code is usually arg 1
                                    source_word = args.get("2", "") # Source word is usually arg 2
                                ...
                                if source_word and isinstance(source_word, str):
                                    ...
                                    if source_word_clean and source_word_clean != "-" and source_word_clean.lower() != word.lower():
            ...
            for field, rel_type in top_level_relations_map.items():
                if field in entry and isinstance(entry[field], list):
                    for related_item in entry[field]:
                        ...
                        elif isinstance(related_item, dict) and "word" in related_item and isinstance(related_item["word"], str):
                            related_word_str = related_item["word"]

                        if related_word_str:
                            related_word_clean = clean_html(related_word_str)
                            if related_word_clean and related_word_clean.lower() != word.lower():
                                ...
                                related_cache_key = f"{related_word_clean.lower()}|{related_lang}"

                                if related_cache_key in word_cache:
                                    related_word_id = word_cache[related_cache_key]
                                else:
                                    related_word_id = get_or_create_word_id(
                                        cur, related_word_clean, related_lang, source_identifier
                                    )
                                    if related_word_id:
                                ...
                                if related_word_id and related_word_id != word_id:
                                    ...
                                    rel_id = insert_relation(
                                        cur, word_id, related_word_id,
                                        rel_type, source_identifier, metadata
                                    )
                                    if rel_id:
                                        local_stats["relations"] += 1 # Increment general relations count
                                    ...
                                    if not rel_type.bidirectional:
                                        inverse_rel = rel_type.get_inverse()
                                        if inverse_rel:
                                            # Need to flip word_id and related_word_id for inverse
                                            insert_relation(
                                                cur, related_word_id, word_id,
                                                inverse_rel, source_identifier, metadata
                                            )
