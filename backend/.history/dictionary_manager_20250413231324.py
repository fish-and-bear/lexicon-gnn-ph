...
def process_kaikki_lemma(lemma):
    ...
    # PRESERVE TESTS - Do these first to immediately return for special cases
    
    # 1. Check for no-space parenthesized content like "word(variant)" which should be preserved
    if re.search(r'\w+\([^)]+\)', lemma):
        return lemma
    
    # 2. Preserve standalone numbers (CRUCIAL FIX)
    if re.match(r'^\d+$', lemma):  # Matches strings that are only digits
        return lemma
        
    # 3. Preserve numbers with commas (e.g., "1,000")
    if re.search(r'\d,\d', lemma):
        return lemma
        
    # 4. Preserve hyphenated numbers (e.g., "pre-1977", "ika-100")
    if re.search(r'[-–]\d', lemma):
        return lemma
        
    # 5. Preserve numbers that follow words with spaces (e.g., "article on 100")
    if re.search(r'\s\d+', lemma):
        return lemma
        
    # PROCESSING - Only do this for the remaining cases
    
    # Only apply trailing number removal for words with trailing digits directly attached
    # Example: "suyo1" -> "suyo"
    result = lemma
    
    # Modified regex to only match letters followed by numbers (NOT digits after punctuation)
    if re.search(r'[a-zA-Z]\d+$', lemma):
        result = re.sub(r'([a-zA-Z])(\d+)$', r'\1', lemma)
        
        # CRUCIAL: Never return an empty string
        if not result:
            logger.warning(f"Processing would result in empty string, preserving original: '{lemma}'")
            return lemma
