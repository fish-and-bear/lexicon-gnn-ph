...
# Add this special helper function for Kaikki processing
def process_kaikki_lemma(lemma):
    ...
    # For numbers, only apply trailing number removal if it's a word with a trailing number
    # not a standalone number or number preceded by text with space
    if re.match(r'^\d+$', lemma):
        return lemma  # Preserve standalone numbers
    
    if re.search(r'\s\d+$', lemma):
        return lemma  # Preserve numbers at end with space before them
        
    # Apply normal trailing number removal for word1, word2, etc.
    # Only removing digits directly attached to words
    if re.search(r'\w\d+$', lemma):
        return re.sub(r'(\w)(\d+)$', r'\1', lemma)
