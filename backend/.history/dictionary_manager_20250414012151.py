        ...
        try:
            ...
            if not exists:
                ...
                try:
                    ...
                    issues.append(f"Failed to create required index {index_info['name']}: {create_err}")
        except Exception as e:
            issues.append(f"Error checking index '{index_info['name']}': {e}")
    
    # Return success if we fixed all issues or none were found
    if issues:
        for issue in issues:
            logger.error(f"Schema validation failed: {issue}")
        return False
