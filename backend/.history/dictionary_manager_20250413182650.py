...
@with_transaction(commit=True)
def process_capitalized_words(cur, _=None):
    """
    Process fully capitalized words and match them to properly cased words if they exist.
    If no match is found, the capitalized word is kept as is.
    
    This function is meant to be the last migration step.
    
    Args:
        cur: The database cursor
        _: Placeholder to match the signature pattern of other process_* functions
        
    Returns:
        Tuple[int, int]: Count of matched words and total capitalized words processed
    """
    logger.info("Processing fully capitalized words...")
    
    # Find all fully capitalized words (where every character is uppercase)
    cur.execute("""
        ...
        AND lemma !~ '[a-z0-9]'
    """)
    capitalized_words = cur.fetchall()
    
    if not capitalized_words:
        logger.info("No fully capitalized words found.")
        return 0, 0
    
    matched_count = 0
    total_count = len(capitalized_words)
    
    for word_id, lemma, language_code in capitalized_words:
        # Look for matching words with proper casing (either proper case or lowercase)
        # We'll check for case-insensitive matches but exclude the word itself
        cur.execute("""
            SELECT id, lemma
            FROM words
            WHERE lower(lemma) = lower(%s)
            AND language_code = %s
            AND id != %s
            ORDER BY 
                CASE WHEN lemma = %s THEN 0
                     WHEN lemma = lower(%s) THEN 1
                     ELSE 2 END,
                id ASC
            LIMIT 1
        """, (lemma, language_code, word_id, lemma.title(), lemma.lower()))
        
        match = cur.fetchone()
        
        if match:
            match_id, match_lemma = match
            logger.info(f"Matched capitalized word '{lemma}' with '{match_lemma}'")
            
            # Update all relevant relationships to point to the matched word
            tables_to_update = [
                ("relations", "from_word_id"),
                ("relations", "to_word_id"),
                ("definition_relations", "word_id"),
                ...
                ("definitions", "word_id"),
                ("credits", "word_id"),
                ("word_templates", "word_id"),
                ("word_forms", "word_id")
            ]
            
            for table, column in tables_to_update:
                # For each table, move references from the capitalized word to the matched word
                cur.execute(f"""
                    INSERT INTO {table} (
                        SELECT 
            ...
            # Delete the capitalized word as its references have been transferred
            cur.execute("DELETE FROM words WHERE id = %s", (word_id,))
            matched_count += 1
        # If no match is found, keep the capitalized word as is
    
    logger.info(f"Matched {matched_count} out of {total_count} capitalized words.")
...
def migrate_data(args):
    ...
    sources = [
        ...
        },
        {
         ...
         {
            "name": "Capitalized Words Processing",
            "file": None,  # No file needed for this handler
            "handler": process_capitalized_words,
            "required": True,  # Always process this
            "is_directory": False,
            "is_last": True,  # Mark as the last migration step to run
         },
    ]
