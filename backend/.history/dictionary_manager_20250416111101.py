...
def extract_parenthesized_text(text: str) -> Tuple[str, Optional[str]]:
    """
    Extract text in parentheses from a string, but preserve acronyms.
    ...
    Examples:
        ...
        "normal" -> ("normal", None)
        "Person Under Investigation (PUI)" -> ("Person Under Investigation (PUI)", None)
        "Organization (with details)" -> ("Organization", "with details")
    """
    if not text:
        return "", None
    
    # We'll build a new text by selectively removing non-acronym parenthesized content
    original_text = text
    extracted_parts = []
    ...
    # Sort matches in reverse order (from end to beginning) to not mess up indices
    # when we replace parts of the string
    matches.sort(key=lambda x: x.start(), reverse=True)
    
    # Create a list of spans to remove
    spans_to_remove = []
    
    for match in matches:
        content = match.group(1)
        
        # Check if the content is an acronym
        is_acronym = (
            re.match(r'^[A-Z0-9\s\-\.]+$', content) and 
            len(content) <= 10 and
            any(c.isalpha() for c in content)  # At least one letter
        )
        
        if not is_acronym:
            # Mark this span for removal
    ...
    # Only return extracted parts if we actually found any
    if extracted_parts:
        extracted_text = ", ".join(extracted_parts)
        logger.info(f"Extracted parenthesized text from lemma: original='{original_text}' → lemma='{result_text}', note='{extracted_text}'")
        return result_text, extracted_text
    else:
