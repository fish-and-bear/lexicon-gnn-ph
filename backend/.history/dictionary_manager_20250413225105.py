...
def process_kaikki_lemma(lemma):
    ...
    # Always preserve standalone numbers
    if re.match(r'^\d+$', lemma):
        return lemma
        
    # Always preserve numbers with separators (commas, hyphens)
    ...
    result = lemma
    if re.search(r'\w\d+$', lemma):
        result = re.sub(r'(\w)(\d+)$', r'\1', lemma)
        ...
        if result != lemma:
            logger.info(f"Removed trailing number(s) from lemma: '{lemma}' → '{result}'")
