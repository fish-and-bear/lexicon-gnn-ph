...
def extract_parenthesized_text(text: str) -> Tuple[str, Optional[str]]:
    ...
    for match in matches:
        ...
        # Check if the content is an acronym (all uppercase letters)
        # We'll consider it an acronym if:
        # 1. It's all uppercase letters (allowing for spaces)
        # 2. It's relatively short (<=10 chars)
        is_acronym = (
        ...
        if is_acronym:
            # Don't extract acronyms, keep them in the main text
            continue
        else:
            # Extract non-acronym parenthesized content
            parenthesized_parts.append(content)
            clean_text = clean_text.replace(match.group(0), "")
    
    # Clean up extra spaces
    clean_text = clean_text.strip()
    
    # Only return the parenthesized text if we actually extracted something
    if parenthesized_parts:
        original = text  # For logging
        logger.info(f"Extracted parenthesized text from lemma: original='{original}' → lemma='{clean_text}', note='{', '.join(parenthesized_parts)}'")
