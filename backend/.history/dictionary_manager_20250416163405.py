                ...
                if field in entry and isinstance(entry[field], list):
                    for related_item in entry[field]:
                        ...
                        if related_word_str:
                            ...
                            is_non_word = related_word_clean.lower() in NON_WORD_STRINGS
                       
                            if is_non_word:
                                logger.debug(f"Found non-word string '{related_word_clean}' for word '{word}'")
                                
                                # Add this information to the definition's metadata directly with a simple update
                                try:
                                    # First, determine what type of information this is
                                    ...
                                    note_text = f"{note_type}: {related_word_clean} (from {field})"
                                    
                                    # Update existing definitions directly
                                    cur.execute("""
                                        UPDATE definitions 
                                        SET usage_notes = CASE
                                            WHEN usage_notes IS NULL OR usage_notes = '' THEN %s
                                            WHEN usage_notes NOT LIKE %s THEN usage_notes || '; ' || %s
                                            ELSE usage_notes
                                        END
                                        WHERE word_id = %s
                                    """, (note_text, f"%{note_text}%", note_text, word_id))
                                    
                                    logger.info(f"Added {note_type} '{related_word_clean}' to definitions for word '{word}'")
                                except Exception as e:
                                    logger.error(f"Error storing note for '{related_word_clean}': {e}")
