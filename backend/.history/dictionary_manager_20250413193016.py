...
def process_tagalog_words(cur, filename: str, add_etymology: bool = False):
    ...
    if not entries or not isinstance(entries, list):
        logger.error(
            f"File {filename} does not contain a 'words' list or it's empty. {type(entries)}"
        )
        stats["errors"] += 1
        error_types["NoWordsList"] = error_types.get("NoWordsList", 0) + 1
        raise ValueError(f"No words list found in {filename}")
    ...
    with tqdm(total=len(entries), desc=f"Processing {source_identifier}") as pbar:
        for entry_index, entry in enumerate(entries):
            ...
            try:
                ...
                if not isinstance(entry, dict):
                    logger.warning(
                        f"Entry {entry_index} is not a dictionary. Skipping. Type: {type(entry)}"
                    )
                    stats["skipped"] += 1
                    cur.execute(f"RELEASE SAVEPOINT {savepoint_name}")
                    pbar.update(1)
                ...
                # Extract metadata
                item_word = entry.get("word", "").strip()
                if not item_word:
                ...
                language_code = "tl"
                language_name = entry.get("language", "").strip().lower()
                ...
                if pronunciations and isinstance(pronunciations, list):
                    for pronunciation in pronunciations:
                        ...
                        try:
                            ...
                            ipa = pronunciation.get("ipa", "").strip()
                            if ipa:
                ...
                if add_etymology and "etymology" in entry:
                ...
                # Cross-references
                crossref = entry.get("cross-references", [])
                if crossref and isinstance(crossref, list):
                    for xref in crossref:
                        xref_word = xref.get("word", "").strip()
                        if not xref_word or xref_word == item_word:
                            continue  # Skip empty or self-references
                        
                        # Add to relations batch
                        relations_batch.append({
                            'word_id': word_id,
                            'related_word': xref_word,
                ...
                if not definitions or not isinstance(definitions, dict):
                    logger.debug(
                        f"No valid definitions for word '{item_word}' at index {entry_index}"
                    )
                else:
                    for pos, pos_data in definitions.items():
                        if not isinstance(pos_data, dict):
                            continue

                        # Get the detailed definitions
                        detailed_defs = pos_data.get("definitions", [])
                        if not isinstance(detailed_defs, list):
                            continue

                        for def_index, def_item in enumerate(detailed_defs):
                            if not isinstance(def_item, dict):
                                continue
                            ...
                            examples_json = (
                                json.dumps(examples) if examples and isinstance(examples, list) else None
                            )
                            ...
                            try:
                                ...
                                if def_id:
                                    ...
                                    # Add antonyms to batch
                                    if antonyms and isinstance(antonyms, list):
                                        for ant in antonyms:
                                            if isinstance(ant, str) and ant.strip() and ant.strip() != item_word:
                                                relations_batch.append({
                                                    'word_id': word_id,
                ...
                # Process other fields that need to be implemented
                # - related_words: added above as part of relations batch
                # - baybayin: process below
                # - affixes: process below

                # Process Baybayin
                if "baybayin" in entry and entry["baybayin"]:
                    baybayin_text = entry["baybayin"].strip()
                    if baybayin_text:
                        try:
                            # Check if Baybayin exists first
                            cur.execute(
                                """
                                SELECT id FROM word_baybayin 
                                WHERE word_id = %s AND baybayin = %s
                                """,
                                (word_id, baybayin_text),
                            )
                            if not cur.fetchone():
                                cur.execute(
                                    """
                                    INSERT INTO word_baybayin (word_id, baybayin, source_identifier)
                                    VALUES (%s, %s, %s)
                                    """,
                                    (word_id, baybayin_text, source_identifier),
                                )
                        except psycopg2.Error as e:
                            logger.error(
                                f"Error adding Baybayin for word '{item_word}': {e}"
                            )
                            error_types[f"BaybayinError: {type(e).__name__}"] = (
                                error_types.get(f"BaybayinError: {type(e).__name__}", 0) + 1
                            )

                # Process affixes
                if "affixes" in entry and isinstance(entry["affixes"], list):
                    affixes = entry["affixes"]
                    for affix_data in affixes:
                        if not isinstance(affix_data, dict):
                            continue

                        affix_type = affix_data.get("type", "").strip()
                        affix_form = affix_data.get("form", "").strip()

                        if not affix_type or not affix_form:
                            continue
                        ...
                        try:
                            cur.execute(
                                """
                                INSERT INTO word_affixes 
                                (word_id, affix_type, affix_form, source_identifier)
                                VALUES (%s, %s, %s, %s)
                                ON CONFLICT (word_id, affix_type, affix_form) 
                                DO NOTHING
                                """,
                                (word_id, affix_type, affix_form, source_identifier),
                            )
                            ...
                            logger.error(
                                f"Error adding affix for word '{item_word}': {e}"
                            )
                            error_types[f"AffixError: {type(e).__name__}"] = (
                                error_types.get(f"AffixError: {type(e).__name__}", 0) + 1
                            )
                ...
                logger.error(
                    f"Error processing entry at index {entry_index} (word: '{entry.get('word', '<unknown>')}'):"
                )
