...
def process_tagalog_words(cur, filename: str, add_etymology: bool = True):
    """
    Process a JSON file containing Tagalog words and their definitions.
    ...
    """
    logger.info(f"Processing Tagalog words file: {filename}")
    
    # Initialize statistics dictionary
# Fix for the transaction reset issue in process_tagalog_words

# Replace this section at around line 6543 (initialization part)
    # Initialize statistics dictionary
    stats = {
        "entries_processed": 0,
        "skipped": 0,
        ...
        "etymologies_added": 0,
        "relations_added": 0,
        "capitalized_words_matched": 0,
        "capitalized_words_new": 0,
        "errors": 0,
    }
    error_types = {}  # Track error types for analysis
    conn = cur.connection  # Store connection for transaction management
    
    # Add a proper reset_transaction function
    def reset_transaction_state():
        """Reset transaction state to recover from errors"""
        nonlocal conn, cur
        try:
            # Fully rollback the current transaction
            conn.rollback()
            ...
            logger.error(f"Critical error when resetting transaction: {e}")
            return False
    ...
    if not source_identifier:
        source_identifier = "tagalog_json"
    logger.info(f"Using standardized source identifier: '{source_identifier}'")
    
    # Check table structure to avoid errors
    required_columns = {
        "pronunciations": ["word_id", "pronunciation", "source_identifier"],
        "etymologies": ["word_id", "etymology", "source_identifier"],
        "definitions": ["word_id", "definition", "part_of_speech", "source_identifier"],
        "examples": ["definition_id", "example", "source_identifier"],
        "word_tags": ["word_id", "tag", "source_identifier"]
    }
    ...
    for table, columns in required_columns.items():
        ...
        try:
            ...
            table_column_map[table] = existing_columns
            
            missing_columns = set(columns) - set(existing_columns)
            if missing_columns:
                logger.warning(f"Table {table} is missing required columns: {', '.join(missing_columns)}")
        except Exception as e:
            logger.warning(f"Error checking columns for table {table}: {e}")
    ...
    try:
        ...
        logger.info(f"Found {len(data)} word entries in {filename}")
        if len(data) == 0:
            logger.info("No entries found in file. Nothing to process.")
            return stats
        
        # Process each entry
        with tqdm(total=len(data), desc=f"Processing {source_identifier}") as pbar:
            for entry_index, (word_key, entry) in enumerate(data.items()):
                # Reset transaction state if needed
                # Replace the "Reset transaction state if needed" section with this:
                # Check transaction status and reset if needed
                if conn.status != psycopg2.extensions.STATUS_READY:
                    logger.info(f"Resetting transaction state before entry {entry_index}")
                    if not reset_transaction_state():
                        logger.error("Failed to reset transaction state. Skipping entry.")
                        stats["skipped"] += 1
                        pbar.update(1)
                        continue
                    try:
                ...
                if not isinstance(entry, dict):
                    logger.warning(f"Skipping entry {word_key}: not a dictionary")
                    stats["skipped"] += 1
                    pbar.update(1)
                    continue
                
                # Create a savepoint for this entry
                savepoint_name = f"entry_{entry_index}"
                
                try:
                    cur.execute(f"SAVEPOINT {savepoint_name}")
                    
                    # Extract word from entry
                    item_word = entry.get("word", word_key).strip()
                    if not item_word:
                        item_word = word_key.strip()
                    
                    # Skip empty words
                    if not item_word:
                        logger.warning(f"Skipping entry at index {entry_index}: empty word")
                        stats["skipped"] += 1
                        cur.execute(f"RELEASE SAVEPOINT {savepoint_name}")
                        pbar.update(1)
                        continue
                    ...
                    try:
                        ...
                        word_id_cache[item_word] = word_id
                    except psycopg2.Error as e:
                        logger.error(f"Error getting/creating word '{item_word}': {e}")
                        error_types[f"WordCreationError: {type(e).__name__}"] = (
                            error_types.get(f"WordCreationError: {type(e).__name__}", 0) + 1
                        )
                        raise
                    
                    # Process pronunciation only if all required columns exist
                    ...
                    if pronunciation and set(required_columns["pronunciations"]).issubset(table_column_map["pronunciations"]):
                        try:
                            cur.execute(
                                ...
                                (word_id, pronunciation, source_identifier)
                                VALUES (%s, %s, %s)
                            ...
                            logger.warning(
                                f"Error adding pronunciation for word '{item_word}': {e}"
                            )
                            error_types[f"PronunciationError: {type(e).__name__}"] = (
                                error_types.get(f"PronunciationError: {type(e).__name__}", 0) + 1
                            )
                            # Don't raise the error, continue processing
                    
                    # Process etymology only if all required columns exist
                    if add_etymology and "etymology" in entry and entry["etymology"] and set(required_columns["etymologies"]).issubset(table_column_map["etymologies"]):
                        ...
                        if isinstance(etymology, str) and etymology.strip():
                            try:
                                cur.execute(
                                    """
                                    INSERT INTO etymologies 
                                    (word_id, etymology, source_identifier)
                                    VALUES (%s, %s, %s)
                                    ON CONFLICT (word_id, etymology) 
                                    DO NOTHING
                                    """,
                                    (word_id, etymology.strip(), source_identifier),
                                )
                                stats["etymologies_added"] += 1
                            except psycopg2.Error as e:
                                logger.warning(
                                    f"Error adding etymology for word '{item_word}': {e}"
                                )
                                error_types[f"EtymologyError: {type(e).__name__}"] = (
                                    error_types.get(f"EtymologyError: {type(e).__name__}", 0) + 1
                                )
                                # Don't raise the error, continue processing
                    ...
                    if isinstance(cross_refs, list):
                        for ref in cross_refs:
                            ...
                            # Add to relations batch for later processing
                            relations_batch.append({
                                'word_id': word_id,
                                'related_word': ref.strip(),
                                'rel_type': 'cross-reference',
                                'source_identifier': source_identifier
                            })
                    
                    # Process definitions by part of speech only if all required columns exist
                            ...
                            for def_item in def_list:
                                ...
                                try:
                                    ...
                                    cur.execute(
                                        """
                                        INSERT INTO definitions 
                                        (word_id, definition, part_of_speech, source_identifier)
                                        VALUES (%s, %s, %s, %s)
                                        RETURNING id
                                        """,
                                        (word_id, definition_text, pos, source_identifier),
                                    )
                                    def_id = cur.fetchone()[0]
                                    stats["definitions_added"] += 1
                                    
                                    # Add examples only if all required columns exist
                                            ...
                                            if isinstance(example, str) and example.strip():
                                                try:
                                                    cur.execute(
                                                        ...
                                                        (definition_id, example, source_identifier)
                                                        VALUES (%s, %s, %s)
                                                        ON CONFLICT (definition_id, example) 
                                                        DO NOTHING
                                                        """,
                                                        (def_id, example.strip(), source_identifier),
                                                    )
                                                except psycopg2.Error as e:
                                                    logger.warning(
                                                        f"Error adding example for definition {def_id}: {e}"
                                                    )
                                                    error_types[f"ExampleError: {type(e).__name__}"] = (
                                                        error_types.get(f"ExampleError: {type(e).__name__}", 0) + 1
                                                    )
                                                    # Don't raise the error, continue processing
                                    ...
                                    logger.error(
                                        f"Error adding definition for word '{item_word}': {e}"
                                    )
                                    error_types[f"DefinitionError: {type(e).__name__}"] = (
                                        error_types.get(f"DefinitionError: {type(e).__name__}", 0) + 1
                                    )
                                    # Don't raise, we'll just continue with the next definition
                    
                    # Process relations in batches (every 100 entries)
                    if len(relations_batch) >= 100:
                        try:
                            stats = process_relations_batch(cur, relations_batch, stats, word_id_cache)
                            relations_batch = []
                        except Exception as rel_err:
                            logger.warning(f"Error processing relations batch: {rel_err}")
                            relations_batch = []  # Clear batch on error
                    
                    # Process domains as tags only if all required columns exist
                        ...
                        for domain in domains:
                            ...
                            try:
                                cur.execute(
                                    ...
                                    (word_id, tag, source_identifier)
                                    VALUES (%s, %s, %s)
                                    ON CONFLICT (word_id, tag) 
                                    DO NOTHING
                                    """,
                                ...
                                logger.warning(
                                    f"Error adding domain tag for word '{item_word}': {e}"
                                )
                                error_types[f"DomainTagError: {type(e).__name__}"] = (
                                    error_types.get(f"DomainTagError: {type(e).__name__}", 0) + 1
                                )
                                # Don't raise the error, continue processing
                    
                    # Check for capitalized word variant
                    if item_word and len(item_word) > 1 and not item_word[0].isupper() and item_word[0].isalpha():
                        try:
                            ...
                            # Check if capitalized word exists in cache first
                            if capitalized_word in word_id_cache:
                                cap_word_id = word_id_cache[capitalized_word]
                                stats["capitalized_words_matched"] += 1
                            else:
                                # Check if capitalized variant exists in database
                                try:
                                    cur.execute(
                                        """
                                        SELECT id FROM words
                                    ...
                                    cap_word_result = cur.fetchone()
                                    
                                    if cap_word_result:
                                        cap_word_id = cap_word_result[0]
                                        word_id_cache[capitalized_word] = cap_word_id
                                        stats["capitalized_words_matched"] += 1
                                    else:
                                        # Create capitalized variant and establish relation
                                        cap_word_id = get_or_create_word_id(
                                            cur,
                                            capitalized_word,
                                            language_code=language_code,
                                            source_identifier=source_identifier,
                                        ...
                                        word_id_cache[capitalized_word] = cap_word_id
                                        stats["capitalized_words_new"] += 1
                                    
                                    # Add capitalized variant relation to batch
                                    relations_batch.append({
                                        'word_id': word_id,
                                        'related_word_id': cap_word_id,
                                        'rel_type': 'capitalized-variant',
                                        'source_identifier': source_identifier
                                    })
                                except psycopg2.Error as e:
                                    logger.warning(
                                        f"Error processing capitalized variant for '{item_word}': {e}"
                                    )
                                    error_types[f"CapitalizedVariantError: {type(e).__name__}"] = (
                                        error_types.get(f"CapitalizedVariantError: {type(e).__name__}", 0) + 1
                                    )
                                    # Don't raise the error, continue processing
                        except Exception as cap_err:
                            logger.warning(f"Error with capitalized variant for '{item_word}': {cap_err}")
                            # Don't raise, we'll keep going
                    
                    # Release savepoint (commit this entry)
                    cur.execute(f"RELEASE SAVEPOINT {savepoint_name}")
                    stats["entries_processed"] += 1
                
                except Exception as e:
                    # An error occurred for this entry - roll it back and continue with next
                    logger.error(
                        f"Error processing entry at index {entry_index} (word: '{word_key}'):"
                    )
                    logger.error(f"Error details: {e}")
                    logger.debug(f"Entry data: {str(entry)[:200]}...")
                    error_types[f"EntryProcessingError: {type(e).__name__}"] = (
                        error_types.get(f"EntryProcessingError: {type(e).__name__}", 0) + 1
                    )
                    
                    # Reset transaction state completely instead of trying savepoint rollback
                    # which might not work if the transaction is already aborted
                    ...
                    try:
                        ...
                        # If transaction is already aborted, do a full rollback
                        try:
                            conn.rollback()
                        except Exception as reset_error:
                            logger.error(f"Failed to rollback transaction: {reset_error}")
        ...
        # Process any remaining relations in the batch
        if relations_batch:
            try:
                stats = process_relations_batch(cur, relations_batch, stats, word_id_cache)
            except Exception as final_rel_err:
                logger.warning(f"Error processing final relations batch: {final_rel_err}")
    
    except Exception as e:
        logger.error(f"Critical error processing file {filename}: {e}", exc_info=True)
        stats["errors"] += 1
        error_types[f"CriticalError: {type(e).__name__}"] = (
            error_types.get(f"CriticalError: {type(e).__name__}", 0) + 1
        )
        try:
            conn.rollback()
        except Exception as rb_err:
            logger.error(f"Failed to rollback after critical error: {rb_err}")
    
    # Log final statistics
    logger.info(f"Processed {stats['entries_processed']} entries")
    if stats["errors"] > 0:
        logger.warning(f"Encountered {stats['errors']} errors during processing")
        logger.debug(f"Error types: {error_types}")
    ...
    logger.info(f"Added/updated {stats['relations_added']} relations")
    logger.info(f"  Capitalized words matched: {stats['capitalized_words_matched']}")
    logger.info(f"  Capitalized words created: {stats['capitalized_words_new']}")
