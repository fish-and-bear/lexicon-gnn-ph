...
def remove_trailing_numbers(text: str) -> str:
    """
    Remove trailing numbers from a string, but only when directly attached to a word.
    ...
    Examples:
        ...
        "1,000" -> "1,000" (unchanged - not attached to a word)
        "article on 100" -> "article on 100" (unchanged - space between word and number)
    """
    if not text:
        return ""
    
    # Only remove numbers that are directly attached to a word character
    # This regex matches a word character followed by digits at the end of string
    original = text
    result = re.sub(r'(\w)(\d+)$', r'\1', text)
    
    if original != result:
        logger.info(f"Removed trailing number(s) from lemma: '{original}' → '{result}'")
...
def process_kaikki_jsonl(cur, filename: str):
    ...
    def process_single_entry(entry):
        ...
        # Modify around line 8798 in process_single_entry function
        word = entry["word"]
