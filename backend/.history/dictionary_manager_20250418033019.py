                    ...
                    for related_item in entry[field]:
                        ...
                        if isinstance(related_item, str):
                            ...
                            if related_word_str:
                                ...
                                # More robust check for non-word strings (language codes, descriptors)
                                is_nonword = False
                                
                                # Direct match with NON_WORD_STRINGS
                                if related_word_clean.lower() in NON_WORD_STRINGS:
                                    is_nonword = True
                                    logger.debug(f"Direct match found: '{related_word_clean}' is in NON_WORD_STRINGS")
                                
                                # Special case for 'Spanish-based orthography' which might have variations
                                elif "spanish" in related_word_clean.lower() and "orthography" in related_word_clean.lower():
                                    is_nonword = True
                                    logger.debug(f"Special case: '{related_word_clean}' contains 'spanish' and 'orthography'")
                                
                                # If it's a non-word, store it as metadata and skip
                                if is_nonword:
                                    logger.info(f"Filtering non-word string '{related_word_clean}' for word '{word}'")
                                    
                                    # Skip creating relationship for this non-word - IMPORTANT!
                                    continue
                                logger.debug(f"Found non-word string '{related_word_clean}' for word '{word}'")
                                ...
                                try:
                                    ...
                                    if len(related_word_clean) <= 3 and related_word_clean.isalpha():
                                        note_type = "Language Code" 
                                    elif related_word_clean.lower() == "spanish-based orthography":
                                        note_type = "Orthography"
                                    elif related_word_clean.lower() in ["obsolete", "archaic", "dialectal"]:
                                        note_type = "Usage"
                                    ...
                                    cur.execute("""
                                        ...
                                        END
                                        WHERE id = %s
                                    """, (note_type, related_word_clean, field, note_type, related_word_clean, field, note_type, related_word_clean, field, word_id))
                                    
                                    logger.info(f"Added {note_type} '{related_word_clean}' to word_metadata for '{word}'")
                                except Exception as e:
                                    logger.error(f"Error storing metadata for '{related_word_clean}': {e}")
