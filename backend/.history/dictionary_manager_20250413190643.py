...
def process_tagalog_words(cur, filename: str):
    ...
    stats = {
        ...
        "capitalized_words_matched": 0,  # New stat for capitalized words matched to existing
        "capitalized_words_new": 0,  # New stat for capitalized words that didn't match existing
    }
                                            ...
                                            for item_idx, item_word in enumerate(
                                                ...
                                                if (
                                                    ...
                                                    if (
                                                        ...
                                                        try:
                                                            # Check if word is fully capitalized and try to find case-insensitive match
                                                            matched_word_id = None
                                                            is_fully_capitalized = item_word_clean.isupper() and len(item_word_clean) > 1
                                                            
                                                            if is_fully_capitalized:
                                                                # Try to find a case-insensitive match using a parameterized query for better performance
                                                                # Add an index hint if available to speed up this operation
                                                                lowercase_lookup = item_word_clean.lower()
                                                                cur.execute("""
                                                                    SELECT id FROM words 
                                                                    WHERE LOWER(lemma) = %s 
                                                                    AND lemma != %s
                                                                    ORDER BY created_at ASC LIMIT 1
                                                                """, (lowercase_lookup, item_word_clean))
                                                                result = cur.fetchone()
                                                                if result:
                                                                    matched_word_id = result[0]
                                                                    stats["capitalized_words_matched"] += 1
                                                                    logger.debug(f"Matched capitalized word '{item_word_clean}' to existing word ID {matched_word_id}")
                                                            
                                                            # Use matched word ID if found, otherwise create new
                                                            if matched_word_id:
                                                                item_id = matched_word_id
                                                            else:
                                                                # Check word_id_cache first before hitting database
                                                                if item_word_clean in word_id_cache:
                                                                    item_id = word_id_cache[item_word_clean]
                                                                else:
                                                                    if is_fully_capitalized:
                                                                    ...
                                                                    if item_id:
                                                                        word_id_cache[item_word_clean] = item_id
                        ...
                        if isinstance(baybayin_text, str) and baybayin_text.strip():
                            ...
                            try:
                                if validate_baybayin_entry(baybayin_form_raw):
                                    clean_baybayin = clean_baybayin_text(baybayin_form_raw)
                                    if clean_baybayin:
                                        try:
                                            ...
                                            logger.error(
                                                ...
                                                exc_info=False  # Avoid full traceback for cleaner logs
                                            )
                                else:
                                    logger.warning(
                                ...
                                logger.warning(
                                    f"Exception during Baybayin validation for '{lemma}': {validate_err}"
                                )
