...
def extract_etymology_components(etymology_text):
    ...
    # Check if this is a simple language code in brackets (e.g. "[ Ing ]", "[ Esp ]")
    # This pattern indicates a borrowed word, not actual etymology components
    language_code_pattern = re.compile(r'^\s*\[\s*([A-Za-z]+)\s*\]\s*$')
    match = language_code_pattern.match(etymology_text) if etymology_text else None
    
    if match:
        lang_code = match.group(1)
        # Return a structured representation that indicates this is a borrowed word
        return {
            "original_text": etymology_text,
            "is_borrowed": True,
            "source_language": lang_code,
            "components": []  # No actual components
        }

    # For non-bracketed language codes, try to extract actual components
    # Here you could implement more structured etymology parsing
