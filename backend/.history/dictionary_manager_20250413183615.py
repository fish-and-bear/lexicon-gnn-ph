...
def get_or_create_word_id(
    ...
    **kwargs,
    ...
    Args:
        cur: Database cursor
        lemma: Word lemma
        language_code: Language code (default: DEFAULT_LANGUAGE_CODE)
        source_identifier: Source identifier (optional)
        check_exists: Whether to check if the word exists before inserting (default: False)
        **kwargs: Additional keyword arguments for word creation
    ...
    cur.execute(
        ...
        SELECT id
        FROM words
        WHERE lemma = %s AND language_code = %s
    """,
    ...
    try:
        cur.execute(
            """
            INSERT INTO words (lemma, language_code, source_identifier)
            VALUES (%s, %s, %s)
            RETURNING id
        """,
            (normalized_lemma, language_code, source_identifier),
        )
        word_id = cur.fetchone()[0]

        # Insert source information
        if source_identifier:
            insert_source(cur, word_id, source_identifier)

        # Insert credit information
        insert_credit(cur, word_id, source_identifier)

        return word_id

    except Exception as e:
        logger.error(
            f"Error inserting word '{lemma}' from '{source_identifier}': {e}",
            exc_info=True,
        )
        return None
...
def batch_get_or_create_word_ids(
    ...
    **kwargs,
    ...
    """
    Get or create word IDs for a batch of lemmas.
    ...
    Returns:
        Dictionary mapping lemmas to word IDs
    """
    word_ids = {}
...
def insert_definition(
    ...
    etymology_languages: Optional[List[str]] = None,
    etymology_full_language_names: Optional[List[str]] = None,
    etymology_other_terms: Optional[List[str]] = None,
    etymology_terms: Optional[List[str]] = None,
    domains: Optional[List[str]] = None,
    categories: Optional[List[str]] = None,
    synonyms: Optional[List[str]] = None,
    references: Optional[List[str]] = None,
    pronunciation: Optional[str] = None,
    alternate_pronunciation: Optional[str] = None,
    derivative: Optional[str] = None,
    affix_forms: Optional[List[str]] = None,
    affix_types: Optional[List[str]] = None,
    **kwargs,
) -> int:
    """
    ...
    Args:
        ...
        etymology_languages: Languages in etymology (optional)
        etymology_full_language_names: Full language names in etymology (optional)
        etymology_other_terms: Other terms in etymology (optional)
        etymology_terms: Etymology terms (optional)
        domains: Domains (optional)
        categories: Categories (optional)
        synonyms: Synonyms (optional)
        references: References (optional)
        pronunciation: Pronunciation (optional)
        alternate_pronunciation: Alternate pronunciation (optional)
        derivative: Derivative (optional)
        affix_forms: Affix forms (optional)
        affix_types: Affix types (optional)
    ...
    try:
        ...
        cur.execute(
            ...
            INSERT INTO definitions (
                word_id, definition, part_of_speech, source_identifier, counter, example,
                etymology, etymology_languages, etymology_full_language_names,
                etymology_other_terms, etymology_terms, domains, categories,
                synonyms, references, pronunciation, alternate_pronunciation,
                derivative, affix_forms, affix_types
            )
            VALUES (
        ...
        """,
            (
                ...
                etymology,
                etymology_languages,
                etymology_full_language_names,
                etymology_other_terms,
                etymology_terms,
                domains,
                categories,
                synonyms,
        ...
        if categories:
            for category in categories:
                cur.execute(
                    """
                    INSERT INTO definition_categories (definition_id, category_name)
                    VALUES (%s, %s)
                """,
                    (definition_id, category),
                )

        # Insert synonyms and references
        if synonyms or references:
            insert_relations(
                cur,
                word_id,
                definition_id,
        ...
        logger.error(
            f"Error inserting definition for word ID {word_id} from '{source_identifier}': {e}",
            exc_info=True,
        )
        return None
...
def insert_relations(
    ...
    """
    Insert synonyms and references for a definition.
...
def insert_relation(
    ...
    """
    Insert a synonym or reference relation for a definition.
    ...
    Args:
        ...
        relation_type: Type of relation ("synonym" or "reference")
        source_identifier: Source identifier (optional)
    """
    # Normalize related word
    related_word_clean = normalize_lemma(related_word)

    if related_word_clean != related_word:
        logger.debug(
            f"Normalized related word '{related_word}' to '{related_word_clean}'"
        )

    # Check if the related word exists
    cur.execute(
        """
        SELECT id
        FROM words
    ...
    if result:
        ...
        # Related word doesn't exist, create it
    ...
    if related_word_id is None:
        logger.error(
            f"Failed to get or create ID for related word '{related_word_clean}'"
        )
        return
    ...
    try:
        cur.execute(
            ...
            INSERT INTO relations (
                word_id, definition_id, related_word_id, relation_type, source_identifier
            )
            VALUES (%s, %s, %s, %s, %s)
        """,
            (
        ...
        logger.error(
            f"Error inserting {relation_type} relation for word ID {word_id} from '{source_identifier}': {e}",
            exc_info=True,
        )


@with_transaction(commit=True)
def insert_source(cur, word_id: int, source_identifier: str) -> None:
    """
    Insert a source for a word.
    ...
    try:
        cur.execute(
            """
            INSERT INTO sources (word_id, source_identifier)
            VALUES (%s, %s)
        """,
            (word_id, source_identifier),
        )
    except Exception as e:
        logger.error(
            f"Error inserting source for word ID {word_id} from '{source_identifier}': {e}",
            exc_info=True,
        )


@with_transaction(commit=True)
def insert_credit(cur, word_id: int, source_identifier: str) -> None:
    """
    Insert credit information for a word.
    ...
    try:
        cur.execute(
            """
            INSERT INTO credits (word_id, source_identifier)
            VALUES (%s, %s)
        """,
            (word_id, source_identifier),
        )
        ...
        logger.error(
            f"Error inserting credit for word ID {word_id} from '{source_identifier}': {e}",
            exc_info=True,
        )
    ...
    try:
        cur.execute(
            ...
            INSERT INTO word_templates (
                word_id, template_type, template_data, source_identifier
            )
            VALUES (%s, %s, %s, %s)
        """,
            (
        ...
        logger.error(
            f"Error inserting word template for word ID {word_id} from '{source_identifier}': {e}",
            exc_info=True,
        )


@with_transaction(commit=True)
def insert_word_template_data(
    cur,
    word_id: int,
    template_type: str,
    template_data: Dict[str, Any],
    ...
    try:
        cur.execute(
            """
            INSERT INTO word_template_data (
                word_id, template_type, template_data, source_identifier
            )
            VALUES (%s, %s, %s, %s)
        """,
        ...
        logger.error(
            f"Error inserting word template data for word ID {word_id} from '{source_identifier}': {e}",
            exc_info=True,
        )


@with_transaction(commit=True)
def insert_word_template_data_batch(
    cur,
    word_id: int,
    template_type: str,
    ...
    """
    Insert a batch of word template data.

    Args:
        cur: Database cursor
        word_id: ID of the word
        template_type: Type of the template
        template_data_list: List of template data
        source_identifier: Source identifier
    """
    try:
        ...
        logger.error(
            f"Error inserting word template data batch for word ID {word_id} from '{source_identifier}': {e}",
            exc_info=True,
        )


@with_transaction(commit=True)
def insert_word_template_data_batch_with_ids(
    cur,
    word_id: int,
    template_type: str,
    template_data_list: List[Dict[str, Any]],
    ...
    """
    Insert a batch of word template data with IDs.
    ...
    try:
        ...
        logger.error(
            f"Error inserting word template data batch with IDs for word ID {word_id} from '{source_identifier}': {e}",
            exc_info=True,
        )


@with_transaction(commit=True)
def insert_word_template_data_with_id(
    cur,
    word_id: int,
    template_type: str,
    template_data: Dict[str, Any],
    ...
    """
    Insert word template data with ID.
    ...
    try:
        ...
        logger.error(
            f"Error inserting word template data with ID for word ID {word_id} from '{source_identifier}': {e}",
            exc_info=True,
        )


@with_transaction(commit=True)
def insert_word_template_data_with_id_batch(
    cur,
    word_id: int,
    template_type: str,
    template_data_list: List[Dict[str, Any]],
    ...
    try:
        ...
        logger.error(
            f"Error inserting word template data with ID batch for word ID {word_id} from '{source_identifier}': {e}",
            exc_info=True,
        )


@with_transaction(commit=True)
def insert_word_template_data_with_id_batch_with_ids(
    cur,
    word_id: int,
    template_type: str,
    template_data_list: List[Dict[str, Any]],
    ...
    try:
        ...
        logger.error(
            f"Error inserting word template data with ID batch with IDs for word ID {word_id} from '{source_identifier}': {e}",
            exc_info=True,
        )


@with_transaction(commit=True)
def insert_word_template_data_with_id_batch_with_ids_and_ids(
    cur,
    word_id: int,
    template_type: str,
    template_data_list: List[Dict[str, Any]],
    ...
    try:
        ...
        logger.error(
            f"Error inserting word template data with ID batch with IDs and IDs for word ID {word_id} from '{source_identifier}': {e}",
            exc_info=True,
        )


@with_transaction(commit=True)
def insert_word_template_data_with_id_batch_with_ids_and_ids_and_ids(
    cur,
    word_id: int,
    template_type: str,
    template_data_list: List[Dict[str, Any]],
    ...
    try:
        ...
        logger.error(
            f"Error inserting word template data with ID batch with IDs and IDs and IDs for word ID {word_id} from '{source_identifier}': {e}",
            exc_info=True,
        )


@with_transaction(commit=True)
def insert_word_template_data_with_id_batch_with_ids_and_ids_and_ids_and_ids(
    cur,
    word_id: int,
    template_type: str,
    template_data_list: List[Dict[str, Any]],
    ...
    try:
        ...
        logger.error(
            f"Error inserting word template data with ID batch with IDs and IDs and IDs and IDs for word ID {word_id} from '{source_identifier}': {e}",
            exc_info=True,
        )


@with_transaction(commit=True)
def insert_word_template_data_with_id_batch_with_ids_and_ids_and_ids_and_ids_and_ids(
    cur,
    word_id: int,
    template_type: str,
    template_data_list: List[Dict[str, Any]],
    ...
    try:
        ...
        logger.error(
            f"Error inserting word template data with ID batch with IDs and IDs and IDs and IDs and IDs for word ID {word_id} from '{source_identifier}': {e}",
            exc_info=True,
        )


@with_transaction(commit=True)
def insert_word_template_data_with_id_batch_with_ids_and_ids_and_ids_and_ids_and_ids_and_ids(
    cur,
    word_id: int,
    template_type: str,
    template_data_list: List[Dict[str, Any]],
    ...
    try:
        ...
        logger.error(
            f"Error inserting word template data with ID batch with IDs and IDs and IDs and IDs and IDs and IDs for word ID {word_id} from '{source_identifier}': {e}",
            exc_info=True,
        )


@with_transaction(commit=True)
def insert_word_template_data_with_id_batch_with_ids_and_ids_and_ids_and_ids_and_ids_and_ids_and_ids(
    cur,
    word_id: int,
    template_type: str,
    template_data_list: List[Dict[str, Any]],
...
def process_tagalog_words(cur, filename: str):
    ...
    stats = {
        ...
        "capitalized_word_matches": 0,  # Track capitalized words matched to existing proper case
    }
                                            ...
                                            for item_idx, item_word in enumerate(
                                                ...
                                                if (
                                                    ...
                                                    if (
                                                        ...
                                                        try:
                                                            # Check if fully capitalized word has existing proper case match
                                                            existing_item_id = None
                                                            if item_word_clean.isupper() and len(item_word_clean) > 1:
                                                                existing_item_id = get_existing_word_id_with_proper_case(
                                                                    cur, item_word_clean
                                                                )
                                                                if existing_item_id:
                                                                    stats["capitalized_word_matches"] += 1
                                                            
                                                            # If we found an existing match with proper case, use it
                                                            # Otherwise, create the word with original casing
                                                            item_id = existing_item_id or get_or_create_word_id(
                                                                cur,
                                                                item_word_clean,
                                                                language_code,
                                                                source_identifier=source_identifier,
    ...
    # This block will execute if the main try block completes without fatal errors
        ...
        logger.info(f"  Capitalized words matched to existing proper case: {stats['capitalized_word_matches']}")
