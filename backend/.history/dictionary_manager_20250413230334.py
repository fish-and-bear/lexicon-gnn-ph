...
def get_or_create_word_id(
    ...
    **kwargs,
    ...
    # Remove trailing numbers from lemma (e.g., "suyo1" becomes "suyo")
...
def process_tagalog_words(cur, filename: str, add_etymology: bool = False):
    """
    Process the scraped json from the dictionary.
    This file is huge. Rows are word entries. Transactions
    are managed manually using savepoints (one per word entry)
    for individual entry resilience.

    Args:
        cur: Database cursor
        filename: Path to the tagalog_words json file
    ...
    error_types = {}  # Track error types for debugging

    source_identifier = standardize_source_identifier(os.path.basename(filename))
    if not source_identifier:  # Fallback if standardization fails
        source_identifier = "tagalog.com-Dictionary"
    logger.info(f"Using standardized source identifier: '{source_identifier}'")

    conn = cur.connection  # Get the connection for savepoint management

    # Cache for word IDs to avoid repeated database lookups
    word_id_cache = {}
    # Batch collection for relations to process
    relations_batch = []
    ...
    # Check if data is a dictionary (key: word entry format)
    if not isinstance(data, dict):
        logger.error(f"File {filename} does not contain a dictionary at the top level.")
        stats["errors"] += 1
        error_types["InvalidTopLevelFormat"] = (
            error_types.get("InvalidTopLevelFormat", 0) + 1
        )
        raise TypeError(f"Invalid top-level format in {filename}")

    # For the new format, entries are the direct key-value pairs in the dictionary
    entries = list(data.items())
    
    if not entries:
        logger.error(f"No entries found in {filename}")
        stats["errors"] += 1
        error_types["NoEntries"] = error_types.get("NoEntries", 0) + 1
        raise ValueError(f"No entries found in {filename}")

    logger.info(f"Found {len(entries)} entries in {filename}")
    if len(entries) == 0:
        logger.info(f"No entries to process in {filename}")
        return stats

    # Process each entry
    with tqdm(total=len(entries), desc=f"Processing {source_identifier}") as pbar:
        for entry_index, (word_key, entry) in enumerate(entries):
            ...
            try:
                ...
                if not isinstance(entry, dict):
                    logger.warning(
                        f"Entry {word_key} is not a dictionary. Skipping. Type: {type(entry)}"
                    )
                    stats["skipped"] += 1
                    cur.execute(f"RELEASE SAVEPOINT {savepoint_name}")
                    pbar.update(1)
                ...
                # Extract metadata - get word from entry, not key, as key might be modified
                item_word = entry.get("word", "").strip() or word_key.strip()
                if not item_word:
                    logger.warning(f"Entry {entry_index} has no word. Skipping.")
                    stats["skipped"] += 1
                    cur.execute(f"RELEASE SAVEPOINT {savepoint_name}")
                    pbar.update(1)
                    continue

                # Get language code - default to Tagalog
                language_code = "tl"
                
                # Check if word is already in cache
                if item_word in word_id_cache:
                    word_id = word_id_cache[item_word]
                else:
                    ...
                    if not word_id or word_id <= 0:
                        raise ValueError(f"Failed to get/create word ID for '{item_word}'")
                ...
                # Etymology processing - new format has it in an etymology object
                if add_etymology and "etymology" in entry and isinstance(entry["etymology"], dict):
                    etym_obj = entry["etymology"]
                    raw_etym = etym_obj.get("raw", "").strip()
                    if raw_etym:
                        try:
                            # Insert etymology
                            inserted = insert_etymology(
                                cur,
                                word_id,
                                raw_etym,
                                source_identifier=source_identifier,
                ...
                # Process references in senses
                references = []
                senses = entry.get("senses", [])
                if senses and isinstance(senses, list):
                    for sense in senses:
                        if isinstance(sense, dict) and "references" in sense:
                            refs = sense.get("references", [])
                            if refs and isinstance(refs, list):
                                references.extend(refs)
                
                # Add references to relations batch
                for ref in references:
                    if isinstance(ref, str) and ref.strip() and ref.strip() != item_word:
                ...
                if not senses or not isinstance(senses, list):
                    logger.debug(
                        f"No valid senses for word '{item_word}' at index {entry_index}"
                    )
                else:
                    for sense_index, sense in enumerate(senses):
                        ...
                        # Get part of speech
                        pos = None
                        pos_list = sense.get("part_of_speech", [])
                        if pos_list and isinstance(pos_list, list) and len(pos_list) > 0:
                            pos = pos_list[0]  # Take first part of speech
                        
                        # Process examples
                        examples = []
                        example_obj = sense.get("example", {})
                        if isinstance(example_obj, dict) and "examples" in example_obj:
                            examples = example_obj.get("examples", [])
                        
                        examples_json = json.dumps(examples) if examples and isinstance(examples, list) else None
                        ...
                        try:
                            ...
                            if def_id:
                                ...
                                logger.debug(
                                    f"Added definition for word '{item_word}': {definition_text[:30]}..."
                                )
                                ...
                                if synonyms and isinstance(synonyms, list):
                                    for syn in synonyms:
                                        if isinstance(syn, str) and syn.strip() and syn.strip() != item_word:
                                            relations_batch.append({
                                                'word_id': word_id,
...
def process_kaikki_lemma(lemma):
    ...
    # Check for no-space parenthesized content like "word(variant)" which should be preserved
    if re.search(r'\w+\([^)]+\)', lemma):
        return lemma  # Preserve parenthesized variants
    
    # Always preserve standalone numbers or year-like numbers (all digits)
    if re.match(r'^\d+$', lemma):
        return lemma
        
    # Always preserve numbers with separators (commas, hyphens, etc.)
    if any(c in lemma for c in [',', '-', '–', '.']):
        return lemma
    
    # Preserve numbers at end with space before them
    if re.search(r'\s\d+$', lemma):
        return lemma
        
    # Only apply trailing number removal for words with trailing digits
    # This is the case we want to handle - like "suyo1" -> "suyo"
    # We specifically look for a letter followed by digits, not just any word character
    result = lemma
    if re.search(r'[a-zA-Z]\d+$', lemma):
        result = re.sub(r'([a-zA-Z])(\d+)$', r'\1', lemma)
        
        # Critical safety check - never return empty string
        if not result:
            logger.warning(f"Number removal would result in empty string, preserving original: '{lemma}'")
            return lemma
            
        # Only log when we actually make a change
        if result != lemma:
            logger.info(f"Removed trailing numbers from lemma: '{lemma}' → '{result}'")
