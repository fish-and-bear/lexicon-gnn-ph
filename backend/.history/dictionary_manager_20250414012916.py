...
def process_tagalog_words(cur, filename: str, add_etymology: bool = True):
    ...
    # Try to validate/fix schema, but continue even if only indexes are missing
    try:
        # Always returns True now after attempting to create indexes
        validate_schema(cur)
    except Exception as e:
        logger.warning(f"Schema validation warning (continuing anyway): {e}")
                ...
                try:
                    ...
                    if pronunciation:
                        try:
                            ...
                            # Continue processing - don't abort on pronunciation error
                            if "relation" in str(e) and "does not exist" in str(e):
                                logger.error("Missing table: pronunciations. Schema may not be initialized correctly.")
                    
                    # Process etymology
                    if add_etymology and "etymology" in entry and entry["etymology"]:
                        ...
                        if isinstance(etymology, str) and etymology.strip():
                            try:
                                ...
                                # Continue processing - don't abort on etymology error
                                if "relation" in str(e) and "does not exist" in str(e):
                                    logger.error("Missing table: etymologies. Schema may not be initialized correctly.")
                        ...
                        for pos, def_list in definitions.items():
                            ...
                            for def_item in def_list:
                                ...
                                try:
                                        ...
                                        for example in examples:
                                            if isinstance(example, str) and example.strip():
                                                try:
                                                    ...
                                                    if "relation" in str(e) and "does not exist" in str(e):
                                                        logger.error("Missing table: examples. Schema may not be initialized correctly.")
                                    ...
                                    if "relation" in str(e) and "does not exist" in str(e):
                                        logger.error("Missing table: definitions. Schema may not be initialized correctly.")
                    ...
                    if domains and isinstance(domains, list):
                        for domain in domains:
                            ...
                            try:
                                ...
                                if "relation" in str(e) and "does not exist" in str(e):
                                    logger.error("Missing table: word_tags. Schema may not be initialized correctly.")
                    ...
                    try:
                        ...
                        # If transaction is already aborted, try to reset
                        try:
                            conn = cur.connection
                            conn.rollback()
                            conn.commit()
                        except Exception as reset_error:
                            logger.error(f"Failed to reset transaction state: {reset_error}")
