...
VALID_BAYBAYIN_REGEX = re.compile(r"^[\u1700-\u171F\s\u1735]+$") # Added \u1735 for Philippine Single Punctuation
    ...
    # These should not be treated as actual dictionary words.
NON_WORD_STRINGS = {
    # Language Codes (ISO 639)
    "tl", "en", "es", "la", "ceb", "hil", "ilo", "bik", "war", "pam", "pag", "kpm",
    "mdh", "mrw", "tsg", "yka", "zh", "ms", "ar", "fr", "de", "ja", "ko", "hi",
    "nan", "nan-hbl", # Min Nan variants
    "grc", # Ancient Greek
    "sa", # Sanskrit
    "nl", # Dutch
    "it", # Italian
...
def remove_trailing_numbers(text: str) -> str:
    ...
    Returns:
        The processed text with trailing numbers removed (or original if exempted)
    ...
    Examples:
        ...
        "d2" -> "d2" (unchanged)
        "e1" -> "e1" (unchanged)
        "word1a" -> "word1a" (unchanged, number not trailing)
        "word22" -> "word" (changed)
    """
    if not text:
        return ""

    # Exception: Check if text is a single letter followed by digits
    # Uses re.match which anchors at the start of the string implicitly.
    # ^[a-zA-Z] matches a single letter at the start.
    # \d+$ matches one or more digits at the end.
    if re.match(r'^[a-zA-Z]\d+$', text):
        return text # Return original if it matches the exception pattern (e.g., "d2")

    # Original logic: Remove trailing numbers if not exempted
    return re.sub(r'\d+$', '', text)

# --- End of remove_trailing_numbers ---
...
def extract_parenthesized_text(text: str) -> Tuple[str, Optional[str]]:
    ...
    for start, end in sorted(matches_indices, reverse=True):
        cleaned_text_intermediate = cleaned_text_intermediate[:start] + cleaned_text_intermediate[end:]
                    ...
                    for related_item in entry[field]:
                        ...
                        if related_word_str:
                            if related_word_clean.lower() in NON_WORD_STRINGS:
                                logger.debug(f"Detected non-word string '{related_word_clean}' for word '{word}'")
                                
                                # Create or update a usage note for the definitions
                                try:
                                    # First, categorize the non-word string
                                    metadata_category = "Language Code"  # Default category
                                    
                                    # Try to identify the type of metadata
                                    if related_word_clean.lower() in ["obsolete", "archaic", "dialectal", "neologism", 
                                                                    "slang", "vulgar", "colloquial", "formal", "informal"]:
                                        metadata_category = "Usage Note"
                                    elif related_word_clean.lower() in ["spanish-based orthography"]:
                                        metadata_category = "Orthography Note"
                                    elif len(related_word_clean) <= 3:  # Most language codes are 2-3 chars
                                        metadata_category = "Language Code"
                                    else:
                                        metadata_category = "Linguistic Note"
                                    
                                    # Find existing definitions for this word to add the usage note
                                    cur.execute("""
                                        SELECT id, usage_notes FROM definitions WHERE word_id = %s
                                    """, (word_id,))
                                    
                                    definitions = cur.fetchall()
                                    if definitions:
                                        note_text = f"{metadata_category}: {related_word_clean} (from {field})"
                                        
                                        for def_id, current_notes in definitions:
                                            # Append to existing notes or create new
                                            if current_notes:
                                                if note_text not in current_notes:  # Avoid duplicates
                                                    updated_notes = f"{current_notes}; {note_text}"
                                                else:
                                                    updated_notes = current_notes
                                            else:
                                                updated_notes = note_text
                                                
                                            # Update the definition with the new usage notes
                                            cur.execute("""
                                                UPDATE definitions 
                                                SET usage_notes = %s
                                                WHERE id = %s
                                            """, (updated_notes, def_id))
                                        
                                        logger.info(f"Added {metadata_category} '{related_word_clean}' to usage notes for word '{word}'")
                                    else:
                                        # No definitions yet, store temporarily
                                        if "temp_linguistic_notes" not in entry:
                                            entry["temp_linguistic_notes"] = []
                                        entry["temp_linguistic_notes"].append({
                                            "category": metadata_category,
                                            "value": related_word_clean,
                                            "source_field": field
                                        })
                                        logger.info(f"Stored {metadata_category} '{related_word_clean}' for word '{word}' for later definition processing")
                                except Exception as e:
                                    logger.error(f"Error storing metadata for non-word string '{related_word_clean}': {e}")
                                    
                                # Skip creating a relationship for this non-word string
                                continue
