...
class GraphQLValidationError(Exception):
    """Custom exception for GraphQL validation errors."""
    def __init__(self, message: str, field: Optional[str] = None):
        super().__init__(message)
        self.field = field

class GraphQLAuthorizationError(Exception):
    """Custom exception for GraphQL authorization errors."""
    pass

# Validation Schemas
class WordInputSchema(Schema):
    """Schema for validating word input."""
    lemma = fields.String(required=True, validate=validate.Length(min=1, max=255))
    language_code = fields.String(required=True, validate=validate.Length(min=2, max=5))
    normalized_lemma = fields.String(validate=validate.Length(max=255))
    has_baybayin = fields.Boolean()
    baybayin_form = fields.String(validate=validate.Length(max=255))
    romanized_form = fields.String(validate=validate.Length(max=255))
    is_proper_noun = fields.Boolean()
    is_abbreviation = fields.Boolean()
    is_initialism = fields.Boolean()
    preferred_spelling = fields.String(validate=validate.Length(max=255))
    tags = fields.List(fields.String())

class SearchInputSchema(Schema):
    """Schema for validating search input."""
    query = fields.String(required=True, validate=validate.Length(min=1, max=255))
    mode = fields.String(validate=validate.OneOf(['all', 'exact', 'baybayin', 'trigram']))
    language = fields.String(validate=validate.Length(min=2, max=5))
    min_similarity = fields.Float(validate=validate.Range(min=0.0, max=1.0))
...
class ValidationErrorType(graphene.ObjectType):
    ...
    errors = graphene.List(ErrorType, required=True)

# Response Types
class WordResponseType(graphene.ObjectType):
    """Type for word response with error handling."""
    word = graphene.Field(lambda: WordType)
    errors = graphene.List(ErrorType)
    success = graphene.Boolean(required=True)

class SearchResponseType(graphene.ObjectType):
    """Type for search response with error handling."""
    words = graphene.List(lambda: WordType)
    total = graphene.Int()
    errors = graphene.List(ErrorType)
    success = graphene.Boolean(required=True)
...
class WordFormType(SQLAlchemyObjectType):
    """Type for word forms."""
    class Meta:
        model = WordFormModel
        interfaces = (graphene.relay.Node,)
    ...
    def resolve_word(self, info):
        """Resolve word relationship."""
        return WordModel.query.get(self.word_id)

class WordTemplateType(SQLAlchemyObjectType):
    """Type for word templates."""
    class Meta:
        model = WordTemplateModel
        interfaces = (graphene.relay.Node,)
...
class Query(graphene.ObjectType):
    ...
    # Cache statistics
    cache_stats = graphene.Field(graphene.JSONString)
    
    # Single item queries with error handling
    word = graphene.Field(
        WordResponseType,
        id=graphene.ID(),
    ...
    search_by_affix = graphene.List(
        WordType,
        affix_type=graphene.String(required=True),
        language=graphene.String(),
        limit=graphene.Int()
    )
    
    search_by_template = graphene.List(
        WordType,
        template_type=graphene.String(required=True),
        language=graphene.String(),
        limit=graphene.Int()
    )
    
    find_related_words = graphene.List(
        WordType,
        word_id=graphene.ID(required=True),
        relation_type=graphene.String(),
        category=graphene.String(),
        direction=graphene.String(),
        min_strength=graphene.Float(),
        limit=graphene.Int()
    )
    
    find_similar_words = graphene.List(
        WordType,
    ...
    find_word_templates = graphene.List(
        WordTemplateType,
        word_id=graphene.ID(required=True),
        template_type=graphene.String(),
        limit=graphene.Int()
    ...
    find_affixations = graphene.List(
        AffixationType,
        word_id=graphene.ID(required=True),
        affix_type=graphene.String(),
        direction=graphene.String(),
    ...
    @cached(timeout=3600, key_prefix="word")  # Cache word lookups for 1 hour
    def resolve_word(self, info, id=None, lemma=None, language=None):
        """Resolve word query with error handling."""
        try:
            # Validate input
            if not id and not lemma:
                raise GraphQLValidationError("Either id or lemma must be provided")
            
            if lemma:
                # Validate lemma input
                errors = WordInputSchema().validate({"lemma": lemma, "language_code": language or "tl"})
                if errors:
                    return WordResponseType(
                        success=False,
                        errors=[ErrorType(field=field, message=msg) for field, msg in errors.items()]
                    )
            
            # Query database
            if id:
                word = WordModel.query.get(id)
            else:
            ...
            if not word:
                return WordResponseType(
                    success=False,
                    errors=[ErrorType(message="Word not found")]
                )
            ...
            logger.error(f"Error in word query: {str(e)}")
            GRAPHQL_ERRORS.labels(error_type='word_query_error').inc()
            return WordResponseType(
                success=False,
                errors=[ErrorType(message="Internal server error")]
            )
    ...
    def resolve_all_words(self, info, language=None, has_baybayin=None,
                         has_etymology=None, is_root=None, is_proper_noun=None,
                         limit=100, offset=0):
        ...
        if is_root is not None:
            query = query.filter(WordModel.root_word_id.is_(None) == is_root)
        if is_proper_noun is not None:
            query = query.filter(WordModel.is_proper_noun == is_proper_noun)
        
        return query.order_by(WordModel.lemma).offset(offset).limit(limit).all()
    
    @cached(timeout=300, key_prefix="search")  # Cache search results for 5 minutes
    def resolve_search_words(self, info, query, mode='all', language=None,
                           include_relations=True, min_similarity=0.3, limit=10):
        """Resolve search_words query with error handling."""
        try:
            # Validate input
            errors = SearchInputSchema().validate({
                "query": query,
                "mode": mode,
                "language": language,
            ...
            if mode == 'exact':
                ...
                base_query = base_query.filter(
                    func.similarity(WordModel.normalized_lemma, normalized_query) >= min_similarity
                )
            else:
                base_query = base_query.filter(
                    or_(
    ...
    def resolve_search_by_affix(self, info, affix_type, language=None, limit=10):
        """Resolve search_by_affix query."""
        query = WordModel.query.join(AffixationModel).filter(
            AffixationModel.affix_type == affix_type
        )
    ...
    def resolve_search_by_template(self, info, template_type, language=None, limit=10):
        """Resolve search_by_template query."""
        query = WordModel.query.join(WordTemplateModel).filter(
            WordTemplateModel.template_type == template_type
        )
    ...
    @cached(timeout=3600, key_prefix="related")  # Cache related words for 1 hour
    def resolve_find_related_words(self, info, word_id, relation_type=None,
                                 category=None, direction='both', min_strength=None,
                                 limit=10):
        """Resolve find_related_words query with error handling."""
        try:
            ...
            if direction == 'outgoing':
                query = query.filter_by(from_word_id=word_id)
            elif direction == 'incoming':
                query = query.filter_by(to_word_id=word_id)
            else:
                query = query.filter(
                    or_(
                        RelationModel.from_word_id == word_id,
                        RelationModel.to_word_id == word_id
                    )
                )
            ...
            if category:
                query = query.join(RelationshipType).filter(
                    RelationshipType.category == category
                )
            
            if min_strength:
                query = query.filter(
                    db.text("(metadata->>'strength')::float >= :min_strength")
            ...
            for relation in relations:
                if relation.from_word_id == word_id:
                    related_words.append(relation.to_word)
                else:
                    related_words.append(relation.from_word)
            
            return related_words
            
        except Exception as e:
            logger.error(f"Error in related words query: {str(e)}")
            GRAPHQL_ERRORS.labels(error_type='related_words_error').inc()
            return []
    
    def resolve_find_similar_words(self, info, word, language=None,
                                 min_similarity=0.3, limit=10):
        """Resolve find_similar_words query."""
        normalized_word = normalize_lemma(word)
        
        query = WordModel.query.filter(
            func.similarity(WordModel.normalized_lemma, normalized_word) >= min_similarity
        )
        ...
        return query.order_by(
            func.similarity(WordModel.normalized_lemma, normalized_word).desc()
        ).limit(limit).all()
    ...
    def resolve_word_statistics(self, info, language=None):
        ...
        try:
            query = db_session.query(
                ...
                func.count(case([(WordModel.is_proper_noun == True, 1)])).label('proper_nouns'),
                func.count(case([(WordModel.is_abbreviation == True, 1)])).label('abbreviations'),
                func.count(case([(WordModel.is_initialism == True, 1)])).label('initialisms'),
            ...
            return {
                ...
                'proper_nouns': stats.proper_nouns,
                'abbreviations': stats.abbreviations,
                'initialisms': stats.initialisms,
    ...
    @cached(timeout=3600, key_prefix="word_forms")  # Cache word forms for 1 hour
    def resolve_find_word_forms(self, info, word_id, form_type=None, limit=10):
        """Resolve find_word_forms query."""
        query = WordFormModel.query.filter_by(word_id=word_id)
    ...
    @cached(timeout=3600, key_prefix="word_templates")  # Cache word templates for 1 hour
    def resolve_find_word_templates(self, info, word_id, template_type=None, limit=10):
        """Resolve find_word_templates query."""
        query = WordTemplateModel.query.filter_by(word_id=word_id)
    ...
    @cached(timeout=3600, key_prefix="affixations")  # Cache affixations for 1 hour
    def resolve_find_affixations(self, info, word_id, affix_type=None,
                                direction='both', limit=10):
        """Resolve find_affixations query."""
        query = AffixationModel.query
        
        if direction == 'root':
            query = query.filter_by(root_word_id=word_id)
        elif direction == 'affixed':
            query = query.filter_by(affixed_word_id=word_id)
        else:
            query = query.filter(
                or_(
                    AffixationModel.root_word_id == word_id,
                    AffixationModel.affixed_word_id == word_id
                )
            )
        
        if affix_type:
            query = query.filter_by(affix_type=affix_type)
